...
- **note** (fgdb-write-ordered-silent-revert-2d80i): the same period produced a message/diff mismatch. `d8ce9e90` ("chore: fold unique working-tree extras onto origin/main", which said it preserved local work) was, file for file, the inverse of `ebabc3ae` (+12/−407 against +407/−12): a stale worktree folded onto `main` deleted `WriteTxn::write_ordered`, `retain_birth_ordinals` and four lab tests while its message claimed preservation. Not an exact revert by patch-id (context shifted), so no Reverts row claims one. Detection: read `git show --stat` of any commit that claims to preserve, and treat deletions in the paths it names as a revert. `64556cb0` later re-implemented `write_ordered` but not birth retention; a ported `ebabc3ae` test showed observed births being renumbered (V5,V6: [2,1] became [1,2]), and the retention law was restored under that bead.
