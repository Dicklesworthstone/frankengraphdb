...
# authoritative when the tree remains stable; any HEAD or worktree movement
# voids the run. Output directories are never removed or overwritten.
...
stderr separately, and write a checksum-bound manifest naming the exact commit
and stable worktree state. DIR must be outside the repository and must not exist.
USAGE
}
...
printf '%s\n' "$before_status" > "$output/status-before.txt"
...
printf '%s\n' "$after_status" > "$output/status-after.txt"
...

{
  ...
  printf 'verdict=%s\n' "$verdict"
  printf 'authority=check.sh exit code on the exact stable tree; void when HEAD or worktree state moves\n'
} > "$output/manifest.txt"
