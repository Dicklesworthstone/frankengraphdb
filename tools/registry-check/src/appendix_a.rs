...
use crate::{architecture, command_contracts, laws, model};
...
pub const EXPECTED_PROJECTION_ROW_COUNT: usize = 4073;
pub const EXPECTED_PROJECTION_ROW_IDS_SHA256: &str =
    "81900ea8a0e69c76127221978e41e1fc024a8fb93f6946f11a2816135398ea0c";
pub const EXPECTED_PROJECTION_FALLBACK_COUNT: usize = 370;
pub const EXPECTED_TARGET_SOURCE_ASSIGNMENT_SHA256: &str =
    "2cf90ac355155d864fbb8102fd443f00eb046144fd37bfdc11f2ff53746b9217";
...
pub const EXPECTED_EXISTING_TYPE_RESERVATION_COUNT: usize = 486;
pub const EXPECTED_RESERVED_TYPE_RESERVATION_COUNT: usize = 327;
pub const EXPECTED_RESERVATION_HIGH_WATER: u16 = 0x051d;
pub const EXPECTED_RESERVATION_ASSIGNMENT_SHA256: &str =
    "32392e56a91860370203541d86311ddfeadb3482d55982610361d042a6a860a8";
...
struct AmbiguityAdjudicationContractPin {
    ...
    /// The canonical law names (registries/laws.toml `name` namespace) this
    /// adjudication's rationale is licensed to cite, extracted with the same
    /// longest-match surface-phrase scan the resolver runs at check time
    /// (names and `cited_as` aliases alike; fgdb-2ngl).
    ///
    /// WHY A PIN-SIDE ALLOWLIST WHEN THE CITATION GUARD ALREADY REQUIRES
    /// RESOLUTION. The guard proves a citation's referent exists and is
    /// `registered`; it cannot prove the citation is the one the adjudication
    /// was approved to lean on. An adjudication that silently starts citing
    /// a different registered law has drifted semantically while every
    /// resolution still succeeds -- the allowlist is what names that drift.
    /// Exact set equality, both directions fail closed:
    ///   cited-but-unpinned  -> catalog_ambiguity_citation_unpinned
    ///   pinned-but-uncited  -> catalog_ambiguity_citation_drift
    ///   pinned-nonregistered-> catalog_ambiguity_citation_unregistered
    cited_laws: &'static [&'static str],
}
...
static AMBIGUITY_ADJUDICATION_CONTRACT: [AmbiguityAdjudicationContractPin; 455] = [
    ...
    AmbiguityAdjudicationContractPin {
        ...
        cited_laws: &["flattened-rendering", "shorthand"],
    },
    AmbiguityAdjudicationContractPin {
    ...
    AmbiguityAdjudicationContractPin {
        ...
        cited_laws: &["compact-phrase", "flattened-rendering", "shorthand"],
    },
    AmbiguityAdjudicationContractPin {
    ...
    AmbiguityAdjudicationContractPin {
        ...
        cited_laws: &["SequenceNeutralSpec", "shorthand"],
    },
    AmbiguityAdjudicationContractPin {
...
pub fn appendix_a_catalog_projection_diff(repo_root: &Path, catalog: &Catalog) -> Vec<Violation> {
    ...
    out.extend(verify_adjudication_law_allowlists(repo_root, catalog));
    sort_violations(&mut out);
    out
}
...
/// match must sit on token boundaries (the neighbor is neither alphanumeric
/// nor `_`/`-`), and a longer phrase consumes its span so a shorter name
/// inside it does not double-count ("SequenceNeutralSpec wrapper" cites
/// FG-LAW-04 once, not twice). Deterministic by construction: the same
/// algorithm populates the pin lists and runs here at check time.
fn extract_cited_law_names(text: &str, laws_registry: &laws::LawRegistry) -> Vec<String> {
    let mut vocab: Vec<(&str, &str)> = Vec::new();
    for law in &laws_registry.laws {
        vocab.push((law.name.as_str(), law.name.as_str()));
        for alias in &law.cited_as {
            vocab.push((alias.as_str(), law.name.as_str()));
        }
    }
    vocab.sort_by_key(|(surface, _)| std::cmp::Reverse(surface.len()));

    let bytes = text.as_bytes();
    let mut used = vec![false; text.len()];
    let mut claimed: BTreeSet<&str> = BTreeSet::new();
    for (surface, canonical) in &vocab {
        let mut start = 0;
        while let Some(offset) = text[start..].find(surface) {
            let begin = start + offset;
            let end = begin + surface.len();
            if used[begin..end].iter().any(|occupied| *occupied) {
                start += 1;
                continue;
            }
    ...
    claimed.into_iter().map(str::to_owned).collect()
}

/// Enforce the pin-side citation allowlists (fgdb-2ngl): every law an
/// adjudication rationale actually cites must be pinned on its contract row,
/// every pinned law must still be cited, and every pinned law must resolve to
/// a `registered` row in registries/laws.toml. All three directions fail
/// closed with their own codes — resolution alone is what makes the allowlist
...
fn verify_adjudication_law_allowlists_with(
    ...
    let status_by_name: BTreeMap<&str, &str> = laws_registry
        .laws
        .iter()
        .map(|law| (law.name.as_str(), law.status.as_str()))
        .collect();
    let pin_map: BTreeMap<&str, &AmbiguityAdjudicationContractPin> =
        pins.iter().map(|pin| (pin.row_id, pin)).collect();
    ...
    for row in rows {
        ...
        for name in &extracted {
            if !pinned.contains(name.as_str()) {
                out.push(Violation::new(
                    "catalog_ambiguity_citation_unpinned",
                    &row.row_id,
                    format!(
                        "rationale cites law {name:?}, which its contract pin does not \
                         license; extend the pin's cited_laws or repair the prose"
                    ),
                ));
            }
        }
        for name in &pinned {
            if !extracted.iter().any(|cited| cited == name) {
                out.push(Violation::new(
                    "catalog_ambiguity_citation_drift",
                    &row.row_id,
                    format!(
                        "contract pin licenses law {name:?}, which the rationale no \
                         longer cites; shrink the pin or restore the citation"
                    ),
                ));
            }
        }
        for name in &pinned {
            match status_by_name.get(*name) {
                Some(status) if *status == "registered" => {}
                Some(status) => out.push(Violation::new(
                    "catalog_ambiguity_citation_unregistered",
                    &row.row_id,
                    format!(
                        "pinned law {name:?} has status {status:?}; only a `registered` \
                         row licenses a citation"
                    ),
                )),
                None => out.push(Violation::new(
                    "catalog_ambiguity_citation_unregistered",
                    &row.row_id,
                    format!(
                        "pinned law {name:?} resolves to no row in {}; a pin naming an \
                         undeclared law is a fabricated referent",
                        laws::REGISTRY_PATH
                    ),
                )),
            }
...
fn verify_adjudication_law_allowlists(repo_root: &Path, catalog: &Catalog) -> Vec<Violation> {
    let laws_registry = match laws::load_from_repo(repo_root) {
        ...
        Err(error) => {
            return vec![Violation::new(
                "catalog_ambiguity_citation_registry_unavailable",
                laws::REGISTRY_PATH,
                error.to_string(),
            )];
        }
    };
    verify_adjudication_law_allowlists_with(
        &AMBIGUITY_ADJUDICATION_CONTRACT,
        &laws_registry,
        &catalog.ambiguity_adjudications,
    )
}
...
"#;
    ...
    fn an_uncertified_census_universe_blocks_the_completeness_claim() {
        ...
        assert_eq!(
            ...
            8,
            "the coverage law must still evaluate a20's remaining uncovered keys \
             after fgdb-rqw4 lands the three ::Sealed precondition rows and the \
             receipt's one-arm state row (12 -> 8); the residue composition is \
             owned by fgdb-a20-restore-promotion-ivsp's tally"
        );
...
#[cfg(test)]
mod adjudication_law_allowlist_tests {
    use super::*;

    fn law(id: &str, name: &str, status: &str) -> laws::Law {
        laws::Law {
            id: id.to_owned(),
            name: name.to_owned(),
            source_location: "a01:1396".to_owned(),
    ...
    fn row(row_id: &str, rationale: &str) -> AmbiguityAdjudication {
        AmbiguityAdjudication {
            row_id: row_id.to_owned(),
            slice_id: "a01".to_owned(),
            ambiguity_source_key: format!("ambiguity|test|{row_id}"),
            source_locations: vec!["a01:1396".to_owned()],
            resolution: "maps-to-source".to_owned(),
            resolved_source_keys: Vec::new(),
            rationale: rationale.to_owned(),
        }
    }
    ...
    fn pin(
        ...
        AmbiguityAdjudicationContractPin {
            ...
            resolution: "maps-to-source",
            resolved_source_keys: &[],
            rationale_sha256: "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
            cited_laws: cited,
        }
    }
    ...
    #[test]
    fn an_unlicensed_citation_fails_as_unpinned() {
    ...
    #[test]
    fn a_shrunk_citation_fails_as_drift() {
    ...
    #[test]
    fn a_pinned_nonregistered_law_fails_closed() {
        // gamma-struck IS cited, so set equality holds; resolution must still
        // reject it because only `registered` licenses a citation.
    ...
    #[test]
    fn the_alias_resolves_to_its_canonical_pin() {
