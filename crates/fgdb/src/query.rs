...
pub enum QueryResult {
    Rows { columns: Vec<String>, rows: Vec<Vec<QueryValue>> },
    /// None means staged in the caller's still-open transaction, not committed.
    Write { receipt: GraphWriteProgramReceipt, completion: Option<EmbeddedTxnCompletion> },
}
...
impl core::fmt::Display for QueryError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::Unsupported { diagnostics } => write!(f, "unsupported query construct: {}", diagnostics.join("; ")),
            Self::PatternText(e) => e.fmt(f), Self::SetText(e) => e.fmt(f),
            Self::PipelineText(e) => e.fmt(f), Self::TemporalText(e) => e.fmt(f),
            Self::TemporalSetText(e) => e.fmt(f), Self::Pattern(e) => e.fmt(f),
            Self::Aggregate(e) => e.fmt(f), Self::Set(e) => e.fmt(f),
        }
    }
}
...
impl<A: core::fmt::Display> core::fmt::Display for QueryWriteError<A> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self { Self::Prepare(e) => e.fmt(f), Self::Execute(e) => e.fmt(f) }
    }
}
...
fn values(columns: Vec<String>, rows: Vec<GraphValueRow>) -> QueryResult {
    QueryResult::Rows { columns, rows: rows.into_iter().map(|row|
        row.values().iter().cloned().map(GraphAggregateValue::Value).collect()).collect() }
}
fn aggregates(columns: Vec<String>, slots: &[GraphAggregateTextSlot], rows: Vec<GraphAggregateRow>) -> QueryResult {
    QueryResult::Rows { columns, rows: rows.into_iter().map(|row| slots.iter().map(|slot| match *slot {
...
impl<V: Vfs + Clone> Database<V> {
    ...
    pub fn query(&self, cx: &QueryCx, text: &str, params: &GqlParameters,
        mut resolver: impl FnMut(GraphSymbolKind, &str) -> Option<GraphSymbol>,
        budget: GqlQueryPolicy) -> Result<QueryResult, QueryError>
    {
        ...
        match PreparedTemporalGraphText::prepare(text, &mut resolve) {
            Ok(prepared) => {
                let query = prepared.bind_parameters(params).map_err(QueryError::TemporalText)?;
                let columns = query.pattern().columns().to_vec();
                let result = self.execute_graph_pattern_governed_at(cx, query.pattern(), query.as_of(), budget)
                    .map_err(QueryError::Pattern)?;
                return Ok(values(columns, result.value));
            }
            Err(error) => diagnostics.push(error.to_string()),
        }
        match PreparedTemporalGraphSetText::prepare(text, &mut resolve) {
            Ok(prepared) => {
                let query = prepared.bind_parameters(params).map_err(QueryError::TemporalSetText)?;
                let result = self.execute_graph_set_governed_at(cx, query.query(), query.as_of(), budget)
                    .map_err(QueryError::Set)?;
                return Ok(values(prepared.columns().to_vec(), result.value));
            }
            Err(error) => diagnostics.push(error.to_string()),
        }
        match PreparedGraphPipelineAggregateText::prepare(text, &mut resolve) {
            Ok(prepared) => {
                let query = prepared.bind_parameters(params).map_err(QueryError::PipelineText)?;
                let result = self.execute_graph_aggregate_governed(cx, &query, budget).map_err(QueryError::Aggregate)?;
                return Ok(aggregates(prepared.columns().to_vec(), prepared.output_slots(), result.value));
            }
            Err(error) => diagnostics.push(error.to_string()),
        }
        match PreparedGraphAggregateText::prepare(text, &mut resolve) {
            Ok(prepared) => {
        ...
        match PreparedGraphText::prepare(text, &mut resolve) {
            Ok(prepared) => {
                let query = prepared.bind_parameters(params).map_err(QueryError::PatternText)?;
                let result = self.execute_graph_pattern_governed(cx, &query, budget).map_err(QueryError::Pattern)?;
                return Ok(values(query.columns().to_vec(), result.value));
            }
            Err(error) => diagnostics.push(error.to_string()),
        }
        match PreparedGraphSetText::prepare(text, &mut resolve) {
            Ok(prepared) => {
                let query = prepared.bind_parameters(params).map_err(QueryError::SetText)?;
                let result = self.execute_graph_set_governed(cx, &query, budget).map_err(QueryError::Set)?;
                Ok(values(prepared.columns().to_vec(), result.value))
            }
            Err(error) => { diagnostics.push(error.to_string()); Err(QueryError::Unsupported { diagnostics }) }
        }
    }
    ...
    pub async fn query_write<A>(&mut self, txcx: &TxnCx, cx: &QueryCx, commit_cx: &CommitCx,
        text: &str, params: &GqlParameters,
        resolver: impl FnMut(GraphSymbolKind, &str) -> Option<GraphSymbol>,
        relation: RelationId, budget: GraphWriteProgramPolicy,
        allocate: impl FnMut(GraphWriteIdentityRequest) -> Result<ElementId, A>)
        -> Result<QueryResult, QueryWriteError<A>>
    {
        let script = PreparedGraphWriteScript::prepare(text, relation, resolver).map_err(QueryWriteError::Prepare)?;
        let (receipt, completion) = self.execute_graph_write_script_autocommit_governed(
            txcx, cx, commit_cx, &script, params, budget, allocate).await.map_err(QueryWriteError::Execute)?;
        Ok(QueryResult::Write { receipt, completion: Some(completion) })
    }
}
...
impl WriteTxn {
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn query_write<V: Vfs + Clone, A>(&mut self, database: &mut Database<V>, cx: &QueryCx,
        text: &str, params: &GqlParameters,
        resolver: impl FnMut(GraphSymbolKind, &str) -> Option<GraphSymbol>,
        relation: RelationId, budget: GraphWriteProgramPolicy,
        allocate: impl FnMut(GraphWriteIdentityRequest) -> Result<ElementId, A>)
    ...
    {
        let script = PreparedGraphWriteScript::prepare(text, relation, resolver).map_err(QueryWriteError::Prepare)?;
        let receipt = self.execute_graph_write_script_governed(database, cx, &script, params, budget, allocate)
            .map_err(QueryWriteError::Execute)?;
        Ok(QueryResult::Write { receipt, completion: None })
    }
}
