...
use fgdb_gql::algebra::{GlaIdentityOutput, GlaOutput, GlaPlan, PreparedGraphPattern, VertexPredicate};
...
impl<'a, R: GqlSnapshotReader + ?Sized, Row: GlaOutput> AdmittedGqlSnapshot<'a, R, Row> {
    fn admit_logical(logical: GlaPlan<Row>, reader: &'a R, as_of: CommitSeq) -> Result<Self, ReadError> {
        ...
        let count = if snapshot.is_some() { 0 } else if logical.scans_edges() {
            ...
            if logical.projects_properties() {
                vertices.extend(reader.gql_vertices_at(as_of)?.into_iter().map(|row| (row.vid, row)));
            }
            edges.len() as u64
        } else {
    ...
    fn property(&self, vid: VId, key: PropertyKeyId) -> Option<&CanonicalScalar> {
        if let Some(tables) = &self.borrowed { return tables.property(vid, key); }
    ...
    pub(crate) fn execute_governed<C>(mut self, policy: fgdb_gql::GqlQueryPolicy, mut checkpoint: impl FnMut() -> Result<(), C>)
        ...
        let result = self.logical.execute_governed_with_properties(self.snapshot_records, self.vertex_ids(), self.edge_triples(),
            |vid, predicates| self.matches(vid, predicates), |vid, key| Ok(self.property(vid, key)),
