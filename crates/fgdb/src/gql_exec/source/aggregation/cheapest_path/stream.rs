...
impl AdmissionUsage {
    pub(crate) fn path_stream_usage(self) -> GlaExecutionStats {
        GlaExecutionStats {
            ...
            scratch_entries: self.scratch_entries,
        }
    }
}
...
impl<V: Vfs + Clone> Database<V> {
    ...
    pub fn stream_graph_cheapest_paths_governed_at<'cx>(
        ...
        let stream =
            cx.with_restriction(|| open_at(self, query, count, as_of, policy, || cx.checkpoint()))?;
    ...
    pub fn stream_graph_cheapest_path_text_governed_at<'cx>(
        ...
        self.stream_graph_cheapest_paths_governed_at(
            cx,
            request.query(),
            request.ranked_count().unwrap_or(1),
            as_of,
    ...
    fn every_database_admission_and_pull_checkpoint_refuses_once_and_retries_identically() {
        let ((), report) = run_async_under_lab(0xc0a5_3016, |root| async move {
            ...
            batch.add_edge(
                ...
                VId(0),
                VId(1),
                vec![(weight, CanonicalScalar::Int(4))],
            );
            batch.add_edge(
                EId(2),
                VId(0),
            ...
            for mode in [
                ...
                .with_mode(mode);
                let mut total = 0;
                let mut good = open_at(&db, &query, 3, basis, policy, || {
                    total += 1;
                    Ok::<_, usize>(())
                })
                .unwrap();
                ...
                for stop in 1..=total {
                        ...
                        Ok(mut stream) => {
                            ...
                            loop {
                                match stream.next_with_checkpoint(&mut checkpoint) {
                                    ...
                                    other => {
                                        panic!("interruption became a different outcome: {other:?}")
                                    }
                                }
                            }
                    ...
                    let mut retry =
                        open_at(&db, &query, 3, basis, policy, || Ok::<_, usize>(())).unwrap();
                    let mut rows = Vec::new();
                    while let Some(row) = retry.next_with_checkpoint(|| Ok::<_, usize>(())).unwrap()
                    {
                        rows.push(row);
                    }
