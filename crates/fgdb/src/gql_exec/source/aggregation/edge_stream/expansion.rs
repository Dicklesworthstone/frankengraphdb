...
pub(super) fn next<C>(
    source: &SnapshotEdgeSource<'_>, endpoint: VId, direction: GlaDirection, after: Option<EId>,
    ...
    source.cx.with_restriction(|| {
        ...
        let result = match direction {
            ...
            GlaDirection::Undirected => (|| {
                ...
                Ok(match (left, right) { (Some(a), Some(b)) => Some(a.min(b)), (a, b) => a.or(b) })
            })(),
        };
...
fn successor<C>(
    face: &IndexMap<VId, IndexMap<EId, ()>>, endpoint: VId, after: Option<EId>,
    control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), C>,
) -> Result<Option<EId>, C> {
    ...
    while let Some(current) = node {
        ...
        match endpoint.cmp(&current.key) {
            ...
            core::cmp::Ordering::Equal => { incidence = Some(&current.value); break; }
        }
    }
    let Some(incidence) = incidence else { return Ok(None); };
...
mod tests {
    ...
    fn every_incidence_seek_checkpoint_is_fallible_and_positions_are_caller_owned() {
        let ((), report) = run_async_under_lab(0x6a6f_6904, |root| async move {
            ...
            let keys = crate::DatabaseKeys::new([0x12; 32], DatabaseSecurityNamespaceId([0x23; 32]), [0x34; 32]);
            let mut db = Database::open_memory(&commit, keys).await.unwrap();
            let mut batch = crate::WriteBatch::new(RelationId(1));
            for v in 0..8 { batch.create_vertex(VId(v), vec![], vec![]); }
            for i in 0..32 { batch.add_edge(EId(i), VId(i % 8), VId((i + 1) % 8), vec![]); }
            batch.add_edge(EId(u128::MAX), VId(0), VId(0), vec![]);
            db.write(&commit, batch).await.unwrap();
            let source = SnapshotEdgeSource { view: db.read_session().unwrap(), cx: &cx, as_of: db.frontier().unwrap(), after: None };
            for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
                for after in [None, Some(EId(0)), Some(EId(31)), Some(EId(u128::MAX))] {
                    let mut total = 0;
                    let expected = next(&source, VId(0), direction, after, &mut |_| { total += 1; Ok::<_, usize>(()) }).unwrap();
                    assert!(total > 0);
                    for stop in 1..=total {
                        let mut calls = 0;
                        ...
                        assert!(matches!(result, Err(EdgeExpansionSourceError::Read(EdgeScanSourceError::Control(at))) if at == stop));
                        assert_eq!(calls, stop);
                        assert_eq!(next(&source, VId(0), direction, after, &mut |_| Ok::<_, usize>(())).unwrap(), expected);
                        assert_eq!(source.after, None, "nested seeks cannot advance the root cursor");
                    }
                }
            }
        });
