...
fn open_aggregate<'q>(
    view: EmbeddedReadView, cx: &'q QueryCx, plan: &EdgeAggregatePlan,
    as_of: CommitSeq, policy: GqlQueryPolicy,
) -> Result<EdgeAggregateCursor<SnapshotEdgeSource<'q>,
    impl FnMut() -> Result<(), Cancel> + 'q + use<'q>>, AggregateError> {
    ...
    Ok(EdgeAggregateCursor::new(
        SnapshotEdgeSource { view, cx, as_of, after: None }, plan.clone(), policy,
...
impl<V: Vfs + Clone> Database<V> {
    ...
    pub fn stream_global_edge_aggregate_governed<'q>(
        &self, cx: &'q QueryCx, plan: &EdgeAggregatePlan, policy: GqlQueryPolicy,
    ) -> Result<EdgeAggregateCursor<SnapshotEdgeSource<'q>,
        impl FnMut() -> Result<(), Cancel> + 'q + use<'q, V>>, AggregateError> {
    ...
    pub fn stream_global_edge_aggregate_governed_at<'q>(
        &self, cx: &'q QueryCx, plan: &EdgeAggregatePlan,
        as_of: CommitSeq, policy: GqlQueryPolicy,
    ) -> Result<EdgeAggregateCursor<SnapshotEdgeSource<'q>,
        impl FnMut() -> Result<(), Cancel> + 'q + use<'q, V>>, AggregateError> {
        open_aggregate(self.read_session().map_err(admission_error)?, cx, plan, as_of, policy)
    }
}
