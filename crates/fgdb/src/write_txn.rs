...
impl WriteTxn {
    ...
    pub fn execute_gql<V: Vfs + Clone>(
        ...
        let src_prop_gt_ok = match plan.src_prop_gt {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let src_prop_ge_ok = match plan.src_prop_ge {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let src_prop_le_ok = match plan.src_prop_le {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let dst_prop_ne_ok = match plan.dst_prop_ne {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let dst_prop_gt_ok = match plan.dst_prop_gt {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let dst_prop_lt_ok = match plan.dst_prop_lt {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let dst_prop_ge_ok = match plan.dst_prop_ge {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let hop2_dst_prop_gt_ok = match plan.hop2_dst_prop_gt {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let hop2_dst_prop_lt_ok = match plan.hop2_dst_prop_lt {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let hop2_dst_prop_ge_ok = match plan.hop2_dst_prop_ge {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let hop2_dst_prop_le_ok = match plan.hop2_dst_prop_le {
            None => None,
            Some((key, value)) => {
                let mut holders = std::collections::BTreeSet::new();
                for vid in vertices.iter().copied() {
        ...
        let destinations = crate::execute_bound_plan_over(&plan, anchors, |source, relation| {
            ...
            let hop2_step = |via: VId| {
                ...
                if let Some(holders) = hop2_dst_prop_gt_ok.as_ref() {
                    far_ends.retain(|far_end| holders.contains(far_end));
                }
                if let Some(holders) = hop2_dst_prop_lt_ok.as_ref() {
                    far_ends.retain(|far_end| holders.contains(far_end));
                }
                if let Some(holders) = hop2_dst_prop_ge_ok.as_ref() {
                    far_ends.retain(|far_end| holders.contains(far_end));
                }
                if let Some(holders) = hop2_dst_prop_le_ok.as_ref() {
                    far_ends.retain(|far_end| holders.contains(far_end));
                }
                far_ends
            };
