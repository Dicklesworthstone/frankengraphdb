...
async fn encoded(db: &Database<MemVfs>, cx: &QueryCx, id: ObjectId) -> Vec<u8> {
    cx.with_restriction_async(async {
        let mut file = db.vfs.open(&db.store.path(id), &OpenOptions::new().read(true))
            .await.unwrap();
...
async fn damage(db: &Database<MemVfs>, cx: &QueryCx, id: ObjectId) {
    cx.with_restriction_async(async {
        let mut file = db.vfs.open(&db.store.path(id), &OpenOptions::new().write(true))
            .await.unwrap();
...
fn bounded_reopen_preserves_complete_adjacency_and_properties_across_history() {
    ...
    runtime.block_on(async {
        ...
        for cut in [old.frontier(), current.frontier()] {
            let expected = db.store.reopen(&query, current.partition_root()).await.unwrap();
            let (root, blocks, props) = db.store.reopen_adjacency_bounded(
                &query, current.partition_root(), cut, RootReadLimits::default(),
            ).await.unwrap();
            ...
            let image = db.store.seal_partition_with_source_limits(
                &query, current.partition_root(), cut, limits, RootReadLimits::default(),
            ).await.unwrap();
            let opts = FnxReadOptions { as_of: Some(cut), ..options() };
            let actual = current.execute_fnx_sealed(
                &query, &image, &dijkstra_call(), opts, memory(),
            ).unwrap();
            let decoded = current.execute_fnx(&query, &dijkstra_call(), opts).unwrap();
            assert_eq!(actual.analytics.rows, decoded.analytics.rows);
            assert_eq!(actual.analytics.certificate.result_digest, decoded.analytics.certificate.result_digest);
...
fn raw_source_counts_and_exact_encoded_bytes_are_admitted_independently() {
    ...
    runtime.block_on(async {
        ...
        let (root, blocks, props, patches) = db.store.reopen(&query, view.partition_root()).await.unwrap();
        ...
        let exact = RootReadLimits {
            ...
            max_vertex_versions: patches.iter().map(Vec::len).sum(),
        };
        assert!(exact.max_incidences > 0 && exact.max_vertex_versions > 0 && source_bytes > 0);
        db.store.reopen_adjacency_bounded(&query, view.partition_root(), view.frontier(), exact)
            .await.unwrap();
        for (tight, resource) in [
            (RootReadLimits { max_source_bytes: exact.max_source_bytes - 1, ..exact }, "source encoded bytes"),
            (RootReadLimits { max_blocks: exact.max_blocks - 1, ..exact }, "source blocks"),
            (RootReadLimits { max_vertex_patches: exact.max_vertex_patches - 1, ..exact }, "source vertex patches"),
            (RootReadLimits { max_incidences: exact.max_incidences - 1, ..exact }, "source incidences"),
            (RootReadLimits { max_vertex_versions: exact.max_vertex_versions - 1, ..exact }, "source vertex versions"),
        ] {
        ...
        assert!(matches!(db.store.reopen_adjacency_bounded(
            ...
            RootReadLimits { max_root_bytes: exact.max_root_bytes - 1, ..exact },
        ).await, Err(RootReadError::Store(_))));
...
fn incidence_and_reference_limits_stop_before_a_corrupt_later_payload() {
    ...
    runtime.block_on(async {
        ...
        db.write(&commit, small_fixture()).await.unwrap();
        let view = db.read_session().unwrap();
        let root = db.store.get_root(&query, view.partition_root()).await.unwrap();
        assert!(root.blocks.len() >= 2);
        assert_ne!(root.blocks[0].block_id, root.blocks.last().unwrap().block_id);
        ...
        assert!(db.store.reopen(&query, view.partition_root()).await.is_err());
        assert!(matches!(db.store.seal_partition(
            &query, view.partition_root(), view.frontier(),
            SealedLimits { max_incidences: 0, ..SealedLimits::default() },
        ).await, Err(SealedError::Limit { resource: "source incidences", .. })));
        assert!(matches!(db.store.reopen_adjacency_bounded(
            &query, view.partition_root(), view.frontier(),
            RootReadLimits { max_blocks: 0, ..RootReadLimits::default() },
        ).await, Err(RootReadError::Limit { resource: "source blocks", .. })));
        assert!(matches!(db.store.seal_partition(
            &query, view.partition_root(), CommitSeq(view.frontier().0 + 1), SealedLimits::default(),
        ).await, Err(SealedError::InvalidFloor)));
        assert!(matches!(db.store.reopen_adjacency_bounded(
            &query, view.partition_root(), view.frontier(), RootReadLimits::default(),
        ).await, Err(RootReadError::Store(_))));
    });
}
...
fn hosted_property_bytes_are_charged_before_their_decode() {
    ...
    runtime.block_on(async {
        ...
        db.write(&commit, small_fixture()).await.unwrap();
        let view = db.read_session().unwrap();
        let (root, _, props, _) = db.store.reopen(&query, view.partition_root()).await.unwrap();
        assert!(props[0].is_some());
        let first_bytes = db.store.get_bytes(&query, DeltaBlockVersion(root.blocks[0].block_id))
            .await.unwrap().len();
...
fn adjacency_only_reopen_does_not_skip_vertex_history_validation() {
    ...
    runtime.block_on(async {
        ...
        assert!(matches!(db.store.reopen_adjacency_bounded(
            &query, view.partition_root(), view.frontier(),
            RootReadLimits { max_vertex_versions: 0, ..RootReadLimits::default() },
        ).await, Err(RootReadError::Limit { resource: "source vertex versions", .. })));
        damage(&db, &query, root.vertex_patches[0].patch_id).await;
        assert!(matches!(db.store.reopen_adjacency_bounded(
            &query, view.partition_root(), view.frontier(), RootReadLimits::default(),
        ).await, Err(RootReadError::Store(_))));
        assert!(matches!(db.store.seal_partition(
            &query, view.partition_root(), view.frontier(), SealedLimits::default(),
        ).await, Err(SealedError::Store(_))));
    });
}
...
fn cancellation_at_every_source_and_sealing_checkpoint_never_returns_a_prefix() {
    ...
    runtime.block_on(async {
        ...
        let baseline = db.store.reopen_adjacency_bounded(
            &controlled, view.partition_root(), view.frontier(), RootReadLimits::default(),
        ).await.unwrap();
        ...
        for stop in 1..=source_checkpoints {
            ...
            assert!(matches!(db.store.reopen_adjacency_bounded(
                &controlled, view.partition_root(), view.frontier(), RootReadLimits::default(),
            ).await, Err(RootReadError::Interrupted(_))));
            assert_eq!(probe.calls(), stop);
        }
        let probe = Arc::new(SimulationCheckpointProbe::new(None));
        let controlled = query.with_checkpoint_probe(Arc::clone(&probe));
        let baseline_image = db.store.seal_partition(
            &controlled, view.partition_root(), view.frontier(), SealedLimits::default(),
        ).await.unwrap();
        ...
        for stop in 1..=sealing_checkpoints {
            ...
            assert!(matches!(db.store.seal_partition(
                &controlled, view.partition_root(), view.frontier(), SealedLimits::default(),
            ).await, Err(SealedError::Interrupted(_))));
            assert_eq!(probe.calls(), stop);
        }
        let after = db.store.reopen_adjacency_bounded(
            &query, view.partition_root(), view.frontier(), RootReadLimits::default(),
        ).await.unwrap();
        assert_eq!(after, baseline);
        let image = db.store.seal_partition(
            &query, view.partition_root(), view.frontier(), SealedLimits::default(),
        ).await.unwrap();
...
fn explicit_source_profile_cannot_bypass_the_legacy_incidence_ceiling() {
    ...
    runtime.block_on(async {
        ...
        for (source, output) in [(0, usize::MAX), (usize::MAX, 0)] {
            assert!(matches!(db.store.seal_partition_with_source_limits(
                &query, view.partition_root(), view.frontier(),
                SealedLimits { max_incidences: output, ..SealedLimits::default() },
                RootReadLimits { max_incidences: source, ..RootReadLimits::default() },
            ).await, Err(SealedError::Limit { resource: "source incidences", .. })));
        }
        let empty = Database::<MemVfs>::open_memory(&commit, keys()).await.unwrap();
        let empty_view = empty.read_session().unwrap();
        let source = RootReadLimits {
            max_source_bytes: 0, max_blocks: 0, max_vertex_patches: 0,
            max_incidences: 0, max_vertex_versions: 0, ..RootReadLimits::default()
        };
        let image = empty.store.seal_partition_with_source_limits(
            &query, empty_view.partition_root(), empty_view.frontier(), SealedLimits::default(), source,
        ).await.unwrap();
        assert_eq!(image.stats().incidences, 0);
        // Still authenticate the empty root. A random identity is no permit.
        assert!(empty.store.reopen_adjacency_bounded(
            &query, PartitionRootVersion(ObjectId([0xff; 32])), empty_view.frontier(), source,
        ).await.is_err());
    });
}
