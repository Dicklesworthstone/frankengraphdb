...
use fgdb_gql::{GlaExecutionEvent, GqlQueryError, GraphAggregateError, GraphAggregateRow,
...
fn copy_group(row: &GraphAggregateRow, meter: &mut Meter<'_>) -> Result<GroupKey, StandingQueryFailure> {
    ...
    pub(super) fn prepare(
        ...
        for wanted in [-1, 1] {
            for (row, weight) in delta.iter() {
                ...
                let projected = self.definition.project_incremental_output(row, &mut |event| {
                    govern(meter, event)
                }).map_err(|error| match error {
                    GqlQueryError::Interrupted(reason) => reason,
                    GqlQueryError::Source(GraphAggregateError::OutputExpression { column, error }) => {
                ...
                if !distinct {
                    row_count = if sign < 0 { row_count.checked_sub(1) } else { row_count.checked_add(1) }
                        .ok_or(StandingQueryFailure::InvalidDelta)?;
                    meter.charge(ZSetEvent::ScratchEntry)?;
                    output.push((projected, ZWeight::from_i128(sign)));
                    continue;
                }
                let identity = projected.incremental_distinct_key(&mut |event| govern(meter, event))?;
                let key = copy_group(row, meter)?;
                let retained = self.classes.get(&identity).and_then(|class| class.get(&key));
        ...
        if distinct {
            for (identity, patch) in &changes {
                ...
                let old = retained.and_then(|class| class.first_key_value()).map(|(_, row)| row);
                let mut next = None;
                ...
                for (key, row) in patch {
                    meter.charge(ZSetEvent::Work)?;
                    if let Some(row) = row {
                        if next.is_none_or(|(current, _)| key < current) { next = Some((key, row)); }
                        break;
                    }
                }
                let new = next.map(|(_, row)| row);
                row_count = row_count.checked_sub(u128::from(old.is_some()))
        ...
        if meter.policy.rows.max_result_rows().is_some_and(|limit| row_count > u128::from(limit)) {
            return Err(StandingQueryFailure::ResultBudget);
        }
        let limbs = LimbLimit::new(4);
        let output = ZSet::from_updates(output, limbs, &mut |event| meter.charge(event)).map_err(zset_error)?;
        for (row, _) in output.iter() { reserve_row(row, meter)?; }
        let sink = self.rows.prepare_update(&output, limbs, &mut |event| meter.charge(event)).map_err(zset_error)?;
        for (row, _) in output.iter() {
            ...
            if let Some(weight) = sink.weight(row) {
                ...
                if weight <= 0 || (distinct && weight != 1) { return Err(StandingQueryFailure::InvalidDelta); }
            }
        }
        (meter.checkpoint)()?;
...
fn publish_members(members: &mut Members, changes: BTreeMap<GroupKey, Option<Arc<GraphAggregateRow>>>) {
    for (key, value) in changes {
        match value {
            Some(row) => { members.insert(key, row); }
        ...
        for (identity, changes) in changes {
            match classes.entry(identity) {
                ...
                Entry::Vacant(entry) => {
                    ...
                    if !members.is_empty() { entry.insert(members); }
                }
            }
        }
        *count = row_count;
...
mod tests {
    ...
    fn definition(distinct: bool) -> PreparedGraphAggregate {
        ...
        PreparedGraphAggregate::prepare(input, &[0], &[GraphAggregate::count_rows("count")], 0, None)
            .unwrap().with_output_projection(vec![GraphSetProjection::new("value", GraphSetValue::Integer(expression))])
    ...
    fn delta(rows: &[(i64, u64, i128)]) -> ZSet<GraphAggregateRow> {
        ...
        ZSet::from_updates(rows.iter().map(|&(group, count, sign)| {
            (definition.materialize_incremental_row(vec![GraphValue::Scalar(CanonicalScalar::Int(group))],
                vec![GraphAggregateValue::Count(count)]).unwrap(), ZWeight::from_i128(sign))
        }), LimbLimit::new(4), &mut |_| Ok::<_, ()>(())).unwrap()
    }

    fn apply(state: &mut State, rows: &[(i64, u64, i128)], limit: u64) -> Result<(), StandingQueryFailure> {
        let mut checkpoint = || Ok(());
        let mut meter = Meter { policy: GqlQueryPolicy::new(100_000, limit, 10_000_000, 10_000_000),
    ...
    fn representative_replacement_preserves_exact_variant_and_retires_empty_classes() {
        ...
        assert_eq!(state.rows.iter().next().unwrap().0.values(), &[GraphAggregateValue::Count(2)]);
        assert_eq!(state.classes.values().next().unwrap().len(), 3);
        apply(&mut state, &[(1, 2, -1)], 1).unwrap();
        assert_eq!(state.rows.iter().next().unwrap().0.values(),
    ...
    fn all_budget_counts_occurrences_and_distinct_updates_are_whole_tick() {
        ...
        assert_eq!(apply(&mut all, &[(2, 1, 1), (3, 1, 1)], 1), Err(StandingQueryFailure::ResultBudget));
    ...
    fn every_output_checkpoint_and_dropped_guard_preserves_support_and_retries() {
        ...
        for stop in 1..=calls {
            ...
            let mut seen = 0;
            {
                ...
                let mut meter = Meter { policy, stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                assert!(matches!(candidate.prepare(&change, &mut meter), Err(StandingQueryFailure::Interrupted)));
            }
    ...
    fn whole_maintenance_cancellation_and_budget_refusal_preserve_source_and_output() {
        ...
        let ((), report) = run_async_under_lab(0x6a80, |root| async move {
            ...
            let seed = || {
                ...
                for id in 1..=3 {
                    rows.create_vertex(VId(id), vec![], vec![(PropertyKeyId(1), CanonicalScalar::Int(id as i64))]);
                }
                rows
            };
            ...
            changes.create_vertex(VId(4), vec![], vec![(PropertyKeyId(1), CanonicalScalar::Int(1))]);
            ...
            let stats = {
                let mut checkpoint = || { calls += 1; Ok(()) };
                let mut meter = Meter { policy, stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                successful.maintain_with_output(&delta, &mut meter, Some(&mut after)).unwrap();
                meter.stats
            };
            assert_eq!(after.row_count, 1);
            assert_eq!(after.rows.iter().next().unwrap().0.values(), &[GraphAggregateValue::Count(2)]);
            for stop in 1..=calls {
                ...
                let mut seen = 0;
                {
                    ...
                    let mut meter = Meter { policy, stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                    assert_eq!(candidate.maintain_with_output(&delta, &mut meter, Some(&mut output)),
                ...
                let mut checkpoint = || Ok(());
                let mut meter = Meter { policy, stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                candidate.maintain_with_output(&delta, &mut meter, Some(&mut output)).unwrap();
