...
fn bag() -> ZSet<u8> {
    ZSet::from_updates(
        [(1, ZWeight::from_i128(2)), (2, ZWeight::ONE)],
        LimbLimit::new(4),
        &mut |_| Ok::<_, ()>(()),
    )
    .unwrap()
...
fn every_delivery_checkpoint_refuses_without_changing_the_published_generation() {
    ...
    let view = StandingQueryView {
        rows: &bag,
        ordered: Some(&ordered),
        frontier: CommitSeq(7),
        stats: &stats,
    };
    let run = |stop, policy| {
        let mut calls = 0;
    ...
    assert_eq!(
        ...
        &vec![
            vec![QueryValue::Count(2)],
            vec![QueryValue::Count(1)],
            vec![QueryValue::Count(1)]
        ]
    );
    for stop in 1..=calls {
    ...
    assert_eq!(
        run(
            usize::MAX,
            GqlQueryPolicy::new(0, 3, measured.work_units, measured.scratch_entries)
        )
        .0,
        expected
    );
    for (work, scratch, rows, error) in [
        (
            ...
            measured.scratch_entries,
            3,
            StandingQueryFailure::WorkBudget,
        ),
        (
            measured.work_units,
            measured.scratch_entries - 1,
            3,
        ...
        assert_eq!(
            run(usize::MAX, GqlQueryPolicy::new(0, rows, work, scratch)).0,
            Err(error)
        );
    }
...
fn bag_delivery_preserves_weights_and_refuses_impossible_counts_or_row_width() {
    ...
    let mut meter = Meter {
        policy: GqlQueryPolicy::new(0, 10, 1000, 1000),
        stats,
        checkpoint: &mut checkpoint,
    };
    let bag = bag();
    ...
    let actual = collect(&view, 1, &mut meter, |row, _| {
        Ok(vec![QueryValue::Count(u64::from(*row))])
    })
    .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        collect(&view, 2, &mut meter, |_, _| Ok(vec![])),
        Err(StandingQueryFailure::InvalidDelta)
    );
    for (weight, error) in [
        (ZWeight::from_i128(-1), StandingQueryFailure::InvalidDelta),
        (
        ...
        assert_eq!(
            collect(&view, 1, &mut meter, |_, _| panic!(
                "must refuse before projecting"
            )),
            Err(error)
        );
    }
...
fn failed_native_registrations_do_not_leave_registry_entries() {
    ...
    let ((), report) = run_async_under_lab(0x6e73_7110, |root| async move {
        ...
        for text in [
            "not a query",
            "MATCH (n) WHERE n.p > $missing RETURN n.p AS p",
        ] {
            assert!(matches!(
                db.register_standing_native(&cx, text, &params, resolve, policy),
        ...
        for (text, class) in [
            (
                "MATCH (n) FOR SYSTEM_TIME AS OF SEQ 0 RETURN n.p AS p",
                NativeReadClass::TemporalPattern,
            ),
            (
                "MATCH (n) FOR SYSTEM_TIME AS OF SEQ 0 RETURN COUNT(*) AS c",
                NativeReadClass::TemporalAggregate,
            ),
            (
                "MATCH (n) FOR SYSTEM_TIME AS OF SEQ 0 RETURN n.p AS p UNION ALL MATCH (m) RETURN m.p AS p",
                NativeReadClass::TemporalSet,
            ),
        ] {
            assert!(
                matches!(db.register_standing_native(&cx, text, &params, resolve, policy),
                Err(StandingQueryError::NativeClassUnsupported { facade }) if facade == class)
            );
            assert_eq!(db.standing_queries.len(), before);
        }
        assert!(
            db.register_standing_native(
                &cx,
        ...
        let raw = db
            .register_standing_rows(
                &cx,
                fgdb_gql::PreparedGraphText::prepare("MATCH (n) RETURN n", resolve)
                    .unwrap()
            ...
            .unwrap();
        assert!(matches!(
            db.standing_native_query(&cx, &raw, policy),
