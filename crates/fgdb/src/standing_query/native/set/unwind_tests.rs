...
use fgdb_gql::{GraphSetProjection, GraphSetValue, GraphSymbol, GraphSymbolKind, PreparedGraphText};
...
fn expansion() -> PreparedGraphSet {
    leaf().unwind("element".into(), GraphSetValue::List(vec![
        GraphSetValue::Column(0), GraphSetValue::Column(0)])).unwrap()
}
fn composed() -> PreparedGraphSet {
    expansion().project(vec![GraphSetProjection::new("element", GraphSetValue::Column(1))],
...
fn expected<V: Vfs + Clone>(db: &Database<V>, distinct: bool) -> (CommitSeq, QueryResult) {
    ...
    for vertex in db.vertices_at(at).unwrap() {
        let value = vertex.props.iter().find(|(k, _)| *k == P).map(|(_, v)| v.clone())
    ...
    values.sort(); if distinct { values.dedup(); }
    (at, QueryResult::Rows { columns: vec!["x".into()], rows: values.into_iter()
...
fn native_unwind_pipelines_maintain_duplicates_and_distinct_after_real_property_updates() {
    let ((), report) = run_async_under_lab(0x756e_7701, |root| async move {
        ...
        let all = db.register_standing_native(&cx, all_text, &GqlParameters::new(), symbols, policy()).unwrap();
        let distinct = db.register_standing_native(&cx, distinct_text, &GqlParameters::new(), symbols, policy()).unwrap();
        for tick in 0..5 {
            assert_eq!(db.standing_native_query(&cx, &all, policy()).unwrap(), expected(&db, false));
            assert_eq!(db.standing_native_query(&cx, &distinct, policy()).unwrap(), expected(&db, true));
            let mut integrated = db.standing_projection(&cx, &all).unwrap().rows()
            ...
            match tick {
                ...
                1 => { change.set_vertex_property(VId(3), P, Some(CanonicalScalar::Int(7))); }
            ...
            integrated.integrate(db.standing_projection_delta(&cx, &all).unwrap().unwrap().rows(),
                LimbLimit::new(4), &mut |_| Ok::<_, ()>(())).unwrap();
            assert_eq!(&integrated, db.standing_projection(&cx, &all).unwrap().rows());
        }
        assert_eq!(db.standing_native_query(&cx, &all, policy()).unwrap(), expected(&db, false));
        db.rebuild_standing_query(&cx, &all, policy()).unwrap();
        db.rebuild_standing_query(&cx, &distinct, policy()).unwrap();
        let mut change = WriteBatch::new(RelationId(1)); change.create_vertex(VId(u128::MAX), vec![], vec![(P, CanonicalScalar::Int(9))]);
        db.write(&commit, change).await.unwrap();
        assert_eq!(db.standing_native_query(&cx, &all, policy()).unwrap(), expected(&db, false));
        assert_eq!(db.standing_native_query(&cx, &distinct, policy()).unwrap(), expected(&db, true));
    });
...
fn bound_list_parameters_remain_frozen_and_preserve_nested_values_and_full_width_ids() {
    let ((), report) = run_async_under_lab(0x756e_7702, |root| async move {
        let c = PurposeContexts::narrow_runtime_root(&root); let cx = c.query(); let commit = c.commit();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let nested = GraphValue::List(vec![GraphValue::Vertex(VId(u128::MAX)),
            GraphValue::Scalar(CanonicalScalar::Null)].into_boxed_slice());
        let mut args = GqlParameters::new().with_list("xs", vec![nested.clone(), nested.clone()]).unwrap();
        let handle = db.register_standing_native(&cx,
            "MATCH (n) WITH n AS id UNWIND $xs AS x RETURN id,x", &args, symbols, policy()).unwrap();
        ...
        let mut seed = WriteBatch::new(RelationId(1)); seed.create_vertex(VId(5), vec![], vec![]);
        db.write(&commit, seed).await.unwrap();
        let row = vec![QueryValue::Value(GraphValue::Vertex(VId(5))), QueryValue::Value(nested)];
        let expected = (db.frontier().unwrap(), QueryResult::Rows {
            columns: vec!["id".into(), "x".into()], rows: vec![row.clone(), row] });
        assert_eq!(db.standing_native_query(&cx, &handle, policy()).unwrap(), expected);
        db.rebuild_standing_query(&cx, &handle, policy()).unwrap();
        assert_eq!(db.standing_native_query(&cx, &handle, policy()).unwrap(), expected);
        assert!(matches!(db.standing_native_query(&cx, &handle, GqlQueryPolicy::new(0, 1, 1_000_000, 1_000_000)),
...
fn expansion_quota_and_nonlist_failures_fence_dependents_without_undoing_durable_writes() {
    let ((), report) = run_async_under_lab(0x756e_7703, |root| async move {
        ...
        db.write(&commit, seed).await.unwrap();
        let hidden = leaf().unwind("element".into(), GraphSetValue::Column(0)).unwrap();
        let invalidated = db.register_standing_relation(&cx, &hidden, policy()).unwrap();
        assert!(db.standing_projection(&cx, &invalidated).unwrap().rows().is_empty());
        let expanded = db.register_standing_relation(&cx, &expansion(),
            GqlQueryPolicy::new(100_000, 2, 10_000_000, 10_000_000)).unwrap();
        let child = db.register_standing_set(&cx, &expanded, &expanded, SetOperation::UnionDistinct, policy()).unwrap();
        let mut change = WriteBatch::new(RelationId(1));
        change.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(1)));
        change.create_vertex(VId(2), vec![], vec![]);
        db.write(&commit, change).await.unwrap();
        assert!(matches!(db.standing_projection(&cx, &invalidated), Err(StandingQueryError::Unavailable {
            reason: StandingQueryFailure::OutputExpression { column: 1, error: fgdb_gql::GraphIntegerError {
                kind: fgdb_gql::GraphIntegerErrorKind::IncompatibleOperands, .. } }, .. })));
        assert!(matches!(db.standing_projection(&cx, &expanded), Err(StandingQueryError::Unavailable {
        ...
        assert_eq!(db.standing_projection_total(&cx, &expanded).unwrap(), &ZWeight::from_i128(4));
        assert!(db.rebuild_standing_query(&cx, &invalidated, policy()).is_err());
        let mut fix = WriteBatch::new(RelationId(1)); fix.set_vertex_property(VId(1), P, None);
        db.write(&commit, fix).await.unwrap();
        db.rebuild_standing_query(&cx, &invalidated, policy()).unwrap();
        assert!(db.standing_projection_delta(&cx, &invalidated).unwrap().is_none());
        assert!(db.standing_projection(&cx, &invalidated).unwrap().rows().is_empty());
        assert_eq!(db.standing_projection_total(&cx, &expanded).unwrap(), &ZWeight::from_i128(4));
...
fn every_unwind_circuit_stage_refusal_and_unsupported_page_preserves_existing_nodes() {
    let ((), report) = run_async_under_lab(0x756e_7704, |root| async move {
        ...
        let mut seed = WriteBatch::new(RelationId(1)); seed.create_vertex(VId(1), vec![], vec![]);
        db.write(&commit, seed).await.unwrap();
        db.register_standing_relation(&cx, &leaf(), policy()).unwrap();
        let before = saved(&db.standing_queries); let definition = composed();
        let mut calls = 0;
        ...
        for query in [expansion().with_page(0, Some(0)),
            leaf().with_page(0, Some(0)).unwind("element".into(), GraphSetValue::Column(0)).unwrap()] {
            assert!(db.register_standing_relation(&cx, &query, policy()).is_err());
            assert_eq!(saved(&db.standing_queries), accepted);
        }
        let mut change = WriteBatch::new(RelationId(1)); change.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(4)));
        db.write(&commit, change).await.unwrap();
        assert_eq!(db.standing_projection_total(&cx, &handle).unwrap(), &ZWeight::ONE);
        assert_eq!(db.standing_native_query(&cx, &handle, policy()).unwrap().1,
            QueryResult::Rows { columns: vec!["element".into()],
                rows: vec![vec![QueryValue::Value(GraphValue::Scalar(CanonicalScalar::Int(4)))]] });
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
