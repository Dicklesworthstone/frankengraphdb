...
fn snapshot<V: Vfs + Clone>(
    ...
    let result = query
        .execute_governed(
            policy(),
            |pattern, allowance| db.execute_graph_pattern_governed_at(cx, pattern, at, allowance),
            || cx.checkpoint(),
        )
        .unwrap();
    QueryResult::Rows {
...
fn native_with_pipelines_keep_nested_pages_distinct_and_parameters_through_commits() {
    let ((), report) = run_async_under_lab(0x7769_0301, |root| async move {
        ...
        let original = GqlParameters::new()
            .with_uint64("top", 4)
            .unwrap()
        ...
        let handles: Vec<_> = cases
            .iter()
            .map(|(text, params)| {
                db.register_standing_native(&cx, text, params, symbols, policy())
                    .unwrap()
            })
            .collect();
        assert_eq!(
        ...
        assert_eq!(
            db.standing_native_query(&cx, &handles[1], policy())
                .unwrap()
                .1,
            expected(&[7])
        );
        ...
        assert_eq!(
            db.standing_native_query(&cx, &handles[3], policy())
                .unwrap()
                .1,
            expected(&[9, 7])
        );
        for tick in 0..6 {
            ...
            match tick {
                ...
                1 => {
                    edit.set_vertex_property(VId(2), P, Some(CanonicalScalar::Int(1)));
                }
                2 => {
                    edit.create_vertex(VId(u128::MAX), vec![], vec![]);
                }
                3 => {
                    edit.set_vertex_property(VId(3), P, Some(CanonicalScalar::Int(11)));
                }
                4 => {
                    edit.set_vertex_property(VId(u128::MAX), P, Some(CanonicalScalar::Int(8)));
                }
                _ => {
                    edit.delete_vertex(VId(4));
                }
            }
...
fn filters_scopes_and_bare_pages_preserve_inherited_rank_but_projection_resets_it() {
    let ((), report) = run_async_under_lab(0x7769_0302, |root| async move {
        ...
        let queries = [
            filtered.clone(),
            filtered.clone().with_page(1, Some(2)),
        ...
        let handles: Vec<_> = queries
            .iter()
            .map(|query| db.register_standing_relation(&cx, query, policy()).unwrap())
            .collect();
        for (at, values) in [
            &[7, 7, 5][..],
            &[7, 5],
        ...
        {
            assert_eq!(
                db.standing_native_query(&cx, &handles[at], policy())
                    .unwrap()
                    .1,
                expected(values)
            );
        ...
        assert!(matches!(
            &db.standing_queries[handles[0].index],
            StandingQuery::Window(_)
        ));
        let mut integrated = db
            .standing_window(&cx, &handles[0])
            .unwrap()
            .rows()
        ...
        integrated
            .integrate(
                db.standing_window_delta(&cx, &handles[0])
                    .unwrap()
                    .unwrap()
                    .rows(),
                LIMBS,
        ...
        assert_eq!(
            &integrated,
            db.standing_window(&cx, &handles[0]).unwrap().rows()
        );
        for (handle, query) in handles.iter().zip(&queries) {
            assert_eq!(
                db.standing_native_query(&cx, handle, policy()).unwrap().1,
                snapshot(&db, &cx, query)
            );
            db.rebuild_standing_query(&cx, handle, policy()).unwrap();
            assert_eq!(
                db.standing_native_query(&cx, handle, policy()).unwrap().1,
...
fn every_nested_registration_and_rebuild_boundary_keeps_the_old_circuit_intact() {
    let ((), report) = run_async_under_lab(0x7769_0303, |root| async move {
        ...
        let mut calls = 0;
        {
            ...
            staged
                .compile_root(&cx, &query, policy(), &mut || {
                    calls += 1;
                    Ok(())
                })
                .unwrap();
        ...
        let size = db.standing_queries.len();
        let mut calls = 0;
...
fn rank_before_expression_excludes_off_page_errors_then_fences_and_repairs_when_they_enter() {
    let ((), report) = run_async_under_lab(0x7769_0304, |root| async move {
        ...
        .unwrap();
        let query = ranked(leaf(), 2)
            .project(
                vec![GraphSetProjection::new("p", GraphSetValue::Integer(divide))],
                GraphSetQuantifier::All,
            )
            .unwrap()
            .with_order_by(&[GraphValueOrder::descending(0)])
        ...
        let handle = db
            ...
            .unwrap();
        let dependent = db
            .register_standing_set(&cx, &handle, &handle, SetOperation::UnionAll, policy())
            .unwrap();
        let sibling = db
            .register_standing_relation(&cx, &leaf(), policy())
            .unwrap();
        ...
        let mut edit = WriteBatch::new(RelationId(1));
        edit.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(-1)));
        ...
        assert!(db.standing_queries.iter().any(|query| matches!(
            query.status().2,
        ...
        fix.set_vertex_property(VId(3), P, Some(CanonicalScalar::Int(1)));
        ...
        next.set_vertex_property(VId(2), P, Some(CanonicalScalar::Int(6)));
        ...
        assert_eq!(
            db.standing_set_total(&cx, &dependent).unwrap().to_i128(),
            Some(4)
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn zero_ranked_pages_do_not_hide_bad_descendants_or_unknown_enumeration() {
    let ((), report) = run_async_under_lab(0x7769_0305, |root| async move {
        ...
        assert!(matches!(
            db.register_standing_relation(&cx, &unknown, policy()),
        ...
        let bad = ranked(leaf(), 6)
            ...
            .unwrap();
        assert!(matches!(
            db.register_standing_relation(&cx, &ranked(bad, 0), policy()),
            Err(StandingQueryError::Maintenance(
        ...
        db.write(&commit, edit).await.unwrap();
        assert!(
            db.standing_window_delta(&cx, &handle)
                .unwrap()
                .unwrap()
                .rows()
                .is_empty()
