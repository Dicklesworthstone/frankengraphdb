...
use fgdb_gql::{
    ...
    GraphSetPredicateOp, GraphSetProjection, GraphSetValue, GraphSymbol, GraphSymbolKind,
    PreparedGraphText,
};
...
fn oracle<V: Vfs + Clone>(db: &Database<V>) -> QueryResult {
    ...
    for vertex in db.vertices_at(db.frontier().unwrap()).unwrap() {
        let (_, CanonicalScalar::Int(value)) = &vertex.props[0] else {
...
fn text_and_bound_terminal_windows_deliver_exact_ranked_rows_after_updates_and_rebuild() {
    let ((), report) = run_async_under_lab(0x7769_0201, |root| async move {
        ...
        let bound = db
            ...
            .unwrap();
        assert_eq!(
            db.standing_native_query(&cx, &native, policy()).unwrap().1,
            scalar_result([9, 7, 7])
        );
        for tick in 0..3 {
            ...
            match tick {
                ...
                _ => {
                    edit.create_vertex(VId(u128::MAX), vec![], vec![(P, CanonicalScalar::Int(8))]);
                }
            }
        ...
        assert!(matches!(
            db.standing_native_query(&cx, &bound, small),
            Err(StandingQueryError::Delivery(
                StandingQueryFailure::ResultBudget
            ))
        ));
        assert_eq!(
            db.standing_native_query(&cx, &bound, policy()).unwrap(),
            before
        );
    });
        ...
        for quantifier in [GraphSetQuantifier::All, GraphSetQuantifier::Distinct] {
            ...
            let rows = expected
                ...
                .map(|(a, b)| {
                    [a, b]
                        .into_iter()
                        .map(|v| QueryValue::Value(GraphValue::Scalar(CanonicalScalar::Int(v))))
                        .collect()
                })
                .collect();
            assert_eq!(
                ...
                QueryResult::Rows {
                    columns: vec!["left_value".into(), "right_value".into()],
                    rows
                }
            );
            let snapshot = query
...
fn terminal_zero_windows_admit_every_child_and_do_not_erase_nested_sequence_semantics() {
    let ((), report) = run_async_under_lab(0x7769_0204, |root| async move {
        ...
        let bad = leaf()
            ...
            .unwrap();
        assert!(matches!(
            db.register_standing_relation(&cx, &ordered(bad, 0, 0), policy()),
            Err(StandingQueryError::Maintenance(
        ...
        assert_eq!(saved(&db.standing_queries), before);
        let empty = db
            .register_standing_relation(&cx, &ordered(leaf(), 0, 0), policy())
            .unwrap();
        assert_eq!(
