...
fn computed(quantifier: GraphSetQuantifier) -> PreparedGraphSet {
    ...
    let mapped = sum
        .project(
            vec![GraphSetProjection::new("p", GraphSetValue::Integer(parity))],
            quantifier,
        )
        .unwrap();
    let shifted = GraphIntegerExpression::prepare(&[
        Op::Column(0),
        Op::Literal(Some(10)),
...
fn expected<V: Vfs + Clone>(db: &Database<V>, q: GraphSetQuantifier) -> (CommitSeq, QueryResult) {
    ...
    for row in db.vertices_at(at).unwrap() {
        ...
        values.push(GraphValue::Scalar(value.clone()));
        values.push(GraphValue::Scalar(value));
    }
    values.sort();
...
fn compiled_projection_set_trees_deliver_exact_native_values_after_each_commit() {
    let ((), report) = run_async_under_lab(0x7072_6420, |root| async move {
        ...
        let distinct = register(
            &mut db,
            &cx,
            &computed(GraphSetQuantifier::Distinct),
            policy(),
        )
        .unwrap();
        for tick in 0..4 {
            for (handle, q) in [
                ...
                assert_eq!(
                    db.standing_native_columns(&cx, handle).unwrap(),
                    &["computed"]
                );
            }
            ...
            match tick {
                0 => {
                    update.set_vertex_property(VId(2), P, Some(CanonicalScalar::Int(5)));
                }
                1 => {
                ...
                2 => {
                    update.create_vertex(VId(99), vec![], vec![(P, CanonicalScalar::Int(0))]);
                }
                _ => {
        ...
        assert_eq!(
            ...
            expected(&db, GraphSetQuantifier::All)
        );
        assert_eq!(
...
fn every_projection_circuit_stage_refusal_preserves_old_nodes_and_rebuild_rebases_dependencies() {
    let ((), report) = run_async_under_lab(0x7072_6421, |root| async move {
        ...
        let definition = computed(GraphSetQuantifier::Distinct);
        let mut calls = 0;
        {
...
fn unsupported_projection_descendants_and_late_expression_failures_leave_no_partial_registration() {
    let ((), report) = run_async_under_lab(0x7072_6422, |root| async move {
        ...
        let query = leaf()
            ...
            .unwrap();
        assert!(matches!(
            register(&mut db, &cx, &query, policy()),
            Err(StandingQueryError::Maintenance(
