...
use fgdb_gql::{
    GqlParameters, GqlQueryError, GqlQueryPolicy, GraphSetColumnType, GraphSetExecutionError,
...
fn leaf(label: &str) -> PreparedGraphSet {
    PreparedGraphText::prepare(
        &format!("MATCH (n:{label}) RETURN n.k AS k, n.p AS p"),
        symbols,
    )
    .unwrap()
    ...
    let ((), report) = run_async_under_lab(0x6f6e_0501, |root| async move {
        ...
        for kind in KINDS {
            for keyed in [true, false] {
                ...
                let handle = db
                    ...
                    .unwrap();
                let rows = db
                    .execute_graph_set_governed(&cx, &definition, policy())
                    .unwrap()
                    .value;
                assert_eq!(
                ...
                assert_eq!(
                    db.standing_join_spec(&cx, &handle).unwrap(),
                    definition.incremental_join().unwrap().2
                );
        ...
        for tick in 0..4 {
            ...
            match tick {
                ...
                1 => {
                    change.set_vertex_property(VId(5), K, None);
                }
                2 => {
                    change.delete_vertex(VId(2));
                    vertex(&mut change, 6, 2, 1, 15);
                }
                _ => {
...
fn live_historical_pinned_and_staged_readers_share_join_semantics_without_crossing_frontiers() {
    let ((), report) = run_async_under_lab(0x6f6e_0502, |root| async move {
        ...
        let before = vec![
            row(&[Some(1), Some(10), Some(1), Some(20)]),
            row(&[Some(1), Some(30), None, None]),
        ];
        assert_eq!(
            db.execute_graph_set_governed(&cx, &definition, policy())
                .unwrap()
                .value,
            before
        );
        ...
        let staged = vec![
            row(&[Some(1), Some(10), Some(1), Some(40)]),
            row(&[Some(1), Some(30), Some(1), Some(40)]),
        ];
        assert_eq!(
            tx.execute_graph_set_governed(&db, &cx, &definition, policy())
                .unwrap()
                .value,
            staged
        );
        assert_eq!(
            db.standing_native_query(&cx, &handle, policy()).unwrap(),
            native(basis, &definition, &before)
        );
        assert_eq!(
            pinned
                .execute_graph_set_governed(&cx, &definition, policy())
        ...
        assert_eq!(
            db.standing_native_query(&cx, &handle, policy()).unwrap(),
            native(after, &definition, &staged)
        );
        assert_eq!(
            db.execute_graph_set_governed_at(&cx, &definition, basis, policy())
                .unwrap()
                .value,
            before
        );
        assert_eq!(
            pinned
                .execute_graph_set_governed_at(&cx, &definition, basis, policy())
                .unwrap()
                .value,
            before
        );
        assert!(
            pinned
                .execute_graph_set_governed_at(&cx, &definition, after, policy())
                .is_err()
        );
...
fn saved(queries: &[StandingQuery]) -> Vec<Saved> {
    queries
        ...
        .map(|query| {
            ...
            let rows = sets::rows(query)
                .unwrap()
                .checked_clone(LimbLimit::new(4), &mut |_| Ok::<_, ()>(()))
                .unwrap();
            let spec = if let StandingQuery::Join(query) = query {
                Some(query.spec().clone())
            } else {
                None
            };
...
fn join_projection_window_registration_and_rebuild_are_atomic_at_every_circuit_stage() {
    let ((), report) = run_async_under_lab(0x6f6e_0503, |root| async move {
        ...
        db.write(&commit, seed).await.unwrap();
        let sibling = db
            .register_standing_relation(&cx, &leaf("L"), policy())
            .unwrap();
        ...
        let handle = db
            .register_standing_relation(&cx, &definition, policy())
            .unwrap();
        let Some(Layout::Circuit { first, .. }) = handle.native.as_deref() else {
            panic!()
        };
        let first = *first;
        let mut change = WriteBatch::new(RelationId(1));
