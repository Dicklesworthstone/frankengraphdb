...
use super::*;
use asupersync::{fs::MemVfs, lab::run_async_under_lab};
...
fn product() -> PreparedGraphSet { leaf("a").cross_join(leaf("b")).unwrap() }
fn distinct_product() -> PreparedGraphSet {
    product().nested().unwrap().project(vec![GraphSetProjection::new("a", GraphSetValue::Column(0)),
        GraphSetProjection::new("b", GraphSetValue::Column(1))], GraphSetQuantifier::Distinct).unwrap()
}
fn expected<V: Vfs + Clone>(db: &Database<V>, distinct: bool) -> (CommitSeq, QueryResult) {
    let at = db.frontier().unwrap();
    let values: Vec<_> = db.vertices_at(at).unwrap().into_iter().map(|v| {
        GraphValue::Scalar(v.props.iter().find(|(k, _)| *k == P).map(|(_, v)| v.clone())
            .unwrap_or(CanonicalScalar::Null))
    }).collect();
    let mut rows = Vec::new();
    for left in &values { for right in &values { rows.push(vec![left.clone(), right.clone()]); } }
    rows.sort(); if distinct { rows.dedup(); }
    (at, QueryResult::Rows { columns: vec!["a".into(), "b".into()], rows: rows.into_iter()
        .map(|row| row.into_iter().map(QueryValue::Value).collect()).collect() })
}
...
fn public_relational_products_deliver_exact_bags_and_both_input_cross_terms_after_commits() {
    let ((), report) = run_async_under_lab(0x6372_6f01, |root| async move {
        ...
        for (id, value) in [(1, Some(1)), (2, Some(1)), (3, None)] {
            seed.create_vertex(VId(id), vec![], value.map(|v| vec![(P, CanonicalScalar::Int(v))]).unwrap_or_default());
        }
        db.write(&commit, seed).await.unwrap();
        // These temporary bound trees are dropped immediately after registration.
        let all = db.register_standing_relation(&cx, &product(), policy()).unwrap();
        let distinct = db.register_standing_relation(&cx, &distinct_product(), policy()).unwrap();
        ...
        assert_eq!(db.standing_join_columns(&cx, &all).unwrap(), &["left.a", "right.b"]);
        assert!(db.standing_join_delta(&cx, &all).unwrap().is_none());
        assert_eq!(db.standing_native_query(&cx, &all, policy()).unwrap(), expected(&db, false));
        for tick in 0..5 {
            let mut integrated = db.standing_join(&cx, &all).unwrap().rows()
                .checked_clone(LimbLimit::new(4), &mut |_| Ok::<_, ()>(())).unwrap();
            let mut change = WriteBatch::new(RelationId(1));
            match tick {
                0 => { change.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(2))); }
                1 => { change.set_vertex_property(VId(3), P, Some(CanonicalScalar::Int(3))); }
                2 => { change.delete_vertex(VId(2)); }
                3 => { change.create_vertex(VId(4), vec![], vec![]); }
                _ => { for id in [1, 3, 4] { change.delete_vertex(VId(id)); } }
            }
            ...
            for (handle, is_distinct) in [(&all, false), (&distinct, true)] {
                assert_eq!(db.standing_native_query(&cx, handle,
                    GqlQueryPolicy::new(0, 100_000, 10_000_000, 10_000_000)).unwrap(), expected(&db, is_distinct));
            }
            integrated.integrate(db.standing_join_delta(&cx, &all).unwrap().unwrap().rows(),
            ...
            assert_eq!(db.standing_set_total(&cx, &doubled).unwrap(),
                &total.checked_mul(&ZWeight::from_i128(2), LimbLimit::new(4)).unwrap());
        }
    });
...
fn saved(queries: &[StandingQuery]) -> Vec<Saved> {
    queries.iter().map(|q| { let rows = sets::rows(q).unwrap(); let (policy, at, failure) = q.status();
        (std::ptr::from_ref(rows) as usize, policy, at, failure,
            rows.checked_clone(LimbLimit::new(4), &mut |_| Ok::<_, ()>(())).unwrap()) }).collect()
}
...
fn product_circuit_admission_and_rebuild_are_atomic_at_every_stage_and_rebase_both_inputs() {
    let ((), report) = run_async_under_lab(0x6372_6f02, |root| async move {
        ...
        db.write(&commit, seed).await.unwrap();
        let sibling = db.register_standing_relation(&cx, &leaf("sibling"), policy()).unwrap();
        let before = saved(&db.standing_queries); let definition = distinct_product();
        let mut calls = 0;
        ...
        let mut change = WriteBatch::new(RelationId(1)); change.create_vertex(VId(2), vec![], vec![]);
        db.write(&commit, change).await.unwrap();
        assert_eq!(db.standing_native_query(&cx, &handle, policy()).unwrap(), expected(&db, true));
...
fn public_shared_products_fence_only_dependents_and_rebuild_their_unconditional_definition() {
    let ((), report) = run_async_under_lab(0x6372_6f03, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut seed = WriteBatch::new(RelationId(1));
        for id in 1..=2 { seed.create_vertex(VId(id), vec![], vec![(P, CanonicalScalar::Int(1))]); }
        db.write(&commit, seed).await.unwrap();
        let parent = db.register_standing_rows(&cx, pattern("MATCH (n) RETURN n.p AS p"), policy()).unwrap();
        assert!(matches!(db.register_standing_join(&cx, &parent, &parent, &[], policy()),
            Err(StandingQueryError::JoinSchema(fgdb_gql::row_join::RowJoinBuildError::EmptyKeys))));
        let cross = db.register_standing_cross_join(&cx, &parent, &parent,
            GqlQueryPolicy::new(100_000, 4, 10_000_000, 10_000_000)).unwrap();
        let child = db.register_standing_set(&cx, &cross, &cross, SetOperation::UnionDistinct, policy()).unwrap();
        assert_eq!(db.standing_join_total(&cx, &cross).unwrap(), &ZWeight::from_i128(4));
        let mut change = WriteBatch::new(RelationId(1)); change.create_vertex(VId(3), vec![], vec![]);
        db.write(&commit, change).await.unwrap();
        assert!(matches!(db.standing_join(&cx, &cross), Err(StandingQueryError::Unavailable {
        ...
        assert_eq!(db.standing_join_total(&cx, &cross).unwrap(), &ZWeight::from_i128(9));
        ...
        assert!(matches!(db.register_standing_cross_join(&cx, &parent, &parent,
        ...
        assert!(matches!(foreign.register_standing_cross_join(&cx, &parent, &parent, policy()),
...
fn an_empty_operand_never_suppresses_peer_errors_or_unsupported_scope_semantics() {
    let ((), report) = run_async_under_lab(0x6372_6f04, |root| async move {
        ...
        let bad = leaf("b").project(vec![GraphSetProjection::new("b", GraphSetValue::Integer(divide))], GraphSetQuantifier::All).unwrap();
        let definition = empty.clone().cross_join(bad).unwrap();
        assert!(matches!(db.register_standing_relation(&cx, &definition, policy()),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::OutputExpression { .. }))));
        assert_eq!(saved(&db.standing_queries), before);
        for definition in [product().with_page(0, Some(0)),
            empty.cross_join(leaf("b").with_page(0, Some(0))).unwrap()] {
            assert!(db.register_standing_relation(&cx, &definition, policy()).is_err());
            assert_eq!(saved(&db.standing_queries), before);
        }
        let healthy = db.register_standing_relation(&cx, &product(), policy()).unwrap();
        assert!(matches!(db.standing_native_query(&cx, &healthy, GqlQueryPolicy::new(0, 0, 100_000, 100_000)),
            Err(StandingQueryError::Delivery(StandingQueryFailure::ResultBudget))));
        assert_eq!(db.standing_native_query(&cx, &healthy, policy()).unwrap(), expected(&db, false));
        db.standing_native_query(&cx, &sibling, policy()).unwrap();
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn native_list_products_and_compaction_reopen_keep_domains_and_session_ownership() {
    let ((), report) = run_async_under_lab(0x6372_6f05, |root| async move {
        ...
        let lists = ids.project(vec![GraphSetProjection::new("xs", GraphSetValue::List(vec![
            GraphSetValue::Column(0)]))], GraphSetQuantifier::All).unwrap();
        let definition = lists.cross_join(leaf("p")).unwrap();
        let handle = db.register_standing_relation(&cx, &definition, policy()).unwrap();
        let mut seed = WriteBatch::new(RelationId(1)); seed.create_vertex(VId(u128::MAX), vec![], vec![]);
        db.write(&commit, seed).await.unwrap();
        let expected = db.standing_native_query(&cx, &handle, policy()).unwrap();
        let QueryResult::Rows { rows, .. } = &expected.1 else { panic!("row result") };
        assert_eq!(rows, &vec![vec![QueryValue::Value(GraphValue::List(vec![
            GraphValue::Vertex(VId(u128::MAX))].into_boxed_slice())),
        ...
        assert!(matches!(db.standing_native_query(&cx, &handle, policy()), Err(StandingQueryError::ForeignHandle)));
        let handle = db.register_standing_relation(&cx, &definition, policy()).unwrap();
        assert_eq!(db.standing_native_query(&cx, &handle, policy()).unwrap(), expected);
