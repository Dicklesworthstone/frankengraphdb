    ...
    for at in 0..count {
        for (id, label, value) in [(at + 1, 1, at as i64), (at + 10_001, 2, at as i64 + 1000)] {
            batch.create_vertex(
                ...
                vec![LabelId(label)],
                vec![
                    (K, CanonicalScalar::Int(at as i64)),
...
fn selective_circuit_avoids_cartesian_admission_and_keeps_exact_committed_deltas() {
    let ((), report) = run_async_under_lab(0x6f6e_0601, |root| async move {
        ...
        assert!(matches!(
            db.register_standing_relation(&cx, &product, policy()),
        ...
        let StandingQuery::Join(join) = &db.standing_queries[first + 2] else {
            panic!("missing selected join")
        };
        assert_eq!(join.spec().keys(), &[(0, 0)]);
        assert!(join.spec().predicate().is_some());
        assert_eq!(check(&db, &cx, &query, &handle).values().sum::<i128>(), 64);
        let mut before = sets::rows(&db.standing_queries[handle.index])
            .unwrap()
        ...
        before
            .integrate(
                sets::delta(&db.standing_queries[handle.index]).unwrap(),
                LIMBS,
                &mut |_| Ok::<_, ()>(()),
            )
            .unwrap();
        assert_eq!(
            &before,
            sets::rows(&db.standing_queries[handle.index]).unwrap()
        );
        let mut change = WriteBatch::new(RelationId(1));
        ...
        assert_eq!(check(&db, &cx, &query, &handle), expected);
        assert!(
            sets::delta(&db.standing_queries[handle.index])
                .unwrap()
                .is_empty()
        );
    });
...
fn false_on_discards_pairs_but_empty_left_cannot_hide_unsupported_right_input() {
    let ((), report) = run_async_under_lab(0x6f6e_0602, |root| async move {
        ...
        let invalid = leaf("M")
            .cross_join(leaf("R").with_page(1, None))
            .unwrap()
            .filter(&[GraphSetPredicateOp::Truth(Some(false))])
            .unwrap();
        assert!(matches!(
            db.register_standing_relation(&cx, &invalid, policy()),
...
fn native_unwind_match_with_where_uses_selected_maintenance_without_new_syntax() {
    ...
    let value_row = |cells: &[i64]| {
        GraphValueRow::from_owned_values(
            cells
                .iter()
                .map(|&cell| GraphValue::Scalar(CanonicalScalar::Int(cell)))
                .collect(),
        )
    };
    let ((), report) = run_async_under_lab(0x6f6e_0604, move |root| async move {
        ...
        let joins: Vec<_> = db
            ...
            .iter()
            .filter_map(|state| {
                if let StandingQuery::Join(join) = state {
                    Some(join)
                } else {
                    None
                }
        ...
        assert_eq!(
            joins[0].spec().left_types(),
            &[fgdb_gql::GraphSetColumnType::Any]
        );
        ...
        db.write(&commit, change).await.unwrap();
        assert_eq!(
            check(&db, &cx, &query, &handle),
            Bag::from([(value_row(&[1, 1, 1]), 1)])
        );
        let mut change = WriteBatch::new(RelationId(1));
        change.create_vertex(
            VId(500_000),
            vec![LabelId(1)],
            vec![(K, CanonicalScalar::Int(2)), (P, CanonicalScalar::Int(99))],
        );
        db.write(&commit, change).await.unwrap();
...
fn every_selected_circuit_registration_and_rebuild_checkpoint_preserves_previous_states() {
    let ((), report) = run_async_under_lab(0x6f6e_0605, |root| async move {
        ...
        let sibling = db
            .register_standing_relation(&cx, &leaf("R"), policy())
            .unwrap();
        let query = projected_query();
