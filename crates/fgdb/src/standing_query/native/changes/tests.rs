...
fn native_projection_commutes_with_exact_small_bag_derivatives() {
    ...
    for before in 0..81_usize {
        for after in 0..81_usize {
            ...
            let a = counts(before);
            let b = counts(after);
            let delta = bag((0..4_u8).map(|key| {
                (key, ZWeight::from_i128(b[key as usize] - a[key as usize]))
            }));
            let mapped = run(&delta, true, 2, policy(), |key| {
                vec![scalar(i64::from(*key / 3)); 2]
            }).unwrap();
            let expected = bag((0..2).map(|class| {
                let count: i128 = (0..4).filter(|key| key / 3 == class)
                    .map(|key| b[key] - a[key]).sum();
            ...
            baseline.integrate(&mapped, LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
            let source = bag((0..4_u8).map(|key| (key, ZWeight::from_i128(b[key as usize]))));
            assert_eq!(baseline, run(&source, false, 2, policy(), |key| {
                vec![scalar(i64::from(*key / 3)); 2]
            }).unwrap());
        }
    }
...
fn promoted_signed_weights_never_expand_and_quota_follows_consolidation() {
    ...
    assert_eq!(
        run(&negative, false, 1, bounded, |_| panic!("negative baseline must refuse")),
        Err(StandingQueryFailure::InvalidDelta)
    );
    let retraction = run(&negative, true, 1, bounded, |_| vec![scalar(7)]).unwrap();
    assert_eq!(retraction.weight(&vec![scalar(7)]).unwrap().to_i128(), Some(i128::MIN));
    let cancellation = bag([
        (1_u8, positive.weight(&1).unwrap().checked_clone(LIMBS).unwrap()),
        (2_u8, ZWeight::from_i128(i128::MIN)),
    ]);
    assert!(run(&cancellation, true, 1, GqlQueryPolicy::new(0, 0, 1000, 1000),
        |_| vec![scalar(7)]).unwrap().is_empty());
    assert_eq!(run(&positive, false, 2, bounded, |_| vec![scalar(7)]),
        Err(StandingQueryFailure::InvalidDelta));
    assert_eq!(run(&positive, false, 1, GqlQueryPolicy::new(0, 0, 1000, 1000),
        |_| vec![scalar(7)]), Err(StandingQueryFailure::ResultBudget));
}
...
fn exact_aggregate_cells_follow_repeated_and_permuted_return_slots() {
    ...
    let row = definition.materialize_incremental_row(
        vec![GraphValue::Vertex(VId(u128::MAX))],
        vec![QueryValue::Count(u64::MAX), QueryValue::Integer(i128::MIN), QueryValue::Average(average)],
    ).unwrap();
    let slots = [
        GraphAggregateTextSlot::Aggregate(2), GraphAggregateTextSlot::GroupKey(0),
    ...
    assert_eq!(aggregate_cells(&row, &slots, &mut meter).unwrap(), vec![
        QueryValue::Average(average), QueryValue::Value(GraphValue::Vertex(VId(u128::MAX))),
        QueryValue::Count(u64::MAX), QueryValue::Integer(i128::MIN), QueryValue::Average(average),
    ]);
    assert_eq!(aggregate_cells(&row, &[GraphAggregateTextSlot::Aggregate(3)], &mut meter),
        Err(StandingQueryFailure::InvalidDelta));
}

#[test]
fn every_delivery_checkpoint_and_exact_allowance_preserves_the_source_bag() {
    let source = bag([(1_u8, ZWeight::from_i128(-2)), (2, ZWeight::ONE), (3, ZWeight::ONE)]);
    ...
    assert_eq!(attempt(usize::MAX, GqlQueryPolicy::new(0, 3, stats.work_units, stats.scratch_entries)).0, expected);
    for (work, scratch, rows, error) in [
        (stats.work_units - 1, stats.scratch_entries, 3, StandingQueryFailure::WorkBudget),
        (stats.work_units, stats.scratch_entries - 1, 3, StandingQueryFailure::ScratchBudget),
        (stats.work_units, stats.scratch_entries, 2, StandingQueryFailure::ResultBudget),
    ] {
        assert_eq!(attempt(usize::MAX, GqlQueryPolicy::new(0, rows, work, scratch)).0, Err(error));
    }
    for stop in 1..=calls {
        let mut seen = 0;
...
fn result_bag(result: QueryResult) -> ZSet<Vec<QueryValue>> {
    let QueryResult::Rows { rows, .. } = result else { panic!("read result expected") };
...
fn native_bags_and_derivatives_integrate_across_operator_and_aggregate_shapes() {
    let ((), report) = run_async_under_lab(0x6dde_11, |root| async move {
        ...
        for text in texts {
            let handle = db.register_standing_native(&cx, text, &params, resolve, policy()).unwrap();
            let (at, baseline) = db.standing_native_bag(&cx, &handle, policy()).unwrap();
            assert_eq!(at, first);
            assert_eq!(baseline, result_bag(db.query(&cx, text, &params, resolve, policy()).unwrap()));
            assert!(matches!(db.standing_native_delta(&cx, &handle, CommitSeq(first.0 - 1), policy()),
        ...
        for tick in 0..3 {
            ...
            if tick == 0 {
                ...
                batch.set_vertex_property(VId(2), PropertyKeyId(99), Some(CanonicalScalar::Int(tick)));
            }
            let at = db.write(&commit, batch).await.unwrap();
            for (index, handle) in handles.iter().enumerate() {
                let (frontier, delta) = db.standing_native_delta(&cx, handle, from, policy()).unwrap();
                assert_eq!(frontier, at);
                if tick > 0 || index == texts.len() - 1 { assert!(delta.is_empty()); }
                let retry = db.standing_native_delta(&cx, handle, from, policy()).unwrap();
                assert_eq!(retry, (at, copied(&delta)));
                bags[index].integrate(&delta, LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
                assert_eq!(&bags[index], &db.standing_native_bag(&cx, handle, policy()).unwrap().1);
                assert_eq!(bags[index], result_bag(db.query(&cx, texts[index], &params, resolve, policy()).unwrap()));
                assert!(matches!(db.standing_native_delta(&cx, handle, at, policy()),
                    Err(StandingQueryError::DeltaUnavailable { .. })));
            }
            from = at;
        }
...
fn missed_frontiers_rebuild_baselines_and_delivery_refusals_are_not_empty_ticks() {
    let ((), report) = run_async_under_lab(0x6dde_12, |root| async move {
        ...
        let (_, baseline) = db.standing_native_bag(&cx, &handle, GqlQueryPolicy::new(0, 3, 100_000, 100_000)).unwrap();
        assert_eq!(baseline.len(), 3); // four occurrences fit three support slots
        assert!(matches!(db.standing_native_bag(&cx, &handle, GqlQueryPolicy::new(0, 2, 100_000, 100_000)),
            Err(StandingQueryError::Delivery(StandingQueryFailure::ResultBudget))));
        let mut batch = WriteBatch::new(RelationId(1));
        batch.delete_vertex(VId(1));
        let second = db.write(&commit, batch).await.unwrap();
        let (_, delta) = db.standing_native_delta(&cx, &handle, first, policy()).unwrap();
        assert_eq!(delta.weight(&vec![scalar(3)]).unwrap().to_i128(), Some(-1));
        assert!(matches!(db.standing_native_delta(&cx, &handle, first, GqlQueryPolicy::new(0, 0, 100_000, 100_000)),
            Err(StandingQueryError::Delivery(StandingQueryFailure::ResultBudget))));
        assert_eq!(delta, db.standing_native_delta(&cx, &handle, first, policy()).unwrap().1);
        assert!(db.rebuild_standing_query(&cx, &handle, GqlQueryPolicy::new(0, 0, 0, 0)).is_err());
        assert_eq!(delta, db.standing_native_delta(&cx, &handle, first, policy()).unwrap().1);
        db.rebuild_standing_query(&cx, &handle, policy()).unwrap();
        assert!(matches!(db.standing_native_delta(&cx, &handle, first, policy()),
        ...
        assert!(matches!(db.standing_native_delta(&cx, &handle, second, policy()),
            Err(StandingQueryError::DeltaUnavailable { .. })));
        assert!(matches!(db.standing_native_delta(&cx, &handle, CommitSeq(u64::MAX), policy()),
            Err(StandingQueryError::DeltaUnavailable { .. })));
        assert!(db.standing_native_bag(&cx, &handle, policy()).is_ok());
        let foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(foreign.standing_native_delta(&cx, &handle, second, policy()),
...
fn unavailable_aggregate_does_not_export_an_old_derivative_after_durable_write() {
    let ((), report) = run_async_under_lab(0x6dde_13, |root| async move {
        ...
        let first = db.write(&commit, seed()).await.unwrap();
        let params = GqlParameters::new();
        let handle = db.register_standing_native(&cx, "MATCH (n) RETURN SUM(n.p) AS s", &params, resolve, policy()).unwrap();
        ...
        assert!(matches!(db.standing_native_bag(&cx, &handle, policy()),
            Err(StandingQueryError::Unavailable { .. })));
