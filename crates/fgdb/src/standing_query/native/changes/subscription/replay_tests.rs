...
fn property(id: u128, key: u64, value: i64) -> WriteBatch {
    let mut batch = WriteBatch::new(RelationId(1));
    batch.set_vertex_property(VId(id), PropertyKeyId(key), Some(CanonicalScalar::Int(value)));
    batch
}
...
fn independent_consumers_share_payloads_and_replay_every_delayed_tick() {
    let ((), report) = run_async_under_lab(0x6dde_71, |root| async move {
        ...
        let mut first = db.subscribe_native(
            &cx, "SUBSCRIBE TO MATCH (n) RETURN n.p AS p", &params, resolve, policy(),
        ).unwrap();
        assert_eq!(first.enable_replay(&mut db, &cx, 16, 1000, 100000, policy()).unwrap(), start);
        let count = db.standing_queries.len();
        let mut second = first.fork(&db, &cx).unwrap();
        assert_eq!(db.standing_queries.len(), count);
        assert_eq!(first.replay_handle().unwrap().index, second.replay_handle().unwrap().index);
        ...
        let mut cuts = vec![at];
        let mut expected = vec![db.standing_native_bag(&cx, first.handle(), policy()).unwrap().1];
        ...
        for change in [property(3, 1, 2), property(2, 99, 4)] {
            ...
            expected.push(db.standing_native_bag(&cx, first.handle(), policy()).unwrap().1);
        }
        // Pending delivery is pinned even though the source has advanced twice.
        assert!(Arc::ptr_eq(&pending, &first.poll(&db, &cx, GqlQueryPolicy::new(0, 0, 0, 0)).unwrap().unwrap()));
        let mut after = start;
        for (tick, at) in cuts.iter().enumerate() {
            ...
            let retained = db.standing_replay_next(&cx, first.replay_handle().unwrap(), after, delivery).unwrap().unwrap();
            assert!(Arc::ptr_eq(&a.rows, &retained.shared_rows()));
            assert!(matches!(first.acknowledge(b.receipt()), Err(SubscriptionError::InvalidReceipt)));
            for (bag, frame) in bags.iter_mut().zip([&a, &b]) {
                bag.integrate(frame.rows(), LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
                assert_eq!(&*bag, &expected[tick]);
            }
            if tick == 2 { assert!(a.rows().is_empty()); }
        ...
        assert!(matches!(first.poll(&db, &cx, policy()), Err(SubscriptionError::Closed)));
    });
...
fn evicted_pending_frame_replays_but_a_missing_successor_requires_rebaseline() {
    let ((), report) = run_async_under_lab(0x6dde_72, |root| async move {
        ...
        sub.enable_replay(&mut db, &cx, 2, 1000, 100000, policy()).unwrap();
        ...
        for value in 9..13 { current = db.write(&commit, property(2, 1, value)).await.unwrap(); }
        assert!(Arc::ptr_eq(&pending, &sub.poll(&db, &cx, policy()).unwrap().unwrap()));
        assert_eq!(sub.acknowledged_frontier(), Some(start));
        sub.acknowledge(pending.receipt()).unwrap();
        assert!(matches!(sub.poll(&db, &cx, policy()), Err(SubscriptionError::Query(
        ...
        assert!(matches!(sub.acknowledge(pending.receipt()), Err(SubscriptionError::InvalidReceipt)));
...
fn attachment_refusals_do_not_leak_sinks_or_fabricate_old_history() {
    let ((), report) = run_async_under_lab(0x6dde_73, |root| async move {
        ...
        let count = db.standing_queries.len();
        assert!(matches!(sub.enable_replay(&mut db, &cx, 0, 100, 1000, policy()),
            Err(SubscriptionError::Query(StandingQueryError::InvalidReplayLimits))));
        assert!(sub.replay_handle().is_none());
        assert_eq!(db.standing_queries.len(), count);
        let baseline = sub.poll(&db, &cx, policy()).unwrap().unwrap();
        assert!(matches!(sub.enable_replay(&mut db, &cx, 8, 100, 1000, policy()), Err(SubscriptionError::Unacknowledged)));
        assert_eq!(db.standing_queries.len(), count);
        assert!(Arc::ptr_eq(&baseline, &sub.poll(&db, &cx, policy()).unwrap().unwrap()));
        sub.acknowledge(baseline.receipt()).unwrap();
        let at = db.write(&commit, property(2, 1, 8)).await.unwrap();
        assert!(matches!(sub.enable_replay(&mut db, &cx, 8, 100, 1000, policy()),
        ...
        assert_eq!(sub.enable_replay(&mut db, &cx, 8, 100, 1000, policy()).unwrap(), at);
        assert_eq!(db.standing_queries.len(), count + 1);
        assert!(matches!(sub.enable_replay(&mut db, &cx, 8, 100, 1000, policy()), Err(SubscriptionError::ReplayAlreadyEnabled)));
        assert_eq!(db.standing_queries.len(), count + 1);
        let mut foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(sub.fork(&foreign, &cx), Err(SubscriptionError::Query(StandingQueryError::ForeignHandle))));
        assert!(matches!(sub.poll(&foreign, &cx, policy()), Err(SubscriptionError::Query(StandingQueryError::ForeignHandle))));
        let mut other = db.open_standing_subscription(&cx, sub.handle()).unwrap();
        assert!(matches!(other.enable_replay(&mut foreign, &cx, 8, 100, 1000, policy()),
...
fn delivery_refusal_and_shared_sink_rebuild_preserve_acknowledgements() {
    let ((), report) = run_async_under_lab(0x6dde_74, |root| async move {
        ...
        let params = GqlParameters::new();
        let mut sub = db.subscribe_native(
            &cx, "SUBSCRIBE TO MATCH (n) RETURN COUNT(*) AS c,SUM(n.p) AS s", &params, resolve, policy(),
        ).unwrap();
        sub.enable_replay(&mut db, &cx, 8, 100, 10000, policy()).unwrap();
        let baseline = sub.poll(&db, &cx, policy()).unwrap().unwrap();
        sub.acknowledge(baseline.receipt()).unwrap();
        let at = db.write(&commit, property(2, 1, 8)).await.unwrap();
        for delivery in [GqlQueryPolicy::new(0, 0, 1000, 1000), GqlQueryPolicy::new(0, 1000, 0, 1000), GqlQueryPolicy::new(0, 1000, 1000, 0)] {
            assert!(matches!(sub.poll(&db, &cx, delivery), Err(SubscriptionError::Query(StandingQueryError::Delivery(_)))));
            assert_eq!(sub.acknowledged_frontier(), Some(start));
            assert!(sub.pending.is_none());
        }
        let pending = sub.poll(&db, &cx, policy()).unwrap().unwrap();
        assert_eq!(pending.frontier(), at);
        db.rebuild_standing_query(&cx, sub.replay_handle().unwrap(), policy()).unwrap();
        assert!(Arc::ptr_eq(&pending, &sub.poll(&db, &cx, policy()).unwrap().unwrap()));
        sub.acknowledge(pending.receipt()).unwrap();
        ...
        db.rebuild_standing_query(&cx, sub.replay_handle().unwrap(), policy()).unwrap();
        assert!(matches!(sub.poll(&db, &cx, policy()), Err(SubscriptionError::Query(StandingQueryError::ReplayGap { .. }))));
...
fn replay_failure_does_not_rollback_writes_or_erase_an_already_delivered_frame() {
    let ((), report) = run_async_under_lab(0x6dde_75, |root| async move {
        ...
        sub.enable_replay(&mut db, &cx, 8, 0, 1000, policy()).unwrap();
        let baseline = sub.poll(&db, &cx, policy()).unwrap().unwrap();
        ...
        assert_eq!(sub.acknowledged_frontier(), Some(start));
        assert!(matches!(sub.poll(&db, &cx, policy()), Err(SubscriptionError::Query(
            StandingQueryError::Unavailable { reason: StandingQueryFailure::ResultBudget, .. }
        ))));
        assert!(sub.pending.is_none());
        db.rebuild_standing_query(&cx, sub.replay_handle().unwrap(), policy()).unwrap();
        sub.restart_from_current().unwrap();
