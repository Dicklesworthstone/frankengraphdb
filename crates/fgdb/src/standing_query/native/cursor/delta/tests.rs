...
fn make(rows: &ZSet<GraphValueRow>, policy: GqlQueryPolicy) -> Pull<'_> {
    Pull::new(
        view_runs(rows, None, NativeRow::Values),
        metadata(),
        CommitSeq(7),
        policy,
        StandingQueryStats {
...
fn drain(pull: &mut Pull<'_>) -> Result<Vec<(Vec<QueryValue>, ZWeight)>, StandingQueryFailure> {
    ...
    while let Some(change) = pull.advance(&mut || Ok(()), pull_change) {
        result.push(change?);
    }
    Ok(result)
}
...
fn all_small_signed_bags_deliver_one_exact_frame_per_changed_tuple() {
    for code in 0..625_usize {
        ...
        assert!(
            pull.advance(&mut || panic!("EOF called control"), pull_change)
                .is_none()
        );
        pull.close();
        assert_eq!(pull.state, VertexScanState::Exhausted);
...
fn huge_positive_and_negative_weights_remain_compressed_under_two_frame_allowance() {
    ...
    let negative = ZWeight::from_i128(i128::MIN)
        .checked_add(&ZWeight::from_i128(i128::MIN), LimbLimit::new(4))
        .unwrap();
    let delta = ZSet::from_updates(
        [(row(1), huge), (row(2), negative)],
        LimbLimit::new(4),
        &mut |_| Ok::<_, ()>(()),
    )
    .unwrap();
    ...
    assert_eq!(frames[0].1.to_i128(), None);
    assert_eq!(frames[1].1.to_i128(), None);
    assert_eq!(pull.delivered, 2);
    for ((_, got), (_, expected)) in frames.iter().zip(delta.iter()) {
        assert_eq!(got, expected);
    }
    let mut limited = make(&delta, GqlQueryPolicy::new(0, 1, 100, 100));
    assert!(
        limited
            .advance(&mut || Ok(()), pull_change)
            .unwrap()
            .is_ok()
    );
    assert_eq!(
        limited.advance(&mut || Ok(()), pull_change),
...
fn every_delta_checkpoint_and_panic_preserves_input_and_fuses_incomplete_delivery() {
    let delta = ZSet::from_updates(
        [
            (row(1), ZWeight::from_i128(-2)),
            (row(3), ZWeight::from_i128(2)),
        ],
        LimbLimit::new(4),
        &mut |_| Ok::<_, ()>(()),
    )
    ...
    let before = delta
        .checked_clone(LimbLimit::new(4), &mut |_| Ok::<_, ()>(()))
        .unwrap();
    let mut good = make(&delta, policy());
    let mut calls = 0;
    let mut expected = Vec::new();
    while let Some(change) = good.advance(
        &mut || {
            calls += 1;
            Ok(())
        },
    ...
    for stop in 1..=calls {
        let mut candidate = make(&delta, policy());
        let mut seen = 0;
        ...
        assert!(
            candidate
                .advance(&mut || panic!("failed cursor called control"), pull_change)
                .is_none()
        );
        assert_eq!(delta, before);
            ...
            std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                while candidate
                    .advance(
                        &mut || {
                            seen += 1;
                            assert_ne!(seen, stop, "injected delta unwind");
                            Ok(())
                        },
                        pull_change,
                    )
    ...
    for (work, scratch, cause) in [
        ...
        assert_eq!(
            drain(&mut make(&delta, GqlQueryPolicy::new(0, 2, work, scratch))),
            Err(cause)
        );
    }
}
...
fn gap_checks_are_exact_including_sequence_exhaustion_and_zero() {
    for frontier in [0, 1, 2, 19, u64::MAX] {
        for after in [0, 1, 18, 19, u64::MAX - 1, u64::MAX] {
            ...
            if after.checked_add(1) == Some(frontier) {
                assert!(got.is_ok());
            } else {
                assert!(
                    matches!(got, Err(StandingQueryError::DeltaGap { after: a, frontier: f })
...
fn zero_weights_missing_signed_support_and_partial_schema_never_escape() {
    ...
    for weight in [None, Some(&zero)] {
        ...
        assert_eq!(
            pull.advance(&mut || Ok(()), pull_change),
            Some(Err(StandingQueryFailure::InvalidDelta))
        );
        assert_eq!(pull.delivered, 0);
    }
    let r = GraphValueRow::from_owned_values(vec![]);
    let one = ZWeight::ONE;
    let mut pull = Pull::new(
        Box::new(std::iter::once(Run {
            row: NativeRow::Values(&r),
