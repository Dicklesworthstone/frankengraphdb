...
fn every_native_result_family_pulls_the_same_order_cells_and_frontier_as_collection() {
    let ((), report) = run_async_under_lab(0x5c11_0001, |root| async move {
        ...
        for text in [
            ...
            let handle = db
                .register_standing_native(&cx, text, &params, symbols, maintain())
                .unwrap();
            let (at, QueryResult::Rows { columns, rows }) =
            ...
            let mut cursor = db
                .standing_native_cursor(&cx, &temporary, policy())
                .unwrap();
            ...
            cursor.close();
            assert_eq!(cursor.state(), VertexScanState::Exhausted);
            assert_eq!(cursor.evaluator_stats(), usage);
            drop(cursor);
            db.rebuild_standing_query(&cx, &handle, maintain()).unwrap();
            assert_eq!(
                db.standing_native_cursor(&cx, &handle, policy())
                    .unwrap()
...
fn independent_delivery_quotas_and_early_close_do_not_fence_committed_maintenance() {
    let ((), report) = run_async_under_lab(0x5c11_0002, |root| async move {
        ...
        {
            let mut short = db
                .standing_native_cursor(&cx, &handle, GqlQueryPolicy::new(0, 1, 1000, 1000))
                .unwrap();
            ...
            assert_eq!(short.state(), VertexScanState::Failed);
            assert!(all.next().unwrap().is_ok());
            assert!(all.next().is_none());
            assert_eq!(short.snapshot_seq(), at);
            assert_eq!(all.snapshot_seq(), at);
            drop(short);
            drop(all);
            let mut closed = db.standing_native_cursor(&cx, &handle, policy()).unwrap();
            closed.close();
            let usage = closed.evaluator_stats();
            assert!(closed.next().is_none());
            assert_eq!(closed.evaluator_stats(), usage);
        ...
        assert_eq!(
            rows,
            vec![
                vec![QueryValue::Value(int(7))],
                vec![QueryValue::Value(int(7))],
                vec![QueryValue::Value(int(9))]
            ]
        );
        let stats = next.evaluator_stats();
        drop(next);
        let exact = GqlQueryPolicy::new(0, 3, stats.work_units, stats.scratch_entries);
        assert_eq!(
            db.standing_native_cursor(&cx, &handle, exact)
                .unwrap()
                .collect::<Result<Vec<_>, _>>()
                .unwrap(),
            rows
...
fn admission_refuses_foreign_unavailable_and_non_native_handles_without_row_delivery() {
    let ((), report) = run_async_under_lab(0x5c11_0003, |root| async move {
        ...
        let handle = db
            ...
            .unwrap();
        assert!(matches!(
            other.standing_native_cursor(&cx, &handle, policy()),
        ...
        assert!(matches!(
            db.standing_native_cursor(&cx, &raw, policy()),
            Err(StandingQueryError::Unsupported)
        ));
        for allowance in [
            GqlQueryPolicy::new(0, 100, 0, 100),
            GqlQueryPolicy::new(0, 100, 100, 0),
        ] {
            assert!(matches!(
                db.standing_native_cursor(&cx, &handle, allowance),
