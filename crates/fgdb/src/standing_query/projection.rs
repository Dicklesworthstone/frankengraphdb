...
fn projection_error(error: RowProjectionError<StandingQueryFailure>) -> StandingQueryFailure {
    match error {
        ...
        RowProjectionError::Expression { column, error } => StandingQueryFailure::OutputExpression { column, error },
        RowProjectionError::ResultBudget { .. } => StandingQueryFailure::ResultBudget,
        _ => StandingQueryFailure::InvalidDelta,
    }
}
impl State {
    pub(super) fn spec(&self) -> &RowProjectionSpec { self.operator.spec() }
    ...
    fn prepare_and_publish(&mut self, delta: &ZSet<GraphValueRow>, meter: &mut Meter<'_>)
        ...
        let pending = self.operator.prepare(delta, LIMBS, meter.policy.rows.max_result_rows(),
            &mut |event| meter.charge(event)).map_err(projection_error)?;
        (meter.checkpoint)()?;
...
impl<V: Vfs + Clone> Database<V> {
    ...
    pub fn register_standing_projection(&mut self, cx: &QueryCx, input: &StandingQueryHandle,
        projection: Vec<GraphSetProjection>, quantifier: GraphSetQuantifier, policy: GqlQueryPolicy,
        ...
        let state = self.prepare_standing_projection(cx, input.index, spec, policy, self.standing_queries.len())?;
    ...
    pub(super) fn prepare_standing_projection(&self, cx: &QueryCx, input: usize,
        ...
        cx.with_restriction(|| {
            ...
            let mut state = State { input, columns, operator: IncrementalRowProjection::new(spec),
    ...
    pub fn standing_projection<'a>(&'a self, cx: &QueryCx, handle: &StandingQueryHandle)
    ...
    pub fn standing_projection_delta<'a>(&'a self, cx: &QueryCx, handle: &StandingQueryHandle)
    ...
    pub fn standing_projection_columns<'a>(&'a self, cx: &QueryCx, handle: &StandingQueryHandle)
    ...
    pub fn standing_projection_total<'a>(&'a self, cx: &QueryCx, handle: &StandingQueryHandle)
