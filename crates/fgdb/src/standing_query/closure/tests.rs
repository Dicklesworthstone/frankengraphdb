...
fn bag(rows: impl IntoIterator<Item = (GraphValueRow, i128)>) -> ZSet<GraphValueRow> {
    ZSet::from_updates(
        rows.into_iter()
            .map(|(row, count)| (row, ZWeight::from_i128(count))),
        LIMBS,
        &mut |_| Ok::<_, ()>(()),
    )
    .unwrap()
...
fn oracle(rows: &ZSet<GraphValueRow>, endpoints: [usize; 2]) -> ZSet<GraphValueRow> {
    ...
    for (row, count) in rows.iter() {
        ...
        let (GraphValue::Vertex(a), GraphValue::Vertex(b)) =
            (&row.values()[endpoints[0]], &row.values()[endpoints[1]])
        else {
            continue;
        };
    ...
    for source in edges.keys() {
        ...
        while let Some(destination) = pending.pop() {
            ...
            if let Some(next) = edges.get(&destination) {
                pending.extend(next);
            }
        }
        output.extend(
            seen.into_iter()
                .map(|destination| (pair(source.0, destination.0), 1)),
        );
    }
    bag(output)
}
...
fn all_small_graph_transitions_match_complete_source_oracle() {
    for old in 0..64_u8 {
        for new in 0..64_u8 {
            ...
            delivered
                .integrate(state.delta().unwrap(), LIMBS, &mut |_| Ok::<_, ()>(()))
                .unwrap();
...
fn promoted_parallel_support_null_endpoints_and_malformed_rows() {
    ...
    let input = ZSet::from_updates(
        [(pair(1, 2), huge.checked_clone(LIMBS).unwrap())],
        LIMBS,
        &mut |_| Ok::<_, ()>(()),
    )
    .unwrap();
    ...
    let nearly_all = huge
        ...
        .unwrap();
    let delta =
        ZSet::from_updates([(pair(1, 2), nearly_all)], LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
    apply(&mut state, &delta);
    ...
    for invalid in [
        ...
        assert_eq!(
            state.prepare(&bag([(invalid, 1)]), &mut meter).map(drop),
    ...
    assert_eq!(
        state
            .prepare(&bag([(pair(3, 4), -1)]), &mut meter)
            .map(drop),
        Err(StandingQueryFailure::InvalidDelta)
    );
...
fn multiplicity_only_ticks_do_not_scan_unrelated_closure_rows() {
    ...
    for size in [8_u128, 128] {
        ...
        apply(
            &mut state,
            &bag((0..size).map(|n| (pair(2 * n, 2 * n + 1), 1))),
        );
...
fn filtered_parent_closure_nested_circuit_and_acknowledged_replay_share_commits() {
    let ((), report) = run_async_under_lab(0x6c05_01, |root| async move {
        ...
        let parent = db
            ...
            .unwrap();
        let closure = db
            .register_standing_closure(&cx, &parent, [0, 1], policy())
            .unwrap();
        let nested = db
            .register_standing_closure(&cx, &closure, [0, 1], policy())
            .unwrap();
...
fn failed_closure_never_rejects_commit_and_rebuild_resets_delta_baseline() {
    let ((), report) = run_async_under_lab(0x6c05_02, |root| async move {
        ...
        let tight = db
            ...
            .unwrap();
        let sibling = db
            .register_standing_closure(&cx, &source, [1, 0], policy())
            .unwrap();
        let count = db.standing_queries.len();
        assert!(matches!(
            db.register_standing_closure(&cx, &source, [0, 8], policy()),
        ...
        assert!(
            matches!(db.standing_closure(&cx, &tight), Err(StandingQueryError::Unavailable {
            frontier, reason: StandingQueryFailure::ResultBudget,
        }) if frontier == basis)
        );
        assert_eq!(db.standing_closure(&cx, &sibling).unwrap().rows().len(), 3);
        assert!(
            db.rebuild_standing_query(&cx, &tight, GqlQueryPolicy::new(0, 10, 10000, 10000))
                .is_err()
        );
        assert!(matches!(
            db.standing_closure(&cx, &tight),
        ...
        assert!(matches!(
            db.standing_native_delta(&cx, &tight, basis, policy()),
            Err(StandingQueryError::DeltaUnavailable { .. })
        ));
        let mut foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(
            foreign.register_standing_closure(&cx, &source, [0, 1], policy()),
...
fn empty_input_schema_is_not_inferred_from_values_and_same_slot_is_an_explicit_edge() {
    let ((), report) = run_async_under_lab(0x6c05_03, |root| async move {
        ...
        let count = db.standing_queries.len();
        assert!(matches!(
            db.register_standing_closure(&cx, &scalar, [0, 0], policy()),
            Err(StandingQueryError::Unsupported)
        ));
        assert_eq!(db.standing_queries.len(), count);
        let vertices = db
            ...
            .unwrap();
        let closure = db
            .register_standing_closure(&cx, &vertices, [0, 0], policy())
            .unwrap();
        assert!(
            db.standing_closure(&cx, &closure)
                .unwrap()
                .rows()
                .is_empty()
        );
        ...
        assert_eq!(
            db.standing_closure(&cx, &closure).unwrap().rows(),
            &bag([(pair(u128::MAX, u128::MAX), 1)])
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0x6c05_04, |root| async move {
        ...
        let source = db
            .register_standing_native(
                ...
                &GqlParameters::new(),
                symbols,
                GqlQueryPolicy::new(100, 1, 100000, 100000),
            )
            .unwrap();
        let child = db
            .register_standing_closure(&cx, &source, [0, 0], policy())
            .unwrap();
        let mut next = WriteBatch::new(RelationId(1));
        ...
        assert_eq!(
            db.standing_closure(&cx, &child).unwrap().rows(),
            &bag([(pair(1, 1), 1), (pair(2, 2), 1)])
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
