...
impl State {
    pub(super) fn spec(&self) -> &RowFilterSpec {
        self.operator.spec()
    }
...
impl<V: Vfs + Clone> Database<V> {
    ...
    pub fn register_standing_filter(
        &mut self,
        cx: &QueryCx,
        ...
        let state = self.prepare_standing_filter(
            cx,
            input.index,
            spec,
            policy,
    ...
    pub(super) fn prepare_standing_filter(
        &self,
        cx: &QueryCx,
        input: usize,
        spec: RowFilterSpec,
        ...
        cx.with_restriction(|| {
            ...
            if names.len() != spec.column_types().len() {
    ...
    pub fn standing_filter_delta<'a>(
        &'a self,
        cx: &QueryCx,
        handle: &StandingQueryHandle,
    ...
    pub fn standing_filter_columns<'a>(
        &'a self,
        cx: &QueryCx,
        handle: &StandingQueryHandle,
    ...
    pub fn standing_filter_total<'a>(
        &'a self,
        cx: &QueryCx,
        handle: &StandingQueryHandle,
