...
use fgdb_gql::{
    GqlParameters, GraphIntegerBinary, GraphIntegerExpression, GraphIntegerOp,
    GraphSetProjection, GraphSetQuantifier, GraphSetValue, PreparedGraphText,
};
...
fn relation() -> PreparedGraphSet {
    PreparedGraphSet::singleton()
        .unwind("x".into(), GraphSetValue::List([Some(3), Some(1), Some(3), None]
            .into_iter().map(|n| GraphSetValue::Value(value(n))).collect()))
        .unwrap()
}
...
fn sequence(state: &State) -> Vec<Vec<GraphValue>> {
    state.ordered.iter().map(|row| row.values().to_vec()).collect()
}

#[test]
fn source_free_pages_and_zero_column_multiplicities_are_exact() {
    let state = build(relation().with_page(1, Some(3)), policy(3)).unwrap();
    assert_eq!(sequence(&state), vec![vec![value(Some(1))], vec![value(Some(3))], vec![value(None)]]);
    ...
    for q in [GraphSetQuantifier::All, GraphSetQuantifier::Distinct] {
        let empty_columns = relation().project(vec![], q).unwrap();
        let state = build(empty_columns, policy(4)).unwrap();
        let count = if q == GraphSetQuantifier::All { 4 } else { 1 };
        assert_eq!(state.ordered.len(), count);
        assert_eq!(state.rows.len(), 1);
        assert_eq!(state.rows.iter().next().unwrap().1.to_i128(), Some(count as i128));
    }
}
...
fn constant_execution_and_retention_share_exact_budgets_and_checkpoints() {
    ...
    for (p, reason) in [
        (GqlQueryPolicy::new(0, 4, stats.work_units - 1, stats.scratch_entries), StandingQueryFailure::WorkBudget),
        (GqlQueryPolicy::new(0, 4, stats.work_units, stats.scratch_entries - 1), StandingQueryFailure::ScratchBudget),
        (GqlQueryPolicy::new(0, 3, stats.work_units, stats.scratch_entries), StandingQueryFailure::ResultBudget),
    ] {
...
fn hidden_expression_errors_and_graph_sources_never_become_constants() {
    ...
    let invalid = PreparedGraphSet::singleton().project(
        vec![GraphSetProjection::new("bad", GraphSetValue::Integer(divide))],
        GraphSetQuantifier::All,
    ).unwrap().with_page(0, Some(0));
    assert!(matches!(build(invalid, policy(0)), Err(StandingQueryFailure::InputExpression { column: 0, .. })));
    let graph: PreparedGraphSet = PreparedGraphText::prepare("MATCH (n) RETURN n", |_, _: &str| None)
        .unwrap().bind_parameters(&GqlParameters::new()).unwrap().into();
    for query in [graph.clone(), graph.with_page(0, Some(0))] {
        assert!(matches!(build(query, policy(0)), Err(StandingQueryFailure::InvalidDelta)));
    }
}
...
fn constant_registry_advances_without_evaluation_and_rebuild_is_atomic() {
    let ((), report) = run_async_under_lab(0x636f_6e01, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, crate::DatabaseKeys::new(
        ...
        assert_eq!(db.rebuild_standing_query(&cx, &handle, policy(4)).unwrap(), before);
        ...
        let mut other = Database::open_memory(&commit, crate::DatabaseKeys::new(
            [14; 32], DatabaseSecurityNamespaceId([15; 32]), [16; 32],
        )).await.unwrap();
        assert!(matches!(other.standing_rows(&cx, &handle), Err(StandingQueryError::ForeignHandle)));
        assert!(matches!(other.rebuild_standing_query(&cx, &handle, policy(4)), Err(StandingQueryError::ForeignHandle)));
    });
