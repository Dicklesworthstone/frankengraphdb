...
pub(super) fn contributions<'a>(
    ...
    let row = query.evaluate_incremental_input(source, &mut |event| {
        meter.charge(ZSetEvent::Work)?;
        match event {
        ...
        GqlQueryError::Source(GraphAggregateError::InputExpression { column, error, .. }) => {
    ...
    for (index, aggregate) in query.aggregates().iter().enumerate() {
        ...
        if support::uses_support(aggregate.function()) {
            ...
            if !value.is_null() {
                ...
                output.push((((Arc::clone(&key), index, Some(Arc::new(value.clone()))), Some(0)),
...
mod tests {
    ...
    use fgdb_gql::{GraphAggregate, GraphAggregateColumn, GraphAggregateFilter, GraphAggregateTest,
    ...
    fn definition() -> PreparedGraphAggregate {
        ...
        let product = GraphIntegerExpression::prepare(&[
            GraphIntegerOp::Column(2), GraphIntegerOp::Column(3),
        ...
        PreparedGraphAggregate::prepare_projected(input, vec![
            ...
            GraphAggregate::min("min", 0), GraphAggregate::count_distinct("distinct", 0),
            GraphAggregate::sum_int("sum", 0), GraphAggregate::average_int("average", 0),
            GraphAggregate::count_rows("count"),
        ], 0, None).unwrap().with_result_clauses(&[
            GraphAggregateFilter { column: GraphAggregateColumn::Aggregate(2),
                test: GraphAggregateTest::Integer { comparison: IntegerComparison::Greater, value: 0 } },
        ], &[]).unwrap()
    }

    fn advance(query: &mut StandingQuery, batch: &LogicalDeltaBatch,
    ...
    fn seeded(batch: &LogicalDeltaBatch) -> StandingQuery {
        ...
        let mut query = StandingQuery {
            definition, policy: GqlQueryPolicy::new(100_000, 100_000, 10_000_000, 10_000_000),
            vertices: BTreeMap::new(), edges: None, aggregate: IncrementalAggregate::new(),
            rows: ZSet::new(), frontier: CommitSeq::ORIGIN,
    ...
    fn every_computed_tick_checkpoint_and_budget_aborts_all_state_then_retries() {
        let ((), report) = run_async_under_lab(0x6a60, |root| async move {
            ...
            let mut calls = 0;
            let stats = advance(&mut success, &batch, &mut || { calls += 1; Ok(()) }).unwrap();
            ...
            for stop in 1..=calls {
                ...
                let mut seen = 0;
                assert_eq!(advance(&mut query, &batch, &mut || {
                    seen += 1;
                ...
                StandingQueryFailure::ResultBudget]
            {
                let mut query = seeded(&initial);
                match reason {
                    StandingQueryFailure::WorkBudget => query.policy.evaluator.max_work_units = stats.work_units - 1,
                    StandingQueryFailure::ScratchBudget => query.policy.evaluator.max_scratch_entries = stats.scratch_entries - 1,
