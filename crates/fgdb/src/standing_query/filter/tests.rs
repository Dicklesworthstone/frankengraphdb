...
use fgdb_gql::{
    GqlParameters, GraphSetColumnType, GraphSetOperand, GraphSymbol, GraphSymbolKind,
    PreparedGraphText,
};
    ...
    let ((), report) = run_async_under_lab(0x464c_0201, |root| async move {
        ...
        let build = || {
            ...
            let mut state = State {
                input: parent.index,
                columns: vec!["p".into()],
                operator: IncrementalRowFilter::new(spec),
                last_delta: None,
                policy: policy(),
                frontier: basis,
            ...
            let mut meter = Meter {
                policy: policy(),
                stats: StandingQueryStats::default(),
                checkpoint: &mut check,
            };
            state.prepare_and_publish(&baseline, &mut meter).unwrap();
        ...
        for (work, scratch, rows, expected) in [
            (stats.work_units, stats.scratch_entries, 1, None),
            (
                stats.work_units - 1,
            ...
            let mut check = || Ok(());
            let mut meter = Meter {
                policy: GqlQueryPolicy::new(100, rows, work, scratch),
                stats: StandingQueryStats::default(),
                checkpoint: &mut check,
            };
            ...
            if let Some(error) = expected {
                ...
                result.unwrap();
                assert_eq!(state.operator, success.operator);
                assert_eq!(state.last_delta, success.last_delta);
            }
        }
