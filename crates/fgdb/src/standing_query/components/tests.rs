...
fn unchanged(actual: &State, before: &State) {
    assert_eq!(actual.input, before.input); assert_eq!(actual.components, before.components);
    assert_eq!(actual.rows, before.rows); assert_eq!(actual.frontier, before.frontier);
    assert_eq!(actual.stats, before.stats); assert_eq!(actual.policy, before.policy);
...
fn every_composed_refusal_and_exact_budget_preserves_atomic_publication() {
    let ((), report) = run_async_under_lab(0x6363_6410, |root| async move {
        ...
        for v in 0..5 { seed.create_vertex(VId(v), vec![], vec![]); }
        for (eid, a, b) in [(1, 0, 1), (2, 1, 2), (3, 3, 4)] {
            seed.add_edge(EId(eid), VId(a), VId(b), vec![]);
        }
        db.write(&commit, seed).await.unwrap();
        let baseline = Arc::clone(&db.snapshot); let before = build(&baseline);
        let mut change = WriteBatch::new(RelationId(1));
        change.delete_vertex(VId(0)); change.create_vertex(VId(6), vec![], vec![]);
        ...
        let mut success = build(&baseline); let mut calls = 0;
        let stats = {
            let mut checkpoint = || { calls += 1; Ok(()) };
            let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            success.maintain(&commit, batch, &mut meter).unwrap(); meter.stats
        };
        assert_eq!(success.rows, expected.rows); assert_eq!(success.components, expected.components);
        assert_eq!(success.input, expected.input);
        assert!(calls > 0 && stats.work_units > 0 && stats.scratch_entries > 0);
        for stop in 1..=calls {
            let mut state = build(&baseline); let mut seen = 0;
            {
        ...
        for (work, scratch, rows, refusal) in [
            ...
            let mut state = build(&baseline); let mut checkpoint = || Ok(());
            let mut meter = Meter { policy: GqlQueryPolicy::new(100_000, rows, work, scratch),
                stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            let result = state.maintain(&commit, batch, &mut meter);
            if let Some(reason) = refusal { assert_eq!(result, Err(reason)); unchanged(&state, &before); }
            else { result.unwrap(); assert_eq!(state.rows, expected.rows); assert_eq!(state.components, expected.components); }
        }
        ...
        for records in [physical as u64, physical as u64 - 1] {
            let mut checkpoint = || Ok(());
            let mut meter = Meter { policy: GqlQueryPolicy::new(records, 5, 10_000_000, 10_000_000),
                stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            let result = State::from_snapshot(&db.snapshot, RelationId(1), &mut meter);
            if records == physical as u64 { assert_eq!(result.unwrap().rows, expected.rows); }
            else { assert!(matches!(result, Err(StandingQueryFailure::SnapshotBudget))); }
        }
    });
