...
fn batch(support: usize) -> ZSet<Vec<QueryValue>> {
    ZSet::from_updates(
        (0..support).map(|key| (vec![QueryValue::Integer(key as i128)], ZWeight::from_i128(-1))),
        LIMBS, &mut |_| Ok::<_, ()>(()),
    ).unwrap()
}
fn append(history: &mut History, support: usize, limits: Limits) -> Result<(), StandingQueryFailure> {
    ...
    history.prepare(at, batch(support), limits, &mut meter)?.commit();
    Ok(())
}
...
fn unchanged(history: &History, window: StandingReplayWindow, frames: &[Arc<StandingReplayBatch>]) {
    ...
    for (a, b) in history.frames.iter().zip(frames) { assert!(Arc::ptr_eq(a, b)); }
}
                    ...
                    for _ in 0..4 {
                        ...
                        if n > rows || cost > units {
                            ...
                            for (_, nr, nu) in oracle.iter().rev() {
                                if keep == ticks || total_rows + nr > rows || total_units + nu > units { break; }
                                total_rows += nr;
                                total_units += nu;
                                keep += 1;
                            }
                        ...
                        assert_eq!(history.rows, oracle.iter().map(|entry| entry.1).sum::<usize>());
                        assert_eq!(history.units, oracle.iter().map(|entry| entry.2).sum::<usize>());
                        let oldest = oracle.first().map_or(CommitSeq(7), |entry| CommitSeq(entry.0.0 - 1));
                        ...
                        assert!(matches!(history.next(CommitSeq(oldest.0 - 1)), Err(StandingQueryError::ReplayGap { .. })));
                        assert!(matches!(history.next(frontier.checked_successor().unwrap()), Err(StandingQueryError::ReplayGap { .. })));
                    }
                }
            }
        }
...
fn every_preparation_checkpoint_and_unwind_preserves_retained_history() {
    ...
    for _ in 0..3 { append(&mut history, 1, limits).unwrap(); }
    let before = history.window();
    let frames = references(&history);
    let mut calls = 0;
    let mut checkpoint = || { calls += 1; Ok(()) };
    let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
    drop(history.prepare(CommitSeq(4), batch(2), limits, &mut meter).unwrap());
    ...
    for stop in 1..=calls {
        ...
        let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
        let result = history.prepare(CommitSeq(4), batch(2), limits, &mut meter).map(drop);
    ...
    for stop in 1..=calls {
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            ...
            let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            history.prepare(CommitSeq(4), batch(2), limits, &mut meter).map(drop)
        }));
        assert!(result.is_err());
        unchanged(&history, before, &frames);
    }
...
fn exact_weights_empty_ticks_limits_and_sequence_exhaustion() {
    ...
    for _ in 0..20 { append(&mut empty, 0, Limits::new(2, 0, 2).unwrap()).unwrap(); }
    assert_eq!(empty.window(), StandingReplayWindow {
        retained_after: CommitSeq(18), frontier: CommitSeq(20), ticks: 2, rows: 0, payload_units: 2,
    });
    let huge = ZWeight::from_i128(i128::MAX).checked_add(&ZWeight::ONE, LIMBS).unwrap();
    let negative = huge.checked_add(&ZWeight::ONE, LIMBS).unwrap().checked_neg(LIMBS).unwrap();
    ...
    let mut checkpoint = || Ok(());
    let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
    history.prepare(CommitSeq(101), delta, Limits::new(1, 2, 100).unwrap(), &mut meter).unwrap().commit();
    let frame = history.next(CommitSeq(100)).unwrap().unwrap();
    assert_eq!(frame.rows().len(), 2);
    assert!(frame.rows().iter().all(|(_, weight)| weight.to_i128().is_none()));
    ...
    let result = terminal.prepare(CommitSeq::ORIGIN, batch(0), Limits::new(1, 0, 1).unwrap(), &mut meter).map(drop);
...
fn preparation_work_does_not_scan_unaffected_retained_prefix() {
    ...
    for count in [8, 128] {
        let mut history = History::new(CommitSeq::ORIGIN);
        for _ in 0..count { append(&mut history, 1, limits).unwrap(); }
        let mut checkpoint = || Ok(());
        let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
        drop(history.prepare(CommitSeq(count + 1), batch(1), limits, &mut meter).unwrap());
...
fn result_bag(result: QueryResult) -> ZSet<Vec<QueryValue>> {
    let QueryResult::Rows { rows, .. } = result else { panic!("query rows required"); };
    ZSet::from_updates(rows.into_iter().map(|row| (row, ZWeight::ONE)), LIMBS, &mut |_| Ok::<_, ()>(())).unwrap()
}
...
fn commit_hook_retains_every_final_output_tick_without_polling_or_graph_replay() {
    let ((), report) = run_async_under_lab(0x6dde_61, |root| async move {
        ...
        for text in texts {
            let view = db.register_standing_native(&cx, text, &params, resolve, policy()).unwrap();
            ...
            logs.push(db.register_standing_replay(&cx, &view, 16, 1000, 100000, replay_policy).unwrap());
            views.push(view);
        }
        ...
        for tick in 0..3 {
            let mut batch = WriteBatch::new(RelationId(1));
            if tick == 0 { batch.delete_vertex(VId(1)); }
            else if tick == 1 { batch.set_vertex_property(VId(3), PropertyKeyId(1), Some(CanonicalScalar::Int(2))); }
            else { batch.set_vertex_property(VId(2), PropertyKeyId(99), Some(CanonicalScalar::Int(1))); }
            cuts.push(db.write(&commit, batch).await.unwrap());
            expected.push(texts.iter().map(|text| {
        ...
        for (index, log) in logs.iter().enumerate() {
            ...
            for (tick, at) in cuts.iter().enumerate() {
                let frame = db.standing_replay_next(&cx, log, after, policy()).unwrap().unwrap();
                assert_eq!(frame.from(), after);
                assert_eq!(frame.frontier(), *at);
                assert!(Arc::ptr_eq(&frame, &db.standing_replay_next(&cx, log, after, policy()).unwrap().unwrap()));
                if tick == 2 || index == texts.len() - 1 { assert!(frame.rows().is_empty()); }
                bags[index].integrate(frame.rows(), LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
                assert_eq!(bags[index], expected[tick][index]);
                after = *at;
            }
            assert!(db.standing_replay_next(&cx, log, after, policy()).unwrap().is_none());
            ...
            assert_eq!(bags[index], db.standing_native_bag(&cx, &views[index], policy()).unwrap().1);
        }
    });
...
fn sink_overflow_isolated_from_durable_commit_and_rebuild_cannot_fabricate_history() {
    let ((), report) = run_async_under_lab(0x6dde_62, |root| async move {
        ...
        let start = db.write(&commit, seed()).await.unwrap();
        let params = GqlParameters::new();
        let view = db.register_standing_native(&cx, "MATCH (n) RETURN n.p AS p", &params, resolve, policy()).unwrap();
        let bounded = db.register_standing_replay(&cx, &view, 2, 0, 10, policy()).unwrap();
        let healthy = db.register_standing_replay(&cx, &view, 2, 100, 10000, policy()).unwrap();
        let count = db.standing_queries.len();
        assert!(matches!(db.register_standing_replay(&cx, &view, 0, 1, 1, policy()), Err(StandingQueryError::InvalidReplayLimits)));
        ...
        assert!(matches!(db.standing_replay_next(&cx, &bounded, start, policy()),
            Err(StandingQueryError::Unavailable { frontier, reason: StandingQueryFailure::ResultBudget }) if frontier == start));
        assert!(db.standing_replay_next(&cx, &healthy, start, policy()).unwrap().is_some());
        assert!(matches!(db.standing_replay_next(&cx, &healthy, start, GqlQueryPolicy::new(0, 0, 100, 100)),
        ...
        assert!(db.standing_replay_next(&cx, &bounded, at, policy()).unwrap().is_none());
        let foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(foreign.standing_replay_next(&cx, &healthy, start, policy()), Err(StandingQueryError::ForeignHandle)));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
