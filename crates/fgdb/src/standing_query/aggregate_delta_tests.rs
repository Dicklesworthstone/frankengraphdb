    ...
    let ((), report) = run_async_under_lab(0x6dde_01, |root| async move {
        ...
        for tick in 0..3 {
            ...
            for (index, handle) in handles.iter().enumerate() {
                ...
                if tick != 0 { assert!(view.rows().is_empty()); }
                bags[index].integrate(view.rows(), LimbLimit::new(4), &mut |_| Ok::<_, ()>(())).unwrap();
                assert_eq!(&bags[index], db.standing_query(&cx, handle).unwrap().rows());
                assert_eq!(view.rows(), db.standing_query_delta(&cx, handle).unwrap().unwrap().rows());
            }
        }
        for handle in &handles {
            let before = clone_bag(db.standing_query_delta(&cx, handle).unwrap().unwrap().rows());
            assert!(db.rebuild_standing_query(&cx, handle, GqlQueryPolicy::new(100_000, 100_000, 0, 0)).is_err());
            assert_eq!(&before, db.standing_query_delta(&cx, handle).unwrap().unwrap().rows());
    ...
    let ((), report) = run_async_under_lab(0x6dde_02, |root| async move {
        ...
        for kind in [1, 2, 3] {
            let make = || {
                ...
                let source = basis.prepare_standing_query_with_output(&cx, producer, policy(), Some(&mut output)).unwrap();
                (source, output)
            };
            ...
            let mut calls = 0;
            {
                ...
                success.maintain_with_output(&delta, &mut meter, Some(&mut result)).unwrap();
            }
            let mut integrated = clone_bag(&before_output.rows);
            integrated.integrate(success.last_delta.as_ref().unwrap(), LimbLimit::new(4), &mut |_| Ok::<_, ()>(())).unwrap();
            ...
            for stop in 1..=calls {
                ...
                let mut seen = 0;
                {
                    ...
                    let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                    assert_eq!(candidate.maintain_with_output(&delta, &mut meter, Some(&mut output)), Err(StandingQueryFailure::Interrupted));
                }
                assert_eq!(seen, stop);
...
fn aggregate_baseline_empty_ticks_and_failed_views_are_distinct() {
    let ((), report) = run_async_under_lab(0x6dde_03, |root| async move {
        ...
        let commit = contexts.commit();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let handle = db.register_standing_query(&cx, definition(0), policy()).unwrap();
        assert!(db.standing_query_delta(&cx, &handle).unwrap().is_none());
        db.write(&commit, seed()).await.unwrap();
        assert!(db.standing_query_delta(&cx, &handle).unwrap().unwrap().rows().iter().all(|(_, w)| w == &ZWeight::ONE));
        ...
        assert!(matches!(db.standing_query_delta(&cx, &handle), Err(StandingQueryError::Unavailable { .. })));
        let foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(foreign.standing_query_delta(&cx, &handle), Err(StandingQueryError::ForeignHandle)));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
