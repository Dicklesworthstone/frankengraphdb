...
fn every_composed_refusal_preserves_accepted_state_and_baselines_never_become_empty_ticks() {
    let ((), report) = run_async_under_lab(0x726a_0201, |root| async move {
        ...
        let expected = build(
            &copy(db.standing_rows(&cx, &left).unwrap().rows()),
            &copy(db.standing_rows(&cx, &right).unwrap().rows()),
            at,
        );
        ...
        for stop in 1..=calls {
            let mut state = build(&old_left, &old_right, basis);
            let mut visited = 0;
            {
                let mut checkpoint = || {
                    visited += 1;
        ...
        for (work, scratch, rows, error) in [
            ...
            let mut checkpoint = || Ok(());
            let mut meter = Meter {
                policy: GqlQueryPolicy::new(1000, rows, work, scratch),
                stats: StandingQueryStats::default(),
                checkpoint: &mut checkpoint,
            };
        ...
        assert!(
            db.prepare_standing_join(
                ...
                &[(0, 0)],
                RowJoinKind::Inner,
                GqlQueryPolicy::new(3, 1000, 1_000_000, 1_000_000),
                2
            )
            .is_ok()
        );
...
fn outer_and_presence_registry_publication_is_atomic_at_every_checkpoint() {
    ...
    let bag = |key: i64, payload: i64, count: i128| {
        ZSet::from_updates(
            [(
