...
use fgdb_gql::{
    GqlParameters, GraphSetColumnType, GraphSetOperand, GraphSetPredicateOp, GraphSymbol,
...
fn sources<V: Vfs + Clone>(
    ...
    let mut register = |text| {
        let definition = PreparedGraphText::prepare(text, symbols)
            .unwrap()
...
fn put(out: &mut Bag, values: Vec<GraphValue>, count: i128) {
    *out.entry(GraphValueRow::from_owned_values(values))
        .or_default() += count;
}
...
fn oracle(left: &Bag, right: &Bag, keyed: bool, kind: RowJoinKind) -> Bag {
    let matches = |l: &GraphValueRow, r: &GraphValueRow| {
        ...
        let r = r.values();
        (!keyed
            || (!matches!(l[0], GraphValue::Scalar(CanonicalScalar::Null))
                && !matches!(r[0], GraphValue::Scalar(CanonicalScalar::Null))
                && l[0] == r[0]))
            && matches!(
                (&l[1], &r[1]),
    ...
    let ((), report) = run_async_under_lab(0x6f6e_0401, |root| async move {
        ...
        for keyed in [true, false] {
            for kind in KINDS {
                ...
                let handle = db
                    .register_standing_join_spec(&cx, &left, &right, &definition, policy())
                    .unwrap();
                let expected = oracle(
                    &plain(db.standing_rows(&cx, &left).unwrap().rows()),
                    &plain(db.standing_rows(&cx, &right).unwrap().rows()),
                    keyed,
                    kind,
                );
                assert_eq!(
            ...
            match tick {
                0 => {
                    change.set_vertex_property(
                        VId(1),
                        PropertyKeyId(2),
                        Some(CanonicalScalar::Int(35)),
                    );
                    change.set_vertex_property(
                        VId(4),
                        PropertyKeyId(2),
                        Some(CanonicalScalar::Int(40)),
                    );
                }
                1 => {
                    change.set_vertex_property(VId(5), PropertyKeyId(2), None);
                }
                2 => {
                ...
                3 => {
                    vertex(&mut change, 7, 1, 2, 5);
                    vertex(&mut change, 8, 2, 2, 15);
                }
                _ => {
                    change.set_vertex_property(VId(4), PropertyKeyId(1), None);
                }
            }
...
fn quota_failure_fences_dependents_but_rebuild_preserves_on_not_just_equality_keys() {
    let ((), report) = run_async_under_lab(0x6f6e_0402, |root| async move {
        ...
        let limited = db
            .register_standing_join_spec(
                &cx,
                &left,
                &right,
                &definition,
                GqlQueryPolicy::new(1000, 2, 1_000_000, 1_000_000),
            )
            .unwrap();
        let sibling = db
            .register_standing_join_spec(&cx, &left, &right, &definition, policy())
            .unwrap();
        let child = db
            .register_standing_cross_join(&cx, &limited, &right, policy())
            .unwrap();
        ...
        assert!(matches!(
            db.standing_join(&cx, &child),
            Err(StandingQueryError::Unavailable {
                reason: StandingQueryFailure::DependencyUnavailable,
                ..
            })
        ));
        let StandingQuery::Join(retained) = &db.standing_queries[limited.index] else {
            panic!()
        };
        ...
        assert_eq!(
            db.standing_join_total(&cx, &limited).unwrap(),
            &ZWeight::from_i128(3)
        );
        assert_eq!(
            db.standing_join_total(&cx, &child).unwrap(),
            &ZWeight::from_i128(6)
        );
        assert_eq!(
...
fn complete_schema_and_owner_checks_precede_empty_results_and_never_append_failed_views() {
    let ((), report) = run_async_under_lab(0x6f6e_0403, |root| async move {
        ...
        for side in 0..2 {
            ...
            let definition = RowJoinSpec::cross(&types[0], &types[1])
                .unwrap()
        ...
        for denied in [
            ...
            GqlQueryPolicy::new(1000, 1000, 1000, 0),
        ] {
            assert!(
                db.register_standing_join_spec(&cx, &left, &right, &definition, denied)
                    .is_err()
            );
            assert_eq!(db.standing_queries.len(), before);
        }
