...
fn definition(label: &str) -> PreparedGraphPattern<GraphValueRow> {
    PreparedGraphText::prepare(&format!("MATCH (n:{label}) RETURN n.p AS p"),
...
fn copy_accepted(source: &State) -> State {
    let mut copy = State {
        inputs: source.inputs, columns: source.columns.clone(), types: source.types.clone(),
        input: IncrementalSet::new(source.operation()), rows: ZSet::new(), total: ZWeight::ZERO,
        last_delta: None, policy: source.policy, frontier: source.frontier,
        stats: source.stats, failure: source.failure,
    };
    let mut checkpoint = || Ok(());
    let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
    copy.prepare(source.input.left_counts(), source.input.right_counts(), &mut meter).unwrap().commit();
    copy.last_delta = source.last_delta.as_ref().map(|d|
        d.checked_clone(LIMBS, &mut |_| Ok::<_, ()>(())).unwrap());
    copy
}
...
fn every_composition_refusal_preserves_both_inputs_result_total_and_last_delta() {
    let ((), report) = run_async_under_lab(0x7365_7410, |root| async move {
        ...
        for (id, label, value) in [(1, 1, 1), (2, 1, 2), (3, 2, 2)] {
            seed.create_vertex(VId(id), vec![LabelId(label)], vec![(PropertyKeyId(1), CanonicalScalar::Int(value))]);
        }
        ...
        let before = db.prepare_standing_set(&cx, deps, SetOperation::UnionAll, policy(), 2).unwrap();
        ...
        let expected = db.prepare_standing_set(&cx, deps, SetOperation::UnionAll, policy(), 2).unwrap();
        let mut success = copy_accepted(&before);
        let mut calls = 0;
        let stats = {
            let mut checkpoint = || { calls += 1; Ok(()) };
            let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            success.maintain(&batch, &db.standing_queries, &mut meter).unwrap();
            meter.stats
        };
        assert!(calls > 0 && stats.work_units > 0 && stats.scratch_entries > 0);
        assert_eq!(success.input, expected.input); assert_eq!(success.rows, expected.rows);
        assert_eq!(success.total, expected.total); assert_eq!(success.total.to_i128(), Some(3));
        for stop in 1..=calls {
            ...
            let mut seen = 0;
            {
                ...
                let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                assert_eq!(state.maintain(&batch, &db.standing_queries, &mut meter),
                    Err(StandingQueryFailure::Interrupted), "checkpoint {stop}");
            }
            assert_eq!(seen, stop); unchanged(&state, &before);
        }
        for (work, scratch, rows, error) in [
            ...
            let mut state = copy_accepted(&before); let mut checkpoint = || Ok(());
            let mut meter = Meter { policy: GqlQueryPolicy::new(0, rows, work, scratch),
            ...
            else { result.unwrap(); assert_eq!(state.rows, expected.rows); assert_eq!(state.input, expected.input); }
        }
        let mut state = copy_accepted(&before); let mut checkpoint = || Ok(());
        let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
        ...
        state.maintain(&batch, &db.standing_queries, &mut meter).unwrap();
        ...
        assert_eq!(stale.maintain(&batch, &db.standing_queries, &mut meter),
...
fn snapshot_support_limits_are_checked_before_baseline_installation() {
    let ((), report) = run_async_under_lab(0x7365_7411, |root| async move {
        ...
        let mut seed = WriteBatch::new(RelationId(1));
        for id in 1..=8 {
            seed.create_vertex(VId(id), vec![LabelId(1)], vec![(PropertyKeyId(1), CanonicalScalar::Int(4))]);
        }
        db.write(&commit, seed).await.unwrap();
        let parent = db.register_standing_rows(&cx, definition("L"), policy()).unwrap();
        let before = db.standing_queries.len();
        assert!(matches!(db.register_standing_set(&cx, &parent, &parent, SetOperation::UnionAll,
        ...
        let h = db.register_standing_set(&cx, &parent, &parent, SetOperation::UnionAll,
            GqlQueryPolicy::new(2, 16, 100_000, 100_000)).unwrap();
        assert_eq!(db.standing_set(&cx, &h).unwrap().rows().len(), 1);
        assert_eq!(db.standing_set_total(&cx, &h).unwrap().to_i128(), Some(16));
        assert!(matches!(db.rebuild_standing_query(&cx, &h,
            GqlQueryPolicy::new(2, 15, 100_000, 100_000)),
