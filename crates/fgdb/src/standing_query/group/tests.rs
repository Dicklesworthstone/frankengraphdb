...
fn definition(mode: usize) -> PreparedGraphSetAggregate {
    ...
    let mut result = PreparedGraphSetAggregate::prepare(input, &[0], &specs,
        u64::from(mode == 2), (mode == 2).then_some(1)).unwrap();
    if mode == 1 {
        result = result.with_key_output_columns(&[]).unwrap()
            .with_aggregate_output_prefix(1).unwrap().with_distinct_output(true);
    }
    if mode == 2 {
        result = result.with_result_clauses(&[], &[
...
fn empty(definition: PreparedGraphSetAggregate) -> State {
    let operator = IncrementalGroupAggregate::new(definition.complete_groups().unwrap(),
        definition.input().column_types()).unwrap();
    State { input: 0, operator, output: output::State::new(definition), last_delta: None,
        policy: policy(), frontier: CommitSeq(0), stats: StandingQueryStats::default(), failure: None }
}
...
fn z(values: impl IntoIterator<Item = (GraphValueRow, i128)>) -> ZSet<GraphValueRow> {
    ZSet::from_updates(values.into_iter().map(|(row, n)| (row, ZWeight::from_i128(n))),
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap()
}
fn apply(state: &mut State, rows: &ZSet<GraphValueRow>) -> StandingQueryStats {
    let mut checkpoint = || Ok(());
    let mut meter = Meter { policy: state.policy, stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
...
fn every_small_bag_transition_integrates_the_final_group_projection_and_rank_delta() {
    let bag = |n: usize| z([(1, Some(2)), (1, None), (2, Some(7))].into_iter().enumerate()
        .map(|(i, (key, value))| (row(key, value), ((n / 3usize.pow(i as u32)) % 3) as i128)));
    for mode in 0..3 { for before in 0..27 { for after in 0..27 {
        let old = bag(before); let new = bag(after);
        let mut candidate = empty(definition(mode)); apply(&mut candidate, &old);
        let mut expected = empty(definition(mode)); apply(&mut expected, &new);
        let changes = new.minus(&old, LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
        let mut integrated = copy(candidate.rows());
        apply(&mut candidate, &changes);
        integrated.integrate(candidate.last_delta.as_ref().unwrap(), LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
        assert_eq!(&integrated, expected.rows(), "mode {mode}: {before}->{after}");
        ...
        apply(&mut candidate, &changes.negated(LIMBS, &mut |_| Ok::<_, ()>(())).unwrap());
        assert_eq!(candidate.last_delta.as_ref().unwrap(),
...
fn all_group_output_and_final_checkpoint_refusals_leave_the_whole_state_retryable() {
    ...
    let change = z([(row(1, Some(2)), -1), (row(2, Some(7)), -1), (row(3, Some(4)), 2)]);
    for mode in 0..3 {
        let make = || { let mut s = empty(definition(mode)); apply(&mut s, &initial); s };
        let before = make(); let mut success = make(); let mut calls = 0;
        let stats = {
            ...
            success.apply(&change, &mut meter).unwrap(); meter.stats
        };
        for stop in 1..=calls {
            let mut s = make(); let mut seen = 0;
            {
                let mut checkpoint = || { seen += 1;
                    if seen == stop { Err(StandingQueryFailure::Interrupted) } else { Ok(()) } };
                let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                assert_eq!(s.apply(&change, &mut meter), Err(StandingQueryFailure::Interrupted));
            }
            assert_eq!(seen, stop); same(&s, &before);
            apply(&mut s, &change); same(&s, &success);
        }
        for (work, scratch) in [(stats.work_units - 1, stats.scratch_entries),
            (stats.work_units, stats.scratch_entries - 1)] {
            let mut s = make(); let mut checkpoint = || Ok(());
            let mut meter = Meter { policy: GqlQueryPolicy::new(100_000, 100_000, work, scratch),
                stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            assert!(s.apply(&change, &mut meter).is_err()); same(&s, &before);
        }
        let mut s = make();
        let panic = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let mut checkpoint = || Ok(());
            let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            let groups = s.operator.prepare(&change, LIMBS, None, &mut |event| meter.charge(event)).unwrap();
            let _output = s.output.prepare(groups.delta(), &mut meter).unwrap();
            panic!("downstream refusal after both operators prepared");
        }));
        assert!(panic.is_err()); same(&s, &before);
...
fn hidden_output_does_not_mask_native_count_overflow_or_noninteger_data() {
    ...
    let definition = PreparedGraphSetAggregate::prepare(definition.input().clone(), &[0],
        &[GraphAggregate::count_rows("count"), GraphAggregate::sum_int("sum", 1)], 0, Some(0)).unwrap();
    let mut s = empty(definition.clone());
    let mut before = empty(definition);
    let initial = z([(row(1, Some(1)), i128::from(u64::MAX))]);
    apply(&mut s, &initial); apply(&mut before, &initial);
    assert!(s.rows().is_empty());
    for (change, expected) in [
        (z([(row(1, Some(1)), 1)]), StandingQueryFailure::Arithmetic),
        (z([(GraphValueRow::from_owned_values(vec![GraphValue::Scalar(CanonicalScalar::Int(2)),
            GraphValue::Scalar(CanonicalScalar::Bool(true))]), 1)]),
            StandingQueryFailure::NonIntegerAggregate { column: 1 }),
        (z([(row(9, Some(1)), -1)]), StandingQueryFailure::InvalidDelta),
    ] {
        let mut checkpoint = || Ok(());
        let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
        assert_eq!(s.apply(&change, &mut meter), Err(expected)); same(&s, &before);
    }
}
