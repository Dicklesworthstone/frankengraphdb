...
use fgdb_gql::{
    GqlParameters, GraphSetColumnType, GraphSetValue, GraphSymbol, GraphSymbolKind,
    PreparedGraphText,
};
...
fn every_composed_checkpoint_and_exact_quota_boundary_preserves_the_accepted_generation() {
    let ((), report) = run_async_under_lab(0x7072_6410, |root| async move {
        let c = fgdb_types::PurposeContexts::narrow_runtime_root(&root);
        let cx = c.query();
        let commit = c.commit();
        ...
        for (work, scratch, expected) in [
            ...
            let mut check = || Ok(());
            let mut meter = Meter {
                policy: GqlQueryPolicy::new(100, 1, work, scratch),
                stats: StandingQueryStats::default(),
                checkpoint: &mut check,
            };
