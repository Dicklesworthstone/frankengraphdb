...
fn retire(db: &mut Database<MemVfs>) {
    ...
    std::sync::Arc::make_mut(&mut db.snapshot).delta_index.retire_prefix(at).unwrap();
...
fn oracle(db: &Database<MemVfs>, relation: RelationId) -> BTreeSet<Pair> {
    ...
    let mut pairs: BTreeSet<_> = db.edges().unwrap().iter()
    ...
    for middle in &vertices {
        for source in &vertices {
            for destination in &vertices {
                if pairs.contains(&(source.vid, middle.vid)) && pairs.contains(&(middle.vid, destination.vid)) {
...
fn plain(rows: &ZSet<Pair>) -> BTreeSet<Pair> {
    rows.iter().map(|(pair, weight)| { assert_eq!(weight, &ZWeight::ONE); *pair }).collect()
}
...
fn fully_retired_registration_rebuild_and_live_cascades_use_one_snapshot_cut() {
    let ((), report) = run_async_under_lab(0x6a81, |root| async move {
        ...
        let maintained = db.register_standing_reachability(&query, R, limited).unwrap();
        ...
        assert!(matches!(db.rebuild_standing_query(&query, &maintained, limited),
        ...
        let late_r = db.register_standing_reachability(&query, R, policy()).unwrap();
        let late_s = db.register_standing_reachability(&query, S, policy()).unwrap();
        check(&db, &query, &late_r, R);
        check(&db, &query, &late_s, S);
        assert_eq!(db.standing_reachability(&query, &late_r).unwrap().last_maintenance().delta_rows, 0);
        let mut remove_cycle = WriteBatch::new(R);
        remove_cycle.delete_edge(EId(4));
        let current = db.write(&commit, remove_cycle).await.unwrap();
        retire(&mut db);
        assert_eq!(db.rebuild_standing_query(&query, &maintained, limited).unwrap(), current);
        ...
        check(&db, &query, &maintained, R);
        check(&db, &query, &late_r, R);
        assert_eq!(db.standing_reachability(&query, &late_r).unwrap().rows().len(), 3);
        ...
        for (handle, relation) in [(&maintained, R), (&late_r, R), (&late_s, S)] {
            ...
            check(&db, &query, handle, relation);
            assert!(db.standing_reachability(&query, handle).unwrap().rows().is_empty());
        }
    });
...
fn every_snapshot_bootstrap_checkpoint_and_budget_refusal_preserves_the_owner() {
    let ((), report) = run_async_under_lab(0x6a82, |root| async move {
        ...
        let old_stats = *db.standing_reachability(&query, &handle).unwrap().last_maintenance();
        ...
        for stop in 1..=calls {
            let mut seen = 0;
            {
                ...
                let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
                assert!(matches!(State::from_snapshot(&db.snapshot, R, &mut meter), Err(StandingQueryFailure::Interrupted)));
            }
            assert_eq!(seen, stop);
            ...
            assert_eq!(plain(db.standing_reachability(&query, &handle).unwrap().rows()), old_rows);
            assert_eq!(db.standing_reachability(&query, &handle).unwrap().last_maintenance(), &old_stats);
        }
        let records = db.snapshot.blocks.iter().map(|block| block.len() as u64).sum::<u64>();
        for (bounded, wanted) in [
            (GqlQueryPolicy::new(records - 1, 100_000, 10_000_000, 10_000_000), StandingQueryFailure::SnapshotBudget),
            (GqlQueryPolicy::new(100_000, 100_000, success.stats.work_units - 1, 10_000_000), StandingQueryFailure::WorkBudget),
            (GqlQueryPolicy::new(100_000, 100_000, 10_000_000, success.stats.scratch_entries - 1), StandingQueryFailure::ScratchBudget),
            (GqlQueryPolicy::new(100_000, success.rows.len() as u64 - 1, 10_000_000, 10_000_000), StandingQueryFailure::ResultBudget),
        ] {
        ...
        assert!(matches!(db.rebuild_standing_query(&query, &handle, policy()),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::InvalidDelta))));
        assert_eq!(plain(db.standing_reachability(&query, &handle).unwrap().rows()), old_rows);
