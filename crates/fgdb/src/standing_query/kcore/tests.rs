...
fn all_composed_refusals_and_exact_quota_boundaries_leave_accepted_state_intact() {
    let ((), report) = run_async_under_lab(0x6b63_0110, |root| async move {
        ...
        for (id, a, b) in [(1, 0, 1), (2, 0, 2), (3, 0, 3), (4, 1, 2), (5, 1, 3), (6, 2, 3)] {
            seed.add_edge(EId(id), VId(a), VId(b), vec![]);
        }
        db.write(&commit, seed).await.unwrap();
        let baseline = Arc::clone(&db.snapshot);
        let before = build(&baseline);
        let mut change = WriteBatch::new(RelationId(1)); change.delete_edge(EId(1));
        ...
        let mut state = build(&baseline); let mut calls = 0;
        let stats = {
            let mut checkpoint = || { calls += 1; Ok(()) };
            let mut meter = Meter { policy: policy(), stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            state.maintain(&commit, batch, &mut meter).unwrap(); meter.stats
        };
        assert_eq!(state.rows, expected.rows); assert_eq!(state.cores, expected.cores);
        assert_eq!(state.input, expected.input);
        for stop in 1..=calls {
            let mut state = build(&baseline); let mut visited = 0;
            {
                let mut checkpoint = || { visited += 1;
        ...
        for (result, work, scratch, expected_error) in [
            ...
            (3, stats.work_units, stats.scratch_entries, Some(StandingQueryFailure::ResultBudget)),
            (4, stats.work_units - 1, stats.scratch_entries, Some(StandingQueryFailure::WorkBudget)),
            (4, stats.work_units, stats.scratch_entries - 1, Some(StandingQueryFailure::ScratchBudget)),
        ] {
            let mut state = build(&baseline); let mut checkpoint = || Ok(());
            let mut meter = Meter { policy: GqlQueryPolicy::new(100_000, result, work, scratch),
                stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            let outcome = state.maintain(&commit, batch, &mut meter);
            if let Some(error) = expected_error { assert_eq!(outcome, Err(error)); unchanged(&state, &before); }
            else { outcome.unwrap(); assert_eq!(state.rows, expected.rows); assert_eq!(state.cores, expected.cores); }
        }
        // Combined source admission, not just edge records, precedes bootstrap.
        let records: usize = baseline.blocks.iter().map(|b| b.len())
            .chain(baseline.patches.iter().map(|p| p.len())).sum();
        assert!(records > 0 && !baseline.patches.is_empty());
        for allowance in [records as u64, records as u64 - 1] {
            let mut checkpoint = || Ok(());
            let mut meter = Meter { policy: GqlQueryPolicy::new(allowance, 4, 10_000_000, 10_000_000),
                stats: StandingQueryStats::default(), checkpoint: &mut checkpoint };
            let result = State::from_snapshot(&baseline, RelationId(1), &mut meter);
            if allowance == records as u64 { assert_eq!(result.unwrap().rows, before.rows); }
