...
use fgdb_gql::{
    GqlQueryPolicy, GraphAggregate, GraphAggregateColumn, GraphAggregateOrder, GraphAggregateValue,
    GraphNullPlacement, GraphSetOperation, GraphSetQuantifier, PreparedGraphSet,
...
fn definitions(
    distinct: bool,
    ranked: bool,
) -> (PreparedGraphAggregate, PreparedGraphSetAggregate) {
    ...
    let compound =
        PreparedGraphSetAggregate::prepare(relation, &[0], &specs, 0, ranked.then_some(1))
            .unwrap()
            .with_result_clauses(&[], &order)
            .unwrap()
            .with_key_output_columns(&[])
...
fn delta<D: GroupDefinition>(
    definition: &D,
    values: &[(i64, u64, i128, i128)],
) -> ZSet<GraphAggregateRow> {
    let complete = definition.complete_groups().unwrap();
    ZSet::from_updates(
        values.iter().map(|&(key, count, sum, sign)| {
            (
                complete
...
fn apply<D: GroupDefinition>(
    state: &mut State<D>,
    change: &ZSet<GraphAggregateRow>,
) -> ZSet<GraphAggregateRow> {
...
fn visible<D: GroupDefinition>(state: &State<D>) -> Vec<GraphAggregateRow> {
    state.ordered_rows().map_or_else(Vec::new, |rows| {
