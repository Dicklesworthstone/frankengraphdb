...
fn definition(distinct: bool) -> PreparedGraphAggregate {
    ...
    PreparedGraphAggregate::prepare(
        ...
        &[
            ...
            GraphAggregate::sum_int("score_sum", 1),
        ],
        0,
        Some(2),
    )
...
fn delta(rows: &[(i64, u64, i128, i128)]) -> ZSet<GraphAggregateRow> {
    ...
    ZSet::from_updates(
        rows.iter().map(|&(key, count, sum, sign)| {
            (
                source
...
fn entries(rows: &Ranked) -> Vec<(GraphAggregateRow, GraphAggregateRow)> {
    rows.iter()
        .map(|(rank, value)| (rank.group.as_ref().clone(), value.as_ref().clone()))
        .collect()
}
fn same(actual: &Output, expected: &Output) {
    ...
    let groups = |state: &State| {
        state
            .groups
            .iter()
            .map(|(key, rank)| (key.clone(), rank.group.clone()))
            .collect::<Vec<_>>()
    };
    let classes = |state: &State| {
        state
            .classes
            .iter()
            .map(|(key, members)| (key.clone(), entries(members)))
            .collect::<Vec<_>>()
    };
...
fn every_rank_checkpoint_budget_and_dropped_guard_preserves_all_indexes_then_retries() {
    ...
    for distinct in [false, true] {
        ...
        let mut calls = 0;
        let stats = apply(&mut complete, &change, policy(), &mut || {
            calls += 1;
            Ok(())
        })
        .unwrap();
        ...
        for stop in 1..=calls {
            ...
            let mut seen = 0;
            assert_eq!(
                apply(&mut candidate, &change, policy(), &mut || {
                    seen += 1;
                    if seen == stop {
        ...
        for reason in [
            ...
            assert_eq!(
                apply(&mut candidate, &change, bounded, &mut || Ok(())),
                Err(reason)
            );
            same(&candidate, &before);
            apply(&mut candidate, &change, policy(), &mut || Ok(())).unwrap();
...
fn unchanged_rank_refreshes_full_rows_and_bad_before_images_never_publish() {
    for distinct in [false, true] {
        ...
        let current = output
            ...
            .groups
            .values()
            .find(|rank| rank.group.keys() == [GraphValue::Scalar(CanonicalScalar::Int(2))])
            .unwrap();
        ...
        for malformed in [
            ...
            .unwrap();
            assert_eq!(
                apply(&mut output, &malformed, policy(), &mut || Ok(())),
                Err(StandingQueryFailure::InvalidDelta)
            );
            same(&output, &original);
        }
        apply(&mut output, &delta(&[(2, 7, 9, -1)]), policy(), &mut || {
            Ok(())
        })
        .unwrap();
...
fn whole_ranked_maintenance_aborts_source_result_and_page_at_every_checkpoint() {
    ...
    let ((), report) = run_async_under_lab(0x6a97, |root| async move {
        ...
        let stats = {
            ...
            complete
                .maintain_with_output(&delta, &mut meter, Some(&mut final_output))
                .unwrap();
            meter.stats
        };
        ...
        assert_eq!(
            final_output.ordered_rows().unwrap()[0]
                .get(0)
                .unwrap()
                .as_count(),
            Some(3)
