...
use fgdb_prism::{
    Directedness, FnxCallSpec, FnxExecutionError, FnxGraphKind, FnxParameters,
    FnxReadError, FnxReadOptions, FnxValue, ParallelEdgePolicy, ProjectionBuildError,
    ProjectionEdge, ProjectionError, SelfLoopPolicy, SnapshotBinding, SnapshotGraphView,
};
...
impl<V: Vfs + Clone> Database<V> {
    ...
    pub fn execute_fnx_authorized(
        ...
        authorized_with_errors(
            self, cx, authority, token, branch, options.as_of, clock, control_error,
            |snapshot, at, scope, execution| {
                ...
                let binding = SnapshotBinding { root: view.partition_root().0, as_of: at };
...
fn execute<Clock: FnMut() -> u64>(
    ...
    let mut control = |event| {
        ...
        let (counter, limit, resource) = match event {
            SourceEvent::Work | SourceEvent::SnapshotRecord => {
                (&mut work, options.source_limits.max_work_units, "work units")
            }
            SourceEvent::ScratchEntry => {
                (&mut scratch, options.source_limits.max_scratch_entries, "scratch entries")
            }
        };
    ...
    source::visit_vertices(&snapshot.patches, binding.as_of, &mut control, |row, control| {
        ...
        push_staged(
            &mut vertices, row.vid, "vertices", options.projection_limits.max_vertices,
            &mut staged_bytes, options.source_limits.max_staging_bytes,
        )
    })?;
    let mut edges = Vec::new();
    source::visit_edges_with_properties(snapshot, binding.as_of, &mut control, |entry, props, control| {
        control(SourceEvent::Work)?;
        if !scope.allows_relation(entry.relation)
            || options.selection.relation.is_some_and(|relation| relation != entry.relation)
        ...
        if entry.src == entry.dst {
            match options.projection.self_loops {
                ...
                SelfLoopPolicy::Reject => return Err(Error::Projection(ProjectionError::SelfLoop(entry.eid))),
                SelfLoopPolicy::Keep => {}
            }
        }
        let weight = if options.projection.parallel_edges == ParallelEdgePolicy::CollapseUnit {
            ...
            let value = options.selection.weight.property_key()
                .filter(|&key| scope.allows_property(key))
                .and_then(|key| props.binary_search_by_key(&key, |(key, _)| *key).ok())
                .map(|index| &props[index].1);
            options.selection.weight.resolve(value)
                .map_err(|reason| Error::Weight { edge: entry.eid, reason })?
        };
        push_staged(
            &mut edges,
            ProjectionEdge { eid: entry.eid, source: entry.src, target: entry.dst, weight },
            "input edges", options.projection_limits.max_input_edges,
            &mut staged_bytes, options.source_limits.max_staging_bytes,
        )
    })?;
    let graph = SnapshotGraphView::build_owned_with_checkpoint(
        binding, vertices, edges, options.projection, options.projection_limits,
    ...
    limits.max_result_rows = limits.max_result_rows.min(usize::try_from(signed_rows).unwrap_or(usize::MAX));
    ...
    if let Err(FnxExecutionError::LimitExceeded { resource: "result rows", requested, .. }) = &result {
        if *requested as u128 > u128::from(signed_rows) {
            execution.borrow_mut().deliver(*requested).map_err(control_error)?;
        }
    }
