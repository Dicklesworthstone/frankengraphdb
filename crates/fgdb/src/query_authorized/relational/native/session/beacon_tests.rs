    ...
    if hidden {
        for id in 100..120 {
            rows.create_vertex(
                ...
                vec![HIDDEN],
                vec![
                    (TEXT, text("red red red red red")),
                    (X, CanonicalScalar::Int(0)),
                ],
            );
        }
        rows.create_vertex(
            VId(120),
            vec![HIDDEN],
            vec![(TEXT, CanonicalScalar::Int(9)), (X, text("not a vector"))],
        );
    }
...
fn scoped_session_search_matches_visible_corpus_and_keeps_its_pin() {
    let ((), report) = run_async_under_lab(0xbeac_3001, |root| async move {
        ...
        let mut session = full
            .authorized_read_session(&query, &issuer, &token, BRANCH, no_catalog, host(), || NOW)
            .unwrap();
        let expected =
            searches().map(|search| clean.beacon_search(&query, &options(), search).unwrap());
        for (search, rows) in searches().into_iter().zip(&expected) {
            assert_eq!(
                session.beacon_search(&query, &options(), search).unwrap(),
                *rows
            );
            assert_eq!(
        ...
        // options() selects LABEL vertices, which excludes the malformed HIDDEN
        // vertex even from a privileged search; drop that selection so the
        // unscoped search actually reaches it (x7qs8).
        let mut unselected = options();
        unselected.vertex_label = None;
        assert!(
            full.beacon_search(&query, &unselected, searches()[0])
                .is_err()
        );
        let mut update = WriteBatch::new(RelationId(2));
        update.delete_vertex(VId(1));
        update.create_vertex(
            VId(4),
            vec![LABEL],
            vec![(TEXT, text("red")), (X, CanonicalScalar::Int(1))],
        );
        ...
        session.close();
        assert!(matches!(
            session.beacon_search(&query, &options(), searches()[0]),
            Err(SearchError::Interrupted(QueryError::Authorization(
...
fn historical_winners_and_property_masks_precede_search_validation() {
    let ((), report) = run_async_under_lab(0xbeac_3002, |root| async move {
        ...
        let mut session = db
            ...
            .unwrap();
        let current = session
            .beacon_search(&query, &options(), searches()[0])
            .unwrap();
        assert!(
            !ids(&current).contains(&VId(1)),
            "a hidden successor cannot revive its predecessor"
        );
        let mut historical = options();
        historical.as_of = Some(old);
        assert!(
            ids(&session
                .beacon_search(&query, &historical, searches()[0])
                .unwrap())
            .contains(&VId(1))
        );
        historical.as_of = Some(CommitSeq(db.frontier().unwrap().0 + 1));
        assert!(matches!(
            session.beacon_search(&query, &historical, searches()[0]),
        ...
        assert!(
            session
                .beacon_search(&query, &masked, searches()[0])
                .unwrap()
                .is_empty()
        );
        masked = options();
        ...
        assert!(
            session
                ...
                .unwrap()
                .is_empty(),
            "masked coordinates omit the lane before type validation"
        );
...
fn request_limits_cannot_widen_host_or_signed_session_limits() {
    let ((), report) = run_async_under_lab(0xbeac_3003, |root| async move {
        ...
        request.policy = ReadPolicy {
            ...
            max_source_scratch: usize::MAX,
            max_staging_rows: usize::MAX,
        ...
        for (case, limits) in [
            ...
            match case {
                0 | 1 => assert!(matches!(
                    result,
                    Err(SearchError::Interrupted(QueryError::Pattern(
                        GqlQueryError::Rows(_)
                    )))
                )),
                2 => assert!(matches!(
                    result,
                    Err(SearchError::Index(BeaconError::WorkBudgetExceeded))
                )),
                _ => assert!(matches!(
                    result,
                    Err(SearchError::Index(BeaconError::ResourceLimit { .. }))
                )),
            }
            assert!(
                !session.is_closed(),
                "ordinary native refusal retains the pin"
            );
        }
        ...
        let mut session = db
            ...
            .unwrap();
        assert!(matches!(
            session.beacon_search(&query, &request, searches()[0]),
            Err(SearchError::Interrupted(QueryError::Authorization(
                WardenError::LimitExceeded(LimitDimension::Rows)
            )))
        ));
        assert!(!session.is_closed());
        let small = Search::Text {
            query: "red",
            k: 2,
            mode: TextMatch::Any,
        };
        assert_eq!(
            session
                .beacon_search(&query, &request, small)
                .unwrap()
                .len(),
            2
        );
