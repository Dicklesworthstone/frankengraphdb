...
fn grant() -> Grant {
    let mut grant = Grant::read_only("main", EXPIRES, QueryLimits {
        max_nodes: 4, max_work: 1_000_000, max_rows: 4,
    });
    ...
    if hidden {
        for id in 100..116 {
            batch.create_vertex(VId(id), vec![H], vec![
                ...
                (X, CanonicalScalar::ucs_basic_text("hidden bad type").unwrap()),
            ]);
...
fn scoped_session_uses_the_native_three_lane_result_in_every_direction() {
    let ((), report) = run_async_under_lab(0xbeac_4201, |root| async move {
        ...
        for direction in [Direction::Outgoing, Direction::Incoming, Direction::Undirected] {
            for mode in [VectorSearch::Exact, VectorSearch::Approximate { ef_search: 16 }] {
                ...
                for seeds in [vec![VId(1)], vec![VId(4), VId(4)], vec![VId(100)], vec![]] {
                    ...
                    let actual = session.beacon_search_graph(&c.query(), &options(), q, e).unwrap();
                    assert_eq!(actual, clean.beacon_search_graph(&c.query(), &options(), q, e).unwrap());
                    assert_eq!(actual, db.beacon_search_graph_authorized(
                        &c.query(), &issuer, &token, "main", &options(), q, e, || NOW,
                    ).unwrap());
                }
            }
        }
        ...
        assert!(session.beacon_search_graph(&c.query(), &options(), query(), hidden_relation)
...
fn writer_changes_and_drop_cannot_mix_new_topology_into_the_session_pin() {
    let ((), report) = run_async_under_lab(0xbeac_4202, |root| async move {
        ...
        let expected = session.beacon_search_graph(&c.query(), &options(), query(), expansion(&[VId(1)])).unwrap();
        let mut update = WriteBatch::new(R);
        update.set_vertex_label(VId(2), L, false);
        update.set_vertex_property(VId(2), X, Some(CanonicalScalar::ucs_basic_text("now hidden").unwrap()));
        update.delete_edge(EId(4));
        db.write(&c.commit(), update).await.unwrap();
        let mut current = db.authorized_read_session(
            &c.query(), &issuer, &token, "main", catalog, host(), || NOW,
        ).unwrap();
        assert_ne!(current.beacon_search_graph(&c.query(), &options(), query(), expansion(&[VId(1)])).unwrap(), expected);
        let mut historical = options();
        historical.as_of = Some(at);
        assert_eq!(current.beacon_search_graph(&c.query(), &historical, query(), expansion(&[VId(1)])).unwrap(), expected);
        historical.as_of = Some(db.frontier().unwrap());
        assert!(matches!(session.beacon_search_graph(&c.query(), &historical, query(), expansion(&[VId(1)])),
            Err(super::super::SearchError::Read(_))));
        drop(db);
        assert_eq!(session.beacon_search_graph(&c.query(), &options(), query(), expansion(&[VId(1)])).unwrap(), expected);
        current.close();
        session.close();
        assert_eq!(c.outstanding_obligations(), 0);
    });
...
fn vertices_and_parallel_edges_share_record_and_scratch_allowances() {
    let ((), report) = run_async_under_lab(0xbeac_4203, |root| async move {
        ...
        for hidden in [false, true] {
            ...
            let mut session = db.authorized_read_session(
                &c.query(), &issuer, &token, "main", catalog,
                GqlQueryPolicy::new(7, 4, 1_000_000, 1000), || NOW,
            ).unwrap();
            let result = session.beacon_search_graph(&c.query(), &options(), query(), expansion(&[VId(1)]));
            ...
            assert!(matches!(session.beacon_search_graph(&c.query(), &o, query(), expansion(&[VId(1)])),
                Err(super::super::SearchError::Index(BeaconError::ResourceLimit {
                    resource: "admitted graph scratch", limit: 7,
                }))));
            o.policy.max_source_scratch = 8;
            assert_eq!(session.beacon_search_graph(&c.query(), &o, query(), expansion(&[VId(1)])).unwrap().len(), 4);
            let mut e = expansion(&[VId(1)]);
            e.limits.max_input_edges = 3;
            assert!(matches!(session.beacon_search_graph(&c.query(), &options(), query(), e),
                Err(super::super::SearchError::Index(BeaconError::ResourceLimit {
                    resource: "expansion input edges", limit: 3,
                }))));
            session.close();
        }
        assert_eq!(c.outstanding_obligations(), 0);
...
fn final_k_and_graph_structure_limits_cannot_escape_host_policy() {
    let ((), report) = run_async_under_lab(0xbeac_4204, |root| async move {
        ...
        assert!(matches!(session.beacon_search_graph(&c.query(), &options(), too_many, expansion(&[VId(1)])),
            Err(super::super::SearchError::Interrupted(QueryError::Authorization(
                WardenError::LimitExceeded(fgdb_warden::LimitDimension::Rows),
            )))));
        let mut e = expansion(&[VId(1)]);
        e.limits = ExpansionLimits {
            max_vertices: 0, max_input_edges: 0, max_visited_vertices: 0,
            max_seed_ids: 0, max_source_scratch: 0,
        };
        for (weight, candidates) in [(0, 4), (100, 0)] {
            let mut q = query();
            q.graph_weight = weight;
            q.graph_candidates = candidates;
            let rows = session.beacon_search_graph(&c.query(), &options(), q, e).unwrap();
        ...
        let mut session = db.authorized_read_session(
            &c.query(), &issuer, &token, "main", catalog,
            GqlQueryPolicy::new(100, 4, 1_000_000, 4), || NOW,
        ).unwrap();
        assert!(matches!(session.beacon_search_graph(&c.query(), &options(), query(),
            expansion(&[VId(1), VId(1), VId(1), VId(1), VId(1)])),
            Err(super::super::SearchError::Index(BeaconError::ResourceLimit {
                resource: "expansion seed IDs", limit: 4,
            }))));
        session.close();
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn every_graph_checkpoint_refuses_without_rows_and_retry_uses_the_same_pin() {
    let ((), report) = run_async_under_lab(0xbeac_4205, |root| async move {
        ...
        let expected = session.beacon_search_graph(&c.query().with_checkpoint_probe(probe.clone()),
            &options(), query(), expansion(&[VId(1)])).unwrap();
        assert!(probe.calls() > 20);
        for cut in 1..=probe.calls() {
            let cut_probe = Arc::new(SimulationCheckpointProbe::new(Some(cut)));
            let result = session.beacon_search_graph(&c.query().with_checkpoint_probe(cut_probe),
                &options(), query(), expansion(&[VId(1)]));
...
fn expiry_retires_the_session_even_at_the_final_empty_delivery_boundary() {
    let ((), report) = run_async_under_lab(0xbeac_4206, |root| async move {
        ...
        for empty in [false, true] {
            let mut o = options();
            if empty { o.vertex_label = Some(H); }
            let seen = Cell::new(0);
            let mut baseline = db.authorized_read_session(
            ...
            seen.set(0);
            let rows = baseline.beacon_search_graph(&c.query(), &o, query(), expansion(&[VId(1)])).unwrap();
            ...
            for cut in [0, 1, calls / 2, calls - 2, calls - 1] {
                ...
                let mut session = db.authorized_read_session(
                    &c.query(), &issuer, &token, "main", catalog, host(), || {
                        let at = seen.get();
                        seen.set(at + 1);
                        if stop.get() && at >= cut { EXPIRES } else { NOW }
                    },
                ).unwrap();
                seen.set(0);
                stop.set(true);
                assert!(matches!(session.beacon_search_graph(&c.query(), &o, query(), expansion(&[VId(1)])),
                    Err(super::super::SearchError::Interrupted(QueryError::Authorization(WardenError::Expired)))));
                assert!(session.is_closed());
