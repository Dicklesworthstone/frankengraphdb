    ...
    if hidden {
        for id in 100..132 {
            batch.create_vertex(
                VId(id),
                vec![H],
                vec![(T, CanonicalScalar::Int(1)), (X, text("not a coordinate"))],
            );
        }
    }
...
fn reusable_search_freezes_definition_and_matches_one_shot_after_writer_drop() {
    let ((), report) = run_async_under_lab(0xbeac_4101, |root| async move {
        ...
        let prepared = session
            .prepare_beacon_index(&c.query(), &definition)
            .unwrap();
        ...
        let expected =
            queries().map(|query| clean.beacon_search(&c.query(), &options(), query).unwrap());
        for (query, expected) in queries().into_iter().zip(&expected) {
            assert_eq!(
                session
                    .search_beacon_index(&c.query(), &prepared, query, ReadPolicy::default())
                    .unwrap(),
                *expected
            );
            assert_eq!(
        ...
        for (query, expected) in queries().into_iter().zip(expected) {
            assert_eq!(
                session
                    .search_beacon_index(&c.query(), &copy, query, ReadPolicy::default())
                    .unwrap(),
                expected
            );
        }
        session.close();
        denied(
            session.search_beacon_index(&c.query(), &copy, queries()[0], ReadPolicy::default()),
...
fn identical_or_narrower_sessions_cannot_reuse_a_foreign_corpus() {
    let ((), report) = run_async_under_lab(0xbeac_4102, |root| async move {
        ...
        let mut second = db
            ...
            .unwrap();
        denied(
            second.search_beacon_index(&c.query(), &prepared, queries()[4], ReadPolicy::default()),
        ...
        assert!(
            second
                .search_beacon_index(&c.query(), &own, queries()[4], ReadPolicy::default())
                .is_ok()
        );
        ...
        let mut third = db
            ...
            .unwrap();
        denied(
            third.search_beacon_index(&c.query(), &prepared, queries()[2], ReadPolicy::default()),
            WardenError::ScopeDenied,
        );
        let masked = third.prepare_beacon_index(&c.query(), &options()).unwrap();
        assert!(
            third
                .search_beacon_index(&c.query(), &masked, queries()[2], ReadPolicy::default())
                .unwrap()
                .is_empty()
        );
        issuer.retire();
        ...
        denied(
            second.search_beacon_index(&c.query(), &prepared, queries()[0], ReadPolicy::default()),
...
fn historical_cuts_and_all_enabled_lanes_are_validated_at_preparation() {
    let ((), report) = run_async_under_lab(0xbeac_4103, |root| async move {
        ...
        let old = session
            .prepare_beacon_index(&c.query(), &historical)
            .unwrap();
        let current = session
            .prepare_beacon_index(&c.query(), &options())
            .unwrap();
        assert_eq!(old.source_sequence(), at);
        assert_eq!(current.source_sequence(), db.frontier().unwrap());
        let rows = session
            .search_beacon_index(&c.query(), &old, queries()[2], ReadPolicy::default())
            .unwrap();
        ...
        let rows = session
            .search_beacon_index(&c.query(), &current, queries()[2], ReadPolicy::default())
            .unwrap();
        // ubs:ignore -- test assertion on a vertex id, not secret material.
        ...
        assert!(matches!(
            session.prepare_beacon_index(&c.query(), &invalid),
            Err(super::super::SearchError::Index(
                BeaconError::InvalidConfig(_)
            ))
        ));
        historical.as_of = Some(CommitSeq(db.frontier().unwrap().0 + 1));
        assert!(matches!(
            session.prepare_beacon_index(&c.query(), &historical),
            Err(super::super::SearchError::Read(_))
        ));
        let mut text_only = options();
        text_only.index.vector = None;
        let text_only = session
            .prepare_beacon_index(&c.query(), &text_only)
            .unwrap();
        assert!(matches!(
            session.search_beacon_index(
                &c.query(),
                &text_only,
                queries()[2],
                ReadPolicy::default()
            ),
            Err(super::super::SearchError::Index(BeaconError::Disabled(
                "vector"
            )))
        ));
        ...
        assert!(
            session
                .search_beacon_index(&c.query(), &old, queries()[0], ReadPolicy::default())
                .is_ok()
        );
        session.close();
    });
...
fn reuse_does_not_rescan_and_cannot_widen_frozen_or_host_limits() {
    let ((), report) = run_async_under_lab(0xbeac_4104, |root| async move {
        ...
        assert_eq!(
            session
                .search_beacon_index(&c.query(), &prepared, queries()[2], request)
                .unwrap()
                .len(),
            3
        );
        ...
        assert!(matches!(
            session.search_beacon_index(&c.query(), &prepared, queries()[2], request),
            Err(super::super::SearchError::Index(
        ...
        assert!(matches!(
            session.search_beacon_index(&c.query(), &narrow, queries()[2], ReadPolicy::default()),
            Err(super::super::SearchError::Index(
                BeaconError::ResourceLimit {
                    resource: "result rows",
                    limit: 2
        ...
        assert!(matches!(
            session.prepare_beacon_index(&c.query(), &zero),
            Err(super::super::SearchError::Index(
                BeaconError::ResourceLimit { .. }
            ))
        ));
                ...
                let mut session = db
                    .authorized_read_session(
                        ...
                        || {
                            let at = seen.get();
                            seen.set(at + 1);
                            if stop.get().is_some_and(|cut| at >= cut) {
                                EXPIRES
                            } else {
                                NOW
                            }
