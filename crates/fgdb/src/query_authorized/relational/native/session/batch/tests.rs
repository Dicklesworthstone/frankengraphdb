...
fn visible() -> QueryResult {
    rows(&["p"], vec![vec![value(10)], vec![value(30)]])
}
...
fn all_seven_native_classes_keep_independent_schemas_and_one_pinned_generation() {
    let ((), report) = run_async_under_lab(0x5ec0_6101, |root| async move {
        ...
        let statements = [
            ...
            (
                "MATCH (n) RETURN sum(n.p) AS total, count(*) AS rows",
                &params,
            ),
            (
                "MATCH (n) WITH n.p AS p RETURN sum(p) AS total, count(*) AS rows",
                &params,
            ),
            (
            ...
            (
                "MATCH (n) FOR SYSTEM_TIME AS OF SEQ 1 RETURN sum(n.p) AS total, count(*) AS rows",
                &params,
            ),
            (
                "MATCH (n) FOR SYSTEM_TIME AS OF SEQ 1 RETURN n.p AS p UNION ALL MATCH (n) RETURN n.p AS p",
                &params,
            ),
        ];
        let summary = rows(
...
fn exact_combined_delivery_and_source_limits_do_not_reset_between_statements() {
    let ((), report) = run_async_under_lab(0x5ec0_6102, |root| async move {
        ...
        let issuer = issuer(702);
        let mut permitted = grant();
        permitted.limits.max_rows = 3;
        permitted.limits.max_nodes = 4;
        ...
        let pair = [
            ("MATCH (n) RETURN n.p AS p", &params),
            ("MATCH (n) RETURN count(*) AS count", &params),
        ];
        let expected = vec![
            visible(),
            rows(&["count"], vec![vec![QueryValue::Count(2)]]),
        ];
        ...
        refusal(
            session.query_batch(&cx, &[pair[0], pair[1], ("RETURN 1 AS one", &params)]),
            AuthorizationError::LimitExceeded(LimitDimension::Rows),
        );
        assert!(!session.is_closed());
        assert_eq!(session.query_batch(&cx, &pair).unwrap(), expected);
        refusal(
            session.query_batch(&cx, &[pair[1], pair[1], pair[1]]),
        ...
        assert!(
            zero.query_batch(&cx, &empty)
                .unwrap()
                .iter()
                .all(|result| matches!(result, QueryResult::Rows { rows, .. } if rows.is_empty()))
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn prepared_batches_rebind_repeated_handles_without_catalog_callbacks_or_double_row_charges() {
    let ((), report) = run_async_under_lab(0x5ec0_6103, |root| async move {
        ...
        let cx = c.query();
        let db = database(&c.commit()).await;
        let issuer = issuer(703);
        let mut grant = grant();
        grant.limits.max_rows = 3;
        let token = issuer.issue_at(&grant, 100).unwrap();
        ...
        let prepared = session
            .prepare(&cx, "MATCH (n) WHERE n.p >= $minimum RETURN n.p AS p", &low)
            .unwrap();
        let frozen = resolutions.get();
        let batch = [(&prepared, &low), (&prepared, &mid), (&prepared, &high)];
        let expected = vec![
            visible(),
            rows(&["p"], vec![vec![value(30)]]),
            rows(&["p"], vec![]),
        ];
        ...
        assert!(
            session
                .execute_batch(
                    &cx,
                    &[(&prepared, &low), (&prepared, &mid), (&prepared, &unknown)]
                )
                .is_err()
        );
        assert!(!session.is_closed());
...
fn every_prepared_owner_and_branch_are_checked_before_the_first_source() {
    let ((), report) = run_async_under_lab(0x5ec0_6104, |root| async move {
        ...
        let cx = c.query();
        let db = database(&c.commit()).await;
        let issuer = issuer(704);
        let token = issuer.issue_at(&grant(), 100).unwrap();
        let denied = GqlQueryPolicy::new(0, 0, 1_000_000, 1_000_000);
        let mut session = db
            .authorized_read_session(&cx, &issuer, &token, BRANCH, symbols, denied, || 100)
            .unwrap();
        let mut other = db
            .authorized_read_session(&cx, &issuer, &token, BRANCH, symbols, denied, || 100)
            .unwrap();
        let params = GqlParameters::new();
        let own = session
            .prepare(&cx, "MATCH (n) RETURN n.p AS p", &params)
            .unwrap();
        let foreign = other
            .prepare(&cx, "RETURN 1 AS one LIMIT 0", &params)
            .unwrap();
        ...
        refusal(
            session.execute_batch(&cx, &[(&own, &params), (&foreign, &params)]),
            AuthorizationError::WrongAuthority,
        );
        let branch = GqlParameters::new().with_text("route", BRANCH).unwrap();
        let wrong = GqlParameters::new()
            .with_text("route", "elsewhere")
            .unwrap();
        let selected = session
            .prepare(&cx, "AT BRANCH $route RETURN 1 AS one LIMIT 0", &branch)
            .unwrap();
        refusal(
            session.execute_batch(&cx, &[(&own, &params), (&selected, &wrong)]),
            AuthorizationError::ScopeDenied,
        );
        // With preflight satisfied, the original native source refusal is kept.
        assert!(matches!(
            session.execute_batch(&cx, &[(&own, &params), (&selected, &branch)]),
...
fn empty_and_maximum_batches_are_bounded_and_still_authenticate() {
    let ((), report) = run_async_under_lab(0x5ec0_6105, |root| async move {
        ...
        let cx = c.query();
        let db = database(&c.commit()).await;
        let issuer = issuer(705);
        let mut grant = grant();
        grant.limits.max_nodes = 0;
        grant.limits.max_rows = MAX_STATEMENTS as u64;
        let token = issuer.issue_at(&grant, 100).unwrap();
        ...
        assert_eq!(
            session.query_batch(&cx, &max).unwrap(),
            vec![rows(&["one"], vec![vec![value(1)]]); MAX_STATEMENTS]
        );
        let oversized = vec![("RETURN 1 AS one", &params); MAX_STATEMENTS + 1];
        assert!(matches!(
            session.query_batch(&cx, &oversized),
            Err(QueryError::Unsupported { .. })
        ));
        assert!(!session.is_closed());
        let prepared = session.prepare(&cx, "RETURN 1 AS one", &params).unwrap();
        assert!(matches!(
            session.execute_batch(&cx, &vec![(&prepared, &params); MAX_STATEMENTS + 1]),
            Err(QueryError::Unsupported { .. })
        ));
        now.set(1000);
...
fn late_errors_discard_completed_results_and_do_not_run_later_statements_or_writes() {
    let ((), report) = run_async_under_lab(0x5ec0_6106, |root| async move {
        ...
        for failing in [
            "UNWIND [1, 0] AS x RETURN 10 / x AS quotient LIMIT 0",
            "INSERT (n:L)",
        ] {
            let batch = [
                ("RETURN 1 AS one", &params),
                (failing, &params),
                ("MATCH (n:late_marker) RETURN n", &params),
            ];
        ...
        assert_eq!(
            session
                .query_batch(&cx, &[("RETURN 1 AS one", &params)])
                .unwrap(),
...
fn every_batch_expiry_cut_keeps_all_results_private_and_closes_the_session() {
    let ((), report) = run_async_under_lab(0x5ec0_6107, |root| async move {
        ...
        let clock = || {
            let at = calls.get() + 1;
            calls.set(at);
            if at >= cutoff.get() { 1000 } else { 100 }
        };
        let params = GqlParameters::new();
        let batch = [
            ("RETURN 1 AS one", &params),
            ("MATCH (n:L) RETURN count(*) AS rows", &params),
        ];
        ...
        for prepared_mode in [false, true] {
            ...
            if prepared_mode {
                session
                    .execute_batch(&cx, &[(&a, &params), (&b, &params)])
                    .unwrap();
            } else {
                session.query_batch(&cx, &batch).unwrap();
            }
            let cuts = calls.get();
            session.close();
            for stop in 1..=cuts {
                ...
                let result = if prepared_mode {
                    session.execute_batch(&cx, &[(&a, &params), (&b, &params)])
                } else {
                    session.query_batch(&cx, &batch)
                };
