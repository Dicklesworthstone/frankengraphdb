...
async fn database(cx: &CommitCx) -> Database<MemVfs> {
    ...
    for (id, label, p) in [(1, 1, 10), (2, 99, 20), (3, 1, 30)] {
        batch.create_vertex(VId(id), vec![LabelId(label)], vec![(PropertyKeyId(1), CanonicalScalar::Int(p))]);
    }
...
fn first_authorization_cause_is_stable_and_no_failed_permit_resumes() {
    let ((), report) = run_async_under_lab(0x5ec0_6001, |root| async move {
        ...
        for (case, expected) in [
            ...
            let now = Cell::new(100); let calls = Cell::new(0);
            let mut execution = Execution::new(&cx, permit, || {
                calls.set(calls.get() + 1); now.get()
            });
            match case {
                0 => now.set(1000),
                1 => now.set(99),
            ...
            let used = execution.permit.usage(); let sampled = calls.get();
            for _ in 0..4 {
                ...
                assert_eq!(calls.get(), sampled, "failed execution must not call host code again");
            }
            assert_eq!(execution.permit.checkpoint_at(100), Err(AuthorizationError::ExecutionStopped),
...
fn every_text_and_prepared_expiry_boundary_closes_and_releases_the_pin() {
    let ((), report) = run_async_under_lab(0x5ec0_6002, |root| async move {
        let c = PurposeContexts::narrow_runtime_root(&root); let cx = c.query();
        let db = database(&c.commit()).await; let issuer = issuer(610);
        let token = issuer.issue_at(&grant(), 100).unwrap();
        let calls = Cell::new(0); let cutoff = Cell::new(usize::MAX);
        let clock = || { let next = calls.get() + 1; calls.set(next); if next >= cutoff.get() { 1000 } else { 100 } };
        ...
        for prepared_mode in [false, true] {
            ...
            calls.set(0);
            if prepared_mode { session.execute(&cx, &prepared, &params).unwrap(); }
            else { session.query(&cx, text, &params).unwrap(); }
            ...
            for stop in 1..=checkpoints {
                ...
                let result = if prepared_mode { session.execute(&cx, &prepared, &params) }
                    else { session.query(&cx, text, &params) };
                refusal(result, AuthorizationError::Expired);
                assert!(session.is_closed(), "expiry at checkpoint {stop} must close immediately");
                ...
                refusal(session.query(&cx, text, &params), AuthorizationError::ExecutionStopped);
...
fn signed_quota_refusals_keep_the_actual_dimension_and_allow_a_new_statement() {
    let ((), report) = run_async_under_lab(0x5ec0_6003, |root| async move {
        let c = PurposeContexts::narrow_runtime_root(&root); let cx = c.query();
        let db = database(&c.commit()).await; let issuer = issuer(611);
        for dimension in [LimitDimension::Nodes, LimitDimension::Rows] {
            ...
            let params = GqlParameters::new();
            refusal(session.query(&cx, "MATCH (n) RETURN n.p AS p", &params), AuthorizationError::LimitExceeded(dimension));
            assert!(!session.is_closed());
            assert!(matches!(session.query(&cx, "RETURN 1 AS one", &params).unwrap(),
...
fn retirement_in_successful_or_failing_catalog_callback_closes_in_the_same_call() {
    let ((), report) = run_async_under_lab(0x5ec0_6004, |root| async move {
        ...
        for valid_symbol in [false, true] {
            let issuer = issuer(612); let token = issuer.issue_at(&grant(), 100).unwrap();
            let called = Cell::new(false);
            let mut session = db.authorized_read_session(&cx, &issuer, &token, BRANCH,
                |kind, name: &str| {
                    called.set(true); issuer.retire();
                    if valid_symbol { symbols(kind, name) } else { None }
                }, policy(), || 100).unwrap();
            refusal(session.query(&cx, "MATCH (n:L) RETURN n", &GqlParameters::new()), AuthorizationError::AuthorityRetired);
            assert!(called.get()); assert!(session.is_closed());
