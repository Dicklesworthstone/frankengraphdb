...
fn policy() -> GqlQueryPolicy {
    GqlQueryPolicy::new(10000, 1000, 10_000_000, 10_000_000)
}
...
async fn database(cx: &CommitCx) -> Database<MemVfs> {
    let mut db = Database::open_memory(cx, DatabaseKeys::new([0x16; 32], NS, [0x26; 32]))
        .await
        .unwrap();
...
fn scoped_edges_and_fixed_joins_aggregate_only_admitted_topology_and_fields() {
    ...
    for (text, expected) in [
        (
            ...
            vec![vec![
                ...
                Count(3),
                Integer(14),
                Average(GraphExactAverage::new(14, 3).unwrap()),
                scalar(2),
                scalar(8),
                Count(3),
                Integer(14),
        ...
        (
            "MATCH (a)-[e:R]->(b)-[f:R]->(c) RETURN COUNT(*) AS n,SUM(f.p) AS total",
        ...
        (
            "MATCH (a)-[e:R]->(b) WHERE NOT (e.hidden = 55) RETURN COUNT(*) AS n",
            vec![vec![Count(0)]],
        ),
        (
            "MATCH (a:H)-[e:R]->(b) RETURN COUNT(*) AS n",
            vec![vec![Count(0)]],
        ),
        (
            "MATCH (a)-[e:R]->(b) RETURN SUM(e.hidden) AS total",
            vec![vec![null()]],
        ),
        (
            "MATCH (a)-[e:S]->(b) RETURN COUNT(*) AS n",
            vec![vec![Count(0)]],
        ),
        (
            "MATCH (a)-[e:R]->(b) RETURN a.p AS key,COUNT(*) AS n,SUM(e.p) AS total GROUP BY a.p HAVING COUNT(*) > 1 ORDER BY key DESC LIMIT 1",
            vec![vec![scalar(7), Count(3), Integer(6)]],
        ),
    ] {
    ...
    let prepared = session
        .prepare(
            &cx,
            "MATCH (a)-[e:R]->(b) WHERE e.p > $wanted RETURN COUNT(*) AS n",
            &low,
        )
        .unwrap();
    for (params, n) in [(&low, 2), (&high, 0), (&low, 2)] {
        assert_eq!(
            rows(&mut session.stream_aggregate(&cx, &prepared, params).unwrap()),
            vec![vec![Count(n)]]
        );
    }
}
...
fn orientation_parallel_edges_and_collection_profiles_keep_native_identity_order() {
    ...
    for (pattern, count, sum) in [
        ...
        let prepared = session
            .prepare(
                &cx,
                &format!("MATCH {pattern} RETURN COUNT(*) AS n,SUM(e.p) AS total"),
                &params,
            )
            .unwrap();
        assert_eq!(
    ...
    .enumerate()
    {
        let list = QueryValue::Value(GraphValue::List(
            value
                .into_iter()
        ...
        assert_eq!(
            got[at],
            vec![
                QueryValue::Value(GraphValue::Edge(EId(eid))),
                QueryValue::Value(GraphValue::Vertex(VId(start))),
                list.clone(),
                list
            ]
        );
...
fn snapshot_pins_history_and_property_versions_survive_writer_compaction_and_drop() {
    ...
    assert_eq!(
        rows(&mut cursor),
        vec![vec![QueryValue::Count(4), QueryValue::Integer(14)]]
    );
    drop(cursor);
    let mut fresh = db
        .authorized_read_session(&cx, &issuer, &token, "main", symbols, policy(), || 100)
    ...
    assert_eq!(
        rows(&mut fresh.stream_aggregate(&cx, &prepared, &params).unwrap()),
        vec![vec![QueryValue::Count(1), QueryValue::Integer(16)]]
    );
    let history = format!(
        "MATCH (a)-[e:R]->(b) FOR SYSTEM_TIME AS OF SEQ {} RETURN COUNT(*) AS n,SUM(e.p) AS total",
        at.0
    );
...
fn source_and_signed_limits_refuse_without_partial_groups_even_under_zero_limit() {
    ...
    {
        let mut cursor = empty_budget
            .stream_aggregate(&cx, &prepared, &params)
            .unwrap();
        cursor.close();
        assert!(cursor.next().is_none());
    }
    ...
    let mut session = db
        .authorized_read_session(&cx, &issuer, &limited, "main", symbols, policy(), || 100)
        .unwrap();
    let prepared = session
        .prepare(
            &cx,
            "MATCH (a)-[e:R]->(b) RETURN a.p AS key,COUNT(*) AS n GROUP BY a.p",
            &params,
        )
        .unwrap();
    {
    ...
    for suffix in ["", " LIMIT 0"] {
        let prepared = fresh
            .prepare(
                &cx,
                &format!("MATCH (a)-[e:R]->(b) RETURN SUM(e.p) AS total{suffix}"),
                &params,
            )
            .unwrap();
        ...
        assert!(matches!(
            cursor.next(),
            Some(Err(QueryError::EdgeAggregateStream(GqlQueryError::Source(
...
impl EdgeScanSource for NoReads {
    ...
    fn next_edge<C>(
        ...
        panic!("denied relation opened root index")
    }
    fn edge<'a, C>(
        &'a self,
        _: EId,
        ...
        panic!("denied relation opened history")
    }
    fn vertex<'a, C>(
        &'a self,
        _: VId,
        ...
        panic!("denied relation opened endpoint")
    }
}
#[test]
fn denied_root_relations_open_no_directory_and_unscoped_access_refuses() {
    ...
    let execution: Shared<'_> = Rc::new(RefCell::new(Execution::new(
        &cx,
        verified.begin_read_at("main", 100).unwrap(),
        Box::new(|| 100),
    )));
    ...
    assert!(matches!(
        source.next_incident_edge(VId(0), GlaDirection::Forward, None, &mut control),
        Err(EdgeExpansionSourceError::Unavailable)
    ));
    assert!(matches!(
        source.next_edge(&mut control),
...
fn aggregate_edge_error_translation_keeps_terminal_and_arithmetic_causes() {
    ...
    assert!(matches!(
        source_error(EdgeScanError::ExpansionUnavailable),
        QueryError::EdgeStream(GqlQueryError::Source(EdgeScanError::ExpansionUnavailable))
    ));
}
