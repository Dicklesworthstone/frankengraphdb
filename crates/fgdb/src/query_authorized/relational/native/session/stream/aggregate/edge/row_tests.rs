...
const ROWS: &str =
    "MATCH (a)-[e:R]->(b) RETURN e AS edge,a AS start,b AS target,e.p AS weight,e.hidden AS masked";
...
fn row(id: u128, from: u128, to: u128, weight: Option<i64>) -> Vec<GraphValue> {
    vec![
        ...
        GraphValue::Vertex(VId(from)),
        GraphValue::Vertex(VId(to)),
        value(weight),
        value(None),
    ]
}
fn cells(cursor: &mut AuthorizedRowCursor<'_>) -> Vec<Vec<GraphValue>> {
    cursor
        .by_ref()
        .map(|row| row.unwrap().values().to_vec())
        .collect()
}
fn native(columns: &[String], rows: &[Vec<GraphValue>]) -> QueryResult {
    QueryResult::Rows {
        ...
        rows: rows
            .iter()
            .map(|row| row.iter().cloned().map(QueryValue::Value).collect())
            .collect(),
    }
}
macro_rules! context {
    ...
    for (direction, pattern) in [
        ...
        for (id, a, b, weight) in VISIBLE {
            match direction {
                ...
                _ => {
                    ...
                    if a != b {
                        expected.push(row(id, a.max(b), a.min(b), weight));
                    }
                }
            }
        }
        for quantifier in ["", "DISTINCT "] {
            for (skip, take) in [(0, 20), (1, 3), (3, 0), (20, 2)] {
                let text = format!(
                    "MATCH {pattern} RETURN {quantifier}e AS edge,a AS start,b AS target,e.p AS weight,e.hidden AS masked SKIP {skip} LIMIT {take}"
                );
                ...
                assert_eq!(
                    calls.get() - before,
                    count,
                    "hidden population changed live charging: {text}"
                );
            }
        }
    }
...
fn connected_rows_keep_captured_paths_and_rebind_scoped_predicates() {
    ...
    let eager = session.execute(&cx, &prepared, &params).unwrap();
    {
        ...
        let expected = [
            ...
            (11, 0, 12, 1, 3, Some(8)),
            (12, 1, 13, 3, 3, None),
            (12, 1, u128::MAX, 3, u128::MAX, Some(16)),
            (13, 3, 13, 3, 3, None),
            (13, 3, u128::MAX, 3, u128::MAX, Some(16)),
        ];
        ...
        for (actual, (first, start, second, middle, target, weight)) in got.iter().zip(expected) {
            assert_eq!(
                &actual[..7],
                &[
                    GraphValue::Edge(EId(first)),
                    GraphValue::Vertex(VId(start)),
                    GraphValue::Edge(EId(second)),
                    GraphValue::Vertex(VId(middle)),
                    GraphValue::Vertex(VId(target)),
                    value(weight),
                    value(None),
                ]
            );
            let GraphValue::Path(path) = &actual[7] else {
                panic!("native path was narrowed")
            };
            assert_eq!(path.start(), VId(start));
            assert_eq!(
                path.steps(),
                &[(EId(first), VId(middle)), (EId(second), VId(target))]
            );
        }
        assert_eq!(native(cursor.columns(), &got), eager);
    }
    ...
    let prepared = session
        .prepare(
            &cx,
            "MATCH (a)-[e:R]->(b) WHERE e.p > $wanted RETURN e AS edge,a AS start",
            &low,
        )
        .unwrap();
    for (args, ids) in [
        (&low, vec![EId(12), EId(u128::MAX)]),
        (&high, vec![]),
        (&low, vec![EId(12), EId(u128::MAX)]),
    ] {
        ...
        assert_eq!(
            ...
            ids.into_iter().map(GraphValue::Edge).collect::<Vec<_>>()
        );
    }
}
...
fn opening_checks_owner_branch_shape_and_cut_but_does_not_scan_the_unread_graph() {
    ...
    let mut zero = db
        ...
        .unwrap();
    assert!(matches!(
        zero.stream(&cx, &prepared, &params),
    ...
    assert!(matches!(
        zero.stream(&cx, &prepared, &params).unwrap().next(),
        Some(Err(QueryError::EdgeStream(GqlQueryError::Rows(_))))
    ));
    let empty = zero
        .prepare(&cx, &format!("{ROWS} LIMIT 0"), &params)
        .unwrap();
    assert!(zero.stream(&cx, &empty, &params).unwrap().next().is_none());
    let denied = zero
        .prepare(
            &cx,
            "MATCH (a)-[e:S]->(b) RETURN e AS edge,a AS start",
            &params,
        )
        .unwrap();
    ...
    let prepared = session.prepare(&cx, ROWS, &params).unwrap();
    {
        let mut cursor = session.stream(&cx, &prepared, &params).unwrap();
        assert_eq!(
            cursor.next().unwrap().unwrap().values(),
            row(10, 0, 1, Some(2))
        );
        assert!(matches!(
            cursor.next(),
    ...
    let first = session
        .prepare(&cx, &format!("{ROWS} LIMIT 1"), &params)
        .unwrap();
    assert_eq!(
        cells(&mut session.stream(&cx, &first, &params).unwrap()),
        vec![row(10, 0, 1, Some(2))]
    );
    let mut one = db
        .authorized_read_session(
            &cx,
            ...
            "main",
            symbols,
            GqlQueryPolicy::new(1, 1, 1_000_000, 1_000_000),
            || 100,
        )
        .unwrap();
    let first = one
        ...
        .unwrap();
    assert_eq!(
        cells(&mut one.stream(&cx, &first, &params).unwrap()),
        vec![row(10, 0, 1, Some(2))]
    );
}
...
fn paused_edge_cursor_retains_exact_history_after_writes_compaction_and_writer_drop() {
    ...
    assert_eq!(
        ...
        VISIBLE[1..]
            .iter()
            .map(|&(id, a, b, w)| row(id, a, b, w))
            .collect::<Vec<_>>()
    );
    drop(cursor);
    assert!(!session.is_closed());
    assert_eq!(
        cells(&mut fresh.stream(&cx, &now, &params).unwrap()),
        vec![
            row(13, 3, 3, Some(32)),
            row(u128::MAX, 3, u128::MAX, Some(16))
        ]
    );
    assert_eq!(
        cells(&mut fresh.stream(&cx, &old, &params).unwrap()),
        VISIBLE
            .iter()
            .map(|&(id, a, b, w)| row(id, a, b, w))
            .collect::<Vec<_>>()
...
fn every_expiry_cut_and_host_unwind_fuses_edge_delivery_and_closes_the_session() {
    ...
    let mut session = db
        .authorized_read_session(&cx, &issuer, &token, "main", symbols, policy(), || {
            assert!(!panic_now.get(), "host callback unwind");
            100
        })
        .unwrap();
    let prepared = session.prepare(&cx, ROWS, &params).unwrap();
