...
fn independent_shared_endpoint_appends_commit_once_and_reopen() {
    let ((), report) = run_async_under_lab(0xa99e_0001, |root| async move {
        ...
        let second = right
            .commit_append_only_rebased(&mut database, &cx, 1)
            .await
            .unwrap();
        ...
        assert_eq!(
            database.edge(EId(20)).unwrap().unwrap().entry.created_at,
            second
        );
    });
...
fn dependent_multi_relation_creations_preserve_births_and_exact_row_admission() {
    let ((), report) = run_async_under_lab(0xa99e_0002, |root| async move {
        ...
        for limit in [5, 6, 7] {
            ...
            transaction
                .write_ordered(&mut database, vec![a, b])
                .unwrap();
            ...
            let result = transaction
                .commit_append_only_rebased(&mut database, &cx, limit)
                .await;
            if limit == 5 {
                assert!(matches!(
                    result,
                ...
                assert_eq!(
                    database.edge(EId(50)).unwrap().unwrap().entry.relation,
                    RelationId(9)
                );
                assert_eq!(
                    database.edge(EId(60)).unwrap().unwrap().entry.relation,
                    RelationId(2)
                );
                assert_eq!(database.delta_since(frontier).unwrap().count(), 1);
            }
            assert_eq!(
                database.vertex(VId(1)).unwrap().unwrap().props,
    ...
    let ((), report) = run_async_under_lab(0xa99e_0003, |root| async move {
        ...
        for case in 0..6 {
            ...
            match case {
                ...
                1 => {
                    batch.ensure_edge_by_triple(EId(51), VId(1), VId(2), vec![]);
                }
                2 => {
            ...
            assert!(matches!(
                transaction
                    .commit_append_only_rebased(&mut database, &cx, 100)
                    .await,
...
fn wrong_owner_and_unpolled_future_preserve_the_real_owner_workspace() {
    let ((), report) = run_async_under_lab(0xa99e_0006, |root| async move {
        ...
        assert!(matches!(
            transaction
                .commit_append_only_rebased(&mut foreign, &cx, 1)
                .await,
...
fn every_prepublication_checkpoint_refusal_aborts_without_a_new_marker() {
    let ((), report) = run_async_under_lab(0xa99e_0007, |root| async move {
        ...
        transaction
            .complete_with_basis_controlled(&mut database, &cx, None, true, Some(1), || {
                checkpoints += 1;
                Ok(())
            })
            .await
...
fn rebased_completion_keeps_the_existing_unknown_outcome_fence() {
    ...
    let ((), report) = run_async_under_lab(0xa99e_0008, |root| async move {
        ...
        for crash in [CrashPoint::BeforeCapsule, CrashPoint::AfterMarkerBeforeD2] {
            ...
            let frontier = database.frontier().unwrap();
            let result = transaction
                .complete_with_basis_controlled(
                    &mut database,
                    &cx,
                    Some(crash),
                    true,
