...
fn keys() -> DatabaseKeys { DatabaseKeys::new([0x74; 32], NS, [0x76; 32]) }
fn authority() -> Authority {
    Authority::new(AuthKey::from_seed(9975), NS, "graph", SchemaEpoch(1), 1).unwrap()
}
fn grant() -> Grant {
    Grant {
        branch: "main".into(), labels: Scope::only([L, ACTIVE]),
        relations: Scope::only([R]), properties: Scope::only([P, Q]),
        rights: Rights::ReadWrite,
        limits: QueryLimits { max_nodes: 100_000, max_work: 1_000_000, max_rows: 0 },
...
fn auth_error(error: Fault) -> Error {
    match error {
        ...
        | Fault::Interrupted { source: WriteTxnError::Authorization(error), .. }
        | Fault::Statement { source: GqlQueryError::Interrupted(WriteTxnError::Authorization(error)), .. }
        | Fault::Statement { source: GqlQueryError::Source(GraphMutationError::Source(WriteTxnError::Authorization(error))), .. } => error,
        other => panic!("unexpected error: {other:?}"),
    }
}
fn lab<T, F>(seed: u64, body: impl FnOnce(PurposeContexts) -> F + Send + 'static) -> T
where T: Send + 'static, F: std::future::Future<Output = T> + Send + 'static {
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx, hidden: bool) {
    ...
    batch.create_vertex(VId(2), if hidden { vec![L, HIDDEN] } else { vec![L] },
...
fn dependent_statements_see_canonical_masked_effects_and_publish_once() {
    lab(0xa975, |contexts| async move {
        ...
        let hidden = (db.vertex(VId(3)).unwrap(), db.edge(EId(13)).unwrap(), db.edge(EId(21)).unwrap());
        let (stats, completion) = db.execute_graph_mutation_program_authorized(
            &txn, &query, &commit, &authority, &token, "main", &dependent(), policy(), || NOW,
        ).await.unwrap();
        assert_eq!((stats.completed_statements, stats.effects, stats.selection.result_rows), (3, 7, 4));
        assert_eq!(db.vertex(VId(1)).unwrap().unwrap().props,
            vec![(P, CanonicalScalar::Int(11)), (Q, CanonicalScalar::Int(10)), (SECRET, CanonicalScalar::Int(77))]);
        assert_eq!(db.vertex(VId(2)).unwrap().unwrap().props,
            vec![(P, CanonicalScalar::Int(110)), (Q, CanonicalScalar::Int(10))]);
        for id in [11, 12] {
            assert_eq!(db.edge(EId(id)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(11))]);
        }
        assert_eq!((db.vertex(VId(3)).unwrap(), db.edge(EId(13)).unwrap(), db.edge(EId(21)).unwrap()), hidden);
        let seq = db.frontier().unwrap(); assert_eq!(seq.0, before.0 + 1);
...
fn forbidden_or_conflicting_tail_discards_prior_statements() {
    lab(0xa976, |contexts| async move {
        ...
        for tail in [
            ...
            let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit, true).await;
            ...
            let error = db.execute_graph_mutation_program_authorized(
...
fn native_program_quotas_accumulate_even_when_effects_cancel() {
    lab(0xa977, |contexts| async move {
        ...
        for selected_limit in [false, true] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit, false).await;
            let before = (db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap());
            let limited = if selected_limit {
                GraphMutationPolicy::new(GqlQueryPolicy::new(10_000, 1, 1_000_000, 100_000), 100)
            } else { GraphMutationPolicy { max_effects: 1, ..policy() } };
            let error = db.execute_graph_mutation_program_authorized(
                &txn, &query, &commit, &authority, &token, "main", &program, limited, || NOW,
            ).await.unwrap_err();
            assert!(matches!(error, Fault::Budget { statement: 1, dimension: GraphMutationProgramDimension::Effects | GraphMutationProgramDimension::SelectedRows, .. }));
            assert_eq!((db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap()), before);
            assert_eq!(txn.outstanding_obligations(), 0);
        }
    });
...
fn deleted_incidence_never_reappears_in_a_later_match() {
    lab(0xa978, |contexts| async move {
        ...
        let mut full = grant(); full.labels = Scope::All; full.relations = Scope::All; full.properties = Scope::All;
        ...
        let (stats, _) = db.execute_graph_mutation_program_authorized(
            &txn, &query, &commit, &authority, &token, "main", &program, policy(), || NOW,
        ).await.unwrap();
...
fn empty_program_execution_read_closes_but_still_requires_readwrite() {
    lab(0xa979, |contexts| async move {
        let commit = contexts.commit(); let query = contexts.query(); let txn = contexts.txn();
        let authority = authority();
        let program = program(&["MATCH (a:Visible) WHERE a.p = 999 SET a.p = 1", "MATCH (a:Hidden) SET a.p = 2"]);
        let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit, true).await;
        let before = (db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap());
...
fn hidden_overlay_data_changes_neither_signed_thresholds_nor_visible_stats() {
    lab(0xa97a, |contexts| async move {
        ...
        for dimension in [LimitDimension::Work, LimitDimension::Nodes] {
            let mut floors = Vec::new(); let mut successful_stats = Vec::new();
            for hidden in [false, true] {
                ...
                while low < high {
                    ...
                    let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit, hidden).await;
                    let before = (db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap());
                    match db.execute_graph_mutation_program_authorized(
                        &txn, &query, &commit, &authority, &limited, "main", &dependent(), policy(), || NOW,
                    ).await {
                        Ok((stats, _)) => { high = middle; successful_stats.push(stats); }
                        Err(error) => {
                ...
                assert!(low > 0 && low < 8192); floors.push(low);
            }
            assert_eq!(floors[0], floors[1], "hidden overlay changed {dimension:?}");
            assert!(successful_stats.iter().all(|stats| *stats == successful_stats[0]));
        }
    });
}
...
fn expiry_during_program_or_final_validation_never_commits_a_prefix() {
    lab(0xa97b, |contexts| async move {
        ...
        db.execute_graph_mutation_program_authorized(
            &txn, &query, &commit, &authority, &token, "main", &dependent(), policy(), || {
