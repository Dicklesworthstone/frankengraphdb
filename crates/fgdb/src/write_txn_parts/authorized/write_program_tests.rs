...
use fgdb_gql::{
    ...
    PreparedGraphEdgeMergeText, PreparedGraphVertexMerge, PreparedGraphVertexMergeText,
    PreparedGraphVertexUpsert, PreparedGraphVertexUpsertText, GraphVertexMergeError,
    GraphVertexMergeOutcome, GraphVertexUpsertAction, GraphVertexUpsertError,
};
...
fn authorization(error: Fault) -> Error {
    ...
    match error {
        ...
        | Fault::Delete { source: GqlQueryError::Source(GraphDeleteError::Source(WriteTxnError::Authorization(error))), .. }
        | Fault::VertexMerge { source: GqlQueryError::Interrupted(WriteTxnError::Authorization(error)), .. }
        | Fault::VertexMerge { source: GqlQueryError::Source(GraphVertexMergeError::Source(WriteTxnError::Authorization(error))), .. }
        | Fault::VertexMerge { source: GqlQueryError::Source(GraphVertexMergeError::Creation(GraphInsertError::Source(WriteTxnError::Authorization(error)))), .. }
        | Fault::VertexUpsert { source: GqlQueryError::Interrupted(WriteTxnError::Authorization(error)), .. }
        | Fault::VertexUpsert { source: GqlQueryError::Source(GraphVertexUpsertError::Staging(WriteTxnError::Authorization(error))), .. }
        | Fault::VertexUpsert { source: GqlQueryError::Source(GraphVertexUpsertError::Merge(GraphVertexMergeError::Source(WriteTxnError::Authorization(error)))), .. }
        | Fault::VertexUpsert { source: GqlQueryError::Source(GraphVertexUpsertError::Merge(GraphVertexMergeError::Creation(GraphInsertError::Source(WriteTxnError::Authorization(error))))), .. } => error,
    ...
    lab(0xa986, |contexts| async move {
        ...
        for mode in 0..3 {
            ...
            let tail = match mode {
                ...
                _ => PreparedGraphEdgeMergeText::prepare("MATCH (a:Visible), (b:Visible) MERGE (a)-[:R]->(b)", R, symbols).unwrap()
                    .bind_parameters(&GqlParameters::new()).unwrap().into(),
            };
...
fn scoped_merge_grant() -> Grant {
    Grant { labels: Scope::only([L]), properties: Scope::only([P, Q]), ..grant() }
}
async fn seed_merge(db: &mut Database<MemVfs>, cx: &CommitCx, hidden: bool) {
    ...
    if hidden { props.push((SECRET, CanonicalScalar::Int(77))); }
    batch.create_vertex(VId(1), vec![L], props);
    if hidden {
        batch.create_vertex(VId(900), vec![LabelId(9)], vec![(P, CanonicalScalar::Int(7))]);
...
fn vertex_merge_and_branch_actions_observe_prior_steps_publish_once_and_reopen() {
    lab(0xa991, |contexts| async move {
        let commit = contexts.commit(); let query = contexts.query(); let txn = contexts.txn();
        let vfs = MemVfs::new().unwrap(); let path = vfs.database_dir();
        let mut db = Database::create_with_vfs(&commit, vfs.clone(), &path, keys()).await.unwrap();
        let before = db.frontier().unwrap();
        let authority = authority(); let token = authority.issue_at(&scoped_merge_grant(), NOW).unwrap();
        let (receipt, completion) = db.execute_graph_write_program_returning_authorized(
            &txn, &query, &commit, &authority, &token, "main", &merge_program(), policy(), || NOW,
        ).await.unwrap();
        assert_eq!((receipt.stats().completed_statements, receipt.stats().created_vertices,
            receipt.stats().mutation_effects), (3, 1, 1));
        assert_eq!(receipt.steps(), &[
            GraphWriteStepReceipt::VertexMerge { outcome: GraphVertexMergeOutcome::Created(VId(1)) },
            GraphWriteStepReceipt::VertexUpsert { outcome: GraphVertexMergeOutcome::Matched(VId(1)) },
            GraphWriteStepReceipt::VertexMerge { outcome: GraphVertexMergeOutcome::Matched(VId(1)) },
        ]);
        let seq = db.frontier().unwrap();
        assert_eq!(seq.0, before.0 + 1);
        assert_eq!(completion, EmbeddedTxnCompletion::WriteCommitted { commit_seq: seq });
        ...
        let matched = PreparedGraphWriteProgram::prepare(vec![merge_step("MERGE (a:Visible {p:7})")]).unwrap();
        let limited = GraphWriteProgramPolicy { max_created_vertices: 0, ..policy() };
        let (receipt, completion) = db.execute_graph_write_program_returning_authorized(
            &txn, &query, &commit, &authority, &token, "main", &matched, limited, || NOW,
        ).await.unwrap();
        assert_eq!(receipt.steps()[0].merged_vertex(), Some(GraphVertexMergeOutcome::Matched(VId(1))));
        assert_eq!(completion, EmbeddedTxnCompletion::ReadClosed { snapshot_seq: seq, validated_through: seq });
        assert_eq!(db.allocate_identity(&query, GraphInsertRequest::Vertex { row: 0, vertex: 0 }).unwrap(), ElementId::Vertex(VId(2)));
        assert_eq!(txn.outstanding_obligations(), 0);
    });
}
...
fn vertex_upsert_executes_only_the_selected_branch_and_preserves_hidden_fields() {
    lab(0xa992, |contexts| async move {
        ...
        for existing in [false, true] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            if existing { seed_merge(&mut db, &commit, true).await; }
            ...
            if existing { expected.push((SECRET, CanonicalScalar::Int(77))); }
...
fn hidden_matching_vertices_neither_suppress_creation_nor_make_a_match_ambiguous() {
    lab(0xa993, |contexts| async move {
        ...
        for visible in [false, true] {
            ...
            let mut batch = crate::WriteBatch::new(R);
            batch.create_vertex(VId(900), vec![LabelId(9)], vec![(P, CanonicalScalar::Int(7))]);
            if visible { batch.create_vertex(VId(1), vec![L], vec![(P, CanonicalScalar::Int(7))]); }
            db.write(&commit, batch).await.unwrap();
            let hidden = db.vertex(VId(900)).unwrap();
            let program = PreparedGraphWriteProgram::prepare(vec![merge_step("MERGE (a:Visible {p:7})")]).unwrap();
            let (receipt, _) = db.execute_graph_write_program_returning_authorized(
                &txn, &query, &commit, &authority, &token, "main", &program, policy(), || NOW,
            ).await.unwrap();
            ...
            if visible { assert_eq!(outcome.vertex(), VId(1)); }
...
fn duplicate_match_occurrences_collapse_but_distinct_vertices_abort_the_whole_program() {
    lab(0xa994, |contexts| async move {
        let commit = contexts.commit(); let query = contexts.query(); let txn = contexts.txn();
        let authority = authority(); let token = authority.issue_at(&grant(), NOW).unwrap();
        let GraphWriteStatement::VertexMerge(base) = merge_step("MERGE (a:Visible {p:7})") else { unreachable!() };
        let GraphWriteStatement::Mutation(selection) = mutation(
            "MATCH (a:Visible)-[:R]->(b:Visible) WHERE a.p = 7 SET a.p = 7",
        ) else { unreachable!() };
        let merge = PreparedGraphVertexMerge::prepare(selection.selection().clone(), R, 0, base.creation().clone()).unwrap();
        let program = PreparedGraphWriteProgram::prepare(vec![
            insert("INSERT (prefix:Visible {p:123})"), merge.into(),
        ]).unwrap();
        for ambiguous in [false, true] {
            ...
            if ambiguous {
                assert!(matches!(result, Err(Fault::VertexMerge { statement: 1,
                    source: GqlQueryError::Source(GraphVertexMergeError::AmbiguousMatches { observed: 2 }),
                })));
                assert_eq!((db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap()), before);
            } else {
                let (receipt, _) = result.unwrap();
                assert_eq!(receipt.steps()[1].merged_vertex(), Some(GraphVertexMergeOutcome::Matched(VId(1))));
                assert_eq!(receipt.stats().created_vertices, 1, "only the explicit prefix was created");
            }
            assert_eq!(txn.outstanding_obligations(), 0);
        }
    });
...
fn forbidden_merge_creation_and_selected_actions_or_scope_escape_roll_back_every_step() {
    lab(0xa995, |contexts| async move {
        ...
        for mode in 0..4 {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            if mode >= 2 { seed_merge(&mut db, &commit, true).await; }
            let before = (db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap());
            let tail = match mode {
                ...
                _ => {
                    let GraphWriteStatement::VertexMerge(base) = merge_step("MERGE (a:Visible {p:7})") else { unreachable!() };
                    PreparedGraphVertexUpsert::prepare(base,
                        vec![GraphVertexUpsertAction::SetLabel { label: L, present: false }], vec![]).unwrap().into()
                }
            };
            let program = PreparedGraphWriteProgram::prepare(vec![insert("INSERT (prefix:Visible {p:123})"), tail]).unwrap();
            let error = db.execute_graph_write_program_returning_authorized(
                &txn, &query, &commit, &authority, &token, "main", &program, policy(), || NOW,
            ).await.unwrap_err();
            assert_eq!(authorization(error), fgdb_warden::Error::ScopeDenied, "mode={mode}");
            assert_eq!((db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap()), before);
            assert_eq!(txn.outstanding_obligations(), 0);
        }
    });
...
fn merge_receipts_cost_one_row_per_outcome_and_stats_only_cost_no_rows() {
    lab(0xa996, |contexts| async move {
        ...
        for rows in 0..=3 {
            ...
            let limited = token.attenuate(Restriction::MaxRows(rows)).unwrap();
            let result = db.execute_graph_write_program_returning_authorized(
                &txn, &query, &commit, &authority, &limited, "main", &merge_program(), policy(), || NOW,
            ).await;
            if rows < 3 {
                assert_eq!(authorization(result.unwrap_err()), fgdb_warden::Error::LimitExceeded(LimitDimension::Rows));
                assert_eq!(db.frontier().unwrap(), before);
                assert!(db.vertices().unwrap().is_empty());
            } else { assert_eq!(result.unwrap().0.steps().len(), 3); }
            assert_eq!(txn.outstanding_obligations(), 0);
        }
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let zero = token.attenuate(Restriction::MaxRows(0)).unwrap();
        let (stats, _) = db.execute_graph_write_program_authorized(
            &txn, &query, &commit, &authority, &zero, "main", &merge_program(), policy(), || NOW,
        ).await.unwrap();
    ...
    lab(0xa997, |contexts| async move {
        ...
        for actions in [false, true] {
            ...
            let (program, limited) = if actions {
                ...
                ]).unwrap(), GraphWriteProgramPolicy::new(policy().mutations.query, 1, 100, 100))
            } else {
                (PreparedGraphWriteProgram::prepare(vec![
                    merge_step("MERGE (a:Visible {p:7})"), merge_step("MERGE (b:Visible {p:8})"),
                ]).unwrap(), GraphWriteProgramPolicy { max_created_vertices: 1, ..policy() })
            };
            let error = db.execute_graph_write_program_authorized(
                &txn, &query, &commit, &authority, &token, "main", &program, limited, || NOW,
            ).await.unwrap_err();
            if actions {
                assert!(matches!(error, Fault::Program(GraphMutationProgramError::Budget {
                    statement: 1, dimension: fgdb_gql::GraphMutationProgramDimension::Effects,
                    limit: 1, observed: 2,
                })));
            } else {
                assert!(matches!(error, Fault::CreationBudget {
                    statement: 1, dimension: GraphInsertLimitDimension::Vertices, limit: 1, observed: 2,
                }));
            }
            assert_eq!(db.frontier().unwrap(), before);
            assert!(db.vertices().unwrap().is_empty());
...
fn merge_and_upsert_require_readwrite_before_a_prefix_can_allocate() {
    lab(0xa998, |contexts| async move {
        ...
        for rights in [Rights::Read, Rights::Write] {
            for tail in [merge_step("MERGE (a:Visible {p:7})"),
                upsert_step("MERGE (a:Visible {p:7}) ON CREATE SET a.q = 8")] {
                let mut db = Database::open_memory(&commit, keys()).await.unwrap();
                let before = db.frontier().unwrap();
                let mut grant = grant(); grant.rights = rights; grant.limits.max_work = 0;
                let token = authority.issue_at(&grant, NOW).unwrap();
                let program = PreparedGraphWriteProgram::prepare(vec![insert("INSERT (prefix:Visible)"), tail]).unwrap();
                let error = db.execute_graph_write_program_authorized(
                    &txn, &query, &commit, &authority, &token, "main", &program, policy(), || NOW,
                ).await.unwrap_err();
...
fn hidden_merge_data_changes_neither_stats_nor_work_and_node_refusal_thresholds() {
    lab(0xa999, |contexts| async move {
        ...
        for dimension in [LimitDimension::Work, LimitDimension::Nodes] {
            let mut floors = Vec::new(); let mut stats_seen = Vec::new();
            for hidden in [false, true] {
                ...
                while low < high {
                    ...
                    match db.execute_graph_write_program_authorized(
                        ...
                        Ok((stats, _)) => { high = middle; stats_seen.push(stats); }
                        Err(error) => {
                            assert_eq!(authorization(error), fgdb_warden::Error::LimitExceeded(dimension));
                            assert_eq!((db.frontier().unwrap(), db.vertices().unwrap(), db.edges().unwrap()), before);
                            low = middle + 1;
                        }
                    }
                ...
                assert!(low > 0 && low < 4096); floors.push(low);
            }
        ...
        let matched = PreparedGraphWriteProgram::prepare(vec![
            merge_step("MERGE (a:Visible {p:7})"), merge_step("MERGE (a:Visible {p:7})"),
        ]).unwrap();
        for nodes in [1, 2] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            seed_merge(&mut db, &commit, true).await;
            let limited = token.attenuate(Restriction::MaxNodes(nodes)).unwrap();
            let result = db.execute_graph_write_program_authorized(
                &txn, &query, &commit, &authority, &limited, "main", &matched, policy(), || NOW,
            ).await;
            if nodes == 1 {
                assert_eq!(authorization(result.unwrap_err()), fgdb_warden::Error::LimitExceeded(LimitDimension::Nodes));
            } else { assert_eq!(result.unwrap().0.created_vertices, 0); }
            assert_eq!(txn.outstanding_obligations(), 0);
        }
    });
}
...
fn expiry_during_merge_creation_branch_actions_or_final_validation_rolls_back() {
    lab(0xa99a, |contexts| async move {
        ...
        let calls = AtomicU64::new(0);
        db.execute_graph_write_program_returning_authorized(
            &txn, &query, &commit, &authority, &token, "main", &merge_program(), policy(), || {
                calls.fetch_add(1, Ordering::Relaxed); NOW
            },
        ).await.unwrap();
        let count = calls.load(Ordering::Relaxed); assert!(count > 10);
        for cutoff in [1, count / 3, 2 * count / 3, count - 1] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let before = db.frontier().unwrap(); let calls = AtomicU64::new(0);
            let error = db.execute_graph_write_program_returning_authorized(
                &txn, &query, &commit, &authority, &token, "main", &merge_program(), policy(), || {
                    if calls.fetch_add(1, Ordering::Relaxed) < cutoff { NOW } else { 10_000 }
                },
