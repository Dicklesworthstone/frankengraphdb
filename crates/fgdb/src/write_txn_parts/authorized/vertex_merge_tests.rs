...
fn scoped_merge_creates_once_matches_without_a_commit_and_reopens() {
    lab(0xa981, |contexts| async move {
        ...
        let (stats, first, completion) = db
            .execute_graph_vertex_merge_authorized(
                &txn,
                &query,
                &commit,
                &authority,
        ...
        let (stats, second, completion) = db
            .execute_graph_vertex_merge_authorized(
                &txn,
                &query,
                &commit,
...
fn ambiguity_counts_only_visible_identities_and_never_allocates_or_publishes() {
    lab(0xa982, |contexts| async move {
        ...
        for visible in [1_u128, 2] {
            ...
            for id in 1..=visible {
                seed(&mut db, &commit, vec![L], id).await;
            }
            for id in 100..104 {
                seed(&mut db, &commit, vec![HIDDEN], id).await;
            }
            let basis = db.frontier().unwrap();
    ...
    lab(0xa985, |contexts| async move {
        ...
        for dimension in [
            ...
            for below in [false, true] {
                ...
                let bounded = GraphVertexMergePolicy::new(GqlQueryPolicy::new(
                    10_000,
                    10_000,
                    max_work,
                    max_scratch,
                ...
                if below {
                    match result {
                        Err(GqlQueryError::Evaluator(error)) => {
                            ...
                            assert_eq!(
                                error.limit,
                                if dimension == GlaLimitDimension::WorkUnits {
                                    max_work
                                } else {
                                    max_scratch
                                }
...
fn expiry_at_every_live_check_refuses_before_publication_and_releases_the_pin() {
    lab(0xa986, |contexts| async move {
        ...
        for existing in [false, true] {
            let mut baseline = Database::open_memory(&commit, keys()).await.unwrap();
            if existing {
                seed(&mut baseline, &commit, vec![L], 1).await;
            }
            ...
            for expire_at in 1..=checks {
                let mut db = Database::open_memory(&commit, keys()).await.unwrap();
                if existing {
                    seed(&mut db, &commit, vec![L], 1).await;
                }
                let basis = db.frontier().unwrap();
                let rows = db.vertices().unwrap();
                let mut calls = 0_u64;
                let result = db
                    .execute_graph_vertex_merge_authorized(
                        ...
                        move || {
                            calls += 1;
                            if calls >= expire_at { EXPIRES + 1 } else { NOW }
                        },
                    )
                    .await;
                assert!(
                    result.is_err(),
                    "expiry check {expire_at}/{checks}, matched={existing}"
                );
                assert_eq!(db.frontier().unwrap(), basis);
                assert_eq!(db.vertices().unwrap(), rows);
                assert_eq!(txn.outstanding_obligations(), 0);
    ...
    lab(0xa987, |contexts| async move {
        ...
        assert!(matches!(
            ...
            Err(WriteTxnError::Write(
                crate::WriteError::FirstCommitterWins {
