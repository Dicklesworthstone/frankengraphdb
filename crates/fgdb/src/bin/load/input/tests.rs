...
fn fixture(bytes: &[u8]) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("fgdb-checked-input-{}-{}",
...
fn open(path: &Path) -> Input {
    Input::open_controlled(path, b"bindings", MAX_INPUT_BYTES, MAX_RECORD_BYTES,
        1_000_000, &mut || Ok(())).unwrap()
}
...
fn standard_line_semantics_across_blocks_unicode_and_independent_clones() {
    for text in ["", "\n", "\r", "a\r\n\r\nx\n", "a\nb\r"].map(str::to_owned)
        .into_iter().chain([BLOCK_BYTES - 2, BLOCK_BYTES - 1, BLOCK_BYTES, BLOCK_BYTES + 1]
            .map(|n| format!("{}λ\r\nnext\n\r\nlast\r", "x".repeat(n)))) {
...
fn exact_source_record_and_count_bounds_are_checked_during_sealing() {
    ...
    let make = |bytes, record, count| Input::open_controlled(&path, b"", bytes, record,
        count, &mut || Ok(()));
    assert!(make(8, 4, 2).is_ok());
    for (bytes, record, count, expected) in [
        (7, 4, 2, "input_bytes"), (8, 3, 2, "record_bytes"), (8, 4, 1, "source_rows"),
    ] {
    ...
    assert!(Input::open_controlled(&long, b"", u64::MAX, BLOCK_BYTES, 1,
        &mut || Ok(())).is_err());
}
...
fn complete_hash_keeps_the_existing_raw_bytes_plus_bindings_transcript() {
    ...
    let changed = Input::open_controlled(&path, b"other", MAX_INPUT_BYTES,
        MAX_RECORD_BYTES, 10, &mut || Ok(())).unwrap();
...
fn changed_uncached_blocks_are_rejected_instead_of_trusting_clone() {
    ...
    assert_eq!(reader.next_controlled(&mut || Ok(())).unwrap().as_deref(), Some("first"));
    ...
    for reader in [&mut reader, &mut cloned] {
        assert!(reader.next_controlled(&mut || Ok(())).unwrap_err().to_string().contains("SourceChanged"));
        assert!(reader.next_controlled(&mut || panic!("terminal reader must not perform I/O")).unwrap().is_none());
    }
}
...
fn a_clone_revalidates_the_previously_cached_block() {
    ...
    assert!(cloned.next_controlled(&mut || Ok(())).unwrap_err().to_string().contains("block changed"));
    // Already retained immutable bytes still denote the sealed source image.
    assert_eq!(reader.next_controlled(&mut || Ok(())).unwrap().as_deref(), Some("second"));
}
...
fn length_drift_is_checked_at_eof_as_well_as_at_first_fetch() {
    for truncate in [false, true] {
        ...
        if truncate {
            std::fs::OpenOptions::new().write(true).open(&path).unwrap().set_len(2).unwrap();
        } else {
            std::fs::OpenOptions::new().append(true).open(&path).unwrap().write_all(b"extra\n").unwrap();
        }
        for reader in [&mut at_end, &mut fresh] {
            assert!(reader.next_controlled(&mut || Ok(())).unwrap_err().to_string().contains("length changed"));
...
fn utf8_failure_retains_its_absolute_line_and_fuses() {
    ...
    let input = open(&path);
    let mut reader = input.reader();
    assert_eq!(reader.next_controlled(&mut || Ok(())).unwrap().as_deref(), Some("ok"));
    assert_eq!(reader.line(), 2);
    assert!(reader.next_controlled(&mut || Ok(())).unwrap_err().to_string().contains("UTF-8"));
    assert_eq!(reader.line(), 2);
    assert!(reader.clone().next_controlled(&mut || Ok(())).unwrap().is_none());
}
...
fn every_io_checkpoint_can_stop_sealing_or_reading_without_a_successful_prefix() {
    ...
    let mut calls = 0;
    Input::open_controlled(&path, b"", MAX_INPUT_BYTES, MAX_RECORD_BYTES, 10,
        &mut || { calls += 1; Ok(()) }).unwrap();
    for stop in 1..=calls {
        let mut seen = 0;
        let result = Input::open_controlled(&path, b"", MAX_INPUT_BYTES, MAX_RECORD_BYTES,
            10, &mut || { seen += 1; if seen == stop { Err(io::Error::from(io::ErrorKind::Interrupted)) } else { Ok(()) } });
        assert!(result.is_err());
        assert_eq!(seen, stop);
    }
    let input = open(&path);
    let mut calls = 0;
    input.reader().next_controlled(&mut || { calls += 1; Ok(()) }).unwrap();
    for stop in 1..=calls {
        let mut reader = input.reader();
        let mut seen = 0;
        ...
        assert!(result.is_err());
        assert_eq!(seen, stop);
        assert!(reader.next_controlled(&mut || panic!("must remain terminal")).unwrap().is_none());
    }
...
fn generated_source_retains_only_a_block_cache_and_seal_metadata() {
    ...
    let record = format!("{}\n", "x".repeat(10_000));
    for _ in 0..1000 { file.write_all(record.as_bytes()).unwrap(); }
    drop(file);
    let input = open(&path);
    ...
    assert_eq!(input.image.blocks.len(), (record.len() * 1000).div_ceil(BLOCK_BYTES));
}
