...
fn diff_flags_are_exact_required_unique_and_do_not_change_other_commands() {
    for (before, after) in [(0, 0), (u64::MAX, 0), (1, u64::MAX)] {
        ...
        assert_eq!(parsed.diff.output_bytes.unwrap_or(DEFAULT_OUTPUT_BYTES), 16 * 1024 * 1024);
    }
    for flag in ["--before", "--after", "--max-snapshot-records", "--max-result-rows",
        "--max-work-units", "--max-scratch-entries", "--max-output-bytes"] {
    ...
    let ((), report) = run_async_under_lab(0xd1ff_0101, |root| async move {
        ...
        for distinct in [false, true] {
            let query = if distinct { "MATCH (n:L) RETURN DISTINCT n.score AS score" }
                else { "MATCH (n:L) RETURN ALL n.score AS score" };
...
fn unchanged_results_keep_schema_and_large_revision_metadata_is_decimal_text() {
    ...
    let mut bytes = Vec::new();
    okay(render(&diff, true, DEFAULT_OUTPUT_BYTES, &mut bytes, &mut || Ok(())));
    let output = String::from_utf8(bytes).unwrap();
    ...
    fn write(&mut self, bytes: &[u8]) -> std::io::Result<usize> {
        let count = if let Some(limit) = self.fail_after_bytes {
            if self.bytes.len() >= limit {
                return Err(std::io::Error::new(std::io::ErrorKind::BrokenPipe, "receiver closed"));
            }
...
fn output_bytes_are_utf8_cumulative_and_admit_whole_frames_before_any_write() {
    ...
    let promoted = ZWeight::from_i128(i128::MAX)
        .checked_mul_i128(2, fgdb_delta_types::LimbLimit::new(4)).unwrap();
    ...
    let mut output = Output { fail_flush: Some(1), ..Output::default() };
    used = 0;
    assert_eq!(frame(&mut output, "abc", &mut used, 4).err().unwrap().code, 5);
...
fn every_delivery_checkpoint_and_broken_output_is_terminal_with_an_honest_prefix() {
    let ((), report) = run_async_under_lab(0xd1ff_0201, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut seed = WriteBatch::new(RelationId(1));
        for (id, value) in [(1, 2), (2, 5), (3, 8)] { add(&mut seed, id, value); }
        db.write(&commit, seed).await.unwrap();
        let opts = options("MATCH (n:L) RETURN ALL n.score AS score", 0, 1);
        let diff = db.query_diff(&cx, &opts.text, &opts.params, &opts,
            CommitSeq(0), CommitSeq(1), policy()).unwrap();
        let mut expected = Output::default(); let mut calls = 0;
        okay(render(&diff, true, u64::MAX, &mut expected, &mut || { calls += 1; Ok(()) }));
        ...
        for stop in 1..=calls {
            let mut visited = 0; let mut out = Output::default();
            let error = render(&diff, true, u64::MAX, &mut out, &mut || {
                visited += 1;
                if visited == stop { Err(Failure::query("cancelled delivery")) } else { Ok(()) }
            }).err().expect("injected cancellation");
            ...
            assert_eq!(delivered, wanted[..delivered.len()].iter().map(String::as_str).collect::<Vec<_>>());
        ...
        for fail in 1..=expected.flushes {
            let mut out = Output { fail_flush: Some(fail), ..Output::default() };
            let error = render(&diff, true, u64::MAX, &mut out, &mut || Ok(())).err().unwrap();
            assert_eq!(error.code, 5);
            assert_eq!(out.flushes, fail);
            assert_eq!(out.bytes, frames[..fail].concat().as_bytes());
            let sent = fail.saturating_sub(2).min(3);
            assert!(error.message.starts_with(&format!("diff incomplete after {sent} fully flushed change(s)")));
        ...
        for stop in 0..expected.bytes.len() {
            let mut out = Output { fail_after_bytes: Some(stop), ..Output::default() };
            let error = render(&diff, true, u64::MAX, &mut out, &mut || Ok(())).err().unwrap();
            assert_eq!(error.code, 5);
...
fn cli_delivery_limit_includes_summary_without_masking_endpoint_errors() {
    let ((), report) = run_async_under_lab(0xd1ff_0202, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut seed = WriteBatch::new(RelationId(1));
        seed.create_vertex(VId(1), vec![LabelId(1)], vec![(PropertyKeyId(1),
        ...
        for robot in [false, true] {
            ...
            for limit in [0, 1, size - 1] {
                ...
                if limit < 2 { assert!(out.bytes.is_empty()); }
                let text = String::from_utf8(out.bytes).unwrap();
                assert!(!text.contains(r#""event":"result""#));
...
fn native_diff_delivery_preserves_complete_queries_and_exact_domains_after_reopen() {
    ...
    fn bag(result: QueryResult) -> Bag {
        let QueryResult::Rows { rows, .. } = result else { panic!("a native read returned writes") };
        let mut bag = Bag::new();
    ...
    let ((), report) = run_async_under_lab(0xd1ff_0203, |root| async move {
        ...
        for (id, score, bucket) in [(0, Some(7), Some(1)), (1, Some(7), Some(1)),
            ...
            if let Some(value) = score { props.push((PropertyKeyId(1), CanonicalScalar::Int(value))); }
            if let Some(value) = bucket { props.push((PropertyKeyId(2), CanonicalScalar::Int(value))); }
            seed.create_vertex(VId(id), vec![LabelId(1)], props);
        }
        for id in [0, u128::MAX] {
            seed.add_edge(EId(id), VId(0), VId(1), vec![(PropertyKeyId(1), CanonicalScalar::Int(5))]);
        }
        let basis = db.write(&commit, seed).await.unwrap(); let old = db.read_session().unwrap();
        let mut edit = WriteBatch::new(RelationId(1));
        edit.set_vertex_property(VId(1), PropertyKeyId(1), Some(CanonicalScalar::Int(9)));
        edit.set_vertex_property(VId(u128::MAX), PropertyKeyId(2), Some(CanonicalScalar::Int(2)));
        edit.delete_edge(EId(0));
        edit.add_edge(EId(3), VId(1), VId(2), vec![(PropertyKeyId(1), CanonicalScalar::Int(-4))]);
        let at = db.write(&commit, edit).await.unwrap(); let new = db.read_session().unwrap();
        ...
        for text in [
            ...
            for reverse in [false, true] {
                ...
                argv.extend(["--relation".into(), "R=1".into(), "--property".into(), "bucket=2".into()]);
                if text.contains("$floor") { argv.extend(["--param".into(), "floor=int:8".into()]); }
                ...
                for (name, raw) in &opts.raw_params {
                    opts.params.insert(name, okay(crate::parameter(raw, None))).unwrap();
                }
                let (left, right) = if reverse { (&new, &old) } else { (&old, &new) };
                let before_rows = bag(left.query(&cx, text, &opts.params, &opts, policy()).unwrap());
                let mut expected = bag(right.query(&cx, text, &opts.params, &opts, policy()).unwrap());
                for (row, count) in before_rows { *expected.entry(row).or_default() -= count; }
                expected.retain(|_, count| *count != 0);
                let expected_lines: Vec<_> = expected.iter().map(|(row, count)| {
                    let cells: Vec<_> = row.iter().map(|value| okay(crate::cell(value))).collect();
                    format!(r#"{{"v":1,"event":"change","weight":"{count}","cells":[{}]}}"#, cells.join(","))
                }).collect();
                let mut bytes = Vec::new();
                okay(run(&db, &cx, &opts, true, &mut bytes));
                let result = String::from_utf8(bytes).unwrap();
                assert_eq!(changes(&result), expected_lines.iter().map(String::as_str).collect::<Vec<_>>(), "{text}");
                assert!(result.contains(&format!(r#""before":"{}","after":"{}""#, before.0, after.0)));
