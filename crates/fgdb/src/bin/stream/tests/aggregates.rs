    ...
    let ((), report) = run_async_under_lab(0x636c_a601, |root| async move {
        ...
        for (id, value) in [
            ...
            seed.create_vertex(
                ...
                value
                    .into_iter()
                    .map(|value| (PropertyKeyId(1), CanonicalScalar::Int(value)))
                    .collect(),
            );
        }
        seed.create_vertex(
            VId(u128::MAX),
            vec![],
            vec![(PropertyKeyId(1), CanonicalScalar::Null)],
        );
        db.write(&contexts.commit(), seed).await.unwrap();
        let options = options(SUMMARY);
        let eager = db
            .query(&cx, SUMMARY, &options.params, &options, policy())
            .unwrap();
        ...
        assert!(human.contains(&format!(
            "5\t3\t22\t22/3\t4\t11\tvertex 0\tvertex {}\n",
            u128::MAX
        )));
        ...
        let opts = super::options(
            "MATCH (n) WHERE n.p IS NOT NULL RETURN MIN(n.p) AS low, MAX(n.p) AS high",
        );
        let eager = db
            .query(&cx, &opts.text, &opts.params, &opts, policy())
            .unwrap();
        let mut expected = Vec::new();
        ...
        assert_eq!(
            row_lines(&String::from_utf8(bytes).unwrap()),
            row_lines(&String::from_utf8(expected).unwrap())
        );
    });
...
fn temporal_parameters_empty_input_and_repeated_aggregates_keep_exact_metadata() {
    let ((), report) = run_async_under_lab(0x636c_a602, |root| async move {
        ...
        for (seq, count, sum, average) in [
            ...
            let avg_cell = average.map_or_else(
                || "{\"type\":\"null\"}".to_owned(),
                |value| format!("{{\"type\":\"average\",\"value\":\"{value}/1\"}}"),
            );
            let sum_cell = sum.map_or_else(
                || "{\"type\":\"null\"}".to_owned(),
        ...
        let mut options =
            options("MATCH (n) WHERE n.p > $floor RETURN COUNT(*) AS total, AVG(n.p) AS a");
        for (floor, count) in [(0, 2), (10, 1), (100, 0)] {
            ...
            let text = String::from_utf8(bytes).unwrap();
            assert!(
                row_lines(&text)[0].contains(&format!("\"type\":\"count\",\"value\":\"{count}\""))
            );
...
fn aggregate_late_data_and_quota_failures_release_no_partial_summary_or_success() {
    let ((), report) = run_async_under_lab(0x636c_a604, |root| async move {
        ...
        let mut db = Database::open_memory(&contexts.commit(), keys())
            ...
            .unwrap();
        for id in 1..=3 {
            db.write(&contexts.commit(), batch(id, id as i64))
                .await
                .unwrap();
        }
        ...
        for allowance in [
            ...
            let mut cursor = prepared
                .stream_aggregate(&db, &cx, &options.params, allowance)
                .unwrap();
            let columns = cursor.columns().to_vec();
            let mut bytes = Vec::new();
            let error = deliver(
