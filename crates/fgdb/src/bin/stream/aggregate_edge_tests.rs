...
const SUMMARY: &str = "MATCH (a)-[r:R]->(b) RETURN COUNT(*) AS total,COUNT(r.p) AS present,SUM(r.p) AS sum";
fn okay<T>(value: Result<T, Failure>) -> T { value.unwrap_or_else(|error| panic!("{}: {}", error.class, error.message)) }
fn keys() -> DatabaseKeys { DatabaseKeys::new([0xc1; 32], DatabaseSecurityNamespaceId([0xc2; 32]), [0xc3; 32]) }
fn options(text: &str) -> Options {
    let args = ["--db", "unused", "--key-file", "unused", "--relation", "R=1", "--relation", "S=2",
        "--property", "p=1", "--stream", text].map(str::to_owned);
    okay(crate::parse(&args, "query"))
}
fn row_lines(text: &str) -> Vec<&str> { text.lines().filter(|line| line.contains(r#""event":"row""#)).collect() }
async fn seed(db: &mut Database<fgdb::MemVfs>, cx: &CommitCx) {
    let mut r = WriteBatch::new(R);
    for id in [START, VId(1), END] { r.create_vertex(id, vec![], vec![]); }
    for (id, from, to, value) in [(0, START, VId(1), Some(i64::MAX)), (1, START, VId(1), Some(i64::MAX)),
        (2, VId(1), END, None), (u128::MAX, START, START, Some(-3))] {
        r.add_edge(EId(id), from, to, value.into_iter().map(|value| (P, CanonicalScalar::Int(value))).collect());
    }
...
fn cli_edges_chains_and_probes_keep_eager_cells_and_independent_wide_sums() {
    let ((), report) = run_async_under_lab(0x636c_e601, |root| async move {
        ...
        for (body, count, present, sum) in [
            ...
            ("(a)-[r:R]->(b) WHERE EXISTS { MATCH (b)-[:S*1..3]->(c) }", 2, 2, 2 * max),
            ("(a)-[r:R]->(b) WHERE NOT EXISTS { MATCH (b)-[:S]->(c) }", 2, 1, -3),
        ] {
            let text = format!("MATCH {body} RETURN COUNT(*) AS total,COUNT(r.p) AS present,SUM(r.p) AS sum");
            ...
            let mut bytes = Vec::new(); okay(run(&db, &cx, &options, true, &mut bytes));
            let output = String::from_utf8(bytes).unwrap();
            assert_eq!(row_lines(&output), row_lines(&String::from_utf8(expected).unwrap()));
            let independent = format!(r#"{{"v":1,"event":"row","cells":[{{"type":"count","value":"{count}"}},{{"type":"count","value":"{present}"}},{{"type":"wideint","value":"{sum}"}}]}}"#);
            assert_eq!(row_lines(&output), vec![independent.as_str()]);
            assert_eq!(output.lines().count(), 3);
            assert!(output.ends_with("\"seq\":1,\"count\":1}\n"));
            let mut human = Vec::new(); okay(run(&db, &cx, &options, false, &mut human));
...
fn temporal_rebinding_empty_summary_and_alias_order_do_not_follow_the_writer() {
    let ((), report) = run_async_under_lab(0x636c_e602, |root| async move {
        ...
        let cx = contexts.query(); let commit = contexts.commit();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit).await;
        let mut edit = WriteBatch::new(R); edit.set_edge_property(EId(0), P, Some(CanonicalScalar::Int(-7)));
        ...
        for (seq, count, sum) in [(0, 0, None), (1, 4, Some(2 * i128::from(i64::MAX) - 3)), (2, 4, Some(i128::from(i64::MAX) - 10))] {
            let mut opts = options(text); opts.params = GqlParameters::new().with_uint64("seq", seq).unwrap();
            let mut bytes = Vec::new(); okay(run(&db, &cx, &opts, true, &mut bytes));
            let output = String::from_utf8(bytes).unwrap();
            let value = sum.map_or_else(|| "{\"type\":\"null\"}".to_owned(),
                |sum| format!(r#"{{"type":"wideint","value":"{sum}"}}"#));
            let row = format!(r#"{{"v":1,"event":"row","cells":[{value},{{"type":"count","value":"{count}"}},{value}]}}"#);
        ...
        let mut opts = options("MATCH (a)-[r:R]->(b) WHERE r.p > $floor RETURN COUNT(*) AS n,SUM(r.p) AS s");
        ...
        drop(opts); drop(prepared); drop(db);
        let columns = cursor.columns().to_vec();
        let mut bytes = Vec::new();
        okay(deliver(&columns, cursor.snapshot_seq().0, &mut cursor, true, &mut bytes, || Ok(())));
        let output = String::from_utf8(bytes).unwrap();
...
impl Write for Output {
    ...
    fn flush(&mut self) -> std::io::Result<()> {
        ...
        if self.flushes == self.fail { Err(std::io::Error::new(std::io::ErrorKind::BrokenPipe, "receiver closed")) } else { Ok(()) }
    }
}
...
fn output_failure_and_predemand_cancellation_do_not_start_or_repeat_aggregation() {
    let ((), report) = run_async_under_lab(0x636c_e603, |root| async move {
        ...
        let cx = contexts.query(); let commit = contexts.commit();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit).await;
        let opts = options(SUMMARY); let prepared = PreparedNativeRead::prepare(SUMMARY, &opts.params, &opts).unwrap();
        for fail in [1, 2] {
            let mut cursor = prepared.stream_aggregate(&db, &cx, &opts.params, policy()).unwrap();
            let columns = cursor.columns().to_vec(); let pulls = Rc::new(Cell::new(0));
            let mut output = Output { bytes: Vec::new(), pulls: pulls.clone(), flushes: 0, fail };
            let error = deliver(&columns, cursor.snapshot_seq().0,
                &mut cursor.by_ref().inspect(|_| pulls.set(pulls.get() + 1)), true, &mut output,
                || cx.checkpoint().map_err(Failure::query)).err().expect("injected flush failure");
            ...
            let before = (cursor.row_stats(), cursor.evaluator_stats()); cursor.close();
        ...
        let mut cursor = prepared.stream_aggregate(&db, &cx, &opts.params, policy()).unwrap();
        let columns = cursor.columns().to_vec(); let mut bytes = Vec::new(); let mut calls = 0;
        let error = deliver(&columns, cursor.snapshot_seq().0, &mut cursor, true, &mut bytes, || {
            calls += 1; if calls == 2 { Err(Failure::query("cancelled before demand")) } else { Ok(()) }
        }).err().expect("cancelled delivery");
        assert_eq!(error.code, 3);
        assert_eq!(cursor.row_stats().snapshot_records, 0);
        assert_eq!(cursor.evaluator_stats().work_units, 0);
        cursor.close(); assert!(cursor.next().is_none());
...
fn late_sum_and_quota_failures_never_encode_partial_summaries() {
    let ((), report) = run_async_under_lab(0x636c_e604, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit).await;
        let opts = options(SUMMARY); let prepared = PreparedNativeRead::prepare(SUMMARY, &opts.params, &opts).unwrap();
        let mut full = prepared.stream_aggregate(&db, &cx, &opts.params, policy()).unwrap();
        full.next().unwrap().unwrap(); let r = full.row_stats(); let e = full.evaluator_stats();
        for quota in [GqlQueryPolicy::new(r.snapshot_records - 1, 1, u64::MAX, u64::MAX),
            ...
            let mut cursor = prepared.stream_aggregate(&db, &cx, &opts.params, quota).unwrap();
            let columns = cursor.columns().to_vec(); let mut bytes = Vec::new();
            let error = deliver(&columns, cursor.snapshot_seq().0, &mut cursor, true, &mut bytes,
                || cx.checkpoint().map_err(Failure::query)).err().expect("quota refusal");
            assert_eq!(error.code, 3);
        ...
        let mut edit = WriteBatch::new(R); edit.set_edge_property(EId(u128::MAX), P,
            Some(CanonicalScalar::ucs_basic_text("private late invalid operand").unwrap()));
        db.write(&commit, edit).await.unwrap();
        let mut bytes = Vec::new(); let error = run(&db, &cx, &opts, true, &mut bytes).err().expect("late SUM refusal");
        assert_eq!(error.code, 3); assert!(!error.message.contains("private late"));
        let output = String::from_utf8(bytes).unwrap();
...
fn edge_admission_precedes_headers_and_io_errors_keep_their_cause() {
    let ((), report) = run_async_under_lab(0x636c_e605, |root| async move {
        ...
        for text in [
            ...
            let mut bytes = Vec::new(); assert!(run(&db, &cx, &opts, true, &mut bytes).is_err());
