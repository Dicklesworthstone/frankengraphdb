...
fn options(text: &str) -> Options {
    okay(crate::parse(&["--db", "unused", "--key-file", "unused", "--property", "p=1",
        "--property", "q=2", "--stream", text].map(str::to_owned), "query"))
}
fn seed() -> WriteBatch {
    let mut batch = WriteBatch::new(RelationId(1));
    for (id, key, n) in [(0, 2, 5), (1, 1, 7), (2, 2, 2), (u128::MAX, 1, 3)] {
        batch.create_vertex(VId(id), vec![], vec![(PropertyKeyId(1), CanonicalScalar::Int(key)),
            (PropertyKeyId(2), CanonicalScalar::Int(n))]);
    }
    batch
}
const TEXT: &str = "MATCH (n) RETURN SUM(n.q) AS total, n.p AS bucket, AVG(n.q) AS mean, COUNT(*) AS count";
fn rows(text: &str) -> Vec<&str> {
    text.lines().filter(|line| line.contains("\"event\":\"row\"")).collect()
}
...
fn grouped_cli_cells_follow_return_slots_and_keep_exact_fractions_and_temporal_cuts() {
    let ((), report) = run_async_under_lab(0x636c_a701, |root| async move {
        ...
        assert!(String::from_utf8(human).unwrap().contains("10\t1\t5\t2\n7\t2\t7/2\t2\n"));
        for seq in [0, 1] {
            let text = format!("MATCH (n) FOR SYSTEM_TIME AS OF SEQ {seq} RETURN COUNT(*) AS count, n AS id");
            let mut bytes = Vec::new(); okay(run(&db, &cx, &self::options(&text), true, &mut bytes));
            let text = String::from_utf8(bytes).unwrap();
            assert_eq!(rows(&text).len(), if seq == 0 { 0 } else { 4 });
            assert!(text.contains(&format!("\"seq\":{seq}")));
            if seq == 1 { assert!(text.contains(&u128::MAX.to_string())); }
        }
    });
...
fn grouped_transport_stops_before_demand_or_between_groups_without_success() {
    let ((), report) = run_async_under_lab(0x636c_a702, |root| async move {
        let c = PurposeContexts::narrow_runtime_root(&root); let cx = c.query();
        let mut db = Database::open_memory(&c.commit(), keys()).await.unwrap();
        db.write(&c.commit(), seed()).await.unwrap(); let options = options(TEXT);
        ...
        for (fail_at, cancelled) in [(Some(1), false), (Some(2), false), (None, true)] {
            ...
            let slots = cursor.output_slots().to_vec(); let columns = cursor.columns().to_vec();
            let flushed = Rc::new(Cell::new(0)); let mut out = Output { bytes: Vec::new(),
                flushed: Rc::clone(&flushed), fail_at };
            let mut pulls = 0;
            let error = deliver(&columns, cursor.snapshot_seq().0,
                &mut cursor.by_ref().map(|result| {
                    pulls += 1; result.map(|row| AggregateDeliveryRow { row, slots: &slots })
                }), true, &mut out, || {
                    if cancelled && flushed.get() == 2 { Err(Failure::query("cancelled")) }
                    else { cx.checkpoint().map_err(Failure::query) }
                }).err().expect("injected delivery failure");
            ...
            cursor.close(); assert_eq!(cursor.state(), VertexScanState::Closed);
            assert!(cursor.next().is_none()); assert_eq!((cursor.row_stats(), cursor.evaluator_stats()), stats);
            assert!(!String::from_utf8(out.bytes).unwrap().contains("\"event\":\"result\""));
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
