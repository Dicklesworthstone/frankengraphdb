...
fn options(text: &str) -> Options {
    let args = ["--db", "unused", "--key-file", "unused", "--property", "p=1", "--stream", text]
        .map(str::to_owned);
    okay(crate::parse(&args, "query"))
}
fn batch(vid: u128, value: i64) -> WriteBatch {
    let mut batch = WriteBatch::new(RelationId(1));
    batch.create_vertex(VId(vid), vec![], vec![(PropertyKeyId(1), CanonicalScalar::Int(value))]);
    batch
}
...
fn cli_pull_rows_equal_native_results_with_exact_temporal_metadata() {
    let ((), report) = run_async_under_lab(0x636c_7301, |root| async move {
        ...
        for (vid, value) in [(0, 4), (2, 11), (9, 7), (u128::MAX, 23)] {
            db.write(&contexts.commit(), batch(vid, value)).await.unwrap();
        }
        let cases = [
            ("MATCH (n) RETURN n AS id, n.p AS p", 4, 4),
            ("MATCH (n) WHERE n.p > 10 OR n.p = 4 RETURN n AS id, n.p AS p", 4, 3),
            ("MATCH (n) RETURN DISTINCT n AS id, n.p AS p SKIP 1 LIMIT 2", 4, 2),
            ("MATCH (n) FOR SYSTEM_TIME AS OF SEQ 1 RETURN n AS id, n.p AS p", 1, 1),
        ];
...
impl Write for ObservedOutput {
    ...
    fn flush(&mut self) -> std::io::Result<()> {
        ...
        assert!(self.pulls.get() <= flush - 1, "pulled ahead of output demand");
...
fn failed_header_or_row_flush_stops_demand_without_draining_the_cursor() {
    let ((), report) = run_async_under_lab(0x636c_7302, |root| async move {
        ...
        let cx = contexts.query();
        let mut db = Database::open_memory(&contexts.commit(), keys()).await.unwrap();
        for vid in 1..=4 { db.write(&contexts.commit(), batch(vid, vid as i64)).await.unwrap(); }
        ...
        for fail in [1, 2, 3] {
            ...
            let mut out = ObservedOutput { bytes: Vec::new(), pulls: pulls.clone(),
...
fn cumulative_native_quota_failure_leaves_a_prefix_but_never_a_success_result() {
    let ((), report) = run_async_under_lab(0x636c_7303, |root| async move {
        ...
        let options = options("MATCH (n) RETURN n AS id, n.p AS p");
        let prepared = PreparedNativeRead::prepare(&options.text, &options.params, &options).unwrap();
        for allowance in [GqlQueryPolicy::new(100, 1, 100_000, 100_000),
            ...
            let error = deliver(&columns, cursor.snapshot_seq().0, &mut cursor, true, &mut bytes,
                || cx.checkpoint().map_err(Failure::query)).err().unwrap();
            assert_eq!(error.code, 3);
            assert!(error.message.starts_with("stream incomplete after 1 fully flushed row(s)"), "{}", error.message);
...
fn cancellation_after_a_flushed_row_does_not_request_another() {
    let ((), report) = run_async_under_lab(0x636c_7304, |root| async move {
        ...
        let mut out = ObservedOutput { bytes: Vec::new(), pulls: pulls.clone(),
            flushed: flushed.clone(), fail_on_flush: None };
        let error = deliver(&columns, cursor.snapshot_seq().0,
            &mut cursor.by_ref().inspect(|_| pulls.set(pulls.get() + 1)), true, &mut out,
            || if flushed.get() == 2 { Err(Failure::query("cancelled")) } else { Ok(()) })
...
fn unsupported_plans_or_future_cuts_emit_no_header_but_limit_zero_completes() {
    let ((), report) = run_async_under_lab(0x636c_7305, |root| async move {
        ...
        for text in ["MATCH (n) RETURN n.p AS p", "MATCH (n) RETURN COUNT(*) AS total",
            ...
            assert!(bytes.is_empty(), "preparation/admission must precede transport");
        }
        let mut bytes = Vec::new();
        okay(run(&db, &cx, &options("MATCH (n) RETURN n AS id LIMIT 0"), true, &mut bytes));
        let text = String::from_utf8(bytes).unwrap();
...
fn stream_is_query_only_unique_and_incompatible_with_eager_certification() {
    for (command, tail) in [
        ...
        ("query", vec!["--stream", "--certify-to", "x", "MATCH (n) RETURN n"]),
        ("query", vec!["--certify-to", "x", "--stream", "MATCH (n) RETURN n"]),
        ("write", vec!["--stream", "CREATE (n)"]),
        ("create", vec!["--stream"]),
        ("transaction", vec!["--stream", "--query", "MATCH (n) RETURN n"]),
    ] {
        let mut args = vec!["--db", "x", "--key-file", "y"];
        args.extend(tail);
        assert!(crate::parse(&args.into_iter().map(str::to_owned).collect::<Vec<_>>(), command).is_err());
    }
    let args = ["--db", "x", "--key-file", "y", "MATCH (n) RETURN n", "--stream"].map(str::to_owned);
