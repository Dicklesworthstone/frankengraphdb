...
fn fixture() -> WriteBatch {
    ...
    for (id, from, to, value) in [(0, 1, 0, 4), (1, 1, 0, 7), (2, 1, 1, 8),
        (u128::MAX, 0, u128::MAX, i64::MIN)] {
        batch.add_edge(EId(id), VId(from), VId(to), vec![(PropertyKeyId(1), CanonicalScalar::Int(value))]);
    }
    batch
}
fn row_lines(text: &str) -> Vec<&str> {
    text.lines().filter(|line| line.contains(r#""event":"row""#)).collect()
}
...
fn cli_edge_streams_preserve_lossless_rows_parameters_and_exact_temporal_metadata() {
    let ((), report) = run_async_under_lab(0x636c_7401, |root| async move {
        ...
        for (left, right) in [("-", "->"), ("<-", "-"), ("-", "-")] {
            for (filter, page, distinct) in [("", "", "ALL"),
                ("WHERE r.p > 0 AND a.p IS NOT NULL", "SKIP 1 LIMIT 2", "DISTINCT")] {
                for historical in [false, true] {
                    let temporal = if historical { format!("FOR SYSTEM_TIME AS OF SEQ {}", basis.0) } else { String::new() };
                    let text = format!("MATCH (a){left}[r:R]{right}(b) {temporal} {filter} RETURN {distinct} r, a, b, r.p AS weight {page}");
                    let options = options(&text);
                    let expected = db.query(&cx, &text, &options.params, &options, policy()).unwrap();
                    let seq = if historical { basis.0 } else { latest.0 };
                    let mut eager = Vec::new(); okay(crate::render(expected, seq, "rows", true, &mut eager));
                    let mut bytes = Vec::new(); okay(run(&db, &cx, &options, true, &mut bytes));
                    let eager = String::from_utf8(eager).unwrap(); let streamed = String::from_utf8(bytes).unwrap();
                    assert_eq!(row_lines(&streamed), row_lines(&eager), "{text}");
                    let count = row_lines(&streamed).len();
                    assert!(streamed.starts_with(&format!("{{\"v\":1,\"event\":\"columns\",\"stream\":true,\"seq\":{seq},")));
                    assert!(streamed.ends_with(&format!("{{\"v\":1,\"event\":\"result\",\"kind\":\"rows\",\"stream\":true,\"seq\":{seq},\"count\":{count}}}\n")));
                    if filter.is_empty() {
                        assert!(streamed.contains(&format!("{{\"type\":\"edge\",\"value\":\"{}\"}}", u128::MAX)));
                        assert!(streamed.contains(&format!("{{\"type\":\"int\",\"value\":\"{}\"}}", i64::MIN)));
                    }
                }
            }
        }
        let mut options = options("MATCH (a)-[r:R]->(b) WHERE r.p > $floor RETURN r, a, b, r.p AS weight");
...
impl Write for Output {
    ...
    fn flush(&mut self) -> std::io::Result<()> {
        let at = self.flushed.get() + 1; self.flushed.set(at);
        assert!(self.pulls.get() <= at - 1, "source advanced ahead of flushed output");
        if self.fail_at == Some(at) {
            Err(std::io::Error::new(std::io::ErrorKind::BrokenPipe, "receiver closed"))
        } else { Ok(()) }
    }
}
...
fn broken_output_or_delivery_cancellation_stops_even_the_second_edge_orientation() {
    let ((), report) = run_async_under_lab(0x636c_7402, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let cx = contexts.query(); let commit = contexts.commit();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap(); db.write(&commit, fixture()).await.unwrap();
        let options = options(QUERY);
        let prepared = PreparedNativeRead::prepare(QUERY, &options.params, &options).unwrap();
        for (fail_at, cancel) in [(Some(1), false), (Some(2), false), (Some(3), false), (None, true)] {
            let (columns, mut cursor) = prepared.stream(&db, &cx, &options.params, policy()).unwrap();
            assert_eq!(cursor.kind(), ScanKind::Edge);
            let pulls = Rc::new(Cell::new(0)); let flushed = Rc::new(Cell::new(0));
            let mut out = Output { bytes: Vec::new(), pulls: pulls.clone(), flushed: flushed.clone(), fail_at };
            let error = deliver(&columns, cursor.snapshot_seq().0,
                &mut cursor.by_ref().inspect(|_| pulls.set(pulls.get() + 1)), true, &mut out,
                || if cancel && flushed.get() == 2 { Err(Failure::query("delivery cancelled")) } else { Ok(()) })
                .err().expect("delivery must refuse");
            ...
            assert_eq!(cursor.row_stats().snapshot_records, u64::from(requested > 0));
            ...
            assert!(!String::from_utf8(out.bytes).unwrap().contains(r#""event":"result""#));
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn edge_source_and_result_quotas_do_not_become_success_after_a_flushed_prefix() {
    let ((), report) = run_async_under_lab(0x636c_7403, |root| async move {
        ...
        let options = options(QUERY);
        let prepared = PreparedNativeRead::prepare(QUERY, &options.params, &options).unwrap();
        for (allowance, count) in [(GqlQueryPolicy::new(100, 1, 100_000, 100_000), 1),
            (GqlQueryPolicy::new(1, 100, 100_000, 100_000), 2)] {
            let (columns, mut cursor) = prepared.stream(&db, &cx, &options.params, allowance).unwrap();
            let mut bytes = Vec::new();
            let error = deliver(&columns, cursor.snapshot_seq().0, &mut cursor, true, &mut bytes,
                || cx.checkpoint().map_err(Failure::query)).err().expect("quota exhausted");
            assert_eq!(error.code, 3);
            assert!(error.message.starts_with(&format!("stream incomplete after {count} fully flushed row(s)")));
...
fn unsupported_edge_shapes_future_cuts_and_missing_parameters_emit_no_columns() {
    let ((), report) = run_async_under_lab(0x636c_7404, |root| async move {
        ...
        let cx = contexts.query(); let commit = contexts.commit();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap(); db.write(&commit, fixture()).await.unwrap();
        for text in ["MATCH (a)-[r:R]->(b) RETURN a, r LIMIT 0",
            ...
            let mut bytes = Vec::new();
            assert!(run(&db, &cx, &options(text), true, &mut bytes).is_err(), "{text}");
            assert!(bytes.is_empty(), "admission must finish before a header");
        }
        let mut bytes = Vec::new(); okay(run(&db, &cx, &options(&(QUERY.to_owned() + " LIMIT 0")), true, &mut bytes));
        let text = String::from_utf8(bytes).unwrap();
...
fn native_scan_error_wrapping_preserves_transport_io_classification_after_a_row() {
    let ((), report) = run_async_under_lab(0x636c_7405, |root| async move {
        ...
        let prepared = PreparedNativeRead::prepare(QUERY, &options.params, &options).unwrap();
        let (columns, mut cursor) = prepared.stream(&db, &cx, &options.params, policy()).unwrap();
        let row = cursor.next().unwrap().unwrap(); cursor.close();
        ...
        let error = GqlQueryError::<ScanError<std::io::Error>, std::io::Error>::Source(
            ScanError::Edge(EdgeScanError::Source(std::io::Error::other("source I/O failed"))));
        let mut input = vec![Ok(row), Err(error)].into_iter(); let mut bytes = Vec::new();
        let error = deliver(&columns, cursor.snapshot_seq().0, &mut input, true, &mut bytes, || Ok(()))
            .err().expect("injected source failure");
        assert_eq!(error.code, 5);
        assert!(error.message.starts_with("stream incomplete after 1 fully flushed row(s)"));
        let text = String::from_utf8(bytes).unwrap();
