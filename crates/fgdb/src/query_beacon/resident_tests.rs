...
fn reusable_generation_owns_its_definition_and_survives_writer_changes_and_drop() {
    let ((), report) = run_async_under_lab(0xbeac_1001, |root| async move {
        ...
        db.write(&commit, document(1, "graph graph", 1, 2))
            .await
            .unwrap();
        db.write(&commit, document(2, "storage graph", 3, 4))
            .await
            .unwrap();
        ...
        let expected_vector = db
            .beacon_search(&query, &definition, vector_query())
            .unwrap();
        ...
        assert_eq!(
            resident.search(&query, text_query(), delivery).unwrap(),
            expected_text
        );
        assert_eq!(
            pinned.search(&query, vector_query(), delivery).unwrap(),
            expected_vector
        );
        assert_eq!(
            resident.search(&query, approximate, delivery).unwrap(),
            expected_approximate
        );
        assert_eq!(
            pinned.search(&query, hybrid, delivery).unwrap(),
            expected_hybrid
        );

        db.write(&commit, document(3, "graph storage", 0, 0))
            .await
            .unwrap();
        assert_ne!(
            db.beacon_search(&query, &options(), vector_query())
                .unwrap(),
            expected_vector
        );
        assert_eq!(resident.source_sequence(), basis);
        ...
        drop(resident);
        assert_eq!(
            pinned.search(&query, text_query(), delivery).unwrap(),
            expected_text
        );
        assert_eq!(
            pinned.search(&query, vector_query(), delivery).unwrap(),
        ...
        assert_eq!(
            pinned.search(&query, approximate, delivery).unwrap(),
            expected_approximate
        );
        assert_eq!(
            pinned.search(&query, hybrid, delivery).unwrap(),
...
fn initial_historical_cut_uses_old_properties_labels_and_tombstones_for_both_lanes() {
    let ((), report) = run_async_under_lab(0xbeac_1002, |root| async move {
        ...
        let basis = db
            .write(&commit, document(2, "storage", 2, 0))
            .await
            .unwrap();
        ...
        assert!(
            db.beacon_search(&query, &options(), text_query())
                .unwrap()
                .is_empty()
        );
        definition.as_of = Some(fgdb_types::CommitSeq(db.frontier().unwrap().0 + 1));
        assert!(matches!(
            db.prepare_beacon_index(&query, &definition),
...
fn retained_construction_admits_explicit_lanes_without_changing_one_shot_lane_elision() {
    let ((), report) = run_async_under_lab(0xbeac_1003, |root| async move {
        ...
        db.write(&commit, document(1, "graph", 16_777_217, 0))
            .await
            .unwrap();
        let mut definition = options();
        assert_eq!(
            db.beacon_search(&query, &definition, text_query())
                .unwrap()
                .len(),
            1
        );
        ...
        assert_eq!(
            resident
                .search(&query, text_query(), ReadPolicy::default())
                .unwrap()
                .len(),
            1
        );
...
fn query_and_build_budgets_refuse_without_consuming_the_retained_generation() {
    let ((), report) = run_async_under_lab(0xbeac_1004, |root| async move {
        ...
        let small = ReadPolicy {
            max_result_rows: 9,
            ..ReadPolicy::default()
        };
        assert!(matches!(
            resident.search(&query, text_query(), small),
            Err(Error::Index(BeaconError::ResourceLimit {
                resource: "result rows",
                limit: 9
            }))
        ));
        let no_work = ReadPolicy {
            max_work_units: 0,
            ..ReadPolicy::default()
        };
        assert!(
...
fn refresh_updates_both_lanes_statistics_and_membership_without_changing_old_pins() {
    let ((), report) = run_async_under_lab(0xbeac_1005, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        db.write(&commit, document(1, "graph", 1, 0)).await.unwrap();
        db.write(&commit, document(2, "storage", 2, 0))
            .await
            .unwrap();
        db.write(
            &commit,
            document(3, "unrelated words change corpus statistics", 3, 0),
        )
        .await
        .unwrap();
        ...
        let before_text = before
            .search(&query, text_query(), ReadPolicy::default())
            .unwrap();
        let before_vector = before
            .search(&query, vector_query(), ReadPolicy::default())
            .unwrap();
...
fn refresh_uses_the_requested_cut_not_newer_values_and_can_retry_a_failed_projection() {
    let ((), report) = run_async_under_lab(0xbeac_1006, |root| async move {
        ...
        resident
            .refresh(&query, &db, Some(target), ReadPolicy::default())
            .unwrap();
        ...
        assert_eq!(
            rows,
            db.beacon_search(&query, &at_target, vector_query())
                .unwrap()
        );
        assert!(matches!(
            resident.refresh(&query, &db, None, ReadPolicy::default()),
            Err(Error::Index(BeaconError::InvalidQuery(_)))
        ));
        ...
        assert_eq!(
            old.search(&query, vector_query(), ReadPolicy::default())
                .unwrap(),
            rows
        );
    });
...
fn edge_only_and_normalized_noop_commits_advance_without_graph_source_admission() {
    let ((), report) = run_async_under_lab(0xbeac_1007, |root| async move {
        ...
        assert_eq!(
            resident.search(&query, text_query(), no_source).unwrap(),
...
fn foreign_backwards_future_and_resource_refusals_preserve_sequence_and_contents() {
    let ((), report) = run_async_under_lab(0xbeac_1008, |root| async move {
        ...
        assert!(matches!(
            resident.refresh(&query, &other, None, ReadPolicy::default()),
        ...
        assert!(matches!(
            resident.refresh(
                &query,
                &db,
                Some(fgdb_types::CommitSeq(u64::MAX)),
                ReadPolicy::default()
            ),
            Err(Error::Source(crate::ReadError::BeyondFrontier { .. }))
        ));
...
fn indexed_refresh_uses_only_affected_histories_under_a_two_entry_source_budget() {
    let ((), report) = run_async_under_lab(0xbeac_1010, |root| async move {
        ...
        for search in [text_query(), vector_query()] {
            assert_eq!(
                resident.search(&query, search, policy).unwrap(),
...
fn every_public_refresh_checkpoint_preserves_typed_interruption_and_retry() {
    ...
    let ((), report) = run_async_under_lab(0xbeac_1011, |root| async move {
        ...
        let before = [text_query(), vector_query()]
            .map(|search| old.search(&query, search, ReadPolicy::default()).unwrap());
        ...
        successful
            .refresh(&observed, &db, None, ReadPolicy::default())
            .unwrap();
        ...
        for stop in 1..=calls {
            ...
            assert!(
                matches!(
                    candidate.refresh(&interrupted, &db, None, ReadPolicy::default()),
            ...
            candidate
                .refresh(&interrupted, &db, None, ReadPolicy::default())
                .unwrap();
