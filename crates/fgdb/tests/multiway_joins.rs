...
fn pattern() -> PreparedGraphPattern<GraphValueRow> {
    PreparedGraphText::prepare(&format!("{HEAD} RETURN c.score AS score,a,c"), symbols)
        .unwrap()
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<Plain> {
    ...
    for first in edges.iter().filter(|e| e.entry.relation == RelationId(1)) {
        for second in edges
            .iter()
            .filter(|e| e.entry.relation == RelationId(2) && e.entry.src == first.entry.dst)
        {
            for _ in edges.iter().filter(|e| {
                e.entry.relation == RelationId(3)
                    && e.entry.src == first.entry.src
                    && e.entry.dst == second.entry.dst
            }) {
                for _ in edges.iter().filter(|e| {
                    e.entry.relation == RelationId(4)
                        && e.entry.src == second.entry.dst
                        && e.entry.dst == first.entry.src
                }) {
                    for _ in edges.iter().filter(|e| {
                        e.entry.relation == RelationId(5)
                            && e.entry.src == first.entry.dst
                            && e.entry.dst == second.entry.dst
                    }) {
                        let vertex = vertices
                            .iter()
...
fn plain(rows: &[GraphValueRow]) -> Vec<Plain> {
    rows.iter()
        .map(|row| {
            ...
            (
                score,
                row.get(1).unwrap().as_vertex().unwrap(),
                row.get(2).unwrap().as_vertex().unwrap(),
            )
        })
        .collect()
}
fn expected_summary(rows: &[Plain]) -> Vec<Summary> {
    ...
    for &(score, key, _) in rows {
        ...
        if let Some(score) = score {
            entry.1 = Some(entry.1.unwrap_or(0) + i128::from(score));
        }
    }
    let mut rows: Vec<_> = groups
        ...
        .collect();
    rows.sort_by_key(|row| (std::cmp::Reverse(row.1), row.0));
    rows
}
...
fn multiconstraint_bags_and_aggregates_agree_across_all_five_read_entrypoints() {
    let ((), report) = run_async_under_lab(0x4d57_0001, |root| async move {
        ...
        assert_eq!(
            ...
            vec![(VId(0), 24, Some(120)), (VId(1), 1, Some(7))]
        );
        for result in [
            db.execute_graph_pattern_governed(&cx, &pattern, wide())
                .unwrap(),
            db.execute_graph_pattern_governed_at(&cx, &pattern, basis, wide())
                .unwrap(),
            view.execute_graph_pattern_governed(&cx, &pattern, wide())
                .unwrap(),
            view.execute_graph_pattern_governed_at(&cx, &pattern, basis, wide())
                .unwrap(),
            txn.execute_graph_pattern_governed(&db, &cx, &pattern, wide())
                .unwrap(),
        ] {
            assert_eq!(plain(&result.value), expected);
        }
        for result in [
            db.execute_graph_aggregate_governed(&cx, &aggregate, wide())
                .unwrap(),
            db.execute_graph_aggregate_governed_at(&cx, &aggregate, basis, wide())
                .unwrap(),
            view.execute_graph_aggregate_governed(&cx, &aggregate, wide())
                .unwrap(),
            view.execute_graph_aggregate_governed_at(&cx, &aggregate, basis, wide())
                .unwrap(),
            txn.execute_graph_aggregate_governed(&db, &cx, &aggregate, wide())
                .unwrap(),
        ] {
        ...
        for cap in [
            GqlQueryPolicy::new(1000, 24, u64::MAX, u64::MAX),
...
fn multiconstraint_results_track_canonical_staging_and_reopened_history() {
    let ((), report) = run_async_under_lab(0x4d57_0002, |root| async move {
        ...
        assert_eq!(
            expected_summary(&expected),
            vec![(VId(0), 14, Some(18)), (VId(1), 1, Some(7))]
        );
        assert_eq!(
            plain(
                &txn.execute_graph_pattern_governed(&db, &cx, &pattern, wide())
        ...
        assert_eq!(
            summary(
                &reopened
                    .execute_graph_aggregate_governed(&cx, &aggregate, wide())
                    .unwrap()
                    .value
            ),
            expected_summary(&expected)
        ...
        assert_eq!(
            plain(
                &reopened
                    .execute_graph_pattern_governed_at(&cx, &pattern, basis, wide())
                    .unwrap()
                    .value
            ),
            old
        ...
        for refused in [false, true] {
            for change in 0..4 {
                ...
                if refused {
                    ...
                    assert_eq!(result.unwrap().value.len(), 25);
                }
                ...
                match change {
                    ...
                    1 => {
                        winner.add_edge(EId(91), VId(10), VId(21), vec![]);
                    }
                    2 => {
                        winner.set_vertex_property(VId(30), SCORE, Some(CanonicalScalar::Int(20)));
                    }
                    _ => {
                        winner.add_edge(EId(92), VId(30), VId(0), vec![]);
                    }
                }
