...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (id, labels, value) in [(1, vec![A], Some(10)), (2, vec![], Some(20)),
        (3, vec![B], Some(30)), (4, vec![A], Some(40)), (5, vec![B], Some(20)), (6, vec![B], None)] {
        batch.create_vertex(VId(id), labels, vec![(P, value.map_or(CanonicalScalar::Null, CanonicalScalar::Int))]);
    }
...
fn scalar(row: &VertexRow) -> Option<i64> {
    row.props.iter().find_map(|(key, value)| match value {
        CanonicalScalar::Int(n) if *key == P => Some(*n), _ => None,
    })
}
fn plain(rows: &[GraphValueRow]) -> Vec<Option<i64>> {
    rows.iter().map(|row| match row.get(0).unwrap().as_scalar().unwrap() {
        CanonicalScalar::Null => None, CanonicalScalar::Int(n) => Some(*n),
    ...
    for root in vertices.iter().filter(|row| row.labels.contains(&A)) {
        ...
        for depth in 0..=2 {
            for vid in &layer {
                ...
                if let Some(n) = scalar(row).filter(|n| *n >= 0) { *counts.entry(n).or_default() += 1; }
            }
            if depth == 2 { break; }
            layer = layer.into_iter().flat_map(|source| edges.iter()
                .filter(move |edge| edge.entry.relation == R && edge.entry.src == source)
                .map(|edge| edge.entry.dst)).collect();
        }
    }
    for row in vertices.iter().filter(|row| row.labels.contains(&B)) {
        if let Some(n) = scalar(row).filter(|n| *n >= 0) {
            if let Some(count) = counts.get_mut(&n) { *count = count.saturating_sub(1); }
        }
    }
    counts.into_iter().rev().flat_map(|(n, count)| std::iter::repeat_n(Some(n), count)).skip(1).take(2).collect()
}
...
fn compound_text_reuses_one_definition_across_snapshots_staged_changes_and_reopen() {
    let ((), report) = run_async_under_lab(0x5e77_1001, |root| async move {
        ...
        assert_eq!(calls.len(), 4); assert!(calls.values().all(|n| *n == 1));
        assert_eq!(template.parameter_schema()[0].occurrences, 2);
        let args = GqlParameters::new().with_int64("min", 0).unwrap().with_uint64("skip", 1).unwrap()
            .with_uint64("limit", 2).unwrap();
        ...
        assert_eq!(plain(&txn.execute_graph_set_governed(&db, &cx, &query, policy()).unwrap().value), new);
        assert_eq!(plain(&db.execute_graph_set_governed(&cx, &query, policy()).unwrap().value), old);
        txn.commit(&mut db, &commit).await.unwrap();
        db.compact(&commit).await.unwrap(); drop(db);
        let reopened = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        assert_eq!(plain(&reopened.execute_graph_set_governed(&cx, &query, policy()).unwrap().value), new);
        assert_eq!(plain(&reopened.execute_graph_set_governed_at(&cx, &query, basis, policy()).unwrap().value), old);
        assert_eq!(plain(&pinned.execute_graph_set_governed(&cx, &query, policy()).unwrap().value), old);
...
fn right_operand_dependencies_survive_zero_output_and_final_budget_refusal() {
    let ((), report) = run_async_under_lab(0x5e77_1002, |root| async move {
        ...
        let template = PreparedGraphSetText::prepare(
            "MATCH (a:A) RETURN a.p AS value EXCEPT MATCH (b:B) RETURN b.p AS other LIMIT $limit", symbols,
        ).unwrap();
        for mode in 0..3 {
            for mutation in 0..3 {
                ...
                let args = GqlParameters::new().with_uint64("limit", if mode == 2 { 0 } else { 1 }).unwrap();
                let query = template.bind_parameters(&args).unwrap();
                let result = txn.execute_graph_set_governed(&db, &cx, &query,
                ...
                if mutation == 0 { winner.create_vertex(VId(8), vec![B], vec![(P, CanonicalScalar::Int(10))]); }
                else { winner.set_vertex_property(VId(5), P, Some(CanonicalScalar::Int(40))); }
...
fn optional_walks_and_existential_arms_obey_shared_limits_and_authority_fences() {
    let ((), report) = run_async_under_lab(0x5e77_1003, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let basis = seed(&mut db, &commit).await;
        let optional = query("MATCH (a:A) OPTIONAL MATCH WALK (a)-[:R*1..2]->(b) RETURN b.p AS value \
            UNION ALL MATCH (c:B) RETURN c.p AS other ORDER BY value NULLS FIRST");
        let measured = db.execute_graph_set_governed(&cx, &optional, policy()).unwrap();
        assert_eq!(plain(&measured.value), vec![None, None, Some(20), Some(20), Some(20), Some(30), Some(30), Some(30)]);
        let exact = GqlQueryPolicy::new(measured.rows.snapshot_records, 8,
            measured.evaluator.work_units, measured.evaluator.scratch_entries);
        assert_eq!(db.execute_graph_set_governed(&cx, &optional, exact).unwrap(), measured);
        for refused in [
        ...
        ] { assert!(db.execute_graph_set_governed(&cx, &optional, refused).is_err()); }
        let existence = query("MATCH (a:A) WHERE NOT EXISTS { MATCH WALK (a)-[:R*1..2]->(b) WHERE b.p IN [20,30] } \
            RETURN a.p AS value UNION MATCH (c:B) WHERE c.p IS NULL RETURN c.p AS other ORDER BY value NULLS FIRST");
        assert_eq!(plain(&db.execute_graph_set_governed(&cx, &existence, policy()).unwrap().value), vec![None, Some(40)]);
        let zero = GqlQueryPolicy::new(0, 0, 0, 0);
        assert!(matches!(db.execute_graph_set_governed_at(&cx, &optional, CommitSeq(basis.0 + 1), zero),
        ...
        assert!(matches!(txn.execute_graph_set_governed(&foreign, &cx, &optional, zero),
