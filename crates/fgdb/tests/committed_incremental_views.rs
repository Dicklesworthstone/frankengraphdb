...
use fgdb_delta_types::{LimbLimit, LocalDeltaBatchIndex, PropertyKeyId, RelationId, ZSet, ZSetEvent, ZWeight};
...
use fgdb_types::{BranchId, CanonicalScalar, CommitCx, CommitSeq, DatabaseSecurityNamespaceId,
    EId, GraphId, PurposeContexts, VId};
...
impl Pipeline {
    fn new() -> Self {
        Self { input: CommittedEdgeInput::new(GraphId(1), BranchId(1)),
            join: IncrementalJoin::new(), bag: ZSet::new(), distinct: IncrementalDistinct::new(),
            aggregate: IncrementalAggregate::new(), sink: ZSet::new() }
    }
    fn tick(&mut self, source: &LocalDeltaBatchIndex,
        control: &mut impl FnMut(ZSetEvent) -> Result<(), usize>) -> Result<Option<CommitSeq>, ()>
    {
        let Some(input) = self.input.prepare_next(source, LIMBS, control).map_err(|_| ())?
            else { return Ok(None); };
        let left = input.delta().filter(|tuple| Ok(tuple.0 == R), LIMBS, control)
            .map_err(|_| ())?.map(|&(_, source, via)| Ok((via, source)), LIMBS, control).map_err(|_| ())?;
        let right = input.delta().filter(|tuple| Ok(tuple.0 == S), LIMBS, control)
            .map_err(|_| ())?.map(|&(_, via, target)| Ok((via, target)), LIMBS, control).map_err(|_| ())?;
        let joined = self.join.prepare(&left, &right, LIMBS, control).map_err(|_| ())?;
        let projected = joined.delta().map(|&(_, source, target)| Ok((source, target)), LIMBS, control)
            .map_err(|_| ())?;
        let bag = self.bag.prepare_update(&projected, LIMBS, control).map_err(|_| ())?;
        let distinct = self.distinct.prepare(&projected, LIMBS, control).map_err(|_| ())?;
        let values = distinct.delta().map(|&(source, target)| {
            i128::try_from(target.0).map(|value| (source, Some(value))).map_err(|_| usize::MAX)
        }, LIMBS, control).map_err(|_| ())?;
        let aggregate = self.aggregate.prepare(&values, LIMBS, control).map_err(|_| ())?;
        let sink = self.sink.prepare_update(aggregate.delta(), LIMBS, control).map_err(|_| ())?;
    ...
    fn bag_rows(&self) -> BTreeMap<Pair, i128> {
        self.bag.iter().map(|(pair, weight)| (*pair, weight.to_i128().unwrap())).collect()
    }
    fn summaries(&self) -> BTreeMap<VId, Summary> {
        self.sink.iter().map(|((source, values), weight)| {
            assert_eq!(weight, &ZWeight::ONE);
            assert_eq!(self.aggregate.get(source), Some(values.as_ref()));
            (*source, (values.count_rows().to_i128().unwrap(), values.count_values().to_i128().unwrap(),
                values.sum().map(|value| value.to_i128().unwrap()), values.minimum(), values.maximum()))
        }).collect()
    }
}
...
fn oracle(db: &Database<MemVfs>, seq: CommitSeq) -> (BTreeMap<Pair, i128>, BTreeMap<VId, Summary>) {
    ...
    for left in &edges {
        if left.entry.relation != R { continue; }
        for right in &edges {
            if right.entry.relation == S && left.entry.dst == right.entry.src {
                *bag.entry((left.entry.src, right.entry.dst)).or_default() += 1;
                targets.entry(left.entry.src).or_default().insert(i128::try_from(right.entry.dst.0).unwrap());
            }
        }
    }
    let summary = targets.into_iter().map(|(source, targets)| {
        (source, (targets.len() as i128, targets.len() as i128, Some(targets.iter().sum()),
            targets.first().copied(), targets.last().copied()))
    }).collect();
    (bag, summary)
}
fn check(db: &Database<MemVfs>, pipeline: &Pipeline) {
    ...
    assert_eq!(pipeline.input.edge_count(), db.edges_at(pipeline.input.frontier()).unwrap().len());
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    let mut vertices = WriteBatch::new(RelationId(9));
    for id in 1..=8 { vertices.create_vertex(VId(id), vec![], vec![]); }
    ...
    db.write_atomic(cx, vec![vertices, right, left]).await.unwrap()
}
...
fn committed_view_tracks_atomic_writes_cascades_and_recovered_history() {
    let ((), report) = run_async_under_lab(0xc017_0001, |root| async move {
        ...
        check(&db, &pipeline);
        assert_eq!(pipeline.bag_rows(), BTreeMap::from([((VId(1), VId(4)), 5), ((VId(1), VId(5)), 1)]));
        ...
        assert_eq!(pipeline.tick(db.delta_index().unwrap(), &mut allow).unwrap(), Some(seq));
        ...
        assert_eq!(pipeline.tick(db.delta_index().unwrap(), &mut allow).unwrap(), Some(replacement));
        assert_eq!(pipeline.sink, frozen);
        check(&db, &pipeline);
        ...
        assert!(pipeline.tick(db.delta_index().unwrap(), &mut allow).unwrap().is_none(),
        ...
        assert_eq!(pipeline.tick(db.delta_index().unwrap(), &mut allow).unwrap(), Some(committed));
        check(&db, &pipeline);
        ...
        assert_eq!(pipeline.tick(db.delta_index().unwrap(), &mut allow).unwrap(), Some(retired));
        check(&db, &pipeline);
        assert_eq!(pipeline.bag_rows(), BTreeMap::from([((VId(1), VId(4)), 1), ((VId(1), VId(5)), 1)]));
        assert_eq!(pinned.edges().unwrap().len(), 7);
        assert_eq!(Pipeline::replay(&old_source).sink, frozen);
        assert_eq!(oracle(&db, first).0, Pipeline::replay(&old_source).bag_rows());
        ...
        assert_eq!(pipeline.sink, frozen, "bag changed but distinct aggregate did not");
        ...
        assert!(matches!(early.input.prepare_next(foreign.delta_index().unwrap(), LIMBS, &mut allow),
...
fn every_downstream_refusal_preserves_the_committed_cursor_and_all_operators() {
    let ((), report) = run_async_under_lab(0xc017_0002, |root| async move {
        ...
        let mut total = 0;
        assert_eq!(expected.tick(db.delta_index().unwrap(), &mut |_| {
            total += 1; Ok(())
        }).unwrap(), Some(next));
        check(&db, &expected);
        for stop in 1..=total {
            ...
            let mut seen = 0;
            assert!(state.tick(db.delta_index().unwrap(), &mut |_| {
                seen += 1; if seen == stop { Err(stop) } else { Ok(()) }
            }).is_err());
            assert_eq!(seen, stop);
            assert_eq!(state.input.frontier(), first);
            assert_eq!(state, Pipeline::replay(&before));
            assert_eq!(state.tick(db.delta_index().unwrap(), &mut allow).unwrap(), Some(next));
    ...
    let ((), report) = run_async_under_lab(0xc017_0003, |root| async move {
        ...
        for tick in 0..40_u128 {
            ...
            for (offset, relation) in [(0, R), (1, S)] {
                ...
                batch.add_edge(EId(100 + 2 * tick + offset), VId(1 + u128::from(random % 8)),
                    VId(1 + u128::from((random >> 8) % 8)), vec![]);
                if tick % 2 == 0 {
                    let candidates: Vec<_> = old.iter().filter(|edge| edge.entry.relation == relation).collect();
        ...
        while replayed.tick(db.delta_index().unwrap(), &mut allow).unwrap().is_some() {
            check(&db, &replayed);
        }
