...
#[test]
fn undirected_destination_bang_inequality_sees_staged_dest_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x63_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-undirected-where-dst-bang-ne-overlay-{}",
            std::process::id()
        ));
        ...
        let filtered = "MATCH (a)-[:R]-(b) WHERE b.k != 1 RETURN b";
        assert_eq!(
