...
use fgdb_gql::algebra::{
    GlaDirection, GraphColumn, GraphMatchClause, GraphPatternBuilder, GraphValue,
    IntegerComparison,
};
use fgdb_gql::{
...
fn definition(shape: Shape, direction: GlaDirection, grouped: bool) -> PreparedGraphAggregate {
    ...
    let cost = GraphIntegerExpression::prepare(&[
        ...
        Op::Column(3), Op::Literal(Some(1)), Op::Coalesce,
        Op::Binary(GraphIntegerBinary::Multiply),
    ]).unwrap();
    let category = GraphIntegerExpression::prepare_scalar(&[
        Op::ScalarColumn(1), Op::Lower,
    ]).unwrap();
    let query = PreparedGraphAggregate::prepare_projected(input, vec![
        ...
        GraphSetProjection::new("one", GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Int(1)))),
    ], if grouped { &[1] } else { &[] }, &[
    ...
    let having = GraphHavingExpression::prepare(&[
        Having::Compare { left: Operand::Column(Column::Aggregate(6)),
            comparison: IntegerComparison::GreaterOrEqual, right: Operand::Integer(2) },
        Having::Compare { left: Operand::Column(Column::Aggregate(2)),
            comparison: IntegerComparison::Greater, right: Operand::Integer(3) },
        Having::Or,
    ]).unwrap();
...
fn computed_inputs_match_snapshot_across_all_admitted_shapes_and_orientations() {
    let ((), report) = run_async_under_lab(0x6a61, |root| async move {
        ...
        for (id, category, quantity, price) in [
            ...
            if let Some(category) = category { props.push((CATEGORY, text(category))); }
            if let Some(quantity) = quantity { props.push((QUANTITY, CanonicalScalar::Int(quantity))); }
            if let Some(price) = price { props.push((PRICE, CanonicalScalar::Int(price))); }
        ...
        for id in [1,3,4,5] { delete.delete_vertex(VId(id)); }
...
fn quotient_definition() -> PreparedGraphAggregate {
    ...
    let quotient = GraphIntegerExpression::prepare(&[
        Op::Column(1), Op::Column(2), Op::Binary(GraphIntegerBinary::Divide),
    ]).unwrap();
    PreparedGraphAggregate::prepare_projected(input, vec![
        GraphSetProjection::new("group", GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Int(1)))),
        GraphSetProjection::new("quotient", GraphSetValue::Integer(quotient)),
    ], &[0], &[
        GraphAggregate::min("minimum", 1), GraphAggregate::sum_int("sum", 1),
        GraphAggregate::count_rows("count"),
    ], 0, None).unwrap().with_result_clauses(&[
        GraphAggregateFilter { column: Column::Aggregate(2), test: GraphAggregateTest::Integer {
    ...
    let ((), report) = run_async_under_lab(0x6a62, |root| async move {
        ...
        for (id, quantity) in [(1,10), (2,8)] {
            seed.create_vertex(VId(id), vec![], vec![
                (QUANTITY, CanonicalScalar::Int(quantity)), (PRICE, CanonicalScalar::Int(2)),
            ]);
        }
        ...
        repair.create_vertex(VId(3), vec![], vec![
            (QUANTITY, CanonicalScalar::Int(6)), (PRICE, CanonicalScalar::Int(3)),
        ]);
        ...
        assert_eq!(db.rebuild_standing_query(&query, &handle, policy()).unwrap(), repaired);
        check(&db, &query, &handle, &definition);
        assert_eq!(db.standing_query(&query, &handle).unwrap().rows().iter().next()
            .unwrap().0.get(1).unwrap().as_integer(), Some(14));
        ...
        check(&db, &query, &handle, &definition);
        assert_eq!(db.standing_query(&query, &handle).unwrap().rows().iter().next()
            .unwrap().0.get(1).unwrap().as_integer(), Some(19));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn unused_computed_column_executes_but_unselected_coalesce_fallback_does_not() {
    let ((), report) = run_async_under_lab(0x6a63, |root| async move {
        ...
        let fallback = GraphIntegerExpression::prepare(&[
            Op::Column(0), Op::Literal(Some(1)), Op::Literal(Some(0)),
            Op::Binary(GraphIntegerBinary::Divide), Op::Coalesce,
        ]).unwrap();
...
fn computed_input_work_is_independent_of_unrelated_groups_and_properties() {
    let ((), report) = run_async_under_lab(0x6a64, |root| async move {
        ...
        for extra in [0, 500] {
            ...
            for id in [1,2] {
                seed.create_vertex(VId(id), vec![], vec![
                    (CATEGORY, text("books")), (QUANTITY, CanonicalScalar::Int(2)),
                    (PRICE, CanonicalScalar::Int(3)),
                ]);
            }
            for id in 10..10 + extra {
                seed.create_vertex(VId(id), vec![], vec![
                    (CATEGORY, text(&format!("other{id}"))), (QUANTITY, CanonicalScalar::Int(2)),
                    (PRICE, CanonicalScalar::Int(3)),
                ]);
            }
            ...
            unrelated.set_vertex_property(VId(1), PropertyKeyId(99), Some(CanonicalScalar::Int(123)));
...
fn unsupported_computed_collections_refuse_before_registering_even_on_empty_input() {
    let ((), report) = run_async_under_lab(0x6a65, |root| async move {
        ...
        let definition = PreparedGraphAggregate::prepare_projected(input, vec![
            GraphSetProjection::new("unused", GraphSetValue::List(vec![GraphSetValue::Column(0)])),
        ], &[], &[GraphAggregate::count_rows("count")], 0, None).unwrap();
