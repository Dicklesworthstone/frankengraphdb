...
fn staged_path_is_visible_only_to_owner_until_one_atomic_commit() {
    let ((), report) = run_async_under_lab(0xa702_0001, |root| async move {
        ...
        let bind = RelationBind::new()
            ...
            .with_relation("S", S);
        let query = txn
            .prepare_gql_query("MATCH (a)-[:R]->(b)-[:S]->(c) RETURN c", &bind)
            .unwrap();
        assert_eq!(
        ...
        assert_eq!(
            txn.execute_prepared_query_limited(&db, &query, GlaExecutionLimits::new(1000, 1000))
                .unwrap()
                .value,
            vec![VId(3)]
        );
...
fn failed_atomic_staging_preserves_prior_effects_and_evidence() {
    let ((), report) = run_async_under_lab(0xa702_0003, |root| async move {
        ...
        assert!(matches!(
            txn.write_atomic(&mut foreign, vec![]),
...
#[test]
fn oversized_atomic_groups_refuse_before_publication_and_preserve_staging() {
    let ((), report) = run_async_under_lab(0xa702_0005, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let cx = contexts.commit();
        let txn_cx = contexts.txn();
        for edge_property in [false, true] {
            ...
            valid.create_vertex(VId(99), vec![], vec![]);
            let mut invalid = WriteBatch::new(S);
            let props = vec![(P, CanonicalScalar::bytes(vec![0x51; 20_000]).unwrap())];
            if edge_property {
                invalid.add_edge(EId(77), VId(1), VId(3), props);
            } else {
                invalid.create_vertex(VId(100), vec![], props);
            }
            let groups = vec![valid, invalid];
            let assert_admission = |error: WriteTxnError| {
                assert!(matches!(
                    (edge_property, error),
                    (
                        false,
                        WriteTxnError::Write(WriteError::VertexStorageAdmission { .. })
                    ) | (
                        true,
                        WriteTxnError::Write(WriteError::EdgeStorageAdmission { .. })
                    )
                ));
            };
            assert_admission(db.prepare_atomic_writes(groups.clone()).unwrap_err());
            assert_admission(db.write_atomic(&cx, groups.clone()).await.unwrap_err());
            ...
            let mut txn = db.begin(&txn_cx).unwrap();
            txn.write_atomic(&mut db, path()).unwrap();
            let staged_digest = txn.staged_effect_digest().unwrap();
            assert_admission(txn.write_atomic(&mut db, groups).unwrap_err());
            assert_eq!(txn.staged_effect_digest().unwrap(), staged_digest);
            assert_eq!(db.frontier().unwrap(), frontier);
            assert!(txn.vertex(&db, VId(99)).unwrap().is_none());
            assert!(txn.vertex(&db, VId(100)).unwrap().is_none());
            assert!(txn.edge(&db, EId(77)).unwrap().is_none());
            assert_eq!(
                txn.commit(&mut db, &cx).await.unwrap(),
