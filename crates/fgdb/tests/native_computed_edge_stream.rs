...
use fgdb::{
    Database, DatabaseKeys, MemVfs, NativeAggregateCursor, PreparedNativeRead, QueryError,
    QueryResult, WriteBatch,
};
...
use fgdb_gql::{
    ...
    GraphAggregateValue, GraphExactAverage, RelationBind,
};
use fgdb_types::{
...
fn symbols() -> RelationBind {
    RelationBind::new()
        .with_relation("R", R)
        .with_property("quantity", Q)
        .with_property("price", P)
}
...
fn edges(cut: u64) -> Vec<Edge> {
    ...
    let mut rows = vec![
        (10, VId(0), VId(1), Some(2), 5),
        (11, VId(0), VId(1), Some(5), 2),
        (12, VId(1), HIGH, Some(3), 7),
        (13, VId(0), HIGH, None, 7),
        (14, HIGH, VId(0), Some(-2), 4),
        (15, HIGH, HIGH, Some(0), 9),
    ];
    if cut >= 2 {
        rows[0].3 = Some(4);
        rows.retain(|e| e.0 != 11);
    }
    if cut >= 3 {
        rows.retain(|e| e.1 != VId(1) && e.2 != VId(1));
    }
    rows
}
fn seed() -> WriteBatch {
    let mut batch = WriteBatch::new(R);
    for vid in [VId(0), VId(1), HIGH, VId(99)] {
...
fn text(direction: usize, hops: usize, grouped: bool) -> String {
    ...
    let key = if grouped {
        format!(", {end} AS destination")
    } else {
        String::new()
    };
    let repeated = if grouped {
        format!(", {end} AS repeated")
    } else {
        String::new()
    };
    ...
    format!(
        "MATCH {pattern} FOR SYSTEM_TIME AS OF SEQ $cut RETURN SUM({value}) AS total{key}, COUNT(*) AS occurrences, AVG({value}) AS average{repeated}, COUNT(DISTINCT {value}) AS support{clause}"
    )
}
...
fn oracle(
    ...
    for &(_, b, value) in &oriented {
        if hops == 1 {
            complete.push((b, value));
        } else {
            for &(from, to, _) in &oriented {
                if from == b {
                    complete.push((to, value));
                }
            }
        }
    }
    ...
    for (end, value) in complete {
        groups
            .entry(grouped.then_some(end))
            .or_default()
            .push(value);
    }
    groups
        ...
        .map(|(key, all)| {
            ...
            let mut row = vec![if values.is_empty() {
                null()
            } else {
                GraphAggregateValue::Integer(values.iter().sum())
            }];
            if let Some(key) = key {
                row.push(GraphAggregateValue::Value(GraphValue::Vertex(key)));
            }
            ...
            row.push(GraphAggregateValue::Count(
...
fn native_computed_inputs_keep_bound_parameters_layouts_and_pinned_history_through_reopen() {
    let ((), report) = run_async_under_lab(0xc0a5_5001, |root| async move {
        ...
        for direction in 0..3 {
            for hops in 1..=2 {
                for grouped in [false, true] {
                    ...
                    let params = arguments(1, 2);
                    let prepared =
                        PreparedNativeRead::prepare(&statement, &params, symbols()).unwrap();
                    let cursor = prepared
                        .stream_aggregate(&db, &cx, &params, wide())
                        .unwrap();
                    fn send(_: &impl Send) {}
                    send(&cursor);
        ...
        for (direction, hops, grouped, mut cursor) in paused {
            ...
            let statement = text(direction, hops, grouped);
            let prepared =
                PreparedNativeRead::prepare(&statement, &arguments(1, 1), symbols()).unwrap();
            for cut in 0..=3 {
                for scale in [1, 2] {
                    let params = arguments(cut, scale);
                    let QueryResult::Rows { columns, rows } =
                        prepared.execute(&db, &cx, &params, wide()).unwrap()
                    else {
                        panic!("read");
                    };
            ...
            let mut pinned = prepared
                .stream_aggregate_in_view(&pin, &cx, &arguments(1, 1), wide())
                .unwrap();
...
fn native_expression_failures_and_exact_quotas_never_release_partial_summaries() {
    let ((), report) = run_async_under_lab(0xc0a5_5002, |root| async move {
        ...
        assert_eq!(
            drain(&mut prepared.stream_aggregate(&db, &cx, &params, exact).unwrap()),
            expected
        );
        for allowance in [
            ...
            let mut cursor = prepared
                .stream_aggregate(&db, &cx, &params, allowance)
                .unwrap();
        ...
        let prepared =
            PreparedNativeRead::prepare(invalid, &GqlParameters::new(), symbols()).unwrap();
        for value in [
            CanonicalScalar::Int(i64::MAX),
            CanonicalScalar::ucs_basic_text("private arithmetic operand").unwrap(),
        ] {
            let mut edit = WriteBatch::new(R);
            edit.set_edge_property(EId(15), Q, Some(value));
            db.write(&commit, edit).await.unwrap();
            let mut cursor = prepared
                .stream_aggregate(&db, &cx, &GqlParameters::new(), wide())
                .unwrap();
            let error = cursor.next().unwrap().unwrap_err();
            assert!(matches!(
                &error,
                GqlQueryError::Source(GraphAggregateError::InputExpression { row: 0, .. })
            ));
...
fn computed_source_admission_and_unsupported_relational_children_never_fall_back() {
    let ((), report) = run_async_under_lab(0xc0a5_5003, |root| async move {
        ...
        assert!(matches!(
            prepared.stream_aggregate(&db, &cx, &arguments(2, 2), GqlQueryPolicy::new(0, 0, 0, 0)),
        ...
        assert_eq!(row.values()[1].as_count(), Some(0));
        assert!(cursor.next().is_none());
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn output_text(direction: usize, hops: usize, grouped: bool) -> String {
    text(direction, hops, grouped)
        + " HAVING COUNT(*)>0 AND (average>$floor OR total IS NULL) SKIP $skip LIMIT $limit"
}
...
fn output_oracle(
    ...
    oracle(cut, scale, direction, hops, grouped)
        .into_iter()
        .filter(|row| {
            row[count].as_count().unwrap() > 0
                && (row[0].is_null()
                    || row[average].as_average().is_some_and(|value| {
                        value.numerator() > i128::from(floor) * i128::from(value.denominator())
                    }))
        })
        .skip(skip)
        .take(limit)
...
fn native_completed_output_clauses_preserve_history_parameters_and_textual_layouts() {
    let ((), report) = run_async_under_lab(0xc0a5_5101, |root| async move {
        ...
        for direction in 0..3 {
            for hops in 1..=2 {
                for grouped in [false, true] {
                    ...
                    let prepared =
                        PreparedNativeRead::prepare(&statement, &args, symbols()).unwrap();
        ...
        for (direction, hops, grouped, first, mut cursor) in paused {
            ...
            assert_eq!(
                rows,
                output_oracle(1, 2, direction, hops, grouped, -99, 0, 2)
            );
            assert_eq!(cursor.snapshot_seq(), basis);
            ...
            assert!(matches!(
                template.stream_aggregate_in_view(
                    &view,
                    &cx,
                    &future,
                    GqlQueryPolicy::new(0, 0, 0, 0)
    ...
    let ((), report) = run_async_under_lab(0xc0a5_5102, |root| async move {
        ...
        let cases = [
            ...
            (
                ...
                vec![
                    vec![
                        GraphAggregateValue::Value(GraphValue::Vertex(VId(0))),
                        GraphAggregateValue::Integer(-7),
                    ],
                    vec![
                        GraphAggregateValue::Value(GraphValue::Vertex(VId(1))),
                        GraphAggregateValue::Integer(22),
                    ],
                ],
            ),
            (
                "MATCH (a)-[r:R]->(b) RETURN SUM(r.quantity*r.price)/0 AS never GROUP BY b HAVING COUNT(*)<0 LIMIT 1",
                vec![],
            ),
            (
        ...
        for (statement, expected) in cases {
            ...
            let QueryResult::Rows { rows, .. } = db
                .query(&cx, statement, &params, symbols(), wide())
                .unwrap()
            else {
                panic!("read rows");
            };
            let mut stream = db
                .query_aggregate_stream(&cx, statement, &params, symbols(), wide())
                .unwrap();
        ...
        later.set_edge_property(
            EId(12),
            Q,
            Some(CanonicalScalar::ucs_basic_text("private having value").unwrap()),
        );
        later.set_edge_property(EId(15), Q, None);
        db.write(&commit, later).await.unwrap();
        for limit in [0, 1] {
            let statement = format!(
                "MATCH (a)-[r:R]->(b) RETURN b,MIN(r.quantity) AS m GROUP BY b HAVING m>0 OR TRUE LIMIT {limit}"
            );
            let mut stream = db
                .query_aggregate_stream(&cx, &statement, &GqlParameters::new(), symbols(), wide())
                .unwrap();
            let error = stream.next().unwrap().unwrap_err();
            assert!(matches!(
                &error,
                GqlQueryError::Source(GraphAggregateError::NonIntegerHaving { .. })
            ));
        ...
        for limit in [0, 1] {
            let statement = format!(
                "MATCH (a)-[r:R]->(b) RETURN b,1/(COUNT(*)-3) AS bad GROUP BY b LIMIT {limit}"
            );
            let mut stream = db
                .query_aggregate_stream(&cx, &statement, &GqlParameters::new(), symbols(), wide())
                .unwrap();
    ...
    let ((), report) = run_async_under_lab(0xc0a5_5103, |root| async move {
        ...
        for id in 0..1024 {
            batch.add_edge(
                ...
                VId(0),
                VId(id),
                vec![(Q, CanonicalScalar::Int(2)), (P, CanonicalScalar::Int(3))],
            );
        }
        ...
        assert_eq!(
            expected,
            vec![vec![
                GraphAggregateValue::Value(GraphValue::Vertex(VId(1023))),
        ...
        let mut stream = db
            .query_aggregate_stream(
                ...
                &params,
                symbols(),
                GqlQueryPolicy::new(1024, 0, u64::MAX, u64::MAX),
            )
            .unwrap();
        assert!(stream.next().is_none());
        assert_eq!(stream.row_stats().result_rows, 0);
