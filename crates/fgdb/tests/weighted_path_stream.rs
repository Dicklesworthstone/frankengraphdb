...
const MODES: [(&str, GraphCheapestPathMode); 4] = [
...
fn policy() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 1000, 5_000_000, 1_000_000)
}
...
fn request(
    ...
    source: VId,
    target: VId,
) -> BoundGraphCheapestPathQuery {
    let selector = if count.is_some() {
        "CHEAPEST $k"
    } else {
        "ANY CHEAPEST"
    };
    ...
    let input = format!(
        "MATCH p = {selector} {mode} (s){left}[e:ROAD*0..$max]{right}(t) COST e.cost RETURN p AS route"
    );
    let mut args = GqlParameters::new().with_uint64("max", 3).unwrap();
    if let Some(k) = count {
        args = args.with_uint64("k", k).unwrap();
    }
    PreparedGraphCheapestPathText::prepare(&input, resolver)
        .unwrap()
        .bind(source, target, &args)
        .unwrap()
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    let mut batch = WriteBatch::new(R);
    for id in [START, VId(1), END, VId(99)] {
...
fn drain(cursor: &mut Cursor<'_>) -> Vec<GraphCostPath> {
    cursor.by_ref().map(Result::unwrap).collect()
}
    ...
    let ((), report) = run_async_under_lab(0xc0a5_3011, |root| async move {
        ...
        for (name, _) in MODES {
            for direction in [
                ...
                for count in [None, Some(7)] {
                    ...
                    let expected = db
                        .execute_graph_cheapest_paths_governed(&cx, req.query(), k, policy())
                        .unwrap();
                    let mut live = db
                        .stream_graph_cheapest_path_text_governed(&cx, &req, policy())
                        .unwrap();
                    ...
                    let mut pinned = view
                        .stream_graph_cheapest_paths_governed(&cx, req.query(), k, policy())
                        .unwrap();
        ...
        for (req, expected, first, mut opened) in cases {
            ...
            for mut historical in [
                db.stream_graph_cheapest_path_text_governed_at(&cx, &req, basis, policy())
                    .unwrap(),
                view.stream_graph_cheapest_path_text_governed_at(&cx, &req, basis, policy())
                    .unwrap(),
                view.stream_graph_cheapest_path_text_governed(&cx, &req, policy())
                    .unwrap(),
            ] {
            ...
            let mut current = db
                .stream_graph_cheapest_paths_governed(
                    &cx,
                    req.query(),
                    req.ranked_count().unwrap_or(1),
                    policy(),
                )
                .unwrap();
        ...
        for (name, _) in MODES {
            for direction in [
                ...
                for count in [None, Some(7)] {
                    let req = request(name, direction, count, source, target);
                    let expected = txn
                        .execute_graph_cheapest_paths_governed(
                            &db,
                            &cx,
                            req.query(),
                            count.unwrap_or(1),
                        ...
                        .value;
                    let stream = txn
                        .stream_graph_cheapest_path_text_governed(&db, &cx, &req, policy())
                        .unwrap();
                    ...
                    assert!(
                        expected
                            .iter()
                            .all(|row| row.path().edges().all(|eid| eid != EId(888)))
                    );
            ...
            for outcome in 0..6 {
                for change in 0..3 {
                    ...
                    if outcome == 5 {
                        ...
                        invalid.set_edge_property(EId(10), W, None);
                    ...
                    let req = request(
                        mode,
                        GlaDirection::Forward,
                        Some(if outcome == 2 { 0 } else { 7 }),
                        START,
                        if outcome == 3 { VId(99) } else { END },
                    );
                    let allowance = if outcome == 4 {
                        GqlQueryPolicy::new(1000, 0, 5_000_000, 1_000_000)
                    } else {
                        policy()
                    };
                    let opened =
                        txn.stream_graph_cheapest_path_text_governed(&db, &cx, &req, allowance);
                    if outcome == 5 {
                        assert!(matches!(
                            opened,
                    ...
                    match change {
                        0 => {
                            winner.set_edge_property(EId(10), W, Some(CanonicalScalar::Int(-30)));
                        }
                        1 => {
                            winner.add_edge(
                                EId(60),
    ...
    let ((), report) = run_async_under_lab(0xc0a5_3014, |root| async move {
        ...
        for (mode, _) in MODES {
            for count in [0, 1, 3] {
                ...
                let mut repeat = db
                    .stream_graph_cheapest_path_text_governed(&cx, &req, exact)
                    .unwrap();
                ...
                if r.result_rows > 0 {
                    limits.push(GqlQueryPolicy::new(
                        1000,
                        r.result_rows - 1,
                        u64::MAX,
                        u64::MAX,
                ...
                for limit in limits {
                    let mut stream =
                        match db.stream_graph_cheapest_path_text_governed(&cx, &req, limit) {
                            Ok(stream) => stream,
                            ...
                            Some(Err(GqlQueryError::Evaluator(error))) => {
                                let original = match error.dimension {
                                    fgdb_gql::GlaLimitDimension::WorkUnits => {
                                        limit.evaluator.max_work_units
                                    }
                                    fgdb_gql::GlaLimitDimension::ScratchEntries => {
                                        limit.evaluator.max_scratch_entries
                                    }
                                };
                ...
                for result in [
                    db.stream_graph_cheapest_path_text_governed_at(
                        &cx,
                        &req,
                        CommitSeq(basis.0 + 1),
                        GqlQueryPolicy::new(0, 0, 0, 0),
                    ),
                    pinned.stream_graph_cheapest_path_text_governed_at(
                        &cx,
                        &req,
                        CommitSeq(basis.0 + 1),
                        GqlQueryPolicy::new(0, 0, 0, 0),
                ...
                assert!(matches!(
                    txn.stream_graph_cheapest_path_text_governed(
                        &foreign,
                        &cx,
                        &req,
                        GqlQueryPolicy::new(0, 0, 0, 0)
        ...
        assert!(matches!(
            db.stream_graph_cheapest_path_text_governed(&cx, &req, policy()),
            Err(GqlQueryError::Source(GraphCheapestPathError::Cost(
                GraphPathCostError::MissingWeight
            )))
        ));
...
fn database_backpressure_can_stop_after_one_of_a_trillion_paths() {
    let ((), report) = run_async_under_lab(0xc0a5_3015, |root| async move {
        ...
        let expected = db
            .execute_graph_cheapest_paths_governed(&cx, &query, 1, small)
            .unwrap();
        let mut stream = db
            .stream_graph_cheapest_paths_governed(&cx, &query, u64::MAX, small)
            .unwrap();
        assert_eq!(stream.row_stats().result_rows, 0);
        assert_eq!(stream.next().unwrap().unwrap(), expected.value[0]);
        assert_eq!(stream.evaluator_stats(), expected.evaluator);
        stream.close();
        let before = stream.evaluator_stats();
