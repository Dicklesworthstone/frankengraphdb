...
#[test]
fn incoming_two_hop_far_end_property_bang_inequality_sees_the_staged_continuation() {
    let ((), report) = run_async_under_lab(0x77_55, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-two-hop-where-dst-bang-ne-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
        ...
        // Durably, only the k=1 origin's chain is complete — it fails
        // != 1, so the base answer is empty.
        ...
        // The overlay stages the continuation onto the k=9 origin's chain.
