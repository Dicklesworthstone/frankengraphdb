...
fn keys() -> DatabaseKeys { DatabaseKeys::new([0x44; 32], DatabaseSecurityNamespaceId([0x55; 32]), [0x66; 32]) }
...
fn text(direction: GlaDirection, shape: usize, cut: Option<CommitSeq>, suffix: &str) -> String {
    ...
    let second = match shape { 1 => format!(", (a){left}[s:R]{right}(c)"),
        2 => format!("{left}[s:R]{right}(a)"), _ => format!("{left}[s:R]{right}(c)") };
    let temporal = cut.map(|s| format!(" FOR SYSTEM_TIME AS OF SEQ {}", s.0)).unwrap_or_default();
    let end = if shape == 2 { "a AS c" } else { "c" };
    format!("MATCH (a){left}[r:R]{right}(b){second}{temporal} RETURN r, a, s, b, {end}, r.p AS rp, s.p AS sp, b.p AS bp{suffix}")
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    let mut batch = WriteBatch::new(R);
    for id in IDS { batch.create_vertex(id, vec![], vec![(P, CanonicalScalar::Int(3))]); }
    for (id, a, b, value) in [(0, 0, 1, Some(2)), (1, 1, 2, Some(-4)), (2, 1, 2, Some(7)),
        (3, 2, 0, None), (u128::MAX, 1, 1, Some(9))] {
        batch.add_edge(EId(id), IDS[a], IDS[b], value.map(|v| vec![(P, CanonicalScalar::Int(v))]).unwrap_or_default());
    }
    db.write(cx, batch).await.unwrap()
}
fn plain(rows: &[GraphValueRow]) -> Vec<Vec<GraphValue>> { rows.iter().map(|row| row.values().to_vec()).collect() }
fn property(props: &[(PropertyKeyId, CanonicalScalar)]) -> GraphValue {
    GraphValue::Scalar(props.iter().find(|(key, _)| *key == P).map(|(_, v)| v.clone()).unwrap_or(CanonicalScalar::Null))
}
...
fn oracle(db: &Database<MemVfs>, cut: CommitSeq, direction: GlaDirection, shape: usize) -> Vec<Vec<GraphValue>> {
    let edges = db.edges_at(cut).unwrap();
    let orient = |a, b| match direction { GlaDirection::Forward => vec![(a, b)], GlaDirection::Reverse => vec![(b, a)],
        GlaDirection::Undirected if a == b => vec![(a, b)], _ => vec![(a, b), (b, a)] };
    let mut rows = Vec::new();
    for r in &edges { for (a, b) in orient(r.entry.src, r.entry.dst) {
        if r.entry.relation != R { continue; }
        for s in &edges { for (from, c) in orient(s.entry.src, s.entry.dst) {
            if s.entry.relation != R || from != (if shape == 1 { a } else { b }) || (shape == 2 && c != a) { continue; }
            rows.push(vec![GraphValue::Edge(r.entry.eid), GraphValue::Vertex(a), GraphValue::Edge(s.entry.eid),
                GraphValue::Vertex(b), GraphValue::Vertex(c), property(&r.props), property(&s.props),
...
fn native_and_direct_join_streams_preserve_history_properties_and_pins_through_reopen() {
    let ((), report) = run_async_under_lab(0x6a6f_6901, |root| async move {
        ...
        assert_eq!(opened.row_stats().snapshot_records, 0); assert_eq!(opened.evaluator_stats().work_units, 0);
        let first = opened.next().unwrap().unwrap(); drop(q);
        ...
        update.delete_edge(EId(2)); let changed = db.write(&commit, update).await.unwrap();
        let mut cascade = WriteBatch::new(R); cascade.delete_vertex(IDS[1]); let retired = db.write(&commit, cascade).await.unwrap();
        ...
        for cut in [CommitSeq(0), basis, changed, retired] {
            for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
                for shape in 0..3 {
                    ...
                    let eager = db.execute_graph_pattern_governed_at(&cx, &q, cut, policy()).unwrap().value;
                    assert_eq!(plain(&eager), wanted);
                    let mut stream = db.stream_graph_edges_governed_at(&cx, &q, cut, policy()).unwrap();
                    assert_eq!(plain(&stream.by_ref().collect::<Result<Vec<_>, _>>().unwrap()), wanted);
                    let native = PreparedNativeRead::prepare(&text(direction, shape, Some(cut), ""), &GqlParameters::new(), symbols).unwrap();
        ...
        let mut remaining = vec![first]; remaining.extend(opened.by_ref().map(Result::unwrap));
        assert_eq!(plain(&remaining), expected); assert_eq!(opened.snapshot_seq(), basis);
        ...
        let q = prepare(&input);
        let mut old = pinned.stream_graph_edges_governed(&cx, &q, policy()).unwrap();
        assert_eq!(plain(&old.by_ref().collect::<Result<Vec<_>, _>>().unwrap()), expected);
        let native = PreparedNativeRead::prepare(&input, &GqlParameters::new(), symbols).unwrap();
        let (_, mut old) = native.stream_in_view(&pinned, &cx, &GqlParameters::new(), policy()).unwrap();
        assert_eq!(plain(&old.by_ref().collect::<Result<Vec<_>, _>>().unwrap()), expected);
    });
...
fn total_quotas_parameter_binding_and_frontier_fences_still_precede_partial_delivery() {
    let ((), report) = run_async_under_lab(0x6a6f_6902, |root| async move {
        ...
        for p in [GqlQueryPolicy::new(r.snapshot_records - 1, 1000, u64::MAX, u64::MAX),
            ...
            let mut c = db.stream_graph_edges_governed(&cx, &q, p).unwrap(); let mut prefix = Vec::new();
            loop { match c.next() {
            ...
            assert_eq!(c.state(), EdgeScanState::Failed); c.close(); assert!(c.next().is_none());
        }
        ...
        let mut calls = 0;
        let native = PreparedNativeRead::prepare(input, &args, |kind, name: &str| { calls += 1; symbols(kind, name) }).unwrap();
        let resolved = calls;
        for floor in [-5, 3, 10] {
            let args = GqlParameters::new().with_int64("floor", floor).unwrap();
            let q = PreparedGraphText::prepare(input, symbols).unwrap().bind_parameters(&args).unwrap();
            let want = db.execute_graph_pattern_governed(&cx, &q, policy()).unwrap().value;
            let (_, mut c) = native.stream_in_view(&pinned, &cx, &args, policy()).unwrap();
            assert_eq!(c.by_ref().collect::<Result<Vec<_>, _>>().unwrap(), want);
        }
        assert_eq!(calls, resolved);
        assert!(native.stream(&db, &cx, &GqlParameters::new(), policy()).is_err());
        let zero = prepare(&text(GlaDirection::Forward, 0, None, " LIMIT 0"));
        for future in [
            db.stream_graph_edges_governed_at(&cx, &zero, CommitSeq(basis.0 + 1), GqlQueryPolicy::new(0, 0, 0, 0)).map(|_| ()),
            pinned.stream_graph_edges_governed_at(&cx, &zero, CommitSeq(basis.0 + 1), GqlQueryPolicy::new(0, 0, 0, 0)).map(|_| ()),
        ] { assert!(matches!(future, Err(GqlQueryError::Source(EdgeScanError::Source(_))))); }
        let mut zero = db.stream_graph_edges_governed(&cx, &zero, GqlQueryPolicy::new(0, 0, 1, 0)).unwrap();
        assert!(zero.next().is_none()); assert_eq!(zero.row_stats().snapshot_records, 0);
        let invalid = prepare("MATCH (a)-[r:R]->(b)-[s:R]->(c) RETURN r, a, c");
        assert!(matches!(db.stream_graph_edges_governed(&cx, &invalid, GqlQueryPolicy::new(0, 0, 0, 0)),
...
fn limit_one_joins_only_two_candidate_histories_despite_unrelated_graph_size() {
    let ((), report) = run_async_under_lab(0x6a6f_6903, |root| async move {
        ...
        for id in 0..2051 { b.create_vertex(VId(id), vec![], vec![]); }
        b.add_edge(EId(0), VId(0), VId(1), vec![]); b.add_edge(EId(1), VId(1), VId(2), vec![]);
        for id in 3..2051 { b.add_edge(EId(id), VId(id), VId(id), vec![]); }
        db.write(&commit, b).await.unwrap();
        let q = prepare("MATCH (a)-[r:R]->(b)-[s:R]->(c) RETURN r, a, s, b, c LIMIT 1");
        let mut c = db.stream_graph_edges_governed(&cx, &q, GqlQueryPolicy::new(2, 1, 2000, 256)).unwrap();
        assert_eq!(c.row_stats().snapshot_records, 0);
        let row = c.next().unwrap().unwrap();
        assert_eq!(row.values(), &[GraphValue::Edge(EId(0)), GraphValue::Vertex(VId(0)), GraphValue::Edge(EId(1)),
            GraphValue::Vertex(VId(1)), GraphValue::Vertex(VId(2))]);
        assert_eq!(c.row_stats().snapshot_records, 2); assert_eq!(c.state(), EdgeScanState::Exhausted);
        let e = c.evaluator_stats(); assert!(c.next().is_none()); assert_eq!(c.evaluator_stats(), e);
        let q = prepare("MATCH (a)-[r:R]->(b)-[s:R]->(c) RETURN r, a, s, b, c");
        let mut c = db.stream_graph_edges_governed(&cx, &q, GqlQueryPolicy::new(2, 1, 2000, 256)).unwrap();
        assert_eq!(c.next().unwrap().unwrap(), row); c.close(); assert!(c.next().is_none()); assert_eq!(c.evaluator_stats(), e);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
