...
fn oracle(db: &Database<MemVfs>, relation: RelationId) -> BTreeMap<VId, VId> {
    let mut labels: BTreeMap<_, _> = db
        .vertices()
        .unwrap()
        .iter()
        .map(|row| (row.vid, row.vid))
        .collect();
...
fn check(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle, relation: RelationId) {
    ...
    assert_eq!(
        db.standing_component(cx, handle, VId(999_999)).unwrap(),
        None
    );
}
...
fn writes_maintain_isolates_parallel_edges_relation_scope_and_vertex_cascades() {
    let ((), report) = run_async_under_lab(0x6363_6401, |root| async move {
        ...
        assert_eq!(
            db.standing_reachability(&cx, &reachability)
                .unwrap()
                .frontier(),
            db.frontier().unwrap()
        );
        let mut split = WriteBatch::new(R);
        split.delete_edge(EId(12));
        split.delete_edge(EId(14));
        ...
        invalid.add_edge(EId(99), VId(0), VId(1), vec![]);
...
fn failure_fences_only_the_derived_view_and_rebuild_uses_current_membership() {
    let ((), report) = run_async_under_lab(0x6363_6402, |root| async move {
        ...
        assert!(matches!(
            db.standing_component_count(&cx, &limited),
            Err(StandingQueryError::Unavailable { .. })
        ));
        assert!(matches!(
            db.standing_component(&cx, &limited, VId(100)),
            Err(StandingQueryError::Unavailable { .. })
        ));
        ...
        let mut retire = WriteBatch::new(S);
        retire.delete_vertex(VId(0));
        ...
        let before = *db
            ...
            .last_maintenance();
        assert!(matches!(
            db.rebuild_standing_query(&cx, &limited, GqlQueryPolicy::new(0, 2, 100_000, 100_000)),
            Err(StandingQueryError::Maintenance(
        ...
        assert_eq!(
            db.standing_components(&cx, &late)
                .unwrap()
                .last_maintenance()
                .delta_rows,
            0
...
fn equal_size_membership_replacement_is_admitted_at_exact_final_row_limit() {
    let ((), report) = run_async_under_lab(0x6363_6403, |root| async move {
        ...
        db.write(&commit, seed).await.unwrap();
        let handle = db
            .register_standing_components(&cx, R, GqlQueryPolicy::new(100_000, 2, 100_000, 100_000))
            .unwrap();
        let mut swap = WriteBatch::new(R);
        swap.create_vertex(VId(1), vec![], vec![]);
        swap.delete_vertex(VId(9));
        swap.add_edge(EId(2), VId(1), VId(10), vec![]);
        db.write(&commit, swap).await.unwrap();
        check(&db, &cx, &handle, R);
        assert_eq!(
            db.standing_component(&cx, &handle, VId(10)).unwrap(),
            Some(VId(1))
        );
    ...
    let ((), report) = run_async_under_lab(0x6363_6404, |root| async move {
        ...
        for mask in 0..64 {
            ...
            for (index, (a, b)) in sides.into_iter().enumerate() {
                if mask & (1 << index) != 0 {
                    batch.add_edge(EId(index as u128 + 1), VId(a), VId(b), vec![]);
                }
            }
            db.write(&commit, batch).await.unwrap();
            check(&db, &cx, &handle, R);
            let mut delete = WriteBatch::new(R);
            delete.delete_vertex(VId(0));
...
fn compaction_reopen_and_handle_kind_checks_preserve_current_state_boundaries() {
    let ((), report) = run_async_under_lab(0x6363_6405, |root| async move {
        ...
        db.write(&commit, seed).await.unwrap();
        assert!(matches!(
            db.standing_components(&cx, &reach),
            Err(StandingQueryError::Unsupported)
        ));
        assert!(matches!(
            db.standing_component_count(&cx, &reach),
            Err(StandingQueryError::Unsupported)
        ));
        assert!(matches!(
        ...
        let fresh = db.register_standing_components(&cx, R, policy()).unwrap();
        check(&db, &cx, &fresh, R);
        let mut split = WriteBatch::new(R);
        split.delete_edge(EId(1));
        db.write(&commit, split).await.unwrap();
        check(&db, &cx, &fresh, R);
