...
use fgdb_gql::algebra::{GlaDirection, GraphColumn, GraphExistence, GraphPatternBuilder,
    GraphValue, GraphValueRow, IntegerComparison, PreparedGraphPattern, VertexPredicate};
...
fn pattern(anti: bool) -> PreparedGraphPattern<GraphValueRow> {
    let mut outer = GraphPatternBuilder::new(); outer.vertex("person").unwrap();
    outer.filter("person", VertexPredicate::HasLabel(PERSON)).unwrap();
    let mut probe = GraphPatternBuilder::new();
    for name in ["person", "bridge", "company"] { probe.vertex(name).unwrap(); }
    probe.edge("person", R, GlaDirection::Forward, "bridge").unwrap();
    probe.edge("bridge", S, GlaDirection::Forward, "company").unwrap();
    probe.filter("company", VertexPredicate::IntegerProperty {
    ...
    outer.prepare_values_with_existence(&[constraint], &[
        GraphColumn::vertex("owner", "person"), GraphColumn::property("meta", "person", META),
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for vid in [VId(1), VId(2), VId(3), VId(6), HIGH] {
        first.create_vertex(vid, vec![PERSON], if vid == VId(2) {
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord], anti: bool) -> Vec<Vec<GraphValue>> {
    ...
    for person in vertices.iter().filter(|row| row.labels.contains(&PERSON)) {
        let exists = edges.iter().filter(|edge| edge.entry.relation == R && edge.entry.src == person.vid)
            .any(|first| edges.iter().filter(|edge| edge.entry.relation == S && edge.entry.src == first.entry.dst)
                .any(|second| vertices.iter().any(|row| row.vid == second.entry.dst
                    && row.props.iter().any(|(key, value)| *key == SCORE
                        && matches!(value, CanonicalScalar::Int(score) if *score >= 5)))));
        if exists != anti {
            answer.push(vec![GraphValue::Vertex(person.vid), GraphValue::Scalar(
                person.props.iter().find(|(key, _)| *key == META).map(|(_, value)| value.clone())
...
fn all_five_read_surfaces_keep_isolated_vertices_and_count_both_admitted_tables() {
    let ((), report) = run_async_under_lab(0xe715_0001, |root| async move {
        ...
        let exact = GqlQueryPolicy::new(15, 4, full.evaluator.work_units, full.evaluator.scratch_entries);
        assert_eq!(db.execute_graph_pattern_governed(&cx, &query, exact).unwrap(), full);
        for cap in [GqlQueryPolicy::new(15, 4, full.evaluator.work_units - 1, u64::MAX),
...
fn canonical_staging_moves_rows_between_exists_and_not_exists_without_rebasing_history() {
    let ((), report) = run_async_under_lab(0xe715_0002, |root| async move {
        ...
        let basis = seed(&mut db, &commit).await; let pinned = db.read_session().unwrap();
        let query = pattern(true); let before = db.execute_graph_pattern_governed(&cx, &query, policy()).unwrap().value;
        let mut txn = db.begin(&txn_cx).unwrap();
        let mut stage = WriteBatch::new(R);
        stage.delete_edge(EId(1)); stage.delete_edge(EId(2));
        ...
        assert_eq!(plain(&staged.value), expected); assert_eq!(ids(&staged.value), vec![VId(1), HIGH]);
        assert_eq!(staged.rows.snapshot_records, 13);
        let exact = GqlQueryPolicy::new(13, 2, staged.evaluator.work_units, staged.evaluator.scratch_entries);
        assert_eq!(txn.execute_graph_pattern_governed(&db, &cx, &query, exact).unwrap(), staged);
        assert_eq!(db.execute_graph_pattern_governed(&cx, &query, policy()).unwrap().value, before);
        ...
        assert_eq!(reopened.execute_graph_pattern_governed_at(&cx, &query, basis, policy()).unwrap().value, before);
        assert_eq!(pinned.execute_graph_pattern_governed(&cx, &query, policy()).unwrap().value, before);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0xe715_0003, |root| async move {
        ...
        for refused in [false, true] {
            for change in 0..3 {
                ...
                let query = pattern(true); let mut txn = db.begin(&txn_cx).unwrap();
                ...
                else { assert!(ids(&result.unwrap().value).contains(&VId(3))); }
                ...
                if change == 0 { result.unwrap(); assert!(db.vertex(VId(99)).unwrap().is_some()); }
                else {
...
fn edge_root_correlations_and_streaming_aggregation_reuse_the_same_probe() {
    let ((), report) = run_async_under_lab(0xe715_0004, |root| async move {
        ...
        for name in ["person", "bridge"] { outer.vertex(name).unwrap(); }
        outer.edge("person", R, GlaDirection::Forward, "bridge").unwrap();
        let mut probe = GraphPatternBuilder::new();
        for name in ["bridge", "company"] { probe.vertex(name).unwrap(); }
        probe.edge("bridge", S, GlaDirection::Forward, "company").unwrap();
        probe.filter("company", VertexPredicate::IntegerProperty { key: SCORE, comparison: IntegerComparison::GreaterOrEqual, value: 5 }).unwrap();
        let query = outer.prepare_values_with_existence(&[GraphExistence::exists(&probe)],
            &[GraphColumn::vertex("owner", "person")], 0, None).unwrap().with_duplicates();
        let rows = db.execute_graph_pattern_governed(&cx, &query, policy()).unwrap();
        assert_eq!(rows.rows.snapshot_records, 6); assert_eq!(ids(&rows.value), vec![VId(1), VId(1)]);
        let summary = PreparedGraphAggregate::prepare(query, &[], &[GraphAggregate::count_rows("qualified")], 0, None).unwrap();
        assert_eq!(db.execute_graph_aggregate_governed(&cx, &summary, policy()).unwrap().value[0].get(0).unwrap().as_count(), Some(2));
        let summary = PreparedGraphAggregate::prepare(pattern(true), &[], &[GraphAggregate::count_rows("missing")], 0, None).unwrap();
        let txn = db.begin(&txn_cx).unwrap();
        let rows = txn.execute_graph_aggregate_governed(&db, &cx, &summary, policy()).unwrap();
        assert_eq!(rows.rows.snapshot_records, 15); assert_eq!(rows.value[0].get(0).unwrap().as_count(), Some(4));
        txn.abort();
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn wrong_owner_future_snapshot_and_cancellation_never_become_successful_absence() {
    let ((), report) = run_async_under_lab(0xe715_0005, |root| async move {
        ...
        let txn = db.begin(&txn_cx).unwrap(); let query = pattern(true);
        root.cancel_with(CancelKind::User, Some("correlated existence cancellation"));
        assert!(matches!(txn.execute_graph_pattern_governed(&foreign, &cx, &query, policy()), Err(GqlQueryError::Source(WriteTxnError::WrongDatabase))));
