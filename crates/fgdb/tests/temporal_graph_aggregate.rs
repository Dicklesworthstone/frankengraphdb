    ...
    let ((), report) = run_async_under_lab(0x7e4a_0001, |root| async move {
        ...
        for (seq, expected) in [(seq1, (2, 6)), (seq2, (3, 18)), (seq3, (2, 14))] {
            ...
            let temporal = db
                ...
                .unwrap();
            let explicit = db
                .execute_graph_aggregate_governed_at(&query, bound.aggregate(), seq, policy())
                .unwrap();
            assert_eq!(temporal, explicit);
            assert_eq!(
                values(&temporal),
                expected,
                "aggregate history mismatch at {seq:?}"
            );
        }
        ...
        assert_eq!(
            values(
                &pinned
                    .execute_temporal_graph_aggregate_text_governed(&query, &bind(seq1), policy())
                    .unwrap()
            ),
            (2, 6)
        );
        assert_eq!(
            values(
                &pinned
                    .execute_temporal_graph_aggregate_text_governed(&query, &bind(seq2), policy())
                    .unwrap()
            ),
            (3, 18)
        );
        assert!(
            pinned
                .execute_temporal_graph_aggregate_text_governed(&query, &bind(seq3), policy())
                .is_err(),
            "pinned view cannot read its future"
        );
        ...
        for (seq, expected) in [(seq1, (2, 6)), (seq2, (3, 18)), (seq3, (2, 14))] {
            assert_eq!(
                values(
                    &db.execute_temporal_graph_aggregate_text_governed(
                        &query,
                        &bind(seq),
                        policy()
                    )
...
fn temporal_grouping_and_having_rebind_snapshot_without_recompilation() {
    let ((), report) = run_async_under_lab(0x7e4a_0002, |root| async move {
        ...
        let future = template
            ...
            .unwrap();
        assert!(
            db.execute_temporal_graph_aggregate_text_governed(&query, &future, policy())
                .is_err()
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
