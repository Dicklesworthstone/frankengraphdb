...
fn keys() -> DatabaseKeys {
    DatabaseKeys::new(
        [0x14; 32],
        DatabaseSecurityNamespaceId([0x25; 32]),
        [0x36; 32],
    )
}
...
fn statement(
    ...
    let temporal = cut
        ...
        .unwrap_or_default();
    format!(
        "MATCH (a){}{temporal} WHERE {}EXISTS {{ MATCH (b){} WHERE x.p > 5 }} RETURN r, a, b, b.p AS bp",
        atom("r", "R", "b", outer),
        if anti { "NOT " } else { "" },
        atom("", "Q", "x", inner)
    )
        ...
        let props = if at == 3 {
            ...
            vec![(
                ...
                if at == 4 {
                    ...
                    CanonicalScalar::Int(if at == 2 { 7 } else { 1 })
                },
            )]
        };
...
fn orient(from: VId, to: VId, direction: GlaDirection) -> Vec<(VId, VId)> {
    match direction {
        GlaDirection::Forward => vec![(from, to)],
        GlaDirection::Reverse => vec![(to, from)],
        GlaDirection::Undirected if from == to => vec![(from, to)],
        _ => vec![(from, to), (to, from)],
    }
}
        ...
        for (a, b) in orient(edge.entry.src, edge.entry.dst, outer) {
            ...
            for child in edges.iter().filter(|e| e.entry.relation == Q) {
                for (from, x) in orient(child.entry.src, child.entry.dst, inner) {
                    if from == b && matches!(field(db, cut, x), CanonicalScalar::Int(v) if v > 5) {
                        witnesses += 1;
                    }
                }
            }
            if (witnesses > 0) != anti {
                rows.push(vec![
                    GraphValue::Edge(edge.entry.eid),
                    GraphValue::Vertex(a),
                    GraphValue::Vertex(b),
...
fn probe_truth_and_absence_are_exact_at_six_history_cuts_and_remain_pinned_through_reopen() {
    let ((), report) = run_async_under_lab(0x7072_6f01, |root| async move {
                ...
                for inner in directions {
                    for anti in [false, true] {
                        ...
                        let native = PreparedNativeRead::prepare(
                            &statement(outer, inner, anti, Some(cut)),
                            &params,
                            symbols,
                        )
                        .unwrap();
        ...
        old.extend(paused.by_ref().map(Result::unwrap));
        assert_eq!(plain(&old), expected);
...
fn one_witness_uses_two_candidate_records_and_anti_join_quota_refusal_is_not_absence() {
    let ((), report) = run_async_under_lab(0x7072_6f03, |root| async move {
        ...
        let mut roots = WriteBatch::new(R);
        for i in 0..5 {
            roots.create_vertex(VId(i), vec![], vec![]);
        }
        roots.add_edge(EId(0), VId(0), VId(1), vec![]);
        let mut probes = WriteBatch::new(Q);
        probes.add_edge(EId(1), VId(1), VId(2), vec![]);
        for id in 2..2050 {
            probes.add_edge(EId(id), VId(3), VId(4), vec![]);
        }
        db.write_atomic(&commit, vec![roots, probes]).await.unwrap();
        let positive = prepare(
            "MATCH (a)-[r:R]->(b) WHERE EXISTS { MATCH (b)-[:Q]->(x) } RETURN r, a, b LIMIT 1",
        );
        let small = GqlQueryPolicy::new(2, 1, 10_000, 10_000);
        let mut stream = db
            .stream_graph_edges_governed(&cx, &positive, small)
            .unwrap();
        assert_eq!(
            stream.next().unwrap().unwrap().values()[0],
            GraphValue::Edge(EId(0))
        );
        assert_eq!(stream.row_stats().snapshot_records, 2);
        assert!(stream.next().is_none());
        let negative = prepare(
            "MATCH (a)-[r:R]->(b) WHERE NOT EXISTS { MATCH (b)-[:Q]->(a) } RETURN r, a, b LIMIT 1",
        );
        let mut stream = db
            .stream_graph_edges_governed(&cx, &negative, GqlQueryPolicy::new(1, 1, 10_000, 10_000))
            .unwrap();
        ...
        assert!(stream.next().is_none());
        let mut retry = db
            .stream_graph_edges_governed(&cx, &negative, small)
            .unwrap();
...
fn probe_limits_are_cumulative_and_binding_frontier_and_unsupported_errors_precede_delivery() {
    let ((), report) = run_async_under_lab(0x7072_6f04, |root| async move {
        ...
        for limited in [
            ...
            let mut stream = db.stream_graph_edges_governed(&cx, &q, limited).unwrap();
        ...
        let zero = prepare(
            &(statement(GlaDirection::Forward, GlaDirection::Forward, false, None) + " LIMIT 0"),
        );
        let mut zero_stream = db
            .stream_graph_edges_governed(&cx, &zero, GqlQueryPolicy::new(0, 0, 1, 0))
            .unwrap();
        assert!(zero_stream.next().is_none());
        assert_eq!(zero_stream.row_stats().snapshot_records, 0);
        assert!(matches!(
