...
#[test]
fn two_hop_far_end_bang_inequality_sees_the_staged_continuation() {
    let ((), report) = run_async_under_lab(0x56_02, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-two-hop-where-dst-bang-ne-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
        ...
        assert_eq!(
            txn.execute_gql(&db, filtered, &bind)
                .expect("overlay far-end bang inequality MATCH executes"),
            vec![VId(6)]
        );
        assert_eq!(
            db.execute_gql(filtered, &bind)
                .expect("base far-end bang inequality MATCH executes"),
            Vec::<VId>::new()
        );
        txn.abort();
    });
