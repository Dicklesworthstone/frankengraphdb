...
use fgdb_types::{
    CanonicalScalar, CanonicalScalarKind, CommitSeq, DatabaseSecurityNamespaceId,
...
const WITNESSES: &[&str] = &[
    "", "a", "ab", "ab-tail", "ab-reject", "ac", "z", "zz",
    "\u{ff}", "\u{ff}-tail", "\u{100}", "\u{7ff}", "\u{7ff}-tail", "\u{800}",
    "é", "éclair", "e\u{301}", "e\u{301}clair", "東京", "東京駅",
    "z\u{10ffff}", "z\u{10ffff}tail", "\u{10ffff}", "\u{10ffff}tail",
    "\u{10ffff}\u{10ffff}", "\u{10ffff}\u{10ffff}tail",
];
...
fn ids(rows: &[GraphValueRow]) -> Vec<VId> {
    let mut result: Vec<_> = rows.iter().map(|r| r.get(0).unwrap().as_vertex().unwrap()).collect();
...
fn remember(history: &mut History, id: VId, value: &Option<CanonicalScalar>) {
    if let Some(CanonicalScalar::Text(value)) = value {
        history.entry(id).or_default().push(value.as_str().to_owned());
    }
}
...
fn candidates(history: &History, prefix: &str) -> u64 {
    ...
    history.values().filter(|values| values.iter().any(|v| v.starts_with(prefix))).count() as u64
}
...
impl Case {
    fn new(prefix: Option<&'static str>, literal: bool, residual: bool) -> Self {
        ...
        let operand = if literal {
            prefix.map_or_else(|| "NULL".to_owned(), |p| format!("'{}'", p.replace('\'', "''")))
        } else {
            "$prefix".to_owned()
        };
        let predicate = format!("n.name STARTS WITH {operand}{}", if residual { " AND n.name <> 'ab-reject'" } else { "" });
        let args = if literal { GqlParameters::new() } else { GqlParameters::new().with_scalar("prefix", value.clone()).unwrap() };
        let declarations = if literal { vec![] } else { vec![("prefix", GqlParameterType::Scalar(CanonicalScalarKind::of(&value)))] };
        ...
        Self {
            ...
            scan: bind(format!("MATCH (n) WHERE {predicate} OPTIONAL MATCH (other) WHERE other=n RETURN DISTINCT n")),
        }
    }
}
...
fn cases() -> Vec<Case> {
    let mut result: Vec<_> = [
        "", "ab", "\u{ff}", "\u{7ff}", "é", "e\u{301}", "東京", "z\u{10ffff}",
        "\u{10ffff}", "\u{10ffff}\u{10ffff}", "absent",
    ].into_iter().map(|prefix| Case::new(Some(prefix), false, false)).collect();
    result.extend([
...
fn oracle(db: &Database<MemVfs>, at: CommitSeq, case: &Case) -> Vec<VId> {
    db.vertices_at(at).unwrap().into_iter().filter(|row| {
        row.props.iter().any(|(key, value)| {
            *key == NAME && matches!(value, CanonicalScalar::Text(value)
...
fn boundary(
    db: &Database<MemVfs>, cx: &QueryCx, at: CommitSeq,
    pattern: &PreparedGraphPattern<GraphValueRow>, expected: &[VId], charge: u64,
) {
    let exact = db.execute_graph_pattern_governed_at(
        cx, pattern, at, GqlQueryPolicy::new(charge, 1_000_000, u64::MAX, u64::MAX),
    ).unwrap();
    ...
    for seed in [11_u64, 23, 0xdead_f00d] {
        let ((), report) = run_async_under_lab(0xd10_0000 + seed, move |root| async move {
            ...
            for id in 100..124 {
                ...
                initial.create_vertex(VId(id), vec![], value.into_iter().map(|v| (NAME, v)).collect());
            }
            for id in 150..154 {
            ...
            let temporal = PreparedTemporalGraphText::prepare(
                "MATCH (n) FOR SYSTEM_TIME AS OF SEQ $at WHERE n.name STARTS WITH 'ab' RETURN n", symbols,
            ).unwrap();
            ...
            for step in 0..=4_u64 {
                if step != 0 {
                    ...
                    for (id, value) in [
                        (150, Some(text(if step % 2 == 0 { "ab-moving" } else { "outside" }))),
                        (151, if step % 2 == 0 { Some(text("ab-moving")) } else { Some(CanonicalScalar::Null) }),
                        (152, if step % 2 == 0 { Some(text("ab-moving")) } else { None }),
                    ] {
                        remember(&mut history, VId(id), &value);
                        batch.set_vertex_property(VId(id), NAME, value);
                    }
                    if step == 2 { batch.delete_vertex(VId(153)); }
                ...
                if step == 3 {
                    drop(db);
                    db = Database::open_with_vfs(&commit, vfs.clone(), &path, keys()).await.unwrap();
                }
                for (case, before) in cases.iter().zip(&original) {
                    ...
                    let retained = pinned.execute_graph_pattern_governed(&cx, &case.indexed, policy()).unwrap();
                    assert_eq!(ids(&retained.value), *before, "seed={seed} step={step} prefix={:?}", case.prefix);
                    assert_eq!(retained.rows.snapshot_records, case.prefix.map_or(original_scan_count, |p| candidates(&pinned_history, p)));
                    let retained_scan = pinned.execute_graph_pattern_governed(&cx, &case.scan, policy()).unwrap();
                ...
                for at in &cuts {
                    ...
                    let run = db.execute_temporal_graph_text_governed(&cx, &bound, policy()).unwrap();
                    assert_eq!(ids(&run.value), oracle(&db, *at, &temporal_case));
                    assert_eq!(run.rows.snapshot_records, candidates(&history, "ab"));
                }
                let bound = temporal.bind_parameters(&GqlParameters::new().with_uint64("at", old.0).unwrap()).unwrap();
                let run = pinned.execute_temporal_graph_text_governed(&cx, &bound, policy()).unwrap();
...
fn prefix_boolean_and_multiple_domain_boundaries_preserve_scan_semantics() {
    let ((), report) = run_async_under_lab(0xd10_0040, |root| async move {
        ...
        for (statement, expected) in [
            ("MATCH (n) WHERE n.name STARTS WITH 'ab' OR n.name = 'z' RETURN n", vec![VId(1), VId(2), VId(3)]),
            ("MATCH (n) WHERE NOT (n.name STARTS WITH 'ab') RETURN n", vec![VId(3), VId(4)]),
            ("MATCH (n) WHERE (n.name STARTS WITH 'ab' OR n.name = 'z') AND n.name <> 'ab-reject' RETURN n", vec![VId(1), VId(3)]),
            ("MATCH (n) OPTIONAL MATCH (other) WHERE other=n AND other.name STARTS WITH 'ab' RETURN DISTINCT n", (1..=6).map(VId).collect()),
            ("MATCH (n) WHERE n.name STARTS WITH 'ab' OPTIONAL MATCH (other) WHERE other.name='z' RETURN DISTINCT other", vec![VId(3)]),
        ] {
...
fn prefix_nontext_properties_preserve_scan_unknowns_including_null_prefix() {
    let ((), report) = run_async_under_lab(0xd10_0041, |root| async move {
        ...
        let commit = contexts.commit();
        let cx = contexts.query();
        for value in [CanonicalScalar::Int(7), CanonicalScalar::Bool(true), CanonicalScalar::bytes(vec![97, 98]).unwrap()] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let mut batch = WriteBatch::new(R);
            ...
            for prefix in [Some("ab"), Some(""), None] {
                ...
                let expected = if prefix.is_some() { vec![VId(1)] } else { vec![] };
...
fn narrow_prefix_charges_ignore_growth_from_one_thousand_to_fifty_thousand() {
        ...
        let (charged, report) = run_async_under_lab(seed, move |root| async move {
            ...
            for start in (0..total).step_by(1_000) {
                ...
                for id in start..(start + 1_000).min(total) {
                    ...
                    let name = if id < 10 { format!("narrow-{id}") } else if id % 2 == 0 { format!("before-{id}") } else { format!("outside-{id}") };
            ...
            let run = db.execute_graph_pattern_governed_at(&cx, &case.indexed, at, policy()).unwrap();
    ...
    assert_eq!(charges[0].1, charges[1].1, "prefix work must not grow with unrelated vertices");
}
