...
use fgdb::{
    Database, DatabaseKeys, GqlError, MemVfs, ReadError, RelationBind, WriteBatch, WriteError,
    WriteTxnError,
};
use fgdb_delta_types::{LabelId, PropertyKeyId, RelationId};
use fgdb_gql::{
    BudgetedGqlError, GlaExecutionError, GlaExecutionLimits, GqlEvidenceAuditError,
    GqlExecutionBudget, GqlParameters, PreparedGqlTemplate,
};
use fgdb_types::{
...
fn template() -> PreparedGqlTemplate {
    PreparedGqlTemplate::prepare(
        "MATCH (a)-[:R]->(b) WHERE b.n>=$min RETURN b SKIP$skip LIMIT$cap",
        &bind(),
    )
...
fn bindings_flow_through_live_historical_pinned_staged_budgeted_and_limited_queries() {
    let ((), report) = run_async_under_lab(0x7061_0001, |root| async move {
        ...
        assert_eq!(
            db.execute_prepared_query(&wide).unwrap(),
        ...
        assert_eq!(
            pinned
                .execute_prepared_query_limited(&page, limits)
                .unwrap()
                .value,
            vec![VId(3)]
        );
        ...
        assert_eq!(
            txn.execute_prepared_query(&db, &high).unwrap(),
            vec![VId(2), VId(5)]
        );
        assert_eq!(
            txn.execute_prepared_query_budgeted(&db, &high, GqlExecutionBudget::new(4, 2))
                .unwrap()
                .value,
            vec![VId(2), VId(5)]
        );
        assert_eq!(
            txn.execute_prepared_query_limited(&db, &high, limits)
                .unwrap()
                .value,
            vec![VId(2), VId(5)]
        );
        ...
        assert_eq!(
            db.execute_prepared_query(&high).unwrap(),
            vec![VId(2), VId(5)]
        );
        assert!(
            db.execute_prepared_query_at(&high, basis)
                .unwrap()
                .is_empty()
        );
        ...
        assert!(matches!(
            txn.execute_prepared_query(&db, &high),
...
fn evidence_replays_concrete_values_and_rejects_other_bindings_even_with_identical_rows() {
    let ((), report) = run_async_under_lab(0x7061_0002, |root| async move {
        ...
        assert_eq!(
            db.execute_prepared_query(&low).unwrap(),
            db.execute_prepared_query(&equivalent_rows).unwrap()
        );
        let artifact = db.execute_prepared_query_artifact(&low).unwrap();
        let bytes = artifact.to_bytes();
        assert_eq!(
            db.audit_prepared_query_artifact(&low, &bytes)
                .unwrap()
                .rows(),
            artifact.rows()
        );
        assert!(matches!(
            db.audit_prepared_query_artifact(&equivalent_rows, &bytes),
        ...
        assert_eq!(
            db.audit_prepared_query_artifact(&rebuilt, &bytes)
                .unwrap()
                .rows(),
            artifact.rows()
        );
        ...
        let mut txn = db.begin(&txn_cx).unwrap();
        let overlay = txn
            .execute_prepared_query_overlay_artifact(&db, &low)
            .unwrap();
        let overlay_bytes = overlay.to_bytes();
        let mut cursor = txn
            .open_untrusted_prepared_query_overlay_artifact_cursor(&db, &low, &overlay_bytes)
            .unwrap();
        ...
        txn.write(&mut db, staged).unwrap();
        assert!(matches!(
            txn.audit_prepared_query_overlay_artifact(&db, &low, &overlay_bytes),
...
fn parameter_queries_preserve_owner_fences_and_empty_scan_conflicts() {
    let ((), report) = run_async_under_lab(0x7061_0003, |root| async move {
        ...
        let mut txn = db.begin(&txn_cx).unwrap();
        assert!(matches!(
            txn.execute_prepared_query(&foreign, &query),
        ...
        assert!(matches!(
            db.execute_prepared_query_at(&query, CommitSeq(frontier.0 + 1)),
