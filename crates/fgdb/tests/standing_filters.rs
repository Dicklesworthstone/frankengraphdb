...
use fgdb_gql::{
    GqlParameters, GqlQueryPolicy, GqlScalarParameter, GraphSetOperand, GraphSetPredicateOp,
...
fn gt(column: usize) -> GraphSetPredicateOp {
    GraphSetPredicateOp::Compare {
        left: GraphSetOperand::Column(column),
...
fn difference(new: &Bag, old: &Bag) -> Bag {
    ...
    for (r, w) in old {
        *delta.entry(r.clone()).or_default() -= w;
    }
...
fn expected(db: &Database<MemVfs>, mode: usize) -> Bag {
    ...
    for v in db.vertices_at(db.frontier().unwrap()).unwrap() {
        ...
        if keep {
            *out.entry(vec![GraphValue::Scalar(value)]).or_default() += 1;
        }
    }
    out
}
...
fn filters_follow_mixed_commits_and_compose_with_shared_sets_and_joins() {
    let ((), report) = run_async_under_lab(0x464c_0101, |root| async move {
        ...
        for (id, value) in [
            (1, Some(CanonicalScalar::Int(2))),
            (2, Some(CanonicalScalar::Int(2))),
            (3, Some(CanonicalScalar::Int(-1))),
            (4, None),
            (5, Some(CanonicalScalar::Bool(true))),
            (u128::MAX, Some(CanonicalScalar::Int(3))),
        ] {
        ...
        let codes = [
            vec![gt(0)],
            vec![gt(0), GraphSetPredicateOp::Not],
            vec![GraphSetPredicateOp::IsNull {
                operand: GraphSetOperand::Column(0),
                is_null: true,
            }],
        ];
        let filters: Vec<_> = codes
            .iter()
            .map(|code| {
                db.register_standing_filter(&cx, &source, code, policy())
                    .unwrap()
            })
            .collect();
        let duplicate = db
            ...
            .unwrap();
        let again = db
            .register_standing_filter(&cx, &duplicate, &[gt(0)], policy())
            .unwrap();
        let join = db
            .register_standing_join(&cx, &again, &filters[0], &[(0, 0)], policy())
            .unwrap();
        let filtered_join = db
            .register_standing_filter(&cx, &join, &[gt(1)], policy())
            .unwrap();
        let mut previous: Vec<_> = filters
            .iter()
            .map(|h| plain(db.standing_filter(&cx, h).unwrap().rows()))
            .collect();
        for h in &filters {
            assert!(db.standing_filter_delta(&cx, h).unwrap().is_none());
        }
        for step in 0..5 {
            ...
            match step {
                ...
                1 => {
                    change.set_vertex_property(VId(4), P, Some(CanonicalScalar::Int(2)));
                ...
                3 => {
                    add(&mut change, 10, Some(CanonicalScalar::Int(4)));
                }
                _ => {
            ...
            for (mode, h) in filters.iter().enumerate() {
                let wanted = expected(&db, mode);
                let view = db.standing_filter(&cx, h).unwrap();
                assert_eq!(view.frontier(), at);
                assert_eq!(plain(view.rows()), wanted);
                assert!(view.ordered_rows().is_none());
                assert_eq!(
                    plain(db.standing_filter_delta(&cx, h).unwrap().unwrap().rows()),
                    difference(&wanted, &previous[mode])
                );
                assert_eq!(
                    db.standing_filter_total(&cx, h).unwrap().to_i128(),
                    Some(wanted.values().sum())
                );
                assert_eq!(db.standing_filter_columns(&cx, h).unwrap(), &["p"]);
                previous[mode] = wanted;
            }
            let wanted: Bag = expected(&db, 0)
                .into_iter()
                .map(|(r, w)| ([r.clone(), r].concat(), 2 * w * w))
                .collect();
            assert_eq!(
                plain(db.standing_filter(&cx, &filtered_join).unwrap().rows()),
                wanted
            );
        }
        assert_eq!(
            db.standing_filter(&cx, &again)
                .unwrap()
                .last_maintenance()
                .delta_rows,
            0
...
fn final_swaps_fit_quotas_and_failed_views_rebuild_without_undoing_commits() {
    let ((), report) = run_async_under_lab(0x464c_0102, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut seed = WriteBatch::new(RelationId(1));
        add(&mut seed, 1, Some(CanonicalScalar::Int(2)));
        add(&mut seed, 2, Some(CanonicalScalar::Int(-1)));
        db.write(&commit, seed).await.unwrap();
        let source = db
            .register_standing_rows(&cx, pattern("MATCH (n) RETURN n.p AS p"), policy())
            .unwrap();
        let filter = db
            .register_standing_filter(&cx, &source, &[gt(0)], bounded(1))
            .unwrap();
        let child = db
            .register_standing_filter(&cx, &filter, &[gt(0)], policy())
            .unwrap();
        let sibling = db
        ...
        assert_eq!(
            db.standing_filter_total(&cx, &filter).unwrap().to_i128(),
            Some(1)
        );
        let mut grow = WriteBatch::new(RelationId(1));
        grow.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(3)));
        let at = db.write(&commit, grow).await.unwrap();
        assert!(at > before);
        assert!(
            matches!(db.standing_filter(&cx,&filter),Err(StandingQueryError::Unavailable {
            frontier,reason:StandingQueryFailure::ResultBudget }) if frontier==before)
        );
        assert!(matches!(
            db.standing_filter(&cx, &child),
        ...
        assert!(
            matches!(db.standing_filter(&cx,&filter),Err(StandingQueryError::Unavailable {frontier,..}) if frontier==before)
        );
        ...
        assert_eq!(
            plain(db.standing_filter(&cx, &child).unwrap().rows()),
            expected(&db, 0)
        );
        let mut change = WriteBatch::new(RelationId(1));
        change.delete_vertex(VId(1));
        ...
        assert_eq!(
            db.standing_filter_total(&cx, &child).unwrap().to_i128(),
            Some(1)
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn schema_owner_and_snapshot_limits_survive_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0x464c_0103, |root| async move {
        ...
        let mut db = Database::create_with_vfs(&commit, vfs.clone(), &path, keys())
            ...
            .unwrap();
        let source = db
            .register_standing_rows(&cx, pattern("MATCH (n) RETURN n AS a, n AS b"), policy())
            .unwrap();
        assert!(matches!(
            db.register_standing_filter(&cx, &source, &[gt(0)], policy()),
            Err(StandingQueryError::FilterSchema(_))
        ));
        let equal = GraphSetPredicateOp::Compare {
        ...
        let source = db
            ...
            .unwrap();
        let filter = db
            .register_standing_filter(&cx, &source, &[equal], policy())
            .unwrap();
        assert_eq!(
            plain(db.standing_filter(&cx, &filter).unwrap().rows()),
            wanted
...
fn filter_maintenance_does_not_traverse_unchanged_parent_results() {
    let ((), report) = run_async_under_lab(0x464c_0104, |root| async move {
        ...
        for size in [2, 1024] {
            ...
            let mut seed = WriteBatch::new(RelationId(1));
            for id in 1..=size {
                add(&mut seed, id, Some(CanonicalScalar::Int(id as i64)));
            }
            db.write(&commit, seed).await.unwrap();
            let source = db
                .register_standing_rows(&cx, pattern("MATCH (n) RETURN n.p AS p"), policy())
                .unwrap();
            let filter = db
                .register_standing_filter(&cx, &source, &[gt(0)], policy())
                .unwrap();
            let mut change = WriteBatch::new(RelationId(1));
            ...
            assert_eq!(stats.delta_rows, 2);
            work.push((stats.work_units, stats.scratch_entries));
        }
