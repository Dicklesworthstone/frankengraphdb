...
use fgdb_gql::{
    ...
    GraphIntegerOp as Op, GraphNullPlacement, GraphSetProjection as Projection,
...
fn descending(query: PreparedGraphAggregate) -> PreparedGraphAggregate {
    query
        .with_result_clauses(
            &[],
            &[GraphAggregateOrder::descending(Column::Aggregate(1))],
        )
        .unwrap()
}
fn identity_output(query: PreparedGraphAggregate) -> PreparedGraphAggregate {
    query
        .with_output_projection(vec![Projection::new("id", Value::Column(0))])
        .unwrap()
}
...
fn ids(rows: &[GraphAggregateRow]) -> Vec<i64> {
    rows.iter()
        .map(
            |row| match row.get(0).unwrap().as_value().unwrap().as_scalar().unwrap() {
...
fn matrix_definition(
    ...
    let base = PreparedGraphAggregate::prepare_projected(
        ...
        &[
            ...
            GraphAggregate::count_distinct("distinct_scores", 0),
        ],
        u64::from(grouped),
        Some(2),
    )
        ...
        .with_result_clauses(
            ...
            &[
                GraphAggregateOrder {
                    ...
                    descending: true,
                    nulls: if direction == GlaDirection::Reverse {
    ...
    let amount = GraphIntegerExpression::prepare(&[
        Op::Column(key_width + 1),
        Op::Literal(Some(0)),
        Op::Coalesce,
    ])
    .unwrap();
    base.with_having_expression(&having)
        .unwrap()
...
fn ranked_sequences_match_snapshot_across_computed_having_and_join_shapes() {
    let ((), report) = run_async_under_lab(0x6a91, |root| async move {
            ...
            for direction in [
                ...
                for grouped in [false, true] {
                    for distinct in [false, true] {
                        ...
                        let handle = db
                            .register_standing_query(&query, definition.clone(), policy(2))
                            .unwrap();
        ...
        for id in 1..=6 {
            ...
            if id != 4 {
                props.push((SCORE, CanonicalScalar::Int((id % 2) as i64)));
            }
        ...
        for (step, writes) in batches.into_iter().enumerate() {
            ...
            for (handle, definition) in &registered {
                ...
                if step == 2 {
                    assert_eq!(
                        db.rebuild_standing_query(&query, handle, policy(2))
                            .unwrap(),
                        at
                    );
                    check(&db, &query, handle, definition);
...
fn rank_only_changes_reorder_equal_bags_and_deletions_refill_from_outside_page() {
    let ((), report) = run_async_under_lab(0x6a92, |root| async move {
        ...
        assert_eq!(
            before.iter().collect::<std::collections::BTreeSet<_>>(),
        ...
        for definition in [
            simple(1, Some(2)),
            descending(simple(0, None)),
            simple(u64::MAX, None),
        ] {
            let handle = db
                .register_standing_query(&query, definition.clone(), policy(10))
                .unwrap();
            check(&db, &query, &handle, &definition);
        }
    });
...
fn distinct_representatives_follow_hidden_rank_and_refresh_unchanged_rank_keys() {
    let ((), report) = run_async_under_lab(0x6a93, |root| async move {
        ...
        assert_eq!(
            check(&db, &query, &handle, &definition)[0].values(),
...
fn zero_and_out_of_range_pages_do_not_hide_output_errors_and_rebuild_keeps_windows() {
    let ((), report) = run_async_under_lab(0x6a94, |root| async move {
        ...
        let sibling = db
            .register_standing_query(&query, simple(0, None), policy(10))
            .unwrap();
        let mut registered = Vec::new();
        for (offset, count) in [(0, Some(0)), (u64::MAX, Some(1))] {
            ...
            .unwrap();
            let definition = simple(offset, count)
                .with_result_clauses(
                ...
                .unwrap()
                .with_output_projection(vec![Projection::new("quotient", Value::Integer(quotient))])
                .unwrap();
            let handle = db
                .register_standing_query(&query, definition.clone(), policy(0))
                .unwrap();
        ...
        assert!(
            db.standing_query(&query, &sibling)
                .unwrap()
                .ordered_rows()
                .is_none()
        );
        for (handle, _) in &registered {
            ...
            assert!(matches!(
                db.rebuild_standing_query(&query, handle, policy(0)),
...
fn selected_occurrence_quota_ignores_candidates_but_fences_real_page_growth() {
    let ((), report) = run_async_under_lab(0x6a95, |root| async move {
        ...
        assert!(matches!(
            db.rebuild_standing_query(&query, &handle, policy(1)),
            Err(StandingQueryError::Maintenance(
        ...
        for id in 3..=6 {
            db.write(&commit, insert(id, id as i64, 0)).await.unwrap();
        }
        assert_eq!(ids(&check(&db, &query, &handle, &definition)), vec![1, 2]);
        let all = descending(simple(1, Some(2)))
            .with_key_output_columns(&[])
            .unwrap()
            .with_aggregate_output_prefix(0)
            .unwrap();
        assert!(matches!(
            db.register_standing_query(&query, all.clone(), policy(1)),
            Err(StandingQueryError::Maintenance(
                StandingQueryFailure::ResultBudget
            ))
        ));
        let bag = db
            .register_standing_query(&query, all.clone(), policy(2))
            .unwrap();
        assert_eq!(check(&db, &query, &bag, &all).len(), 2);
        assert_eq!(
            db.standing_query(&query, &bag)
                .unwrap()
                .rows()
                .iter()
                .next()
        ...
        let unique = db
            .register_standing_query(&query, distinct.clone(), policy(0))
            .unwrap();
    ...
    let ((), report) = run_async_under_lab(0x6a96, |root| async move {
        ...
        for extra in [0, 500] {
            ...
            for id in 10..10 + extra {
                seed.extend(insert(id, id as i64, -(id as i64))).unwrap();
            }
            db.write(&commit, seed).await.unwrap();
            let handle = db
                .register_standing_query(&query, definition.clone(), policy(2))
