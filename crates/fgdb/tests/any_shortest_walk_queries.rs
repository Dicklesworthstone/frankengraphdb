...
fn query(selector: &str) -> PreparedGraphPattern<GraphValueRow> {
    PreparedGraphText::prepare(&format!(
        ...
        symbols,
    ).unwrap().bind_parameters(&GqlParameters::new().with_int64("source", 1).unwrap()).unwrap()
}
...
fn any_pairs_obey_pinned_history_and_exact_caps_through_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0xa117_0001, |root| async move {
        ...
        assert_eq!(ids(&db.execute_graph_pattern_governed(&cx, &query, policy()).unwrap().value),
            vec![VId(2), VId(3), VId(4)]);
        assert_eq!(ids(&pinned.execute_graph_pattern_governed(&cx, &query, policy()).unwrap().value),
            vec![VId(2), VId(3)]);
        assert!(pinned.execute_graph_pattern_governed_at(&cx, &query, new, policy()).is_err());
        db.compact(&commit).await.unwrap();
        drop(pinned);
        drop(db);
        let db = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        assert_eq!(ids(&db.execute_graph_pattern_governed_at(&cx, &query, old, policy()).unwrap().value),
            vec![VId(2), VId(3)]);
        assert_eq!(ids(&db.execute_graph_pattern_governed_at(&cx, &query, new, policy()).unwrap().value),
            vec![VId(2), VId(3), VId(4)]);
    });
...
fn staged_topology_changes_affect_any_search_without_publishing() {
    let ((), report) = run_async_under_lab(0xa117_0002, |root| async move {
        ...
        assert_eq!(ids(&txn.execute_graph_pattern_governed(&mut db, &cx, &query, policy()).unwrap().value),
            vec![VId(2), VId(3)]);
...
fn any_selected_ingestion_creates_per_pair_and_late_refusal_restores_outer_work() {
    let ((), report) = run_async_under_lab(0xa117_0003, |root| async move {
        ...
        seed(&mut db, &commit).await;
        let frontier = db.frontier().unwrap();
        assert_eq!(ids(&db.execute_graph_pattern_governed(&cx, &query("ALL"), policy()).unwrap().value),
        ...
        let args = GqlParameters::new().with_int64("source", 1).unwrap().with_int64("tag", 7).unwrap();
        let mut txn = db.begin(&txcx).unwrap();
        ...
        let failed = txn.execute_graph_write_script_governed(&mut db, &cx, &script, &args,
            GraphWriteProgramPolicy::new(policy(), 0, 2, 0), |request| {
                allocations += 1;
                assert_eq!(request.statement, 0, "the second step must refuse before allocation");
        ...
        let receipt = txn.execute_graph_write_script_governed(&mut db, &cx, &script, &args,
            GraphWriteProgramPolicy::new(policy(), 0, 3, 0), |request| {
                let fgdb_gql::insertion::GraphInsertRequest::Vertex { row, vertex: 0 } = request.request
                    else { panic!("one vertex per selected endpoint pair") };
                let id = if request.statement == 0 { 200 } else { 201 + row as u128 };
                Ok::<_, ()>(ElementId::Vertex(VId(id)))
            }).unwrap();
        assert_eq!(receipt.stats().created_vertices, 3);
        assert_eq!(receipt.steps()[1].created_vertices(), Some(&[VId(201), VId(202)][..]));
...
fn concurrent_shortcut_invalidates_any_search_read_close() {
    let ((), report) = run_async_under_lab(0xa117_0004, |root| async move {
        ...
        let rows = reader.execute_graph_pattern_governed(&mut db, &cx, &query("ANY"), policy()).unwrap();
...
fn database_any_search_coalesces_parallel_cycles_at_the_maximum_hop_bound() {
    let ((), report) = run_async_under_lab(0xa117_0005, |root| async move {
        ...
        for id in 10..18 { batch.add_edge(EId(id), VId(1), VId(1), vec![]); }
        db.write(&commit, batch).await.unwrap();
        let frontier = db.frontier().unwrap();
        let prepare = |selector| PreparedGraphText::prepare(&format!(
            "MATCH {selector} SHORTEST WALK (a)-[:R*1024]->(b) RETURN ALL b"), symbols)
            .unwrap().bind_parameters(&GqlParameters::new()).unwrap();
        let cap = GqlQueryPolicy::new(100, 10, 50_000, 10_000);
        let rows = db.execute_graph_pattern_governed(&cx, &prepare("ANY"), cap).unwrap();
        assert_eq!(ids(&rows.value), vec![VId(1)]);
        assert!(db.execute_graph_pattern_governed(&cx, &prepare("ALL"), cap).is_err(),
