...
fn pattern(count: Option<u64>) -> PreparedGraphPattern<GraphValueRow> {
    PreparedGraphText::prepare(
        &format!(
            "{HEAD} RETURN a,b,b.n AS n ORDER BY n DESC NULLS LAST,a,b{}",
...
fn aggregate() -> PreparedGraphAggregate {
    PreparedGraphAggregateText::prepare(
        &format!("{HEAD} RETURN a,COUNT(*) AS n,COUNT(b) AS present GROUP BY a ORDER BY a"),
        symbols,
    )
    .unwrap()
    .bind_parameters(&GqlParameters::new())
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    batch.create_vertex(
        VId(40),
        vec![OWNER],
        vec![(FLAG, CanonicalScalar::Bool(true))],
    );
    for (id, value) in [(10, 8), (11, 1), (13, 20)] {
        batch.create_vertex(VId(id), vec![], vec![(N, CanonicalScalar::Int(value))]);
    }
...
fn property(row: &VertexRow, key: PropertyKeyId) -> Option<&CanonicalScalar> {
    row.props.iter().find(|(k, _)| *k == key).map(|(_, v)| v)
}
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<Plain> {
    ...
    for owner in vertices.iter().filter(|row| row.labels.contains(&OWNER)) {
        ...
        for edge in edges
            .iter()
            .filter(|edge| edge.entry.relation == R && edge.entry.src == owner.vid)
        {
            let other = vertices
                .iter()
            ...
            let not_false = match property(owner, FLAG) {
...
fn plain(rows: &[GraphValueRow]) -> Vec<Plain> {
    rows.iter()
        .map(|row| {
            let value = match row.get(2).unwrap().as_scalar().unwrap() {
...
fn summaries(rows: &[Plain]) -> Vec<Summary> {
    ...
    for (owner, destination, _) in rows {
        let group = groups.entry(*owner).or_default();
        group.0 += 1;
        group.1 += u64::from(destination.is_some());
    }
    groups
        .into_iter()
        .map(|(owner, (rows, present))| (owner, rows, present))
        .collect()
}
...
fn boolean_rows_and_groups_cover_all_reads_staging_and_reopened_history() {
    let ((), report) = run_async_under_lab(0xb001_0001, |root| async move {
        ...
        assert_eq!(
            old,
            vec![
                (VId(0), Some(VId(10)), Some(8)),
                (VId(0), Some(VId(10)), Some(8)),
                (VId(1), None, None),
                (VId(2), Some(VId(12)), None),
                (VId(3), None, None),
                (VId(40), None, None)
            ]
        );
        for result in [
            db.execute_graph_pattern_governed(&cx, &query, wide())
                .unwrap(),
            db.execute_graph_pattern_governed_at(&cx, &query, basis, wide())
                .unwrap(),
            pinned
                .execute_graph_pattern_governed(&cx, &query, wide())
                .unwrap(),
            pinned
                .execute_graph_pattern_governed_at(&cx, &query, basis, wide())
                .unwrap(),
        ...
        for result in [
            ...
            pinned
                ...
                .unwrap(),
            pinned
                .execute_graph_aggregate_governed_at(&cx, &aggregate, basis, wide())
                .unwrap(),
            txn.execute_graph_aggregate_governed(&db, &cx, &aggregate, wide())
                .unwrap(),
        ] {
            assert_eq!(summary(&result.value), summaries(&old));
        }
    ...
    let ((), report) = run_async_under_lab(0xb001_0002, |root| async move {
        ...
        for mode in 0..3 {
            for change in 0..4 {
                ...
                match mode {
                    0 => assert_eq!(
                        plain(&result.unwrap().value),
                        vec![(VId(0), Some(VId(10)), Some(8))]
                    ),
                ...
                match change {
                    ...
                    1 => {
                        winner.set_vertex_property(VId(11), N, Some(CanonicalScalar::Int(100)));
                    }
                    2 => {
                        winner.set_vertex_property(VId(3), FLAG, Some(CanonicalScalar::Bool(true)));
                    }
                    _ => {
                        winner.add_edge(EId(90), VId(0), VId(13), vec![]);
                    }
                }
...
fn boolean_sources_share_exact_limits_and_preserve_owner_and_snapshot_errors() {
    let ((), report) = run_async_under_lab(0xb001_0003, |root| async move {
        ...
        assert!(matches!(
            db.execute_graph_pattern_governed_at(&cx, &query, CommitSeq(basis.0 + 1), zero),
