...
use fgdb_gql::{
    ...
    GraphSymbolKind, PreparedGraphMutationProgram, PreparedGraphMutationProgramTemplate,
...
fn program(statements: &[&str]) -> PreparedGraphMutationProgram {
    PreparedGraphMutationProgram::prepare(
        statements
            .iter()
            .map(|text| input(text).bind_parameters(&GqlParameters::new()).unwrap())
            .collect(),
    )
    .unwrap()
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in 1..=3 {
        batch.create_vertex(
            VId(id),
            vec![],
            vec![(P, CanonicalScalar::Int(id as i64 * 10))],
        );
    }
...
fn logical(rows: &[VertexRow]) -> Logical {
    rows.iter()
        .map(|row| (row.vid, row.props.clone()))
        .collect()
}
        ...
        for mode in 0..5 {
            for insertion in [false, true] {
                ...
                let first = if mode == 3 {
                    ...
                    "MATCH (a)-[:R]->(b) WHERE b.p<25 SET a.q=COALESCE(a.q,0)+1"
                };
...
fn deleting_then_failing_restores_incident_edges_and_a_successful_retry_is_durable() {
    let ((), report) = run_async_under_lab(0xb10c_2002, |root| async move {
        ...
        txn.write(&mut db, prefix()).unwrap();
        let failed =
            txn.execute_graph_mutation_program_governed(&mut db, &query, &failing, policy());
        assert!(matches!(
            failed,
            Err(Error::Statement {
                statement: 1,
        ...
        assert_eq!(
            db.edges().unwrap().len(),
            4,
            "all discarded cascade effects were restored"
        );
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let stats = txn
            .execute_graph_mutation_program_governed(&mut db, &query, &accepted, policy())
            .unwrap();
        assert_eq!(
            stats.effects, 3,
            "one explicit delete and two updates, not cascade-cost accounting"
        );
        assert!(txn.edges(&db).unwrap().is_empty());
        assert_eq!(
            db.edges().unwrap().len(),
            4,
            "staging is not durable visibility"
        );
        txn.commit(&mut db, &commit).await.unwrap();
        ...
        assert_eq!(
            property(&reopened.vertex(VId(1)).unwrap().unwrap(), Q),
            Some(&CanonicalScalar::Int(11))
        );
        assert_eq!(
            property(&reopened.vertex(VId(3)).unwrap().unwrap(), Q),
...
fn parameterized_programs_recover_as_a_whole_before_or_after_the_commit_marker() {
    let ((), report) = run_async_under_lab(0xb10c_2003, |root| async move {
        ...
        let program = template
            .bind_parameters(&GqlParameters::new().with_int64("step", 2).unwrap())
            .unwrap();
        let frozen = program.canonical_bytes();
        for crash in [
            Some(CrashPoint::BeforeCapsule),
            ...
            if crash.is_some() {
                ...
                assert_eq!(result.unwrap().0, basis.0 + 1);
            }
            drop(db);
            let reopened = Database::open_with_vfs(&commit, vfs, &path, keys())
                .await
            ...
            if crash.is_some() {
                ...
                for row in reopened.vertices().unwrap() {
                    assert_eq!(
                        property(&row, P),
                        Some(&CanonicalScalar::Int(row.vid.0 as i64 * 10 + 2))
                    );
