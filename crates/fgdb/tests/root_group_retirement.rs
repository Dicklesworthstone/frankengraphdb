...
fn query(count: u64) -> PreparedGraphAggregate {
    template().bind_parameters(&arguments(2, 0, count)).unwrap()
}
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord], minimum: i64) -> Vec<Expected> {
    ...
    for edge in edges.iter().filter(|edge| edge.entry.relation == R) {
        let source = vertices
            .iter()
            .find(|row| row.vid == edge.entry.src)
            .unwrap();
        if !source.labels.contains(&L) {
            continue;
        }
    ...
    for (root, bag) in groups {
        ...
        let mean = (!values.is_empty()).then(|| {
            (
                values.iter().map(|n| i128::from(*n)).sum(),
                values.len() as u64,
            )
        });
        let unique = (!values.is_empty()).then(|| {
            values
                .into_iter()
                .collect::<BTreeSet<_>>()
                .into_iter()
    ...
    for (row, expected) in rows.iter().zip(expected) {
        ...
        match expected.mean {
            ...
            Some((sum, count)) => {
                ...
                assert_eq!(
                    mean.numerator() * i128::from(count),
...
fn retired_groups_use_the_same_canonical_overlay_and_all_historical_entrypoints() {
    let ((), report) = run_async_under_lab(0x817e_0001, |root| async move {
        ...
        assert_eq!(
            expected.iter().map(|row| row.root).collect::<Vec<_>>(),
        ...
        staged.delete_edge(EId(1));
        staged.ensure_edge_by_triple(EId(999), VId(1), VId(11), vec![]);
        staged.set_vertex_property(VId(13), P, None);
        staged.set_vertex_property(VId(15), P, Some(CanonicalScalar::Int(9)));
        staged.add_edge(EId(90), VId(4), VId(15), vec![]);
        staged.delete_vertex(VId(10));
        ...
        assert_eq!(
            changed.iter().map(|row| row.root).collect::<Vec<_>>(),
            vec![VId(4), VId(1), VId(3)]
        );
        check(
            &txn.execute_graph_aggregate_governed(&db, &cx, &plan, wide())
                .unwrap()
        ...
        check(
            &reopened
                .execute_graph_aggregate_governed(&cx, &relaxed, wide())
                .unwrap()
                .value,
            &all[1..3],
        );
        assert_eq!(plan.canonical_bytes(), frozen);
        assert_ne!(relaxed.canonical_bytes(), frozen);
    });
        ...
        for mode in 0..3 {
            for change in 0..5 {
                ...
                match change {
                    ...
                    1 => {
                        winner.set_vertex_property(VId(10), P, Some(CanonicalScalar::Int(100)));
                    }
                    2 => {
                        winner.set_vertex_property(VId(15), P, Some(CanonicalScalar::Int(9)));
