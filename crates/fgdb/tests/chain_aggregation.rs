...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 1000, 2_000_000, 100_000)
}
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in [
        0, 1, 10, 11, 20, 21, 22, 23, 30, 31, 32, 40, 41, 50, 51, 60, 61,
    ] {
...
fn oracle(edges: &[EdgeRecord]) -> Vec<Summary> {
    ...
    for root in edges.iter().filter(|e| e.entry.relation == R) {
            ...
            .filter(|e| e.entry.relation == S && e.entry.src == root.entry.dst)
        {
            for middle in edges
                .iter()
                .filter(|e| e.entry.relation == U && e.entry.src == first.entry.dst)
            {
                for last in edges
                    .iter()
                    .filter(|e| e.entry.relation == S && e.entry.src == middle.entry.dst)
                {
                    for _side in edges
                        .iter()
                        .filter(|e| e.entry.relation == T && e.entry.src == first.entry.dst)
                    {
                        for _close in edges.iter().filter(|e| {
                            e.entry.relation == V
                                && e.entry.src == last.entry.dst
                                && e.entry.dst == root.entry.src
                        }) {
                            for _tail in edges
                                .iter()
                                .filter(|e| e.entry.relation == W && e.entry.src == last.entry.dst)
                            {
                                let group =
                                    groups.entry((root.entry.src, last.entry.dst)).or_default();
                                group.0 += 1;
                                group.1.insert(root.entry.dst);
                            }
                        }
                    }
                }
...
fn joint_endpoints_composed_with_forests_cover_all_reads_staging_and_reopened_history() {
    let ((), report) = run_async_under_lab(0xc0a1_0001, |root| async move {
        ...
        assert_eq!(
            old,
            vec![
                (VId(0), VId(40), 44, 1, VId(10), VId(10)),
                (VId(1), VId(41), 1, 1, VId(11), VId(11))
            ]
        );
        ...
        for result in [
            ...
            assert_eq!(result.rows.snapshot_records, 26);
        }
        let measured = db
        ...
        for cap in [
            GqlQueryPolicy::new(25, 2, u64::MAX, u64::MAX),
            ...
            assert!(
                db.execute_graph_aggregate_governed(&cx, &query, cap)
                    .is_err()
            );
        }
        ...
        assert_eq!(
            expected,
            vec![
                (VId(0), VId(40), 24, 1, VId(10), VId(10)),
                (VId(1), VId(41), 1, 1, VId(11), VId(11))
            ]
        );
        assert_eq!(
        ...
        for mode in 0..3 {
            for change in 0..4 {
                ...
                match change {
                    ...
                    1 => {
                        winner.add_edge(EId(90), VId(23), VId(30), vec![]);
                    }
                    2 => {
                        winner.delete_edge(EId(21));
                    }
                    _ => {
                        winner.delete_edge(EId(61));
                    }
                }
...
fn database_returns_joint_endpoints_of_a_billion_walks_with_original_source_limits() {
    let ((), report) = run_async_under_lab(0xc0a1_0003, |root| async move {
        ...
        first.add_edge(EId(1), VId(0), VId(1), vec![]);
        let mut transitions = WriteBatch::new(S);
        let mut eid = 10;
        for a in 1..=8 {
            for b in 1..=8 {
                transitions.add_edge(EId(eid), VId(a), VId(b), vec![]);
                eid += 1;
            }
        }
        db.write_atomic(&commit, vec![vertices, first, transitions])
            .await
            .unwrap();
        let mut statement = "MATCH (a)-[:R]->(b)".to_owned();
        for at in 1..=10 {
            statement.push_str(&format!("-[:S]->(x{at})"));
        }
        ...
        let result = db
            .execute_graph_aggregate_governed(
                &cx,
                &query,
                GqlQueryPolicy::new(65, 8, 131_072, 16_384),
            )
            .unwrap();
        ...
        assert!(matches!(
            db.execute_graph_aggregate_governed(
                &cx,
                &query,
                GqlQueryPolicy::new(64, 8, u64::MAX, u64::MAX)
            ),
