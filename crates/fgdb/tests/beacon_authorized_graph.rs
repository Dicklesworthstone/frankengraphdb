...
async fn database(cx: &CommitCx, hidden: bool, seed: u64) -> Database<MemVfs> {
    ...
    for id in 1..=3 {
        ...
        let mut props = vec![
            ...
            (
                X,
                CanonicalScalar::Int((id * 7 + u128::from(seed % 5)) as i64),
            ),
        ];
        if hidden {
                ...
                for direction in [
                    ...
                    for seeds in [
                        ...
                        for include in [false, true] {
                            ...
                            .unwrap();
                            let expected = clean
                                .beacon_search_graph(&c.query(), &options(), query(mode), e)
                                .unwrap();
                            assert_eq!(
                                actual, expected,
                                "seed={seed}, direction={direction:?}, seeds={seeds:?}"
                            );
                        }
                    }
                }
...
fn historical_winners_precede_authorization_and_retired_edges_do_not_reappear() {
    let ((), report) = run_async_under_lab(0xbeac_2401, |root| async move {
        ...
        update.set_vertex_property(
            VId(2),
            X,
            Some(text("hidden successor must not be projected")),
        );
        ...
        assert_eq!(
            search(&db, &c.query(), &issuer, &token, &old, q, e).unwrap(),
            before
        );
        ...
        assert!(
            search(&db, &c.query(), &issuer, &token, &options(), q, e)
                .unwrap()
                .iter()
                .all(|hit| hit.graph_hops.is_none())
        );
...
fn hidden_history_moves_no_signed_or_native_work_threshold() {
    let ((), report) = run_async_under_lab(0xbeac_2402, |root| async move {
        ...
        for native in [false, true] {
            let measure = |db: &Database<MemVfs>| {
                threshold(1_000_000, |limit| {
                    let mut o = options();
        ...
        for (restriction, dimension) in [
            ...
            for db in [&full, &clean] {
                refused(
                    search(db, &c.query(), &issuer, &restricted, &options(), q, e),
        ...
        for db in [&full, &clean] {
            assert_eq!(
                search(db, &c.query(), &issuer, &exact, &options(), q, bounded)
                    .unwrap()
                    .len(),
                3
            );
            ...
            assert!(matches!(
                search(db, &c.query(), &issuer, &exact, &options(), q, too_small),
...
fn authentication_precedes_source_and_graph_budget_errors() {
    let ((), report) = run_async_under_lab(0xbeac_2404, |root| async move {
        ...
        refused(
            search(&db, &c.query(), &authority(999), &token, &o, q, e),
            Error::Unauthenticated,
        );
        let write = token.attenuate(Restriction::Rights(Rights::Write)).unwrap();
        refused(
            search(&db, &c.query(), &issuer, &write, &o, q, e),
            Error::PermissionDenied,
        );
        assert!(matches!(
            search(&db, &c.query(), &issuer, &token, &o, q, e),
