...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (id, value) in [(10, Some(30)), (11, Some(20)), (12, None)] {
        batch.create_vertex(
            ...
            vec![],
            value
                .map(|n| (RANK, CanonicalScalar::Int(n)))
                .into_iter()
                .collect(),
        );
    }
...
fn ordered_optional_pages_cover_all_five_read_entrypoints_and_canonical_staging() {
    let ((), report) = run_async_under_lab(0x0d3e_0001, |root| async move {
        ...
        for distinct in [false, true] {
            for (offset, count) in [(0, 0), (0, 1), (1, 3), (3, 2), (u64::MAX, 1), (1, u64::MAX)] {
                let query = query(distinct, offset, count);
                let expected = oracle(&vertices, &edges, distinct, offset, count);
                for result in [
        ...
        assert_eq!(
            expected,
            vec![
                (VId(0), Some(-1), Some(VId(10))),
                (VId(0), None, Some(VId(11))),
                (VId(1), None, None)
            ]
        );
        ...
        for mode in 0..3 {
            for change in 0..3 {
                ...
                match change {
                    ...
                    1 => {
                        winner.set_vertex_property(VId(11), RANK, Some(CanonicalScalar::Int(40)));
                    }
                    _ => {
                        winner.create_vertex(
                            VId(13),
                            vec![],
                            vec![(RANK, CanonicalScalar::Int(100))],
                        );
...
fn ordered_database_pages_share_the_existing_exact_resource_policy() {
    let ((), report) = run_async_under_lab(0x0d3e_0003, |root| async move {
        ...
        for cap in [
            GqlQueryPolicy::new(100, 2, u64::MAX, u64::MAX),
        ...
        let own = txn
            ...
            .unwrap();
        let exact = GqlQueryPolicy::new(
            own.rows.snapshot_records,
            own.rows.result_rows,
            own.evaluator.work_units,
            own.evaluator.scratch_entries,
        );
        assert_eq!(
