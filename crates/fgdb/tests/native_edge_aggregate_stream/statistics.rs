...
fn native_statistics_match_eager_across_parameters_directions_and_captured_paths() {
    let ((), report) = run_async_under_lab(0xa66e_4001, |root| async move {
        ...
        for (floor, count, total) in [(-100, 4, Some(0)), (0, 2, Some(6)), (100, 0, None)] {
            ...
            assert_eq!(row.values()[0].as_average(), total.and_then(|sum| GraphExactAverage::new(sum, count)));
            if count == 0 { assert!(row.values()[0].is_null()); }
        }
    });
...
fn exact_history_and_unique_value_state_survive_writer_compaction_and_drop() {
    let ((), report) = run_async_under_lab(0xa66e_4002, |root| async move {
        ...
        let prepared = PreparedNativeRead::prepare(text, &args, symbols()).unwrap();
        let mut old = prepared.stream_aggregate_in_view(&view, &cx, &args, wide()).unwrap();
        ...
        drop(view); drop(db); drop(args); drop(future); drop(prepared);
        ...
        assert_eq!(row.values()[2].as_value(), Some(&GraphValue::Edge(EId(u128::MAX))));
...
fn extended_statistics_preserve_exact_quotas_typed_late_failures_and_nondraining_close() {
    let ((), report) = run_async_under_lab(0xa66e_4003, |root| async move {
        ...
        let records = baseline.row_stats().snapshot_records; let work = baseline.evaluator_stats();
        let exact = GqlQueryPolicy::new(records, 1, work.work_units, work.scratch_entries);
        assert_eq!(prepared.stream_aggregate(&db, &cx, &args, exact).unwrap().next().unwrap().unwrap(), result);
        for p in [GqlQueryPolicy::new(records-1,1,u64::MAX,u64::MAX),
        ...
        assert_eq!(cursor.evaluator_stats().work_units, 0);
        let mut edit = WriteBatch::new(R);
        edit.set_edge_property(EId(u128::MAX), P, Some(CanonicalScalar::ucs_basic_text("private invalid number").unwrap()));
        db.write(&commit, edit).await.unwrap();
        let text = "MATCH (a)-[r:R]->(b) RETURN COUNT(DISTINCT r) AS count,AVG(DISTINCT r.score) AS mean";
        let mut cursor = db.query_aggregate_stream(&cx, text, &args, symbols(), wide()).unwrap();
        assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(GraphAggregateError::NonIntegerAverage { aggregate: 1 })))));
        assert_eq!(cursor.row_stats().result_rows, 0);
