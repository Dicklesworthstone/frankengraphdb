...
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryPolicy, GraphDeleteError,
    GraphMutationProgramDimension, GraphMutationProgramError, GraphSymbol, GraphSymbolKind,
    GraphWriteProgramError, GraphWriteProgramPolicy, GraphWriteStatement,
...
fn delete(text: &str) -> GraphWriteStatement {
    PreparedGraphDeleteText::prepare(text, R, symbols).unwrap().bind_parameters(&GqlParameters::new()).unwrap().into()
}
fn insert(text: &str) -> GraphWriteStatement {
    PreparedGraphInsertText::prepare(text, R, symbols).unwrap().bind_parameters(&GqlParameters::new()).unwrap().into()
}
fn mutation(text: &str) -> GraphWriteStatement {
    PreparedGraphMutationText::prepare(text, R, symbols).unwrap().bind_parameters(&GqlParameters::new()).unwrap().into()
}
fn policy(effects: u64) -> GraphWriteProgramPolicy {
    GraphWriteProgramPolicy::new(GqlQueryPolicy::new(50_000, 50_000, 5_000_000, 5_000_000), effects, 10, 10)
}
...
fn delete_sees_earlier_creation_and_later_creation_sees_the_deletion() {
    let ((), report) = run_async_under_lab(0xd31e_2001, |root| async move {
        ...
        let program = PreparedGraphWriteProgram::prepare(vec![
            insert("CREATE (n {p:1})"), delete("MATCH (n) WHERE n.p=1 DELETE n"),
            insert("CREATE (n {p:2})"), delete("MATCH (n) WHERE n.p=1 DELETE n"),
        ]).unwrap();
        let before = db.frontier().unwrap();
        let (receipt, _) = db.execute_graph_write_program_returning_autocommit_governed(
            &txcx, &query, &commit, &program, policy(1), |request| {
        ...
        assert_eq!((receipt.stats().created_vertices, receipt.stats().mutation_effects), (2, 1));
        ...
        assert_eq!(db.vertex(VId(3)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(2))]);
...
fn attached_target_refuses_all_steps_preserves_outer_prefix_and_retains_witnesses() {
    let ((), report) = run_async_under_lab(0xd31e_2002, |root| async move {
        ...
        for id in 1..=3_u128 { seed.create_vertex(VId(id), vec![], vec![(P, CanonicalScalar::Int(id as i64))]); }
        seed.add_edge(EId(10), VId(1), VId(2), vec![]);
        db.write(&commit, seed).await.unwrap();
        let mut txn = db.begin(&txcx).unwrap();
        let mut prefix = WriteBatch::new(R); prefix.create_vertex(VId(99), vec![], vec![]);
        txn.write(&mut db, prefix).unwrap();
        let before = txn.staged_effect_digest().unwrap();
        let program = PreparedGraphWriteProgram::prepare(vec![
            mutation("MATCH (n) WHERE n.p=3 SET n.p=30"),
            delete("MATCH (n) WHERE n.p=1 DELETE n"), insert("CREATE (n)"),
        ]).unwrap();
        let result = txn.execute_graph_write_program_returning_governed(
            &mut db, &query, &program, policy(10),
        ...
        assert_eq!(txn.vertex(&db, VId(3)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(3))]);
        assert!(txn.edge(&db, EId(10)).unwrap().is_some());
        assert!(txn.vertex(&db, VId(99)).unwrap().is_some());
        let mut winner = WriteBatch::new(R); winner.delete_edge(EId(10));
        db.write(&commit, winner).await.unwrap();
        assert!(txn.finish(&mut db, &commit).await.is_err(), "failed deletion's topology observations remain witnesses");
...
fn prior_detach_removes_incidence_and_plain_delete_uses_the_updated_overlay() {
    let ((), report) = run_async_under_lab(0xd31e_2003, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let stats = txn.execute_graph_write_program_governed(&mut db, &query, &program, policy(2),
            |_| -> Result<ElementId, ()> { panic!("deletion cannot allocate") }).unwrap();
...
fn updates_and_deletes_share_the_effect_quota_and_late_refusal_rolls_back_updates() {
    let ((), report) = run_async_under_lab(0xd31e_2004, |root| async move {
        ...
        let commit = contexts.commit(); let query = contexts.query(); let txcx = contexts.txn();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut seed = WriteBatch::new(R); seed.create_vertex(VId(1), vec![], vec![(P, CanonicalScalar::Int(1))]);
        db.write(&commit, seed).await.unwrap();
        let before = db.frontier().unwrap();
        let program = PreparedGraphWriteProgram::prepare(vec![
            mutation("MATCH (n) SET n.p=2"), delete("MATCH (n) DELETE n"),
        ]).unwrap();
        let result = db.execute_graph_write_program_returning_autocommit_governed(
            &txcx, &query, &commit, &program, policy(1),
