...
fn oracle(edges: &[EdgeRecord]) -> Vec<Plain> {
    ...
    for first in edges.iter().filter(|row| row.entry.relation == R) {
        for _ in edges.iter().filter(|row| {
            row.entry.relation == S
                && row.entry.src == first.entry.dst
                && row.entry.dst == first.entry.src
        }) {
            groups
                .entry(first.entry.src)
                .or_default()
                .push(first.entry.dst);
        }
    }
...
fn weighted_counts_cover_all_read_surfaces_and_canonical_staging_and_history() {
    let ((), report) = run_async_under_lab(0xfa67_0001, |root| async move {
        ...
        let aggregate = prepare(STATEMENT);
        let frozen = aggregate.canonical_bytes();
        let old = oracle(&db.edges().unwrap());
        assert_eq!(
            old,
            vec![
                (VId(0), 8, 2, VId(10), VId(11)),
                (VId(1), 1, 1, VId(11), VId(11))
            ]
        );
        ...
        for result in [
            ...
            assert_eq!(
                result.rows.snapshot_records, 11,
                "do not substitute compressed topology cardinality"
            );
        }
        let measured = db
        ...
        for policy in [
            GqlQueryPolicy::new(10, 2, u64::MAX, u64::MAX),
        ...
        assert_eq!(
            expected,
            vec![
                (VId(0), 3, 1, VId(10), VId(10)),
                (VId(1), 2, 1, VId(11), VId(11))
            ]
        );
        assert_eq!(
            plain(
        ...
        for mode in 0..3 {
            for change in 0..3 {
                ...
                match change {
                    ...
                    1 => {
                        winner.add_edge(EId(90), VId(10), VId(1), vec![]);
                    }
                    _ => {
...
fn actual_database_counts_a_billion_parallel_walks_under_a_small_work_allowance() {
    let ((), report) = run_async_under_lab(0xfa67_0003, |root| async move {
        ...
        for id in 100..108 {
            seed.add_edge(EId(id), VId(5), VId(5), vec![]);
        }
        db.write(&commit, seed).await.unwrap();
        let aggregate = prepare(&format!(
            "MATCH (a){} RETURN COUNT(*) AS paths,COUNT(DISTINCT a) AS owners",
            "-[:R]->(a)".repeat(10)
        ));
        let result = db
            .execute_graph_aggregate_governed(
                &cx,
                &aggregate,
                GqlQueryPolicy::new(8, 1, 65_536, 2048),
            )
            .unwrap();
        assert_eq!(
