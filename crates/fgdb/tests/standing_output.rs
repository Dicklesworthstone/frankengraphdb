...
use fgdb_gql::{GqlQueryPolicy, GraphAggregate, GraphAggregateColumn as Column,
    ...
    GraphIntegerExpression, GraphIntegerOp as Op, GraphSetProjection as Projection,
    GraphSetValue as Value, PreparedGraphAggregate};
...
fn insert(id: u128, group: i64, amount: i64) -> WriteBatch {
    ...
    write.create_vertex(VId(id), vec![], vec![
        (GROUP, CanonicalScalar::Int(group)), (AMOUNT, CanonicalScalar::Int(amount)),
    ]);
    write
}
...
fn recomputed(db: &Database<MemVfs>, definition: &PreparedGraphAggregate) -> ZSet<GraphAggregateRow> {
    ...
    ZSet::from_updates(result.value.into_iter().map(|row| (row, ZWeight::ONE)),
...
fn matrix_definition(shape: Shape, direction: GlaDirection, grouped: bool, distinct: bool) -> PreparedGraphAggregate {
    ...
    let columns = [GraphColumn::property("group", "a", GROUP),
        GraphColumn::property("amount", endpoint, AMOUNT), GraphColumn::vertex("endpoint", endpoint)];
    ...
    let base = PreparedGraphAggregate::prepare_projected(input, vec![
        ...
        GraphAggregate::count_rows("count"), GraphAggregate::sum_int("sum", 2),
        GraphAggregate::average_int("average", 2), GraphAggregate::min("min", 2),
        GraphAggregate::max("max", 2), GraphAggregate::count_distinct("endpoints", 0),
    ], 0, None).unwrap();
    ...
    let result = if grouped {
        ...
        base.with_key_output_columns(&[]).unwrap().with_aggregate_output_prefix(3).unwrap()
    };
...
fn projected_outputs_match_snapshot_across_computed_inputs_having_and_join_shapes() {
    let ((), report) = run_async_under_lab(0x6a81, |root| async move {
        ...
        for (id, group, amount) in [(1, 1, Some(2)), (2, 1, Some(2)), (3, 2, Some(2)),
            (4, 2, None), (5, 3, Some(-1))]
        {
        ...
        for id in [1, 2, 4, 5] { empty.delete_vertex(VId(id)); }
...
fn deleting_the_first_distinct_group_replaces_the_concrete_numeric_variant() {
    let ((), report) = run_async_under_lab(0x6a82, |root| async move {
        ...
        for (id, group) in [(2, 1), (3, 2), (4, 3)] { seed.extend(insert(id, group, 2)).unwrap(); }
        ...
        let all = simple(true, 0, None).with_output_projection(vec![
            Projection::new("value", Value::Integer(expression)),
        ]).unwrap();
        let distinct = all.clone().with_distinct_output(true);
        let hd = db.register_standing_query(&query, distinct.clone(), bounded(1)).unwrap();
        let ha = db.register_standing_query(&query, all.clone(), policy()).unwrap();
        check(&db, &query, &hd, &distinct);
        assert_eq!(db.standing_query(&query, &hd).unwrap().rows().iter().next().unwrap().0.values(),
        ...
        for (handle, definition) in [(&hd, &distinct), (&ha, &all)] { check(&db, &query, handle, definition); }
        assert_eq!(db.standing_query(&query, &hd).unwrap().rows().iter().next().unwrap().0.values(),
        ...
        assert_eq!(db.standing_query(&query, &hd).unwrap().rows().iter().next().unwrap().0.values(),
            &[GraphAggregateValue::Count(2)]);
        let mut empty = WriteBatch::new(R);
        for id in [3, 4, 5, 6] { empty.delete_vertex(VId(id)); }
...
fn projected_bag_quotas_count_occurrences_while_distinct_retains_hidden_group_support() {
    let ((), report) = run_async_under_lab(0x6a83, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut seed = WriteBatch::new(R);
        for id in 1..=6 { seed.extend(insert(id, id as i64, 2)).unwrap(); }
        let basis = db.write(&commit, seed).await.unwrap();
        let all = simple(true, 0, None).with_key_output_columns(&[]).unwrap()
            .with_aggregate_output_prefix(0).unwrap();
        let distinct = all.clone().with_distinct_output(true);
        assert!(matches!(db.register_standing_query(&query, all.clone(), bounded(1)),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::ResultBudget))));
        let ha = db.register_standing_query(&query, all.clone(), bounded(6)).unwrap();
        let hd = db.register_standing_query(&query, distinct.clone(), bounded(1)).unwrap();
        ...
        assert!(matches!(db.rebuild_standing_query(&query, &ha, bounded(6)),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::ResultBudget))));
        assert!(matches!(db.standing_query(&query, &ha),
            Err(StandingQueryError::Unavailable { frontier, .. }) if frontier == basis));
        assert_eq!(db.rebuild_standing_query(&query, &ha, bounded(7)).unwrap(), at);
        ...
        check(&db, &query, &ha, &all);
        check(&db, &query, &hd, &distinct);
        assert_eq!(db.standing_query(&query, &ha).unwrap().rows().iter().next().unwrap().1.to_i128(), Some(7));
        assert_eq!(db.standing_query(&query, &hd).unwrap().rows().iter().next().unwrap().1, &ZWeight::ONE);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn having_precedes_output_errors_and_rebuild_preserves_the_original_projection() {
    let ((), report) = run_async_under_lab(0x6a84, |root| async move {
        ...
        let sibling = db.register_standing_query(&query, base.clone(), policy()).unwrap();
        ...
        assert!(matches!(db.rebuild_standing_query(&query, &handle, policy()),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::OutputExpression { column: 1, .. }))));
        assert!(matches!(db.standing_query(&query, &handle),
            Err(StandingQueryError::Unavailable { frontier, .. }) if frontier == basis));
        ...
        assert_eq!(view.rows().iter().next().unwrap().0.values(),
            &[GraphAggregateValue::Count(2), GraphAggregateValue::Integer(5)]);
        ...
        check(&db, &query, &handle, &definition);
        assert_eq!(db.standing_query(&query, &handle).unwrap().rows().iter().next().unwrap().0.values(),
...
fn list_output_uses_shared_exact_expression_rules_and_ranked_pages_still_refuse() {
    let ((), report) = run_async_under_lab(0x6a85, |root| async move {
        ...
        let query = contexts.query();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let list = Value::List(vec![Value::Column(0), Value::Value(GraphValue::Scalar(
            CanonicalScalar::ucs_basic_text("kept").unwrap()))]);
        let definition = simple(false, 0, None).with_output_projection(vec![
            Projection::new("list", list.clone()),
            Projection::new("size", Value::Size(Box::new(list.clone()))),
            Projection::new("last", Value::Index {
                list: Box::new(list), index: Box::new(Value::Value(GraphValue::Scalar(CanonicalScalar::Int(-1)))),
            }),
...
fn unrelated_output_classes_do_not_increase_sparse_maintenance_counters() {
    let ((), report) = run_async_under_lab(0x6a86, |root| async move {
        ...
        let commit = contexts.commit();
        let query = contexts.query();
        let definition = simple(true, 0, None).with_output_projection(vec![
            Projection::new("amount", Value::Column(2)),
        ]).unwrap().with_distinct_output(true);
        let mut measured = Vec::new();
        for extra in [0, 500] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let mut seed = insert(1, 1, 2);
            for id in 10..10 + extra { seed.extend(insert(id, id as i64, 100 + id as i64)).unwrap(); }
            db.write(&commit, seed).await.unwrap();
            let handle = db.register_standing_query(&query, definition.clone(), policy()).unwrap();
