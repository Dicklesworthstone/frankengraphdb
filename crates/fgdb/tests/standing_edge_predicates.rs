...
use fgdb_types::{CanonicalF64, CanonicalScalar, DatabaseSecurityNamespaceId, EId, PurposeContexts, QueryCx, VId};
...
const OPS: [IntegerComparison; 6] = [
    IntegerComparison::Equal, IntegerComparison::NotEqual, IntegerComparison::Less,
    IntegerComparison::LessOrEqual, IntegerComparison::Greater, IntegerComparison::GreaterOrEqual,
];
...
fn definition(edge: bool, comparison: IntegerComparison) -> PreparedGraphAggregate {
    ...
    pattern.compare_properties("a", L, comparison, if edge { "b" } else { "a" }, R).unwrap();
...
fn storage_count(db: &Database<MemVfs>, edge: bool, operator: usize) -> u64 {
    ...
    let pairs = if edge {
        db.edges().unwrap().into_iter().filter(|e| e.entry.relation == REL)
            .map(|e| (e.entry.src, e.entry.dst)).collect::<Vec<_>>()
    } else {
        vertices.iter().map(|v| (v.vid, v.vid)).collect()
    };
    pairs.into_iter().filter(|(a, b)| {
        let left = rows[a].props.iter().find(|(key, _)| *key == L).map(|(_, value)| value);
        let right = rows[b].props.iter().find(|(key, _)| *key == R).map(|(_, value)| value);
        let (Some(a), Some(b)) = (left, right) else { return false; };
        if matches!(a, CanonicalScalar::Null) || matches!(b, CanonicalScalar::Null)
            || core::mem::discriminant(a) != core::mem::discriminant(b) { return false; }
        ...
        let order = a.cmp(b);
        match operator {
            0 => order == Ordering::Equal, 1 => order != Ordering::Equal,
            2 => order == Ordering::Less, 3 => order != Ordering::Greater,
            4 => order == Ordering::Greater, 5 => order != Ordering::Less,
            _ => unreachable!(),
        }
    }).count() as u64
}

fn assert_count(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle,
    query: &PreparedGraphAggregate, count: u64) {
...
fn every_scalar_pair_and_comparison_matches_public_storage_for_one_and_two_slots() {
    let ((), report) = run_async_under_lab(0x7e11, |root| async move {
        ...
        let values = [
            None, Some(CanonicalScalar::Null), Some(CanonicalScalar::Bool(false)),
            Some(CanonicalScalar::Bool(true)), Some(CanonicalScalar::Int(i64::MIN)),
            Some(CanonicalScalar::Int(0)), Some(CanonicalScalar::Int(i64::MAX)),
        ...
        for left in &values {
            for right in &values {
                ...
                change.set_vertex_property(VId(1), PropertyKeyId(99), Some(CanonicalScalar::Int(ordinal)));
