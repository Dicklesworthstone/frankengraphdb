...
fn vertex_patterns(seed: u64) -> String {
    format!(
        ...
        10 + seed, 20 + seed, 30 + seed, 40 + seed,
    )
}
...
fn multi_statement(seed: u64, verb: &str) -> String {
    format!(
        ...
        vertex_patterns(seed), 2000 + seed, 2100 + seed, 2200 + seed,
    )
}
...
fn relation_statements(seed: u64, verb: &str) -> Vec<(RelationId, String)> {
    ...
    .into_iter()
    .map(|(relation, name, src, src_label, src_key, dst, dst_label, dst_key, edge_key, since)| {
...
fn identity_counter(offset: u128) -> impl FnMut(GraphWriteIdentityRequest) -> Result<ElementId, ()> {
...
fn graph<V: Vfs + Clone>(db: &Database<V>) -> LogicalGraph {
    ...
    let names: BTreeMap<_, _> = rows
        .iter()
        .map(|row| {
            let name = row.props.iter().find(|(key, _)| *key == NAME).unwrap().1.clone();
            (row.vid, name)
        })
        .collect();
...
fn properties(name: String, since: u64) -> Properties {
    vec![(NAME, text(&name)), (SINCE, CanonicalScalar::Int(since as i64))]
}
    ...
    for seed in 0..3_u64 {
        let ((), report) = run_async_under_lab(0x9e57_0001 + seed, move |root| async move {
            ...
            for verb in ["INSERT", "CREATE"] {
                let mut multi = Database::open_memory(&contexts.commit(), keys(0x61)).await.unwrap();
                let mut split = Database::open_memory(&contexts.commit(), keys(0x62)).await.unwrap();
                ...
                run_scripts(&mut multi, &contexts, &[(KNOWS, multi_statement(seed, verb))], &mut multi_ids).await;
                ...
                assert_eq!(answers(&multi, &contexts), answers(&split, &contexts), "{verb}, seed {seed}");
            }
        });
...
fn three_relation_merge_script_matches_split_commits_across_seeds() {
    for seed in 0..3_u64 {
        let ((), report) = run_async_under_lab(0x9e57_0010 + seed, move |root| async move {
            ...
            let script = statements.iter().map(|(_, text)| text.as_str()).collect::<Vec<_>>().join(" ");
            ...
            assert_eq!(answers(&multi, &contexts), answers(&split, &contexts), "seed {seed}");
        });
        assert!(report.lab_test_passed(), "seed {seed}: {report:?}");
    }
}
...
fn multi_relation_statement_recovers_complete_graph_or_original_graph_at_marker_boundary() {
    let ((), report) = run_async_under_lab(0x9e57_0020, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        ...
        for (name, crash, tear, committed) in [
            ...
            let path = std::env::temp_dir().join(format!(
                "fgdb-multi-relation-script-{}-{name}", std::process::id(),
            ));
            ...
            let prepared = PreparedGraphWriteScript::prepare(&multi_statement(7, "INSERT"), KNOWS, symbols).unwrap();
            let mut txn = db.begin(&txcx).unwrap();
            txn.execute_graph_write_script_governed(
                &mut db, &contexts.query(), &prepared, &GqlParameters::new(), policy(), &mut ids,
            ).unwrap();
            assert_eq!(db.frontier().unwrap(), basis, "{name}: staging is not durable");
            ...
            if matches!(crash, Some(CrashPoint::AfterMarkerBeforeD2 | CrashPoint::AfterMarkerFileSyncBeforeDirectorySync)) {
                assert!(matches!(result, Err(WriteTxnError::Write(WriteError::CommitOutcomeUnknown { .. }))));
            ...
            for rebuilding in [false, true] {
                let reopened = if rebuilding {
                    Database::open_rebuilding(&commit, &path, keys(0x63)).await.unwrap()
                } else {
                    Database::open(&commit, &path, keys(0x63)).await.unwrap()
                };
                assert_eq!(reopened.frontier().unwrap(), CommitSeq(basis.0 + u64::from(committed)), "{name}, rebuilding={rebuilding}");
                assert_eq!(reopened.delta_since(basis).unwrap().count(), usize::from(committed), "{name}");
                assert_eq!(&graph(&reopened), if committed { &after } else { &before }, "{name}, rebuilding={rebuilding}");
                assert_eq!(&answers(&reopened, &contexts), if committed { &after_answers } else { &before_answers }, "{name}, rebuilding={rebuilding}");
            }
        }
    });
