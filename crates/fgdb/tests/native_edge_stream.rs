...
use fgdb::{Database, DatabaseKeys, MemVfs, PreparedNativeRead, QueryError, QueryResult, ReadError, WriteBatch};
...
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryPolicy, GraphAggregateValue,
...
fn seed() -> WriteBatch {
    ...
    for (id, from, to, value) in [(0, WIDE, VId(0), 4), (2, WIDE, VId(0), 5),
        (3, WIDE, WIDE, 8), (u128::MAX, VId(0), VId(u128::MAX), -1)] {
...
fn result(columns: Vec<String>, rows: Vec<GraphValueRow>) -> QueryResult {
    QueryResult::Rows { columns, rows: rows.into_iter().map(|row| row.values().iter().cloned()
...
fn native_dispatch_matches_existing_edge_cursors_and_eager_rows_in_every_direction() {
    let ((), report) = run_async_under_lab(0x6e65_0001, |root| async move {
            ...
            for filter in ["", "WHERE r.p > 0 AND a.p IS NOT NULL", "WHERE a = b"] {
                for distinct in ["ALL", "DISTINCT"] {
                    for page in ["", "SKIP 1 LIMIT 2", "LIMIT 0"] {
                        ...
                        let pattern = PreparedGraphText::prepare(&text, symbols).unwrap().bind_parameters(&args).unwrap();
                        let eager = prepared.execute(&db, &cx, &args, policy()).unwrap();
                        let mut direct = db.stream_graph_edges_governed(&cx, &pattern, policy()).unwrap();
                        ...
                        assert_eq!(actual, direct.by_ref().collect::<Result<Vec<_>, _>>().unwrap(), "{text}");
        ...
        let prepared = PreparedNativeRead::prepare("MATCH (n) RETURN n, n.p AS p", &args, symbols).unwrap();
        let (_, mut cursor) = prepared.stream(&db, &cx, &args, policy()).unwrap();
        assert_eq!(cursor.kind(), ScanKind::Vertex);
        assert_eq!(cursor.by_ref().collect::<Result<Vec<_>, _>>().unwrap().len(), 3);
    });
...
fn paused_native_edge_streams_and_temporal_reads_do_not_switch_generations() {
    let ((), report) = run_async_under_lab(0x6e65_0002, |root| async move {
        ...
        let (_, mut pinned) = prepared.stream_in_view(&view, &cx, &args, policy()).unwrap();
        drop(prepared); drop(text); drop(args);
        ...
        let mut cascade = WriteBatch::new(R); cascade.delete_vertex(WIDE);
        ...
        let mut actual = vec![first]; actual.extend(stream.by_ref().collect::<Result<Vec<_>, _>>().unwrap());
        assert_eq!(result(columns.clone(), actual), expected);
        assert_eq!(result(columns, pinned.by_ref().collect::<Result<Vec<_>, _>>().unwrap()), expected);
        assert_eq!(stream.snapshot_seq(), basis);
        let args = GqlParameters::new();
        let temporal = format!("MATCH (a)-[r:R]-(b) FOR SYSTEM_TIME AS OF SEQ {} RETURN r, a, b, r.p AS cost", basis.0);
        let prepared = PreparedNativeRead::prepare(&temporal, &args, symbols).unwrap();
        let (columns, mut cursor) = prepared.stream(&db, &cx, &args, policy()).unwrap();
        assert_eq!(cursor.snapshot_seq(), basis);
        assert_eq!(result(columns, cursor.by_ref().collect::<Result<Vec<_>, _>>().unwrap()), expected);
        let (columns, mut cursor) = prepared.stream_in_view(&view, &cx, &args, policy()).unwrap();
        assert_eq!(cursor.snapshot_seq(), basis);
        assert_eq!(result(columns, cursor.by_ref().collect::<Result<Vec<_>, _>>().unwrap()), expected);
        let fresh = PreparedNativeRead::prepare("MATCH (a)-[r:R]->(b) RETURN r, a, b", &args, symbols).unwrap();
...
fn native_edge_rebinding_refusals_and_future_fences_precede_source_work() {
    let ((), report) = run_async_under_lab(0x6e65_0003, |root| async move {
        ...
        let cx = contexts.query(); let commit = contexts.commit();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let basis = db.write(&commit, seed()).await.unwrap(); let view = db.read_session().unwrap();
        ...
        let prepared = PreparedNativeRead::prepare(
            "MATCH (a)-[r:R]->(b) WHERE r.p > $floor RETURN r, a, b", &low,
            |kind: GraphSymbolKind, name: &str| { calls.set(calls.get() + 1); symbols(kind, name) }).unwrap();
        ...
        assert!(matches!(prepared.stream_in_view(&view, &cx, &GqlParameters::new(), GqlQueryPolicy::new(0, 0, 0, 0)),
        ...
        for text in [
            ...
            let error = plan.stream_in_view(&view, &cx, &empty, GqlQueryPolicy::new(0, 0, 0, 0)).unwrap_err();
            assert!(matches!(&error, QueryError::EdgeStream(GqlQueryError::Source(EdgeScanError::Plan(_)))), "{error:?}");
            assert!(std::error::Error::source(&error).is_some());
        }
        let future = format!("MATCH (a)-[r:R]->(b) FOR SYSTEM_TIME AS OF SEQ {} RETURN r, a, b LIMIT 0", basis.0 + 1);
        let plan = PreparedNativeRead::prepare(&future, &empty, symbols).unwrap();
        assert!(matches!(plan.stream_in_view(&view, &cx, &empty, GqlQueryPolicy::new(0, 0, 0, 0)),
            Err(QueryError::EdgeStream(GqlQueryError::Source(EdgeScanError::Source(ReadError::BeyondFrontier { .. }))))));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn native_edge_quotas_keep_prefixes_and_exact_counters_across_pulls() {
    let ((), report) = run_async_under_lab(0x6e65_0004, |root| async move {
        ...
        let args = GqlParameters::new();
        let plan = PreparedNativeRead::prepare("MATCH (a)-[r:R]->(b) RETURN r, a, b, r.p AS cost", &args, symbols).unwrap();
...
fn native_edge_limit_and_close_never_drive_an_unrequested_candidate_suffix() {
    let ((), report) = run_async_under_lab(0x6e65_0005, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut batch = WriteBatch::new(R);
        batch.create_vertex(VId(0), vec![], vec![]); batch.create_vertex(WIDE, vec![], vec![]);
        for id in 0..512 { batch.add_edge(EId(id), VId(0), WIDE, vec![]); }
        db.write(&commit, batch).await.unwrap();
        let args = GqlParameters::new();
        for limit in [0, 1] {
            let text = format!("MATCH (a)-[r:R]->(b) RETURN r, a, b LIMIT {limit}");
            let prepared = PreparedNativeRead::prepare(&text, &args, symbols).unwrap();
            let (_, mut cursor) = prepared.stream(&db, &cx, &args, GqlQueryPolicy::new(limit, limit, 10_000, 1000)).unwrap();
            assert_eq!(cursor.row_stats().snapshot_records, 0);
            let actual = cursor.by_ref().collect::<Result<Vec<_>, _>>().unwrap();
            assert_eq!(actual.len() as u64, limit);
            assert_eq!(cursor.row_stats().snapshot_records, limit);
        }
        let prepared = PreparedNativeRead::prepare("MATCH (a)-[r:R]->(b) RETURN r, a, b", &args, symbols).unwrap();
        let (_, mut cursor) = prepared.stream(&db, &cx, &args, policy()).unwrap();
