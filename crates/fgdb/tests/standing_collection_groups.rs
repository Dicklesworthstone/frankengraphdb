...
use fgdb::{Database, DatabaseKeys, MemVfs, PreparedNativeRead, QueryResult, StandingQueryError,
...
fn keys() -> DatabaseKeys { DatabaseKeys::new([0xb4; 32], DatabaseSecurityNamespaceId([0xb5; 32]), [0xb6; 32]) }
...
fn seed() -> WriteBatch {
    ...
    for (id, bucket, amount) in [(1, 1, Some(7)), (2, 1, Some(7)),
        (3, 2, None), (u128::MAX, 2, Some(2))] {
        batch.create_vertex(VId(id), vec![], vec![(B, CanonicalScalar::Int(bucket)),
...
fn matches_snapshot(db: &Database<MemVfs>, cx: &QueryCx, text: &str, args: &GqlParameters,
    handle: &StandingQueryHandle) {
    let (at, actual) = db.standing_native_query(cx, handle, policy()).unwrap();
    assert_eq!(at, db.frontier().unwrap());
    assert_eq!(actual, db.query(cx, text, args, symbols, policy()).unwrap(), "{text}");
}
...
fn list_keys_any_extrema_and_nine_numeric_aggregates_follow_commits_and_exact_deltas() {
    let ((), report) = run_async_under_lab(0x4c47_0101, |root| async move {
        ...
        let args = GqlParameters::new(); let texts = queries();
        let handles: Vec<_> = texts.iter().map(|text|
            db.register_standing_native(&cx, text, &args, symbols, policy()).unwrap()).collect();
        let QueryResult::Rows { rows, .. } = db.standing_native_query(&cx, &handles[0], policy()).unwrap().1
            else { panic!("read returned a write") };
        ...
        for handle in &handles { assert!(db.standing_group_delta(&cx, handle).unwrap().is_none()); }
        let mut integrated: Vec<_> = handles.iter().map(|h| copy(db.standing_query(&cx, h).unwrap().rows())).collect();
        for step in 0..5 {
            for (text, handle) in texts.iter().zip(&handles) { matches_snapshot(&db, &cx, text, &args, handle); }
            let mut edit = WriteBatch::new(RelationId(1));
            match step {
                0 => { edit.set_vertex_property(VId(2), N, Some(CanonicalScalar::Int(-4))); }
                1 => { edit.set_vertex_property(VId(3), N, Some(CanonicalScalar::Int(11))); }
                2 => { edit.delete_vertex(VId(1)); }
                3 => { edit.set_vertex_property(VId(u128::MAX), B, Some(CanonicalScalar::Int(9))); }
                _ => { edit.create_vertex(VId(99), vec![], vec![(B, CanonicalScalar::Int(9)), (N, CanonicalScalar::Int(11))]); }
            }
            ...
            for ((text, handle), rows) in texts.iter().zip(&handles).zip(&mut integrated) {
                ...
                rows.integrate(delta.rows(), LimbLimit::new(4), &mut |_| Ok::<_, ()>(())).unwrap();
        ...
        assert!(matches!(db.standing_native_query(&cx, &handles[0], GqlQueryPolicy::new(0, 0, 1_000_000, 1_000_000)),
...
fn late_dynamic_numeric_failure_fences_only_its_view_and_rebuild_repairs_the_circuit() {
    let ((), report) = run_async_under_lab(0x4c47_0102, |root| async move {
        ...
        let numeric = db.register_standing_native(&cx, NINE, &args, symbols, policy()).unwrap();
        let other = db.register_standing_native(&cx, healthy, &args, symbols, policy()).unwrap();
        let mut edit = WriteBatch::new(RelationId(1));
        ...
        assert!(matches!(db.standing_native_query(&cx, &numeric, policy()),
            Err(StandingQueryError::Unavailable { reason: StandingQueryFailure::NonIntegerAggregate { .. }, .. })));
...
fn empty_expansions_keep_global_nulls_and_zero_pages_never_hide_operand_errors() {
    let ((), report) = run_async_under_lab(0x4c47_0103, |root| async move {
        ...
        for text in [
            ...
            let handle = db.register_standing_native(&cx, text, &args, symbols, policy()).unwrap();
            matches_snapshot(&db, &cx, text, &args, &handle);
            db.rebuild_standing_query(&cx, &handle, policy()).unwrap();
            matches_snapshot(&db, &cx, text, &args, &handle);
        }
        for tail in ["", " HAVING count < 0", " LIMIT 0"] {
            let text = format!("MATCH (n) WITH [[n.amount]] AS xs UNWIND xs AS x RETURN COUNT(*) AS count,SUM(x) AS total{tail}");
            let prepared = PreparedNativeRead::prepare(&text, &args, symbols).unwrap();
            assert!(matches!(prepared.register_standing(&mut db, &cx, &args, policy()),
                Err(StandingQueryError::Maintenance(StandingQueryFailure::NonIntegerAggregate { .. }))));
...
fn collection_pipeline_parameters_are_frozen_and_reopening_rebuilds_from_storage() {
    let ((), report) = run_async_under_lab(0x4c47_0104, |root| async move {
        let c = PurposeContexts::narrow_runtime_root(&root); let cx = c.query(); let commit = c.commit();
        let vfs = MemVfs::new().unwrap(); let directory = vfs.database_dir();
        let mut db = Database::create_with_vfs(&commit, vfs.clone(), &directory, keys()).await.unwrap();
        db.write(&commit, seed()).await.unwrap();
        ...
        let prepared = PreparedNativeRead::prepare(text, &args, symbols).unwrap();
        let first = prepared.register_standing(&mut db, &cx, &args, policy()).unwrap();
        args = GqlParameters::new().with_int64("extra", 17).unwrap();
        let second = prepared.register_standing(&mut db, &cx, &args, policy()).unwrap();
        drop(prepared);
        let old = GqlParameters::new().with_int64("extra", 3).unwrap();
        assert_ne!(db.standing_native_query(&cx, &first, policy()).unwrap(),
        ...
        let mut db = Database::open_with_vfs(&commit, vfs, &directory, keys()).await.unwrap();
        assert!(matches!(db.standing_query(&cx, &first), Err(StandingQueryError::ForeignHandle)));
        let current = db.register_standing_native(&cx, text, &args, symbols, policy()).unwrap();
