...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (id, n) in [(START, Some(3)), (VId(1), None), (END, Some(-2))] {
        r.create_vertex(
            id,
            vec![],
            n.map(|n| vec![(P, CanonicalScalar::Int(n))])
                .unwrap_or_default(),
        );
    }
    for (id, from, to, n) in [
        (1, START, VId(1), Some(7)),
        (2, START, VId(1), None),
        (3, END, START, Some(-4)),
        (4, START, START, Some(2)),
    ] {
        r.add_edge(
            EId(id),
            from,
...
fn shapes() -> Vec<String> {
    ...
    for direction in 0..3 {
        ...
        for shape in 0..3 {
            if shape == 1 {
                body.push_str(&edge("s", "S", "c"));
            }
            if shape == 2 {
                body.push_str(&edge("t", "R", "a"));
            }
    ...
    let ((), report) = run_async_under_lab(0xc0a5_5011, |root| async move {
        ...
        for text in shapes() {
            ...
            let expected = db
                ...
                .value;
            let cursor = db
                .stream_global_edge_aggregate_governed(&cx, &plan, policy())
                .unwrap();
        ...
        for (q, plan, expected, mut cursor) in paused {
            ...
            let mut old = db
                .stream_global_edge_aggregate_governed_at(&cx, &plan, basis, policy())
                .unwrap();
            ...
            assert_eq!(vec![old.next().unwrap().unwrap()], expected);
            let want = db
                .execute_graph_aggregate_governed_at(&cx, &q, edited, policy())
                .unwrap()
                .value;
            let mut live = db
                .stream_global_edge_aggregate_governed(&cx, &plan, policy())
...
fn cumulative_candidate_work_scratch_and_output_limits_refuse_atomically() {
    let ((), report) = run_async_under_lab(0xc0a5_5012, |root| async move {
        ...
        let exact = GqlQueryPolicy::new(
            ...
            1,
            eval.work_units,
            eval.scratch_entries,
        );
        assert_eq!(
            db.stream_global_edge_aggregate_governed(&cx, &plan, exact)
                .unwrap()
                .next()
                .unwrap()
                .unwrap(),
        ...
        for p in [
            ...
            GqlQueryPolicy::new(u64::MAX, 1, eval.work_units - 1, u64::MAX),
            GqlQueryPolicy::new(u64::MAX, 1, u64::MAX, eval.scratch_entries - 1),
        ] {
            let mut stream = db
                .stream_global_edge_aggregate_governed(&cx, &plan, p)
                .unwrap();
        ...
        assert_eq!(stopped.row_stats().snapshot_records, 0);
        assert!(matches!(
            db.stream_global_edge_aggregate_governed_at(
                &cx,
                &plan,
                CommitSeq(basis.0 + 1),
...
fn late_invalid_sums_do_not_publish_the_accumulated_count_and_old_views_stay_valid() {
    let ((), report) = run_async_under_lab(0xc0a5_5013, |root| async move {
        ...
        bad.set_edge_property(EId(4), P, Some(CanonicalScalar::Bool(true)));
        ...
        assert_eq!(row.values()[1].as_integer(), Some(5));
        let empty = EdgeAggregatePlan::compile(&query(
            "MATCH (a)-[r:R]->(b) WHERE a.p=999 RETURN COUNT(*) AS n,SUM(r.p) AS total",
        ))
        .unwrap();
        let row = view
            .stream_global_edge_aggregate_governed(&cx, &empty, policy())
            .unwrap()
            .next()
            .unwrap()
            .unwrap();
...
fn thousands_of_parallel_inputs_need_one_output_row_allowance() {
    let ((), report) = run_async_under_lab(0xc0a5_5014, |root| async move {
        ...
        let plan = EdgeAggregatePlan::compile(&q).unwrap();
        let mut stream = db
            .stream_global_edge_aggregate_governed(
                &cx,
                &plan,
                GqlQueryPolicy::new(4096, 1, 10_000_000, 1_000_000),
            )
            .unwrap();
        let result = stream.next().unwrap().unwrap();
        assert_eq!(result.values()[0].as_count(), Some(4096));
        assert_eq!(
            result.values()[1].as_integer(),
            Some(4096 * i128::from(i64::MAX))
        );
