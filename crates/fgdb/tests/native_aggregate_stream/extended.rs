...
fn all_nine_native_aggregates_preserve_aliases_parameters_nulls_and_duplicate_support() {
    let ((), report) = run_async_under_lab(0xa66e_2001, |root| async move {
        ...
        let mut empty = db.query_aggregate_stream(&cx, &text, &GqlParameters::new(), symbols(), wide()).unwrap();
        let row = empty.next().unwrap().unwrap();
        assert!(row.values()[..3].iter().all(|value| value.as_count() == Some(0)));
        ...
        duplicate.create_vertex(VId(3), vec![LABEL], vec![
            (SCORE, CanonicalScalar::Int(14)),
            (TAG, CanonicalScalar::ucs_basic_text("' $not_syntax").unwrap()),
        ]);
        db.write(&commit, duplicate).await.unwrap();
        let mut all = db.query_aggregate_stream(&cx, &text, &GqlParameters::new(), symbols(), wide()).unwrap();
        ...
        for (text, args) in [
            ...
            (String::from("MATCH (n:L) WHERE n.tag=$tag RETURN MAX(n) AS last, MIN(n.tag) AS tag, AVG(DISTINCT n.score) AS first_average, AVG(DISTINCT n.score) AS second_average"),
...
fn temporal_exact_values_and_owned_extrema_survive_writes_compaction_and_source_drop() {
    let ((), report) = run_async_under_lab(0xa66e_2002, |root| async move {
        ...
        let text = format!("MATCH (n:L) FOR SYSTEM_TIME AS OF SEQ $seq RETURN {EXACT}, COUNT(DISTINCT n) AS vertices, MIN(n) AS first, MAX(n) AS last, MIN(n.tag) AS first_tag, MAX(n.tag) AS last_tag");
        let args = GqlParameters::new().with_uint64("seq", 1).unwrap();
        let prepared = PreparedNativeRead::prepare(&text, &args, symbols()).unwrap();
        let QueryResult::Rows { rows: old_rows, .. } = prepared.execute(&db, &cx, &args, wide()).unwrap()
            else { panic!("expected read rows"); };
        let mut pinned = prepared.stream_aggregate_in_view(&old_view, &cx, &args, wide()).unwrap();
        ...
        change.set_vertex_property(VId(1), TAG, Some(CanonicalScalar::ucs_basic_text(&"private-text".repeat(512)).unwrap()));
        ...
        assert!(matches!(prepared.stream_aggregate_in_view(&old_view, &cx, &future, GqlQueryPolicy::new(0, 0, 0, 0)),
            Err(QueryError::AggregateStream(GqlQueryError::Source(GraphAggregateError::Source(
        ...
        let QueryResult::Rows { rows: latest_rows, .. } = prepared.execute(&db, &cx, &future, wide()).unwrap()
            else { panic!("expected read rows"); };
        let mut latest = prepared.stream_aggregate(&db, &cx, &future, wide()).unwrap();
        ...
        assert_eq!(old.values()[10].as_value(), Some(&GraphValue::Vertex(VId(0))));
        assert_eq!(old.values()[11].as_value(), Some(&GraphValue::Vertex(VId(u128::MAX))));
...
fn new_native_aggregate_state_obeys_exact_cumulative_quotas_and_nondraining_close() {
    let ((), report) = run_async_under_lab(0xa66e_2003, |root| async move {
        ...
        db.write(&commit, seed()).await.unwrap();
        let mut change = WriteBatch::new(RelationId(1));
        change.set_vertex_property(VId(u128::MAX), TAG,
            Some(CanonicalScalar::ucs_basic_text(&"z-payload".repeat(256)).unwrap()));
        db.write(&commit, change).await.unwrap();
        let text = format!("MATCH (n:L) RETURN {EXACT}, COUNT(DISTINCT n.tag) AS tags, MIN(n.tag) AS first_tag, MAX(n.tag) AS last_tag");
        ...
        let exact = GqlQueryPolicy::new(rows.snapshot_records, 1, stats.work_units, stats.scratch_entries);
...
fn late_numeric_domain_errors_do_not_publish_prior_distinct_or_extremum_cells() {
    let ((), report) = run_async_under_lab(0xa66e_2004, |root| async move {
        ...
        db.write(&commit, seed()).await.unwrap();
        let mut change = WriteBatch::new(RelationId(1));
        change.set_vertex_property(VId(u128::MAX), SCORE,
            Some(CanonicalScalar::ucs_basic_text("private invalid operand").unwrap()));
        db.write(&commit, change).await.unwrap();
        for (function, average) in [("AVG(n.score)", true), ("AVG(DISTINCT n.score)", true),
            ("SUM(DISTINCT n.score)", false)] {
            let text = format!("MATCH (n:L) RETURN COUNT(DISTINCT n.tag) AS tags, MIN(n.tag) AS first_tag, {function} AS numeric");
            let mut cursor = db.query_aggregate_stream(&cx, &text, &GqlParameters::new(), symbols(), wide()).unwrap();
            let error = cursor.next().unwrap().unwrap_err();
            if average {
                assert!(matches!(error, GqlQueryError::Source(GraphAggregateError::NonIntegerAverage { aggregate: 2 })));
            } else {
                assert!(matches!(error, GqlQueryError::Source(GraphAggregateError::NonIntegerSum { aggregate: 2 })));
            }
            assert_eq!(cursor.row_stats().result_rows, 0);
            assert_eq!(cursor.state(), VertexScanState::Failed);
