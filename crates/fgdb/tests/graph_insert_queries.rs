...
fn allocator(request: GraphInsertRequest) -> Result<ElementId, &'static str> {
    Ok(match request {
        GraphInsertRequest::Vertex { row, vertex } => {
            ElementId::Vertex(VId(1_000 + row as u128 * 4 + vertex as u128))
        }
        GraphInsertRequest::Edge { row, edge } => {
            ElementId::Edge(EId(10_000 + row as u128 * 4 + edge as u128))
        }
    })
}
    ...
    for (row, source, destination, p, q) in [
        ...
        assert_eq!(
            ...
            vec![
                ...
                (Q, q.map_or(CanonicalScalar::Null, CanonicalScalar::Int))
            ]
        );
...
fn creation_consumes_staged_values_publishes_once_and_survives_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0xc8ea_0001, |root| async move {
        ...
        let stats = txn
            .execute_graph_insert_governed(&mut db, &cx, &insertion, policy(), allocator)
            .unwrap();
        assert_eq!(
            (
        ...
        assert_eq!(
            db.vertices().unwrap(),
            initial,
            "staging does not mutate the durable snapshot"
        );
        ...
        assert_eq!(
            committed.0,
            basis.0 + 1,
            "one ordinary commit publishes the whole insertion"
        );
        verify_created(&db);
        assert_eq!(calls.get(), 5);
        assert_eq!(
        ...
        let db = Database::open_with_vfs(&commit, vfs, &path, keys())
            ...
            .unwrap();
        verify_created(&db);
        let copies = PreparedGraphText::prepare("MATCH (n:Copy) RETURN n", symbols)
            .unwrap()
            .bind_parameters(&GqlParameters::new())
            .unwrap();
        assert!(
            db.execute_graph_pattern_governed_at(&cx, &copies, basis, policy().query)
                .unwrap()
                .value
                .is_empty()
        );
        assert_eq!(
            db.execute_graph_pattern_governed(&cx, &copies, policy().query)
                .unwrap()
                .value
                .len(),
            3
...
fn late_allocator_and_storage_identity_failures_preserve_the_exact_prior_workspace() {
    let ((), report) = run_async_under_lab(0xc8ea_0002, |root| async move {
        ...
        for mode in 0..5 {
            ...
            let initial = db.vertices().unwrap();
            let initial_edges = db.edges().unwrap();
            let mut txn = db.begin(&txcx).unwrap();
            let mut prior = WriteBatch::new(R);
            ...
            let result =
                txn.execute_graph_insert_governed(&mut db, &cx, &standard(), policy(), |request| {
                    requested.set(requested.get() + 1);
                    match (mode, request) {
                        (0, GraphInsertRequest::Edge { row: 2, edge: 1 }) => {
                            Err("late allocation refusal")
                        }
                        (1, GraphInsertRequest::Vertex { row: 2, .. }) => {
                            Ok(ElementId::Vertex(VId(1)))
                        }
                        (2, GraphInsertRequest::Vertex { row: 2, .. }) => {
                            Ok(ElementId::Vertex(VId(98)))
                        }
                        (3, GraphInsertRequest::Edge { row: 2, edge: 1 }) => {
                            Ok(ElementId::Edge(EId(11)))
                        }
                        (4, GraphInsertRequest::Vertex { row: 2, .. }) => {
                            Ok(ElementId::Vertex(VId(1_000)))
                        }
                        _ => allocator(request),
                    }
                });
            ...
            assert!(
                requested.get() >= 7,
                "exercise failure after real partial allocation"
            );
            ...
            for id in [1_000, 1_004, 1_008, 98] {
...
fn data_and_count_refusals_precede_allocation_and_do_not_discard_earlier_staging() {
    let ((), report) = run_async_under_lab(0xc8ea_0003, |root| async move {
        ...
        for (insertion, allowance) in [
            ...
            (
                query(
                    "MATCH (a:Source) OPTIONAL MATCH (a)-[:S]->(b) CREATE (copy),(b)-[:R]->(copy)",
                ),
                policy(),
            ),
            (
                query("MATCH (a:Source)-[:R]->(b) CREATE (copy)"),
                GraphInsertPolicy::new(policy().query, 2, 100),
            ),
            (
                query("MATCH (a:Source)-[:R]->(b) CREATE (a)-[:R]->(b)"),
                GraphInsertPolicy::new(policy().query, 100, 2),
            ),
        ] {
            assert!(
    ...
    let ((), report) = run_async_under_lab(0xc8ea_0004, |root| async move {
        ...
        for mode in 0..5 {
            for conflict in 0..3 {
                ...
                let allowance =
                    GraphInsertPolicy::new(policy().query, if mode == 1 { 0 } else { 100 }, 100);
                ...
                match conflict {
                    ...
                    1 => {
                        winner.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(1001)));
                    }
                    _ => {
                        winner.add_edge(EId(14), VId(1), VId(3), vec![]);
                    }
                }
...
fn owner_basis_relation_and_exact_query_budgets_guard_the_public_creation_entrypoint() {
    let ((), report) = run_async_under_lab(0xc8ea_0005, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        assert_eq!(
            txn.execute_graph_insert_governed(&mut db, &cx, &insertion, exact, allocator)
                .unwrap(),
            measured
        );
        txn.abort();
        for query_budget in [
            GqlQueryPolicy::new(
                measured.selection.snapshot_records - 1,
                100,
                u64::MAX,
                u64::MAX,
            ),
            ...
            assert!(
                txn.execute_graph_insert_governed(
                    ...
                    &cx,
                    &insertion,
                    GraphInsertPolicy::new(query_budget, 3, 6),
                    allocator
                )
                .is_err()
            );
            assert!(txn.vertex(&db, VId(1_000)).unwrap().is_none());
            txn.abort();
        }
        let mut txn = db.begin(&txcx).unwrap();
        let calls = Cell::new(0);
        let result =
            txn.execute_graph_insert_governed(&mut foreign, &cx, &insertion, policy(), |request| {
                calls.set(calls.get() + 1);
                allocator(request)
            });
        assert!(matches!(
