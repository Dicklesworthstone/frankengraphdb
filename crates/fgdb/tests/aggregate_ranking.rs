...
use fgdb_gql::{GraphAggregateError, GraphAggregateRow, GraphSymbol, GraphSymbolKind,
    GqlBudgetDimension, GqlParameters, GqlQueryError, GqlQueryPolicy, PreparedGraphAggregateText};
...
fn plain(rows: &[GraphAggregateRow]) -> Vec<Summary> {
    rows.iter().map(|row| (row.keys()[0].as_vertex().unwrap(),
        row.get(0).unwrap().as_count().unwrap(), row.get(1).unwrap().as_integer().unwrap(),
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord], minimum: u64, off: usize, take: usize) -> Vec<Summary> {
    let mut groups: BTreeMap<VId, Vec<Option<i64>>> = BTreeMap::new();
    for left in edges.iter().filter(|edge| edge.entry.relation == R) {
        for right in edges.iter().filter(|edge| edge.entry.relation == S && edge.entry.src == left.entry.dst) {
            let item = vertices.iter().find(|row| row.vid == right.entry.dst).unwrap();
            let value = item.props.iter().find(|(key, _)| *key == P).map(|(_, value)| value);
    ...
    let mut rows: Vec<_> = groups.into_iter().filter_map(|(key, values)| {
        let nonnull: Vec<_> = values.iter().filter_map(|value| *value).collect();
        (values.len() as u64 >= minimum && !nonnull.is_empty()).then(|| (
            key, values.len() as u64, nonnull.iter().map(|value| i128::from(*value)).sum::<i128>(), nonnull.len() as u64,
        ))
    }).collect();
...
fn all_five_sources_rank_before_slicing_and_use_one_combined_policy() {
    let ((), report) = run_async_under_lab(0xa680_0001, |root| async move {
        ...
        for value in [
            db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value,
            ...
            txn.execute_graph_aggregate_governed(&db, &cx, &query, policy()).unwrap().value,
        ...
        let exact = GqlQueryPolicy::new(8, 2, full.evaluator.work_units, full.evaluator.scratch_entries);
        assert_eq!(db.execute_graph_aggregate_governed(&cx, &query, exact).unwrap(), full);
        for cap in [GqlQueryPolicy::new(8, 2, full.evaluator.work_units - 1, u64::MAX),
            GqlQueryPolicy::new(8, 2, u64::MAX, full.evaluator.scratch_entries - 1)] {
            assert!(matches!(db.execute_graph_aggregate_governed(&cx, &query, cap),
                Err(GqlQueryError::Evaluator(error)) if error.observed == u128::from(error.limit) + 1));
        }
        assert!(matches!(db.execute_graph_aggregate_governed(&cx, &query, GqlQueryPolicy::new(8, 1, u64::MAX, u64::MAX)),
        ...
        assert_eq!(plain(&db.execute_graph_aggregate_governed(&cx, &page, policy()).unwrap().value),
            vec![(VId(6), 3, 14, 2)]);
...
fn canonical_staging_changes_rank_while_history_and_pinned_generations_keep_old_order() {
    let ((), report) = run_async_under_lab(0xa680_0002, |root| async move {
        ...
        let query = PreparedGraphAggregateText::prepare(TEXT, symbols).unwrap()
            .bind_parameters(&arguments(1, 0, 2)).unwrap();
        let old = db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value;
        ...
        let expected = oracle(&txn.vertices(&db).unwrap(), &txn.edges(&db).unwrap(), 1, 0, 2);
        ...
        assert_eq!(txn.execute_graph_aggregate_governed(&db, &cx, &query, exact).unwrap(), staged);
        assert_eq!(db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value, old);
        txn.commit(&mut db, &commit).await.unwrap();
        assert_eq!(plain(&db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value), expected);
        ...
        assert_eq!(reopened.execute_graph_aggregate_governed_at(&cx, &query, basis, policy()).unwrap().value, old);
        assert_eq!(pinned.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value, old);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0xa680_0003, |root| async move {
        ...
        for minimum in [1, 999] {
            for conflict in [false, true] {
                ...
                let query = PreparedGraphAggregateText::prepare(TEXT, symbols).unwrap()
                    .bind_parameters(&arguments(minimum, 0, 1)).unwrap();
                let mut txn = db.begin(&txn_cx).unwrap();
                let mut stage = WriteBatch::new(R); stage.create_vertex(VId(99), vec![], vec![]);
                txn.write(&mut db, stage).unwrap();
                let result = txn.execute_graph_aggregate_governed(&db, &cx, &query, policy()).unwrap();
                if minimum == 1 { assert_eq!(result.value[0].keys()[0].as_vertex(), Some(VId(4))); }
                ...
                winner.set_vertex_property(if conflict { VId(3) } else { VId(90) }, P, Some(CanonicalScalar::Int(1000)));
                ...
                } else { committed.unwrap(); assert!(db.vertex(VId(99)).unwrap().is_some()); }
            }
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn ranked_empty_counts_and_owner_future_cancellation_errors_retain_their_meaning() {
    let ((), report) = run_async_under_lab(0xa680_0004, |root| async move {
        ...
        let query = PreparedGraphAggregateText::prepare(
            "MATCH (a)-[:R]->(b) RETURN COUNT(*) AS n HAVING n > 0 ORDER BY n DESC", symbols)
