...
fn expected(edges: &[EdgeRecord], q: &PreparedGraphCheapestPath, count: usize) -> Vec<Answer> {
    oracle(edges, q, usize::MAX).into_iter().filter(|(_, steps)| {
        if q.mode() == GraphCheapestPathMode::Trail {
            return steps.iter().map(|step| step.0).collect::<BTreeSet<_>>().len() == steps.len();
        }
        let mut vertices: Vec<_> = std::iter::once(q.source()).chain(steps.iter().map(|step| step.1)).collect();
        if q.mode() == GraphCheapestPathMode::Simple && vertices.len() > 1 && vertices.last() == Some(&q.source()) {
            vertices.pop();
        }
...
fn constrained_modes_share_pinned_history_canonical_overlays_and_recovered_sources() {
    let ((), report) = run_async_under_lab(0xc0a5_1011, |root| async move {
        ...
        let before = db.edges().unwrap(); let pinned = db.read_session().unwrap();
        let mut txn = db.begin(&txcx).unwrap();
        for mode in MODES {
            for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
                let (start, end) = if direction == GlaDirection::Reverse { (3, 0) } else { (0, 3) };
                let q = query(start, end, direction, 1, 4).with_mode(mode);
                let want = expected(&before, &q, 7);
                assert!(!want.is_empty());
                for rows in [
                    db.execute_graph_cheapest_paths_governed(&cx, &q, 7, policy()).unwrap(),
                    db.execute_graph_cheapest_paths_governed_at(&cx, &q, 7, basis, policy()).unwrap(),
                    pinned.execute_graph_cheapest_paths_governed(&cx, &q, 7, policy()).unwrap(),
                    pinned.execute_graph_cheapest_paths_governed_at(&cx, &q, 7, basis, policy()).unwrap(),
                    txn.execute_graph_cheapest_paths_governed(&db, &cx, &q, 7, policy()).unwrap(),
                ] { assert_eq!(plain(&rows.value), want, "{mode:?} {direction:?}"); }
                for rows in [
                    db.execute_graph_cheapest_path_governed(&cx, &q, policy()).unwrap(),
                    pinned.execute_graph_cheapest_path_governed(&cx, &q, policy()).unwrap(),
                    txn.execute_graph_cheapest_path_governed(&db, &cx, &q, policy()).unwrap(),
        ...
        for mode in MODES {
            ...
            assert_eq!(plain(&txn.execute_graph_cheapest_paths_governed(&db, &cx, &q, 7, policy()).unwrap().value), expected(&staged, &q, 7));
            assert_eq!(plain(&db.execute_graph_cheapest_paths_governed(&cx, &q, 7, policy()).unwrap().value), expected(&before, &q, 7));
        }
        ...
        for mode in MODES {
            let q = query(0, 3, GlaDirection::Forward, 0, 4).with_mode(mode);
            assert_eq!(plain(&db.execute_graph_cheapest_paths_governed(&cx, &q, 7, policy()).unwrap().value), expected(&staged, &q, 7));
            assert_eq!(plain(&db.execute_graph_cheapest_paths_governed_at(&cx, &q, 7, basis, policy()).unwrap().value), expected(&before, &q, 7));
            assert_eq!(plain(&pinned.execute_graph_cheapest_paths_governed(&cx, &q, 7, policy()).unwrap().value), expected(&before, &q, 7));
        }
        ...
        for mode in MODES {
            let q = query(0, 3, GlaDirection::Forward, 0, 4).with_mode(mode);
            assert_eq!(plain(&db.execute_graph_cheapest_paths_governed(&cx, &q, 7, policy()).unwrap().value), expected(&cascaded, &q, 7));
            assert_eq!(plain(&pinned.execute_graph_cheapest_paths_governed(&cx, &q, 7, policy()).unwrap().value), expected(&before, &q, 7));
        }
    });
        ...
        for mode in MODES {
            for outcome in 0..5 {
                for change in 0..3 {
                    ...
                    txn.write(&mut db, stage).unwrap();
                    let q = query(0, if outcome == 2 { 99 } else { 3 }, GlaDirection::Forward, 1, 4).with_mode(mode);
                    let result = txn.execute_graph_cheapest_paths_governed(&db, &cx, &q,
                        if outcome == 1 { 0 } else { 3 }, GqlQueryPolicy::new(1000,
                            if outcome == 3 { 0 } else { 1000 }, 5_000_000, 1_000_000));
                    match outcome {
...
fn constrained_mode_limits_and_owner_frontier_fences_share_the_original_database_boundaries() {
    let ((), report) = run_async_under_lab(0xc0a5_1013, |root| async move {
        ...
        for mode in MODES {
            ...
            let baseline = db.execute_graph_cheapest_paths_governed(&cx, &q, 3, policy()).unwrap();
            ...
            assert_eq!(db.execute_graph_cheapest_paths_governed(&cx, &q, 3, exact).unwrap(), baseline);
            for bad in [
                GqlQueryPolicy::new(baseline.rows.snapshot_records - 1, 3, u64::MAX, u64::MAX),
                GqlQueryPolicy::new(baseline.rows.snapshot_records, baseline.rows.result_rows - 1, u64::MAX, u64::MAX),
            ...
            ] { assert!(db.execute_graph_cheapest_paths_governed(&cx, &q, 3, bad).is_err()); }
            assert_eq!(db.execute_graph_cheapest_paths_governed(&cx, &q, 3, exact).unwrap(), baseline);
            assert!(matches!(db.execute_graph_cheapest_paths_governed_at(&cx, &q, 0, CommitSeq(basis.0 + 1),
                GqlQueryPolicy::new(0, 0, 0, 0)), Err(GqlQueryError::Source(GraphCheapestPathError::Source(GqlError::Read(_))))));
            assert!(matches!(txn.execute_graph_cheapest_paths_governed(&foreign, &cx, &q, 0,
        ...
        for mode in MODES {
            let q = query(0, 3, GlaDirection::Forward, 1, 4).with_mode(mode);
            assert!(matches!(db.execute_graph_cheapest_paths_governed(&cx, &q, 0, policy()),
