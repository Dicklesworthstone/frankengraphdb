...
fn symbols() -> RelationBind {
    RelationBind::new().with_relation("R", R).with_property("score", P)
}
...
fn seed() -> WriteBatch {
    ...
    for (id, value) in [(0, Some(2)), (1, None), (u128::MAX, Some(7))] {
        b.create_vertex(VId(id), vec![], vec![(P, value.map_or(CanonicalScalar::Null, CanonicalScalar::Int))]);
    }
    for (id, a, z, value) in [(1, 0, 1, 5), (2, 0, 1, 5), (3, u128::MAX, 1, 1), (4, 1, u128::MAX, -2)] {
        b.add_edge(EId(id), VId(a), VId(z), vec![(P, CanonicalScalar::Int(value))]);
    }
    b
}
fn cells(row: &fgdb_gql::GraphAggregateRow, slots: &[GraphAggregateTextSlot]) -> Vec<GraphAggregateValue> {
...
fn native_edge_groups_keep_textual_layouts_and_all_exact_aggregate_domains() {
    let ((), report) = run_async_under_lab(0xa66e_5101, |root| async move {
        ...
        for text in [
            ...
            let QueryResult::Rows { columns, rows } = db.query(&query, text, &args, symbols(), wide()).unwrap()
                else { panic!("aggregate read returned a write"); };
            let mut cursor = db.query_aggregate_stream(&query, text, &args, symbols(), wide()).unwrap();
            ...
            let actual: Vec<_> = cursor.by_ref().map(|row| cells(&row.unwrap(), &slots)).collect();
...
fn grouped_edge_history_parameters_and_views_survive_writer_and_template_drop() {
    let ((), report) = run_async_under_lab(0xa66e_5102, |root| async move {
        ...
        let args = GqlParameters::new().with_uint64("cut", 1).unwrap().with_int64("floor", 0).unwrap();
        let prepared = PreparedNativeRead::prepare(text, &args, symbols()).unwrap();
        let view = db.read_session().unwrap();
        let QueryResult::Rows { rows: expected, .. } = prepared.execute(&db, &query, &args, wide()).unwrap()
            else { panic!("not rows"); };
        let mut retained = prepared.stream_aggregate_in_view(&view, &query, &args, wide()).unwrap();
        ...
        for cut in 0..=2 {
            ...
            let QueryResult::Rows { rows, .. } = prepared.execute(&db, &query, &args, wide()).unwrap()
                else { panic!("not rows"); };
            let mut cursor = prepared.stream_aggregate(&db, &query, &args, wide()).unwrap();
            assert_eq!(cursor.snapshot_seq(), CommitSeq(cut));
            let positions = cursor.output_slots().to_vec();
            assert_eq!(cursor.by_ref().map(|row| cells(&row.unwrap(), &positions)).collect::<Vec<_>>(), rows);
        }
        let future = GqlParameters::new().with_uint64("cut", 2).unwrap().with_int64("floor", 0).unwrap();
        assert!(prepared.stream_aggregate_in_view(&view, &query, &future, GqlQueryPolicy::new(0, 0, 0, 0)).is_err());
        drop(prepared);
        drop(view);
        drop(args);
        drop(db);
        assert_eq!(retained.by_ref().map(|row| cells(&row.unwrap(), &slots)).collect::<Vec<_>>(), expected);
...
fn native_group_quotas_and_late_domain_failure_do_not_publish_partial_groups() {
    let ((), report) = run_async_under_lab(0xa66e_5103, |root| async move {
        ...
        let args = GqlParameters::new();
        let prepared = PreparedNativeRead::prepare(text, &args, symbols()).unwrap();
        let mut baseline = prepared.stream_aggregate(&db, &query, &args, wide()).unwrap();
        ...
        assert_eq!(prepared.stream_aggregate(&db, &query, &args, exact).unwrap().collect::<Result<Vec<_>, _>>().unwrap(), expected);
        let mut short = prepared.stream_aggregate(&db, &query, &args,
            GqlQueryPolicy::new(u64::MAX, r.result_rows - 1, u64::MAX, u64::MAX)).unwrap();
        assert!(matches!(short.next(), Some(Err(GqlQueryError::Rows(_)))));
        assert_eq!(short.row_stats().result_rows, 0);
        assert!(short.next().is_none());
        let mut edit = WriteBatch::new(R);
        edit.add_edge(EId(6), VId(0), VId(1), vec![(P, CanonicalScalar::Bool(true))]);
        db.write(&commit, edit).await.unwrap();
        let mut invalid = prepared.stream_aggregate(&db, &query, &args, wide()).unwrap();
