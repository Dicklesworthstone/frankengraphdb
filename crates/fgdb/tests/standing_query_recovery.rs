...
use fgdb_gql::{
    GqlQueryPolicy, GraphAggregate, GraphAggregateRow, PreparedGraphAggregate,
};
use fgdb_types::{
...
fn definition() -> PreparedGraphAggregate {
    ...
    PreparedGraphAggregate::prepare(input, &[0], &[
        GraphAggregate::count_rows("count"), GraphAggregate::sum_int("total", 1),
...
fn rows(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle) -> Vec<GraphAggregateRow> {
    let view = db.standing_query(cx, handle).unwrap();
    assert_eq!(view.frontier(), db.frontier().unwrap());
    view.rows().iter().map(|(row, weight)| {
        assert_eq!(weight, &ZWeight::ONE);
        row.clone()
    }).collect()
}
fn verify(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle) {
    let mut expected = db.execute_graph_aggregate_governed(cx, &definition(), wide()).unwrap().value;
    expected.sort();
...
fn failed_group_limit_rebuilds_in_place_at_current_frontier_and_resumes_commits() {
    let ((), report) = run_async_under_lab(0x6b01, |root| async move {
        ...
        let handle = db.register_standing_query(&cx, definition(), narrow).unwrap();
        ...
        assert!(matches!(db.rebuild_standing_query(&cx, &handle, narrow),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::ResultBudget))));
        assert!(matches!(db.standing_query(&cx, &alias), Err(StandingQueryError::Unavailable {
            frontier, reason: StandingQueryFailure::ResultBudget,
        }) if frontier == basis));
        assert_eq!(db.rebuild_standing_query(&cx, &handle, wide()).unwrap(), current);
        assert_eq!(db.frontier().unwrap(), current, "rebuild is not a database write");
        verify(&db, &cx, &alias);
        let snapshot = rows(&db, &cx, &alias);
        assert_eq!(snapshot[0].get(1).unwrap().as_integer(), Some(17));
        insert(&mut db, &commit, 3, 3, 11).await;
        verify(&db, &cx, &handle);
        assert_eq!(rows(&db, &cx, &alias).len(), 3, "successful rebuild installs its wider policy");
...
fn failed_rebuild_preserves_healthy_policy_and_corrected_data_can_repair_numeric_failure() {
    let ((), report) = run_async_under_lab(0x6b02, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        insert(&mut db, &commit, 1, 1, 7).await;
        let handle = db.register_standing_query(&cx, definition(), wide()).unwrap();
        ...
        for policy in [
            ...
            assert!(matches!(db.rebuild_standing_query(&cx, &handle, policy),
                Err(StandingQueryError::Maintenance(_))));
            assert_eq!(rows(&db, &cx, &handle), before);
            assert_eq!(*db.standing_query(&cx, &handle).unwrap().last_maintenance(), stats);
        }
        insert(&mut db, &commit, 2, 2, 3).await;
        verify(&db, &cx, &handle);
        let healthy = db.frontier().unwrap();
        let mut invalid = WriteBatch::new(R);
        invalid.set_vertex_property(VId(1), SCORE,
            Some(CanonicalScalar::ucs_basic_text("not-a-number").unwrap()));
        db.write(&commit, invalid).await.unwrap();
        assert!(matches!(db.rebuild_standing_query(&cx, &handle, wide()),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::NonIntegerSum))));
        assert!(matches!(db.standing_query(&cx, &handle), Err(StandingQueryError::Unavailable {
...
fn rebuild_exact_work_and_scratch_limits_succeed_and_one_below_preserves_the_view() {
    let ((), report) = run_async_under_lab(0x6b03, |root| async move {
        ...
        for (work, scratch, passes) in [
            ...
            let result = db.rebuild_standing_query(&cx, &handle,
                GqlQueryPolicy::new(100_000, 100_000, work, scratch));
            assert_eq!(result.is_ok(), passes);
            if passes { assert_eq!(result.unwrap(), frontier); }
            assert_eq!(rows(&db, &cx, &handle), before);
            assert_eq!(*db.standing_query(&cx, &handle).unwrap().last_maintenance(), stats);
...
fn rebuild_never_adopts_foreign_or_reopened_handles() {
    let ((), report) = run_async_under_lab(0x6b04, |root| async move {
        ...
        assert!(matches!(foreign.rebuild_standing_query(&cx, &handle, wide()),
            Err(StandingQueryError::ForeignHandle)));
        verify(&db, &cx, &handle);
        db.compact(&commit).await.unwrap();
        drop(db);
        let mut reopened = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        assert!(matches!(reopened.rebuild_standing_query(&cx, &handle, wide()),
            Err(StandingQueryError::ForeignHandle)));
        let fresh = reopened.register_standing_query(&cx, definition(), wide()).unwrap();
        reopened.rebuild_standing_query(&cx, &fresh, wide()).unwrap();
...
fn fenced_publication_requires_database_recovery_before_any_view_rebuild() {
    let ((), report) = run_async_under_lab(0x6b05, |root| async move {
        ...
        assert!(matches!(db.write_with_publication_failure(&commit, change,
            DerivedPublicationStage::PublishPartitionRoot).await,
            Err(WriteError::CommittedNeedsRecovery { .. })));
        assert!(matches!(db.rebuild_standing_query(&cx, &handle, wide()),
            Err(StandingQueryError::Read(_))));
        assert!(matches!(db.standing_query(&cx, &handle), Err(StandingQueryError::Read(_))));
        drop(db);
        ...
        let handle = db.register_standing_query(&cx, definition(), wide()).unwrap();
        verify(&db, &cx, &handle);
        assert_eq!(rows(&db, &cx, &handle)[0].get(1).unwrap().as_integer(), Some(13));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
