...
use fgdb_prism::{
    Directedness, FnxAlgorithm, FnxArgument, FnxCallSpec, FnxExecutionError,
    FnxExecutionLimits, FnxMemoryLimits, FnxParameters, FnxReadOptions,
    FnxResult, FnxSealedExecutionError, FnxSealedReadError, FnxSelection, FnxSourceLimits,
    FnxWeightSpec, MissingWeightPolicy, ParallelEdgePolicy, ProjectionLimits,
    ProjectionSpec, SealedGraphView, SealedProjectionError, SelfLoopPolicy, PageRankOptions,
};
...
fn memory() -> FnxMemoryLimits {
    FnxMemoryLimits { max_kernel_workspace_bytes: 1 << 24, max_result_bytes: 1 << 24 }
}
fn quantum(value: usize) -> NonZeroUsize { NonZeroUsize::new(value).unwrap() }
...
fn drive<F: Future>(future: F, probe: &SimulationCheckpointProbe, q: usize) -> (F::Output, usize) {
    ...
    loop {
        ...
        assert!(probe.calls() - before <= q.saturating_mul(32).saturating_add(128),
...
fn compare_execution(actual: &FnxResult, expected: &FnxResult) {
    ...
    assert_eq!(actual.certificate.estimated_work, expected.certificate.estimated_work);
    assert_eq!(actual.certificate.kernel_workspace_bytes, expected.certificate.kernel_workspace_bytes);
    assert_eq!(actual.certificate.snapshot, expected.certificate.snapshot);
    assert_eq!(actual.certificate.projection_digest, expected.certificate.projection_digest);
    assert_ne!(actual.certificate.execution_kernel, expected.certificate.execution_kernel);
}
    ...
    runtime.block_on(async {
        ...
        for direction in [Directedness::Directed, Directedness::Reversed, Directedness::Undirected] {
            for at in [original, db.frontier().unwrap()] {
                ...
                let graph = projection(&db, &cx, opt).await;
                let view = db.read_session().unwrap();
                let decoded = view.prism_projection_at(&cx, at, opt.selection, opt.projection,
                    opt.projection_limits, opt.source_limits).unwrap();
                let mut calls = vec![];
                for weighted in [false, true] {
                    calls.push(FnxCallSpec::pagerank(PageRankOptions::new(0.85, 1000, 1e-9, weighted).unwrap()));
                }
                for source in [VId(0), VId(u128::MAX)] {
                    for cutoff in [None, Some(0), Some(1)] {
                        calls.push(FnxCallSpec::single_source_shortest_path_length(source, cutoff));
                    }
                }
                for call in calls {
                    ...
                    for q in [1, 7, 257] {
                        ...
                        if q == 1 { assert!(pending > graph.node_count()); }
...
async fn small_database(cx: &CommitCx, n: usize, negative: bool) -> Database<MemVfs> {
    let mut db = Database::<MemVfs>::open_memory(cx, DatabaseKeys::new(
        [0x91; 32], DatabaseSecurityNamespaceId([0x92; 32]), [0x93; 32],
    )).await.unwrap();
    if n == 0 { return db; }
    let mut batch = WriteBatch::new(RelationId(1));
    for id in 0..n { batch.create_vertex(VId(id as u128), vec![LabelId(1)], vec![]); }
    if n == 3 {
        for (id, s, t) in [(1, 0, 1), (2, 1, 2)] {
            batch.add_edge(EId(id), VId(s), VId(t), vec![(PropertyKeyId(1),
...
fn cooperative_numeric_refusal_nonconvergence_and_empty_or_isolated_populations_are_exact() {
    ...
    runtime.block_on(async {
        ...
        for negative in [false, true] {
            ...
            let error = call.execute_sealed_cooperative(&cx, &graph, opt.execution_limits,
                memory(), quantum(1), yield_now).await.unwrap_err();
            if negative {
                assert!(matches!(error, FnxSealedExecutionError::Execution(FnxExecutionError::NegativeWeight)));
            } else {
                assert!(matches!(error, FnxSealedExecutionError::Execution(FnxExecutionError::NotConverged { max_iterations: 1, .. })));
            }
            let unweighted = FnxCallSpec::pagerank(PageRankOptions::new(0.85, 1000, 1e-9, false).unwrap());
            compare_execution(&unweighted.execute_sealed_cooperative(&cx, &graph, opt.execution_limits,
                memory(), quantum(1), yield_now).await.unwrap(),
                &unweighted.execute_sealed(&cx, &graph, opt.execution_limits, memory()).unwrap());
        }
        for n in [0, 512] {
            ...
            let probe = Arc::new(SimulationCheckpointProbe::new(None));
            let controlled = cx.with_checkpoint_probe(Arc::clone(&probe));
            let (result, yields) = drive(call.execute_sealed_cooperative(&controlled, &graph,
            ...
            if n != 0 {
                ...
                let (result, yields) = drive(call.execute_sealed_cooperative(&controlled, &graph,
                    FnxExecutionLimits { max_result_rows: 1, ..opt.execution_limits },
                    memory(), quantum(1), yield_now), &probe, 1);
                assert_eq!(result.unwrap().rows.len(), 1);
                assert!(yields >= n * 2, "initialization AND sparse result encoding must yield");
            }
        }
    });
}
...
fn every_cooperative_checkpoint_cancels_and_every_suspension_can_be_dropped_without_leaks() {
    ...
    runtime.block_on(async {
        ...
        for call in [FnxCallSpec::single_source_shortest_path_length(VId(0), None),
            ...
            let probe = Arc::new(SimulationCheckpointProbe::new(None));
            let controlled = cx.with_checkpoint_probe(Arc::clone(&probe));
            let (expected, yields) = drive(call.execute_sealed_cooperative(&controlled, &graph,
            ...
            for stop in 1..=total {
                ...
                let (actual, _) = drive(call.execute_sealed_cooperative(&controlled, &graph,
                    opt.execution_limits, memory(), quantum(3), yield_now), &probe, 3);
                assert!(matches!(actual, Err(FnxSealedExecutionError::Cancelled(_))), "cut={stop}");
                assert_eq!(probe.calls(), stop);
            }
            let wakes = Arc::new(Wakes::default());
            ...
            for stop in 0..=yields {
                let mut future = Box::pin(call.execute_sealed_cooperative(&cx, &graph,
...
fn live_refusal(error: FnxSealedExecutionError, expected: usize) {
    match error {
        FnxSealedExecutionError::Source(SealedProjectionError::Guard(cause)) => {
            assert_eq!(cause.downcast_ref::<LiveRefusal>(), Some(&LiveRefusal(expected)));
        }
...
fn cooperative_live_guards_span_every_checkpoint_without_changing_successful_results() {
    ...
    runtime.block_on(async {
        ...
        for n in [0, 3] {
            ...
            if n != 0 {
                calls.push(FnxCallSpec::single_source_shortest_path_length(VId(0), None));
            }
            for call in calls {
                for q in [1, 7] {
                    let mut count = 0;
                    let expected = call.execute_sealed_cooperative_with_checkpoint(
                        &cx, &graph, opt.execution_limits, memory(), quantum(q), yield_now,
                        || { count += 1; Ok(()) },
                    ).await.unwrap();
                    assert!(count > n);
                    assert_eq!(call.execute_sealed_cooperative(&cx, &graph, opt.execution_limits,
                        memory(), quantum(q), yield_now).await.unwrap(), expected);
                    // Includes initialization, source opens, raw pulls, scalar
                    // passes, before/after yields, row conversion and delivery.
                    for stop in 1..=count {
                        let mut seen = 0;
                        let actual = call.execute_sealed_cooperative_with_checkpoint(
                            &cx, &graph, opt.execution_limits, memory(), quantum(q), yield_now,
                            || {
                                seen += 1;
                                if seen == stop {
                    ...
                    let cancelled = cx.with_checkpoint_probe(Arc::new(
                        SimulationCheckpointProbe::new(Some(1)),
                    ));
                    assert!(matches!(call.execute_sealed_cooperative_with_checkpoint(
                        &cancelled, &graph, opt.execution_limits, memory(), quantum(q), yield_now,
                        || panic!("runtime cancellation must precede the additional guard"),
                    ).await, Err(FnxSealedExecutionError::Cancelled(_))));
                }
            }
        }
        assert_eq!(contexts.outstanding_obligations(), 0);
...
fn revocation_during_each_suspension_precedes_any_resumed_source_or_result_work() {
    ...
    runtime.block_on(async {
        ...
        for call in [FnxCallSpec::single_source_shortest_path_length(VId(0), None),
            ...
            let (expected, suspensions) = drive(call.execute_sealed_cooperative_with_checkpoint(
                &measured, &graph, opt.execution_limits, memory(), quantum(3), yield_now, || Ok(()),
            ), &probe, 3);
            let expected = expected.unwrap();
            ...
            assert_eq!(call.execute_sealed_cooperative_with_checkpoint(&cx, &graph,
                opt.execution_limits, memory(), quantum(7), yield_now, || Ok(())).await.unwrap(), expected);
        }
    });
}
