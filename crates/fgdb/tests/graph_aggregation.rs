...
use fgdb_gql::{GraphAggregate, GraphAggregateError, GraphAggregateRow, GraphSymbol, GraphSymbolKind,
    GqlBudgetDimension, GqlParameters, GqlQueryError, GqlQueryPolicy, PreparedGraphAggregate, PreparedGraphText};
...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(1000, 100, 1_000_000, 100_000) }
fn child() -> PreparedGraphPattern<GraphValueRow> {
    ...
    b.prepare_values(&[GraphColumn::vertex("owner", "a"), GraphColumn::vertex("via", "b"),
        GraphColumn::property("amount", "c", P)], 0, None).unwrap().with_duplicates()
}
fn aggregate(grouped: bool) -> PreparedGraphAggregate {
    PreparedGraphAggregate::prepare(child(), if grouped { &[0] } else { &[] }, &[
        GraphAggregate::count_rows("paths"), GraphAggregate::count("present", 2),
        GraphAggregate::count_distinct("unique", 2), GraphAggregate::sum_int("sum", 2),
        GraphAggregate::min("min", 2), GraphAggregate::max("max", 2),
    ], 0, None).unwrap()
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    let mut first = WriteBatch::new(R);
    for id in 1..=6 {
        first.create_vertex(VId(id), vec![], if id == 3 { vec![(P, CanonicalScalar::Int(7))] } else { vec![] });
    }
...
fn plain(rows: &[GraphAggregateRow]) -> Vec<Summary> {
    rows.iter().map(|row| {
        let extreme = |at| match row.get(at).unwrap().as_value().and_then(GraphValue::as_scalar) {
        ...
        (row.keys()[0].as_vertex().unwrap(), row.get(0).unwrap().as_count().unwrap(),
            row.get(1).unwrap().as_count().unwrap(), row.get(2).unwrap().as_count().unwrap(),
            row.get(3).unwrap().as_integer(), extreme(4), extreme(5))
    }).collect()
}
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<Summary> {
    ...
    for first in edges.iter().filter(|edge| edge.entry.relation == R) {
        for second in edges.iter().filter(|edge| edge.entry.relation == S) {
            if first.entry.dst != second.entry.src { continue; }
            let row = vertices.iter().find(|row| row.vid == second.entry.dst).unwrap();
            let value = row.props.iter().find(|(key, _)| *key == P).map(|(_, value)| value);
...
fn all_five_surfaces_share_the_summary_and_one_combined_allowance() {
    let ((), report) = run_async_under_lab(0xa660_0001, |root| async move {
        ...
        assert_eq!(expected, vec![(VId(1), 6, 4, 1, Some(28), Some(7), Some(7)),
            (VId(4), 3, 2, 1, Some(14), Some(7), Some(7))]);
        for rows in [
            db.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value,
            db.execute_graph_aggregate_governed_at(&cx, &aggregate, basis, policy()).unwrap().value,
            pinned.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value,
            pinned.execute_graph_aggregate_governed_at(&cx, &aggregate, basis, policy()).unwrap().value,
            txn.execute_graph_aggregate_governed(&db, &cx, &aggregate, policy()).unwrap().value,
        ] { assert_eq!(plain(&rows), expected); }
        let full = db.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap();
        assert_eq!(full.rows.snapshot_records, 6);
        assert_eq!(full.rows.result_rows, 2);
        let exact = GqlQueryPolicy::new(6, 2, full.evaluator.work_units, full.evaluator.scratch_entries);
        assert_eq!(db.execute_graph_aggregate_governed(&cx, &aggregate, exact).unwrap(), full);
        for cap in [GqlQueryPolicy::new(6, 2, full.evaluator.work_units - 1, u64::MAX),
            GqlQueryPolicy::new(6, 2, u64::MAX, full.evaluator.scratch_entries - 1)] {
            assert!(matches!(db.execute_graph_aggregate_governed(&cx, &aggregate, cap),
...
fn canonical_staging_and_durable_history_keep_exact_counts_and_values() {
    let ((), report) = run_async_under_lab(0xa660_0002, |root| async move {
        ...
        let pinned = db.read_session().unwrap();
        let aggregate = aggregate(true);
        let before = db.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value;
        ...
        let staged = txn.execute_graph_aggregate_governed(&db, &cx, &aggregate, policy()).unwrap();
        ...
        assert_eq!(txn.execute_graph_aggregate_governed(&db, &cx, &aggregate, exact).unwrap(), staged);
        assert_eq!(db.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value, before);
        txn.commit(&mut db, &commit).await.unwrap();
        assert_eq!(plain(&db.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value), expected);
        assert_eq!(db.execute_graph_aggregate_governed_at(&cx, &aggregate, basis, policy()).unwrap().value, before);
        db.compact(&commit).await.unwrap();
        drop(db);
        let reopened = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        assert_eq!(plain(&reopened.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value), expected);
        assert_eq!(reopened.execute_graph_aggregate_governed_at(&cx, &aggregate, basis, policy()).unwrap().value, before);
        assert_eq!(pinned.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value, before);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn zero_count_and_refused_group_output_preserve_read_dependencies() {
    let ((), report) = run_async_under_lab(0xa660_0003, |root| async move {
        ...
        for empty in [false, true] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            if !empty { seed(&mut db, &commit).await; }
            let mut txn = db.begin(&txn_cx).unwrap();
            let mut stage = WriteBatch::new(R); stage.create_vertex(VId(99), vec![], vec![]);
            txn.write(&mut db, stage).unwrap();
            let aggregate = aggregate(false);
            if empty {
                let zero = txn.execute_graph_aggregate_governed(&db, &cx, &aggregate, policy()).unwrap();
                assert_eq!(zero.value.len(), 1);
                assert_eq!(zero.value[0].get(0).unwrap().as_count(), Some(0));
                seed(&mut db, &commit).await;
            } else {
                assert!(matches!(txn.execute_graph_aggregate_governed(&db, &cx, &aggregate,
                    GqlQueryPolicy::new(100, 0, 1_000_000, 100_000)), Err(GqlQueryError::Rows(_))));
...
fn data_owner_snapshot_and_cancellation_errors_keep_their_domains() {
    let ((), report) = run_async_under_lab(0xa660_0004, |root| async move {
        ...
        assert!(matches!(db.execute_graph_aggregate_governed(&cx, &aggregate, policy()),
        ...
        assert!(matches!(txn.execute_graph_aggregate_governed(&foreign, &cx, &aggregate, policy()),
        ...
        assert!(matches!(db.execute_graph_aggregate_governed_at(&cx, &aggregate, future, policy()),
            Err(GqlQueryError::Source(GraphAggregateError::Source(GqlError::Read(ReadError::BeyondFrontier { .. }))))));
        assert!(matches!(db.execute_graph_aggregate_governed(&cx, &aggregate, policy()), Err(GqlQueryError::Interrupted(_))));
        txn.abort();
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn parameterized_connected_text_feeds_aggregation_without_materializing_matches() {
    let ((), report) = run_async_under_lab(0xa660_0005, |root| async move {
        ...
        for (minimum, expected_count, expected_sum) in [(0, 6, Some(42)), (10, 0, None)] {
            let input = template.bind_parameters(&GqlParameters::new().with_int64("min", minimum).unwrap()).unwrap();
            let aggregate = PreparedGraphAggregate::prepare(input, &[], &[
                GraphAggregate::count_rows("paths"), GraphAggregate::sum_int("total", 1),
            ], 0, None).unwrap();
            let result = db.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap();
            assert_eq!(result.value.len(), 1);
            assert_eq!(result.value[0].get(0).unwrap().as_count(), Some(expected_count));
