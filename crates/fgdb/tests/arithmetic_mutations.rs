...
use fgdb_gql::{
    GqlParameters, GqlQueryError, GqlQueryPolicy, GraphIntegerErrorKind, GraphMutationError,
    GraphMutationPolicy, GraphSymbol, GraphSymbolKind, PreparedGraphMutation,
...
fn frozen_arithmetic_and_walk_counters_publish_once_and_preserve_pinned_history() {
    let ((), report) = run_async_under_lab(0xa817_0001, |root| async move {
        ...
        let result = txn
            .execute_graph_mutation_governed(&mut db, &query, &reciprocal, policy())
            .unwrap();
        assert_eq!((result.selection.result_rows, result.effects), (3, 2));
        let rows = txn.vertices(&db).unwrap();
        assert_eq!(
            value(rows.iter().find(|row| row.vid == VId(1)).unwrap(), P),
            Some(&CanonicalScalar::Int(21))
        );
        assert_eq!(
            value(rows.iter().find(|row| row.vid == VId(2)).unwrap(), P),
            Some(&CanonicalScalar::Int(13))
        );
        txn.execute_graph_mutation_governed(
            &mut db,
            &query,
            &mutation("MATCH (n) SET n.q=COALESCE(n.q,0)+1,n.p=COALESCE(n.p,0)+2"),
            policy(),
        )
        .unwrap();
        let counter = mutation("MATCH WALK (a)-[:R*0..2]->(b) SET b.q=COALESCE(b.q,0)+1");
        let result = txn
            .execute_graph_mutation_governed(&mut db, &query, &counter, policy())
            .unwrap();
    ...
    let ((), report) = run_async_under_lab(0xa817_0002, |root| async move {
        ...
        for (expression, bad, expected_error) in [
            (
                ...
                Some(GraphIntegerErrorKind::Overflow),
            ),
            (
                "10/n.p",
                CanonicalScalar::Int(0),
                Some(GraphIntegerErrorKind::DivisionByZero),
            ),
            (
                "n.p*2",
                CanonicalScalar::Bool(true),
                Some(GraphIntegerErrorKind::NonInteger),
            ),
            (
                "-n.p",
            ...
            for row in db.vertices().unwrap() {
                assert!(value(&row, Q).is_none(), "{expression}");
            }
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
    ...
    let ((), report) = run_async_under_lab(0xa817_0003, |root| async move {
        ...
        for mode in 0..4 {
            for insertion in [false, true] {
                ...
                let mut budget = policy();
                if mode == 2 {
                    budget.max_effects = 0;
                }
                let result =
                    txn.execute_graph_mutation_governed(&mut db, &query, &mutation(text), budget);
                match mode {
...
fn all_five_exact_limits_cover_expression_execution_before_workspace_publication() {
    let ((), report) = run_async_under_lab(0xa817_0004, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let measured = txn
            .execute_graph_mutation_governed(&mut db, &query, &plan, policy())
            .unwrap();
        txn.abort();
        ...
        let mut txn = db.begin(&txcx).unwrap();
        assert_eq!(
            txn.execute_graph_mutation_governed(&mut db, &query, &plan, exact)
                .unwrap(),
            measured
        );
        txn.abort();
        for budget in [
            ...
            let mut txn = db.begin(&txcx).unwrap();
            assert!(
                txn.execute_graph_mutation_governed(&mut db, &query, &plan, budget)
                    .is_err()
            );
