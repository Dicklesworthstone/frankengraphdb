...
fn pattern(floor: i64, limit: Option<u64>) -> PreparedGraphPattern<GraphValueRow> {
    ...
    PreparedGraphText::prepare(
        &format!("{HEAD} RETURN a,b,b.n AS score ORDER BY a,b{tail}"),
        symbols,
    )
    .unwrap()
...
fn grouped(floor: i64) -> PreparedGraphAggregate {
    PreparedGraphAggregateText::prepare(
        &format!(
            "{HEAD} RETURN a,COUNT(*) AS walks,COUNT(DISTINCT b) AS reached,SUM(b.n) AS total \
        ...
        for depth in 0..=3 {
            for (&target, &count) in &layer {
                ...
                if let Some(value) = integer(row).filter(|value| *value >= floor) {
                    for _ in 0..count {
                        output.push((source.vid, target, value));
                    }
                }
            }
            if depth == 3 {
            ...
            for (at, count) in layer {
                for edge in edges
                    .iter()
                    .filter(|edge| edge.entry.relation == R && edge.entry.src == at)
                {
...
fn oracle_summary(rows: &[Plain]) -> Vec<Summary> {
    ...
    groups
        .into_iter()
        .map(|(source, (count, reached, sum))| (source, count, reached.len() as u64, sum))
        .collect()
}
...
fn walk_rows_and_aggregates_follow_staged_effects_but_pinned_history_survives_reopen() {
    let ((), report) = run_async_under_lab(0x7a1c_0001, |root| async move {
        ...
        assert_eq!(
            ...
            vec![(VId(1), 5, 4, 117), (VId(5), 1, 1, 50), (VId(9), 5, 5, 197)]
        );
        assert_eq!(
            plain(
                &txn.execute_graph_pattern_governed(&db, &cx, &query, policy())
...
fn mixed_fixed_optional_walks_admit_intermediate_properties_and_zero_hop_isolates() {
    let ((), report) = run_async_under_lab(0x7a1c_0002, |root| async move {
        ...
        let query = PreparedGraphText::prepare(text, symbols)
            .unwrap()
            .bind_parameters(&arguments(25))
            .unwrap();
        assert!(
            !query.plan().scans_edges(),
            "both base tables must be admitted for the WALK child"
        );
        let rows = db
            .execute_graph_pattern_governed(&cx, &query, policy())
            .unwrap();
        ...
        assert_eq!(
            optional(&rows.value),
            vec![
                (VId(5), VId(1), Some(VId(3)), Some(30)),
                (VId(5), VId(1), Some(VId(3)), Some(30)),
                (VId(5), VId(1), Some(VId(4)), Some(40)),
                (VId(5), VId(1), Some(VId(4)), Some(40)),
        ...
        assert_eq!(
            optional(
                &db.execute_graph_pattern_governed(&cx, &zero, policy())
                    .unwrap()
                    .value
            ),
            vec![
                (VId(5), VId(1), None, None),
                (VId(5), VId(9), Some(VId(9)), Some(90)),
            ]
        );
        let absent = PreparedGraphText::prepare(
            "MATCH (a:Root) WHERE NOT EXISTS { MATCH WALK (a)-[:R*1..2]->(b) \
             WHERE b.n >= $floor } RETURN a",
            symbols,
        )
        .unwrap()
        .bind_parameters(&arguments(25))
    ...
    let ((), report) = run_async_under_lab(0x7a1c_0003, |root| async move {
        ...
        for mode in 0..3 {
            for mutation in 0..4 {
                ...
                let output = txn.execute_graph_pattern_governed(
                    &db,
                    &cx,
                    &query,
