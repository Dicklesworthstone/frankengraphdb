...
use fgdb::{
    Database, DatabaseKeys, RelationBind, WriteBatch, WriteError, WriteMismatchPolicy,
    WriteTxnError,
};
...
fn vertices(ensure: bool) -> WriteBatch {
    ...
    for (vid, value) in [(VId(1), 7), (VId(2), 8), (HIGH, 9), (VId(4), 10)] {
        ...
        if ensure {
            batch.ensure_vertex(vid, vec![L], props);
        } else {
            batch.create_vertex(vid, vec![L], props);
        }
    }
    batch
}
...
fn shared_creations_publish_once_before_dependent_coordinates_with_unique_births() {
    let ((), report) = run_async_under_lab(0xa704_0001, |root| async move {
        ...
        assert!(
            db.execute_prepared_query_at(&query(), CommitSeq(0))
                .unwrap()
                .is_empty()
        );
        ...
        assert_eq!(
            coordinates.iter().map(|c| c.relation).collect::<Vec<_>>(),
            vec![R, S]
        );
            ...
            for row in &coordinate.rows {
                match row {
                    DeltaRow::CreateVertex {
                        ...
                        assert_eq!(
                            coordinate.relation, R,
                            "the earliest coordinate owns endpoints"
                        );
                        assert!(
                            born.insert(*birth_ordinal, ElementId::Vertex(*vid))
                                .is_none()
                        );
                    }
                    DeltaRow::CreateEdge {
        ...
        assert_eq!(
            born.keys().copied().collect::<Vec<_>>(),
            vec![1, 2, 3, 4, 5, 7]
        );
...
fn ensure_prefixes_are_idempotent_and_preserve_existing_vertex_contents() {
    let ((), report) = run_async_under_lab(0xa704_0002, |root| async move {
        ...
        assert_eq!(
            fresh.vertex(VId(1)).unwrap().unwrap().props,
            vec![(P, CanonicalScalar::Int(7))]
        );
...
fn changing_or_erasing_a_shared_prefix_and_conflicting_suffixes_refuse_atomically() {
    let ((), report) = run_async_under_lab(0xa704_0003, |root| async move {
        ...
        for mode in 0..5 {
            ...
            match mode {
                0 => {
                    groups[1].set_vertex_property(VId(4), P, Some(CanonicalScalar::Int(99)));
                }
                1 => {
                    groups[1].set_vertex_label(VId(4), L, false);
                }
                // VId(4) is unused by the path. Its erased creation must still
                // be caught by complete-prefix verification, not endpoint tests.
                2 => {
                    groups[1].delete_vertex(VId(4));
                }
                3 => {
                    groups[1].compare_and_set_vertex_property(
                        VId(4),
                        P,
                ...
                _ => {
                    groups[1].add_edge(EId(10), VId(2), HIGH, vec![]);
                }
            }
            assert!(
                matches!(
                    db.write_atomic(&cx, groups).await,
...
fn transaction_prefix_reuses_existing_staging_and_preserves_it_after_rejected_edits() {
    let ((), report) = run_async_under_lab(0xa704_0006, |root| async move {
        ...
        assert_eq!(
            txn.execute_prepared_query_budgeted(&db, &query, GqlExecutionBudget::new(2, 1))
                .unwrap()
                .value,
            vec![HIGH]
        );
        assert_eq!(
            txn.execute_prepared_query_governed(
                ...
                &query_cx,
                &query,
                GqlQueryPolicy::new(2, 1, 100_000, 100_000)
            )
            .unwrap()
            .value,
            vec![HIGH]
        ...
        assert!(matches!(
            txn.write_atomic(&mut foreign, suffixes(false)),
        ...
        txn.audit_prepared_query_overlay_artifact(&db, &query, &artifact.to_bytes())
            .unwrap();
        ...
        assert_eq!(
            txn.edge(&db, EId(30)).unwrap().unwrap().entry.relation,
            RelationId(3)
        );
        assert!(matches!(
            txn.audit_prepared_query_overlay_artifact(&db, &query, &artifact.to_bytes()),
    ...
    let ((), report) = run_async_under_lab(0xa704_0007, |root| async move {
        ...
        for (name, point, tear, committed) in [
            ...
            let path = std::env::temp_dir().join(format!(
                "fgdb-shared-prefix-recovery-{}-{name}",
                std::process::id()
            ));
            ...
            assert_eq!(
                reopened.vertices().unwrap().len(),
                if committed { 4 } else { 0 },
                "{name}"
            );
            assert_eq!(
                reopened.edges().unwrap().len(),
                if committed { 2 } else { 0 },
                "{name}"
            );
            assert_eq!(
                reopened.delta_since(CommitSeq(0)).unwrap().count(),
                usize::from(committed)
            );
            assert_eq!(
                reopened.execute_prepared_query(&query()).unwrap(),
...
fn shared_initialization_versions_and_history_survive_updates_compaction_and_rebuild() {
    let ((), report) = run_async_under_lab(0xa704_0008, |root| async move {
        ...
        let cx = contexts.commit();
        let path =
            std::env::temp_dir().join(format!("fgdb-shared-prefix-compact-{}", std::process::id()));
        let mut db = Database::create(&cx, &path, keys()).await.unwrap();
        let created = db.write_atomic(&cx, graph(false)).await.unwrap();
        let pinned = db.read_session().unwrap();
        let original_births: Vec<_> = db
            .vertices()
            .unwrap()
            .into_iter()
            .map(|row| (row.vid, row.birth_ordinal))
            .collect();
        let mut r = WriteBatch::new(R);
        ...
        assert_eq!(
            db.execute_prepared_query_at(&query(), created).unwrap(),
            vec![HIGH]
        );
        let history: Vec<_> = (0..=retired.0)
            .map(|seq| {
        ...
        for rebuild in [false, true] {
            let reopened = if rebuild {
                ...
                Database::open(&cx, &path, keys()).await.unwrap()
            };
            ...
            for (seq, (vertices, edges)) in history.iter().enumerate() {
                assert_eq!(
                    reopened.vertices_at(CommitSeq(seq as u64)).unwrap(),
                    *vertices
                );
            ...
            assert_eq!(
                reopened
                    .execute_prepared_query_at(&query(), created)
                    .unwrap(),
                vec![HIGH]
            );
        }
