...
fn issuer(namespace: DatabaseSecurityNamespaceId) -> Authority {
    Authority::new(AuthKey::from_seed(9701), namespace, "graph", SchemaEpoch(1), 1).unwrap()
}
fn grant() -> Grant {
    Grant {
        branch: BRANCH.into(),
...
fn scratch(name: &str) -> PathBuf {
    std::env::temp_dir().join(format!("fgdb-authorized-writes-{}-{name}", std::process::id()))
}
...
fn write_only_vertex_batch_commits_once_and_reopens() {
    under_lab(0xa901, |contexts| async move {
        ...
        assert_eq!(reopened.vertex(VId(1)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(7))]);
...
fn forbidden_noop_tail_discards_the_entire_allowed_prefix() {
    under_lab(0xa902, |contexts| async move {
        let cx = contexts.commit();
        let txn = contexts.txn();
        let mut db = Database::create(&cx, &scratch("denied-tail"), keys()).await.unwrap();
        ...
        assert_eq!(db.vertex(VId(1)).unwrap().unwrap().props, vec![(SECRET, CanonicalScalar::Int(99))]);
        ...
        db.write_authorized(&txn, &cx, &authority, &token, BRANCH, retry, || NOW).await.unwrap();
...
fn permitted_updates_preserve_hidden_fields_and_hide_missing_targets() {
    under_lab(0xa903, |contexts| async move {
        let cx = contexts.commit();
        let txn = contexts.txn();
        let mut db = Database::create(&cx, &scratch("masked-update"), keys()).await.unwrap();
        let mut seed = WriteBatch::new(R);
        seed.create_vertex(VId(1), vec![L], vec![(P, CanonicalScalar::Int(1)), (SECRET, CanonicalScalar::Int(99))]);
        ...
        db.write_authorized(&txn, &cx, &authority, &token, BRANCH, update, || NOW).await.unwrap();
...
fn admission_and_shared_budget_refuse_without_publishing_or_leaking_pins() {
    under_lab(0xa904, |contexts| async move {
        let cx = contexts.commit();
        let txn = contexts.txn();
        let mut db = Database::create(&cx, &scratch("admission"), keys()).await.unwrap();
        let frontier = db.frontier().unwrap();
        let baseline = txn.outstanding_obligations();
        let authority = issuer(NAMESPACE);
        let mut batch = WriteBatch::new(R);
        ...
        assert!(matches!(
            db.write_authorized(&txn, &cx, &authority, &token, BRANCH, batch.clone(), || NOW).await,
        ...
        assert!(matches!(
            db.write_authorized(&txn, &cx, &foreign, &token, BRANCH, batch, || NOW).await,
