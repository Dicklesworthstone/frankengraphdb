...
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryPolicy, GraphDeleteError,
    ...
    GraphWriteScriptExecutionError, PreparedGraphWriteScript};
...
fn policy() -> GraphWriteProgramPolicy {
    GraphWriteProgramPolicy::new(GqlQueryPolicy::new(50_000, 50_000, 5_000_000, 5_000_000), 3, 3, 0)
}
...
fn repeated_delete_create_records_replace_in_one_commit_and_survive_reopen() {
    let ((), report) = run_async_under_lab(0xd31e_3001, |root| async move {
        ...
        let batch = script("MATCH (n) WHERE n.p=$key DELETE n;CREATE (n {p:$key})")
            .bind_parameter_sets(&[values.clone(), values.clone(), values]).unwrap();
        let before = db.frontier().unwrap();
        let (receipt, _) = db.execute_graph_write_program_returning_autocommit_governed(
            &txcx, &query, &commit, batch.program(), policy(), |request| {
        ...
        assert_eq!((receipt.stats().mutation_effects, receipt.stats().created_vertices), (3, 3));
        ...
        assert_eq!(db.vertices().unwrap().len(), 1);
        assert_eq!(db.vertex(VId(12)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(7))]);
...
fn native_plain_delete_refuses_attached_vertex_but_explicit_detach_succeeds() {
    let ((), report) = run_async_under_lab(0xd31e_3002, |root| async move {
        ...
        let result = db.execute_graph_write_script_autocommit_governed(
            &txcx, &query, &commit, &script("CREATE (x {p:9});MATCH (n) WHERE n.p=1 DELETE n"),
            &GqlParameters::new(), policy(), |_| Ok::<_, ()>(ElementId::Vertex(VId(9))),
        ).await;
        assert!(matches!(result, Err(GraphWriteScriptExecutionError::Program(
            GraphWriteProgramError::Delete { statement: 1,
        ...
        let (receipt, _) = db.execute_graph_write_script_autocommit_governed(
            &txcx, &query, &commit, &script("CREATE (x {p:9});MATCH (n) WHERE n.p=1 DETACH DELETE n"),
            &GqlParameters::new(), policy(), |_| Ok::<_, ()>(ElementId::Vertex(VId(19))),
        ).await.unwrap();
