...
#[test]
fn destination_property_match_sees_the_staged_overlay_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x42_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-where-dst-prop-overlay-{}",
            std::process::id()
        ));
        ...
        seed.create_vertex(VId(2), vec![], vec![(key, CanonicalScalar::Int(1))]);
        seed.create_vertex(VId(3), vec![], vec![]);
        seed.create_vertex(VId(4), vec![], vec![(key, CanonicalScalar::Int(9))]);
        ...
        staged.create_vertex(VId(8), vec![], vec![(key, CanonicalScalar::Int(1))]);
        staged.add_edge(EId(12), VId(7), VId(8), vec![]);
        txn.write(&mut db, staged).expect("overlay stages");

        let bind = RelationBind::new()
            .with_relation("R", relation)
            .with_property("k", key);
        let filtered = "MATCH (a)-[:R]->(b) WHERE b.k = 1 RETURN a";
        assert_eq!(
            txn.execute_gql(&db, filtered, &bind)
                .expect("overlay filtered MATCH executes"),
            vec![VId(1), VId(7)]
        );
        assert_eq!(
            db.execute_gql(filtered, &bind)
                .expect("base filtered MATCH executes"),
            vec![VId(1)]
        );

        let unfiltered = "MATCH (a)-[:R]->(b) RETURN a";
        assert_eq!(
            txn.execute_gql(&db, unfiltered, &bind)
                .expect("unfiltered overlay MATCH executes"),
            vec![VId(1), VId(3), VId(7)]
        );
        txn.abort();
    });
