...
use fgdb::{
    ...
    WriteTxn, WriteTxnError,
};
use fgdb_delta_types::{LabelId, PropertyKeyId, RelationId};
use fgdb_gql::{
    GlaExecutionLimits, GlaLimitDimension, GqlBudgetDimension, GqlEvidenceLimits,
    GqlExecutionBudget, GqlQueryError, GqlQueryPolicy, PreparedGqlQuery,
};
use fgdb_types::{
...
fn oracle(txn: &WriteTxn, db: &Database<MemVfs>) -> Vec<(Vec<VId>, u64)> {
    ...
    let qualifies = |vid: VId| {
        vertices.get(&vid).is_some_and(|row| {
            row.props.iter().any(|(key, scalar)| {
    ...
    let nodes = vertices
        .values()
        .filter(|row| row.labels.contains(&L) && qualifies(row.vid))
        .map(|row| row.vid)
        .collect();
    let destinations: BTreeSet<_> = edges
        .iter()
        .filter(|row| row.entry.relation == R && qualifies(row.entry.dst))
        .map(|row| row.entry.dst)
        .collect();
    ...
    for a in edges.iter().filter(|row| row.entry.relation == R) {
        for b in &edges {
            if a.entry.dst == b.entry.src {
                if b.entry.relation == R {
                    two.insert(b.entry.dst);
                }
                if b.entry.relation == S && a.entry.src == b.entry.dst {
                    closed.insert(a.entry.src);
                }
            }
        }
    }
    let loops: BTreeSet<_> = edges
        .iter()
        .filter(|row| row.entry.relation == R && row.entry.src == row.entry.dst)
        .map(|row| row.entry.src)
        .collect();
    let ends: BTreeSet<_> = edges
        .iter()
        .filter(|row| row.entry.relation == R)
        .flat_map(|row| [row.entry.src, row.entry.dst])
        .collect();
    let page = destinations.iter().copied().skip(1).take(2).collect();
    let count = edges.len() as u64;
    vec![
        (nodes, vertices.len() as u64),
        (destinations.into_iter().collect(), count),
        (two.into_iter().collect(), count),
        (closed.into_iter().collect(), count),
        (loops.into_iter().collect(), count),
        (ends.into_iter().collect(), count),
        (page, count),
    ]
}
    ...
    for (query, (rows, records)) in queries().iter().zip(&expected) {
        ...
        let budgeted = txn
            .execute_prepared_query_budgeted(
                db,
                query,
                GqlExecutionBudget::new(*records, rows.len() as u64),
            )
            .unwrap();
        ...
        assert_eq!(
            txn.execute_prepared_query_limited(
                db,
                query,
                GlaExecutionLimits::new(1_000_000, 1_000_000)
            )
            .unwrap()
            .value,
            *rows
        ...
        let run = txn
            .execute_prepared_query_governed(db, cx, query, generous())
            .unwrap();
        ...
        assert_eq!(
            txn.audit_prepared_query_overlay_artifact_governed(
                db,
                cx,
                query,
                &artifact.to_bytes(),
...
fn canonical_overlays_agree_with_independent_rows_then_with_committed_history() {
    let ((), report) = run_async_under_lab(0xc0a3_0001, |root| async move {
        ...
        stage.ensure_edge_by_triple(
            EId(99),
            VId(1),
            VId(2),
            vec![(N, CanonicalScalar::Int(111))],
        );
        ...
        stage.compare_and_set_vertex_property(
            VId(3),
            N,
        ...
        for ((query, (rows, _)), (old, _)) in queries().iter().zip(&after).zip(&before) {
            assert_eq!(
                db.execute_prepared_query_governed(&cx, query, generous())
                    .unwrap()
                    .value,
                *rows
            );
...
fn every_work_and_scratch_prefix_uses_the_original_shared_allowance() {
    let ((), report) = run_async_under_lab(0xc0a3_0002, |root| async move {
        ...
        assert_eq!(
            txn.execute_prepared_query_governed(&db, &cx, &query, exact)
                .unwrap(),
            measured
        );
        for limit in 0..measured.evaluator.work_units {
            ...
            assert!(
                matches!(txn.execute_prepared_query_governed(&db, &cx, &query, policy),
        ...
        assert!(
            matches!(txn.execute_prepared_query_governed(&db, &cx, &query,
...
fn deleted_basis_rows_do_not_consume_final_overlay_record_budget() {
    let ((), report) = run_async_under_lab(0xc0a3_0003, |root| async move {
        ...
        let mut txn = db.begin(&txn_cx).unwrap();
        let mut stage = WriteBatch::new(R);
        for vid in [VId(1), VId(2), VId(3), VId(4), VId(5), HIGH] {
            stage.delete_vertex(vid);
        }
        txn.write(&mut db, stage).unwrap();
        for query in queries() {
            let run = txn
                .execute_prepared_query_governed(
                    &db,
                    &cx,
                    &query,
            ...
            assert!(
                txn.execute_prepared_query_budgeted(&db, &query, GqlExecutionBudget::new(0, 0))
                    .unwrap()
                    .value
                    .is_empty()
            );
...
fn staged_payload_size_does_not_turn_borrowed_query_admission_into_scalar_copying() {
    let ((), report) = run_async_under_lab(0xc0a3_0004, |root| async move {
        ...
        for bytes in [1, 8_000] {
            ...
            let mut stage = WriteBatch::new(R);
            stage.set_vertex_property(VId(2), N, Some(CanonicalScalar::Int(10)));
            stage.set_vertex_property(
                VId(2),
                PAYLOAD,
                Some(CanonicalScalar::bytes(vec![0x41; bytes]).unwrap()),
            );
            stage.set_edge_property(
                EId(10),
                PAYLOAD,
    ...
    let ((), report) = run_async_under_lab(0xc0a3_0005, |root| async move {
        ...
        for result_limit in [false, true] {
            for winner_kind in 0..4 {
                ...
                let policy = if result_limit {
                    GqlQueryPolicy::new(100, 0, 1_000_000, 1_000_000)
                } else {
                    GqlQueryPolicy::new(0, 100, 1_000_000, 1_000_000)
                };
                assert!(matches!(
                    txn.execute_prepared_query_governed(&db, &cx, &query, policy),
                    Err(GqlQueryError::Rows(_))
                ));
                let mut winner = WriteBatch::new(R);
                match winner_kind {
                    0 => {
                        winner.set_vertex_property(VId(2), N, Some(CanonicalScalar::Int(20)));
                    }
                    1 => {
...
fn multi_relation_source_keeps_coordinates_and_failed_staging_leaves_it_unchanged() {
    let ((), report) = run_async_under_lab(0xc0a3_0006, |root| async move {
        ...
        assert_eq!(
            txn.execute_prepared_query_governed(&db, &cx, &query, generous())
                .unwrap(),
            before
        );
        txn.commit(&mut db, &commit).await.unwrap();
        assert_eq!(
            db.execute_prepared_query(&query).unwrap(),
            vec![VId(1), VId(4)]
        );
...
fn interleaved_live_history_does_not_rebase_the_borrowed_overlay() {
    let ((), report) = run_async_under_lab(0xc0a3_0007, |root| async move {
        ...
        assert_eq!(
            txn.execute_prepared_query_budgeted(&db, &query, GqlExecutionBudget::new(8, 3))
                .unwrap()
                .value,
            expected
        );
