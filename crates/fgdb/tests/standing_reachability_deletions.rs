...
fn tail_edge_deletion_uses_sparse_maintenance_and_reinsertions_keep_both_indexes_sound() {
    let ((), report) = run_async_under_lab(0xdec1_7101, |root| async move {
        ...
        for n in [64_u128, 128] {
            ...
            assert!(
                u128::from(update_work) < 64 * n + 512,
                "work={update_work}, n={n}"
            );
            assert!(
                u128::from(update_scratch) < 32 * n + 512,
                "scratch={update_scratch}, n={n}"
            );
            let expected: BTreeSet<_> = (0..n)
                .flat_map(|source| (source + 1..n).map(move |target| (VId(source), VId(target))))
                .collect();
            ...
            assert!(
                db.standing_reachability(&cx, &rebuilt)
                    .unwrap()
                    .last_maintenance()
                    .work_units
                    > update_work
    ...
    let ((), report) = run_async_under_lab(0xdec1_7102, |root| async move {
        ...
        for eid in [1, 2, 5, 4] {
            ...
            if eid <= 2 {
                assert_eq!(
                    expected, baseline,
                    "parallel or alternate path still supplies support"
                );
            } else {
...
fn a_refused_rebuild_preserves_the_accepted_decremental_generation_and_its_policy() {
    let ((), report) = run_async_under_lab(0xdec1_7103, |root| async move {
        ...
        let before_stats = *db
            .standing_reachability(&cx, &healthy)
            .unwrap()
            .last_maintenance();
        assert!(matches!(
            db.rebuild_standing_query(&cx, &healthy, GqlQueryPolicy::new(100_000, 100_000, 0, 0)),
        ...
        assert_eq!(
            db.standing_reachability(&cx, &healthy)
                .unwrap()
                .last_maintenance(),
            &before_stats
        );
