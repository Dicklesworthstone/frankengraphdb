use asupersync::lab::run_async_under_lab;
use fgdb::{Database, DatabaseKeys, RelationBind, WriteBatch};
use fgdb_delta_types::{PropertyKeyId, RelationId};
use fgdb_types::context::PurposeContexts;
use fgdb_types::ids::DatabaseSecurityNamespaceId;
use fgdb_types::{CanonicalScalar, EId, VId};

#[test]
fn destination_property_less_than_sees_staged_dest_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x63_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-where-dst-prop-lt-overlay-{}",
            std::process::id()
        ));
        let relation = RelationId(1);
        let key = PropertyKeyId(7);
        let mut db = Database::create(
            &commit,
            &dir,
            DatabaseKeys::new(
                [0x5a; 32],
                DatabaseSecurityNamespaceId([0x77; 32]),
                [0x3c; 32],
            ),
        )
        .await
        .expect("database creates");

        let mut seed = WriteBatch::new(relation);
        seed.create_vertex(VId(1), vec![], vec![]);
        seed.create_vertex(VId(2), vec![], vec![(key, CanonicalScalar::Int(1))]);
        seed.add_edge(EId(10), VId(1), VId(2), vec![]);
        db.write(&commit, seed).await.expect("fixture commits");

        let mut txn = db.begin(&txn_cx).expect("transaction begins");
        let mut staged = WriteBatch::new(relation);
        staged.create_vertex(VId(5), vec![], vec![]);
        staged.create_vertex(VId(6), vec![], vec![(key, CanonicalScalar::Int(0))]);
        staged.add_edge(EId(11), VId(5), VId(6), vec![]);
        txn.write(&mut db, staged).expect("overlay stages");

        let bind = RelationBind::new()
            .with_relation("R", relation)
            .with_property("k", key);
        let unfiltered = "MATCH (a)-[:R]->(b) RETURN a";
        assert_eq!(
            txn.execute_gql(&db, unfiltered, &bind)
                .expect("unfiltered overlay MATCH executes"),
            vec![VId(1), VId(5)]
        );
        assert_eq!(
            db.execute_gql(unfiltered, &bind)
                .expect("unfiltered base MATCH executes"),
            vec![VId(1)]
        );

        let filtered = "MATCH (a)-[:R]->(b) WHERE b.k < 1 RETURN a";
        assert_eq!(
            txn.execute_gql(&db, filtered, &bind)
                .expect("overlay less-than MATCH executes"),
            vec![VId(5)]
        );
        assert_eq!(
            db.execute_gql(filtered, &bind)
                .expect("base less-than MATCH executes"),
            Vec::<VId>::new()
        );
        txn.abort();
    });
    assert!(report.lab_test_passed(), "lab run failed: {report:?}");
}
