...
use fgdb_beacon::{
    BeaconError, EditDistance, ExactHybridQuery, ExactRrfProfile, GraphHybridQuery, TextMatch,
    VectorSearch,
};
...
use fgdb_warden::{
    Authority, Error as Denial, Grant, LimitDimension, QueryLimits, Restriction, Scope,
};
...
async fn database(cx: &CommitCx, hidden: bool) -> Database<MemVfs> {
    ...
    if hidden {
        ...
        batch.create_vertex(
            VId(101),
            vec![HIDDEN],
            vec![(TEXT, CanonicalScalar::Int(17))],
        );
...
fn fuzzy_reads_follow_transaction_changes_history_and_resident_refresh() {
    let ((), report) = run_async_under_lab(0xbeac_4101, |root| async move {
        ...
        assert_eq!(
            db.beacon_search(&query, &options, request()).unwrap(),
            before
        );
        ...
        assert_eq!(
            resident
                .search(&query, request(), ReadPolicy::default())
                .unwrap(),
            staged
        );
        ...
        assert_eq!(
            db.beacon_search(&query, &historical, request()).unwrap(),
            before
        );
        drop(db);
        assert_eq!(
            view.beacon_search(&query, &options, request()).unwrap(),
            before
        );
        assert_eq!(
            old.search(&query, request(), ReadPolicy::default())
                .unwrap(),
            before
        );
...
fn graph_fusion_does_not_award_text_ranks_to_unsatisfied_fuzzy_groups() {
    let ((), report) = run_async_under_lab(0xbeac_4102, |root| async move {
        ...
        assert_eq!(
            (before[0].score.numerator(), before[0].score.denominator()),
            (100, 61)
        );
                ...
                let threshold = |db: &Database<MemVfs>| {
                    ...
                    while low < high {
                        ...
                        let restricted = token
                            .attenuate(Restriction::MaxWork(if native {
                                1_000_000
                            } else {
                                middle
                            }))
                        ...
                        match db.beacon_search_authorized(
                            ...
                            Err(ReadError::Index(BeaconError::WorkBudgetExceeded)) if native => {
                                low = middle + 1
                            }
...
fn nonmatching_rows_remain_conflict_dependencies_after_savepoint_rollback() {
    let ((), report) = run_async_under_lab(0xbeac_4104, |root| async move {
        ...
        let rows = txn
            .beacon_search(&db, &c.query(), &options(), request())
            .unwrap();
        ...
        assert!(matches!(
            txn.finish(&mut db, &c.commit()).await,
