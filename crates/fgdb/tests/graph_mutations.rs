...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in 1..=3_u128 {
        data.create_vertex(
            ...
            vec![],
            vec![
                (P, CanonicalScalar::Int(id as i64 * 10)),
                (OLD, CanonicalScalar::Bool(true)),
            ],
        );
    }
...
fn simultaneous_updates_commit_as_one_batch_and_pinned_history_survives_reopen() {
    let ((), report) = run_async_under_lab(0x6a7e_0001, |root| async move {
        ...
        assert_eq!(
            db.frontier().unwrap(),
            basis,
            "staging must not publish a marker"
        );
...
fn failed_assignments_and_quotas_preserve_prior_staged_work_and_abort_discards_success() {
    let ((), report) = run_async_under_lab(0x6a7e_0002, |root| async move {
        ...
        for mode in 0..3 {
            ...
            if mode == 2 {
                budget.query = GqlQueryPolicy::new(10_000, 0, 5_000_000, 2_000_000);
            }
            ...
            assert_eq!(
                value(&db.vertex(VId(1)).unwrap().unwrap(), P),
...
fn explicit_detach_delete_deduplicates_targets_and_retires_incident_edges_durably() {
    let ((), report) = run_async_under_lab(0x6a7e_0003, |root| async move {
        ...
        assert!(matches!(
            txn.execute_graph_mutation_governed(&mut db, &query, &plan, limited),
            Err(GqlQueryError::Source(
                GraphMutationError::EffectLimit { .. }
            ))
        ));
        ...
        for mode in 0..4 {
            for changed_domain in 0..2 {
                ...
                let plan = mutation(if mode == 2 {
                    ...
                    "MATCH (a)-[:R]->(b) WHERE b.p<25 SET a.q=7"
                });
                let mut budget = policy();
                if mode == 1 {
                    budget.query = GqlQueryPolicy::new(10_000, 0, 5_000_000, 2_000_000);
...
fn all_exact_limits_and_owner_basis_relation_precedence_apply_before_workspace_change() {
    let ((), report) = run_async_under_lab(0x6a7e_0005, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        assert!(matches!(
            txn.execute_graph_mutation_governed(&mut foreign, &query, &plan, zero),
        ...
        assert!(matches!(
            txn.execute_graph_mutation_governed(&mut db, &query, &other_relation, zero),
            Err(GqlQueryError::Source(GraphMutationError::Source(
        ...
        assert!(matches!(
            txn.execute_graph_mutation_governed(
                &mut db,
                &query,
                &mutation("MATCH (a) WHERE a.p<0 SET a.q=1"),
                zero
            ),
            Err(GqlQueryError::Source(GraphMutationError::Source(
