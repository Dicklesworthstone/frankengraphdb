...
use fgdb_types::{CanonicalScalar, DatabaseSecurityNamespaceId, EId, EmbeddedTxnCompletion, PurposeContexts, VId};
...
async fn seed(db: &mut Database<MemVfs>, cx: &fgdb_types::CommitCx) {
    ...
    for (id, value) in [(1, Some(CanonicalScalar::Int(7))), (2, Some(CanonicalScalar::Int(7))),
        (3, Some(CanonicalScalar::Int(9))), (4, Some(CanonicalScalar::Null)), (5, None)] {
        let mut properties = Vec::new();
        if let Some(value) = value { properties.push((P, value)); }
...
fn captured_property_joins_pin_history_through_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0x0ca9_0001, |root| async move {
        ...
        assert_eq!(ids(&db.execute_graph_pattern_governed(&cx, &query, wide()).unwrap().value), before);
        db.write(&commit, set_p(VId(1), CanonicalScalar::Int(9))).await.unwrap();
        let after = vec![Some(VId(1)), Some(VId(3))];
        assert_eq!(ids(&db.execute_graph_pattern_governed(&cx, &query, wide()).unwrap().value), after);
        assert_eq!(ids(&pinned.execute_graph_pattern_governed(&cx, &query, wide()).unwrap().value), before);
        assert_eq!(ids(&db.execute_graph_pattern_governed_at(&cx, &query, old, wide()).unwrap().value), before);
...
fn nullable_captures_and_staged_outer_values_read_the_canonical_overlay() {
    let ((), report) = run_async_under_lab(0x0ca9_0002, |root| async move {
        ...
        let query = pattern("MATCH (a) WHERE a.q=1 OPTIONAL MATCH (a)-[:R]->(b) MATCH (c) WHERE b IS NULL OR c.p=b.p RETURN c");
        let all = (1..=5).map(|id| Some(VId(id))).collect::<Vec<_>>();
        let mut txn = db.begin(&txcx).unwrap();
        assert_eq!(ids(&txn.execute_graph_pattern_governed(&mut db, &cx, &query, wide()).unwrap().value), all);
        let mut edge = WriteBatch::new(R);
        edge.add_edge(EId(10), VId(1), VId(2), vec![]);
        txn.write(&mut db, edge).unwrap();
        assert_eq!(ids(&txn.execute_graph_pattern_governed(&mut db, &cx, &query, wide()).unwrap().value),
            vec![Some(VId(1)), Some(VId(2))]);
        txn.write(&mut db, set_p(VId(2), CanonicalScalar::Int(9))).unwrap();
        assert_eq!(ids(&txn.execute_graph_pattern_governed(&mut db, &cx, &query, wide()).unwrap().value),
            vec![Some(VId(2)), Some(VId(3))]);
        // A real vertex with a stored-null property is NOT a null vertex value.
        txn.write(&mut db, set_p(VId(2), CanonicalScalar::Null)).unwrap();
        assert!(txn.execute_graph_pattern_governed(&mut db, &cx, &query, wide()).unwrap().value.is_empty());
        assert_eq!(ids(&db.execute_graph_pattern_governed(&cx, &query, wide()).unwrap().value), all);
        txn.abort();
...
fn both_successful_and_empty_outer_property_joins_retain_conflict_observations() {
    let ((), report) = run_async_under_lab(0x0ca9_0003, |root| async move {
        ...
        for initially_empty in [false, true] {
            ...
            let rows = reader.execute_graph_pattern_governed(&mut db, &cx, &query, wide()).unwrap();
            assert_eq!(rows.value.len(), usize::from(!initially_empty));
            db.write(&commit, set_p(VId(1), CanonicalScalar::Int(if initially_empty { 7 } else { 9 }))).await.unwrap();
...
fn captured_join_creation_and_late_update_failure_restore_outer_work_without_reusing_ids() {
    let ((), report) = run_async_under_lab(0x0ca9_0004, |root| async move {
        ...
        let result = txn.execute_graph_write_script_governed(
            &mut db, &cx, &script, &args, GraphWriteProgramPolicy::new(wide(), 1, 2, 0), |_| {
        ...
        assert_eq!(allocated, 2, "the late failure must happen after joined creation");
        assert_eq!(txn.staged_effect_digest().unwrap(), original);
        assert!(txn.vertex(&db, VId(90)).unwrap().is_some());
        for id in [VId(100), VId(101)] { assert!(txn.vertex(&db, id).unwrap().is_none()); }
        let mut next = 200;
        let receipt = txn.execute_graph_write_script_governed(
            &mut db, &cx, &script, &args, GraphWriteProgramPolicy::new(wide(), 2, 2, 0), |_| {
                let id = VId(next);
                next += 1;
                Ok::<_, ()>(ElementId::Vertex(id))
            },
        ...
        assert_eq!(receipt.steps()[0].created_vertices(), Some(&[VId(200), VId(201)][..]));
...
fn captured_joins_in_parameter_batches_skip_empty_records_and_share_creation_quota() {
    let ((), report) = run_async_under_lab(0x0ca9_0005, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let cx = contexts.query();
        let txcx = contexts.txn();
        ...
        let arguments = [2, 3, 2].into_iter().map(|peer|
            GqlParameters::new().with_int64("owner", 1).unwrap().with_int64("peer", peer).unwrap()).collect::<Vec<_>>();
        let batch = script.bind_parameter_sets(&arguments).unwrap();
        let mut allocations = 0;
        let failed = db.execute_bound_graph_write_script_batch_autocommit_governed(
            &txcx, &cx, &commit, &batch, GraphWriteProgramPolicy::new(wide(), 0, 1, 0), |_| {
                allocations += 1;
        ...
        let (receipt, completion) = db.execute_bound_graph_write_script_batch_autocommit_governed(
            &txcx, &cx, &commit, &batch, GraphWriteProgramPolicy::new(wide(), 0, 2, 0), |_| {
                let id = VId(next); next += 1;
                Ok::<_, ()>(ElementId::Vertex(id))
            },
        ).await.unwrap();
        assert_eq!(receipt.stats().created_vertices, 2);
        assert_eq!(batch.record_receipts(&receipt, 0).unwrap()[0].created_vertices(), Some(&[VId(400)][..]));
        assert_eq!(batch.record_receipts(&receipt, 1).unwrap()[0].created_vertices(), Some(&[][..]));
        assert_eq!(batch.record_receipts(&receipt, 2).unwrap()[0].created_vertices(), Some(&[VId(401)][..]));
    ...
    let ((), report) = run_async_under_lab(0x0ca9_0006, |root| async move {
        ...
        for dimension in 0..4 {
            ...
            assert!(db.execute_graph_pattern_governed(&cx, &query,
                GqlQueryPolicy::new(short[0], short[1], short[2], short[3])).is_err());
            assert_eq!(db.frontier().unwrap(), frontier);
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
