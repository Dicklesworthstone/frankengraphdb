...
fn arguments() -> GqlParameters { GqlParameters::new().with_int64("floor", 5).unwrap() }
...
fn aggregate() -> fgdb_gql::PreparedGraphAggregate {
    PreparedGraphAggregateText::prepare(&format!("{HEAD} RETURN a,COUNT(*) AS witnesses,COUNT(c) AS qualifying,\
        COUNT(DISTINCT c) AS companies,SUM(c.n) AS total GROUP BY a HAVING witnesses >= 1 \
        ORDER BY qualifying ASC,a ASC"), symbols).unwrap().bind_parameters(&arguments()).unwrap()
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    let mut vertices = WriteBatch::new(RelationId(9));
    for vid in [VId(0), VId(1), VId(2), HIGH] { vertices.create_vertex(vid, vec![PERSON], vec![]); }
    for vid in [VId(10), VId(11), VId(90)] { vertices.create_vertex(vid, vec![], vec![]); }
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<Plain> {
    ...
    for owner in vertices.iter().filter(|row| row.labels.contains(&PERSON)) {
        if edges.iter().any(|edge| edge.entry.src == owner.vid && edge.entry.relation == BLOCKED) { continue; }
        let first: Vec<_> = edges.iter().filter(|edge| edge.entry.src == owner.vid && edge.entry.relation == R).collect();
        if first.is_empty() { result.push((owner.vid, None, None, None)); }
        for edge in first {
            let mut matched = false;
            for second in edges.iter().filter(|candidate| candidate.entry.src == edge.entry.dst && candidate.entry.relation == S) {
                let company = vertices.iter().find(|row| row.vid == second.entry.dst).unwrap();
                if let Some(score) = integer(company).filter(|score| *score >= 5) {
                    matched = true;
                    result.push((owner.vid, Some(edge.entry.dst), Some(company.vid), Some(score)));
                }
            }
            if !matched { result.push((owner.vid, Some(edge.entry.dst), None, None)); }
        }
    }
    result.sort(); result
}
fn plain(rows: &[GraphValueRow]) -> Vec<Plain> {
    rows.iter().map(|row| {
        let nullable = |at| {
            ...
            if cell.is_null() { None } else { Some(cell.as_vertex().unwrap()) }
        };
        ...
        (row.get(0).unwrap().as_vertex().unwrap(), nullable(1), nullable(2), score)
    }).collect()
}
fn expected_summary(rows: &[Plain]) -> Vec<Summary> {
    let mut groups: BTreeMap<VId, Vec<&Plain>> = BTreeMap::new();
    for row in rows { groups.entry(row.0).or_default().push(row); }
    let mut result: Vec<_> = groups.into_iter().map(|(key, rows)| {
        let present: Vec<_> = rows.iter().filter_map(|row| row.3).collect();
        (key, rows.len() as u64, present.len() as u64,
            rows.iter().filter_map(|row| row.2).collect::<BTreeSet<_>>().len() as u64,
            (!present.is_empty()).then(|| present.into_iter().map(i128::from).sum()))
    }).collect();
    result.sort_by_key(|row| (row.2, row.0)); result
}
fn summary(rows: &[GraphAggregateRow]) -> Vec<Summary> {
    rows.iter().map(|row| (row.keys()[0].as_vertex().unwrap(),
        ...
        row.get(2).unwrap().as_count().unwrap(), row.get(3).unwrap().as_integer())).collect()
}
...
fn optional_and_anti_text_and_zero_preserving_aggregates_cover_all_five_read_surfaces() {
    let ((), report) = run_async_under_lab(0x0f71_0001, |root| async move {
        ...
        let query = query(); let aggregate = aggregate();
        ...
        let exact = GqlQueryPolicy::new(15, 7, full.evaluator.work_units, full.evaluator.scratch_entries);
        assert_eq!(db.execute_graph_pattern_governed(&cx, &query, exact).unwrap(), full);
        for cap in [GqlQueryPolicy::new(14, 7, u64::MAX, u64::MAX), GqlQueryPolicy::new(15, 6, u64::MAX, u64::MAX),
...
fn canonical_staging_changes_nullable_rows_and_summary_without_rewriting_history() {
    let ((), report) = run_async_under_lab(0x0f71_0002, |root| async move {
        ...
        assert_eq!(expected.len(), 5); assert_ne!(expected, old);
        ...
        assert_eq!(summary(&view.execute_graph_aggregate_governed(&cx, &aggregate, policy()).unwrap().value), expected_summary(&old));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0x0f71_0003, |root| async move {
        ...
        for refused in [false, true] {
            for change in 0..5 {
                ...
                else { assert!(plain(&result.unwrap().value).contains(&(VId(2), None, None, None))); }
...
fn empty_outer_sources_and_authority_refusals_keep_their_meaning() {
    let ((), report) = run_async_under_lab(0x0f71_0004, |root| async move {
        ...
        let cx = contexts.query(); let commit = contexts.commit(); let txn_cx = contexts.txn();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(db.execute_graph_pattern_governed(&cx, &query(), policy()).unwrap().value.is_empty());
        assert!(db.execute_graph_aggregate_governed(&cx, &aggregate(), policy()).unwrap().value.is_empty());
        let global = PreparedGraphAggregateText::prepare(&format!("{HEAD} RETURN COUNT(*) AS n,COUNT(c) AS present"), symbols)
            .unwrap().bind_parameters(&arguments()).unwrap();
        let zero = db.execute_graph_aggregate_governed(&cx, &global, policy()).unwrap();
        ...
        let query = query(); root.cancel_with(CancelKind::User, Some("scoped text error ordering"));
        ...
        assert!(matches!(view.execute_graph_pattern_governed_at(&cx, &query, future, policy()),
            Err(GqlQueryError::Source(GqlError::Read(ReadError::BeyondFrontier { .. })))));
