...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(10_000,10_000,5_000_000,2_000_000) }
...
fn arguments(skip: u64, limit: u64) -> GqlParameters {
    GqlParameters::new().with_int64("scale",2).unwrap()
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (id,label,p,q) in [(1,A,Some(3),2),(2,A,Some(-3),0),(3,A,None,4),(4,B,None,2),(5,B,Some(10),2)] {
...
fn integer(row: &VertexRow, key: PropertyKeyId) -> Option<i64> {
    match row.props.iter().find(|(property,_)| *property == key).map(|(_,value)| value) {
...
fn scalar_row(row: &VertexRow, absolute: bool) -> Plain {
    ...
    let ratio = p.zip(integer(row,Q).filter(|value| *value != 0)).map(|(p,q)| p/q);
    (score,ratio)
}
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<Plain> {
    ...
    for row in vertices.iter().filter(|row| row.labels.contains(&B)) {
        if let Some(count) = left.get_mut(&scalar_row(row,false)) { *count = count.saturating_sub(1); }
    }
    let mut output = Vec::new();
    for (row,count) in left { for _ in 0..count { output.push(row); } }
    output.sort_by(|a,b| b.0.cmp(&a.0).then_with(|| match (a.1,b.1) {
...
fn plain(rows: &[GraphValueRow]) -> Vec<Plain> {
    rows.iter().map(|row| {
        ...
        let CanonicalScalar::Int(score) = row.values()[0].as_scalar().unwrap() else { panic!("score kind") };
...
fn computed_walk_difference_reads_staged_values_and_retains_history_after_reopen() {
    let ((),report) = run_async_under_lab(0xc011_0001,|root| async move {
        ...
        assert_eq!(old,vec![(6,Some(1)),(6,None),(6,None),(6,None),(0,None),(0,None),(0,None)]);
        let mut txn = db.begin(&txcx).unwrap();
        for result in [
            db.execute_graph_set_governed(&cx,&plan,policy()).unwrap(),
            db.execute_graph_set_governed_at(&cx,&plan,basis,policy()).unwrap(),
            pinned.execute_graph_set_governed(&cx,&plan,policy()).unwrap(),
            pinned.execute_graph_set_governed_at(&cx,&plan,basis,policy()).unwrap(),
            txn.execute_graph_set_governed(&db,&cx,&plan,policy()).unwrap(),
        ] { assert_eq!(plain(&result.value),old); }
        let page = template.bind_parameters(&arguments(1,2)).unwrap();
        assert_eq!(plain(&db.execute_graph_set_governed(&cx,&page,policy()).unwrap().value),old[1..3]);
        ...
        assert_eq!(expected,vec![(10,Some(5)),(10,Some(5)),(8,Some(2)),(8,Some(2)),(6,Some(1))]);
        assert_eq!(plain(&txn.execute_graph_set_governed(&db,&cx,&plan,policy()).unwrap().value),expected);
        assert_eq!(plain(&db.execute_graph_set_governed(&cx,&plan,policy()).unwrap().value),old);
        txn.commit(&mut db,&commit).await.unwrap();
        db.compact(&commit).await.unwrap(); drop(db);
        let reopened = Database::open_with_vfs(&commit,vfs,&path,keys()).await.unwrap();
        assert_eq!(plain(&reopened.execute_graph_set_governed(&cx,&plan,policy()).unwrap().value),expected);
        assert_eq!(plain(&reopened.execute_graph_set_governed_at(&cx,&plan,basis,policy()).unwrap().value),old);
        assert_eq!(plain(&pinned.execute_graph_set_governed(&cx,&plan,policy()).unwrap().value),old);
    ...
    let ((),report) = run_async_under_lab(0xc011_0002,|root| async move {
        ...
        for mode in 0..4 {
            for insertion in [false,true] {
                ...
                let budget = GqlQueryPolicy::new(10_000,u64::from(mode != 1),5_000_000,2_000_000);
                ...
                if insertion { winner.create_vertex(VId(6),vec![A],vec![(P,CanonicalScalar::Int(7))]); }
                else { winner.set_vertex_property(VId(2),P,Some(CanonicalScalar::Int(4))); }
...
fn exact_computed_read_limits_and_source_authority_precede_result_publication() {
    let ((),report) = run_async_under_lab(0xc011_0003,|root| async move {
        ...
        let mut db = Database::open_memory(&commit,keys()).await.unwrap();
        let basis = seed(&mut db,&commit).await;
        let plan = PreparedGraphSetText::prepare(TEXT,symbols).unwrap().bind_parameters(&arguments(1,2)).unwrap();
        ...
        assert_eq!(db.execute_graph_set_governed(&cx,&plan,exact).unwrap(),measured);
        for refused in [
        ...
        assert_eq!(db.frontier().unwrap(),basis,"computed reads cannot publish a write");
