...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 1000, 5_000_000, 2_000_000)
}
...
fn optional(take: u64) -> PreparedGraphPattern<GraphValueRow> {
    PreparedGraphText::prepare(
        "MATCH (a:Root) OPTIONAL MATCH (f:Flag) WHERE f.p >= 0 \
        RETURN a,f,f.p AS value ORDER BY a,f LIMIT $take",
        symbols,
    )
    .unwrap()
    .bind_parameters(&GqlParameters::new().with_uint64("take", take).unwrap())
    .unwrap()
}
...
fn probe(anti: bool, take: u64) -> PreparedGraphPattern<GraphValueRow> {
    ...
    PreparedGraphText::prepare(
        &format!(
            "MATCH (a:Root) WHERE {head} {{ MATCH (f:Flag) WHERE f.p >= {floor} }} \
        RETURN a LIMIT {take}"
        ),
        symbols,
    )
...
fn oracle(vertices: &[VertexRow]) -> Vec<Plain> {
    let flags: Vec<_> = vertices
        .iter()
        .filter(|row| row.labels.contains(&FLAG))
        .filter_map(|row| {
            integer(row)
                .filter(|value| *value >= 0)
                .map(|value| (row.vid, value))
        })
        .collect();
    let mut result = Vec::new();
    for owner in vertices.iter().filter(|row| row.labels.contains(&ROOT)) {
        if flags.is_empty() {
            result.push((owner.vid, None, None));
        } else {
            for &(flag, value) in &flags {
                result.push((owner.vid, Some(flag), Some(value)));
            }
        }
    }
    result.sort();
...
fn expected_summaries(rows: &[Plain]) -> Vec<Summary> {
    ...
    for &(owner, flag, value) in rows {
        ...
        group.0 += 1;
        group.1 += u64::from(flag.is_some());
        if let Some(value) = value {
            group.2 = Some(group.2.unwrap_or(0) + i128::from(value));
        }
    }
    groups
        .into_iter()
        .map(|(id, (rows, flags, total))| (id, rows, flags, total))
        .collect()
}
...
fn independent_rows_and_aggregates_follow_staging_but_preserve_pinned_history() {
    let ((), report) = run_async_under_lab(0x1ade_0001, |root| async move {
        ...
        let plan = optional(100);
        let grouped = aggregate(100);
        let frozen = plan.canonical_bytes();
        let old = oracle(&db.vertices().unwrap());
        assert_eq!(
            old,
            vec![
                (VId(1), Some(VId(10)), Some(7)),
                (VId(2), Some(VId(10)), Some(7))
            ]
        );
        let mut txn = db.begin(&txn_cx).unwrap();
        for result in [
            db.execute_graph_pattern_governed(&cx, &plan, wide())
                .unwrap(),
            db.execute_graph_pattern_governed_at(&cx, &plan, basis, wide())
                .unwrap(),
            pinned
                .execute_graph_pattern_governed(&cx, &plan, wide())
                .unwrap(),
            pinned
                .execute_graph_pattern_governed_at(&cx, &plan, basis, wide())
                .unwrap(),
            txn.execute_graph_pattern_governed(&db, &cx, &plan, wide())
                .unwrap(),
        ] {
        ...
        for result in [
            db.execute_graph_aggregate_governed(&cx, &grouped, wide())
                .unwrap(),
            db.execute_graph_aggregate_governed_at(&cx, &grouped, basis, wide())
                .unwrap(),
            pinned
                .execute_graph_aggregate_governed(&cx, &grouped, wide())
                .unwrap(),
            pinned
                .execute_graph_aggregate_governed_at(&cx, &grouped, basis, wide())
                .unwrap(),
            txn.execute_graph_aggregate_governed(&db, &cx, &grouped, wide())
                .unwrap(),
        ] {
        ...
        assert_eq!(
            ...
            vec![(VId(1), 2, 2, Some(20)), (VId(2), 2, 2, Some(20))]
        );
        assert_eq!(
            plain(
                &txn.execute_graph_pattern_governed(&db, &cx, &plan, wide())
        ...
        for kind in 0..4 {
            for mode in 0..3 {
                for change in 0..4 {
                    ...
                    let policy =
                        GqlQueryPolicy::new(1000, u64::from(mode == 0), 5_000_000, 2_000_000);
                    if kind == 3 {
                        let result = txn.execute_graph_aggregate_governed(
                            &db,
                            &cx,
                            &aggregate(take),
                            policy,
                        ...
                        if mode == 1 {
                            ...
                            assert_eq!(result.unwrap().value.len(), usize::from(mode == 0));
                        }
                    } else {
                        let plan = if kind == 0 {
                            optional(take)
                        ...
                        match change {
                            ...
                            2 => {
                                winner.set_vertex_property(
                                    VId(11),
                                    P,
                                    Some(CanonicalScalar::Int(150)),
                                );
                            }
                            _ => {
                                winner.delete_vertex(VId(10));
                            }
                        }
...
fn mixed_edge_roots_and_independent_children_admit_both_tables_under_one_policy() {
    let ((), report) = run_async_under_lab(0x1ade_0003, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        seed(&mut db, &commit).await;
        let plan = PreparedGraphText::prepare(
            "MATCH (a:Root)-[:R]->(b) \
            OPTIONAL MATCH (x:Flag)-[:R]->(y) RETURN a,b,x,y LIMIT 2",
            symbols,
        )
        .unwrap()
        .bind_parameters(&GqlParameters::new())
        ...
        let measured = db
            .execute_graph_pattern_governed(&cx, &plan, wide())
            .unwrap();
        assert_eq!(measured.rows.snapshot_records, 8);
        assert_eq!(measured.value.len(), 2);
        for row in &measured.value {
            assert_eq!(
                row.values()
                    .iter()
                    .map(|value| value.as_vertex())
                    .collect::<Vec<_>>(),
        ...
        assert_eq!(
            db.execute_graph_pattern_governed(&cx, &plan, GqlQueryPolicy::new(8, 2, work, scratch))
                .unwrap(),
            measured
        );
        for policy in [
            GqlQueryPolicy::new(7, 2, u64::MAX, u64::MAX),
            GqlQueryPolicy::new(8, 1, u64::MAX, u64::MAX),
            GqlQueryPolicy::new(8, 2, work - 1, u64::MAX),
            GqlQueryPolicy::new(8, 2, u64::MAX, scratch - 1),
        ] {
            assert!(
        ...
        let absent = PreparedGraphText::prepare(
            "MATCH (a:Root) OPTIONAL MATCH (f:Flag) WHERE f.p>99 \
            RETURN a,f,f.p AS value",
            symbols,
        )
        .unwrap()
        .bind_parameters(&GqlParameters::new())
        ...
        assert_eq!(
            plain(
                &db.execute_graph_pattern_governed(&cx, &absent, wide())
                    .unwrap()
                    .value
            ),
