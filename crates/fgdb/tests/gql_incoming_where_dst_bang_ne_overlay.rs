...
#[test]
fn incoming_destination_bang_inequality_sees_staged_dest_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x62_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-where-dst-bang-ne-overlay-{}",
            std::process::id()
        ));
        ...
        staged.create_vertex(VId(4), vec![], vec![(key, CanonicalScalar::Int(9))]);
        ...
        let filtered = "MATCH (a)<-[:R]-(b) WHERE a.k != 1 RETURN a";
        assert_eq!(
            txn.execute_gql(&db, filtered, &bind)
                .expect("overlay destination bang-inequality MATCH executes"),
            vec![VId(4)]
        );
        assert_eq!(
            db.execute_gql(filtered, &bind)
                .expect("base destination bang-inequality MATCH executes"),
            Vec::<VId>::new()
        );
        txn.abort();
    });
