...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(100_000, 10_000, 5_000_000, 5_000_000) }
...
fn query(mode: &str, bounds: &str) -> PreparedGraphPattern<GraphValueRow> {
    PreparedGraphText::prepare(&format!(
        "MATCH {mode} (a)-[:R*{bounds}]->(b) WHERE a.p=1 RETURN ALL b"), symbols)
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in 1..=3 { batch.create_vertex(VId(id), vec![], vec![(P, CanonicalScalar::Int(id as i64))]); }
...
fn restricted_paths_read_staged_topology_and_pinned_history_through_reopen() {
    let ((), report) = run_async_under_lab(0xac1c_0001_u64, |root| async move {
        ...
        let acyclic = query("ACYCLIC", "1..3"); let simple = query("SIMPLE", "1..3");
        ...
        for (plan, expected) in [(&acyclic, &old_acyclic), (&simple, &old_simple)] {
            assert_eq!(&ids(&db.execute_graph_pattern_governed(&cx, plan, policy()).unwrap().value), expected);
            assert_eq!(&ids(&pinned.execute_graph_pattern_governed(&cx, plan, policy()).unwrap().value), expected);
        }
        ...
        for (plan, expected) in [(&acyclic, &new_acyclic), (&simple, &new_simple)] {
            assert_eq!(&ids(&txn.execute_graph_pattern_governed(&db, &cx, plan, policy()).unwrap().value), expected);
        }
        assert_eq!(ids(&db.execute_graph_pattern_governed(&cx, &simple, policy()).unwrap().value), old_simple);
        txn.commit(&mut db, &commit).await.unwrap();
        db.compact(&commit).await.unwrap(); drop(db);
        let db = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        for (plan, current, historic) in [(&acyclic, &new_acyclic, &old_acyclic), (&simple, &new_simple, &old_simple)] {
            assert_eq!(&ids(&db.execute_graph_pattern_governed(&cx, plan, policy()).unwrap().value), current);
            assert_eq!(&ids(&db.execute_graph_pattern_governed_at(&cx, plan, basis, policy()).unwrap().value), historic);
            assert_eq!(&ids(&pinned.execute_graph_pattern_governed(&cx, plan, policy()).unwrap().value), historic);
        }
...
fn empty_restricted_paths_keep_phantom_witnesses_without_extra_transaction_reads() {
    let ((), report) = run_async_under_lab(0xac1c_0002, |root| async move {
        ...
        for (mode, bounds) in [("ACYCLIC", "1..8"), ("SIMPLE", "2..8")] {
            ...
            let mut txn = db.begin(&txcx).unwrap();
            let mut prefix = WriteBatch::new(R); prefix.create_vertex(VId(999), vec![], vec![]);
            txn.write(&mut db, prefix).unwrap();
            assert!(txn.execute_graph_pattern_governed(&db, &cx, &query(mode, bounds), policy()).unwrap().value.is_empty());
...
fn restricted_path_ingestion_rolls_back_a_late_action_failure_and_can_be_retried_explicitly() {
    let ((), report) = run_async_under_lab(0xac1c_0003, |root| async move {
        ...
        let failed = txn.execute_graph_write_script_governed(&mut db, &cx, &script, &args,
            GraphWriteProgramPolicy::new(policy(), 3, 4, 0), |request| {
                assert_eq!(request.statement, 0);
                let fgdb_gql::insertion::GraphInsertRequest::Vertex { row, vertex: 0 } = request.request
                    else { panic!("one vertex per selected path occurrence") };
                allocations += 1;
                Ok::<_, ()>(ElementId::Vertex(VId(100 + row as u128)))
            });
        assert!(failed.is_err()); assert_eq!(allocations, 4);
        assert_eq!(txn.staged_effect_digest().unwrap(), before);
        for id in 100..104 { assert!(txn.vertex(&db, VId(id)).unwrap().is_none()); }
        ...
        for id in 200..204 { assert_eq!(db.vertex(VId(id)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(0))]); }
...
fn source_membership_and_result_phases_share_the_exact_database_allowance() {
    let ((), report) = run_async_under_lab(0xac1c_0004, |root| async move {
        ...
        for mode in ["ACYCLIC", "SIMPLE"] {
            ...
            let measured = db.execute_graph_pattern_governed(&cx, &plan, policy()).unwrap();
            ...
            assert_eq!(db.execute_graph_pattern_governed(&cx, &plan, GqlQueryPolicy::new(caps[0], caps[1], caps[2], caps[3])).unwrap(), measured);
            for dimension in 0..4 {
                let mut cap = caps; cap[dimension] -= 1;
                assert!(db.execute_graph_pattern_governed(&cx, &plan, GqlQueryPolicy::new(cap[0], cap[1], cap[2], cap[3])).is_err());
            }
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn captured_query(mode: &str) -> PreparedGraphPattern<GraphValueRow> {
    PreparedGraphText::prepare(&format!(
        "MATCH route = {mode} (a)-[:R*1..3]->(b) WHERE a.p=1 RETURN ALL route"), symbols)
        .unwrap().bind_parameters(&GqlParameters::new()).unwrap()
}
...
fn captured_restricted_routes_keep_parallel_identities_and_pinned_generations() {
    let ((), report) = run_async_under_lab(0xac1c_0005, |root| async move {
        ...
        let basis = seed(&mut db, &commit).await;
        let pinned = db.read_session().unwrap();
        let acyclic = captured_query("ACYCLIC"); let simple = captured_query("SIMPLE");
        let old_acyclic = vec![vec![EId(10)], vec![EId(10), EId(13)],
            vec![EId(11)], vec![EId(11), EId(13)]];
        let old_simple = vec![vec![EId(10)], vec![EId(10), EId(12)], vec![EId(10), EId(13)],
            vec![EId(11)], vec![EId(11), EId(12)], vec![EId(11), EId(13)]];
        let old_rows = db.execute_graph_pattern_governed(&cx, &simple, policy()).unwrap().value;
        assert_eq!(edge_routes(&old_rows), old_simple);
        assert_eq!(edge_routes(&db.execute_graph_pattern_governed(&cx, &acyclic, policy()).unwrap().value), old_acyclic);
        let mut txn = db.begin(&txcx).unwrap();
        let mut changes = WriteBatch::new(R);
        changes.delete_edge(EId(11));
        changes.add_edge(EId(14), VId(3), VId(1), vec![]);
        ...
        let new_simple = vec![vec![EId(10)], vec![EId(10), EId(12)],
            vec![EId(10), EId(13)], vec![EId(10), EId(13), EId(14)]];
        for (plan, expected) in [(&acyclic, &new_acyclic), (&simple, &new_simple)] {
            assert_eq!(&edge_routes(&txn.execute_graph_pattern_governed(&db, &cx, plan, policy()).unwrap().value), expected);
        }
        assert_eq!(pinned.execute_graph_pattern_governed(&cx, &simple, policy()).unwrap().value, old_rows);
        txn.commit(&mut db, &commit).await.unwrap();
        db.compact(&commit).await.unwrap(); drop(db);
        let db = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        for (plan, expected) in [(&acyclic, &new_acyclic), (&simple, &new_simple)] {
            assert_eq!(&edge_routes(&db.execute_graph_pattern_governed(&cx, plan, policy()).unwrap().value), expected);
        }
        assert_eq!(db.execute_graph_pattern_governed_at(&cx, &simple, basis, policy()).unwrap().value, old_rows);
        assert_eq!(pinned.execute_graph_pattern_governed(&cx, &simple, policy()).unwrap().value, old_rows);
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
