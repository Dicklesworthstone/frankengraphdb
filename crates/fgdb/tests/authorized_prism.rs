...
use fgdb_prism::{
    Directedness, FnxCallSpec, FnxExecutionError, FnxExecutionLimits, FnxGraphKind, FnxParameters,
    FnxReadError, FnxReadOptions, FnxSelection, FnxSourceLimits, FnxValue, FnxWeightError,
    FnxWeightSpec, MissingWeightPolicy, ParallelEdgePolicy, ProjectionError, ProjectionLimits,
    ProjectionSpec, SelfLoopPolicy,
};
...
async fn database(cx: &CommitCx, hidden: bool) -> Database<MemVfs> {
    ...
    for (eid, source, target, weight) in [
        ...
        if hidden {
            props.push((PropertyKeyId(2), CanonicalScalar::Bool(true)));
        }
...
fn all_eight_calls_match_a_physically_masked_database_in_every_compatible_direction() {
    let ((), report) = run_async_under_lab(0x5ec0_3001, |root| async move {
            ...
            for policy in [
                ...
                for loops in [SelfLoopPolicy::Keep, SelfLoopPolicy::Drop] {
                    ...
                    for text in CALLS {
                        ...
                        assert_eq!(
                            read(&db, &cx, &issuer, &token, text, opt),
                            expected,
                            "{text} {direction:?} {policy:?} {loops:?}"
                        );
                        assert_eq!(
                            db.execute_fnx_authorized(
                                &cx,
                                &issuer,
                                &token,
                                BRANCH,
        ...
        let raw = db
            .call_fnx(&cx, BFS, &FnxParameters::new(), opt)
            .unwrap()
            .analytics
            .rows;
...
fn hidden_weight_values_and_label_names_are_absent_before_selection_or_reduction() {
    let ((), report) = run_async_under_lab(0x5ec0_3002, |root| async move {
        ...
        let mut raw = opt;
        raw.selection.vertex_label = Some(LabelId(1));
        raw.selection.relation = Some(RelationId(1));
        assert!(matches!(
            db.call_fnx(&cx, DIJKSTRA, &FnxParameters::new(), raw),
            Err(FnxReadError::Weight {
        ...
        for (missing, expected) in [
            ...
            assert_eq!(
                result[2],
                vec![FnxValue::Vertex(VId(3)), FnxValue::Float(expected)]
            );
        }
        ...
        assert_eq!(
            read(&db, &cx, &issuer, &token, DIJKSTRA, opt)[2][1],
            FnxValue::Float(2.0)
        );
        let both = token
        ...
        assert_eq!(
            read(&db, &cx, &issuer, &denied_relations, CC, opt),
            isolates
        );
        ...
        assert!(matches!(
            ...
            Err(FnxReadError::Projection(ProjectionError::SelfLoop(_)))
        ));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0x5ec0_3003, |root| async move {
        ...
        for resource in 0..6 {
            ...
            match resource {
                ...
                2 | 3 => assert!(matches!(
                    error,
                    FnxReadError::Projection(ProjectionError::LimitExceeded { .. })
                )),
                _ => assert!(matches!(
                    error,
        ...
        let mut empty = opt;
        empty.selection.vertex_label = Some(LabelId(99));
        assert!(
            read(
                &db,
                &cx,
    ...
    let ((), report) = run_async_under_lab(0x5ec0_3004, |root| async move {
        ...
        authorization(
            db.call_fnx_authorized(
                ...
                &FnxParameters::new(),
                opt,
                || panic!("namespace refusal must precede clock access"),
            )
            .unwrap_err(),
        ...
        assert!(matches!(
            ...
            Err(FnxReadError::Read(ReadError::BeyondFrontier { .. }))
        ));
        opt.as_of = None;
        assert!(matches!(
            db.call_fnx_authorized(
        ...
        assert!(matches!(
            db.call_fnx_authorized(
                ...
                &token,
                BRANCH,
                "CALL fnx.not_registered()",
                &FnxParameters::new(),
                opt,
                || panic!("binding must not read the graph or clock")
            ),
            Err(FnxReadError::Bind(_))
        ));
        opt = options(Directedness::Directed);
        for id in [99, 100] {
            // hidden existing identity and absent identity
            let call = FnxCallSpec::single_source_shortest_path_length(VId(id), None);
            assert!(
                matches!(db.execute_fnx_authorized(&cx, &issuer, &token, BRANCH,
...
fn historical_winners_cannot_resurrect_a_hidden_successor_even_after_compaction() {
    let ((), report) = run_async_under_lab(0x5ec0_3005, |root| async move {
        ...
        let raw = db
            .call_fnx(&cx, TRIANGLES, &FnxParameters::new(), opt)
            .unwrap()
            .analytics
            .rows;
        assert!(
            raw.iter()
                .any(|row| row[0] == FnxValue::Vertex(VId(2)) && row[1] == FnxValue::Integer(1))
        );
        ...
        for compacted in [false, true] {
            ...
            for (at, expected) in [(before, &original), (hidden_at, &hidden)] {
                ...
                assert_eq!(
                    &read(&db, &cx, &issuer, &token, TRIANGLES, historical),
                    expected
                );
                assert!(
                    read(&db, &cx, &issuer, &denied, TRIANGLES, historical).is_empty(),
...
fn every_source_builder_kernel_and_delivery_clock_observes_expiry_and_retirement() {
    let ((), report) = run_async_under_lab(0x5ec0_3006, |root| async move {
        ...
        for empty in [false, true] {
            ...
            assert_eq!(
                seen, calls,
                "retire at final delivery, even for an empty result"
            );
            authorization(
                db.call_fnx_authorized(
                    &cx,
        ...
        for (text, direction) in [
            ...
            for stop in 1..=calls {
                ...
                assert!(matches!(
                    ...
                    Err(FnxReadError::Cancelled(QueryError::Pattern(
                ...
                assert_eq!(
                    probe.calls(),
                    stop,
                    "stop at the first failed source/build/kernel checkpoint"
                );
...
fn signed_rows_admit_only_reachable_output_and_empty_sources_still_require_authority() {
    let ((), report) = run_async_under_lab(0x5ec0_3008, |root| async move {
        ...
        assert_eq!(
            db.execute_fnx_authorized(&cx, &issuer, &one, BRANCH, &call, opt, || 100)
                .unwrap(),
        ...
        assert_eq!(
            db.execute_fnx_authorized(&cx, &issuer, &one, BRANCH, &isolated, opt, || 100)
                .unwrap(),
        ...
        authorization(
            db.execute_fnx_authorized(&cx, &issuer, &zero, BRANCH, &call, opt, || 100)
                .unwrap_err(),
            Error::LimitExceeded(LimitDimension::Rows),
        );
        let empty = Database::<MemVfs>::open_memory(
            &c.commit(),
            DatabaseKeys::new([0x31; 32], NS, [0x32; 32]),
        )
        .await
        .unwrap();
