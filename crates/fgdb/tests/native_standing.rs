...
use fgdb::{
    Database, DatabaseKeys, PreparedNativeRead, QueryError, QueryResult, QueryValue,
...
fn native_patterns_aggregates_and_pipelines_stay_equal_after_each_commit() {
    let ((), report) = run_async_under_lab(0x6e73_7101, |root| async move {
        ...
        let text = [
            ...
            "MATCH (n:L) RETURN SUM(n.p) AS total, n.p AS key, COUNT(*) AS amount GROUP BY key ORDER BY key DESC",
            "MATCH (n:L) RETURN COUNT(*) AS amount, SUM(n.p) AS total, AVG(n.p) AS mean",
            "MATCH (n:L) RETURN SUM(n.p) AS total, COUNT(*) AS amount",
        ];
        let params = GqlParameters::new();
        let handles: Vec<_> = text
            .iter()
            .map(|q| {
                db.register_standing_native(&cx, q, &params, symbols, policy())
                    .unwrap()
            })
            .collect();
        for (id, value) in [(1, 4), (2, 4), (3, 9), (4, 2)] {
            ...
            for (q, handle) in text.iter().zip(&handles) {
                ...
                assert_eq!(
                    actual,
                    db.query(&cx, q, &params, symbols, policy()).unwrap(),
                    "{q}"
                );
            }
        }
        ...
        assert_eq!(
            db.standing_native_columns(&cx, &handles[2]).unwrap(),
            &["total", "key", "amount"]
        );
        db.rebuild_standing_query(&cx, &handles[2], policy())
            .unwrap();
        assert_eq!(
...
fn prepared_registration_freezes_arguments_without_retaining_resolver_or_template() {
    let ((), report) = run_async_under_lab(0x6e73_7102, |root| async move {
        ...
        let b = prepared
            ...
            .unwrap();
        assert!(
            matches!(prepared.register_standing(&mut db, &cx, &GqlParameters::new(), policy()),
        ...
        assert!(matches!(
            other.standing_native_query(&cx, &a, GqlQueryPolicy::new(0, 0, 0, 0)),
            Err(StandingQueryError::ForeignHandle)
        ));
        assert!(matches!(
            other.standing_native_columns(&cx, &a),
            Err(StandingQueryError::ForeignHandle)
        ));
...
fn delivery_refusals_do_not_fence_views_and_rebuild_retains_native_layout() {
    let ((), report) = run_async_under_lab(0x6e73_7103, |root| async move {
        ...
        for allowance in [
            ...
            GqlQueryPolicy::new(0, 10, 0, 100_000),
            GqlQueryPolicy::new(0, 10, 100_000, 0),
        ] {
            assert!(matches!(
                db.standing_native_query(&cx, &healthy, allowance),
        ...
        assert!(
            db.rebuild_standing_query(&cx, &bounded, GqlQueryPolicy::new(0, 0, 0, 0))
                .is_err()
        );
        assert!(matches!(
            db.standing_native_query(&cx, &bounded, policy()),
        ...
        assert_eq!(
            db.standing_native_query(&cx, &bounded, policy()).unwrap().1,
            integers(&[4, 7])
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
