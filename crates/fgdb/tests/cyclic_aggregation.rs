...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 1000, 4_000_000, 1_000_000)
}
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in [0, 1, 10, 11, 20, 21, 22, 30, 31, 32, 50, 51] {
...
fn oracle(edges: &[EdgeRecord]) -> Vec<Summary> {
    ...
    for root in edges.iter().filter(|e| e.entry.relation == R) {
            ...
            .filter(|e| e.entry.relation == S && e.entry.src == root.entry.dst)
        {
            ...
            {
                for _closing in edges.iter().filter(|e| {
                    e.entry.relation == U
                        && e.entry.src == second.entry.dst
                        && e.entry.dst == root.entry.dst
                }) {
                    for _leaf in edges
                        .iter()
                        .filter(|e| e.entry.relation == V && e.entry.src == root.entry.dst)
                    {
                        *counts
                            .entry((root.entry.src, root.entry.dst))
                            .or_insert(0_u64) += 1;
                    }
                }
            }
    ...
    rows.sort_by_key(|row| (std::cmp::Reverse(row.2), row.0, row.1));
    rows
}
...
fn cycles_and_attached_forests_cover_all_read_surfaces_and_reopened_history() {
    let ((), report) = run_async_under_lab(0xc1c1_0001, |root| async move {
        ...
        for result in [
            ...
            assert_eq!(result.rows.snapshot_records, 19);
        }
        let measured = db
            .execute_graph_aggregate_governed(&cx, &query, wide())
            .unwrap();
        ...
        for cap in [
            GqlQueryPolicy::new(19, 1, u64::MAX, u64::MAX),
        ...
        assert_eq!(expected, vec![(VId(0), VId(10), 10)]);
        assert_eq!(
            plain(
                &txn.execute_graph_aggregate_governed(&db, &cx, &query, wide())
                    .unwrap()
    ...
    let ((), report) = run_async_under_lab(0xc1c1_0002, |root| async move {
        ...
        for mode in 0..3 {
            for change in 0..4 {
                ...
                let mut winner = WriteBatch::new(match change {
                    2 => T,
                    3 => V,
                    _ => U,
                });
                match change {
                    ...
                    1 => {
                        winner.add_edge(EId(90), VId(32), VId(10), vec![]);
                    }
                    2 => {
                        winner.delete_edge(EId(20));
                    }
                    _ => {
                        winner.delete_edge(EId(42));
                    }
                }
...
fn database_counts_eight_hidden_cycles_with_original_source_admission() {
    let ((), report) = run_async_under_lab(0xc1c1_0003, |root| async move {
        ...
        let mut middle = WriteBatch::new(T);
        let mut last = WriteBatch::new(U);
        let mut eid = 10;
        for a in 2..10 {
            second.add_edge(EId(eid), VId(1), VId(a), vec![]);
            eid += 1;
            for b in 10..18 {
                middle.add_edge(EId(eid), VId(a), VId(b), vec![]);
                eid += 1;
            }
        }
        for b in 10..18 {
            last.add_edge(EId(eid), VId(b), VId(1), vec![]);
            eid += 1;
        }
        db.write_atomic(&commit, vec![vertices, first, second, middle, last])
            .await
            .unwrap();
        let mut statement = "MATCH (a)-[:R]->(b)".to_owned();
        for at in 0..8 {
            statement.push_str(&format!(",(b)-[:S]->(x{at})-[:T]->(y{at})-[:U]->(b)"));
        }
        ...
        let result = db
            .execute_graph_aggregate_governed(
                &cx,
                &query,
                GqlQueryPolicy::new(81, 1, 1_000_000, 100_000),
            )
            .unwrap();
        assert_eq!(
        ...
        assert!(matches!(
            db.execute_graph_aggregate_governed(
                &cx,
                &query,
                GqlQueryPolicy::new(80, 1, u64::MAX, u64::MAX)
            ),
