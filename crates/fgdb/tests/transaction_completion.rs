...
use fgdb::{
    Database, DatabaseKeys, DatabaseState, DerivedPublicationStage, MemVfs, ReadError, WriteBatch,
    WriteError, WriteMismatchPolicy, WriteTxnError,
};
use fgdb_delta_types::{LabelId, PropertyKeyId, RelationId};
use fgdb_gql::{GqlParameters, GqlQueryPolicy, GraphSymbol, GraphSymbolKind, PreparedGraphText};
use fgdb_types::{
    CanonicalScalar, CommitCx, CommitSeq, DatabaseSecurityNamespaceId, EmbeddedTxnCompletion,
...
fn read_close_preserves_every_published_identity_and_consumes_no_sequence() {
    let ((), report) = run_async_under_lab(0xf171_0001, |root| async move {
        ...
        for read in [false, true] {
            ...
            if read {
                assert!(transaction.vertex(&database, VId(1)).unwrap().is_some());
            }
        ...
        assert_eq!(
            database.write(&cx, write).await.unwrap(),
            CommitSeq(basis.0 + 1)
        );
    });
...
fn failed_preparation_observations_are_validated_even_without_a_prepared_write() {
    let ((), report) = run_async_under_lab(0xf171_0004, |root| async move {
        ...
        failed.compare_and_set_vertex_property(
            VId(1),
            P,
        ...
        assert!(matches!(
            transaction.write(&mut database, failed),
...
fn no_op_writes_remain_writes_and_legacy_commit_still_requires_a_batch() {
    let ((), report) = run_async_under_lab(0xf171_0005, |root| async move {
        ...
        assert!(matches!(
            empty.commit(&mut database, &cx).await,
...
fn foreign_owner_and_unpolled_completion_preserve_the_active_workspace() {
    let ((), report) = run_async_under_lab(0xf171_0006, |root| async move {
        ...
        assert!(matches!(
            transaction.finish(&mut foreign, &cx).await,
        ...
        assert!(matches!(
            transaction.finish(&mut owner, &cx).await.unwrap(),
            EmbeddedTxnCompletion::WriteCommitted { .. }
        ));
...
fn commit_faults_keep_abort_and_unknown_distinct_and_recover_whole_writes() {
    let ((), report) = run_async_under_lab(0xf171_0007, |root| async move {
        ...
        let txcx = contexts.txn();
        for point in [
            fgdb::CrashPoint::BeforeCapsule,
            fgdb::CrashPoint::AfterD1,
            ...
            assert!(
                transaction
                    .finish_with_crash(&mut database, &cx, Some(point))
                    .await
                    .is_err()
            );
            ...
            assert!(
                vertices.is_empty()
                    || vertices.iter().map(|row| row.vid).collect::<Vec<_>>()
                        == vec![VId(1), VId(2)]
            );
            if point != fgdb::CrashPoint::AfterMarkerBeforeD2 {
