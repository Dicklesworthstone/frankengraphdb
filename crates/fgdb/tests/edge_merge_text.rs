...
use fgdb_gql::{
    GqlParameters, GqlQueryPolicy, GraphEdgeMergeOutcome, GraphEdgeMergePolicy, GraphSymbol,
    ...
    let ((), report) = run_async_under_lab(0xed9e_2001, |root| async move {
        ...
        let (_, outcome, completion) = db
            .execute_graph_edge_merge_autocommit_governed(
                ...
                policy(),
                |_| -> Result<ElementId, ()> {
                    panic!("reverse spelling must find same relationship")
                },
            )
            .await
            .unwrap();
