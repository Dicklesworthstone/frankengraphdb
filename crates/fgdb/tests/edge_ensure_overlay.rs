...
fn ensure_noops_do_not_invent_edge_aliases_or_replace_existing_properties() {
    let ((), report) = run_async_under_lab(0xe050_0001, |root| async move {
        ...
        staged.ensure_edge_by_triple(
            EId(999),
            VId(1),
            VId(2),
            vec![(property, CanonicalScalar::Int(99))],
        );
        staged.ensure_edge_by_triple(
            EId(10),
            VId(1),
            VId(2),
            vec![(property, CanonicalScalar::Int(88))],
        );
        txn.write(&mut db, staged)
            .expect("both ensures resolve to the existing edge");
        assert!(txn.edge(&db, EId(999)).expect("unused alias").is_none());
        let expected = db.edge(EId(10)).expect("durable point").expect("edge");
        assert_eq!(
            txn.edge(&db, EId(10)).expect("overlay point"),
            Some(expected.clone())
        );
        ...
        let query = txn
            ...
            .expect("query");
        let result = txn
            .execute_prepared_query_budgeted(&db, &query, GqlExecutionBudget::new(1, 1))
...
fn deleting_the_old_triple_then_ensuring_materializes_only_the_real_successor() {
    let ((), report) = run_async_under_lab(0xe050_0002, |root| async move {
        ...
        assert_eq!(
            txn.edges(&db).expect("same canonical net table"),
            vec![point]
        );
        txn.commit(&mut db, &cx).await.expect("publish");
        assert!(
            db.edge(EId(999))
                .expect("alias after publication")
                .is_none()
        );
        assert_eq!(
            db.edge(EId(20))
