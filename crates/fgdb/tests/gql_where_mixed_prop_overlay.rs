...
#[test]
fn mixed_property_conjunction_sees_the_staged_pair_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x56_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-where-mixed-prop-overlay-{}",
            std::process::id()
        ));
        ...
        staged.create_vertex(VId(3), vec![], vec![(k, CanonicalScalar::Int(1))]);
        staged.create_vertex(VId(4), vec![], vec![(m, CanonicalScalar::Int(0))]);
