...
impl DriftingSource {
    fn new(preflight: Vec<BulkRow>, replay: Vec<BulkRow>) -> Self {
        Self { preflight: preflight.into(), replay: replay.into(), at: 0, audited: false }
    }
}
impl Clone for DriftingSource {
    fn clone(&self) -> Self {
        Self {
            preflight: self.preflight.clone(), replay: self.replay.clone(),
            at: self.at, audited: true,
        }
    }
}
impl Iterator for DriftingSource {
    ...
    fn next(&mut self) -> Option<BulkRow> {
        let rows = if self.audited { &self.preflight } else { &self.replay };
        let row = rows.get(self.at).cloned();
        if row.is_some() { self.at += 1; }
        row
    }
}
...
fn changed_vertex_fields_and_row_order_never_allocate_or_publish() {
    let ((), report) = run_async_under_lab(0xb51_01, |root| async move {
        ...
        for field in 0..5 {
            ...
            if field == 4 { changed.swap(0, 1); }
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let before = db.frontier().unwrap();
            let error = db.bulk_load(&cx, &commit, DriftingSource::new(expected, changed), policy())
                .await.unwrap_err();
            assert!(matches!(error.kind, BulkLoadErrorKind::SourceChanged { row: 0 }));
            assert_eq!(error.committed.next_row, 0);
            assert!(error.pending.is_none());
            assert_eq!(db.frontier().unwrap(), before);
            assert!(db.vertices().unwrap().is_empty());
            assert_eq!(db.allocate_identity(&cx, GraphInsertRequest::Vertex { row: 0, vertex: 0 }).unwrap(),
...
fn changed_edge_fields_never_publish_an_unchecked_chunk_or_panic_on_endpoints() {
    let ((), report) = run_async_under_lab(0xb51_02, |root| async move {
        ...
        for field in 0..8 {
            ...
            if field == 6 { changed.swap(2, 3); }
            if field == 7 { changed[2] = vertex("different-kind"); }
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let mut acknowledgements = 0;
            let error = db.bulk_load_with_checkpoint(
                &cx, &commit, DriftingSource::new(expected, changed), policy(), None,
                |_| { acknowledgements += 1; Ok(()) },
            ).await.unwrap_err();
            assert!(matches!(error.kind, BulkLoadErrorKind::SourceChanged { row: 2 }));
            ...
            assert_eq!(db.allocate_identity(&cx, GraphInsertRequest::Edge { row: 0, edge: 0 }).unwrap(),
...
fn every_truncation_and_appended_suffix_is_a_typed_refusal() {
    let ((), report) = run_async_under_lab(0xb51_03, |root| async move {
        ...
        let error = db.bulk_load(&cx, &commit, DriftingSource::new(expected, changed), policy())
            .await.unwrap_err();
        assert!(matches!(error.kind, BulkLoadErrorKind::SourceChanged { row: 4 }));
        assert_eq!(error.committed.next_row, 2, "check EOF before committing the last chunk");
        assert!(db.edges().unwrap().is_empty());
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let error = db.bulk_load(&cx, &commit,
            DriftingSource::new(vec![], vec![vertex("not-empty")]), policy()).await.unwrap_err();
        assert!(matches!(error.kind, BulkLoadErrorKind::SourceChanged { row: 0 }));
        assert!(db.vertices().unwrap().is_empty());
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn resume_verifies_skipped_source_prefix_and_can_continue_after_refusal() {
    let ((), report) = run_async_under_lab(0xb51_04, |root| async move {
        ...
        let original = rows();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let stopped = db.bulk_load_with_checkpoint(&cx, &commit, original.clone(), policy(), None,
            |_| Err(std::io::Error::other("stop after one durable chunk")))
            .await.unwrap_err();
        ...
        changed.swap(0, 1);
        let error = db.bulk_load(&cx, &commit, DriftingSource::new(original.clone(), changed), resumed.clone())
            .await.unwrap_err();
        assert!(matches!(error.kind, BulkLoadErrorKind::SourceChanged { row: 0 }));
        assert_eq!(error.committed.next_row, 2);
        assert_eq!(error.committed.frontier, stopped.committed.frontier);
        assert!(error.pending.is_none());
        let done = db.bulk_load(&cx, &commit, original.clone(), resumed).await.unwrap();
        ...
        assert_eq!(db.frontier().unwrap(), before, "completed resume cannot mint another marker");
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn canonical_scalars_and_length_delimited_keys_do_not_alias() {
    let ((), report) = run_async_under_lab(0xb51_05, |root| async move {
        ...
        let cx = contexts.query();
        let commit = contexts.commit();
        for replacement in [CanonicalScalar::Bool(true), CanonicalScalar::Null,
            CanonicalScalar::ucs_basic_text("7").unwrap(),
            CanonicalScalar::bytes(vec![7]).unwrap()] {
            let expected = vec![vertex("a\0bc")];
            let mut actual = expected.clone();
            if let BulkRow::Vertex(v) = &mut actual[0] { v.props[0].1 = replacement; }
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let error = db.bulk_load(&cx, &commit, DriftingSource::new(expected, actual), policy())
                .await.unwrap_err();
            assert!(matches!(error.kind, BulkLoadErrorKind::SourceChanged { row: 0 }));
            assert!(db.vertices().unwrap().is_empty());
        }
