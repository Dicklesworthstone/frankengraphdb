...
fn vertex(id: u128) -> QueryValue {
    QueryValue::Value(GraphValue::Vertex(VId(id)))
}
...
fn ids() -> QueryResult {
    result(&["id"], vec![vec![vertex(1)], vec![vertex(3)]])
}
...
fn session_and_prepared_reads_keep_one_generation_after_writes_and_writer_drop() {
    let ((), report) = run_async_under_lab(0x5ec0_5001, |root| async move {
        ...
        let prepared = session
            .prepare(
                &cx,
                "MATCH (n) WHERE n.p >= $min RETURN n AS id, n.p AS p",
                &params,
            )
            .unwrap();
        ...
        let old = result(
            &["id", "p"],
            vec![vec![vertex(1), value(7)], vec![vertex(3), value(19)]],
        );
        ...
        assert_eq!(
            ...
            result(
                &["id", "p"],
                vec![vec![vertex(1), value(70)], vec![vertex(3), value(19)]]
            )
        );
        assert!(matches!(
            session.query(
                &cx,
                &format!(
                    "MATCH (n) FOR SYSTEM_TIME AS OF SEQ {} RETURN n AS id",
                    future.0
                ),
                &GqlParameters::new()
            ),
        ...
        drop(db);
        drop(token); // Neither the writer nor bearer bytes are retained borrows.
        assert_eq!(session.execute(&cx, &prepared, &params).unwrap(), old);
        ...
        assert_eq!(
            fresh
                .query(&cx, "MATCH (n) RETURN n AS id", &GqlParameters::new())
                .unwrap(),
            ids()
        );
    });
...
fn exact_session_owns_prepared_catalogs_and_scope_cannot_cross_handles() {
    let ((), report) = run_async_under_lab(0x5ec0_5002, |root| async move {
        ...
        let resolver = |kind, name: &str| {
            calls.set(calls.get() + 1);
            symbols(kind, name)
        };
        let mut first = db
            .authorized_read_session(&cx, &issuer, &token, "main", resolver, policy(), || 100)
            .unwrap();
        let mut second = db
            .authorized_read_session(&cx, &issuer, &token, "main", symbols, policy(), || 100)
            .unwrap();
        let none = token
            .attenuate(fgdb_warden::Restriction::Labels(Scope::only([])))
            .unwrap();
        let mut denied = db
            .authorized_read_session(&cx, &issuer, &none, "main", symbols, policy(), || 100)
            .unwrap();
        let args = GqlParameters::new();
        let prepared = first
            .prepare(&cx, "MATCH (n:L) RETURN n AS id", &args)
            .unwrap();
        let after_prepare = calls.get();
        assert!(after_prepare > 0);
        for _ in 0..3 {
            assert_eq!(first.execute(&cx, &prepared, &args).unwrap(), ids());
        }
        assert_eq!(
            calls.get(),
            after_prepare,
            "execution must not re-resolve a prepared catalog"
        );
        assert!(matches!(
            second.execute(&cx, &prepared, &args),
            Err(QueryError::Authorization(Error::WrongAuthority))
        ));
        assert!(matches!(
            denied.execute(&cx, &prepared, &args),
            Err(QueryError::Authorization(Error::WrongAuthority))
        ));
        assert!(!second.is_closed());
        assert!(!denied.is_closed());
        assert_eq!(
            denied
                .query(&cx, "MATCH (n) RETURN n AS id", &args)
                .unwrap(),
            result(&["id"], vec![])
        );
        let masked = first
            .query(
                &cx,
                "MATCH (n) WHERE n.hidden IS NULL RETURN n AS id",
                &args,
            )
            .unwrap();
        ...
        assert_eq!(
            first
                .query(&cx, "MATCH (n:H) RETURN n AS id", &args)
                .unwrap(),
            result(&["id"], vec![])
        );
    });
...
fn branch_selectors_and_argument_rebinding_do_not_redirect_session_authority() {
    let ((), report) = run_async_under_lab(0x5ec0_5003, |root| async move {
        ...
        let prepared = session
            .prepare(
                &cx,
                "AT BRANCH $route MATCH (n) WHERE n.p >= $min RETURN n AS id",
                &params,
            )
            .unwrap();
        assert_eq!(prepared.parameter_schema().len(), 1);
        ...
        let other = GqlParameters::new()
            .with_text("route", "other")
            .unwrap()
            .with_int64("min", 0)
            .unwrap();
        assert!(matches!(
            session.execute(&cx, &prepared, &other),
        ...
        assert!(
            session
                .execute(
                    &cx,
                    &prepared,
                    &GqlParameters::new().with_int64("min", 0).unwrap()
                )
                .is_err()
        );
        ...
        let shared = session
            .prepare(&cx, "AT BRANCH $route RETURN $route AS route", &only_route)
            .unwrap();
        assert_eq!(
            session.execute(&cx, &shared, &only_route).unwrap(),
            result(
                &["route"],
                vec![vec![QueryValue::Value(GraphValue::Scalar(
                    CanonicalScalar::ucs_basic_text("main").unwrap()
                )),]]
            )
        );
        assert!(
...
fn fixed_native_and_signed_limits_span_each_complete_statement_not_each_input() {
    let ((), report) = run_async_under_lab(0x5ec0_5004, |root| async move {
        ...
        assert_eq!(
            session
                .query(&cx, "MATCH (n) RETURN COUNT(*) AS n", &args)
                .unwrap(),
            count
        );
        assert!(matches!(
            session.query(&cx, "MATCH (n) RETURN n AS id", &args),
        ...
        assert!(matches!(
            session.query(
                &cx,
                "MATCH (n) RETURN n AS id UNION ALL MATCH (n) RETURN n AS id",
                &args
            ),
            Err(QueryError::Authorization(Error::LimitExceeded(
        ...
        let mut small = db
            ...
            .unwrap();
        assert!(matches!(
            small.query(&cx, "MATCH (n) RETURN n AS id", &args),
...
fn authentication_precedes_callbacks_and_time_cannot_restart_a_closed_session() {
    let ((), report) = run_async_under_lab(0x5ec0_5005, |root| async move {
        ...
        now.set(300);
        session
            .query(&cx, "RETURN 1 AS n", &GqlParameters::new())
            .unwrap();
        now.set(200); // Across statements, not only inside one permit.
        assert!(matches!(
            session.query(&cx, "RETURN 1 AS n", &GqlParameters::new()),
        ...
        now.set(1000);
        assert!(matches!(
            session.query(&cx, "not valid GQL", &GqlParameters::new()),
    ...
    let ((), report) = run_async_under_lab(0x5ec0_5006, |root| async move {
        ...
        let mut session = db
            .authorized_read_session(
                ...
                &token,
                "main",
                |_, _: &str| -> Option<GraphSymbol> { panic!("injected host resolver unwind") },
                policy(),
                || 100,
            )
            .unwrap();
