...
#[test]
fn incoming_both_bang_inequalities_see_the_staged_pair_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x66_02, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-where-both-prop-both-bang-ne-overlay-{}",
            std::process::id()
        ));
        ...
        staged.add_edge(EId(11), VId(8), VId(7), vec![]);
