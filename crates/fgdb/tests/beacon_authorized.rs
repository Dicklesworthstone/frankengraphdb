...
use fgdb_beacon::{
    BeaconError, DistanceMetric, ExactHybridQuery, ExactRrfProfile, HnswConfig,
...
async fn database(cx: &CommitCx, hidden: bool) -> Database<MemVfs> {
    ...
    for (id, labels, word, value) in [
        ...
        if hidden { props.push((SECRET, text("invalid coordinate"))); }
    ...
    if hidden {
        ...
        batch.create_vertex(VId(116), vec![H], vec![
            (T, CanonicalScalar::Int(8)), (X, CanonicalScalar::Int(16_777_217)),
        ]);
    }
...
fn refused(result: Result<Rows, SearchError>, expected: Error) {
    match result {
        Err(ReadError::Interrupted(QueryError::Authorization(error))) => assert_eq!(error, expected),
...
fn hidden_documents_never_enter_bm25_statistics_ann_routing_or_fusion() {
    let ((), report) = run_async_under_lab(0xbeac_2001, |root| async move {
        ...
        for query in searches() {
            let rows = full.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &options(), query, || NOW).unwrap();
            let clean = scrubbed.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &options(), query, || NOW).unwrap();
            assert_eq!(rows, clean); // Includes BM25/distance/fusion values, not only IDs.
            assert_eq!(rows, scrubbed.beacon_search(&c.query(), &options(), query).unwrap());
            assert!(ids(&rows).iter().all(|id| id.0 < 100));
        }
        // The hidden invalid scalar really would fail unscoped execution.
        assert!(full.beacon_search(&c.query(), &options(), searches()[0]).is_err());
        assert!(full.beacon_search(&c.query(), &options(), searches()[1]).is_err());
    });
...
fn forbidden_properties_are_absent_before_type_checks_and_coordinate_assembly() {
    let ((), report) = run_async_under_lab(0xbeac_2002, |root| async move {
        ...
        for query in [searches()[0], searches()[3]] {
            let rows = full.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &o, query, || NOW).unwrap();
            assert_eq!(rows, clean.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &o, query, || NOW).unwrap());
            if let Rows::Text(rows) = rows { assert!(rows.is_empty()); }
        }
        o = options();
        o.projection.vector = vec![X, SECRET];
        o.index.vector.as_mut().unwrap().dimensions = 2;
        let query = Search::Vector { query: &[0.0, 0.0], k: 3, mode: VectorSearch::Exact };
        assert!(full.beacon_search_authorized(&c.query(), &issuer, &token,
            BRANCH, &o, query, || NOW).unwrap().is_empty());
        // A later positive scope cannot undo an explicit property denial.
        let denied = token.attenuate(Restriction::DenyProperties([T].into())).unwrap()
            .attenuate(Restriction::Properties(Scope::All)).unwrap();
        assert!(full.beacon_search_authorized(&c.query(), &issuer, &denied,
            BRANCH, &options(), searches()[0], || NOW).unwrap().is_empty());
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn original_label_clauses_and_masked_user_selection_are_separate() {
    let ((), report) = run_async_under_lab(0xbeac_2003, |root| async move {
        ...
        let token = issuer.issue_at(&grant(), NOW).unwrap();
        let both = token.attenuate(Restriction::Labels(Scope::only([H]))).unwrap();
        let rows = db.beacon_search_authorized(&c.query(), &issuer, &both,
            BRANCH, &options(), searches()[0], || NOW).unwrap();
        assert_eq!(ids(&rows), [VId(1)]); // L AND H, not intersection(L,H).
        for label in [L, H] {
            let mut o = options();
            o.vertex_label = Some(label);
            assert!(db.beacon_search_authorized(&c.query(), &issuer, &both,
                BRANCH, &o, searches()[0], || NOW).unwrap().is_empty());
        }
        let mut o = options();
        o.vertex_label = Some(H);
        assert!(db.beacon_search_authorized(&c.query(), &issuer, &token,
            BRANCH, &o, searches()[0], || NOW).unwrap().is_empty());
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn hidden_successors_do_not_resurrect_older_allowed_versions() {
    let ((), report) = run_async_under_lab(0xbeac_2004, |root| async move {
        ...
        let before: Vec<_> = searches().into_iter().map(|q|
            db.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &options(), q, || NOW).unwrap()).collect();
        ...
        for (q, expected) in searches().into_iter().zip(before) {
            assert_eq!(db.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &historical, q, || NOW).unwrap(), expected);
            let now = db.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &options(), q, || NOW).unwrap();
...
fn signed_allowances_cover_admission_native_work_and_actual_delivery() {
    let ((), report) = run_async_under_lab(0xbeac_2005, |root| async move {
        ...
        for (restriction, dimension) in [
            ...
            refused(db.beacon_search_authorized(&c.query(), &issuer, &restricted,
                BRANCH, &options(), searches()[1], || NOW), Error::LimitExceeded(dimension));
        }
        let exact = token.attenuate(Restriction::MaxNodes(3)).unwrap()
            .attenuate(Restriction::MaxRows(3)).unwrap();
        for q in searches() {
            assert!(db.beacon_search_authorized(&c.query(), &issuer, &exact,
                BRANCH, &options(), q, || NOW).is_ok());
        }
        ...
        refused(db.beacon_search_authorized(&c.query(), &issuer, &nodes,
            BRANCH, &none, searches()[0], || NOW), Error::LimitExceeded(LimitDimension::Nodes));
        let mut native = options();
        native.policy.max_work_units = 0;
        assert!(matches!(db.beacon_search_authorized(&c.query(), &issuer, &token,
            BRANCH, &native, searches()[0], || NOW), Err(ReadError::Index(BeaconError::WorkBudgetExceeded))));
        let zero = token.attenuate(Restriction::MaxRows(0)).unwrap();
        let q = Search::Text { query: "red", k: 0, mode: TextMatch::Any };
        assert!(db.beacon_search_authorized(&c.query(), &issuer, &zero,
            BRANCH, &options(), q, || NOW).unwrap().is_empty());
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn every_clock_boundary_including_empty_delivery_can_expire_without_results() {
    let ((), report) = run_async_under_lab(0xbeac_2006, |root| async move {
        ...
        for q in searches().into_iter().chain([
            ...
            db.beacon_search_authorized(&c.query(), &issuer, &token,
                BRANCH, &options(), q, || { count.set(count.get() + 1); NOW }).unwrap();
            assert!(count.get() > 10);
            for cut in 0..count.get() {
                let seen = Cell::new(0usize);
                refused(db.beacon_search_authorized(&c.query(), &issuer, &token,
                    BRANCH, &options(), q, || {
                        let at = seen.get();
                        seen.set(at + 1);
...
fn live_retirement_and_clock_rollback_retain_their_original_typed_causes() {
    let ((), report) = run_async_under_lab(0xbeac_2007, |root| async move {
        let c = PurposeContexts::narrow_runtime_root(&root);
        let db = database(&c.commit(), false).await;
        for q in [searches()[0], searches()[3], Search::Text { query: "absent", k: 3, mode: TextMatch::Any }] {
...
fn authentication_precedes_bad_frontier_or_data_and_does_not_grant_read_rights() {
    let ((), report) = run_async_under_lab(0xbeac_2008, |root| async move {
        ...
        let foreign = Authority::new(AuthKey::from_seed(2008), DatabaseSecurityNamespaceId([0x33; 32]),
            "host-graph", SchemaEpoch(0), 1).unwrap();
        refused(db.beacon_search_authorized(&c.query(), &foreign, &token,
            BRANCH, &o, searches()[0], || NOW), Error::WrongAuthority);
        let other_key = authority(2009);
        refused(db.beacon_search_authorized(&c.query(), &other_key, &token,
            BRANCH, &o, searches()[0], || NOW), Error::Unauthenticated);
        refused(db.beacon_search_authorized(&c.query(), &issuer, &token,
            "different-branch", &o, searches()[0], || NOW), Error::ScopeDenied);
        let write = token.attenuate(Restriction::Rights(Rights::Write)).unwrap();
        refused(db.beacon_search_authorized(&c.query(), &issuer, &write,
            BRANCH, &o, searches()[0], || NOW), Error::PermissionDenied);
        assert!(matches!(db.beacon_search_authorized(&c.query(), &issuer, &token,
            BRANCH, &o, searches()[0], || NOW), Err(ReadError::Read(_))));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
