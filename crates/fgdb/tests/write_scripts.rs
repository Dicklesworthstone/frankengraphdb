...
use fgdb_gql::{GqlParameters, GqlQueryPolicy, GraphEdgeMergeOutcome, GraphSymbol,
    GraphSymbolKind, GraphWriteProgramPolicy, PreparedGraphWriteScript};
...
fn arguments() -> GqlParameters {
    GqlParameters::new().with_int64("left", 1).unwrap().with_int64("right", 2).unwrap()
        .with_int64("created", 10).unwrap().with_int64("matched", 20).unwrap()
}
fn policy(edges: u64) -> GraphWriteProgramPolicy {
    GraphWriteProgramPolicy::new(GqlQueryPolicy::new(50_000, 50_000, 5_000_000, 5_000_000), 20, 2, edges)
}
...
fn native_script_creates_then_updates_the_same_graph_and_survives_reopen() {
    let ((), report) = run_async_under_lab(0x5c71_0001, |root| async move {
        ...
        assert_eq!(db.edge(EId(10)).unwrap().unwrap().props, vec![(Q, CanonicalScalar::Int(10))]);

        let (receipt, completion) = db.execute_graph_write_program_returning_autocommit_governed(
            &txcx, &query, &commit, &script.bind_parameters(&arguments()).unwrap(),
            GraphWriteProgramPolicy { max_created_vertices: 0, ..policy(0) },
...
fn late_script_failure_rolls_back_all_script_steps_but_preserves_outer_work() {
    let ((), report) = run_async_under_lab(0x5c71_0002, |root| async move {
        ...
        let result = txn.execute_graph_write_program_returning_governed(
            &mut db, &query, &program, policy(0), |request| {
                allocations += 1;
                assert!(request.statement < 2, "edge quota must refuse before allocation");
                Ok::<_, ()>(ElementId::Vertex(VId(request.statement as u128 + 1)))
            },
        );
        assert!(result.is_err());
        assert_eq!(allocations, 2, "allocated identities are not reclaimed by rollback");
