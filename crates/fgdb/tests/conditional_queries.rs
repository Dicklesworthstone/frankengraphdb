...
fn read_query() -> PreparedGraphSet {
    PreparedGraphSetText::prepare(
        &format!("MATCH (n:A) RETURN n,{BUCKET} AS bucket,{AMOUNT} AS amount ORDER BY n"),
        symbols,
    )
    .unwrap()
...
fn aggregate_query() -> PreparedGraphAggregate {
    PreparedGraphAggregateText::prepare(
        &format!(
            "MATCH (n:A) RETURN {BUCKET} AS bucket,COUNT(*) AS count,SUM({AMOUNT}) AS total, \
            AVG({AMOUNT}) AS mean GROUP BY {BUCKET} HAVING SUM({AMOUNT})>=-100 ORDER BY bucket"
        ),
        symbols,
    )
    .unwrap()
...
fn oracle(rows: &[VertexRow]) -> Vec<Plain> {
    rows.iter()
        .filter(|row| row.labels.contains(&A))
        .map(|row| {
...
fn cell(value: &GraphValue) -> i64 {
    let Some(CanonicalScalar::Int(value)) = value.as_scalar() else {
...
fn conditional_reads_groups_and_frozen_writes_survive_commit_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0xca5e_0001, |root| async move {
        ...
        let before_statement: BTreeMap<_, _> = expected_vertices
            .iter()
            .map(|row| (row.vid, property(row, P)))
            .collect();
        ...
        let stats = txn
            .execute_graph_mutation_governed(&mut db, &cx, &mutation, mutation_policy())
            .unwrap();
        assert_eq!(
            stats.selection.result_rows, 14,
            "duplicate WALK paths must not multiply assignments"
        );
        assert_eq!(stats.target_vertices, 5);
        assert_eq!(stats.effects, 10);
        ...
        assert_eq!(
            plain(
                &reopened
                    .execute_graph_set_governed_at(&cx, &read, basis, policy())
                    .unwrap()
                    .value
            ),
            old
        ...
        assert_eq!(
            grouped(
                &reopened
                    .execute_graph_aggregate_governed_at(&cx, &summary, basis, policy())
                    .unwrap()
                    .value
            ),
            oracle_groups(&old)
...
fn selected_branch_failure_and_refusals_preserve_the_complete_prior_workspace() {
    let ((), report) = run_async_under_lab(0xca5e_0002, |root| async move {
        ...
        prior.set_vertex_property(
            VId(3),
            Q,
            Some(CanonicalScalar::ucs_basic_text("private bad numeric input").unwrap()),
        );
        txn.write(&mut db, prior).unwrap();
        let before = txn.vertices(&db).unwrap();
        for (expression, expected_kind) in [
            (
                "CASE WHEN n.p IS NULL OR n.p=0 THEN 1 ELSE 10/(n.p-3) END",
                GraphIntegerErrorKind::DivisionByZero,
            ),
            (
                "CASE WHEN n.p=3 THEN 9223372036854775807+1 ELSE 0 END",
                GraphIntegerErrorKind::Overflow,
            ),
            (
                "CASE WHEN n.p=3 THEN n.q+1 ELSE 0 END",
                GraphIntegerErrorKind::NonInteger,
            ),
        ] {
            let query = PreparedGraphMutationText::prepare(
                &format!("MATCH (n:A) SET n.p={expression}"),
                R,
                symbols,
            )
            .unwrap()
            ...
            .unwrap();
            let result =
                txn.execute_graph_mutation_governed(&mut db, &cx, &query, mutation_policy());
            assert!(
                matches!(result, Err(GqlQueryError::Source(GraphMutationError::Arithmetic { error, .. }))
        ...
        let safe = PreparedGraphMutationText::prepare(
            "MATCH (n:A) SET n.p=CASE WHEN n.p IS NULL THEN 0 ELSE n.p+1 END",
            R,
            symbols,
        )
        .unwrap()
        ...
        .unwrap();
        for refusal in [
            GraphMutationPolicy::new(policy(), 4),
            GraphMutationPolicy::new(GqlQueryPolicy::new(10_000, 0, 10_000_000, 10_000_000), 100),
        ] {
            assert!(
                txn.execute_graph_mutation_governed(&mut db, &cx, &safe, refusal)
                    .is_err()
            );
            assert_eq!(txn.vertices(&db).unwrap(), before);
        }
        ...
        let lazy = PreparedGraphSetText::prepare(
            "MATCH (n:A) RETURN CASE WHEN TRUE THEN 7 ELSE n.q+1 END AS value",
            symbols,
        )
        .unwrap()
        .bind_parameters(&GqlParameters::new())
        .unwrap();
        assert_eq!(
            txn.execute_graph_set_governed(&db, &cx, &lazy, policy())
                .unwrap()
                .value
                .len(),
            5
        ...
        for mode in 0..5 {
            for winner in 0..3 {
                ...
                let query = PreparedGraphSetText::prepare(
                    &format!("MATCH (n:A) WHERE {predicate} RETURN {expression} AS value{tail}"),
                    symbols,
                )
                .unwrap()
                .bind_parameters(&GqlParameters::new())
                .unwrap();
                let allowance = if mode == 2 {
