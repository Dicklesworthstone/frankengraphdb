...
use fgdb_gql::{
    ...
    GraphAggregateRow, GraphIntegerBinary, GraphIntegerErrorKind, GraphIntegerExpression,
    GraphIntegerOp, GraphSetProjection, GraphSetValue, GraphWalkBounds, PreparedGraphAggregate,
...
fn aggregate(relational: bool) -> PreparedGraphAggregate {
    let declarations = [
        ...
        GraphAggregate::sum_int("values", 2),
    ];
    if relational {
        PreparedGraphAggregate::prepare_relation(input().into(), &[], &declarations, 0, None)
            .unwrap()
    } else {
        PreparedGraphAggregate::prepare(input(), &[], &declarations, 0, None).unwrap()
    }
}
...
fn input_pipeline_pages_preserve_owned_captured_keys_across_reopen() {
    let ((), report) = run_async_under_lab(0x31a6_0002, |root| async move {
        ...
        let plan = PreparedGraphAggregate::prepare_relation(
            relation,
            &[0],
        ...
        let paths: Vec<_> = old
            .iter()
            .map(|row| row.keys()[0].as_path().unwrap().steps().to_vec())
            .collect();
        assert_eq!(
            paths,
            vec![
                vec![(EId(11), VId(2)), (EId(13), VId(3))],
                vec![(EId(12), VId(2))]
            ]
        );
        db.compact(&commit).await.unwrap();
...
fn hidden_or_failed_path_summaries_retain_parallel_edge_phantom_observations() {
    let ((), report) = run_async_under_lab(0x31a6_0003, |root| async move {
        ...
        for mode in 0..3 {
            ...
            let plan = match mode {
                1 => bad_projection(),
                2 => PreparedGraphAggregate::prepare(
                    input(),
                    &[],
                    &[GraphAggregate::count_rows("n")],
                    0,
            ...
            if mode == 1 {
                ...
                assert_eq!(numbers(&result.unwrap().value), (5, 5, 7, 13));
            }
...
fn failed_path_aggregate_preserves_an_outer_write_that_can_still_commit() {
    let ((), report) = run_async_under_lab(0x31a6_0004, |root| async move {
        ...
        let before = txn.staged_effect_digest().unwrap();
        assert!(matches!(
            txn.execute_graph_aggregate_governed(&db, &cx, &bad_projection(), policy()),
...
fn source_route_and_group_costs_share_exact_limits_and_preflight_precedence() {
    let ((), report) = run_async_under_lab(0x31a6_0005, |root| async move {
        ...
        for relational in [false, true] {
            ...
            assert!(matches!(
                txn.execute_graph_aggregate_governed(&other, &cx, &plan, zero),
...
fn native_hidden_capture_count_executes_instead_of_refusing_identified_edges() {
    let ((), report) = run_async_under_lab(0x31a6_0006, |root| async move {
        ...
        for mode in [
            ...
            let text = format!(
                "MATCH route = {mode} (a)-[:R*1..2]->(b) WHERE a.p=1 RETURN COUNT(*) AS paths"
            );
