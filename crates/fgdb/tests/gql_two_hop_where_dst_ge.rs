//! **Two-hop `WHERE c.k >= 1` keeps inclusive greater far ends**
//! (`fgdb-w5-parsers-nje.38`).
//!
//! The non-strict comparator at the composed path's far end, on the
//! four-chain fixture where `>=` cannot cheat as `>` or `<>`: the
//! boundary far end (`k = 1`) is IN — the exact row a renamed `>` drops —
//! while the `k = 0` far end is OUT — the exact row a `<>` cheat admits —
//! and the keyless far end satisfies no comparator at all. Each of those
//! three is asserted by name on top of the exact `[3, 6]`. The strict
//! `>`, strict `<`, and equality siblings are re-pinned beside it (`>=`
//! visibly the union of `=` and `>`), and three refusals hold the edges:
//! `<=` and the C-style `!=` on `c` stay Parse, and a WHERE on the
...
#[test]
fn two_hop_far_end_greater_or_equal_includes_the_boundary_chain() {
    let ((), report) = run_async_under_lab(0x38_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-two-hop-where-dst-ge-{}",
            std::process::id()
        ));
        ...
        let inclusive = db
            .execute_gql(DST_GE, &bind)
            .expect("far-end greater-or-equal MATCH executes");
        assert_eq!(
            inclusive,
            vec![VId(3), VId(6)],
            "boundary AND above-boundary far ends answer"
        );
        assert!(
        ...
        assert!(
            !inclusive.contains(&VId(12)),
            "the k=0 far end is OUT: 0 >= 1 is false — a <> cheat admits it"
        );
        assert!(
        ...
        assert_eq!(
            ...
            vec![VId(6)],
            "nje.36 unmoved: >= is visibly the union of = and >"
        );
        assert_eq!(
            db.execute_gql(DST_LT, &bind)
                .expect("the strict-less sibling still executes"),
            vec![VId(12)],
            "nje.37 unmoved beside the new spelling"
        );
        assert_eq!(
