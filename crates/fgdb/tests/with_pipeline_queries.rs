...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(100_000, 10_000, 20_000_000, 20_000_000) }
...
fn ranked() -> PreparedGraphSet {
    query("MATCH (n) WITH n AS owner,n.p+1 AS score ORDER BY score DESC LIMIT 2 WHERE score>2 RETURN owner,score*10 AS rank ORDER BY owner")
}
fn pairs(rows: &[GraphValueRow]) -> Vec<(VId, i64)> {
    rows.iter().map(|row| {
        let Some(CanonicalScalar::Int(score)) = row.get(1).unwrap().as_scalar() else { panic!("integer rank") };
...
fn ranked_pipeline_pins_history_and_survives_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0x71f0_0001, |root| async move {
        ...
        assert_eq!(pairs(&db.execute_graph_set_governed(&cx, &query, policy()).unwrap().value), original);
        let latest = db.write(&commit, change(1, 9)).await.unwrap();
        let current = vec![(VId(1), 100), (VId(3), 40)];
        assert_eq!(pairs(&db.execute_graph_set_governed(&cx, &query, policy()).unwrap().value), current);
        assert_eq!(pairs(&pinned.execute_graph_set_governed(&cx, &query, policy()).unwrap().value), original);
        assert_eq!(pairs(&db.execute_graph_set_governed_at(&cx, &query, old, policy()).unwrap().value), original);
        assert!(matches!(pinned.execute_graph_set_governed_at(&cx, &query, latest, GqlQueryPolicy::new(0, 0, 0, 0)),
...
fn pipeline_reads_canonical_staged_values_without_publishing_or_changing_effects() {
    let ((), report) = run_async_under_lab(0x71f0_0002, |root| async move {
        ...
        let first = txn.execute_graph_set_governed(&db, &cx, &query, policy()).unwrap();
        assert_eq!(pairs(&first.value), vec![(VId(1), 100), (VId(3), 40)]);
        assert_eq!(txn.execute_graph_set_governed(&db, &cx, &query, policy()).unwrap(), first);
        assert_eq!(txn.staged_effect_digest().unwrap(), digest);
        assert_eq!(pairs(&db.execute_graph_set_governed(&cx, &query, policy()).unwrap().value),
            vec![(VId(2), 30), (VId(3), 40)]);
        txn.abort();
        assert_eq!(db.frontier().unwrap(), frontier);
        let mut reader = db.begin(&txcx).unwrap();
        reader.execute_graph_set_governed(&db, &cx, &query, policy()).unwrap();
        reader.finish(&mut db, &commit).await.unwrap();
        assert_eq!(db.frontier().unwrap(), frontier, "read-only completion must not publish a marker");
...
fn filtered_empty_and_limited_rows_keep_transaction_conflict_witnesses() {
    let ((), report) = run_async_under_lab(0x71f0_0003, |root| async move {
        ...
        for text in [
            ...
            let mut txn = db.begin(&txcx).unwrap();
            txn.execute_graph_set_governed(&db, &cx, &query(text), policy()).unwrap();
...
fn relational_adapters_share_all_caps_and_validate_lifecycle_before_zero_budget() {
    let ((), report) = run_async_under_lab(0x71f0_0004, |root| async move {
        ...
        assert_eq!(db.execute_graph_set_governed(&cx, &query,
            GqlQueryPolicy::new(caps[0], caps[1], caps[2], caps[3])).unwrap(), measured);
        for dimension in 0..4 {
            let mut limit = caps; limit[dimension] -= 1;
            assert!(db.execute_graph_set_governed(&cx, &query,
                GqlQueryPolicy::new(limit[0], limit[1], limit[2], limit[3])).is_err());
            assert_eq!(db.frontier().unwrap(), frontier);
        }
        let zero = GqlQueryPolicy::new(0, 0, 0, 0);
        assert!(matches!(db.execute_graph_set_governed_at(&cx, &query, CommitSeq(frontier.0 + 1), zero),
            Err(GqlQueryError::Source(GraphSetExecutionError::Source(GqlError::Read(ReadError::BeyondFrontier { .. }))))));
        ...
        assert!(matches!(finished.execute_graph_set_governed(&db, &cx, &query, zero),
            Err(GqlQueryError::Source(GraphSetExecutionError::Source(WriteTxnError::Finished)))));
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn late_arithmetic_failure_preserves_outer_effects_and_observed_reads() {
    let ((), report) = run_async_under_lab(0x71f0_0005, |root| async move {
        ...
        assert!(matches!(txn.execute_graph_set_governed(&db, &cx, &query, policy()),
...
fn parallel_edges_keep_bag_multiplicity_through_with_and_union() {
    let ((), report) = run_async_under_lab(0x71f0_0006, |root| async move {
        ...
        for (text, expected) in [
            ("MATCH (a)-[:R]->(b) WITH b.p AS score RETURN score", vec![2, 2, 3]),
            ("MATCH (a)-[:R]->(b) WITH DISTINCT b.p AS score RETURN score", vec![2, 3]),
            ("MATCH (a)-[:R]->(b) WITH b.p AS score WHERE score=2 RETURN score UNION ALL MATCH (n) WITH n.p AS score WHERE score=3 RETURN score", vec![2, 2, 3]),
        ] {
            let rows = db.execute_graph_set_governed(&cx, &query(text), policy()).unwrap().value;
            let actual = rows.iter().map(|row| match row.get(0).unwrap().as_scalar() {
                Some(CanonicalScalar::Int(value)) => *value, _ => panic!("integer score"),
            }).collect::<Vec<_>>();
