...
fn policy() -> GraphWriteProgramPolicy {
    GraphWriteProgramPolicy::new(GqlQueryPolicy::new(100_000, 100_000, 10_000_000, 10_000_000), 10_000, 10_000, 10_000)
}
...
fn identity(request: GraphWriteIdentityRequest) -> Result<ElementId, &'static str> {
    ...
    Ok(match request.request {
        GraphInsertRequest::Vertex { row, vertex } => ElementId::Vertex(VId(1_000 + at + row as u128 * 100 + vertex as u128)),
        GraphInsertRequest::Edge { row, edge } => ElementId::Edge(EId(100_000 + at + row as u128 * 100 + edge as u128)),
    })
}
...
fn dependent_creations_updates_and_deletion_publish_one_exact_graph() {
    let ((), report) = run_async_under_lab(0x6d17_0001, |root| async move {
        ...
        let before = db.vertices().unwrap(); let before_edges = db.edges().unwrap();
        ...
        let stats = txn.execute_graph_write_program_governed(&mut db, &cx, &query, policy(), |request| {
            requests.borrow_mut().push(request); identity(request)
        }).unwrap();
        assert_eq!((stats.completed_statements, stats.selection.result_rows), (5, 6));
        assert_eq!((stats.created_vertices, stats.created_edges, stats.mutation_effects, stats.target_vertex_visits), (4, 2, 3, 3));
        assert_eq!(stats.proposed_effects(), 9, "deleting a transient creation does not refund proposals");
        assert_eq!(requests.borrow().len(), 6);
        let mut expected = txn.vertices(&db).unwrap();
        let mut expected_edges = txn.edges(&db).unwrap();
        let facts: Vec<_> = expected.iter().map(|row| (row.vid, row.labels.clone(), row.props.clone())).collect();
        assert_eq!(facts, vec![
        ...
        let topology: Vec<_> = expected_edges.iter().map(|row| (row.entry.eid, row.entry.src, row.entry.dst)).collect();
        assert_eq!(topology, vec![(EId(11), VId(1), VId(2)), (EId(12), VId(1), VId(2)), (EId(120000), VId(1001), VId(21000))]);
        assert_eq!(db.vertices().unwrap(), before);
        assert_eq!(db.edges().unwrap(), before_edges);
        let EmbeddedTxnCompletion::WriteCommitted { commit_seq } = txn.finish(&mut db, &commit).await.unwrap() else { panic!("write completion") };
        assert_eq!(commit_seq.0, basis.0 + 1);
        for row in &mut expected {
            if [VId(777), VId(1001), VId(21000), VId(41000)].contains(&row.vid) {
                assert_eq!(row.created_at, basis); row.created_at = commit_seq;
            }
        }
        for row in &mut expected_edges {
            if row.entry.eid == EId(120000) { assert_eq!(row.entry.created_at, basis); row.entry.created_at = commit_seq; }
        }
...
fn late_failures_restore_cascades_properties_and_the_prior_prefix_without_reclaiming_ids() {
    let ((), report) = run_async_under_lab(0x6d17_0002, |root| async move {
        ...
        for mode in 0..5 {
            ...
            let last = if mode == 0 { "MATCH (n:Base) SET n.p=1/0" }
                else { "CREATE (tail:Tail {p:99})-[:R]->(other:Tail {p:100})" };
            let query = program(&[
            ...
            if mode == 3 {
                let mut measuring = db.begin(&txcx).unwrap(); measuring.write(&mut db, prefix()).unwrap();
                let measured = measuring.execute_graph_write_program_governed(&mut db, &cx, &query, allowance, identity).unwrap();
                measuring.abort();
                allowance.mutations.query.evaluator.max_work_units = measured.evaluator.work_units - 1;
            }
            let mut txn = db.begin(&txcx).unwrap(); txn.write(&mut db, prefix()).unwrap();
            let before = txn.staged_effect_digest().unwrap();
            let requests = RefCell::new(Vec::new());
            let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                txn.execute_graph_write_program_governed(&mut db, &cx, &query, allowance, |request| {
                    requests.borrow_mut().push(request);
                    if request.statement == 3 {
                        if mode == 1 && matches!(request.request, GraphInsertRequest::Edge { .. }) { return Err("late identity refusal"); }
                        if mode == 2 && matches!(request.request, GraphInsertRequest::Vertex { vertex: 1, .. }) { return Ok(ElementId::Vertex(VId(2))); }
                        assert!(mode != 4, "injected allocator unwind");
                    }
                    identity(request)
                })
            }));
            if mode == 4 { assert!(outcome.is_err()); } else { assert!(outcome.unwrap().is_err(), "mode {mode}"); }
            assert!(requests.borrow().len() >= 3, "earlier CREATE issued real identities");
            ...
            assert_eq!(txn.vertex(&db, VId(2)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(20))]);
            assert_eq!(txn.vertices(&db).unwrap().len(), 3);
            txn.commit(&mut db, &commit).await.unwrap();
            assert!(db.vertex(VId(777)).unwrap().is_some());
            assert_eq!(db.edges().unwrap(), old_edges);
            assert_eq!(db.vertices().unwrap().len(), 3, "only the pre-program write commits");
            assert_eq!(txcx.outstanding_obligations(), 0);
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
    ...
    let ((), report) = run_async_under_lab(0x6d17_0003, |root| async move {
        ...
        for mode in 0..4 {
            for winner_kind in 0..3 {
                let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit).await;
                let last = if mode == 2 { "MATCH (n:Base) SET n.p=1/0" } else { "CREATE (tail:Tail {p:1})" };
                let query = program(&["CREATE (x:New {p:1})", "MATCH (n:Base) WHERE n.p<0 SET n.p=0", last]);
                let mut allowance = policy();
                if mode == 3 {
                    let mut measuring = db.begin(&txcx).unwrap(); measuring.write(&mut db, prefix()).unwrap();
                    let measured = measuring.execute_graph_write_program_governed(&mut db, &cx, &query, allowance, identity).unwrap();
                    measuring.abort(); allowance.mutations.query.evaluator.max_work_units = measured.evaluator.work_units - 1;
                }
                let mut txn = db.begin(&txcx).unwrap(); txn.write(&mut db, prefix()).unwrap();
                let result = txn.execute_graph_write_program_governed(&mut db, &cx, &query, allowance, |request| {
                    if mode == 1 && request.statement == 2 { Err("allocator unavailable") } else { identity(request) }
                });
                ...
                match winner_kind {
                    ...
                    1 => { winner.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(-1))); }
                    _ => { winner.create_vertex(VId(888), vec![BASE], vec![(P, CanonicalScalar::Int(-1))]); }
                }
...
fn preflight_fences_win_over_zero_quotas_and_allocator_side_effects() {
    let ((), report) = run_async_under_lab(0x6d17_0004, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let result = txn.execute_graph_write_program_governed(&mut foreign, &cx, &query, zero, |request| { calls.set(calls.get() + 1); identity(request) });
        assert!(matches!(result, Err(GraphWriteProgramError::Program(GraphMutationProgramError::Preflight(WriteTxnError::WrongDatabase)))));
        assert_eq!(txn.state(), EmbeddedTxnState::Active); txn.abort();
        let mut txn = db.begin(&txcx).unwrap();
        let mut other_relation = WriteBatch::new(RelationId(2)); other_relation.create_vertex(VId(99), vec![], vec![]);
        txn.write(&mut db, other_relation).unwrap();
        let result = txn.execute_graph_write_program_governed(&mut db, &cx, &query, zero, |request| { calls.set(calls.get() + 1); identity(request) });
        assert!(matches!(result, Err(GraphWriteProgramError::Program(GraphMutationProgramError::Preflight(WriteTxnError::RelationMismatch { .. })))));
        txn.abort();
        let mut txn = db.begin(&txcx).unwrap();
        db.write(&commit, prefix()).await.unwrap();
        let result = txn.execute_graph_write_program_governed(&mut db, &cx, &query, zero, |request| { calls.set(calls.get() + 1); identity(request) });
        assert!(matches!(result, Err(GraphWriteProgramError::Program(GraphMutationProgramError::Preflight(WriteTxnError::SnapshotAdvanced { .. })))));
        txn.abort();
        let mut txn = db.begin(&txcx).unwrap(); txn.finish(&mut db, &commit).await.unwrap();
        let result = txn.execute_graph_write_program_governed(&mut db, &cx, &query, zero, |request| { calls.set(calls.get() + 1); identity(request) });
        assert!(matches!(result, Err(GraphWriteProgramError::Program(GraphMutationProgramError::Preflight(WriteTxnError::Finished)))));
        assert_eq!(calls.get(), 0); assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn deleting_a_transient_creation_does_not_license_identity_revival() {
    let ((), report) = run_async_under_lab(0x6d17_0005, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap(); txn.write(&mut db, prefix()).unwrap();
        let before = txn.staged_effect_digest().unwrap();
        let query = program(&["CREATE (x:New {p:1})", "MATCH (x:New) DETACH DELETE x", "CREATE (y:New {p:2})"]);
        let result = txn.execute_graph_write_program_governed(&mut db, &cx, &query, policy(),
            |_| Ok::<_, &'static str>(ElementId::Vertex(VId(1000))));
