...
use fgdb_gql::{
    ...
    GraphMutationProgramDimension as Dimension, GraphMutationProgramError as Error, GraphSymbol,
    GraphSymbolKind, PreparedGraphMutationProgram, PreparedGraphMutationText,
};
use fgdb_types::{
...
fn program_at(statements: &[&str], relation: RelationId) -> PreparedGraphMutationProgram {
    PreparedGraphMutationProgram::prepare(
        statements
            .iter()
            .map(|text| {
                PreparedGraphMutationText::prepare(text, relation, |kind, name| {
                    match (kind, name) {
...
fn dependent_steps_see_prior_effects_then_publish_one_commit_with_reopen_and_history() {
    let ((), report) = run_async_under_lab(0xb10c_1001, |root| async move {
        ...
        let stats = txn
            .execute_graph_mutation_program_governed(&mut db, &query, &program, policy())
            .unwrap();
...
fn later_arithmetic_conflict_and_quota_errors_restore_all_earlier_program_steps() {
    let ((), report) = run_async_under_lab(0xb10c_1002, |root| async move {
        ...
        for mode in 0..4 {
            ...
            if mode == 2 {
                budget.max_effects = 6;
            }
            if mode == 3 {
                budget.query = GqlQueryPolicy::new(100_000, 3, 10_000_000, 10_000_000);
            }
            let result =
                txn.execute_graph_mutation_program_governed(&mut db, &query, &program, budget);
            match mode {
                0 => assert!(matches!(
                    result,
                    Err(Error::Statement {
                ...
                _ => assert!(matches!(
                    ...
                    Err(Error::Budget {
                        statement: 1,
                        dimension: Dimension::SelectedRows,
                        ..
                    })
                )),
            }
            ...
            for row in db.vertices().unwrap() {
                ...
                if row.vid != VId(777) {
                    assert_eq!(
                        property(&row, P),
                        Some(&CanonicalScalar::Int(row.vid.0 as i64 * 10))
                    );
                }
            }
        }
...
fn exact_cumulative_limits_and_the_final_acceptance_boundary_precede_workspace_visibility() {
    let ((), report) = run_async_under_lab(0xb10c_1003, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        assert_eq!(
            txn.execute_graph_mutation_program_governed(&mut db, &query, &program, exact)
                .unwrap(),
            stats
        );
        txn.abort();
        for (dimension, budget) in [
            (
                Dimension::SnapshotRecords,
                GraphMutationPolicy::new(
                    GqlQueryPolicy::new(
                        stats.selection.snapshot_records - 1,
                        u64::MAX,
                        u64::MAX,
                        u64::MAX,
                    ),
            ...
            (
                Dimension::SelectedRows,
                GraphMutationPolicy::new(
                    GqlQueryPolicy::new(
                        u64::MAX,
                        stats.selection.result_rows - 1,
                        u64::MAX,
                        u64::MAX,
                    ),
                    u64::MAX,
            ...
            (
                Dimension::ScratchEntries,
                GraphMutationPolicy::new(
                    GqlQueryPolicy::new(
                        u64::MAX,
                        u64::MAX,
                        u64::MAX,
                        stats.evaluator.scratch_entries - 1,
                    ),
                    u64::MAX,
                ),
            ),
            ...
            match result {
                Err(Error::Budget {
                    ...
                    if dimension == Dimension::WorkUnits {
                        assert_eq!(statement, 2, "both stages completed before final refusal");
                    }
                }
...
fn empty_steps_continue_and_owner_lifecycle_basis_and_relation_checks_win_over_zero_budget() {
    let ((), report) = run_async_under_lab(0xb10c_1004, |root| async move {
        ...
        seed(&mut db, &commit).await;
        let plan = program(&[
            "MATCH (n) WHERE n.p<0 SET n.q=9",
            "MATCH (n) WHERE n.p>=10 SET n.q=1",
        ]);
        let mut txn = db.begin(&txcx).unwrap();
        let stats = txn
            .execute_graph_mutation_program_governed(&mut db, &query, &plan, policy())
            .unwrap();
        assert_eq!((stats.completed_statements, stats.effects), (2, 2));
        txn.commit(&mut db, &commit).await.unwrap();
        let zero = GraphMutationPolicy::new(GqlQueryPolicy::new(0, 0, 0, 0), 0);
        assert!(matches!(
            txn.execute_graph_mutation_program_governed(&mut db, &query, &plan, zero),
            Err(Error::Preflight(WriteTxnError::Finished))
        ));
        let mut foreign = Database::open_memory(&commit, keys()).await.unwrap();
        let mut txn = db.begin(&txcx).unwrap();
        assert!(matches!(
            txn.execute_graph_mutation_program_governed(&mut foreign, &query, &plan, zero),
            Err(Error::Preflight(WriteTxnError::WrongDatabase))
        ));
        txn.write(&mut db, prefix()).unwrap();
        let different = program_at(&["MATCH (n) SET n.q=1"], RelationId(2));
        assert!(matches!(
            txn.execute_graph_mutation_program_governed(&mut db, &query, &different, zero),
