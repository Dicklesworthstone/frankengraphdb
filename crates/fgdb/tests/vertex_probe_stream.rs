...
fn atom(target: &str, d: GlaDirection) -> String {
    match d {
        GlaDirection::Forward => format!("-[:R]->({target})"),
...
fn statement(d: GlaDirection, anti: bool, cycle: bool, cut: Option<CommitSeq>) -> String {
    let temporal = cut
        ...
        .unwrap_or_default();
    format!(
        "MATCH (a){temporal} WHERE {}EXISTS {{ MATCH (a){}{} WHERE x.p > 5 }} RETURN a, a.p AS p",
        if anti { "NOT " } else { "" },
        atom("x", d),
        if cycle { atom("a", d) } else { String::new() }
    )
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (at, id) in IDS[..5].iter().copied().enumerate() {
        let props = match at {
            0 => vec![(P, CanonicalScalar::Int(1))],
            1 => vec![(P, CanonicalScalar::Null)],
            2 => vec![(P, CanonicalScalar::Int(7))],
            3 => vec![(P, CanonicalScalar::Int(-2))],
            _ => vec![],
        };
...
fn oracle(
    ...
    let relation: Vec<_> = edges
        ...
        .flat_map(|e| orient(e.entry.src, e.entry.dst, d))
        .collect();
    let mut result = Vec::new();
    for a in IDS {
        if db.vertex_at(a, cut).unwrap().is_none() {
            continue;
        }
        let mut witnesses = 0;
        for &(from, x) in &relation {
            if from != a || !matches!(field(db,cut,x),CanonicalScalar::Int(v) if v > 5) {
                continue;
            }
            if cycle {
                for &(via, end) in &relation {
                    if via == x && end == a {
                        witnesses += 1;
                    }
                }
        ...
        if (witnesses > 0) != anti {
            result.push(vec![
                GraphValue::Vertex(a),
                GraphValue::Scalar(field(db, cut, a)),
            ]);
        }
    }
    result.sort();
...
fn vertex_probe_truth_is_snapshot_exact_and_preserves_isolates_across_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0x7670_0001, |root| async move {
        ...
        let q = prepare(&input);
        let old = oracle(&db, basis, GlaDirection::Forward, true, false);
            ...
            for d in [
                ...
                for anti in [false, true] {
                    for cycle in [false, true] {
                        let want = oracle(&db, cut, d, anti, cycle);
                        ...
                        let mut direct = db
                            .stream_graph_values_governed_at(&cx, &q, cut, policy())
                            .unwrap();
                        assert_eq!(
                            plain(&direct.by_ref().collect::<Result<Vec<_>, _>>().unwrap()),
                            want
                        );
                        let native = PreparedNativeRead::prepare(
                            &statement(d, anti, cycle, Some(cut)),
                            &params,
                            symbols,
                        )
                        .unwrap();
                        let (columns, mut stream) =
                            native.stream(&db, &cx, &params, policy()).unwrap();
    ...
    let ((), report) = run_async_under_lab(0x7670_0002, |root| async move {
        ...
        for input in [
            ...
            let expected = db
                ...
                .value;
            let native =
                PreparedNativeRead::prepare(input, &GqlParameters::new(), symbols).unwrap();
        ...
        assert!(matches!(
            native.stream(&db, &cx, &GqlParameters::new(), policy()),
        ...
        assert_eq!(
            low.by_ref().collect::<Result<Vec<_>, _>>().unwrap().len(),
            1
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn one_root_and_one_witness_ignore_unrelated_edges_and_limit_zero_never_reads_the_source() {
    let ((), report) = run_async_under_lab(0x7670_0003, |root| async move {
        ...
        for i in 0..5 {
            seed.create_vertex(VId(i), vec![], vec![]);
        }
        seed.add_edge(EId(0), VId(0), VId(1), vec![]);
        for i in 1..=2048 {
            seed.add_edge(EId(i), VId(2), VId(3), vec![]);
        }
        ...
        let (_, mut stream) = native
            .stream(&db, &cx, &GqlParameters::new(), small)
            .unwrap();
        assert_eq!(
            stream.next().unwrap().unwrap().values(),
            &[GraphValue::Vertex(VId(0))]
        );
        assert_eq!(stream.row_stats().snapshot_records, 2);
        assert!(stream.next().is_none());
        let negative =
            prepare("MATCH (a) WHERE NOT EXISTS { MATCH (a)-[:R]->(a) } RETURN a LIMIT 1");
        let mut bad = db
            .stream_graph_values_governed(&cx, &negative, GqlQueryPolicy::new(1, 1, 10_000, 10_000))
            .unwrap();
        assert!(matches!(bad.next(), Some(Err(GqlQueryError::Rows(_)))));
        assert_eq!(bad.state(), VertexScanState::Failed);
        assert_eq!(bad.row_stats().result_rows, 0);
        assert!(bad.next().is_none());
        let mut retry = db
            .stream_graph_values_governed(&cx, &negative, small)
            .unwrap();
        assert_eq!(
            retry.next().unwrap().unwrap().values(),
            &[GraphValue::Vertex(VId(0))]
        );
        assert_eq!(retry.row_stats().snapshot_records, 2);
        let zero = prepare("MATCH (a) WHERE NOT EXISTS { MATCH (a)-[:R]->(x) } RETURN a LIMIT 0");
        let mut cursor = db
            .stream_graph_values_governed(&cx, &zero, GqlQueryPolicy::new(0, 0, 1, 0))
            .unwrap();
...
fn exact_cumulative_limits_and_admission_precedence_survive_vertex_probe_dispatch() {
    let ((), report) = run_async_under_lab(0x7670_0004, |root| async move {
        ...
        let input = statement(GlaDirection::Undirected, true, false, None);
        let q = prepare(&input);
        ...
        for bad in [
            ...
            let mut stream = db.stream_graph_values_governed(&cx, &q, bad).unwrap();
        ...
        assert!(matches!(
            db.stream_graph_values_governed_at(&cx, &unavailable, CommitSeq(basis.0 + 1), none),
        ...
        let view = db.read_session().unwrap();
        assert!(matches!(
            view.stream_graph_values_governed_at(&cx, &q, CommitSeq(basis.0 + 1), none),
            Err(GqlQueryError::Source(VertexScanError::Source(
                ReadError::BeyondFrontier { .. }
            )))
        ));
