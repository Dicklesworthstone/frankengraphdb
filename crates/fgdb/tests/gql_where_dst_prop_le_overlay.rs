...
#[test]
fn destination_property_less_or_equal_includes_boundary_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x67_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-where-dst-prop-le-overlay-{}",
            std::process::id()
        ));
        ...
        staged.create_vertex(VId(6), vec![], vec![(key, CanonicalScalar::Int(0))]);
        ...
        assert_eq!(
            txn.execute_gql(&db, filtered, &bind)
                .expect("overlay less-or-equal MATCH executes"),
            vec![VId(1), VId(5)]
        );
        assert_eq!(
            db.execute_gql(filtered, &bind)
                .expect("base less-or-equal MATCH executes"),
            vec![VId(1)]
        );
        txn.abort();
    });
