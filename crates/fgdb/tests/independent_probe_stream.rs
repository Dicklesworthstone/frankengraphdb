...
fn statement(
    ...
    let pattern = match direction {
        GlaDirection::Forward => "(x)-[:Q]->(y)",
        GlaDirection::Reverse => "(x)<-[:Q]-(y)",
        GlaDirection::Undirected => "(x)-[:Q]-(y)",
    };
    ...
    let time = cut
        ...
        .unwrap_or_default();
    format!(
        "MATCH {}{time} WHERE {}EXISTS {{ {body} }} RETURN {}",
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (i, &id) in IDS[..5].iter().enumerate() {
        let value = match i {
            0 | 1 => CanonicalScalar::Int(1),
            2 => CanonicalScalar::Null,
            3 => CanonicalScalar::Int(7),
            _ => CanonicalScalar::Int(-2),
        };
...
fn oracle(
    ...
    let relation: Vec<_> = edges
        .iter()
        .filter(|e| e.entry.relation == Q)
        .flat_map(|e| {
        ...
        if shape == 2 {
            return IDS
                ...
                .filter(|&&id| {
                    db.vertex_at(id, cut)
                        .unwrap()
                        .is_some_and(|v| v.labels.contains(&L))
                })
                .count();
        }
    ...
    let mut rows: Vec<_> = if edge {
        edges
            .iter()
            .filter(|e| e.entry.relation == R && (witnesses(e.entry.src) > 0) != anti)
            .map(|e| {
                vec![
        ...
        IDS.iter()
            .filter(|&&a| db.vertex_at(a, cut).unwrap().is_some() && (witnesses(a) > 0) != anti)
...
fn independent_native_queries_preserve_history_captures_isolates_and_paused_pins() {
    let ((), report) = run_async_under_lab(0x696e_6401, |root| async move {
        ...
        for edge in [false, true] {
            ...
            paused.push((text, expected, first, cursor));
        }
        let mut edit = WriteBatch::new(Q);
        edit.delete_edge(EId(10));
        ...
        edit.set_vertex_property(IDS[2], P, Some(CanonicalScalar::Int(8)));
        let changed = db.write(&commit, edit).await.unwrap();
        let mut edit = WriteBatch::new(R);
        edit.delete_vertex(IDS[2]);
        let cascaded = db.write(&commit, edit).await.unwrap();
        let mut edit = WriteBatch::new(Q);
        edit.delete_vertex(IDS[4]);
        let no_isolate = db.write(&commit, edit).await.unwrap();
        let mut edit = WriteBatch::new(R);
        edit.create_vertex(IDS[5], vec![L], vec![]);
            ...
            for edge in [false, true] {
                for d in [
                    ...
                    for anti in [false, true] {
                        for shape in 0..3 {
                            let expected = oracle(&db, cut, edge, d, anti, shape);
                            let q = prepare(&statement(edge, d, anti, shape, None));
                            let eager = db
                                .execute_graph_pattern_governed_at(&cx, &q, cut, wide())
                                .unwrap()
                                .value;
                            assert_eq!(plain(&eager), expected);
                            let (actual, r, e) = if edge {
                                let mut c = db
                                    .stream_graph_edges_governed_at(&cx, &q, cut, wide())
                                    .unwrap();
                                (
                                    c.by_ref().collect::<Result<Vec<_>, _>>().unwrap(),
                                    c.row_stats(),
                                    c.evaluator_stats(),
                                )
                            } else {
                                let mut c = db
                                    .stream_graph_values_governed_at(&cx, &q, cut, wide())
                                    .unwrap();
                                (
                                    c.by_ref().collect::<Result<Vec<_>, _>>().unwrap(),
                                    c.row_stats(),
                            ...
                            let native =
                                PreparedNativeRead::prepare(&input, &args, symbols).unwrap();
                            let (_, mut c) = native.stream(&db, &cx, &args, wide()).unwrap();
                            assert_eq!(c.snapshot_seq(), cut);
                            assert_eq!(
                                plain(&c.by_ref().collect::<Result<Vec<_>, _>>().unwrap()),
                                expected
                            );
        ...
        for (input, expected, first, mut c) in paused {
            ...
            rows.extend(c.by_ref().map(Result::unwrap));
...
fn first_independent_witness_uses_three_candidates_despite_unrelated_topology() {
    let ((), report) = run_async_under_lab(0x696e_6402, |root| async move {
        ...
        let mut roots = WriteBatch::new(R);
        for id in [0, 1, 2, 3, 100, 101] {
            roots.create_vertex(VId(id), vec![], vec![(P, CanonicalScalar::Int(7))]);
        }
        roots.add_edge(EId(0), VId(1), VId(2), vec![]);
        let mut probes = WriteBatch::new(Q);
        probes.add_edge(EId(1), VId(0), VId(3), vec![]);
        for id in 2..2050 {
            probes.add_edge(EId(id), VId(100), VId(101), vec![]);
        }
        ...
        for edge in [false, true] {
            ...
            let native = PreparedNativeRead::prepare(&input, &args, symbols).unwrap();
            let (_, mut c) = native
                .stream(&db, &cx, &args, GqlQueryPolicy::new(3, 1, 10_000, 10_000))
                .unwrap();
            assert!(c.next().unwrap().is_ok());
            assert_eq!(c.row_stats().snapshot_records, 3);
            assert!(c.next().is_none());
            let negative = statement(edge, GlaDirection::Forward, true, 0, None)
                .replace("> 5", "> 99")
                + " LIMIT 1";
            let native = PreparedNativeRead::prepare(&negative, &args, symbols).unwrap();
            let (_, mut c) = native
                .stream(&db, &cx, &args, GqlQueryPolicy::new(2, 1, 10_000, 10_000))
                .unwrap();
            ...
            let (_, mut c) = native
                .stream(&db, &cx, &args, GqlQueryPolicy::new(0, 0, 1, 0))
                .unwrap();
            assert!(c.next().is_none());
...
fn independent_scope_parameters_and_budgets_do_not_reset_or_leak_local_bindings() {
    let ((), report) = run_async_under_lab(0x696e_6403, |root| async move {
        ...
        for input in [
            ...
            let q = prepare(input);
            let expected = db
                .execute_graph_pattern_governed(&cx, &q, wide())
                .unwrap()
                .value;
            let native = PreparedNativeRead::prepare(input, &args, symbols).unwrap();
            let (_, mut c) = native.stream(&db, &cx, &args, wide()).unwrap();
            assert_eq!(c.by_ref().collect::<Result<Vec<_>, _>>().unwrap(), expected);
            ...
            for p in [
                GqlQueryPolicy::new(r.snapshot_records - 1, 10_000, u64::MAX, u64::MAX),
                GqlQueryPolicy::new(100_000, r.result_rows - 1, u64::MAX, u64::MAX),
                GqlQueryPolicy::new(100_000, 10_000, e.work_units - 1, u64::MAX),
                GqlQueryPolicy::new(100_000, 10_000, u64::MAX, e.scratch_entries - 1),
            ] {
                let (_, mut failed) = native.stream(&db, &cx, &args, p).unwrap();
                let mut prefix = Vec::new();
                loop {
                    match failed.next() {
                        Some(Ok(row)) => prefix.push(row),
                        Some(Err(_)) => break,
                        None => panic!("quota became EOF"),
                    }
                }
                assert_eq!(prefix, expected[..prefix.len()]);
                assert_eq!(failed.row_stats().result_rows, prefix.len() as u64);
                assert!(failed.next().is_none());
            }
        }
        let input =
            statement(false, GlaDirection::Forward, false, 0, None).replace("> 5", "> $floor");
        let low = GqlParameters::new().with_int64("floor", 5).unwrap();
        let high = GqlParameters::new().with_int64("floor", 99).unwrap();
        let native = PreparedNativeRead::prepare(&input, &low, symbols).unwrap();
        assert!(matches!(
            native.stream(&db, &cx, &args, wide()),
            Err(QueryError::PatternText(_))
        ));
        let (_, mut low) = native.stream(&db, &cx, &low, wide()).unwrap();
        let (_, mut high) = native.stream(&db, &cx, &high, wide()).unwrap();
        assert!(low.next().unwrap().is_ok());
        assert!(high.next().is_none());
        let future = statement(
            false,
            GlaDirection::Forward,
            false,
            0,
            Some(CommitSeq(basis.0 + 1)),
        ) + " LIMIT 0";
        let native = PreparedNativeRead::prepare(&future, &args, symbols).unwrap();
        assert!(
            native
                .stream(&db, &cx, &args, GqlQueryPolicy::new(0, 0, 0, 0))
                .is_err()
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
