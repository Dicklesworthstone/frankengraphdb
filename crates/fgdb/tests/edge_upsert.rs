...
fn keys() -> DatabaseKeys {
    DatabaseKeys::new(
        [0x11; 32],
        DatabaseSecurityNamespaceId([0x12; 32]),
        [0x13; 32],
    )
}
...
fn on_create_decorates_new_edge_and_survives_reopen() {
    let ((), report) = run_async_under_lab(0xed9e_3002, |root| async move {
        ...
        assert_eq!(
            txn.edge(&db, EId(20)).unwrap().unwrap().props,
            vec![(W, CanonicalScalar::Int(200))]
        );
        txn.commit(&mut db, &commit).await.unwrap();
        ...
        let db = Database::open_with_vfs(&commit, vfs, &path, keys())
            ...
            .unwrap();
        assert_eq!(
            db.edge(EId(20)).unwrap().unwrap().props,
            vec![(W, CanonicalScalar::Int(200))]
        );
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
...
fn create_branch_action_limit_rolls_back_created_edge() {
    let ((), report) = run_async_under_lab(0xed9e_3004, |root| async move {
        ...
        assert!(matches!(
            result,
            Err(GqlQueryError::Source(GraphEdgeUpsertError::ActionLimit {
                limit: 0,
                observed: 1
            }))
        ));
...
fn native_autocommit_creates_then_updates_with_no_second_identity() {
    let ((), report) = run_async_under_lab(0xed9e_3101, |root| async move {
        ...
        let (created, outcome, completion) = db
            .execute_graph_edge_upsert_autocommit_governed(
                &txcx,
                &query,
                &commit,
        ...
        assert_eq!(
            created.evaluator.work_units,
            created.merge.evaluator.work_units + 2
        );
        assert_eq!(
            created.evaluator.scratch_entries,
            created.merge.evaluator.scratch_entries + 1
        );
        assert_eq!(
            db.edge(EId(20)).unwrap().unwrap().props,
            vec![(W, CanonicalScalar::Int(200))]
...
fn exact_branch_work_and_scratch_limits_accept_and_one_below_rolls_back() {
    let ((), report) = run_async_under_lab(0xed9e_3102, |root| async move {
        ...
        let (expected, _) = reference
            .execute_graph_edge_upsert_governed(&mut db, &query, &prepared, policy(1), |_| {
                Ok::<_, ()>(ElementId::Edge(EId(30)))
            })
            .unwrap();
        reference.abort();
        for mode in 0..3 {
            ...
            let result =
                txn.execute_graph_edge_upsert_governed(&mut db, &query, &prepared, limits, |_| {
                    Ok::<_, ()>(ElementId::Edge(eid))
                });
            if mode < 2 {
                ...
                assert_eq!(
                    txn.edge(&db, eid).unwrap().unwrap().props,
                    vec![(W, CanonicalScalar::Int(200))]
                );
            }
            txn.abort();
...
fn refused_action_keeps_outer_prefix_and_negative_relationship_witness() {
    let ((), report) = run_async_under_lab(0xed9e_3103, |root| async move {
        ...
        assert!(
            txn.execute_graph_edge_upsert_governed(
                &mut db,
                &query,
                &upsert(2, 3),
                policy(0),
        ...
        assert_eq!(
            allocations.get(),
            1,
            "rollback does not rewind the allocator"
        );
        assert_eq!(txn.staged_effect_digest().unwrap(), before);
        ...
        assert!(
            ...
            "failed MERGE still observed absence"
        );
