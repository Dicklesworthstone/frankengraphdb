...
fn oracle(db: &Database<MemVfs>, relation: RelationId) -> BTreeMap<VId, u64> {
    let mut neighbors: BTreeMap<_, BTreeSet<VId>> = db
        .vertices()
        .unwrap()
        .into_iter()
        .map(|row| (row.vid, BTreeSet::new()))
        .collect();
    ...
    for k in 1..neighbors.len() {
        ...
        loop {
            let remove: Vec<_> = alive
                .iter()
                .copied()
                .filter(|v| neighbors[v].iter().filter(|u| alive.contains(*u)).count() < k)
                .collect();
            if remove.is_empty() {
                break;
            }
...
fn committed_core_changes_preserve_relation_scope_isolates_parallel_edges_and_cascades() {
    let ((), report) = run_async_under_lab(0x6b63_0101, |root| async move {
        ...
        for v in domain {
            vertices.create_vertex(VId(v), vec![], vec![]);
        }
        db.write(&commit, vertices).await.unwrap();
        check(&db, &cx, &r, R);
        check(&db, &cx, &s, S);
        assert_eq!(
            db.standing_core_number(&cx, &r, VId(u128::MAX)).unwrap(),
            Some(0)
        );
        ...
        for i in 0..4 {
            for j in i + 1..4 {
                edges.add_edge(EId(eid), VId(domain[i]), VId(domain[j]), vec![]);
                eid += 1;
            }
        }
        ...
        assert_eq!(db.standing_core_number(&cx, &r, VId(0)).unwrap(), Some(3));
        let mut final_support = WriteBatch::new(R);
        final_support.delete_edge(EId(7));
        ...
        assert_eq!(
            db.standing_components(&cx, &sibling).unwrap().frontier(),
            db.frontier().unwrap()
        );
        let before = db.frontier().unwrap();
        let mut invalid = WriteBatch::new(R);
        invalid.add_edge(EId(99), VId(12345), VId(0), vec![]);
        ...
        for mask in 0..64_u64 {
            ...
            for (at, &(a, b)) in pairs.iter().enumerate() {
                ...
                match (live.get(&at).copied(), wanted) {
                    (Some(id), false) => {
                        change.delete_edge(EId(id));
                        live.remove(&at);
                    }
                    (None, true) => {
                        change.add_edge(EId(eid), VId(a), VId(b), vec![]);
                        live.insert(at, eid);
                        eid += 1;
                    }
                    _ => {}
...
fn result_failures_do_not_undo_writes_and_rebuild_uses_current_not_historical_size() {
    let ((), report) = run_async_under_lab(0x6b63_0103, |root| async move {
        ...
        let mut add = WriteBatch::new(S);
        add.create_vertex(VId(9), vec![], vec![]);
        db.write(&commit, add).await.unwrap();
        assert!(
            matches!(db.standing_core_numbers(&cx, &limited), Err(StandingQueryError::Unavailable {
            frontier, reason: StandingQueryFailure::ResultBudget }) if frontier == basis)
        );
        assert!(matches!(
            db.standing_core_number(&cx, &limited, VId(9)),
            Err(StandingQueryError::Unavailable { .. })
        ));
        check(&db, &cx, &healthy, R);
        assert!(matches!(
            db.rebuild_standing_query(&cx, &limited, small),
            Err(StandingQueryError::Maintenance(
                StandingQueryFailure::ResultBudget
            ))
        ));
        let mut remove = WriteBatch::new(S);
        remove.delete_vertex(VId(9));
        let current = db.write(&commit, remove).await.unwrap();
        assert_eq!(
            db.rebuild_standing_query(&cx, &limited, small).unwrap(),
            current
        );
        check(&db, &cx, &limited, R);
        let before = *db
            .standing_core_numbers(&cx, &limited)
            .unwrap()
            .last_maintenance();
        assert!(
            db.rebuild_standing_query(&cx, &limited, GqlQueryPolicy::new(0, 3, 0, 0))
                .is_err()
        );
        assert_eq!(
            db.standing_core_numbers(&cx, &limited)
                .unwrap()
                .last_maintenance(),
            &before
        );
        ...
        swap.create_vertex(VId(10), vec![], vec![]);
...
fn compact_reopen_rebuild_and_wrong_owner_or_kind_keep_the_native_boundaries() {
    let ((), report) = run_async_under_lab(0x6b63_0104, |root| async move {
        ...
        assert!(matches!(
            db.standing_core_numbers(&cx, &wrong_kind),
            Err(StandingQueryError::Unsupported)
        ));
        assert!(matches!(
            db.standing_query(&cx, &old),
            Err(StandingQueryError::Unsupported)
        ));
        assert!(matches!(
            db.standing_components(&cx, &old),
            Err(StandingQueryError::Unsupported)
        ));
        let mut seed = WriteBatch::new(R);
        for v in [0, 1, u128::MAX] {
            seed.create_vertex(VId(v), vec![], vec![]);
        }
        ...
        let mut db = Database::open_with_vfs(&commit, vfs, &path, keys())
            ...
            .unwrap();
        assert!(matches!(
            db.standing_core_numbers(&cx, &old),
            Err(StandingQueryError::ForeignHandle)
        ));
        assert!(matches!(
            db.rebuild_standing_query(&cx, &old, policy()),
            Err(StandingQueryError::ForeignHandle)
        ));
        ...
        assert_eq!(
            db.standing_core_number(&cx, &new, VId(u128::MAX)).unwrap(),
            Some(0)
        );
        assert_eq!(db.standing_core_number(&cx, &new, VId(88)).unwrap(), None);
        assert_eq!(
            db.standing_core_numbers(&cx, &new)
                .unwrap()
                .last_maintenance()
                .delta_rows,
            0
