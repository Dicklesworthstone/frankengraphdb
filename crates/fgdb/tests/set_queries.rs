...
use fgdb_gql::{
    GqlParameters, GqlQueryError, GqlQueryPolicy, GraphSetExecutionError, GraphSetOperation as Op,
    GraphSetQuantifier as Q, GraphSymbol, GraphSymbolKind, PreparedGraphSet, PreparedGraphText,
};
use fgdb_types::{
...
fn query(operation: Op, quantifier: Q) -> PreparedGraphSet {
    PreparedGraphSet::from(pattern("MATCH (n:Left) RETURN n.n AS value"))
        .combine(
            operation,
            quantifier,
            pattern("MATCH (n:Right) RETURN n.n AS other_name").into(),
        )
        .unwrap()
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (id, value, label) in [
        ...
        data.create_vertex(
            ...
            vec![label],
            value
                .map(|value| (N, CanonicalScalar::Int(value)))
                .into_iter()
                .collect(),
        );
    }
...
fn oracle(rows: &[VertexRow], operation: Op, quantifier: Q) -> Vec<CanonicalScalar> {
    ...
    counts
        .into_iter()
        .flat_map(|(value, (mut l, mut r))| {
            if quantifier == Q::Distinct {
                l = usize::from(l != 0);
                r = usize::from(r != 0);
            }
...
fn compound_relations_follow_staged_effects_and_preserve_pinned_history_after_reopen() {
    let ((), report) = run_async_under_lab(0x5e70_0001, |root| async move {
        ...
        let old_rows = db.vertices().unwrap();
        let pinned = db.read_session().unwrap();
        let mut txn = db.begin(&txn_cx).unwrap();
        for op in [Op::Union, Op::Intersect, Op::Except] {
            for quantifier in [Q::All, Q::Distinct] {
                let query = query(op, quantifier);
                let expected = oracle(&old_rows, op, quantifier);
                for result in [
    ...
    let ((), report) = run_async_under_lab(0x5e70_0002, |root| async move {
        ...
        for mode in 0..3 {
            for mutation in 0..4 {
                ...
                let mut txn = db.begin(&txn_cx).unwrap();
                let mut staged = WriteBatch::new(REL);
                ...
                let output = txn.execute_graph_set_governed(
                    &db,
                    &cx,
                    &query,
...
fn compound_limits_cover_both_sources_and_keep_authority_error_precedence() {
    let ((), report) = run_async_under_lab(0x5e70_0003, |root| async move {
        ...
        assert_eq!(
            db.execute_graph_set_governed(&cx, &query, exact).unwrap(),
            measured
        );
        for denied in [
        ...
        let measured_txn = txn
            .execute_graph_set_governed(&db, &cx, &query, policy())
            .unwrap();
        let exact_txn = GqlQueryPolicy::new(
            measured_txn.rows.snapshot_records,
            1,
            measured_txn.evaluator.work_units,
            measured_txn.evaluator.scratch_entries,
        );
        assert_eq!(
            txn.execute_graph_set_governed(&db, &cx, &query, exact_txn)
                .unwrap(),
            measured_txn
        );
        ...
        assert!(matches!(
            txn.execute_graph_set_governed(&foreign, &cx, &query, zero),
            Err(GqlQueryError::Source(GraphSetExecutionError::Source(
                WriteTxnError::WrongDatabase
            )))
        ));
        assert!(matches!(
            db.execute_graph_set_governed_at(&cx, &query, CommitSeq(basis.0 + 1), zero),
            Err(GqlQueryError::Source(GraphSetExecutionError::Source(
        ...
        let pinned = db.read_session().unwrap();
        assert!(matches!(
            pinned.execute_graph_set_governed_at(&cx, &query, CommitSeq(basis.0 + 1), zero),
            Err(GqlQueryError::Source(GraphSetExecutionError::Source(
                GqlError::Read(ReadError::BeyondFrontier { .. })
            )))
        ));
...
fn walk_and_optional_operands_keep_occurrences_and_null_extension_in_set_algebra() {
    let ((), report) = run_async_under_lab(0x5e70_0004, |root| async move {
        ...
        for (op, mut expected) in [
            ...
            expected.sort();
            let query = PreparedGraphSet::from(walks.clone())
                .combine(op, Q::All, nullable.clone().into())
                .unwrap();
            assert_eq!(
                plain(
                    &db.execute_graph_set_governed(&cx, &query, policy())
