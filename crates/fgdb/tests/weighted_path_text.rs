...
fn weighted_text_has_native_accounting_across_live_history_pins_overlays_and_recovery() {
    let ((), report) = run_async_under_lab(0xc0a5_2011, |root| async move {
        ...
        for (name, mode) in MODES {
            for direction in [
                ...
                for count in [None, Some(7)] {
                    ...
                    let typed = PreparedGraphCheapestPath::new(
                        source,
                        target,
                        R,
                        direction,
                    ...
                    let baseline = match count {
                        Some(k) => {
                            db.execute_graph_cheapest_paths_governed(&cx, &typed, k, policy())
                        }
                    ...
                    for result in [
                        db.execute_graph_cheapest_path_text_governed(&cx, &req, policy())
                            .unwrap(),
                        db.execute_graph_cheapest_path_text_governed_at(&cx, &req, basis, policy())
                            .unwrap(),
                        pinned
                            .execute_graph_cheapest_path_text_governed(&cx, &req, policy())
                            .unwrap(),
                        pinned
                            .execute_graph_cheapest_path_text_governed_at(
                                &cx,
                                &req,
                                basis,
                                policy(),
                    ...
                    let expected_txn = match count {
                        Some(k) => {
                            txn.execute_graph_cheapest_paths_governed(&db, &cx, &typed, k, policy())
                        }
                        None => {
                            txn.execute_graph_cheapest_path_governed(&db, &cx, &typed, policy())
                        }
                    }
                    .unwrap();
                    assert_eq!(
                        txn.execute_graph_cheapest_path_text_governed(&db, &cx, &req, policy())
                            .unwrap()
                            .value,
                        expected_txn.value
                    );
        ...
        for (req, old) in &cases {
            let result = txn
                .execute_graph_cheapest_path_text_governed(&db, &cx, req, policy())
                .unwrap();
            let typed = match req.ranked_count() {
                Some(k) => {
                    txn.execute_graph_cheapest_paths_governed(&db, &cx, req.query(), k, policy())
                }
        ...
        for (req, old) in &cases {
            let result = db
                ...
                .unwrap();
            assert!(
                result.value.iter().all(|row| row
                    .path()
                    .steps()
                    .iter()
                    .all(|step| step.1 != VId(1)))
            );
            assert_eq!(
                &pinned
                    .execute_graph_cheapest_path_text_governed(&cx, req, policy())
        ...
        for (mode, _) in MODES {
            for outcome in 0..6 {
                for change in 0..3 {
                    ...
                    txn.write(&mut db, stage).unwrap();
                    let count = if outcome == 0 {
                        None
                    } else {
                        Some(if outcome == 2 { 0 } else { 3 })
                    };
                    let req = request(
                        mode,
                        GlaDirection::Forward,
                    ...
                    let result =
                        txn.execute_graph_cheapest_path_text_governed(&db, &cx, &req, allowance);
                    match outcome {
    ...
    let ((), report) = run_async_under_lab(0xc0a5_2013, |root| async move {
        ...
        for (mode, _) in MODES {
            for count in [None, Some(3), Some(0)] {
                ...
                assert_eq!(
                    db.execute_graph_cheapest_path_text_governed(&cx, &req, exact)
                        .unwrap(),
                    result
                );
                for bad in [
                    ...
                    assert!(
                        db.execute_graph_cheapest_path_text_governed(&cx, &req, bad)
                            .is_err()
                    );
                }
                if result.rows.result_rows > 0 {
                    assert!(matches!(
                        db.execute_graph_cheapest_path_text_governed(
                            &cx,
                            &req,
                            GqlQueryPolicy::new(
                                1000,
                ...
                for future in [
                    db.execute_graph_cheapest_path_text_governed_at(
                        &cx,
                        &req,
                        CommitSeq(basis.0 + 1),
                        GqlQueryPolicy::new(0, 0, 0, 0),
                    ),
                    pinned.execute_graph_cheapest_path_text_governed_at(
                        &cx,
                        &req,
                        CommitSeq(basis.0 + 1),
                        GqlQueryPolicy::new(0, 0, 0, 0),
                ...
                assert!(matches!(
                    txn.execute_graph_cheapest_path_text_governed(
                        &foreign,
                        &cx,
                        &req,
                        GqlQueryPolicy::new(0, 0, 0, 0)
