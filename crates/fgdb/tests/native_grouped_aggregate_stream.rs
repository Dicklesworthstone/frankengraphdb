...
fn seed() -> WriteBatch {
    ...
    for (id, score, a, b) in [
        ...
        (
            u128::MAX,
            Some(i64::MIN),
            Some(text("' $private_key")),
            CanonicalScalar::Bool(true),
        ),
    ] {
        let mut props = vec![(
            SCORE,
            score.map_or(CanonicalScalar::Null, CanonicalScalar::Int),
        )];
        if let Some(a) = a {
            props.push((A, a));
        }
...
fn native_grouped_layouts_preserve_return_aliases_keys_exact_values_and_group_order() {
    let ((), report) = run_async_under_lab(0xa66e_4001, |root| async move {
        ...
        let mut cursor = db
            .query_aggregate_stream(&cx, SUMMARY, &params, symbols(), wide())
            .unwrap();
        assert_eq!(cursor.key_columns(), ["a", "b"]);
        assert_eq!(
            cursor.aggregate_columns(),
        ...
        let west = groups
            .iter()
            .find(|row| {
                row.values()[0].as_count() == Some(3) && row.values()[1].as_integer() == Some(8)
            })
            .unwrap();
        ...
        let west = groups
            .iter()
            .find(|row| {
                row.values()[0].as_count() == Some(2) && row.values()[1].as_integer() == Some(3)
            })
            .unwrap();
        let avg = west.values()[2].as_average().unwrap();
...
fn grouped_budgets_are_cumulative_and_delivery_refusals_preserve_only_complete_prefixes() {
    let ((), report) = run_async_under_lab(0xa66e_4003, |root| async move {
        ...
        let usage = full.evaluator_stats();
...
fn empty_grouped_input_differs_from_global_input_and_unsupported_clauses_stay_intact() {
    let ((), report) = run_async_under_lab(0xa66e_4004, |root| async move {
        ...
        let mut grouped = db
            .query_aggregate_stream(&cx, SUMMARY, &args, symbols(), zero)
            .unwrap();
        ...
        for statement in [
            ...
            assert!(
                matches!(
                    prepared.stream_aggregate(&db, &cx, &args, zero),
