...
use fgdb_gql::algebra::{
    GlaDirection, GraphBooleanExpression, GraphBooleanOp as B, GraphBooleanOperand as Arg,
    GraphColumn, GraphMatchClause, GraphPatternBuilder, IntegerComparison,
};
use fgdb_gql::{
    GqlQueryPolicy, GraphAggregate, GraphAggregateColumn as Column, GraphAggregateOrder,
    GraphAggregateRow, GraphHavingExpression, GraphHavingOp as H, GraphHavingOperand as A,
    GraphIntegerBinary, GraphIntegerExpression, GraphIntegerOp as I,
...
fn gate(variable: &str, key: PropertyKeyId) -> GraphBooleanExpression {
    ...
    GraphBooleanExpression::prepare(&[B::Compare {
        left: Arg::Property { variable, key },
...
fn definition(
    ...
    root.vertex("a")
        .unwrap()
        .filter_boolean(&gate("a", ROOT_GATE))
        .unwrap();
    ...
    child
        .edge(
            if matches!(shape, Shape::Branch) {
                "a"
            } else {
                "b"
            },
            if matches!(shape, Shape::SelfJoin) {
                R
            } else {
                S
            },
    ...
    if matches!(shape, Shape::Cycle) {
        child.edge("c", R, direction, "a").unwrap();
    }
    ...
    let input = root
        .prepare_values_with_clauses(
            ...
            &[
                ...
                GraphColumn::property("amount", end, AMOUNT),
        ...
        .with_duplicates();
    if !decorate {
        return PreparedGraphAggregate::prepare(
            input,
            if grouped { &[0] } else { &[] },
            &[
                ...
                GraphAggregate::average_int("average", 3),
    ...
    let doubled = GraphIntegerExpression::prepare(&[
        I::Column(3),
        I::Literal(Some(2)),
        I::Binary(GraphIntegerBinary::Multiply),
    ])
    .unwrap();
    let base = PreparedGraphAggregate::prepare_projected(
        input,
        vec![
            Projection::new("group", Value::Column(4)),
            Projection::new("amount", Value::Integer(doubled)),
            Projection::new("middle", Value::Column(1)),
            Projection::new("end", Value::Column(2)),
            Projection::new("root", Value::Column(0)),
        ],
        if grouped { &[0] } else { &[] },
        &[
            GraphAggregate::count_rows("rows"),
            GraphAggregate::count("middles", 2),
            GraphAggregate::count("endpoints", 3),
    ...
    let having = GraphHavingExpression::prepare(&[
        H::Compare {
            left: A::Column(Column::Aggregate(0)),
            comparison: IntegerComparison::GreaterOrEqual,
            right: A::Integer(2),
        },
        H::IsNull {
            operand: A::Column(Column::Aggregate(3)),
            is_null: true,
        },
        H::Or,
    ])
    ...
    .with_output_projection(vec![
        Projection::new("rows", Value::Column(at)),
        Projection::new("sum", Value::Column(at + 3)),
        Projection::new("average", Value::Column(at + 4)),
        Projection::new("distinct_endpoints", Value::Column(at + 6)),
    ])
    .unwrap()
    .with_distinct_output(matches!(shape, Shape::Branch | Shape::Cycle))
}
...
fn check(
    ...
    let bag = ZSet::from_updates(
        expected.into_iter().map(|row| (row, ZWeight::ONE)),
        LimbLimit::new(4),
        &mut |_| Ok::<_, ()>(()),
    )
    .unwrap();
...
fn multi_hop_scopes_match_snapshot_sequences_across_shapes_and_result_stages() {
    let ((), report) = run_async_under_lab(0x6ab3, |root| async move {
        ...
        for id in 1..=6 {
            put(&mut seed, id, (id != 4).then_some(id as i64));
        }
...
fn whole_path_replacement_and_result_refusal_preserve_public_lifecycle() {
    let ((), report) = run_async_under_lab(0x6ab4, |root| async move {
        ...
        let he = db
            .register_standing_query(&query, exists.clone(), bounded(1))
            .unwrap();
        let ho = db
            .register_standing_query(&query, optional.clone(), policy())
            .unwrap();
        let ha = db
            .register_standing_query(&query, anti.clone(), policy())
            .unwrap();
        let reachability = db
        ...
        assert!(matches!(db.standing_query(&query, &he),
            Err(StandingQueryError::Unavailable { frontier, reason: StandingQueryFailure::ResultBudget })
                if frontier == basis));
        assert!(matches!(
            db.rebuild_standing_query(&query, &he, bounded(1)),
        ...
        assert_eq!(
            db.rebuild_standing_query(&query, &he, bounded(2)).unwrap(),
            at
        );
...
fn self_join_occurrences_preserve_parallel_edges_and_nonnull_identity_witnesses() {
    let ((), report) = run_async_under_lab(0x6ab5, |root| async move {
        ...
        for mode in [Mode::Optional, Mode::Exists, Mode::Anti] {
            let definition =
                definition(mode, Shape::SelfJoin, GlaDirection::Undirected, true, false);
            let handle = db
                .register_standing_query(&query, definition.clone(), policy())
                .unwrap();
            check(&db, &query, &handle, &definition);
...
fn independent_and_multiple_scopes_remain_explicitly_unsupported() {
    let ((), report) = run_async_under_lab(0x6ab7, |root| async move {
        ...
        independent
            ...
            .unwrap();
        independent
            .edge("x", R, GlaDirection::Forward, "y")
            .unwrap();
        independent
            .edge("y", S, GlaDirection::Forward, "z")
            .unwrap();
        ...
        for clauses in [
            ...
            vec![
                GraphMatchClause::exists(&correlated),
                GraphMatchClause::not_exists(&correlated),
            ],
        ] {
            let input = outer
                .prepare_values_with_clauses(&clauses, &[GraphColumn::vertex("root", "a")], 0, None)
                .unwrap()
                .with_duplicates();
