...
fn definition(
    ...
    let query = builder
        .prepare_values(&[GraphColumn::property("p", "n", P)], offset, limit)
        .unwrap();
    if distinct {
        query
    } else {
...
fn check(
    ...
    previous: &mut ZSet<GraphValueRow>,
) {
    ...
    let eager = db
        .execute_graph_pattern_governed(cx, definition, policy())
        .unwrap()
        .value;
    let expected = ZSet::from_updates(
        eager.into_iter().map(|row| (row, ZWeight::ONE)),
        LIMBS,
        &mut |_| Ok::<_, ()>(()),
    )
    .unwrap();
...
fn latest_deltas_integrate_to_bags_and_distinct_pages_after_each_real_commit() {
    let ((), report) = run_async_under_lab(0x6465_6c01, |root| async move {
        ...
        let definitions = [
            definition(false, 0, None),
            definition(false, 1, Some(2)),
            definition(true, 1, Some(2)),
        ];
        let handles: Vec<_> = definitions
            .iter()
            .map(|q| db.register_standing_rows(&cx, q.clone(), policy()).unwrap())
            .collect();
        ...
        for batch in [edit, retire, unrelated] {
            ...
            for ((h, q), old) in handles.iter().zip(&definitions).zip(&mut previous) {
                ...
                assert_eq!(
                    db.standing_row_delta(&cx, h).unwrap().unwrap().frontier(),
                    at
                );
            }
        }
    ...
    let ((), report) = run_async_under_lab(0x6465_6c02, |root| async move {
        ...
        let limited = db
            .register_standing_rows(
                &cx,
                q.clone(),
                GqlQueryPolicy::new(100, 1, 1_000_000, 1_000_000),
            )
            .unwrap();
        ...
        assert_eq!(
            db.standing_row_delta(&cx, &healthy)
                .unwrap()
                .unwrap()
                .frontier(),
            current
        ...
        assert!(
            db.standing_row_delta(&cx, &limited)
                .unwrap()
                .unwrap()
                .rows()
                .iter()
...
fn delta_access_checks_kind_owner_and_reopen_before_exposing_changes() {
    let ((), report) = run_async_under_lab(0x6465_6c03, |root| async move {
        ...
        let mut db = Database::create_with_vfs(&commit, vfs.clone(), &path, keys())
            ...
            .unwrap();
        let row = db
            .register_standing_rows(&cx, definition(false, 0, None), policy())
            .unwrap();
        let topology = db
            .register_standing_reachability(&cx, RelationId(1), policy())
            .unwrap();
        assert!(matches!(
            db.standing_row_delta(&cx, &topology),
            Err(StandingQueryError::Unsupported)
        ));
        let foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(
            foreign.standing_row_delta(&cx, &row),
        ...
        let reopened = Database::open_with_vfs(&commit, vfs, &path, keys())
            ...
            .unwrap();
        assert!(matches!(
            reopened.standing_row_delta(&cx, &row),
            Err(StandingQueryError::ForeignHandle)
        ));
    });
    assert!(report.lab_test_passed(), "{report:?}");
