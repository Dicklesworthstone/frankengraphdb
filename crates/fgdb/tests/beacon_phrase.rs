...
use fgdb_warden::{Authority, Error as WardenError, Grant, LimitDimension, QueryLimits, Restriction, Scope};
...
fn keys() -> DatabaseKeys { DatabaseKeys::new([0xd3; 32], NS, [0xd5; 32]) }
...
fn phrase() -> Search<'static> { Search::Text { query: "red blue", k: 10, mode: TextMatch::Phrase } }
...
async fn database(cx: &CommitCx, hidden: bool) -> Database<MemVfs> {
    ...
    for (id, words) in [(1, "blue red"), (2, "prefix red blue tail"), (3, "red red blue")] {
        batch.create_vertex(VId(id), if hidden { vec![L, LabelId(99)] } else { vec![L] },
            vec![(T, text(words))]);
    }
    if hidden {
        batch.create_vertex(VId(90), vec![LabelId(99)], vec![(T, text("red blue red blue red blue"))]);
        batch.create_vertex(VId(91), vec![LabelId(99)], vec![(T, CanonicalScalar::Int(7))]);
    }
...
fn phrase_reads_follow_staged_properties_labels_and_historical_index_generations() {
    let ((), report) = run_async_under_lab(0xbeac_3101, |root| async move {
        ...
        assert_eq!(db.beacon_search(&query, &options, phrase()).unwrap(), before);
        txn.commit(&mut db, &commit).await.unwrap();
        assert_eq!(db.beacon_search(&query, &options, phrase()).unwrap(), staged);
        resident.refresh(&query, &db, None, ReadPolicy::default()).unwrap();
        assert_eq!(resident.search(&query, phrase(), ReadPolicy::default()).unwrap(), staged);
        let mut historical = options.clone();
        historical.as_of = Some(at);
        db.compact(&commit).await.unwrap();
        assert_eq!(db.beacon_search(&query, &historical, phrase()).unwrap(), before);
        drop(db);
        assert_eq!(view.beacon_search(&query, &options, phrase()).unwrap(), before);
        assert_eq!(retained.search(&query, phrase(), ReadPolicy::default()).unwrap(), before);
...
fn hybrid() -> ExactHybridQuery<'static> {
    ExactHybridQuery {
        vector: &[], text: "red blue", k: 10, vector_candidates: 0, text_candidates: 10,
        vector_mode: VectorSearch::Exact, text_mode: TextMatch::Phrase,
...
fn phrase_candidate_ranks_compose_with_graph_only_hits_before_final_top_k() {
    let ((), report) = run_async_under_lab(0xbeac_3102, |root| async move {
        ...
        let spec = ExpansionSpec {
            seeds: &[VId(2)], relation: Some(RelationId(1)), direction: ExpansionDirection::Outgoing,
            max_hops: 1, include_seeds: false, limits: ExpansionLimits::default(),
        };
        let hits = db.beacon_search_graph(&query, &options, q, spec).unwrap();
        assert_eq!(hits.len(), 3);
        assert_eq!(hits[0].id, VId(1));
        assert_eq!(hits[0].text_rank, None, "a reversed phrase contributes no text rank");
        ...
        txn.commit(&mut db, &commit).await.unwrap();
        assert_eq!(staged, db.beacon_search_graph(&query, &options, q, spec).unwrap());
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn grant() -> Grant {
    let mut grant = Grant::read_only("main", 1000, QueryLimits {
        max_nodes: 100, max_rows: 100, max_work: 1_000_000,
    });
...
fn scoped_phrase_scores_and_work_thresholds_ignore_hidden_text_and_masked_fields() {
    let ((), report) = run_async_under_lab(0xbeac_3103, |root| async move {
        ...
        let issuer = Authority::new(AuthKey::from_seed(3103), NS, "host-graph", SchemaEpoch(0), 1).unwrap();
        let token = issuer.issue_at(&grant(), 100).unwrap();
        for search in [phrase(), Search::Hybrid(hybrid())] {
            let expected = clean.beacon_search_authorized(&query, &issuer, &token, "main", &options(), search, || 100).unwrap();
            assert_eq!(ids(&expected), [VId(2), VId(3)]);
            assert_eq!(full.beacon_search_authorized(&query, &issuer, &token, "main", &options(), search, || 100).unwrap(), expected);
            let threshold = |db: &Database<MemVfs>| {
                ...
                while low < high {
                    ...
                    match db.beacon_search_authorized(&query, &issuer, &bounded, "main", &options(), search, || 100) {
                        Ok(rows) => { assert_eq!(rows, expected); high = mid; }
                        Err(ReadError::Interrupted(QueryError::Authorization(
                            WardenError::LimitExceeded(LimitDimension::Work)))) => low = mid + 1,
        ...
        assert!(full.beacon_search_authorized(&query, &issuer, &masked, "main", &options(), phrase(), || 100)
            .unwrap().is_empty());
...
fn phrase_reads_retain_non_hit_conflicts_across_savepoint_rollback() {
    let ((), report) = run_async_under_lab(0xbeac_3104, |root| async move {
        ...
        assert_eq!(ids(&txn.beacon_search(&db, &c.query(), &options(), phrase()).unwrap()), [VId(2), VId(3)]);
...
fn every_public_phrase_checkpoint_interrupts_without_releasing_a_result_prefix() {
    ...
    let ((), report) = run_async_under_lab(0xbeac_3105, |root| async move {
        ...
        for k in [0, 1] {
            let request = Search::Text { query: "red blue", k, mode: TextMatch::Phrase };
            ...
            for stop in 1..=trace.calls() {
                ...
                assert!(matches!(db.beacon_search(&interrupted, &options(), request),
                    Err(ReadError::Interrupted(_))), "k={k}, checkpoint={stop}");
