...
use fgdb_gql::{
    GqlEvidenceAuditError, GqlEvidenceLimitedAuditError, GqlEvidencePageAuditError,
...
fn result_bound_checkpoints_resume_only_the_audited_result() {
    let ((), report) = run_async_under_lab(0x71_31, |root| async move {
        ...
        let mut resumed = database
            .resume_untrusted_prepared_query_artifact_cursor(&query, &bytes, &checkpoint)
        ...
        let wrong_result = database
            .resume_untrusted_prepared_query_artifact_cursor(&query, &current_bytes, &checkpoint)
