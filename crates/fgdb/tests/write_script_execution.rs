...
fn policy(effects: u64, vertices: u64, edges: u64) -> GraphWriteProgramPolicy {
    GraphWriteProgramPolicy::new(
        GqlQueryPolicy::new(20_000, 20_000, 2_000_000, 2_000_000), effects, vertices, edges,
    )
}
...
fn arguments(fresh: i64, seen: i64) -> GqlParameters {
    GqlParameters::new().with_int64("left", 1).unwrap().with_int64("right", 2).unwrap()
        .with_int64("fresh", fresh).unwrap().with_int64("seen", seen).unwrap()
}
...
fn parameterized_script_commits_once_rebinds_without_allocation_and_survives_reopen() {
    let ((), report) = run_async_under_lab(0x5c71_0001, |root| async move {
        ...
        let (receipt, completion) = db.execute_graph_write_script_autocommit_governed(
            ...
            |request| {
                ...
                Ok::<_, ()>(match (request.statement, request.request) {
                    ...
                    (2, GraphInsertRequest::Edge { row: 0, edge: 0 }) => ElementId::Edge(EId(10)),
        ...
        assert_eq!(db.edge(EId(10)).unwrap().unwrap().props, vec![(Q, CanonicalScalar::Int(200))]);

        let (receipt, completion) = db.execute_graph_write_script_autocommit_governed(
            &txcx, &query, &commit, &definition, &arguments(999, 101), policy(1, 0, 0),
        ...
        assert_eq!(receipt.steps()[2].merged_edge(), Some(GraphEdgeMergeOutcome::Matched(EId(10))));
        ...
        assert_eq!(db.edge(EId(10)).unwrap().unwrap().props, vec![(Q, CanonicalScalar::Int(101))]);
    });
...
fn late_binding_refusal_cannot_run_the_earlier_creation_or_allocate_an_identity() {
    let ((), report) = run_async_under_lab(0x5c71_0002, |root| async move {
        ...
        let result = db.execute_graph_write_script_autocommit_governed(
            &txcx, &query, &commit, &definition, &GqlParameters::new(), policy(1, 1, 0),
            |_| -> Result<ElementId, ()> { panic!("a bind refusal must precede allocation") },
        ).await;
        assert!(matches!(result, Err(GraphWriteScriptExecutionError::Binding(error))
        ...
        assert!(matches!(transaction.execute_graph_write_script_governed(
            &mut db, &query, &definition, &GqlParameters::new(), policy(1, 1, 0),
...
fn late_creation_quota_rolls_back_the_script_not_the_outer_transaction_prefix() {
    let ((), report) = run_async_under_lab(0x5c71_0003, |root| async move {
        ...
        let result = transaction.execute_graph_write_script_governed(
            ...
            &GqlParameters::new(), policy(0, 1, 0), |_| {
        ...
        assert_eq!(allocations.get(), 1, "second creation must refuse before allocation");
...
fn matched_read_only_script_publishes_no_marker_and_uses_no_creation_quota() {
    let ((), report) = run_async_under_lab(0x5c71_0004, |root| async move {
        ...
        let before = db.frontier().unwrap();
        let definition = script("MERGE (n:Person {p:1}); MERGE (n:Person {p:2});
            MATCH (a),(b) WHERE a.p=1 AND b.p=2 MERGE (a)-[:R]->(b);");
        let (receipt, completion) = db.execute_graph_write_script_autocommit_governed(
            &txcx, &query, &commit, &definition, &GqlParameters::new(), policy(0, 0, 0),
...
fn no_input_relationship_does_not_skip_a_later_statement_or_consume_branch_quota() {
    let ((), report) = run_async_under_lab(0x5c71_0005, |root| async move {
        ...
        let txcx = contexts.txn();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let definition = script("MATCH (a),(b) MERGE (a)-[e:R]->(b) ON CREATE SET e.q=1;
            MERGE (n:Person {p:3});");
        let (receipt, _) = db.execute_graph_write_script_autocommit_governed(
            &txcx, &query, &commit, &definition, &GqlParameters::new(), policy(0, 1, 0),
            |request| {
