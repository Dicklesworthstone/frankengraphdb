...
#[test]
fn undirected_both_bang_inequalities_see_the_staged_pair_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x66_03, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-undirected-where-both-prop-both-bang-ne-overlay-{}",
            std::process::id()
        ));
