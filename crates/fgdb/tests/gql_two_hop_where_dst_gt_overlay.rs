...
#[test]
fn two_hop_far_end_property_greater_than_sees_the_staged_continuation() {
    let ((), report) = run_async_under_lab(0x72_09, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-two-hop-where-dst-gt-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
        ...
        assert_eq!(
            txn.execute_gql(&db, filtered, &bind)
                .expect("overlay far-end greater-than MATCH executes"),
            vec![VId(6)]
        );
        assert_eq!(
            db.execute_gql(filtered, &bind)
                .expect("base far-end greater-than MATCH executes"),
            Vec::<VId>::new()
        );
        txn.abort();
    });
