...
#[test]
fn incoming_two_hop_near_end_property_inequality_sees_the_staged_continuation() {
    let ((), report) = run_async_under_lab(0x91_49, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-two-hop-where-src-ne-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
        ...
        seed_s.add_edge(EId(13), VId(6), VId(5), vec![]);
        ...
        staged_s.add_edge(EId(11), VId(3), VId(2), vec![]);
