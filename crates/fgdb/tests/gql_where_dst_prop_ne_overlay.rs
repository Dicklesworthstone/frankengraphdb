...
#[test]
fn destination_property_inequality_sees_staged_dest_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x52_02, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-where-dst-prop-ne-overlay-{}",
            std::process::id()
        ));
