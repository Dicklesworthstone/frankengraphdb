...
fn pattern(floor: i64, limit: Option<u64>) -> PreparedGraphPattern<GraphValueRow> {
    ...
    PreparedGraphText::prepare(
        &format!(
            "{HEAD} RETURN company,filing,filing.score AS score \
        ORDER BY company,filing{tail}"
        ),
        symbols,
    )
    .unwrap()
    .bind_parameters(&arguments(floor))
    .unwrap()
}
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (id, cik) in [(1, 10), (2, 20)] {
        batch.create_vertex(
            VId(id),
            vec![COMPANY],
            vec![(CIK, CanonicalScalar::Int(cik))],
        );
    }
    ...
    batch.create_vertex(
        ...
        vec![FILING],
        vec![
            (CIK, CanonicalScalar::Null),
            (SCORE, CanonicalScalar::Int(9)),
        ],
    );
    batch.create_vertex(
        VId(14),
        vec![FILING],
        vec![(SCORE, CanonicalScalar::Int(10))],
    );
    db.write(cx, batch).await.unwrap()
}
fn integer(row: &VertexRow, key: PropertyKeyId) -> Option<i64> {
    row.props.iter().find_map(|(actual, value)| match value {
        CanonicalScalar::Int(value) if *actual == key => Some(*value),
        _ => None,
    })
}
fn oracle(vertices: &[VertexRow], floor: i64) -> Vec<Pair> {
    ...
    for company in vertices.iter().filter(|row| row.labels.contains(&COMPANY)) {
        let Some(cik) = integer(company, CIK) else {
            continue;
        };
        for filing in vertices.iter().filter(|row| row.labels.contains(&FILING)) {
            if integer(filing, CIK) != Some(cik) {
                continue;
            }
...
fn plain(rows: &[GraphValueRow]) -> Vec<Pair> {
    rows.iter()
        .map(|row| {
            let CanonicalScalar::Int(score) = row.values()[2].as_scalar().unwrap() else {
                panic!("fixture score")
            };
            (
...
fn expected_summary(rows: &[Pair]) -> Vec<Summary> {
    ...
    for &(company, filing, score) in rows {
        ...
        group.0 += 1;
        group.1.insert(filing);
        group.2 += i128::from(score);
    }
    groups
        .into_iter()
        .map(|(company, (count, unique, sum))| (company, count, unique.len() as u64, sum))
        .collect()
}
    ...
    rows.iter()
        .map(|row| {
            (
                ...
                row.values()[2].as_integer().unwrap(),
            )
        })
        .collect()
}
...
fn isolated_joins_and_aggregates_share_live_historical_pinned_and_staged_sources() {
    let ((), report) = run_async_under_lab(0xd15c_0001, |root| async move {
        ...
        assert!(
            db.edges().unwrap().is_empty(),
            "all filings are isolated vertices"
        );
        let view = db.read_session().unwrap();
        let query = pattern(0, None);
        let grouped = aggregate(0);
        let frozen = query.canonical_bytes();
        let old = oracle(&db.vertices().unwrap(), 0);
        assert_eq!(
            old,
            vec![
                (VId(1), VId(10), 4),
        ...
        for result in [
            db.execute_graph_aggregate_governed(&cx, &grouped, policy())
                .unwrap(),
            db.execute_graph_aggregate_governed_at(&cx, &grouped, basis, policy())
                .unwrap(),
            view.execute_graph_aggregate_governed(&cx, &grouped, policy())
                .unwrap(),
            txn.execute_graph_aggregate_governed(&db, &cx, &grouped, policy())
                .unwrap(),
        ] {
        ...
        assert_eq!(
            plain(
                &db.execute_graph_pattern_governed(&cx, &filtered, policy())
                    .unwrap()
                    .value
            ),
        ...
        changes.create_vertex(
            ...
            vec![FILING],
            vec![
                (CIK, CanonicalScalar::Int(20)),
                (SCORE, CanonicalScalar::Int(5)),
            ],
        );
        ...
        assert_eq!(
            summary(
                &reopened
                    .execute_graph_aggregate_governed(&cx, &grouped, policy())
                    .unwrap()
                    .value
            ),
            expected_summary(&expected)
        ...
        assert_eq!(
            summary(
                &reopened
                    .execute_graph_aggregate_governed_at(&cx, &grouped, basis, policy())
                    .unwrap()
                    .value
            ),
...
fn mixed_edge_and_vertex_components_admit_isolates_and_all_policy_dimensions() {
    let ((), report) = run_async_under_lab(0xd15c_0002, |root| async move {
        ...
        let work = result.evaluator.work_units;
        let scratch = result.evaluator.scratch_entries;
        assert_eq!(
            run(GqlQueryPolicy::new(11, 5, work, scratch)).unwrap(),
            result
        );
        for cap in [
            GqlQueryPolicy::new(10, 5, work, scratch),
            GqlQueryPolicy::new(11, 4, work, scratch),
            GqlQueryPolicy::new(11, 5, work - 1, scratch),
            GqlQueryPolicy::new(11, 5, work, scratch - 1),
        ] {
        ...
        for mode in 0..4 {
            for change in 0..3 {
                ...
                if mode == 3 {
                    ...
                    let cap = GqlQueryPolicy::new(
                        1_000,
                        if mode == 1 { 0 } else { 1_000 },
                        5_000_000,
                        2_000_000,
                    );
                    ...
                    if mode == 1 {
                        ...
                        assert_eq!(result.unwrap().value.len(), if mode == 2 { 0 } else { 3 });
                    }
                }
                ...
                match change {
                    0 => {
                        winner.create_vertex(
                            ...
                            vec![FILING],
                            vec![
                                (CIK, CanonicalScalar::Int(10)),
                                (SCORE, CanonicalScalar::Int(1)),
                            ],
                        );
                    }
                    1 => {
                        winner.set_vertex_property(VId(15), CIK, Some(CanonicalScalar::Int(10)));
                    }
                    _ => {
                        winner.set_vertex_property(VId(14), CIK, Some(CanonicalScalar::Int(20)));
                    }
                }
...
fn absent_optional_and_anti_join_witnesses_remain_vertex_phantom_dependencies() {
    let ((), report) = run_async_under_lab(0xd15c_0004, |root| async move {
        ...
        for text in [
            ...
            if result.value[0].values().len() == 2 {
            ...
            winner.create_vertex(
                VId(888),
                vec![FILING],
                vec![(CIK, CanonicalScalar::Int(10))],
            );
            db.write(&commit, winner).await.unwrap();
            assert!(matches!(
                txn.commit(&mut db, &commit).await,
