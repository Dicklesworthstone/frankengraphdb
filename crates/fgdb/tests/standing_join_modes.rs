...
fn input(db: &mut Database<MemVfs>, cx: &QueryCx, label: &str) -> StandingQueryHandle {
    db.register_standing_rows(cx,
        definition(&format!("MATCH (n:{label}) RETURN n.k AS k, n.p AS p")), policy()).unwrap()
}
...
fn source(db: &Database<MemVfs>, label: u64) -> Bag {
    ...
    for row in db.vertices_at(db.frontier().unwrap()).unwrap() {
        if row.labels.contains(&LabelId(label)) {
            let values = [K, P].map(|key| GraphValue::Scalar(row.props.iter().find(|(k, _)| *k == key)
...
fn evaluate(kind: RowJoinKind, left: &Bag, right: &Bag, keys: &[(usize, usize)], right_width: usize) -> Bag {
    let mut output = Bag::new();
    for (left, lw) in left {
        let matches: Vec<_> = right.iter().filter(|(right, _)| keys.iter().all(|&(a, b)|
            !left[a].is_null() && !right[b].is_null() && left[a] == right[b])).collect();
        match kind {
            RowJoinKind::Inner | RowJoinKind::Left if !matches.is_empty() => {
                for (right, rw) in matches {
                    *output.entry(left.iter().chain(right).cloned().collect()).or_default() += lw * rw;
                }
            }
            RowJoinKind::Left => {
            ...
            RowJoinKind::Semi if !matches.is_empty() => { *output.entry(left.clone()).or_default() += lw; }
            RowJoinKind::Anti if matches.is_empty() => { *output.entry(left.clone()).or_default() += lw; }
            _ => {}
        }
    }
    output
...
fn difference(next: &Bag, previous: &Bag) -> Bag {
    ...
    for (row, weight) in previous { *result.entry(row.clone()).or_default() -= weight; }
...
fn first_last_duplicate_witnesses_and_simultaneous_changes_propagate_through_composed_views() {
    let ((), report) = run_async_under_lab(0x726a_0301, |root| async move {
        ...
        for id in [1, 2] { add(&mut seed, id, 1, Some(1), 10); }
        for id in [3, 4] { add(&mut seed, id, 2, Some(1), 20); }
        add(&mut seed, 5, 1, Some(2), 30);
        add(&mut seed, 6, 1, None, 60); add(&mut seed, 7, 2, None, 70);
        db.write(&commit, seed).await.unwrap();
        let left = input(&mut db, &cx, "L"); let right = input(&mut db, &cx, "R");
        let kinds = [RowJoinKind::Left, RowJoinKind::Semi, RowJoinKind::Anti];
        let mut joins = Vec::new();
        let mut previous = Vec::new();
        for kind in kinds {
            let handle = db.register_standing_join_with_kind(&cx, &left, &right, &[(0, 0)], kind, policy()).unwrap();
            ...
            previous.push(expected); joins.push(handle);
        }
        // Semi and Anti partition the complete left bag, including NULL keys.
        let partition = db.register_standing_set(&cx, &joins[1], &joins[2], SetOperation::UnionAll, policy()).unwrap();
        // A NULL-extended frame is a legal child input, but cannot be a witness.
        let nested = db.register_standing_join_with_kind(&cx, &joins[0], &right, &[(2, 0)], RowJoinKind::Semi, policy()).unwrap();
        for step in 0..6 {
            let mut batch = WriteBatch::new(RelationId(1));
            match step {
                0 => { batch.delete_vertex(VId(3)); }
                1 => { batch.delete_vertex(VId(4)); add(&mut batch, 8, 2, Some(1), 21);
                    batch.set_vertex_property(VId(2), P, Some(CanonicalScalar::Int(11))); }
                2 => { batch.delete_vertex(VId(8)); }
                3 => { add(&mut batch, 9, 2, Some(2), 40);
                    batch.set_vertex_property(VId(1), K, Some(CanonicalScalar::Int(2))); }
                4 => { batch.delete_vertex(VId(6)); batch.delete_vertex(VId(7)); }
                _ => { batch.set_vertex_property(VId(1), PropertyKeyId(99), Some(CanonicalScalar::Int(1))); }
            }
            let at = db.write(&commit, batch).await.unwrap();
            let l = source(&db, 1); let r = source(&db, 2);
            for (index, kind) in kinds.into_iter().enumerate() {
                let expected = evaluate(kind, &l, &r, &[(0, 0)], 2);
                let result = db.standing_join(&cx, &joins[index]).unwrap();
                assert_eq!(result.frontier(), at); assert_eq!(plain(result.rows()), expected);
                ...
                assert_eq!(db.standing_join_total(&cx, &joins[index]).unwrap().to_i128(), Some(expected.values().sum()));
                previous[index] = expected;
            }
            assert_eq!(plain(db.standing_set(&cx, &partition).unwrap().rows()), l);
            assert_eq!(plain(db.standing_join(&cx, &nested).unwrap().rows()),
...
fn quota_failure_isolated_from_durable_commits_and_rebuild_retains_each_join_kind() {
    let ((), report) = run_async_under_lab(0x726a_0302, |root| async move {
        ...
        for kind in [RowJoinKind::Left, RowJoinKind::Semi, RowJoinKind::Anti] {
            ...
            db.write(&commit, seed).await.unwrap();
            let left = input(&mut db, &cx, "L"); let right = input(&mut db, &cx, "R");
            let limited = db.register_standing_join_with_kind(&cx, &left, &right, &[(0, 0)], kind, bounded(1)).unwrap();
            let healthy = db.register_standing_join_with_kind(&cx, &left, &right, &[(0, 0)], kind, policy()).unwrap();
            let child = db.register_standing_set(&cx, &limited, &limited, SetOperation::UnionAll, policy()).unwrap();
            let before = plain(db.standing_join(&cx, &limited).unwrap().rows());
            assert!(matches!(db.rebuild_standing_query(&cx, &limited, bounded(0)),
                Err(StandingQueryError::Maintenance(StandingQueryFailure::ResultBudget))));
            assert_eq!(plain(db.standing_join(&cx, &limited).unwrap().rows()), before);
            assert_eq!(db.standing_join_kind(&cx, &limited).unwrap(), kind);
            let mut grow = WriteBatch::new(RelationId(1)); add(&mut grow, 3, 1, Some(1), 30);
            ...
            assert_eq!(db.standing_join_total(&cx, &healthy).unwrap().to_i128(), Some(2));
            assert!(db.rebuild_standing_query(&cx, &child, policy()).is_err());
            assert!(db.rebuild_standing_query(&cx, &limited, bounded(1)).is_err());
            assert_eq!(db.rebuild_standing_query(&cx, &limited, policy()).unwrap(), at);
            assert_eq!(db.standing_join_kind(&cx, &limited).unwrap(), kind);
            ...
            next.set_vertex_property(VId(2), K, Some(CanonicalScalar::Int(if matching { 2 } else { 1 })));
            db.write(&commit, next).await.unwrap();
            let expected = evaluate(kind, &source(&db, 1), &source(&db, 2), &[(0, 0)], 2);
            for handle in [&limited, &healthy] {
                assert_eq!(plain(db.standing_join(&cx, handle).unwrap().rows()), expected);
            }
            let twice: Bag = expected.iter().map(|(row, count)| (row.clone(), count * 2)).collect();
...
fn nullable_vertex_schemas_compose_and_old_handles_refuse_after_reopen() {
    let ((), report) = run_async_under_lab(0x726a_0303, |root| async move {
        ...
        db.write(&commit, seed).await.unwrap();
        let left = db.register_standing_rows(&cx, definition("MATCH (n:L) RETURN n AS id"), policy()).unwrap();
        let right = db.register_standing_rows(&cx, definition("MATCH (n:R) RETURN n AS id"), policy()).unwrap();
        let outer = db.register_standing_join_with_kind(&cx, &left, &right, &[(0, 0)], RowJoinKind::Left, policy()).unwrap();
        let present = db.register_standing_join_with_kind(&cx, &outer, &right, &[(1, 0)], RowJoinKind::Semi, policy()).unwrap();
        let absent = db.register_standing_join_with_kind(&cx, &outer, &right, &[(1, 0)], RowJoinKind::Anti, policy()).unwrap();
        let combined = db.register_standing_set(&cx, &present, &absent, SetOperation::UnionAll, policy()).unwrap();
        let expected = Bag::from([
            (vec![GraphValue::Vertex(VId(0)), GraphValue::Vertex(VId(0))], 1),
            (vec![GraphValue::Vertex(VId(u128::MAX)), GraphValue::Scalar(CanonicalScalar::Null)], 1),
        ]);
        assert_eq!(plain(db.standing_join(&cx, &outer).unwrap().rows()), expected);
        assert_eq!(plain(db.standing_set(&cx, &combined).unwrap().rows()), expected);
        let scalar = input(&mut db, &cx, "L");
        assert!(matches!(db.register_standing_join_with_kind(&cx, &left, &scalar, &[(0, 0)], RowJoinKind::Anti, policy()),
            Err(StandingQueryError::JoinSchema(RowJoinBuildError::KeyType { .. }))));
        let mut foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(foreign.register_standing_join_with_kind(&cx, &left, &right, &[(0, 0)], RowJoinKind::Left, policy()),
            Err(StandingQueryError::ForeignHandle)));
        db.compact(&commit).await.unwrap();
        assert_eq!(plain(db.standing_join(&cx, &outer).unwrap().rows()), expected);
        drop(db);
        let mut db = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        assert!(matches!(db.standing_join_kind(&cx, &outer), Err(StandingQueryError::ForeignHandle)));
        let left = db.register_standing_rows(&cx, definition("MATCH (n:L) RETURN n AS id"), policy()).unwrap();
        let right = db.register_standing_rows(&cx, definition("MATCH (n:R) RETURN n AS id"), policy()).unwrap();
        let restored = db.register_standing_join_with_kind(&cx, &left, &right, &[(0, 0)], RowJoinKind::Left, policy()).unwrap();
        assert_eq!(plain(db.standing_join(&cx, &restored).unwrap().rows()), expected);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0x726a_0304, |root| async move {
        ...
        for kind in [RowJoinKind::Semi, RowJoinKind::Anti] {
            ...
            for size in [2_u128, 128] {
                ...
                db.write(&commit, seed).await.unwrap();
                let left = input(&mut db, &cx, "L"); let right = input(&mut db, &cx, "R");
                let join = db.register_standing_join_with_kind(&cx, &left, &right, &[(0, 0)], kind, policy()).unwrap();
                let mut remove = WriteBatch::new(RelationId(1)); remove.delete_vertex(VId(size + 1));
                db.write(&commit, remove).await.unwrap();
                let view = db.standing_join(&cx, &join).unwrap();
                measured.push(*view.last_maintenance());
                assert_eq!(plain(view.rows()), evaluate(kind, &source(&db, 1), &source(&db, 2), &[(0, 0)], 2));
                assert!(db.standing_join_delta(&cx, &join).unwrap().unwrap().rows().is_empty());
            }
