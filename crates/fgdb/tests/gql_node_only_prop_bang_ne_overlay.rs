...
#[test]
fn node_only_property_bang_inequality_sees_the_staged_isolate_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x59_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-node-only-prop-bang-ne-overlay-{}",
            std::process::id()
        ));
        ...
        staged.create_vertex(VId(3), vec![person], vec![(key, CanonicalScalar::Int(9))]);
