...
use fgdb_types::{CommitSeq, DatabaseSecurityNamespaceId, EId, EmbeddedTxnCompletion,
...
impl Pause {
    ...
    fn blocks(&self, path: &Path, sync: bool) -> bool {
        ...
        let matches = match mode {
            1 if !sync => path.parent().and_then(Path::file_name)
        ...
        if matches { self.reached.store(true, Ordering::SeqCst); }
        matches
    }
}
...
impl AsyncWrite for PausingFile {
    fn poll_write(self: Pin<&mut Self>, cx: &mut Context<'_>, buf: &[u8]) -> Poll<io::Result<usize>> {
        ...
        if this.pause.blocks(&this.path, false) { return Poll::Pending; }
...
impl VfsFile for PausingFile {
    async fn metadata(&self) -> io::Result<Metadata> { self.inner.metadata().await }

    async fn sync_all(&self) -> io::Result<()> {
        std::future::poll_fn(|_| {
            if self.pause.blocks(&self.path, true) { Poll::Pending } else { Poll::Ready(()) }
        }).await;
    ...
    async fn set_len(&self, size: u64) -> io::Result<()> { self.inner.set_len(size).await }
    async fn set_permissions(&self, perm: Permissions) -> io::Result<()> { self.inner.set_permissions(perm).await }
}
...
impl Vfs for PausingVfs {
    ...
    async fn metadata(&self, path: &Path) -> io::Result<Metadata> { self.inner.metadata(path).await }
    async fn symlink_metadata(&self, path: &Path) -> io::Result<Metadata> { self.inner.symlink_metadata(path).await }
    async fn set_permissions(&self, path: &Path, perm: Permissions) -> io::Result<()> { self.inner.set_permissions(path, perm).await }
    async fn create_dir(&self, path: &Path) -> io::Result<()> { self.inner.create_dir(path).await }
    async fn create_dir_all(&self, path: &Path) -> io::Result<()> { self.inner.create_dir_all(path).await }
    async fn remove_dir(&self, path: &Path) -> io::Result<()> { self.inner.remove_dir(path).await }
    async fn remove_file(&self, path: &Path) -> io::Result<()> { self.inner.remove_file(path).await }
    async fn read_dir(&self, path: &Path) -> io::Result<ReadDir> { self.inner.read_dir(path).await }
    async fn remove_dir_all(&self, path: &Path) -> io::Result<()> { self.inner.remove_dir_all(path).await }
    async fn rename(&self, from: &Path, to: &Path) -> io::Result<()> { self.inner.rename(from, to).await }
    async fn copy(&self, src: &Path, dst: &Path) -> io::Result<u64> { self.inner.copy(src, dst).await }
    async fn hard_link(&self, original: &Path, link: &Path) -> io::Result<()> { self.inner.hard_link(original, link).await }
    async fn canonicalize(&self, path: &Path) -> io::Result<PathBuf> { self.inner.canonicalize(path).await }
    async fn read_link(&self, path: &Path) -> io::Result<PathBuf> { self.inner.read_link(path).await }
    async fn read(&self, path: &Path) -> io::Result<Vec<u8>> { self.inner.read(path).await }
    async fn read_to_string(&self, path: &Path) -> io::Result<String> { self.inner.read_to_string(path).await }

    async fn write(&self, path: &Path, contents: &[u8]) -> io::Result<()> {
        let mut file = self.open(path, &OpenOptions::new().write(true).create(true).truncate(true)).await?;
    ...
    let ((), report) = run_async_under_lab(0xf171_0201, |root| async move {
        ...
        for legacy_commit in [false, true] {
            for mode in 1..=3 {
                ...
                let keys = DatabaseKeys::new([0x81; 32], DatabaseSecurityNamespaceId([0x82; 32]), [0x83; 32]);
                let mut database = Database::create_with_vfs(&cx, vfs, &path, keys).await.unwrap();
                ...
                let mut future = Box::pin(async {
                    if legacy_commit {
                        transaction.commit(&mut database, &cx).await
                ...
                std::future::poll_fn(|task| match future.as_mut().poll(task) {
                    ...
                    Poll::Ready(result) => panic!("durability pause {mode} was not reached: {result:?}"),
                }).await;
                drop(future);
                assert_eq!(txcx.outstanding_obligations(), 0, "the transaction is still retained");
                let expected = if mode == 3 {
                    EmbeddedTxnState::CommittedNeedsRecovery { commit_seq: CommitSeq(basis.0 + 1) }
                } else {
                    EmbeddedTxnState::CommitOutcomeUnknown { published_frontier: basis }
                };
                ...
                if mode == 1 {
                    ...
                    assert_eq!(recovered.vertices().unwrap().iter().map(|row| row.vid).collect::<Vec<_>>(), vec![VId(1), VId(2)]);
