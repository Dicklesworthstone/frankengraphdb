...
use fgdb_gql::algebra::{
    GraphColumn, GraphPatternBuilder, GraphValue, IntegerComparison, VertexPredicate,
};
use fgdb_gql::{
    GqlParameters, GqlQueryPolicy, GraphAggregate, GraphAggregateRow, GraphExactAverage,
...
fn definition(grouped: bool, identity: bool) -> PreparedGraphAggregate {
    ...
    input
        .filter("n", VertexPredicate::HasLabel(PERSON))
        .unwrap();
    input
        .filter(
            "n",
        ...
        .unwrap();
    let key = if identity {
        GraphColumn::vertex("team", "n")
    } else {
        GraphColumn::property("team", "n", TEAM)
    };
    let input = input
        .prepare_values(&[key, GraphColumn::property("score", "n", SCORE)], 0, None)
        .unwrap()
        .with_duplicates();
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) {
    ...
    for (id, member, selected, team, score) in [
        ...
        if let Some(team) = team {
            props.push((TEAM, team));
        }
        if let Some(score) = score {
            props.push((SCORE, score));
        }
...
fn oracle(db: &Database<MemVfs>, grouped: bool, identity: bool) -> Vec<Summary> {
    ...
    for row in db.vertices().unwrap() {
        ...
        if !row.labels.contains(&PERSON)
            || !matches!(get(SELECTED), Some(CanonicalScalar::Int(value)) if *value >= 1)
        {
            continue;
        }
        let key = if !grouped {
            ...
            vec![GraphValue::Vertex(row.vid)]
        } else {
            vec![GraphValue::Scalar(
                get(TEAM).cloned().unwrap_or(CanonicalScalar::Null),
            )]
        };
...
fn grouped_commits_move_keys_retract_groups_and_keep_null_and_average_semantics() {
    let ((), report) = run_async_under_lab(0x6a01, |root| async move {
        ...
        let grouped_handle = db
            .register_standing_query(&query_cx, query.clone(), policy())
            .unwrap();
        let global_handle = db
            .register_standing_query(&query_cx, global.clone(), policy())
            .unwrap();
        let identity_handle = db
            .register_standing_query(&query_cx, identity.clone(), policy())
            .unwrap();
        assert!(
            db.standing_query(&query_cx, &grouped_handle)
                .unwrap()
                .rows()
                .is_empty()
        );
        ...
        for step in 0..9 {
            for (handle, plan, grouped, id) in [
                (&grouped_handle, &query, true, false),
                (&global_handle, &global, false, false),
            ...
            match step {
                0 => {
                    batch.set_vertex_property(VId(1), TEAM, Some(text("green")));
                }
                1 => {
                    batch.set_vertex_label(VId(3), PERSON, false);
                }
                2 => {
                    batch.set_vertex_property(VId(2), SCORE, Some(CanonicalScalar::Int(7)));
                }
                3 => {
                    batch.set_vertex_property(VId(4), SELECTED, Some(CanonicalScalar::Int(1)));
                }
                4 => {
                    batch.set_vertex_property(VId(6), TEAM, Some(CanonicalScalar::Null));
                }
                5 => {
                    batch.delete_vertex(VId(2));
                }
                6 => {
                    batch.set_vertex_label(VId(3), PERSON, true);
                }
                7 => {
                    batch.set_vertex_property(VId(6), TEAM, None);
                }
                8 => {
                    for id in [1, 3, 4, 5, 6] {
...
fn parsed_multikey_label_query_and_unfiltered_global_query_are_maintainable() {
    let ((), report) = run_async_under_lab(0x6a02, |root| async move {
        ...
        let handles = plans
            .iter()
            .map(|plan| {
                db.register_standing_query(&cx, plan.clone(), policy())
                    .unwrap()
            })
            .collect::<Vec<_>>();
        seed(&mut db, &commit).await;
        for step in 0..3 {
            ...
            batch.set_vertex_property(
                VId(1),
                TEAM,
                if step == 0 { None } else { Some(text("blue")) },
            );
...
fn grouped_result_limit_is_final_state_and_does_not_abort_durable_writes() {
    let ((), report) = run_async_under_lab(0x6a03, |root| async move {
        ...
        let handle = db
            .register_standing_query(&cx, query.clone(), narrow)
            .unwrap();
        let mut batch = WriteBatch::new(R);
        batch.create_vertex(
            VId(1),
            vec![PERSON],
            vec![
                (SELECTED, CanonicalScalar::Int(1)),
                (TEAM, text("only")),
        ...
        growth.create_vertex(
            VId(2),
            vec![PERSON],
            vec![
                (SELECTED, CanonicalScalar::Int(1)),
                (TEAM, text("new-group")),
                (SCORE, CanonicalScalar::Int(2)),
            ],
        );
...
fn irrelevant_ticks_do_not_revisit_vertices_or_leak_staged_values() {
    let ((), report) = run_async_under_lab(0x6a04, |root| async move {
        ...
        for step in 0..3 {
            ...
            match step {
                0 => {
                    batch.set_vertex_property(VId(1), UNUSED, Some(text("private-payload-8723")));
                }
                1 => {
        ...
        for size in [8_u128, 400] {
            ...
            for id in 1..=size {
                seed.create_vertex(
                    ...
                    vec![
                        (SELECTED, CanonicalScalar::Int(1)),
                        (TEAM, CanonicalScalar::Int(id as i64)),
