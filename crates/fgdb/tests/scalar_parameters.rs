...
use fgdb_gql::{GqlParameterType, GqlParameters, GqlQueryError, GqlQueryPolicy,
    GraphAggregateRow, GraphPatternTextErrorKind, GraphSymbol, GraphSymbolKind,
...
const TYPES: [(&str, GqlParameterType); 3] = [
    ...
    ("active", GqlParameterType::Scalar(CanonicalScalarKind::Bool)),
...
fn arguments() -> GqlParameters {
    GqlParameters::new().with_text("status", "ready").unwrap()
        .with_bool("active", true).unwrap().with_text("category", WANTED).unwrap()
        .with_uint64("take", 100).unwrap()
}
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (vid, status, active) in [(VId(0), "ready", true), (VId(1), "ready", true),
        (VId(2), "other", true), (VId(3), "ready", false), (HIGH, "ready", true)] {
        batch.create_vertex(vid, vec![PERSON], vec![(STATUS, text(status)), (ACTIVE, CanonicalScalar::Bool(active))]);
    }
    batch.create_vertex(VId(10), vec![], vec![(CATEGORY, text(WANTED))]);
    batch.create_vertex(VId(11), vec![], vec![(CATEGORY, text("other"))]);
    for (eid, source, destination) in [(1, VId(0), VId(10)), (2, VId(0), VId(10)),
        (3, VId(2), VId(10)), (4, HIGH, VId(11))] {
        batch.add_edge(EId(eid), source, destination, vec![]);
    }
    db.write(cx, batch).await.unwrap()
}
fn property(row: &VertexRow, key: PropertyKeyId) -> Option<&CanonicalScalar> {
    row.props.iter().find(|(found, _)| *found == key).map(|(_, value)| value)
}
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<Plain> {
    let ready = text("ready"); let active = CanonicalScalar::Bool(true); let wanted = text(WANTED);
    let mut result = Vec::new();
    for person in vertices.iter().filter(|row| row.labels.contains(&PERSON)
        && property(row, STATUS) == Some(&ready) && property(row, ACTIVE) == Some(&active)) {
        let mut matched = false;
        for edge in edges.iter().filter(|edge| edge.entry.src == person.vid && edge.entry.relation == R) {
            let company = vertices.iter().find(|row| row.vid == edge.entry.dst).unwrap();
            if property(company, CATEGORY) == Some(&wanted) {
                matched = true;
                result.push((person.vid, Some(company.vid), Some(wanted.clone())));
            }
        }
        if !matched { result.push((person.vid, None, None)); }
    }
    result.sort(); result
}
fn plain(rows: &[GraphValueRow]) -> Vec<Plain> {
    rows.iter().map(|row| {
        ...
        (row.get(0).unwrap().as_vertex().unwrap(), row.get(1).unwrap().as_vertex(),
...
fn expected_summary(rows: &[Plain]) -> Vec<Summary> {
    ...
    for row in rows { groups.entry(row.0).or_default().push(row.1); }
    let mut result: Vec<_> = groups.into_iter().map(|(owner, values)| {
        let present: Vec<_> = values.iter().flatten().copied().collect();
        (owner, values.len() as u64, present.len() as u64,
            present.into_iter().collect::<BTreeSet<_>>().len() as u64)
    }).collect();
    result.sort_by(|a, b| b.2.cmp(&a.2).then(a.0.cmp(&b.0))); result
}
fn summaries(rows: &[GraphAggregateRow]) -> Vec<Summary> {
    rows.iter().map(|row| (row.keys()[0].as_vertex().unwrap(), row.get(0).unwrap().as_count().unwrap(),
        row.get(1).unwrap().as_count().unwrap(), row.get(2).unwrap().as_count().unwrap())).collect()
}
...
fn typed_arguments_cover_all_read_surfaces_staging_and_retained_generations() {
    let ((), report) = run_async_under_lab(0x5ca1_0001, |root| async move {
        ...
        let prepared = template(); let args = arguments();
        let pattern = prepared.bind_parameters(&args).unwrap(); let frozen = pattern.canonical_bytes();
        let summary = aggregate_template().bind_parameters(&args.clone().with_int64("minimum", 1).unwrap()).unwrap();
        ...
        for result in [db.execute_graph_aggregate_governed(&cx, &summary, policy()).unwrap(),
            db.execute_graph_aggregate_governed_at(&cx, &summary, basis, policy()).unwrap(),
            view.execute_graph_aggregate_governed(&cx, &summary, policy()).unwrap(),
            view.execute_graph_aggregate_governed_at(&cx, &summary, basis, policy()).unwrap(),
            txn.execute_graph_aggregate_governed(&db, &cx, &summary, policy()).unwrap()] {
            assert_eq!(summaries(&result.value), expected_summary(&old));
        }
        let null_args = GqlParameters::new().with_text("status", "ready").unwrap().with_bool("active", true).unwrap()
            .with_null("category").unwrap().with_uint64("take", 100).unwrap();
        let null_pattern = prepared.bind_parameters(&null_args).unwrap();
        let null_rows = db.execute_graph_pattern_governed(&cx, &null_pattern, policy()).unwrap();
        assert_eq!(plain(&null_rows.value), vec![(VId(0), None, None), (VId(1), None, None), (HIGH, None, None)]);
        ...
        assert_eq!(expected, vec![(VId(0), None, None), (HIGH, Some(VId(11)), Some(text(WANTED)))]);
        assert_eq!(plain(&txn.execute_graph_pattern_governed(&db, &cx, &pattern, policy()).unwrap().value), expected);
        assert_eq!(summaries(&txn.execute_graph_aggregate_governed(&db, &cx, &summary, policy()).unwrap().value), expected_summary(&expected));
        ...
        assert_eq!(summaries(&reopened.execute_graph_aggregate_governed(&cx, &summary, policy()).unwrap().value), expected_summary(&expected));
        ...
        assert_eq!(summaries(&view.execute_graph_aggregate_governed(&cx, &summary, policy()).unwrap().value), expected_summary(&old));
        assert_eq!(pattern.canonical_bytes(), frozen);
    });
    ...
    let ((), report) = run_async_under_lab(0x5ca1_0002, |root| async move {
        ...
        for refused in [false, true] {
            for change in 0..5 {
                ...
                else { assert_eq!(plain(&result.unwrap().value), old); }
                ...
                match change {
                    ...
                    3 => winner.create_vertex(VId(88), vec![PERSON], vec![(STATUS, text("ready")), (ACTIVE, CanonicalScalar::Bool(true))]),
                    _ => winner.set_vertex_property(VId(3), ACTIVE, Some(CanonicalScalar::Bool(true))),
                };
...
fn exact_limits_and_runtime_cancellation_preserve_source_and_argument_errors() {
    let ((), report) = run_async_under_lab(0x5ca1_0003, |root| async move {
        ...
        let cx = contexts.query(); let commit = contexts.commit(); let txn_cx = contexts.txn();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap(); seed(&mut db, &commit).await;
        let prepared = template(); let pattern = prepared.bind_parameters(&arguments()).unwrap();
        ...
        for cap in [GqlQueryPolicy::new(full.rows.snapshot_records - 1, 100, u64::MAX, u64::MAX),
        ...
        let wrong = GqlParameters::new().with_text("status", "ready").unwrap().with_text("active", "true").unwrap()
            .with_text("category", WANTED).unwrap().with_uint64("take", 100).unwrap();
        ...
        let txn = db.begin(&txn_cx).unwrap(); let view = db.read_session().unwrap();
        ...
        assert_eq!(txn.execute_graph_pattern_governed(&db, &cx, &pattern, policy()).unwrap().value, full.value);
        assert_ne!(db.execute_graph_pattern_governed(&cx, &pattern, policy()).unwrap().value, full.value);
        root.cancel_with(CancelKind::User, Some("scalar argument cancellation ordering"));
        assert!(matches!(txn.execute_graph_pattern_governed(&foreign, &cx, &pattern, policy()),
            Err(GqlQueryError::Source(WriteTxnError::WrongDatabase))));
        let future = CommitSeq(db.frontier().unwrap().0 + 1);
        assert!(matches!(db.execute_graph_pattern_governed_at(&cx, &pattern, future, policy()),
            Err(GqlQueryError::Source(GqlError::Read(ReadError::BeyondFrontier { .. })))));
        assert!(matches!(view.execute_graph_pattern_governed_at(&cx, &pattern, future, policy()),
            Err(GqlQueryError::Source(GqlError::Read(ReadError::BeyondFrontier { .. })))));
