...
use fgdb_gql::{GqlParameters, GqlQueryPolicy, GraphSetBuildError, GraphSetColumnType,
...
const OPERATIONS: [SetOperation; 6] = [SetOperation::UnionAll, SetOperation::UnionDistinct,
    SetOperation::IntersectAll, SetOperation::IntersectDistinct, SetOperation::ExceptAll, SetOperation::ExceptDistinct];
...
fn eager(db: &Database<MemVfs>, cx: &QueryCx, q: &PreparedGraphPattern<GraphValueRow>) -> Bag {
    let mut bag = Bag::new();
    for row in db.execute_graph_pattern_governed(cx, q, policy()).unwrap().value {
        *bag.entry(row).or_default() += 1;
    }
    bag
}
fn oracle(left: &Bag, right: &Bag, operation: SetOperation) -> Bag {
    left.keys().chain(right.keys()).collect::<BTreeSet<_>>().into_iter().filter_map(|row| {
        let l = left.get(row).copied().unwrap_or(0); let r = right.get(row).copied().unwrap_or(0);
...
fn check(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle, wanted: &Bag) {
    ...
    assert_eq!(db.standing_set_total(cx, handle).unwrap().to_i128(), Some(wanted.values().sum()));
}
...
fn all_six_operations_and_nested_shared_inputs_follow_whole_commits() {
    let ((), report) = run_async_under_lab(0x7365_7401, |root| async move {
        ...
        let l = db.register_standing_rows(&cx, lq.clone(), policy()).unwrap();
        let r = db.register_standing_rows(&cx, rq.clone(), policy()).unwrap();
        let handles: Vec<_> = OPERATIONS.iter().map(|&op|
            db.register_standing_set(&cx, &l, &r, op, policy()).unwrap()).collect();
        let nested = db.register_standing_set(&cx, &handles[0], &handles[2], SetOperation::ExceptAll, policy()).unwrap();
        let doubled = db.register_standing_set(&cx, &nested, &nested, SetOperation::UnionAll, policy()).unwrap();
        let self_except = db.register_standing_set(&cx, &nested, &nested, SetOperation::ExceptDistinct, policy()).unwrap();
        for handle in &handles {
            assert_eq!(db.standing_set_columns(&cx, handle).unwrap(), &["left_name"]);
            assert!(db.standing_set_delta(&cx, handle).unwrap().is_none());
        }
        let mut previous: Vec<_> = handles.iter().map(|h|
            db.standing_set(&cx, h).unwrap().rows().checked_clone(LIMBS, &mut |_| Ok::<_, ()>(())).unwrap()).collect();
        ...
        for batch in [first, second, empty] {
            ...
            for ((handle, operation), old) in handles.iter().zip(OPERATIONS).zip(&mut previous) {
                ...
                old.integrate(delta.rows(), LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
                assert_eq!(*old, *db.standing_set(&cx, handle).unwrap().rows());
            }
            let wanted = oracle(&oracle(&left, &right, SetOperation::UnionAll),
                &oracle(&left, &right, SetOperation::IntersectAll), SetOperation::ExceptAll);
            check(&db, &cx, &nested, &wanted);
            check(&db, &cx, &doubled, &oracle(&wanted, &wanted, SetOperation::UnionAll));
...
fn final_occurrence_limits_allow_swaps_and_dependency_failures_require_ordered_repair() {
    let ((), report) = run_async_under_lab(0x7365_7402, |root| async move {
        ...
        let r = db.register_standing_rows(&cx, query("MATCH (n:R) RETURN n.p AS p"), policy()).unwrap();
        let child = db.register_standing_set(&cx, &l, &r, SetOperation::UnionAll, one).unwrap();
        let grandchild = db.register_standing_set(&cx, &child, &child, SetOperation::UnionDistinct, one).unwrap();
        let sibling = db.register_standing_set(&cx, &r, &r, SetOperation::UnionAll, policy()).unwrap();
        ...
        assert_eq!(db.standing_set_delta(&cx, &child).unwrap().unwrap().rows().len(), 2);
        ...
        for h in [&child, &grandchild] {
            assert!(matches!(db.standing_set(&cx, h), Err(StandingQueryError::Unavailable {
        ...
        assert!(matches!(db.rebuild_standing_query(&cx, &child, one),
        ...
        db.rebuild_standing_query(&cx, &grandchild, policy()).unwrap();
        ...
        let limited = db.register_standing_set(&cx, &l, &r, SetOperation::UnionAll, one).unwrap();
        let mut insert = WriteBatch::new(RelationId(1));
        ...
        assert!(matches!(db.standing_set(&cx, &limited), Err(StandingQueryError::Unavailable {
            reason: StandingQueryFailure::ResultBudget, ..
        })));
        assert_eq!(db.standing_set_total(&cx, &child).unwrap().to_i128(), Some(2));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn operand_windows_wide_identities_schema_and_owner_checks_are_not_sampled_from_rows() {
    let ((), report) = run_async_under_lab(0x7365_7403, |root| async move {
        ...
        let vertex = db.register_standing_rows(&cx, query("MATCH (n) RETURN n AS id"), policy()).unwrap();
        let scalar = db.register_standing_rows(&cx, query("MATCH (n) RETURN n.p AS p"), policy()).unwrap();
        assert!(matches!(db.register_standing_set(&cx, &vertex, &scalar, SetOperation::UnionAll, policy()),
            Err(StandingQueryError::SetSchema(GraphSetBuildError::ColumnType {
                column: 0, left: GraphSetColumnType::Vertex, right: GraphSetColumnType::Scalar,
            }))));
        let pair = db.register_standing_rows(&cx, query("MATCH (n) RETURN n AS id, n.p AS p"), policy()).unwrap();
        assert!(matches!(db.register_standing_set(&cx, &pair, &vertex, SetOperation::UnionAll, policy()),
            Err(StandingQueryError::SetSchema(GraphSetBuildError::ColumnCount { left: 2, right: 1 }))));
        let topology = db.register_standing_components(&cx, RelationId(1), policy()).unwrap();
        assert!(matches!(db.register_standing_set(&cx, &topology, &vertex, SetOperation::UnionAll, policy()),
            Err(StandingQueryError::Unsupported)));
        let identities = db.register_standing_set(&cx, &vertex, &vertex, SetOperation::UnionDistinct, policy()).unwrap();
        let mut seed = WriteBatch::new(RelationId(1));
        for (id, value) in [(0, 4), (1, 4), (1_u128 << 100, 7), (u128::MAX, 9)] {
            seed.create_vertex(VId(id), vec![], vec![(P, CanonicalScalar::Int(value))]);
        }
        db.write(&commit, seed).await.unwrap();
        assert!(db.standing_set(&cx, &identities).unwrap().rows().iter().any(|(r, _)|
            r.values() == [GraphValue::Vertex(VId(u128::MAX))]));
        ...
        let left = db.register_standing_rows(&cx, lq.clone(), policy()).unwrap();
        let right = db.register_standing_rows(&cx, rq.clone(), policy()).unwrap();
        let window = db.register_standing_set(&cx, &left, &right, SetOperation::ExceptAll, policy()).unwrap();
        check(&db, &cx, &window, &oracle(&eager(&db, &cx, &lq), &eager(&db, &cx, &rq), SetOperation::ExceptAll));
        let mut change = WriteBatch::new(RelationId(1)); change.delete_vertex(VId(0));
        db.write(&commit, change).await.unwrap();
        check(&db, &cx, &window, &oracle(&eager(&db, &cx, &lq), &eager(&db, &cx, &rq), SetOperation::ExceptAll));
        let mut foreign = Database::open_memory(&commit, keys()).await.unwrap();
        let local = foreign.register_standing_rows(&cx, query("MATCH (n) RETURN n AS id"), policy()).unwrap();
        assert!(matches!(foreign.register_standing_set(&cx, &local, &vertex, SetOperation::UnionAll, policy()),
            Err(StandingQueryError::ForeignHandle)));
        assert!(matches!(db.standing_rows(&cx, &window), Err(StandingQueryError::Unsupported)));
        assert!(matches!(db.standing_set(&cx, &left), Err(StandingQueryError::Unsupported)));
        db.compact(&commit).await.unwrap(); drop(db);
        let mut reopened = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        assert!(matches!(reopened.standing_set(&cx, &window), Err(StandingQueryError::ForeignHandle)));
        let l = reopened.register_standing_rows(&cx, lq.clone(), policy()).unwrap();
        let r = reopened.register_standing_rows(&cx, rq.clone(), policy()).unwrap();
        let fresh = reopened.register_standing_set(&cx, &l, &r, SetOperation::ExceptAll, policy()).unwrap();
        check(&reopened, &cx, &fresh, &oracle(&eager(&reopened, &cx, &lq), &eager(&reopened, &cx, &rq), SetOperation::ExceptAll));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn empty_operand_changes_do_not_rescan_large_retained_set_inputs() {
    let ((), report) = run_async_under_lab(0x7365_7404, |root| async move {
        ...
        for size in [4, 128] {
            ...
            db.write(&commit, seed).await.unwrap();
            let parent = db.register_standing_rows(&cx, query("MATCH (n) RETURN n.p AS p"), policy()).unwrap();
            let h = db.register_standing_set(&cx, &parent, &parent, SetOperation::UnionAll, policy()).unwrap();
            ...
            irrelevant.set_vertex_property(VId(0), PropertyKeyId(99), Some(CanonicalScalar::Int(1)));
            ...
            assert!(db.standing_set_delta(&cx, &h).unwrap().unwrap().rows().is_empty());
            measurements.push((changed.work_units, changed.scratch_entries, empty.work_units, empty.scratch_entries));
        }
        assert_eq!(measurements[0], measurements[1], "set work depends on changed keys, not retained support");
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
