...
fn template(only_keys: bool, paginated: bool) -> PreparedGraphAggregateText {
    ...
    PreparedGraphAggregateText::prepare(
        &format!(
         ...
         ORDER BY AVG_INT(b.p) DESC NULLS LAST,SUM(DISTINCT b.p) DESC,a{page}"
        ),
        symbols,
    )
    .unwrap()
...
fn query(only_keys: bool, count: Option<u64>) -> PreparedGraphAggregate {
    ...
    if let Some(count) = count {
        args = args.with_uint64("take", count).unwrap();
    }
    template(only_keys, count.is_some())
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord], minimum: i64) -> Vec<(VId, u64)> {
    ...
    for owner in vertices.iter().filter(|row| row.labels.contains(&OWNER)) {
        ...
        {
            paths += 1;
            let destination = vertices
                .iter()
                .find(|row| row.vid == edge.entry.dst)
                .unwrap();
            if let Some((_, CanonicalScalar::Int(value))) =
                destination.props.iter().find(|(key, _)| *key == P)
            {
                values.push(*value);
            }
        }
        if !values.is_empty() && (values.len() as i128) < i128::from(minimum) {
            continue;
        }
        let mean = (!values.is_empty()).then(|| {
            (
                values.iter().map(|n| i128::from(*n)).sum::<i128>(),
                values.len() as i128,
            )
        });
...
fn check(rows: &[GraphAggregateRow], expected: &[(VId, u64)], only_keys: bool) {
    ...
    for (row, (owner, paths)) in rows.iter().zip(expected) {
        ...
        if !only_keys {
            assert_eq!(row.get(0).unwrap().as_count(), Some(*paths));
        }
    }
}
...
fn private_text_summaries_cover_all_reads_canonical_staging_and_reopened_history() {
    let ((), report) = run_async_under_lab(0x1dde_0001, |root| async move {
        ...
        for only_keys in [false, true] {
            ...
            assert_eq!(
                plan.evaluation_aggregate_columns().len(),
                if only_keys { 3 } else { 4 }
            );
            for result in [
                db.execute_graph_aggregate_governed(&cx, &plan, wide())
                    .unwrap(),
                db.execute_graph_aggregate_governed_at(&cx, &plan, basis, wide())
                    .unwrap(),
                pinned
                    .execute_graph_aggregate_governed(&cx, &plan, wide())
                    .unwrap(),
                pinned
                    .execute_graph_aggregate_governed_at(&cx, &plan, basis, wide())
                    .unwrap(),
                txn.execute_graph_aggregate_governed(&db, &cx, &plan, wide())
                    .unwrap(),
            ] {
                check(&result.value, &old, only_keys);
            }
        }
        let mut changes = WriteBatch::new(R);
        changes.delete_edge(EId(1));
        changes.ensure_edge_by_triple(EId(999), VId(0), VId(11), vec![]);
        changes.set_vertex_property(VId(11), P, Some(CanonicalScalar::Int(7)));
        changes.set_vertex_property(VId(13), P, None);
        changes.add_edge(EId(90), VId(2), VId(12), vec![]);
        changes.delete_vertex(VId(10));
        ...
        for only_keys in [false, true] {
            ...
            let plan = template
                .bind_parameters(&GqlParameters::new().with_int64("minimum", 2).unwrap())
                .unwrap();
            ...
            let less = template
                .bind_parameters(&GqlParameters::new().with_int64("minimum", 1).unwrap())
                .unwrap();
            assert_ne!(less.canonical_bytes(), frozen);
            assert_eq!(plan.canonical_bytes(), frozen);
            let relaxed = oracle(&reopened.vertices().unwrap(), &reopened.edges().unwrap(), 1);
            check(
                &reopened
                    .execute_graph_aggregate_governed(&cx, &less, wide())
                    .unwrap()
                    .value,
                &relaxed,
                only_keys,
            ...
            for mode in 0..3 {
                for change in 0..5 {
                    ...
                    match change {
                        ...
                        1 => {
                            winner.add_edge(EId(90), VId(1), VId(11), vec![]);
                        }
                        2 => {
                        ...
                        3 => {
                            winner.delete_edge(EId(1));
                        }
                        _ => {
                            winner.set_vertex_property(VId(11), P, None);
                        }
                    }
