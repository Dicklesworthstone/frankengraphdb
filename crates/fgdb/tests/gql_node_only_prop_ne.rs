//! **Node-only `WHERE a.k <> 1` keeps the non-equal labeled isolate**
//! (`fgdb-w5-parsers-nje.15`, node-only slice).
//!
//! The property inequality on the edgeless labeled scan, with all three
//! carrier states in one fixture — `k = 1`, `k = 9`, and no `k` at all —
//! so each statement's exact answer convicts a different cheat: the
//! inequality answering the keyless 5 is the complement-of-equality
//! reading; the equality answering anything but 1 is a broken predicate;
//! and the unfiltered scan answering fewer than all three means the WHERE
//! machinery leaked into the plain statement.
...
fn scratch(name: &str) -> PathBuf {
    std::env::temp_dir().join(format!("fgdb-node-prop-ne-{}-{name}", std::process::id()))
}
...
/// One fixture, three exact answers: inequality, equality, unfiltered.
#[test]
fn the_inequality_keeps_the_carrier_of_a_different_value_only() {
    under_lab(0x9e_01, |cx| async move {
        let cx = &cx;
