...
fn keys() -> DatabaseKeys {
    DatabaseKeys::new([0x21; 32], DatabaseSecurityNamespaceId([0x22; 32]), [0x23; 32])
}
...
fn args(p: i64, matched: i64, created: i64) -> GqlParameters {
    GqlParameters::new().with_int64("p", p).unwrap()
        .with_int64("matched", matched).unwrap().with_int64("created", created).unwrap()
}
...
fn native_upsert_executes_the_selected_branch_and_publishes_once() {
    let ((), report) = run_async_under_lab(0x6e29_4001, |root| async move {
        ...
        let (stats, outcome, completion) = db.execute_graph_vertex_upsert_autocommit_governed(
            &txcx, &query, &commit, &matched, policy(),
        ...
        let (stats, outcome, completion) = db.execute_graph_vertex_upsert_autocommit_governed(
            &txcx, &query, &commit, &created, policy(),
...
fn unconfigured_match_branch_stays_read_only() {
    let ((), report) = run_async_under_lab(0x6e29_4002, |root| async move {
        ...
        let template = PreparedGraphVertexUpsertText::prepare(
            "MERGE (n:Person {p:$p}) ON CREATE SET n.q=200,n:Created", R, symbols,
        ).unwrap();
        let bound = template.bind_parameters(
            &GqlParameters::new().with_int64("p", 7).unwrap(),
        ).unwrap();
        let (stats, outcome, completion) = db.execute_graph_vertex_upsert_autocommit_governed(
            &txcx, &query, &commit, &bound, policy(),
