...
fn oracle(
    ...
    for edge in edges.iter().filter(|edge| edge.entry.relation == R) {
        let source = vertices
            ...
            .unwrap();
        if !source.labels.contains(&OWNER) {
            continue;
        }
...
fn canonical_pages_cover_all_read_surfaces_staging_and_reopened_history() {
    let ((), report) = run_async_under_lab(0x70f0_0001, |root| async move {
        ...
        assert_eq!(
            expected,
            vec![(Some(-1), VId(0), VId(10)), (Some(0), VId(0), VId(13))]
        );
        ...
        for mode in 0..3 {
            for change in 0..3 {
                ...
                match change {
                    ...
                    1 => {
                        winner.set_vertex_property(VId(10), RANK, Some(CanonicalScalar::Int(-5)));
                    }
                    _ => {
                        winner.create_vertex(
                            VId(13),
                            vec![],
                            vec![(RANK, CanonicalScalar::Int(-10))],
                        );
...
fn page_sources_keep_exact_limits_and_owner_and_snapshot_error_precedence() {
    let ((), report) = run_async_under_lab(0x70f0_0003, |root| async move {
        ...
        assert!(matches!(
            txn.execute_graph_pattern_governed(&foreign, &cx, &pattern, zero),
            Err(GqlQueryError::Source(WriteTxnError::WrongDatabase))
        ));
        assert!(matches!(
            db.execute_graph_pattern_governed_at(&cx, &pattern, CommitSeq(basis.0 + 1), zero),
