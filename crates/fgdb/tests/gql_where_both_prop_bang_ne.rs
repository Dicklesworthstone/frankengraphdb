//! **`WHERE a.k != 1 AND b.m <> 9` — the C-style conjunct aliases `<>`**
//! (`fgdb-w5-parsers-nje-64-0avu`).
//!
//! The C-style spelling inside the both-end conjunction: `!=` on the
//! source conjunct filters exactly like `<>`, so beside the literal `[8]`
//! pin the `!=` rows are asserted EQUAL to the all-`<>` rows themselves —
//! one comparator, two spellings, under the same missing-is-OUT law
//! (the `9→10` edge's keyless source excludes rather than trivially
//! satisfying the inequality). Swapping the conjunct order changes
//! nothing, the mixed `!=`/`=` conjunction answers the nje.18 sibling
//! lock `[6]`, and one refusal holds the grammar's edge: a SAME-end
//! conjunction stays typed Parse.
...
const BANG_BOTH: &str = "MATCH (a)-[:R]->(b) WHERE a.k != 1 AND b.m <> 9 RETURN b";
const BANG_BOTH_REVERSED: &str = "MATCH (a)-[:R]->(b) WHERE b.m <> 9 AND a.k != 1 RETURN b";
...
#[test]
fn both_end_bang_inequality_aliases_the_angle_spelling() {
    let ((), report) = run_async_under_lab(0x64_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-both-prop-bang-ne-{}",
            std::process::id()
        ));
        ...
        let bang = db
            .execute_gql(BANG_BOTH, &bind)
            .expect("both-end C-style conjunction MATCH executes");
        assert_eq!(
            bang,
            vec![VId(8)],
        ...
        assert_eq!(
            db.execute_gql(BANG_BOTH_REVERSED, &bind)
