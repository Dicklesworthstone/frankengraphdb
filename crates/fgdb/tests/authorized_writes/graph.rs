...
fn mixed_graph_batch_uses_native_ensure_aliases_and_reopens() {
    under_lab(0xa911, |contexts| async move {
        ...
        let seq = db.write_authorized(&txn, &cx, &authority, &token, BRANCH, batch, || NOW)
            .await.unwrap();
        assert_eq!(seq.0, frontier.0 + 1);
        assert_eq!(txn.outstanding_obligations(), baseline);
        assert_eq!(db.edge_at(EId(10), seq).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(7))]);
        ...
        assert_eq!(reopened.edge_at(EId(10), seq).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(7))]);
        assert!(reopened.edge_at(EId(99), seq).unwrap().is_none());
        assert!(reopened.edge_at(EId(12), seq).unwrap().is_none());
        assert_eq!(reopened.neighbours(VId(1), R).unwrap(), vec![VId(1), VId(2)]);
    });
}
...
fn vertex_delete_authorizes_and_removes_all_prefix_incidence() {
    under_lab(0xa912, |contexts| async move {
        let cx = contexts.commit();
        let txn = contexts.txn();
        let mut db = Database::create(&cx, &scratch("prefix-cascade"), keys()).await.unwrap();
        let authority = issuer(NAMESPACE);
        let token = authority.issue_at(&grant(), NOW).unwrap();
        let baseline = txn.outstanding_obligations();
        let mut batch = WriteBatch::new(R);
        ...
        db.write_authorized(&txn, &cx, &authority, &token, BRANCH, recreate, || NOW).await.unwrap();
...
fn hidden_relation_in_a_cascade_refuses_the_whole_batch() {
    under_lab(0xa913, |contexts| async move {
        ...
        assert!(matches!(
            db.write_authorized(&txn, &cx, &authority, &token, BRANCH, delete, || NOW).await,
        ...
        let mut delete = WriteBatch::new(R);
        delete.delete_vertex(VId(1));
        let seq = db.write_authorized(&txn, &cx, &authority, &token, BRANCH, delete, || NOW)
            .await.unwrap();
        assert_eq!(seq.0, frontier.0 + 1);
        assert_eq!(txn.outstanding_obligations(), baseline);
        drop(db);
...
fn hidden_endpoint_blocks_edge_create_update_and_cascade() {
    under_lab(0xa914, |contexts| async move {
        let cx = contexts.commit();
        let txn = contexts.txn();
        let mut db = Database::create(&cx, &scratch("hidden-endpoint"), keys()).await.unwrap();
        let mut seed = WriteBatch::new(R);
        ...
        for batch in [create, update, delete] {
            ...
            assert_eq!(db.edge_at(EId(10), frontier).unwrap().unwrap().props,
...
fn ignored_ensure_alias_cannot_capture_an_unrelated_hidden_edge() {
    under_lab(0xa915, |contexts| async move {
        let cx = contexts.commit();
        let txn = contexts.txn();
        let mut db = Database::create(&cx, &scratch("alias-collision"), keys()).await.unwrap();
        let mut seed = WriteBatch::new(R);
        seed.create_vertex(VId(1), vec![L], vec![]);
        ...
        hidden.add_edge(EId(99), VId(3), VId(4), vec![(SECRET, CanonicalScalar::Int(99))]);
...
fn whole_object_deletion_cannot_erase_untouched_hidden_properties() {
    under_lab(0xa916, |contexts| async move {
        let cx = contexts.commit();
        let txn = contexts.txn();
        let mut db = Database::create(&cx, &scratch("hidden-fields-delete"), keys()).await.unwrap();
        let mut seed = WriteBatch::new(R);
        ...
        let mut update = WriteBatch::new(R);
        update.set_edge_property(EId(10), P, Some(CanonicalScalar::Int(7)));
        let frontier = db.write_authorized(&txn, &cx, &authority, &token, BRANCH, update, || NOW)
            .await.unwrap();
        assert_eq!(db.edge_at(EId(10), frontier).unwrap().unwrap().props,
            vec![(P, CanonicalScalar::Int(7)), (SECRET, CanonicalScalar::Int(88))]);
        let baseline = txn.outstanding_obligations();
