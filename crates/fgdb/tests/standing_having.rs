...
use fgdb_gql::algebra::{GlaDirection, GraphColumn, GraphMatchClause, GraphPatternBuilder, IntegerComparison};
use fgdb_gql::{
    GraphAggregate, GraphAggregateColumn as Column, GraphAggregateFilter, GraphAggregateOrder,
    GraphAggregateRow, GraphAggregateTest, GraphHavingExpression, GraphHavingOp as Op,
    GraphHavingOperand as Arg, GqlQueryPolicy, PreparedGraphAggregate,
};
...
fn threshold(query: PreparedGraphAggregate, comparison: IntegerComparison, value: i128) -> PreparedGraphAggregate {
...
fn combined_having(query: PreparedGraphAggregate) -> PreparedGraphAggregate {
    let having = GraphHavingExpression::prepare(&[
        ...
        Op::Compare { left: Arg::Column(Column::Aggregate(3)),
            comparison: IntegerComparison::GreaterOrEqual, right: Arg::Integer(2) },
        Op::And,
        Op::IsNull { operand: Arg::Column(Column::Aggregate(2)), is_null: true },
        Op::Or,
    ]).unwrap();
    ...
    let result = query.execute_governed(
        ...
        |vid, predicates| {
            ...
            Ok::<_, ()>(predicates.iter().all(|predicate| {
                predicate.matches_borrowed(row.labels.iter().copied(),
...
fn check(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle, query: &PreparedGraphAggregate) {
...
fn having_matches_snapshot_across_grouped_global_and_all_admitted_join_shapes() {
    let ((), report) = run_async_under_lab(0x6a51, |root| async move {
        ...
        for id in 1..=5 {
            ...
            if id != 4 { props.push((VALUE, CanonicalScalar::Int(id as i64))); }
        ...
        for (handle, definition) in &registered {
            ...
            assert_eq!(db.standing_query(&query, handle).unwrap().rows().len(), expected);
        }
    });
...
fn global_empty_rows_and_late_registration_cross_having_without_phantom_retractions() {
    let ((), report) = run_async_under_lab(0x6a52, |root| async move {
        ...
        let query = contexts.query();
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let zero = threshold(definition(Shape::Vertex, GlaDirection::Forward, false), IntegerComparison::Equal, 0);
        let two = threshold(definition(Shape::Vertex, GlaDirection::Forward, false), IntegerComparison::GreaterOrEqual, 2);
        let h0 = db.register_standing_query(&query, zero.clone(), policy()).unwrap();
        let h2 = db.register_standing_query(&query, two.clone(), policy()).unwrap();
        ...
        let late = db.register_standing_query(&query, zero.clone(), policy()).unwrap();
        ...
        for (write, counts) in [
            (unrelated, (0, 0)), (second, (0, 1)), (remove_first, (0, 0)), (remove_second, (1, 0)),
        ] {
            db.write(&commit, write).await.unwrap();
            check(&db, &query, &h0, &zero);
            check(&db, &query, &late, &zero);
            check(&db, &query, &h2, &two);
            assert_eq!(db.standing_query(&query, &h0).unwrap().rows().len(), counts.0);
            assert_eq!(db.standing_query(&query, &h2).unwrap().rows().len(), counts.1);
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0x6a53, |root| async move {
        ...
        for id in 1..=6 {
            seed.create_vertex(VId(id), vec![], vec![
                (GROUP, CanonicalScalar::Int(id as i64)), (VALUE, CanonicalScalar::Int(id as i64)),
            ]);
        }
        db.write(&commit, seed).await.unwrap();
        let definition = threshold(definition(Shape::Vertex, GlaDirection::Forward, true), IntegerComparison::GreaterOrEqual, 2);
        let bounded = GqlQueryPolicy::new(100_000, 1, 10_000_000, 10_000_000);
        let handle = db.register_standing_query(&query, definition.clone(), bounded).unwrap();
        ...
        let add = |id, group| {
            ...
            write.create_vertex(VId(id), vec![], vec![
                (GROUP, CanonicalScalar::Int(group)), (VALUE, CanonicalScalar::Int(10)),
            ]);
            write
        };
        ...
        assert!(matches!(db.rebuild_standing_query(&query, &handle, bounded),
...
fn invalid_having_domains_are_not_hidden_by_false_filters_or_true_disjunctions() {
    let ((), report) = run_async_under_lab(0x6a54, |root| async move {
        ...
        let ordinary = base.clone().with_result_clauses(&[
            GraphAggregateFilter { column: Column::Aggregate(0), test: GraphAggregateTest::Integer {
                comparison: IntegerComparison::Less, value: 0,
            }},
            GraphAggregateFilter { column: Column::GroupKey(0), test: GraphAggregateTest::Integer {
                comparison: IntegerComparison::Equal, value: 1,
            }},
        ], &[]).unwrap();
        let boolean = base.with_having_expression(&GraphHavingExpression::prepare(&[
            Op::Truth(Some(true)),
            Op::Compare { left: Arg::Column(Column::GroupKey(0)),
                comparison: IntegerComparison::Equal, right: Arg::Integer(1) }, Op::Or,
        ]).unwrap()).unwrap();
        let mut registered = Vec::new();
        ...
        for (handle, definition) in &registered {
            ...
            assert!(matches!(db.rebuild_standing_query(&query, handle, policy()),
                Err(StandingQueryError::Maintenance(StandingQueryFailure::NonIntegerHaving))));
            assert!(matches!(db.register_standing_query(&query, definition.clone(), policy()),
                Err(StandingQueryError::Maintenance(StandingQueryFailure::NonIntegerHaving))));
        }
        ...
        for (handle, definition) in &registered {
            ...
            assert!(matches!(db.standing_query(&query, handle), Err(StandingQueryError::Unavailable { .. })));
            assert_eq!(db.rebuild_standing_query(&query, handle, policy()).unwrap(), at);
            check(&db, &query, handle, definition);
        }
    });
...
fn unrelated_hidden_groups_do_not_increase_having_maintenance_work() {
    let ((), report) = run_async_under_lab(0x6a55, |root| async move {
        ...
        for extra in [0, 500] {
            ...
            let mut seed = WriteBatch::new(R);
            for id in 1..=2 {
                seed.create_vertex(VId(id), vec![], vec![(GROUP, CanonicalScalar::Int(0)), (VALUE, CanonicalScalar::Int(1))]);
            }
            for id in 10..10 + extra {
                seed.create_vertex(VId(id), vec![], vec![
                    (GROUP, CanonicalScalar::Int(id as i64)), (VALUE, CanonicalScalar::Int(1)),
                ]);
            }
            db.write(&commit, seed).await.unwrap();
            let handle = db.register_standing_query(&query, definition.clone(), policy()).unwrap();
