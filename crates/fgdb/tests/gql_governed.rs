...
use fgdb::{
    Database, DatabaseKeys, DerivedPublicationStage, GqlError, MemVfs, ReadError, RelationBind,
...
fn governed_aliases_agree_with_expected_rows_across_every_product_surface() {
    let ((), report) = run_async_under_lab(0xc0a2_0001, |root| async move {
        ...
        for (statement, expected) in [
            ...
            assert_eq!(
                db.execute_prepared_query_governed_at(&query_cx, &query, basis, generous())
                    .unwrap(),
                live
            );
            assert_eq!(
            ...
            assert_eq!(
                pinned
                    .execute_prepared_query_governed_at(&query_cx, &query, basis, generous())
                    .unwrap(),
                live
            );
            let overlay = txn
        ...
        let template = PreparedGqlTemplate::prepare(
            "MATCH (a)-[:R]->(a) WHERE a.n=$n RETURN a LIMIT$cap",
            &names(),
        )
        .unwrap();
        let query = template
            ...
            .unwrap();
        assert_eq!(
            db.execute_prepared_query_governed(&query_cx, &query, generous())
                .unwrap()
                .value,
            vec![VId(2)]
        );
        ...
        let wrong =
            GqlPreparedResultArtifact::new(&query, basis, certificate.digest, vec![VId(1), VId(2)]);
        assert!(matches!(
            db.audit_prepared_query_artifact(&query, &wrong.to_bytes()),
...
fn four_dimensions_refuse_independently_with_exact_boundary_success() {
    let ((), report) = run_async_under_lab(0xc0a2_0002, |root| async move {
        ...
        assert_eq!(
            db.execute_prepared_query_governed(&query_cx, &query, exact)
                .unwrap(),
            measured
        );
        assert!(
        ...
        let skipped =
            PreparedGqlQuery::prepare("MATCH (a)-[:R]->(b) RETURN b SKIP 1", &names()).unwrap();
        let empty = db
    ...
    let ((), report) = run_async_under_lab(0xc0a2_0003, |root| async move {
        ...
        for winner_kind in 0..3 {
            ...
            winner.create_vertex(
                VId(77),
                if winner_kind == 1 { vec![L] } else { vec![] },
                vec![],
            );
...
fn invalid_sources_are_not_disguised_as_zero_budget_errors() {
    let ((), report) = run_async_under_lab(0xc0a2_0004, |root| async move {
        ...
        assert!(matches!(
            db.execute_prepared_query_governed_at(&query_cx, &query, future, zero),
            Err(GqlQueryError::Source(GqlError::Read(
                ReadError::BeyondFrontier { .. }
            )))
        ));
        assert!(matches!(
            pinned.execute_prepared_query_governed_at(&query_cx, &query, future, zero),
            Err(GqlQueryError::Source(GqlError::Read(
                ReadError::BeyondFrontier { .. }
            )))
        ));
        ...
        assert!(matches!(
            txn.execute_prepared_query_governed(&foreign, &query_cx, &query, zero),
        ...
        assert!(matches!(
            db.execute_prepared_query_governed(&query_cx, &query, zero),
            Err(GqlQueryError::Source(GqlError::Read(
...
fn governed_artifact_replay_keeps_all_identity_and_staged_effect_checks() {
    ...
    let ((), report) = run_async_under_lab(0xc0a2_0005, |root| async move {
        ...
        assert!(matches!(
            db.audit_prepared_query_artifact_governed(&query_cx, &query, &bytes, limits, stop),
            Err(GqlEvidenceLimitedAuditError::Audit(
                GqlEvidenceAuditError::Execution(GqlQueryError::Evaluator(_))
            ))
        ));
        assert_eq!(
        ...
        assert_eq!(
            pinned
                .audit_prepared_query_artifact_governed(
                    &query_cx,
                    &query,
                    &bytes,
                    limits,
        ...
        assert!(matches!(
            db.audit_prepared_query_artifact_governed(&query_cx, &other, &bytes, limits, stop),
            Err(GqlEvidenceLimitedAuditError::Audit(
                GqlEvidenceAuditError::InputMismatch
            ))
        ));
        let mut txn = db.begin(&txn_cx).unwrap();
        let overlay = txn
