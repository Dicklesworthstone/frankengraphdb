...
fn governed_aliases_agree_with_expected_rows_across_every_product_surface() {
    let ((), report) = run_async_under_lab(0xc0a2_0001, |root| async move {
        ...
        for (statement, expected) in [
            ...
            let overlay = txn.execute_prepared_query_governed(&db, &query_cx, &query, generous()).unwrap();
...
fn source_history_is_metered_and_property_versions_do_not_multiply_candidate_scratch() {
    let ((), report) = run_async_under_lab(0xc0a2_0006, |root| async move {
        ...
        let mut db = seeded(&commit).await;
        let query = PreparedGqlQuery::prepare("MATCH (a)-[:R]->(b) RETURN b", &names()).unwrap();
        let before = db.execute_prepared_query_governed(&query_cx, &query, generous()).unwrap();
        for byte in 1..=4 {
            let mut update = WriteBatch::new(R);
            update.set_edge_property(EId(10), N, Some(CanonicalScalar::bytes(vec![byte; 8_000]).unwrap()));
            db.write(&commit, update).await.unwrap();
        }
        let after = db.execute_prepared_query_governed(&query_cx, &query, generous()).unwrap();
        assert_eq!(after.value, before.value);
        assert_eq!(after.rows, before.rows);
        assert_eq!(after.evaluator.scratch_entries, before.evaluator.scratch_entries);
        ...
        for work in 0..after.evaluator.work_units {
            ...
            assert!(matches!(db.execute_prepared_query_governed(&query_cx, &query, policy),
...
fn source_oracle(db: &Database<MemVfs>, at: CommitSeq) -> Vec<(Vec<VId>, u64)> {
    ...
    let vertices: BTreeMap<_, _> = db.vertices_at(at).unwrap().into_iter().map(|r| (r.vid, r)).collect();
    let edges = db.edges_at(at).unwrap();
    let qualifying = |vid: VId| vertices.get(&vid).is_some_and(|row| row.props.iter().any(|(key, value)|
        *key == N && matches!(value, CanonicalScalar::Int(n) if *n >= 3)));
    let nodes = vertices.values().filter(|row| row.labels.contains(&L) && qualifying(row.vid))
        .map(|row| row.vid).collect();
    let destinations: BTreeSet<_> = edges.iter().filter(|r| r.entry.relation == R && qualifying(r.entry.dst))
        .map(|r| r.entry.dst).collect();
    ...
    vec![(nodes, vertices.len() as u64), (destinations.into_iter().collect(), edges.len() as u64),
        (closed.into_iter().collect(), edges.len() as u64)]
}

fn check_source_history(db: &Database<MemVfs>, query_cx: &fgdb_types::QueryCx) -> Vec<Vec<(Vec<VId>, u64)>> {
    let queries: Vec<_> = [
        "MATCH (a:L) WHERE a.n>=3 RETURN a",
        "MATCH (a)-[:R]->(b) WHERE b.n>=3 RETURN b",
        "MATCH (a)-[:R]->(b)-[:S]->(a) RETURN a",
    ].into_iter().map(|text| PreparedGqlQuery::prepare(text, &names()).unwrap()).collect();
    ...
    for seq in 0..=db.frontier().unwrap().0 {
        ...
        for (query, (rows, count)) in queries.iter().zip(&expected) {
            ...
            let run = db.execute_prepared_query_governed_at(query_cx, query, at, generous()).unwrap();
            assert_eq!(run.value, *rows);
            assert_eq!(run.rows.snapshot_records, *count);
            assert_eq!(view.execute_prepared_query_governed_at(query_cx, query, at, generous()).unwrap(), run);
        }
...
fn borrowed_sources_match_independent_storage_merges_through_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0xc0a2_0007, |root| async move {
        ...
        let pinned = db.read_session().unwrap();
        let pinned_query = PreparedGqlQuery::prepare("MATCH (a)-[:R]->(b) WHERE b.n>=3 RETURN b", &names()).unwrap();
        ...
        update.set_edge_property(EId(10), N, Some(CanonicalScalar::bytes(vec![0x42; 8_000]).unwrap()));
        db.write(&commit, update).await.unwrap();
        ...
        assert_eq!(pinned.execute_prepared_query_governed(&query_cx, &pinned_query, generous()).unwrap().value, old);
        db.compact(&commit).await.unwrap();
