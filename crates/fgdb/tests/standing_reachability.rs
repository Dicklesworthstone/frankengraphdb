...
fn keys() -> DatabaseKeys { DatabaseKeys::new([1; 32], DatabaseSecurityNamespaceId([2; 32]), [3; 32]) }
...
fn oracle(db: &Database<MemVfs>, relation: RelationId) -> BTreeSet<(VId, VId)> {
    ...
    let mut pairs: BTreeSet<_> = db.edges().unwrap().into_iter()
...
fn check(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle, relation: RelationId) {
    ...
    assert_eq!(view.rows().iter().map(|(pair, weight)| {
        assert_eq!(weight, &ZWeight::ONE); *pair
    }).collect::<BTreeSet<_>>(), oracle(db, relation));
}
fn count_definition() -> PreparedGraphAggregate {
    let mut graph = GraphPatternBuilder::new();
    graph.vertex("n").unwrap();
    let input = graph.prepare_values(&[GraphColumn::vertex("n", "n")], 0, None)
        .unwrap().with_duplicates();
    PreparedGraphAggregate::prepare(input, &[], &[GraphAggregate::count_rows("count")], 0, None).unwrap()
}
fn check_count(db: &Database<MemVfs>, cx: &QueryCx, handle: &StandingQueryHandle) {
    ...
    assert_eq!(row.values(), &[GraphAggregateValue::Count(db.vertices().unwrap().len() as u64)]);
}
...
fn commits_automatically_maintain_cycles_parallel_edges_cascades_and_native_siblings() {
    let ((), report) = run_async_under_lab(0x6a72, |root| async move {
        ...
        let r = db.register_standing_reachability(&query, R, policy()).unwrap();
        let s = db.register_standing_reachability(&query, S, policy()).unwrap();
        let count = db.register_standing_query(&query, count_definition(), policy()).unwrap();
        let wide = 1_u128 << 100;
        let verify = |db: &Database<MemVfs>| {
            check(db, &query, &r, R); check(db, &query, &s, S); check_count(db, &query, &count);
        };
        verify(&db);
        ...
        for (eid, from, to) in [(10, 0, 1), (11, 0, 1), (12, 1, 2), (13, 2, wide), (14, 3, 3)] {
        ...
        verify(&db);
        assert!(db.standing_reachability(&query, &r).unwrap().last_maintenance().work_units < 500);
        let mut partial = WriteBatch::new(R);
        partial.delete_edge(EId(10));
        db.write(&commit, partial).await.unwrap();
        verify(&db);
        assert!(db.standing_reachability(&query, &r).unwrap().rows().weight(&(VId(0), VId(0))).is_some());
        let mut a = WriteBatch::new(R);
        ...
        verify(&db);
        let late = db.register_standing_reachability(&query, R, policy()).unwrap();
        ...
        for id in [0, 1, 3, wide] { empty.delete_vertex(VId(id)); }
        db.write(&commit, empty).await.unwrap();
        verify(&db);
        check(&db, &query, &late, R);
        assert!(db.standing_reachability(&query, &r).unwrap().rows().is_empty());
    });
...
fn failed_views_do_not_abort_durable_writes_and_rebuild_uses_final_not_historical_size() {
    let ((), report) = run_async_under_lab(0x6a73, |root| async move {
        ...
        let r = db.register_standing_reachability(&query, R, limited).unwrap();
        let s = db.register_standing_reachability(&query, S, policy()).unwrap();
        let count = db.register_standing_query(&query, count_definition(), policy()).unwrap();
        ...
        check(&db, &query, &s, S); check_count(&db, &query, &count);
        assert!(matches!(db.rebuild_standing_query(&query, &r, limited),
        ...
        assert!(matches!(db.standing_reachability(&query, &r),
            Err(StandingQueryError::Unavailable { frontier, reason: StandingQueryFailure::ResultBudget }) if frontier == basis));
        assert_eq!(db.rebuild_standing_query(&query, &r, limited).unwrap(), current);
        check(&db, &query, &r, R);
        ...
        check(&db, &query, &late, R);
        let before = *db.standing_reachability(&query, &r).unwrap().last_maintenance();
        assert!(matches!(db.rebuild_standing_query(&query, &r,
            GqlQueryPolicy::new(0, 100_000, 10_000_000, 10_000_000)),
            Err(StandingQueryError::Maintenance(StandingQueryFailure::SnapshotBudget))));
        check(&db, &query, &r, R);
        assert_eq!(db.standing_reachability(&query, &r).unwrap().last_maintenance(), &before);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn typed_access_ownership_work_refusal_and_final_cardinality_swaps_are_enforced() {
    let ((), report) = run_async_under_lab(0x6a74, |root| async move {
        ...
        assert!(matches!(db.standing_query(&query, &one), Err(StandingQueryError::Unsupported)));
        assert!(matches!(db.standing_reachability(&query, &native), Err(StandingQueryError::Unsupported)));
        let mut foreign = Database::open_memory(&commit, keys()).await.unwrap();
        assert!(matches!(foreign.standing_reachability(&query, &one), Err(StandingQueryError::ForeignHandle)));
        assert!(matches!(foreign.rebuild_standing_query(&query, &one, policy()), Err(StandingQueryError::ForeignHandle)));
        let mut first = WriteBatch::new(R);
        for id in [1, 2, 9, 10] { first.create_vertex(VId(id), vec![], vec![]); }
        first.add_edge(EId(1), VId(9), VId(10), vec![]);
        db.write(&commit, first).await.unwrap();
        assert!(matches!(db.standing_reachability(&query, &tiny),
            Err(StandingQueryError::Unavailable { frontier: CommitSeq::ORIGIN, reason: StandingQueryFailure::WorkBudget })));
        check(&db, &query, &one, R); check_count(&db, &query, &native);
        ...
        assert_eq!(db.standing_reachability(&query, &one).unwrap().rows().len(), 1);
...
fn sparse_live_maintenance_does_not_rescan_unrelated_components() {
    let ((), report) = run_async_under_lab(0x6a75, |root| async move {
        ...
        for components in [1_u128, 1_000] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let handle = db.register_standing_reachability(&query, R, policy()).unwrap();
            let mut seed = WriteBatch::new(R);
            for id in 0..components * 2 { seed.create_vertex(VId(id), vec![], vec![]); }
