...
fn seed() -> WriteBatch {
    ...
    for (id, price, qty, kind) in [
        ...
        batch.create_vertex(
            ...
            vec![
                (
                    PRICE,
                    price.map_or(CanonicalScalar::Null, CanonicalScalar::Int),
                ),
    ...
    for (id, from, to, price, qty, kind) in [
        ...
        (1, 0, 1, Some(-2), 3, "a"),
        (2, 1, 2, None, 5, "B"),
        (u128::MAX, u128::MAX, 0, Some(4), 2, "b"),
    ] {
        batch.add_edge(
            EId(id),
            VId(from),
...
fn native_computed_keys_aliases_and_duplicate_arguments_match_batch_and_hand_answers() {
    let ((), report) = run_async_under_lab(0xa66e_c001, |root| async move {
        ...
        for text in [
            ...
            let actual = cursor
                .by_ref()
                .map(|r| visible(&r.unwrap(), &slots))
                .collect::<Vec<_>>();
            assert_eq!(actual, rows, "{text}");
            assert_eq!(cursor.state(), VertexScanState::Exhausted);
        }
        let rows = db
            .query_aggregate_stream(&cx, SUMMARY, &GqlParameters::new(), symbols(), wide())
            .unwrap()
...
fn projection_quota_and_late_overflow_refusals_are_terminal_and_never_publish_partial_groups() {
    let ((), report) = run_async_under_lab(0xa66e_c003, |root| async move {
        ...
        let args = GqlParameters::new();
        let mut empty = db
            .query_aggregate_stream(&cx, SUMMARY, &args, symbols(), wide())
            .unwrap();
        ...
        for text in [
            ...
            assert!(
                matches!(
                    query.stream_aggregate(&db, &cx, &args, GqlQueryPolicy::new(0, 0, 0, 0)),
                    Err(QueryError::EdgeAggregateStreamPlan(_))
                ),
                "{text}"
            );
        }
    ...
    let ((), report) = run_async_under_lab(0xa66e_c004, |root| async move {
        ...
        for text in [
            ...
            let actual: Vec<_> = cursor
                .by_ref()
                .map(|row| visible(&row.unwrap(), &slots))
                .collect();
            assert_eq!(actual, rows, "{text}");
            assert_eq!(cursor.state(), VertexScanState::Exhausted);
            assert!(cursor.next().is_none());
...
fn computed_vertex_history_and_scalar_parameters_survive_compaction_and_handle_drop() {
    let ((), report) = run_async_under_lab(0xa66e_c005, |root| async move {
        ...
        let mut current = prepared
            .stream_aggregate(&db, &cx, &latest, wide())
            .unwrap();
        ...
        assert_eq!(pinned.row_stats().snapshot_records, 0);
        assert_eq!(
            pinned.next().unwrap().unwrap().values()[0].as_integer(),
            Some(12)
        );
        assert_eq!(
            current.next().unwrap().unwrap().values()[0].as_integer(),
            Some(36)
        );
...
fn computed_vertex_empty_results_quotas_and_late_errors_do_not_bypass_admission() {
    let ((), report) = run_async_under_lab(0xa66e_c006, |root| async move {
        ...
        let args = GqlParameters::new();
        let mut grouped = db
            .query_aggregate_stream(&cx, VERTEX_SUMMARY, &args, symbols(), wide())
            .unwrap();
        ...
        let mut empty = db
            .query_aggregate_stream(
                &cx,
                "MATCH (n:L) RETURN COUNT(*) AS n, SUM(1/0) AS total",
                &args,
                symbols(),
                wide(),
            )
        ...
        assert!(matches!(
            bad.next(),
            Some(Err(GqlQueryError::Source(
                GraphAggregateError::InputExpression { row: 0, .. }
            )))
        ));
        assert_eq!(bad.row_stats().result_rows, 0);
        assert!(bad.next().is_none());
