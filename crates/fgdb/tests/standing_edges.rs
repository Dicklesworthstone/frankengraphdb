...
fn definition(direction: GlaDirection, identity: Option<bool>) -> PreparedGraphAggregate {
    ...
    pattern
        .filter("a", VertexPredicate::HasLabel(SOURCE))
        .unwrap();
    pattern
        .filter(
            "b",
    ...
    if let Some(equal) = identity {
        pattern.identity("a", "b", equal).unwrap();
    }
    let input = pattern
        .prepare_values(
            &[
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) {
    ...
    for (id, labelled, selected, bucket, amount) in [
        ...
        let mut props = vec![
            (BUCKET, CanonicalScalar::Int(bucket)),
            (SELECTED, CanonicalScalar::Int(selected)),
        ];
        if let Some(value) = amount {
            props.push((AMOUNT, CanonicalScalar::Int(value)));
        }
...
fn assert_state(
    ...
    for edge in db.edges().unwrap() {
        ...
        for (a, b) in orientations {
            if identity.is_some_and(|equal| (a == b) != equal) {
                continue;
            }
            let (a, b) = (by_id[&a], by_id[&b]);
            if !a.labels.contains(&SOURCE)
                || !b.props.iter().any(|(key, value)| {
                    *key == SELECTED && matches!(value, CanonicalScalar::Int(1))
                })
            {
                continue;
            }
            ...
            expected
                .entry(vec![GraphValue::Scalar(bucket)])
                .or_default()
                .push(amount);
        }
    }
    ...
    for (row, weight) in maintained.rows().iter() {
        ...
        if numbers.is_empty() {
            ...
            assert_eq!(
                row.get(3).unwrap().as_average(),
                GraphExactAverage::new(sum, numbers.len() as u64)
            );
        }
    }
...
fn directions_parallel_edges_endpoint_changes_and_cascades_match_storage() {
    let ((), report) = run_async_under_lab(0x7e01, |root| async move {
        ...
        for step in 0..7 {
            ...
            match step {
                0 => {
                    // Both endpoints change in the same tick; shared edges retract once.
                ...
                4 => {
                    // Deletes parallel, incoming, outgoing and self-loop occurrences.
                    batch.delete_vertex(VId(2));
                    batch.set_vertex_property(VId(1), AMOUNT, Some(CanonicalScalar::Int(9)));
                }
                5 => {
                    // An explicit delete and two endpoint cascades overlap.
        ...
        for (query, old, direction, identity) in queries {
            assert!(matches!(
                reopened.standing_query(&cx, &old),
...
fn initialization_admits_edges_and_failed_maintenance_can_rebuild() {
    let ((), report) = run_async_under_lab(0x7e04, |root| async move {
        ...
        assert!(matches!(
            db.register_standing_query(
                &cx,
                query.clone(),
                GqlQueryPolicy::new(5, 10_000, 10_000_000, 10_000_000)
            ),
            Err(fgdb::StandingQueryError::Maintenance(
                fgdb::StandingQueryFailure::SnapshotBudget
            ))
        ));
        let probe = db
            .register_standing_query(&cx, query.clone(), policy())
        ...
        let handle = db
            .register_standing_query(&cx, query.clone(), exact)
            .unwrap();
        for cap in [
        ...
        assert!(matches!(
            ...
            Err(fgdb::StandingQueryError::Unavailable { .. })
        ));
        ...
        for same in [false, true] {
            ...
            let query = PreparedGraphAggregate::prepare(
                ...
                &[
                    ...
                    GraphAggregate::count("vertices", 0),
                ],
                0,
                None,
            )
