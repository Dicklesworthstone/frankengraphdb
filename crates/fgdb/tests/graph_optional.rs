...
use fgdb_gql::{GraphAggregate, GqlBudgetDimension, GqlQueryError, GqlQueryPolicy, PreparedGraphAggregate};
...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(1000, 1000, 1_000_000, 100_000) }
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in [10, 11, 12] { vertices.create_vertex(VId(id), vec![], vec![]); }
...
fn outer() -> GraphPatternBuilder {
    ...
    outer.filter("person", VertexPredicate::HasLabel(PERSON)).unwrap();
    outer
}
fn parts() -> (GraphPatternBuilder, GraphPatternBuilder, GraphPatternBuilder) {
    let mut first = GraphPatternBuilder::new();
    for name in ["person", "bridge"] { first.vertex(name).unwrap(); }
    first.edge("person", R, GlaDirection::Forward, "bridge").unwrap();
    let mut second = GraphPatternBuilder::new();
    for name in ["bridge", "company"] { second.vertex(name).unwrap(); }
    second.edge("bridge", S, GlaDirection::Forward, "company").unwrap();
    second.filter("company", VertexPredicate::IntegerProperty {
        key: SCORE, comparison: IntegerComparison::GreaterOrEqual, value: 5,
    }).unwrap();
    let mut whole = first.clone();
    whole.vertex("company").unwrap();
    whole.edge("bridge", S, GlaDirection::Forward, "company").unwrap();
    whole.filter("company", VertexPredicate::IntegerProperty {
        key: SCORE, comparison: IntegerComparison::GreaterOrEqual, value: 5,
    }).unwrap();
    (first, second, whole)
}
fn columns() -> [GraphColumn<'static>; 4] {
    [GraphColumn::vertex("owner", "person"), GraphColumn::vertex("via", "bridge"),
...
fn pattern(split: bool) -> PreparedGraphPattern<GraphValueRow> {
    ...
    let outer = outer();
    if split {
        outer.prepare_values_with_clauses(&[GraphMatchClause::optional(&first), GraphMatchClause::optional(&second)],
            &columns(), 0, None).unwrap().with_duplicates()
    } else {
        outer.prepare_values_with_clauses(&[GraphMatchClause::optional(&whole)], &columns(), 0, None).unwrap().with_duplicates()
    }
}
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord], split: bool) -> Vec<Plain> {
    ...
    for person in vertices.iter().filter(|row| row.labels.contains(&PERSON)) {
        let before = rows.len();
        for first in edges.iter().filter(|edge| edge.entry.src == person.vid && edge.entry.relation == R) {
            let previous = rows.len();
            for second in edges.iter().filter(|edge| edge.entry.src == first.entry.dst && edge.entry.relation == S) {
                let company = vertices.iter().find(|row| row.vid == second.entry.dst).unwrap();
                let value = company.props.iter().find(|(key, _)| *key == SCORE).map(|(_, value)| value);
                if let Some(CanonicalScalar::Int(score)) = value && *score >= 5 {
                    rows.push((person.vid, Some(first.entry.dst), Some(company.vid), Some(*score)));
                }
            }
        ...
        if rows.len() == before { rows.push((person.vid, None, None, None)); }
    }
    rows.sort();
    rows
}
...
fn all_five_surfaces_preserve_optional_bags_and_share_one_policy() {
    let ((), report) = run_async_under_lab(0x0f71_0001, |root| async move {
        ...
        let pinned = db.read_session().unwrap(); let txn = db.begin(&txn_cx).unwrap();
        ...
        for split in [false, true] {
            ...
            assert_eq!(expected.iter().find(|row| row.0 == VId(2)).unwrap().1, split.then_some(VId(12)));
            for rows in [
                db.execute_graph_pattern_governed(&cx, &pattern, policy()).unwrap().value,
                db.execute_graph_pattern_governed_at(&cx, &pattern, basis, policy()).unwrap().value,
                pinned.execute_graph_pattern_governed(&cx, &pattern, policy()).unwrap().value,
                pinned.execute_graph_pattern_governed_at(&cx, &pattern, basis, policy()).unwrap().value,
                txn.execute_graph_pattern_governed(&db, &cx, &pattern, policy()).unwrap().value,
            ...
            let exact = GqlQueryPolicy::new(17, 8, full.evaluator.work_units, full.evaluator.scratch_entries);
            assert_eq!(db.execute_graph_pattern_governed(&cx, &pattern, exact).unwrap(), full);
            for cap in [GqlQueryPolicy::new(17, 8, full.evaluator.work_units - 1, u64::MAX),
                GqlQueryPolicy::new(17, 8, u64::MAX, full.evaluator.scratch_entries - 1)] {
                assert!(matches!(db.execute_graph_pattern_governed(&cx, &pattern, cap),
                    Err(GqlQueryError::Evaluator(error)) if error.observed == u128::from(error.limit) + 1));
            }
            assert!(matches!(db.execute_graph_pattern_governed(&cx, &pattern, GqlQueryPolicy::new(16, 8, u64::MAX, u64::MAX)),
                Err(GqlQueryError::Rows(error)) if error.dimension == GqlBudgetDimension::SnapshotRecords && error.observed == 17));
            assert!(matches!(db.execute_graph_pattern_governed(&cx, &pattern, GqlQueryPolicy::new(17, 7, u64::MAX, u64::MAX)),
...
fn staged_ensures_property_removal_and_cascades_match_committed_and_historical_rows() {
    let ((), report) = run_async_under_lab(0x0f71_0002, |root| async move {
        ...
        let before: Vec<_> = patterns.iter().map(|pattern|
            db.execute_graph_pattern_governed(&cx, pattern, policy()).unwrap().value).collect();
        ...
        let vertices = txn.vertices(&db).unwrap(); let edges = txn.edges(&db).unwrap();
        ...
        for (at, pattern) in patterns.iter().enumerate() {
            ...
            let result = txn.execute_graph_pattern_governed(&db, &cx, pattern, policy()).unwrap();
            assert_eq!(plain(&result.value), expected);
            assert_eq!(result.value.len(), 5); assert_eq!(result.rows.snapshot_records, 15);
            let exact = GqlQueryPolicy::new(15, 5, result.evaluator.work_units, result.evaluator.scratch_entries);
            assert_eq!(txn.execute_graph_pattern_governed(&db, &cx, pattern, exact).unwrap(), result);
            assert_eq!(db.execute_graph_pattern_governed(&cx, pattern, policy()).unwrap().value, before[at]);
            staged.push(result.value);
        }
        txn.commit(&mut db, &commit).await.unwrap();
        for (at, pattern) in patterns.iter().enumerate() {
            assert_eq!(db.execute_graph_pattern_governed(&cx, pattern, policy()).unwrap().value, staged[at]);
            assert_eq!(db.execute_graph_pattern_governed_at(&cx, pattern, basis, policy()).unwrap().value, before[at]);
        }
        db.compact(&commit).await.unwrap(); drop(db);
        let reopened = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        for (at, pattern) in patterns.iter().enumerate() {
            assert_eq!(reopened.execute_graph_pattern_governed(&cx, pattern, policy()).unwrap().value, staged[at]);
            assert_eq!(reopened.execute_graph_pattern_governed_at(&cx, pattern, basis, policy()).unwrap().value, before[at]);
            assert_eq!(pinned.execute_graph_pattern_governed(&cx, pattern, policy()).unwrap().value, before[at]);
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
    ...
    let ((), report) = run_async_under_lab(0x0f71_0003, |root| async move {
        ...
        for refused in [false, true] {
            for change in 0..3 {
                ...
                let pattern = pattern(false); let mut txn = db.begin(&txn_cx).unwrap();
                let mut staged = WriteBatch::new(R); staged.create_vertex(VId(99), vec![], vec![]);
                ...
                let old = if refused {
                    assert!(matches!(result, Err(GqlQueryError::Rows(_)))); None
                } else {
                    let rows = result.unwrap().value;
                    assert!(plain(&rows).contains(&(VId(2), None, None, None))); Some(rows)
                };
                ...
                if let Some(old) = old {
                    assert_eq!(txn.execute_graph_pattern_governed(&db, &cx, &pattern, policy()).unwrap().value, old);
                }
...
fn optional_values_compose_with_semijoins_antijoins_and_streaming_grouping() {
    let ((), report) = run_async_under_lab(0x0f71_0004, |root| async move {
        ...
        let mut company = GraphPatternBuilder::new(); company.vertex("company").unwrap();
        for exists in [false, true] {
            let outer = outer();
            let query = outer.prepare_values_with_clauses(&[
                GraphMatchClause::optional(&first), GraphMatchClause::optional(&second),
                if exists { GraphMatchClause::exists(&company) } else { GraphMatchClause::not_exists(&company) },
            ], &columns(), 0, None).unwrap().with_duplicates();
            let all = oracle(&db.vertices().unwrap(), &db.edges().unwrap(), true);
            let expected: Vec<_> = all.into_iter().filter(|row| row.2.is_some() == exists).collect();
        ...
        for split in [false, true] {
            let summary = PreparedGraphAggregate::prepare(pattern(split), &[0], &[
                GraphAggregate::count_rows("rows"), GraphAggregate::count("bridges", 1),
                GraphAggregate::count("companies", 2), GraphAggregate::count_distinct("unique_companies", 2),
                GraphAggregate::sum_int("sum", 3),
            ], 0, None).unwrap();
            let rows = db.execute_graph_aggregate_governed(&cx, &summary, policy()).unwrap().value;
            assert_eq!(rows.len(), 5);
            for row in &rows {
                let owner = row.keys()[0].as_vertex().unwrap();
                let qualified = owner == VId(0);
                let bridge = qualified || split && [VId(1), VId(2)].contains(&owner);
                assert_eq!(row.get(0).unwrap().as_count(), Some(if qualified { 4 } else { 1 }));
                assert_eq!(row.get(1).unwrap().as_count(), Some(if qualified { 4 } else { u64::from(bridge) }));
                assert_eq!(row.get(2).unwrap().as_count(), Some(if qualified { 4 } else { 0 }));
            ...
            assert_eq!(pinned.execute_graph_aggregate_governed(&cx, &summary, policy()).unwrap().value, rows);
            assert_eq!(txn.execute_graph_aggregate_governed(&db, &cx, &summary, policy()).unwrap().value, rows);
        }
        txn.abort();
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn optional_read_authority_and_snapshot_errors_precede_runtime_interruption() {
    let ((), report) = run_async_under_lab(0x0f71_0005, |root| async move {
        ...
        let pinned = db.read_session().unwrap(); let txn = db.begin(&txn_cx).unwrap();
        let query = pattern(false); let future = CommitSeq(db.frontier().unwrap().0 + 1);
        root.cancel_with(CancelKind::User, Some("optional scope interruption"));
        assert!(matches!(txn.execute_graph_pattern_governed(&foreign, &cx, &query, policy()),
            Err(GqlQueryError::Source(WriteTxnError::WrongDatabase))));
        assert!(matches!(db.execute_graph_pattern_governed_at(&cx, &query, future, policy()),
            Err(GqlQueryError::Source(GqlError::Read(ReadError::BeyondFrontier { .. })))));
        assert!(matches!(pinned.execute_graph_pattern_governed_at(&cx, &query, future, policy()),
            Err(GqlQueryError::Source(GqlError::Read(ReadError::BeyondFrontier { .. })))));
        assert!(matches!(db.execute_graph_pattern_governed(&cx, &query, policy()), Err(GqlQueryError::Interrupted(_))));
        assert!(matches!(txn.execute_graph_pattern_governed(&db, &cx, &query, policy()), Err(GqlQueryError::Interrupted(_))));
        txn.abort();
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
