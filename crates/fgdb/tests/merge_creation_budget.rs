...
fn merge(value: i64) -> PreparedGraphVertexMerge {
    ...
    let selection = PreparedGraphText::prepare(
        "MATCH (n) WHERE n.p=$value RETURN n", symbols,
    ).unwrap().bind_parameters(&arguments).unwrap();
    let creation = PreparedGraphInsertText::prepare(
        "CREATE (n {p:$value})", R, symbols,
    ).unwrap().bind_parameters(&arguments).unwrap();
...
fn zero_creation_budget_allows_matches_and_refuses_missing_before_allocation() {
    let ((), report) = run_async_under_lab(0x6e29_1001, |root| async move {
        ...
        let policy = GraphVertexMergePolicy::new(
        ...
        let error = txn.execute_graph_vertex_merge_governed(
            &mut db, &query, &merge(2), policy,
            |_| -> Result<ElementId, ()> { panic!("exhausted creation quota must not allocate") },
        ).unwrap_err();
        ...
        let (stats, outcome) = txn.execute_graph_vertex_merge_governed(
            &mut db, &query, &merge(2), policy.with_creation_limit(1),
