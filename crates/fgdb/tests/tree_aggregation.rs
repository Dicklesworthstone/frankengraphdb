...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    db.write_atomic(cx, vec![vertices, first, second, third])
        .await
        .unwrap()
}
fn oracle(edges: &[EdgeRecord]) -> Vec<Summary> {
    ...
    for ab in edges.iter().filter(|e| e.entry.relation == R) {
        for bc in edges
            .iter()
            .filter(|e| e.entry.relation == S && e.entry.src == ab.entry.dst)
        {
            for _cd in edges
                .iter()
                .filter(|e| e.entry.relation == T && e.entry.src == bc.entry.dst)
            {
                for _be in edges
                    .iter()
                    .filter(|e| e.entry.relation == S && e.entry.src == ab.entry.dst)
    ...
    rows.iter()
        .map(|row| {
            (
                ...
                row.get(4).unwrap().as_value().unwrap().as_vertex(),
            )
        })
        .collect()
}
...
fn forest_results_follow_canonical_staging_and_all_retained_read_surfaces() {
    let ((), report) = run_async_under_lab(0xf07e_0001, |root| async move {
        ...
        assert_eq!(
            expected,
            vec![
                (VId(0), 24, 24, 1, Some(VId(10)), Some(VId(10))),
        ...
        assert_eq!(
            staged,
            vec![(VId(0), 2, 2, 1, Some(VId(10)), Some(VId(10)))]
        );
        assert_eq!(
            plain(
                &txn.execute_graph_aggregate_governed(&db, &cx, &query, wide())
        ...
        for mode in 0..3 {
            for change in 0..3 {
                ...
                match change {
                    ...
                    1 => {
                        winner.add_edge(EId(90), VId(22), VId(30), vec![]);
                    }
                    _ => {
...
fn distinct_vertex_assignments_are_factored_in_the_actual_database_entrypoint() {
    let ((), report) = run_async_under_lab(0xf07e_0003, |root| async move {
        ...
        let mut first = WriteBatch::new(R);
        first.add_edge(EId(10), VId(0), VId(1), vec![]);
        let mut choices = WriteBatch::new(S);
        for id in 2..10 {
            choices.add_edge(EId(100 + id), VId(1), VId(id), vec![]);
        }
        db.write_atomic(&commit, vec![vertices, first, choices])
            .await
            .unwrap();
        let mut statement = "MATCH (a)-[:R]->(b)".to_owned();
        for at in 0..10 {
            statement.push_str(&format!(",(b)-[:S]->(x{at})"));
        }
        statement.push_str(" RETURN COUNT(*) AS n,COUNT(DISTINCT b) AS d");
        let query = prepare(&statement);
        let result = db
            .execute_graph_aggregate_governed(&cx, &query, GqlQueryPolicy::new(9, 1, 65_536, 2048))
            .unwrap();
        assert_eq!(
