...
fn options() -> FnxReadOptions {
    FnxReadOptions {
        as_of: None,
        selection: FnxSelection { vertex_label: Some(LabelId(1)), relation: Some(RelationId(1)), weight: FnxWeightSpec::Unit },
        ...
        projection_limits: ProjectionLimits { max_vertices: 100, max_input_edges: 1000,
            max_adjacency_entries: 2000, max_workspace_bytes: 1 << 22 },
        execution_limits: FnxExecutionLimits { max_iterations: 0, max_result_rows: 100, max_estimated_work: 1 << 24 },
    }
}
fn vertex(value: u128) -> FnxValue { FnxValue::Vertex(VId(value)) }
fn components(values: &[(u128, u128)]) -> Vec<Vec<FnxValue>> {
    values.iter().map(|&(id, component)| vec![vertex(id), vertex(component)]).collect()
}
...
fn traversal_calls_follow_selected_historical_generations_across_writes_and_drop() {
    ...
    runtime.block_on(async {
        ...
        for id in [1, 2, u128::MAX] { first.create_vertex(VId(id), vec![LabelId(1)], vec![]); }
        ...
        assert_eq!(old_bfs.analytics.rows, vec![
            vec![vertex(1), FnxValue::Integer(0)], vec![vertex(2), FnxValue::Integer(1)],
        ]);
        ...
        assert_eq!(db.call_fnx(&query, wcc, &parameters, options()).unwrap().analytics.rows, expected);
        ...
        assert_eq!(reached.analytics.rows[2], vec![vertex(u128::MAX), FnxValue::Integer(2)]);
        let connected = components(&[(1,1), (2,1), (u128::MAX,1)]);
        assert_eq!(db.call_fnx(&query, wcc, &parameters, options()).unwrap().analytics.rows, connected);
        assert_eq!(db.call_fnx(&query, scc, &parameters, options()).unwrap().analytics.rows, expected);
        ...
        assert_eq!(latest.call_fnx(&query, scc, &parameters, options()).unwrap().analytics.rows, connected);
        let historical = latest.call_fnx(&query, scc, &parameters,
            FnxReadOptions { as_of: Some(old.frontier()), ..options() }).unwrap();
        ...
        assert_eq!(latest.call_fnx(&query, "CALL fnx.connected_components()", &parameters, undirected).unwrap().analytics.rows, connected);
        assert!(matches!(latest.call_fnx(&query, "CALL fnx.connected_components()", &parameters, options()),
            Err(FnxReadError::Execution(FnxExecutionError::GraphKind { required: FnxGraphKind::Undirected }))));

        drop(db);
        assert_eq!(old_bfs, old.call_fnx(&query, bfs, &parameters, options()).unwrap());
        assert_eq!(old_scc, old.call_fnx(&query, scc, &parameters, options()).unwrap());
    });
}
