...
fn upsert(value: i64) -> PreparedGraphVertexUpsert {
    ...
    PreparedGraphVertexUpsert::prepare(
        merge,
        vec![
            GraphVertexUpsertAction::SetProperty { key: Q, value: scalar(100) },
            GraphVertexUpsertAction::SetLabel { label: MARKED, present: true },
        ],
        vec![
            GraphVertexUpsertAction::SetProperty { key: Q, value: scalar(200) },
            GraphVertexUpsertAction::SetLabel { label: MARKED, present: true },
        ],
    ).unwrap()
}
...
fn on_match_updates_existing_vertex_without_allocating() {
    let ((), report) = run_async_under_lab(0x6e29_3001, |root| async move {
        ...
        let (stats, outcome) = txn.execute_graph_vertex_upsert_governed(
            &mut db, &query, &upsert(7), policy(2),
        ...
        assert_eq!(staged.props, vec![(P, CanonicalScalar::Int(7)), (Q, CanonicalScalar::Int(100))]);
        txn.commit(&mut db, &commit).await.unwrap();
        let row = db.vertex(VId(1)).unwrap().unwrap();
        assert_eq!(row.labels, vec![MARKED]);
        assert_eq!(row.props, vec![(P, CanonicalScalar::Int(7)), (Q, CanonicalScalar::Int(100))]);
...
fn on_create_decorates_new_vertex_atomically_and_survives_reopen() {
    let ((), report) = run_async_under_lab(0x6e29_3002, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let (stats, outcome) = txn.execute_graph_vertex_upsert_governed(
            &mut db, &query, &upsert(9), policy(2),
        ...
        assert_eq!(staged.labels, vec![MARKED]);
        assert_eq!(staged.props, vec![(P, CanonicalScalar::Int(9)), (Q, CanonicalScalar::Int(200))]);
        ...
        assert_eq!(row.labels, vec![MARKED]);
        assert_eq!(row.props, vec![(P, CanonicalScalar::Int(9)), (Q, CanonicalScalar::Int(200))]);
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn create_branch_action_limit_refusal_rolls_back_created_vertex_but_not_allocator() {
    let ((), report) = run_async_under_lab(0x6e29_3003, |root| async move {
        ...
        let result = txn.execute_graph_vertex_upsert_governed(
            &mut db, &query, &upsert(11), policy(1),
            |_| Ok::<_, ()>(ElementId::Vertex(VId(11))),
        );
        assert!(matches!(result,
            Err(GqlQueryError::Source(GraphVertexUpsertError::ActionLimit { limit: 1, observed: 2 }))));
...
fn ambiguity_refuses_before_branch_actions_and_preserves_workspace() {
    let ((), report) = run_async_under_lab(0x6e29_3004, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let before = txn.staged_effect_digest().unwrap();
        assert!(txn.execute_graph_vertex_upsert_governed(
            &mut db, &query, &upsert(7), policy(2),
            |_| Ok::<_, ()>(ElementId::Vertex(VId(99))),
        ).is_err());
        assert_eq!(txn.staged_effect_digest().unwrap(), before);
        assert!(txn.vertex(&db, VId(1)).unwrap().unwrap().props.iter().all(|(key, _)| *key != Q));
        assert!(txn.vertex(&db, VId(2)).unwrap().unwrap().props.iter().all(|(key, _)| *key != Q));
        txn.abort();
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
