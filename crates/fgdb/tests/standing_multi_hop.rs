...
use fgdb::{
    ...
    StandingQueryHandle, StandingQueryStats, WriteBatch,
};
...
use fgdb_gql::{GraphAggregate, GqlQueryPolicy, PreparedGraphAggregate};
use fgdb_types::{
...
fn definition(
    ...
    if predicate {
        ...
        builder.compare_properties("a", RANK, IntegerComparison::LessOrEqual, "c", RANK).unwrap();
    }
...
fn check(
    db: &Database<MemVfs>, cx: &QueryCx, definition: &PreparedGraphAggregate,
    handle: &StandingQueryHandle, at: CommitSeq,
) -> StandingQueryStats {
    let vertices = db.vertices().unwrap();
    let edges = db.edges().unwrap();
    let rows: BTreeMap<_, _> = vertices.iter().map(|row| (row.vid, row)).collect();
    let expected = definition.execute_governed(
        ...
        |vid, key| Ok::<_, ()>(rows.get(&vid).unwrap().props.iter()
...
fn committed_joins_match_snapshots_for_orientations_self_joins_branches_and_cycles() {
    let ((), report) = run_async_under_lab(0x6a30, |root| async move {
        ...
        for id in 1..=4 { vertex(&mut seed, id, (id % 2) as i64); }
        let at = db.write(&commit, seed).await.unwrap();
        for (definition, handle) in &views { check(&db, &query, definition, handle, at); }
        let mut r = WriteBatch::new(R);
        for (id, a, b) in [(1,1,2), (2,1,2), (3,2,2), (4,2,3), (5,3,1), (6,4,3)] {
        ...
        for (definition, handle) in &views { check(&db, &query, definition, handle, at); }
        let late = chain(true);
        let handle = db.register_standing_query(&query, late.clone(), policy()).unwrap();
        ...
        for id in [1,3,4] { empty.delete_vertex(VId(id)); }
...
fn sparse_commit_work_does_not_scan_unrelated_complete_join_components() {
    let ((), report) = run_async_under_lab(0x6a31, |root| async move {
        ...
        for extra in [0, 100_u128] {
            let mut db = Database::open_memory(&commit, keys()).await.unwrap();
            let mut seed = WriteBatch::new(R);
            for id in 1..=3 { vertex(&mut seed, id, 0); }
            for component in 0..extra {
                for offset in 0..3 {
                    vertex(&mut seed, 100 + component * 3 + offset, component as i64 + 1);
                }
            }
            ...
            for component in 0..extra {
                ...
                r.add_edge(EId(1000 + component * 2), VId(start), VId(start + 1), vec![]);
                s.add_edge(EId(1001 + component * 2), VId(start + 1), VId(start + 2), vec![]);
            }
...
fn failed_join_view_does_not_undo_commit_and_rebuild_resumes_incremental_updates() {
    let ((), report) = run_async_under_lab(0x6a32, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut seed = WriteBatch::new(R);
        for id in 1..=4 { vertex(&mut seed, id, id as i64); }
...
fn disconnected_patterns_remain_explicitly_unsupported() {
    let ((), report) = run_async_under_lab(0x6a33, |root| async move {
        ...
        assert!(matches!(db.register_standing_query(&contexts.query(), disconnected, policy()),
