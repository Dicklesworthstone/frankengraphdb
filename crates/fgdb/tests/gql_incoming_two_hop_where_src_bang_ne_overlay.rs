...
#[test]
fn incoming_two_hop_near_end_property_bang_ne_sees_the_staged_continuation() {
    let ((), report) = run_async_under_lab(0x98_54, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-two-hop-where-src-bang-ne-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
        ...
        // fails != 1, so the base answer is empty.
        ...
        let bind = RelationBind::new()
            ...
            .with_relation("S", s)
            .with_property("k", key);
        let query = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE a.k != 1 RETURN c";
