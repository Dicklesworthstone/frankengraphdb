...
#[test]
fn incoming_two_hop_far_end_property_greater_or_equal_sees_the_staged_boundary() {
    let ((), report) = run_async_under_lab(0x81_10, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-two-hop-where-dst-ge-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
