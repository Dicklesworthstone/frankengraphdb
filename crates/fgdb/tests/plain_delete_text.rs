...
fn parameterized_native_delete_refuses_attached_then_succeeds_after_staged_unlink() {
    let ((), report) = run_async_under_lab(0xd31e_2001, |root| async move {
        ...
        let template = PreparedGraphDeleteText::prepare(
            "MATCH (n) WHERE n.p = $value DELETE n", R, symbols,
        ).unwrap();
        let delete = template.bind_parameters(
            &GqlParameters::new().with_int64("value", 10).unwrap(),
        ).unwrap();
