...
use fgdb_gql::{
    ...
    GraphWriteScriptExecutionError, PreparedGraphText, PreparedGraphWriteScript,
};
use fgdb_types::{
...
fn ordered_join_bags_follow_pinned_history_through_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0x4d41_0001, |root| async move {
        ...
        assert!(
            pinned
                .execute_graph_pattern_governed_at(&cx, &query, second, policy())
                .is_err()
        );
        ...
        for (seq, count) in [(first, 4), (second, 5), (third, 3)] {
            assert_eq!(
                ids(&db
                    .execute_graph_pattern_governed_at(&cx, &query, seq, policy())
                    .unwrap()
                    .value),
                vec![VId(3); count],
                "historical join changed at {seq:?}"
            );
        }
    });
...
fn a_new_required_witness_conflicts_even_when_the_reader_returned_no_rows() {
    let ((), report) = run_async_under_lab(0x4d41_0003, |root| async move {
        ...
        for empty in [false, true] {
            ...
            let query = if empty {
                prepare(
                    "MATCH (a) WHERE a.p=4 OPTIONAL MATCH (a)-[:R]->(b) MATCH (b)-[:S]->(c) RETURN c",
                )
            } else {
                joined()
            };
...
fn joined_creation_and_later_updates_share_one_rollback_guard_and_one_commit() {
    let ((), report) = run_async_under_lab(0x4d41_0004, |root| async move {
        ...
        assert!(
            failed.is_err(),
            "four update targets exceed the shared effect limit"
        );
        assert_eq!(
            allocated, 4,
            "the first joined creation step must actually have run"
        );
        assert_eq!(txn.staged_effect_digest().unwrap(), before);
        assert_eq!(db.frontier().unwrap(), frontier);
        assert!(txn.vertex(&db, VId(99)).unwrap().is_some());
        for vid in 100..104 {
            assert!(txn.vertex(&db, VId(vid)).unwrap().is_none());
        }
        let mut created = 0;
        let receipt = txn
        ...
        assert_eq!(
            ...
            Some(&[VId(200), VId(201), VId(202), VId(203)][..])
        );
        assert!(matches!(
        ...
        for vid in 200..204 {
            ...
            assert_eq!(
                row.props,
                vec![(P, CanonicalScalar::Int(77)), (Q, CanonicalScalar::Int(77))]
            );
        }
...
fn parameter_batches_continue_after_an_empty_join_and_keep_global_creation_limits() {
    let ((), report) = run_async_under_lab(0x4d41_0005, |root| async move {
        ...
        let args = [(1, 7), (4, 8), (1, 9)]
            .into_iter()
            .map(|(source, tag)| {
                GqlParameters::new()
                    .with_int64("source", source)
                    .unwrap()
                    .with_int64("tag", tag)
                    .unwrap()
            })
        ...
        let error = db
            .execute_bound_graph_write_script_batch_autocommit_governed(
                ...
                &commit,
                &batch,
                GraphWriteProgramPolicy::new(policy(), 0, 7, 0),
                |_| {
        ...
        assert_eq!(db.frontier().unwrap(), frontier);
        for vid in 100..104 {
            assert!(db.vertex(VId(vid)).unwrap().is_none());
        }
        assert_eq!(txcx.outstanding_obligations(), 0);
        let mut issued = 0;
        let (receipt, completion) = db
            .execute_bound_graph_write_script_batch_autocommit_governed(
                ...
                &commit,
                &batch,
                GraphWriteProgramPolicy::new(policy(), 0, 8, 0),
                |request| {
        ...
        assert!(
            batch.record_receipts(&receipt, 1).unwrap()[0]
                .created_vertices()
                .unwrap()
                .is_empty()
        );
        ...
        for vid in 200..208 {
            ...
            assert_eq!(
                db.vertex(VId(vid)).unwrap().unwrap().props,
                vec![(Q, CanonicalScalar::Int(tag))]
            );
        }
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
