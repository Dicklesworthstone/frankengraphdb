...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(10_000, 10_000, 5_000_000, 1_000_000) }
...
fn query(direction: GlaDirection, filter: &str, skip: u64, limit: u64) -> PreparedGraphPattern<GraphValueRow> {
    let (left, right) = match direction {
        GlaDirection::Forward => ("-", "->"), GlaDirection::Reverse => ("<-", "-"), GlaDirection::Undirected => ("-", "-"),
    };
    prepared(&format!("MATCH (a){left}[r:R]{right}(b) {filter} RETURN r, a, b, r.p AS ep, a.p AS ap, b.p AS bp SKIP {skip} LIMIT {limit}"))
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    let mut batch = WriteBatch::new(R);
    for (id, n) in IDS.into_iter().zip([4, 2, 1]) { batch.create_vertex(id, vec![LabelId(1)], vec![(P, CanonicalScalar::Int(n))]); }
    for (eid, src, dst, value) in [(0, 0, 1, Some(2)), (1, 2, 0, Some(-1)),
        (2, 2, 0, Some(5)), (3, 1, 1, None), (u128::MAX, 1, 2, Some(7))] {
        batch.add_edge(EId(eid), IDS[src], IDS[dst], value.map(|v| vec![(P, CanonicalScalar::Int(v))]).unwrap_or_default());
    }
...
fn oracle(db: &Database<MemVfs>, cut: CommitSeq, direction: GlaDirection) -> Vec<Vec<GraphValue>> {
    let vertex = |id| {
        let row = db.vertex_at(id, cut).unwrap().expect("admitted edge endpoint");
        GraphValue::Scalar(row.props.into_iter().find(|(key, _)| *key == P).map(|(_, v)| v).unwrap_or(CanonicalScalar::Null))
    };
    let mut rows = Vec::new();
    for edge in db.edges_at(cut).unwrap() {
        if edge.entry.relation != R { continue; }
        let entry = edge.entry;
        let property = GraphValue::Scalar(edge.props.into_iter().find(|(key, _)| *key == P).map(|(_, v)| v).unwrap_or(CanonicalScalar::Null));
        ...
        for (src, dst) in ends {
            rows.push(vec![GraphValue::Edge(entry.eid), GraphValue::Vertex(src), GraphValue::Vertex(dst),
                property.clone(), vertex(src), vertex(dst)]);
        }
    }
    rows.sort(); rows
}
...
fn streams_match_independent_rows_on_every_cut_before_and_after_recovery_and_cascades() {
    let ((), report) = run_async_under_lab(0xed6e_1001, |root| async move {
        ...
        let mut cascade = WriteBatch::new(R); cascade.delete_vertex(IDS[1]);
        ...
        for seq in [CommitSeq(0), basis, changed, retired, frontier] {
            for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
                ...
                let mut stream = db.stream_graph_edges_governed_at(&cx, &q, seq, policy()).unwrap();
        ...
        for filter in ["WHERE a <> b", "WHERE r.p IS NULL OR r.p > a.p", "WHERE a.p >= 1 AND b.p IS NOT NULL"] {
            for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
                let q = query(direction, filter, 1, 3);
                let expected = db.execute_graph_pattern_governed_at(&cx, &q, changed, policy()).unwrap().value;
                let mut stream = db.stream_graph_edges_governed_at(&cx, &q, changed, policy()).unwrap();
                assert_eq!(stream.by_ref().collect::<Result<Vec<_>, _>>().unwrap(), expected);
            }
        }
        ...
        let mut old_at = pinned.stream_graph_edges_governed_at(&cx, &q, basis, policy()).unwrap();
        assert_eq!(plain(&old_at.by_ref().collect::<Result<Vec<_>, _>>().unwrap()), expected);
    });
...
fn exact_quotas_are_cumulative_and_error_precedence_does_not_turn_into_empty_results() {
    let ((), report) = run_async_under_lab(0xed6e_1002, |root| async move {
        ...
        assert_eq!(rows, expected); assert_eq!(retry.evaluator_stats(), e);
        ...
        for result in [
            db.stream_graph_edges_governed_at(&cx, &empty, CommitSeq(basis.0+1), GqlQueryPolicy::new(0,0,0,0)).map(|_| ()),
            pinned.stream_graph_edges_governed_at(&cx, &empty, CommitSeq(basis.0+1), GqlQueryPolicy::new(0,0,0,0)).map(|_| ()),
        ] { assert!(matches!(result, Err(GqlQueryError::Source(EdgeScanError::Source(_))))); }
        ...
        let mut zero = db.stream_graph_edges_governed(&cx, &empty, GqlQueryPolicy::new(0,0,1,0)).unwrap();
...
fn limit_one_reads_one_history_not_a_thousand_edge_table_and_close_does_not_drain() {
    let ((), report) = run_async_under_lab(0xed6e_1003, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut batch = WriteBatch::new(R);
        for id in IDS { batch.create_vertex(id, vec![], vec![]); }
        for id in 0..1024 { batch.add_edge(EId(id), IDS[0], IDS[2], vec![(P, CanonicalScalar::Int(id as i64))]); }
        ...
        let stats = one.evaluator_stats(); assert!(one.next().is_none()); assert_eq!(one.evaluator_stats(), stats);
        ...
        assert_eq!(early.state(), EdgeScanState::Closed); assert!(early.next().is_none());
