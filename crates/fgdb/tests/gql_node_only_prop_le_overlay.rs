...
#[test]
fn node_only_less_or_equal_includes_boundary_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x69_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-node-only-prop-le-overlay-{}",
            std::process::id()
        ));
        ...
        staged.create_vertex(VId(2), vec![person], vec![(key, CanonicalScalar::Int(0))]);
