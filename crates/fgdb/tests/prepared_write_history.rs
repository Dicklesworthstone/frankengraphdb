...
use fgdb::{Database, DatabaseKeys, DatabaseState, MemVfs, WriteBatch, WriteError, WriteMismatchPolicy};
...
async fn reset_with_unrelated_writes(db: &mut Database<MemVfs>, cx: &CommitCx) {
    for value in 1..=3 {
        ...
        db.write(cx, write).await.expect("intervening ordinary write");
    }
}

fn assert_conflict(result: Result<CommitSeq, WriteError>) {
    assert!(matches!(result, Err(WriteError::FirstCommitterWins { law: "FG-LAW-FCW-01", .. })));
}
    ...
    let ((), report) = run_async_under_lab(0xfc20_0001, |root| async move {
        ...
        for case in 0..6 {
            ...
            match case {
                ...
                1 => {
                    staged.compare_and_set_vertex_property(VId(1), P,
                        Some(CanonicalScalar::Int(99)), CanonicalScalar::Int(5), WriteMismatchPolicy::NoOp);
            ...
            assert_eq!(db.frontier().expect("no lost-write marker"), frontier, "case {case}");
            assert!(matches!(db.state(), DatabaseState::Healthy { .. }));
            assert!(db.vertex(VId(99)).expect("no partial publication").is_none());
        }
    });
...
fn commits_at_or_before_the_basis_are_not_conflicts() {
    let ((), report) = run_async_under_lab(0xfc20_0002, |root| async move {
        ...
        db.write(&cx, before).await.expect("change before the basis");
        let mut staged = WriteBatch::new(R);
        staged.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(5)));
        let prepared = db.prepare_write(staged).expect("prepare after observing the change");
        reset_with_unrelated_writes(&mut db, &cx).await;
        db.commit_prepared(&cx, prepared).await.expect("disjoint suffix");
        assert_eq!(db.vertex(VId(1)).expect("vertex read").expect("vertex").props,
...
fn independent_parallel_edge_creates_remain_committable() {
    let ((), report) = run_async_under_lab(0xfc20_0003, |root| async move {
        ...
        db.commit_prepared(&cx, second).await.expect("unconstrained parallel edge");
