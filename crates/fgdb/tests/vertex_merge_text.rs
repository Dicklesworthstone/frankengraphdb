...
use fgdb_gql::{
    GqlParameterType, GqlParameters, GqlQueryError, GqlQueryPolicy, GraphSymbol,
    GraphSymbolKind, GraphVertexMergeError, GraphVertexMergeOutcome, GraphVertexMergePolicy,
...
fn args(p: i64, name: &str) -> GqlParameters {
    GqlParameters::new().with_int64("p", p).unwrap().with_text("name", name).unwrap()
}
...
fn native_merge_matches_then_creates_then_matches_created_vertex() {
    let ((), report) = run_async_under_lab(0x6e29_2001, |root| async move {
        ...
        let (_, outcome, completion) = db.execute_graph_vertex_merge_autocommit_governed(
            &txcx, &query, &commit, &existing, policy(),
        ...
        let (_, outcome, completion) = db.execute_graph_vertex_merge_autocommit_governed(
            &txcx, &query, &commit, &missing, policy(),
