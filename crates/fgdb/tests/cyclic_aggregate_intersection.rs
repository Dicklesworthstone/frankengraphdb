...
fn wide() -> GqlQueryPolicy { GqlQueryPolicy::new(100_000, 100_000, 20_000_000, 5_000_000) }
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in [0,1,2,3,HI,u128::MAX] { vertices.create_vertex(VId(id),vec![],vec![]); }
    ...
    for (relation, records) in [
        ...
        (4, vec![(40,2,3),(41,2,3),(42,3,u128::MAX),(43,1,u128::MAX)]),
    ] {
        let mut batch = WriteBatch::new(RelationId(relation));
        for (id, from, to) in records { batch.add_edge(EId(id),VId(from),VId(to),vec![]); }
        batches.push(batch);
    }
...
fn cyclic_counts_preserve_historical_pinned_and_canonical_staged_multiplicities() {
    let ((), report) = run_async_under_lab(0x1a73_0001, |root| async move {
        ...
        let pinned = db.read_session().unwrap(); let definitions = cases();
        let mut old = Vec::new();
        for (fast, ordinary) in &definitions {
            let expected = db.execute_graph_aggregate_governed(&cx,ordinary,wide()).unwrap().value;
            assert_eq!(db.execute_graph_aggregate_governed(&cx,fast,wide()).unwrap().value,expected);
            assert_eq!(pinned.execute_graph_aggregate_governed(&cx,fast,wide()).unwrap().value,expected);
            old.push(expected);
        }
        assert!(old[0].iter().any(|row| row.get(0).unwrap().as_count().unwrap() > 1));
        ...
        let mut retirement = WriteBatch::new(RelationId(3));
        retirement.delete_edge(EId(31)); txn.write(&mut db,retirement).unwrap();
        let mut staged = Vec::new();
        for (fast, ordinary) in &definitions {
            let expected = txn.execute_graph_aggregate_governed(&db,&cx,ordinary,wide()).unwrap().value;
            assert_eq!(txn.execute_graph_aggregate_governed(&db,&cx,fast,wide()).unwrap().value,expected);
        ...
        for (((fast, ordinary), expected_old), expected_new) in definitions.iter().zip(&old).zip(&staged) {
            assert_eq!(&db.execute_graph_aggregate_governed(&cx,fast,wide()).unwrap().value,expected_new);
            assert_eq!(&db.execute_graph_aggregate_governed(&cx,ordinary,wide()).unwrap().value,expected_new);
            assert_eq!(&db.execute_graph_aggregate_governed_at(&cx,fast,basis,wide()).unwrap().value,expected_old);
            assert_eq!(&pinned.execute_graph_aggregate_governed(&cx,fast,wide()).unwrap().value,expected_old);
        }
        ...
        for ((fast, ordinary), expected_old) in definitions.iter().zip(&old) {
            assert_eq!(db.execute_graph_aggregate_governed(&cx,fast,wide()).unwrap().value,
                db.execute_graph_aggregate_governed(&cx,ordinary,wide()).unwrap().value);
            assert_eq!(&pinned.execute_graph_aggregate_governed_at(&cx,fast,basis,wide()).unwrap().value,expected_old);
        }
    });
    ...
    let ((), report) = run_async_under_lab(0x1a73_0002, |root| async move {
        ...
        for outcome in 0..3 {
            for change in 0..3 {
                ...
                let baseline = db.execute_graph_aggregate_governed(&cx,&fast,wide()).unwrap();
                ...
                assert_eq!(db.execute_graph_aggregate_governed(&cx,&fast,exact).unwrap(),baseline);
                for policy in [
                    GqlQueryPolicy::new(baseline.rows.snapshot_records-1,100_000,u64::MAX,u64::MAX),
                    GqlQueryPolicy::new(100_000,baseline.rows.result_rows-1,u64::MAX,u64::MAX),
                    GqlQueryPolicy::new(100_000,100_000,baseline.evaluator.work_units-1,u64::MAX),
                    GqlQueryPolicy::new(100_000,100_000,u64::MAX,baseline.evaluator.scratch_entries-1),
                ] { assert!(db.execute_graph_aggregate_governed(&cx,&fast,policy).is_err()); }
                assert!(matches!(db.execute_graph_aggregate_governed_at(&cx,&fast,CommitSeq(basis.0+1),
                    GqlQueryPolicy::new(0,0,0,0)),Err(GqlQueryError::Source(GraphAggregateError::Source(_)))));
                let mut txn = db.begin(&txcx).unwrap();
                let mut edit = WriteBatch::new(RelationId(1)); edit.create_vertex(VId(777),vec![],vec![]);
                txn.write(&mut db,edit).unwrap();
                let q = if outcome == 1 {
                    query("MATCH (a)-[:R]->(b)-[:S]->(c)-[:T]->(a) WHERE a<>a RETURN a,b,c,COUNT(*) AS n GROUP BY a,b,c")
                } else { fast };
                let policy = if outcome == 2 { GqlQueryPolicy::new(100_000,0,20_000_000,5_000_000) } else { wide() };
                ...
                match change {
                    0 => { winner.add_edge(EId(91),VId(0),VId(1),vec![]); }
                    1 => { winner.delete_edge(EId(11)); }
...
fn database_cyclic_join_handles_sparse_closure_without_quadratic_intermediates() {
    let ((), report) = run_async_under_lab(0x1a73_0003, |root| async move {
        ...
        let n = 1024_u128;
        let mut vertices = WriteBatch::new(RelationId(1));
        for id in 0..=2*n { vertices.create_vertex(VId(id),vec![],vec![]); }
        ...
        let q = query("MATCH (a)-[:R]->(b)-[:S]->(c)-[:T]->(a) RETURN a,b,c,COUNT(*) AS n GROUP BY a,b,c");
        ...
        let result = db.execute_graph_aggregate_governed(&cx,&q,
            GqlQueryPolicy::new(10_000,n as u64,1_000_000,500_000)).unwrap();
