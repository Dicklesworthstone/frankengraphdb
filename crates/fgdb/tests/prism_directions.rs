...
fn options(direction: Directedness) -> FnxReadOptions {
    FnxReadOptions {
        ...
        projection: ProjectionSpec { directedness: direction,
        ...
        projection_limits: ProjectionLimits { max_vertices: 100, max_input_edges: 1000,
            max_adjacency_entries: 2000, max_workspace_bytes: 1 << 20 },
...
fn fixture() -> WriteBatch {
    ...
    for v in [1, 2, 3, 4, u128::MAX] { batch.create_vertex(VId(v), vec![LabelId(1)], vec![]); }
    ...
    for (eid, s, t, weight) in [
        (1, 1, 2, 1), (2, 2, 1, 1i64 << 53), (3, 1, 2, 1),
        (4, 2, 3, 0), (5, 3, 4, 2), (6, 4, 2, 1), (7, 2, 2, 3),
        (8, 4, u128::MAX, 4),
    ] {
    ...
    for eid in 100..110 {
        batch.add_edge(EId(eid), VId(1), VId(3), vec![(PropertyKeyId(1), CanonicalScalar::Int(1))]);
    }
    for (eid, s, t) in [(900, 99, 2), (901, 3, 99)] {
        batch.add_edge(EId(eid), VId(s), VId(t), vec![(PropertyKeyId(1), CanonicalScalar::Bool(true))]);
    }
    batch
}
...
fn row(graph: &SealedGraphView, cx: &QueryCx, source: usize, lower: Option<VId>) -> Vec<(usize, u64)> {
    let mut cursor = graph.neighbor_cursor_from(cx, source, lower).unwrap();
    let mut values = Vec::new();
    while let Some((target, weight)) = cursor.next(cx).unwrap() { values.push((target, weight.to_bits())); }
    assert!(cursor.next(cx).unwrap().is_none());
    values
}
fn call(text: &str) -> FnxCallSpec { FnxCallSpec::bind(text, &FnxParameters::new()).unwrap() }
...
fn all_directions_reductions_loops_and_full_width_seeks_match_decoded_rows() {
    ...
    runtime.block_on(async {
        ...
        for direction in [Directedness::Directed, Directedness::Reversed, Directedness::Undirected] {
            for policy in [ParallelEdgePolicy::Sum, ParallelEdgePolicy::Minimum,
                ...
                for loops in [SelfLoopPolicy::Keep, SelfLoopPolicy::Drop] {
                    ...
                    opt.projection.parallel_edges = policy; opt.projection.self_loops = loops;
                    let graph = project(&db, &query, opt).await;
                    let oracle = view.prism_projection_at(&query, view.frontier(), opt.selection,
                    ...
                    for source in 0..graph.node_count() {
                        ...
                        let expected: Vec<_> = targets.iter().copied().zip(weights.iter().map(|w| w.to_bits())).collect();
                        arcs += targets.len();
                        assert_eq!(row(&graph, &query, source, None), expected,
                        ...
                        for lower in [VId(0), VId(1), VId(2), VId(3), VId(5), VId(u128::MAX)] {
                            let suffix: Vec<_> = expected.iter().copied()
                                .filter(|(index, _)| graph.vertex_id(*index).unwrap() >= lower).collect();
                            assert_eq!(row(&graph, &query, source, Some(lower)), suffix);
                        }
                    }
                    assert_eq!(graph.adjacency_entry_count(), arcs);
                    assert_eq!(graph.incoming_index_stats().is_some(), direction != Directedness::Directed);
                    if direction == Directedness::Undirected && policy == ParallelEdgePolicy::Sum {
                        assert_eq!(row(&graph, &query, 0, None)[0], (1, ((1u64 << 53) as f64).to_bits()));
                        assert_eq!(row(&graph, &query, 1, None)[0], (0, ((1u64 << 53) as f64).to_bits()));
                    }
                }
            }
        }
...
fn registered_procedures_reach_directional_images_from_database_and_reusable_projection() {
    ...
    runtime.block_on(async {
        ...
        for direction in [Directedness::Directed, Directedness::Reversed, Directedness::Undirected] {
            ...
            if direction == Directedness::Undirected { texts.push(CC); }
            else { texts.extend([WCC, SCC]); }
            for text in texts {
                let call = call(text);
                let expected = view.execute_fnx(&query, &call, opt).unwrap();
                let output = db.execute_fnx_sealed(&query, &call, opt, memory(), SealedLimits::default()).await.unwrap();
                assert_eq!(output.analytics.columns, expected.analytics.columns);
                assert_eq!(output.analytics.rows, expected.analytics.rows, "{direction:?} {text}");
                assert_eq!(output.analytics.certificate.result_digest, expected.analytics.certificate.result_digest);
                assert_eq!(output.analytics.certificate.adapter, AdapterPath::CompressedCursor);
                assert_eq!(output.analytics.certificate.edges, graph.edge_count());
                assert_eq!(output.analytics, call.execute_sealed(&query, &graph,
                    opt.execution_limits, memory()).unwrap());
                if text == CC {
                    assert_eq!(output.analytics.certificate.execution_kernel,
                        "fgdb-prism/sealed-union-find-v1");
                    assert!(output.analytics.rows.iter().all(|r| r[1] == FnxValue::Vertex(VId(1))));
                    assert_eq!(output.analytics.rows.last().unwrap()[0], FnxValue::Vertex(VId(u128::MAX)));
                }
            }
        }
    });
...
fn incoming_build_peak_and_undirected_arcs_are_admitted_not_just_simple_edges() {
    ...
    runtime.block_on(async {
        ...
        for direction in [Directedness::Reversed, Directedness::Undirected] {
            ...
            assert_eq!(graph.charged_workspace_bytes(), directory + stats.charged_workspace_bytes);
            ...
            assert!(matches!(db.prism_sealed_projection_at(&query, None, opt.selection, opt.projection,
                opt.projection_limits, opt.source_limits, SealedLimits::default()).await,
                Err(FnxSealedReadError::Projection(SealedProjectionError::Read(SealedError::Limit { .. })))));
            ...
            assert!(matches!(db.prism_sealed_projection_at(&query, None, opt.selection, opt.projection,
                opt.projection_limits, opt.source_limits, SealedLimits::default()).await,
                Err(FnxSealedReadError::Projection(SealedProjectionError::Projection(
                    ProjectionError::LimitExceeded { resource: "adjacency entries", .. })))));
            opt = options(direction);
            opt.projection_limits.max_input_edges = 19;
            assert!(matches!(db.prism_sealed_projection_at(&query, None, opt.selection, opt.projection,
                opt.projection_limits, opt.source_limits, SealedLimits::default()).await,
                Err(FnxSealedReadError::Projection(SealedProjectionError::Projection(
                    ProjectionError::LimitExceeded { resource: "source incidences", .. })))));
            ...
            let output = call(BFS).execute_sealed(&query, &graph,
                opt.execution_limits, memory()).unwrap();
            let exact = FnxExecutionLimits {
                max_estimated_work: output.certificate.estimated_work,
                ..opt.execution_limits
            };
            assert_eq!(call(BFS).execute_sealed(&query, &graph, exact, memory()).unwrap(), output);
            assert!(matches!(call(BFS).execute_sealed(&query, &graph,
                FnxExecutionLimits { max_estimated_work: exact.max_estimated_work - 1, ..exact },
                memory()), Err(FnxSealedExecutionError::Execution(
                    FnxExecutionError::LimitExceeded { resource: "estimated work", .. }))));
        }
    });
}
...
fn reciprocal_eids_are_parallel_but_a_self_loop_is_never_its_own_parallel_edge() {
    ...
    runtime.block_on(async {
        ...
        for v in [1, 2] { initial.create_vertex(VId(v), vec![LabelId(1)], vec![]); }
        initial.add_edge(EId(1), VId(1), VId(1), vec![(PropertyKeyId(1), CanonicalScalar::Int(3))]);
        ...
        assert_eq!((graph.edge_count(), graph.input_edge_count(), graph.adjacency_entry_count()), (1, 1, 1));
        let mut update = WriteBatch::new(RelationId(1));
        update.delete_edge(EId(1));
        for (eid, s, t) in [(2, 1, 2), (3, 2, 1)] {
            update.add_edge(EId(eid), VId(s), VId(t), vec![(PropertyKeyId(1), CanonicalScalar::Int(2))]);
        }
...
fn historical_directional_projections_remain_immutable_after_writer_progress_and_drop() {
    ...
    runtime.block_on(async {
        ...
        for direction in [Directedness::Reversed, Directedness::Undirected] {
            let opt = options(direction);
            let graph = project(&db, &query, opt).await;
            let expected = call(BFS).execute_sealed(&query, &graph, opt.execution_limits, memory()).unwrap();
        ...
        for (direction, graph, expected) in &pinned {
            ...
            for source in 0..graph.node_count() {
                assert_eq!(row(&historical, &query, source, None), row(graph, &query, source, None));
            }
            assert_eq!(call(BFS).execute_sealed(&query, &historical, opt.execution_limits, memory()).unwrap().rows, expected.rows);
            opt.as_of = None;
            let current = project(&db, &query, opt).await;
            assert_eq!(current.node_count(), 4);
            assert_eq!(call(BFS).execute_sealed(&query, &current, opt.execution_limits, memory()).unwrap().rows,
                latest.execute_fnx(&query, &call(BFS), opt).unwrap().analytics.rows);
        }
        drop(db); drop(latest); drop(old);
        for (direction, graph, expected) in pinned {
            assert_eq!(call(BFS).execute_sealed(&query, &graph.clone(),
                options(direction).execution_limits, memory()).unwrap(), expected);
        }
    });
}
...
fn every_directional_checkpoint_refuses_atomically_and_failed_row_cursors_are_fused() {
    ...
    runtime.block_on(async {
        ...
        let mut db = Database::<MemVfs>::open_memory(&commit, keys()).await.unwrap();
        let mut batch = WriteBatch::new(RelationId(1));
        for v in 1..=3 { batch.create_vertex(VId(v), vec![LabelId(1)], vec![]); }
        for (id, s, t) in [(1, 1, 2), (2, 2, 1), (3, 2, 3)] {
            batch.add_edge(EId(id), VId(s), VId(t), vec![(PropertyKeyId(1), CanonicalScalar::Int(1))]);
        }
        db.write(&commit, batch).await.unwrap();
        let before = db.read_session().unwrap();
        for direction in [Directedness::Reversed, Directedness::Undirected] {
            let opt = options(direction);
            let procedure = call(if direction == Directedness::Undirected { CC } else { WCC });
            let probe = Arc::new(SimulationCheckpointProbe::new(None));
            let measured = query.with_checkpoint_probe(Arc::clone(&probe));
            let expected = db.execute_fnx_sealed(&measured, &procedure, opt, memory(), SealedLimits::default()).await.unwrap();
            ...
            for stop in 1..=total {
                ...
                assert!(db.execute_fnx_sealed(&controlled, &procedure, opt, memory(), SealedLimits::default()).await.is_err());
                assert_eq!(probe.calls(), stop);
                assert_eq!(db.read_session().unwrap().partition_root(), before.partition_root());
            }
            assert_eq!(db.execute_fnx_sealed(&query, &procedure, opt, memory(), SealedLimits::default()).await.unwrap(), expected);
            let graph = project(&db, &query, opt).await;
            let probe = Arc::new(SimulationCheckpointProbe::new(None));
            let measured = query.with_checkpoint_probe(Arc::clone(&probe));
...
fn empty_images_and_isolates_work_in_every_direction_without_invented_sources() {
    ...
    runtime.block_on(async {
        ...
        for direction in [Directedness::Directed, Directedness::Reversed, Directedness::Undirected] {
            let mut opt = options(direction);
            opt.projection_limits = ProjectionLimits { max_vertices: 0, max_input_edges: 0,
                max_adjacency_entries: 0, max_workspace_bytes: 0 };
            let graph = project(&db, &query, opt).await;
            assert_eq!(graph.charged_workspace_bytes(), 0);
            assert!(call(RANK).execute_sealed(&query, &graph, opt.execution_limits, memory()).unwrap().rows.is_empty());
            assert!(matches!(graph.neighbor_cursor(&query, usize::MAX), Err(SealedProjectionError::UnknownOrdinal(_))));
        }
        let mut batch = WriteBatch::new(RelationId(1));
        batch.create_vertex(VId(u128::MAX), vec![LabelId(1)], vec![]);
        db.write(&commit, batch).await.unwrap();
        let graph = project(&db, &query, options(Directedness::Undirected)).await;
        let output = call(CC).execute_sealed(&query, &graph,
            options(Directedness::Undirected).execution_limits, memory()).unwrap();
        assert_eq!(output.rows, vec![vec![FnxValue::Vertex(VId(u128::MAX)), FnxValue::Vertex(VId(u128::MAX))]]);
    });
}
