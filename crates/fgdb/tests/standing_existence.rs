...
use fgdb_gql::{
    GqlParameters, GqlQueryPolicy, GraphSymbol, GraphSymbolKind, PreparedGraphAggregate,
    ...
    let ((), report) = run_async_under_lab(0x8f11, |runtime| async move {
        ...
        for direction in 0..3 {
            ...
            for anti in [false, true] {
                ...
                let query = parsed(&format!(
                    "MATCH (a:Owner) WHERE {not}EXISTS {{ MATCH {pattern} WHERE a.amount < b.amount }} RETURN a,COUNT(*) AS hits,SUM(a.amount) AS amount GROUP BY a"
                ));
                let handle = db
        ...
        for step in 0..11 {
            ...
            match step {
                ...
                2 => {
                    batch.set_vertex_property(VId(2), AMOUNT, Some(CanonicalScalar::Int(7)));
                }
                3 => {
            ...
            let value = |vid| {
                vertices.iter().find(|row| row.vid == vid).and_then(|row| {
                    row.props.iter().find_map(|(key, value)| match value {
                        CanonicalScalar::Int(n) if *key == AMOUNT => Some(*n),
                        _ => None,
                    })
                })
            };
            let all: BTreeSet<_> = vertices
                .iter()
                .filter(|row| row.labels.contains(&OWNER))
                .map(|row| row.vid)
                .collect();
            ...
            for (direction, anti, query, handle) in &queries {
                ...
                let full = db
                    ...
                    .unwrap()
                    .value;
                let actual: BTreeSet<_> = full
                    .iter()
                    .map(|row| row.keys()[0].as_vertex().unwrap())
                    .collect();
                assert_eq!(
                    actual, expected,
                    "step={step} direction={direction} anti={anti}"
                );
...
fn silent_witness_updates_advance_counts_and_last_deletion_flips_presence_once() {
    let ((), report) = run_async_under_lab(0x8f12, |runtime| async move {
        ...
        for anti in [false, true] {
            let not = if anti { "NOT " } else { "" };
            let query = parsed(&format!(
                "MATCH (a:Owner) WHERE {not}EXISTS {{ MATCH (a)-[:R]->(b) }} RETURN COUNT(*) AS hits,SUM(a.amount) AS total"
            ));
            let handle = db
                .register_standing_query(&cx, query.clone(), policy())
                .unwrap();
        ...
        for step in 0..5 {
            ...
            match step {
                ...
                3 => {
                    batch.delete_edge(EId(2));
                    batch.add_edge(EId(3), VId(1), VId(2), vec![]);
                }
                _ => {
            ...
            for (anti, query, handle) in &queries {
                let full = db
                    ...
                    .value;
                assert_eq!(
                    full[0].get(0).unwrap().as_count(),
                    Some(u64::from((step < 4) != *anti))
                );
                let view = db.standing_query(&cx, handle).unwrap();
                assert_eq!(view.frontier(), at);
                assert_eq!(view.rows().weight(&full[0]), Some(&ZWeight::ONE));
                if step == 3 {
