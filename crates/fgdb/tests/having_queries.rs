...
use fgdb::{
    Database, DatabaseKeys, EdgeRecord, MemVfs, VertexRow, WriteBatch, WriteError, WriteTxnError,
};
use fgdb_delta_types::{LabelId, PropertyKeyId, RelationId};
use fgdb_gql::{
    GqlParameterType, GqlParameters, GqlQueryError, GqlQueryPolicy, GraphAggregateRow, GraphSymbol,
...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 1000, 2_000_000, 1_000_000)
}
...
fn bind(template: &PreparedGraphAggregateText, category: &str) -> PreparedGraphAggregate {
    template
        .bind_parameters(&GqlParameters::new().with_text("wanted", category).unwrap())
        .unwrap()
}
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for id in 0..5 {
        batch.create_vertex(
            VId(id),
            vec![OWNER],
            vec![(CATEGORY, text(if id == 2 { "hold" } else { "active" }))],
        );
    }
    for (id, p) in [(10, 8), (11, -4), (13, 20), (14, 5)] {
        batch.create_vertex(VId(id), vec![], vec![(P, CanonicalScalar::Int(p))]);
    }
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord], wanted: &str) -> Vec<Summary> {
    ...
    for owner in vertices.iter().filter(|v| v.labels.contains(&OWNER)) {
        ...
        for edge in edges
            .iter()
            .filter(|e| e.entry.relation == R && e.entry.src == owner.vid)
        {
            n += 1;
            let target = vertices.iter().find(|v| v.vid == edge.entry.dst).unwrap();
        ...
        if property(owner, CATEGORY) == Some(&wanted)
            && (total.is_none() || sum > i128::from(present))
            && n != 0
        {
...
fn having_covers_all_five_reads_canonical_staging_and_reopened_history() {
    let ((), report) = run_async_under_lab(0x4841_0001, |root| async move {
        ...
        let prepared = template(None);
        let query = bind(&prepared, "active");
        ...
        assert_eq!(
            plain(
                &db.execute_graph_aggregate_governed(&cx, &hold, wide())
                    .unwrap()
                    .value,
                "hold"
            ),
            vec![(VId(2), 1, 1, Some(20))]
        );
        ...
        changes.set_vertex_property(VId(14), P, None);
    ...
    let ((), report) = run_async_under_lab(0x4841_0002, |root| async move {
        ...
        for mode in 0..3 {
            for change in 0..5 {
                ...
                match mode {
                    0 => assert_eq!(
                        plain(&result.unwrap().value, "active"),
                        vec![(VId(0), 3, 2, Some(16))]
                    ),
                ...
                match change {
                    ...
                    1 => {
                        winner.set_vertex_property(VId(11), P, Some(CanonicalScalar::Int(100)));
                    }
                    2 => {
                        winner.set_vertex_property(VId(2), CATEGORY, Some(text("active")));
                    }
                    3 => {
                        winner.add_edge(EId(90), VId(3), VId(13), vec![]);
                    }
                    _ => {
                        winner.delete_edge(EId(2));
                    }
                }
...
fn having_shares_source_and_result_allowances_and_keeps_empty_global_semantics() {
    let ((), report) = run_async_under_lab(0x4841_0003, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        seed(&mut db, &commit).await;
        let query = bind(&template(Some(2)), "active");
        let measured = db
            .execute_graph_aggregate_governed(&cx, &query, wide())
            .unwrap();
        assert_eq!(
            plain(&measured.value, "active"),
            vec![(VId(0), 3, 2, Some(16)), (VId(4), 1, 1, Some(5))]
        );
        ...
        let empty = db
            .execute_graph_aggregate_governed(&cx, &zero, wide())
            .unwrap();
        assert!(empty.value.is_empty());
        assert_eq!(empty.rows.snapshot_records, measured.rows.snapshot_records);
        let blank = Database::open_memory(&commit, keys()).await.unwrap();
        for (condition, count) in [
            ("n=0 AND (total IS NULL OR total>n)", 1),
            ("NOT (total=0)", 0),
        ] {
            let query = PreparedGraphAggregateText::prepare(
                &format!(
                    "MATCH (a:Owner) RETURN COUNT(*) AS n,SUM(a.p) AS total HAVING {condition}"
                ),
                symbols,
            )
            .unwrap()
            ...
            if count == 1 {
                ...
                assert!(result.value[0].get(1).unwrap().is_null());
            }
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
