...
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryPolicy, GraphEdgeMergeError,
    GraphEdgeMergeOutcome, GraphEdgeMergePolicy, GraphSymbol, GraphSymbolKind,
    PreparedGraphEdgeMerge, PreparedGraphEdgeMergeText};
...
fn merge(left: i64, right: i64) -> PreparedGraphEdgeMerge {
    PreparedGraphEdgeMergeText::prepare(
        ...
        .with_int64("left", left).unwrap().with_int64("right", right).unwrap()).unwrap()
}
...
fn zero_edge_quota_preserves_matches_and_no_input_but_refuses_new_allocation() {
    let ((), report) = run_async_under_lab(0xed9e_3001, |root| async move {
        ...
        let policy = GraphEdgeMergePolicy::new(
            GqlQueryPolicy::new(100_000, 100_000, 10_000_000, 10_000_000),
        ).with_creation_limit(0);
        let mut txn = db.begin(&txcx).unwrap();
        let before = txn.staged_effect_digest().unwrap();
        let (_, matched) = txn.execute_graph_edge_merge_governed(
            &mut db, &query, &merge(1, 2), policy,
            |_| -> Result<ElementId, ()> { panic!("existing edge must not allocate") },
        ).unwrap();
        assert_eq!(matched, GraphEdgeMergeOutcome::Matched(EId(10)));
        let (_, missing_endpoint) = txn.execute_graph_edge_merge_governed(
            &mut db, &query, &merge(1, 99), policy,
            |_| -> Result<ElementId, ()> { panic!("NoInput must not allocate") },
        ).unwrap();
        assert_eq!(missing_endpoint, GraphEdgeMergeOutcome::NoInput);
        let error = txn.execute_graph_edge_merge_governed(
            &mut db, &query, &merge(1, 3), policy,
            |_| -> Result<ElementId, ()> { panic!("exhausted quota must refuse before allocation") },
        ).unwrap_err();
        assert!(matches!(error, GqlQueryError::Source(GraphEdgeMergeError::CreationLimit {
            limit: 0, observed: 1,
        })));
        assert_eq!(txn.staged_effect_digest().unwrap(), before);
        let (stats, created) = txn.execute_graph_edge_merge_governed(
            &mut db, &query, &merge(1, 3), policy.with_creation_limit(1),
