...
fn parameters() -> GqlParameters {
    GqlParameters::new().with_uint64("take", 3).unwrap().with_int64("minimum", 1).unwrap()
}
...
fn top_groups() -> PreparedGraphAggregate {
    PreparedGraphPipelineAggregateText::prepare(TOP_GROUPS, symbols).unwrap()
        .bind_parameters(&parameters()).unwrap()
}
...
fn oracle(vertices: &[VertexRow], take: usize) -> Vec<Summary> {
    ...
    for (_, value) in rows.into_iter().take(take) {
        if let Some(value) = value { groups.entry(value % 2).or_default().push(value); }
    }
    let mut result = groups.into_iter().map(|(bucket, values)| {
...
fn summaries(rows: &[GraphAggregateRow]) -> Vec<Summary> {
    rows.iter().map(|row| {
        ...
        (*bucket, row.values()[0].as_count().unwrap(), row.values()[1].as_integer().unwrap(),
...
fn top_k_then_grouping_uses_live_historical_and_pinned_data_through_reopen() {
    let ((), report) = run_async_under_lab(0xa661_0001, |root| async move {
        ...
        assert_eq!(summaries(&db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value), old);
        ...
        assert_eq!(summaries(&db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value), current);
        assert_eq!(summaries(&db.execute_graph_aggregate_governed_at(&cx, &query, basis, policy()).unwrap().value), old);
        assert_eq!(summaries(&pinned.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value), old);
        assert_eq!(summaries(&pinned.execute_graph_aggregate_governed_at(&cx, &query, basis, policy()).unwrap().value), old);
        db.compact(&commit).await.unwrap(); drop(db);
        let db = Database::open_with_vfs(&commit, vfs, &path, keys()).await.unwrap();
        assert_eq!(summaries(&db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value), current);
        assert_eq!(summaries(&db.execute_graph_aggregate_governed_at(&cx, &query, basis, policy()).unwrap().value), old);
        assert_eq!(summaries(&pinned.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value), old);
        let selective = template.bind_parameters(&GqlParameters::new()
            .with_uint64("take", 3).unwrap().with_int64("minimum", 2).unwrap()).unwrap();
        assert_eq!(summaries(&db.execute_graph_aggregate_governed(&cx, &selective, policy()).unwrap().value), vec![(1, 2, 14, (7, 1))]);
        assert_eq!(query.canonical_bytes(), frozen);
        assert_eq!(template.bind_parameters(&parameters()).unwrap().canonical_bytes(), frozen);
        assert_eq!(calls, 1);
    });
...
fn staged_values_change_input_ranking_without_publishing_or_rewriting_effects() {
    let ((), report) = run_async_under_lab(0xa661_0002, |root| async move {
        ...
        let rows = txn.execute_graph_aggregate_governed(&db, &cx, &query, policy()).unwrap();
        ...
        assert_eq!(summaries(&db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap().value), expected);
        let mut reader = db.begin(&txcx).unwrap();
        let _ = reader.execute_graph_aggregate_governed(&db, &cx, &query, policy()).unwrap();
        assert!(matches!(reader.finish(&mut db, &commit).await.unwrap(), EmbeddedTxnCompletion::ReadClosed { .. }));
    ...
    let ((), report) = run_async_under_lab(0xa661_0003, |root| async move {
        ...
        for (mode, text) in texts.iter().enumerate() {
            for insertion in [false, true] {
                ...
                match mode {
                    2 => assert!(matches!(result, Err(GqlQueryError::Source(GraphAggregateError::InputRelation(
                ...
                if insertion { winner.create_vertex(VId(77), vec![], vec![(P, CanonicalScalar::Int(50))]); }
                else { winner.set_vertex_property(VId(1), P, Some(CanonicalScalar::Int(50))); }
...
fn source_pipeline_and_groups_share_limits_and_preflight_precedes_zero_budgets() {
    let ((), report) = run_async_under_lab(0xa661_0004, |root| async move {
        ...
        let basis = seed(&mut db, &commit).await;
        let query = top_groups();
        let measured = db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap();
        ...
        assert_eq!(caps[1], 1, "private input rows do not spend the public group quota");
        assert_eq!(db.execute_graph_aggregate_governed(&cx, &query,
            GqlQueryPolicy::new(caps[0], caps[1], caps[2], caps[3])).unwrap(), measured);
        for dimension in 0..4 {
            let mut cap = caps; cap[dimension] -= 1;
            assert!(db.execute_graph_aggregate_governed(&cx, &query,
                GqlQueryPolicy::new(cap[0], cap[1], cap[2], cap[3])).is_err());
            assert_eq!(db.frontier().unwrap(), basis);
        }
        let zero = GqlQueryPolicy::new(0, 0, 0, 0);
        assert!(matches!(db.execute_graph_aggregate_governed_at(&cx, &query, CommitSeq(basis.0 + 1), zero),
            Err(GqlQueryError::Source(GraphAggregateError::Source(GqlError::Read(ReadError::BeyondFrontier { .. }))))));
        let pinned = db.read_session().unwrap();
        assert!(matches!(pinned.execute_graph_aggregate_governed_at(&cx, &query, CommitSeq(basis.0 + 1), zero),
            Err(GqlQueryError::Source(GraphAggregateError::Source(GqlError::Read(ReadError::BeyondFrontier { .. }))))));
        let other = Database::open_memory(&commit, keys()).await.unwrap();
        let txn = db.begin(&txcx).unwrap();
        assert!(matches!(txn.execute_graph_aggregate_governed(&other, &cx, &query, zero),
            Err(GqlQueryError::Source(GraphAggregateError::Source(WriteTxnError::WrongDatabase)))));
        let measured = txn.execute_graph_aggregate_governed(&db, &cx, &query, policy()).unwrap();
        assert_eq!(txn.execute_graph_aggregate_governed(&db, &cx, &query,
            GqlQueryPolicy::new(measured.rows.snapshot_records, measured.rows.result_rows,
                measured.evaluator.work_units, measured.evaluator.scratch_entries)).unwrap(), measured);
        txn.abort();
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn late_pipeline_failure_preserves_an_outer_write_that_can_still_commit() {
    let ((), report) = run_async_under_lab(0xa661_0005, |root| async move {
        ...
        txn.write(&mut db, prefix).unwrap();
        let digest = txn.staged_effect_digest().unwrap();
        let fails = query("MATCH (n) WITH n.p AS value WITH 10/(value-5) AS q RETURN SUM(q) AS total");
        assert!(matches!(txn.execute_graph_aggregate_governed(&db, &cx, &fails, policy()),
        ...
        assert_eq!(db.frontier().unwrap(), basis);
        let safe = query("MATCH (n) WITH n.p AS value WHERE value <> 5 WITH 10/(value-5) AS q RETURN SUM(q) AS total");
        let result = txn.execute_graph_aggregate_governed(&db, &cx, &safe, policy()).unwrap();
        ...
        assert_eq!(db.vertex(VId(2)).unwrap().unwrap().props, vec![(P, CanonicalScalar::Int(5))]);
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn staged_shortcuts_change_shortest_multiplicity_before_grouping_not_afterward() {
    let ((), report) = run_async_under_lab(0xa661_0006, |root| async move {
        ...
        for (mode, old_count, new_count) in [("ALL", 4, 3), ("ANY", 2, 2)] {
            let query = query(&format!("MATCH (a) OPTIONAL MATCH {mode} SHORTEST WALK (a)-[:R*1..2]->(b) WITH a AS owner,b AS peer,b.p AS value RETURN owner,COUNT(*) AS rows,COUNT(peer) AS matched,SUM(value) AS total GROUP BY owner ORDER BY owner"));
            let old = db.execute_graph_aggregate_governed(&cx, &query, policy()).unwrap();
            let staged = txn.execute_graph_aggregate_governed(&db, &cx, &query, policy()).unwrap();
            for (rows, count) in [(&old.value, old_count), (&staged.value, new_count)] {
                ...
                assert_eq!(rows[0].values()[2].as_integer(), Some(i128::from(count) * 5));
