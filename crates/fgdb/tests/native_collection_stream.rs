...
use fgdb::{
    Database, DatabaseKeys, MemVfs, NativeAggregateCursor, PreparedNativeRead, QueryResult,
    WriteBatch,
};
...
fn vertices(cut: u64) -> Vec<(VId, i64, Option<i64>)> {
    ...
    if cut >= 2 {
        rows[0].2 = Some(2);
        rows.retain(|(id, _, _)| *id != VId(1));
    }
    if cut >= 3 {
        rows.push((VId(3), 1, None));
...
fn seed() -> WriteBatch {
    ...
    for (id, bucket, p) in vertices(1) {
        let mut props: Vec<_> = p
            .into_iter()
            .map(|p| (P, CanonicalScalar::Int(p)))
            .collect();
        props.push((B, CanonicalScalar::Int(bucket)));
        b.create_vertex(id, vec![L], props);
    }
    b
}
...
fn expected(cut: u64, skip: usize, limit: usize) -> Vec<Vec<GraphAggregateValue>> {
    ...
    for (id, bucket, value) in vertices(cut) {
        groups.entry(bucket).or_default().push((id, value));
    }
    groups
        .into_iter()
        .rev()
        ...
        .map(|(bucket, rows)| {
            ...
            let all: Vec<_> = rows
                .iter()
                .filter_map(|(_, p)| p.map(|p| GraphValue::Scalar(CanonicalScalar::Int(p))))
                .collect();
            let distinct: Vec<_> = all
                .iter()
            ...
            vec![
                list(all),
                GraphAggregateValue::Value(GraphValue::Scalar(CanonicalScalar::Int(bucket))),
                list(distinct),
                list(rows.iter().map(|(id, _)| GraphValue::Vertex(*id)).collect()),
...
fn native_collection_layouts_and_pinned_lists_survive_changes_compaction_and_reopen() {
    let ((), report) = run_async_under_lab(0xc011_1001, |root| async move {
        ...
        let mut paused = prepared
            .stream_aggregate(&db, &cx, &args(1, 0, 2), wide())
            .unwrap();
        fn send(_: &impl Send) {}
        send(&paused);
        assert_eq!(paused.row_stats().snapshot_records, 0);
        assert_eq!(paused.kind(), ScanKind::Vertex);
        let mut edit = WriteBatch::new(R);
        edit.set_vertex_property(VId(0), P, Some(CanonicalScalar::Int(2)));
        edit.delete_vertex(VId(1));
        db.write(&commit, edit).await.unwrap();
        let mut add = WriteBatch::new(R);
        add.create_vertex(VId(3), vec![L], vec![(B, CanonicalScalar::Int(1))]);
        ...
        let db = Database::open_with_vfs(&commit, vfs, &path, keys())
            .await
            .unwrap();
        assert_eq!(drain(&mut paused), expected(1, 0, 2));
        assert_eq!(paused.snapshot_seq(), CommitSeq(1));
        for cut in 0..=3 {
            for (skip, limit) in [(0, 0), (0, 1), (0, 2), (1, 1), (9, 2)] {
        ...
        let mut pinned = prepared
            .stream_aggregate_in_view(&view, &cx, &args(1, 0, 2), wide())
            .unwrap();
        assert_eq!(drain(&mut pinned), expected(1, 0, 2));
        assert!(
            prepared
                .stream_aggregate_in_view(&view, &cx, &args(2, 0, 0), wide())
                .is_err()
        );
    });
...
fn native_collection_errors_limits_and_empty_input_keep_atomic_result_semantics() {
    let ((), report) = run_async_under_lab(0xc011_1002, |root| async move {
        ...
        for policy in [
            ...
            let mut c = prepared
                .stream_aggregate(&db, &cx, &params, policy)
                .unwrap();
        ...
        for text in [
            "MATCH (n:L) FOR SYSTEM_TIME AS OF SEQ 0 RETURN COLLECT(n) AS ids",
        ...
        edit.set_vertex_property(
            HIGH,
            P,
            Some(CanonicalScalar::ucs_basic_text("private sum operand").unwrap()),
        );
        db.write(&commit, edit).await.unwrap();
        for limit in [0, 1] {
            let text =
                format!("MATCH (n:L) RETURN COLLECT(n.p) AS vals,SUM(n.p) AS total LIMIT {limit}");
            let mut c = db
                .query_aggregate_stream(&cx, &text, &params, symbols(), wide())
                .unwrap();
            assert!(matches!(
...
fn native_identified_edge_collections_preserve_order_and_refuse_unproved_inputs() {
    let ((), report) = run_async_under_lab(0xc011_1003, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut b = seed();
        for (id, a, to, value) in [
            (1, HIGH, VId(0), Some(9)),
            (2, VId(0), VId(1), Some(-4)),
            (3, HIGH, VId(0), Some(9)),
            (4, VId(1), VId(1), None),
            (5, VId(1), HIGH, Some(7)),
        ] {
        ...
        for direction in 0..3 {
            for hops in 1..=2 {
                let atom = |name, end| match direction {
                    0 => format!("-[{name}:R]->({end})"),
                    1 => format!("<-[{name}:R]-({end})"),
                    _ => format!("-[{name}:R]-({end})"),
                };
                ...
                let mut c = db
                    .query_aggregate_stream(&cx, &text, &params, symbols(), wide())
                    .unwrap();
                assert_eq!(c.kind(), ScanKind::Edge);
                assert_eq!(drain(&mut c), rows);
                assert_eq!(c.row_stats().result_rows, 1);
                if direction == 0 && hops == 1 {
                    assert_eq!(
                        rows[0][rows[0].len() - 1],
                        list(
                            vec![9, -4, 7]
                                .into_iter()
                                .map(|n| GraphValue::Scalar(CanonicalScalar::Int(n)))
                                .collect()
                        )
                    );
                }
        ...
        for text in [
            "MATCH (a)-[r:R]->(b) RETURN COLLECT(b) AS vals",
            "MATCH (a)-[r:R]->(b) RETURN COLLECT(r.p) AS vals",
        ] {
            let p = PreparedNativeRead::prepare(text, &GqlParameters::new(), symbols()).unwrap();
            assert!(
                p.stream_aggregate(&db, &cx, &GqlParameters::new(), wide())
                    .is_err()
            );
        }
    });
