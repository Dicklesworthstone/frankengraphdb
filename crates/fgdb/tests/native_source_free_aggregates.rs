...
use fgdb_gql::{
    GqlListParameter, GqlParameterValue, GqlParameters, GqlQueryError, GqlQueryPolicy,
...
fn list_parameters(step: i64) -> GqlParameters {
    ...
    args.insert(
        ...
        GqlParameterValue::List(
            GqlListParameter::new(values.into_iter().map(GraphValue::Scalar).collect()).unwrap(),
        ),
    )
    .unwrap();
    args
...
fn database_view_and_reusable_native_template_execute_list_aggregates_without_a_scan() {
    let ((), report) = run_async_under_lab(0x7366_6101, |root| async move {
        ...
        let text =
            "UNWIND $xs AS x RETURN SUM(x*$step) AS total,COUNT(*) AS rows,AVG(x*$step) AS mean";
        ...
        let PreparedNativeRead::PipelineAggregate(native) = &prepared else {
            unreachable!()
        };
        ...
        assert_eq!(
            view.query(&cx, text, &args, no_symbols, policy()).unwrap(),
            first
        );
        assert_eq!(db.frontier().unwrap(), basis);
        db.write(&commit, batch(2, 900)).await.unwrap();
        assert_eq!(
            db.query(&cx, text, &args, no_symbols, policy()).unwrap(),
            first
        );
        drop(db);
        assert_eq!(
        ...
        check_summary(
            &prepared
                .execute_in_view(&view, &cx, &list_parameters(3), policy())
                .unwrap(),
            57,
            4,
        );
        ...
        assert!(
            prepared
                .execute_in_view(&view, &cx, &GqlParameters::new(), policy())
                .is_err()
        );
    ...
    let ((), report) = run_async_under_lab(0x7366_6102, |root| async move {
        ...
        for (text, count) in [
            ...
            assert_eq!(
                db.query(&cx, text, &args, no_symbols, policy()).unwrap(),
                QueryResult::Rows {
                    columns: vec!["rows".to_owned()],
                    rows: vec![vec![GraphAggregateValue::Count(count)]],
                }
            );
        }
        assert!(matches!(
        ...
        assert!(matches!(
            db.query(&cx, "RETURN SUM(1/0) LIMIT 0", &args, no_symbols, policy()),
...
fn source_free_transaction_reads_retain_owner_checks_staged_effects_and_graph_lane() {
    let ((), report) = run_async_under_lab(0x7366_6103, |root| async move {
        ...
        let expected = QueryResult::Rows {
            ...
            rows: vec![vec![GraphAggregateValue::Integer(7)]],
        };
        assert_eq!(
            transaction
                .query(&db, &cx, text, &params, no_symbols, policy())
                .unwrap(),
            expected
        );
        assert_eq!(
            prepared
                .execute_in_transaction(&transaction, &db, &cx, &params, policy())
                .unwrap(),
            expected
        );
        let wrong = transaction.query(
            &other,
            &cx,
            text,
            &params,
        ...
        let wrong = prepared.execute_in_transaction(
            &transaction,
            &other,
            &cx,
            &params,
        ...
        let PreparedNativeRead::PipelineAggregate(native) = &graph_prepared else {
            unreachable!()
        };
        assert!(!native.is_source_free());
        let graph_policy = GqlQueryPolicy::new(100, 100, 1_000_000, 1_000_000);
        let staged = graph_prepared
            .execute_in_transaction(&transaction, &db, &cx, &params, graph_policy)
            .unwrap();
        assert_eq!(
            staged,
            QueryResult::Rows {
                columns: vec!["total".to_owned()],
                rows: vec![vec![GraphAggregateValue::Integer(11)]],
            }
        );
        assert_eq!(
            db.query(&cx, graph, &params, symbols, graph_policy)
                .unwrap(),
            QueryResult::Rows {
                columns: vec!["total".to_owned()],
                rows: vec![vec![GraphAggregateValue::Integer(4)]],
            }
        );
