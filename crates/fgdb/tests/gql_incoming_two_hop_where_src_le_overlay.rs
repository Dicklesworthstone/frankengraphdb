...
#[test]
fn incoming_two_hop_near_end_property_less_equal_sees_the_staged_continuation() {
    let ((), report) = run_async_under_lab(0x97_53, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-two-hop-where-src-le-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
        ...
        // Durably, only the chain landing on the k=9 dest is complete. It
        ...
        // The overlay stages the continuation onto the k=1 dest's chain,
        // which meets <= 1 on the boundary.
