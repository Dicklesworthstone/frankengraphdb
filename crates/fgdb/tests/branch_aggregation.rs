...
fn prepare(text: &str) -> PreparedGraphAggregate {
    PreparedGraphAggregateText::prepare(text, |kind, name| match kind {
        GraphSymbolKind::Relation => match name {
            "R" => Some(GraphSymbol::Relation(R)),
            "S" => Some(GraphSymbol::Relation(S)),
            "T" => Some(GraphSymbol::Relation(T)),
            "U" => Some(GraphSymbol::Relation(U)),
            "V" => Some(GraphSymbol::Relation(V)),
            _ => None,
        },
        _ => None,
    })
...
fn oracle(edges: &[EdgeRecord]) -> Vec<Summary> {
    ...
    for root in edges.iter().filter(|e| e.entry.relation == R) {
        for hidden in edges
            .iter()
            .filter(|e| e.entry.relation == S && e.entry.src == root.entry.dst)
        {
            for _leaf in edges
                .iter()
                .filter(|e| e.entry.relation == T && e.entry.src == hidden.entry.dst)
            {
                for middle in edges
                    .iter()
                    .filter(|e| e.entry.relation == U && e.entry.src == root.entry.dst)
                {
                    for _close in edges.iter().filter(|e| {
                        e.entry.relation == V
                            && e.entry.src == middle.entry.dst
                            && e.entry.dst == root.entry.src
                    }) {
                        for _sibling in edges
                            .iter()
                            .filter(|e| e.entry.relation == S && e.entry.src == middle.entry.dst)
                        {
                            for last in edges
                                .iter()
                                .filter(|e| e.entry.relation == U && e.entry.src == root.entry.src)
                            {
                                let group = groups
                                    .entry((middle.entry.dst, last.entry.dst))
                                    .or_default();
                                group.0 += 1;
...
fn interleaved_forests_and_cycles_cover_all_reads_canonical_staging_and_reopening() {
    let ((), report) = run_async_under_lab(0xf0ce_0001, |root| async move {
        ...
        assert_eq!(
            old,
            vec![
                (VId(40), VId(60), 40, 1, VId(0), VId(0)),
                (VId(40), VId(61), 40, 1, VId(0), VId(0)),
                (VId(41), VId(61), 1, 1, VId(1), VId(1))
            ]
        );
        let mut txn = db.begin(&txn_cx).unwrap();
        for result in [
            db.execute_graph_aggregate_governed(&cx, &query, wide())
                .unwrap(),
            db.execute_graph_aggregate_governed_at(&cx, &query, basis, wide())
                .unwrap(),
            view.execute_graph_aggregate_governed(&cx, &query, wide())
                .unwrap(),
            view.execute_graph_aggregate_governed_at(&cx, &query, basis, wide())
                .unwrap(),
            txn.execute_graph_aggregate_governed(&db, &cx, &query, wide())
                .unwrap(),
        ] {
        ...
        let measured = db
            .execute_graph_aggregate_governed(&cx, &query, wide())
            .unwrap();
        ...
        for policy in [
            GqlQueryPolicy::new(22, 2, u64::MAX, u64::MAX),
        ...
        assert_eq!(
            expected,
            vec![
                (VId(40), VId(60), 12, 1, VId(0), VId(0)),
                (VId(40), VId(61), 12, 1, VId(0), VId(0)),
                (VId(41), VId(61), 1, 1, VId(1), VId(1))
            ]
        );
        assert_eq!(
        ...
        assert_eq!(
            plain(
                &reopened
                    .execute_graph_aggregate_governed_at(&cx, &query, basis, wide())
                    .unwrap()
                    .value
            ),
            old
        ...
        for mode in 0..3 {
            for change in 0..3 {
                ...
                match change {
                    ...
                    1 => {
                        winner.add_edge(EId(91), VId(22), VId(30), vec![]);
                    }
                    _ => {
                        winner.delete_edge(EId(20));
                    }
                }
...
fn early_billion_assignment_branches_are_folded_before_the_returned_cycle() {
    let ((), report) = run_async_under_lab(0xf0ce_0003, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let mut vertices = WriteBatch::new(RelationId(99));
        for id in (0..10).chain(std::iter::once(20)) {
            vertices.create_vertex(VId(id), vec![], vec![]);
        }
        let mut root_edge = WriteBatch::new(R);
        root_edge.add_edge(EId(1), VId(0), VId(1), vec![]);
        let mut choices = WriteBatch::new(S);
        for id in 2..10 {
            choices.add_edge(EId(100 + id), VId(1), VId(id), vec![]);
        }
        let mut target = WriteBatch::new(U);
        target.add_edge(EId(2), VId(1), VId(20), vec![]);
        let mut closing = WriteBatch::new(V);
        closing.add_edge(EId(3), VId(20), VId(0), vec![]);
        db.write_atomic(&commit, vec![vertices, root_edge, choices, target, closing])
            .await
            .unwrap();
        let mut statement = "MATCH (a)-[:R]->(b)".to_owned();
        for at in 0..10 {
            statement.push_str(&format!(",(b)-[:S]->(h{at})"));
        }
        statement.push_str(
            ",(b)-[:U]->(c)-[:V]->(a) RETURN c,COUNT(*) AS n,COUNT(DISTINCT a) AS d GROUP BY c",
        );
        let query = prepare(&statement);
        let result = db
            .execute_graph_aggregate_governed(&cx, &query, GqlQueryPolicy::new(11, 1, 65_536, 4096))
            .unwrap();
