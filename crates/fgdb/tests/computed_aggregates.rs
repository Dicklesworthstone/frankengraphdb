...
use fgdb::{Database, DatabaseKeys, EdgeRecord, GqlError, MemVfs, ReadError,
    VertexRow, WriteBatch, WriteError, WriteTxnError};
use fgdb_delta_types::{LabelId, PropertyKeyId, RelationId};
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryPolicy, GraphAggregateError,
    GraphAggregateRow, GraphIntegerErrorKind, GraphSymbol, GraphSymbolKind,
...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(10_000, 1_000, 5_000_000, 2_000_000) }
...
fn parameters() -> GqlParameters {
    GqlParameters::new().with_int64("floor", 0).unwrap().with_int64("scale", 2).unwrap()
}
...
async fn seed(db: &mut Database<MemVfs>, cx: &CommitCx) -> CommitSeq {
    ...
    for (id, source, target) in [(101, 1, 11), (102, 1, 11), (103, 11, 12),
        (104, 2, 12), (105, 2, 13), (106, 13, 12)] {
...
fn oracle(vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<Summary> {
    ...
    for owner in vertices.iter().filter(|row| row.labels.contains(&OWNER)) {
        ...
        for _ in 0..2 {
            ...
            for source in layer {
                for edge in edges.iter().filter(|edge| edge.entry.relation == R && edge.entry.src == source) {
        ...
        if matches.is_empty() { matches.push(None); }
        groups.entry(integer(owner).map(i64::abs)).or_default().extend(matches);
    }
    let mut result: Vec<_> = groups.into_iter().map(|(bucket, values)| {
        let count = values.len() as u64;
        let present = values.iter().flatten().count() as u64;
        let sum: i128 = values.iter().map(|value| i128::from(value.unwrap_or(0))).sum();
        let unique = values.iter().flatten().map(|value| value.abs()).collect::<BTreeSet<_>>().len() as u64;
        (bucket, count, present, sum * 2, fraction(sum, count), unique)
    }).collect();
...
fn summaries(rows: &[GraphAggregateRow]) -> Vec<Summary> {
    rows.iter().map(|row| {
        ...
        (bucket, row.get(0).unwrap().as_count().unwrap(), row.get(1).unwrap().as_count().unwrap(),
            row.get(2).unwrap().as_integer().unwrap(), (mean.numerator(), mean.denominator()),
...
fn computed_walk_groups_reuse_live_historical_pinned_and_staged_snapshots() {
    let ((), report) = run_async_under_lab(0xa991_0001, |root| async move {
        ...
        assert_eq!(old, vec![(Some(2), 6, 6, 68, (17, 3), 2), (Some(5), 1, 0, 0, (0, 1), 0)]);
        ...
        assert_eq!(expected, vec![(Some(2), 1, 1, 8, (4, 1), 1),
            (Some(5), 1, 1, 8, (4, 1), 1), (Some(8), 2, 2, 16, (4, 1), 1)]);
        assert_eq!(summaries(&txn.execute_graph_aggregate_governed(&db, &cx, &plan, policy()).unwrap().value), expected);
        assert_eq!(summaries(&db.execute_graph_aggregate_governed(&cx, &plan, policy()).unwrap().value), old);
        ...
        assert_eq!(summaries(&reopened.execute_graph_aggregate_governed(&cx, &plan, policy()).unwrap().value), expected);
        assert_eq!(summaries(&reopened.execute_graph_aggregate_governed_at(&cx, &plan, basis, policy()).unwrap().value), old);
        assert_eq!(summaries(&pinned.execute_graph_aggregate_governed(&cx, &plan, policy()).unwrap().value), old);
        assert_eq!(frozen, template.bind_parameters(&parameters()).unwrap().canonical_bytes());
        assert_eq!(calls.get(), 3);
    });
    ...
    let ((), report) = run_async_under_lab(0xa991_0002, |root| async move {
        ...
        for mode in 0..5 {
            for insertion in [false, true] {
                ...
                let plan = prepare(&format!("MATCH (a:Owner)-[:R]->(b) WHERE b.p>={floor} \
                    RETURN COUNT(*) AS c,SUM({expression}) AS total{page}"));
                let cap = if mode == 2 { GqlQueryPolicy::new(10_000, 0, 5_000_000, 2_000_000) } else { policy() };
                let result = txn.execute_graph_aggregate_governed(&db, &cx, &plan, cap);
                match mode {
                    1 => assert!(matches!(result, Err(GqlQueryError::Source(GraphAggregateError::InputExpression { error, .. }))
...
fn aggregate_source_projection_and_output_share_exact_cumulative_limits() {
    let ((), report) = run_async_under_lab(0xa991_0003, |root| async move {
        ...
        let mut db = Database::open_memory(&commit, keys()).await.unwrap();
        let basis = seed(&mut db, &commit).await;
        let plan = PreparedGraphAggregateText::prepare(QUERY, symbols).unwrap().bind_parameters(&parameters()).unwrap();
        ...
        for budget in [
            ...
            GqlQueryPolicy::new(10_000, 100, u64::MAX, measured.evaluator.scratch_entries - 1),
        ] { assert!(db.execute_graph_aggregate_governed(&cx, &plan, budget).is_err()); }
        assert_eq!(db.frontier().unwrap(), basis);
        let future = CommitSeq(basis.0 + 1);
        assert!(matches!(db.execute_graph_aggregate_governed_at(&cx, &plan, future,
            GqlQueryPolicy::new(0, 0, 0, 0)),
            Err(GqlQueryError::Source(GraphAggregateError::Source(GqlError::Read(ReadError::BeyondFrontier { .. }))))));
        let pinned = db.read_session().unwrap();
        assert!(matches!(pinned.execute_graph_aggregate_governed_at(&cx, &plan, future,
            GqlQueryPolicy::new(0, 0, 0, 0)),
            Err(GqlQueryError::Source(GraphAggregateError::Source(GqlError::Read(ReadError::BeyondFrontier { .. }))))));
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn hidden_computed_aggregate_overflow_refuses_without_changing_prior_workspace() {
    let ((), report) = run_async_under_lab(0xa991_0004, |root| async move {
        ...
        for tail in ["HAVING SUM(b.p*2)>0", "ORDER BY SUM(b.p*2) LIMIT 0"] {
            let plan = prepare(&format!("MATCH (a:Owner)-[:R]->(b) RETURN COUNT(*) AS c {tail}"));
            assert!(matches!(txn.execute_graph_aggregate_governed(&db, &cx, &plan, policy()),
        ...
        assert_eq!(integer(&db.vertex(VId(12)).unwrap().unwrap()), Some(i64::MAX));
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
