...
use fgdb_types::{CanonicalScalar, CommitCx, CommitSeq, DatabaseSecurityNamespaceId,
    EmbeddedTxnState, EId, PurposeContexts, VId};
use std::cell::Cell;
...
fn policy() -> GraphInsertPolicy {
    GraphInsertPolicy::new(GqlQueryPolicy::new(0, 1, 1_000_000, 1_000_000), 1_000, 1_000)
}
fn read_policy() -> GqlQueryPolicy { GqlQueryPolicy::new(1_000, 1_000, 1_000_000, 1_000_000) }
...
fn allocator(base: u128, request: GraphInsertRequest) -> Result<ElementId, &'static str> {
    Ok(match request {
        GraphInsertRequest::Vertex { row, vertex } => ElementId::Vertex(VId(base + row as u128 * 16 + vertex as u128)),
        GraphInsertRequest::Edge { row, edge } => ElementId::Edge(EId(base + row as u128 * 16 + edge as u128)),
    })
}
...
fn verify_bootstrap(db: &Database<MemVfs>, base: u128, seed: i64) {
    for (offset, value) in [(0, Some(seed)), (1, Some(seed + 1)), (2, None)] {
        ...
        assert_eq!(vertex.props, vec![(P, value.map_or(CanonicalScalar::Null, CanonicalScalar::Int))]);
    }
...
fn empty_database_bootstrap_and_dependent_match_publish_once_and_reopen_with_history() {
    let ((), report) = run_async_under_lab(0xc8eb_0001, |root| async move {
        ...
        let first = template.bind_parameters(&GqlParameters::new().with_int64("seed", 10).unwrap()).unwrap();
        let second = template.bind_parameters(&GqlParameters::new().with_int64("seed", 20).unwrap()).unwrap();
        let frozen = first.canonical_bytes();
        let mut txn = db.begin(&txcx).unwrap();
        let before = txn.execute_graph_insert_governed(&mut db, &cx, &first, policy(), |request| allocator(100, request)).unwrap();
        let after = txn.execute_graph_insert_governed(&mut db, &cx, &second, policy(), |request| allocator(200, request)).unwrap();
        assert_eq!(before, after, "unit cost/cardinality must not grow with staged graph population");
        assert_eq!((before.selection.snapshot_records, before.selection.result_rows), (0, 1));
        ...
        let all = PreparedGraphText::prepare("MATCH (n) RETURN n", symbols).unwrap()
            .bind_parameters(&GqlParameters::new()).unwrap();
        assert!(reopened.execute_graph_pattern_governed_at(&cx, &all, basis, read_policy()).unwrap().value.is_empty());
        assert_eq!(reopened.execute_graph_pattern_governed(&cx, &all, read_policy()).unwrap().value.len(), 7);
...
fn unit_insertion_does_not_invent_phantoms_or_erase_earlier_absence_observations() {
    let ((), report) = run_async_under_lab(0xc8eb_0002, |root| async move {
        ...
        for observe in [false, true] {
            ...
            let mut txn = db.begin(&txcx).unwrap();
            if observe { assert!(txn.vertex(&db, VId(900)).unwrap().is_none()); }
            let insertion = query("CREATE (a:Node {p:1})-[:R]->(b:Node {p:2})");
            let stats = txn.execute_graph_insert_governed(&mut db, &cx, &insertion, policy(), |request| allocator(100, request)).unwrap();
            ...
            if observe {
                assert!(matches!(completed, Err(WriteTxnError::Write(WriteError::FirstCommitterWins {
                ...
                assert_eq!(completed.unwrap().commit_seq(), Some(CommitSeq(frontier.0 + 1)));
...
fn standalone_failures_preserve_prior_staging_without_recycling_allocated_ids() {
    let ((), report) = run_async_under_lab(0xc8eb_0003, |root| async move {
        ...
        for mode in 0..7 {
            ...
            } else { "CREATE (a {p:1})-[:R]->(b {p:2})" });
            let allowance = if mode == 1 { GraphInsertPolicy::new(policy().query, 1, 10) } else { policy() };
            let calls = Cell::new(0);
            let result = txn.execute_graph_insert_governed(&mut db, &cx, &insertion, allowance, |request| {
                ...
                match (mode, request) {
                    ...
                    (5, GraphInsertRequest::Vertex { vertex: 0, .. }) => Ok(ElementId::Vertex(VId(1))),
                    (6, GraphInsertRequest::Vertex { vertex: 0, .. }) => Ok(ElementId::Vertex(VId(98))),
            ...
            for id in [98, 100, 101] { assert!(db.vertex(VId(id)).unwrap().is_none()); }
...
fn nullable_match_endpoints_are_not_reinterpreted_as_inline_new_nodes() {
    let ((), report) = run_async_under_lab(0xc8eb_0004, |root| async move {
        ...
        seed(&mut db, &commit).await;
        let mut txn = db.begin(&txcx).unwrap();
        let insertion = query("MATCH (n:Node) OPTIONAL MATCH (n)-[:R]->(missing) \
            CREATE (missing)-[:R]->(copy:Copy {p:1})");
        assert_eq!(insertion.vertices_per_row(), 1);
        let calls = Cell::new(0);
        let result = txn.execute_graph_insert_governed(&mut db, &cx, &insertion,
            GraphInsertPolicy::new(read_policy(), 100, 100), |request| {
                calls.set(calls.get() + 1); allocator(100, request)
            });
        assert!(matches!(result, Err(GqlQueryError::Source(GraphInsertError::NullEndpoint { .. }))));
        assert_eq!(calls.get(), 0);
        assert_eq!(txn.finish(&mut db, &commit).await.unwrap().commit_seq(), None);
        let mut txn = db.begin(&txcx).unwrap();
        let standalone = query("CREATE (missing)-[:R]->(copy:Copy {p:1})");
        assert_eq!(standalone.vertices_per_row(), 2);
        txn.execute_graph_insert_governed(&mut db, &cx, &standalone, policy(), |request| allocator(100, request)).unwrap();
        txn.finish(&mut db, &commit).await.unwrap();
        assert!(db.vertex(VId(100)).unwrap().is_some());
...
fn standalone_creation_still_checks_owner_and_basis_before_allocating() {
    let ((), report) = run_async_under_lab(0xc8eb_0005, |root| async move {
        ...
        let result = txn.execute_graph_insert_governed(&mut foreign, &cx, &insertion, denied, |request| {
            calls.set(calls.get() + 1); allocator(100, request)
        });
        ...
        db.write(&commit, winner).await.unwrap();
        let result = txn.execute_graph_insert_governed(&mut db, &cx, &insertion, denied, |request| {
            calls.set(calls.get() + 1); allocator(100, request)
        });
        assert!(matches!(result, Err(GqlQueryError::Source(GraphInsertError::Source(WriteTxnError::SnapshotAdvanced { .. })))));
        assert_eq!(calls.get(), 0);
        txn.abort();
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
...
fn standalone_bootstrap_recovery_never_exposes_a_partial_created_structure() {
    let ((), report) = run_async_under_lab(0xc8eb_0006, |root| async move {
        ...
        for phase in 0..4 {
            ...
            let mut db = Database::create_with_vfs(&commit, vfs.clone(), &path, keys()).await.unwrap();
            let mut txn = db.begin(&txcx).unwrap();
            let insertion = PreparedGraphInsertText::prepare(BOOTSTRAP, R, symbols).unwrap()
                .bind_parameters(&GqlParameters::new().with_int64("seed", 10).unwrap()).unwrap();
            txn.execute_graph_insert_governed(&mut db, &cx, &insertion, policy(), |request| allocator(100, request)).unwrap();
            let crash = match phase {
                0 => Some(CrashPoint::BeforeCapsule), 1 => Some(CrashPoint::AfterD1),
                2 => Some(CrashPoint::AfterMarkerBeforeD2), _ => None,
            };
            ...
            if phase < 2 { assert_eq!(counts, (0, 0)); }
            else if phase == 3 { assert_eq!(counts, (3, 3)); }
            else { assert!(counts == (0, 0) || counts == (3, 3)); }
            if counts == (3, 3) { verify_bootstrap(&db, 100, 10); }
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
