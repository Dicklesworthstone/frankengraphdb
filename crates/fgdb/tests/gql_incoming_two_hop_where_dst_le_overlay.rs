...
#[test]
fn incoming_two_hop_far_end_property_less_or_equal_sees_the_staged_boundary() {
    let ((), report) = run_async_under_lab(0x82_10, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-incoming-two-hop-where-dst-le-overlay-{}",
            std::process::id()
        ));
        let r = RelationId(1);
        let s = RelationId(2);
        ...
        seed_r.create_vertex(VId(7), vec![], vec![]);
        seed_r.create_vertex(VId(8), vec![], vec![]);
        seed_r.create_vertex(VId(9), vec![], vec![]);
        seed_r.add_edge(EId(10), VId(2), VId(1), vec![]);
        seed_r.add_edge(EId(12), VId(5), VId(4), vec![]);
        seed_r.add_edge(EId(14), VId(8), VId(7), vec![]);
