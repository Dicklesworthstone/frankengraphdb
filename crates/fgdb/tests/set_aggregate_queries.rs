...
use fgdb_gql::{
    ...
    GraphIntegerErrorKind, GraphSetExecutionError, GraphSymbol, GraphSymbolKind, PreparedGraphSet,
    PreparedGraphSetAggregate, PreparedGraphSetText,
};
use fgdb_types::{
...
fn grouped(rows: &[GraphAggregateRow]) -> Vec<(i64, u64, i128)> {
    rows.iter()
        .map(|row| {
            let Some(CanonicalScalar::Int(key)) = row.keys()[0].as_scalar() else {
                panic!("integer key")
            };
            (
                *key,
...
fn top_oracle(vertices: &[VertexRow]) -> Vec<(i64, u64, i128)> {
    ...
    groups
        .into_iter()
        .map(|(value, count)| (value, count, i128::from(value) * i128::from(count)))
        .collect()
}
...
fn compound_local_top_k_groups_survive_staging_compaction_history_and_reopen() {
    let ((), report) = run_async_under_lab(0xc057_0001, |root| async move {
        ...
        for result in [
            db.execute_graph_set_aggregate_governed(&cx, &query, wide())
                .unwrap(),
            db.execute_graph_set_aggregate_governed_at(&cx, &query, basis, wide())
                .unwrap(),
            pinned
                .execute_graph_set_aggregate_governed(&cx, &query, wide())
                .unwrap(),
            pinned
                .execute_graph_set_aggregate_governed_at(&cx, &query, basis, wide())
                .unwrap(),
            txn.execute_graph_set_aggregate_governed(&db, &cx, &query, wide())
                .unwrap(),
        ] {
        ...
        assert_eq!(
            identity,
            summary(template.bind_parameters(&arguments).unwrap(), true, None).canonical_bytes()
        );
        assert_eq!(catalog_calls, 3);
    });
...
fn all_operand_admission_and_grouping_use_one_exact_budget_in_each_reader() {
    let ((), report) = run_async_under_lab(0xc057_0002, |root| async move {
        ...
        let set = db
            .execute_graph_set_governed(&cx, query.input(), wide())
            .unwrap();
        ...
        for mode in 0..3 {
            let execute = |policy| match mode {
                0 => db
                    .execute_graph_set_aggregate_governed(&cx, &query, policy)
                    .map_err(|error| format!("{error:?}")),
                1 => pinned
                    .execute_graph_set_aggregate_governed(&cx, &query, policy)
                    .map_err(|error| format!("{error:?}")),
                _ => txn
                    .execute_graph_set_aggregate_governed(&db, &cx, &query, policy)
                    .map_err(|error| format!("{error:?}")),
            };
            let measured = execute(wide()).unwrap();
            if mode != 2 {
                assert_eq!(measured.rows.snapshot_records, set.rows.snapshot_records);
            }
            ...
            assert_eq!(caps[1], 1);
            assert_eq!(
                execute(GqlQueryPolicy::new(caps[0], caps[1], caps[2], caps[3])).unwrap(),
                measured
            );
        ...
        for operation in ["EXCEPT", "INTERSECT"] {
            for absent in [false, true] {
                for mode in 0..3 {
                    ...
                    if !absent {
                        base.create_vertex(VId(2), vec![B], vec![(P, CanonicalScalar::Int(7))]);
                    }
                    ...
                    let input = relation(&format!(
                        "MATCH (a:A) RETURN a.p AS amount {operation} DISTINCT MATCH (b:B) RETURN b.p AS amount"
                    ));
                    ...
                    let result = txn
                        .execute_graph_set_aggregate_governed(&db, &cx, &query, wide())
                        .unwrap();
                    if mode != 0 {
                    ...
                    if absent {
                        winner.create_vertex(VId(2), vec![B], vec![(P, CanonicalScalar::Int(7))]);
                    } else {
...
fn late_second_operand_arithmetic_failure_preserves_outer_effects_and_observations() {
    let ((), report) = run_async_under_lab(0xc057_0004, |root| async move {
        ...
        for concurrent_change in [false, true] {
            ...
            let query = summary(
                relation(
                    "MATCH (a:A) RETURN a.p AS amount UNION ALL MATCH (b:B) RETURN 10/b.p AS amount",
                ),
                false,
                Some(0),
            );
            ...
            txn.write(&mut db, prefix).unwrap();
            assert!(
                matches!(txn.execute_graph_set_aggregate_governed(&db, &cx, &query, wide()),
...
fn distinct_shortest_selectors_in_separate_arms_observe_the_same_staged_topology() {
    let ((), report) = run_async_under_lab(0xc057_0005, |root| async move {
        ...
        let query = summary(
            relation(
                "MATCH ALL SHORTEST WALK (a:Source)-[:R*1..2]->(b) RETURN b.p AS amount UNION ALL MATCH ANY SHORTEST WALK (a:Source)-[:R*1..2]->(b) RETURN b.p AS amount",
            ),
            false,
            None,
        );
...
fn wrong_owner_finished_transaction_and_future_history_precede_zero_budgets() {
    let ((), report) = run_async_under_lab(0xc057_0006, |root| async move {
        ...
        for result in [
            ...
            pinned.execute_graph_set_aggregate_governed_at(
                &cx,
                &query,
                CommitSeq(basis.0 + 1),
                zero,
        ...
        let mut txn = db.begin(&txcx).unwrap();
        assert!(matches!(
            txn.execute_graph_set_aggregate_governed(&other, &cx, &query, zero),
            Err(GqlQueryError::Source(GraphAggregateError::Source(
        ...
        assert!(matches!(
            txn.execute_graph_set_aggregate_governed(&db, &cx, &query, zero),
            Err(GqlQueryError::Source(GraphAggregateError::Source(
