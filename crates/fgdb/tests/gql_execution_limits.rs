...
use fgdb::{Database, DatabaseKeys, GqlError, ReadError, RelationBind, WriteBatch, WriteError, WriteTxnError};
...
fn limited_queries_agree_across_live_pinned_historical_and_staged_surfaces() {
    let ((), report) = run_async_under_lab(0x61a1_0001, |root| async move {
        ...
        let bind = RelationBind::new().with_relation("R", RelationId(1)).with_label("L", LabelId(1));
        ...
        for statement in [
            ...
            let reference = db.execute_prepared_query(&query).expect("durable reference");
            let live = db.execute_prepared_query_limited(&query, limits).expect("limited live");
            assert_eq!(live.value, reference);
            assert_eq!(pinned.execute_prepared_query_limited(&query, limits).expect("limited pinned"), live);
            assert_eq!(db.execute_prepared_query_limited_at(&query, basis, limits).expect("limited history"), live);
            assert_eq!(txn.execute_prepared_query_limited(&db, &query, limits).expect("limited overlay").value, reference);
        }
        let query = db.prepare_gql_query("MATCH (a)-[:R]->(b)-[:R]->(c) RETURN c", &bind).expect("prepare");
        let old = pinned.execute_prepared_query_limited(&query, limits).expect("old view").value;
        ...
        let overlay = txn.execute_prepared_query_limited(&db, &query, limits).expect("staged query").value;
        assert_eq!(overlay, txn.execute_prepared_query(&db, &query).expect("ordinary overlay"));
        assert_ne!(overlay, old);
        let published = txn.commit(&mut db, &cx).await.expect("commit");
        assert_eq!(db.execute_prepared_query_limited(&query, limits).expect("published query").value, overlay);
        assert_eq!(db.execute_prepared_query_limited_at(&query, basis, limits).expect("retained old query").value, old);
        assert_eq!(pinned.execute_prepared_query_limited(&query, limits).expect("still pinned").value, old);
        assert!(matches!(
            pinned.execute_prepared_query_limited_at(&query, published, GlaExecutionLimits::new(0, 0)),
...
fn execution_refusal_preserves_phantom_validation_and_owner_checks() {
    let ((), report) = run_async_under_lab(0x61a1_0002, |root| async move {
        ...
        db.write(&cx, seed).await.expect("seed");
        let mut txn = db.begin(&txn_cx).expect("begin");
        let query = db.prepare_gql_query("MATCH (a)-[:R]->(b) RETURN b",
            &RelationBind::new().with_relation("R", RelationId(1))).expect("prepare");
        assert!(matches!(
        ...
        txn.write(&mut db, stage).expect("continue after refused query");
        ...
        assert!(matches!(db.execute_prepared_query_limited_at(&query, CommitSeq(frontier.0 + 1),
