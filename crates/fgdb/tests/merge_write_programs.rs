...
fn merge(value: i64) -> GraphWriteStatement {
    PreparedGraphVertexMergeText::prepare("MERGE (n:Person {p:$key})", R, symbols)
        .unwrap().bind_parameters(&GqlParameters::new().with_int64("key", value).unwrap())
        .unwrap().into()
}
fn upsert(value: i64) -> GraphWriteStatement {
    PreparedGraphVertexUpsertText::prepare(
        "MERGE (n:Person {p:$key}) ON MATCH SET n.q=41 ON CREATE SET n.q=5", R, symbols,
    ).unwrap().bind_parameters(&GqlParameters::new().with_int64("key", value).unwrap())
        .unwrap().into()
}
...
fn mixed_merge_create_and_update_steps_share_overlay_and_return_exact_identities() {
    let ((), report) = run_async_under_lab(0x6e29_2001, |root| async move {
        ...
        let update = PreparedGraphMutationText::prepare(
            "MATCH (n:Person) WHERE n.p=1 SET n.q=n.q+1", R, symbols,
        ).unwrap().bind_parameters(&GqlParameters::new()).unwrap();
        let program = PreparedGraphWriteProgram::prepare(vec![
            merge(1), upsert(1), insert.into(), update.into(),
        ]).unwrap();
        let mut requests = Vec::new();
        let mut txn = db.begin(&txcx).unwrap();
        let receipt = txn.execute_graph_write_program_returning_governed(
            &mut db, &query, &program, policy(2, 2), |request| {
        ...
        assert_eq!(requests.iter().map(|request| request.statement).collect::<Vec<_>>(), vec![0, 2]);
        let stats = receipt.stats();
        assert_eq!((stats.completed_statements, stats.created_vertices, stats.created_edges), (4, 2, 0));
        assert_eq!((stats.mutation_effects, stats.target_vertex_visits), (2, 2));
        assert_eq!(receipt.steps()[0].merged_vertex(), Some(GraphVertexMergeOutcome::Created(VId(100))));
        assert_eq!(receipt.steps()[1].merged_vertex(), Some(GraphVertexMergeOutcome::Matched(VId(100))));
        ...
        let read = PreparedGraphText::prepare(
            "MATCH (n:Person) WHERE n.p=1 AND n.q=42 RETURN n", symbols,
        ).unwrap().bind_parameters(&GqlParameters::new()).unwrap();
        let rows = txn.execute_graph_pattern_governed(&mut db, &query, &read, query_policy()).unwrap();
        assert_eq!(rows.rows.result_rows, 1, "later assignments must observe the MERGE branch update");
...
fn repeated_merges_match_the_earlier_creation_and_allow_zero_quota_reexecution() {
    let ((), report) = run_async_under_lab(0x6e29_2002, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let receipt = txn.execute_graph_write_program_returning_governed(
            &mut db, &query, &program, policy(0, 1), |request| {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let stats = txn.execute_graph_write_program_governed(
...
fn exhausted_creation_quota_restores_existing_transaction_prefix() {
    let ((), report) = run_async_under_lab(0x6e29_2003, |root| async move {
        ...
        let error = txn.execute_graph_write_program_returning_governed(
            &mut db, &query, &program, policy(0, 1), |request| {
                assert_eq!(request.statement, 0, "over-quota step must refuse before allocation");
                allocations += 1;
                Ok::<_, ()>(ElementId::Vertex(VId(100)))
            },
        ).unwrap_err();
        ...
        assert_eq!(allocations, 1, "rollback does not reclaim an externally issued ID");
        assert_eq!(txn.staged_effect_digest().unwrap(), before);
        txn.finish(&mut db, &commit).await.unwrap();
        assert!(db.vertex(VId(200)).unwrap().is_some(), "pre-program staged prefix survives");
        assert!(db.vertex(VId(100)).unwrap().is_none(), "failed program prefix must not become durable");
        assert_eq!(txcx.outstanding_obligations(), 0);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn create_branch_actions_count_without_fabricating_match_rows_and_late_action_failure_rolls_back() {
    let ((), report) = run_async_under_lab(0x6e29_2004, |root| async move {
        ...
        let mut txn = db.begin(&txcx).unwrap();
        let stats = txn.execute_graph_write_program_governed(
            &mut db, &query, &one, policy(1, 1),
            |_| Ok::<_, ()>(ElementId::Vertex(VId(100))),
        ).unwrap();
        assert_eq!(stats.selection.result_rows, 0, "a creation unit is not a MATCH occurrence");
        assert_eq!((stats.created_vertices, stats.mutation_effects, stats.target_vertex_visits), (1, 1, 1));
        txn.abort();
        ...
        let mut allocations = 0;
        let error = txn.execute_graph_write_program_returning_governed(
            &mut db, &query, &program, policy(1, 1), |_| {
                allocations += 1;
