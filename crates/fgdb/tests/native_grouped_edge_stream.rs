...
use fgdb::{Database, DatabaseKeys, MemVfs, PreparedNativeRead, QueryError, QueryResult, QueryValue, WriteBatch};
...
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryPolicy, GraphAggregateError,
    GraphAggregateRow, GraphAggregateTextSlot, RelationBind};
...
fn symbols() -> RelationBind {
    RelationBind::new().with_relation("R", R).with_relation("S", S).with_property("p", P)
}
...
fn seed() -> Vec<WriteBatch> {
    ...
    for id in [0, 1, 2, 99, u128::MAX] {
        first.create_vertex(VId(id), vec![], vec![(P, if id == 1 { CanonicalScalar::Null } else { CanonicalScalar::Int(2) })]);
    }
    for (eid, a, b, w) in [(1, 0, 1, Some(5)), (2, 0, 1, Some(5)), (3, 2, 0, Some(-2)),
        (4, 1, 1, None), (5, u128::MAX, 0, Some(7)), (6, 0, 0, Some(i64::MAX))] {
        first.add_edge(EId(eid), VId(a), VId(b), w.into_iter().map(|w| (P, CanonicalScalar::Int(w))).collect());
    }
    let mut second = WriteBatch::new(S);
    for (eid, a, b, w) in [(10, 1, 2, Some(3)), (11, 1, 1, None), (12, 0, u128::MAX, Some(4))] {
        second.add_edge(EId(eid), VId(a), VId(b), w.into_iter().map(|w| (P, CanonicalScalar::Int(w))).collect());
    }
    vec![first, second]
}
fn definition(shape: usize, direction: usize) -> String {
    ...
    format!("MATCH {pattern} RETURN COUNT(*) AS n,{end} AS destination,SUM(r.p) AS total,{end} AS again,AVG(r.p) AS mean,COUNT(DISTINCT r.p) AS unique_values,MIN(r) AS first_edge GROUP BY {end}")
}
...
fn eager_rows(result: QueryResult) -> (Vec<String>, Vec<Vec<QueryValue>>) {
    let QueryResult::Rows { columns, rows } = result else { panic!("query returned write"); };
    (columns, rows)
}
    ...
    let ((), report) = run_async_under_lab(0xe66e_4001, |root| async move {
        ...
        for shape in 0..3 {
            for direction in 0..3 {
                ...
                let (columns, expected) = eager_rows(prepared.execute(&db, &cx, &args, wide()).unwrap());
                ...
                assert_eq!(cursor.output_slots()[3], GraphAggregateTextSlot::GroupKey(0));
                assert_eq!(cursor.row_stats().snapshot_records, 0);
                let slots = cursor.output_slots().to_vec();
                let first = cursor.next().transpose().unwrap();
                if let Some(first) = &first { assert_eq!(flatten(first, &slots), expected[0]); }
        ...
        edit.add_edge(EId(100), VId(u128::MAX), VId(1), vec![(P, CanonicalScalar::Int(2))]);
        ...
        for (text, slots, expected, first, mut cursor) in cases {
            ...
            let args = GqlParameters::new();
            let mut from_pin = pinned.query_aggregate_stream(&cx, &text, &args, symbols(), wide()).unwrap();
            assert_eq!(from_pin.by_ref().map(|row| flatten(&row.unwrap(), &slots)).collect::<Vec<_>>(), expected);
            let historical = text.replacen(" RETURN ", " FOR SYSTEM_TIME AS OF SEQ $seq RETURN ", 1);
            for seq in 0..=3 {
                let args = GqlParameters::new().with_uint64("seq", seq).unwrap();
                let (names, expected) = eager_rows(db.query(&cx, &historical, &args, symbols(), wide()).unwrap());
                let mut stream = db.query_aggregate_stream(&cx, &historical, &args, symbols(), wide()).unwrap();
                assert_eq!(stream.snapshot_seq(), CommitSeq(seq));
                assert_eq!(stream.columns(), names);
                let layout = stream.output_slots().to_vec();
                assert_eq!(stream.by_ref().map(|row| flatten(&row.unwrap(), &layout)).collect::<Vec<_>>(), expected);
            }
        }
    });
...
fn grouped_native_budgets_late_data_refusals_and_nondraining_close_keep_original_boundaries() {
    let ((), report) = run_async_under_lab(0xe66e_4002, |root| async move {
        ...
        let basis = db.write_atomic(&commit, seed()).await.unwrap();
        let pinned = db.read_session().unwrap();
        let text = definition(0, 0); let args = GqlParameters::new();
        let prepared = PreparedNativeRead::prepare(&text, &args, symbols()).unwrap();
        ...
        assert_eq!(prepared.stream_aggregate(&db, &cx, &args, exact).unwrap().map(Result::unwrap).collect::<Vec<_>>(), expected);
        for p in [
        ...
        for pulled in [false, true] {
            let mut cursor = prepared.stream_aggregate(&db, &cx, &args, wide()).unwrap();
            if pulled { cursor.next().unwrap().unwrap(); }
            let stats = (cursor.row_stats(), cursor.evaluator_stats());
            cursor.close(); cursor.close();
            assert!(cursor.next().is_none());
            assert_eq!((cursor.row_stats(), cursor.evaluator_stats()), stats);
            if !pulled { assert_eq!(stats.0.snapshot_records, 0); assert_eq!(stats.1.work_units, 0); }
        }
        let temporal = text.replacen(" RETURN ", " FOR SYSTEM_TIME AS OF SEQ $seq RETURN ", 1);
        let future = GqlParameters::new().with_uint64("seq", basis.0 + 1).unwrap();
        assert!(matches!(pinned.query_aggregate_stream(&cx, &temporal, &future, symbols(),
            GqlQueryPolicy::new(0, 0, 0, 0)), Err(QueryError::EdgeAggregateStream(GqlQueryError::Source(_)))));
        let mut invalid = WriteBatch::new(R);
        invalid.set_edge_property(EId(6), P, Some(CanonicalScalar::ucs_basic_text("private invalid sum").unwrap()));
        db.write(&commit, invalid).await.unwrap();
        let mut failed = prepared.stream_aggregate(&db, &cx, &args, wide()).unwrap();
        assert!(matches!(failed.next(), Some(Err(GqlQueryError::Source(GraphAggregateError::NonIntegerSum { aggregate: 1 })))));
        assert_eq!(failed.row_stats().result_rows, 0);
        assert!(failed.next().is_none());
        assert!(!format!("{failed:?}").contains("private invalid sum"));
        assert_eq!(prepared.stream_aggregate_in_view(&pinned, &cx, &args, wide()).unwrap().map(Result::unwrap).collect::<Vec<_>>(), expected);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn native_group_count_not_input_occurrences_controls_output_allowance() {
    let ((), report) = run_async_under_lab(0xe66e_4003, |root| async move {
        ...
        for id in 0..4 { batch.create_vertex(VId(id), vec![], vec![]); }
        for id in 0..4096 {
            batch.add_edge(EId(id), VId(0), VId(id % 4), vec![(P, CanonicalScalar::Int(i64::MAX))]);
        }
        ...
        let mut cursor = db.query_aggregate_stream(&cx, q, &GqlParameters::new(), symbols(),
            GqlQueryPolicy::new(4096, 4, 10_000_000, 10_000_000)).unwrap();
        for id in 0..4 {
            let row = cursor.next().unwrap().unwrap();
            assert_eq!(row.keys(), &[fgdb_gql::algebra::GraphValue::Vertex(VId(id))]);
            assert_eq!(row.values()[0].as_count(), Some(1024));
            assert_eq!(row.values()[1].as_integer(), Some(1024 * i128::from(i64::MAX)));
            assert_eq!(row.values()[2], QueryValue::Average(fgdb_gql::GraphExactAverage::new(i128::from(i64::MAX), 1).unwrap()));
