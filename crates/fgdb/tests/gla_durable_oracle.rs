...
fn props_match(row: Option<&VertexRow>, tests: [Option<(PropertyKeyId, i64)>; 6]) -> bool {
    tests.into_iter().enumerate().all(|(comparison, test)| {
        let Some((key, expected)) = test else {
            return true;
        };
        ...
        row.props.iter().any(|(found_key, value)| {
            let CanonicalScalar::Int(actual) = value else {
                return false;
            };
            *found_key == key
                && match comparison {
...
fn oracle(plan: &BoundPlan, vertices: &[VertexRow], edges: &[EdgeRecord]) -> Vec<VId> {
    ...
    let labeled = |vid, label: Option<LabelId>| {
        label.is_none_or(|l| rows.get(&vid).is_some_and(|row| row.labels.contains(&l)))
    };
    let mut bag = Vec::new();
    if let Some(relation) = plan.relation {
        ...
        bag.extend(
            vertices
                .iter()
                .filter(|row| labeled(row.vid, plan.src_label) && props_match(Some(row), source))
                .map(|row| row.vid),
        );
    }
    bag.sort_unstable();
            ...
            for projection in [
                ...
                for filter in 0..24 {
                    ...
                    match filter {
                        ...
                        22 => {
                            p.src_prop_ge = Some((P, i64::MIN));
                            p.hop2_dst_prop_le = Some((P, i64::MAX));
                        }
                        _ => {}
                    }
                    plans.push(p);
...
fn production_gla_answers_match_independent_enumeration_before_and_after_compaction() {
    let ((), report) = run_async_under_lab(0x61a2_0001, |root| async move {
        ...
        for plan in &plans {
            ...
            assert_eq!(
                db.execute_prepared_gql(plan).expect("production GLA"),
                expected,
                "{plan:?}"
            );
            assert_eq!(
                pinned.execute_prepared_gql(plan).expect("pinned GLA"),
                expected
            );
        }
        ...
        for plan in &plans {
            ...
            assert_eq!(
                db.execute_prepared_gql(plan).expect("compacted GLA"),
                expected,
                "{plan:?}"
            );
            assert_eq!(
                db.execute_prepared_gql_at(plan, basis)
                    .expect("historical GLA"),
                historical
            );
            assert_eq!(
                pinned
                    .execute_prepared_gql(plan)
                    .expect("old immutable view"),
                historical
            );
        }
    });
