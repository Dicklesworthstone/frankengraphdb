...
fn policy() -> GqlQueryPolicy {
    GqlQueryPolicy::new(100_000, 100_000, 10_000_000, 5_000_000)
}
fn query(text: &str) -> PreparedGraphAggregate {
    PreparedGraphAggregateText::prepare(text, |kind, name| {
        Some(match kind {
            GraphSymbolKind::Relation => GraphSymbol::Relation(RelationId(match name {
                "R" => 1,
                "S" => 2,
                _ => 3,
            })),
...
fn definitions() -> Vec<(PreparedGraphAggregate, PreparedGraphAggregate)> {
    ...
    for body in [
        ...
        for (key, page) in [("a.p", ""), ("a", " LIMIT 2")] {
            ...
            cases.push((
                query(&format!("{prefix} GROUP BY {key}{page}")),
                query(&format!(
...
fn property_cycles_match_ordinary_bags_across_history_staging_and_recovery() {
    let ((), report) = run_async_under_lab(0x1a74_0001, |root| async move {
        ...
        for (fast, reference) in &cases {
            let want = db
                .execute_graph_aggregate_governed(&cx, reference, policy())
                .unwrap()
                .value;
            assert_eq!(
            ...
            assert_eq!(
                pinned
                    .execute_graph_aggregate_governed(&cx, fast, policy())
                    .unwrap()
                    .value,
                want
            );
        ...
        for ((fast, reference), before) in cases.iter().zip(&old) {
            let want = txn
                .execute_graph_aggregate_governed(&db, &cx, reference, policy())
                .unwrap()
                .value;
            assert_eq!(
                txn.execute_graph_aggregate_governed(&db, &cx, fast, policy())
                    .unwrap()
                    .value,
                want
            );
        ...
        for (((fast, reference), before), after) in cases.iter().zip(&old).zip(&staged) {
            ...
            assert_eq!(
                &db.execute_graph_aggregate_governed(&cx, reference, policy())
                    .unwrap()
                    .value,
                after
            );
            assert_eq!(
                &db.execute_graph_aggregate_governed_at(&cx, fast, basis, policy())
                    .unwrap()
                    .value,
                before
            );
    ...
    let ((), report) = run_async_under_lab(0x1a74_0002, |root| async move {
        ...
        for outcome in 0..3 {
            for change in 0..3 {
                ...
                let basis = seed(&mut db, &commit).await;
                let q = query(
                    "MATCH (a)-[:R]->(b)-[:S]->(c)-[:T]->(a) WHERE a.p=c.p RETURN a,COUNT(c.p) AS n GROUP BY a",
                );
                let baseline = db
                    .execute_graph_aggregate_governed(&cx, &q, policy())
                    .unwrap();
                ...
                assert_eq!(
                    db.execute_graph_aggregate_governed(&cx, &q, exact).unwrap(),
                    baseline
                );
                for short in [
                    GqlQueryPolicy::new(
                ...
                let q = if outcome == 1 {
                    query(
                        "MATCH (a)-[:R]->(b)-[:S]->(c)-[:T]->(a) WHERE a.p<>a.p RETURN a,COUNT(*) AS n GROUP BY a",
                    )
                } else {
                    q
                };
                let limits = if outcome == 2 {
                    GqlQueryPolicy::new(1000, 0, 10_000_000, 5_000_000)
                } else {
                    policy()
                };
                ...
                match change {
                    0 => {
                        winner.set_vertex_property(VId(2), P, Some(CanonicalScalar::Int(9)));
                    }
                    1 => {
    ...
    let ((), report) = run_async_under_lab(0x1a74_0003, |root| async move {
        ...
        for id in 0..=2 * n {
            ...
            vertices.create_vertex(
                VId(id),
                vec![],
                vec![(P, CanonicalScalar::Int(value as i64))],
            );
        }
        ...
        for page in ["", " LIMIT 2"] {
            let q = query(&format!(
                "MATCH (a)-[:R]->(b)-[:S]->(c)-[:T]->(a) WHERE a.p=c.p RETURN a,COUNT(c.p) AS n,MIN(c.p) AS lo GROUP BY a{page}"
            ));
            let result = db
            ...
            assert_eq!(
                result.value.len(),
                if page.is_empty() { n as usize } else { 2 }
            );
            assert!(
                result
                    .value
