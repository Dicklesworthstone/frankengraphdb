...
use fgdb_gql::{
    GqlQueryPolicy, GraphAggregate, GraphAggregateRow, GraphIntegerExpression, GraphIntegerOp,
...
fn definition(shape: Shape, direction: GlaDirection) -> PreparedGraphAggregate {
    ...
    let input = if scoped {
        ...
        root.filter_boolean(&predicate(output, shape == Shape::Vertex))
            .unwrap();
    ...
    let ((), report) = run_async_under_lab(0x6a41, |root| async move {
        ...
        for shape in [
            ...
            for direction in [
                ...
                let handle = db
                    .register_standing_query(&cx, definition.clone(), policy())
                    .unwrap();
        ...
        let handle = db
            .register_standing_query(&cx, late.clone(), policy())
            .unwrap();
...
fn global_count(filter: &GraphBooleanExpression) -> PreparedGraphAggregate {
    ...
    let input = builder
        .prepare_values(&[GraphColumn::vertex("id", "a")], 0, None)
        .unwrap()
        .with_duplicates();
...
fn not_keeps_unknown_and_nested_text_programs_track_hidden_properties() {
    let ((), report) = run_async_under_lab(0x6a42, |root| async move {
        ...
        let prefix = ScalarPredicate::new(
            CanonicalScalar::ucs_basic_text("AL").unwrap(),
        ...
        let scalar = GraphIntegerExpression::prepare_scalar(&[
            ...
            GraphIntegerOp::Scalar(prefix),
            GraphIntegerOp::StartsWith,
        ])
        .unwrap();
        let columns = [Arg::Property {
            variable: "a",
        ...
        let text = global_count(
            &GraphBooleanExpression::prepare(&[Op::Expression {
                expression: &scalar,
                columns: &columns,
            }])
            .unwrap(),
        ...
        let not_handle = db
            .register_standing_query(&cx, not.clone(), policy())
            .unwrap();
        let text_handle = db
            .register_standing_query(&cx, text.clone(), policy())
            .unwrap();
        let mut seed = WriteBatch::new(R);
        for (at, value) in [
            None,
        ...
        update.set_vertex_property(
            VId(2),
            NAME,
            Some(CanonicalScalar::ucs_basic_text("albert").unwrap()),
        );
        ...
        update.set_vertex_property(
            VId(1),
            NAME,
            Some(CanonicalScalar::ucs_basic_text("Élie").unwrap()),
        );
        ...
        assert_eq!(
            db.standing_query(&cx, &not_handle).unwrap().rows(),
            &oracle(&db, &not)
        );
        assert_eq!(
            db.standing_query(&cx, &text_handle).unwrap().rows(),
            &oracle(&db, &text)
        );
    });
    assert!(report.lab_test_passed(), "{report:?}");
...
fn boolean_witness_replacement_preserves_optional_semi_and_anti_boundaries() {
    let ((), report) = run_async_under_lab(0x6a43, |root| async move {
        ...
        for id in 1..=3 {
            seed.create_vertex(
                ...
                vec![
                    ...
                    (FLAG, CanonicalScalar::Bool(id == 2)),
                    (GATE, CanonicalScalar::Bool(true)),
                ],
            );
        }
        ...
        let (row, weight) = optional
            .rows()
            .iter()
            .find(|(row, _)| row.keys() == [GraphValue::Vertex(VId(1))])
            .unwrap();
        assert_eq!(weight, &ZWeight::ONE);
    ...
    let ((), report) = run_async_under_lab(0x6a44, |root| async move {
        ...
        for id in 1..=2 {
            seed.create_vertex(
                ...
                vec![],
                vec![
                    (AMOUNT, CanonicalScalar::Int(id as i64)),
                    (P, CanonicalScalar::Int(7)),
                    (FLAG, CanonicalScalar::Bool(false)),
                    (GATE, CanonicalScalar::Bool(true)),
                ],
            );
        }
        ...
        let handle = db
            .register_standing_query(&cx, definition.clone(), bounded)
            .unwrap();
        ...
        assert!(matches!(db.standing_query(&cx, &handle),
            Err(StandingQueryError::Unavailable { frontier, reason: StandingQueryFailure::ResultBudget })
                if frontier == basis));
        assert!(matches!(
            db.rebuild_standing_query(&cx, &handle, bounded),
        ...
        assert_eq!(
            db.standing_query(&cx, &handle).unwrap().rows(),
            &oracle(&db, &definition)
        );
