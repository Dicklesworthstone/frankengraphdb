...
use fgdb_gql::algebra::{
    GlaDirection, GraphBindingRow, GraphColumn, GraphPatternBuilder, GraphValue, GraphValueRow,
...
fn builder() -> GraphPatternBuilder {
    ...
    b.edge("source", R, GlaDirection::Forward, "middle")
        .unwrap();
    b.edge("middle", S, GlaDirection::Forward, "destination")
        .unwrap();
    b
}
...
fn pattern(offset: u64, count: Option<u64>) -> PreparedGraphPattern<GraphBindingRow> {
    builder()
        .prepare_bindings(&["source", "destination"], offset, count)
        .unwrap()
        .with_duplicates()
}
...
fn all_five_read_surfaces_keep_correlated_occurrences_and_ordinary_distinct_is_unchanged() {
    let ((), report) = run_async_under_lab(0xbab1_0001, |root| async move {
        ...
        for value in [
            db.execute_graph_pattern_governed(&query_cx, &all, policy())
                .unwrap()
                .value,
            db.execute_graph_pattern_governed_at(&query_cx, &all, at, policy())
                .unwrap()
                .value,
            pinned
                .execute_graph_pattern_governed(&query_cx, &all, policy())
                .unwrap()
                .value,
            pinned
                .execute_graph_pattern_governed_at(&query_cx, &all, at, policy())
                .unwrap()
                .value,
            txn.execute_graph_pattern_governed(&db, &query_cx, &all, policy())
                .unwrap()
                .value,
        ] {
        ...
        let distinct = builder()
            .prepare_bindings(&["source", "destination"], 0, None)
            .unwrap();
        ...
        assert_eq!(
            plain(
                db.execute_graph_pattern_governed(&query_cx, &distinct, policy())
                    .unwrap()
                    .value
            ),
            unique
        ...
        assert_eq!(
            plain(
                db.execute_graph_pattern_governed(&query_cx, &paged, policy())
                    .unwrap()
                    .value
            ),
...
fn canonical_staging_changes_multiplicity_without_inventing_ensure_aliases_or_rebasing_history() {
    let ((), report) = run_async_under_lab(0xbab1_0002, |root| async move {
        ...
        let run = txn
            .execute_graph_pattern_governed(&db, &query_cx, &all, policy())
            .unwrap();
        ...
        let exact = GqlQueryPolicy::new(
            ...
            run.evaluator.scratch_entries,
        );
        assert_eq!(
            txn.execute_graph_pattern_governed(&db, &query_cx, &all, exact)
                .unwrap(),
            run
        );
        assert_eq!(
        ...
        assert_eq!(
            plain(
                db.execute_graph_pattern_governed_at(&query_cx, &all, basis, policy())
                    .unwrap()
                    .value
            ),
            before
    ...
    let ((), report) = run_async_under_lab(0xbab1_0003, |root| async move {
        ...
        for conflict in [false, true] {
            ...
            if conflict {
                ...
                result.expect(
                    "unrelated vertex insertion does not conflict with an edge-pattern read",
                );
...
fn historical_bags_survive_compaction_and_reopening() {
    let ((), report) = run_async_under_lab(0xbab1_0004, |root| async move {
        ...
        assert_eq!(
            plain(
                reopened
                    .execute_graph_pattern_governed_at(&query_cx, &all, basis, policy())
                    .unwrap()
                    .value
            ),
            old
...
fn property_oracle(vertices: &[fgdb::VertexRow], edges: &[EdgeRecord]) -> Vec<Vec<GraphValue>> {
    ...
    let mut rows: Vec<_> = oracle(edges)
        .into_iter()
        .map(|pair| {
            vec![
                GraphValue::Vertex(pair[0]),
                value(pair[0], PropertyKeyId(1)),
                value(pair[1], PropertyKeyId(2)),
            ]
        })
        .collect();
    rows.sort();
...
fn property_bags_keep_correlation_nulls_and_owned_payloads_across_staging_and_history() {
    let ((), report) = run_async_under_lab(0xbab1_0005, |root| async move {
        ...
        values.set_vertex_property(
            VId(1),
            PropertyKeyId(1),
            Some(CanonicalScalar::ucs_basic_text("secret alpha").unwrap()),
        );
        values.set_vertex_property(
            VId(3),
            PropertyKeyId(2),
            Some(CanonicalScalar::bytes(vec![5; 129]).unwrap()),
        );
        ...
        for run in [
            db.execute_graph_pattern_governed(&query_cx, &pattern, policy())
                .unwrap(),
            db.execute_graph_pattern_governed_at(&query_cx, &pattern, basis, policy())
                .unwrap(),
            pinned
                .execute_graph_pattern_governed(&query_cx, &pattern, policy())
                .unwrap(),
            pinned
                .execute_graph_pattern_governed_at(&query_cx, &pattern, basis, policy())
                .unwrap(),
        ...
        stage.set_vertex_property(
            VId(4),
            PropertyKeyId(1),
            Some(CanonicalScalar::ucs_basic_text("beta").unwrap()),
        );
        txn.write(&mut db, stage).unwrap();
        ...
        assert_ne!(expected, before);
        let staged = txn
            .execute_graph_pattern_governed(&db, &query_cx, &pattern, policy())
            .unwrap();
...
fn property_bag_limits_and_cancellation_preserve_the_original_error_classes() {
    let ((), report) = run_async_under_lab(0xbab1_0006, |root| async move {
        // Cancel the query's child task, not the lab supervisor. The child
        // must acknowledge cancellation, finish cleanup and join successfully.
        let mut handle = root
            .spawn(|root| async move {
                ...
                values.set_vertex_property(
                    VId(3),
                    PropertyKeyId(2),
                    Some(CanonicalScalar::bytes(vec![8; 129]).unwrap()),
                );
                ...
                for (work, scratch, dimension) in [
                    ...
                    assert!(
                        matches!(db.execute_graph_pattern_governed(&query_cx, &pattern,
            ...
            if error.dimension == GqlBudgetDimension::ResultRows && error.observed == 3)
                ...
                assert!(matches!(
                    txn.execute_graph_pattern_governed(&foreign, &query_cx, &pattern, policy()),
