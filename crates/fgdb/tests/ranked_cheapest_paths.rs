        ...
        for (end, cost, path) in layer {
            for edge in edges.iter().filter(|edge| edge.entry.relation == R) {
                ...
                for destination in destinations {
                    ...
                    child.push((entry.eid, destination));
...
fn ranked_paths_share_live_historical_pinned_and_canonical_transaction_sources() {
    let ((), report) = run_async_under_lab(0xc0a5_1001, |root| async move {
        ...
        for direction in [
            ...
            for result in [
                db.execute_graph_cheapest_paths_governed(&cx, &query, 7, policy())
                    .unwrap(),
                db.execute_graph_cheapest_paths_governed_at(&cx, &query, 7, basis, policy())
                    .unwrap(),
                pinned
                    .execute_graph_cheapest_paths_governed(&cx, &query, 7, policy())
                    .unwrap(),
                pinned
                    .execute_graph_cheapest_paths_governed_at(&cx, &query, 7, basis, policy())
                    .unwrap(),
                txn.execute_graph_cheapest_paths_governed(&db, &cx, &query, 7, policy())
                    .unwrap(),
            ] {
...
fn ranked_prefixes_share_original_source_and_evaluator_limits() {
    let ((), report) = run_async_under_lab(0xc0a5_1002, |root| async move {
        ...
        let baseline = db
            .execute_graph_cheapest_paths_governed(&cx, &query, 3, policy())
            .unwrap();
        ...
        assert_eq!(
            db.execute_graph_cheapest_paths_governed(&cx, &query, 3, exact)
                .unwrap(),
            baseline
        );
        for bad in [
            ...
            assert!(
                db.execute_graph_cheapest_paths_governed(&cx, &query, 3, bad)
                    .is_err()
            );
        }
        assert_eq!(
            db.execute_graph_cheapest_paths_governed(
                &cx,
                &query,
                0,
        ...
        assert!(matches!(
            db.execute_graph_cheapest_paths_governed(&cx, &query, 0, policy()),
    ...
    let ((), report) = run_async_under_lab(0xc0a5_1004, |root| async move {
        ...
        let result = db
            .execute_graph_cheapest_paths_governed(
                ...
                &query,
                4,
                GqlQueryPolicy::new(121, 4, 100_000, 50_000),
            )
            .unwrap();
        ...
        for (ordinal, row) in result.value.iter().enumerate() {
            ...
            assert_eq!(
                row.path().edges().collect::<Vec<_>>(),
                (0..40_u128)
                    .map(|depth| EId(depth * 2 + (((ordinal as u128) >> (39 - depth)) & 1)))
                    .collect::<Vec<_>>()
            );
        }
    });
