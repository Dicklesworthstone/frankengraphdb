...
#[test]
fn source_property_bang_inequality_sees_the_staged_overlay_without_dirty_reads() {
    let ((), report) = run_async_under_lab(0x58_01, |root| async move {
        let contexts = PurposeContexts::narrow_runtime_root(&root);
        let commit = contexts.commit();
        let txn_cx = contexts.txn();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-where-prop-bang-ne-overlay-{}",
            std::process::id()
        ));
        ...
        seed.add_edge(EId(11), VId(5), VId(6), vec![]);
        ...
        staged.add_edge(EId(12), VId(3), VId(4), vec![]);
        ...
        let overlay = txn
            .execute_gql(&db, unfiltered, &bind)
