...
fn statement(direction: usize, hops: usize, grouped: bool) -> String {
    text(direction, hops, grouped) + " HAVING COUNT(*)>0 AND (average>$floor OR total IS NULL) ORDER BY average DESC NULLS FIRST,occurrences DESC SKIP $skip LIMIT $limit"
}
#[allow(clippy::too_many_arguments)]
fn expected(cut: u64, scale: i64, direction: usize, hops: usize, grouped: bool,
    ...
    rows.sort_by(|a, b| {
        ...
        order.then_with(|| b[count].as_count().cmp(&a[count].as_count()))
            .then_with(|| if grouped { a[1].cmp(&b[1]) } else { Ordering::Equal })
    });
...
fn ranked_native_pages_keep_bound_layouts_and_snapshot_history_after_reopen() {
    let ((), report) = run_async_under_lab(0xc0a5_5201, |root| async move {
        ...
        for direction in 0..3 { for hops in 1..=2 { for grouped in [false, true] {
            ...
            let template = PreparedNativeRead::prepare(&statement(direction, hops, grouped), &args, symbols()).unwrap();
            ...
            let first = cursor.next().transpose().unwrap().map(|row| slots.iter().map(|slot| match *slot {
        ...
        db.write(&commit, cascade).await.unwrap(); db.compact(&commit).await.unwrap(); drop(db);
        ...
        for (direction, hops, grouped, first, mut cursor) in paused {
            ...
            let template = PreparedNativeRead::prepare(&statement(direction, hops, grouped),
                &output_arguments(1, 1, 0, 0, 1), symbols()).unwrap();
            for cut in 0..=3 { for scale in [1, 2] { for floor in [-1, 10] {
                for (skip, limit) in [(0, 0), (0, 1), (1, 2)] {
                    ...
                    assert_eq!(rows, expected(cut, scale, direction, hops, grouped, floor, skip as usize, limit as usize));
            ...
            assert_eq!(drain(&mut pinned), expected(1, 2, direction, hops, grouped, -99, 0, 2));
        }
    });
...
fn native_hidden_ranks_obey_selected_row_quotas_and_cannot_hide_late_bad_input() {
    let ((), report) = run_async_under_lab(0xc0a5_5202, |root| async move {
        ...
        for id in 0..1024 { batch.add_edge(EId(id), VId(0), VId(id),
            vec![(Q, CanonicalScalar::Int(id as i64)), (P, CanonicalScalar::Int(3))]); }
        ...
        let args = GqlParameters::new(); let template = PreparedNativeRead::prepare(text, &args, symbols()).unwrap();
        let mut full = template.stream_aggregate(&db, &cx, &args, GqlQueryPolicy::new(1024,1,u64::MAX,u64::MAX)).unwrap();
        let rows = drain(&mut full);
        assert_eq!(rows, vec![vec![GraphAggregateValue::Value(GraphValue::Vertex(VId(1022))),
            GraphAggregateValue::Count(1), GraphAggregateValue::Integer(3067)]]);
        ...
        assert_eq!(drain(&mut template.stream_aggregate(&db,&cx,&args,exact).unwrap()), rows);
        for policy in [GqlQueryPolicy::new(r.snapshot_records-1,1,u64::MAX,u64::MAX),
            ...
            assert_eq!(stream.state(),VertexScanState::Failed); assert!(stream.next().is_none());
        }
        let mut edit = WriteBatch::new(R);
        edit.set_edge_property(EId(1023), Q, Some(CanonicalScalar::Bool(true)));
        db.write(&commit,edit).await.unwrap();
        for count in [0,1] {
            let text = format!("MATCH (a)-[r:R]->(b) RETURN b,COUNT(*) AS n GROUP BY b ORDER BY AVG(r.quantity*r.price) DESC LIMIT {count}");
            let mut stream = db.query_aggregate_stream(&cx,&text,&args,symbols(),wide()).unwrap();
