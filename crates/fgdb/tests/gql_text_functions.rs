...
fn every_text_operator_filters_and_projects_committed_properties() {
    let ((), report) = run_async_under_lab(0x7e87_0001, |root| async move {
        ...
        for (expression, expected, predicate, selected) in [
            ("n.t STARTS WITH 'Al'", vec![Bool(true), Bool(false), Bool(false), Null, Bool(false), Null],
                "n.t STARTS WITH 'Al'", vec![VId(1)]),
            ("n.t ENDS WITH 'β'", vec![Bool(true), Bool(true), Bool(false), Null, Bool(false), Null],
                "n.t ENDS WITH 'β'", vec![VId(1), VId(2)]),
            ("n.t CONTAINS 'Alpha'", vec![Bool(true), Bool(false), Bool(false), Null, Bool(false), Null],
                "n.t CONTAINS 'Alpha'", vec![VId(1)]),
            ("n.t STARTS WITH n.u", vec![Bool(true), Bool(false), Bool(true), Null, Null, Null],
                "n.t STARTS WITH n.u", vec![VId(1), VId(3)]),
            ("n.t ENDS WITH n.u", vec![Bool(false), Bool(true), Bool(true), Null, Null, Null],
                "n.t ENDS WITH n.u", vec![VId(2), VId(3)]),
            ("n.t CONTAINS n.u", vec![Bool(true), Bool(true), Bool(true), Null, Null, Null],
                "n.t CONTAINS n.u", vec![VId(1), VId(2), VId(3)]),
            ("UPPER(n.t)", vec![text("ALPHAΒ"), text("ALPHAΒ"), text(""), Null, text("X"), Null],
                "UPPER(n.t) = 'ALPHAΒ'", vec![VId(1), VId(2)]),
            ("LOWER(n.t)", vec![text("alphaβ"), text("alphaβ"), text(""), Null, text("x"), Null],
                "LOWER(n.t) = 'alphaβ'", vec![VId(1), VId(2)]),
            ("TRIM(n.t)", vec![text("Alphaβ"), text("alphaβ"), text(""), Null, text("x"), Null],
                "TRIM(n.t) = 'Alphaβ'", vec![VId(1)]),
            ("SUBSTRING(n.t,2,3)", vec![text("lph"), text("lph"), text(""), Null, text(""), Null],
                "SUBSTRING(n.t,2,3) = 'lph'", vec![VId(1), VId(2)]),
            ("CHAR_LENGTH(n.t)", vec![Int(6), Int(6), Int(0), Null, Int(1), Null],
                "CHAR_LENGTH(n.t) = 6", vec![VId(1), VId(2)]),
            ("n.t || n.u", vec![text("AlphaβAlpha"), text("alphaββ"), text(""), Null, Null, Null],
                "n.t || n.u = 'alphaββ'", vec![VId(2)]),
            ("n.t IN ['Alphaβ','x']", vec![Bool(true), Bool(false), Bool(false), Null, Bool(true), Null],
                "n.t IN ['Alphaβ','x']", vec![VId(1), VId(5)]),
        ] {
            let projection = query(&format!("MATCH (n) RETURN n,{expression} AS value ORDER BY n"));
            let result = db.execute_graph_set_governed(&cx, &projection, policy()).unwrap();
            assert_eq!(ids(&result.value), (1..=6).map(VId).collect::<Vec<_>>());
            assert_eq!(cells(&result.value), expected.into_iter().map(|value| vec![value]).collect::<Vec<_>>(), "{expression}");
            let filtered = query(&format!("MATCH (n) WHERE {predicate} RETURN n ORDER BY n"));
            assert_eq!(ids(&db.execute_graph_set_governed(&cx, &filtered, policy()).unwrap().value), selected, "{predicate}");
        }
    });
...
fn null_on_either_side_stays_unknown_in_return_and_is_not_true_in_where() {
    let ((), report) = run_async_under_lab(0x7e87_0002, |root| async move {
        ...
        seed(&mut db, &commit).await;
        for expression in [
            "NULL STARTS WITH n.t", "n.t STARTS WITH NULL",
            "NULL ENDS WITH n.t", "n.t ENDS WITH NULL",
            "NULL CONTAINS n.t", "n.t CONTAINS NULL",
            "NULL || n.t", "n.t || NULL", "UPPER(NULL)", "LOWER(NULL)", "TRIM(NULL)",
            "CHAR_LENGTH(NULL)", "SUBSTRING(NULL,1,1)",
            "SUBSTRING(n.t,NULL,1)", "SUBSTRING(n.t,1,NULL)",
            "NULL IN ['Alphaβ','x']", "n.t IN [NULL]",
        ] {
            let projection = query(&format!("MATCH (n) RETURN n,{expression} AS value ORDER BY n"));
        ...
        let membership = query("MATCH (n) RETURN n,n.t IN ['Alphaβ',NULL] AS member,n.t NOT IN ['Alphaβ',NULL] AS nonmember ORDER BY n");
        assert_eq!(cells(&db.execute_graph_set_governed(&cx, &membership, policy()).unwrap().value), vec![
...
fn unicode_scalar_offsets_case_expansion_and_unicode_whitespace_are_observable() {
    let ((), report) = run_async_under_lab(0x7e87_0003, |root| async move {
        ...
        let scalar = query("MATCH (n) RETURN n,CHAR_LENGTH(n.t) AS size,SUBSTRING(n.t,2,3) AS middle,SUBSTRING(n.t,20,1) AS beyond,SUBSTRING(n.t,1,0) AS empty ORDER BY n");
        assert_eq!(cells(&db.execute_graph_set_governed(&cx, &scalar, policy()).unwrap().value), vec![
        ...
        let casing = query("MATCH (n) RETURN n,UPPER(TRIM(n.t)) AS upper,LOWER(TRIM(n.t)) AS lower ORDER BY n");
        assert_eq!(cells(&db.execute_graph_set_governed(&cx, &casing, policy()).unwrap().value), vec![
            vec![text("AÉ🦀E\u{301}Z"), text("aé🦀e\u{301}z")],
            vec![text("STRASSE"), text("straße")],
            vec![text("IİSS"), text("ii\u{307}ß")],
        ]);
        let filtered = query("MATCH (n) WHERE TRIM(n.t) = 'Straße' AND CHAR_LENGTH(TRIM(n.t)) = 6 RETURN n");
        assert_eq!(ids(&db.execute_graph_set_governed(&cx, &filtered, policy()).unwrap().value), vec![VId(2)]);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn temporal_text_predicates_and_projections_use_the_selected_committed_version() {
    let ((), report) = run_async_under_lab(0x7e87_0004, |root| async move {
        ...
        for (seq, expected_ids, expected_cells) in [
            (first, vec![VId(1)], vec![vec![text("ALPHAΒ:lp"), CanonicalScalar::Int(6)]]),
            (second, vec![], vec![]),
        ] {
            let bound = template.bind_parameters(&GqlParameters::new().with_uint64("at", seq.0).unwrap()).unwrap();
            let result = db.execute_temporal_graph_set_text_governed(&cx, &bound, policy()).unwrap();
...
fn query_selected_text_assignments_are_frozen_staged_and_durably_committed() {
    let ((), report) = run_async_under_lab(0x7e87_0005, |root| async move {
        ...
        let before = db.execute_graph_set_governed(&cx, &read, policy()).unwrap().value;
        ...
        let result = txn.execute_graph_mutation_governed(&mut db, &cx, &mutation, GraphMutationPolicy::new(policy(), 100)).unwrap();
        ...
        assert_eq!(cells(&txn.execute_graph_set_governed(&db, &cx, &read, policy()).unwrap().value), expected);
        assert_eq!(db.execute_graph_set_governed(&cx, &read, policy()).unwrap().value, before);
        txn.commit(&mut db, &commit).await.unwrap();
        assert_eq!(cells(&db.execute_graph_set_governed(&cx, &read, policy()).unwrap().value), expected);
        assert_eq!(db.execute_graph_set_governed_at(&cx, &read, basis, policy()).unwrap().value, before);
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
...
fn governed_rows_and_string_growth_alone_refuse_at_their_typed_limits() {
    let ((), report) = run_async_under_lab(0x7e87_0006, |root| async move {
        ...
        let long = query(&format!("MATCH (n) RETURN n,CHAR_LENGTH(n.t || '{}') AS value", "x".repeat(2048)));
        let small = db.execute_graph_set_governed(&cx, &short, policy()).unwrap();
        assert_eq!(cells(&small.value), vec![vec![CanonicalScalar::Int(2)]]);
        assert!(matches!(db.execute_graph_set_governed(&cx, &short,
            GqlQueryPolicy::new(10_000, 0, 10_000_000, 10_000_000)), Err(GqlQueryError::Rows(_))));
        ...
        for (dimension, allowance) in [
            (GlaLimitDimension::WorkUnits, GqlQueryPolicy::new(10_000, 10_000, small.evaluator.work_units, 10_000_000)),
            (GlaLimitDimension::ScratchEntries, GqlQueryPolicy::new(10_000, 10_000, 10_000_000, small.evaluator.scratch_entries)),
        ] {
            assert_eq!(db.execute_graph_set_governed(&cx, &short, allowance).unwrap(), small);
            let error = db.execute_graph_set_governed(&cx, &long, allowance).unwrap_err();
            assert!(matches!(error, GqlQueryError::Evaluator(limit) if limit.dimension == dimension), "{dimension:?}");
        }
    });
    assert!(report.lab_test_passed(), "{report:?}");
}
