...
fn native_shortest_match_feeds_atomic_creation_with_tie_multiplicity_and_rollback() {
    let ((), report) =
        run_async_under_lab(0x5a07_1004, |root| async move {
            let contexts = PurposeContexts::narrow_runtime_root(&root);
            let commit = contexts.commit();
            let cx = contexts.query();
            let txcx = contexts.txn();
            ...
            assert_eq!(
                allocations, 0,
                "three shortest occurrences exceed the cap before any allocation"
            );
                ...
                .execute_graph_write_script_governed(
                    ...
                    |request| {
                        let fgdb_gql::insertion::GraphInsertRequest::Vertex { row, vertex: 0 } =
                            ...
                            panic!("one vertex per selected shortest occurrence")
                        };
