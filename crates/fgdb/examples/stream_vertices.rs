    ...
    runtime.block_on(async {
        ...
        for (id, value) in [(1, "Ada"), (2, "Grace"), (3, "Katherine")] {
            seed.create_vertex(
                VId(id),
                vec![],
                vec![(name, CanonicalScalar::ucs_basic_text(value).unwrap())],
            );
        }
        ...
        let mut stream = database
            .stream_graph_values_governed(&query, &prepared, policy)
            .unwrap();
        assert_eq!(stream.row_stats().snapshot_records, 0);
        let first = stream.next().expect("first row").unwrap();
        assert_eq!(
            first.get(0).and_then(|value| value.as_vertex()),
            Some(VId(1))
        );
        ...
        edit.set_vertex_property(
            VId(2),
            name,
            Some(CanonicalScalar::ucs_basic_text("updated").unwrap()),
        );
        database
            .write(&commit, edit)
            .await
            .expect("write during open stream");
        assert!(database.frontier().unwrap() > pinned);
        let second = stream.next().expect("second row").unwrap();
        assert_eq!(
            second.get(1).and_then(|value| value.as_scalar()),
        ...
        assert_eq!(stream.row_stats().snapshot_records, 2);
        println!(
            "streamed {} complete rows from {:?}; closed before the final candidate",
            stream.row_stats().result_rows,
            pinned
        );
    });
}
