...
use fgdb_prism::{
    ...
    ParallelEdgePolicy, ProjectionBuildError, ProjectionEdge, ProjectionLimits, ProjectionSpec,
...
fn owned_projection_executes_registered_call_with_identical_results_and_certificate() {
    ...
    let borrowed =
        SnapshotGraphView::build(binding(), &vertices, &edges, spec(), projection_limits())
            .unwrap();
    ...
    let baseline = call
        .execute(&borrowed, execution_limits(), || Ok::<(), &'static str>(()))
        .unwrap();
    let actual = call
        .execute(&owned, execution_limits(), || Ok::<(), &'static str>(()))
        .unwrap();
...
fn cancellation_propagates_across_the_complete_projection_and_call_pipeline() {
    ...
    .unwrap();
    call.execute(&graph, execution_limits(), &mut control)
        .unwrap();
    let baseline = call
        .execute(&graph, execution_limits(), || Ok::<(), &'static str>(()))
        .unwrap();
    ...
    for stop in 1..=all_checks {
        ...
        match SnapshotGraphView::build_owned_with_checkpoint(
            binding(),
            vertices,
            edges,
            spec(),
            ...
            Err(ProjectionBuildError::Cancelled("stop")) => {
                projection_cuts += 1;
            }
            Ok(projection) => {
                assert!(matches!(
                    call.execute(&projection, execution_limits(), &mut control),
    ...
    assert_eq!(
        call.execute(&graph, execution_limits(), || Ok::<(), &'static str>(()))
            .unwrap(),
        baseline
    );
}
