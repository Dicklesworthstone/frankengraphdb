...
const PAGERANK_PARAMETERS: &[FnxParameterSpec] = &[
    FnxParameterSpec { name: "alpha", value_type: FnxParameterType::FiniteFloat, default: Some(FnxArgument::Float(0.85)) },
    FnxParameterSpec { name: "max_iter", value_type: FnxParameterType::PositiveInteger, default: Some(FnxArgument::Integer(100)) },
    FnxParameterSpec { name: "tol", value_type: FnxParameterType::FiniteFloat, default: Some(FnxArgument::Float(1e-6)) },
    FnxParameterSpec { name: "weighted", value_type: FnxParameterType::Boolean, default: Some(FnxArgument::Boolean(true)) },
];
...
const SIGNATURES: &[FnxSignature] = &[
    FnxSignature {
        name: "fnx.pagerank", graph_input_arity: 1, parameters: PAGERANK_PARAMETERS,
        ...
        numeric_profile: FNX_NUMERIC_PROFILE, rng_policy: "none",
        graph_laws: "explicit projection; finite nonnegative weights when weighted; isolates retained",
        complexity: "O(k * (|V| + |E|))", execution_kernel: "fgdb-prism/pagerank-row-cursor-v1",
    },
    FnxSignature {
        name: "fnx.single_source_shortest_path_length", graph_input_arity: 1, parameters: BFS_PARAMETERS,
        ...
        complexity: "O(|V| + |E|)", execution_kernel: "fgdb-prism/bfs-row-cursor-v1",
    },
    FnxSignature {
        name: "fnx.connected_components", graph_input_arity: 1, parameters: &[],
        ...
        complexity: "O(|V| + |E|)", execution_kernel: "fgdb-prism/components-row-cursor-v1",
    },
    FnxSignature {
        name: "fnx.weakly_connected_components", graph_input_arity: 1, parameters: &[],
        outputs: &[FnxOutput::Vertex, FnxOutput::Component],
        implementation: FnxImplementationClass::InCoreDecodedCache, graph_kind: FnxGraphKind::Directed,
        numeric_profile: FNX_DISCRETE_PROFILE, rng_policy: "none",
        graph_laws: "explicit directed projection; successors and predecessors; unweighted; component label is minimum member VId",
        complexity: "O(|V| + |E|)", execution_kernel: "fgdb-prism/weak-components-row-cursor-v1",
    },
    FnxSignature {
        name: "fnx.strongly_connected_components", graph_input_arity: 1, parameters: &[],
        outputs: &[FnxOutput::Vertex, FnxOutput::Component],
        implementation: FnxImplementationClass::InCoreDecodedCache, graph_kind: FnxGraphKind::Directed,
        numeric_profile: FNX_DISCRETE_PROFILE, rng_policy: "none",
...
impl core::fmt::Display for FnxBindError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "Prism CALL bind error at byte {}: {:?}", self.at, self.kind)
    }
}
...
impl PageRankOptions {
    pub fn new(alpha: f64, max_iter: usize, tol: f64, weighted: bool) -> Result<Self, FnxBindError> {
        let invalid = |message| FnxBindError { at: 0, kind: FnxBindErrorKind::InvalidArgument(message) };
        if !alpha.is_finite() || !(0.0..1.0).contains(&alpha) {
            return Err(invalid("alpha must be finite and in [0,1)"));
        }
        if max_iter == 0 { return Err(invalid("max_iter must be positive")); }
        if !tol.is_finite() || tol <= 0.0 { return Err(invalid("tol must be finite and positive")); }
        Ok(Self { alpha: if alpha == 0.0 { 0.0 } else { alpha }, max_iter, tol, weighted })
    }
    pub const fn alpha(self) -> f64 { self.alpha }
    pub const fn max_iter(self) -> usize { self.max_iter }
    pub const fn tolerance(self) -> f64 { self.tol }
    pub const fn weighted(self) -> bool { self.weighted }
}
impl Default for PageRankOptions {
    fn default() -> Self { Self { alpha: 0.85, max_iter: 100, tol: 1e-6, weighted: true } }
}
...
impl FnxCallSpec {
    pub fn pagerank(options: PageRankOptions) -> Self { Self::new(FnxAlgorithm::PageRank(options)) }
    ...
    pub fn bind(text: &str, parameters: &FnxParameters) -> Result<Self, FnxBindError> {
        if text.len() > MAX_FNX_CALL_BYTES {
            return Err(FnxBindError { at: MAX_FNX_CALL_BYTES, kind: FnxBindErrorKind::TextTooLong });
        }
        ...
        for index in count..signature.parameters.len() {
            arguments[index] = signature.parameters[index].default.ok_or_else(|| {
                parser.error(FnxBindErrorKind::MissingArgument(signature.parameters[index].name))
            })?;
        }
        let invalid = |index, message| FnxBindError {
            at: positions[index], kind: FnxBindErrorKind::InvalidArgument(message),
        };
        let algorithm = match signature.name {
            "fnx.pagerank" => {
                let alpha = float_argument(arguments[0]).ok_or_else(|| invalid(0, "finite exact alpha required"))?;
                ...
                let tol = float_argument(arguments[2]).ok_or_else(|| invalid(2, "finite exact tol required"))?;
            ...
            "fnx.single_source_shortest_path_length" => {
                ...
                let cutoff = match arguments[1] {
                    ...
                    FnxArgument::Integer(value) if value >= 0 => Some(usize::try_from(value)
        ...
        if parser.keyword("YIELD") {
            if !parser.take(b'*') {
                ...
                loop {
                    ...
                    let field = signature.outputs.iter().copied().find(|field| field.name() == name)
                        .ok_or_else(|| parser.error(FnxBindErrorKind::UnknownYield))?;
                    let alias = if parser.keyword("AS") { parser.word()? } else { name };
                    if outputs.iter().any(|column| column.field == field || column.name == alias) {
                        return Err(parser.error(FnxBindErrorKind::DuplicateYield));
                    }
                    outputs.push(FnxOutputColumn { field, name: alias.to_owned() });
                    if !parser.take(b',') { break; }
                }
            }
        }
        parser.take(b';');
        parser.space();
        if parser.pos != text.len() { return Err(parser.error(FnxBindErrorKind::TrailingInput)); }
    ...
    fn compiled(algorithm: FnxAlgorithm, outputs: Vec<FnxOutputColumn>) -> Self {
        ...
        Self { algorithm, outputs, digest: hash.finalize() }
    }
}
fn default_outputs(signature: &FnxSignature) -> Vec<FnxOutputColumn> {
    signature.outputs.iter().copied().map(|field| FnxOutputColumn { field, name: field.name().to_owned() }).collect()
}
fn float_argument(argument: FnxArgument) -> Option<f64> {
    match argument {
        FnxArgument::Float(value) if value.is_finite() => Some(value),
        FnxArgument::Integer(value) if (-9_007_199_254_740_992..=9_007_199_254_740_992).contains(&value) => Some(value as f64),
        _ => None,
    }
}
...
impl<'a> Parser<'a> {
    fn error(&self, kind: FnxBindErrorKind) -> FnxBindError { FnxBindError { at: self.pos, kind } }
    fn space(&mut self) {
        while self.text.as_bytes().get(self.pos).is_some_and(u8::is_ascii_whitespace) { self.pos += 1; }
    }
    fn take(&mut self, byte: u8) -> bool {
        self.space();
        if self.text.as_bytes().get(self.pos) == Some(&byte) { self.pos += 1; true } else { false }
    }
    fn expect(&mut self, byte: u8, name: &'static str) -> Result<(), FnxBindError> {
        if self.take(byte) { Ok(()) } else { Err(self.error(FnxBindErrorKind::Expected(name))) }
    }
    fn word(&mut self) -> Result<&'a str, FnxBindError> {
        ...
        self.pos += 1;
        while self.text.as_bytes().get(self.pos).is_some_and(|byte| byte.is_ascii_alphanumeric() || *byte == b'_') {
            self.pos += 1;
        }
    ...
    fn keyword(&mut self, expected: &str) -> bool {
        let before = self.pos;
        if self.word().is_ok_and(|word| word.eq_ignore_ascii_case(expected)) { true }
        else { self.pos = before; false }
    }
    fn expect_keyword(&mut self, expected: &'static str) -> Result<(), FnxBindError> {
        if self.keyword(expected) { Ok(()) } else { Err(self.error(FnxBindErrorKind::Expected(expected))) }
    }
    fn argument(&mut self, parameters: &FnxParameters) -> Result<FnxArgument, FnxBindError> {
        ...
        if self.keyword("true") { return Ok(FnxArgument::Boolean(true)); }
        if self.keyword("false") { return Ok(FnxArgument::Boolean(false)); }
        if self.keyword("null") { return Ok(FnxArgument::Null); }
        self.space();
        let start = self.pos;
        while self.text.as_bytes().get(self.pos).is_some_and(|byte| {
            byte.is_ascii_digit() || matches!(*byte, b'+' | b'-' | b'.' | b'e' | b'E')
        }) { self.pos += 1; }
        if self.pos == start { return Err(self.error(FnxBindErrorKind::Expected("typed argument"))); }
        let token = &self.text[start..self.pos];
        if token.bytes().any(|byte| matches!(byte, b'.' | b'e' | b'E')) {
            token.parse::<f64>().map(FnxArgument::Float)
            ...
            token.parse::<u128>().map(|value| FnxArgument::Vertex(VId(value)))
