...
impl Fixture {
    fn new(rows: Vec<Vec<usize>>, retained: Vec<usize>) -> Self {
        ...
        Self { rows, retained, steps: Cell::new(0), fail_at: usize::MAX, opens }
    }
    fn step(&self) -> Result<()> {
    ...
    fn reset(&self) {
        self.steps.set(0);
        for count in &self.opens { count.set(0); }
    }
    fn execute(&self, coefficients: bool, limit: usize, control: &mut impl FnMut() -> Result<()>)
        -> Result<(KernelOutput, usize)>
    {
        let history = self.retained.iter().sum();
        let plan = prepare(self, history, |source| Ok(self.retained[source]),
            coefficients, limit, control)?;
        let work = plan.work;
...
impl Rows for Fixture {
    type Cursor<'a> = FixtureRow<'a> where Self: 'a;
    fn node_count(&self) -> usize { self.rows.len() }
    fn degree(&self, source: usize) -> Option<usize> { self.rows.get(source).map(Vec::len) }
    fn open(&self, source: usize) -> Result<Self::Cursor<'_>> {
        self.step()?;
        let targets = self.rows.get(source).ok_or(ExecutionError::InvalidUpstreamResult)?;
        self.opens[source].set(self.opens[source].get() + 1);
        Ok(FixtureRow { fixture: self, targets, at: 0 })
    }
}
...
fn oracle(rows: &[Vec<usize>]) -> (Vec<u64>, Vec<f64>) {
    ...
    let scores = rows.iter().enumerate().map(|(v, row)| {
        let d = row.iter().filter(|&&target| target != v).count();
        if d < 2 { 0.0 } else { (2 * counts[v]) as f64 / (d * (d - 1)) as f64 }
    }).collect();
    (counts, scores)
}
...
fn every_four_vertex_graph_and_loop_assignment_matches_cubic_and_decoded_oracles() {
    ...
    for mask in 0..(1 << pairs.len()) {
        ...
        for (bit, &(s, t)) in pairs.iter().enumerate() {
            if mask & (1 << bit) != 0 {
                rows[s].push(t);
                if s != t { rows[t].push(s); }
                edges.push(ProjectionEdge { eid: EId(bit as u128), source: vertices[s],
                    target: vertices[t], weight: -7.0 });
            }
        }
        for row in &mut rows { row.sort_unstable(); }
        let (counts, scores) = oracle(&rows);
        let decoded = SnapshotGraphView::build(
            SnapshotBinding { root: ObjectId([4; 32]), as_of: CommitSeq(3) },
            &vertices, &edges,
            ProjectionSpec { directedness: Directedness::Undirected,
                parallel_edges: ParallelEdgePolicy::Reject, self_loops: SelfLoopPolicy::Keep },
            ProjectionLimits { max_vertices: 4, max_input_edges: 10,
                max_adjacency_entries: 16, max_workspace_bytes: 1 << 20 },
        ).unwrap();
        let expected = triangle_statistics(&decoded, FnxExecutionLimits {
            max_iterations: 0, max_result_rows: 4, max_estimated_work: usize::MAX,
        }, || Ok::<(), Infallible>(())).unwrap();
        assert_eq!(expected.triangles, counts);
        assert_eq!(expected.clustering, scores);
        for retained in [rows.iter().map(Vec::len).collect(), vec![1000, 0, 30, 4],
            vec![0; 4], vec![4, 3, 2, 1]] {
            ...
            let KernelValues::Counts(actual) = actual.values else { panic!("counts"); };
            assert_eq!(actual, counts, "mask={mask}");
            let (actual, score_work) = graph.execute(true, usize::MAX, &mut || Ok(())).unwrap();
            let KernelValues::Scores(actual) = actual.values else { panic!("scores"); };
...
fn retained_cost_orientation_minimizes_the_charged_inner_scans_not_visible_degree() {
    ...
    let costs: Vec<_> = graph.retained.iter().map(|&h| row_work(3, history, h).unwrap()).collect();
    let (output, work) = graph.execute(false, usize::MAX, &mut || Ok(())).unwrap();
    let KernelValues::Counts(counts) = output.values else { panic!("counts"); };
    assert_eq!(counts, [1, 1, 1]);
    assert_eq!(graph.opens[0].get(), 3, "heavy row is never an inner rescan");
    let inner = costs[0].min(costs[1]) + costs[0].min(costs[2]) + costs[1].min(costs[2]);
    assert_eq!(work, 3 * (32 + 4 * bits(history)) + 3 * costs.iter().sum::<usize>() + inner);
    let degree_order_inner = costs[0] * 2 + costs[1];
    assert!(degree_order_inner > inner, "negative control penalizes hidden history");
    graph.reset();
    graph.execute(false, work, &mut || Ok(())).unwrap();
    assert!(matches!(graph.execute(false, work - 1, &mut || Ok(())),
        Err(Error::Execution(ExecutionError::LimitExceeded { resource: "estimated work", .. }))));
    graph.reset();
    assert!(graph.execute(false, 0, &mut || Ok(())).is_err());
    assert_eq!(graph.steps.get(), 0, "zero budget precedes all source opens");
}
...
fn every_planning_counting_and_conversion_checkpoint_discards_partial_results() {
    ...
    for coefficients in [false, true] {
        let mut calls = 0;
        graph.execute(coefficients, usize::MAX, &mut || { calls += 1; Ok(()) }).unwrap();
        assert!(calls > 30);
        for stop in 1..=calls {
            let mut seen = 0;
            let result = graph.execute(coefficients, usize::MAX, &mut || {
                seen += 1;
                if seen == stop { Err(Error::Cancelled(std::io::Error::other("stop").into())) }
                else { Ok(()) }
            });
...
fn failure_at_every_source_open_pull_and_eof_never_continues() {
    for coefficients in [false, true] {
        ...
        graph.execute(coefficients, usize::MAX, &mut || Ok(())).unwrap();
        let operations = graph.steps.get();
        for stop in 1..=operations {
            graph.reset();
            graph.fail_at = stop;
            assert!(matches!(graph.execute(coefficients, usize::MAX, &mut || Ok(())),
                Err(Error::Source(SealedProjectionError::UnknownOrdinal(usize::MAX)))));
...
fn empty_isolated_loop_only_and_malformed_rows_have_explicit_outcomes() {
    for rows in [vec![], vec![vec![]], vec![vec![0]], vec![vec![0], vec![1]]] {
        ...
        for coefficients in [false, true] {
            let (actual, _) = graph.execute(coefficients, usize::MAX, &mut || Ok(())).unwrap();
    ...
    for row in [vec![usize::MAX], vec![1], vec![0, 0]] {
        ...
        assert!(matches!(graph.execute(false, usize::MAX, &mut || Ok(())),
...
fn twenty_thousand_vertex_hub_uses_vertex_workspace_and_linear_row_opens() {
    struct Star { n: usize, opens: Cell<usize> }
    struct StarRow { source: usize, n: usize, at: usize }
    impl Cursor for StarRow {
        fn next(&mut self) -> Result<Option<(usize, f64)>> {
            let target = if self.source == 0 {
                (self.at + 1 < self.n).then_some(self.at + 1)
            } else { (self.at == 0).then_some(0) };
            self.at += 1;
            Ok(target.map(|target| (target, 1.0)))
        }
    }
    impl Rows for Star {
        type Cursor<'a> = StarRow where Self: 'a;
        ...
        fn open(&self, source: usize) -> Result<StarRow> {
            ...
            Ok(StarRow { source, n: self.n, at: 0 })
        }
    }
    let graph = Star { n: 20_000, opens: Cell::new(0) };
    let plan = prepare(&graph, graph.n - 1, |v| Ok(graph.degree(v).unwrap()), true,
        usize::MAX, &mut || Ok(())).unwrap();
    ...
    assert_eq!(workspace(graph.n, true).unwrap(), graph.n *
        (3 * size_of::<usize>() + size_of::<u64>()).max(size_of::<usize>() + 16));
}
