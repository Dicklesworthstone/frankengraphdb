...
use crate::{ComplexityWitness, FnxExecutionError, FnxExecutionLimits, FnxGraphKind,
...
pub fn triangle_statistics<C>(
    ...
    if graph.is_directed() {
        return Err(FnxExecutionError::GraphKind { required: FnxGraphKind::Undirected });
    }
    admit("result rows", graph.node_count(), limits.max_result_rows)?;
    admit("estimated work", estimated_work(graph, &mut checkpoint)?, limits.max_estimated_work)?;
...
fn add<C>(counter: &mut usize, value: usize) -> Result<(), FnxExecutionError<C>> {
    *counter = counter.checked_add(value).ok_or(FnxExecutionError::SizeOverflow)?;
    Ok(())
}
...
pub(crate) fn estimated_work<C>(
    ...
    let mut work = graph.node_count().checked_mul(2).ok_or(FnxExecutionError::SizeOverflow)?;
    for source in 0..graph.node_count() {
        checkpoint().map_err(FnxExecutionError::Cancelled)?;
        let row = graph.neighbors_indices(source).ok_or(FnxExecutionError::InvalidUpstreamResult)?;
        add(&mut work, row.len().checked_mul(2).ok_or(FnxExecutionError::SizeOverflow)?)?;
        add(&mut work, bits(row.len()))?;
        for &target in row {
            checkpoint().map_err(FnxExecutionError::Cancelled)?;
            if source < target {
                let other = graph.neighbors_indices(target).ok_or(FnxExecutionError::InvalidUpstreamResult)?;
...
pub(crate) fn workspace_bytes<C>(n: usize) -> Result<usize, FnxExecutionError<C>> {
    n.checked_mul(std::mem::size_of::<u64>() + std::mem::size_of::<usize>()
        + std::mem::size_of::<f64>()).ok_or(FnxExecutionError::SizeOverflow)
}
...
pub(crate) fn run<C>(
    ...
    for source in 0..n {
        ...
        for &target in row {
            ...
            if source != target { marked[target] = source; }
        }
        for &target in row {
            checkpoint().map_err(FnxExecutionError::Cancelled)?;
            add(&mut witness.edges_scanned, 1)?;
            let other = graph.neighbors_indices(target).ok_or(FnxExecutionError::InvalidUpstreamResult)?;
            if (other.len(), target) >= (row.len(), source) { continue; }
            for &third in other {
                checkpoint().map_err(FnxExecutionError::Cancelled)?;
                add(&mut witness.edges_scanned, 1)?;
                if third != target && third != source && marked[third] == source {
                    triangles[third] = triangles[third].checked_add(1)
    ...
    for node in 0..n {
        checkpoint().map_err(FnxExecutionError::Cancelled)?;
        let row = graph.neighbors_indices(node).ok_or(FnxExecutionError::InvalidUpstreamResult)?;
        let mut low = 0;
        ...
        while low < high {
            ...
            match row[middle].cmp(&node) {
                ...
                std::cmp::Ordering::Equal => { has_loop = true; break; }
            }
        }
        ...
        let coefficient = if degree < 2 {
            ...
            let denominator = degree.checked_mul(degree - 1).ok_or(FnxExecutionError::SizeOverflow)?;
            let numerator = u128::from(triangles[node]) * 2;
            if numerator > denominator { return Err(FnxExecutionError::InvalidUpstreamResult); }
            numerator as f64 / denominator as f64
        };
        clustering.push(coefficient);
    }
    Ok(TriangleStatistics { triangles, clustering, witness })
}
...
mod tests {
    ...
    fn graph(n: usize, edges: &[(usize, usize)]) -> SnapshotGraphView {
        ...
        let edges: Vec<_> = edges.iter().enumerate().map(|(id, &(s, t))| ProjectionEdge {
            eid: EId(id as u128), source: vertices[s], target: vertices[t], weight: -3.0,
        }).collect();
        SnapshotGraphView::build(
            SnapshotBinding { root: ObjectId([3; 32]), as_of: CommitSeq(4) },
            &vertices, &edges,
            ProjectionSpec { directedness: Directedness::Undirected,
                parallel_edges: ParallelEdgePolicy::CollapseUnit, self_loops: SelfLoopPolicy::Keep },
            ProjectionLimits { max_vertices: n, max_input_edges: edges.len(),
                max_adjacency_entries: edges.len() * 2, max_workspace_bytes: 1 << 26 },
        ).unwrap()
    }
    fn limits() -> FnxExecutionLimits {
        FnxExecutionLimits { max_iterations: 0, max_result_rows: 10000, max_estimated_work: 1 << 26 }
    }
    ...
    fn all_four_vertex_topologies_including_loops_match_fnx_and_brute_force() {
        ...
        for mask in 0usize..(1 << pairs.len()) {
            let edges: Vec<_> = pairs.iter().enumerate().filter(|(bit,_)| mask & (1 << bit) != 0)
            ...
            for a in 0..4 {
                for b in (a + 1)..4 {
                    for c in (b + 1)..4 {
                        if edges.contains(&(a,b)) && edges.contains(&(a,c)) && edges.contains(&(b,c)) {
                            expected[a] += 1; expected[b] += 1; expected[c] += 1;
                        }
                    }
                }
            }
            ...
            for name in view.nodes_ordered() { let _ = oracle.add_node(name); }
            for &(s,t) in &edges {
                oracle.add_edge(view.get_node_name(s).unwrap(), view.get_node_name(t).unwrap()).unwrap();
            }
            for value in fnx_algorithms::triangles(&oracle).triangles {
                assert_eq!(actual.triangles[view.get_node_index(&value.node).unwrap()], value.count as u64);
            }
            for value in fnx_algorithms::clustering_coefficient(&oracle).scores {
                assert_eq!(actual.clustering[view.get_node_index(&value.node).unwrap()].to_bits(), value.score.to_bits());
            }
    ...
    fn hubs_are_linear_and_parallel_edges_do_not_multiply_triangles() {
        ...
        let empty = triangle_statistics(&graph(0, &[]), limits(), || Ok::<(), Infallible>(())).unwrap();
    ...
    fn row_and_work_limits_and_every_checkpoint_refuse_without_partial_output() {
        ...
        for cap in [FnxExecutionLimits { max_result_rows: 3, ..limits() },
            FnxExecutionLimits { max_estimated_work: 0, ..limits() }] {
            assert!(matches!(triangle_statistics(&view, cap, || Ok::<(), Infallible>(())),
                Err(FnxExecutionError::LimitExceeded { .. })));
        }
        let mut total = 0;
        triangle_statistics(&view, limits(), || { total += 1; Ok::<(), &'static str>(()) }).unwrap();
        for stop in 1..=total {
            let mut count = 0;
