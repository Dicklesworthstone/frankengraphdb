...
impl Cluster {
    ...
    fn step(&mut self, member: MemberId, event: Event<u64>) {
        ...
        for applied in &output.committed {
            ...
            assert_eq!(
                &applied.entry,
                &self.disk[&member].entries()[applied.index as usize - 1]
            );
        }
    ...
    fn restart(&mut self, member: u128) {
        ...
        self.nodes.insert(
            id,
            Raft::recover(id, self.disk[&id].clone(), self.limits).unwrap(),
        );
    }
}
...
fn joint_admission_rejects_empty_duplicate_zero_and_learner_overlap() {
    ...
    assert_eq!(
        old,
        &BTreeSet::from([MemberId(1), MemberId(2), MemberId(3)])
    );
    assert_eq!(
        new,
        &BTreeSet::from([MemberId(3), MemberId(4), MemberId(5)])
    );
}
...
fn membership_bound_counts_unique_union_not_twice_the_overlap() {
    ...
    assert_eq!(
        joint(&members, &[1], &[1025]),
        Err(Error::InvalidConfiguration)
    );
    assert_eq!(
        joint(&[1], &members, &[1025]),
        Err(Error::InvalidConfiguration)
    );
}
...
fn elections_require_both_groups_for_old_new_and_shared_candidates() {
    ...
    assert_eq!(
        node.role().unwrap(),
        Role::Leader,
        "overlapping voter counts once in each group"
    );
}
...
fn satisfying_both_groups_still_cannot_commit_an_old_term_without_a_barrier() {
    ...
    for state in cluster.disk.values() {
        ...
        assert_eq!(
            state.commit_index(),
            0,
            "old-term quorum counting is not a commit barrier"
        );
...
fn snapshot_recovery_keeps_joint_voter_groups_and_configuration_identity() {
    let configuration = joint(&[1, 2, 3], &[3, 4, 5], &[]).unwrap();
    let snapshot =
        SnapshotCut::from_authenticated_parts(&configuration, [3; 32], [4; 32], [5; 32], 100, 3)
            .unwrap();
    let state = PersistentState::<u64>::from_authenticated_snapshot(
        configuration.clone(),
        3,
        Some(MemberId(4)),
        100,
