...
fn receive(node: &mut Raft<u64>, from: u128, message: Message<u64>) -> Output<u64> {
    ...
    let envelope = Envelope {
        domain: c.domain(), configuration: c.identity(),
        from: MemberId(from), to: node.id(), message,
    };
...
fn nodes(configuration: &Configuration, window: usize) -> Cluster {
    configuration.voters().union(configuration.learners()).map(|id| {
        let mut node = Raft::new(*id, configuration.clone(), Limits {
            max_log_entries: 128, max_append_entries: 2,
        }).unwrap();
    ...
    for node in nodes.values() {
        ...
        for other in nodes.values() {
            ...
            for index in first..=last {
                assert_eq!(state.entries()[(index - base - 1) as usize],
...
fn burst(nodes: &mut Cluster, count: u64) -> Vec<Envelope<u64>> {
    ...
    for command in 10..10 + count {
        messages.extend(publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Propose(command)).messages);
    }
    messages
}

fn to(messages: &[Envelope<u64>], peer: u128) -> Vec<Envelope<u64>> {
    messages.iter().filter(|message| message.to == MemberId(peer)).cloned().collect()
}

fn coordinates(envelope: &Envelope<u64>) -> (u64, u64, u64, u64) {
    match &envelope.message {
        Message::Append { term, request, prev_index, entries, .. } =>
            (*term, *request, *prev_index, *prev_index + entries.len() as u64),
...
fn bursts_fill_independent_bounded_windows_without_retransmitting_on_each_proposal() {
    for width in [1, 2, 4, 16, 64] {
        ...
        assert_eq!(nodes[&MemberId(1)].durable_state().unwrap().commit_index(), 1);
...
fn later_success_proves_prefix_and_retires_earlier_requests_without_reusing_ids() {
    ...
    let responses: Vec<_> = requests.iter().cloned().map(|r| reply(&mut nodes, r)).collect();
    let out = publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Receive(responses[3].clone()));
    assert_eq!(nodes[&MemberId(1)].durable_state().unwrap().commit_index(), 5);
    assert_eq!(out.committed.len(), 4);
    let last_request = coordinates(&requests[3]).1;
    assert!(to(&out.messages, 2).iter().all(|r| coordinates(r).1 > last_request));
    for request in &requests {
        let (term, serial, _, _) = coordinates(request);
        assert!(!nodes[&MemberId(1)].pending_append_reply(MemberId(2), term, serial).unwrap());
    }
...
fn rejecting_a_later_range_clears_speculation_and_probes_before_refilling() {
    ...
    let out = publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Receive(rejection));
    ...
    for old in &initial {
        ...
        assert!(!nodes[&MemberId(1)].pending_append_reply(MemberId(2), term, request).unwrap());
    }
    ...
    assert_eq!(nodes[&MemberId(1)].durable_state().unwrap().commit_index(), 9);
}
...
fn every_four_request_and_four_response_permutation_converges_with_a_silent_minority() {
    for delivery in permutations() {
        for replies in permutations() {
            ...
            let responses: Vec<_> = delivery.iter().map(|i| reply(&mut nodes, initial[*i].clone())).collect();
            let mut recovery = Vec::new();
            for i in replies {
                recovery.extend(publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Receive(responses[i].clone())).messages);
                common_prefixes(&nodes);
            }
...
fn a_new_leader_does_not_speculate_before_its_first_matching_append_reply() {
    ...
    let out = receive(nodes.get_mut(&MemberId(1)).unwrap(), 2, Message::Vote { term: 1, granted: true });
...
fn compaction_invalidates_windows_and_rewinds_optimism_to_the_proved_frontier() {
    ...
    let cut = SnapshotCut::from_authenticated_parts(&config(), [21; 32], [22; 32], [23; 32], 3, 1).unwrap();
    ...
    let Message::InstallSnapshot { request, .. } = snapshot[0].message else { panic!() };
    assert!(!nodes[&MemberId(1)].pending_append_reply(MemberId(3), 1, request).unwrap());
    // A forged wrong-kind append reply must not consume a snapshot flight.
    let ignored = receive(nodes.get_mut(&MemberId(1)).unwrap(), 3,
        Message::Appended { term: 1, request, success: true, conflict_next: 0 });
    assert!(ignored.messages.is_empty());
    let offer = publish(nodes.get_mut(&MemberId(3)).unwrap(), Event::Receive(snapshot[0].clone()));
    let transfer = offer.snapshot_transfers[0].id();
    // The test model publishes an atomic installed root, not real disk I/O.
    let installed = publish(nodes.get_mut(&MemberId(3)).unwrap(), Event::SnapshotReady(transfer));
    let out = publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Receive(installed.messages[0].clone()));
    ...
    let out = publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Heartbeat);
    pump(&mut nodes, out.messages, &[1, 2, 3]);
    for node in nodes.values() { assert_eq!(node.durable_state().unwrap().commit_index(), 9); }
}

#[test]
fn learners_and_pooled_joint_responses_never_replace_independent_majorities() {
    let config = Configuration::joint(Domain([7; 32]), [8; 32],
    ...
    for peer in [2, 4, 6] {
        let replies: Vec<_> = to(&initial, peer).into_iter().map(|m| reply(&mut nodes, m)).collect();
        publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Receive(replies.last().unwrap().clone()));
    }
    assert_eq!(nodes[&MemberId(1)].durable_state().unwrap().commit_index(), 1);
    let replies: Vec<_> = to(&initial, 3).into_iter().map(|m| reply(&mut nodes, m)).collect();
    publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Receive(replies.last().unwrap().clone()));
    assert_eq!(nodes[&MemberId(1)].durable_state().unwrap().commit_index(), 5);
}
...
fn window_configuration_is_validated_and_cannot_discard_live_or_unpublished_requests() {
    ...
    for maximum in [0, 65, usize::MAX] {
        assert_eq!(node.configure_append_pipeline(maximum), Err(Error::InvalidLimits));
    ...
    assert_eq!(node.configure_append_pipeline(2), Err(Error::AwaitingDurability));
    node.persisted(token).unwrap();
    publish(&mut node, Event::ElectionTimeout);
    assert_eq!(node.configure_append_pipeline(2), Err(Error::PipelineConfigurationBusy));
    receive(&mut node, 2, Message::Vote { term: 1, granted: true });
    assert_eq!(node.configure_append_pipeline(2), Err(Error::PipelineConfigurationBusy));
...
fn higher_term_fences_all_old_requests_and_preserves_committed_state() {
    ...
    receive(nodes.get_mut(&MemberId(1)).unwrap(), 2, Message::Vote { term: 2, granted: false });
...
fn publication_failure_or_counter_exhaustion_never_releases_a_partial_window() {
    let mut nodes = elected(4);
    let pending = nodes.get_mut(&MemberId(1)).unwrap().step(Event::Propose(99)).unwrap().id();
    let node = nodes.get_mut(&MemberId(1)).unwrap();
    assert_eq!(node.pending_append_reply(MemberId(2), 1, 1), Err(Error::AwaitingDurability));
    node.publication_failed();
    assert_eq!(node.persisted(pending).err(), Some(Error::RecoveryRequired));
    let mut nodes = elected(4);
    let node = nodes.get_mut(&MemberId(1)).unwrap();
    node.request = u64::MAX - 1;
    assert_eq!(node.step(Event::Propose(88)).err(), Some(Error::CounterExhausted));
...
fn inherited_old_term_windows_do_not_commit_until_a_current_term_entry_is_matched() {
    ...
    let old = vec![Entry { term: 1, command: Some(11) }; 6];
    let mut nodes = nodes(&configuration, 4);
    let mut leader = Raft::recover(MemberId(1), PersistentState::from_authenticated_parts(
        configuration, 1, None, 0, old), Limits { max_log_entries: 128, max_append_entries: 2 }).unwrap();
    leader.configure_append_pipeline(4).unwrap();
    publish(&mut leader, Event::ElectionTimeout);
    let output = receive(&mut leader, 2, Message::Vote { term: 2, granted: true });
    ...
    assert_eq!(nodes[&MemberId(1)].durable_state().unwrap().commit_index(), 0);
    let requests = to(&out.messages, 2);
    assert_eq!(requests.len(), 3);
    for request in requests {
        let last = coordinates(&request).3;
        let response = reply(&mut nodes, request);
        publish(nodes.get_mut(&MemberId(1)).unwrap(), Event::Receive(response));
