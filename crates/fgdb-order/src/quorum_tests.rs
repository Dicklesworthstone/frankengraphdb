...
fn every_five_member_vote_subset_obeys_both_independent_majorities() {
    for old in 1..32_u32 {
        for new in 1..32_u32 {
            let configuration = Configuration::joint(Domain([1; 32]), [2; 32], members(old), members(new), []).unwrap();
            for votes in 0..32_u32 {
                ...
                assert_eq!(configuration.quorum(&received), expected, "old={old} new={new} votes={votes}");
                if old == new {
                    let stable = Configuration::stable(Domain([1; 32]), [3; 32], members(old), []).unwrap();
    ...
    for old in 1..16_u32 {
        for new in 1..16_u32 {
            ...
            for encoded in 0..81_u32 {
                ...
                let expected = (0..=2_u64).rev().find(|cut| {
                    [old, new].into_iter().all(|group| {
                        let reached = (0..4).filter(|slot| {
                            group & (1 << slot) != 0 && indices[*slot] >= *cut
                        }).count();
                        reached > group.count_ones() as usize / 2
                    })
                }).unwrap();
                let matched = |member: MemberId| indices[member.0 as usize - 1];
                assert_eq!(configuration.quorum_index(matched), expected,
