...
impl Cluster {
    ...
    fn deliver(&mut self, messages: Vec<Envelope<u64>>, allow: impl Fn(MemberId) -> bool) {
        ...
        while let Some(message) = queue.pop_front() {
            ...
            let output = step(
                self.nodes.get_mut(&message.to).unwrap(),
...
fn one_publication_releases_separate_contiguous_commands_once() {
    ...
    let pending = node
        .step(Event::ProposeBatch(vec![10, 20, 20, 30]))
        .unwrap();
    ...
    assert_eq!(
        output
            .committed
            .iter()
            .map(|entry| (entry.index, entry.entry.command))
            .collect::<Vec<_>>(),
...
fn empty_oversized_and_full_log_batches_reject_without_a_prefix() {
    let mut node = single(Limits {
        max_log_entries: 5,
        max_append_entries: 4,
    });
    for (commands, expected) in [
        (vec![], Error::InvalidMessage),
        (vec![1; 5], Error::AppendTooLarge),
    ] {
        let before = node.state.clone();
        let counters = (node.generation, node.request);
        assert!(
            matches!(node.step(Event::ProposeBatch(commands)), Err(error) if error == expected)
        );
    ...
    assert!(matches!(
        node.step(Event::ProposeBatch(vec![4, 5])),
        Err(Error::LogFull)
    ));
    assert_eq!(node.durable_state().unwrap(), &before);
...
fn absolute_index_and_generation_exhaustion_reject_the_whole_range() {
    ...
    let base = u64::MAX - 4;
    let cut =
        SnapshotCut::from_authenticated_parts(&configuration, [3; 32], [4; 32], [5; 32], base, 1)
            .unwrap();
    let mut node = Raft::<u64>::recover(
        MemberId(1),
        PersistentState::from_authenticated_snapshot(configuration, 1, None, base, cut, Vec::new()),
        Limits::default(),
    )
    .unwrap();
    ...
    let generation = node.generation;
    assert!(matches!(
        node.step(Event::ProposeBatch(vec![1, 2, 3])),
    ...
    assert!(matches!(
        node.step(Event::ProposeBatch(vec![3])),
        Err(Error::CounterExhausted)
    ));
    ...
    let before = node.state.clone();
    assert!(matches!(
        node.step(Event::ProposeBatch(vec![1, 2])),
        Err(Error::CounterExhausted)
    ));
    assert_eq!(node.durable_state().unwrap(), &before);
}
...
fn nonleaders_and_learners_cannot_admit_a_batch() {
    for phase in 0..3 {
        ...
        if phase == 2 {
            step(&mut node, Event::LivenessTimeout);
        }
        let before = node.state.clone();
        assert!(matches!(
            node.step(Event::ProposeBatch(vec![1, 2])),
    ...
    let configuration =
        Configuration::stable(Domain([1; 32]), [2; 32], [MemberId(1)], [MemberId(2)]).unwrap();
    let mut learner = Raft::<u64>::new(MemberId(2), configuration, Limits::default()).unwrap();
    assert!(matches!(
        learner.step(Event::ProposeBatch(vec![1])),
        Err(Error::NotLeader)
    ));
...
fn actual_follower_persists_before_reply_and_commit_remains_separate() {
    ...
    assert!(
        step(cluster.leader(), Event::Receive(reply))
            .committed
            .is_empty()
    );
}
...
fn joint_batch_needs_each_majority_and_learners_do_not_count() {
    ...
    for (peer, expected) in [(2, 1), (6, 1), (3, 1), (4, 3)] {
        let request = output
            .messages
            .iter()
            .find(|message| message.to == MemberId(peer))
            .unwrap()
            .clone();
        let response = step(
            cluster.nodes.get_mut(&MemberId(peer)).unwrap(),
...
fn reordered_pipelined_batches_converge_with_a_silent_minority() {
    for order in [
        ...
        for first in [10, 20, 30] {
            let output = step(
                cluster.leader(),
                Event::ProposeBatch(vec![first, first + 1]),
            );
            assert!(output.committed.is_empty());
            batches.push(
                output
        ...
        assert_eq!(
            cluster.nodes[&MemberId(2)].state.entries,
            cluster.nodes[&MemberId(1)].state.entries
        );
...
fn acknowledgement_of_a_prefix_does_not_commit_the_later_batch() {
    ...
    let request = first
        ...
        .unwrap();
    let response = step(
        cluster.nodes.get_mut(&MemberId(2)).unwrap(),
        Event::Receive(request),
    );
    let output = step(
        cluster.leader(),
        Event::Receive(response.messages[0].clone()),
    );
    assert_eq!(
        output
            .committed
...
fn internal_request_exhaustion_fences_the_complete_unpublished_batch() {
    ...
    assert!(matches!(
        node.step(Event::ProposeBatch(vec![10, 20])),
        Err(Error::CounterExhausted)
    ));
...
fn late_batch_rejection_preserves_pending_replication_identities() {
    ...
    let before = node.state.clone();
    let counters = (node.generation, node.request);
    let requests: Vec<_> = node.progress[&MemberId(2)]
        .in_flight
        .iter()
        .cloned()
        .collect();
    assert!(matches!(
        node.step(Event::ProposeBatch(vec![40, 50])),
        Err(Error::LogFull)
    ));
    assert_eq!(node.durable_state().unwrap(), &before);
    assert_eq!((node.generation, node.request), counters);
    for request in requests {
        if let InFlight::Append { request, .. } = request {
            assert!(
                node.pending_append_reply(MemberId(2), before.term(), request)
                    .unwrap()
            );
        }
    }
...
fn minority_behind() -> Cluster {
    ...
    let limits = Limits {
        max_log_entries: 32,
        max_append_entries: 3,
    };
    let nodes = configuration
        .members()
        .map(|id| (id, Raft::new(id, configuration.clone(), limits).unwrap()))
        .collect();
...
fn catchup_can_commit_a_prefix_inside_one_local_batch() {
    ...
    let first = retry
        .messages
        .into_iter()
        .find(|message| message.to == MemberId(3))
        .unwrap();
    let response = step(
        cluster.nodes.get_mut(&MemberId(3)).unwrap(),
        Event::Receive(first),
    );
    let next = step(
        cluster.leader(),
    ...
    let request = next
        ...
        .unwrap();
    assert!(
        matches!(&request.message, Message::Append { prev_index: 1, entries, .. } if entries.len() == 3)
    );
    let response = step(
        cluster.nodes.get_mut(&MemberId(3)).unwrap(),
        Event::Receive(request),
...
fn batches_preserve_the_snapshot_lane_of_a_lagging_member() {
    let mut cluster = minority_behind();
    let cut =
        SnapshotCut::from_authenticated_parts(&config(3), [3; 32], [4; 32], [5; 32], 2, 1).unwrap();
    ...
    let snapshot = output
        ...
        .find(|message| message.to == MemberId(3))
        .unwrap();
    let Message::InstallSnapshot { request, .. } = &snapshot.message else {
        panic!("lagging peer needs the snapshot")
    };
    ...
    assert!(
        !later
            .messages
            .iter()
            .any(|message| message.to == MemberId(3))
    );
    assert!(
        matches!(cluster.leader().progress[&MemberId(3)].in_flight.front(),
