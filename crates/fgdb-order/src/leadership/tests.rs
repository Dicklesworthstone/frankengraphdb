...
impl Cluster {
    ...
    fn node(&mut self, id: u128) -> &mut Raft<u64> {
        self.0.get_mut(&id).unwrap()
    }
    fn event(&mut self, id: u128, event: Event<u64>) -> Output<u64> {
        step(self.node(id), event)
    }
...
fn caught_up_handoff_requires_a_persisted_next_term_vote_and_normal_quorum() {
    ...
    assert_eq!(
        output.role,
        Role::Leader,
        "request is not stepdown or election success"
    );
    assert!(
        !output.reset_election_timer,
        "handoff must not renew quorum contact"
    );
    ...
    assert!(
        campaign
            .messages
            .iter()
            .all(|envelope| matches!(envelope.message, Message::RequestVote { term: 2, .. }))
    );
...
fn optimistic_pipeline_sends_do_not_authorize_handoff_but_later_prefix_ack_does() {
    ...
    for value in 1..=3 {
        appends.extend(
            cluster
                .event(1, Event::Propose(value))
                .messages
                .into_iter()
                .filter(|m| m.to == MemberId(2)),
        );
    }
    ...
    assert!(matches!(
        cluster.node(1).step(Event::Propose(99)),
        Err(Error::LeadershipTransferInProgress)
    ));
    let mut replies = Vec::new();
    for append in appends {
        replies.extend(cluster.deliver(append).messages);
    }
    assert_eq!(replies.len(), 3);
    let pending = cluster
        .node(1)
        .step(Event::Receive(replies[2].clone()))
        .unwrap();
    assert!(
        pending.requires_write(),
        "newly advanced commit must publish before all output"
    );
    assert_eq!(pending.state().commit_index(), 4);
    let id = pending.id();
    let released = cluster.node(1).persisted(id).unwrap();
    assert_eq!(
        released.messages.iter().filter(|m| is_timeout(m)).count(),
        1
    );
...
fn snapshot_offer_and_installed_base_are_not_the_required_retained_suffix() {
    ...
    let cut =
        SnapshotCut::from_authenticated_parts(&configuration, [21; 32], [22; 32], [23; 32], 9, 3)
            .unwrap();
    ...
    let mut messages = cluster
        .deliver(installed.messages.into_iter().next().unwrap())
        .messages;
    assert!(
        !messages.iter().any(is_timeout),
        "snapshot only covers index 9, not 10/11"
    );
    let mut handoff = None;
    for _ in 0..8 {
        let mut next = Vec::new();
        for envelope in messages {
            if is_timeout(&envelope) {
                handoff = Some(envelope);
                continue;
            }
...
fn retries_cannot_extend_deadline_and_abort_neither_loses_logs_nor_reuses_identity() {
    ...
    for _ in 0..10 {
        ...
        assert_eq!(
            cluster.node(1).leadership_transfer().unwrap().unwrap().id(),
            id
        );
        assert!(!cluster.event(1, Event::Heartbeat).reset_election_timer);
    }
    assert!(matches!(
        cluster.node(1).step(Event::TransferLeadership(MemberId(3))),
        Err(Error::LeadershipTransferInProgress)
    ));
    ...
    assert_ne!(id, second);
    assert!(matches!(
        cluster.node(1).step(Event::AbortLeadershipTransfer(id)),
        Err(Error::StaleLeadershipTransfer)
    ));
    cluster.event(1, Event::AbortLeadershipTransfer(second));
    assert_eq!(cluster.node(1).durable_state().unwrap(), &state);
    cluster.event(1, Event::Propose(89));
    assert_eq!(
        cluster.node(1).state.entries.last().unwrap().command,
        Some(89)
    );
}
...
fn lost_quorum_still_steps_down_and_stale_timers_cannot_cross_recovery() {
    ...
    assert!(matches!(
        recovered.step(Event::AbortLeadershipTransfer(id)),
        Err(Error::StaleLeadershipTransfer)
    ));
}
...
fn only_current_leader_exact_endpoint_and_voter_can_trigger_campaign() {
    ...
    let snapshot =
        SnapshotCut::from_authenticated_parts(&stable(), [31; 32], [32; 32], [33; 32], 20, 1)
            .unwrap();
    let offer = cluster.deliver(message(
        1,
        2,
...
fn malformed_or_cross_domain_handoffs_do_not_mutate_live_state() {
    ...
    for case in 0..11 {
        ...
        match case {
            ...
            9 => {
                bad.message = Message::TimeoutNow {
                    term: u64::MAX,
                    round: 1,
                    last_index: 1,
                    last_term: 1,
        ...
        assert!(
            cluster.node(2).step(Event::Receive(bad)).is_err(),
            "case {case}"
        );
    ...
    for target in [1, 4, 99] {
        assert!(matches!(
            cluster
                .node(1)
                .step(Event::TransferLeadership(MemberId(target))),
            Err(Error::InvalidLeadershipTarget)
        ));
    }
    assert!(matches!(
        cluster.node(2).step(Event::TransferLeadership(MemberId(1))),
        Err(Error::NotLeader)
    ));
    let higher = message(
        1,
...
fn handoff_election_requires_both_joint_majorities() {
    ...
    for voter in [1, 2] {
        ...
        let reply = cluster
            ...
            .unwrap();
        assert_eq!(
            cluster.deliver(reply).role,
            Role::Candidate,
            "pooled quorum is insufficient"
        );
    }
    let request = campaign
        .messages
        .into_iter()
        .find(|m| m.to == MemberId(4))
        .unwrap();
    let reply = cluster
        .deliver(request)
        .messages
...
fn compaction_preserves_the_frozen_endpoint_and_rejects_retired_acknowledgements() {
    ...
    let cut = SnapshotCut::from_authenticated_parts(&stable(), [41; 32], [42; 32], [43; 32], 2, 1)
        .unwrap();
    cluster.event(1, Event::Compact(cut));
    assert!(
        !cluster.deliver(reply).messages.iter().any(is_timeout),
        "compaction retired that append ID"
    );
...
fn exhausted_term_and_pre_candidate_state_cannot_start_a_handoff_election() {
    ...
    assert!(matches!(
        solo.step(Event::TransferLeadership(MemberId(1))),
        Err(Error::InvalidLeadershipTarget)
    ));

    // An actual term at the integer frontier admits no next-term election.
    let max =
        PersistentState::from_authenticated_parts(stable(), u64::MAX - 1, None, 0, Vec::new());
    ...
    assert!(matches!(
        node.step(Event::TransferLeadership(MemberId(2))),
...
fn handoff_freezes_batched_and_scalar_proposals_before_any_admission_mutation() {
    for caught_up in [false, true] {
        ...
        for commands in [vec![], vec![10], vec![10, 11, 12], vec![10; 5]] {
            assert!(matches!(
                node.step(Event::ProposeBatch(commands)),
                Err(Error::LeadershipTransferInProgress)
            ));
            assert_eq!(node.durable_state().unwrap(), &before);
...
fn identical_numeric_attempts_in_distinct_incarnations_cannot_cancel_each_other() {
    ...
    assert_eq!(
        ...
        "exercise the equal-scalar case"
    );
    assert_ne!(a, b);
    assert!(matches!(
        right.node(1).step(Event::AbortLeadershipTransfer(a)),
        Err(Error::StaleLeadershipTransfer)
    ));
    assert_eq!(
        right.node(1).leadership_transfer().unwrap().unwrap().id(),
        b
    );
}
