...
impl Output {
    fn put(&mut self, bytes: &[u8]) -> Result<(), SealedError> {
        let wanted = self.bytes.len().checked_add(bytes.len()).ok_or(SealedError::SizeOverflow)?;
        check_limit("image bytes", wanted, self.limit)?;
        self.bytes.try_reserve(bytes.len()).map_err(|_| SealedError::AllocationFailed)?;
        self.bytes.extend_from_slice(bytes);
        Ok(())
    }

    fn count(&mut self, count: usize) -> Result<(), SealedError> {
        self.put(&u32::try_from(count).map_err(|_| SealedError::SizeOverflow)?.to_le_bytes())
    }

    fn identity<T: ElementIdentity>(&mut self, column: &IdentityColumn<T>) -> Result<(), SealedError> {
        let descriptor = column.descriptor();
        let payload = column.try_scalar_payload(self.limit).map_err(SealedError::Identity)?;
    ...
    fn ef(&mut self, column: &EliasFano) -> Result<(), SealedError> {
        let payload = ef_payload::encode(column, EfPayloadLimits {
            max_entries: column.len(), max_bytes: self.limit,
...
pub(super) fn encode(
    ...
    let mut output = Output { bytes: Vec::new(), limit: limits.max_image_bytes };
    output.put(MAGIC)?;
    ...
    for row in &image.rows {
        ...
        match row.storage {
            Storage::Inline(index) => {
                ...
                let len = image.inlines[index].encode_into(&mut payload).map_err(SealedError::Inline)?;
                output.put(&[0])?;
                output.count(len)?;
    ...
    for chunk in image.properties.chunks(MAX_PROPERTY_PATCH_ROWS as usize) {
        ...
        property_bytes = property_bytes.checked_add(payload.len().checked_sub(10)
            .ok_or(SealedError::NonCanonical)?).ok_or(SealedError::SizeOverflow)?;
...
impl<'a> Input<'a> {
    ...
    fn u64(&mut self) -> Result<u64, SealedError> {
        Ok(u64::from_le_bytes(self.take(8)?.try_into().expect("bounded u64")))
    }

    fn identity<T: ElementIdentity>(&mut self, count: usize, limits: SealedLimits) -> Result<IdentityColumn<T>, SealedError> {
        let representation = crate::representation_from_tag(self.byte()?).ok_or(SealedError::NonCanonical)?;
        let prefixes = self.count()?;
        let length = self.count()?;
        let payload = self.take(length)?;
        IdentityColumn::try_from_scalar_payload(payload,
            IdentityColumnDescriptor::new(representation, count, prefixes), identity_limits(count, limits))
            .map_err(SealedError::Identity)
    }

    fn ef(&mut self, count: usize, maximum_bound: u64, limits: SealedLimits) -> Result<EliasFano, SealedError> {
        let maximum = self.u64()?;
        if maximum > maximum_bound { return Err(SealedError::NonCanonical); }
        let length = self.count()?;
        ef_payload::decode(self.take(length)?, count, maximum, EfPayloadLimits {
            max_entries: count, max_bytes: limits.max_image_bytes,
        }).map_err(SealedError::Payload)
    }
}
...
pub(super) fn decode(
    ...
    if input.take(4)? != MAGIC || u16::from_le_bytes(input.take(2)?.try_into().expect("bounded version")) != VERSION {
    ...
    if row_count > incidence_count || destination_count > incidence_count || property_count > incidence_count {
        return Err(SealedError::NonCanonical);
    }
    let dictionary = input.identity::<VId>(destination_count, limits)?;
    let offsets = input.ef(row_count.checked_add(1).ok_or(SealedError::SizeOverflow)?, incidence_count as u64, limits)?;
    ...
    for row_index in 0..row_count {
        ...
        let end = offsets.select(row_index + 1).ok_or(SealedError::NonCanonical)?;
        let count = end.checked_sub(start).and_then(|count| usize::try_from(count).ok())
            .filter(|&count| count != 0).ok_or(SealedError::NonCanonical)?;
        match input.byte()? {
            0 => {
                if count > INLINE_CAPACITY { return Err(SealedError::NonCanonical); }
                let length = input.count()?;
                let inline = InlineAdjacency::decode(input.take(length)?).map_err(SealedError::Inline)?;
                if inline.len() != count { return Err(SealedError::NonCanonical); }
                let key = inline.descriptor();
                let index = inlines.len();
                inlines.try_reserve(1).map_err(|_| SealedError::AllocationFailed)?;
                inlines.push(inline);
                rows.push(Row { key, count, storage: Storage::Inline(index) });
            }
            1 => {
                if count <= INLINE_CAPACITY { return Err(SealedError::NonCanonical); }
                let src = VId(u128::from_le_bytes(input.take(16)?.try_into().expect("bounded source")));
                let relation = RelationId(input.u64()?);
                if input.byte()? != 0 { return Err(SealedError::NonCanonical); }
                let key = DescriptorKey { src, relation, direction: Direction::Outbound };
                let neighbors = input.ef(count, destination_count.saturating_sub(1) as u64, limits)?;
                let edge_ids = input.identity::<EId>(count, limits)?;
                let span_count = input.count()?;
                if span_count == 0 || span_count > count { return Err(SealedError::NonCanonical); }
                // Prove the minimum remaining framing before allocating arrays.
                let minimum = span_count.checked_mul(20).and_then(|n| count.checked_mul(4).and_then(|m| n.checked_add(m)))
                    .ok_or(SealedError::SizeOverflow)?;
                if minimum > input.bytes.len() - input.at { return Err(SealedError::Truncated); }
                let mut spans = reserved(span_count)?;
                let mut start_row = 0;
                for _ in 0..span_count {
                    let end_row = u32::try_from(input.count()?).map_err(|_| SealedError::SizeOverflow)?;
                    let created_at = CommitSeq(input.u64()?);
                    let retired = input.u64()?;
                    spans.push(VisibilityInterval { start_row, end_row, created_at,
                ...
                for at in 0..count {
                    if at % 256 == 0 { checkpoint()?; }
                    locators.push(u32::try_from(input.count()?).map_err(|_| SealedError::SizeOverflow)?);
                }
                let index = runs.len();
                runs.try_reserve(1).map_err(|_| SealedError::AllocationFailed)?;
                runs.push(Run { neighbors, edge_ids, spans, locators });
                rows.push(Row { key, count, storage: Storage::Run(index) });
            }
    ...
    while properties.len() < property_count {
        ...
        let length = input.count()?;
        property_bytes = property_bytes.checked_add(length.checked_sub(10).ok_or(SealedError::NonCanonical)?)
            .ok_or(SealedError::SizeOverflow)?;
        check_limit("property bytes", property_bytes, limits.max_property_bytes)?;
        ...
        if chunk.len() != expected { return Err(SealedError::NonCanonical); }
        properties.extend(chunk);
    }
    if input.at != bytes.len() { return Err(SealedError::TrailingBytes); }
    checkpoint()?;
    Ok(Image { dictionary, offsets, rows, inlines, runs, properties, incidences: incidence_count })
}
