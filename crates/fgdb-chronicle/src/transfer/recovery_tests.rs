...
use crate::symbolize::{decode_object, blocks::repair_encoder};
...
impl Fixture {
    fn new(blocks: usize) -> Self {
        let plaintext: Vec<u8> = (0..blocks * 2 * SIZE - 16).map(|i| (i % 251) as u8).collect();
        ...
        let protected = object.protect(&DEK, CipherDescriptor {
            ...
            data_crypto_profile: 1, dek_id: [9; 16], object_nonce: [31; 24], object_tag_len: 16,
        }, &plaintext).unwrap();
        ...
        Self { encoding, protected: bytes, plaintext, blocks }
    }

    fn target(&self) -> RecoveryTarget<'static> {
        RecoveryTarget { k_oid: &KEY, namespace: namespace(), object_id: self.encoding.object_id(),
            canonical_header: HEADER, protected_len: self.protected.len() }
    }
    ...
    fn record(&self, block: u32, esi: u32) -> Vec<u8> {
        ...
        if esi < 2 {
            layout.copy_source_symbol(&self.protected, block, esi, &mut payload).unwrap();
        } else {
            let mut source = vec![vec![0; SIZE]; 2];
            for (index, symbol) in source.iter_mut().enumerate() {
                layout.copy_source_symbol(&self.protected, block, index as u32, symbol).unwrap();
            }
            payload = repair_encoder(&self.encoding, &source).unwrap().try_repair_symbol(esi).unwrap();
        }
    ...
    fn limits(&self, records: usize) -> PullLimits {
        PullLimits { max_symbols: records, max_in_flight: 1,
            max_wire_bytes: records * self.record(0, 0).len(), max_requests: 1_000_000,
            max_decode_attempts: 3, ..PullLimits::default() }
    }
    ...
    fn initial(&self, pull: &mut BondedPull<'_>, deficient: &[u32], pair: (u32, u32)) {
        while (0..self.blocks).any(|block| pull.block_symbol_count(block as u32) != Some(2)) {
            ...
            if keep {
                pull.accept_reply(request, &self.record(request.source_block, request.esi), &mut Vec::new()).unwrap();
            } else {
    ...
    fn rescue(&self, block: u32, pair: (u32, u32)) -> u32 {
        ...
        for number in 0..self.blocks as u32 {
            for esi in if number == block { [pair.0, pair.1] } else { [0, 1] } {
                records.push(self.record(number, esi));
            }
        }
        assert_eq!(decode_object(&self.encoding, &records, self.target(), &DEK, &mut Vec::new()),
            Err(SymbolizeError::InsufficientSymbols));
        for esi in pair.1 + 1..pair.1 + 257 {
            records.push(self.record(block, esi));
            let result = decode_object(&self.encoding, &records, self.target(), &DEK, &mut Vec::new());
            records.pop();
            match result {
                Ok(bytes) => { assert_eq!(bytes, self.plaintext); return esi; }
                Err(SymbolizeError::InsufficientSymbols) => {}
    ...
    fn deliver(&self, pull: &mut BondedPull<'_>, block: u32, esi: u32) {
        loop {
            let request = pull.schedule(1).unwrap()[0];
            assert_eq!(request.source_block, block, "a satisfied block must not consume the last slot");
            assert!(request.esi <= esi);
            if request.esi == esi {
                pull.accept_reply(request, &self.record(block, esi), &mut Vec::new()).unwrap();
                break;
            }
            pull.expire(request).unwrap();
        }
...
fn one_rank_deficient_block_can_use_the_last_object_wide_record_slot() {
    ...
    for bad in 0..2 {
        let mut pull = BondedPull::new(&fixture.encoding, fixture.target(), &DEK,
            &[DonorId(1)], fixture.limits(5)).unwrap();
        ...
        assert_eq!(pull.try_recover(&mut events).unwrap().unwrap().plaintext(), fixture.plaintext);
        assert_eq!(events.iter().filter(|event| event.operation == VerificationOperation::SymbolRecord).count(), 3);
        assert_eq!(pull.recovery.passes[1 - bad as usize], 1, "unchanged healthy block not decoded again");
...
fn unchanged_or_duplicate_input_does_not_retry_a_failed_round() {
    ...
    assert!(pull.try_recover(&mut Vec::new()).unwrap().is_none());
    for _ in 0..32 {
        assert_eq!(pull.accept(DonorId(1), &fixture.record(0, 0), &mut Vec::new()).unwrap(), SymbolAdmission::Duplicate);
        let mut events = Vec::new();
...
fn every_deficient_block_must_improve_before_another_object_round() {
    ...
    while pull.block_symbol_count(0) != Some(3) || held.is_none() {
        ...
        if request.esi == chosen {
            if request.source_block == 2 {
                ...
                pull.accept_reply(request, &fixture.record(0, chosen), &mut Vec::new()).unwrap();
            }
        } else {
    ...
    pull.accept_reply(request, &fixture.record(2, request.esi), &mut Vec::new()).unwrap();
...
fn donor_failure_and_reactivation_preserve_reconstructed_blocks_and_spent_work() {
    ...
    assert_eq!((pull.requests, pull.verifications, pull.decode_attempts()), attempts);
    assert_eq!(pull.recovered_block_count(), 1);
...
fn paused_block_work_resumes_without_a_second_round_or_repeated_authentication() {
    ...
    for steps in 0..=5 {
        let mut pull = BondedPull::new(&fixture.encoding, fixture.target(), &DEK,
            &[DonorId(1)], fixture.limits(10)).unwrap();
        fixture.initial(&mut pull, &[], (0, 0));
        for _ in 0..steps {
            assert!(matches!(pull.advance_recovery(&mut Vec::new()).unwrap(), recovery::Advance::Progress));
        }
        assert_eq!(pull.recovered_block_count(), steps);
        let mut events = Vec::new();
        assert_eq!(pull.try_recover(&mut events).unwrap().unwrap().plaintext(), fixture.plaintext);
        assert_eq!(events.iter().filter(|event| event.operation == VerificationOperation::SymbolRecord).count(), 2 * (5 - steps));
...
fn late_extra_equation_invalidates_only_its_own_cached_block() {
    ...
    for request in requests.into_iter().filter(|request| request.esi < 2) {
        pull.accept_reply(request, &fixture.record(request.source_block, request.esi), &mut Vec::new()).unwrap();
    }
    assert!(matches!(pull.advance_recovery(&mut Vec::new()).unwrap(), recovery::Advance::Progress));
    assert_eq!(pull.recovered_block_count(), 1);
    pull.accept_reply(late, &fixture.record(0, 2), &mut Vec::new()).unwrap();
    assert_eq!(pull.recovered_block_count(), 0);
    assert_eq!(pull.decode_attempts(), 1, "interrupted work is not refunded");
    assert_eq!(pull.try_recover(&mut Vec::new()).unwrap().unwrap().plaintext(), fixture.plaintext);
...
fn cached_blocks_cannot_bypass_the_final_namespace_identity_boundary() {
    ...
    let mut pull = BondedPull::new(&fixture.encoding, target, &DEK,
        &[DonorId(1)], fixture.limits(5)).unwrap();
    fixture.initial(&mut pull, &[1], pair);
    assert!(pull.try_recover(&mut Vec::new()).unwrap().is_none());
    fixture.deliver(&mut pull, 1, fixture.rescue(1, pair));
...
fn observation_panics_fence_the_pull_at_symbol_open_and_completion_boundaries() {
    let fixture = Fixture::new(2);
    for operation in [VerificationOperation::SymbolRecord,
        VerificationOperation::RecoveredObjectOpen, VerificationOperation::ObjectRecovery] {
        let mut pull = BondedPull::new(&fixture.encoding, fixture.target(), &DEK,
            &[DonorId(1)], fixture.limits(4)).unwrap();
        fixture.initial(&mut pull, &[], (0, 0));
        assert!(catch_unwind(AssertUnwindSafe(|| pull.try_recover(&mut PanicAt(operation)))).is_err());
        assert_eq!(pull.decode_attempts(), 1);
        assert!(matches!(pull.try_recover(&mut Vec::new()), Err(PullError::Closed)));
...
fn round_limit_is_object_wide_and_cannot_be_reset_by_donor_reactivation() {
    ...
    pull.donor_available(DonorId(1)).unwrap();
    for _ in 0..8 {
        assert!(matches!(pull.try_recover(&mut Vec::new()), Err(PullError::DecodeBudget)));
    }
    assert_eq!(pull.recovery.passes, [1, 1]);
    assert_eq!(pull.recovered_block_count(), 1);
}
