...
impl Fixture {
    fn new(total: usize, size: usize, blocks: usize, subblocks: usize, padding: usize) -> Self {
        ...
        let object = IdentifiedObject::new(&KEY, DatabaseSecurityNamespaceId([9; 32]), 2, HEADER, &plaintext);
        ...
        let encoding = protected.encode(EncodingDescriptor {
            fec_profile: 1, transfer_length: bytes as u64,
        ...
        for block in 0..blocks {
            ...
            for (esi, symbol) in source.into_iter().enumerate() {
                records.push(SymbolRecord::for_encoding(&encoding, block as u32, esi as u32, 0, symbol)
                    .serialize(&encoding.symbol_auth_key(&DEK)));
            }
        }
        Self { encoding, protected, plaintext, records }
    }
    ...
    fn restore(&self, records: &[Vec<u8>], native: &mut Vec<u32>) -> Result<Vec<u8>, SymbolizeError> {
        decode_protected_observed(&self.encoding, records, self.protected.len(), &DEK,
            &mut Vec::new(), |block| native.push(block))
    }
}
...
fn complete_originals_restore_unequal_subblocks_without_any_native_decoder() {
    for (total, size, blocks, subblocks, padding) in [
        (17, 64, 1, 1, 7), (17, 64, 3, 3, 7), (31, 32, 5, 7, 3), (255, 4, 255, 1, 0),
    ] {
        ...
        assert_eq!(fixture.restore(&records, &mut native).unwrap(), fixture.protected);
        assert!(native.is_empty());
        assert_eq!(decode_object(&fixture.encoding, &records, fixture.target(), &DEK,
            &mut Vec::new()).unwrap(), fixture.plaintext);
    }
}
...
fn complete_source_path_and_actual_foundation_decoder_agree_in_same_invocation() {
    ...
    for number in 0..layout.blocks() {
        ...
        let decoder = InactivationDecoder::try_new(block.symbols, layout.symbol_size, code_seed(&fixture.encoding)).unwrap();
    ...
    let mut native = Vec::new();
    assert_eq!(fixture.restore(&fixture.records, &mut native).unwrap(), reference);
...
fn only_erased_blocks_use_native_reconstruction() {
    let fixture = Fixture::new(17, 64, 3, 3, 7);
    let layout = Layout::new(&fixture.encoding, fixture.protected.len()).unwrap();
    let mut records: Vec<_> = fixture.records.iter().filter(|raw| {
        let record = fixture.record(raw);
...
fn source_presence_not_equation_count_controls_the_shortcut() {
    for k in 1..=6 {
        for mask in 0..(1usize << (k + 2)) {
            let group: BTreeMap<_, _> = (0..k + 2).filter(|esi| mask & (1 << esi) != 0)
                .map(|esi| (esi as u32, (0, Vec::new()))).collect();
            assert_eq!(complete_systematic(&group, k), mask == (1 << k) - 1);
        }
    }
    let fixture = Fixture::new(17, 64, 3, 3, 7);
    let mut records: Vec<_> = fixture.records.iter().filter(|raw| fixture.record(raw).source_block != 2)
        .cloned().collect();
    records.extend_from_slice(&fixture.records[..3]);
    let mut native = Vec::new();
    assert_eq!(fixture.restore(&records, &mut native), Err(SymbolizeError::InsufficientSymbols));
    assert!(native.is_empty());
}
...
fn trailing_bad_mac_and_authenticated_conflicts_are_not_skipped() {
    ...
    *bad.last_mut().unwrap() ^= 1;
    records.push(bad);
    let mut native = Vec::new();
    assert!(matches!(fixture.restore(&records, &mut native), Err(SymbolizeError::Symbol(_))));
    assert!(native.is_empty());
    records.pop();
    ...
    assert_eq!(fixture.restore(&records, &mut native), Err(SymbolizeError::DecodeFailed));
    assert!(native.is_empty());
}
...
fn direct_sources_still_require_zero_padding_and_whole_object_authentication() {
    ...
    *records.last_mut().unwrap() = padding.serialize(&fixture.encoding.symbol_auth_key(&DEK));
    ...
    assert!(matches!(decode_object(&fixture.encoding, &records, fixture.target(), &DEK, &mut Vec::new()),
        Err(SymbolizeError::AuthenticationFailed | SymbolizeError::CiphertextIdentityMismatch)));
    assert_eq!(decode_object(&fixture.encoding, &fixture.records,
        RecoveryTarget { namespace: DatabaseSecurityNamespaceId([8; 32]), ..fixture.target() },
        &DEK, &mut Vec::new()), Err(SymbolizeError::IdentityMismatch));
}
...
fn thousands_of_original_symbols_round_trip_without_encoder_or_decoder_matrices() {
    ...
    let mut donor = BondedDonor::new(&fixture.encoding, &fixture.protected, fixture.target(), &DEK,
        DonorId(7), &[DonorId(7)], DonorLimits { max_repair_source_symbols: 0,
            max_matrix_cells: 0, max_cached_encoder_bytes: 0, max_encoder_builds: 0,
            ..DonorLimits::default() }, &mut Vec::new(), || Ok::<(), ()>(())).unwrap();
    let mut records = Vec::new();
    for esi in (0..4096).rev() {
        records.push(donor.respond(PullRequest { donor: DonorId(7),
            object_id: fixture.encoding.object_id(), encoding_id: fixture.encoding.encoding_id(),
            source_block: 0, esi }, || Ok::<(), ()>(())).unwrap());
    }
    assert_eq!(donor.usage().encoder_builds, 0);
    let protected = decode_protected_observed(&fixture.encoding, &records, fixture.protected.len(),
        &DEK, &mut Vec::new(), |_| panic!("complete original symbols must not allocate an erasure decoder")).unwrap();
