...
use fgdb_chronicle::transfer::{
    BondedPull, DonorId, PullError, PullLimits, PullRequest, SymbolAdmission,
};
    ...
    for blocks in [2, 3, 5] {
        ...
        for maximum in [1, 2, 7, 64] {
            let limits = PullLimits {
                max_source_symbols: total,
                max_symbols: total,
                max_in_flight: total,
                max_wire_bytes: total * f.records[0][0].len(),
                max_requests: total as u64,
                max_verifications: total as u64,
                max_esi: f.sources[0] as u32 - 1,
                ..PullLimits::default()
            };
            ...
            for _ in 0..total {
                ...
                for request in requests.into_iter().rev() {
                    ...
                    p.accept_reply(request, bytes(&f, request), &mut Vec::new())
                        .unwrap();
                }
                if p.symbol_count() == total {
                    break;
                }
            }
            for (block, count) in f.sources.iter().enumerate() {
                ...
                for esi in 0..*count {
                    assert!(coordinates.contains(&(block as u32, esi as u32)));
                }
            }
            assert!(p.schedule(1).unwrap().is_empty());
            assert_eq!(
                p.try_recover(&mut Vec::new()).unwrap().unwrap().plaintext(),
                f.plaintext
            );
...
fn donor_residue_streams_are_independent_in_every_block() {
    ...
    for block in 0..3 {
        let got: BTreeSet<_> = requests
            .iter()
            .filter(|r| r.source_block == block)
            .map(|r| r.esi)
            .collect();
...
fn exact_reply_binding_does_not_steal_equal_esi_from_another_block() {
    ...
    assert!(matches!(
        p.accept_reply(requests[0], bytes(&f, requests[1]), &mut Vec::new()),
        Err(PullError::UnrequestedSymbol)
    ));
    assert_eq!(p.pending_count(), 3);
    assert_eq!(p.symbol_count(), 0);
    p.accept_reply(requests[0], bytes(&f, requests[0]), &mut Vec::new())
        .unwrap();
    ...
    assert_eq!(p.pending_count(), 2);
    p.accept_reply(requests[1], bytes(&f, requests[1]), &mut Vec::new())
        .unwrap();
...
fn expiration_and_bad_mac_leave_other_blocks_credit_intact() {
    ...
    *bad.last_mut().unwrap() ^= 1;
    assert!(matches!(
        p.accept_reply(requests[1], &bad, &mut Vec::new()),
    ...
    for request in p.schedule(8).unwrap() {
        assert_ne!(
            (request.source_block, request.esi),
            (requests[0].source_block, requests[0].esi)
        );
    }
}
...
fn contradictory_authenticated_symbols_do_not_increment_block_progress() {
    ...
    p.accept_reply(request, bytes(&f, request), &mut Vec::new())
        .unwrap();
    let mut record =
        SymbolRecord::verify(bytes(&f, request), &f.encoding, &DEK, &mut Vec::new()).unwrap();
    record.payload[0] ^= 1;
    let conflict = record.serialize(&f.encoding.symbol_auth_key(&DEK));
    assert!(matches!(
        p.accept_reply(request, &conflict, &mut Vec::new()),
...
fn requests_and_mac_work_are_single_object_budgets_across_blocks() {
    ...
    *bad.last_mut().unwrap() ^= 1;
    for _ in 0..total {
        assert!(matches!(
            p.accept(DonorId(1), &bad, &mut Vec::new()),
            Err(PullError::Symbol(_))
        ));
    }
    assert!(matches!(
...
fn donor_reactivation_preserves_all_block_streams_and_accepted_equations() {
    ...
    let first = initial[0];
    p.accept_reply(first, bytes(&f, first), &mut Vec::new())
        .unwrap();
    p.donor_failed(DonorId(1)).unwrap();
    for request in &initial {
        if request.donor == DonorId(2) {
            p.expire(*request).unwrap();
        }
    }
    assert_eq!(p.pending_count(), 0);
...
fn decoder_shape_can_span_many_blocks_without_multiplying_object_limits() {
    ...
    let target = fgdb_chronicle::symbolize::RecoveryTarget {
        ...
        canonical_header: support::HEADER,
    ...
    let limits = PullLimits {
        max_source_symbols: total,
        max_symbols: total,
        max_wire_bytes: total * 256,
        max_esi: total.div_ceil(2) as u32 - 1,
        ..PullLimits::default()
    };
