...
use fgdb_chronicle::seed::{ReplicaSeed, SeedAnchor, SeedError, SeedLimits, SeedObjectSpec, SeedPlan};
...
fn specs(objects: &[Fixture]) -> Vec<SeedObjectSpec> {
    objects.iter().map(|fixture| SeedObjectSpec {
...
fn every_object_barrier_and_exact_root_barrier_precede_completion() {
    ...
    assert!(matches!(seed.begin_install(), Err(SeedError::MissingObjects { remaining: 4 })));
    for (offset, object) in objects.iter().enumerate() {
        ...
        assert!(matches!(seed.begin_install(), Err(SeedError::AwaitingObjectPublication)));
        assert_eq!(seed.object_published(id).unwrap(), object.encoding.object_id());
    }
    assert!(seed.completion().is_none());
    ...
    let complete = seed.finish_install(id, &evidence(&expected_anchor)).unwrap();
...
fn partial_closure_never_produces_installation_even_when_state_root_exists() {
    ...
    seed.object_published(id).unwrap();
    assert!(matches!(seed.begin_install(), Err(SeedError::MissingObjects { remaining: 3 })));
    assert!(seed.missing_objects().any(|object| object.object_id == objects[2].encoding.object_id()));
    assert!(seed.completion().is_none());
}
...
fn cross_session_stale_and_wrong_kind_tokens_do_not_release_publication() {
    ...
    assert!(matches!(second.object_published(a.clone()), Err(SeedError::StalePublication)));
    first.object_published(a.clone()).unwrap();
    second.object_published(b).unwrap();
    let next = first.stage(objects[1].verified()).unwrap().id();
    assert!(matches!(first.object_published(a.clone()), Err(SeedError::StalePublication)));
    ...
    assert!(matches!(first.finish_install(a, &evidence(&anchor(&objects))), Err(SeedError::StalePublication)));
    first.finish_install(install, &evidence(&anchor(&objects))).unwrap();
}
...
fn cancelled_object_publication_remains_pending_and_failure_requires_recovery() {
    ...
    assert!(matches!(seed.stage(objects[1].verified()), Err(SeedError::AwaitingObjectPublication)));
    seed.publication_failed();
    assert!(matches!(seed.object_published(id), Err(SeedError::RecoveryRequired)));
    assert!(matches!(seed.pending_publication(), Err(SeedError::RecoveryRequired)));
    assert!(matches!(seed.begin_install(), Err(SeedError::RecoveryRequired)));
    assert!(seed.completion().is_none());
}
...
fn wrong_namespace_kind_length_and_unlisted_objects_never_become_durable() {
    ...
    let wrong_plan = SeedPlan::from_authenticated_inventory(wrong_anchor, specs(&objects), SeedLimits::default()).unwrap();
    let mut seed = ReplicaSeed::new(wrong_plan);
    assert!(matches!(seed.stage(objects[0].verified()), Err(SeedError::WrongNamespace)));
    assert_eq!(seed.published_count(), 0);
    ...
    let mut seed = ReplicaSeed::new(SeedPlan::from_authenticated_inventory(
        anchor(&objects), incorrect, SeedLimits::default(),
    ).unwrap());
    assert!(matches!(seed.stage(objects[0].verified()), Err(SeedError::KindMismatch)));
    ...
    assert!(matches!(seed.stage(objects[0].verified()), Err(SeedError::LengthMismatch)));
    assert!(matches!(seed.stage(Fixture::new(80).verified()), Err(SeedError::UnexpectedObject)));
    assert_eq!(seed.published_count(), 0);
}
...
fn duplicate_verified_delivery_does_not_change_inventory_or_progress() {
    ...
    let id = seed.stage(objects[0].verified()).unwrap().id();
    seed.object_published(id).unwrap();
    assert!(matches!(seed.stage(objects[0].verified()), Err(SeedError::AlreadyPublished)));
...
fn installation_retry_keeps_one_cut_and_rejects_wrong_root_evidence() {
    ...
    assert!(matches!(seed.stage(objects[0].verified()), Err(SeedError::InstallPending)));
    let mut wrong = evidence(&expected);
    wrong.root_manifest_oid[0] ^= 1;
    assert!(matches!(seed.finish_install(id.clone(), &wrong), Err(SeedError::RootEvidenceMismatch)));
...
fn failed_final_publication_never_reports_an_installed_replica() {
    ...
    let id = seed.begin_install().unwrap().id();
    seed.publication_failed();
    assert!(matches!(seed.finish_install(id, &evidence(&anchor(&objects))), Err(SeedError::RecoveryRequired)));
    assert!(seed.completion().is_none());
...
fn inventory_admission_rejects_missing_roots_duplicates_and_budget_overflow() {
    ...
    assert!(matches!(SeedPlan::from_authenticated_inventory(anchor(&objects), duplicate, SeedLimits::default()),
        Err(SeedError::DuplicateObject)));
    assert!(matches!(SeedPlan::from_authenticated_inventory(anchor(&objects), specs(&objects)[..3].iter().copied(), SeedLimits::default()),
        Err(SeedError::MissingRoot)));
    assert!(matches!(SeedPlan::from_authenticated_inventory(anchor(&objects), specs(&objects), SeedLimits {
        max_objects: 3, ..SeedLimits::default()
    }), Err(SeedError::ObjectCountBudget)));
    assert!(matches!(SeedPlan::from_authenticated_inventory(anchor(&objects), specs(&objects), SeedLimits {
        max_object_bytes: 4095, ..SeedLimits::default()
    }), Err(SeedError::ObjectSizeBudget)));
    let huge = [
        SeedObjectSpec { object_id: ObjectId([1; 32]), object_kind: KIND, compressed_len: u64::MAX },
        SeedObjectSpec { object_id: ObjectId([2; 32]), object_kind: KIND, compressed_len: 1 },
    ];
    assert!(matches!(SeedPlan::from_authenticated_inventory(anchor(&objects), huge, SeedLimits {
        max_objects: 4, max_object_bytes: u64::MAX, max_total_bytes: u64::MAX,
...
fn snapshot_cut_requires_a_term_and_destination_generation() {
    ...
    bad.raft_term = 0;
    assert!(matches!(SeedPlan::from_authenticated_inventory(bad, specs(&objects), SeedLimits::default()),
