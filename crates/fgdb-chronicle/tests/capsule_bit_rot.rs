...
fn an_undamaged_capsule_reads_back_exactly() {
    ...
    under_lab(1, move |cx| {
        ...
        let coordinator = CommitCoordinator::open(&dir, keys()).expect("reopen");
        assert_eq!(coordinator.read_capsule(oid).expect("reads"), plaintext());
...
fn a_single_rotted_symbol_heals_invisibly() {
    ...
    under_lab(2, move |cx| {
        ...
        let coordinator = CommitCoordinator::open(&dir, keys()).expect("reopen");
        assert_eq!(
            coordinator.read_capsule(oid).expect("heals"),
            plaintext(),
...
fn rot_within_the_repair_budget_heals() {
    ...
    under_lab(3, move |cx| {
        ...
        let coordinator = CommitCoordinator::open(&dir, keys()).expect("reopen");
        assert_eq!(
            coordinator.read_capsule(oid).expect("heals at the budget"),
            plaintext(),
...
fn rot_beyond_the_repair_budget_fails_closed() {
    ...
    under_lab(4, move |cx| {
        ...
        let coordinator = CommitCoordinator::open(&dir, keys()).expect("reopen");
        let result = coordinator.read_capsule(oid);
        assert!(
            result.is_err(),
...
fn the_repair_budget_is_the_exact_boundary() {
    ...
    let heals = {
        ...
        under_lab(5, move |cx| {
            ...
            CommitCoordinator::open(&dir, keys())
                .expect("reopen")
                .read_capsule(oid)
                .is_ok()
        })
...
fn rot_in_the_container_header_is_refused() {
    ...
    under_lab(7, move |cx| {
        ...
        let coordinator = CommitCoordinator::open(&dir, keys()).expect("reopen");
        assert!(
            coordinator.read_capsule(oid).is_err(),
...
fn healed_bytes_still_recompute_their_identity() {
    ...
    under_lab(9, move |cx| {
        ...
        let coordinator = CommitCoordinator::open(&dir, keys()).expect("reopen");
        let healed = coordinator.read_capsule(oid).expect("heals");
        assert_eq!(
...
fn healing_a_read_does_not_repair_the_file() {
    ...
    under_lab(10, move |cx| {
        ...
        let coordinator = CommitCoordinator::open(&dir, keys()).expect("reopen");
        assert_eq!(coordinator.read_capsule(oid).expect("heals"), plaintext());
        assert_eq!(
            coordinator.read_capsule(oid).expect("heals again"),
            plaintext()
        );
        assert_eq!(
