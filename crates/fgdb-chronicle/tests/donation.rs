    ...
    for (blocks, subblocks) in [(1, 1), (2, 1), (3, 3), (5, 7)] {
        ...
        for block in 0..blocks {
            ...
            for esi in (0..batch.len()).rev() {
                let record = donor
                    .respond(fixture.request(7, block as u32, esi as u32), check)
                    .unwrap();
                assert_eq!(record, batch[esi]);
                let symbol =
                    SymbolRecord::verify(&record, &fixture.encoding, &DEK, &mut Vec::new())
                        .unwrap();
                assert_eq!(
                    (symbol.source_block, symbol.esi),
                    (block as u32, esi as u32)
                );
                if esi < sources.len() {
                    assert_eq!(symbol.payload, sources[esi]);
                }
            }
        }
...
fn maximum_esi_is_one_response_not_an_enormous_generated_prefix() {
    ...
    assert_eq!(
        record.payload,
        native.try_repair_symbol(request.esi).unwrap()
    );
    ...
    assert_eq!(
        donor.usage().charged_wire_bytes,
        3 * donor.record_len() as u64
    );
}
...
fn complete_object_authentication_precedes_all_donation() {
    ...
    assert!(matches!(
        ...
        Err(DonorError::Encoding(
            SymbolizeError::AuthenticationFailed | SymbolizeError::CiphertextIdentityMismatch
        ))
    ));
    for target in [
        RecoveryTarget {
...
fn object_and_roster_refusals_precede_cryptographic_work() {
    ...
    for limits in [
        DonorLimits {
            max_protected_bytes: fixture.protected.len() - 1,
            ..DonorLimits::default()
        },
        DonorLimits {
            max_identity_header_bytes: HEADER.len() - 1,
            ..DonorLimits::default()
        },
    ] {
        assert!(matches!(
            ...
            Err(DonorError::ObjectBudget)
        ));
    }
    for roster in [
        vec![],
        vec![DonorId(0)],
        vec![DonorId(7), DonorId(7)],
        vec![DonorId(8)],
    ] {
        assert!(matches!(
            BondedDonor::new(
            ...
            Err(DonorError::InvalidDonors)
        ));
    }
    assert!(verification.is_empty());
    let exact = DonorLimits {
        max_protected_bytes: fixture.protected.len(),
        max_identity_header_bytes: HEADER.len(),
        ..DonorLimits::default()
    };
...
fn every_request_coordinate_and_fixed_donor_residue_is_checked() {
    let fixture = Fixture::new(3, 3);
    let mut donor = fixture.donor(
        DonorId(9),
        &[DonorId(7), DonorId(9), DonorId(11)],
        DonorLimits::default(),
    );
    ...
    for request in [
        ...
        assert_eq!(
            donor.respond(request, check),
            Err(DonorError::ForeignRequest)
        );
    }
    assert_eq!(
        donor.respond(PullRequest { esi: 0, ..valid }, check),
        Err(DonorError::UnownedEsi)
    );
...
fn original_sources_need_no_repair_matrix_or_cache() {
    ...
    assert_eq!(
        donor.respond(fixture.request(7, 0, 99), check),
        Err(DonorError::EncoderShapeBudget)
    );
}
...
fn request_and_record_byte_budgets_include_retries() {
    ...
    assert_eq!(
        donor.respond(request, check),
        Err(DonorError::RequestBudget)
    );
    let mut donor = fixture.donor(
        DonorId(7),
        &[DonorId(7)],
        DonorLimits {
            max_wire_bytes: bytes.len() as u64,
            ..DonorLimits::default()
        },
    );
...
fn exact_encoder_shape_and_cache_admission_precede_builds() {
    ...
    let exact = DonorLimits {
        max_matrix_cells: p.l * p.l,
        max_repair_source_symbols: k,
        max_cached_encoder_bytes: charge,
        ..DonorLimits::default()
    };
    ...
    assert_eq!(
        donor.respond(fixture.request(7, 1, 99), check),
        Err(DonorError::EncoderCacheBudget)
    );
    assert_eq!(donor.usage().encoder_builds, 1);
    for (limits, expected) in [
        (
            DonorLimits {
                max_matrix_cells: p.l * p.l - 1,
                ..exact
            },
            DonorError::EncoderShapeBudget,
        ),
        (
            DonorLimits {
                max_repair_source_symbols: k - 1,
                ..exact
            },
            DonorError::EncoderShapeBudget,
        ),
        (
            DonorLimits {
                max_cached_encoder_bytes: charge - 1,
                ..exact
            },
            DonorError::EncoderCacheBudget,
        ),
    ] {
        let mut donor = fixture.donor(DonorId(7), &[DonorId(7)], limits);
        assert_eq!(
...
fn explicit_cache_release_does_not_refund_build_or_wire_work() {
    ...
    assert_eq!(
        donor.respond(request, check),
        Err(DonorError::EncoderBuildBudget)
    );
...
fn every_response_checkpoint_can_refuse_without_returning_partial_bytes() {
    ...
    for esi in [0, 99] {
        ...
        for stop in 1..=total {
            ...
            let mut calls = 0;
            assert_eq!(
                donor.respond(request, || {
                    calls += 1;
                    if calls == stop {
                        Err("revoked/cancelled")
                    } else {
                        Ok(())
                    }
                }),
    ...
    assert!(
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let _ = donor.respond(fixture.request(7, 0, 99), || {
                calls += 1;
                assert_ne!(calls, 3, "interrupt admitted encoder preparation");
                check()
            });
        }))
        .is_err()
    ...
    for blocks in [1, 3, 5] {
        ...
        for _ in 0..128 {
            for request in pull.schedule_bonded(12, 12).unwrap() {
                let donor = if request.donor == roster[0] {
                    &mut first
                } else {
                    &mut second
                };
                let record = donor.respond(request, check).unwrap();
                pull.accept_reply(request, &record, &mut Vec::new())
                    .unwrap();
            }
        ...
        assert_eq!(
            recovered
                .expect("healthy donors generate fresh repair equations")
                .plaintext(),
            fixture.plaintext
        );
