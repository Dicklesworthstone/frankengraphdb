...
fn recover(fixture: &Fixture, records: &[Vec<u8>]) -> Result<Vec<u8>, SymbolizeError> {
    decode_object(&fixture.encoding, records, fixture.target(), &DEK, &mut Vec::new())
}
    ...
    for blocks in [2, 3, 5] {
        for subs in [1, 3, 8] {
            ...
            for block in 0..usize::from(blocks) {
                let encoded = encode_object(&fixture.encoding, &fixture.protected, KIND, block as u32, 8, &DEK).unwrap();
...
fn each_blocks_missing_source_uses_only_its_own_repair_equations() {
    ...
    for missing in 0..fixture.sources[0] {
        let records: Vec<_> = fixture.records.iter().flat_map(|block| {
            block.iter().enumerate().filter(|(esi, _)| *esi != missing).map(|(_, record)| record.clone())
        }).collect();
        assert_eq!(recover(&fixture, &records).unwrap(), fixture.plaintext);
    }
}
...
fn surplus_and_duplicate_symbols_cannot_replace_a_missing_block() {
    ...
    assert_eq!(recover(&fixture, &records), Err(SymbolizeError::InsufficientSymbols));
}
...
fn all_records_authenticate_even_when_an_earlier_block_is_missing() {
    ...
    *records.last_mut().unwrap().last_mut().unwrap() ^= 1;
    assert!(matches!(recover(&fixture, &records), Err(SymbolizeError::Symbol(_))));
}
...
fn authenticated_conflicting_equations_and_foreign_blocks_fail_closed() {
    ...
    let mut record = SymbolRecord::verify(&fixture.records[1][0], &fixture.encoding, &DEK, &mut Vec::new()).unwrap();
    record.payload[0] ^= 1;
    let mut records = fixture.all();
    records.push(record.serialize(&key));
    assert_eq!(recover(&fixture, &records), Err(SymbolizeError::DecodeFailed));
    record.source_block = 3;
    assert_eq!(recover(&fixture, &[record.serialize(&key)]), Err(SymbolizeError::InvalidParameters));
}
...
fn substituted_block_labels_cannot_authenticate_as_the_original_object() {
    ...
    for raw in &mut records {
        ...
        if record.source_block < 2 { record.source_block = 1 - record.source_block; }
...
fn complete_reassembly_still_requires_namespace_and_logical_identity() {
    ...
    assert_eq!(decode_object(&fixture.encoding, &records, wrong, &DEK, &mut Vec::new()), Err(SymbolizeError::IdentityMismatch));
    wrong = fixture.target();
    wrong.namespace.0[0] ^= 1;
    assert_eq!(decode_object(&fixture.encoding, &records, wrong, &DEK, &mut Vec::new()), Err(SymbolizeError::IdentityMismatch));
    wrong = fixture.target();
    wrong.protected_len -= 1;
    assert_eq!(decode_object(&fixture.encoding, &records, wrong, &DEK, &mut Vec::new()), Err(SymbolizeError::InvalidParameters));
}
...
fn oti_disagreement_and_impossible_shapes_fail_before_symbol_materialization() {
    for mutation in 0..11 {
        ...
        assert_eq!(encode_object(&encoding, &protected, KIND, 0, 0, &DEK), Err(SymbolizeError::InvalidParameters));
        let target = RecoveryTarget { k_oid: &KEY, namespace: namespace(), object_id: encoding.object_id(),
            canonical_header: HEADER, protected_len: protected.len() };
        assert_eq!(decode_object(&encoding, &[], target, &DEK, &mut Vec::new()), Err(SymbolizeError::InvalidParameters));
    }
}
...
fn invalid_selected_blocks_and_esi_exhaustion_fail_before_encoding() {
    ...
    for block in [3, u32::MAX] {
        assert_eq!(encode_object(&fixture.encoding, &fixture.protected, KIND, block, 0, &DEK), Err(SymbolizeError::InvalidParameters));
    }
    assert_eq!(encode_object(&fixture.encoding, &fixture.protected, KIND, 0, 0x0100_0000, &DEK), Err(SymbolizeError::InvalidParameters));
    ...
    assert_eq!(encode_object(&fixture.encoding, &fixture.protected[..256], KIND, 0, 0, &DEK), Err(SymbolizeError::InvalidParameters));
}
...
fn source_ceiling_applies_per_block_not_to_the_whole_object() {
    for blocks in [1, 2] {
        ...
        let expected = if blocks == 1 { SymbolizeError::InvalidParameters } else { SymbolizeError::InsufficientSymbols };
        assert_eq!(decode_object(&encoding, &[], target, &DEK, &mut Vec::new()), Err(expected));
    }
}
...
fn one_block_encoded_bytes_keep_the_established_contiguous_interpretation() {
    ...
    for (esi, raw) in fixture.records[0].iter().enumerate() {
        ...
        assert_eq!(&record.payload[..length], &fixture.protected[start..start + length]);
        assert!(record.payload[length..].iter().all(|byte| *byte == 0));
    }
    assert_eq!(recover(&fixture, &fixture.all()).unwrap(), fixture.plaintext);
}
