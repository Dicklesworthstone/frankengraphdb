...
use fgdb_chronicle::seed::{
    ReplicaSeed, SeedAnchor, SeedError, SeedLimits, SeedObjectPull, SeedObjectSpec, SeedPlan,
    SeedPublicationId, SeedPullError,
};
...
fn stage(pull: &mut SeedObjectPull<'_, '_>, object: &Fixture) -> SeedPublicationId {
    for request in pull.schedule(object.sources).unwrap() {
        pull.accept(
            request.donor,
            &object.records[request.esi as usize],
            &mut Vec::new(),
        )
        .unwrap();
    }
...
fn full_seed_bonds_three_donors_and_survives_one_failure_per_object() {
    ...
    for (number, object) in objects.iter().enumerate() {
        ...
        for round in 0..8 {
            ...
            if round == 0 {
                ...
                pull.accept(
                    first.donor,
                    &object.records[first.esi as usize],
                    &mut Vec::new(),
                )
                .unwrap();
        ...
        assert!(matches!(
            pull.schedule(1),
            Err(SeedPullError::Seed(SeedError::AwaitingObjectPublication))
        ));
        assert_eq!(
    ...
    let evidence = RootPublicationEvidence {
        written_index: 1,
        slot_generation: expected.publication_generation,
...
fn target_and_inventory_mismatches_are_rejected_before_a_pull_exists() {
    ...
    for wrong_kind in [true, false] {
        ...
        if wrong_kind {
            assert!(matches!(
                error,
                SeedPullError::Seed(SeedError::KindMismatch)
            ));
        } else {
            assert!(matches!(
                error,
                SeedPullError::Seed(SeedError::LengthMismatch)
            ));
        }
        assert_eq!(seed.published_count(), 0);
    }
...
fn cancelled_publication_keeps_owned_bytes_and_the_same_publication_id() {
    ...
    let id = {
        ...
        assert!(
            ...
            "reacquiring staged bytes must not decode again"
        );
    ...
    assert_eq!(
        seed.pending_publication().unwrap().object().plaintext(),
        object.plaintext
    );
    assert!(matches!(
        seed.begin_pull(
    ...
    assert!(matches!(
        ...
        Err(SeedPullError::Seed(SeedError::AlreadyPublished))
    ));
}
...
fn wrong_session_acknowledgment_leaves_staged_object_recoverable() {
    ...
    let id = stage(&mut pull, object);
    assert!(matches!(
        pull.object_published(other_id),
        Err(SeedPullError::Seed(SeedError::StalePublication))
    ));
    assert_eq!(seed.published_count(), 0);
    assert_eq!(seed.pending_publication().unwrap().id(), id);
    seed.object_published(id).unwrap();
...
fn failed_publication_poisons_seed_instead_of_only_resetting_the_pull() {
    ...
    assert!(matches!(
        ...
        Err(SeedPullError::Seed(SeedError::RecoveryRequired))
    ));
    assert!(seed.completion().is_none());
}
...
fn bad_mac_does_not_consume_credit_and_expired_answers_cannot_reuse_it() {
    ...
    corrupt[end] ^= 1;
    assert!(matches!(
        pull.accept(DonorId(1), &corrupt, &mut Vec::new()),
        Err(SeedPullError::Pull(PullError::Symbol(_)))
    ));
    ...
    assert!(matches!(
        pull.accept(
            DonorId(1),
            &object.records[requests[0].esi as usize],
            &mut Vec::new()
        ),
    ...
    assert_eq!(
        pull.accept(
            DonorId(1),
            &object.records[requests[1].esi as usize],
            &mut Vec::new()
        )
        .unwrap(),
...
fn donor_resume_uses_fresh_equations_and_request_budget_is_not_reset() {
    ...
    let limits = PullLimits {
        max_requests: object.sources as u64,
        ..PullLimits::default()
    };
    let mut pull = seed
        .begin_pull(
    ...
    assert!(matches!(
        pull.schedule(1),
        Err(SeedPullError::Pull(PullError::NoAvailableDonor))
    ));
    ...
    assert!(matches!(
        pull.schedule(1),
        Err(SeedPullError::Pull(PullError::RequestBudget))
    ));
    assert!(pull.try_stage(&mut Vec::new()).unwrap().is_none());
}
...
fn invalid_donors_or_limits_leave_seed_available_for_a_valid_pull() {
    ...
    assert!(matches!(
        ...
        Err(SeedPullError::Pull(PullError::InvalidDonors))
    ));
    assert!(matches!(
        seed.begin_pull(
            &object.encoding,
