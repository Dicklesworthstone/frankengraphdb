...
fn layout(count: usize, maximum: u64, limits: EfPayloadLimits) -> Result<Layout, EfPayloadError> {
    if count > limits.max_entries {
        return Err(EfPayloadError::EntryLimit { count, limit: limits.max_entries });
    }
    ...
    if count == 0 {
        ...
        return Ok(Layout { width: 0, low_bits: 0, low_words: 0, high_bits: 0, high_words: 0, bytes: 0 });
    }
    let ratio = maximum / count as u64;
    let width = if ratio == 0 { 0 } else { u64::BITS - 1 - ratio.leading_zeros() };
    let low_bits = count.checked_mul(width as usize).ok_or(EfPayloadError::SizeOverflow)?;
    let high_bits = (maximum >> width).checked_add(count as u64)
        .and_then(|bits| usize::try_from(bits).ok()).ok_or(EfPayloadError::SizeOverflow)?;
    let low_words = low_bits.div_ceil(64);
    let high_words = high_bits.div_ceil(64);
    let bytes = low_words.checked_add(high_words).and_then(|words| words.checked_mul(8))
        .ok_or(EfPayloadError::SizeOverflow)?;
    if bytes > limits.max_bytes {
        return Err(EfPayloadError::ByteLimit { bytes, limit: limits.max_bytes });
    }
    Ok(Layout { width, low_bits, low_words, high_bits, high_words, bytes })
}

pub fn encoded_len(count: usize, maximum: u64, limits: EfPayloadLimits) -> Result<usize, EfPayloadError> {
    Ok(layout(count, maximum, limits)?.bytes)
}

fn word(bytes: &[u8], index: usize) -> u64 {
    u64::from_le_bytes(bytes[index * 8..index * 8 + 8].try_into().expect("bounded EF word"))
}
...
pub fn encode(encoded: &EliasFano, limits: EfPayloadLimits) -> Result<Vec<u8>, EfPayloadError> {
    ...
    bytes.try_reserve_exact(plan.bytes).map_err(|_| EfPayloadError::AllocationFailed)?;
    bytes.resize(plan.bytes, 0);
    for index in 0..encoded.len() {
        let value = encoded.select(index).expect("scalar EF retains all entries");
        if plan.width != 0 {
        ...
        let position = usize::try_from(value >> plan.width).map_err(|_| EfPayloadError::SizeOverflow)?
            .checked_add(index).ok_or(EfPayloadError::SizeOverflow)?;
        or_word(&mut bytes, plan.low_words + position / 64, 1u64 << (position % 64));
    }
    Ok(bytes)
}

fn validate_padding(bytes: &[u8], base: usize, bits: usize, words: usize) -> Result<(), EfPayloadError> {
...
pub fn decode(
    ...
    if bytes.len() != plan.bytes {
        return Err(EfPayloadError::Length { expected: plan.bytes, found: bytes.len() });
    }
    ...
    values.try_reserve_exact(count).map_err(|_| EfPayloadError::AllocationFailed)?;
    for high_word in 0..plan.high_words {
        ...
        while remaining != 0 {
            ...
            let high = position.checked_sub(index).ok_or(EfPayloadError::InvalidHighBits)?;
...
mod tests {
    ...
    fn limits() -> EfPayloadLimits {
        EfPayloadLimits { max_entries: 10000, max_bytes: 100000 }
    }
    ...
    fn golden_unary_high_word_and_zero_padding() {
        ...
        assert_eq!(decode(&0x4du64.to_le_bytes(), 4, 3, limits()).unwrap(), scalar);
        assert!(matches!(decode(&0xcdu64.to_le_bytes(), 4, 3, limits()), Err(EfPayloadError::NonzeroPadding)));
    }

    #[test]
    fn extreme_and_repeated_sequences_have_one_round_trip() {
        for values in [vec![], vec![0], vec![u64::MAX], vec![0, u64::MAX], vec![42; 1025], (0..1000).collect()] {
            let scalar = EliasFano::try_new(&values, EntryLimit::new(values.len())).unwrap();
            let payload = encode(&scalar, limits()).unwrap();
            let decoded = decode(&payload, values.len(), values.last().copied().unwrap_or(0), limits()).unwrap();
    ...
    fn padding_in_the_low_vector_is_rejected() {
        ...
        assert!(matches!(decode(&bytes, 1, u64::MAX, limits()), Err(EfPayloadError::NonzeroPadding)));
    }
    ...
    fn byte_and_entry_limits_precede_materialization() {
        ...
        let payload = encode(&scalar, limits()).unwrap();
        assert!(matches!(encode(&scalar, EfPayloadLimits { max_entries: 0, ..limits() }), Err(EfPayloadError::EntryLimit { .. })));
        assert!(matches!(decode(&payload, 1, 7, EfPayloadLimits { max_bytes: payload.len() - 1, ..limits() }), Err(EfPayloadError::ByteLimit { .. })));
        assert!(decode(&payload, 1, 7, EfPayloadLimits { max_bytes: payload.len(), ..limits() }).is_ok());
    }
    ...
    fn deterministic_generated_sequences_round_trip_across_word_boundaries() {
        ...
        for count in 0..256 {
            ...
            assert_eq!(decode(&bytes, count, values.last().copied().unwrap_or(0), limits()).unwrap(), scalar);
        }
    }
}
