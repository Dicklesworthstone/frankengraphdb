...
fn batch(seq: u64) -> LogicalDeltaBatch {
    LogicalDeltaBatch::from_parts_for_test(vec![], [seq as u8; 32],
...
fn boundary_identity_survives_empty_window_noop_clone_and_future_insertion() {
    ...
    for at in 1..=3 { index.insert(batch(at)).unwrap(); }
    let expected = (batch(2).format(), batch(2).commit_marker_identity(), [2; 32]);
    ...
    assert!(matches!(index.since(CommitSeq(2)), Err(IndexError::CursorRetired { .. })));
    index.verify().unwrap();
}
...
fn bare_floor_and_malformed_boundaries_cannot_mint_retired_evidence() {
    ...
    for entries in [vec![], vec![(CommitSeq(2), batch(1))]] {
        let mut malformed = LocalDeltaBatchIndex::from_parts_for_test(CommitSeq(0), CommitSeq(2), entries);
    ...
    let bad = LogicalDeltaBatch::from_parts_for_test(vec![], [2; 32],
        batch(1).commit_marker_identity(), CommitSeq(2), CommitSeq(2));
    let mut malformed = LocalDeltaBatchIndex::from_parts_for_test(CommitSeq(0), CommitSeq(2),
        vec![(CommitSeq(1), batch(1)), (CommitSeq(2), bad)]);
    let before = malformed.clone();
    assert!(matches!(malformed.retire_prefix(CommitSeq(2)), Err(IndexError::WrongMarker { .. })));
    assert_eq!(malformed, before);
}
