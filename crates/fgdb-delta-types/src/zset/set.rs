    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::NegativeMultiplicity { input } => {
                write!(f, "negative integrated {input:?} set input")
            }
        }
    }
}
...
impl<T: Ord> IncrementalSet<T> {
    ...
    pub fn left_counts(&self) -> &ZSet<T> {
        &self.left
    }
    pub fn right_counts(&self) -> &ZSet<T> {
        &self.right
    }
}
...
impl<T: Ord + Clone> IncrementalSet<T> {
    ...
    pub fn prepare<E>(
        ...
        let right = self
            .right
            .prepare_integration(delta_right, limbs, control)?;
        ...
        loop {
            ...
            for (input, next) in [(SetInput::Left, next_left), (SetInput::Right, next_right)] {
                ...
                if next < &ZWeight::ZERO {
                    return Err(SetError::NegativeMultiplicity { input });
                }
            }
            let before = self.operation.weight(old_left, old_right, limbs, control)?;
            let after = self
                .operation
                .weight(next_left, next_right, limbs, control)?;
            event(control, ZSetEvent::Work)?;
...
impl<T: Ord> core::fmt::Debug for SetUpdate<'_, T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("SetUpdate")
            .field("changed_support", &self.delta.len())
            .field("data", &"[REDACTED]")
            .finish()
    }
}
