...
fn edges(updates: &[(u32, u32, i128)]) -> ZSet<(u32, u32)> {
    ZSet::from_updates(updates.iter().map(|&(s, t, w)| ((s, t), ZWeight::from_i128(w))),
        LIMBS, &mut allow).unwrap()
}
fn plain(value: &ZSet<(u32, u32)>) -> BTreeMap<(u32, u32), i128> {
    value.iter().map(|(key, weight)| (*key, weight.to_i128().unwrap())).collect()
}
fn input(mask: u32) -> ZSet<(u32, u32)> {
    let rows: Vec<_> = (0..9).filter(|bit| mask & (1 << bit) != 0)
        .map(|bit| (bit / 3, bit % 3, 1)).collect();
    edges(&rows)
}
...
fn oracle(mask: u32) -> ZSet<(u32, u32)> {
    ...
    for (source, row) in reach.iter().enumerate() {
        for (destination, &present) in row.iter().enumerate() {
            if present { result.push((source as u32, destination as u32, 1)); }
        }
    }
    edges(&result)
}
...
fn every_three_vertex_graph_handles_single_and_simultaneous_support_changes() {
    for before in 0..512 {
        ...
        for flip in flips {
            ...
            assert_eq!(operator.snapshot(LIMBS, &mut allow).unwrap(), expected_after);
            let inverse = update.negated(LIMBS, &mut allow).unwrap();
            operator.apply(&inverse, LIMBS, &mut allow).unwrap();
            assert_eq!(operator.snapshot(LIMBS, &mut allow).unwrap(), expected_before);
        }
    }
}
...
fn parallel_edges_cycles_and_alternate_routes_do_not_self_support_deletions() {
    let mut operator = IncrementalReachability::new();
    operator.apply(&edges(&[(1, 2, 2), (2, 1, 1), (2, 3, 1), (1, 3, 1)]), LIMBS, &mut allow).unwrap();
    assert!(operator.contains(&1, &1));
    assert!(operator.apply(&edges(&[(1, 2, -1)]), LIMBS, &mut allow).unwrap().is_empty());
    let delta = operator.apply(&edges(&[(1, 2, -1)]), LIMBS, &mut allow).unwrap();
    assert_eq!(plain(&delta), BTreeMap::from([((1, 1), -1), ((1, 2), -1), ((2, 2), -1)]));
    assert!(operator.contains(&1, &3));
    assert!(!operator.contains(&3, &3));
    let delta = operator.apply(&edges(&[(1, 3, -1), (2, 1, -1), (2, 3, -1)]), LIMBS, &mut allow).unwrap();
...
fn every_refusal_and_dropped_guard_preserve_all_arrangements() {
    ...
    success.apply(&delta, LIMBS, &mut |_| { total += 1; Ok::<_, usize>(()) }).unwrap();
    for stop in 1..=total {
        ...
        assert_eq!(result, Err(ReachabilityError::Delta(ZSetError::Control(stop))));
        assert_eq!(seen, stop);
    ...
    let mut sink = operator.snapshot(LIMBS, &mut allow).unwrap();
    {
        ...
        assert!(sink.prepare_update(prepared.delta(), LIMBS, &mut |_| Err(123)).is_err());
    }
    assert_eq!(operator, before);
    assert_eq!(sink, before.snapshot(LIMBS, &mut allow).unwrap());
    let prepared = operator.prepare(&delta, LIMBS, &mut allow).unwrap();
    let output = sink.prepare_update(prepared.delta(), LIMBS, &mut allow).unwrap();
    output.commit();
    prepared.commit();
    assert_eq!(operator, success);
...
fn negative_counts_and_late_promotion_refuse_atomically() {
    ...
    assert_eq!(operator.apply(&edges(&[(3, 4, 1), (5, 6, -1)]), LIMBS, &mut allow),
    ...
    assert!(matches!(operator.apply(&edges(&[(1, 2, 1)]), LimbLimit::new(0), &mut allow),
        Err(ReachabilityError::Delta(ZSetError::Arithmetic(_)))));
    assert_eq!(operator.edge_weight(&(1, 2)), Some(&ZWeight::from_i128(i128::MAX)));
    assert!(operator.apply(&edges(&[(1, 2, 1)]), LIMBS, &mut allow).unwrap().is_empty());
...
fn a_small_changed_component_does_not_scan_unrelated_graphs() {
    ...
    let mut events = 0;
    let delta = operator.apply(&edges(&[(1, 0, 1)]), LIMBS, &mut |_| {
        events += 1;
        if events > 200 { Err(events) } else { Ok(()) }
    }).unwrap();
    assert_eq!(plain(&delta), BTreeMap::from([((0, 0), 1), ((1, 0), 1), ((1, 1), 1)]));
