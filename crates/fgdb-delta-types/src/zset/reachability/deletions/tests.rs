...
fn z(rows: impl IntoIterator<Item = ((usize, usize), i128)>) -> ZSet<(usize, usize)> {
    ZSet::from_updates(
        rows.into_iter().map(|(edge, weight)| (edge, ZWeight::from_i128(weight))),
        LIMBS,
        &mut allow,
    ).unwrap()
}
...
fn oracle(edges: &BTreeSet<(usize, usize)>, n: usize) -> BTreeSet<(usize, usize)> {
    ...
    for &(source, target) in edges { paths[source][target] = true; }
    for middle in 0..n {
        for source in 0..n {
            for target in 0..n {
    ...
    paths.iter().enumerate().flat_map(|(source, row)| {
        row.iter().enumerate().filter_map(move |(target, &present)| present.then_some((source, target)))
    }).collect()
}

fn pairs(state: &IncrementalReachability<usize>) -> BTreeSet<(usize, usize)> {
    state.pairs().map(|(&source, &target)| (source, target)).collect()
}
...
fn arrangements(
    ...
    let flatten = |relation: &Relation<usize>| -> BTreeSet<(usize, usize)> {
        relation.iter().flat_map(|(&source, row)| row.iter().map(move |&target| (source, target))).collect()
    };
    assert_eq!(flatten(&state.outgoing), *edges);
    assert_eq!(flatten(&state.incoming), edges.iter().map(|&(s, t)| (t, s)).collect::<BTreeSet<_>>());
    assert_eq!(flatten(&state.predecessors), expected.iter().map(|&(s, t)| (t, s)).collect::<BTreeSet<_>>());
    for relation in [&state.outgoing, &state.incoming, &state.reachable, &state.predecessors] {
        assert!(relation.values().all(|row| !row.is_empty()), "empty retained arrangement group");
    }
...
fn every_three_vertex_decremental_transition_and_reinsertion_matches_batch() {
    ...
    for mut code in 0..3_usize.pow(9) {
        ...
        for edge in 0..9 {
            ...
            let state = code % 3;
            if state != 0 { old.insert(pair); }
            if state == 2 { removed.insert(pair); }
            code /= 3;
        }
        let mut operator = IncrementalReachability::new();
        operator.apply(&z(old.iter().map(|&edge| (edge, 1))), LIMBS, &mut allow).unwrap();
        let before = oracle(&old, 3);
        arrangements(&operator, &old, &before);
        let update = operator.prepare(&z(removed.iter().map(|&edge| (edge, -1))), LIMBS, &mut allow).unwrap();
        assert!(update.replacements.is_empty(), "deletion copied an old source row");
        ...
        assert_eq!(delta.iter().map(|(edge, _)| *edge).collect::<BTreeSet<_>>(), losses);
        ...
        operator.apply(&z(removed.iter().map(|&edge| (edge, 1))), LIMBS, &mut allow).unwrap();
        arrangements(&operator, &old, &before);
    }
}
...
fn boundary_paths_rederive_cycles_but_disconnected_cycles_cannot_self_support() {
    ...
    let mut operator = IncrementalReachability::new();
    operator.apply(&z(edges.iter().map(|&edge| (edge, 1))), LIMBS, &mut allow).unwrap();
    let delta = operator.apply(&z([((0, 1), -1)]), LIMBS, &mut allow).unwrap();
    assert!(delta.is_empty(), "the alternate boundary path still reaches the cycle");
    edges.remove(&(0, 1));
    arrangements(&operator, &edges, &oracle(&edges, 4));
    operator.apply(&z([((0, 3), -1)]), LIMBS, &mut allow).unwrap();
    edges.remove(&(0, 3));
    ...
    arrangements(&operator, &edges, &oracle(&edges, 4));
    operator.apply(&z([((2, 1), -1)]), LIMBS, &mut allow).unwrap();
    edges.remove(&(2, 1));
    assert!(!operator.contains(&1, &1));
    assert!(!operator.contains(&2, &2));
...
fn every_cancellation_boundary_and_downstream_abort_preserve_all_arrangements() {
    let base = z([((0, 1), 1), ((1, 2), 1), ((2, 1), 1), ((2, 3), 1),
        ((0, 4), 1), ((4, 2), 1), ((3, 5), 1)]);
    ...
    let mut calls = 0;
    {
        let update = operator.prepare(&removal, LIMBS, &mut |_| {
            calls += 1;
            Ok::<_, usize>(())
        }).unwrap();
    ...
    let sink_before = reference.snapshot(LIMBS, &mut allow).unwrap();
    {
        ...
        assert!(sink.prepare_update(update.delta(), LIMBS, &mut |_| Err::<(), _>(123usize)).is_err());
        drop(update);
    }
    assert_eq!(operator, reference);
    assert_eq!(sink, sink_before);
    let expected = operator.prepare(&removal, LIMBS, &mut allow).unwrap().commit();
...
fn tail_deletion_work_and_scratch_scale_with_lost_pairs_not_unchanged_paths() {
    for n in [32, 64, 128] {
        let mut operator = IncrementalReachability::new();
        operator.apply(&z((0..n).map(|a| ((a, a + 1), 1))), LIMBS, &mut allow).unwrap();
        let (mut work, mut scratch) = (0, 0);
        let delta = operator.apply(&z([((n - 1, n), -1)]), LIMBS, &mut |event| {
            match event { ZSetEvent::Work => work += 1, ZSetEvent::ScratchEntry => scratch += 1 }
            Ok::<_, usize>(())
        }).unwrap();
        assert_eq!(delta.len(), n);
        assert!(delta.iter().all(|(&(source, target), weight)| source < n && target == n
...
fn unrelated_components_do_not_enter_deletion_work_or_scratch() {
    let measure = |unrelated: usize| {
        let mut operator = IncrementalReachability::new();
        let base = z([(0, 1), (1, 2), (2, 3)].into_iter()
            .chain((10..10 + unrelated).map(|a| (a, a + 1))).map(|edge| (edge, 1)));
        operator.apply(&base, LIMBS, &mut allow).unwrap();
        let mut events = (0, 0);
        let delta = operator.apply(&z([((2, 3), -1)]), LIMBS, &mut |event| {
            match event { ZSetEvent::Work => events.0 += 1, ZSetEvent::ScratchEntry => events.1 += 1 }
            Ok::<_, usize>(())
        }).unwrap();
