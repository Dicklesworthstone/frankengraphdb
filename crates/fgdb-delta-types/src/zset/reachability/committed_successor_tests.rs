...
fn edge(id: u128, from: u128, to: u128) -> DeltaRow {
    DeltaRow::CreateEdge { eid: EId(id), birth_ordinal: id as u64,
        src: VId(from), relation: RelationId(1), dst: VId(to),
        canonical_key: None, props: vec![], valid_time: None }
}
fn batch(at: u64, rows: Vec<DeltaRow>) -> LogicalDeltaBatch {
    let template = LogicalDeltaTemplate::build(ObjectId([1; 32]), [2; 32], vec![
        CoordinateEntry { graph: GraphId(1), branch: BranchId(1), relation: RelationId(1),
...
fn live_and_indexed_recursive_ticks_are_identical_and_every_refusal_is_retryable() {
    ...
    let build = || {
        let mut state = view();
        state.prepare_committed_successor(&cx, &first, LIMBS, &mut allow).unwrap().commit();
        state
    };
    let before = build();
    let mut indexed = view();
    indexed.prepare_next(&history, LIMBS, &mut allow).unwrap().unwrap().commit();
    assert_eq!(indexed, before);
    let mut indexed_events = Vec::new();
    let expected = indexed.prepare_next(&history, LIMBS, &mut |e| {
        indexed_events.push(e); Ok::<_, usize>(())
    }).unwrap().unwrap().commit();
    let mut success = build();
    let mut live_events = Vec::new();
    let actual = success.prepare_committed_successor(&cx, &second, LIMBS, &mut |e| {
        live_events.push(e); Ok::<_, usize>(())
    }).unwrap().commit();
    ...
    for stop in 1..=live_events.len() {
        let mut state = build();
        let mut seen = 0;
        let error = state.prepare_committed_successor(&cx, &second, LIMBS, &mut |_| {
            seen += 1; if seen == stop { Err(stop) } else { Ok(()) }
        }).unwrap_err();
        match error {
            CommittedReachabilityError::Input(EdgeInputError::Delta(ZSetError::Control(at)))
            | CommittedReachabilityError::Reachability(ReachabilityError::Delta(ZSetError::Control(at))) => assert_eq!(at, stop),
        ...
        {
            let pending = state.prepare_committed_successor(&cx, &second, LIMBS, &mut allow).unwrap();
            assert!(sink.prepare_update(pending.delta(), LIMBS, &mut |_| Err(0)).is_err());
        }
        assert_eq!(state, before);
        let pending = state.prepare_committed_successor(&cx, &second, LIMBS, &mut allow).unwrap();
    ...
    assert!(success.prepare_committed_successor(&cx, &second, LIMBS, &mut allow).is_err());
    assert!(success.prepare_next(&history, LIMBS, &mut allow).unwrap().is_none());
}
