...
impl<V: Ord> IncrementalTriangles<V> {
    pub fn new(quantifier: TriangleQuantifier) -> Self {
        Self { quantifier, edges: ZSet::new(), neighbors: BTreeMap::new(), total: ZWeight::ZERO }
    }
    pub fn quantifier(&self) -> TriangleQuantifier { self.quantifier }
...
fn candidates<'a, V: Ord>(
    old: &'a Neighbors<V>, inserted: &'a Neighbors<V>, a: &V,
) -> impl Iterator<Item = &'a V> {
    old.get(a).into_iter().flatten().chain(inserted.get(a).into_iter().flatten())
}
...
fn stage_neighbor<V: Ord + Clone, E>(
    inserted: &mut Neighbors<V>, a: &V, b: &V,
...
fn remove_neighbor<V: Ord>(neighbors: &mut Neighbors<V>, a: &V, b: &V) {
    if let Some(row) = neighbors.get_mut(a) {
        row.remove(b);
        if row.is_empty() { neighbors.remove(a); }
    }
}
...
impl<V: Ord + Clone> IncrementalTriangles<V> {
    ...
    pub fn prepare<E>(
        ...
        let weights = self.edges.prepare_integration(&normalized, limbs, control)?;
        let mut effective = BTreeSet::new();
        let mut inserted = Neighbors::new();
        for (edge, next) in &weights {
            event(control, ZSetEvent::Work)?;
            if next < &ZWeight::ZERO { return Err(TriangleError::NegativeMultiplicity); }
        ...
        for edge in &effective {
            event(control, ZSetEvent::Work)?;
            let (a, b) = edge;
            let (scan, probe) = if degree(&self.neighbors, &inserted, a)
                <= degree(&self.neighbors, &inserted, b) { (a, b) } else { (b, a) };
            ...
            for third in candidates(&self.neighbors, &inserted, scan) {
                ...
                let (x, y, z) = if third < a { (third, a, b) }
                    else if third < b { (a, third, b) } else { (a, b, third) };
                for _ in 0..3 { event(control, ZSetEvent::ScratchEntry)?; }
                let sides = [(x.clone(), y.clone()), (x.clone(), z.clone()), (y.clone(), z.clone())];
                let mut first = None;
                for side in &sides {
                    event(control, ZSetEvent::Work)?;
                    if effective.contains(side) { first = Some(side); break; }
                }
                if first != Some(edge) { continue; }
                ...
                event(control, ZSetEvent::Work)?;
                let change = next.checked_add(&inverse, limbs).map_err(ZSetError::Arithmetic)?;
                if !change.is_zero() {
                    event(control, ZSetEvent::ScratchEntry)?;
        ...
        event(control, ZSetEvent::Work)?;
        let total = self.total.checked_add(&change, limbs).map_err(ZSetError::Arithmetic)?;
        debug_assert!(total >= ZWeight::ZERO);
        event(control, ZSetEvent::Work)?;
        Ok(TriangleUpdate { owner: self, weights, inserted, total, delta: output })
    }

    pub fn apply<E>(
        &mut self, delta: &ZSet<(V, V)>, limbs: LimbLimit,
        control: &mut impl FnMut(ZSetEvent) -> Result<(), E>,
    ...
    fn triangle_weight<E>(
        &self, sides: &[(V, V); 3], replacements: Option<&Weights<V>>,
        ...
        for (at, edge) in sides.iter().enumerate() {
            event(control, ZSetEvent::Work)?;
            weights[at] = replacements.and_then(|rows| rows.get(edge)).or_else(|| self.edges.weight(edge));
        }
        if weights.iter().any(|weight| weight.is_none_or(ZWeight::is_zero)) {
            return Ok(ZWeight::ZERO);
        }
        if self.quantifier == TriangleQuantifier::Distinct { return Ok(ZWeight::ONE); }
        let mut product = ZWeight::ONE;
        for weight in weights.into_iter().flatten() {
            event(control, ZSetEvent::Work)?;
            product = product.checked_mul(weight, limbs).map_err(ZSetError::Arithmetic)?;
        }
        Ok(product)
    }
    ...
    pub fn snapshot<E>(
        ...
        for (edge, _) in self.edges.iter() {
            ...
            let (a, b) = edge;
            if a == b { continue; }
            let (scan, probe) = if degree(&self.neighbors, &empty, a) <= degree(&self.neighbors, &empty, b) {
                (a, b)
            } else { (b, a) };
            for third in candidates(&self.neighbors, &empty, scan) {
                event(control, ZSetEvent::Work)?;
                if third <= b || !adjacent(&self.neighbors, &empty, probe, third) { continue; }
                for _ in 0..3 { event(control, ZSetEvent::ScratchEntry)?; }
                let sides = [(a.clone(), b.clone()), (a.clone(), third.clone()), (b.clone(), third.clone())];
                let weight = self.triangle_weight(&sides, None, limbs, control)?;
                event(control, ZSetEvent::ScratchEntry)?;
                output.accumulate((a.clone(), b.clone(), third.clone()), weight, limbs, control)?;
            }
        }
        Ok(output)
    }
...
impl<V: Ord> TriangleUpdate<'_, V> {
    pub fn delta(&self) -> &ZSet<(V, V, V)> { &self.delta }
    pub fn total(&self) -> &ZWeight { &self.total }
}
impl<V: Ord + Clone> TriangleUpdate<'_, V> {
    pub fn commit(self) -> ZSet<(V, V, V)> {
        let Self { owner, weights, inserted, total, delta } = self;
        ...
        for (vertex, row) in inserted { owner.neighbors.entry(vertex).or_default().extend(row); }
        owner.total = total;
        delta
    }
}
impl<V: Ord> core::fmt::Debug for TriangleUpdate<'_, V> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("TriangleUpdate").field("delta_support", &self.delta.len())
            .field("data", &"[REDACTED]").finish()
    }
}
