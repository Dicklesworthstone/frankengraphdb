...
fn initial() -> LogicalDeltaBatch {
    batch(
        ...
        vec![
            coordinate(
                R,
                vec![edge(1, R, 1, 2), edge(2, R, 1, 2), edge(3, R, 2, 3)],
            ),
...
fn snapshot_baseline_preserves_identity_bags_then_retracts_cross_relation_cascades() {
    ...
    let delta = input
        ...
        .commit();
    assert_eq!(
        delta.weight(&(R, VId(1), VId(2))).unwrap().to_i128(),
        Some(-2)
    );
    ...
    assert_eq!(
        input.prepare_next(&index, LIMBS, &mut allow).unwrap_err(),
...
fn missing_prefix_is_unneeded_and_boundary_identity_survives_but_forks_refuse() {
    ...
    for marker in [false, true] {
        let changed = LogicalDeltaBatch::from_parts_for_test(
            anchor.coordinate_entries().to_vec(),
            if marker {
            ...
            MarkerRef {
                marker_oid: if marker {
                    ObjectId([99; 32])
                } else {
                    anchor.commit_marker_identity().marker_oid
                },
        ...
        assert_eq!(
            input.prepare_next(&fork, LIMBS, &mut allow).unwrap_err(),
...
fn every_builder_checkpoint_refuses_without_an_exportable_partial_baseline() {
    let anchor = initial();
    let mut calls = 0;
    let complete = baseline(&anchor, &mut |_| {
        calls += 1;
        Ok(())
    })
    .unwrap();
    for stop in 1..=calls {
        let mut seen = 0;
        assert_eq!(
            baseline(&anchor, &mut |_| {
                seen += 1;
                if seen == stop { Err(stop) } else { Ok(()) }
            })
            .unwrap_err(),
            SnapshotInputError::Input(EdgeInputError::Delta(ZSetError::Control(stop)))
        );
        assert_eq!(seen, stop);
    ...
    let mut builder =
        EdgeSnapshotBuilder::new(GraphId(1), BranchId(1), Some(&anchor), &mut allow).unwrap();
    assert!(
        builder
            .insert(
                EId(1),
    ...
    assert_eq!(
        builder.record_epoch(R, SchemaEpoch(0), &mut allow),
        Err(SnapshotInputError::Refused)
    );
    assert!(matches!(
        builder.finish(&mut allow),
        Err(SnapshotInputError::Refused)
    ));
}
...
fn origin_duplicates_inconsistent_schemas_and_malformed_anchors_fail_closed() {
    let origin = EdgeSnapshotBuilder::new(GraphId(1), BranchId(1), None, &mut allow)
        .unwrap()
        .finish(&mut allow)
        .unwrap();
    ...
    for duplicate in [false, true] {
        ...
        assert_eq!(
            error,
            SnapshotInputError::Input(if duplicate {
                EdgeInputError::DuplicateEdge
            } else {
                EdgeInputError::SchemaChanged
            })
        );
    ...
    assert!(matches!(
        EdgeSnapshotBuilder::new(GraphId(1), BranchId(1), Some(&zero), &mut allow),
    ...
    for marker in [false, true] {
        let bad = LogicalDeltaBatch::from_parts_for_test(
            ...
            MarkerRef {
                ...
                commit_seq: CommitSeq(if marker { 2 } else { 1 }),
            },
            CommitSeq(1),
            CommitSeq(if marker { 1 } else { 2 }),
        );
        assert!(matches!(
            EdgeSnapshotBuilder::new(GraphId(1), BranchId(1), Some(&bad), &mut allow),
...
fn closure(edges: &BTreeMap<EId, EdgeTuple>, relation: RelationId) -> BTreeSet<(VId, VId)> {
    let mut pairs: BTreeSet<_> = edges
        .values()
        .filter(|tuple| tuple.0 == relation)
    ...
    for seq in 1..=160 {
        ...
        if let Some(view) = &mut baseline_view {
            view.prepare_next(&index, LIMBS, &mut allow)
                ...
                .commit();
            assert_eq!(
                view.pairs().collect::<BTreeSet<_>>(),
                closure(&identities, R)
            );
        }
        let mut builder =
            EdgeSnapshotBuilder::new(GraphId(1), BranchId(1), Some(&next), &mut allow).unwrap();
        for relation in [R, S] {
            builder
                .record_epoch(relation, SchemaEpoch(0), &mut allow)
                .unwrap();
        }
        for (&id, &tuple) in &identities {
            builder
                .insert(id, tuple, SchemaEpoch(0), LIMBS, &mut allow)
                .unwrap();
        }
        let view = CommittedReachability::from_snapshot(
            builder.finish(&mut allow).unwrap(),
            R,
            LIMBS,
            &mut allow,
        )
        ...
        if seq > 1 {
            index.retire_prefix(CommitSeq(seq - 1)).unwrap();
        }
    }
}
...
fn every_recursive_baseline_checkpoint_is_private_and_retryable() {
    let anchor = initial();
    let mut calls = 0;
    let expected = CommittedReachability::from_snapshot(
        baseline(&anchor, &mut allow).unwrap(),
        R,
        LIMBS,
        &mut |_| {
            calls += 1;
    ...
    for stop in 1..=calls {
        let mut seen = 0;
        let error = CommittedReachability::from_snapshot(
            baseline(&anchor, &mut allow).unwrap(),
            R,
            LIMBS,
            &mut |_| {
        ...
        assert_eq!(
            error,
            CommittedReachabilityError::Reachability(ReachabilityError::Delta(ZSetError::Control(
                stop
            )))
        );
        assert_eq!(seen, stop);
        let actual = CommittedReachability::from_snapshot(
            baseline(&anchor, &mut allow).unwrap(),
            R,
            LIMBS,
            &mut allow,
