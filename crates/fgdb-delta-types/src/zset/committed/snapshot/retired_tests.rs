...
fn full_retirement_preserves_the_same_baseline_then_cross_relation_cascades() {
    ...
    assert_eq!(
        indexed_baseline(&index, &mut allow).unwrap().into_input(),
        legacy
    );
    ...
    assert_eq!(
        closure.pairs().collect::<BTreeSet<_>>(),
    ...
    assert!(matches!(
        behind.prepare_next(&index, LIMBS, &mut allow),
        Err(EdgeInputError::Index(
            crate::IndexError::CursorRetired { .. }
        ))
    ));
    ...
    assert_eq!(
        current.prepare_next(&index, LIMBS, &mut allow).unwrap_err(),
...
fn every_retired_index_bootstrap_checkpoint_refuses_privately_and_retries() {
    ...
    let mut calls = 0;
    let expected = recursive(&index, &mut |_| {
        calls += 1;
        Ok(())
    })
    .unwrap();
    ...
    let mut builder =
        EdgeSnapshotBuilder::from_index(GraphId(1), BranchId(1), &index, &mut allow).unwrap();
    assert!(
        builder
            .record_epoch(R, SchemaEpoch(0), &mut |_| Err(17_usize))
            .is_err()
    );
    assert!(matches!(
...
fn index_constructor_never_invents_origin_data_or_missing_anchor_evidence() {
    ...
    let empty = EdgeSnapshotBuilder::from_index(GraphId(1), BranchId(1), &origin, &mut allow)
        .unwrap()
        .finish(&mut allow)
        .unwrap();
    ...
    let mut nonempty =
        EdgeSnapshotBuilder::from_index(GraphId(1), BranchId(1), &origin, &mut allow).unwrap();
    assert_eq!(
        nonempty.insert(
            EId(1),
            (R, VId(1), VId(2)),
    ...
    assert_eq!(
        EdgeSnapshotBuilder::from_index(GraphId(1), BranchId(1), &bare, &mut allow).unwrap_err(),
    ...
    for marker in [false, true] {
        ...
        let index = LocalDeltaBatchIndex::from_parts_for_test(
            CommitSeq::ORIGIN,
            CommitSeq(1),
            vec![(CommitSeq(1), malformed)],
        );
        assert!(matches!(
            EdgeSnapshotBuilder::from_index(GraphId(1), BranchId(1), &index, &mut allow),
...
fn retired_anchor_compares_marker_and_template_independently() {
    ...
    let mut state = indexed_baseline(&original, &mut allow)
        .unwrap()
        .into_input();
    for marker in [false, true] {
        ...
        assert_eq!(
            state,
            indexed_baseline(&original, &mut allow)
                .unwrap()
                .into_input()
        );
    }
