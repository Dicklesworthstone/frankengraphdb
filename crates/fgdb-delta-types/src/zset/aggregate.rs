...
impl<K: Ord> IncrementalAggregate<K> {
    ...
    pub fn input_weight(&self, key: &K, value: Option<i128>) -> Option<&ZWeight> {
        self.groups.get(key).and_then(|group| group.counts.weight(&value))
    }
...
impl<K: Ord + Clone> IncrementalAggregate<K> {
    ...
    pub fn prepare<E>(
        ...
        while let Some(((key, _), _)) = changes.peek().copied() {
            ...
            let (mut rows, mut values, mut sum, mut distinct, mut distinct_sum) = if let Some(group) = old {
                ...
                (
                    row.rows.checked_clone(limbs).map_err(ZSetError::Arithmetic)?,
                    row.values.checked_clone(limbs).map_err(ZSetError::Arithmetic)?,
                    match &row.sum {
            ...
            while let Some(((next_key, value), change)) = changes.peek().copied() {
                ...
                event(control, ZSetEvent::Work)?;
                rows = rows.checked_add(change, limbs).map_err(ZSetError::Arithmetic)?;
                if let Some(value) = value {
                    event(control, ZSetEvent::Work)?;
                    values = values.checked_add(change, limbs).map_err(ZSetError::Arithmetic)?;
                    event(control, ZSetEvent::Work)?;
            ...
            if old.map(|group| &group.summary) != summary.as_ref() {
                ...
                if let Some(row) = &summary {
                    output.accumulate((key.clone(), Arc::clone(row)), ZWeight::ONE, limbs, control)?;
                }
            }
            event(control, ZSetEvent::ScratchEntry)?;
        ...
        event(control, ZSetEvent::Work)?;
        Ok(AggregateUpdate { owner: self, patches, delta: output })
    }
    ...
    pub fn snapshot<E>(
        ...
        for (key, group) in &self.groups {
            event(control, ZSetEvent::Work)?;
            result.accumulate((key.clone(), Arc::clone(&group.summary)), ZWeight::ONE, limbs, control)?;
        }
        Ok(result)
    }
}
...
fn extremum<E>(
    ...
    Ok(match (old_value, changed_value) {
        (Some(left), Some(right)) => Some(if maximum { left.max(right) } else { left.min(right) }),
...
impl<K: Ord> AggregateUpdate<'_, K> {
    ...
    pub fn commit(self) -> AggregateDelta<K> {
        let Self { owner, patches, delta } = self;
...
mod tests {
    ...
    fn input(rows: impl IntoIterator<Item = (i32, Option<i128>, i128)>) -> Input {
        ZSet::from_updates(
            rows.into_iter().map(|(key, value, weight)| ((key, value), ZWeight::from_i128(weight))),
            LIMBS, &mut allow,
        ).unwrap()
    }
    ...
    fn every_small_bag_transition_matches_full_recomputation_and_output_differentiation() {
        let bag = |mut code: usize| {
            input([(0, None), (0, Some(-2)), (0, Some(3)), (1, Some(7))]
    ...
    fn nulls_zero_sum_and_last_row_removal_have_distinct_results() {
        ...
        let delta = state.apply(&initial.negated(LIMBS, &mut allow).unwrap(), LIMBS, &mut allow).unwrap();
    ...
    fn unchanged_summary_still_advances_counts_and_retractions_replace_extrema() {
        let mut state = seeded(&input([(0, Some(0), 1), (0, Some(2), 1), (0, Some(8), 1), (0, Some(10), 1)]));
        let unchanged = state.apply(&input([
            (0, Some(2), -1), (0, Some(8), -1), (0, Some(4), 1), (0, Some(6), 1),
        ]), LIMBS, &mut allow).unwrap();
        assert!(unchanged.is_empty());
        assert!(state.input_weight(&0, Some(2)).is_none());
        assert_eq!(state.input_weight(&0, Some(4)), Some(&ZWeight::ONE));
        state.apply(&input([(0, Some(0), -1), (0, Some(10), -1)]), LIMBS, &mut allow).unwrap();
        let row = state.get(&0).unwrap();
        assert_eq!((row.minimum(), row.maximum()), (Some(4), Some(6)));
        state.apply(&input([(0, Some(4), -1), (0, Some(6), -1)]), LIMBS, &mut allow).unwrap();
        assert_eq!(state.group_count(), 0);
    }
    ...
    fn negative_per_value_counts_refuse_even_when_the_group_total_would_be_positive() {
        ...
        assert_eq!(state.apply(&bad, LIMBS, &mut allow), Err(AggregateError::NegativeMultiplicity));
        assert_eq!(state, seeded(&before));
        assert_eq!(state.apply(&input([(99, Some(1), -1)]), LIMBS, &mut allow), Err(AggregateError::NegativeMultiplicity));
        assert_eq!(state, seeded(&before));
    }

    #[test]
    fn dropped_guards_and_every_control_refusal_preserve_all_groups_and_allow_retry() {
        let before = input([(0, None, 1), (0, Some(1), 2), (0, Some(7), 1), (1, Some(9), 1)]);
        let changes = input([(0, Some(1), -2), (0, Some(3), 1), (1, Some(9), -1), (2, None, 1)]);
        let mut state = seeded(&before);
        {
        ...
        assert_eq!(state, seeded(&before));
        let mut calls = 0;
        let expected_delta = state.apply(&changes, LIMBS, &mut |_| {
            calls += 1;
            Ok::<_, usize>(())
        }).unwrap();
        for stop in 1..=calls {
            ...
            let mut seen = 0;
            assert_eq!(attempt.apply(&changes, LIMBS, &mut |_| {
                seen += 1;
                if seen == stop { Err(stop) } else { Ok(()) }
            }), Err(AggregateError::ZSet(ZSetError::Control(stop))));
            assert_eq!(seen, stop);
            assert_eq!(attempt, seeded(&before));
            assert_eq!(attempt.apply(&changes, LIMBS, &mut allow).unwrap(), expected_delta);
    ...
    fn promotion_is_exact_and_arithmetic_or_weight_admission_refuses_before_publication() {
        ...
        let mut state = IncrementalAggregate::new();
        assert!(matches!(state.apply(&large, LimbLimit::new(0), &mut allow),
        ...
        state.apply(&input([(0, Some(2), 1 - i128::MAX)]), LIMBS, &mut allow).unwrap();
        ...
        let promoted = ZWeight::from_i128(i128::MAX).checked_add(&ZWeight::ONE, LIMBS).unwrap();
        let transferred = ZSet::from_updates([((3, Some(0)), promoted)], LIMBS, &mut allow).unwrap();
        assert!(matches!(state.apply(&transferred, LimbLimit::new(0), &mut allow),
    ...
    fn exact_work_and_growth_limits_succeed_and_one_below_refuses_atomically() {
        ...
        measured.apply(&delta, LIMBS, &mut |event| {
            work += 1;
            scratch += usize::from(event == ZSetEvent::ScratchEntry);
            Ok::<_, usize>(())
        }).unwrap();
        for (max_work, max_scratch, pass) in [(work, scratch, true), (work - 1, scratch, false), (work, scratch - 1, false)] {
            let mut state = seeded(&initial);
            ...
            let result = state.apply(&delta, LIMBS, &mut |event| {
                ...
                if seen_work > max_work || seen_scratch > max_scratch { Err(1) } else { Ok(()) }
            });
            assert_eq!(result.is_ok(), pass);
            if pass { assert_eq!(state, measured); } else { assert_eq!(state, seeded(&initial)); }
        }
    }
    ...
    fn debug_output_does_not_export_group_keys_or_summary_values() {
        let mut state = IncrementalAggregate::new();
        let rows = ZSet::from_updates([(("private-group", Some(919191)), ZWeight::ONE)], LIMBS, &mut allow).unwrap();
        let delta = state.apply(&rows, LIMBS, &mut allow).unwrap();
        let text = format!("{state:?} {delta:?} {:?}", state.get(&"private-group").unwrap());
