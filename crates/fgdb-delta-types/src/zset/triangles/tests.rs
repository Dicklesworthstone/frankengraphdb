...
fn edges(rows: impl IntoIterator<Item = ((usize, usize), i128)>) -> ZSet<(usize, usize)> {
    ZSet::from_updates(rows.into_iter().map(|(edge, weight)| (edge, ZWeight::from_i128(weight))), LIMBS, &mut allow).unwrap()
}
fn graph(mask: u64, n: usize) -> ZSet<(usize, usize)> {
    ...
    for a in 0..n {
        for b in a + 1..n {
            if mask & (1 << bit) != 0 { rows.push(((a, b), 1)); }
            bit += 1;
        }
    }
    edges(rows)
...
fn oracle(input: &ZSet<(usize, usize)>, n: usize, quantifier: TriangleQuantifier) -> ZSet<(usize, usize, usize)> {
    ...
    for a in 0..n {
        for b in a + 1..n {
            for c in b + 1..n {
                ...
                if product > 0 {
                    let weight = if quantifier == TriangleQuantifier::Distinct { 1 } else { product };
...
fn check_index(state: &IncrementalTriangles<usize>, n: usize) {
    for a in 0..n {
        for b in 0..n {
            assert_eq!(state.neighbors.get(&a).is_some_and(|row| row.contains(&b)), a != b && state.edge_weight(&a, &b).is_some());
        }
    }
}
    ...
    for quantifier in [TriangleQuantifier::Distinct, TriangleQuantifier::All] {
        for before in 0..64 {
            ...
            for after in 0..64 {
                ...
                assert_eq!(state.apply(&initial, LIMBS, &mut allow).unwrap(), expected_before);
                ...
                let expected_delta = expected_after.minus(&expected_before, LIMBS, &mut allow).unwrap();
                assert_eq!(state.apply(&change, LIMBS, &mut allow).unwrap(), expected_delta, "before={before}, after={after}");
                assert_eq!(state.snapshot(LIMBS, &mut allow).unwrap(), expected_after);
                assert_eq!(state.total(), &expected_after.total_weight(LIMBS, &mut allow).unwrap());
                check_index(&state, 4);
                state.apply(&change.negated(LIMBS, &mut allow).unwrap(), LIMBS, &mut allow).unwrap();
...
fn weighted_mixed_streams_include_all_same_tick_cross_terms() {
    for quantifier in [TriangleQuantifier::Distinct, TriangleQuantifier::All] {
        ...
        for tick in 0..1000 {
            ...
            assert_eq!(pending.total(), &expected.total_weight(LIMBS, &mut allow).unwrap());
            let publication = sink.prepare_update(pending.delta(), LIMBS, &mut allow).unwrap();
...
fn opposite_directions_parallel_edges_and_self_loops_have_explicit_semantics() {
    let seed = edges([((0, 1), 2), ((1, 0), 3), ((0, 2), 7), ((2, 1), 11), ((0, 0), 23)]);
    for quantifier in [TriangleQuantifier::Distinct, TriangleQuantifier::All] {
        let mut state = IncrementalTriangles::new(quantifier);
        let first = state.apply(&seed, LIMBS, &mut allow).unwrap();
        let expected = if quantifier == TriangleQuantifier::Distinct { 1 } else { 385 };
        assert_eq!(first.weight(&(0, 1, 2)), Some(&ZWeight::from_i128(expected)));
        ...
        assert!(state.apply(&edges([((0, 1), -2), ((1, 0), 2)]), LIMBS, &mut allow).unwrap().is_empty());
        let delta = state.apply(&edges([((0, 1), -2), ((0, 0), -23)]), LIMBS, &mut allow).unwrap();
        if quantifier == TriangleQuantifier::Distinct { assert!(delta.is_empty()); }
        else { assert_eq!(delta.weight(&(0, 1, 2)), Some(&ZWeight::from_i128(-154))); }
        state.apply(&edges([((1, 0), -3)]), LIMBS, &mut allow).unwrap();
        assert!(state.total().is_zero());
        assert!(state.snapshot(LIMBS, &mut allow).unwrap().is_empty());
        assert!(matches!(state.prepare(&edges([((0, 0), -1)]), LIMBS, &mut allow), Err(TriangleError::NegativeMultiplicity)));
        check_index(&state, 3);
    }
}
...
fn every_control_refusal_and_downstream_drop_preserve_all_arrangements() {
    for quantifier in [TriangleQuantifier::Distinct, TriangleQuantifier::All] {
        ...
        let mut events = 0;
        drop(state.prepare(&change, LIMBS, &mut |_| { events += 1; Ok::<_, usize>(()) }).unwrap());
        ...
        for stop in 1..=events {
            ...
            match state.prepare(&change, LIMBS, &mut |_| {
                ...
                Ok(update) => { drop(update); panic!("missed refusal {stop}"); }
            }
            assert_eq!(seen, stop);
            assert_eq!(state, reference);
        }
        ...
        {
            let pending = state.prepare(&change, LIMBS, &mut allow).unwrap();
            assert!(sink.prepare_update(pending.delta(), LIMBS, &mut |_| Err(9)).is_err());
        }
        assert_eq!(state, reference);
...
fn promotion_and_late_negative_counts_refuse_without_partial_publication() {
    ...
    reference.apply(&seed, LIMBS, &mut allow).unwrap();
    assert!(matches!(state.apply(&change, LimbLimit::new(0), &mut allow), Err(TriangleError::Delta(ZSetError::Arithmetic(_)))));
    assert_eq!(state, reference);
    assert!(matches!(state.apply(&edges([((0, 2), 1), ((100, 101), -1)]), LIMBS, &mut allow), Err(TriangleError::NegativeMultiplicity)));
    assert_eq!(state, reference);
    let expected = ZWeight::from_i128(i128::MAX).checked_mul(&ZWeight::from_i128(i128::MAX), LIMBS).unwrap()
        .checked_mul(&ZWeight::from_i128(3), LIMBS).unwrap();
...
fn local_changes_do_not_scan_ten_thousand_unrelated_components() {
    for quantifier in [TriangleQuantifier::Distinct, TriangleQuantifier::All] {
        let seed = edges([((0, 1), 1), ((1, 2), 1)].into_iter()
            .chain((0..10_000).map(|n| ((100 + 2 * n, 101 + 2 * n), 1))));
        let mut state = IncrementalTriangles::new(quantifier);
        state.apply(&seed, LIMBS, &mut allow).unwrap();
        for sign in [1, -1] {
            let mut events = 0;
            let delta = state.apply(&edges([((0, 2), sign)]), LIMBS, &mut |_| {
                events += 1;
...
fn empty_updates_and_debug_output_do_not_invent_rows_or_leak_keys() {
    let mut state = IncrementalTriangles::new(TriangleQuantifier::All);
    assert!(state.apply(&ZSet::<(String, String)>::new(), LIMBS, &mut allow).unwrap().is_empty());
    let seed = ZSet::from_updates([(("private-a".to_owned(), "private-b".to_owned()), ZWeight::from_i128(919191))], LIMBS, &mut allow).unwrap();
    state.apply(&seed, LIMBS, &mut allow).unwrap();
