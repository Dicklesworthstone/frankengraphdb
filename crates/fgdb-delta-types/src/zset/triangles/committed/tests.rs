    ...
    index
        .insert(batch(
            ...
            vec![
                ...
                coordinate(2, vec![create(20, 2, 1, 3)]),
            ],
        ))
        .unwrap();
...
fn count(state: &CommittedTriangles) -> i128 {
    state.total().to_i128().unwrap()
}
fn one(delta: &ZSet<(VId, VId, VId)>, weight: i128) {
    assert_eq!(delta.len(), usize::from(weight != 0));
    assert_eq!(
        delta
            .weight(&(VId(1), VId(2), VId(3)))
            .map(ZWeight::to_i128),
        if weight == 0 {
            None
        } else {
    ...
    for q in [TriangleQuantifier::All, TriangleQuantifier::Distinct] {
        ...
        index
            .insert(batch(
                ...
                vec![coordinate(
                    1,
                    vec![delete(10), create(15, 1, 2, 3), create(16, 1, 1, 3)],
                )],
            ))
            .unwrap();
...
fn zero_derivative_ticks_still_publish_identity_and_history_changes() {
    ...
    index
        .insert(batch(
            2,
            vec![coordinate(1, vec![delete(10), create(99, 1, 1, 2)])],
        ))
        .unwrap();
    ...
    assert_eq!(state.frontier(), CommitSeq(3));
    index
        .insert(batch(4, vec![coordinate(1, vec![delete(99)])]))
        .unwrap();
    ...
    index
        .insert(batch(5, vec![coordinate(2, vec![delete(900)])]))
        .unwrap();
    assert!(matches!(
        state.prepare_next(&index, LIMBS, &mut allow),
...
fn snapshot_bootstrap_uses_current_edges_then_accepts_the_same_successor() {
    ...
    for (eid, relation, a, b) in [
        ...
        builder
            .insert(
                EId(eid),
                (RelationId(relation), VId(a), VId(b)),
                SchemaEpoch(0),
                LIMBS,
                &mut allow,
            )
    ...
    let mut rebuilt = CommittedTriangles::from_snapshot(
        builder.finish(&mut allow).unwrap(),
        RelationId(1),
        TriangleQuantifier::All,
        LIMBS,
        &mut allow,
    )
    .unwrap();
    ...
    assert_eq!(
        rebuilt.snapshot(LIMBS, &mut allow).unwrap(),
        replayed.snapshot(LIMBS, &mut allow).unwrap()
    );
    index
        .insert(batch(3, vec![coordinate(1, vec![delete(12)])]))
        .unwrap();
    assert_eq!(
        advance(&mut rebuilt, &index),
        advance(&mut replayed, &index)
    );
...
fn caught_up_calls_reject_a_substituted_history_anchor_without_state_change() {
    ...
    let changed = LogicalDeltaBatch::from_parts_for_test(
        changed.coordinate_entries().to_vec(),
        [77; 32],
        MarkerRef {
    ...
    assert!(matches!(
        state.prepare_next(&fork, LIMBS, &mut allow),
        Err(CommittedTrianglesError::Input(_))
    ));
    assert_eq!(state, before);
}
