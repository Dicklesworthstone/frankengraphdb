...
fn z<T: Ord>(updates: impl IntoIterator<Item = (T, i128)>) -> ZSet<T> {
    ZSet::from_updates(updates.into_iter().map(|(key, weight)| (key, ZWeight::from_i128(weight))),
...
fn oracle(vertices: &[u128], edges: &ZSet<(u128, u128)>) -> BTreeMap<u128, u64> {
    let mut adjacency: BTreeMap<u128, BTreeSet<u128>> = vertices.iter()
        .map(|v| (*v, BTreeSet::new())).collect();
    ...
    for threshold in 1..vertices.len() {
        ...
        loop {
            let doomed: Vec<_> = alive.iter().copied().filter(|v|
                adjacency[v].iter().filter(|neighbor| alive.contains(*neighbor)).count() < threshold
            ).collect();
            if doomed.is_empty() { break; }
            for v in doomed { alive.remove(&v); }
        }
        for v in alive { result.insert(v, threshold as u64); }
    }
    result
}
fn graph(mask: u64) -> ZSet<(u128, u128)> {
    let sides = [(0, 1), (0, 2), (0, 3), (1, 2), (1, 3), (2, 3)];
    z(sides.into_iter().enumerate().filter_map(|(at, edge)|
        (mask & (1 << at) != 0).then_some((edge, 1))))
}
fn build(v: &[u128], edges: &ZSet<(u128, u128)>) -> IncrementalCoreNumbers<u128> {
    let mut state = IncrementalCoreNumbers::new();
    state.apply(&vertices(v.iter().copied()), edges, LIMBS, &mut allow).unwrap();
    state
}
...
fn every_four_vertex_transition_and_reverse_matches_threshold_fixed_points() {
    ...
    for before in 0..64 {
        for after in 0..64 {
            ...
            let delta = state.apply(&ZSet::new(), &change, LIMBS, &mut allow).unwrap();
            ...
            state.apply(&ZSet::new(), &inverse, LIMBS, &mut allow).unwrap();
...
fn dense_core_decrements_cascade_but_isolates_and_wide_ids_remain_exact() {
    ...
    for i in 0..domain.len() {
        for j in i + 1..domain.len() { edges.push(((domain[i], domain[j]), 1)); }
    }
    ...
    {
        let pending = state.prepare(&z([(0, -1)]), &retired, LIMBS, &mut allow).unwrap();
        assert_eq!(pending.core_number(&0), None);
        assert_eq!(pending.core_number(&u128::MAX), Some(2));
        assert_eq!(pending.vertex_count(), 3);
        drop(pending);
    }
    assert_eq!(state.core_number(&0), Some(3), "guard drop must not retire the vertex");
    state.apply(&z([(0, -1)]), &retired, LIMBS, &mut allow).unwrap();
    assert!(rows(&state).values().all(|n| *n == 2));
    assert_eq!(state.core_number(&0), None);
    state.apply(&vertices([0]), &ZSet::new(), LIMBS, &mut allow).unwrap();
    ...
    state.apply(&ZSet::new(), &remaining.negated(LIMBS, &mut allow).unwrap(), LIMBS, &mut allow).unwrap();
...
fn parallel_opposite_edges_and_loops_have_explicit_simple_support_semantics() {
    ...
    assert_eq!(rows(&state), BTreeMap::from([(0, 2), (1, 2), (2, 2), (3, 0)]));
    let pending = state.prepare(&ZSet::new(), &z([((1, 0), 1)]), LIMBS, &mut allow).unwrap();
    ...
    let pending = state.prepare(&ZSet::new(), &z([((0, 1), -i128::MAX)]), LIMBS, &mut allow).unwrap();
    assert_eq!(pending.affected_vertices(), 0);
    assert!(pending.delta().is_empty());
    let _ = pending.commit();
    assert_eq!(state.core_number(&0), Some(2));
    state.apply(&ZSet::new(), &z([((1, 0), -1)]), LIMBS, &mut allow).unwrap();
    assert_eq!(rows(&state), BTreeMap::from([(0, 1), (1, 1), (2, 1), (3, 0)]));
    state.apply(&z([(3, -1)]), &z([((3, 3), -100)]), LIMBS, &mut allow).unwrap();
...
fn every_topology_peeling_output_and_downstream_refusal_preserves_the_old_state() {
    ...
    let mut calls = 0;
    let pending = success.prepare(&ZSet::new(), &change, LIMBS, &mut |_| {
        calls += 1;
        Ok::<_, usize>(())
    }).unwrap();
    assert!(!pending.delta().is_empty());
    ...
    for stop in 1..=calls {
        ...
        let mut visited = 0;
        let result = state.prepare(&ZSet::new(), &change, LIMBS, &mut |_| {
            visited += 1;
            if visited == stop { Err(stop) } else { Ok(()) }
        }).map(|pending| { drop(pending); });
        ...
        state.apply(&ZSet::new(), &change, LIMBS, &mut allow).unwrap();
        assert_eq!(state, success);
    }
    let mut state = build(&[0, 1, 2, 3], &old);
    let mut sink = state.snapshot(LIMBS, &mut allow).unwrap();
    let old_sink = sink.checked_clone(LIMBS, &mut allow).unwrap();
    let pending = state.prepare(&ZSet::new(), &change, LIMBS, &mut allow).unwrap();
    assert!(sink.prepare_update(pending.delta(), LIMBS, &mut |_| Err(99usize)).is_err());
    drop(pending);
    assert_eq!(state, reference);
...
fn bad_endpoints_counts_and_promotion_refuse_before_any_accepted_change() {
    ...
    for (vertices, delta) in cases {
        ...
        assert!(matches!(state.prepare(&vertices, &delta, LIMBS, &mut allow), Err(CoreError::Topology(_))));
        assert_eq!(state, before);
    }
    let mut state = build(&[0, 1, 2], &edges);
    assert!(matches!(state.prepare(&ZSet::new(), &z([((0, 1), 1)]), LimbLimit::new(0), &mut allow),
        Err(CoreError::Topology(ComponentError::Delta(ZSetError::Arithmetic(_))))));
    assert_eq!(state, before);
    let pending = state.prepare(&z([(0, -1), (9, 1)]), &z([((0, 1), -i128::MAX), ((1, 9), 1)]),
        LIMBS, &mut allow).unwrap();
    assert_eq!(pending.core_number(&0), None);
...
fn unrelated_components_do_not_increase_update_work_or_scratch() {
    ...
    for extra in [0, 64, 512] {
        ...
        let mut counts = (0, 0);
        let pending = state.prepare(&ZSet::new(), &z([((0, 1), -1)]), LIMBS, &mut |event| {
            match event { ZSetEvent::Work => counts.0 += 1, ZSetEvent::ScratchEntry => counts.1 += 1 }
            Ok::<_, usize>(())
        }).unwrap();
