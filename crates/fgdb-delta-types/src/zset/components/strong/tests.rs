...
fn vertices(ids: impl IntoIterator<Item = (u8, i128)>) -> ZSet<u8> {
    ZSet::from_updates(ids.into_iter().map(|(v, w)| (v, ZWeight::from_i128(w))),
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap()
}
fn edges(rows: impl IntoIterator<Item = ((u8, u8), i128)>) -> ZSet<(u8, u8)> {
    ZSet::from_updates(rows.into_iter().map(|(e, w)| (e, ZWeight::from_i128(w))),
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap()
}
fn seed() -> IncrementalStrongComponents<u8> {
    let mut state = IncrementalStrongComponents::new();
    state.apply(&vertices((0..5).map(|v| (v, 1))),
        &edges([((0, 1), 2), ((1, 0), 1), ((1, 2), 1), ((3, 4), 1), ((4, 3), 1)]),
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
    state
}
...
fn every_three_vertex_graph_and_single_arc_change_matches_independent_oracle() {
    ...
    for mask in 0..512u16 {
        let mut state = IncrementalStrongComponents::new();
        let initial = state.apply(&vertices, &mask_edges(mask), LIMBS,
            &mut |_| Ok::<_, ()>(())).unwrap();
        ...
        for bit in 0..9 {
            ...
            let delta = state.apply(&ZSet::new(), &edges([(edge, sign)]), LIMBS,
                &mut |_| Ok::<_, ()>(())).unwrap();
            let after = oracle(next_mask);
            assert_eq!(labels(&state), after, "mask {mask} bit {bit}");
            assert_eq!(delta, derivative(&expected, &after));
            assert_eq!(state.component_count(), after.values().collect::<BTreeSet<_>>().len());
            assert_eq!(state.vertex_count(), 3);
            let reverse = state.apply(&ZSet::new(), &edges([(edge, -sign)]), LIMBS,
                &mut |_| Ok::<_, ()>(())).unwrap();
...
fn direction_changes_rederive_even_when_the_weak_delta_is_empty() {
    let mut state = IncrementalStrongComponents::new();
    state.apply(&vertices((0..3).map(|v| (v, 1))),
        &edges([((0, 1), 1), ((1, 2), 1), ((0, 2), 1)]),
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
    assert_eq!(state.component_count(), 3);
    let pending = state.prepare(&ZSet::new(),
        &edges([((0, 2), -1), ((2, 0), 1)]), LIMBS,
        &mut |_| Ok::<_, ()>(())).unwrap();
...
fn malformed_directional_counts_and_dangling_endpoints_leave_all_state_unchanged() {
    ...
    for (vertex_delta, edge_delta, expected) in [
        (vertices([]), edges([((2, 1), -1), ((1, 2), 1)]),
            ComponentError::NegativeEdgeMultiplicity),
        (vertices([(0, -1)]), edges([((0, 1), -1), ((1, 0), -1)]),
            ComponentError::MissingEndpoint),
        (vertices([]), edges([((0, 9), 1)]), ComponentError::MissingEndpoint),
        (vertices([(0, 1)]), edges([]), ComponentError::VertexMultiplicity),
    ] {
        assert_eq!(state.prepare(&vertex_delta, &edge_delta, LIMBS,
            &mut |_| Ok::<_, ()>(())).unwrap_err(), expected);
...
fn every_control_refusal_and_dropped_update_preserves_every_arrangement() {
    ...
    let mut total = 0;
    let expected = {
        let pending = state.prepare(&vertex_delta, &edge_delta, LIMBS, &mut |_| {
            total += 1;
            Ok::<_, usize>(())
        }).unwrap();
        pending.delta().map(|row| Ok(*row), LIMBS,
            &mut |_| Ok::<_, usize>(())).unwrap()
    };
    assert!(total > 20);
    assert_eq!(state, seed());
    for stop in 1..=total {
        let mut seen = 0;
        let error = state.prepare(&vertex_delta, &edge_delta, LIMBS, &mut |_| {
            seen += 1;
    ...
    assert_eq!(state, seed());
    let actual = state.apply(&vertex_delta, &edge_delta, LIMBS,
        &mut |_| Ok::<_, usize>(())).unwrap();
...
fn iterative_walk_handles_a_long_cycle_and_local_updates_skip_unrelated_vertices() {
    ...
    let vertex_delta = ZSet::from_updates((0..10002).map(|v| (v, ZWeight::ONE)),
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
    let edge_delta = ZSet::from_updates((0..10000).map(|v| {
        ((v, (v + 1) % 10000), ZWeight::ONE)
    }).chain([((10000, 10001), ZWeight::ONE)]),
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
    state.apply(&vertex_delta, &edge_delta, LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
    assert_eq!(state.component_count(), 3);
    assert_eq!(state.representative(&9999), Some(&0));
    let edge_delta = ZSet::from_updates([((10001, 10000), ZWeight::ONE)],
        LIMBS, &mut |_| Ok::<_, ()>(())).unwrap();
    let mut work = 0;
    let pending = state.prepare(&ZSet::new(), &edge_delta, LIMBS, &mut |event| {
        if event == ZSetEvent::Work { work += 1; }
        Ok::<_, ()>(())
    }).unwrap();
    assert_eq!(pending.affected_vertices(), 2);
    assert!(work < 256, "unrelated 10000-vertex cycle was traversed: {work}");
    pending.commit();
