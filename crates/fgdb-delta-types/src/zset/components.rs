...
impl<E: core::fmt::Display> core::fmt::Display for ComponentError<E> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::VertexMultiplicity => f.write_str("component vertex membership is not zero or one"),
...
impl<V: Ord> IncrementalComponents<V> {
    pub fn new() -> Self {
        Self { vertices: ZSet::new(), edges: ZSet::new(), neighbors: Relation::new(),
            labels: BTreeMap::new(), members: Relation::new() }
    }
    pub fn vertex_count(&self) -> usize { self.labels.len() }
    pub fn component_count(&self) -> usize { self.members.len() }
    pub fn representative(&self, vertex: &V) -> Option<&V> { self.labels.get(vertex) }
    pub fn pairs(&self) -> impl Iterator<Item = (&V, &V)> { self.labels.iter() }
}
impl<V: Ord> core::fmt::Debug for IncrementalComponents<V> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("IncrementalComponents").field("vertices", &self.vertex_count())
            .field("components", &self.component_count()).field("data", &"[REDACTED]").finish()
    }
}
...
fn insert<V: Ord + Clone, E>(set: &mut BTreeSet<V>, value: &V,
    control: &mut impl FnMut(ZSetEvent) -> Result<(), E>) -> Result<(), ZSetError<E>> {
...
fn link<V: Ord + Clone, E>(rows: &mut Relation<V>, a: &V, b: &V,
    control: &mut impl FnMut(ZSetEvent) -> Result<(), E>) -> Result<(), ZSetError<E>> {
    event(control, ZSetEvent::Work)?;
    // Stage group/member and conservatively reserve eventual retained entries.
    for _ in 0..4 { event(control, ZSetEvent::ScratchEntry)?; }
...
fn unlink<V: Ord>(rows: &mut Relation<V>, a: &V, b: &V) {
    if let Some(row) = rows.get_mut(a) {
        row.remove(b);
        if row.is_empty() { rows.remove(a); }
    }
}
fn present<V: Ord>(base: &ZSet<V>, next: &Weights<V>, vertex: &V) -> bool {
    next.get(vertex).or_else(|| base.weight(vertex)).is_some_and(|weight| !weight.is_zero())
}
...
impl<V: Ord + Clone> IncrementalComponents<V> {
    ...
    pub fn prepare<E>(&mut self, vertex_delta: &ZSet<V>, edge_delta: &ZSet<(V, V)>,
        limbs: LimbLimit, control: &mut impl FnMut(ZSetEvent) -> Result<(), E>,
    ) -> Result<ComponentUpdate<'_, V>, ComponentError<E>> {
        event(control, ZSetEvent::Work)?;
        let vertices = self.vertices.prepare_integration(vertex_delta, limbs, control)?;
        let normalized = edge_delta.map(|(a, b)| Ok(canonical(a, b)), limbs, control)?;
        let edges = self.edges.prepare_integration(&normalized, limbs, control)?;
        ...
        for (vertex, weight) in &vertices {
            ...
            if self.vertices.weight(vertex).is_some() != !weight.is_zero() {
                ...
                if !weight.is_zero() { event(control, ZSetEvent::ScratchEntry)?; }
            }
            if weight.is_zero() {
                ...
                for neighbor in self.neighbors.get(vertex).into_iter().flatten() {
                    event(control, ZSetEvent::Work)?;
                    if !edges.get(&canonical(vertex, neighbor)).is_some_and(ZWeight::is_zero) {
        ...
        for ((a, b), weight) in &edges {
            event(control, ZSetEvent::Work)?;
            if weight < &ZWeight::ZERO { return Err(ComponentError::NegativeEdgeMultiplicity); }
            let live = !weight.is_zero();
            if live && (!present(&self.vertices, &vertices, a) || !present(&self.vertices, &vertices, b)) {
                return Err(ComponentError::MissingEndpoint);
            }
            if live == self.edges.weight(&(a.clone(), b.clone())).is_some() { continue; }
            ...
            if a != b { link(changed, b, a, control)?; }
            if live { event(control, ZSetEvent::ScratchEntry)?; }
        }
        let mut roots = BTreeSet::new();
        for vertex in &touched {
            event(control, ZSetEvent::Work)?;
            if let Some(root) = self.labels.get(vertex) { insert(&mut roots, root, control)?; }
        }
        let mut candidates = touched;
        for root in &roots {
            event(control, ZSetEvent::Work)?;
            for vertex in &self.members[root] { insert(&mut candidates, vertex, control)?; }
        }
        ...
        for vertex in &candidates {
            ...
            if self.labels.contains_key(vertex) {
                old_vertices = old_vertices.checked_add(1).ok_or(ComponentError::CardinalityOverflow)?;
            }
            if present(&self.vertices, &vertices, vertex) { insert(&mut remaining, vertex, control)?; }
        }
        ...
        while let Some(root) = remaining.pop_first() {
            ...
            while let Some(vertex) = pending.pop() {
                ...
                for _ in 0..2 { event(control, ZSetEvent::ScratchEntry)?; }
                labels.insert(vertex.clone(), root.clone());
                insert(&mut group, &vertex, control)?;
                for neighbor in self.neighbors.get(&vertex).into_iter().flatten()
                    .chain(inserted.get(&vertex).into_iter().flatten()) {
                    event(control, ZSetEvent::Work)?;
                    if removed.get(&vertex).is_some_and(|row| row.contains(neighbor)) { continue; }
        ...
        let count = self.members.len().checked_sub(roots.len())
            .and_then(|old| old.checked_add(members.len())).ok_or(ComponentError::CardinalityOverflow)?;
        let vertex_count = self.labels.len().checked_sub(old_vertices)
            .and_then(|old| old.checked_add(labels.len())).ok_or(ComponentError::CardinalityOverflow)?;
        ...
        event(control, ZSetEvent::Work)?;
        Ok(ComponentUpdate { owner: self, vertices, edges, inserted, removed, roots,
            candidates, labels, members, count, vertex_count, delta })
    }

    pub fn apply<E>(&mut self, vertices: &ZSet<V>, edges: &ZSet<(V, V)>, limbs: LimbLimit,
    ...
    pub fn snapshot<E>(&self, limbs: LimbLimit,
        control: &mut impl FnMut(ZSetEvent) -> Result<(), E>,
    ) -> Result<ZSet<(V, V)>, ComponentError<E>> {
        Ok(ZSet::from_updates(self.labels.iter().map(|(v, root)|
            ((v.clone(), root.clone()), ZWeight::ONE)), limbs, control)?)
    }
}
...
impl<V: Ord + Clone> ComponentUpdate<'_, V> {
    ...
    pub fn commit(self) -> ZSet<(V, V)> {
        let Self { owner, vertices, edges, inserted, removed, roots, candidates,
            labels, members, delta, .. } = self;
        owner.vertices.publish(vertices);
        owner.edges.publish(edges);
        for (a, row) in removed { for b in row { unlink(&mut owner.neighbors, &a, &b); } }
        for (a, row) in inserted { owner.neighbors.entry(a).or_default().extend(row); }
        for root in roots { owner.members.remove(&root); }
        for vertex in candidates { owner.labels.remove(&vertex); }
...
impl<V: Ord> core::fmt::Debug for ComponentUpdate<'_, V> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("ComponentUpdate").field("affected_vertices", &self.affected_vertices())
