...
fn z(rows: &[(i32, i128)]) -> ZSet<i32> {
    ZSet::from_updates(
        rows.iter().map(|&(key, w)| (key, ZWeight::from_i128(w))),
        LIMBS,
        &mut allow,
    )
    .unwrap()
            ...
            for b in 0..=4 {
                for next_a in 0..=4 {
                    for next_b in 0..=4 {
                        ...
                        assert_eq!(
                            delta,
                            z(&[(1, expected(op, next_a, next_b) - expected(op, a, b))])
                        );
                        output.integrate(&delta, LIMBS, &mut allow).unwrap();
                        assert_eq!(
                            output,
                            z(&[
                                (1, expected(op, next_a, next_b)),
                                (2, expected(op, 7, 0)),
                                (3, expected(op, 0, 9))
                            ])
                        );
...
fn retained_duplicate_and_blocker_counts_make_deletions_and_swaps_exact() {
    ...
    assert_eq!(
        state
            .apply(&z(&[(7, 3)]), &z(&[(7, 2)]), LIMBS, &mut allow)
            .unwrap(),
        z(&[(7, 1)])
    );
    assert!(
        state
            .apply(&z(&[(7, -3)]), &z(&[(7, 1)]), LIMBS, &mut allow)
            .unwrap()
            .is_empty()
    );
    assert!(
        state
            .apply(&z(&[]), &z(&[(7, -2)]), LIMBS, &mut allow)
            .unwrap()
            .is_empty()
    );
    assert_eq!(
        state
            .apply(&z(&[]), &z(&[(7, -1)]), LIMBS, &mut allow)
            .unwrap(),
        z(&[(7, -1)])
    );
    for op in [SetOperation::ExceptAll, SetOperation::ExceptDistinct] {
        ...
        assert!(
            state
                .apply(&z(&[(7, 3)]), &z(&[(7, 5)]), LIMBS, &mut allow)
                .unwrap()
                .is_empty()
        );
        assert_eq!(
            state
                .apply(&z(&[]), &z(&[(7, -4)]), LIMBS, &mut allow)
                .unwrap(),
            z(&[(7, expected(op, 3, 1))])
        );
        assert_eq!(
...
fn invalid_negative_final_inputs_fail_even_for_suppressed_outputs() {
    ...
    assert_eq!(
        state
            .apply(&z(&[(17, -1), (17, 2)]), &z(&[]), LIMBS, &mut allow)
            .unwrap(),
        z(&[(17, 1)])
    );
}
...
fn promoted_weights_are_exact_and_arithmetic_exhaustion_is_atomic() {
    let large = ZWeight::from_i128(i128::MAX)
        .checked_add(&ZWeight::ONE, LIMBS)
        .unwrap();
    let left = ZSet::from_updates(
        [(1, large.checked_clone(LIMBS).unwrap())],
        LIMBS,
        &mut allow,
    )
    .unwrap();
    ...
    assert_eq!(
        difference.apply(&left, &right, LIMBS, &mut allow).unwrap(),
        z(&[(1, 1)])
    );
    let mut state = IncrementalSet::new(SetOperation::UnionAll);
    assert!(matches!(
        state.apply(&right, &z(&[(1, 1)]), LimbLimit::new(0), &mut allow),
        Err(SetError::Delta(ZSetError::Arithmetic(_)))
    ));
    assert_eq!(state, IncrementalSet::new(SetOperation::UnionAll));
    let output = state
        .apply(&right, &z(&[(1, 1)]), LIMBS, &mut allow)
        .unwrap();
    ...
    assert_eq!(
        state
            ...
            .unwrap(),
        output.negated(LIMBS, &mut allow).unwrap()
    );
    assert_eq!(state, IncrementalSet::new(SetOperation::UnionAll));
    ...
    assert_eq!(
        distinct
            .apply(&right, &right, LimbLimit::new(0), &mut allow)
            .unwrap(),
        z(&[(1, 1)])
    );
}
...
fn unchanged_support_is_neither_scanned_nor_cloned_and_empty_ticks_are_cancellable() {
    for op in OPS {
        ...
        large
            .apply(&unrelated, &unrelated, LIMBS, &mut allow)
            .unwrap();
        let mut measurements = Vec::new();
        for state in [&mut small, &mut large] {
            let mut events = Vec::new();
            let delta = state
                .apply(&z(&[(1, -1)]), &z(&[(1, 1)]), LIMBS, &mut |event| {
                    events.push(event);
                    Ok::<_, usize>(())
                })
                .unwrap();
        ...
        assert_eq!(
            large.apply(&z(&[]), &z(&[]), LIMBS, &mut |_| Err(7)),
