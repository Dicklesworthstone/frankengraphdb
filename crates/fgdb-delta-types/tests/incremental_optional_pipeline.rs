...
impl Pipeline {
    fn new() -> Self {
        Self { optional: IncrementalLeftJoin::new(), aggregate: IncrementalAggregate::new(), sink: ZSet::new() }
    }
    fn tick(
        &mut self,
        left: &Left,
        ...
        let optional = self.optional.prepare(left, right, LIMBS, control).map_err(|_| ())?;
        ...
        let projected = optional.delta().map(
            |(group, _, value)| Ok((*group, value.flatten())), LIMBS, control,
        ).map_err(|_| ())?;
        let aggregate = self.aggregate.prepare(&projected, LIMBS, control).map_err(|_| ())?;
    ...
    fn summaries(&self) -> BTreeMap<i32, Summary> {
        ...
        self.sink.iter().map(|((group, values), weight)| {
            assert_eq!(weight, &ZWeight::ONE);
            assert_eq!(self.aggregate.get(group), Some(values.as_ref()));
            (*group, (values.count_rows().to_i128().unwrap(), values.count_values().to_i128().unwrap(),
                values.sum().map(|sum| sum.to_i128().unwrap()), values.minimum(), values.maximum()))
        }).collect()
    }
}
fn seed() -> Pipeline {
    let mut pipeline = Pipeline::new();
    pipeline.tick(&z(&[((1, 10), 2), ((2, 20), 1)]),
        &z(&[((1, Some(7)), 2), ((1, None), 1)]), &mut allow).unwrap();
    pipeline
}
...
fn nullable_counts_and_extrema_follow_witness_loss_and_simultaneous_left_changes() {
    let mut state = seed();
    assert_eq!(state.summaries(), BTreeMap::from([
        (1, (6, 4, Some(28), Some(7), Some(7))), (2, (1, 0, None, None, None)),
    ]));
    ...
    assert_eq!(state.summaries(), BTreeMap::from([
        (1, (4, 0, None, None, None)), (2, (2, 2, Some(18), Some(9), Some(9))),
    ]));
    assert_eq!(old_sink, seed().sink, "previous output generation was mutated");
    ...
    state.tick(&ZSet::new(), &z(&[((2, Some(9)), -2), ((2, Some(-4)), 2)]), &mut allow).unwrap();
    assert_eq!(state.summaries().get(&2), Some(&(2, 2, Some(-8), Some(-4), Some(-4))));
}
...
fn every_pipeline_refusal_including_after_sink_preparation_can_retry_the_same_tick() {
    ...
    let mut calls = 0;
    expected.tick(&left, &right, &mut |_| { calls += 1; Ok(()) }).unwrap();
    for stop in 1..=calls {
        let mut state = seed();
        let mut seen = 0;
        assert!(state.tick(&left, &right, &mut |_| {
...
fn recompute(left: &BTreeMap<(i32, i32), i128>, right: &BTreeMap<(i32, Option<i128>), i128>)
    -> BTreeMap<i32, Summary>
{
    ...
    for (&(key, _), &count) in left {
        for _ in 0..count {
            ...
            if !found { rows.entry(key).or_default().push(None); }
        }
    }
    rows.into_iter().map(|(key, rows)| {
        let values: Vec<_> = rows.iter().filter_map(|value| *value).collect();
        let summary = (rows.len() as i128, values.len() as i128,
...
fn repeated_optional_and_aggregate_ticks_match_independent_whole_bag_recomputation() {
    ...
    for _ in 0..500 {
        ...
        let rk = ((next()%5) as i32, match next()%4 { 0 => None, n => Some(n as i128 - 2) });
        ...
        if lc == 0 { left.remove(&lk); } else { left.insert(lk, lc); }
        if rc == 0 { right.remove(&rk); } else { right.insert(rk, rc); }
