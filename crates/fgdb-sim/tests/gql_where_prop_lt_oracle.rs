...
//!
//! The strictly-less twin of the greater-than oracle, with the boundary
//! staked out from all sides again — now the survivor is BELOW it: the
//! `k = 0` carrier's dest alone answers, `k = 1` ON the boundary convicts
//! a `<=` reading, `k = 9` above it convicts both a flipped comparator
//! and a `<>` reading, and the keyless source convicts
//! treat-missing-as-passing. The derivation keeps dests whose source
//! carries `k` as an `Int` with `*v < 1`, missing key excluded inside.
...
/// Dests of `:R` edges whose source carries `k` as an `Int` strictly less
/// than 1 — the boundary and missing-key exclusions live here.
...
#[test]
fn source_property_less_than_equals_reference_filter() {
    let ((), report) = run_async_under_lab(0x4a_23, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-prop-lt-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("less-than MATCH executes");
        }
        ...
        assert_eq!(
            ...
            "the engine's less-than answer equals the oracle's derivation"
        );
        assert_eq!(rows, vec![VId(6)], "only the k=0 carrier's dest survives");
        assert!(
            !rows.contains(&VId(2)),
        ...
        assert!(
            !rows.contains(&VId(4)),
            "the above-boundary carrier fails: 9 is not-equal but not less — \
             a flipped comparator or a <> reading answers 4"
        );
        assert!(
            !rows.contains(&VId(8)),
