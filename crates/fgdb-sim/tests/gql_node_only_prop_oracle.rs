...
fn reference_people_with_k_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn node_only_label_and_property_match_equals_reference_vertices() {
    let ((), report) = run_async_under_lab(0x44_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-node-only-prop-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("node-only property MATCH executes");
        }
        ...
        assert_eq!(rows, reference_people_with_k_one(graph));
        assert_eq!(rows, vec![VId(1)]);
        assert!(!rows.contains(&VId(2)) && !rows.contains(&VId(4)));
    });
