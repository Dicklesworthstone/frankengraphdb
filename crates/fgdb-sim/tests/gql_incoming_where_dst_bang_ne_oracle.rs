//! **Incoming `WHERE a.k != 1 RETURN a`, differentially against the oracle**
//! (`fgdb-w5-parsers-nje-62-j5vd`).
//!
//! Under the flipped arrow `(a)<-[:R]-(b)` the pattern's `a` is the
//! stored edge's DESTINATION, so `WHERE a.k != 1 RETURN a` both filters
//! and projects `edge.dst` — and `!=` must alias `<>` exactly, so the
//! engine equals the SAME plain-code derivation: destinations of `:R`
//! edges that themselves carry `k` as an Int NOT equal to the literal,
//! missing-`k` excluded inside the derivation (never "trivially
//! unequal"). Every SOURCE in the fixture is keyless, so a kernel that
//! filters the origin instead of the pattern's `a` answers the empty
//! set; the `k = 1` dest fails the inequality and the keyless dest is
//! what a complement-of-equality executor wrongly admits.
...
const IN_DST_BANG_NE: &str = "MATCH (a)<-[:R]-(b) WHERE a.k != 1 RETURN a";

/// Destinations of `:R` edges that themselves carry `k` as an `Int`
/// different from 1 — the `<>` law verbatim, because `!=` is an alias
/// and not a new comparator: missing key out, inside the derivation.
fn reference_unequal_destinations(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn incoming_dest_bang_inequality_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x78_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-where-dst-bang-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            // Keyless sources throughout: a source-side filter answers [].
            ...
            let bind = RelationBind::new()
                ...
                .with_property("k", K);
            rows = db
                .execute_gql(IN_DST_BANG_NE, &bind)
                .expect("incoming WHERE a.k != 1 executes");
        }
        ...
        assert_eq!(
            rows,
            reference_unequal_destinations(graph),
        ...
        assert_eq!(
            rows,
            vec![VId(4)],
            "only the k=9 dest answers — every source is keyless, so a \
