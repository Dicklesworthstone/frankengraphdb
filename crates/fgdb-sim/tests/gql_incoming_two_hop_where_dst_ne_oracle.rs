...
//!
//! The inequality twin of `gql_incoming_two_hop_where_dst_oracle.rs`: in
...
//! (never "trivially unequal"). Every chain is stored reversed, so a
//! kernel that walks the pattern as OUTGOING composes no path at all and
//! answers the empty set; the `k = 1` far end fails the inequality, and
//! the keyless far end is exactly what a missing-as-unequal executor
//! wrongly admits.
...
const IN_TWO_DST_NE: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE c.k <> 1 RETURN c";
...
#[test]
fn incoming_two_hop_far_end_inequality_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x65_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-dst-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            let bind = RelationBind::new()
                ...
                .with_property("k", K);
            rows = db
                .execute_gql(IN_TWO_DST_NE, &bind)
