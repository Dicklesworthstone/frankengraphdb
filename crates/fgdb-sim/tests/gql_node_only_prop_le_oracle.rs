...
//!
//! The inclusive-lower twin of `gql_node_only_prop_ge_oracle.rs`: the
//! filtered scan must equal the reference's labeled vertices that carry `k`
//! as an Int less than or equal to the literal. The `k = 1` Person is the
//! boundary that separates `<=` from `<` (it must be IN), the `k = 9`
...
/// Labeled vertices whose `k` is an Int less than or equal to 1 — a
/// missing `k` excludes the vertex and a missing label excludes it too,
/// mirroring the executor's law.
fn reference_people_with_k_le_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn node_only_property_le_equals_reference_vertices() {
    let ((), report) = run_async_under_lab(0x5d_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-node-only-prop-le-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("node-only less-or-equal MATCH executes");
        }
        ...
        assert_eq!(
            rows,
            reference_people_with_k_le_one(graph),
            "the engine equals the reference derivation"
        );
        assert_eq!(
            rows,
            vec![VId(1), VId(3)],
            "the k=1 boundary Person is IN — a strict < answers [3] and fails"
        );
        assert!(
            !rows.contains(&VId(2)),
            "the k=9 Person fails <= — a <> in disguise answers [2, 3]"
        );
        assert!(
            !rows.contains(&VId(4)),
            "a missing k excludes the Person — not vacuously lesser-or-equal"
        );
        assert!(
            !rows.contains(&VId(5)),
