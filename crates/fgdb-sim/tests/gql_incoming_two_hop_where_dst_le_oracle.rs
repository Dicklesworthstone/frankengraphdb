...
//!
//! The inclusive-lower twin of `gql_incoming_two_hop_where_dst_ge_oracle.rs`:
//! in `MATCH (a)<-[:R]-(b)<-[:S]-(c)` the stored edges run `b -R-> a` and
...
//! that `:S` edge's own source, kept when it carries `k` as an Int less
//! than or equal to the literal, missing-`k` excluded inside the
...
//! set; the `k = 1` boundary far end is the ONLY survivor, so a strict
//! `<` in disguise answers `[]` and fails; the `k = 9` far end separates
//! `<=` from `<>`, and the keyless far end pins the exclusion.
...
/// that carry `k` as an Int less than or equal to 1. The reversed
/// composition AND the missing-key exclusion both live here, in the
/// derivation.
fn reference_incoming_far_ends_with_k_le_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn incoming_two_hop_far_end_le_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x69_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-dst-le-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            // The k=1 boundary far end — the ONLY survivor, stored
            // reversed: 3{k:1} -S-> 2 -R-> 1.
            ...
            // A k=9 far end: separates <= from <>. 6 -S-> 5 -R-> 4.
        ...
        assert_eq!(
            rows,
            reference_incoming_far_ends_with_k_le_one(graph),
            "the engine equals the reversed composed derivation"
        );
        assert_eq!(
            rows,
            vec![VId(3)],
            "the k=1 boundary far end is the only survivor — a strict < in \
             disguise answers [] and fails, as does an outgoing walk on \
             this reversed fixture"
        );
        assert!(
            !rows.contains(&VId(6)),
            "the k=9 far end fails <= — a <> in disguise answers [6]"
        );
        assert!(
            !rows.contains(&VId(9)),
