...
//!
//! The inclusive twin of `gql_incoming_two_hop_where_src_lt_oracle.rs`:
...
//! `k` as an Int greater than or equal to the literal, every `:S` edge
//! arriving at that `:R` edge's source contributes its own source,
//! missing-`k` excluded inside the derivation. `>= 9` keeps exactly the
//! `k = 9` boundary near end (a strict `>` in disguise answers `[]`),
//! `>= 1` answers `[3, 6]` against its own derivation, every far-end `c`
//! is keyless so `WHERE c.k >= 9` answers the empty set, and the
...
const IN_TWO_SRC_GE_NINE: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE a.k >= 9 RETURN c";
const IN_TWO_SRC_GE_ONE: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE a.k >= 1 RETURN c";
const IN_TWO_DST_GE: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE c.k >= 9 RETURN c";
...
#[test]
fn incoming_two_hop_near_end_ge_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x6f_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-src-ge-oracle-{}",
            std::process::id()
        ));
        ...
        let outgoing_rows;
        {
            ...
            // The kept chain: the k=9 BOUNDARY near end, stored reversed:
            // 3 -S-> 2 -R-> 1{k:9}. A strict > in disguise drops it.
            ...
            // A k=1 near end: below the bound. 6 -S-> 5 -R-> 4{k:1}.
            ...
            let bind = RelationBind::new()
                ...
                .with_property("k", K);
            rows = db
                .execute_gql(IN_TWO_SRC_GE_NINE, &bind)
                .expect("incoming two-hop WHERE a.k >= 9 executes");
            above_one_rows = db
                .execute_gql(IN_TWO_SRC_GE_ONE, &bind)
                .expect("the >= 1 spelling executes");
            far_end_rows = db
                .execute_gql(IN_TWO_DST_GE, &bind)
        ...
        assert_eq!(
            rows,
            vec![VId(3)],
            "the k=9 boundary near end is IN — a strict > in disguise \
             answers [] and fails"
        );
        assert!(
            !rows.contains(&VId(6)),
            "the k=1 near end's far end sits below the bound"
        );
        assert!(
            !rows.contains(&VId(9)),
        ...
        assert_eq!(
            ...
            "the >= 1 spelling equals its own derivation"
        );
        assert_eq!(
            above_one_rows,
            vec![VId(3), VId(6)],
            ">= 1 keeps both keyed near ends"
        );
        assert!(
            far_end_rows.is_empty(),
            "every far-end c is keyless, so WHERE c.k >= 9 answers [] — a \
