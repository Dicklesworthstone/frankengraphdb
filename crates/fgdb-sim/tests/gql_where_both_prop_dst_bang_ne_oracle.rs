...
//! (`fgdb-w5-parsers-nje.57`, the nje.65 slice).
//!
//! The mirror of `gql_where_both_prop_bang_ne_oracle.rs`: here the DEST
//! conjunct wears the C-style `!=` while the source conjunct keeps the
//! diamond `<>`, and both must mean the SAME comparator — so the engine
//! equals the verbatim `<>`/`<>` derivation: a row survives only when
//! the SOURCE carries `k` as an `Int` other than 1 AND the DEST carries
//! `m` as an `Int` other than 9, a missing key on EITHER end excluded
//! inside the derivation. Every failure mode keeps its own edge,
//! leaving exactly one surviving row.
...
#[test]
fn both_end_dst_bang_inequality_equals_reference_filter() {
    let ((), report) = run_async_under_lab(0x7b_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-both-prop-dst-bang-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("dest-bang mixed conjunction MATCH executes");
        }
