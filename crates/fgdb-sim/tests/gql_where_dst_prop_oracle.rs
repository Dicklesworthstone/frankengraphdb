...
#[test]
fn destination_property_match_equals_reference_filter() {
    let ((), report) = run_async_under_lab(0x42_02, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-dst-prop-oracle-{}",
            std::process::id()
        ));
        ...
        assert_eq!(rows, reference_sources(graph));
        assert_eq!(rows, vec![VId(1)]);
        assert!(!rows.contains(&VId(3)) && !rows.contains(&VId(5)));
    });
