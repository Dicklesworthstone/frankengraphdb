...
//! (nje.66, bead `fgdb-w5-parsers-nje.58`).
//!
//! Both conjuncts wear the C-style spelling: `!=` must alias `<>` on the
//! source AND the dest at once, so the engine equals the verbatim
//! `<>`/`<>` derivation — a row survives only when the SOURCE carries
//! `k` as an `Int` other than 1 AND the DEST carries `m` as an `Int`
//! other than 9, a missing key on EITHER end excluded inside the
//! derivation. Every failure mode keeps its own edge, leaving exactly
//! one surviving row.
...
#[test]
fn both_end_double_bang_inequality_equals_reference_filter() {
    let ((), report) = run_async_under_lab(0x7c_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-both-prop-both-bang-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("double-bang conjunction MATCH executes");
        }
        ...
        assert_eq!(
            ...
            "the double != conjunction equals the <>/<> derivation — one \
             comparator in two spellings on both ends"
        );
