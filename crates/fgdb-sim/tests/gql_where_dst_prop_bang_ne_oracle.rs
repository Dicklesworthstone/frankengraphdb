...
//!
//! The C-style spelling of `gql_where_dst_prop_ne_oracle.rs`: `!=` must
//! alias `<>` exactly, so the engine's answer equals the SAME plain-code
//! derivation — keep sources of `:R` edges whose DEST carries `k` as an
//! `Int` other than 1, with the missing-key exclusion inside the
//! derivation itself. The concrete `[3]` convicts three kernels at once:
//! the complement-of-equality reading answers 5 (its dest carries no
//! `k`), a source-side filter answers nothing here (no SOURCE carries `k`
//! at all), and a dest projection answers 4 instead of 3.
...
/// than 1 — the `<>` law verbatim, because `!=` is an alias and not a new
/// comparator: the missing-key exclusion and the dest-side filtering both
...
#[test]
fn dest_property_bang_inequality_equals_reference_filter() {
    let ((), report) = run_async_under_lab(0x6a_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-dst-prop-bang-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("dest-property != MATCH executes");
        }
