...
#[test]
fn source_property_bang_inequality_equals_reference_filter() {
    let ((), report) = run_async_under_lab(0x58_02, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-prop-bang-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("bang-property-inequality MATCH executes");
        }
        ...
        assert_eq!(
            ...
            "the engine's bang inequality answer equals the oracle's derivation"
        );
