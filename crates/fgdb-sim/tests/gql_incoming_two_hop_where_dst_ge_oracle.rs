...
//!
//! The inclusive twin of `gql_incoming_two_hop_where_dst_gt_oracle.rs`:
...
//! greater than or equal to the literal, missing-`k` excluded inside the
...
//! set; the `k = 1` boundary far end must be IN (a strict `>` in
//! disguise drops it and fails), and the keyless far end pins the
//! exclusion.
...
const IN_TWO_DST_GE: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE c.k >= 1 RETURN c";

/// Far ends of incoming two-hop paths — `c -S-> b -R-> a` in storage —
/// that carry `k` as an Int greater than or equal to 1. The reversed
/// composition AND the missing-key exclusion both live here, in the
/// derivation.
fn reference_incoming_far_ends_with_k_ge_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn incoming_two_hop_far_end_ge_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x68_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-dst-ge-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            // The k=1 boundary far end, stored reversed:
            // 3{k:1} -S-> 2 -R-> 1 — a strict > in disguise drops it.
            ...
            // A k=9 far end: comfortably in. 6 -S-> 5 -R-> 4.
        ...
        assert_eq!(
            rows,
            reference_incoming_far_ends_with_k_ge_one(graph),
        ...
        assert_eq!(
            ...
            "the k=1 boundary far end is IN beside the k=9 one — a strict \
             > in disguise answers [6] alone and fails; an outgoing walk \
             on this reversed fixture answers []"
        );
        assert!(
            !rows.contains(&VId(9)),
