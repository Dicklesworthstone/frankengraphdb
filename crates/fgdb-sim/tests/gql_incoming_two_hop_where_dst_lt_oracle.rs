...
const IN_TWO_DST_LT: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE c.k < 1 RETURN c";

/// Far ends of incoming two-hop paths — `c -S-> b -R-> a` in storage —
/// that carry `k` as an Int strictly less than 1. The reversed
/// composition AND the missing-key exclusion both live here, in the
/// derivation.
fn reference_incoming_far_ends_with_k_less_than_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn incoming_two_hop_far_end_less_than_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x67_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-dst-lt-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            // boundary that convicts a <= in disguise.
            ...
            let bind = RelationBind::new()
                ...
                .with_property("k", K);
            rows = db
                .execute_gql(IN_TWO_DST_LT, &bind)
        ...
        assert_eq!(
            rows,
            reference_incoming_far_ends_with_k_less_than_one(graph),
