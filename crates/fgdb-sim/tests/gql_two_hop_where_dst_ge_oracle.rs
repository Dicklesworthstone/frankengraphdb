...
//!
//! The inclusive twin of `gql_two_hop_where_dst_gt_oracle.rs`: `RETURN c`
//! answers the far ends of `:R`-then-`:S` paths whose hop-2 destination
//! carries `k` as an Int greater than or equal to the literal, and the
//! derivation composes exactly that in plain code — missing-`k` excluded
//! inside it. Four chains: the `k = 1` boundary far end must be IN (a
//! strict `>` disguise answers `[6]` alone and fails), the `k = 0` far
//! end separates `>=` from `<>`, the keyless far end pins the exclusion,
//! and every hop-1 origin is keyless so a source-side filter answers the
//! empty set — each cheat breaks the oracle equality, not merely a
//! pinned vector.
...
const TWO_DST_GE: &str = "MATCH (a)-[:R]->(b)-[:S]->(c) WHERE c.k >= 1 RETURN c";

/// Far ends of two-hop paths that themselves carry `k` as an Int greater
/// than or equal to 1 — the composition AND the missing-key exclusion
/// both live here, in the derivation.
fn reference_far_ends_with_k_ge_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn two_hop_far_end_ge_filter_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x62_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-two-hop-where-dst-ge-oracle-{}",
            std::process::id()
        ));
        ...
        assert_eq!(
            rows,
            reference_far_ends_with_k_ge_one(graph),
            "the engine equals the composed far-end derivation"
        );
        assert_eq!(
            rows,
            vec![VId(3), VId(6)],
            "the k=1 boundary far end is IN beside the k=9 one — a strict \
             > in disguise answers [6] alone and fails"
        );
        assert!(
