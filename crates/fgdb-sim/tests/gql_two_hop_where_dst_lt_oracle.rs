...
//!
//! The less-than twin of `gql_two_hop_where_dst_gt_oracle.rs`: `RETURN c`
//! answers the far ends of `:R`-then-`:S` paths whose hop-2 destination
//! carries `k` as an Int strictly less than the literal, and the
//! derivation composes exactly that in plain code — missing-`k` excluded
//! inside it. Four chains: the `k = 1` far end separates `<` from `<=`,
//! the `k = 9` far end separates it from `<>` (and from `>`), the keyless
//! far end pins the exclusion, and every hop-1 origin is keyless so a
//! source-side filter answers the empty set — each cheat breaks the
...
const TWO_DST_LT: &str = "MATCH (a)-[:R]->(b)-[:S]->(c) WHERE c.k < 1 RETURN c";

/// Far ends of two-hop paths that themselves carry `k` as an Int strictly
/// less than 1 — the composition AND the missing-key exclusion both live
/// here, in the derivation.
fn reference_far_ends_with_k_less_than_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn two_hop_far_end_less_than_filter_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x61_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-two-hop-where-dst-lt-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            // A k=9 far end: what a <> (or >) in disguise would admit.
            ...
            // The kept path: keyless origin 10, via 11, k=0 far end 12.
        ...
        assert_eq!(
            rows,
            reference_far_ends_with_k_less_than_one(graph),
            "the engine equals the composed far-end derivation"
        );
        assert_eq!(
            rows,
            vec![VId(12)],
            "only the k=0 far end — every origin is keyless, so a kernel \
