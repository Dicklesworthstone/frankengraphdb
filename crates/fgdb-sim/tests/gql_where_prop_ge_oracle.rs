...
//!
//! The inclusive comparison is judged against a plain-code derivation over
//! the reference oracle's own tables: dests of edges whose SOURCE carries
//! `k` as an Int greater than or equal to the literal — missing-`k`
//! excluded in the derivation itself. The `k = 1` source is what separates
//! `>=` from `>` (its dest must be IN), the `k = 0` source separates it
...
/// Dests of `:R` edges whose source carries `k` as an Int greater than or
/// equal to 1 — a missing `k` excludes the edge, mirroring the executor's
/// law.
fn reference_dests_of_ge_sources(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn source_property_ge_equals_reference_dests() {
    let ((), report) = run_async_under_lab(0x58_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-prop-ge-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            let bind = RelationBind::new()
                ...
                .with_property("k", K);
            rows = db
                .execute_gql(SRC_GE, &bind)
                .expect("source greater-or-equal MATCH executes");
        }
        ...
        assert_eq!(
            rows,
            reference_dests_of_ge_sources(graph),
            "the engine equals the reference derivation"
        );
        assert_eq!(
            rows,
            vec![VId(2), VId(4)],
            "the k=1 boundary source is IN — a strict > answers [4] and fails"
        );
        assert!(
            !rows.contains(&VId(6)),
            "the k=0 source fails >= — a <> in disguise answers [2, 4, 6]"
        );
        assert!(
            !rows.contains(&VId(8)),
            "the missing-k source is OUT, not vacuously greater-or-equal"
        );
    });
