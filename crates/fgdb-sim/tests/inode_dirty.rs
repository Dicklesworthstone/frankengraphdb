...
impl Reference {
    ...
    fn open(&mut self, id: usize, name: &'static str, mode: OpenMode) {
        assert!(!self.handles.contains_key(&id), "table reused an open handle");
...
async fn check_state(
    ...
    for name in names {
        ...
        if let Some(&inode) = model.names.get(name) {
            assert_eq!(actual?, model.inodes[inode].durable, "{context}: durable {name}");
        } else {
            assert_eq!(actual.expect_err(context).kind(), io::ErrorKind::NotFound,
                "{context}: absent {name}");
        }
    }
    for (&id, actual) in handles {
        ...
        if expected.generation == model.generation {
            assert_eq!(actual.image()?, model.inodes[expected.inode].volatile,
    ...
    assert_eq!(events.iter().filter(|event| matches!(event.kind, FaultKind::FsyncLie { .. })).count(),
        model.file_lies, "{context}: eligible file sync lies");
    assert_eq!(events.iter().filter(|event| matches!(event.kind, FaultKind::DirentSyncLie { .. })).count(),
        model.directory_lies, "{context}: eligible directory sync lies");
    Ok(())
}
...
async fn refuse_stale(file: &mut FaultFile<UnixVfs>, context: &str) {
    ...
    assert!(file.sync_data().await.is_err(), "{context}: stale sync_data");
    assert!(file.metadata().await.is_err(), "{context}: stale metadata");
    assert!(file.set_len(0).await.is_err(), "{context}: stale set_len");
    assert!(poll_fn(|cx| Pin::new(&mut *file).poll_write(cx, b"bad")).await.is_err(),
        "{context}: stale write");
    assert!(poll_fn(|cx| Pin::new(&mut *file).poll_seek(cx, SeekFrom::Start(0))).await.is_err(),
        "{context}: stale seek");
    let mut bytes = [0; 1];
    let mut buffer = ReadBuf::new(&mut bytes);
    assert!(poll_fn(|cx| Pin::new(&mut *file).poll_read(cx, &mut buffer)).await.is_err(),
        "{context}: stale read");
    assert!(poll_fn(|cx| Pin::new(&mut *file).poll_flush(cx)).await.is_err(),
        "{context}: stale flush");
    assert!(poll_fn(|cx| Pin::new(&mut *file).poll_shutdown(cx)).await.is_err(),
...
async fn run_case(case: &Case, root: &Path) -> io::Result<()> {
    ...
    let vfs = FaultVfs::unix(FaultPlan {
        ...
        dirent_loss: if case.lose_names { Trigger::Always } else { Trigger::Never },
        ..FaultPlan::faultless()
    });
    ...
    for step in case.steps {
        match step {
            Step::Open(_, name, _) => { names.insert(name); }
            Step::Rename(from, to) => { names.insert(from); names.insert(to); }
            _ => {}
        }
    }
    for (index, &step) in case.steps.iter().enumerate() {
        ...
        match step {
            ...
            Step::Seek(id, offset) => {
                let file = handles.get_mut(&id).expect("actual handle");
                assert_eq!(poll_fn(|cx| Pin::new(&mut *file).poll_seek(cx, SeekFrom::Start(offset))).await?,
            ...
            Step::Read(id, count) => {
                ...
                assert_eq!(buffer.filled(), model.read(id, count), "{context}: cursor read");
            }
            Step::SyncAll(id) => {
            ...
            Step::RefuseStale(id) => {
                assert_ne!(model.handles[&id].generation, model.generation,
...
fn inode_dirty_reference_sequences() {
    ...
    use Step::{Close, Crash, Open, Read, RefuseStale, Rename, Seek, SyncAll, SyncData,
        SyncDirectory, Write};
    ...
    let cases = [
        Case {
            ...
            initial: &[("file", b"")],
            file_lies: Lies::Never, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "file", Existing), Open(1, "file", Existing),
                Write(0, b"abcd"), Read(1, 2), Write(1, b"XY"), Write(0, b"ef"),
                Seek(1, 0), Read(1, 6), SyncData(1), Crash,
                RefuseStale(0), RefuseStale(1), Open(2, "file", Existing), Read(2, 6),
            ],
        },
        Case {
            name: "close_reopen_retains_dirty_after_one_lie",
            initial: &[("file", b"")],
            file_lies: Lies::First, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "file", Existing), Write(0, b"payload"), SyncAll(0), Close(0),
                Open(1, "file", Existing), Read(1, 7), SyncData(1), Close(1), Crash,
                Open(2, "file", Existing), Read(2, 7),
            ],
        },
        Case {
            ...
            file_lies: Lies::First, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "staging", Create), Write(0, b"published bytes"), SyncAll(0), Close(0),
                Rename("staging", "published"), Open(1, "published", Existing),
                SyncData(1), SyncDirectory, Crash, RefuseStale(1),
                Open(2, "published", Existing), Read(2, 15),
            ],
        },
        Case {
            name: "staging_both_file_syncs_lie_loses_bytes_not_name",
            initial: &[],
            file_lies: Lies::Always, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "staging", Create), Write(0, b"lost bytes"), SyncAll(0), Close(0),
                Rename("staging", "published"), Open(1, "published", Existing),
                SyncData(1), SyncDirectory, Crash, RefuseStale(1),
                Open(2, "published", Existing), Read(2, 10),
            ],
        },
        Case {
            name: "staging_file_and_directory_syncs_lie_loses_name",
            initial: &[],
            file_lies: Lies::Always, directory_lies: Lies::Always, lose_names: true,
            steps: &[
                Open(0, "staging", Create), Write(0, b"lost bytes"), SyncAll(0), Close(0),
                Rename("staging", "published"), Open(1, "published", Existing),
                SyncData(1), SyncDirectory, Crash, RefuseStale(1),
        ...
        Case {
            ...
            file_lies: Lies::First, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "staging", Existing), Write(0, b"before"), SyncData(0),
                Rename("staging", "published"), Open(1, "published", Existing),
                Seek(1, 6), Write(1, b"-after"), SyncAll(0), SyncDirectory,
                Crash, RefuseStale(0), Open(2, "published", Existing), Read(2, 12),
            ],
        },
        Case {
            ...
            steps: &[
                Open(0, "file", Existing), Open(1, "file", Existing),
                Seek(0, 2), Write(0, b"WXYZ"), Seek(1, 4), Write(1, b"12"),
                SyncAll(0), Seek(1, 513), Write(1, b"end"), Close(1),
                SyncData(0), Crash, RefuseStale(0), Open(2, "file", Existing),
                Read(2, 8), Seek(2, 510), Read(2, 6),
            ],
        },
        Case {
            ...
            file_lies: Lies::First, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "file", Existing), SyncAll(0), Open(1, "file", Create),
                Write(1, b""), SyncData(1), Write(0, b"new"), Open(2, "file", Create),
                SyncAll(2), Close(0), Close(1), Close(2),
                Open(3, "file", Existing), SyncData(3), Crash, Open(4, "file", Existing),
            ],
        },
        Case {
            ...
            file_lies: Lies::First, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "file", Existing), Open(1, "file", Truncate), SyncData(1),
                Close(1), Open(2, "file", Existing), SyncAll(0), Crash,
                RefuseStale(0), RefuseStale(2), Open(3, "file", Existing), Read(3, 16),
            ],
        },
        Case {
            ...
            file_lies: Lies::Always, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "file", Truncate), SyncAll(0), Close(0),
                Open(1, "file", Existing), SyncData(1), Crash,
                RefuseStale(1), Open(2, "file", Existing), Read(2, 16),
            ],
        },
        Case {
            ...
            file_lies: Lies::First, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "file", Existing), Write(0, b"volatile"), Open(1, "file", Truncate),
                SyncData(1), Close(1), Open(2, "file", Create), SyncAll(2),
                Write(0, b"suffix"), SyncData(2), Crash, Open(3, "file", Existing), Read(3, 20),
            ],
        },
        Case {
            ...
            file_lies: Lies::Never, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "file", Existing), Write(0, b"prefix"), SyncData(0),
                Write(0, b"-suffix"), Close(0), Open(1, "file", Existing), Read(1, 13),
                Crash, RefuseStale(1), Open(2, "file", Existing), Read(2, 13),
            ],
        },
        Case {
            name: "directory_noop_then_lie_then_honest_settles_publish",
            initial: &[],
            file_lies: Lies::Never, directory_lies: Lies::First, lose_names: true,
            steps: &[
                SyncDirectory, Open(0, "staging", Create), Write(0, b"data"), SyncAll(0),
                Rename("staging", "published"), SyncDirectory, SyncDirectory,
                Crash, Open(1, "published", Existing), Read(1, 4),
            ],
        },
        Case {
            ...
            file_lies: Lies::Never, directory_lies: Lies::Never, lose_names: true,
            steps: &[
                Open(0, "staging", Existing), Write(0, b"newsrc"), SyncData(0),
                Rename("staging", "published"), Open(1, "published", Existing), Read(1, 6),
                Crash, RefuseStale(0), RefuseStale(1),
                Open(2, "staging", Existing), Open(3, "published", Existing), Read(2, 6), Read(3, 6),
            ],
        },
    ];
    let unique = SystemTime::now().duration_since(UNIX_EPOCH).expect("clock").as_nanos();
    let root = std::env::temp_dir().join(format!("fgdb-inode-dirty-{}-{unique}", std::process::id()));
...
fn run_delayed_case(action: DuringSync) {
    ...
    let (task_id, mut handle) = runtime.state.create_task(region, Budget::INFINITE, async move {
        let unique = SystemTime::now().duration_since(UNIX_EPOCH).expect("clock").as_nanos();
        let root = std::env::temp_dir().join(format!(
            "fgdb-inode-delay-{}-{unique}-{action:?}", std::process::id()));
        std::fs::create_dir_all(&root)?;
        let path = root.join("file");
        std::fs::write(&path, b"")?;
        let cx = asupersync::Cx::current().expect("lab task has Cx");
        let vfs = FaultVfs::unix_with_clock(FaultPlan {
        ...
        assert!(first_poll.is_pending(), "{action:?}: sync must really suspend");
        assert_eq!(vfs.pending_latency_paths(), vec![path.clone()], "{action:?}: waiter");
        assert_eq!(vfs.read(&path).await?, b"", "{action:?}: no premature durability");
        match action {
            DuringSync::Write => {
                poll_fn(|cx| Pin::new(&mut writer).poll_seek(cx, SeekFrom::Start(0))).await?;
                let accepted = poll_fn(|cx| Pin::new(&mut writer).poll_write(cx, b"replacement-plus")).await?;
                assert_eq!(accepted, 16, "write during delay");
                assert_eq!(observer.image()?, b"replacement-plus", "shared image during delay");
                pending.await?;
                ...
                assert_eq!(vfs.read(&path).await?, b"replacement-plus", "racing write was not forgotten");
                vfs.crash().await?;
                assert_eq!(vfs.open(&path, &options).await?.image()?, b"replacement-plus");
            }
            DuringSync::Crash => {
                vfs.crash().await?;
                assert!(pending.await.is_err(), "pre-crash delayed sync must refuse after wake");
                assert_eq!(vfs.read(&path).await?, b"", "resumed old sync cannot resurrect bytes");
                assert_eq!(vfs.open(&path, &options).await?.image()?, b"");
                refuse_stale(&mut writer, "crash during delayed sync").await;
            }
            DuringSync::Cancel => {
                drop(pending);
                assert!(vfs.pending_latency_paths().is_empty(), "cancel retires waiter");
                assert!(vfs.events().is_empty(), "cancel is not a completed delay or sync");
                drop(writer);
                drop(observer);
                let reopened = vfs.open(&path, &options).await?;
                assert_eq!(reopened.image()?, b"original", "cancel and close retain dirty inode");
                reopened.sync_data().await?;
                vfs.crash().await?;
                assert_eq!(vfs.read(&path).await?, b"original", "another handle persists cancelled sync data");
            }
        }
        assert!(vfs.pending_latency_paths().is_empty(), "{action:?}: no waiter leak");
        Ok::<(), io::Error>(())
    }).expect("lab task spawns");
    runtime.scheduler.lock().schedule(task_id, Budget::INFINITE.priority);
    let report = runtime.run_with_auto_advance();
    assert!(matches!(report.termination, AutoAdvanceTermination::Quiescent),
        "{action:?}: lab did not quiesce: {report:?}");
    let lab_report = runtime.report();
    assert!(lab_report.lab_test_passed(), "{action:?}: lab failed: {lab_report:?}");
    let result = handle.try_join().expect("task joined").expect("task finished");
