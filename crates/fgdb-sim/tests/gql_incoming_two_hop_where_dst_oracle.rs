...
//!
//! The incoming twin of `gql_two_hop_where_dst_oracle.rs`: in
...
//! that `:S` edge's own source, kept when it carries `k` as an Int equal
//! to the literal, missing-`k` excluded inside the derivation. Every
//! chain is stored reversed, so a kernel that walks the pattern as
//! OUTGOING composes no path at all and answers the empty set; the
//! `k = 9` far end separates `=` from trivially-true, and the keyless
//! far end pins the exclusion.
...
/// that carry `k` as an Int equal to 1. The reversed composition AND the
/// missing-key exclusion both live here, in the derivation.
fn reference_incoming_far_ends_with_k_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn incoming_two_hop_far_end_filter_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x64_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-dst-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            // The kept chain, stored reversed: 3{k:1} -S-> 2 -R-> 1.
        ...
        assert_eq!(
            rows,
            reference_incoming_far_ends_with_k_one(graph),
            "the engine equals the reversed composed derivation"
        );
        assert_eq!(
            rows,
            vec![VId(3)],
            "only the k=1 far end — every chain is stored reversed, so a \
