...
//!
//! The C-style twin of `gql_incoming_two_hop_where_dst_ne_oracle.rs`: in
...
//! that `:S` edge's own source, kept when it carries `k` as an Int NOT
//! equal to the literal, missing-`k` excluded inside the derivation
//! (never "trivially unequal"). Every chain is stored reversed, so the
//! OUTGOING `<>` spelling (already grammar) composes no path at all and
//! answers the empty set — the executed direction control; the OUTGOING
//! `!=` spelling stays a Parse refusal this slice and is not executed.
//! The `k = 1` far end fails the inequality, and the keyless far end is
...
const OUT_TWO_DST_NE: &str = "MATCH (a)-[:R]->(b)-[:S]->(c) WHERE c.k <> 1 RETURN c";

/// Far ends of incoming two-hop paths — `c -S-> b -R-> a` in storage —
/// that carry `k` as an Int different from 1. The reversed composition
/// AND the missing-key exclusion both live here, in the derivation.
fn reference_incoming_far_ends_with_k_not_one(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn incoming_two_hop_far_end_bang_inequality_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x65_55, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-dst-bang-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            // A k=1 far end, stored reversed: 3{k:1} -S-> 2 -R-> 1.
            ...
            // The direction control: the OUTGOING <> spelling (already
            // grammar) composes nothing on the reversed chains. The
            // outgoing != spelling stays Parse this slice and is not run.
            assert!(
                db.execute_gql(OUT_TWO_DST_NE, &bind)
        ...
        assert_eq!(
            rows,
            reference_incoming_far_ends_with_k_not_one(graph),
