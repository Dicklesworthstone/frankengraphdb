...
//!
//! The ordered twin of `gql_incoming_two_hop_where_src_ne_oracle.rs`: in
...
//! `k` as an Int strictly greater than the literal, every `:S` edge
//! arriving at that `:R` edge's source contributes its own source,
//! missing-`k` excluded inside the derivation. The `k = 1` near end is
//! the boundary a `>=` in disguise wrongly admits; `> 0` answers
//! `[3, 6]`, which a `<>` in disguise (answering `[3]` alone) cannot
//! match; every far-end `c` is keyless so `WHERE c.k > 1` answers the
//! empty set, and reversed storage defeats an outgoing walk.
...
const IN_TWO_SRC_GT: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE a.k > 1 RETURN c";
const IN_TWO_SRC_GT_ZERO: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE a.k > 0 RETURN c";
...
#[test]
fn incoming_two_hop_near_end_greater_than_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x6d_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-src-gt-oracle-{}",
            std::process::id()
        ));
        ...
        let far_end_rows;
        {
            ...
            // A k=1 near end: the boundary a >= in disguise admits.
            ...
            let bind = RelationBind::new()
                ...
                .with_property("k", K);
            rows = db
                .execute_gql(IN_TWO_SRC_GT, &bind)
                .expect("incoming two-hop WHERE a.k > 1 executes");
            above_zero_rows = db
                .execute_gql(IN_TWO_SRC_GT_ZERO, &bind)
                .expect("the > 0 spelling executes");
            far_end_rows = db
        ...
        assert_eq!(
            rows,
            vec![VId(3)],
            "only the k=9 near end's far end — an outgoing walk on this \
             reversed fixture composes nothing and answers []"
        );
        assert!(
            !rows.contains(&VId(6)),
            "the k=1 near end's far end fails strict greater-than — a >= \
        ...
        assert_eq!(
            above_zero_rows,
            reference_far_ends_of_kept_near_ends(graph, |k| k > 0),
            "the > 0 spelling equals its own derivation"
        );
        assert_eq!(
            above_zero_rows,
            vec![VId(3), VId(6)],
            "> 0 keeps both keyed near ends — a <> in disguise answers \
             [3] alone and fails"
        );
        assert!(
