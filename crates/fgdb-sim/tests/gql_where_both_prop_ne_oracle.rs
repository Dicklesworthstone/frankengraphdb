...
//!
//! The two-ended inequality conjunction: a row survives only when the
//! SOURCE carries `k` as an `Int` other than 1 AND the DEST carries `m`
//! as an `Int` other than 9 — and a missing key on EITHER end is out, in
//! the derivation itself, so neither conjunct can quietly decay into a
//! complement-of-equality. Every failure mode gets its own edge: a source
//! failing the `k` conjunct, a dest failing the `m` conjunct, a keyless
//! source, and a keyless dest — leaving exactly one surviving row, whose
//! absence-mates are each asserted out by name.
...
/// carries `m != 9`, both as `Int`s — missing either key excludes the
/// row, inside the derivation.
...
#[test]
fn both_end_inequality_equals_reference_filter() {
    let ((), report) = run_async_under_lab(0x44_17, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-both-prop-ne-oracle-{}",
            std::process::id()
        ));
        ...
        assert_eq!(
            ...
            "the engine's conjunction answer equals the oracle's derivation"
        );
        assert_eq!(rows, vec![VId(8)], "one edge survives both conjuncts");
        assert!(
            !rows.contains(&VId(2)),
            "the k=1 source's dest fails the source conjunct"
        );
        assert!(
            !rows.contains(&VId(4)),
            "the m=9 dest fails the dest conjunct"
        );
        assert!(
            !rows.contains(&VId(6)),
