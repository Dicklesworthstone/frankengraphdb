...
//!
//! The inequality twin of `gql_incoming_two_hop_where_src_oracle.rs`: in
...
//! `k` as an Int NOT equal to the literal, every `:S` edge arriving at
//! that `:R` edge's source contributes its own source, missing-`k`
//! excluded inside the derivation (never "trivially unequal"). Every
//! far-end `c` in the fixture is keyless, so `WHERE c.k <> 1` answers
//! the empty set — the separation between filtering `a` and filtering
//! `c`; the `k = 1` dest's chain fails the inequality, the keyless
//! dest's chain is what missing-as-unequal wrongly admits, and reversed
...
const IN_TWO_SRC_NE: &str = "MATCH (a)<-[:R]-(b)<-[:S]-(c) WHERE a.k <> 1 RETURN c";
...
#[test]
fn incoming_two_hop_near_end_inequality_equals_its_reference() {
    let ((), report) = run_async_under_lab(0x6c_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-incoming-two-hop-where-src-ne-oracle-{}",
            std::process::id()
        ));
        ...
        let far_end_rows;
        {
            ...
            let bind = RelationBind::new()
                ...
                .with_property("k", K);
            rows = db
                .execute_gql(IN_TWO_SRC_NE, &bind)
        ...
        assert_eq!(
            rows,
            reference_far_ends_of_k_not_one_near_ends(graph),
