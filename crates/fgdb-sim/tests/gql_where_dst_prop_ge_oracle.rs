...
//!
//! The dest twin of `gql_where_prop_ge_oracle.rs`: the engine's inclusive
//! dest comparison equals a plain-code derivation over the reference
//! oracle's own tables — sources of edges whose DEST carries `k` as an Int
//! greater than or equal to the literal, missing-`k` excluded in the
//! derivation itself. The `k = 1` dest is the boundary that separates `>=`
//! from `>` (its source must be IN), the `k = 0` dest separates it from
...
/// Sources of `:R` edges whose dest carries `k` as an Int greater than or
/// equal to 1 — a missing `k` excludes the edge, mirroring the executor's
/// law.
fn reference_sources_of_ge_dests(graph: &ReferenceGraph) -> Vec<VId> {
...
#[test]
fn dest_property_ge_equals_reference_sources() {
    let ((), report) = run_async_under_lab(0x59_01, |root| async move {
        let commit = PurposeContexts::narrow_runtime_root(&root).commit();
        let dir = std::env::temp_dir().join(format!(
            "fgdb-gql-where-dst-prop-ge-oracle-{}",
            std::process::id()
        ));
        ...
        let rows;
        {
            ...
            rows = db
                ...
                .expect("dest greater-or-equal MATCH executes");
        }
        ...
        assert_eq!(
            rows,
            reference_sources_of_ge_dests(graph),
            "the engine equals the reference derivation"
        );
        assert_eq!(
            rows,
            vec![VId(1), VId(3)],
            "the k=1 boundary dest is IN — a strict > answers [3] and fails"
        );
        assert!(
            !rows.contains(&VId(5)),
            "the k=0 dest fails >= — a <> in disguise answers [1, 3, 5]"
        );
        assert!(
            !rows.contains(&VId(7)),
            "the missing-k dest's source is OUT, not vacuously greater-or-equal"
        );
    });
