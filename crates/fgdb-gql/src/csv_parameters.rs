...
fn bind_field(
    ...
    field: &Field<'_>,
    spec: &GqlParameterSpec,
    record: usize,
    column: usize,
) -> Result<GqlParameters, CsvParameterError> {
    ...
    let result = if !field.quoted && text == "\\N" {
        ...
        match spec.parameter_type {
            ...
            GqlParameterType::UInt64 => {
                ...
                if spec.requires_positive && value == 0 {
                    return Err(invalid());
                }
...
pub fn decode_csv_parameters(
    input: &str,
    schema: &[GqlParameterSpec],
    limits: CsvParameterLimits,
) -> Result<Vec<GqlParameters>, CsvParameterError> {
    ...
    for (index, spec) in schema.iter().enumerate() {
        if spec.name.len() > crate::parameters::MAX_GQL_PARAMETER_NAME_BYTES
            || name_probe
                .insert(spec.name.as_str(), GqlParameterValue::Int64(0))
                .is_err()
        {
        ...
        if !matches!(
            ...
            GqlParameterType::Int64
                | GqlParameterType::UInt64
                | GqlParameterType::Scalar(
                    CanonicalScalarKind::Null
                        | CanonicalScalarKind::Bool
                        | CanonicalScalarKind::Int
                        | CanonicalScalarKind::Text
                )
        ) {
    ...
    for (column, field) in header.iter().enumerate() {
        let index = *names.get(field.text.as_ref()).ok_or_else(|| {
            error(
                ...
                Some(column),
                field.offset,
                CsvParameterErrorKind::UnknownHeader,
            )
        })?;
        if seen[index] {
            return Err(error(
                ...
                Some(column),
                field.offset,
                CsvParameterErrorKind::DuplicateHeader,
            ));
        }
        seen[index] = true;
        order.push(index);
    ...
    while reader.at < input.len() {
        ...
        let Some(next) = next.filter(|bytes| *bytes <= limits.max_parameter_bytes) else {
            return Err(limit_error(
                ...
                None,
                offset,
                CsvParameterLimit::ParameterBytes,
...
mod tests {
    ...
    fn specs(columns: &[(&str, GqlParameterType)]) -> Vec<GqlParameterSpec> {
        columns
            .iter()
            .map(|(name, parameter_type)| GqlParameterSpec {
                name: (*name).into(),
    ...
    fn null_empty_literal_marker_unicode_and_query_text_remain_distinct() {
        ...
        let expected = [
            ...
            GqlParameters::new()
                .with_text("text", "'; MATCH (n) DETACH DELETE n; --")
                .unwrap(),
        ];
    ...
    fn header_schema_and_row_width_fail_closed() {
        let schema = specs(&[
            ("a", GqlParameterType::Int64),
            ("b", GqlParameterType::Int64),
        ]);
        for (input, kind) in [
        ...
        assert_eq!(
            decode("a,a\n1,2", &duplicates).unwrap_err().kind,
            CsvParameterErrorKind::InvalidSchema
        );
        assert_eq!(
            decode("", &text_schema()).unwrap_err().kind,
    ...
    fn numeric_extremes_and_boolean_values_use_exact_declared_types() {
        ...
        assert_eq!(
            row[0],
            GqlParameters::new()
                .with_int64("i", i64::MIN)
                .unwrap()
                .with_uint64("u", u64::MAX)
                .unwrap()
                .with_bool("b", true)
                .unwrap()
                .with_scalar("c", CanonicalScalar::Int(42))
                .unwrap()
        );
        ...
        assert_eq!(
            decode("n\n\\N", &integer).unwrap_err().kind,
            CsvParameterErrorKind::NullNotAllowed
        );
    }
    ...
    fn unsupported_schema_and_positive_limit_refuse_without_coercion() {
        for kind in [
            ...
            assert_eq!(
                decode("n\n1", &schema).unwrap_err().kind,
                CsvParameterErrorKind::UnsupportedType
            );
        }
    ...
    fn all_admission_dimensions_apply_including_cumulative_transcript_bytes() {
        ...
        for (limits, dimension) in [
            ...
            (
                CsvParameterLimits {
                    max_parameter_bytes: transcript_bytes - 1,
                    ..defaults
                },
                CsvParameterLimit::ParameterBytes,
            ),
            ...
            assert!(is_limit(
                &decode_csv_parameters(input, &text_schema(), limits).unwrap_err(),
                dimension
            ));
        }
        assert!(
            decode_csv_parameters(
                ...
                CsvParameterLimits {
                    ...
                    max_records: 2,
                    max_parameter_bytes: transcript_bytes,
                    ..defaults
                }
            )
            .is_ok()
        );
        let error = decode_csv_parameters(
            "text\nok\n\"unfinished",
            &text_schema(),
            CsvParameterLimits {
                max_records: 1,
    ...
    fn decoded_field_limit_counts_utf8_bytes_and_unescaped_quotes() {
        ...
        assert!(is_limit(
            &decode_csv_parameters("x\n🙂", &schema, limits).unwrap_err(),
            CsvParameterLimit::FieldBytes
        ));
        let limits = CsvParameterLimits {
            max_field_bytes: 1,
