...
fn plain(row: &GraphCostPath) -> Answer {
    (row.cost(), row.path().steps().to_vec())
}
    ...
    for depth in 0..=query.bounds().maximum() {
        if depth >= query.bounds().minimum() {
            answers.extend(
                layer
                    .iter()
                    .filter(|(end, _, _)| *end == query.target())
        ...
        for (end, cost, steps) in layer {
            for &(edge, left, relation, right) in edges {
                ...
                if query.direction() != GlaDirection::Reverse && left == end {
...
fn stopping_and_continuing_at_a_target_are_distinct_partitions() {
    let edges = [
        ...
        (EId(3), VId(1), RELATION, VId(1)),
        (EId(1), VId(1), RELATION, VId(0)),
    ];
    for weight in [-1, 0, 1, i64::MIN, i64::MAX] {
        let weights: BTreeMap<_, _> = edges
            .iter()
            .map(|(id, ..)| (*id, CanonicalScalar::Int(weight)))
            .collect();
        for (start, end) in [(0, 0), (0, 1)] {
            ...
            for count in [0, 1, 2, 7, u64::MAX] {
                ...
                let take = usize::try_from(count)
                    ...
                    .min(expected.len());
                assert_eq!(
                    actual.iter().map(plain).collect::<Vec<_>>(),
                    expected[..take]
                );
            }
        }
...
fn cursor_owns_admitted_costs_preserves_wide_id_ties_and_redacts_state() {
    let edges = [
        (EId(u128::MAX), VId(0), RELATION, VId(u128::MAX)),
        (EId(1_u128 << 100), VId(0), RELATION, VId(u128::MAX)),
    ];
    let mut weights = BTreeMap::from([
        (edges[0].0, CanonicalScalar::Int(5)),
        (edges[1].0, CanonicalScalar::Int(5)),
    ]);
    ...
    assert_eq!(
        plain(
            &cursor
                .next_with_control(|_| Ok::<_, ()>(()))
                .unwrap()
                .unwrap()
        ),
        (5, vec![(edges[1].0, VId(u128::MAX))])
    );
    assert_eq!(
        plain(
            &cursor
    ...
    assert!(
        cursor
            .next_with_control(|_| -> Result<(), ()> {
                panic!("closed cursor resumed");
            })
            .unwrap()
            .is_none()
    );
...
fn first_four_of_two_to_the_fortieth_walks_do_not_enumerate_the_result_bag() {
    ...
    assert!(
        work < 5_000,
        "the first answer must not split its alternatives"
    );
    ...
    for ordinal in 1..4_u128 {
        ...
        assert_eq!(
            ...
            (0..40_u128)
                .map(|depth| (
                    EId(depth * 2 + ((ordinal >> (39 - depth)) & 1)),
                    VId(depth + 1)
                ))
                .collect::<Vec<_>>()
        );
    ...
    let result = query
        .execute_k_governed_with_edge_properties(
            ...
            GqlQueryPolicy::new(121, 4, 50_000, 50_000),
            || Ok::<_, ()>(()),
        )
        .unwrap();
...
fn every_refusal_closes_all_state_and_a_fresh_cursor_retries_exactly() {
    ...
    let baseline = query
        .execute_k_with_control(
            ...
            [VId(0), VId(1)],
            edges,
            |edge, _| Ok::<_, usize>(weights.get(&edge)),
            |_| {
                total += 1;
                Ok(())
            },
    ...
    for stop in 1..=total {
        ...
        match query.cursor_with_control(
            [VId(0), VId(1)],
            edges,
            |edge, _| Ok(weights.get(&edge)),
            &mut control,
        ) {
            ...
            Ok(mut cursor) => {
                ...
                assert!(
                    cursor
                        .next_with_control(|_| -> Result<(), usize> {
...
fn exact_combined_limits_count_only_the_requested_prefix_and_refuse_atomically() {
    ...
    let run = |count, policy| {
        query.execute_k_governed_with_edge_properties(
            count,
            3,
            [VId(0)],
            edges,
    ...
    assert!(matches!(
        run(5, GqlQueryPolicy::new(2, 5, u64::MAX, u64::MAX)),
        Err(GqlQueryError::Rows(_))
    ));
    assert!(matches!(
        run(5, GqlQueryPolicy::new(3, 4, u64::MAX, u64::MAX)),
        Err(GqlQueryError::Rows(_))
    ));
    assert!(matches!(
        run(
            5,
            GqlQueryPolicy::new(3, 5, exact.evaluator.max_work_units - 1, u64::MAX)
        ),
    ...
    assert!(matches!(
        run(
            5,
            GqlQueryPolicy::new(3, 5, u64::MAX, exact.evaluator.max_scratch_entries - 1)
        ),
        Err(GqlQueryError::Evaluator(_))
    ));
    assert_eq!(
        run(0, GqlQueryPolicy::new(3, 0, u64::MAX, u64::MAX))
            .unwrap()
            .rows
            .result_rows,
        0
    ...
    for stop in 1..=total {
        let mut calls = 0;
        let result = query.execute_k_governed_with_edge_properties(
            5,
            3,
            [VId(0)],
            edges,
...
fn source_admission_is_not_bypassed_by_zero_count_or_missing_anchors() {
    ...
    for count in [0, 1] {
        assert_eq!(
            missing.execute_k_with_control(
                count,
                [VId(0), VId(1)],
                [edge],
                |_, _| Ok::<_, ()>(None),
    ...
    let isolate = query(7, 7, GlaDirection::Forward, 0, 8)
        .execute_k_with_control(u64::MAX, [VId(7)], [], |_, _| Ok::<_, ()>(None), |_| Ok(()))
        .unwrap();
