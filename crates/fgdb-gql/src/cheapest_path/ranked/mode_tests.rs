...
fn permitted(mode: GraphCheapestPathMode, source: VId, steps: &[Step]) -> bool {
    if mode == GraphCheapestPathMode::Walk {
        return true;
    }
    ...
    let mut vertices: Vec<_> = std::iter::once(source)
        .chain(steps.iter().map(|step| step.1))
        .collect();
    if mode == GraphCheapestPathMode::Simple
        && vertices.len() > 1
        && vertices.last() == Some(&source)
    {
        vertices.pop();
    }
    ...
    for depth in 0..=q.bounds().maximum() {
        ...
        for (end, cost, steps) in layer {
            for &(eid, left, relation, right) in edges {
                ...
                if q.direction() != GlaDirection::Reverse && left == end {
                    destinations.push(right);
                }
                if q.direction() != GlaDirection::Forward
                    && right == end
                    && (q.direction() != GlaDirection::Undirected || left != right)
                {
...
fn a_more_expensive_prefix_at_the_same_depth_and_vertex_can_be_the_only_legal_answer() {
    for mode in &MODES[1..] {
        let closing = if *mode == GraphCheapestPathMode::Trail {
            (7, 3, 0, 0)
        } else {
            (5, 3, 1, 0)
        };
        ...
        let edges: Vec<_> = fixture
            .iter()
            .map(|&(id, a, b, _)| (EId(id), VId(a), R, VId(b)))
            .collect();
        let weights = fixture
            .iter()
            .map(|&(id, _, _, w)| (EId(id), CanonicalScalar::Int(w)))
            .collect::<BTreeMap<_, _>>();
        let depth = if *mode == GraphCheapestPathMode::Trail {
            5
        } else {
            4
        };
...
fn simple_closure_is_terminal_but_is_not_edge_unique_trail() {
    ...
    for (mode, expected) in [
        (GraphCheapestPathMode::Trail, 0),
        (GraphCheapestPathMode::Acyclic, 0),
        (GraphCheapestPathMode::Simple, 1),
    ] {
...
fn mode_identity_is_distinct_and_default_walk_and_admission_transcripts_are_unchanged() {
    ...
    assert_eq!(
        base.clone()
            .with_mode(GraphCheapestPathMode::Walk)
            .canonical_bytes(),
        original
    );
    let identities: BTreeSet<_> = MODES
        .into_iter()
        .map(|mode| {
            ...
            assert_eq!(
                q.input_pattern().plan().canonical_bytes(),
                base.input_pattern().plan().canonical_bytes()
            );
            q.canonical_bytes()
        })
        .collect();
...
fn forbidden_prefix_families_are_pruned_before_enumerating_exponentially_many_walks() {
    ...
    let weights = BTreeMap::from([
        (EId(1), CanonicalScalar::Int(-2)),
        (EId(2), CanonicalScalar::Int(-1)),
        (EId(3), CanonicalScalar::Int(5)),
    ]);
    for mode in &MODES[1..] {
        ...
        let rows = q
            .execute_k_governed_with_edge_properties(
                ...
                edges,
                |eid, _| Ok::<_, ()>(weights.get(&eid)),
                GqlQueryPolicy::new(5, 5, 10_000, 10_000),
                || Ok::<_, ()>(()),
            )
            .unwrap();
        let expected = if *mode == GraphCheapestPathMode::Trail {
            vec![2, 2, 3, 4, 5]
        } else {
            vec![5]
        };
    ...
    for mode in &MODES[1..] {
        ...
        let baseline = q
            .execute_k_with_control(
                ...
                [VId(0), VId(1)],
                edges,
                |eid, _| Ok::<_, usize>(weights.get(&eid)),
                |_| {
                    total += 1;
                    Ok(())
                },
