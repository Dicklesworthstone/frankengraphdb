...
const EDGES: [(EId, VId, VId, RelationId); 2] =
    [(EId(0), VId(0), VId(1), R), (EId(1), VId(1), VId(2), S)];
struct Source {
    after: Option<VId>,
...
impl Source {
    ...
    fn seek<C>(
        ...
        Ok(EDGES
            .iter()
            .find(|&&(eid, a, b, _)| {
                after.is_none_or(|after| eid > after)
                    && match direction {
...
impl VertexScanSource for Source {
    ...
    fn vertex<'a, C>(
        ...
        control(VertexScanEvent::Work).map_err(VertexScanSourceError::Control)?;
        Ok((id.0 < 3).then_some(VertexScanRow {
            labels: &[],
            properties: &[],
        }))
    }
...
impl EdgeScanSource for Source {
    ...
    fn next_edge<C>(
        ...
        let next = EDGES
            .iter()
            .map(|e| e.0)
            .find(|id| self.edge_after.is_none_or(|after| *id > after));
        if next.is_some() {
            self.edge_after = next;
        }
        Ok(next)
    }
    fn edge<'a, C>(
        ...
        Ok(EDGES
            .iter()
            .find(|e| e.0 == eid)
            .map(|&(_, source, target, relation)| EdgeScanRow {
                source,
                target,
                relation,
                properties: &[],
...
fn fixed_variable_and_zero_length_probes_use_the_exact_relation_before_incidence() {
    ...
    for bounds in ["0..0", "0..3"] {
        let (all, visits) = rows(
            &format!("MATCH (a) WHERE EXISTS {{ MATCH (a)-[:S*{bounds}]->(b) }} RETURN a"),
            false,
        );
        assert_eq!(ids(&all), vec![VId(0), VId(1), VId(2)]);
        assert_eq!(
            visits, 0,
            "identity paths require vertices, not relation access"
        );
    }
}
...
fn allowed_missing_index_is_not_absence_and_each_control_refusal_fuses_the_cursor() {
    ...
    let mut cursor = VertexScanCursor::new(Source::new(true, true), plan(text), policy(), || {
        Ok::<_, ()>(())
    });
    assert!(matches!(
        cursor.next(),
    ...
    let (expected, stats, counts) = {
        let mut cursor =
            VertexScanCursor::new(Source::new(true, false), plan(text), policy(), || {
                checkpoints += 1;
                Ok::<_, usize>(())
            });
    ...
    let got = VertexScanCursor::new(Source::new(true, false), plan(text), exact, || {
        Ok::<_, ()>(())
    })
    .collect::<Result<Vec<_>, _>>()
    .unwrap();
...
fn relation_aware_defaults_preserve_exact_candidate_and_control_transcripts() {
    for after in [None, Some(EId(u128::MAX))] {
        ...
        let value = Legacy
            .next_probe_edge(VId(0), GlaDirection::Forward, after, &mut |e| {
                expected.push(e);
                Ok::<_, ()>(())
            })
            .unwrap();
