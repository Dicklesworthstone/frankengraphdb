...
impl VertexScanSource for Masked {
    ...
    fn next_vertex<C>(
        ...
        let next = IDS
            .into_iter()
            .find(|id| self.after.is_none_or(|after| *id > after));
        if next.is_some() {
            self.after = next;
        }
        Ok(next)
    ...
    fn next_probe_vertex<C>(
        ...
        Ok(IDS
            .into_iter()
            .find(|id| after.is_none_or(|after| *id > after)))
    }
    fn next_probe_edge<C>(
        ...
        for (eid, a, b) in EDGES {
            ...
            if incident && after.is_none_or(|after| eid > after) {
                return Ok(Some(eid));
            }
        }
        Ok(None)
...
fn borrowed_defaults_preserve_lookup_events_and_distinguish_missing_vertices_from_null_fields() {
    ...
    impl VertexScanSource for Borrowed {
        ...
        fn vertex<'a, C>(
            ...
            control(VertexScanEvent::Work).map_err(VertexScanSourceError::Control)?;
            Ok((id == VId(1)).then_some(VertexScanRow {
                labels: &[],
                properties: &[],
            }))
        }
    ...
    assert_eq!(
        source
            .vertex_property(VId(0), P, &mut |event| {
                events.push(event);
                Ok::<_, ()>(())
            })
            .unwrap(),
    ...
    assert_eq!(
        source
            .vertex_property(VId(1), P, &mut |event| {
                events.push(event);
                Ok::<_, ()>(())
            })
            .unwrap(),
    ...
    assert_eq!(events, vec![VertexScanEvent::Work]);
    assert!(matches!(
        source.vertex_property(VId(1), P, &mut |_| Err(73)),
    ...
    impl EdgeScanSource for EdgeBorrowed {
        ...
        ) -> Result<Option<EId>, EdgeScanSourceError<(), C>> {
            Ok(None)
        }
        fn edge<'a, C>(
            &'a self,
            _: EId,
            _: &mut impl FnMut(GlaExecutionEvent) -> Result<(), C>,
        ) -> Result<Option<EdgeScanRow<'a>>, EdgeScanSourceError<(), C>> {
            Ok(None)
        }
        fn vertex<'a, C>(
            &'a self,
    ...
    let source = EdgeBorrowed {
        props: vec![(P, source.0)],
    };
    let mut events = Vec::new();
    let value = source
        .vertex_property(VId(1), P, &mut |event| {
    ...
    assert_eq!(
        events,
        vec![GlaExecutionEvent::Work, GlaExecutionEvent::Work]
    );
    assert!(matches!(
        source
            .vertex_record(VId(1), &mut |_| Ok::<_, ()>(()))
            .unwrap(),
        Some(VertexScanRecord::Borrowed(_))
    ));
    assert_eq!(
        source
            .vertex_property(VId(0), P, &mut |_| Ok::<_, ()>(()))
            .unwrap(),
        None
    );
    assert_eq!(
        source
            .vertex_property(VId(1), H, &mut |_| Ok::<_, ()>(()))
            .unwrap(),
        Some(None)
    );
}
...
fn owned_probe_records_and_scoped_scalars_cover_correlated_independent_and_walk_bindings() {
    for (body, expected) in [
        (
            "EXISTS { MATCH (a)-[:R]->(b) WHERE b.p > a.p AND b.hidden IS NULL }",
            vec![VId(0), VId(2)],
        ),
        (
            "NOT EXISTS { MATCH (a)-[:R]->(b) WHERE NOT (b.hidden = 7) }",
            IDS.to_vec(),
        ),
        (
            "EXISTS { MATCH (x) WHERE x.p = a.p }",
            vec![VId(0), VId(1), VId(2)],
        ),
        ("EXISTS { MATCH (x:H) }", vec![]),
        ("EXISTS { MATCH (a:H) }", vec![]),
        (
            "EXISTS { MATCH (a)-[:R*2..2]->(b) WHERE b.p > a.p }",
            vec![VId(2)],
        ),
        (
            "EXISTS { MATCH (a)-[:R*0..0]->(b) WHERE b = a AND b.hidden IS NULL }",
            IDS.to_vec(),
        ),
    ] {
        let text = format!("MATCH (a) WHERE {body} RETURN a AS id");
        let mut cursor =
            VertexScanCursor::new(Masked::new(Fault::None), plan(&text), policy(), || {
                Ok::<_, ()>(())
            });
    ...
    for (atom, pairs) in [
        ...
        for anti in [false, true] {
            for (skip, take) in [(0, 10), (1, 1), (0, 0)] {
                let text = format!(
                    "MATCH (a) WHERE {}EXISTS {{ MATCH (a){atom}(b) WHERE b.p > a.p }} RETURN DISTINCT a AS id SKIP {skip} LIMIT {take}",
                    if anti { "NOT " } else { "" }
                );
                ...
                let expected: Vec<_> = IDS
                    .into_iter()
                    .filter(|id| pairs.iter().any(|&(a, b)| a == id.0 && p(b) > p(a)) != anti)
                    .skip(skip)
                    .take(take)
                    .collect();
                let rows =
...
fn property_errors_and_missing_bound_vertices_never_turn_into_anti_join_success() {
    for fault in [Fault::Property, Fault::MissingBoundVertex] {
        ...
        match fault {
            Fault::Property => assert!(matches!(
                cursor.next(),
                Some(Err(GqlQueryError::Source(VertexScanError::Source(
                    "property refused"
                ))))
            )),
            Fault::MissingBoundVertex => assert!(matches!(
                cursor.next(),
...
fn every_probe_control_cut_is_terminal_and_native_allowances_remain_cumulative() {
    let text =
        "MATCH (a) WHERE NOT EXISTS { MATCH (a)-[:R]->(b) WHERE b.hidden = a.p } RETURN a AS id";
    let mut calls = 0;
    ...
    for cut in 1..=calls {
        ...
        loop {
            match cursor.next() {
                ...
                Some(Err(GqlQueryError::Interrupted(found))) => {
                    assert_eq!(found, cut);
                    break;
                }
    ...
    let exact = GqlQueryPolicy::new(
        ...
        rows.result_rows,
        usage.work_units,
        usage.scratch_entries,
    );
    assert_eq!(
        VertexScanCursor::new(Masked::new(Fault::None), plan(text), exact, || Ok::<_, ()>(
            ()
        ))
        .collect::<Result<Vec<_>, _>>()
        .unwrap(),
