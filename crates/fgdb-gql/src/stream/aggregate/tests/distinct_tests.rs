...
fn native_vertex_ids_and_mixed_scalar_domains_survive_source_release() {
    ...
    let definition = PreparedGraphAggregate::prepare(
        ...
        &[],
        &[
            GraphAggregate::count_distinct("scalars", 0),
            GraphAggregate::min("lo", 0),
            GraphAggregate::max("hi", 0),
            GraphAggregate::count_distinct("ids", 1),
            GraphAggregate::min("first", 1),
            GraphAggregate::max("last", 1),
        ],
        0,
        None,
    )
    ...
    let rows = vec![
        row(0, Some(CanonicalScalar::Int(7))),
        row(1, Some(secret.clone())),
        row(1_u128 << 100, Some(CanonicalScalar::Int(7))),
        row(u128::MAX, None),
    ];
    let expected = expected(&definition, &rows);
    let input = source(rows);
    ...
    assert_eq!(
        result.values()[4].as_value(),
        Some(&GraphValue::Vertex(VId(0)))
    );
    assert_eq!(
        result.values()[5].as_value(),
...
fn averages_preserve_sub_float_fractions_and_distinct_support() {
    ...
    let high = i64::MAX;
    let rows = vec![
        row(0, Some(CanonicalScalar::Int(high))),
        row(1, Some(CanonicalScalar::Int(high - 1))),
        row(2, Some(CanonicalScalar::Int(high))),
    ];
    let mut cursor = VertexAggregateCursor::new(
        source(rows.clone()),
        VertexAggregatePlan::compile(&definition).unwrap(),
    ...
    assert_eq!(
        result.values()[5].as_average(),
        GraphExactAverage::new(3 * i128::from(high) - 1, 3)
    );
    assert_eq!(
        result.values()[6].as_average(),
        GraphExactAverage::new(2 * i128::from(high) - 1, 2)
    );
    assert_eq!(
        result.values()[4].as_integer(),
        Some(2 * i128::from(high) - 1)
    );
}
...
fn duplicates_do_not_allocate_more_membership_or_extremum_payloads() {
    ...
    for count in [1_u64, 7, 256, 4096] {
        let rows = (0..count)
            .map(|id| row(u128::from(id), Some(CanonicalScalar::Int(7))))
            .collect();
        let mut cursor =
    ...
    let measure = |size: usize, count: u128| {
        ...
        let definition = PreparedGraphAggregate::prepare(
            ...
            &[],
            &[
                GraphAggregate::count_distinct("n", 0),
                GraphAggregate::min("lo", 0),
            ],
            0,
            None,
...
fn all_new_state_growth_and_finalization_checkpoints_are_fused_failures() {
    ...
    let rows = vec![
        row(0, Some(CanonicalScalar::Int(-3))),
    ...
    for (work, scratch, success) in [
        ...
        let mut cursor = VertexAggregateCursor::new(
            source(rows.clone()),
            plan.clone(),
            GqlQueryPolicy::new(2, 1, work, scratch),
            || Ok::<_, ()>(()),
        );
...
fn late_new_numeric_errors_and_source_errors_never_release_retained_values() {
    for (function, average) in [
        (GraphAggregate::average_int("v", 0), true),
        ...
        let definition = PreparedGraphAggregate::prepare(
            input(),
            &[],
            &[GraphAggregate::count_distinct("n", 0), function],
            0,
            None,
        )
        .unwrap();
        ...
        if average {
            assert!(matches!(
                error,
                GqlQueryError::Source(GraphAggregateError::NonIntegerAverage { aggregate: 1 })
            ));
        } else {
            assert!(matches!(
                error,
                GqlQueryError::Source(GraphAggregateError::NonIntegerSum { aggregate: 1 })
            ));
        }
        assert_eq!(cursor.row_stats().result_rows, 0);
        assert!(dropped.get());
    ...
    let mut cursor = VertexAggregateCursor::new(
        input,
        VertexAggregatePlan::compile(&exact_definition()).unwrap(),
        wide(),
        || Ok::<_, ()>(()),
    );
    assert!(matches!(
...
fn distinct_membership_and_numeric_cells_are_atomic_on_growth_or_arithmetic_refusal() {
    ...
    state
        .update::<(), ()>(
            Input::Scalar(Some(&CanonicalScalar::Int(7))),
            0,
            &mut |_| Ok(()),
        )
        .unwrap();
    let refused = state.update::<(), ()>(
        Input::Scalar(Some(&CanonicalScalar::Int(9))),
        0,
        &mut |event| {
    ...
    for (function, numeric) in [
        ...
        assert!(matches!(
            state.update::<(), ()>(
                Input::Scalar(Some(&CanonicalScalar::Int(1))),
                5,
                &mut |_| Ok(())
            ),
    ...
    state
        .update::<(), ()>(Input::Scalar(Some(&value)), 0, &mut |_| Ok(()))
        .unwrap();
    drop(value);
    assert_eq!(state.scalars.len(), 1);
...
fn numeric_distinct_vertex_arguments_preserve_empty_nulls_and_typed_nonempty_errors() {
    ...
    for (function, average) in [
        ...
        let definition =
            PreparedGraphAggregate::prepare(input.clone(), &[], &[function], 0, None).unwrap();
        let plan = VertexAggregatePlan::compile(&definition).unwrap();
        let mut empty =
        ...
        let mut nonempty =
            VertexAggregateCursor::new(source(vec![row(u128::MAX, None)]), plan, wide(), || {
                Ok::<_, ()>(())
            });
...
fn grouped_distinct_support_is_local_and_matches_eager_for_4096_inputs() {
    ...
    for mut code in 0..4096_usize {
        ...
        let rows: Vec<_> = (0..6)
            .map(|at| {
                let bucket = buckets[code % 4].clone();
                code /= 4;
    ...
    let rows = vec![
        grouped_row(0, None, Some(7)),
        grouped_row(1, None, Some(7)),
...
fn grouped_distinct_every_failure_keeps_delivered_prefix_and_releases_pending_support() {
    ...
    let plan = VertexAggregatePlan::compile(&definition).unwrap();
    let rows = vec![
        grouped_row(0, Some(CanonicalScalar::Int(2)), Some(7)),
        grouped_row(1, None, Some(-3)),
        grouped_row(2, Some(CanonicalScalar::Int(1)), Some(7)),
    ];
