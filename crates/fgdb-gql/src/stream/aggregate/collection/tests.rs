...
use crate::{
    GraphAggregate, GraphAggregateColumn, GraphAggregateOrder, GraphSetProjection, GraphSetValue,
};
...
impl VertexScanSource for Source {
    ...
    fn next_vertex<C>(
        ...
        let next = self
            ...
            .next()
            .map(|(&vid, _)| vid);
        if next.is_some() && next == self.fail {
            return Err(VertexScanSourceError::Source("late source failure"));
        }
    ...
    fn vertex<C>(
        ...
        Ok(self.rows.get(&vid).map(|properties| VertexScanRow {
            labels: &[],
            properties,
        }))
    }
...
fn input(identity_first: bool) -> PreparedGraphPattern<GraphValueRow> {
    ...
    let columns = if identity_first {
        vec![
            ...
            GraphColumn::property("bucket", "n", B),
            GraphColumn::property("amount", "n", P),
        ]
    } else {
        vec![GraphColumn::property("amount", "n", P)]
    };
    b.prepare_values(&columns, 0, None)
        .unwrap()
        .with_duplicates()
}
fn definition(grouped: bool, count: Option<u64>) -> PreparedGraphAggregate {
    PreparedGraphAggregate::prepare(
        ...
        &[
            ...
            GraphAggregate::collect_distinct("support", 2),
            GraphAggregate::collect("vertices", 0),
...
fn values(value: &GraphAggregateValue) -> &[GraphValue] {
    let GraphAggregateValue::Value(GraphValue::List(values)) = value else {
...
fn snapshot(state: &NumericState) -> (Vec<GraphValue>, usize, usize, usize, usize) {
    match state {
        ...
        NumericState::Distinct(s) => {
            ...
            result.1 = s.scalars.len();
            result.2 = s.vertices.len();
            result.3 = s.values.len();
            result.4 = s.max_payload;
            result
        }
...
fn collection_preserves_first_occurrence_and_every_native_value_domain() {
    ...
    for function in [
        ...
        let mut s = state(function);
        let mut expected = Vec::new();
        let mut seen = BTreeSet::new();
        for value in &raw {
            s.update_governed::<(), ()>(Input::Value(value), 0, &mut |_| Ok(()))
                .unwrap();
            if !value.is_null()
                && (function == GraphAggregateFunction::Collect || seen.insert(value.clone()))
            {
                expected.push(value.clone());
            }
        }
        s.update_governed::<(), ()>(Input::Scalar(None), 0, &mut |_| Ok(()))
            .unwrap();
        assert_eq!(
            s.finish_governed(&mut |_| Ok::<_, ()>(())).unwrap(),
            list(expected)
        );
        assert_eq!(
            state(function)
                .finish_governed(&mut |_| Ok::<_, ()>(()))
                .unwrap(),
            list(vec![])
        );
    }
...
fn every_append_refusal_or_unwind_leaves_membership_and_list_unchanged() {
    ...
    for function in [
        ...
        let make = || {
            let mut s = state(function);
            s.update_governed::<(), usize>(Input::Value(&first), 0, &mut |_| Ok(()))
                .unwrap();
            s
        };
        let mut full = make();
        let mut calls = 0;
        full.update_governed::<(), usize>(Input::Value(&next), 0, &mut |_| {
            calls += 1;
            Ok(())
        })
        .unwrap();
        for stop in 1..=calls {
            for unwind in [false, true] {
                ...
                if unwind {
                    ...
                    assert!(matches!(result, Ok(Err(GqlQueryError::Interrupted(n))) if n == stop));
                }
                assert_eq!(snapshot(&s), before);
                assert_eq!(at, stop);
                s.update_governed::<(), usize>(Input::Value(&next), 0, &mut |_| Ok(()))
                    .unwrap();
...
fn vertex_collections_match_independent_ordered_groups_and_batch_without_input_bag() {
    ...
    for code in 0..256_usize {
        ...
        for (at, vid) in [0, 1, 1_u128 << 100, u128::MAX].into_iter().enumerate() {
            let mut properties: Vec<_> = choices[(code >> (2 * at)) & 3]
                .clone()
                .into_iter()
                .map(|v| (P, v))
                .collect();
        ...
        for grouped in [false, true] {
            ...
            for (&vid, props) in &rows {
                let bucket = match props.last().unwrap().1 {
                    CanonicalScalar::Int(n) => n,
                    _ => panic!(),
                };
                ...
                if let Some((_, value)) = props
                    .iter()
                    .find(|(k, v)| *k == P && !matches!(v, CanonicalScalar::Null))
                {
                    all.push(GraphValue::Scalar(value.clone()));
                }
            }
            let s = source(rows.clone());
            let drops = s.drops.clone();
            let mut cursor = VertexAggregateCursor::new(
                s,
                VertexAggregatePlan::compile(&q).unwrap(),
                wide(),
                || Ok::<_, ()>(()),
            );
            ...
            for (row, (_, (all, ids))) in actual.iter().zip(groups) {
                ...
                let unique: Vec<_> = all
                    .iter()
                    .filter(|v| support.insert((*v).clone()))
                    .cloned()
                    .collect();
                assert_eq!(
                    row.values(),
                    &[
                        ...
                        list(unique),
                        list(ids.clone()),
                        GraphAggregateValue::Count(ids.len() as u64)
                    ]
                );
            }
...
fn computed_collection_requires_the_childs_existing_canonical_order_proof() {
    for identity_first in [false, true] {
        ...
        let rows = BTreeMap::from([
            (
                VId(0),
                vec![(P, CanonicalScalar::Int(9)), (B, CanonicalScalar::Int(0))],
            ),
            (
                VId(1),
                vec![(P, CanonicalScalar::Int(-4)), (B, CanonicalScalar::Int(0))],
            ),
        ]);
        let expected = batch(&q, &rows);
        if identity_first {
            let result = VertexAggregateCursor::new(
                source(rows),
                VertexAggregatePlan::compile(&q).unwrap(),
                wide(),
                || Ok::<_, ()>(()),
            ...
            assert!(matches!(
                VertexAggregatePlan::compile(&q),
                Err(VertexAggregateBuildError::Scan(_))
            ));
            assert_eq!(
                values(&expected[0].values()[0]),
                &[
                    GraphValue::Scalar(CanonicalScalar::Int(-4)),
                    GraphValue::Scalar(CanonicalScalar::Int(9))
                ]
...
fn completed_collection_outputs_use_shared_ranking_projection_and_distinct() {
    let rows: Rows = (0..8_u128)
        .map(|id| {
            (
                VId(id),
                vec![
                    (P, CanonicalScalar::Int((id % 2) as i64)),
                    (B, CanonicalScalar::Int((id % 3) as i64)),
                ],
            )
        })
        .collect();
            ...
            let q = definition(true, count)
                ...
                .with_output_projection(vec![
                    ...
                    GraphSetProjection::new(
                        "size",
                        GraphSetValue::Size(Box::new(GraphSetValue::Column(1))),
                    ),
                ])
                .unwrap()
                .with_distinct_output(distinct);
            let expected = batch(&q, &rows);
            let actual = VertexAggregateCursor::new(
                source(rows.clone()),
                VertexAggregatePlan::compile(&q).unwrap(),
                wide(),
                || Ok::<_, ()>(()),
...
fn collection_cursor_uses_cumulative_limits_fuses_on_refusal_and_never_drains_on_close() {
    ...
    let plan = VertexAggregatePlan::compile(&q).unwrap();
    let mut calls = 0;
    let mut cursor = VertexAggregateCursor::new(source(rows.clone()), plan.clone(), wide(), || {
        calls += 1;
        Ok::<_, usize>(())
    });
    let expected = cursor.next().unwrap().unwrap();
    let r = cursor.row_stats();
    let e = cursor.evaluator_stats();
    drop(cursor);
    ...
    for p in [
        ...
        let mut c =
            VertexAggregateCursor::new(source(rows.clone()), plan.clone(), p, || Ok::<_, ()>(()));
    ...
    let drops = s.drops.clone();
    let mut c = VertexAggregateCursor::new(s, plan.clone(), wide(), || -> Result<(), ()> {
    ...
    assert_eq!(drops.load(Ordering::SeqCst), 1);
    for limit in [0, 1] {
        let q = definition(false, Some(limit));
        let mut s = source(rows.clone());
        s.fail = Some(VId(u128::MAX));
        let mut c = VertexAggregateCursor::new(
            s,
            VertexAggregatePlan::compile(&q).unwrap(),
            wide(),
            || Ok::<_, ()>(()),
        ...
        assert!(matches!(
            ...
            Some(Err(GqlQueryError::Source(GraphAggregateError::Source(
                VertexScanError::Source("late source failure")
            ))))
        ));
        assert_eq!(c.row_stats().result_rows, 0);
        assert!(c.next().is_none());
...
fn large_collections_are_one_result_row_and_empty_global_is_a_real_list() {
    let rows: Rows = (0..4097_u128)
        .map(|id| {
            (
                VId(id),
                vec![
                    (P, CanonicalScalar::Int((id % 2) as i64)),
                    (B, CanonicalScalar::Int(0)),
                ],
            )
        })
        .collect();
    ...
    let mut c = VertexAggregateCursor::new(
        source(rows),
        VertexAggregatePlan::compile(&q).unwrap(),
        GqlQueryPolicy::new(4097, 1, u64::MAX, u64::MAX),
        || Ok::<_, ()>(()),
    );
    let row = c.next().unwrap().unwrap();
    assert_eq!(values(&row.values()[0]).len(), 4097);
    assert_eq!(values(&row.values()[1]).len(), 2);
    assert_eq!(c.row_stats().result_rows, 1);
    assert!(c.next().is_none());
    for grouped in [false, true] {
        let q = definition(grouped, None);
        let actual = VertexAggregateCursor::new(
