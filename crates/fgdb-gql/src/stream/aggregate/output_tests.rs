...
use crate::{
    GqlParameters, GraphAggregate, GraphAggregateColumn, GraphAggregateFilter, GraphAggregateOrder,
    GraphAggregateTest, GraphNullPlacement, GraphSymbolResolver, PreparedGraphAggregateText,
    RelationBind,
};
use std::cell::Cell;
use std::rc::Rc;
...
impl VertexScanSource for Source {
    ...
    fn next_vertex<C>(
        ...
        if self.fail_at == Some(self.at) {
            return Err(VertexScanSourceError::Source("late source"));
        }
...
fn record(vid: u128, k: Option<i64>, v: Option<i64>) -> Record {
    ...
    if let Some(k) = k {
        props.push((K, CanonicalScalar::Int(k)));
    }
    if let Some(v) = v {
        props.push((V, CanonicalScalar::Int(v)));
    }
    (VId(vid), props)
}
fn fixture(mask: u32) -> Vec<Record> {
    ...
    .map(|(at, (k, v))| record(if at == 5 { u128::MAX } else { at as u128 }, k, v))
    .collect()
}
    ...
    q.execute_governed(
        ...
        |id, tests| {
            Ok::<_, ()>(
                rows.iter()
                    .find(|r| r.0 == id)
                    .is_some_and(|r| tests.iter().all(|p| p.matches(&[L], &r.1))),
            )
        },
        |id, key| {
            Ok(rows
                .iter()
                .find(|r| r.0 == id)
                .and_then(|r| r.1.iter().find(|(k, _)| *k == key).map(|(_, v)| v)))
        },
        wide(),
        || Ok::<_, ()>(()),
    )
...
fn scalar(props: &[(PropertyKeyId, CanonicalScalar)], key: PropertyKeyId) -> Option<i128> {
    props
        ...
        .and_then(|(_, v)| match v {
            CanonicalScalar::Int(v) => Some(i128::from(*v)),
            _ => None,
        })
}
...
fn all_6144_having_distinct_order_and_window_cases_match_independent_groups() {
    ...
    for mask in 0..64 {
        ...
        for (_, props) in &rows {
            let g = groups.entry(scalar(props, K)).or_default();
            g.0 += 1;
            if let Some(v) = scalar(props, V) {
                g.1 += v;
                g.2 += 1;
            }
        }
                        ...
                        for minimum in [1, 2] {
                            for (offset, count) in [(0, 0), (0, 2), (1, 2)] {
                                let q = PreparedGraphAggregate::prepare(
                                    base.input_pattern().clone(),
                                    base.group_key_columns(),
                                    &[
                                        GraphAggregate::count_rows("n"),
                                        GraphAggregate::average_int(
                                            "avg",
                                            base.aggregates()[1].argument_column().unwrap(),
                                        ),
                                    ],
                                    offset,
                                    Some(count),
                                ...
                                .unwrap()
                                .with_key_output_columns(if hidden { &[] } else { &[0] })
                                .unwrap()
                                ...
                                let mut expected: Vec<_> = groups
                                    .iter()
                                    .filter(|(_, (n, _, _))| i128::from(*n) >= minimum)
                                    .collect();
                                ...
                                for (key, (n, sum, count)) in expected {
                                    let key = if hidden {
                                        ...
                                        vec![GraphValue::Scalar(
                                            key.map_or(CanonicalScalar::Null, |k| {
                                                CanonicalScalar::Int(i64::try_from(k).unwrap())
                                            }),
                                        )]
                                    };
                                    let average = if *count == 0 {
                                        ...
                                        GraphAggregateValue::Average(
                                            GraphExactAverage::new(*sum, *count).unwrap(),
                                        )
                                    };
                                    let row = GraphAggregateRow::from_group_values(
                                        key,
                                        vec![GraphAggregateValue::Count(*n), average],
                                    );
                                    if !distinct || !projected.contains(&row) {
                                        projected.push(row);
                                    }
                                }
...
fn zero_full_and_impossible_pages_do_not_hide_source_having_or_expression_errors() {
    for window in [" LIMIT 0", " LIMIT 1", " SKIP 18446744073709551615 LIMIT 1"] {
        for order in ["", " ORDER BY COUNT(*) DESC"] {
            let q = prepare(&format!(
                "MATCH (n:L) RETURN 1/(COUNT(*)-1) AS bad GROUP BY n.k{order}{window}"
            ));
        ...
        let q = prepare(&format!(
            "MATCH (n:L) RETURN DISTINCT COUNT(*) AS n GROUP BY n.k ORDER BY n{window}"
        ));
        let mut s = source(fixture(63));
        s.fail_at = Some(s.rows.len());
        let dropped = s.dropped.clone();
        ...
        assert!(matches!(
            ...
            Some(Err(GqlQueryError::Source(GraphAggregateError::Source(
                VertexScanError::Source("late source")
            ))))
        ));
        assert!(dropped.get());
        assert_eq!(cursor.row_stats().result_rows, 0);
        assert!(cursor.next().is_none());
        let q = prepare(&format!(
            "MATCH (n:L) RETURN MIN(n.v) AS m GROUP BY n.k HAVING m>0 OR TRUE ORDER BY m{window}"
        ));
        let mut rows = fixture(63);
        rows.last_mut()
            .unwrap()
            .1
            .push((V, CanonicalScalar::ucs_basic_text("private").unwrap()));
    ...
    let q =
        prepare("MATCH (n:L) RETURN 1/(COUNT(*)-1) AS bad GROUP BY n.k HAVING COUNT(*)<0 LIMIT 1");
...
fn every_checkpoint_and_exact_boundary_preserve_a_complete_prefix_and_release_the_source() {
    let q = prepare(
        "MATCH (n:L) RETURN DISTINCT AVG(n.v) AS avg GROUP BY n.k HAVING COUNT(*)>0 ORDER BY avg DESC NULLS LAST LIMIT 3",
    );
    let mut calls = 0;
    let mut full = VertexAggregateCursor::new(
        source(fixture(63)),
        VertexAggregatePlan::compile(&q).unwrap(),
        wide(),
        || {
    ...
    for stop in 1..=calls {
        let mut seen = 0;
        let s = source(fixture(63));
        let dropped = s.dropped.clone();
        let mut cursor =
            VertexAggregateCursor::new(s, VertexAggregatePlan::compile(&q).unwrap(), exact, || {
                seen += 1;
                if seen == stop { Err(stop) } else { Ok(()) }
            });
        let mut prefix = Vec::new();
        loop {
            match cursor.next().expect("checkpoint must be visited") {
    ...
    let mut cursor = VertexAggregateCursor::new(
        ...
        VertexAggregatePlan::compile(&q).unwrap(),
        wide(),
        || -> Result<(), ()> { panic!("unpolled close") },
    );
    cursor.close();
    assert!(dropped.get());
    assert_eq!(reads.get(), 0);
    assert!(cursor.next().is_none());
    let mut cursor = run(&q, fixture(63), exact);
...
fn private_groups_and_duplicate_classes_do_not_spend_the_selected_row_allowance() {
    let mut rows: Vec<_> = (0..256)
        .map(|id| record(id, Some(id as i64), Some(7)))
        .collect();
    ...
    for text in [
        ...
        let mut cursor = run(
            &q,
            rows.clone(),
            GqlQueryPolicy::new(257, 1, u64::MAX, u64::MAX),
        );
        let result = cursor.next().unwrap().unwrap();
        assert_eq!(
            result.values(),
            &[GraphAggregateValue::Average(
                GraphExactAverage::new(-1, 1).unwrap()
            )]
        );
        assert_eq!(cursor.row_stats().snapshot_records, 257);
        assert!(cursor.next().is_none());
    }
    for text in [
    ...
    assert_eq!(
        global.next().unwrap().unwrap().values(),
