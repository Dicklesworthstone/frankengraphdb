...
impl Masked {
    fn new() -> Self {
        Self {
            position: 0,
            props: [Some(7), Some(3), Some(999), Some(7), None].into_iter().map(|value| {
                let mut row = Vec::new();
                if let Some(value) = value { row.push((P, CanonicalScalar::Int(value))); }
                row.push((H, CanonicalScalar::ucs_basic_text("hidden-nonnumeric").unwrap()));
                row
            }).collect(),
            reads: Rc::new(Cell::new(0)), dropped: Rc::new(Cell::new(false)), fail_at: None,
        }
    }
}
...
impl VertexScanSource for Masked {
        ...
        -> Result<Option<VertexScanRecord<'a>>, VertexScanSourceError<Self::Error, C>>
    {
        ...
        if self.fail_at == Some(id) { return Err(VertexScanSourceError::Source("record refused")); }
        if id == VId(2) { return Ok(None); }
        let at = IDS.iter().position(|candidate| *candidate == id).unwrap();
        VertexScanRecord::copy_masked(
            VertexScanRow { labels: &[LabelId(1), LabelId(99)], properties: &self.props[at] },
            |id| id == LabelId(1), |key| key == P, control,
...
fn query(text: &str) -> PreparedGraphAggregate {
    PreparedGraphAggregateText::prepare(text, |kind, name: &str| match (kind, name) {
        ...
        (GraphSymbolKind::Relation, "R") => Some(GraphSymbol::Relation(fgdb_delta_types::RelationId(1))),
        _ => None,
...
fn run(text: &str) -> Vec<GraphAggregateRow> {
    VertexAggregateCursor::new(Masked::new(), VertexAggregatePlan::compile(&query(text)).unwrap(),
        policy(), || Ok::<_, usize>(())).collect::<Result<Vec<_>, _>>().unwrap()
}
...
fn all_eleven_cells_group_only_visible_masked_occurrences() {
    ...
    assert_eq!(run(text), vec![GraphAggregateRow::from_group_values(vec![null()], vec![
        Count(4), Count(3), Integer(17), Average(GraphExactAverage::new(17, 3).unwrap()),
        Value(int(3)), Value(int(7)), Count(2), Integer(10),
        Average(GraphExactAverage::new(10, 2).unwrap()),
        Value(GraphValue::List(vec![int(7), int(3), int(7)].into_boxed_slice())),
        Value(GraphValue::List(vec![int(7), int(3)].into_boxed_slice())),
    ])]);
    assert_eq!(run("MATCH (n) RETURN COUNT(n.hidden) AS present,SUM(n.hidden) AS total"),
        vec![GraphAggregateRow::from_global_values(vec![Count(0), Value(null())])]);
    assert_eq!(run("MATCH (n:H) RETURN COUNT(*) AS n"),
        vec![GraphAggregateRow::from_global_values(vec![Count(0)])]);
    assert!(run("MATCH (n) WHERE NOT (n.hidden = 7) RETURN n.p AS key,COUNT(*) AS n GROUP BY n.p").is_empty());
}

#[test]
fn masked_values_precede_computed_input_having_and_ranked_delivery() {
    use crate::algebra::{GraphColumn, GraphPatternBuilder};
    use crate::{GraphAggregate, GraphIntegerBinary, GraphIntegerExpression, GraphIntegerOp,
        GraphSetProjection, GraphSetValue};
    let mut builder = GraphPatternBuilder::new();
    builder.vertex("n").unwrap();
    let input = builder.prepare_values(&[
        GraphColumn::property("hidden", "n", H), GraphColumn::property("p", "n", P),
    ], 0, None).unwrap().with_duplicates();
    let expression = GraphIntegerExpression::prepare(&[
        GraphIntegerOp::Column(0), GraphIntegerOp::Literal(Some(1)),
        GraphIntegerOp::Binary(GraphIntegerBinary::Add),
    ]).unwrap();
    let definition = PreparedGraphAggregate::prepare_projected(input, vec![
        GraphSetProjection::new("key", GraphSetValue::Integer(expression)),
        GraphSetProjection::new("value", GraphSetValue::Column(1)),
    ], &[0], &[GraphAggregate::count_rows("rows"), GraphAggregate::sum_int("sum", 1)], 0, None).unwrap();
    let rows = VertexAggregateCursor::new(Masked::new(), VertexAggregatePlan::compile(&definition).unwrap(),
        policy(), || Ok::<_, usize>(())).collect::<Result<Vec<_>, _>>().unwrap();
    assert_eq!(rows, vec![GraphAggregateRow::from_group_values(vec![null()], vec![
        GraphAggregateValue::Count(4), GraphAggregateValue::Integer(17),
    ])]);
    let rows = run("MATCH (n) RETURN n.p AS key,COUNT(*) AS n GROUP BY n.p HAVING COUNT(*) > 1 ORDER BY key DESC LIMIT 1");
    assert_eq!(rows, vec![GraphAggregateRow::from_group_values(vec![int(7)], vec![GraphAggregateValue::Count(2)])]);
}
...
fn one_record_at_a_time_and_late_failures_release_no_partial_group_even_under_limit_zero() {
    for suffix in ["", " LIMIT 0"] {
        ...
        let mut cursor = VertexAggregateCursor::new(source, VertexAggregatePlan::compile(&definition).unwrap(),
            policy(), || Ok::<_, usize>(()));
        assert_eq!(reads.get(), 0);
        assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(
            GraphAggregateError::Source(VertexScanError::Source("record refused")))))));
        assert!(dropped.get());
    ...
    let reads = Rc::clone(&source.reads);
    let dropped = Rc::clone(&source.dropped);
    let mut cursor = VertexAggregateCursor::new(source, VertexAggregatePlan::compile(&query("MATCH (n) RETURN COUNT(*) AS n")).unwrap(),
        policy(), || Ok::<_, usize>(()));
    cursor.close();
    assert_eq!(reads.get(), 0);
    assert!(dropped.get());
    assert!(cursor.next().is_none());
    let definition = query("MATCH (n) WHERE NOT EXISTS { MATCH (n)-[:R]->(m) } RETURN COUNT(*) AS n");
    let mut cursor = VertexAggregateCursor::new(Masked::new(), VertexAggregatePlan::compile(&definition).unwrap(),
        policy(), || Ok::<_, usize>(()));
...
fn all_cancellation_cuts_and_inclusive_native_budgets_use_the_masked_record_path() {
    let definition = query("MATCH (n) RETURN n.p AS key,SUM(n.p) AS total GROUP BY n.p ORDER BY key DESC");
    ...
    assert_eq!(VertexAggregateCursor::new(Masked::new(), plan.clone(), exact, || Ok::<_, usize>(()))
        .collect::<Result<Vec<_>, _>>().unwrap(), expected);
    for limits in [
        GqlQueryPolicy::new(rows.snapshot_records - 1, rows.result_rows, evaluator.work_units, evaluator.scratch_entries),
        GqlQueryPolicy::new(rows.snapshot_records, rows.result_rows - 1, evaluator.work_units, evaluator.scratch_entries),
        GqlQueryPolicy::new(rows.snapshot_records, rows.result_rows, evaluator.work_units - 1, evaluator.scratch_entries),
        GqlQueryPolicy::new(rows.snapshot_records, rows.result_rows, evaluator.work_units, evaluator.scratch_entries - 1),
    ] {
        assert!(VertexAggregateCursor::new(Masked::new(), plan.clone(), limits, || Ok::<_, usize>(()))
            .collect::<Result<Vec<_>, _>>().is_err());
    }
}
