...
mod tests {
    ...
    use crate::stream::{VertexScanCursor, VertexScanPlan, VertexScanSource, VertexScanSourceError, VertexScanState};
    ...
    impl VertexScanSource for Source {
        type Error = ();
        fn snapshot_seq(&self) -> CommitSeq { CommitSeq(3) }
        ...
        fn vertex<'a, C>(&'a self, _: VId, control: &mut impl FnMut(VertexScanEvent) -> Result<(), C>)
            -> Result<Option<VertexScanRow<'a>>, VertexScanSourceError<(), C>>
        {
            control(VertexScanEvent::Work).map_err(VertexScanSourceError::Control)?;
            Ok(Some(VertexScanRow { labels: &self.labels, properties: &self.properties }))
        }
    }
    ...
    impl VertexScanSource for Masked {
        ...
        {
            self.0.vertex(vid, control)?.map(|row| {
                VertexScanRecord::copy_masked(row, |label| label == LabelId(1),
                    |key| key == PropertyKeyId(1), control).map_err(VertexScanSourceError::Control)
            }).transpose()
        }
    }
    fn source() -> Source {
        Source { sent: false, labels: vec![LabelId(1), LabelId(99)], properties: vec![
            (PropertyKeyId(1), CanonicalScalar::Int(8)),
            (PropertyKeyId(2), CanonicalScalar::ucs_basic_text(&"private".repeat(100)).unwrap()),
        ] }
    }
    ...
    fn default_record_is_the_same_borrow_and_does_not_add_source_events() {
        ...
        let record = source.vertex_record(VId(7), &mut |event| { events.push(event); Ok::<_, ()>(()) }).unwrap().unwrap();
        assert!(matches!(record, VertexScanRecord::Borrowed(_)));
        assert!(std::ptr::eq(record.as_row().properties.as_ptr(), source.properties.as_ptr()));
    ...
    fn owned_masking_precedes_boolean_predicates_and_projections_in_the_existing_cursor() {
        ...
        assert!(raw.next().is_none(), "unmasked negative control sees the hidden property");
        let mut masked = VertexScanCursor::new(Masked(source()), plan(query), policy(), || Ok::<_, ()>(()));
        assert_eq!(masked.next().unwrap().unwrap().values(), &[
            GraphValue::Vertex(VId(7)), GraphValue::Scalar(CanonicalScalar::Int(8)),
            GraphValue::Scalar(CanonicalScalar::Null),
        ]);
        assert!(masked.next().is_none());
        let mut masked = VertexScanCursor::new(Masked(source()), plan("MATCH (n:H) RETURN n AS id"), policy(), || Ok::<_, ()>(()));
        assert!(masked.next().is_none());
    }
    ...
    fn each_record_copy_cut_refuses_without_output_and_hidden_payload_size_adds_no_copy_work() {
        let large = source();
        let tiny = Source { properties: vec![(PropertyKeyId(1), CanonicalScalar::Int(8)),
            (PropertyKeyId(2), CanonicalScalar::Int(0))], ..source() };
        let copy = |source: &Source, stop: usize| {
            let mut calls = 0;
            let result = VertexScanRecord::copy_masked(
                VertexScanRow { labels: &source.labels, properties: &source.properties },
                |id| id == LabelId(1), |key| key == PropertyKeyId(1),
        ...
        assert_eq!(copied.as_row().properties, &[(PropertyKeyId(1), CanonicalScalar::Int(8))]);
        ...
        let visible = VertexScanRecord::copy_masked(
            VertexScanRow { labels: &large.labels, properties: &large.properties },
            |_| true, |_| true, &mut |_| { calls += 1; Ok::<_, ()>(()) },
        ).unwrap();
        assert_eq!(visible.as_row().properties, large.properties);
        assert!(calls > count, "visible scalar payload copies must spend their payload units");
    }
    ...
    fn owned_record_cursor_retains_exact_native_quotas_and_fuses_every_interruption() {
        ...
        let mut cursor = VertexScanCursor::new(Masked(source()), plan(text), exact, || Ok::<_, ()>(()));
        assert_eq!(cursor.by_ref().collect::<Result<Vec<_>, _>>().unwrap(), rows);
        for budget in [GqlQueryPolicy::new(1, 1, stats.work_units - 1, stats.scratch_entries),
            GqlQueryPolicy::new(1, 1, stats.work_units, stats.scratch_entries - 1)]
        {
            let mut cursor = VertexScanCursor::new(Masked(source()), plan(text), budget, || Ok::<_, ()>(()));
