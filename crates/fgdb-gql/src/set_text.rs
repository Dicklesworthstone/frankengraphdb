...
use crate::{
    ...
    GraphPatternTextError, GraphPatternTextErrorKind, GraphSetBuildError,
    GraphSetColumnType, GraphSetOperation, GraphSetQuantifier, GraphSymbol,
    GraphSymbolKind, MAX_GRAPH_SET_OPERANDS, PreparedGraphSet, PreparedGraphText,
};
...
impl core::fmt::Display for GraphSetTextError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "graph set text error at byte {}: {:?}", self.offset, self.kind)
    }
}
...
pub(crate) enum TextKind<'a> {
    Word(&'a str), Digits(&'a str), Parameter(&'a str), Quoted, Punct(u8), End,
}
#[derive(Clone, Copy)]
...
enum NodeKind {
    Leaf(usize),
    Scope(Box<Node>),
    Binary { operation: GraphSetOperation, quantifier: GraphSetQuantifier, left: Box<Node>, right: Box<Node> },
}
#[derive(Clone)]
struct Node {
    kind: NodeKind,
...
impl Node {
    fn new(kind: NodeKind, at: usize, depth: usize) -> Result<Self, GraphSetTextError> {
        if depth > MAX_GRAPH_SET_DEPTH {
            return Err(fail(at, GraphSetTextErrorKind::SetBuild(GraphSetBuildError::TooDeep {
                limit: MAX_GRAPH_SET_DEPTH, observed: depth,
            })));
        }
        Ok(Self { kind, at, depth, order: Vec::new(), offset: PageNumber::Literal(0), count: None })
    }
    fn validate(&mut self, schemas: &[Schema]) -> Result<usize, GraphSetTextError> {
        let first = match &mut self.kind {
            ...
            NodeKind::Binary { left, right, .. } => {
                ...
                for (column, (&left, &right)) in left.iter().zip(right).enumerate() {
                    if left != right {
                        return Err(fail(self.at, GraphSetTextErrorKind::SetBuild(GraphSetBuildError::ColumnType {
                            column, left, right,
                        })));
                    }
                }
        ...
        for key in &mut self.order {
            let column = schema.columns.iter().position(|name| *name == key.name)
                .ok_or_else(|| expected(key.at, "a leftmost output column name"))?;
            if !used.insert(column) {
                return Err(fail(key.at, GraphSetTextErrorKind::OrderBuild(GraphOrderError::DuplicateColumn { column })));
            }
            key.order.column = column;
        }
        Ok(first)
    }
    fn bind(&self, inputs: &mut [Option<PreparedGraphSet>], arguments: &GqlParameters)
        ...
        let mut bound = match &self.kind {
            ...
            NodeKind::Scope(input) => input.bind(inputs, arguments)?.nested()
                .map_err(|kind| fail(self.at, GraphSetTextErrorKind::SetBuild(kind)))?,
            NodeKind::Binary { operation, quantifier, left, right } => {
        ...
        if !self.order.is_empty() {
            ...
            bound = bound.with_order_by(&order)
...
impl<'a> Composition<'a> {
    fn current(&self) -> TextToken<'a> { self.tokens[self.at] }
    fn advance(&mut self) {
        if !matches!(self.current().kind, TextKind::End) { self.at += 1; }
    }
    fn take_word(&mut self, word: &str) -> bool {
        if !self.current().word(word) { return false; }
        self.advance(); true
    }
    fn take(&mut self, ch: u8) -> bool {
        if !self.current().punct(ch) { return false; }
        self.advance(); true
    }
    fn require(&mut self, ch: u8, item: &'static str) -> Result<(), GraphSetTextError> {
        if self.take(ch) { Ok(()) } else { Err(expected(self.current().at, item)) }
    }
    fn expression(&mut self, depth: usize) -> Result<Node, GraphSetTextError> {
        if depth >= MAX_GRAPH_SET_DEPTH {
            return Err(fail(self.current().at, GraphSetTextErrorKind::SetBuild(GraphSetBuildError::TooDeep {
                limit: MAX_GRAPH_SET_DEPTH, observed: depth + 1,
            })));
        }
        let mut node = self.intersection(depth)?;
        while self.current().word("UNION") || self.current().word("EXCEPT") {
            let token = self.current();
            let operation = if token.word("UNION") { GraphSetOperation::Union } else { GraphSetOperation::Except };
            self.advance();
            let quantifier = self.quantifier();
            let right = self.intersection(depth)?;
            let height = 1 + node.depth.max(right.depth);
            node = Node::new(NodeKind::Binary { operation, quantifier, left: Box::new(node), right: Box::new(right) }, token.at, height)?;
        }
        self.tail(&mut node)?;
        Ok(node)
    }
    fn intersection(&mut self, depth: usize) -> Result<Node, GraphSetTextError> {
        ...
        while self.current().word("INTERSECT") {
            ...
            let height = 1 + node.depth.max(right.depth);
            node = Node::new(NodeKind::Binary { operation: GraphSetOperation::Intersect, quantifier,
                left: Box::new(node), right: Box::new(right) }, at, height)?;
        }
        Ok(node)
    }
    fn quantifier(&mut self) -> GraphSetQuantifier {
        if self.take_word("ALL") { GraphSetQuantifier::All }
        else { self.take_word("DISTINCT"); GraphSetQuantifier::Distinct }
    }
    fn term(&mut self, depth: usize) -> Result<Node, GraphSetTextError> {
        ...
        if !self.current().word("MATCH") { return Err(expected(at, "MATCH or parenthesized set expression")); }
        if self.spans.len() == MAX_GRAPH_SET_OPERANDS {
            return Err(fail(at, GraphSetTextErrorKind::SetBuild(GraphSetBuildError::TooManyOperands {
                limit: MAX_GRAPH_SET_OPERANDS, observed: self.spans.len() + 1,
            })));
        }
        ...
        loop {
            let token = self.current();
            if matches!(token.kind, TextKind::End) { break; }
            if delimiters.is_empty() {
                if token.punct(b')') { break; }
                if returned && (token.word("ORDER") || token.word("SKIP") || token.word("LIMIT") || self.set_separator()) { break; }
                if token.word("RETURN") { returned = true; }
            }
            match token.kind {
                ...
                TextKind::Punct(ch @ (b')' | b']' | b'}')) => {
                    let opening = match ch { b')' => b'(', b']' => b'[', _ => b'{' };
                    if delimiters.pop() != Some(opening) { return Err(expected(token.at, "matching graph-expression delimiter")); }
                }
                _ => {}
            }
            self.advance();
        }
        if !delimiters.is_empty() { return Err(expected(self.current().at, "closing graph-expression delimiter")); }
        let leaf = self.spans.len();
        self.spans.push(Span { start: at, end: self.current().at, first_token, last_token: self.at });
        Node::new(NodeKind::Leaf(leaf), at, 1)
    }
    fn set_separator(&self) -> bool {
        let token = self.current();
        if !(token.word("UNION") || token.word("INTERSECT") || token.word("EXCEPT")) { return false; }
        if self.at > 0 && (self.tokens[self.at - 1].punct(b'.') || self.tokens[self.at - 1].word("AS")) { return false; }
        let mut next = self.at + 1;
        if self.tokens.get(next).is_some_and(|token| token.word("ALL") || token.word("DISTINCT")) { next += 1; }
        self.tokens.get(next).is_some_and(|token| token.word("MATCH") || token.punct(b'('))
    }
    fn tail(&mut self, node: &mut Node) -> Result<(), GraphSetTextError> {
        if self.take_word("ORDER") {
            if !self.take_word("BY") { return Err(expected(self.current().at, "BY")); }
            loop {
                let at = self.current().at;
                let TextKind::Word(name) = self.current().kind else { return Err(expected(at, "output column name")); };
                ...
                if node.order.len() == crate::algebra::MAX_PATTERN_VERTICES {
                    return Err(fail(at, GraphSetTextErrorKind::OrderBuild(GraphOrderError::TooManyColumns {
                        limit: crate::algebra::MAX_PATTERN_VERTICES, observed: node.order.len() + 1,
                    })));
                }
                node.order.push(OrderKey { name, at, order: GraphValueOrder { column: 0, descending, nulls_first } });
                if !self.take(b',') { break; }
            }
        }
        if self.take_word("SKIP") { node.offset = self.page_number()?; }
        if self.take_word("LIMIT") { node.count = Some(self.page_number()?); }
        Ok(())
    }
    fn page_number(&mut self) -> Result<PageNumber, GraphSetTextError> {
        let token = self.current();
        let result = match token.kind {
            TextKind::Digits(digits) => PageNumber::Literal(digits.parse::<u64>()
                .map_err(|_| pattern_kind(token.at, GraphPatternTextErrorKind::IntegerOutOfRange))?),
            TextKind::Parameter(name) => {
                let name = name.to_owned();
                self.page_parameters.push((token.at, GqlParameterSpec { name: name.clone(),
...
impl core::fmt::Debug for PreparedGraphSetText {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("PreparedGraphSetText")
            .field("operands", &self.inputs.len()).field("columns", &self.columns.len())
...
impl PreparedGraphSetText {
    ...
    pub fn prepare_with_parameter_types(
        statement: &str, declarations: &[(&str, GqlParameterType)],
        ...
        let mut parser = Composition { tokens, at: 0, spans: Vec::new(), page_parameters: Vec::new() };
        ...
        for span in &parser.spans {
            let names: BTreeSet<_> = parser.tokens[span.first_token..span.last_token].iter()
        ...
        for (offset, spec) in uses {
            if let Some(&at) = parameter_index.get(&spec.name) {
                ...
                if previous.parameter_type != spec.parameter_type {
                    return Err(pattern_kind(offset, GraphPatternTextErrorKind::ConflictingParameterTypes));
                }
        ...
        for &(name, kind) in declarations {
            let Some(&at) = parameter_index.get(name) else {
                return Err(pattern_kind(statement.len(), GraphPatternTextErrorKind::UnusedParameterDeclaration));
            };
            if parameters[at].parameter_type != kind {
                return Err(pattern_kind(parameter_offsets[at], GraphPatternTextErrorKind::ConflictingParameterTypes));
            }
        }
        ...
        let mut symbols = |kind, name: &str| {
            ...
            if let Some(value) = cache.get(&key) { return Some(*value); }
        ...
        for (input, span) in pending.into_iter().zip(&parser.spans) {
            let input = input.resolve(&mut symbols).map_err(|error| pattern_error(span.start, error))?;
            inputs.push((span.start, input));
        }
        Ok(Self { statement: statement.to_owned(), root, inputs,
            columns: schemas[first].columns.clone(), types: schemas[first].types.clone(),
            parameters, parameter_offsets })
    }
    #[must_use]
    ...
    pub fn bind_parameters(&self, arguments: &GqlParameters) -> Result<PreparedGraphSet, GraphSetTextError> {
        ...
        for (spec, &offset) in self.parameters.iter().zip(&self.parameter_offsets) {
            ...
            if !spec.parameter_type.accepts(value.parameter_type()) {
                return Err(pattern_kind(offset, GraphPatternTextErrorKind::ParameterTypeMismatch {
                    expected: spec.parameter_type, found: value.parameter_type(),
                }));
            }
        }
        if arguments.len() != self.parameters.len() {
            return Err(pattern_kind(self.statement.len(), GraphPatternTextErrorKind::UnexpectedArguments));
        }
        let mut inputs = Vec::new();
