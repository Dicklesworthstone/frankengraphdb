...
fn same_value(left: &ReadValueTemplate, right: &ReadValueTemplate) -> bool {
    match (left, right) {
        ...
        (ReadValueTemplate::Parameter { index: a, .. }, ReadValueTemplate::Parameter { index: b, .. }) => a == b,
        (ReadValueTemplate::Integer { program: a, .. }, ReadValueTemplate::Integer { program: b, .. }) => {
            a.len() == b.len() && a.iter().zip(b).all(|(a, b)| match (a, b) {
                (MutationIntegerTemplateOp::Bound(a), MutationIntegerTemplateOp::Bound(b)) => a == b,
                (MutationIntegerTemplateOp::Parameter { index: a, .. },
                    MutationIntegerTemplateOp::Parameter { index: b, .. }) => a == b,
                _ => false,
            })
        }
        _ => false,
...
impl<'a> Parser<'a> {
    pub(in crate::graph_text) fn aggregate_scalar_expression(
        ...
        let operand = if bare_vertex {
            ...
            self.mutation_expression(&mut sources).map_err(scalar_error)?
        };
        computed.sources = sources.iter().map(|source| (source.variable, source.property)).collect();
        if let Operand::Column(column) = operand {
            let source = sources[column];
            return Ok(Expression { variable: source.variable, property: source.property, computed: None });
        }
        ...
        let index = if let Some(index) = computed.operands.iter().position(|old| same_value(old, &value)) {
            index
        } else {
            self.capacity(computed.operands.len(), MAX_PATTERN_VERTICES,
