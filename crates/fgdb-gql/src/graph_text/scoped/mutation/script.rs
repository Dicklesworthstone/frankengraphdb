...
fn next_script_token<'a>(lexer: &mut Lexer<'a>) -> Result<Token<'a>, GraphPatternTextError> {
    ...
    if lexer.text.as_bytes().get(lexer.at) == Some(&b';') {
        let at = lexer.at;
        lexer.at += 1;
        return Ok(Token { at, kind: TokenKind::Punct(b';') });
    }
    lexer.next()
}
...
fn scan(script: &str) -> Result<Vec<Statement<'_>>, GraphWriteScriptError> {
    if script.len() > MAX_GRAPH_WRITE_SCRIPT_BYTES {
        return Err(GraphWriteScriptError {
            statement: None, offset: MAX_GRAPH_WRITE_SCRIPT_BYTES,
            kind: GraphWriteScriptErrorKind::DefinitionTooLarge {
                limit: MAX_GRAPH_WRITE_SCRIPT_BYTES, observed: script.len(),
            },
        });
    }
    let mut lexer = Lexer { text: script, at: 0, tokens: 0 };
    ...
    loop {
        ...
        if end || separator {
            if !nesting.is_empty() {
                return Err(refusal(statement, token.at, GraphWriteScriptErrorKind::Syntax(
                    GraphPatternTextErrorKind::Expected("closing delimiter before statement end"),
                )));
            }
            if !nonempty {
                if end && !result.is_empty() { break; }
                return Err(refusal(statement, token.at, GraphWriteScriptErrorKind::EmptyStatement));
            }
            let bytes = token.at - start;
            if bytes > MAX_GRAPH_TEXT_BYTES {
                return Err(refusal(statement, start + MAX_GRAPH_TEXT_BYTES,
                    GraphWriteScriptErrorKind::DefinitionTooLarge { limit: MAX_GRAPH_TEXT_BYTES, observed: bytes }));
            }
            if result.len() == MAX_GRAPH_MUTATION_STATEMENTS {
                return Err(refusal(statement, start, GraphWriteScriptErrorKind::TooManyStatements {
                    limit: MAX_GRAPH_MUTATION_STATEMENTS, observed: statement + 1,
                }));
            }
            result.push(Statement { span: start..token.at,
                parameters: core::mem::take(&mut parameters), last_on: last_on.take() });
            if end { break; }
            start = lexer.at;
            nonempty = false;
            previous_dot = false;
        ...
        if lexer.at - start > MAX_GRAPH_TEXT_BYTES {
            return Err(refusal(statement, start + MAX_GRAPH_TEXT_BYTES,
                GraphWriteScriptErrorKind::DefinitionTooLarge {
                    limit: MAX_GRAPH_TEXT_BYTES, observed: lexer.at - start,
                }));
        }
        match token.kind {
            TokenKind::Punct(open @ (b'(' | b'[' | b'{')) => nesting.push(open),
            TokenKind::Punct(close @ (b')' | b']' | b'}')) => {
                let expected = match close { b')' => b'(', b']' => b'[', _ => b'{' };
                if nesting.pop() != Some(expected) {
                    return Err(refusal(statement, token.at, GraphWriteScriptErrorKind::Syntax(
                        GraphPatternTextErrorKind::Expected("matching opening delimiter"),
                    )));
                }
            }
            TokenKind::Parameter(name) => { parameters.insert(name); }
            TokenKind::Word(word) if nesting.is_empty() && !previous_dot && word.eq_ignore_ascii_case("ON") => {
...
fn classify(text: &str, statement: &Statement<'_>, declarations: &[(&str, GqlParameterType)])
    -> Result<Kind, GraphPatternTextError>
{
    ...
    if parser.is_word("MERGE") {
        let branch = statement.last_on.is_some_and(|at| at > statement.span.start + parser.current.at);
    ...
    Err(error(parser.current.at, GraphPatternTextErrorKind::Expected("a supported graph write statement")))
}

impl PreparedGraphWriteScript {
    pub fn prepare(script: &str, relation: RelationId,
    ...
    pub fn prepare_with_parameter_types(script: &str, relation: RelationId,
        ...
        -> Result<Self, GraphWriteScriptError>
    {
        ...
        for &(name, _) in declarations {
            if !scanned.iter().any(|statement| statement.parameters.contains(name)) {
                return Err(GraphWriteScriptError {
                    statement: None, offset: 0,
                    kind: GraphWriteScriptErrorKind::Syntax(GraphPatternTextErrorKind::UnusedParameterDeclaration),
                });
            }
        }
        let spans = scanned.iter().map(|statement| statement.span.clone()).collect::<Vec<_>>();
        ...
        for (index, statement) in scanned.iter().enumerate() {
            let local = declarations.iter().copied()
                .filter(|(name, _)| statement.parameters.contains(*name)).collect::<Vec<_>>();
            let kind = classify(&script[statement.span.clone()], statement, &local)
                .map_err(|source| GraphWriteScriptError::syntax(Some(index), statement.span.start, source))?;
            kinds.push(kind);
            locals.push(local);
        }
        ...
        for (statement, definition) in scanned.iter().enumerate() {
            ...
            let prepared: Result<GraphWriteTemplateStatement, GraphWriteProgramTemplateError> = match kinds[statement] {
                Kind::Mutation => PreparedGraphMutationText::prepare_with_parameter_types(text, relation, local, &mut catalog)
                    .map(Into::into).map_err(|source| GraphMutationProgramTemplateError::Bind { statement, source }.into()),
                Kind::Insert => PreparedGraphInsertText::prepare_with_parameter_types(text, relation, local, &mut catalog)
                    .map(Into::into).map_err(|source| GraphWriteProgramTemplateError::InsertBind { statement, source }),
                Kind::VertexMerge => PreparedGraphVertexMergeText::prepare_with_parameter_types(text, relation, local, &mut catalog)
                    .map(Into::into).map_err(|source| GraphWriteProgramTemplateError::VertexMergeBind { statement, source }),
                Kind::VertexUpsert => PreparedGraphVertexUpsertText::prepare_with_parameter_types(text, relation, local, &mut catalog)
                    .map(Into::into).map_err(|source| GraphWriteProgramTemplateError::VertexUpsertBind { statement, source }),
                Kind::EdgeMerge => PreparedGraphEdgeMergeText::prepare_with_parameter_types(text, relation, local, &mut catalog)
        ...
        Ok(Self { script: script.to_owned(), program, spans: spans.into_boxed_slice() })
    }
}
