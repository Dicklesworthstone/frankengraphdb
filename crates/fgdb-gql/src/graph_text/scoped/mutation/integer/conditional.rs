...
impl<'a> Parser<'a> {
    ...
    pub(in crate::graph_text) fn starts_integer_case(&self) -> Result<bool, GraphPatternTextError> {
        ...
        Ok(match next.kind {
            TokenKind::Word(word) => ![
                "AS", "THEN", "ELSE", "END", "AND", "OR", "RETURN", "SET", "REMOVE", "GROUP",
                "HAVING", "ORDER", "SKIP", "LIMIT", "WITH", "WHERE",
            ]
            .iter()
    ...
    pub(super) fn integer_case(
        ...
        loop {
            self.word("WHEN")?;
            if searched {
                self.case_disjunction(columns, depth, program)?;
            } else {
                self.integer_sum(columns, depth, program)?;
            }
            self.word("THEN")?;
        ...
        if self.take_word("ELSE")? {
            ...
            bound(program, GraphIntegerOp::Literal(None), at)?;
        }
        self.word("END")?;
        if searched {
            ...
            for _ in 0..alternatives {
                bound(program, GraphIntegerOp::Case, at)?;
            }
        } else {
    ...
    fn case_negation(
        ...
        let next = self.lexer.clone().next()?;
        let scalar_suffix = matches!(
            next.kind,
            TokenKind::Punct(b'.' | b'+' | b'-' | b'*' | b'/' | b'%' | b'=' | b'!' | b'<' | b'>')
            ...
            match token.kind {
                TokenKind::Punct(b'(') => {
                    ...
                    if depth > MAX_INTEGER_NESTING {
                        return Err(error(
                            token.at,
                            GraphPatternTextErrorKind::Expected("bounded CASE parentheses"),
                        ));
                    }
                }
                TokenKind::Punct(b')') => {
                ...
                TokenKind::End => {
                    return Err(error(
                        token.at,
                        GraphPatternTextErrorKind::Expected("closing CASE parenthesis"),
                    ));
                }
                _ => {}
            }
