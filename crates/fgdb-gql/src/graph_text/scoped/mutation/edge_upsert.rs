...
impl<'a> Parser<'a> {
    fn edge_upsert_pattern(&mut self) -> Result<ParsedEdgeUpsert<'a>, GraphEdgeUpsertTextError> {
        ...
        if self.syntax.variables.iter().any(|variable| variable.text == relationship.text) {
            return Err(error(relationship.at,
                GraphPatternTextErrorKind::Expected("a relationship variable distinct from vertex variables")).into());
        }
        self.punct(b':', ":")?;
            ...
            let create = if self.take_word("MATCH")? {
                if saw_match {
                    return Err(GraphEdgeUpsertTextError {
                        offset: branch_at, kind: GraphEdgeUpsertTextErrorKind::DuplicateBranch,
                    });
                }
                saw_match = true;
                false
            ...
            loop {
                    ...
                    return Err(GraphEdgeUpsertTextError {
                        ...
                        kind: GraphEdgeUpsertTextErrorKind::UpsertBuild(
                            crate::GraphEdgeUpsertBuildError::TooManyActions {
                                branch: if create { GraphEdgeUpsertBranch::Create } else { GraphEdgeUpsertBranch::Match },
                ...
                if actual.text != relationship.text {
                    return Err(error(actual.at,
                        GraphPatternTextErrorKind::Expected("the MERGE relationship variable")).into());
                }
        ...
        if on_match.is_empty() && on_create.is_empty() {
            return Err(error(self.current.at,
                GraphPatternTextErrorKind::Expected("ON MATCH SET or ON CREATE SET")).into());
        }
        self.end()?;
    ...
    pub fn prepare_with_parameter_types(
        ...
        if found != relation {
            return Err(GraphEdgeUpsertTextError {
                ...
                kind: GraphEdgeUpsertTextErrorKind::Merge(GraphEdgeMergeTextErrorKind::RelationMismatch),
            });
        }
        let mut resolve_actions = |parsed: Vec<(Name<'_>, EdgeUpsertValueTemplate)>|
            -> Result<Vec<EdgeUpsertActionTemplate>, GraphPatternTextError>
        {
            parsed.into_iter().map(|(key, value)| {
        ...
        let merge = PreparedGraphEdgeMergeText { selection, relation, source, destination };
