...
pub enum GqlParameterError {
    ...
    ConflictingTypes {
        name: String,
        first: GqlParameterType,
        second: GqlParameterType,
    },
    TypeMismatch {
        name: String,
...
impl core::fmt::Display for GqlParameterError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::UnsupportedPosition { offset } => write!(
                f,
                "parameter at byte {offset} is not a numeric predicate, SKIP, or LIMIT"
            ),
            Self::InvalidArgumentName { name } => write!(
                f,
                "invalid parameter argument name {name:?}; omit the leading $"
            ),
            ...
            Self::ConflictingTypes {
                ...
                f,
                "parameter {name:?} is used as both {first:?} and {second:?}"
            ),
            Self::TypeMismatch {
                name,
                expected,
                ...
                f,
                "parameter {name:?} requires {expected:?}, found {found:?}"
            ),
            Self::PositiveLimitRequired { name } => {
                write!(f, "LIMIT parameter {name:?} must be positive")
            }
            Self::DefinitionMismatch { offset } => write!(
                f,
                "parameter at byte {offset} does not resolve to the expected bound numeric slot"
            ),
        }
    }
}
...
impl Target {
    ...
    fn assign(self, plan: &mut BoundPlan, value: GqlParameterValue) -> bool {
        match (self, value) {
            ...
            (Self::Limit, GqlParameterValue::UInt64(value))
                if value > 0 && plan.limit.is_some() =>
            {
...
impl PreparedGqlTemplate {
    pub fn prepare(
        ...
        Ok(Self {
            ...
            prototype,
            occurrences,
            schema: schema.into_values().collect(),
        })
    }
    ...
    pub fn validate_parameters(&self, parameters: &GqlParameters) -> Result<(), GqlParameterError> {
        for spec in &self.schema {
            ...
            if spec.requires_positive && value == GqlParameterValue::UInt64(0) {
                return Err(GqlParameterError::PositiveLimitRequired {
                    name: spec.name.clone(),
                });
            }
        }
        for name in parameters.values.keys() {
            if self
                .schema
                .binary_search_by(|spec| spec.name.as_str().cmp(name.as_str()))
                .is_err()
            {
    ...
    pub fn bind_parameters(
    ...
    ) -> Result<PreparedGqlQuery, GqlParameterError> {
        ...
        Ok(PreparedGqlQuery::from_parameter_instantiation(
            statement,
            self.bind.clone(),
            plan,
        ))
    ...
    pub fn binding_digest(
        &self,
        parameters: &GqlParameters,
    ) -> Result<fgdb_crypto::Digest, GqlParameterError> {
        self.validate_parameters(parameters)?;
...
mod tests {
    ...
    fn bindings() -> RelationBind {
        RelationBind::new()
            ...
            .with_property("m", PropertyKeyId(5))
    }
    ...
    fn typed_binding_reuses_one_definition_and_preserves_concrete_coherence() {
        ...
        let source =
            "MATCH (a)-[:R]->(b) WHERE a.n=$min AND b.m<>$other RETURN b SKIP $offset LIMIT $cap";
        let template = PreparedGqlTemplate::prepare(source, &bind).unwrap();
        bind.insert("R", RelationId(99));
        let first = GqlParameters::new()
            .with_int64("min", i64::MIN)
            .unwrap()
            .with_int64("other", i64::MAX)
            .unwrap()
            .with_uint64("offset", 0)
            .unwrap()
            .with_uint64("cap", u64::MAX)
            .unwrap();
    ...
    fn repeated_names_share_a_type_and_intersect_count_constraints() {
        let shared = PreparedGqlTemplate::prepare(
            "MATCH (a)-[:R]->(b) WHERE a.n=$x AND b.m=$x RETURN b",
            &bindings(),
        )
        .unwrap();
        ...
        let query = shared
            .bind_parameters(&GqlParameters::new().with_int64("x", -7).unwrap())
            .unwrap();
        ...
        let counts =
            PreparedGqlTemplate::prepare("MATCH (a:L) RETURN a SKIP$n LIMIT$n", &bindings())
                .unwrap();
        assert!(counts.parameter_schema()[0].requires_positive);
        assert!(matches!(
            counts.bind_parameters(&GqlParameters::new().with_uint64("n", 0).unwrap()),
            Err(GqlParameterError::PositiveLimitRequired { .. })
        ));
        assert!(
            counts
                .bind_parameters(&GqlParameters::new().with_uint64("n", 1).unwrap())
                .unwrap()
                .verifies_definition()
        );
        assert!(matches!(
            PreparedGqlTemplate::prepare("MATCH (a:L) WHERE a.n=$x RETURN a LIMIT$x", &bindings()),
    ...
    fn argument_errors_are_explicit_and_do_not_poison_future_bindings() {
        let template = PreparedGqlTemplate::prepare(
            "MATCH (a:L) WHERE a.n=$x RETURN a LIMIT$cap",
            &bindings(),
        )
        .unwrap();
        assert!(matches!(
            template.bind_parameters(&GqlParameters::new()),
            Err(GqlParameterError::Missing { .. })
        ));
        let wrong = GqlParameters::new()
            ...
            .unwrap();
        assert!(matches!(
            template.bind_parameters(&wrong),
            Err(GqlParameterError::TypeMismatch { .. })
        ));
        let valid = GqlParameters::new()
            .with_int64("x", 9)
            .unwrap()
            .with_uint64("cap", 1)
            .unwrap();
        ...
        assert!(matches!(
            ...
            Err(GqlParameterError::Unexpected { .. })
        ));
        let mut duplicate = valid.clone();
        assert!(matches!(
            duplicate.insert("x", GqlParameterValue::Int64(99)),
            Err(GqlParameterError::Duplicate { .. })
        ));
        assert_eq!(duplicate, valid);
        assert!(
            template
                .bind_parameters(&valid)
                .unwrap()
                .verifies_definition()
        );
        assert!(matches!(
            GqlParameters::new().with_int64("$x", 9),
            Err(GqlParameterError::InvalidArgumentName { .. })
        ));
    }
    ...
    fn parameter_bindings_equal_literal_binding_for_every_supported_numeric_position() {
            ...
            for variable in ["a", "b", "c"] {
                for operator in ["=", "<>", "!=", ">", "<", ">=", "<="] {
                    let template_source = format!(
                        "{pattern} WHERE {variable}.n{operator}$value RETURN {returned} SKIP$skip LIMIT$cap"
                    );
                    let probe = format!(
                        "{pattern} WHERE {variable}.n{operator}1 RETURN {returned} SKIP 0 LIMIT 2"
                    );
                    if bind.bind(&probe).is_err() {
                        assert!(
                            PreparedGqlTemplate::prepare(&template_source, &bind).is_err(),
                            "{template_source}"
                        );
                        refused += 1;
                        continue;
        ...
        assert!(
            supported > 40,
            "positive corpus unexpectedly shrank: {supported}"
        );
        assert!(
            refused > 40,
            "refusal corpus unexpectedly shrank: {refused}"
        );
    }
    ...
    fn parameters_cannot_supply_names_operators_clauses_or_unary_expressions() {
        for source in [
            ...
            "MATCH (a:L) RETURN $a",
            "MATCH (a:L) WHERE a.$key=1 RETURN a",
            "MATCH (a:L) WHERE a.n $op 1 RETURN a",
            "MATCH (a:L) WHERE a.n=-$value RETURN a",
            "MATCH (a:L) WHERE a.n=1$value RETURN a",
            "MATCH (a:L) WHERE a.n=$value+1 RETURN a",
            "MATCH (a:L) RETURN a LIMIT $$cap",
            "MATCH (a:L) RETURN a LIMIT $",
            "MATCH (a:L) RETURN a LIMIT $9x",
            "MATCH (a:L) RETURN a LIMIT $雪",
            "MATCH (a:L) WHERE a.n='$value' RETURN a",
        ] {
            assert!(
                PreparedGqlTemplate::prepare(source, &bindings()).is_err(),
                "{source}"
            );
        }
    }
    ...
    fn original_byte_offsets_survive_multiple_placeholders_and_unicode_whitespace() {
        ...
        assert!(
            matches!(error, GqlParameterError::Bind(BindError::Parse(ref error))
    ...
    fn canonical_parameter_identity_is_order_independent_but_type_and_value_sensitive() {
        let first = GqlParameters::new()
            ...
            .with_uint64("n", 2)
            .unwrap();
        let reordered = GqlParameters::new()
            .with_uint64("n", 2)
            .unwrap()
            .with_int64("x", 3)
            .unwrap();
        ...
        let template =
            PreparedGqlTemplate::prepare("MATCH (a:L) WHERE a.n=$x RETURN a LIMIT$n", &bindings())
                .unwrap();
        assert_eq!(
            template.binding_digest(&first).unwrap(),
            template.binding_digest(&reordered).unwrap()
        );
        assert_ne!(
            template.binding_digest(&first).unwrap(),
            template.binding_digest(&changed).unwrap()
        );
        let other =
            PreparedGqlTemplate::prepare("MATCH (a:L) WHERE a.n<>$x RETURN a LIMIT$n", &bindings())
                .unwrap();
        assert_ne!(
            template.binding_digest(&first).unwrap(),
            other.binding_digest(&first).unwrap()
        );
    }
