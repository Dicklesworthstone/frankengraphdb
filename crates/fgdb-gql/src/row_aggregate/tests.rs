...
fn bag(rows: &[(GraphValueRow, i128)]) -> ZSet<GraphValueRow> {
    ZSet::from_updates(
        rows.iter()
            .map(|(row, w)| (row.clone(), ZWeight::from_i128(*w))),
        LIMBS,
        &mut allow,
    )
    .unwrap()
...
fn seed(global: bool, input: &ZSet<GraphValueRow>) -> IncrementalRowAggregate {
    let spec = RowAggregateSpec::new(
        &[GraphSetColumnType::Scalar; 3],
        if global { &[] } else { &[0] },
        1,
    )
    .unwrap();
...
fn signature(row: &RowAggregateRow) -> Signature {
    ...
    (
        ...
        n(row.count_rows()),
        n(row.count_values()),
        n(row.count_distinct()),
        row.sum().map(n),
        row.sum_distinct().map(n),
        row.minimum(),
        row.maximum(),
        row.average_parts().map(|(a, b)| (n(a), n(b))),
        row.average_distinct_parts().map(|(a, b)| (n(a), n(b))),
    )
}
...
fn difference(
    new: &BTreeMap<Signature, i128>,
    old: &BTreeMap<Signature, i128>,
) -> BTreeMap<Signature, i128> {
    let mut result = new.clone();
    for (row, w) in old {
        *result.entry(row.clone()).or_default() -= w;
    }
    result.retain(|_, w| *w != 0);
    result
}
...
fn all_13122_grouped_and_global_transitions_match_independent_full_summaries() {
    ...
    for global in [false, true] {
        for mut code in 0..6561_u32 {
            ...
            let make = |at| {
                bag(&(0..4)
                    .map(|i| (choices[i].clone(), weights[at + i]))
                    .collect::<Vec<_>>())
            };
            let before = make(0);
            let after = make(4);
            ...
            let expected_before = oracle(global, &before);
            let expected_after = oracle(global, &after);
            assert_eq!(observed(state.rows()), expected_before);
            {
                ...
                assert_eq!(
                    ...
                    difference(&expected_after, &expected_before)
                );
            }
...
fn invalid_raw_retractions_and_nonnumeric_values_cannot_hide_in_projection() {
    ...
    let invalid = bag(&[
        (row(Some(0), Some(7), 99), -1),
        (row(Some(0), Some(7), 1), 1),
    ]);
    assert_eq!(
        state
            .prepare(&invalid, LIMBS, None, &mut allow)
            .unwrap_err(),
    ...
    assert_eq!(
        state
            .prepare(&bag(&[(bad, 1)]), LIMBS, None, &mut allow)
            .unwrap_err(),
    ...
    assert_eq!(
        state
            .prepare(&bag(&[(wrong, 1)]), LIMBS, None, &mut allow)
            .unwrap_err(),
...
fn every_control_boundary_and_downstream_unwind_leave_input_support_and_output_unchanged() {
    let before = bag(&[
        (row(Some(0), Some(9), 1), 2),
        (row(Some(0), Some(-3), 2), 1),
    ]);
    let change = bag(&[(row(Some(0), Some(9), 1), -2), (row(Some(1), None, 3), 4)]);
    for global in [false, true] {
        let expected = seed(global, &before);
        let mut complete = seed(global, &before);
        ...
        for stop in 1..=calls {
            ...
            assert_eq!(
                state
                    ...
                    .unwrap_err(),
                RowAggregateError::Delta(ZSetError::Control(stop))
            );
            assert_eq!(at, stop);
...
fn global_empty_null_only_and_final_group_limits_are_distinct() {
    ...
    assert_eq!(empty.count_rows(), &ZWeight::ZERO);
    assert!(empty.sum().is_none());
    assert!(
        state
            .prepare(&ZSet::new(), LIMBS, Some(0), &mut allow)
            .is_err()
    );
    let values = bag(&[(row(Some(1), None, 1), 3)]);
    state
        .prepare(&values, LIMBS, Some(1), &mut allow)
        .unwrap()
        .commit();
    let nulls = state.rows().iter().next().unwrap().0;
    assert_eq!(nulls.count_rows(), &ZWeight::from_i128(3));
    assert_eq!(nulls.count_values(), &ZWeight::ZERO);
    assert!(nulls.sum().is_none());
    assert!(nulls.minimum().is_none());
    state
        .prepare(
            &values.negated(LIMBS, &mut allow).unwrap(),
            LIMBS,
            Some(1),
            &mut allow,
        )
    ...
    let old = bag(&[(row(Some(9), Some(1), 1), 1)]);
    let new = bag(&[(row(Some(0), Some(2), 2), 1)]);
    let mut state = seed(false, &old);
    let delta = new.minus(&old, LIMBS, &mut allow).unwrap();
    state
        .prepare(&delta, LIMBS, Some(1), &mut allow)
        .unwrap()
        .commit();
...
fn native_group_domains_and_wide_integer_statistics_never_narrow() {
    ...
    let input = GraphValueRow::from_owned_values(vec![
    ...
    assert!(matches!(
        state.prepare(&delta, LimbLimit::new(0), None, &mut allow),
    ...
    assert!(result.count_rows().is_promoted());
    assert!(result.sum().unwrap().is_promoted());
    assert_eq!(result.count_distinct(), &ZWeight::ONE);
    assert_eq!(result.minimum(), Some(i128::from(i64::MAX)));
    assert_eq!(
        result.sum_distinct().unwrap(),
        &ZWeight::from_i128(i128::from(i64::MAX))
    );
...
fn admission_and_changed_group_locality_are_explicit() {
    use GraphSetColumnType::{List, Scalar, Vertex};
    assert_eq!(
        RowAggregateSpec::new(&[], &[], 0),
        Err(RowAggregateBuildError::EmptyInput)
    );
    assert_eq!(
        RowAggregateSpec::new(&[Vertex], &[], 0),
        Err(RowAggregateBuildError::RequiresScalarArgument)
    );
    assert_eq!(
        RowAggregateSpec::new(&[Scalar, List], &[], 0),
        Err(RowAggregateBuildError::UnsupportedColumn { column: 1 })
    );
    assert_eq!(
        RowAggregateSpec::new(&[Scalar], &[0, 0], 0),
        Err(RowAggregateBuildError::DuplicateKey { column: 0 })
    );
    assert_eq!(
        RowAggregateSpec::new(&[Scalar], &[1], 0),
        Err(RowAggregateBuildError::UnknownColumn { column: 1 })
    );
    assert_eq!(
        RowAggregateSpec::new(&[Scalar], &[], 1),
        Err(RowAggregateBuildError::UnknownColumn { column: 1 })
    );
    let run = |n: i64| {
        let all = bag(&(0..n)
            .map(|i| (row(Some(i), Some(i), i), 2))
            .collect::<Vec<_>>());
        let mut state = seed(false, &all);
        let mut counts = [0; 2];
        let delta = state
            .prepare(
                &bag(&[(row(Some(0), Some(0), 0), -1)]),
                LIMBS,
                None,
                &mut |event| {
                    counts[usize::from(event == ZSetEvent::ScratchEntry)] += 1;
