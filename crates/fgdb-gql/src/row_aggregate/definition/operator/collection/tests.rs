...
fn definition(global: bool, ordered: bool, having: bool) -> PreparedGraphSetAggregate {
    let input = if ordered {
        source()
            .with_order_by(&[GraphValueOrder::descending(1).with_nulls_first(true)])
            .unwrap()
    } else {
        source()
    };
            ...
            .with_result_clauses(
                &[GraphAggregateFilter {
                    ...
                    test: GraphAggregateTest::Integer {
                        comparison: crate::algebra::IntegerComparison::GreaterOrEqual,
                        value: 2,
                    },
                }],
                &[],
...
fn z(rows: impl IntoIterator<Item = (GraphValueRow, i128)>) -> ZSet<GraphValueRow> {
    ZSet::from_updates(
        rows.into_iter()
            .map(|(row, weight)| (row, ZWeight::from_i128(weight))),
        LIMBS,
        &mut allow,
    )
    .unwrap()
...
fn seed(definition: PreparedGraphSetAggregate, rows: &ZSet<GraphValueRow>) -> Operator {
    ...
    operator
        .prepare(rows, LIMBS, None, &mut allow)
        .unwrap()
        .commit();
    operator
}
                ...
                for before in [0, 1, 4, 10, 26, 50, 80] {
                    ...
                    for after in 0..81 {
                        ...
                        assert_eq!(
                            delta,
                            expected
                                .minus(&oracle(&old, global, ordered, having), LIMBS, &mut allow)
                                .unwrap()
                        );
                        let restored = old.minus(&new, LIMBS, &mut allow).unwrap();
                        state
                            .prepare(&restored, LIMBS, None, &mut allow)
                            .unwrap()
                            .commit();
...
fn count_neutral_replacement_changes_distinct_first_occurrence_and_dropped_guards_preserve_it() {
    ...
    let old = z([
        (row(0, Some(1), Some(10)), 1),
        (row(0, Some(2), Some(20)), 1),
        (row(0, Some(3), Some(10)), 1),
    ]);
    let change = z([
        (row(0, Some(1), Some(10)), -1),
        (row(0, Some(4), Some(10)), 1),
    ]);
    let mut state = seed(query.clone(), &old);
    assert_eq!(
        state.rows().iter().next().unwrap().0.values()[1],
        Value::Value(GraphValue::List(
            vec![scalar(Some(10)), scalar(Some(20))].into_boxed_slice()
        ))
    );
    {
    ...
    assert_eq!(
        state.rows().iter().next().unwrap().0.values()[1],
        Value::Value(GraphValue::List(
            vec![scalar(Some(20)), scalar(Some(10))].into_boxed_slice()
        ))
    );
...
fn any_definition(distinct: bool) -> PreparedGraphSetAggregate {
    let input = PreparedGraphSet::singleton()
        .unwind("item".into(), GraphSetValue::List(vec![]))
        .unwrap()
    ...
    PreparedGraphSetAggregate::prepare(
        ...
        &[],
        &[if distinct {
            GraphAggregate::collect_distinct("items", 0)
        } else {
...
fn one(value: GraphValue) -> GraphValueRow {
    GraphValueRow::from_owned_values(vec![value])
}
fn only_list(state: &Operator) -> &[GraphValue] {
    let Value::Value(GraphValue::List(values)) = &state.rows().iter().next().unwrap().0.values()[0]
    else {
        panic!()
    };
    values
...
fn collection_depth_and_node_bounds_refuse_before_expansion_and_retry_from_accepted_state() {
    ...
    for rows in [
        ...
        assert!(matches!(
            state.prepare(&rows, LIMBS, None, &mut allow),
            Err(GroupError::Arithmetic)
        ));
        assert_eq!(state, seed(query.clone(), &empty));
    }
    let valid = z([(
        one(scalar(Some(1))),
        (GraphValue::MAX_LIST_NODES - 1) as i128,
    )]);
    state
        .prepare(&valid, LIMBS, None, &mut allow)
        .unwrap()
        .commit();
    assert_eq!(only_list(&state).len(), GraphValue::MAX_LIST_NODES - 1);
    let Value::Value(value) = &state.rows().iter().next().unwrap().0.values()[0] else {
        panic!()
    };
    assert!(value.validate_bounds());
    let inverse = empty.minus(&valid, LIMBS, &mut allow).unwrap();
    state
        .prepare(&inverse, LIMBS, None, &mut allow)
        .unwrap()
        .commit();
...
fn every_update_checkpoint_and_exact_quota_preserves_raw_counts_index_summaries_and_output() {
    ...
    for (work_limit, scratch_limit, succeeds) in [
        (work, scratch, true),
        ...
        let (mut w, mut s) = (0, 0);
        {
            let result = state.prepare(&change, LIMBS, None, &mut |event| {
                match event {
                    ZSetEvent::Work => w += 1,
                    ZSetEvent::ScratchEntry => s += 1,
                }
                if w > work_limit || s > scratch_limit {
                    Err(0)
                } else {
                    Ok(())
                }
        ...
        assert_eq!(
            state,
            seed(query.clone(), if succeeds { &new } else { &old })
        );
    }
    let mut state = seed(query.clone(), &old);
    let invalid = z([(row(9, None, Some(4)), -1)]);
    assert!(matches!(
        state.prepare(&invalid, LIMBS, None, &mut allow),
        Err(GroupError::NegativeMultiplicity)
    ));
    assert_eq!(state, seed(query.clone(), &old));
    assert!(matches!(
        state.prepare(&change, LIMBS, Some(0), &mut allow),
...
fn unproved_sequences_refuse_but_complete_definition_and_other_aggregate_profiles_stay_intact() {
    ...
    let graph = PreparedGraphAggregate::prepare(
        pattern,
        &[],
        &[GraphAggregate::collect("items", 2)],
        0,
        None,
    )
    .unwrap();
    assert!(matches!(
        IncrementalGroupAggregate::new(graph, &[GraphSetColumnType::Scalar; 3]),
    ...
    .unwrap();
    assert!(matches!(
        Operator::new(query, &[GraphSetColumnType::Scalar; 6]),
        Err(GroupBuildError::UnsupportedAggregate { aggregate: 0 })
    ));
    ...
    let valid = GraphAggregateRow::from_group_values(
        vec![scalar(Some(0))],
        vec![
    ...
    assert!(
        definition
            .materialize_incremental_row(valid.keys().to_vec(), valid.values().to_vec())
            .is_some()
    );
    for bad in [
        scalar(None),
        GraphValue::List(vec![scalar(None)].into_boxed_slice()),
        ...
        assert!(
            definition
                .materialize_incremental_row(valid.keys().to_vec(), values)
                .is_none()
        );
    }
}
