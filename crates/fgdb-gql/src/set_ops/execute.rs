...
use crate::{GlaExecutionEvent, GlaExecutionStats, GlaLimitDimension, GlaLimitExceeded,
    GqlBudgetExceeded, GqlExecutionStats};
...
fn evaluator_sum<E, C>(used: u64, added: u64, limit: u64, dimension: GlaLimitDimension) -> SetResult<u64, E, C> {
...
impl<Checkpoint> Meter<Checkpoint> {
    ...
    where Checkpoint: FnMut() -> Result<(), C> {
        ...
        let result_rows = if event == GlaExecutionEvent::ResultRow {
            let next = self.rows.result_rows.checked_add(1).ok_or_else(||
                GqlQueryError::Source(GraphSetExecutionError::AccountingOverflow { dimension: GqlBudgetDimension::ResultRows }))?;
            self.policy.rows.check(GqlBudgetDimension::ResultRows, next).map_err(GqlQueryError::Rows)?;
            next
        } else { self.rows.result_rows };
        let work_units = evaluator_sum(self.evaluator.work_units, 1,
            self.policy.evaluator.max_work_units, GlaLimitDimension::WorkUnits)?;
        let scratch_entries = evaluator_sum(self.evaluator.scratch_entries,
            u64::from(event == GlaExecutionEvent::ScratchEntry),
            self.policy.evaluator.max_scratch_entries, GlaLimitDimension::ScratchEntries)?;
    ...
    fn remaining(&self) -> GqlQueryPolicy {
        GqlQueryPolicy::new(
            self.policy.rows.max_snapshot_records().unwrap_or(u64::MAX) - self.rows.snapshot_records,
            u64::MAX,
    ...
    fn absorb<E, C>(&mut self, execution: &GqlQueryExecution<GraphValueRow>, operand: usize) -> SetResult<(), E, C> {
        if u64::try_from(execution.value.len()).ok() != Some(execution.rows.result_rows) {
            return Err(GqlQueryError::Source(GraphSetExecutionError::InvalidSourceStatistics { operand }));
        }
        let snapshot_records = self.rows.snapshot_records.checked_add(execution.rows.snapshot_records)
            .ok_or_else(|| GqlQueryError::Source(GraphSetExecutionError::AccountingOverflow {
                dimension: GqlBudgetDimension::SnapshotRecords,
            }))?;
        self.policy.rows.check(GqlBudgetDimension::SnapshotRecords, snapshot_records).map_err(GqlQueryError::Rows)?;
        let work_units = evaluator_sum(self.evaluator.work_units, execution.evaluator.work_units,
            self.policy.evaluator.max_work_units, GlaLimitDimension::WorkUnits)?;
        let scratch_entries = evaluator_sum(self.evaluator.scratch_entries, execution.evaluator.scratch_entries,
            self.policy.evaluator.max_scratch_entries, GlaLimitDimension::ScratchEntries)?;
        self.rows.snapshot_records = snapshot_records;
        self.evaluator = GlaExecutionStats { work_units, scratch_entries };
        Ok(())
    }
    fn source_error<E, C>(&self, error: GqlQueryError<E, C>) -> GqlQueryError<GraphSetExecutionError<E>, C> {
        match error {
            ...
            GqlQueryError::Evaluator(error) => {
                let (used, limit) = match error.dimension {
                    ...
                    GlaLimitDimension::ScratchEntries => (self.evaluator.scratch_entries, self.policy.evaluator.max_scratch_entries),
                };
                GqlQueryError::Evaluator(GlaLimitExceeded { dimension: error.dimension, limit,
                    observed: error.observed.saturating_add(u128::from(used)) })
            }
            GqlQueryError::Rows(error) if error.dimension == GqlBudgetDimension::SnapshotRecords => {
                match self.rows.snapshot_records.checked_add(error.observed) {
                    Some(observed) => GqlQueryError::Rows(GqlBudgetExceeded {
                        dimension: error.dimension, limit: self.policy.rows.max_snapshot_records().unwrap_or(u64::MAX), observed,
                    }),
                    None => GqlQueryError::Source(GraphSetExecutionError::AccountingOverflow { dimension: error.dimension }),
                }
            }
...
pub(super) fn execute<E, C, S, Checkpoint>(
    query: &PreparedGraphSet, policy: GqlQueryPolicy, source: &mut S, checkpoint: Checkpoint,
) -> SetResult<GqlQueryExecution<GraphValueRow>, E, C>
where
    S: FnMut(&PreparedGraphPattern<GraphValueRow>, GqlQueryPolicy)
        -> Result<GqlQueryExecution<GraphValueRow>, GqlQueryError<E, C>>,
    Checkpoint: FnMut() -> Result<(), C>,
{
    let mut meter = Meter { policy, checkpoint,
    ...
    for _ in &value { meter.event(GlaExecutionEvent::ResultRow)?; }
    (meter.checkpoint)().map_err(GqlQueryError::Interrupted)?;
    Ok(GqlQueryExecution { value, rows: meter.rows, evaluator: meter.evaluator })
}

fn run<E, C, S, Checkpoint>(
    query: &PreparedGraphSet, source: &mut S, meter: &mut Meter<Checkpoint>, operand: &mut usize,
    ...
    Checkpoint: FnMut() -> Result<(), C>,
{
    ...
    let mut rows = match &query.node {
        SetNode::Pattern(pattern) => {
            let at = *operand; *operand += 1;
            let execution = source(pattern, meter.remaining()).map_err(|error| meter.source_error(error))?;
            meter.absorb(&execution, at)?;
            for row in &execution.value {
                meter.event(GlaExecutionEvent::ScratchEntry)?;
                if row.len() != query.types.len() {
                    return Err(GqlQueryError::Source(GraphSetExecutionError::InputSchema { operand: at }));
                }
            ...
            merge::sort(&mut rows, &mut |event| meter.event(event),
        ...
        SetNode::Binary { operation, quantifier, left, right } => {
            ...
            merge::combine(left, right, *operation, *quantifier,
                &mut |event| meter.event(event), &mut |a, b, control| compare_rows(a, b, &[], control))?
        }
    };
    ...
    if query.offset == 0 && query.count.is_none() { return Ok(rows); }
    let mut output = Vec::new();
    ...
    for row in rows {
        ...
        meter.event(GlaExecutionEvent::ScratchEntry)?;
        output.push(row); remaining -= 1;
    }
    Ok(output)
}

fn compare_value<E>(a: &GraphValue, b: &GraphValue,
...
fn compare_rows<E>(a: &GraphValueRow, b: &GraphValueRow, order: &[GraphValueOrder],
    control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), E>) -> Result<Ordering, E> {
    for key in order {
        control(GlaExecutionEvent::Work)?;
        ...
        let result = match (a.is_null(), b.is_null()) {
            ...
            (false, false) => {
                ...
                if key.descending { result.reverse() } else { result }
            }
        };
...
mod tests {
    ...
    fn cumulative_counters_refuse_overflow_without_mutation() {
        ...
        let input = GqlQueryExecution { value: Vec::new(),
            rows: GqlExecutionStats { snapshot_records: 1, result_rows: 0 }, evaluator: GlaExecutionStats::default() };
