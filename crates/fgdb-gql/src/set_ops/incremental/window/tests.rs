...
use crate::{
    GlaExecutionStats, GqlExecutionStats, GqlParameters, GqlQueryExecution, GraphSetOperand,
...
fn leaf() -> PreparedGraphSet {
    PreparedGraphText::prepare(
        "MATCH (n) RETURN n.p AS p,n.q AS q",
            ...
            |_, _| {
                ...
                Ok::<_, GqlQueryError<(), ()>>(GqlQueryExecution {
                    rows: GqlExecutionStats {
                        snapshot_records: rows.len() as u64,
...
fn reverse_columns(query: PreparedGraphSet, q: GraphSetQuantifier) -> PreparedGraphSet {
    query
        .project(
            vec![
                GraphSetProjection::new("q", GraphSetValue::Column(1)),
                GraphSetProjection::new("p", GraphSetValue::Column(0)),
            ],
            q,
        )
        .unwrap()
...
fn expand(query: PreparedGraphSet) -> PreparedGraphSet {
    query
        .unwind(
            "x".into(),
            GraphSetValue::List(vec![GraphSetValue::Column(1), GraphSetValue::Column(0)]),
        )
        .unwrap()
}
fn via_window(query: &PreparedGraphSet) -> Vec<GraphValueRow> {
    ...
    let input = ZSet::from_updates(
        run(&input).into_iter().map(|row| (row, ZWeight::ONE)),
        LimbLimit::new(4),
        &mut |_| Ok::<_, ()>(()),
    )
    .unwrap();
    ...
    window
        .prepare(&input, LimbLimit::new(4), None, &mut |_| Ok::<_, ()>(()))
        .unwrap()
        .commit();
    let mut out = Vec::new();
    for (row, weight) in window.rows() {
        for _ in 0..weight.to_i128().unwrap() {
            out.push(row.clone());
        }
    }
    out
...
fn nested_split_keeps_complete_children_quantifiers_and_definition_identity() {
    ...
    assert_eq!(
        ...
        Some((&[GraphValueOrder::descending(0)][..], Some(4)))
    );
    ...
    for input in [
        ...
        for count in [0, 1, 4] {
            assert!(
                input
                    .clone()
                    .with_page(1, Some(count))
                    .split_incremental_window()
                    .is_none()
            );
        }
        let ranked = input
...
fn set_bounds_follow_bag_laws_and_overflow_never_becomes_an_unlimited_sentinel() {
    ...
    for quantifier in [GraphSetQuantifier::All, GraphSetQuantifier::Distinct] {
        for (operation, cap) in [
            (GraphSetOperation::Union, 5),
            (GraphSetOperation::Intersect, 2),
            (GraphSetOperation::Except, 3),
        ] {
            let query = bounded(3)
                .combine(operation, quantifier, bounded(2))
                .unwrap()
                .with_order_by(&[GraphValueOrder::ascending(1)])
                .unwrap();
        ...
        let intersect = leaf()
            .combine(GraphSetOperation::Intersect, quantifier, bounded(2))
            .unwrap()
            .with_order_by(&[GraphValueOrder::ascending(1)])
            .unwrap();
        assert_eq!(intersect.split_incremental_window().unwrap().3, 2);
        assert_eq!(run(&intersect), via_window(&intersect));
        let too_large = bounded(u64::MAX)
            .combine(GraphSetOperation::Union, quantifier, bounded(1))
            .unwrap()
            .with_order_by(&[GraphValueOrder::descending(0)])
            .unwrap();
