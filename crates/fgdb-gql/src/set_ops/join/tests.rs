...
fn leaf() -> PreparedGraphSet {
    PreparedGraphText::prepare("MATCH (n) RETURN n.k AS k, n.p AS p", |kind, name: &str| {
        match (kind, name) {
...
fn spec(keyed: bool, kind: RowJoinKind) -> RowJoinSpec {
    ...
    spec.with_kind(kind).with_predicate(&[GraphSetPredicateOp::Compare {
...
fn row(values: &[Option<i64>]) -> GraphValueRow {
    GraphValueRow::from_owned_values(values.iter().map(|value| GraphValue::Scalar(
...
fn execution(value: Vec<GraphValueRow>) -> GqlQueryExecution<GraphValueRow> {
    GqlQueryExecution {
        rows: GqlExecutionStats { snapshot_records: value.len() as u64, result_rows: value.len() as u64 },
        value,
        evaluator: GlaExecutionStats::default(),
    }
}
fn run(
    query: &PreparedGraphSet, inputs: [Vec<GraphValueRow>; 2], policy: GqlQueryPolicy,
    ...
    let result = query.execute_governed(
        policy, |_, _| Ok(execution(inputs.next().expect("each source executes once"))), checkpoint,
    );
    if result.is_ok() { assert!(inputs.next().is_none()); }
    result
}
...
fn oracle(left: &[[Option<i64>; 2]], right: &[[Option<i64>; 2]], keyed: bool, kind: RowJoinKind) -> Vec<GraphValueRow> {
    ...
    for a in left {
        ...
        match kind {
            ...
            RowJoinKind::Left | RowJoinKind::Full if !found => out.push(row(&[a[0], a[1], None, None])),
            _ => {}
        }
    }
    if matches!(kind, RowJoinKind::Right | RowJoinKind::Full) {
        for b in right {
            if !left.iter().any(|a| matches(a, b)) { out.push(row(&[None, None, b[0], b[1]])); }
        }
    }
    out.sort();
    out
...
fn all_six_relational_joins_match_independent_occurrence_oracles() {
    for counts in 0..81_usize {
        ...
        let mut amounts = [0; 4];
        for amount in &mut amounts { *amount = counts % 3; counts /= 3; }
        let left: Vec<_> = std::iter::repeat_n([Some(1), Some(10)], amounts[0])
            .chain(std::iter::repeat_n([Some(1), Some(30)], amounts[1])).collect();
        let right: Vec<_> = std::iter::repeat_n([Some(1), Some(20)], amounts[2])
            .chain(std::iter::repeat_n([Some(2), Some(40)], amounts[3])).collect();
        for keyed in [true, false] {
            for kind in KINDS {
                let query = leaf().join(leaf(), spec(keyed, kind)).unwrap();
                let inputs = [left.iter().map(|a| row(a)).collect(), right.iter().map(|b| row(b)).collect()];
                let result = run(&query, inputs, policy(), || Ok(())).unwrap();
                assert_eq!(result.value, oracle(&left, &right, keyed, kind));
                assert_eq!(result.rows.snapshot_records, (left.len() + right.len()) as u64);
                ...
                let expected = if matches!(kind, RowJoinKind::Semi | RowJoinKind::Anti) {
                    vec!["left.k", "left.p"]
                } else { vec!["left.k", "left.p", "right.k", "right.p"] };
...
fn full_join_finishes_child_pages_before_on_and_supports_further_relational_stages() {
    let left = leaf().with_order_by(&[GraphValueOrder::descending(1)]).unwrap().with_page(1, Some(1));
    let right = leaf().with_page(0, Some(2));
    let query = left.join(right, spec(true, RowJoinKind::Full)).unwrap();
    let inputs = [
        vec![row(&[Some(1), Some(10)]), row(&[Some(1), Some(20)]), row(&[Some(1), Some(30)])],
        vec![row(&[Some(1), Some(15)]), row(&[Some(1), Some(25)]), row(&[Some(1), Some(35)])],
    ];
    let result = run(&query, inputs.clone(), policy(), || Ok(())).unwrap();
    assert_eq!(result.value, vec![row(&[None, None, Some(1), Some(15)]), row(&[Some(1), Some(20), Some(1), Some(25)])]);
    let selected = query.clone().filter(&[GraphSetPredicateOp::IsNull {
    ...
    assert_eq!(run(&selected, inputs.clone(), policy(), || Ok(())).unwrap().value,
        vec![row(&[Some(25), Some(20)])]);
    let summary = PreparedGraphSetAggregate::prepare(query, &[], &[
        GraphAggregate::count_rows("rows"), GraphAggregate::count("left_nonnull", 1),
    ], 0, None).unwrap();
    let mut inputs = inputs.into_iter();
    let result = summary.execute_governed(policy(), |_, _| {
        Ok::<_, GqlQueryError<usize, usize>>(execution(inputs.next().unwrap()))
    }, || Ok::<_, usize>(())).unwrap();
...
fn nulls_and_incompatible_values_remain_unknown_under_not_and_zero_width_full_keeps_both_sides() {
    let types = [GraphSetColumnType::Scalar; 2];
    let negative = RowJoinSpec::cross(&types, &types).unwrap().with_kind(RowJoinKind::Full)
        .with_predicate(&[GraphSetPredicateOp::Compare {
            left: GraphSetOperand::Column(1), comparison: IntegerComparison::Less,
            right: GraphSetOperand::Column(3),
        }, GraphSetPredicateOp::Not]).unwrap();
    let text = GraphValueRow::from_owned_values(vec![GraphValue::Scalar(CanonicalScalar::Int(1)),
        GraphValue::Scalar(CanonicalScalar::ucs_basic_text("private").unwrap())]);
    let query = leaf().join(leaf(), negative).unwrap();
    let result = run(&query, [vec![row(&[Some(1), None]), text], vec![row(&[Some(1), Some(10)])]], policy(), || Ok(())).unwrap();
    assert_eq!(result.value.len(), 3); // no UNKNOWN pair was accepted by NOT
    let spec = RowJoinSpec::cross(&[], &[]).unwrap().with_kind(RowJoinKind::Full)
        .with_predicate(&[GraphSetPredicateOp::Truth(Some(false))]).unwrap();
    let query = PreparedGraphSet::singleton().join(PreparedGraphSet::singleton(), spec).unwrap();
    let result = query.execute_governed(policy(), |_, _| -> Result<_, GqlQueryError<usize, usize>> {
        panic!("source-free relation must not invent a graph leaf")
    }, || Ok::<_, usize>(())).unwrap();
    assert_eq!(result.value, vec![GraphValueRow::unit(), GraphValueRow::unit()]);
...
fn empty_left_and_zero_final_limit_do_not_hide_right_source_failure() {
    for kind in KINDS {
        let query = leaf().join(leaf(), spec(true, kind)).unwrap().with_page(0, Some(0));
        let mut calls = 0;
        let result = query.execute_governed(policy(), |_, _| {
            calls += 1;
            if calls == 1 { Ok(execution(vec![])) } else { Err(GqlQueryError::Source(73_usize)) }
        }, || Ok::<_, usize>(()));
        assert!(matches!(result, Err(GqlQueryError::Source(GraphSetExecutionError::Source(73)))));
        assert_eq!(calls, 2);
    }
}
...
fn every_cancellation_checkpoint_and_inclusive_budget_is_fail_closed_and_retryable() {
    ...
    let inputs = [vec![row(&[Some(1), Some(10)]), row(&[Some(1), Some(10)])], vec![row(&[Some(1), Some(20)])]];
    let mut calls = 0;
    let expected = run(&query, inputs.clone(), policy(), || { calls += 1; Ok(()) }).unwrap();
    for stop in 1..=calls {
        let mut seen = 0;
    ...
    let exact = GqlQueryPolicy::new(3, 2, expected.evaluator.work_units, expected.evaluator.scratch_entries);
    assert_eq!(run(&query, inputs.clone(), exact, || Ok(())).unwrap().value, expected.value);
    for (records, rows, work, scratch) in [
        (2, 2, expected.evaluator.work_units, expected.evaluator.scratch_entries),
        (3, 1, expected.evaluator.work_units, expected.evaluator.scratch_entries),
        (3, 2, expected.evaluator.work_units - 1, expected.evaluator.scratch_entries),
        (3, 2, expected.evaluator.work_units, expected.evaluator.scratch_entries - 1),
    ] {
        assert!(run(&query, inputs.clone(), GqlQueryPolicy::new(records, rows, work, scratch), || Ok(())).is_err());
    }
    let empty = query.with_page(0, Some(0));
    assert!(run(&empty, inputs, GqlQueryPolicy::new(3, 0, 1_000_000, 1_000_000), || Ok(())).unwrap().value.is_empty());
}
...
fn bound_join_identity_schema_and_window_accessors_cover_complete_semantics() {
    ...
    for side in 0..2 {
        let (a, b) = if side == 0 { (&changed, &types) } else { (&types, &changed) };
        let definition = RowJoinSpec::cross(a, b).unwrap().with_kind(RowJoinKind::Semi)
            .with_predicate(&[GraphSetPredicateOp::Truth(Some(false))]).unwrap();
        assert_eq!(left.clone().join(leaf(), definition), Err(GraphSetBuildError::JoinInputSchema { side }));
    }
    for code in [
        ...
        let definition = RowJoinSpec::cross(&types, &types).unwrap().with_predicate(&code).unwrap();
        assert!(transcripts.insert(left.clone().join(leaf(), definition).unwrap().canonical_bytes()));
    }
    let mut deep = leaf();
    for _ in 1..MAX_GRAPH_SET_DEPTH { deep = deep.nested().unwrap(); }
    assert!(matches!(deep.join(leaf(), spec(true, RowJoinKind::Inner)), Err(GraphSetBuildError::TooDeep { .. })));
}
