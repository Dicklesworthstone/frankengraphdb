...
fn counted(query: &PreparedGraphSet) -> SetResult<Option<u64>, &'static str, usize> {
    query
        .count_governed(wide(), no_source, || Ok(()))
        .map(|(count, _, _)| count)
}
...
fn nested_constant_unwind_with_aliases_reduces_without_generating_trillion_rows() {
    ...
    let (count, rows, used) = query
        .count_governed(GqlQueryPolicy::new(0, 0, 20_000, 5_000), no_source, || {
            Ok(())
        })
        .unwrap();
            ...
            for skip in [0, 1, u64::MAX] {
                for limit in [None, Some(0), Some(3)] {
                    let query = PreparedGraphSet::singleton()
                        .unwind("a".into(), list(first))
                        .unwrap()
                        .with_page(1, Some(3))
                        .unwind("b".into(), list(second))
                        .unwrap()
                        .project(
                    ...
                    assert_eq!(
                        counted(&query).unwrap(),
                        Some(eager(&query).unwrap().value.len() as u64)
                    );
                }
            }
        }
    ...
    for input_count in [0, 1, 3] {
        for page in [0, 1] {
            let input = PreparedGraphSet::singleton()
                .unwind("a".into(), list(input_count))
                .unwrap();
            for value in [bad_list.clone(), scalar(7), divide_zero()] {
                let query = input
                    ...
                    .unwind("b".into(), value)
                    .unwrap()
                    .with_page(0, Some(page));
                match eager(&query) {
                    Ok(expected) => {
                        assert_eq!(counted(&query).unwrap(), Some(expected.value.len() as u64))
                    }
...
fn fallible_projections_and_dependent_unwind_remain_value_sensitive() {
    ...
    let nested = PreparedGraphSet::singleton()
        .unwind(
            "lists".into(),
...
fn upstream_failure_precedes_constant_failure_even_when_output_page_is_empty() {
    let input = PreparedGraphSet::singleton()
        ...
        .unwrap();
    let query = input
        .unwind("bad".into(), GraphSetValue::List(vec![divide_zero()]))
        .unwrap()
        .with_page(0, Some(0));
