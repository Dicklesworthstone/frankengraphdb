...
fn compare(a: usize, comparison: IntegerComparison, b: usize) -> GraphSetPredicateOp {
    GraphSetPredicateOp::Compare {
        left: GraphSetOperand::Column(a), comparison, right: GraphSetOperand::Column(b),
    }
}
...
fn sequence(values: &[Option<i64>], name: &str) -> PreparedGraphSet {
    PreparedGraphSet::singleton().unwind(name.into(), GraphSetValue::List(
        values.iter().copied().map(|value| GraphSetValue::Value(scalar(value))).collect(),
    )).unwrap()
}
fn execute(query: &PreparedGraphSet, policy: GqlQueryPolicy) -> GqlQueryExecution<GraphValueRow> {
    query.execute_governed(policy, |_, _| -> Result<_, GqlQueryError<usize, usize>> {
...
fn selected_pairs_match_independent_occurrence_oracle_with_composite_keys_and_residuals() {
    ...
    for mask in 0..256 {
        let left_data = [(Some(2), Some(3)), (Some(1), Some(4)), (Some(2), Some(3)), (None, Some(5))];
        let right_data = [(Some(2), Some(8)), (Some(1), Some(4)), (None, Some(9)), (Some(2), Some(3))];
        let left: Vec<_> = left_data.iter().enumerate().filter(|(i, _)| mask & (1 << i) != 0)
            .map(|(_, &(a, b))| row(&[a, b])).collect();
        let right: Vec<_> = right_data.iter().enumerate().filter(|(i, _)| mask & (1 << (i + 4)) != 0)
            .map(|(_, &(a, b))| row(&[a, b])).collect();
        for composite in [false, true] {
            let code = if composite {
                vec![equal(2, 0), equal(1, 3), GraphSetPredicateOp::And]
            } else {
                vec![equal(2, 0), compare(1, IntegerComparison::Less, 3), GraphSetPredicateOp::And]
            };
            validate(4, &code);
            ...
            for (i, &(a, b)) in left_data.iter().enumerate() {
                ...
                for (j, &(c, d)) in right_data.iter().enumerate() {
                    if mask & (1 << (j + 4)) == 0 { continue; }
                    if a.zip(c).is_some_and(|(a, c)| a == c)
                        && b.zip(d).is_some_and(|(b, d)| if composite { b == d } else { b < d })
                    {
                        expected.push(row(&[a, b, c, d]));
                    }
                }
            }
            assert_eq!(collect(&left, &right, &code, None, &mut allow).unwrap(), expected);
        }
    }
}
...
fn boolean_proof_never_extracts_an_equality_from_only_one_or_arm_or_beneath_not() {
    ...
    for (code, expected) in [
        ...
        (vec![eq.clone(), yes.clone(), GraphSetPredicateOp::Or], vec![]),
        (vec![eq.clone(), unknown.clone(), GraphSetPredicateOp::Or], vec![]),
        (vec![eq.clone(), GraphSetPredicateOp::Not], vec![]),
        (vec![eq.clone(), GraphSetPredicateOp::Not, GraphSetPredicateOp::Not], vec![]),
        (vec![eq.clone(), yes.clone(), GraphSetPredicateOp::And], vec![(0, 0)]),
        (vec![eq.clone(), no.clone(), GraphSetPredicateOp::And,
            equal(1, 0), unknown.clone(), GraphSetPredicateOp::And, GraphSetPredicateOp::Or], vec![(0, 0)]),
    ] {
        validate(2, &code);
        ...
        assert_eq!(collect(&left, &right, &code, None, &mut allow).unwrap(), original);
    }
}
...
fn dynamic_scalar_vertex_and_incompatible_keys_use_the_same_predicate_equality_law() {
    let values = vec![
        scalar(None), scalar(Some(1)), scalar(Some(2)),
        ...
        GraphValue::Vertex(VId(1)), GraphValue::Vertex(VId(u128::MAX)),
        GraphValue::List(vec![scalar(Some(1))].into_boxed_slice()),
    ];
    let left: Vec<_> = values.iter().cloned().map(|value| GraphValueRow::from_owned_values(vec![value])).collect();
    let right: Vec<_> = values.into_iter().rev().map(|value| GraphValueRow::from_owned_values(vec![value])).collect();
    let code = [equal(0, 1)];
    validate(2, &code);
    let actual = collect(&left, &right, &code, None, &mut allow).unwrap();
    let expected: Vec<_> = left.iter().filter(|row| !row.values()[0].is_null()
        && matches!(&row.values()[0], GraphValue::Scalar(_) | GraphValue::Vertex(_)))
        .map(|row| GraphValueRow::from_owned_values(vec![row.values()[0].clone(); 2])).collect();
...
fn source_free_pages_preserve_left_major_child_order_and_product_barriers() {
    ...
    let query = product.clone().nested().unwrap().filter(&code).unwrap().with_page(1, Some(3));
    let before = query.canonical_bytes();
    assert!(query.filtered_cross_inputs().is_some());
    assert_eq!(execute(&query, policy()).value,
        vec![row(&[Some(2), Some(2)]), row(&[Some(1), Some(1)]), row(&[Some(2), Some(2)])]);
    ...
    let ordered = product.with_order_by(&[GraphValueOrder::ascending(0)]).unwrap().filter(&code).unwrap();
    assert!(ordered.filtered_cross_inputs().is_none());
    let local_pages = left.with_page(1, Some(2)).cross_join(right.with_page(0, Some(2))).unwrap().filter(&code).unwrap();
    assert_eq!(execute(&local_pages, policy()).value,
...
fn large_selective_join_has_linear_scratch_and_subquadratic_scalar_work() {
    for n in [128_i64, 1024] {
        ...
        if n == 1024 { assert!(work < (n * n / 3) as usize); }
    }
    // A non-equality predicate still gets allocation-free rejected candidates.
    let rows: Vec<_> = (0..256).map(|v| row(&[Some(v)])).collect();
    let mut scratch = 0;
    let output = collect(&rows, &rows, &[GraphSetPredicateOp::Truth(Some(false))], None, &mut |event| {
...
fn every_index_predicate_and_copy_refusal_leaves_no_result_and_is_retryable() {
    let left = [row(&[Some(3)]), row(&[Some(1)]), row(&[Some(1)]), row(&[None])];
    let right = [row(&[Some(1)]), row(&[Some(3)]), row(&[Some(1)]), row(&[None])];
    let code = [equal(0, 1)];
    let mut count = 0;
    let expected = collect(&left, &right, &code, None, &mut |_| { count += 1; Ok::<_, usize>(()) }).unwrap();
    for stop in 1..=count {
        let mut calls = 0;
        assert_eq!(collect(&left, &right, &code, None, &mut |_| {
            calls += 1;
...
fn empty_left_and_outer_limit_zero_never_erase_right_source_failures() {
    let leaf = || {
        ...
        PreparedGraphSet::from(builder.prepare_values(&[crate::algebra::GraphColumn::vertex("n", "n")], 0, None).unwrap())
    };
    let query = leaf().cross_join(leaf()).unwrap().filter(&[equal(0, 1)]).unwrap().with_page(0, Some(0));
    let mut calls = 0;
    let result = query.execute_governed(policy(), |_, _| {
        calls += 1;
        if calls == 1 {
            Ok(GqlQueryExecution { value: vec![], rows: GqlExecutionStats { snapshot_records: 0, result_rows: 0 }, evaluator: GlaExecutionStats::default() })
...
fn native_unwind_match_selection_reaches_the_physical_path_without_new_syntax() {
    ...
    let prepared = PreparedGraphSetText::prepare(
        "UNWIND [2, 1, 2] AS wanted MATCH (n) WITH wanted, n.p AS value WHERE wanted = value RETURN wanted, value",
        |kind, name| {
    ...
    fn fused(query: &PreparedGraphSet) -> bool {
        if query.filtered_cross_inputs().is_some() { return true; }
        match &query.node {
            SetNode::Scope(input) | SetNode::Filter { input, .. }
            | SetNode::Project { input, .. } | SetNode::Unwind { input, .. } => fused(input),
            SetNode::CrossJoin { left, right } | SetNode::Join { left, right, .. }
            | SetNode::Binary { left, right, .. } => fused(left) || fused(right),
            _ => false,
        }
    }
    assert!(fused(&prepared), "native binding must reach the selected-product path");
    let result = prepared.execute_governed(policy(), |pattern, remaining| {
    ...
    assert_eq!(result.value, vec![row(&[Some(2), Some(2)]), row(&[Some(1), Some(1)]), row(&[Some(2), Some(2)])]);
}
...
fn column_projection_reordering_and_repeated_columns_are_borrowed_not_materialized() {
    let product = sequence(&[Some(2), Some(1), Some(2)], "a")
        .cross_join(sequence(&[Some(1), Some(2), Some(2)], "b")).unwrap();
    let projection = vec![
    ...
    let selected = product.clone().project(projection.clone(), GraphSetQuantifier::All).unwrap()
        .nested().unwrap().filter(&[equal(1, 0), equal(2, 0), GraphSetPredicateOp::And]).unwrap();
    assert!(selected.filtered_cross_inputs().is_some());
    let expected = vec![row(&[Some(2); 3]), row(&[Some(2); 3]), row(&[Some(1); 3]), row(&[Some(2); 3]), row(&[Some(2); 3])];
    assert_eq!(execute(&selected, policy()).value, expected);
    let distinct = product.clone().project(projection, GraphSetQuantifier::Distinct).unwrap()
        .filter(&[equal(0, 1)]).unwrap();
    assert!(distinct.filtered_cross_inputs().is_none());
    assert_eq!(execute(&distinct, policy()).value, vec![row(&[Some(1); 3]), row(&[Some(2); 3])]);
    // Even a constant FALSE filter must not suppress a failing child expression.
    let failing = product.project(vec![GraphSetProjection::new("bad",
        GraphSetValue::Size(Box::new(GraphSetValue::Column(0))))], GraphSetQuantifier::All)
        .unwrap().filter(&[GraphSetPredicateOp::Truth(Some(false))]).unwrap();
    assert!(failing.filtered_cross_inputs().is_none());
    let result = failing.execute_governed(policy(), |_, _| -> Result<_, GqlQueryError<usize, usize>> {
        panic!("source-free expression")
    }, || Ok::<_, usize>(()));
    assert!(matches!(result, Err(GqlQueryError::Source(GraphSetExecutionError::Projection { .. }))));
}

#[test]
fn borrowed_projection_matches_materialized_eager_predicates_at_every_input_split() {
    let cells = vec![scalar(Some(2)), scalar(None), scalar(Some(1)), scalar(Some(2))];
    for split in 0..=cells.len() {
        ...
        for mapping in [vec![3, 0, 1, 2, 0], vec![1], vec![0, 3, 2, 1]] {
            let projected = GraphValueRow::from_owned_values(mapping.iter().map(|&i| cells[i].clone()).collect());
            for a in 0..mapping.len() {
                for b in 0..mapping.len() {
                    for op in [IntegerComparison::Equal, IntegerComparison::Less, IntegerComparison::NotEqual] {
                        let code = [compare(a, op, b), GraphSetPredicateOp::Not];
                        validate(mapping.len(), &code);
                        let ordinary = GraphSetPredicateOp::evaluate_row_with_control(&code, &projected, &mut allow).unwrap();
                        let borrowed = GraphSetPredicateOp::evaluate_projected_pair_with_control(
                            &code, &left, &right, Some(&mapping), &mut allow,
                        ).unwrap();
...
fn final_rows_and_cumulative_allowances_still_refuse_at_inclusive_boundaries() {
    ...
    assert_eq!(execute(&query, GqlQueryPolicy::new(0, 3, work, scratch)).value, execution.value);
    for denied in [GqlQueryPolicy::new(0, 2, work, scratch),
        GqlQueryPolicy::new(0, 3, work - 1, scratch), GqlQueryPolicy::new(0, 3, work, scratch - 1)] {
        let result = query.execute_governed(denied, |_, _| -> Result<_, GqlQueryError<usize, usize>> {
            panic!("source-free relation")
        }, || Ok::<_, usize>(()));
