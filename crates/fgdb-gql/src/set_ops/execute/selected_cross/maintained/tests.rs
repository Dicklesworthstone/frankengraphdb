...
fn difference(after: &ZSet<GraphValueRow>, before: &ZSet<GraphValueRow>) -> ZSet<GraphValueRow> {
    bag(plain(after)
        .into_iter()
        .chain(plain(before).into_iter().map(|(row, n)| (row, -n))))
}
...
fn keys_are_necessary_conditions_and_original_names_survive() {
    for (code, keys) in [
        ...
        (
            vec![equal(0, 2), equal(2, 0), GraphSetPredicateOp::Or],
            vec![(0, 0)],
        ),
    ] {
        let query = leaf()
        ...
        let (left, right, spec, output) = query
            .incremental_selected_join_with_control(&mut allow)
            .unwrap()
            .unwrap();
        assert_eq!(left, &leaf());
...
fn input(mask: usize, right: bool) -> ZSet<GraphValueRow> {
    let data = if right {
        [(Some(1), Some(20)), (None, Some(40))]
    } else {
        [(Some(1), Some(10)), (Some(1), Some(30))]
    };
    bag(data
        .into_iter()
        .enumerate()
...
fn simultaneous_updates_and_retractions_match_independent_bag_oracle() {
    ...
    for before in 0..16 {
        for after in 0..16 {
            ...
            let initial = joined
                .prepare(&old_left, &old_right, LIMBS, None, &mut |_| Ok::<_, ()>(()))
                .unwrap()
                .commit();
            projected
                .prepare(&initial, LIMBS, None, &mut |_| Ok::<_, ()>(()))
                .unwrap()
                .commit();
            ...
            let delta = joined
                .prepare(
                    &difference(&next_left, &old_left),
                    &difference(&next_right, &old_right),
                    LIMBS,
                    None,
                    &mut |_| Ok::<_, ()>(()),
                )
                ...
                .commit();
            projected
                .prepare(&delta, LIMBS, None, &mut |_| Ok::<_, ()>(()))
                .unwrap()
                .commit();
        ...
        product
            ...
            .project(
                vec![
                    ...
                    GraphSetProjection::new("b", GraphSetValue::Column(2)),
                ],
...
fn dynamic_any_comparisons_are_residuals_not_a_schema_narrowing() {
    let dynamic = PreparedGraphSet::singleton()
        .unwind(
            "item".into(),
...
fn every_preparation_checkpoint_refuses_without_mutating_the_definition() {
    let query = leaf()
        .cross_join(leaf())
        .unwrap()
        .filter(&[equal(0, 2), equal(1, 3), GraphSetPredicateOp::And])
        .unwrap();
    let frozen = query.canonical_bytes();
    let mut checkpoints = 0;
    let expected = query
        .incremental_selected_join_with_control(&mut |_| {
            checkpoints += 1;
            Ok::<_, usize>(())
        })
        .unwrap()
...
fn selected_bag(
    ...
    let mut projected = IncrementalRowProjection::new(output);
    let delta = joined
        .prepare(left, right, LIMBS, None, &mut |_| Ok::<_, ()>(()))
        .unwrap()
        .commit();
    projected
        .prepare(&delta, LIMBS, None, &mut |_| Ok::<_, ()>(()))
...
fn every_projected_operand_remaps_without_changing_nullable_bag_semantics() {
    ...
    let right = bag([
        (row(&[Some(1), Some(30)]), 3),
        (row(&[Some(1), None]), 1),
        (row(&[Some(2), Some(10)]), 2),
    ]);
    for a in 0..4 {
        for b in 0..4 {
            for c in 0..4 {
                ...
                let query = leaf()
                    ...
                    .filter(&[
                        ...
                        GraphSetPredicateOp::IsNull {
                            operand: GraphSetOperand::Column(2),
                            is_null: false,
                        },
                        GraphSetPredicateOp::And,
                    ])
                ...
                for (l, lw) in left.iter() {
                    for (r, rw) in right.iter() {
                        ...
                        if !cells[a].is_null() && cells[a] == cells[b] && !cells[c].is_null() {
                            let result = GraphValueRow::from_owned_values(
                                slots.iter().map(|&at| (*cells[at]).clone()).collect(),
                            );
                            *expected.entry(result).or_default() +=
                                lw.to_i128().unwrap() * rw.to_i128().unwrap();
                        }
                    }
                }
                assert_eq!(
                    selected_bag(&query, &left, &right),
                    expected,
                    "slots {slots:?}"
                );
                let (_, _, _, output) = query
                    .incremental_selected_join_with_control(&mut allow)
                    .unwrap()
                    .unwrap();
                assert_eq!(
...
fn projected_collisions_integrate_both_parent_retractions_before_publication() {
    ...
    let mut projected = IncrementalRowProjection::new(output);
    let left = bag([
        (row(&[Some(1), Some(10)]), 2),
        (row(&[Some(1), Some(30)]), 1),
    ]);
    let right = bag([
        (row(&[Some(1), Some(20)]), 1),
        (row(&[Some(1), Some(50)]), 3),
    ]);
    let initial = joined
        .prepare(&left, &right, LIMBS, None, &mut |_| Ok::<_, ()>(()))
        .unwrap()
    ...
    assert_eq!(
        plain(projected.rows()),
        Bag::from([(row(&[Some(1), Some(1)]), 12)])
    );
    let left_delta = bag([(row(&[Some(1), Some(10)]), -2)]);
    let right_delta = bag([(row(&[Some(1), Some(50)]), -3)]);
    let pending = joined
        .prepare(&left_delta, &right_delta, LIMBS, None, &mut |_| {
            Ok::<_, ()>(())
        })
        .unwrap()
        .commit();
    let delta = projected
        .prepare(&pending, LIMBS, None, &mut |_| Ok::<_, ()>(()))
        .unwrap()
        .commit();
    assert_eq!(plain(&delta), Bag::from([(row(&[Some(1), Some(1)]), -11)]));
    assert_eq!(
        plain(projected.rows()),
        Bag::from([(row(&[Some(1), Some(1)]), 1)])
    );
}
...
fn projection_lowering_preserves_barriers_and_refuses_at_every_metadata_checkpoint() {
    ...
    for stop in 1..=total {
        let mut seen = 0;
        assert!(
            matches!(query.incremental_selected_join_with_control(&mut |_| {
            seen += 1;
            if seen == stop { Err(stop) } else { Ok(()) }
        }), Err(at) if at == stop)
        );
