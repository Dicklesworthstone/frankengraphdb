...
fn product_and_selected_pages_match_primitive_occurrence_order_not_key_order() {
    ...
    for mask in 0..256 {
        ...
        for filtered in [false, true] {
            let mut query = sequence(&left, "a")
                .cross_join(sequence(&right, "b"))
                .unwrap();
            if filtered {
                query = query.filter(&[equality()]).unwrap();
            }
            let mut all = Vec::new();
            for &a in &left {
                for &b in &right {
...
fn hundred_million_pair_page_copies_only_the_selected_occurrences() {
    ...
    for (skip, count) in [(u64::MAX, u64::MAX), (0, 0)] {
        let result = execute(
            &product.clone().with_page(skip, Some(count)),
            limited,
            || Ok(()),
        )
        .unwrap();
...
fn selected_page_rejects_no_payload_after_the_output_page_is_full() {
    ...
    let query = sequence(&values, "a")
        .cross_join(sequence(&values, "b"))
        .unwrap()
        .filter(&[GraphSetPredicateOp::Truth(Some(true))])
        .unwrap()
        .with_page(20_000, Some(2));
    let observed = execute(&query, GqlQueryPolicy::new(0, 2, 2_000_000, 20_000), || {
        Ok(())
    })
    .unwrap();
    assert_eq!(
        observed.value,
        vec![row(&[Some(78), Some(32)]), row(&[Some(78), Some(33)])]
    );
...
fn nested_pages_distinct_and_computed_stages_keep_their_original_boundaries() {
    ...
    let selected = product
        ...
        .with_page(1, Some(1));
    assert_eq!(
        execute(&selected, policy(), || Ok(())).unwrap().value,
        vec![row(&[Some(2), Some(2)])]
    );
    let distinct = product
        .clone()
        .project(
            vec![GraphSetProjection::new("a", GraphSetValue::Column(0))],
    ...
    assert_eq!(
        execute(&distinct, policy(), || Ok(())).unwrap().value,
        vec![row(&[Some(1)])]
    );
    ...
    let projected = product
        ...
        .with_page(1, Some(3));
    assert_eq!(
        execute(&projected, policy(), || Ok(())).unwrap().value,
        vec![
            row(&[Some(2), Some(2)]),
            row(&[Some(1), Some(1)]),
            row(&[Some(2), Some(1)]),
...
fn every_checkpoint_and_inclusive_result_work_scratch_limit_is_fail_closed() {
    let query = sequence(&[Some(3), Some(1), Some(2)], "a")
        .cross_join(sequence(&[Some(2), Some(3), Some(1)], "b"))
        .unwrap()
        .filter(&[equality()])
        .unwrap()
        .with_page(1, Some(2));
