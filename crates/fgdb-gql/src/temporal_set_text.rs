...
fn scan(text: &str) -> Vec<Token<'_>> {
    ...
    while at < bytes.len() {
        ...
        if parens != 0 || brackets != 0 || braces != 0 {
            at += text[at..].chars().next().map_or(1, char::len_utf8); continue;
        }
        ...
        if ch.is_ascii_alphabetic() || ch == b'_' {
            ...
            } else { out.push(Token { start, end: at, kind: Kind::Other }); }
    ...
    while at < bytes.len() {
        ...
        if bytes[at] == b'$' {
            ...
            if bytes.get(end).is_some_and(|b| b.is_ascii_alphabetic() || *b == b'_') {
                ...
                out.insert(text[start..end].to_owned()); at = end; continue;
            }
        }
        at += text[at..].chars().next().map_or(1, char::len_utf8);
    }
...
fn valid(name: &str) -> bool {
    ...
    !bytes.is_empty() && bytes.len() <= crate::algebra::MAX_PATTERN_NAME_BYTES
    ...
    pub fn bind_parameters(&self, arguments: &GqlParameters)
        ...
        for spec in self.inner.parameter_schema() {
            if let Some(value) = arguments.get(&spec.name) {
                local.insert(spec.name.clone(), value).expect("prepared argument name");
            }
        }
...
pub struct BoundTemporalGraphSetQuery { query: PreparedGraphSet, as_of: CommitSeq }
impl core::fmt::Debug for BoundTemporalGraphSetQuery {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("BoundTemporalGraphSetQuery")
            .field("columns", &self.query.columns().len()).field("snapshot", &"[BOUND]").finish()
    }
}
impl BoundTemporalGraphSetQuery {
    #[must_use] pub fn query(&self) -> &PreparedGraphSet { &self.query }
    #[must_use] pub const fn as_of(&self) -> CommitSeq { self.as_of }
    #[must_use] pub fn canonical_bytes(&self) -> Vec<u8> {
...
mod tests {
    ...
    fn whole_compound_statement_shares_one_temporal_selector_and_catalog() {
        ...
        let args = GqlParameters::new().with_uint64("at", 7).unwrap()
            .with_int64("lo", 1).unwrap().with_int64("hi", 4).unwrap();
        let first = template.bind_parameters(&args).unwrap();
        let second = template.bind_parameters(&GqlParameters::new().with_uint64("at", 8).unwrap()
            .with_int64("lo", 1).unwrap().with_int64("hi", 4).unwrap()).unwrap();
    ...
    fn second_temporal_selector_or_late_selector_refuses_before_catalog() {
        for text in [
            ...
            let calls = Cell::new(0);
            assert!(PreparedTemporalGraphSetText::prepare(text, |kind, name| {
                calls.set(calls.get() + 1); symbols(kind, name)
            }).is_err(), "{text}");
