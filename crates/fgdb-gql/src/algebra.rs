...
mod output;
mod values;
pub use output::{GlaOutput, GlaIdentityOutput, GraphBindingRow};
pub use values::{GraphColumn, GraphValue, GraphValueRow, ValueProjection, GRAPH_VALUE_PAYLOAD_UNIT_BYTES};
pub use pattern::{
...
pub enum GlaOperator {
    ...
    ProjectValues { columns: Vec<ValueProjection> },
    OrderByValues,
}
...
impl<Row> GlaPlan<Row> {
    ...
    pub fn needs_vertex_values(&self) -> bool {
        self.projects_properties() || self.operators.iter()
        ...
        for operator in &self.operators {
            match operator {
                ...
                GlaOperator::ProjectValues { columns } => {
                    bytes.push(12); bytes.extend_from_slice(&(columns.len() as u64).to_be_bytes());
                    for column in columns {
                        match column {
                            ValueProjection::Vertex { slot } => {
                                bytes.push(0); bytes.extend_from_slice(&slot.0.to_be_bytes());
                            }
                            ValueProjection::Property { slot, key } => {
                                bytes.push(1); bytes.extend_from_slice(&slot.0.to_be_bytes());
