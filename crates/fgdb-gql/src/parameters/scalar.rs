...
use crate::algebra::{
    IntegerComparison, MAX_SCALAR_PREDICATE_BYTES, ScalarPredicate, ScalarPredicateError,
};
...
impl GqlScalarParameter {
    pub fn new(value: CanonicalScalar) -> Result<Self, ScalarPredicateError> {
        Ok(Self {
            operand: ScalarPredicate::new(value, IntegerComparison::Equal)?,
        })
    }
    ...
    pub fn canonical_bytes(&self) -> &[u8] {
        self.operand.canonical_value_bytes()
    }
...
impl GqlParameters {
    ...
    pub fn with_text(
        ...
        if value.len() > MAX_SCALAR_PREDICATE_BYTES {
            return Err(GqlParameterError::ScalarLiteral);
        }
        let value =
            CanonicalScalar::ucs_basic_text(value).map_err(|_| GqlParameterError::ScalarLiteral)?;
...
mod tests {
    ...
    fn arguments_share_payloads_across_clones_and_comparison_occurrences() {
        let argument =
            GqlScalarParameter::new(CanonicalScalar::ucs_basic_text(&"x".repeat(4096)).unwrap())
                .unwrap();
    ...
    fn scalar_and_numeric_names_share_one_map_and_numeric_transcripts_are_unchanged() {
        ...
        assert!(matches!(
            arguments.insert("n", GqlParameterValue::Scalar(scalar)),
    ...
    fn scalar_nullability_never_weakens_legacy_numeric_type_checks() {
        ...
        let bind = RelationBind::new()
            ...
            .with_property("p", PropertyKeyId(1));
        let template =
            PreparedGqlTemplate::prepare("MATCH (n:L) WHERE n.p=$value RETURN n", &bind).unwrap();
        for args in [
            GqlParameters::new().with_text("value", "7").unwrap(),
            GqlParameters::new()
                .with_scalar("value", CanonicalScalar::Int(7))
                .unwrap(),
            GqlParameters::new().with_null("value").unwrap(),
        ] {
            assert!(matches!(
                template.bind_parameters(&args),
                Err(GqlParameterError::TypeMismatch { .. })
            ));
        }
        assert!(
            template
                .bind_parameters(&GqlParameters::new().with_int64("value", 7).unwrap())
                .unwrap()
                .verifies_definition()
        );
    }
    ...
    fn scalar_admission_refuses_large_values_and_debug_does_not_export_them() {
        assert!(matches!(
            GqlParameters::new().with_text("secret", &"x".repeat(60_000)),
            Err(GqlParameterError::ScalarLiteral)
        ));
        let bytes = CanonicalScalar::bytes(vec![0; MAX_SCALAR_PREDICATE_BYTES + 1]).unwrap();
        assert!(GqlScalarParameter::new(bytes).is_err());
        let args = GqlParameters::new()
            .with_text("secret", "not query syntax ' $name")
            .unwrap();
