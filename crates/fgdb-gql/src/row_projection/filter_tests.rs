...
fn z(rows: &[(Option<i64>, i128)]) -> ZSet<GraphValueRow> {
    ZSet::from_updates(rows.iter().map(|(value, count)| (row(*value), ZWeight::from_i128(*count))),
        LIMBS, &mut allow).unwrap()
}
fn apply(state: &mut IncrementalRowProjection, changes: &ZSet<GraphValueRow>) -> ZSet<GraphValueRow> {
    state.prepare(changes, LIMBS, None, &mut allow).unwrap().commit()
}
...
fn nonnull() -> Vec<GraphSetPredicateOp> {
    vec![GraphSetPredicateOp::IsNull { operand: GraphSetOperand::Column(0), is_null: false }]
}
fn spec(quantifier: GraphSetQuantifier) -> RowProjectionSpec {
    let parity = GraphIntegerExpression::prepare(&[
        GraphIntegerOp::Column(0), GraphIntegerOp::Literal(Some(2)),
        GraphIntegerOp::Binary(GraphIntegerBinary::Remainder),
    ]).unwrap();
    RowProjectionSpec::new(vec![GraphSetColumnType::Scalar],
        vec![GraphSetProjection::new("parity", GraphSetValue::Integer(parity))], quantifier)
        .unwrap().with_filter(&nonnull()).unwrap()
}
...
fn oracle(input: &ZSet<GraphValueRow>, q: GraphSetQuantifier) -> BTreeMap<GraphValueRow, i128> {
    ...
    if q == GraphSetQuantifier::Distinct {
        for count in result.values_mut() { *count = 1; }
    }
    result
}
...
fn hidden_rows_keep_exact_input_admission_and_final_output_quotas() {
    let definition = RowProjectionSpec::selection(vec![GraphSetColumnType::Scalar], vec!["x".into()],
        &[GraphSetPredicateOp::Truth(Some(false))]).unwrap();
    let seed = || {
        let mut state = IncrementalRowProjection::new(definition.clone());
        state.prepare(&z(&[(None, i128::MAX)]), LIMBS, Some(0), &mut allow).unwrap().commit();
        state.prepare(&z(&[(None, i128::MAX)]), LIMBS, Some(0), &mut allow).unwrap().commit();
        state
    };
    let mut state = seed();
    ...
    assert!(matches!(state.prepare(&z(&[(Some(4), -1)]), LIMBS, Some(0), &mut allow),
    ...
    assert!(matches!(state.prepare(&z(&[(None, -1)]), LIMBS, Some(0), &mut allow),
        Err(RowProjectionError::NegativeMultiplicity)));
    let bad = ZSet::from_updates([(GraphValueRow::from_owned_values(vec![]), ZWeight::ONE)],
        LIMBS, &mut allow).unwrap();
...
fn selection_precedes_output_expressions_without_relaxing_definition_admission() {
    let divide = GraphIntegerExpression::prepare(&[
        GraphIntegerOp::Literal(Some(12)), GraphIntegerOp::Column(0),
        GraphIntegerOp::Binary(GraphIntegerBinary::Divide),
    ]).unwrap();
    let projection = RowProjectionSpec::new(vec![GraphSetColumnType::Scalar],
        vec![GraphSetProjection::new("ratio", GraphSetValue::Integer(divide))], GraphSetQuantifier::All).unwrap();
    let selected = projection.clone().with_filter(&[
        GraphSetPredicateOp::IsNull { operand: GraphSetOperand::Column(0), is_null: true },
    ]).unwrap();
    ...
    assert!(state.rows().is_empty());
    assert!(matches!(projection.clone().with_filter(&[GraphSetPredicateOp::And]),
        Err(RowProjectionBuildError::Filter(GraphSetFilterError::InvalidStack { .. }))));
    assert!(matches!(projection.with_filter(&[
        GraphSetPredicateOp::IsNull { operand: GraphSetOperand::Column(1), is_null: false },
    ]), Err(RowProjectionBuildError::Filter(GraphSetFilterError::UnknownInput { column: 1, .. }))));
}
...
fn selection_preserves_metadata_full_width_domains_and_zero_column_bags() {
    let definition = RowProjectionSpec::selection(
        ...
        vec!["same name".into(), "same name".into()], &[GraphSetPredicateOp::Truth(Some(true))]).unwrap();
    assert_eq!(definition.columns().collect::<Vec<_>>(), vec!["same name", "same name"]);
    let value = GraphValueRow::from_owned_values(vec![GraphValue::Vertex(VId(u128::MAX)),
        GraphValue::Scalar(CanonicalScalar::Int(7))]);
    let input = ZSet::from_updates([(value, ZWeight::from_i128(i128::MAX))], LIMBS, &mut allow).unwrap();
    ...
    let definition = RowProjectionSpec::selection(vec![], vec![], &[GraphSetPredicateOp::Truth(Some(true))]).unwrap();
    let mut state = IncrementalRowProjection::new(definition);
    assert_eq!(apply(&mut state, &input), input);
    assert!(matches!(RowProjectionSpec::selection(vec![GraphSetColumnType::Scalar], vec![], &nonnull()),
...
fn three_valued_boolean_and_incompatible_comparison_semantics_are_shared() {
    for a in [None, Some(false), Some(true)] {
        for b in [None, Some(false), Some(true)] {
            for and in [false, true] {
                let expected = if and {
                    if a == Some(false) || b == Some(false) { Some(false) } else { a.zip(b).map(|(a,b)| a && b) }
                } else if a == Some(true) || b == Some(true) { Some(true) } else { a.zip(b).map(|(a,b)| a || b) };
                for negate in [false, true] {
                    let mut code = vec![GraphSetPredicateOp::Truth(a), GraphSetPredicateOp::Truth(b),
                        if and { GraphSetPredicateOp::And } else { GraphSetPredicateOp::Or }];
                    if negate { code.push(GraphSetPredicateOp::Not); }
                    let definition = RowProjectionSpec::selection(vec![GraphSetColumnType::Scalar], vec!["x".into()], &code).unwrap();
                    let mut state = IncrementalRowProjection::new(definition);
                    let delta = apply(&mut state, &z(&[(Some(1), 3)]));
                    let keep = (if negate { expected.map(|x| !x) } else { expected }) == Some(true);
                    assert_eq!(delta.is_empty(), !keep);
                    let mut seen = 0;
                    GraphSetPredicateOp::evaluate_row_with_control(&code, &row(Some(1)), &mut |_| {
                        seen += 1; Ok::<_, usize>(())
                    }).unwrap();
    ...
    let code = vec![GraphSetPredicateOp::Compare { left: GraphSetOperand::Column(0),
        comparison: IntegerComparison::Equal, right: GraphSetOperand::Column(1) }, GraphSetPredicateOp::Not];
    let definition = RowProjectionSpec::selection(vec![GraphSetColumnType::Scalar; 2], vec!["a".into(), "b".into()], &code).unwrap();
    let text = CanonicalScalar::ucs_basic_text("1").unwrap();
    let rows = [vec![CanonicalScalar::Int(1), text], vec![CanonicalScalar::Int(1), CanonicalScalar::Null],
        vec![CanonicalScalar::Int(1), CanonicalScalar::Int(2)]];
    let input = ZSet::from_updates(rows.into_iter().map(|values| (
        GraphValueRow::from_owned_values(values.into_iter().map(GraphValue::Scalar).collect()), ZWeight::ONE)),
        LIMBS, &mut allow).unwrap();
    let mut state = IncrementalRowProjection::new(definition);
    apply(&mut state, &input);
    assert_eq!(state.rows().len(), 1);
    assert_eq!(state.rows().iter().next().unwrap().0.values()[1], GraphValue::Scalar(CanonicalScalar::Int(2)));
}
...
fn every_selection_checkpoint_drop_and_unwind_preserves_hidden_and_visible_state() {
    for q in [GraphSetQuantifier::All, GraphSetQuantifier::Distinct] {
        let seed = || { let mut state = IncrementalRowProjection::new(spec(q));
            apply(&mut state, &z(&[(None, 3), (Some(2), 2)])); state };
        let changes = z(&[(None, -2), (Some(2), -2), (Some(4), 3), (Some(-1), 1)]);
        let mut success = seed();
        let mut calls = 0;
        let expected = success.prepare(&changes, LIMBS, None, &mut |_| {
            calls += 1; Ok::<_, usize>(())
        }).unwrap().commit();
        for stop in 1..=calls {
            let mut state = seed();
            let mut seen = 0;
            assert_eq!(state.prepare(&changes, LIMBS, None, &mut |_| {
        ...
        let mut state = seed();
        { let pending = state.prepare(&changes, LIMBS, None, &mut allow).unwrap(); assert_eq!(pending.delta(), &expected); }
        assert_eq!(state, seed());
...
fn filter_work_depends_on_changed_keys_not_hidden_support_or_multiplicity() {
    ...
    for size in [8, 1024] {
        for count in [1, 1_000_000] {
            ...
            let mut events = [0; 2];
            state.prepare(&z(&[(None, -count), (Some(2048), count)]), LIMBS, None, &mut |event| {
