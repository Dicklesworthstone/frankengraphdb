...
fn spec() -> RowProjectionSpec {
    RowProjectionSpec::unwind(
        vec![GraphSetColumnType::Any],
        vec!["xs".into()],
        "element".into(),
...
fn plain(rows: &ZSet<GraphValueRow>) -> BTreeMap<GraphValueRow, i128> {
    rows.iter()
        .map(|(r, n)| (r.clone(), n.to_i128().unwrap()))
        .collect()
}
    ...
    for (r, count) in input.iter() {
        if let GraphValue::List(elements) = &r.values()[0] {
            for element in elements.iter() {
                ...
                *output
                    .entry(GraphValueRow::from_owned_values(values))
                    .or_insert(0) += count.to_i128().unwrap();
            }
        }
    }
    output
...
fn nested_lists_identities_null_elements_and_wide_counts_keep_their_native_domains() {
    ...
    assert_eq!(
        state
            .rows()
            .weight(&GraphValueRow::from_owned_values(vec![xs.clone(), nested])),
        Some(&ZWeight::from_i128(i128::MAX))
    );
    assert_eq!(
        state
            .rows()
            .weight(&GraphValueRow::from_owned_values(vec![xs, null()])),
        Some(&ZWeight::from_i128(i128::MAX))
    );
    state
        .prepare(&z([(input, -i128::MAX)]), LIMBS, Some(0), &mut allow)
        .unwrap()
        .commit();
    assert!(state.rows().is_empty());
    // Unit tuples expand literals without manufacturing graph identities.
    let definition = RowProjectionSpec::unwind(
        vec![],
        vec![],
        "n".into(),
        GraphSetValue::List(vec![
            GraphSetValue::Value(int(9)),
            GraphSetValue::Value(int(9)),
        ]),
    )
    .unwrap();
    ...
    state
        .prepare(
            &z([(GraphValueRow::from_owned_values(vec![]), 7)]),
            LIMBS,
            None,
            &mut allow,
        )
...
fn downstream_distinct_retains_last_element_witness_and_all_participants_can_abort() {
    ...
    let definition = RowProjectionSpec::new(
        expansion.spec().column_types().to_vec(),
    ...
    distinct
        .prepare(expansion.rows(), LIMBS, None, &mut allow)
        .unwrap()
        .commit();
    ...
    let change = z([(a.clone(), -2)]);
    {
        ...
        let child = distinct
            .prepare(parent.delta(), LIMBS, Some(2), &mut allow)
            .unwrap();
    ...
    let parent = expansion
        .prepare(&z([(b, -3)]), LIMBS, Some(0), &mut allow)
        .unwrap();
    let child = distinct
        .prepare(parent.delta(), LIMBS, Some(0), &mut allow)
        .unwrap();
    child.commit();
    parent.commit();
    assert!(distinct.rows().is_empty());
    assert!(expansion.rows().is_empty());
}
...
fn hidden_inputs_and_nonlist_errors_are_not_erased_by_empty_output_or_zero_quotas() {
    ...
    for value in [null(), list(vec![])] {
        ...
        assert_eq!(
            state
                .prepare(&z([(input.clone(), -1)]), LIMBS, Some(0), &mut allow)
                .unwrap_err(),
            RowProjectionError::NegativeMultiplicity
        );
        state
            .prepare(&z([(input.clone(), i128::MAX)]), LIMBS, Some(0), &mut allow)
            .unwrap()
            .commit();
        state
            .prepare(&z([(input, -i128::MAX)]), LIMBS, Some(0), &mut allow)
    ...
    assert!(matches!(
        state.prepare(&z([(row(int(1)), 1)]), LIMBS, Some(0), &mut allow),
        Err(RowProjectionError::Expression {
            column: 1,
    ...
    let safe = spec()
        .with_filter(&[GraphSetPredicateOp::Truth(Some(false))])
        .unwrap();
    let mut filtered = IncrementalRowProjection::new(safe);
    filtered
        .prepare(&z([(row(int(1)), 1)]), LIMBS, Some(0), &mut allow)
        .unwrap()
        .commit();
    assert!(filtered.rows().is_empty());
    assert!(matches!(
        filtered.prepare(&z([(row(int(1)), -2)]), LIMBS, Some(0), &mut allow),
        Err(RowProjectionError::NegativeMultiplicity)
    ));
    let invalid = GraphValueRow::from_owned_values(vec![]);
    assert!(matches!(
        filtered.prepare(&z([(invalid, 1)]), LIMBS, Some(0), &mut allow),
...
fn each_checkpoint_and_final_quota_refusal_leave_full_input_and_expansion_unchanged() {
    ...
    let mut successful = seed(&old);
    let mut calls = 0;
    let expected = successful
        .prepare(&delta, LIMBS, None, &mut |_| {
            calls += 1;
            Ok::<_, usize>(())
        })
        .unwrap()
    ...
    assert_eq!(state, seed(&old));
    let a = row(list(vec![int(1), int(1)]));
    let b = row(list(vec![int(2), int(2)]));
    let mut state = seed(&z([(a.clone(), 1)]));
    state
        .prepare(&z([(a, -1), (b.clone(), 1)]), LIMBS, Some(2), &mut allow)
        .unwrap()
        .commit();
    assert!(matches!(
        state.prepare(&z([(b.clone(), 1)]), LIMBS, Some(2), &mut allow),
...
fn work_depends_on_changed_elements_not_unrelated_support_or_duplicate_occurrences() {
    ...
    for size in [8, 1024] {
        for count in [1, 1_000_000] {
            ...
            let mut state = seed(&old);
            let delta = z([
                (row(list(vec![int(0), int(0)])), -count),
                (row(list(vec![int(2048), int(2048)])), count),
            ]);
            let mut events = [0; 2];
            state
                .prepare(&delta, LIMBS, None, &mut |event| {
                    events[match event {
                        ZSetEvent::Work => 0,
...
fn checked_definition_preserves_inherited_names_but_checks_alias_shape_and_expression_scope() {
    ...
    .unwrap();
    assert_eq!(
        spec.columns().collect::<Vec<_>>(),
        vec!["left.x", "left.x", "element"]
    );
    ...
    assert!(
        RowProjectionSpec::unwind(vec![T::List], vec![], "x".into(), GraphSetValue::Column(0))
            .is_err()
    );
    assert!(
        RowProjectionSpec::unwind(
    ...
    assert!(
        RowProjectionSpec::unwind(
            vec![T::List; MAX_PATTERN_VERTICES],
            vec!["xs".into(); MAX_PATTERN_VERTICES],
            "x".into(),
            GraphSetValue::Column(0)
        )
        .is_err()
