...
use crate::{
    ...
    GraphIntegerOp as Op, GraphSetValue,
};
...
fn z(rows: &[(Option<i64>, i128)]) -> ZSet<GraphValueRow> {
    ZSet::from_updates(
        rows.iter().map(|(v, n)| (row(*v), ZWeight::from_i128(*n))),
        LIMBS,
        &mut allow,
    )
    .unwrap()
...
fn collisions_keep_last_support_and_final_quotas_ignore_transient_prefixes() {
    ...
    assert_eq!(
        plain(&delta),
        BTreeMap::from([(row(Some(0)), -1), (row(Some(1)), 1)])
    );
    let baseline = plain(distinct.rows());
    assert!(matches!(
        distinct.prepare(&z(&[(Some(0), 1)]), LIMBS, Some(1), &mut allow),
        Err(RowProjectionError::ResultBudget { limit: 1 })
    ));
    assert_eq!(plain(distinct.rows()), baseline);
    distinct
        .prepare(&z(&[(Some(3), -1)]), LIMBS, Some(0), &mut allow)
        .unwrap()
        .commit();
    ...
    all.prepare(&inverse, LIMBS, Some(0), &mut allow)
        .unwrap()
        .commit();
...
fn invalid_individual_inputs_cannot_hide_behind_cancelling_projected_images() {
    for q in [GraphSetQuantifier::All, GraphSetQuantifier::Distinct] {
        ...
        assert_eq!(
            state
                .prepare(&z(&[(Some(0), -1), (Some(2), 1)]), LIMBS, None, &mut allow)
                .unwrap_err(),
        ...
        let bad = ZSet::from_updates(
            [(
                GraphValueRow::from_owned_values(vec![GraphValue::Vertex(VId(u128::MAX))]),
                ZWeight::ONE,
            )],
            LIMBS,
            &mut allow,
    ...
    assert!(matches!(
        ...
        Err(RowProjectionBuildError::Projection(
            GraphSetProjectionError::UnknownInput { .. }
        ))
    ));
    assert!(matches!(
        RowProjectionSpec::new(
        ...
        Err(RowProjectionBuildError::Projection(
            GraphSetProjectionError::DuplicateName { .. }
        ))
    ));
}
...
fn every_checkpoint_drop_and_unwind_preserves_all_arrangements() {
    for q in [GraphSetQuantifier::All, GraphSetQuantifier::Distinct] {
        let seed = || {
            let mut state = IncrementalRowProjection::new(spec(q));
            apply(&mut state, &z(&[(Some(0), 2), (Some(3), 1)]));
            state
        };
        ...
        for stop in 1..=calls {
            ...
            assert_eq!(apply(&mut state, &delta), expected);
...
fn lazy_native_expressions_lists_and_full_width_values_keep_their_domains() {
    let safe = GraphIntegerExpression::prepare(&[
        Op::Column(0),
        Op::Literal(Some(0)),
    ...
    let projection = vec![
        ...
        GraphSetProjection::new(
            "list",
            GraphSetValue::List(vec![GraphSetValue::Column(0), GraphSetValue::Column(1)]),
        ),
    ];
    let spec = RowProjectionSpec::new(
        vec![GraphSetColumnType::Scalar, GraphSetColumnType::Vertex],
        projection,
        GraphSetQuantifier::All,
    )
    .unwrap();
    ...
    let next = RowProjectionSpec::new(
        state.spec().column_types().to_vec(),
        vec![GraphSetProjection::new(
            "size",
            GraphSetValue::Size(Box::new(GraphSetValue::Column(2))),
        )],
...
fn changed_key_work_is_independent_of_unrelated_support_and_occurrence_counts() {
    ...
    for size in [8, 1024] {
        ...
        apply(
            &mut state,
            &z(&(0..size).map(|i| (Some(i), 1)).collect::<Vec<_>>()),
        );
        let mut stats = [0; 2];
        state
            .prepare(
                &z(&[(Some(0), -1), (Some(2048), 1)]),
                LIMBS,
                None,
                &mut |e| {
                    stats[match e {
