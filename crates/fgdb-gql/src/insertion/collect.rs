...
fn endpoint<E, A, C>(endpoint: GraphInsertEndpoint, row: &[GraphValue], row_at: usize, edge: usize)
    -> ResultOf<Endpoint, E, A, C> {
    match endpoint {
        GraphInsertEndpoint::CreatedVertex(vertex) => Ok(Endpoint::Created(vertex)),
        GraphInsertEndpoint::Column(column) => row[column].as_vertex()
            .map(Endpoint::Existing).ok_or(GqlQueryError::Source(
                GraphInsertError::NullEndpoint { row: row_at, edge, column })),
    }
}

fn fields<E, A, C>(properties: &Properties, row: &[GraphValue], row_at: usize, declaration: usize,
...
pub(super) fn execute<E, A, C>(
    ...
    let (selected_rows, selection, evaluator) = match &insertion.selection {
        ...
        None => (
            Vec::new(),
            GqlExecutionStats { snapshot_records: 0, result_rows: 1 },
            GlaExecutionStats::default(),
        ),
    };
    for (dimension, observed) in [
        (GqlBudgetDimension::SnapshotRecords, selection.snapshot_records),
        (GqlBudgetDimension::ResultRows, selection.result_rows),
    ] {
        policy.query.rows.check(dimension, observed).map_err(GqlQueryError::Rows)?;
    }
    for (dimension, limit, observed) in [
        (GlaLimitDimension::WorkUnits, policy.query.evaluator.max_work_units, evaluator.work_units),
        (GlaLimitDimension::ScratchEntries, policy.query.evaluator.max_scratch_entries, evaluator.scratch_entries),
    ] {
        if observed > limit {
    ...
    let mut meter = Meter { policy: policy.query, evaluator, checkpoint };
    meter.event(GlaExecutionEvent::Work)?;
    let columns = insertion.selection.as_ref().map_or(&[][..], |pattern| pattern.value_columns());
    ...
    Ok(GraphInsertBatch {
        ...
        stats: GraphInsertStats {
            selection, evaluator: meter.evaluator,
