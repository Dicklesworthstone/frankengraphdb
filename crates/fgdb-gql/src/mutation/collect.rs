...
fn equal<E>(left: Value<'_>, right: Value<'_>,
...
fn reserve_copy<E>(value: &CanonicalScalar,
...
pub(super) fn execute<E, C>(
    mutation: &PreparedGraphMutation, policy: GraphMutationPolicy,
    ...
    if u64::try_from(selected.value.len()).ok() != Some(selected.rows.result_rows) {
        return Err(GqlQueryError::Source(GraphMutationError::InvalidSourceStatistics));
    }
    ...
    let mut meter = Meter { policy, evaluator: selected.evaluator, checkpoint };
        ...
        for (action_at, action) in mutation.actions.iter().enumerate() {
            ...
            if let Some(previous) = proposals.get(&(vertex, field)) {
                if !equal(previous.value, value, &mut |event| meter.event(event))? {
                    return Err(GqlQueryError::Source(GraphMutationError::ConflictingAssignment {
                        first_row: previous.row, first_action: previous.action,
                        row: row_at, action: action_at,
                    }));
                }
            } else {
                ...
                if observed > u128::from(policy.max_effects) {
                    return Err(GqlQueryError::Source(GraphMutationError::EffectLimit {
                        limit: policy.max_effects, observed,
                    }));
                }
                meter.event(GlaExecutionEvent::ScratchEntry)?;
                proposals.insert((vertex, field), Proposal { value, row: row_at, action: action_at });
            }
        }
    }
    ...
    for ((vertex, field), proposal) in proposals {
        ...
        if previous_vertex != Some(vertex) {
            target_vertices += 1; previous_vertex = Some(vertex);
        }
        meter.event(GlaExecutionEvent::ScratchEntry)?;
    ...
    let stats = GraphMutationStats {
        ...
        target_vertices, effects: intents.len() as u64,
    };
...
mod tests {
    ...
    fn mutation() -> PreparedGraphMutation {
        PreparedGraphMutation::prepare(query(), RelationId(1), vec![
            GraphMutationAction::SetProperty { target: 0, key: PropertyKeyId(2), value: GraphMutationValue::Column(1) },
        ]).unwrap()
    }
    fn run(values: &[CanonicalScalar], policy: GraphMutationPolicy) -> ResultOf<GraphMutationBatch, (), ()> {
        mutation().execute_governed(policy, |plan, policy| plan.plan().execute_governed_with_properties(
            values.len() as u64, [], (0..values.len()).map(|i| (VId(1), RelationId(1), VId(i as u128 + 10))),
            |_, _| Ok::<_, ()>(true), |vid, _| Ok(Some(&values[vid.0 as usize - 10])), policy, || Ok::<_, ()>(()),
        ), || Ok::<_, ()>(()))
    }
    ...
    fn repeated_assignments_collapse_but_disagreement_never_selects_a_last_writer() {
        ...
        assert_eq!(result.intents(), &[GraphMutationIntent::Property {
            vertex: VId(1), key: PropertyKeyId(2), value: Some(CanonicalScalar::Int(7)),
        }]);
        assert_eq!(result.stats().selection.result_rows, 2);
        assert_eq!((result.stats().target_vertices, result.stats().effects), (1, 1));
        for values in [[CanonicalScalar::Int(7), CanonicalScalar::Int(9)],
            [CanonicalScalar::Int(9), CanonicalScalar::Int(7)],
            [CanonicalScalar::Int(7), CanonicalScalar::Null]] {
            assert!(matches!(run(&values, policy()), Err(GqlQueryError::Source(GraphMutationError::ConflictingAssignment { .. }))));
        }
    }
    #[test]
    fn selection_and_proposals_share_work_scratch_and_exact_effect_limits() {
        ...
        let exact = GraphMutationPolicy::new(GqlQueryPolicy::new(2, 2,
            stats.evaluator.work_units, stats.evaluator.scratch_entries), 1);
        ...
        for cap in [
            ...
            GraphMutationPolicy::new(GqlQueryPolicy::new(2, 2, stats.evaluator.work_units - 1, u64::MAX), 1),
            GraphMutationPolicy::new(GqlQueryPolicy::new(2, 2, u64::MAX, stats.evaluator.scratch_entries - 1), 1),
    ...
    fn schema_is_checked_before_a_source_can_run() {
        for action in [
            ...
            GraphMutationAction::RemoveProperty { target: usize::MAX, key: PropertyKeyId(1) },
            GraphMutationAction::SetProperty { target: 0, key: PropertyKeyId(2), value: GraphMutationValue::Column(0) },
