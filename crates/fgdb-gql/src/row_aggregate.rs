...
impl RowAggregateSpec {
    pub fn new(
        schema: &[GraphSetColumnType],
        keys: &[usize],
        value: usize,
    ) -> Result<Self, RowAggregateBuildError> {
        if schema.is_empty() {
            return Err(RowAggregateBuildError::EmptyInput);
        }
        ...
        for (at, &column) in keys.iter().enumerate() {
            if column >= schema.len() {
                return Err(RowAggregateBuildError::UnknownColumn { column });
            }
            if keys[..at].contains(&column) {
                return Err(RowAggregateBuildError::DuplicateKey { column });
            }
        }
...
impl core::fmt::Debug for RowAggregateSpec {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("RowAggregateSpec")
            .field("input_columns", &self.schema.len())
            .field("group_columns", &self.keys.len())
...
impl RowAggregateRow {
    ...
    pub fn count_rows(&self) -> &ZWeight {
        self.summary
            .as_ref()
            .map_or(&ZERO_COUNT, |s| s.count_rows())
    }
    pub fn count_values(&self) -> &ZWeight {
        self.summary
            .as_ref()
            .map_or(&ZERO_COUNT, |s| s.count_values())
    }
    pub fn count_distinct(&self) -> &ZWeight {
        self.summary
            .as_ref()
            .map_or(&ZERO_COUNT, |s| s.count_distinct())
    }
    pub fn sum(&self) -> Option<&ZWeight> {
        self.summary.as_ref().and_then(|s| s.sum())
    }
    pub fn sum_distinct(&self) -> Option<&ZWeight> {
        self.summary.as_ref().and_then(|s| s.sum_distinct())
    }
    pub fn minimum(&self) -> Option<i128> {
        self.summary.as_ref().and_then(|s| s.minimum())
    }
    pub fn maximum(&self) -> Option<i128> {
        self.summary.as_ref().and_then(|s| s.maximum())
    }
    ...
    pub fn average_distinct_parts(&self) -> Option<(&ZWeight, &ZWeight)> {
        self.summary
            .as_ref()
            .and_then(|s| s.average_distinct_parts())
    }
}
impl core::fmt::Debug for RowAggregateRow {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("RowAggregateRow")
            .field("key_columns", &self.key.len())
...
impl<E: core::fmt::Display> core::fmt::Display for RowAggregateError<E> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::NonIntegerValue { column } => {
                write!(f, "aggregate column {column} requires Int64 or NULL")
            }
            Self::NegativeMultiplicity => {
                f.write_str("negative integrated aggregate input multiplicity")
            }
...
impl IncrementalRowAggregate {
    ...
    pub fn spec(&self) -> &RowAggregateSpec {
        &self.spec
    }
    /// The first accepted prepare (even empty) installs the global zero group.
    pub fn rows(&self) -> &ZSet<RowAggregateRow> {
        &self.rows
    }
    pub fn prepare<E>(
        &mut self,
        ...
        for (row, change) in delta.iter() {
            ...
            if row.len() != self.spec.schema.len() {
                return Err(RowAggregateError::InputSchema);
            }
            ...
            if next < ZWeight::ZERO {
                return Err(RowAggregateError::NegativeMultiplicity);
            }
            let value = match &row.values()[self.spec.value] {
                ...
                _ => {
                    return Err(RowAggregateError::NonIntegerValue {
                        column: self.spec.value,
                    });
                }
            };
            charge(control, ZSetEvent::ScratchEntry)?;
        ...
        let aggregate = self
            .aggregate
            .prepare(&contributions, limbs, control)
        ...
        let count = ZWeight::from_i128(size)
...
impl core::fmt::Debug for IncrementalRowAggregate {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("IncrementalRowAggregate")
...
impl RowAggregateUpdate<'_> {
    pub fn delta(&self) -> &ZSet<RowAggregateRow> {
        &self.delta
    }
...
impl core::fmt::Debug for RowAggregateUpdate<'_> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("RowAggregateUpdate")
            .field("changed_rows", &self.delta.len())
            .finish_non_exhaustive()
    }
}
#[cfg(test)]
