...
impl core::fmt::Debug for PreparedGraphWriteProgramTemplate {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("PreparedGraphWriteProgramTemplate").field("statements", &self.statements.len())
            .field("parameters", &self.parameters.len()).field("definition", &"[REDACTED]").finish()
    }
}
impl PreparedGraphWriteProgramTemplate {
    pub fn prepare(statements: Vec<GraphWriteTemplateStatement>) -> Result<Self, GraphWriteProgramTemplateError> {
    ...
    #[must_use]
    pub fn statements(&self) -> &[GraphWriteTemplateStatement] { &self.statements }
    #[must_use]
    pub fn parameter_schema(&self) -> &[GqlParameterSpec] { &self.parameters }

    pub fn bind_parameters(&self, arguments: &GqlParameters) -> Result<PreparedGraphWriteProgram, GraphWriteProgramTemplateError> {
        ...
        for (statement, input) in self.statements.iter().enumerate() {
            ...
            let bound = match input {
                GraphWriteTemplateStatement::Mutation(input) => GraphWriteStatement::Mutation(
...
mod tests {
    ...
    fn insert(text: &str) -> PreparedGraphInsertText { PreparedGraphInsertText::prepare(text, R, symbols).unwrap() }
    fn mutation(text: &str) -> PreparedGraphMutationText { PreparedGraphMutationText::prepare(text, R, symbols).unwrap() }
    ...
    fn one_argument_map_binds_every_step_without_catalog_reentry() {
        ...
        let a = PreparedGraphInsertText::prepare("CREATE (x {p:$seed,q:$shared})", R, &mut resolve).unwrap();
        let b = PreparedGraphMutationText::prepare("MATCH (n) SET n.p=n.p+$shared", R, &mut resolve).unwrap();
        let c = PreparedGraphInsertText::prepare("MATCH (n) WHERE n.p >= $shared CREATE (y {p:$tail})", R, &mut resolve).unwrap();
        let before = calls.get();
        let template = PreparedGraphWriteProgramTemplate::prepare(vec![a.clone().into(), b.clone().into(), c.clone().into()]).unwrap();
        let schema = template.parameter_schema();
        assert_eq!(schema.iter().map(|spec| (spec.name.as_str(), spec.occurrences)).collect::<Vec<_>>(),
            vec![("seed", 1), ("shared", 3), ("tail", 1)]);
        let arguments = GqlParameters::new().with_int64("seed", 1).unwrap().with_int64("shared", 2).unwrap().with_int64("tail", 3).unwrap();
        let actual = template.bind_parameters(&arguments).unwrap();
        let expected = PreparedGraphWriteProgram::prepare(vec![
            a.bind_parameters(&GqlParameters::new().with_int64("seed", 1).unwrap().with_int64("shared", 2).unwrap()).unwrap().into(),
            b.bind_parameters(&GqlParameters::new().with_int64("shared", 2).unwrap()).unwrap().into(),
            c.bind_parameters(&GqlParameters::new().with_int64("shared", 2).unwrap().with_int64("tail", 3).unwrap()).unwrap().into(),
        ]).unwrap();
        assert_eq!(actual, expected);
        assert_eq!(actual, template.bind_parameters(&arguments).unwrap());
        assert_eq!(calls.get(), before);
        let changed = GqlParameters::new().with_int64("seed", 4).unwrap().with_int64("shared", 2).unwrap().with_int64("tail", 3).unwrap();
        assert_ne!(actual.canonical_bytes(), template.bind_parameters(&changed).unwrap().canonical_bytes());
    ...
    fn parameter_types_relations_and_program_sizes_are_checked_once() {
        ...
        let a = PreparedGraphInsertText::prepare_with_parameter_types("CREATE (x {p:$x})", R,
            &[("x", GqlParameterType::Scalar(boolean))], symbols).unwrap();
        let b = mutation("MATCH (n) SET n.p=$x");
        let error = PreparedGraphWriteProgramTemplate::prepare(vec![a.into(), b.into()]).unwrap_err();
        assert!(matches!(error, GraphWriteProgramTemplateError::Program(GraphMutationProgramTemplateError::ConflictingParameterTypes {
        ...
        assert!(matches!(PreparedGraphWriteProgramTemplate::prepare(vec![insert("CREATE (x)").into(), foreign.into()]),
            Err(GraphWriteProgramTemplateError::Program(GraphMutationProgramTemplateError::Definition(GraphMutationProgramBuildError::MixedRelation { statement: 1 })))));
        assert!(PreparedGraphWriteProgramTemplate::prepare(vec![]).is_err());
        assert!(PreparedGraphWriteProgramTemplate::prepare(vec![insert("CREATE (x)").into(); MAX_GRAPH_MUTATION_STATEMENTS + 1]).is_err());
    }
    ...
    fn later_bind_failures_retain_statement_index_and_original_utf8_offsets() {
        ...
        let template = PreparedGraphWriteProgramTemplate::prepare(vec![
            insert("CREATE (x {p:1})").into(), mutation("MATCH (n) SET n.p=2").into(), insert(text).into(),
        ]).unwrap();
        let error = template.bind_parameters(&GqlParameters::new()).unwrap_err();
        assert!(matches!(error, GraphWriteProgramTemplateError::InsertBind { statement: 2, source }
        ...
        let template = PreparedGraphWriteProgramTemplate::prepare(vec![insert("CREATE (x)").into(), mutation(text).into()]).unwrap();
        let error = template.bind_parameters(&GqlParameters::new()).unwrap_err();
        assert!(matches!(error, GraphWriteProgramTemplateError::Program(GraphMutationProgramTemplateError::Bind { statement: 1, source })
            if source.offset == text.find('$').unwrap()));
        let extra = GqlParameters::new().with_int64("unused", 3).unwrap();
        assert!(matches!(template.bind_parameters(&extra), Err(GraphWriteProgramTemplateError::Program(GraphMutationProgramTemplateError::UnexpectedArguments))));
    }
    ...
    fn text_arguments_remain_values_in_both_creation_and_mutation() {
        ...
        let a = PreparedGraphInsertText::prepare_with_parameter_types("CREATE (x {p:$value})", R,
            &[("value", GqlParameterType::Scalar(kind))], symbols).unwrap();
        let b = PreparedGraphMutationText::prepare_with_parameter_types("MATCH (n) SET n.q=$value", R,
            &[("value", GqlParameterType::Scalar(kind))], symbols).unwrap();
        let template = PreparedGraphWriteProgramTemplate::prepare(vec![a.into(), b.into()]).unwrap();
        ...
        assert!(template.bind_parameters(&GqlParameters::new().with_int64("value", 1).unwrap()).is_err());
    }
    ...
    fn mutation_only_templates_preserve_the_existing_schema_and_bindings() {
        ...
        let old = PreparedGraphMutationProgramTemplate::prepare(vec![a.clone(), b.clone()]).unwrap();
        ...
        let args = GqlParameters::new().with_int64("x", 1).unwrap().with_int64("y", 2).unwrap();
