...
impl<'a> GraphAggregate<'a> {
    ...
    pub const fn count_rows(name: &'a str) -> Self {
        Self { name, function: GraphAggregateFunction::CountRows, column: None }
    }
    #[must_use]
    pub const fn count(name: &'a str, column: usize) -> Self {
        Self { name, function: GraphAggregateFunction::Count, column: Some(column) }
    }
    #[must_use]
    pub const fn count_distinct(name: &'a str, column: usize) -> Self {
        Self { name, function: GraphAggregateFunction::CountDistinct, column: Some(column) }
    }
    #[must_use]
    pub const fn sum_int(name: &'a str, column: usize) -> Self {
        Self { name, function: GraphAggregateFunction::SumInt, column: Some(column) }
    }
    #[must_use]
    pub const fn min(name: &'a str, column: usize) -> Self {
        Self { name, function: GraphAggregateFunction::Min, column: Some(column) }
    }
    #[must_use]
    pub const fn max(name: &'a str, column: usize) -> Self {
        Self { name, function: GraphAggregateFunction::Max, column: Some(column) }
    }
}

impl core::fmt::Debug for GraphAggregate<'_> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("GraphAggregate").field("function", &self.function)
...
impl core::fmt::Display for GraphAggregateBuildError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::RequiresUnpaginatedAll => f.write_str("aggregate input must preserve duplicates and have no pagination"),
            Self::EmptyAggregates => f.write_str("at least one aggregate is required"),
            Self::TooManyColumns { limit, observed } => write!(f, "aggregate output has {observed} columns, limit {limit}"),
...
impl GraphAggregateValue {
    ...
    pub fn as_count(&self) -> Option<u64> {
        match self { Self::Count(value) => Some(*value), _ => None }
    }
    #[must_use]
    pub fn as_integer(&self) -> Option<i128> {
        match self { Self::Integer(value) => Some(*value), _ => None }
    }
    #[must_use]
    pub fn as_value(&self) -> Option<&GraphValue> {
        match self { Self::Value(value) => Some(value), _ => None }
    }
    #[must_use]
...
impl GraphAggregateRow {
    #[must_use]
    pub fn keys(&self) -> &[GraphValue] { &self.keys }
    #[must_use]
    pub fn values(&self) -> &[GraphAggregateValue] { &self.values }
    #[must_use]
    pub fn get(&self, aggregate: usize) -> Option<&GraphAggregateValue> { self.values.get(aggregate) }
}
impl core::fmt::Debug for GraphAggregateRow {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("GraphAggregateRow").field("key_columns", &self.keys.len())
            .field("aggregate_columns", &self.values.len()).field("values", &"[REDACTED]").finish()
    }
}
...
impl core::fmt::Debug for PreparedGraphAggregate {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("PreparedGraphAggregate").field("key_columns", &self.keys.len())
            .field("aggregate_columns", &self.aggregates.len()).field("definition", &"[REDACTED]").finish()
    }
}
...
impl PreparedGraphAggregate {
    pub fn prepare(
        ...
        if !input.preserves_duplicates()
            || !matches!(input.plan().operators().last(), Some(GlaOperator::Limit { offset: 0, count: None }))
        {
            return Err(GraphAggregateBuildError::RequiresUnpaginatedAll);
        }
        if aggregates.is_empty() { return Err(GraphAggregateBuildError::EmptyAggregates); }
        let width = keys.len().saturating_add(aggregates.len());
        if width > MAX_PATTERN_VERTICES {
            return Err(GraphAggregateBuildError::TooManyColumns { limit: MAX_PATTERN_VERTICES, observed: width });
        }
        let columns = input.columns();
        let mut names = BTreeSet::new();
        for (at, column) in keys.iter().enumerate() {
            let name = columns.get(*column).ok_or(GraphAggregateBuildError::UnknownColumn { column: *column })?;
            if keys[..at].contains(column) { return Err(GraphAggregateBuildError::DuplicateKey { column: *column }); }
            names.insert(name.as_str());
        }
        for aggregate in aggregates {
            if let Some(column) = aggregate.column && column >= columns.len() {
                return Err(GraphAggregateBuildError::UnknownColumn { column });
            }
            let name = aggregate.name.as_bytes();
            if name.is_empty() || name.len() > MAX_PATTERN_NAME_BYTES
            ...
            if !names.insert(aggregate.name) { return Err(GraphAggregateBuildError::DuplicateName); }
        }
        let key_names = keys.iter().map(|column| columns[*column].clone()).collect();
        let aggregate_names = aggregates.iter().map(|aggregate| aggregate.name.to_owned()).collect();
    ...
    #[must_use]
    pub fn input_pattern(&self) -> &PreparedGraphPattern<GraphValueRow> { &self.input }
    #[must_use]
    pub fn key_columns(&self) -> &[String] { &self.key_names }
    #[must_use]
    pub fn aggregate_columns(&self) -> &[String] { &self.aggregate_names }
    ...
    pub fn canonical_bytes(&self) -> Vec<u8> {
        ...
        for column in &self.keys { bytes.extend_from_slice(&(*column as u64).to_be_bytes()); }
        bytes.extend_from_slice(&(self.aggregates.len() as u64).to_be_bytes());
        for aggregate in &self.aggregates {
            bytes.push(match aggregate.function {
                GraphAggregateFunction::CountRows => 0, GraphAggregateFunction::Count => 1,
                GraphAggregateFunction::CountDistinct => 2, GraphAggregateFunction::SumInt => 3,
                GraphAggregateFunction::Min => 4, GraphAggregateFunction::Max => 5,
            });
            if let Some(column) = aggregate.column { bytes.extend_from_slice(&(column as u64).to_be_bytes()); }
        }
    ...
    pub fn execute_governed<'a, E, C>(
        ...
        self.input.plan().visit_value_bindings(
            ...
            |columns, bindings, property, control| {
                ...
                let state = groups.get_mut(key).expect("the admitted group was initialized");
...
impl ValueRef<'_> {
    ...
    fn payload_units(self) -> usize {
        let bytes = match self {
            ...
            Self::Scalar(CanonicalScalar::Text(value)) => value.len() + value.canonical_sort_key().map_or(0, <[u8]>::len),
            Self::Scalar(CanonicalScalar::Timestamp(value)) => value.zone().map_or(0, |zone| zone.identifier().len()),
            _ => 0,
        };
...
fn new_group<'a, E>(
    ...
    for aggregate in aggregates {
        ...
        state.push(match aggregate.function {
            GraphAggregateFunction::CountRows | GraphAggregateFunction::Count => Accumulator::Count(0),
            GraphAggregateFunction::CountDistinct => Accumulator::Distinct(BTreeSet::new()),
            GraphAggregateFunction::SumInt => Accumulator::Sum { value: 0, present: false },
...
fn update<'a, E, C>(
    ...
    match state {
        Accumulator::Count(count) => { *count = count.checked_add(1).ok_or_else(overflow)?; }
        ...
        Accumulator::Sum { value: sum, present } => {
        ...
        Accumulator::Extreme(current) => {
            ...
            if current.is_none_or(|old| {
                if function == GraphAggregateFunction::Min { value < old } else { value > old }
            }) {
                *current = Some(value);
            }
        }
