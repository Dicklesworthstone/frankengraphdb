use super::*;
use crate::{GraphSetProjection, GraphSetValue, GraphSetQuantifier,
    GraphIntegerExpression, GraphIntegerOp};

fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(u64::MAX, u64::MAX, u64::MAX, u64::MAX)
}
fn no_source(
    _: &PreparedGraphPattern<GraphValueRow>, _: GqlQueryPolicy,
...
fn factor(values: &[Option<i64>]) -> PreparedGraphSet {
    PreparedGraphSet::singleton().unwind("x".into(), GraphSetValue::List(
        values.iter().map(|value| GraphSetValue::Value(GraphValue::Scalar(
            value.map_or(CanonicalScalar::Null, CanonicalScalar::Int),
        ))).collect(),
    )).unwrap().project(vec![GraphSetProjection::new("x", GraphSetValue::Integer(
...
fn definition(aggregates: &[GraphAggregate<'_>]) -> PreparedGraphAggregate {
    PreparedGraphAggregate::prepare_set_relation(factor(&[Some(1)]), &[], aggregates, 0, None).unwrap()
}
fn values(states: Vec<NumericState>) -> Vec<crate::GraphAggregateValue> {
    states.into_iter().map(|state| state.finish_governed(&mut |_| Ok::<_, ()>(())).unwrap()).collect()
}

#[test]
fn repeated_numeric_and_support_cells_equal_expanded_updates() {
    let q = definition(&[
        GraphAggregate::count_rows("rows"), GraphAggregate::count("count", 0),
    ...
    for repeats in 1..=17 {
        ...
        for value in [None, Some(-7), Some(0), Some(3), Some(-7), None] {
            let cell = GraphValue::Scalar(value.map_or(CanonicalScalar::Null, CanonicalScalar::Int));
            update(&q, &mut compressed, &mut |_| &cell, Some(repeats),
                &mut |_| Ok::<_, Failure<(), ()>>(())).unwrap();
            for _ in 0..repeats {
                unit(&q, &mut expanded, &mut |_| &cell,
                    &mut |_| Ok::<_, Failure<(), ()>>(())).unwrap();
            }
        }
...
fn the_first_failing_occurrence_wins_before_aggregate_column_order() {
    let q = definition(&[GraphAggregate::count_rows("count"), GraphAggregate::sum_int("sum", 0)]);
    let cell = GraphValue::Scalar(CanonicalScalar::Int(1));
    for (count, sum, failing) in [(u64::MAX - 4, i128::MAX - 1, 1),
        (u64::MAX - 1, i128::MAX - 4, 0), (u64::MAX - 1, i128::MAX - 1, 0)] {
        let make = || vec![NumericState::Count(count), NumericState::Sum(Some(sum))];
        let mut compressed = make();
        let actual = update(&q, &mut compressed, &mut |_| &cell, Some(5),
            &mut |_| Ok::<_, Failure<(), ()>>(()));
        let mut expanded = make();
        let expected = (0..5).try_for_each(|_| unit(&q, &mut expanded, &mut |_| &cell,
            &mut |_| Ok::<_, Failure<(), ()>>(())));
...
fn signed_distance_is_not_rejected_for_an_overflowing_standalone_product() {
    ...
    for (initial, step, expected) in [(i128::MIN, 1, i128::MAX),
        (i128::MAX, -1, i128::MIN)] {
        let cell = GraphValue::Scalar(CanonicalScalar::Int(step));
        let mut states = vec![NumericState::Sum(Some(initial))];
        update(&q, &mut states, &mut |_| &cell, Some(u128::MAX),
            &mut |_| Ok::<_, Failure<(), ()>>(())).unwrap();
        assert_eq!(values(states), vec![crate::GraphAggregateValue::Integer(expected)]);
    }
...
fn oversized_repetitions_are_valid_for_zero_null_and_distinct_support() {
    let q = definition(&[GraphAggregate::sum_int("zero", 0),
    ...
    update(&q, &mut states, &mut |_| &zero, None,
        &mut |_| Ok::<_, Failure<(), ()>>(())).unwrap();
    assert_eq!(values(states), vec![crate::GraphAggregateValue::Integer(0),
        crate::GraphAggregateValue::Count(1)]);
    let q = definition(&[GraphAggregate::count("null", 0), GraphAggregate::average_int("avg", 0)]);
    let null = GraphValue::Scalar(CanonicalScalar::Null);
    let mut states = new_states(&q, &mut |_| Ok::<_, Failure<(), ()>>(())).unwrap();
    update(&q, &mut states, &mut |_| &null, None,
        &mut |_| Ok::<_, Failure<(), ()>>(())).unwrap();
...
fn grouped_and_global_summaries_match_materialization_across_occurrence_pages() {
    for grouped in [false, true] {
        for n in 0..=4 {
            for (skip, limit) in [(0, None), (1, None), (1, Some(3)), (0, Some(0)), (u64::MAX, None)] {
                let input = factor(&[None, Some(3), Some(-1), Some(3)])
                    .cross_join(factor(&vec![Some(7); n])).unwrap()
                    .with_page(skip, limit).nested().unwrap().with_page(0, Some(11));
                let q = PreparedGraphAggregate::prepare_set_relation(input,
                    if grouped { &[0] } else { &[] }, &[
                        GraphAggregate::count_rows("rows"), GraphAggregate::count("n", 0),
                        GraphAggregate::sum_int("sum", 0), GraphAggregate::average_int("avg", 0),
                        GraphAggregate::count_distinct("d", 0), GraphAggregate::sum_int_distinct("ds", 0),
                        GraphAggregate::average_int_distinct("da", 0),
                        GraphAggregate::min("min", 0), GraphAggregate::max("max", 0),
                    ], 0, None).unwrap();
                assert!(q.input_relation().unwrap().has_repeated_factor(&q.repeated_columns()));
...
fn power(mut relation: PreparedGraphSet, squarings: usize) -> PreparedGraphSet {
    for _ in 0..squarings { relation = relation.clone().cross_join(relation).unwrap(); }
    relation
}
...
fn trillion_occurrence_groups_fit_factor_sized_budgets_and_keep_exact_averages() {
    let input = factor(&[Some(3), Some(3), Some(-1)])
        .cross_join(power(factor(&[Some(0); 32]), 3)).unwrap();
    let q = PreparedGraphAggregate::prepare_set_relation(input, &[0], &[
        GraphAggregate::count_rows("n"), GraphAggregate::sum_int("s", 0),
        GraphAggregate::average_int("a", 0), GraphAggregate::count_distinct("d", 0),
    ], 0, None).unwrap();
    let result = q.execute_relational_with_source(
        GqlQueryPolicy::new(0, 2, 100_000, 20_000), no_source, || Ok(()),
    ).unwrap();
    let count = 1_u64 << 40;
    assert_eq!(result.value.len(), 2);
    assert_eq!(result.value[0].values()[0].as_count(), Some(count));
    assert_eq!(result.value[1].values()[0].as_count(), Some(2 * count));
    assert_eq!(result.value[1].values()[1], crate::GraphAggregateValue::Integer(6 * i128::from(count)));
    assert_eq!(result.value[1].values()[2], crate::GraphAggregateValue::Average(
        crate::GraphExactAverage::new(3, 1).unwrap()));
...
fn weights_above_u64_do_not_spuriously_overflow_wide_sums_or_support() {
    let q = PreparedGraphAggregate::prepare_set_relation(
        factor(&[Some(1)]).cross_join(power(factor(&[Some(0); 16]), 4)).unwrap(), &[],
        &[GraphAggregate::sum_int("s", 0)], 0, None).unwrap();
    let result = q.execute_relational_with_source(wide(), no_source, || Ok(())).unwrap();
    assert_eq!(result.value[0].values()[0], crate::GraphAggregateValue::Integer(1_i128 << 64));
    let enormous = power(factor(&[Some(0); 256]), 5);
    for step in [0, 1] {
        let q = PreparedGraphAggregate::prepare_set_relation(
            factor(&[Some(step)]).cross_join(enormous.clone()).unwrap(), &[],
            &[GraphAggregate::sum_int("s", 0), GraphAggregate::count_distinct("d", 0)], 0, None,
        ).unwrap();
        let result = q.execute_relational_with_source(wide(), no_source, || Ok(()));
        if step == 0 {
            let result = result.unwrap();
            assert_eq!(result.value[0].values()[0], crate::GraphAggregateValue::Integer(0));
...
fn exact_quotas_and_all_interruption_checkpoints_share_one_allowance() {
    let input = factor(&[Some(1), Some(2)]).cross_join(factor(&[Some(0); 5])).unwrap();
    let q = PreparedGraphAggregate::prepare_set_relation(input, &[0],
        &[GraphAggregate::sum_int("s", 0)], 0, None).unwrap();
    let mut calls = 0;
    let baseline = q.execute_relational_with_source(wide(), no_source, || { calls += 1; Ok(()) }).unwrap();
    let exact = GqlQueryPolicy::new(0, 2, baseline.evaluator.work_units,
    ...
    for policy in [GqlQueryPolicy::new(0, 1, u64::MAX, u64::MAX),
        ...
        assert!(q.execute_relational_with_source(policy, no_source, || Ok(())).is_err());
    }
}
...
fn graph_leaf() -> PreparedGraphSet {
    ...
    builder.prepare_values(&[crate::algebra::GraphColumn::property(
        "v", "n", fgdb_delta_types::PropertyKeyId(1),
...
fn graph_sources_execute_once_and_share_snapshot_admission() {
    ...
    for max_snapshot in [3, 4] {
        ...
        let result = q.execute_relational_with_source(
            ...
            |pattern, remaining| {
                ...
                pattern.plan().execute_governed_with_properties(
                    2, [VId(1), VId(2)], [], |_, _| Ok::<_, &'static str>(true),
                    |_, _| Ok(Some(&scalar)), remaining, || Ok::<_, usize>(()),
                )
            }, || Ok(()),
        );
        assert_eq!(calls, 2);
        if max_snapshot == 4 {
            ...
            assert_eq!(result.value[0].values()[0], crate::GraphAggregateValue::Integer(12));
        } else {
...
fn later_right_source_failure_precedes_an_invalid_left_sum_even_at_limit_zero() {
    let left = PreparedGraphSet::singleton().unwind("bad".into(), GraphSetValue::List(vec![
        GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Bool(true))),
    ])).unwrap();
    let q = PreparedGraphAggregate::prepare_set_relation(
        left.cross_join(graph_leaf()).unwrap(), &[], &[GraphAggregate::sum_int("s", 0)],
        0, Some(0),
    ).unwrap();
    let mut calls = 0;
    let result = q.execute_relational_with_source(wide(), |_, _| {
        calls += 1;
...
fn observed_right_columns_and_explicit_input_sort_remain_value_barriers() {
    let relation = factor(&[Some(2), Some(-1)]).cross_join(factor(&[Some(3); 4])).unwrap();
