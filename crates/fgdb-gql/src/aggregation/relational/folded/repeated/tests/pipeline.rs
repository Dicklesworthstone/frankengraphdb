...
fn constant_list(length: usize) -> GraphSetValue {
    GraphSetValue::List((0..length).map(|at| {
        GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Int(at as i64)))
    }).collect())
}
...
fn sum_definition(input: PreparedGraphSet, count: Option<u64>) -> PreparedGraphAggregate {
    PreparedGraphAggregate::prepare_set_relation(input, &[], &[
        GraphAggregate::sum_int("sum", 0), GraphAggregate::count_rows("rows"),
    ], 0, count).unwrap()
}
...
fn trillion_occurrence_unwind_chains_keep_group_values_and_remapped_aliases() {
    ...
    let input = input.project(vec![
        ...
        GraphSetProjection::new("missing", GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Null))),
    ...
    let q = PreparedGraphAggregate::prepare_set_relation(input, &[0], &[
        ...
        GraphAggregate::average_int("average", 1), GraphAggregate::count_distinct("distinct", 1),
    ...
    assert_eq!(result.value[0].values()[1], crate::GraphAggregateValue::Integer(-i128::from(n)));
    assert_eq!(result.value[1].values()[0].as_count(), Some(2 * n));
    assert_eq!(result.value[1].values()[1], crate::GraphAggregateValue::Integer(6 * i128::from(n)));
...
fn reordered_duplicate_and_literal_columns_keep_original_occurrence_pages() {
    for grouped in [false, true] {
        for length in 0..=4 {
            for (skip, count) in [(0, None), (1, Some(5)), (4, Some(3)),
                (0, Some(0)), (u64::MAX, None)] {
                let input = expand(factor(&[None, Some(2), Some(-3), Some(2)]), "discard", length)
                    .with_page(skip, count).project(vec![
                        GraphSetProjection::new("five", GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Int(5)))),
                        GraphSetProjection::new("key", GraphSetValue::Column(0)),
                        GraphSetProjection::new("amount", GraphSetValue::Column(0)),
                        GraphSetProjection::new("null", GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Null))),
                    ], GraphSetQuantifier::All).unwrap()
                    .nested().unwrap().with_page(1, Some(7));
                let q = PreparedGraphAggregate::prepare_set_relation(input,
                    if grouped { &[1] } else { &[] }, &[
                        GraphAggregate::count_rows("rows"), GraphAggregate::sum_int("fixed", 0),
                        GraphAggregate::sum_int("sum", 2), GraphAggregate::average_int("avg", 2),
                        GraphAggregate::count_distinct("d", 2), GraphAggregate::count("none", 3),
...
fn literal_only_projection_can_drop_all_carrier_columns_without_dropping_rows() {
    let input = expand(factor(&[Some(1), Some(2)]), "discard", 3)
        .project(vec![GraphSetProjection::new("amount", GraphSetValue::Value(
            GraphValue::Scalar(CanonicalScalar::Int(-7)),
        ))], GraphSetQuantifier::All).unwrap().with_page(1, Some(4));
    ...
    assert_eq!(result.value[0].values()[0], crate::GraphAggregateValue::Integer(-28));
...
fn discarded_constant_list_expressions_fail_unless_their_actual_input_is_empty() {
    for empty in [false, true] {
        for output_limit in [None, Some(0)] {
            let input = factor(if empty { &[] } else { &[Some(1)] })
                .unwind("unused".into(), GraphSetValue::List(vec![broken_integer()])).unwrap();
            ...
            assert_eq!(actual.as_ref().map(|result| &result.value),
                expected.as_ref().map(|result| &result.value));
            if empty { assert!(actual.is_ok()); } else { assert!(actual.is_err()); }
        }
    }
    ...
    let input = factor(&[Some(1)]).unwind("unused".into(),
        GraphSetValue::Value(GraphValue::Scalar(CanonicalScalar::Int(7))),
    ).unwrap().with_page(0, Some(0));
    let q = sum_definition(input, None);
    assert!(q.execute_relational_with_source(wide(), no_source, || Ok(())).is_err());
}

#[test]
fn upstream_failure_precedes_a_discarded_constant_error_and_sources_are_not_retried() {
    let q = sum_definition(graph_leaf().unwind("unused".into(),
        GraphSetValue::List(vec![broken_integer()]),
    ).unwrap(), Some(0));
    let mut calls = 0;
...
fn observed_appended_values_and_unused_fallible_projections_stay_value_sensitive() {
    ...
    let q = PreparedGraphAggregate::prepare_set_relation(observed, &[],
        &[GraphAggregate::sum_int("sum", 1)], 0, None).unwrap();
    ...
    let dependent = factor(&[Some(1), Some(2)]).unwind("dependent".into(),
        GraphSetValue::List(vec![GraphSetValue::Column(0)]),
    ).unwrap();
    assert!(!dependent.has_repeated_factor(&[0]));

    let input = expand(factor(&[Some(1)]), "discard", 3).project(vec![
        GraphSetProjection::new("amount", GraphSetValue::Column(0)),
    ...
    assert_eq!(actual.as_ref().map(|r| &r.value), expected.as_ref().map(|r| &r.value));
}
...
fn completed_group_order_hidden_keys_and_output_pages_reuse_the_existing_stage() {
    let input = expand(factor(&[Some(1), Some(1), Some(3), Some(-2)]), "discard", 5)
        .project(vec![GraphSetProjection::new("amount", GraphSetValue::Column(0))],
            GraphSetQuantifier::All).unwrap();
    let q = PreparedGraphAggregate::prepare_set_relation(input, &[0], &[
        GraphAggregate::sum_int("sum", 0), GraphAggregate::count_rows("count"),
    ], 1, Some(1)).unwrap().with_result_clauses(&[], &[
        crate::GraphAggregateOrder::descending(crate::GraphAggregateColumn::Aggregate(0)),
    ...
    assert_eq!(actual.value[0].values(), &[crate::GraphAggregateValue::Integer(10)]);
    assert_eq!(actual.rows.result_rows, 1);
}

#[test]
fn every_pipeline_checkpoint_and_cumulative_quota_can_refuse_without_a_partial_result() {
    let q = sum_definition(expand(factor(&[Some(-2), Some(3)]), "unused", 7)
        .project(vec![GraphSetProjection::new("renamed", GraphSetValue::Column(0))],
            GraphSetQuantifier::All).unwrap().with_page(2, Some(9)), None);
    let mut calls = 0;
    ...
    for policy in [GqlQueryPolicy::new(0, 0, u64::MAX, u64::MAX),
