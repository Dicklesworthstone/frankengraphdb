...
impl<'q, 'a> RootGroups<'q, 'a> {
    pub(in crate::aggregation) fn new(query: &'q PreparedGraphAggregate) -> Option<Self> {
        ...
        let prefix = if count == 0 {
            ...
            usize::try_from(query.offset.checked_add(count)?).ok()?
        };
        Some(Self {
            query,
            prefix,
    ...
    pub(in crate::aggregation) fn push<E, C>(
        ...
        if !matches!(key, ValueRef::Vertex(_)) {
            return Err(GqlQueryError::Source(
                GraphAggregateError::NonMonotonicGroups,
            ));
        }
        ...
        let group = self
            .active
            .as_mut()
            .expect("active root group was admitted");
        update_group(
            &self.query.aggregates,
            &mut group.state,
            values,
            multiplicity,
            control,
    ...
    fn retire<E, C>(
        ...
        let Some(mut candidate) = self.active.take() else {
            return Ok(());
        };
        ...
        if self.heap.len() < self.prefix {
            ...
            while child > 0 {
                ...
                if self
                    .query
                    .compare(self.heap[parent].view(), self.heap[child].view(), control)?
                    != Ordering::Less
                {
                    break;
                }
            ...
            .query
            .compare(candidate.view(), self.heap[0].view(), control)?
            == Ordering::Less
        {
    ...
    fn sift_down<E>(
        ...
        while root < end / 2 {
            ...
            if child + 1 < end
                && self.query.compare(
                    self.heap[child].view(),
                    self.heap[child + 1].view(),
                    control,
                )? == Ordering::Less
            {
                child += 1;
            ...
            if self
                .query
                .compare(self.heap[root].view(), self.heap[child].view(), control)?
                != Ordering::Less
            {
                break;
            }
    ...
    pub(in crate::aggregation) fn finish<E, C>(
        ...
        for group in self.heap.iter().skip(offset).take(count) {
            output.push(group.view().copy_owned(
                self.query.key_output.as_ref(),
                self.query.output_aggregates,
                control,
            )?);
        }
        Ok(output)
...
mod tests {
    ...
    fn allow(_: GlaExecutionEvent) -> Result<(), QueryError<(), usize>> {
        Ok(())
    }
                ...
                for filtered in [false, true] {
                    for offset in 0..=4 {
                        for count in [0, 1, 3] {
                            ...
                            query.having = if filtered {
                                vec![GraphAggregateFilter {
                            ...
                            for hide_key in [false, true] {
                                for prefix in [0, 3, 6] {
                                    let query = query
                                        .clone()
                                        .with_key_output_columns(if hide_key { &[] } else { &[0] })
                                        .unwrap()
                                    ...
                                    for (id, choice) in choices.iter().enumerate() {
                                        for at in bags[*choice] {
                                            let values = [
                                                ValueRef::Vertex(VId(id as u128)),
                                                ValueRef::Scalar(&scalars[*at]),
                                            ];
                                            streaming
                                                .push(
                                                    &values,
                                                ...
                                                .unwrap();
                                            assert!(
                                                streaming.heap.len() <= (offset + count) as usize
                                            );
                                            let states = materialized
                                                .entry(vec![values[0]])
                                                .or_insert_with(|| {
                                                    new_group(&query.aggregates, &mut allow)
                                                        .unwrap()
                                                });
                                            update_group(
                                                &query.aggregates,
                                    ...
                                    assert_eq!(
                                        streaming.finish(&mut allow).unwrap(),
                                        query.finish_groups(&materialized, &mut allow).unwrap()
                                    );
                                }
                            }
                        }
    ...
    fn thousands_of_groups_retain_only_the_page_prefix_without_distinct_sets() {
        ...
        for root in 0..count as usize {
            for at in [root, root, root + 1] {
                streaming
                    .push(
                        &[
                            ValueRef::Vertex(VId(root as u128)),
                            ValueRef::Scalar(&values[at]),
                        ],
                        weighted::Multiplicity::ONE,
                        &mut allow,
                    )
                ...
                for retired in &streaming.heap {
                    ...
                    let Accumulator::Numeric(mean) = &retired.state[3] else {
                        panic!("average accumulator");
                    };
        ...
        assert!(
            rows.iter()
                .all(|row| row.get(0).unwrap().as_count() == Some(3)
                    && row.get(1).unwrap().as_count() == Some(2))
        );
        assert_eq!(
            rows[0].get(3).unwrap().as_average().unwrap().numerator(),
            8187
        );
        assert_eq!(
            rows[0].get(3).unwrap().as_average().unwrap().denominator(),
            2
        );
    }
    ...
    fn a_root_order_contract_violation_refuses_instead_of_splitting_a_group() {
        ...
        for root in [1, 1, 3] {
            streaming
                .push(
                    &[ValueRef::Vertex(VId(root)), ValueRef::Scalar(&scalar)],
                    weighted::Multiplicity::ONE,
                    &mut allow,
                )
                .unwrap();
        }
        let result = streaming.push(
            &[ValueRef::Vertex(VId(1)), ValueRef::Scalar(&scalar)],
            weighted::Multiplicity::ONE,
            &mut allow,
        );
        assert!(matches!(
        ...
        let result = RootGroups::new(&query).unwrap().push(
            &[ValueRef::Scalar(&NULL), ValueRef::Scalar(&scalar)],
            weighted::Multiplicity::ONE,
            &mut allow,
        );
        assert!(matches!(
    ...
    fn deferred_having_errors_do_not_hide_later_source_or_aggregate_input_errors() {
        ...
        for tail in [
            ...
            let query = prepare(&format!(
                "MATCH (a)-[:R]->(b) RETURN a,MIN(b.p) AS least GROUP BY a {tail} LIMIT 0"
            ));
            ...
            let result = query.execute_governed(
                ...
                |vid, _| {
                    reads += 1;
                    if vid == VId(12) {
                        Err("last root unreadable")
                    } else {
                        Ok(Some(if vid == VId(10) { &boolean } else { &integer }))
                    }
                },
                policy(),
                || Ok::<_, ()>(()),
        ...
        let query = prepare(
            "MATCH (a)-[:R]->(b) RETURN a,MIN(b.p) AS least,SUM(b.q) AS total \
        ...
        let result = query.execute_governed(
            ...
            |vid, key| {
                Ok(Some(
                    if (vid == VId(10) && key == PropertyKeyId(1))
                        || (vid == VId(12) && key == PropertyKeyId(2))
                    {
                        &boolean
                    } else {
                        &integer
    ...
    fn every_streaming_update_retirement_and_output_checkpoint_is_cancellable() {
        let query = prepare(&format!(
            "{HEAD} HAVING total IS NOT NULL ORDER BY mean DESC SKIP 1 LIMIT 2"
        ));
