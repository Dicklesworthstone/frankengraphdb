...
impl GraphHavingExpression {
    pub fn prepare(program: &[GraphHavingOp]) -> Result<Self, GraphHavingError> {
        ...
        if depth != 1 {
            return Err(GraphHavingError::InvalidStack {
    ...
    pub(super) fn validate_columns(
        ...
        let validate = |operand: &GraphHavingOperand| {
            if let GraphHavingOperand::Column(column) = operand {
                ...
                if !valid {
                    return Err(GraphHavingError::UnknownColumn { column: *column });
                }
            }
            Ok(())
        };
        for op in self.program.iter() {
            match op {
                GraphHavingOp::Compare { left, right, .. } => {
                    validate(left)?;
                    validate(right)?;
                }
            ...
            let value = match op {
                ...
                GraphHavingOp::And | GraphHavingOp::Or => {
                    ...
                    stack[depth - 1] = match (op, left, right) {
                        (GraphHavingOp::And, Some(false), _)
                        | (GraphHavingOp::And, _, Some(false)) => Some(false),
                        (GraphHavingOp::Or, Some(true), _) | (GraphHavingOp::Or, _, Some(true)) => {
                            Some(true)
                        }
                        (_, Some(left), Some(right)) => Some(if matches!(op, GraphHavingOp::And) {
                            left && right
                        } else {
                            left || right
                        }),
...
fn normalize(operand: &GraphHavingOperand) -> GraphHavingOperand {
    match operand {
        GraphHavingOperand::Scalar(value) => {
            GraphHavingOperand::Scalar(value.with_comparison(IntegerComparison::Equal))
        }
        value => value.clone(),
    }
}
...
fn append_operand(operand: &GraphHavingOperand, bytes: &mut Vec<u8>) {
    match operand {
        ...
        GraphHavingOperand::Integer(value) => {
            bytes.push(2);
...
impl PreparedGraphAggregate {
    ...
    {
        let mut govern = |event| -> Result<
            _,
            GqlQueryError<GraphAggregateError<core::convert::Infallible>, C>,
...
mod tests {
    ...
    fn expression(ops: &[Op]) -> GraphHavingExpression {
        GraphHavingExpression::prepare(ops).unwrap()
    }
    fn evaluate(ops: &[Op], group: Group<'_, '_>) -> bool {
        expression(ops)
            .evaluate(group, &mut |_| {
    ...
    fn postfix_validation_and_complete_three_valued_truth_tables() {
        ...
        for (i, left) in values.iter().copied().enumerate() {
            assert_eq!(
                evaluate(&[Op::Truth(left), Op::Not], group),
                left == Some(false)
            );
            for (j, right) in values.iter().copied().enumerate() {
                for (op, expected) in [(Op::And, values[and[i][j]]), (Op::Or, values[or[i][j]])] {
                    assert_eq!(
                        evaluate(&[Op::Truth(left), Op::Truth(right), op.clone()], group),
                        expected == Some(true)
                    );
                    assert_eq!(
                        evaluate(&[Op::Truth(left), Op::Truth(right), op, Op::Not], group),
                        expected == Some(false)
                    );
                }
            }
        ...
        for ops in [
            vec![],
            vec![Op::Not],
            vec![Op::Truth(None), Op::And],
            vec![Op::Truth(None), Op::Truth(None)],
        ] {
            assert!(GraphHavingExpression::prepare(&ops).is_err());
        }
        let mut too_many = vec![Op::Truth(Some(true)); MAX_AGGREGATE_FILTERS + 1];
        too_many.extend(vec![Op::And; MAX_AGGREGATE_FILTERS]);
        assert!(matches!(
            GraphHavingExpression::prepare(&too_many),
            Err(GraphHavingError::TooManyPredicates { .. })
        ));
        let mut longest = vec![Op::Truth(Some(true))];
        longest.extend(vec![Op::Not; MAX_HAVING_INSTRUCTIONS - 1]);
        assert!(GraphHavingExpression::prepare(&longest).is_ok());
        longest.push(Op::Not);
        assert!(matches!(
            GraphHavingExpression::prepare(&longest),
            Err(GraphHavingError::TooManyInstructions { .. })
        ));
    }
    ...
    fn count_sum_and_scalar_integers_compare_without_narrowing() {
        ...
        for left in choices {
            for right in choices {
                ...
                for (comparison, expected) in [
                    ...
                    assert_eq!(
                        evaluate(
                            &[cmp(Arg::Column(A(0)), comparison, Arg::Column(A(1)))],
                            group
                        ),
                        expected
                    );
                    assert_eq!(
                        evaluate(
                            &[
                                cmp(Arg::Integer(left), comparison, Arg::Column(A(1))),
                                Op::Not
                            ],
                            group
                        ),
        ...
        assert!(evaluate(
            &[cmp(
                Arg::Column(K(0)),
                IntegerComparison::Equal,
                Arg::Integer(i128::from(i64::MIN))
            )],
            Group {
                key: &[ValueRef::Scalar(&scalar)],
                state: &[]
            }
        ));
    }
    ...
    fn null_and_incompatible_values_remain_unknown_under_not() {
        let wanted =
            ScalarPredicate::new(CanonicalScalar::Bool(true), IntegerComparison::Equal).unwrap();
        for actual in [
    ...
    fn schema_and_replacement_are_checked_without_changing_existing_definitions() {
        ...
        let changed = base
            ...
            .unwrap();
        assert!(changed.having().is_empty());
        assert_eq!(changed.having_expression(), Some(&expr));
        ...
        for column in [K(1), A(2)] {
            let invalid = expression(&[cmp(
                Arg::Integer(1),
                IntegerComparison::Equal,
                Arg::Column(column),
            )]);
            assert!(matches!(
                base.clone().with_having_expression(&invalid),
                Err(GraphHavingError::UnknownColumn { .. })
            ));
        }
        let secret = ScalarPredicate::new(
            CanonicalScalar::ucs_basic_text("private-value").unwrap(),
            IntegerComparison::Less,
        )
        .unwrap();
        let a = expression(&[Op::IsNull {
            operand: Arg::Scalar(secret.clone()),
            is_null: false,
        }]);
        let b = expression(&[Op::IsNull {
            operand: Arg::Scalar(secret.with_comparison(IntegerComparison::Greater)),
            is_null: false,
        }]);
        assert_eq!(a, b);
        assert!(!format!("{a:?}").contains("private-value"));
    }
    ...
    fn filtering_precedes_pages_and_every_checkpoint_without_cloning_payloads() {
        ...
        for ordered in [false, true] {
            ...
            if ordered {
                query = query
                    .with_result_clauses(&[], &[GraphAggregateOrder::descending(A(1))])
                    .unwrap();
            }
            ...
            let measured = query
                ...
                .unwrap();
            let ids: Vec<_> = measured
                .iter()
                .map(|row| row.keys()[0].as_vertex().unwrap())
                .collect();
            assert_eq!(
                ids,
                if ordered {
            ...
            for stop in 1..=calls {
                let mut at = 0;
                let result = query.finish_groups(&groups, &mut |_| {
                    at += 1;
                    if at == stop {
    ...
    fn decisive_truth_and_zero_pages_cannot_hide_invalid_numeric_domains() {
        ...
        groups.insert(
            ...
            vec![Accumulator::Count(0), Accumulator::Extreme(None)],
        );
        for (truth, op) in [(false, Op::And), (true, Op::Or)] {
            let expr = expression(&[
                Op::Truth(Some(truth)),
                cmp(Arg::Column(K(0)), IntegerComparison::Equal, Arg::Integer(1)),
                op,
            ]);
                ...
                for ordered in [false, true] {
                    let query = definition(u64::MAX, count)
                        .with_result_clauses(
                            &[],
                            &if ordered {
                                vec![GraphAggregateOrder::ascending(A(0))]
                            } else {
                                vec![]
                            },
                        )
    ...
    fn maintained_having_uses_exact_averages_and_checks_complete_row_schema() {
        ...
        for numerator in [i128::MIN, -7, 0, 7, i128::MAX] {
            for denominator in [1, 2, u64::MAX] {
                ...
                for threshold in [i128::MIN, -2, 0, 2, i128::MAX] {
                    let filter = GraphAggregateFilter {
                        column: A(0),
                    ...
                    let boolean = base
                        .clone()
                        .with_having_expression(&expression(&[cmp(
                            Arg::Column(A(0)),
                            IntegerComparison::Greater,
                            Arg::Integer(threshold),
                        )]))
                        .unwrap();
                    for query in [ordinary, boolean] {
                        ...
                        let row = query
                            .materialize_incremental_row(
                                vec![],
                                vec![
                                    GraphAggregateValue::Average(average),
                        ...
                        assert_eq!(
                            query
                                .evaluate_incremental_having(&row, &mut |_| Ok::<_, ()>(()))
                                .unwrap(),
                        ...
                        assert_eq!(
                            query
                                .evaluate_incremental_having(&malformed, &mut |_| Ok::<_, ()>(()))
                                .unwrap(),
                            None
                        );
                    }
        ...
        for query in [
            ...
            base.with_result_clauses(&[], &[GraphAggregateOrder::ascending(A(0))])
                .unwrap(),
            definition(1, None),
            definition(0, Some(0)),
        ] {
    ...
    fn maintained_having_borrows_payloads_and_every_checkpoint_is_retryable() {
        ...
        let boolean = base
            .materialize_incremental_row(
                vec![GraphValue::Scalar(CanonicalScalar::Bool(true))],
                vec![GraphAggregateValue::Count(1)],
            )
            .unwrap();
        for (truth, op) in [(true, Op::Or), (false, Op::And)] {
            let invalid = query
                ...
                .unwrap();
            assert!(matches!(
                invalid.evaluate_incremental_having(&boolean, &mut |_| Ok::<_, ()>(())),
                Err(GqlQueryError::Source(
                    GraphAggregateError::NonIntegerHaving { predicate: 1 }
                ))
            ));
