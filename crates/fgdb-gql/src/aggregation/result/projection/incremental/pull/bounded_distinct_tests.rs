...
fn row(key: i64, class: u64, rank: Option<(i128, u64)>) -> GraphAggregateRow {
    GraphAggregateRow::from_group_values(
        ...
        vec![
            GraphAggregateValue::Count(class),
            rank.map_or(
...
fn check_index(ranking: &mut StreamedGroupRanking) {
    ...
    for (at, row) in ranking.heap.iter().enumerate() {
        assert_eq!(
            index
                .position(row.distinct_key.as_ref().unwrap(), &mut |_| Ok::<_, ()>(()))
                .unwrap(),
            Some(at)
        );
    }
...
fn improved_residents_and_evicted_classes_update_their_actual_heap_slots() {
    let q = definition(0, Some(2), false, GraphNullPlacement::Last)
    ...
    for (key, class, rank) in [
        ...
        ranking
            .push(&q, row(key, class, Some((rank, 1))), &mut control)
            .unwrap();
        check_index(&mut ranking);
    }
...
fn numeric_equivalence_keeps_the_best_ranked_concrete_representation_and_not_enum_order() {
    ...
    let mut control = |_| Ok::<_, QueryError<(), ()>>(());
    ranking
        .push(&q, row(0, 2, Some((4, 2))), &mut control)
        .unwrap(); // Count(2)
    ranking
        .push(&q, row(1, 3, Some((6, 3))), &mut control)
        .unwrap(); // Average(2/1), better rank
    ranking
        .push(&q, row(2, 4, Some((3, 2))), &mut control)
        .unwrap(); // Distinct 3/2, ranks first
    check_index(&mut ranking);
    ...
    let keys: BTreeSet<_> = values
        ...
        .map(|v| {
            DistinctIndex::key(
                &GraphAggregateRow::from_group_values(vec![], vec![v]),
                &mut |_| Ok::<_, ()>(()),
            )
            .unwrap()
        })
...
fn duplicate_zero_column_output_and_huge_windows_do_not_allocate_by_limit() {
    for skip in [0, 1, u64::MAX] {
        ...
        if skip == 0 {
            assert!(actual[0].keys().is_empty() && actual[0].values().is_empty());
        }
    }
}
...
fn finite_distinct_prefix_never_retains_all_losing_output_classes() {
    let q = definition(1, Some(2), true, GraphNullPlacement::Last)
        .prepare_streamed_output()
        .unwrap();
    ...
    for key in 0..4096_i64 {
        ranking
            .push(
                &q,
                row(key, key as u64, Some((i128::from(key), 1))),
                &mut control,
            )
            .unwrap();
        check_index(&mut ranking);
    ...
    assert_eq!(
        classes(&ranking.finish(&q, &mut control).unwrap()),
        [4094, 4093]
    );
}
...
fn every_distinct_key_index_swap_and_selection_refusal_aborts_and_retries() {
    let q = definition(1, Some(1), false, GraphNullPlacement::First)
        .prepare_streamed_output()
        .unwrap();
    let run = |stop| {
        let mut calls = 0;
...
fn duplicate_or_evicted_outputs_never_hide_late_expression_errors_even_at_limit_zero() {
    for (skip, take) in [(0, Some(1)), (0, Some(0)), (u64::MAX, Some(u64::MAX))] {
            ...
            .with_output_projection(vec![GraphSetProjection::new(
                ...
                GraphSetValue::Integer(
                    GraphIntegerExpression::prepare(&[
                        ...
                        Op::Column(1),
                        Op::Literal(Some(1)),
                        Op::Binary(GraphIntegerBinary::Subtract),
        ...
        for key in 0..2 {
            ranking
                .push(&q, row(key, 2, Some((i128::from(key), 1))), &mut control)
                .unwrap();
        }
        assert!(matches!(
            ranking.push(&q, row(2, 1, Some((100, 1))), &mut control),
