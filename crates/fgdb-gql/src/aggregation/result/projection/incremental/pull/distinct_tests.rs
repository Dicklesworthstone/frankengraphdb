...
fn allow(_: GlaExecutionEvent) -> Result<(), QueryError<Infallible, usize>> {
    Ok(())
}
fn definition() -> PreparedGraphAggregate {
    ...
    let input = builder
        ...
        .unwrap()
        .with_duplicates();
    PreparedGraphAggregate::prepare(input, &[0], &[GraphAggregate::sum_int("sum", 1)], 0, None)
        .unwrap()
}
...
fn distinct_representative_uses_hidden_rank_and_retains_concrete_numeric_domains() {
    ...
    .unwrap();
    let expression =
        GraphIntegerExpression::prepare(&[Op::Column(2), Op::Column(1), Op::Coalesce]).unwrap();
    let input = vec![
        GraphAggregateRow::from_group_values(
            row(0, None).keys.into_vec(),
            vec![
...
fn duplicate_groups_retain_one_class_and_do_not_crowd_a_distinct_page() {
    ...
    assert_eq!(
        plain(&state.finish(&query, &mut allow).unwrap()),
        vec![Some(7), Some(-1)]
    );
    ...
    assert_eq!(
        plain(
            &select(
                &query,
                vec![row(2, Some(-1)), row(1, Some(7)), row(0, Some(7))],
                &mut allow
            )
            .unwrap()
        ),
...
fn losing_or_offpage_output_errors_are_not_hidden_by_distinct_or_empty_prefixes() {
    ...
    for (offset, count) in [(0, 0), (0, 1), (u64::MAX, u64::MAX)] {
        ...
        assert!(matches!(
            select(&query, input.clone(), &mut allow),
