...
impl PreparedGraphAggregate {
    ...
    pub fn project_incremental_output<C>(
        ...
        for _ in row.keys.iter() { govern(GlaExecutionEvent::Work)?; }
        for _ in row.values.iter() { govern(GlaExecutionEvent::Work)?; }
        if !self.accepts_incremental_row(&row.keys, &row.values) { return Ok(None); }
        let input = |column: usize| {
            if column < row.keys.len() {
                Ok(Cell::Value(value_ref(&row.keys[column])))
            } else {
                row.values.get(column - row.keys.len()).map(result_cell)
        ...
        if let Some(projection) = &self.output_projection {
            for (column, output) in projection.iter().enumerate() {
                govern(GlaExecutionEvent::ScratchEntry)?;
                values.push(expression(output.value(), input, column, &mut govern)?
        ...
        Ok(Some(GraphAggregateRow {
            keys: keys.into_boxed_slice(), values: values.into_boxed_slice(),
        }))
    }
}
...
impl GraphAggregateRow {
    ...
    pub fn incremental_distinct_key<E>(
        ...
        for value in &self.values {
            ...
            values.push(match cell.numeric() {
                Some((sum, count)) => {
                    ...
                    for _ in 0..128 { control(GlaExecutionEvent::Work)?; }
        ...
        control(GlaExecutionEvent::Work)?;
        Ok(Self { keys: keys.into_boxed_slice(), values: values.into_boxed_slice() })
    }
}
...
mod tests {
    ...
    use crate::{GraphIntegerBinary, GraphIntegerExpression, GraphIntegerOp as Op,
        GraphSetProjection};
    ...
    fn output_split_preserves_having_and_projects_full_exact_cells() {
        let original = definition().with_result_clauses(&[GraphAggregateFilter {
            column: GraphAggregateColumn::Aggregate(0), test: GraphAggregateTest::IsNotNull,
        ...
        let row = source.materialize_incremental_row(
            vec![GraphValue::Scalar(CanonicalScalar::Int(1))],
            vec![GraphAggregateValue::Count(u64::MAX), GraphAggregateValue::Integer(i128::MAX),
                GraphAggregateValue::Average(GraphExactAverage::new(3, 2).unwrap())],
        ).unwrap();
        let empty = original.project_incremental_output(&row, &mut |_| Ok::<_, ()>(())).unwrap().unwrap();
        assert!(empty.keys().is_empty() && empty.values().is_empty());
        let expressions = original.with_output_projection(vec![
        ...
        let projected = expressions.project_incremental_output(&row, &mut |_| Ok::<_, ()>(())).unwrap().unwrap();
        assert!(projected.keys().is_empty());
        assert_eq!(projected.values(), row.values());
        let reordered = definition().with_key_output_columns(&[0, 0]).unwrap()
            .with_aggregate_output_prefix(1).unwrap();
    ...
    fn distinct_key_matches_numeric_equivalence_but_preserves_original_variants() {
        let choices = [GraphAggregateValue::Count(2), GraphAggregateValue::Integer(2),
        ...
        for a in &choices {
            for b in &choices {
                let row = |value: GraphAggregateValue| GraphAggregateRow {
                    keys: Box::new([]), values: vec![value].into_boxed_slice(),
                };
                let left = row(a.clone());
                let right = row(b.clone());
                let expected = compare_cell(result_cell(a), result_cell(b), false,
                    GraphNullPlacement::First, &mut |_| Ok::<_, ()>(())).unwrap() == Ordering::Equal;
                assert_eq!(left.incremental_distinct_key(&mut |_| Ok::<_, ()>(())).unwrap()
                    == right.incremental_distinct_key(&mut |_| Ok::<_, ()>(())).unwrap(), expected);
                assert_eq!(left.values(), &[a.clone()]);
            }
        }
    }
    ...
    fn output_failures_and_every_projection_checkpoint_are_explicit_and_retryable() {
        let base = definition();
        let row = base.materialize_incremental_row(
            vec![GraphValue::Scalar(CanonicalScalar::Int(1))],
            vec![GraphAggregateValue::Count(2), GraphAggregateValue::Integer(7),
                GraphAggregateValue::Average(GraphExactAverage::new(7, 2).unwrap())],
        ).unwrap();
        let expression = GraphIntegerExpression::prepare(&[
            Op::Column(2), Op::Literal(Some(3)), Op::Binary(GraphIntegerBinary::Multiply),
        ]).unwrap();
        let query = base.clone().with_output_projection(vec![
            GraphSetProjection::new("scaled", GraphSetValue::Integer(expression)),
        ]).unwrap();
        let mut calls = 0;
        let expected = query.project_incremental_output(&row, &mut |_| {
            calls += 1; Ok::<_, usize>(())
        }).unwrap().unwrap();
        ...
        for stop in 1..=calls {
            ...
            assert_eq!(query.project_incremental_output(&row, &mut |_| Ok::<_, usize>(())).unwrap().unwrap(), expected);
        }
        let invalid = GraphIntegerExpression::prepare(&[
            Op::Column(2), Op::Literal(Some(0)), Op::Binary(GraphIntegerBinary::Divide),
        ]).unwrap();
        let query = base.with_output_projection(vec![
            GraphSetProjection::new("ok", GraphSetValue::Column(1)),
            GraphSetProjection::new("invalid", GraphSetValue::Integer(invalid)),
        ]).unwrap();
        assert!(matches!(query.project_incremental_output(&row, &mut |_| Ok::<_, ()>(())),
            Err(GqlQueryError::Source(GraphAggregateError::OutputExpression { column: 1, .. }))));
    }
}
