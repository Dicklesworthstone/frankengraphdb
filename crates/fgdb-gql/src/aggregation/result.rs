...
pub enum GraphAggregateTest {
    Integer { comparison: IntegerComparison, value: i128 },
    IsNull,
    IsNotNull,
}
impl core::fmt::Debug for GraphAggregateTest {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::Integer { comparison, .. } => f.debug_struct("Integer")
                .field("comparison", comparison).field("value", &"[REDACTED]").finish(),
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum GraphNullPlacement { First, Last }
...
impl GraphAggregateOrder {
    ...
    pub const fn ascending(column: GraphAggregateColumn) -> Self {
        Self { column, descending: false, nulls: GraphNullPlacement::Last }
    }
    #[must_use]
    pub const fn descending(column: GraphAggregateColumn) -> Self {
        Self { column, descending: true, nulls: GraphNullPlacement::Last }
    }
}
...
impl<'a> Cell<'a> {
    fn from_state(state: &Accumulator<'a>) -> Self {
        match state {
            ...
            Accumulator::Sum { value, present: true } => Self::Integer(*value),
    ...
    fn is_null(self) -> bool { matches!(self, Self::Value(value) if value.is_null()) }
    ...
    fn payload_units(self) -> usize {
        match self { Self::Value(value) => value.payload_units(), _ => 0 }
    }
...
impl<'a> Group<'_, 'a> {
    ...
    fn copy_owned<E>(self, control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), E>)
        -> Result<GraphAggregateRow, E> {
        control(GlaExecutionEvent::ResultRow)?;
        control(GlaExecutionEvent::ScratchEntry)?;
        let mut keys = Vec::new();
        for cell in self.key { keys.push(cell.copy_owned(control)?); }
        ...
        Ok(GraphAggregateRow { keys: keys.into_boxed_slice(), values: values.into_boxed_slice() })
    }
}
...
impl PreparedGraphAggregate {
    ...
    pub fn with_result_clauses(mut self, having: &[GraphAggregateFilter], ordering: &[GraphAggregateOrder])
        -> Result<Self, GraphAggregateBuildError> {
        if having.len() > MAX_AGGREGATE_FILTERS {
            return Err(GraphAggregateBuildError::TooManyFilters { limit: MAX_AGGREGATE_FILTERS, observed: having.len() });
        }
        if ordering.len() > MAX_PATTERN_VERTICES {
            return Err(GraphAggregateBuildError::TooManyColumns { limit: MAX_PATTERN_VERTICES, observed: ordering.len() });
        }
        let validate = |column| {
            ...
            if valid { Ok(()) } else { Err(GraphAggregateBuildError::UnknownOutputColumn { column }) }
        };
        for filter in having { validate(filter.column)?; }
        for (at, order) in ordering.iter().enumerate() {
            validate(order.column)?;
            if ordering[..at].iter().any(|previous| previous.column == order.column) {
                return Err(GraphAggregateBuildError::DuplicateOrder { column: order.column });
            }
        }
    ...
    #[must_use]
    pub fn having(&self) -> &[GraphAggregateFilter] { &self.having }
    #[must_use]
    pub fn ordering(&self) -> &[GraphAggregateOrder] { &self.ordering }

    pub(super) fn append_result_transcript(&self, bytes: &mut Vec<u8>) {
        if self.having.is_empty() && self.ordering.is_empty() { return; }
    ...
    fn keep<E, C>(&self, group: Group<'_, '_>,
    ...
    fn compare<E>(&self, left: Group<'_, '_>, right: Group<'_, '_>,
        control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), E>) -> Result<Ordering, E> {
        for order in &self.ordering {
            control(GlaExecutionEvent::Work)?;
            ...
            let result = match (a.is_null(), b.is_null()) {
                ...
                (true, false) => if order.nulls == GraphNullPlacement::First { Ordering::Less } else { Ordering::Greater },
                (false, true) => if order.nulls == GraphNullPlacement::First { Ordering::Greater } else { Ordering::Less },
                (false, false) => {
                    for _ in 0..a.payload_units().max(b.payload_units()) { control(GlaExecutionEvent::Work)?; }
                    let result = a.compare(b);
                    if order.descending { result.reverse() } else { result }
                }
            };
    ...
    fn sift_down<E>(&self, heap: &mut [Group<'_, '_>], mut root: usize, end: usize,
        control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), E>) -> Result<(), E> {
        while root < end / 2 {
            let mut child = 2 * root + 1;
            if child + 1 < end && self.compare(heap[child], heap[child + 1], control)? == Ordering::Less {
                child += 1;
            }
            if self.compare(heap[root], heap[child], control)? != Ordering::Less { break; }
            control(GlaExecutionEvent::Work)?;
    ...
    pub(super) fn finish_groups<'a, E, C>(&self, groups: &BTreeMap<Vec<ValueRef<'a>>, Vec<Accumulator<'a>>>,
        control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), GqlQueryError<GraphAggregateError<E>, C>>)
        ...
        if self.ordering.is_empty() {
            ...
            if self.having.is_empty() {
                ...
                for (key, state) in groups {
                    ...
                    if output.len() < count { output.push(group.copy_owned(control)?); }
                }
            }
            return Ok(output);
        }
        let prefix = if count == 0 || offset >= groups.len() { 0 }
            else { offset.saturating_add(count).min(groups.len()) };
        let mut heap = Vec::new();
        for (key, state) in groups {
            control(GlaExecutionEvent::Work)?;
            let candidate = Group { key, state };
            if !self.keep(candidate, control)? || prefix == 0 { continue; }
            if heap.len() < prefix {
                control(GlaExecutionEvent::ScratchEntry)?;
                heap.push(candidate);
                ...
                while child > 0 {
                    ...
                    if self.compare(heap[parent], heap[child], control)? != Ordering::Less { break; }
                    control(GlaExecutionEvent::Work)?;
...
mod tests {
    ...
    fn numeric(column: GraphAggregateColumn, comparison: IntegerComparison, value: i128) -> GraphAggregateFilter {
        GraphAggregateFilter { column, test: GraphAggregateTest::Integer { comparison, value } }
    }
    fn ids(rows: &[GraphAggregateRow]) -> Vec<u128> {
        rows.iter().map(|row| row.keys()[0].as_vertex().unwrap().0).collect()
    }
    ...
    fn run(query: &PreparedGraphAggregate, groups: &Groups<'_>) -> Vec<GraphAggregateRow> {
        query.finish_groups(groups, &mut |_| Ok::<_, GqlQueryError<GraphAggregateError<()>, ()>>(()))
            .unwrap()
    }
    ...
    fn clauses_validate_all_columns_and_do_not_mutate_existing_definitions() {
        ...
        assert_eq!(base.clone().with_result_clauses(&[], &[]).unwrap().canonical_bytes(), bytes);
        let filter = numeric(GraphAggregateColumn::Aggregate(0), IntegerComparison::GreaterOrEqual, i128::MAX);
        let order = GraphAggregateOrder::descending(GraphAggregateColumn::Aggregate(1));
        let changed = base.clone().with_result_clauses(&[filter], &[order]).unwrap();
        ...
        assert!(matches!(base.clone().with_result_clauses(
            &[numeric(GraphAggregateColumn::Aggregate(2), IntegerComparison::Equal, 0)], &[]),
            Err(GraphAggregateBuildError::UnknownOutputColumn { .. })));
        assert!(matches!(base.clone().with_result_clauses(&[],
            &[GraphAggregateOrder::ascending(GraphAggregateColumn::GroupKey(1))]),
            Err(GraphAggregateBuildError::UnknownOutputColumn { .. })));
        assert!(matches!(base.clone().with_result_clauses(&[filter; MAX_AGGREGATE_FILTERS + 1], &[]),
            Err(GraphAggregateBuildError::TooManyFilters { .. })));
        assert!(matches!(base.clone().with_result_clauses(&[], &[order, order]),
            Err(GraphAggregateBuildError::DuplicateOrder { .. })));
        let mut null_first = order;
        null_first.nulls = GraphNullPlacement::First;
        assert_ne!(base.clone().with_result_clauses(&[], &[order]).unwrap().canonical_bytes(),
            base.clone().with_result_clauses(&[], &[null_first]).unwrap().canonical_bytes());
        assert_ne!(base.clone().with_result_clauses(&[filter], &[]).unwrap().canonical_bytes(),
            base.with_result_clauses(&[numeric(filter.column, IntegerComparison::Less, i128::MAX)], &[])
                .unwrap().canonical_bytes());
    }
    ...
    fn bounded_selection_matches_independent_full_sort_filter_and_slice() {
        ...
        for mut encoded in 0..4_usize.pow(4) {
            let mut values = Vec::new();
            for _ in 0..4 { values.push(choices[encoded % 4]); encoded /= 4; }
            let input = groups(&values);
            for descending in [false, true] {
                for nulls in [GraphNullPlacement::First, GraphNullPlacement::Last] {
                    let mut expected: Vec<_> = (0..values.len()).filter(|at| at % 3 + 1 >= 2).collect();
                    expected.sort_by(|a, b| {
                        ...
                        order.then_with(|| (a % 3 + 1).cmp(&(b % 3 + 1))).then_with(|| a.cmp(b))
                    });
                    for offset in 0..=6 {
                        for count in [None, Some(0), Some(1), Some(2), Some(6), Some(u64::MAX)] {
                            let query = definition(offset, count).with_result_clauses(&[
                                numeric(GraphAggregateColumn::Aggregate(0), IntegerComparison::GreaterOrEqual, 2),
                            ], &[
                            ...
                            let expected: Vec<_> = expected.iter().skip(offset as usize)
                                .take(count.and_then(|value| usize::try_from(value).ok()).unwrap_or(usize::MAX))
                                .map(|at| *at as u128).collect();
    ...
    fn having_runs_before_offset_and_result_limits_without_requiring_ordering() {
        ...
        let filter = numeric(GraphAggregateColumn::Aggregate(1), IntegerComparison::GreaterOrEqual, 2);
        assert_eq!(ids(&run(&definition(1, Some(1)).with_result_clauses(&[filter], &[]).unwrap(), &input)), vec![2]);
        let is_null = GraphAggregateFilter { column: GraphAggregateColumn::Aggregate(1), test: GraphAggregateTest::IsNull };
        assert_eq!(ids(&run(&definition(0, None).with_result_clauses(&[is_null], &[]).unwrap(), &input)), vec![3]);
        let nonnull = GraphAggregateFilter { test: GraphAggregateTest::IsNotNull, ..is_null };
        assert_eq!(ids(&run(&definition(0, None).with_result_clauses(&[nonnull], &[]).unwrap(), &input)), vec![0, 1, 2]);
        let ne = numeric(is_null.column, IntegerComparison::NotEqual, 2);
        assert_eq!(ids(&run(&definition(0, None).with_result_clauses(&[ne], &[]).unwrap(), &input)), vec![0, 2]);
    }

    #[test]
    fn numeric_predicates_preserve_full_width_counts_and_sums_and_refuse_bad_types() {
        let key = [ValueRef::Vertex(VId(1))];
        let states = [Accumulator::Count(u64::MAX), Accumulator::Sum { value: i128::MIN, present: true }];
        let group = Group { key: &key, state: &states };
        let query = definition(0, None).with_result_clauses(&[
            numeric(GraphAggregateColumn::Aggregate(0), IntegerComparison::Equal, i128::from(u64::MAX)),
            numeric(GraphAggregateColumn::Aggregate(1), IntegerComparison::Less, i128::MAX),
        ], &[]).unwrap();
        assert!(query.keep(group, &mut |_| Ok::<_, GqlQueryError<GraphAggregateError<()>, ()>>(()))
            .unwrap());
        ...
        input.insert(vec![ValueRef::Scalar(&boolean)], vec![Accumulator::Count(0), Accumulator::Sum { value: 0, present: false }]);
        for count in [None, Some(0)] {
            let query = definition(u64::MAX, count).with_result_clauses(&[
                numeric(GraphAggregateColumn::Aggregate(0), IntegerComparison::Greater, 0),
                numeric(GraphAggregateColumn::GroupKey(0), IntegerComparison::Equal, 1),
            ], &[GraphAggregateOrder::ascending(GraphAggregateColumn::Aggregate(0))]).unwrap();
            assert!(matches!(query.finish_groups(&input, &mut |_| Ok::<_, GqlQueryError<GraphAggregateError<()>, ()>>(())),
    ...
    fn every_comparison_swap_filter_and_copy_can_be_interrupted() {
        ...
        let query = definition(1, Some(3)).with_result_clauses(&[
        ...
        let mut events = 0;
        let complete = query.finish_groups(&input, &mut |_| {
            events += 1; Ok::<_, GqlQueryError<GraphAggregateError<()>, usize>>(())
        }).unwrap();
        ...
        for stop in 1..=events {
            ...
            let refused = query.finish_groups(&input, &mut |_| {
                at += 1;
                if at == stop { Err(GqlQueryError::<GraphAggregateError<()>, _>::Interrupted(stop)) } else { Ok(()) }
            });
    ...
    fn ranking_reserves_only_the_page_prefix_and_copies_only_returned_payloads() {
        ...
        let mut input = Groups::new();
        for at in 0..1000 {
            input.insert(vec![ValueRef::Vertex(VId(at))],
                vec![Accumulator::Count(at as u64), Accumulator::Extreme(Some(ValueRef::Scalar(&payload)))]);
        }
        let query = definition(3, Some(2)).with_result_clauses(&[], &[
            GraphAggregateOrder::descending(GraphAggregateColumn::Aggregate(0)),
        ]).unwrap();
        let mut scratch = 0;
        let result = query.finish_groups(&input, &mut |event| {
        ...
        for row in &result { assert_eq!(row.get(1).unwrap().as_value().unwrap().as_scalar(), Some(&payload)); }
    }
}
