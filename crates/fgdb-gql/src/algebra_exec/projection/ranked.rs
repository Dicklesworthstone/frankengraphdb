...
impl<Row> RowKey for Ranked<Row> {
    fn width(&self) -> usize {
        (self.view)(&self.row).width()
    }
    fn cell(&self, at: usize) -> ValueRef<'_> {
        (self.view)(&self.row).cell(at)
    }
}
...
fn compare_rows(left: &dyn RankedKey, right: &dyn RankedKey) -> Ordering {
    ...
    for column in left.order() {
        ...
        let null_order = match (is_null(a), is_null(b)) {
            (true, false) => Some(if column.nulls_first {
                Ordering::Less
            } else {
                Ordering::Greater
            }),
            (false, true) => Some(if column.nulls_first {
                Ordering::Greater
            } else {
                Ordering::Less
            }),
...
impl<'a, Row: 'a> Borrow<dyn RankedKey + 'a> for Ranked<Row> {
    fn borrow(&self) -> &(dyn RankedKey + 'a) {
        self
    }
}
...
impl ProjectedRows<GraphValueRow> {
    pub(crate) fn should_retain_value<E>(
        ...
        if *distinct && rows.contains(&lookup as &dyn RankedKey) {
            return Ok(false);
        }
        ...
        let largest = rows
            .last()
            .expect("a full nonzero prefix has a largest row");
        if compare_rows(largest, &lookup) != Ordering::Greater {
            return Ok(false);
        }
    ...
    pub(crate) fn insert_value(&mut self, row: GraphValueRow) {
        ...
        debug_assert!(
            inserted,
            "complete admitted rows have unique DISTINCT keys or live ALL slots"
        );
    }
}
...
mod tests {
    use super::*;
    use crate::algebra::{GlaOperator, GraphColumn, GraphPatternBuilder};
        ...
        let p = b
            .prepare_values(
                &[
                    ...
                    GraphColumn::property("rank", "n", PropertyKeyId(1)),
                ],
                0,
                None,
            )
    ...
    fn private_ranked_prefix_matches_full_sort_with_unique_live_occurrence_slots() {
        ...
        for distinct in [false, true] {
            for capacity in 0..=4 {
                let mut rows = ProjectedRows::<GraphValueRow>::for_plan(
                    distinct,
                    &[
                        GlaOperator::OrderByValueColumns {
                            columns: Arc::clone(&order),
                        },
                        GlaOperator::Limit {
                            offset: 0,
                            count: Some(capacity),
                ...
                for at in [3, 1, 4, 4, 0, 0, 2, 2, 1, 0, 4, 2] {
                    ...
                    let Storage::Ranked { rows: live, .. } = &rows.storage else {
                        unreachable!()
                    };
                    assert!(live.iter().all(|entry| Arc::ptr_eq(&entry.order, &order)));
                    if !distinct {
                        let ids: std::collections::BTreeSet<_> =
                            live.iter().map(|entry| entry.slot).collect();
    ...
    fn private_ordered_cutoff_is_transactional_at_every_decision_checkpoint() {
        ...
        let mut rows = ProjectedRows::<GraphValueRow>::for_plan(
            false,
            &[
                GlaOperator::OrderByValueColumns { columns: order },
                GlaOperator::Limit {
                    offset: 0,
                    count: Some(1),
                },
        ...
        assert!(
            rows.should_retain_value(&input[1], &mut |_| Ok::<_, usize>(()))
                .unwrap()
        );
        rows.insert_value(input[1].clone());
        let mut total = 0;
        assert!(
            rows.should_retain_value(&input[0], &mut |_| {
                total += 1;
                Ok::<_, usize>(())
            })
            .unwrap()
        ...
        assert!(
            !rows
                .should_retain_value(&input[3], &mut |_| Ok::<_, usize>(()))
                .unwrap()
        );
    }
}
