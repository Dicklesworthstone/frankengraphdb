...
use crate::free_join::{FreeJoin, FreeJoinPlan, GraphTrie, GraphTrieError, JoinVariable, MAX_JOIN_VARIABLES};
...
pub(super) fn compile<E>(
    ...
    if !matches!(operators.first(), Some(GlaOperator::ScanEdges { .. })) { return Ok(None); }
    let mut width = 2;
    let mut projection = None;
    for (at, operator) in operators.iter().enumerate().skip(1) {
        match operator {
            GlaOperator::Expand { source, .. } if (source.ordinal() as usize) < width && width < MAX_JOIN_VARIABLES => {
                width += 1;
            }
            GlaOperator::VertexIdentity { left, right, .. }
                if (left.ordinal() as usize) < width && (right.ordinal() as usize) < width => {}
            GlaOperator::Project { slot } if (slot.ordinal() as usize) < width => { projection = Some(at); break; }
            GlaOperator::ProjectBindings { slots }
                if !slots.is_empty() && slots.iter().all(|slot| (slot.ordinal() as usize) < width) => {
                projection = Some(at); break;
            }
            GlaOperator::ProjectValues { columns }
                if !columns.is_empty() && columns.iter().all(|column| matches!(column,
    ...
    if width < 4 { return Ok(None); }
    let mut tail = projection + 1;
    let distinct = matches!(operators.get(tail), Some(GlaOperator::Distinct));
    if distinct { tail += 1; }
    let ordered = matches!((&operators[projection], operators.get(tail)),
        (GlaOperator::Project { .. }, Some(GlaOperator::OrderByVertexId))
        | (GlaOperator::ProjectBindings { .. }, Some(GlaOperator::OrderByBindings))
        | (GlaOperator::ProjectValues { .. }, Some(GlaOperator::OrderByValues | GlaOperator::OrderByValueColumns { .. })));
    if !ordered || operators.len() != tail + 2 { return Ok(None); }
    let Some(GlaOperator::Limit { offset, count }) = operators.last() else { return Ok(None); };
    let page_capacity = if *count == Some(0) { Some(0) } else { count.and_then(|count| offset.checked_add(count)) };

    let mut parents: [usize; MAX_JOIN_VARIABLES] = core::array::from_fn(|at| at);
    for operator in &operators[1..projection] {
        control(GlaExecutionEvent::Work)?;
        if let GlaOperator::VertexIdentity { left, right, equal: true } = operator {
    ...
    for operator in &operators[..projection] {
        ...
        let (a, b, relation, direction) = match operator {
            GlaOperator::ScanEdges { relation, direction } => (0, 1, *relation, *direction),
            GlaOperator::Expand { source, relation, direction } => {
                let target = appended;
                appended += 1;
                (source.ordinal() as usize, target, *relation, *direction)
            }
            GlaOperator::VertexIdentity { left, right, equal: false } => {
                control(GlaExecutionEvent::ScratchEntry)?;
        ...
        schemas.push(if a == b { vec![a] } else { vec![a.min(b), a.max(b)] });
        accesses.push(GlaOperator::ScanEdges { relation, direction });
    }
    ...
    let physical = FreeJoinPlan::generic(schemas, order).expect("validated endpoint equivalence classes form a covered join");
    Ok(Some(Topology { physical, accesses, slots, inequalities, projection, distinct, page_capacity }))
}
...
impl Topology {
        ...
        C: FnMut(GlaExecutionEvent) -> Result<(), E>,
    {
        ...
        for (at, access) in self.accesses.iter().enumerate() {
            control(GlaExecutionEvent::ScratchEntry)?;
            let GlaOperator::ScanEdges { relation, direction } = access else { unreachable!() };
            let adjacency = index.get(&(*relation, *direction)).expect("every requested orientation was registered");
            let order = self.physical.required_order(at).expect("one schema per access");
            ...
            let trie = if order.len() == 1 {
                ...
                admitted(GraphTrie::adjacency([order[0], order[1]], adjacency, 0, control))?
            };
            tries.push(trie);
        }
        let join = FreeJoin::new(&self.physical, &tries).expect("each graph access advertises its required order");
        ...
        join.for_each_binding(control, |binding, control| {
            ...
            for &(left, right) in &self.inequalities {
                control(GlaExecutionEvent::Work)?;
                if slots[left] == slots[right] { return Ok(()); }
            }
            let mut emit = |control: &mut C| {
                control(GlaExecutionEvent::Work)?;
                Row::collect_properties(
                    &operators[self.projection], &slots[..self.slots.len()], &mut projected,
            ...
            if self.distinct {
                ...
                let copies = binding.multiplicity_factors().iter().fold(1_u128, |n, &weight| {
                    n.checked_mul(weight as u128).unwrap_or(u128::from(capacity)).min(u128::from(capacity))
                });
                for _ in 0..copies { emit(control)?; }
            } else {
        ...
        let Some(GlaOperator::Limit { offset, count }) = operators.last() else { unreachable!() };
        let offset = usize::try_from(*offset).unwrap_or(usize::MAX);
        let count = count.and_then(|n| usize::try_from(n).ok()).unwrap_or(usize::MAX);
