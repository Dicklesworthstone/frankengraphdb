...
fn triangle(directions: [GlaDirection; 3]) -> GraphPatternBuilder {
    ...
    builder
        .edge("a", RelationId(1), directions[0], "b")
        .unwrap();
    builder
        .edge("b", RelationId(2), directions[1], "c")
        .unwrap();
    builder
        .edge("c", RelationId(3), directions[2], "a")
        .unwrap();
    builder
}
fn forward() -> GraphPatternBuilder {
    triangle([GlaDirection::Forward; 3])
}
            ...
            for source in [VId(0), VId(1), VId(u128::MAX)] {
                for target in [VId(0), VId(1), VId(u128::MAX)] {
                    ...
                    for _ in 0..(seed >> 32) % 3 {
                        input.push((source, RelationId(relation), target));
                    }
                }
            }
        }
                    ...
                    for duplicate in [false, true] {
                        for (offset, count) in [(0, None), (1, Some(3)), (99, Some(0))] {
                            let mut prepared = builder
                                .prepare_bindings(&["b", "a", "c"], offset, count)
                                .unwrap();
                            if duplicate {
                            ...
                            let result = plan
                                .execute_governed(
                                    ...
                                    [],
                                    input.iter().copied(),
                                    |_, _| -> Result<bool, Infallible> {
                                        panic!("topology has no predicate reads")
                                    },
                                    wide(input.len()),
                                    || Ok::<_, Infallible>(()),
                                )
...
fn equality_classes_diagonals_and_inequalities_preserve_real_occurrences() {
    for equal in [false, true] {
        ...
        let prepared = builder
            .prepare_bindings(&["a", "b", "c"], 0, None)
            .unwrap()
            .with_duplicates();
        let input = edges();
        let plan = prepared.plan();
        let expected = plan
            .execute([], input.clone(), |_, _| Ok::<_, Infallible>(true))
            .unwrap();
        let result = plan
            .execute_governed(
                input.len() as u64,
...
fn all_public_governed_value_adapters_select_the_same_topology_plan() {
    ...
    let identified = || {
        input
            ...
            .copied()
            .enumerate()
            .map(|(at, (s, r, d))| (EId(at as u128), s, r, d))
    };
    let ids = plan
...
fn scopes_reads_paths_and_nonterminal_shapes_never_enter_the_join_island() {
    ...
    for barrier in [
        ...
        assert!(
            compile(&operators, &mut |_| -> Result<(), Infallible> {
                panic!("unselected lane must retain its event trace")
            })
            .unwrap()
            .is_none()
        );
    ...
    properties
        .filter("b", VertexPredicate::HasLabel(LabelId(9)))
        .unwrap();
    let prepared = properties.prepare("a", 0, None).unwrap();
    assert!(
        compile(prepared.plan().operators(), &mut allow)
            .unwrap()
            .is_none()
    );
    let result = prepared.plan().execute_governed(
        8,
        [],
        edges(),
        |_, _| Err("source predicate failed"),
        wide(8),
        || Ok::<_, Infallible>(()),
    );
    assert!(matches!(
        result,
        Err(GqlQueryError::Source("source predicate failed"))
    ));
    let prepared = forward()
        .prepare_values(
    ...
    let result = prepared.plan().execute_governed_with_properties(
        ...
        edges(),
        |_, _| Ok(true),
        |_, _| Err("source property failed"),
        wide(8),
        || Ok::<_, Infallible>(()),
    );
    assert!(matches!(
        result,
        Err(GqlQueryError::Source("source property failed"))
    ));
}
...
fn admission_is_before_compilation_and_input_is_consumed_exactly_once() {
    ...
    let rejected = prepared.plan().execute_governed(
        8,
        [],
        input(),
        |_, _| Ok::<_, Infallible>(true),
    ...
    let budgeted = prepared
        ...
        .execute_budgeted(
            ...
            edges(),
            |_, _| Ok::<_, Infallible>(true),
            crate::GqlExecutionBudget::new(8, 1),
        )
        .unwrap();
...
fn every_selected_checkpoint_refuses_without_further_work_or_partial_success() {
    let prepared = forward()
        .prepare_bindings(&["a", "b", "c"], 0, Some(3))
        .unwrap()
        .with_duplicates();
    let mut total = 0;
...
fn finite_pages_and_distinct_do_not_expand_exponential_parallel_edge_products() {
    ...
    for _ in 0..50 {
        builder
            .edge("a", RelationId(1), GlaDirection::Forward, "a")
            .unwrap();
    }
    let input = vec![(VId(0), RelationId(1), VId(0)); 100];
    for duplicate in [false, true] {
        let mut prepared = builder
            .prepare("a", 0, if duplicate { Some(2) } else { None })
            .unwrap();
        if duplicate {
            prepared = prepared.with_duplicates();
        }
        ...
        let result = prepared
            ...
            .execute_governed(
                ...
                input.clone(),
                |_, _| Ok::<_, Infallible>(true),
                GqlQueryPolicy::new(100, 2, 100_000, 100_000),
                || Ok::<_, Infallible>(()),
            )
            .unwrap();
