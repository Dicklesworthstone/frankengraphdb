...
fn decode(input: &[u8], width: usize, limits: CsvRecordLimits) -> Result<Vec<CsvRecord>, CsvRecordError> {
    ...
    if let Some(row) = decoder.finish()? { rows.push(row); }
    assert_eq!(decoder.finish()?, None);
    Ok(rows)
}
fn texts(rows: &[CsvRecord]) -> Vec<Vec<&str>> {
    rows.iter().map(|row| row.fields().iter().map(CsvField::text).collect()).collect()
}

#[test]
fn every_chunk_width_preserves_multiline_utf8_quotes_crlf_and_quote_provenance() {
    let source = b"a,b,c\r\n\"one, two\",\"a\"\"b\",\"\"\n,\\N,\"\\N\"\r\n\"line\r\nline\nend\",\"\r\",last";
    let expected = vec![vec!["a", "b", "c"], vec!["one, two", "a\"b", ""],
        vec!["", "\\N", "\\N"], vec!["line\r\nline\nend", "\r", "last"]];
    ...
    for width in 1..=utf8.len() {
        assert_eq!(texts(&decode(utf8, width, CsvRecordLimits::default()).unwrap()),
            vec![vec!["雪🙂λ", "🙂"]]);
    }
}
...
fn blank_records_trailing_empty_fields_and_eof_are_distinct() {
    for (source, expected) in [
        ...
        assert_eq!(texts(&decode(source.as_bytes(), 1, CsvRecordLimits::default()).unwrap()), expected);
    }
...
fn malformed_source_never_exposes_the_unfinished_record_and_remains_terminal() {
    for (source, kind) in [
        (b"a,\"open".as_slice(), CsvRecordErrorKind::UnterminatedQuote),
        ...
        for width in 1..=source.len() {
            assert_eq!(decode(source, width, CsvRecordLimits::default()).unwrap_err().kind, kind);
        }
        ...
        assert_eq!(decoder.push(b"safe\n").unwrap_err().kind, CsvRecordErrorKind::Terminated);
        assert_eq!(decoder.finish().unwrap_err().kind, CsvRecordErrorKind::Terminated);
    }
}
...
fn exact_inclusive_limits_count_decoded_bytes_and_complete_raw_records() {
    let source = "\"λ\"\"\",x\r\n";
    let exact = CsvRecordLimits { max_record_bytes: source.len(), max_field_bytes: 3, max_columns: 2 };
    assert_eq!(texts(&decode(source.as_bytes(), 1, exact).unwrap()), vec![vec!["λ\"", "x"]]);
    for (limits, kind) in [
        (CsvRecordLimits { max_record_bytes: exact.max_record_bytes - 1, ..exact },
            CsvRecordErrorKind::RecordBytes { limit: exact.max_record_bytes - 1 }),
        (CsvRecordLimits { max_field_bytes: 2, ..exact }, CsvRecordErrorKind::FieldBytes { limit: 2 }),
        (CsvRecordLimits { max_columns: 1, ..exact }, CsvRecordErrorKind::Columns { limit: 1 }),
        (CsvRecordLimits { max_columns: 0, ..exact }, CsvRecordErrorKind::Columns { limit: 0 }),
    ] {
        assert_eq!(decode(source.as_bytes(), source.len(), limits).unwrap_err().kind, kind);
    }
    let empty = CsvRecordLimits { max_record_bytes: 1, max_field_bytes: 0, max_columns: 1 };
    assert!(decode(b"\n\n", 2, empty).is_ok(), "quota resets at a complete record only");
    assert!(decode(b"x", 1, empty).is_err());
    assert!(decode(b"", 1, CsvRecordLimits { max_record_bytes: 0, ..empty }).is_ok());
    assert!(decode(b"\n", 1, CsvRecordLimits { max_record_bytes: 0, ..empty }).is_err());
}
...
fn prefix_delivery_stops_at_the_exact_boundary_and_error_coordinates_are_absolute() {
    ...
    assert_eq!(decoder.position(), CsvPosition { record: 1, column: 0, offset: 5 });
    assert_eq!(decoder.push(b"x,\"bad").unwrap(), (6, None));
    let error = decoder.finish().unwrap_err();
    assert_eq!(error.position, CsvPosition { record: 1, column: 1, offset: 11 });
...
fn framer_matches_decoder_for_exhaustive_small_ascii_inputs() {
    ...
    for length in 0..=6u32 {
        for mut code in 0..alphabet.len().pow(length) {
            ...
            for _ in 0..length { source.push(alphabet[code % alphabet.len()]); code /= alphabet.len(); }
            let mut framer = CsvRecordFramer::new(CsvRecordLimits::default());
            let framed = (|| {
                let mut records = 0;
                for &byte in &source { records += usize::from(framer.push(byte)?); }
...
fn decoding_matches_existing_typed_csv_profile_without_interpolating_values() {
    ...
    let schema = [GqlParameterSpec { name: "value".into(),
        parameter_type: GqlParameterType::Scalar(CanonicalScalarKind::Text),
        requires_positive: false, occurrences: 1 }];
    for value in ["", "simple", "雪🙂", "a,b", "a\"b", "a\r\nb", "\\N", "';DELETE n;"] {
        let source = format!("value\n\"{}\"\n", value.replace('"', "\"\""));
        let actual = decode(source.as_bytes(), 1, CsvRecordLimits::default()).unwrap();
        let parameters = decode_csv_parameters(&source, &schema, CsvParameterLimits::default()).unwrap();
        assert_eq!(actual.len(), 2);
        assert_eq!(parameters, vec![GqlParameters::new().with_text("value", actual[1].fields()[0].text()).unwrap()]);
    }
}
