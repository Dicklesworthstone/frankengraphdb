    ...
    for kind in [RowJoinKind::Right, RowJoinKind::Full] {
        for cross in [false, true] {
            ...
            assert_eq!(
                ...
                vec![GraphSetColumnType::Scalar; 4]
            );
            for before in 0..81 {
...
fn right_and_full_keep_asymmetric_schema_order_and_native_payloads() {
    ...
    let left =
        GraphValueRow::from_owned_values(vec![payload.clone(), id.clone(), scalar(Some(11))]);
    let right = GraphValueRow::from_owned_values(vec![id.clone(), scalar(Some(22))]);
    let expected = GraphValueRow::from_owned_values(vec![
        payload,
        id.clone(),
        scalar(Some(11)),
        id.clone(),
    ...
    let unmatched = GraphValueRow::from_owned_values(vec![
        scalar(None),
        scalar(None),
        scalar(None),
        id,
...
fn empty_tuple_cross_joins_preserve_outer_occurrences_and_promoted_products() {
    ...
    for kind in [RowJoinKind::Right, RowJoinKind::Full] {
        ...
        for left in 0..=2 {
            for right in 0..=2 {
                let expected = if left > 0 && right > 0 {
                    left * right
                } else if right > 0 {
                    right
                } else if kind == RowJoinKind::Full {
                    left
                } else {
                    0
                };
...
fn negative_rows_cannot_hide_in_positive_key_totals_and_diagnostics_keep_original_sides() {
    for kind in [RowJoinKind::Right, RowJoinKind::Full] {
        ...
        for side in 0..2 {
            ...
            assert_eq!(
                state
                    .prepare(left, right, LIMBS, Some(0), &mut allow)
                    .unwrap_err(),
...
fn full_join_quota_applies_to_consolidated_final_bag_and_refusal_is_retryable() {
    ...
    assert_eq!(
        state
            .prepare(&ZSet::new(), &right, LIMBS, Some(5), &mut allow)
            .unwrap_err(),
        RowJoinError::ResultBudget { limit: 5 }
    );
    assert_eq!(state, seed(&spec, &left, &ZSet::new()));
    state
        .prepare(&ZSet::new(), &right, LIMBS, Some(6), &mut allow)
        .unwrap()
        .commit();
    let replacement = z([(row(Some(9), 21), 3)]);
    let dr = replacement.minus(&right, LIMBS, &mut allow).unwrap();
    state
        .prepare(&ZSet::new(), &dr, LIMBS, Some(5), &mut allow)
        .unwrap()
        .commit();
    assert_eq!(
        state.rows(),
        &oracle(&left, &replacement, RowJoinKind::Full, false)
    );
...
fn replacing_same_key_witness_rows_never_exposes_a_spurious_null_extension() {
    ...
    for kind in [RowJoinKind::Right, RowJoinKind::Full] {
        ...
        for next in [
            ...
            assert_eq!(
                state
                    .prepare(&delta, &ZSet::new(), LIMBS, None, &mut allow)
                    .unwrap()
                    .commit(),
