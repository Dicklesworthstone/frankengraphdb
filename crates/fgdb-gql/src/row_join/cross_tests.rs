...
fn z(rows: impl IntoIterator<Item = (GraphValueRow, i128)>) -> ZSet<GraphValueRow> {
    ZSet::from_updates(
        rows.into_iter().map(|(r, w)| (r, ZWeight::from_i128(w))),
        LIMBS,
        &mut allow,
    )
    .unwrap()
...
fn bag(code: usize) -> ZSet<GraphValueRow> {
    z([
        (row(None), (code % 3) as i128),
        (row(Some(7)), (code / 3) as i128),
    ])
}
...
fn seed(
    ...
    state
        .prepare(left, right, LIMBS, None, &mut allow)
        .unwrap()
        .commit();
    state
}
fn plain(rows: &ZSet<GraphValueRow>) -> BTreeMap<GraphValueRow, i128> {
    rows.iter()
        .map(|(r, w)| (r.clone(), w.to_i128().unwrap()))
        .collect()
}
fn oracle(
    left: &ZSet<GraphValueRow>,
    ...
    for (l, lw) in left.iter() {
        ...
        match kind {
            RowJoinKind::Semi if !right.is_empty() => {
                result.insert(l.clone(), lw);
            }
            RowJoinKind::Anti if right.is_empty() => {
                result.insert(l.clone(), lw);
            }
            ...
            RowJoinKind::Inner | RowJoinKind::Left => {
                for (r, rw) in right.iter() {
                    ...
                    *result
                        .entry(GraphValueRow::from_owned_values(values))
                        .or_insert(0) += lw * rw.to_i128().unwrap();
                }
            }
            _ => {}
        }
    ...
    for kind in [
        ...
        for before in 0..81 {
            ...
            for after in 0..81 {
                ...
                assert_eq!(
                    plain(state.rows()),
                    oracle(&next_left, &next_right, kind),
                    "{kind:?}: {before}->{after}"
                );
...
fn products_preserve_native_payloads_and_empty_tuple_multiplicity_without_expansion() {
    ...
    let definition =
        RowJoinSpec::cross(&[GraphSetColumnType::List], &[GraphSetColumnType::Any]).unwrap();
    assert!(definition.is_cross());
    assert!(definition.keys().is_empty());
    assert_eq!(
        definition.column_types().collect::<Vec<_>>(),
        vec![GraphSetColumnType::List, GraphSetColumnType::Any]
    );
    let mut state = IncrementalRowJoin::new(definition);
    state
        .prepare(
            &z([(left.clone(), i128::MAX)]),
            &z([(right.clone(), 2)]),
            LIMBS,
            None,
            &mut allow,
    ...
    let mut state =
        IncrementalRowJoin::new(RowJoinSpec::cross(&[], &[GraphSetColumnType::Scalar]).unwrap());
    state
        .prepare(&z([(unit, 1)]), &bag(8), LIMBS, None, &mut allow)
        .unwrap()
        .commit();
...
fn every_callback_refusal_drop_and_unwind_preserves_both_inputs_output_and_total() {
    ...
    let (dl, dr) = (
        bag(7).minus(&left, LIMBS, &mut allow).unwrap(),
...
fn empty_opposite_input_and_zero_quota_do_not_hide_invalid_source_rows_or_retractions() {
    ...
    assert!(matches!(
        state.prepare(&z([(wrong, 1)]), &ZSet::new(), LIMBS, Some(0), &mut allow),
    ...
    let mut state =
        IncrementalRowJoin::new(RowJoinSpec::cross(&[GraphSetColumnType::Any], &[]).unwrap());
    let invalid = GraphValueRow::from_owned_values(vec![nested]);
    assert!(matches!(
        state.prepare(&z([(invalid, 1)]), &ZSet::new(), LIMBS, Some(0), &mut allow),
        Err(RowJoinError::InputSchema { side: 0 })
    ));
    assert!(state.rows().is_empty());
}
...
fn final_occurrence_quota_is_atomic_and_work_does_not_expand_duplicate_counts() {
    ...
    for count in [1, 1_000_000] {
        ...
        state
            .prepare(
                &z([(row(Some(1)), -count), (row(Some(3)), count)]),
                &ZSet::new(),
                LIMBS,
                None,
                &mut |event| {
...
fn explicit_cross_admission_does_not_turn_invalid_equijoins_into_products() {
    ...
    assert_eq!(
        RowJoinSpec::new(&[T::Scalar], &[T::Scalar], &[]).unwrap_err(),
        RowJoinBuildError::EmptyKeys
    );
    assert_eq!(
        RowJoinSpec::new(&[], &[T::Scalar], &[(0, 0)]).unwrap_err(),
        RowJoinBuildError::EmptyInput
    );
    assert!(matches!(
        RowJoinSpec::cross(&[T::Any; MAX_PATTERN_VERTICES], &[T::Scalar]),
