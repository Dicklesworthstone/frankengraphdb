...
fn less(left: usize, right: usize) -> GraphSetPredicateOp {
    GraphSetPredicateOp::Compare {
        left: GraphSetOperand::Column(left),
...
fn spec(kind: RowJoinKind, keyed: bool) -> RowJoinSpec {
    ...
    let spec = if keyed {
        ...
        RowJoinSpec::cross(&schema, &schema)
    }
    .unwrap();
...
fn oracle(
    kind: RowJoinKind,
    keyed: bool,
    left: &[(GraphValueRow, i128)],
    right: &[(GraphValueRow, i128)],
) -> ZSet<GraphValueRow> {
    ...
    let joined = |l: Option<&GraphValueRow>, r: Option<&GraphValueRow>| {
        ...
        for side in [l, r] {
            if let Some(side) = side {
                values.extend_from_slice(side.values());
            } else {
    ...
    for (l, lw) in left.iter().filter(|(_, count)| *count > 0) {
        let witnesses: Vec<_> = right
            .iter()
            .filter(|(r, rw)| *rw > 0 && matches(l, r))
            .collect();
        if matches!(
            kind,
            RowJoinKind::Inner | RowJoinKind::Left | RowJoinKind::Right | RowJoinKind::Full
        ) {
            for (r, rw) in &witnesses {
                output.push((joined(Some(l), Some(r)), lw * rw));
            }
        }
        match kind {
        ...
        for keyed in [false, true] {
            for old in 0_u8..16 {
                for new in 0_u8..16 {
                    ...
                    join.prepare(&bag(&left), &bag(&right), LIMBS, None, &mut allow)
                        .unwrap()
                        .commit();
                    let before = oracle(kind, keyed, &left, &right);
                    assert_eq!(join.rows(), &before);
                    let dl = bag(&next_left)
                        .minus(&bag(&left), LIMBS, &mut allow)
                        .unwrap();
                    let dr = bag(&next_right)
                        .minus(&bag(&right), LIMBS, &mut allow)
                        .unwrap();
                    let expected = oracle(kind, keyed, &next_left, &next_right);
                    let change = join
                        .prepare(&dl, &dr, LIMBS, None, &mut allow)
                        .unwrap()
                        .commit();
...
fn outer_on_is_not_a_post_filter_and_witnesses_are_row_specific() {
    ...
    for kind in KINDS {
        for keyed in [false, true] {
            ...
            join.prepare(&ZSet::new(), &delta, LIMBS, None, &mut allow)
                .unwrap()
                .commit();
            assert_eq!(
                join.rows(),
                &oracle(kind, keyed, &left, &[right[0].clone(), right[2].clone()])
            );
        }
    }
}
    ...
    for kind in KINDS {
        for truth in [None, Some(false), Some(true)] {
            let spec = RowJoinSpec::cross(&[], &[])
                .unwrap()
                .with_predicate(&[GraphSetPredicateOp::Truth(truth)])
                .unwrap()
                .with_kind(kind);
            assert!(!spec.is_cross());
            let mut join = IncrementalRowJoin::new(spec);
            let count = match (kind, truth == Some(true)) {
                (RowJoinKind::Inner, false)
                | (RowJoinKind::Semi, false)
                | (RowJoinKind::Anti, true) => 0,
                (RowJoinKind::Semi, true)
                | (RowJoinKind::Left, false)
                | (RowJoinKind::Anti, false) => 2,
...
fn null_keys_never_match_and_null_predicate_payloads_remain_unknown_under_not() {
    ...
    let keyed_row = GraphValueRow::from_owned_values(vec![
        null.clone(),
        GraphValue::Scalar(CanonicalScalar::Int(1)),
    ]);
    for kind in KINDS {
        let definition = RowJoinSpec::new(
        ...
        .unwrap()
        .with_kind(kind)
        .with_predicate(&[GraphSetPredicateOp::Truth(Some(true))])
        .unwrap();
        let mut join = IncrementalRowJoin::new(definition);
        join.prepare(
            &bag(&[(keyed_row.clone(), 2)]),
            &bag(&[(keyed_row.clone(), 3)]),
            LIMBS,
            None,
            &mut allow,
        )
    ...
    let mut join = IncrementalRowJoin::new(
        RowJoinSpec::cross(&schema, &schema)
            .unwrap()
            .with_predicate(&code)
            .unwrap()
            .with_kind(RowJoinKind::Left),
    );
    let l = row(&[1]);
    for value in [
        null.clone(),
        ...
        for b in 0..flat.len() {
            let code = [
                ...
                GraphSetPredicateOp::IsNull {
                    operand: GraphSetOperand::Column(b),
                    is_null: true,
                },
                GraphSetPredicateOp::Or,
            ];
            GraphSetPredicateOp::validate_schema(&vec![GraphSetColumnType::Any; flat.len()], &code)
                .unwrap();
            let mut expected_events = Vec::new();
            let expected =
                GraphSetPredicateOp::evaluate_row_with_control(&code, &flat, &mut |event| {
            ...
            for split in 0..=flat.len() {
                ...
                let actual = GraphSetPredicateOp::evaluate_pair_with_control(
                    &code,
                    &left,
                    &right,
                    &mut |event| {
...
fn definition_and_hidden_input_admission_cannot_be_bypassed_by_false_on() {
    let definition =
        RowJoinSpec::cross(&[GraphSetColumnType::Scalar], &[GraphSetColumnType::Scalar]).unwrap();
    assert!(matches!(
        definition.clone().with_predicate(&[]),
        Err(RowJoinBuildError::Predicate(GraphSetFilterError::Empty))
    ));
    assert!(matches!(
        definition
            .clone()
            .with_predicate(&[GraphSetPredicateOp::Not]),
        Err(RowJoinBuildError::Predicate(
            GraphSetFilterError::InvalidStack { .. }
        ))
    ));
    assert!(matches!(
        definition.clone().with_predicate(&[less(0, 2)]),
        Err(RowJoinBuildError::Predicate(
    ...
    for kind in KINDS {
        ...
        for side in 0..2 {
            ...
            assert_eq!(
                join.prepare(&deltas[0], &deltas[1], LIMBS, Some(0), &mut allow)
                    .unwrap_err(),
...
fn selection_and_existence_do_not_multiply_rejected_or_witness_occurrences() {
    ...
    for (kind, truth, expected) in [
        (RowJoinKind::Inner, false, 0),
        (RowJoinKind::Left, false, 2),
        (RowJoinKind::Semi, true, 2),
        (RowJoinKind::Anti, true, 0),
    ] {
        let definition = RowJoinSpec::cross(&schema, &schema)
            .unwrap()
            .with_kind(kind)
            .with_predicate(&[GraphSetPredicateOp::Truth(Some(truth))])
            .unwrap();
        let mut join = IncrementalRowJoin::new(definition);
        join.prepare(
            &bag(&[(row(&[1]), 2)]),
...
fn every_refusal_and_dropped_guard_preserves_inputs_output_and_total_and_can_retry() {
    ...
    for kind in KINDS {
        ...
        if success.total() > &ZWeight::ZERO {
            assert_eq!(
                join.prepare(&left, &right, LIMBS, Some(0), &mut allow)
                    .unwrap_err(),
