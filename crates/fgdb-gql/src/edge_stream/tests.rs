...
fn statement(direction: GlaDirection, predicate: &str, all: bool, skip: u64, limit: u64) -> String {
    ...
    format!("MATCH (a){left}[r:R]{right}(b) {predicate} RETURN {}r, a, b, r.p AS ep, a.p AS ap, b.p AS bp SKIP {skip} LIMIT {limit}",
...
impl Fixture {
    fn new() -> Self {
        let vertices = BTreeMap::from([
            (IDS[0], (vec![LabelId(1)], vec![(P, CanonicalScalar::Int(4))])),
            (IDS[1], (vec![], vec![(P, CanonicalScalar::Null)])),
            (IDS[2], (vec![LabelId(1)], vec![(P, CanonicalScalar::Int(1))])),
        ]);
        ...
        ] { edges.insert(EId(id), (IDS[src], rel, IDS[dst], value.map(|v| vec![(P, v)]).unwrap_or_default())); }
        Self { ids: edges.keys().copied().collect(), at: 0, edges, vertices, fail: None,
            drops: Arc::new(AtomicUsize::new(0)), visits: Arc::new(AtomicUsize::new(0)) }
    }
...
impl EdgeScanSource for Fixture {
    ...
    fn next_edge<C>(&mut self, control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), C>)
        ...
        let id = self.ids.get(self.at).copied(); self.at += usize::from(id.is_some()); Ok(id)
    }
    fn edge<'a, C>(&'a self, id: EId, control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), C>)
        ...
        if self.fail == Some(id) { return Err(EdgeScanSourceError::Source("unreadable edge")); }
        Ok(self.edges.get(&id).map(|(src, rel, dst, props)| EdgeScanRow { source: *src, relation: *rel, target: *dst, properties: props }))
    }
    fn vertex<'a, C>(&'a self, id: VId, control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), C>)
        ...
        control(GlaExecutionEvent::Work).map_err(EdgeScanSourceError::Control)?;
        Ok(self.vertices.get(&id).map(|(labels, props)| VertexScanRow { labels, properties: props }))
    }
}
...
fn oracle(f: &Fixture, direction: GlaDirection, filter: usize, skip: usize, limit: usize) -> Vec<Vec<GraphValue>> {
    let scalar = |value: Option<&CanonicalScalar>| GraphValue::Scalar(value.cloned().unwrap_or(CanonicalScalar::Null));
    let mut rows = Vec::new();
    for (&id, (src, rel, dst, props)) in &f.edges {
        if *rel != R { continue; }
        let endpoints = match direction {
            GlaDirection::Forward => vec![(*src, *dst)], GlaDirection::Reverse => vec![(*dst, *src)],
        ...
        for (a, b) in endpoints {
            let accept = match filter {
                ...
                3 => a != b,
                _ => matches!((edge_value, f.value(a)), (Some(CanonicalScalar::Int(x)), Some(CanonicalScalar::Int(y))) if x > y),
            };
            if accept { rows.push(vec![GraphValue::Edge(id), GraphValue::Vertex(a), GraphValue::Vertex(b),
                scalar(edge_value), scalar(f.value(a)), scalar(f.value(b))]); }
        }
    }
...
fn directed_reverse_undirected_parallel_null_and_predicate_results_match_independent_and_eager_oracles() {
    for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
        for (filter, predicate) in ["", "WHERE r.p > 0", "WHERE r.p IS NULL", "WHERE a <> b", "WHERE r.p > a.p"].iter().enumerate() {
            for all in [false, true] {
                for (skip, limit) in [(0, 100), (1, 2), (2, 0), (99, 4)] {
                    ...
                    let expected = oracle(&fixture, direction, filter, skip as usize, limit as usize);
                    let eager = pattern.plan().execute_governed_with_element_properties(6,
                        fixture.vertices.keys().copied(), fixture.edges.iter().map(|(&id, (a, r, b, _))| (id, *a, *r, *b)),
                        |id, predicates| Ok::<_, ()>(predicates.iter().all(|p| p.matches(&fixture.vertices[&id].0, &fixture.vertices[&id].1))),
                        |id, _| Ok(fixture.value(id)), |id, _| Ok(fixture.edges[&id].3.first().map(|(_, v)| v)),
                        policy(), || Ok::<_, ()>(())).unwrap();
                    assert_eq!(plain(&eager.value), expected);
                    let mut cursor = EdgeScanCursor::new(fixture, plan, policy(), || Ok::<_, ()>(()));
...
fn unread_failing_suffix_zero_limit_and_early_close_do_not_drive_the_source() {
    for limit in [0, 1, 100] {
        ...
        let mut source = Fixture::new(); source.fail = Some(EId(1));
        let visits = source.visits.clone(); let drops = source.drops.clone();
        let mut cursor = EdgeScanCursor::new(source, EdgeScanPlan::compile(pattern.plan()).unwrap(), policy(), || Ok::<_, ()>(()));
        assert_eq!(visits.load(Ordering::SeqCst), 0);
        if limit != 0 { cursor.next().unwrap().unwrap(); }
        if limit == 100 { cursor.close(); }
        assert!(cursor.next().is_none());
...
fn exact_cumulative_limits_and_every_checkpoint_preserve_delivered_prefix_and_fuse() {
    let pattern = prepared(&statement(GlaDirection::Undirected, "WHERE r.p IS NULL OR r.p > 0", true, 0, 100));
    let plan = EdgeScanPlan::compile(pattern.plan()).unwrap();
    let checkpoints = std::cell::Cell::new(0);
    let mut baseline = EdgeScanCursor::new(Fixture::new(), plan.clone(), policy(), || { checkpoints.set(checkpoints.get()+1); Ok::<_, usize>(()) });
    ...
    for stop in 1..=checkpoints.get() {
        ...
        let fixture = Fixture::new(); let drops = fixture.drops.clone();
        ...
        assert!(cursor.next().is_none());
        assert_eq!(calls.get(), stop); assert_eq!(drops.load(Ordering::SeqCst), 1);
    }
...
fn invalid_sources_are_errors_not_missing_rows_and_debug_is_redacted() {
    ...
    for kind in 0..3 {
        ...
        match kind { 0 => fixture.ids.insert(1, EId(0)), 1 => { fixture.vertices.remove(&IDS[1]); }, _ => fixture.fail = Some(EId(1)) }
...
fn unsupported_shape_or_non_streamable_order_is_refused_instead_of_reordered() {
    for statement in [
        ...
        "MATCH (a) RETURN a",
    ] { assert!(EdgeScanPlan::compile(prepared(statement).plan()).is_err(), "accepted {statement}"); }
}
