...
mod saved_regressions {
    ...
    fn grouped_oracle(s: &Source, shape: usize, dir: GlaDirection, key_kind: usize) -> Vec<Answer> {
        ...
        for mut code in 0..domain.len().pow(width) {
            ...
            let choices: Vec<Vec<_>> = atoms[..=shape].iter().map(|&(from, rel, to)| {
                s.edges.iter().filter(|(_, (a, r, b, _))| *r == rel && match dir {
            ...
            let suffix = choices.iter().skip(1).map(|edges| edges.len() as u64).product::<u64>();
            if suffix == 0 { continue; }
            for &(eid, edge) in &choices[0] {
                let key = match key_kind {
                    0 => vec![GraphValue::Vertex(ids[0])],
                    1 => vec![GraphValue::Scalar(edge.3.iter().find(|(k, _)| *k == P)
                        .map_or(CanonicalScalar::Null, |(_, v)| v.clone())), GraphValue::Vertex(ids[width as usize - 1])],
        ...
        groups.into_iter().map(|(key, (count, present, total))| {
            (key, vec![GraphAggregateValue::Count(count), GraphAggregateValue::Count(present), sum(total)])
        }).collect()
    }
        ...
        for mask in 0..64 {
            for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
                ...
                for shape in 0..3 {
                    ...
                    for (key_kind, keys) in ["a".to_owned(), format!("r.p,{end}"), "r".to_owned()].iter().enumerate() {
                        let q = prepare(&format!("MATCH {pattern} RETURN {keys},COUNT(*) AS n,COUNT(r.p) AS present,SUM(r.p) AS total GROUP BY {keys}"));
                        let s = source(mask);
    ...
    fn canonical_null_and_full_width_edge_keys_are_owned_without_domain_coercion() {
        let q = prepare("MATCH (a)-[r:R]->(b) RETURN r.p,COUNT(*) AS n,COUNT(r.p) AS present GROUP BY r.p");
        let mut s = source(0);
        let data = [None, Some(CanonicalScalar::Null), Some(CanonicalScalar::Bool(false)),
            Some(CanonicalScalar::Int(0)), Some(CanonicalScalar::ucs_basic_text("secret key").unwrap())];
        for (id, value) in data.into_iter().enumerate() {
            s.edges.insert(EId(id as u128), (VId(0), R, VId(1), value.into_iter().map(|v| (P, v)).collect()));
        }
        let expected = eager(&q, &s);
        let dropped = s.drops.clone();
        let mut stream = run(&q, s, wide());
        let first = stream.next().unwrap().unwrap();
        assert_eq!(dropped.load(Ordering::SeqCst), 1, "source retained during group delivery");
        assert!(stream.input.source.is_none());
        assert!(!format!("{stream:?}").contains("secret key"));
        let mut actual = vec![first]; actual.extend(stream.by_ref().map(Result::unwrap));
        let null = actual.iter().find(|row| row.keys()[0].is_null()).unwrap();
        assert_eq!(null.values(), &[GraphAggregateValue::Count(2), GraphAggregateValue::Count(0)]);
        ...
        assert_eq!(rows.iter().map(|row| row.keys()[0].clone()).collect::<Vec<_>>(),
    ...
    fn late_source_data_and_group_quota_failures_precede_every_group() {
        ...
        for kind in 0..3 {
            let mut s = source(63);
            let dropped = s.drops.clone();
            let p = if kind == 2 { GqlQueryPolicy::new(1000, 1, u64::MAX, u64::MAX) } else { wide() };
            match kind {
                0 => s.fail = Some(EId(6)),
            ...
            match kind {
                0 => assert!(matches!(stream.next(), Some(Err(GqlQueryError::Source(GraphAggregateError::Source(EdgeScanError::Source("edge unavailable"))))))),
                1 => assert!(matches!(stream.next(), Some(Err(GqlQueryError::Source(GraphAggregateError::NonIntegerSum { aggregate: 1 }))))),
        ...
        assert!(empty.is_none(), "empty grouped input must not fabricate a global group");
    }

    #[test]
    fn every_checkpoint_and_exact_limit_preserves_only_completed_delivery_prefixes() {
        let q = prepare("MATCH (a)-[r:R]-(b) RETURN a,r.p,COUNT(*) AS n,SUM(r.p) AS total,AVG(r.p) AS mean,COUNT(DISTINCT r.p) AS distinct_values,MIN(r) AS first_edge GROUP BY a,r.p");
        ...
        let exact = GqlQueryPolicy::new(r.snapshot_records, r.result_rows, e.work_units, e.scratch_entries);
        assert_eq!(run(&q, source(63), exact).map(Result::unwrap).collect::<Vec<_>>(), expected);
        for limited in [
        ...
        let mut baseline = EdgeAggregateCursor::new(source(63), EdgeAggregatePlan::compile(&q).unwrap(), exact,
            || { calls.set(calls.get() + 1); Ok::<_, usize>(()) });
        assert_eq!(baseline.by_ref().map(Result::unwrap).collect::<Vec<_>>(), expected);
        let total = calls.get();
        drop(baseline);
        for stop in 1..=total {
            calls.set(0);
            let s = source(63); let dropped = s.drops.clone();
    ...
    fn thousands_of_join_occurrences_retain_only_four_numeric_groups_and_close_does_not_drain() {
        ...
        let mut s = source(0);
        for id in 0..4 { s.vertices.insert(VId(id), vec![]); }
        for id in 0..4096 {
            s.edges.insert(EId(id), (VId(0), R, VId(id % 4), vec![(P, CanonicalScalar::Int(i64::MAX))]));
        }
        let dropped = s.drops.clone();
        let mut cursor = run(&q, s, GqlQueryPolicy::new(4096, 4, 2_000_000, 2_000_000));
        let first = cursor.next().unwrap().unwrap();
        assert_eq!(first.keys(), &[GraphValue::Vertex(VId(0))]);
        assert_eq!(first.values(), &[GraphAggregateValue::Count(1024), GraphAggregateValue::Integer(1024 * i128::from(i64::MAX))]);
        ...
        let mut closed = run(&q, s, wide()); closed.close();
    ...
    fn all_shared_statistics_keep_independent_per_group_distinct_support_and_extrema() {
        let q = prepare("MATCH (a)-[r:R]-(b) RETURN b,COUNT(*) AS n,COUNT(r.p) AS present,COUNT(DISTINCT r.p) AS unique_values,SUM(r.p) AS total,SUM(DISTINCT r.p) AS distinct_total,AVG(r.p) AS mean,AVG(DISTINCT r.p) AS distinct_mean,MIN(r.p) AS low,MAX(r.p) AS high,MIN(r) AS first_edge,MAX(r) AS last_edge,COUNT(DISTINCT r) AS edges GROUP BY b");
        for mask in 0..64 {
            let s = source(mask);
            let expected = eager(&q, &s);
            assert_eq!(run(&q, s, wide()).map(Result::unwrap).collect::<Vec<_>>(), expected);
        }
        // The same value must be counted distinctly in EACH group, not globally.
        let q = prepare("MATCH (a)-[r:R]->(b) RETURN b,COUNT(DISTINCT r.p) AS unique_values,SUM(DISTINCT r.p) AS total,AVG(DISTINCT r.p) AS mean GROUP BY b");
        let mut s = source(0);
        for (id, target, value) in [(1, 0, 2), (2, 0, 2), (3, 0, 5), (4, 1, 2), (5, 1, 5), (6, 1, 5)] {
            s.edges.insert(EId(id), (VId(0), R, VId(target), vec![(P, CanonicalScalar::Int(value))]));
        }
        let actual = run(&q, s, wide()).map(Result::unwrap).collect::<Vec<_>>();
        assert_eq!(actual.len(), 2);
        for row in actual {
            assert_eq!(row.values(), &[
                GraphAggregateValue::Count(2), GraphAggregateValue::Integer(7),
