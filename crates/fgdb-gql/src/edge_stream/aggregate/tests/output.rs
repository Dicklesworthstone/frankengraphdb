                    ...
                    for nulls in ["FIRST", "LAST"] {
                        for window in ["", " LIMIT 0", " LIMIT 1", " SKIP 1 LIMIT 2"] {
                            ...
                            let q = prepare(&format!(
                                "MATCH {pattern} RETURN {end} AS key,COUNT(*) AS n,SUM(r.p) AS total,AVG(r.p) AS average GROUP BY {end} HAVING n>0 ORDER BY average {direction} NULLS {nulls},n DESC{window}"
                            ));
                            let before = q.canonical_bytes();
                            let s = source(mask);
                            let expected = eager(&q, &s);
...
fn ranked_cursor_refusals_drop_the_source_and_unpublished_page_at_every_checkpoint() {
    let q = prepare(
        "MATCH (a)-[r:R]-(b) RETURN b,COUNT(*)+1 AS n GROUP BY b HAVING COUNT(*)>0 ORDER BY COUNT(*) DESC,b DESC LIMIT 2",
    );
    let mut calls = 0;
    let mut full = EdgeAggregateCursor::new(
        source(63),
    ...
    let mut cursor = run(&q, source(63), exact);
    cursor.next().unwrap().unwrap();
...
fn rank_selection_cannot_conceal_late_source_having_or_output_expression_errors() {
    for limit in [0, 1] {
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN b,COUNT(*) AS n GROUP BY b ORDER BY n DESC LIMIT {limit}"
        ));
        let mut s = source(63);
        s.fail = Some(EId(6));
        let mut cursor = run(&q, s, wide());
        ...
        assert!(cursor.next().is_none());
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN b,MIN(r.p) AS m GROUP BY b HAVING m>0 OR TRUE ORDER BY m DESC LIMIT {limit}"
        ));
        let mut s = source(63);
        s.edges.get_mut(&EId(1)).unwrap().3 = vec![(
            P,
            CanonicalScalar::ucs_basic_text("private sort operand").unwrap(),
        )];
        let mut cursor = run(&q, s, wide());
        assert!(matches!(
            cursor.next(),
        ...
        assert!(cursor.next().is_none());
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN b,1/(COUNT(*)-1) AS bad GROUP BY b ORDER BY COUNT(*) DESC LIMIT {limit}"
        ));
        let mut s = source(0);
        for (id, to) in [(1, 0), (2, 0), (3, 1)] {
            s.edges.insert(EId(id), (VId(0), R, VId(to), vec![]));
...
fn ordered_rank_prefix_and_rejected_groups_do_not_consume_the_selected_row_allowance() {
    for skip in [0, 1, 1023, u64::MAX] {
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN b,COUNT(*) AS n GROUP BY b ORDER BY b DESC SKIP {skip} LIMIT 1"
        ));
        let mut s = source(0);
        for id in 0..1024 {
            s.vertices.insert(VId(id), vec![]);
        ...
        let mut cursor = run(
            &q,
            s,
            GqlQueryPolicy::new(1024, u64::from(skip != u64::MAX), u64::MAX, u64::MAX),
        );
        let rows = cursor.by_ref().collect::<Result<Vec<_>, _>>().unwrap();
        if skip == u64::MAX {
            ...
            assert_eq!(
                rows[0].keys(),
                &[GraphValue::Vertex(VId(u128::from(1023 - skip)))]
            );
        }
        assert_eq!(cursor.row_stats().result_rows, rows.len() as u64);
        assert_eq!(drops.load(Ordering::SeqCst), 1);
        ...
        for direction in 0..3 {
            for hops in 1..=2 {
                ...
                for projection in [
                    "COUNT(*) AS n",
                    "AVG(r.p) AS n",
                    "COALESCE(SUM(r.p),COUNT(*)) AS n",
                ] {
                    for ordering in ["", " ORDER BY n DESC NULLS LAST", " ORDER BY COUNT(*) DESC"] {
                        for window in ["", " LIMIT 0", " SKIP 1 LIMIT 2"] {
                            if projection.starts_with("COALESCE") && ordering.contains("ORDER BY n")
                            {
                                continue;
                            }
                            let text = format!(
                                "MATCH {pattern} RETURN DISTINCT {projection} GROUP BY {end}{ordering}{window}"
                            );
                            let q = prepare(&text);
                            let before = q.canonical_bytes();
                            let s = source(mask);
...
fn distinct_cursor_refusals_release_classes_source_and_undelivered_page() {
    ...
    for limit in [0, 1] {
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN DISTINCT SUM(r.p) AS total GROUP BY b ORDER BY total LIMIT {limit}"
        ));
        let mut s = source(63);
        s.fail = Some(EId(6));
        let mut cursor = run(&q, s, wide());
