...
fn summary(pattern: &str) -> PreparedGraphAggregate {
    prepare(&format!(
        "MATCH {pattern} RETURN COUNT(*) AS n,COUNT(r.p) AS present,COUNT(DISTINCT r.p) AS different,\
...
fn captured_path_distinct_and_extrema_do_not_reduce_identity_to_an_integer() {
    for text in [
        ...
        let expected = eager(&q, &s);
        assert_eq!(
            run(&q, s, policy()).next().unwrap().unwrap(),
            expected,
            "{text}"
        );
    }
...
fn late_nonnumeric_or_unavailable_input_does_not_expose_a_partial_distinct_count() {
    for function in ["AVG(r.p)", "SUM(DISTINCT r.p)", "AVG(DISTINCT r.p)"] {
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN COUNT(DISTINCT r) AS edges,{function} AS value"
        ));
        let mut s = source(63);
...
fn exact_distinct_average_and_scalar_extrema_keep_their_original_domains() {
    ...
    for (id, value) in [(0, i64::MAX), (1, i64::MAX - 1), (2, i64::MAX)] {
        s.edges.insert(
            EId(id),
            (VId(0), R, VId(1), vec![(P, CanonicalScalar::Int(value))]),
        );
    }
    let row = run(&q, s, policy()).next().unwrap().unwrap();
    assert_eq!(row.values()[2].as_count(), Some(2));
    assert_eq!(
        row.values()[5].as_average(),
        GraphExactAverage::new(3 * i128::from(i64::MAX) - 1, 3)
    );
    assert_eq!(
        row.values()[6].as_average(),
        GraphExactAverage::new(2 * i128::from(i64::MAX) - 1, 2)
    );
    let q = prepare(
        "MATCH (a)-[r:R]->(b) RETURN COUNT(DISTINCT r.p) AS different,MIN(r.p) AS lo,MAX(r.p) AS hi",
    );
    let mut s = source(63);
    s.edges.get_mut(&EId(1)).unwrap().3 =
        vec![(P, CanonicalScalar::ucs_basic_text(&"é".repeat(64)).unwrap())];
    s.edges.get_mut(&EId(6)).unwrap().3 =
        vec![(P, CanonicalScalar::bytes(vec![255; 512]).unwrap())];
    let expected = eager(&q, &s);
