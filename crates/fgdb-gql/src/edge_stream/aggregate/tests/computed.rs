...
use super::*;
use crate::{
    GraphAggregate, GraphExactAverage, GraphIntegerBinary, GraphIntegerExpression,
    GraphIntegerOp, GraphSetProjection, GraphSetValue,
};
...
fn eager(q: &PreparedGraphAggregate, s: &Source) -> Vec<GraphAggregateRow> {
    q.execute_governed_with_element_properties(
        s.edges.len() as u64, s.vertices.keys().copied(),
        s.edges.iter().map(|(&id, (a, r, b, _))| (id, *a, *r, *b)),
        |id, predicates| Ok::<_, ()>(predicates.iter().all(|p|
            p.matches_borrowed([], s.vertices[&id].iter().map(|(k, v)| (*k, v))))),
        |id, key| Ok(s.vertices[&id].iter().find(|(k, _)| *k == key).map(|(_, v)| v)),
        |id, key| Ok(s.edges[&id].3.iter().find(|(k, _)| *k == key).map(|(_, v)| v)),
        wide(), || Ok::<_, ()>(()),
    ).unwrap().value
}
fn projected(columns: Vec<GraphSetProjection>, keys: &[usize], specs: &[GraphAggregate<'_>]) -> PreparedGraphAggregate {
    let input = prepare("MATCH (a)-[r:R]->(b) RETURN SUM(r.p) AS total").input_pattern().clone();
...
fn oracle(s: &Source, hops: usize, dir: GlaDirection, grouped: bool) -> Vec<Answer> {
    ...
    if !grouped { groups.insert(vec![], vec![]); }
    for mut code in 0..domain.len().pow(width) {
        let mut ids = Vec::new();
        for _ in 0..width { ids.push(domain[code % domain.len()]); code /= domain.len(); }
        let mut choices = Vec::new();
        for &(from, relation, to) in &atoms[..hops] {
            let matching: Vec<_> = s.edges.values().filter(|(a, r, b, _)| {
                *r == relation && match dir {
                    GlaDirection::Forward => *a == ids[from] && *b == ids[to],
                    GlaDirection::Reverse => *b == ids[from] && *a == ids[to],
                    GlaDirection::Undirected => (*a == ids[from] && *b == ids[to]) || (*b == ids[from] && *a == ids[to]),
                }
            }).collect();
            choices.push(matching);
        }
        let suffix: usize = choices.iter().skip(1).map(Vec::len).product();
        if suffix == 0 || choices[0].is_empty() { continue; }
        let endpoint = ids[width as usize - 1];
        let key = if grouped { vec![GraphValue::Vertex(endpoint)] } else { vec![] };
    ...
    groups.into_iter().map(|(key, all)| {
        ...
        let total = |v: &[i128]| if v.is_empty() { null() } else { GraphAggregateValue::Integer(v.iter().sum()) };
        (key, vec![GraphAggregateValue::Count(all.len() as u64), GraphAggregateValue::Count(values.len() as u64),
            total(&values), average(&values), GraphAggregateValue::Count(distinct.len() as u64),
            total(&distinct), average(&distinct)])
    }).collect()
}
    ...
    for mask in 0..64 {
        for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
            for hops in 1..=3 {
                ...
                if hops > 1 { pattern += &atom("s", "S", "c"); }
                if hops > 2 { pattern += &atom("t", "R", "a"); }
                let endpoint = if hops == 1 { "b" } else { "c" };
                let value = format!("r.p * {endpoint}.p + 2");
                for grouped in [false, true] {
                    let key = if grouped { format!("{endpoint} AS endpoint, ") } else { String::new() };
                    let clause = if grouped { format!(" GROUP BY {endpoint}") } else { String::new() };
                    let q = prepare(&format!("MATCH {pattern} RETURN {key}COUNT(*) AS rows, COUNT({value}) AS present, SUM({value}) AS total, AVG({value}) AS average, COUNT(DISTINCT {value}) AS support, SUM(DISTINCT {value}) AS distinct_total, AVG(DISTINCT {value}) AS distinct_average{clause}"));
                    ...
                    assert_eq!(rows.iter().map(|r| (r.keys().to_vec(), r.values().to_vec())).collect::<Vec<_>>(), expected);
...
fn computed_keys_and_reordered_columns_are_not_source_positions_or_preprojection_distinct() {
    let square = expression(&[GraphIntegerOp::Column(0), GraphIntegerOp::Column(0), GraphIntegerOp::Binary(GraphIntegerBinary::Multiply)]);
    let q = projected(vec![
    ...
    ], &[0], &[GraphAggregate::count_rows("rows"), GraphAggregate::count_distinct("support", 0),
        GraphAggregate::sum_int("total", 1), GraphAggregate::sum_int("original_total", 2)]);
    let mut s = source(3);
    ...
    assert_eq!(rows[0].keys(), &[GraphValue::Scalar(CanonicalScalar::Int(25))]);
    assert_eq!(rows[0].values(), &[GraphAggregateValue::Count(2), GraphAggregateValue::Count(1),
        GraphAggregateValue::Integer(14), GraphAggregateValue::Integer(0)]);
    for text in [
        ...
        let q = prepare(text); let s = source(63); let batch = eager(&q, &s);
        assert_eq!(run(&q, s, wide()).collect::<Result<Vec<_>, _>>().unwrap(), batch);
    }
}

#[test]
fn unused_expression_errors_are_not_skipped_and_lazy_case_keeps_the_unselected_arm_dormant() {
    let bad = expression(&[GraphIntegerOp::Literal(Some(1)), GraphIntegerOp::Literal(Some(0)), GraphIntegerOp::Binary(GraphIntegerBinary::Divide)]);
    let q = projected(vec![GraphSetProjection::new("unused", bad.clone())], &[], &[GraphAggregate::count_rows("rows")]);
    let mut cursor = run(&q, source(1), wide());
    assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(GraphAggregateError::InputExpression { row: 0, column: 0, .. })))));
    assert_eq!(cursor.row_stats().result_rows, 0);
    assert_eq!(cursor.state(), EdgeScanState::Failed);
    assert!(cursor.next().is_none());
    let mut empty = run(&q, source(0), wide());
    assert_eq!(empty.next().unwrap().unwrap().values()[0].as_count(), Some(0));
    let lazy = expression(&[GraphIntegerOp::Truth(Some(true)), GraphIntegerOp::Column(0),
        GraphIntegerOp::Literal(Some(1)), GraphIntegerOp::Literal(Some(0)),
        GraphIntegerOp::Binary(GraphIntegerBinary::Divide), GraphIntegerOp::Case]);
    let q = projected(vec![GraphSetProjection::new("chosen", lazy)], &[], &[GraphAggregate::sum_int("total", 0)]);
    assert_eq!(run(&q, source(63), wide()).next().unwrap().unwrap().values()[0].as_integer(), Some(2));
    let q = projected(vec![GraphSetProjection::new("copied", GraphSetValue::Column(0)),
        GraphSetProjection::new("unused", bad)], &[0], &[GraphAggregate::count_rows("rows")]);
    let mut cursor = run(&q, source(1), wide());
    assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(GraphAggregateError::InputExpression { column: 1, .. })))));
...
fn predicates_and_probes_precede_projection_but_late_expression_errors_abort_all_groups() {
    for text in [
        ...
        let q = prepare(text); let mut s = source(63);
    ...
    for value in [CanonicalScalar::Int(i64::MAX), CanonicalScalar::Bool(true)] {
        ...
        let mut s = source(63); s.edges.get_mut(&EId(6)).unwrap().3 = vec![(P, value)];
        let dropped = s.drops.clone(); let mut cursor = run(&q, s, wide());
        assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(GraphAggregateError::InputExpression { .. })))));
        assert_eq!(cursor.row_stats().result_rows, 0);
        assert_eq!(dropped.load(Ordering::SeqCst), 1);
        assert!(cursor.pending.is_none()); assert!(cursor.next().is_none());
    }
}

#[test]
fn projection_grouping_and_delivery_share_exact_budgets_and_terminal_cancellation() {
    let q = prepare("MATCH (a)-[r:R]->(b) RETURN b, SUM(r.p*2) AS total, COUNT(DISTINCT r.p*2) AS support GROUP BY b");
    let mut total = 0;
    let mut full = EdgeAggregateCursor::new(source(63), EdgeAggregatePlan::compile(&q).unwrap(), wide(), || { total += 1; Ok::<_, usize>(()) });
    ...
    for limit in [GqlQueryPolicy::new(r.snapshot_records-1, r.result_rows, u64::MAX, u64::MAX),
    ...
    for stop in 1..=total {
        ...
        assert_eq!(dropped.load(Ordering::SeqCst), 1);
        assert!(cursor.pending.is_none()); assert!(cursor.next().is_none()); drop(cursor);
...
fn transformed_input_keeps_supported_result_modifiers_and_maintenance_and_row_scan_boundaries() {
    ...
    let mut cursor = run(&q, s, wide()); cursor.close(); cursor.close();
    assert_eq!(reads.load(Ordering::SeqCst), 0);
    assert_eq!(dropped.load(Ordering::SeqCst), 1); assert!(cursor.next().is_none());
}

#[test]
fn thousands_of_projected_occurrences_need_one_output_row_not_an_input_result_bag() {
    let q = prepare("MATCH (a)-[r:R]->(b) RETURN COUNT(*) AS rows, SUM(r.p+1) AS total, AVG(r.p+1) AS average, COUNT(DISTINCT r.p+1) AS support");
    let mut s = source(0);
    for id in 0..4096 {
        s.edges.insert(EId(id), (VId(0), R, VId(1), vec![(P, CanonicalScalar::Int(i64::MAX-1))]));
    }
    let mut cursor = run(&q, s, policy());
    let row = cursor.next().unwrap().unwrap();
    assert_eq!(row.values(), &[GraphAggregateValue::Count(4096),
