...
fn extreme(value: Option<&i128>) -> GraphAggregateValue {
    GraphAggregateValue::Value(GraphValue::Scalar(
        value.map_or(CanonicalScalar::Null, |value| {
            CanonicalScalar::Int(i64::try_from(*value).unwrap())
        }),
    ))
}
    ...
    groups
        ...
        .map(|(owner, rows)| {
            ...
            let values = vec![
                ...
                exact_average(&present),
                exact_average(&distinct),
                extreme(present.first()),
                extreme(present.last()),
            ];
    ...
    for mask in 0..64 {
        for direction in [
            ...
            let q = prepare(&format!(
                "MATCH {atom} RETURN a AS owner,COUNT(*) AS n,COUNT(r.p) AS present,COUNT(DISTINCT r.p) AS d,SUM(r.p) AS total,SUM(DISTINCT r.p) AS dt,AVG(r.p) AS avg,AVG(DISTINCT r.p) AS da,MIN(r.p) AS lo,MAX(r.p) AS hi GROUP BY a"
            ));
            let s = source(mask);
...
fn shared_cells_keep_nested_and_identity_domains_distinct_without_numeric_coercion() {
    ...
    let list =
        GraphValue::List(vec![scalar.clone(), GraphValue::List(vec![edge.clone()].into())].into());
    let values = [
        scalar.clone(),
        vertex,
        edge,
    ...
    for value in &values {
        count
            .update_governed(Input::from_value(value), 0, &mut allow)
            .unwrap();
    }
    ...
    for value in [&small, large, &small] {
        minimum
            .update_governed(Input::from_value(value), 1, &mut allow)
            .unwrap();
        maximum
            .update_governed(Input::from_value(value), 2, &mut allow)
            .unwrap();
    }
    assert_eq!(
        count.finish_governed(&mut allow).unwrap(),
        GraphAggregateValue::Count(4)
    );
    assert_eq!(
        minimum.finish_governed(&mut allow).unwrap(),
        GraphAggregateValue::Value(small)
    );
    assert_eq!(
        maximum.finish_governed(&mut allow).unwrap(),
        GraphAggregateValue::Value(large.clone())
    );
    let mut sum = NumericState::new_governed(Function::SumIntDistinct, &mut allow).unwrap();
    assert!(matches!(
        sum.update_governed(Input::from_value(large), 3, &mut allow),
...
fn every_grouped_checkpoint_preserves_only_a_complete_prefix_and_releases_all_state() {
    let q = prepare(
        "MATCH (a)-[r:R]-(b) RETURN a AS owner,COUNT(DISTINCT r) AS n,AVG(r.p) AS avg,MIN(b) AS low GROUP BY a",
    );
    let expected: Vec<_> = run(&q, source(63), wide())
        .collect::<Result<_, _>>()
        .unwrap();
    assert!(expected.len() > 1);
    let mut total = 0;
    {
        let mut cursor = EdgeAggregateCursor::new(
            source(63),
...
fn group_quotas_late_data_errors_empty_input_and_close_obey_the_pull_contract() {
    let q = prepare(
        "MATCH (a)-[r:R]-(b) RETURN a AS owner,COUNT(DISTINCT r.p) AS n,AVG(r.p) AS avg GROUP BY a",
    );
    ...
    assert_eq!(cursor.row_stats().result_rows, 0);
    assert!(cursor.next().is_none());
    let mut broken = source(63);
    broken.fail = Some(EId(6));
    ...
    assert_eq!(empty.state(), EdgeScanState::Exhausted);
    ...
    for pull_first in [false, true] {
        ...
        if pull_first {
            ...
            assert_eq!(
                ...
                1,
                "source released before pending delivery"
            );
...
fn thousands_of_joined_occurrences_share_group_local_distinct_support_and_exact_averages() {
    let q = prepare(
        "MATCH (a)-[r:R]->(b) RETURN a AS owner,COUNT(*) AS n,COUNT(DISTINCT r.p) AS d,SUM(r.p) AS total,AVG(DISTINCT r.p) AS avg GROUP BY a",
    );
    let mut s = source(0);
    for id in 0..4096_u128 {
        ...
        s.edges.insert(
            EId(id),
            (owner, R, VId(1), vec![(P, CanonicalScalar::Int(value))]),
        );
    }
    ...
    for (row, owner) in rows.iter().zip([VId(0), VId(u128::MAX)]) {
        ...
        assert_eq!(
            row.values()[2].as_integer(),
            Some(1024 * (2 * i128::from(i64::MAX) - 1))
        );
        let avg = row.values()[3].as_average().unwrap();
        assert_eq!(
            (avg.numerator(), avg.denominator()),
            (2 * i128::from(i64::MAX) - 1, 2)
        );
    }
