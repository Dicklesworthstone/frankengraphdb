...
pub(super) fn compile(plan: &GlaPlan<GraphValueRow>) -> Result<EdgeScanPlan, EdgeScanBuildError> {
    ...
    Ok(EdgeScanPlan {
        ...
        offset: *offset,
        count: *count,
        joined: Some(Arc::new(JoinPlan { expansions, stages })),
    })
}
...
pub(super) fn advance<S: EdgeScanSource, F: FnMut() -> Result<(), C>, C>(
    ...
    loop {
        ...
        let Some(paths) = test_stage(&plan.stages[depth], traversal, source, &mut |event| {
            meter.event(event)
        })?
        else {
            traversal.pop();
    ...
    for op in ops {
        ...
        match op {
            GlaOperator::Select { slot, predicates } => {
                let row = vertex(
                    source,
                    ids[slot.ordinal() as usize].expect("bound vertex"),
                    control,
                )?;
            ...
            GlaOperator::VertexIdentity { left, right, equal } => {
                if (ids[left.ordinal() as usize] == ids[right.ordinal() as usize]) != *equal {
                    return Ok(None);
                }
            }
            GlaOperator::CapturePath {
...
fn project<S: EdgeScanSource, C>(
    ...
    collect_element_properties::<GraphValueRow, _>(
        projection,
        ids,
        paths,
        &mut rows,
