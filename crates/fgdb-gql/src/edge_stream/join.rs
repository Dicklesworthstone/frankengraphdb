    ...
    loop {
        ...
        match op {
            ...
            GlaOperator::Expand { source, relation, direction }
                ...
                expansions.push(Expansion { source: source.ordinal() as usize, relation: *relation, direction: *direction });
                width += 1;
                stages.push(Vec::new());
                at += 1;
                continue;
            ...
            GlaOperator::CapturePath { capture, start, segments }
                if *capture as usize == captures.len() && (start.ordinal() as usize) < width
                    && !segments.is_empty() && segments.len() <= MAX_PATTERN_EDGES
                    && segments.iter().all(|slot| slot.ordinal() > 0 && (slot.ordinal() as usize) < width) => {
                terminal = true;
                captures.push(segments.iter().map(|slot| slot.ordinal() as usize - 1).collect());
            }
            GlaOperator::SelectBoolean { expression } => {
                ...
                let _ = expression.remap_elements(
                    ...
                    |capture| { edges_valid &= captures.get(capture as usize).is_some_and(|steps| steps.len() == 1); capture },
                );
                if !vertices_valid || !edges_valid { return Err(bad()); }
            }
            ...
            GlaOperator::ProjectValues { columns } => {
                ...
                for column in columns {
                    let valid = match column {
                        ValueProjection::Vertex { slot } | ValueProjection::Property { slot, .. } => (slot.ordinal() as usize) < width,
                        ValueProjection::EdgeProperty { capture, .. } => captures.get(*capture as usize).is_some_and(|s| s.len() == 1),
                        ValueProjection::Path { capture, function } => captures.get(*capture as usize).is_some_and(|s|
                            match function {
                                GraphPathFunction::Edge => s.len() == 1,
                                GraphPathFunction::Value | GraphPathFunction::Length | GraphPathFunction::Nodes | GraphPathFunction::Edges => true,
                                _ => false,
                            }),
                        _ => false,
                    };
...
#[derive(Clone, Copy)]
struct Choice { eid: EId, target: VId }
...
impl Traversal {
    fn new<E>(hops: usize, control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), E>) -> Result<Self, E> {
        ...
        for _ in 0..hops * 4 + 1 { control(GlaExecutionEvent::ScratchEntry)?; }
        Ok(Self { choices: Vec::with_capacity(hops), bindings: Vec::with_capacity(hops + 1),
            after: vec![None; hops], resume: false })
    }
    fn pop(&mut self) {
        self.choices.pop();
        if self.choices.is_empty() { self.bindings.clear(); } else { self.bindings.pop(); }
    }
}
...
pub(super) fn advance<S: EdgeScanSource, F: FnMut() -> Result<(), C>, C>(
    ...
    if cursor.plan.count == Some(0) { return Ok(None); }
    ...
    if traversal.resume { traversal.pop(); traversal.resume = false; }
    loop {
        meter.event(GlaExecutionEvent::Work)?;
        let depth = traversal.choices.len();
        let (eid, from, to) = if depth == 0 {
            let (eid, second) = if let Some(eid) = cursor.reverse_pending.take() { (eid, true) } else {
                let Some(eid) = flatten(source.next_edge(&mut |event| meter.event(event)))? else { return Ok(None); };
                meter.event(GlaExecutionEvent::Work)?;
                if cursor.last.is_some_and(|last| eid <= last) { return Err(GqlQueryError::Source(EdgeScanError::NonIncreasingIdentity)); }
            ...
            if edge.relation != cursor.plan.relation { continue; }
            let (from, to) = match cursor.plan.direction {
                ...
                GlaDirection::Undirected => {
                    ...
                    if !second && low != high { cursor.reverse_pending = Some(eid); }
            ...
            let next = source.next_incident_edge(from, expansion.direction, traversal.after[depth], &mut |event| meter.event(event));
            let next = match next {
                Ok(next) => next,
                Err(EdgeExpansionSourceError::Unavailable) => return Err(GqlQueryError::Source(EdgeScanError::ExpansionUnavailable)),
                Err(EdgeExpansionSourceError::Read(error)) => flatten(Err(error))?,
            };
            let Some(eid) = next else { traversal.pop(); continue; };
            meter.event(GlaExecutionEvent::Work)?;
            if traversal.after[depth].is_some_and(|prior| eid <= prior) { return Err(GqlQueryError::Source(EdgeScanError::NonIncreasingIdentity)); }
        ...
        if from != to { vertex(source, to, &mut |event| meter.event(event))?; }
...
fn vertex<'a, S: EdgeScanSource, C>(source: &'a S, vid: VId,
    control: &mut impl FnMut(GlaExecutionEvent) -> ScanResult<(), S::Error, C>,
) -> ScanResult<VertexScanRow<'a>, S::Error, C> {
    flatten(source.vertex(vid, control))?.ok_or(GqlQueryError::Source(EdgeScanError::DanglingEndpoint))
}
fn property<'a, S: EdgeScanSource, C>(source: &'a S, vid: VId, key: PropertyKeyId,
    control: &mut impl FnMut(GlaExecutionEvent) -> ScanResult<(), S::Error, C>,
...
fn edge_property<'a, S: EdgeScanSource, C>(source: &'a S, eid: EId, key: PropertyKeyId,
    control: &mut impl FnMut(GlaExecutionEvent) -> ScanResult<(), S::Error, C>,
) -> ScanResult<Option<&'a CanonicalScalar>, S::Error, C> {
    let edge = flatten(source.edge(eid, control))?.ok_or(GqlQueryError::Source(EdgeScanError::BoundEdgeUnavailable))?;
...
fn project<S: EdgeScanSource, C>(projection: &GlaOperator, ids: &[Option<VId>], paths: &[Option<GraphPath>], source: &S,
    ...
    <GraphValueRow as GlaOutput>::collect_element_properties(projection, ids, paths, &mut rows,
        &mut |vid, key| property(source, vid, key, &mut **meter.borrow_mut()),
        &mut |eid, key| edge_property(source, eid, key, &mut **meter.borrow_mut()),
