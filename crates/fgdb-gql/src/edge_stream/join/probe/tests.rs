...
impl Fixture {
    fn from_edges(edges: impl IntoIterator<Item = (u128, u128, u128)>) -> Self {
        let mut f = Self { edges: BTreeMap::new(), vertices: BTreeSet::new(),
            outgoing: BTreeMap::new(), incoming: BTreeMap::new(), after: None,
            roots: Arc::new(AtomicUsize::new(0)), seeks: Arc::new(AtomicUsize::new(0)),
            drops: Arc::new(AtomicUsize::new(0)), unavailable: false, repeat: false, fail_edge: None };
...
impl EdgeScanSource for Fixture {
    ...
    fn edge<'a, C>(&'a self, id: EId, control: &mut impl FnMut(GlaExecutionEvent) -> Result<(), C>)
        -> Result<Option<EdgeScanRow<'a>>, EdgeScanSourceError<Self::Error, C>> {
        control(GlaExecutionEvent::Work).map_err(EdgeScanSourceError::Control)?;
        if self.fail_edge == Some(id) { return Err(EdgeScanSourceError::Source("read denied")); }
    ...
    fn next_incident_edge<C>(&self, at: VId, d: GlaDirection, after: Option<EId>,
...
fn statement(shape: usize, dirs: [GlaDirection; 2], anti: bool, nullable: bool,
    distinct: bool, skip: usize, limit: usize,
) -> String {
    format!("MATCH (a){} WHERE {}EXISTS {{ {} }} RETURN {}r, a, b SKIP {skip} LIMIT {limit}",
        atom("r", "b", dirs[0]), if anti { "NOT " } else { "" }, body(shape, dirs[1], nullable),
...
fn expected(f: &Fixture, shape: usize, dirs: [GlaDirection; 2], anti: bool,
    nullable: bool, skip: usize, limit: usize,
) -> Vec<Vec<GraphValue>> {
    let inner: Vec<_> = f.edges.values().flat_map(|(a, b, _)| orientations(*a, *b, dirs[1])).collect();
    ...
    for (&eid, (from, to, _)) in &f.edges {
        for (a, b) in orientations(*from, *to, dirs[0]) {
            ...
            for &(start, x) in &inner {
                ...
                if start != b || nullable || x == a { continue; }
                if shape == 0 { witnesses += 1; continue; }
                for &(second, last) in &inner {
                    if second == (if shape == 1 { x } else { b }) && last == a { witnesses += 1; }
                }
            }
            if (witnesses != 0) != anti {
...
fn correlated_semi_and_anti_joins_match_complete_products_and_eager_gla() {
    ...
    for mask in [0, 10, 31, 63] {
        for outer in directions { for inner in directions { for shape in 0..3 {
            for anti in [false, true] { for nullable in [false, true] {
                for distinct in [false, true] { for (skip, limit) in [(0, 100), (1, 3), (0, 0)] {
                    let text = statement(shape, [outer, inner], anti, nullable, distinct, skip, limit);
                    let q = prepare(&text); let f = Fixture::small(mask);
                    let want = expected(&f, shape, [outer, inner], anti, nullable, skip, limit);
                    let eager = q.plan().execute_governed_with_element_properties(f.edges.len() as u64,
                        f.vertices.iter().copied(), f.edges.iter().map(|(&eid, (a, b, _))| (eid, *a, R, *b)),
                        |_, _| Ok::<_, ()>(true), |_, _| Ok(None), |_, _| Ok(None), policy(), || Ok::<_, ()>(())).unwrap();
                    ...
                    let mut rows = Vec::new();
                    for page in [1, 2, 128] { for _ in 0..page {
...
fn first_complete_witness_stops_before_a_bad_suffix_but_failed_probes_are_not_absence() {
    ...
    let mut cursor = EdgeScanCursor::new(f, EdgeScanPlan::compile(q.plan()).unwrap(),
        GqlQueryPolicy::new(2, 1, 100_000, 10_000), || Ok::<_, ()>(()));
    assert_eq!(cursor.next().unwrap().unwrap().values()[0], GraphValue::Edge(EId(0)));
    assert_eq!(cursor.row_stats().snapshot_records, 2);
    assert_eq!(seeks.load(Ordering::SeqCst), 1);
    assert!(cursor.next().is_none());
    assert_eq!(seeks.load(Ordering::SeqCst), 1);

    let q = prepare(&input.replace("WHERE EXISTS", "WHERE NOT EXISTS")
        .replace("MATCH (b)-[:R]->(x)", "MATCH (b)-[:R]->(x) WHERE x = a"));
    let mut f = Fixture::from_edges([(0, 0, 1), (1, 1, 2), (2, 1, 3)]); f.repeat = true;
    let mut cursor = EdgeScanCursor::new(f, EdgeScanPlan::compile(q.plan()).unwrap(), policy(), || Ok::<_, ()>(()));
    assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(EdgeScanError::NonIncreasingIdentity)))));
    ...
    let mut cursor = EdgeScanCursor::new(f, EdgeScanPlan::compile(q.plan()).unwrap(), policy(), || Ok::<_, ()>(()));
    assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(EdgeScanError::ExpansionUnavailable)))));
}

#[test]
fn every_probe_checkpoint_fuses_once_preserves_delivered_prefix_and_retries() {
    for anti in [false, true] {
        let q = prepare(&statement(1, [GlaDirection::Undirected; 2], anti, false, false, 0, 100));
        let mut calls = 0;
        let mut good = EdgeScanCursor::new(Fixture::small(63), EdgeScanPlan::compile(q.plan()).unwrap(), policy(),
            || { calls += 1; Ok::<_, usize>(()) });
        let want = good.by_ref().collect::<Result<Vec<_>, _>>().unwrap(); drop(good);
        for stop in 1..=calls {
            let mut at = 0; let f = Fixture::small(63); let drops = f.drops.clone();
            let mut cursor = EdgeScanCursor::new(f, EdgeScanPlan::compile(q.plan()).unwrap(), policy(),
...
fn probe_work_and_candidate_records_share_the_outer_query_limits() {
    let q = prepare(&statement(2, [GlaDirection::Undirected; 2], false, false, false, 0, 3));
    let run = |p| EdgeScanCursor::new(Fixture::small(63), EdgeScanPlan::compile(q.plan()).unwrap(), p, || Ok::<_, ()>(()));
    let mut good = run(policy()); let want = good.by_ref().collect::<Result<Vec<_>, _>>().unwrap();
...
fn a_deep_probe_finds_one_of_exponentially_many_witnesses_with_linear_demand() {
    for hops in [40, MAX_PATTERN_EDGES - 1] {
        ...
        for i in 0..hops { text.push_str(&format!("-[:R]->(v{})", i + 1)); }
        text.push_str(" } RETURN r, a, b LIMIT 1");
        let q = prepare(&text);
        let edges = std::iter::once((0, 1000, 0)).chain((0..hops as u128)
            .flat_map(|i| [(i * 2 + 1, i, i + 1), (i * 2 + 2, i, i + 1)]));
        let f = Fixture::from_edges(edges); let roots = f.roots.clone(); let seeks = f.seeks.clone();
        let mut cursor = EdgeScanCursor::new(f, EdgeScanPlan::compile(q.plan()).unwrap(),
...
fn variable_length_probes_refuse_before_source_access_even_with_limit_zero() {
    ...
    let f = Fixture::small(63); let seeks = f.seeks.clone();
...
fn a_source_error_in_a_required_probe_is_not_an_anti_join_witness() {
    ...
    let q = prepare(text);
    let mut f = Fixture::from_edges([(0, 0, 1), (1, 1, 2), (2, 1, 3)]); f.fail_edge = Some(EId(2));
    let mut cursor = EdgeScanCursor::new(f, EdgeScanPlan::compile(q.plan()).unwrap(), policy(), || Ok::<_, ()>(()));
    assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Source(EdgeScanError::Source("read denied"))))));
    assert_eq!(cursor.row_stats().result_rows, 0);
    assert_eq!(cursor.state(), EdgeScanState::Failed);
    assert!(cursor.next().is_none());
}
