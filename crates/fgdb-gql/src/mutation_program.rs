...
pub enum GraphMutationProgramError<E, C> {
    ...
    Interrupted { completed_statements: usize, source: C },
    Budget {
        statement: usize,
    ...
    InvalidStatistics { statement: usize },
}
    ...
    for GraphMutationProgramError<E, C>
{
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::Interrupted { completed_statements, source } => {
                write!(f, "mutation program interrupted after {completed_statements} statements: {source}")
            }
            Self::Budget { statement, dimension, limit, observed } => {
                write!(f, "mutation program statement {statement} {dimension:?}: {observed} > {limit}")
            }
            Self::InvalidStatistics { statement } => {
                write!(f, "mutation program statement {statement} returned inconsistent statistics")
            }
        }
    }
}
...
impl PreparedGraphMutationProgram {
    pub fn prepare(statements: Vec<PreparedGraphMutation>) -> Result<Self, GraphMutationProgramBuildError> {
    ...
    #[must_use]
    pub fn statements(&self) -> &[PreparedGraphMutation] { &self.statements }
    #[must_use]
    ...
    pub fn execute_governed<E, C>(
        ...
        mut stage: impl FnMut(&PreparedGraphMutation, GraphMutationPolicy)
        ...
        let mut meter = ProgramMeter { policy, stats: GraphMutationProgramStats::default() };
...
impl ProgramMeter {
    fn counter(&self, dimension: GraphMutationProgramDimension) -> (u64, u64) {
        ...
        match dimension {
            D::SnapshotRecords => (self.stats.selection.snapshot_records,
                self.policy.query.rows.max_snapshot_records().unwrap_or(u64::MAX)),
            D::SelectedRows => (self.stats.selection.result_rows,
                self.policy.query.rows.max_result_rows().unwrap_or(u64::MAX)),
            D::WorkUnits => (self.stats.evaluator.work_units, self.policy.query.evaluator.max_work_units),
            D::ScratchEntries => (self.stats.evaluator.scratch_entries, self.policy.query.evaluator.max_scratch_entries),
            D::Effects => (self.stats.effects, self.policy.max_effects),
        }
    }
    fn add<E, C>(&self, statement: usize, dimension: GraphMutationProgramDimension, amount: u64)
        ...
        if observed > u128::from(limit) {
            return Err(GraphMutationProgramError::Budget { statement, dimension, limit, observed });
        }
        Ok(observed as u64)
    }
    fn boundary<E, C>(&mut self, statement: usize) -> Result<(), GraphMutationProgramError<E, C>> {
        self.stats.evaluator.work_units = self.add(statement, GraphMutationProgramDimension::WorkUnits, 1)?;
        Ok(())
    }
    fn remaining(&self) -> GraphMutationPolicy {
        use GraphMutationProgramDimension as D;
        let remainder = |dimension| { let (used, limit) = self.counter(dimension); limit - used };
        GraphMutationPolicy::new(GqlQueryPolicy::new(
            remainder(D::SnapshotRecords), remainder(D::SelectedRows),
            remainder(D::WorkUnits), remainder(D::ScratchEntries),
        ), remainder(D::Effects))
    }
    fn absorb<E, C>(&mut self, statement: usize, actions: usize, stats: GraphMutationStats)
        ...
        let next = GraphMutationProgramStats {
            ...
            selection: GqlExecutionStats {
                snapshot_records: self.add(statement, D::SnapshotRecords, stats.selection.snapshot_records)?,
            ...
            evaluator: GlaExecutionStats {
                ...
                scratch_entries: self.add(statement, D::ScratchEntries, stats.evaluator.scratch_entries)?,
            },
            ...
            target_vertex_visits: self.stats.target_vertex_visits.checked_add(stats.target_vertices)
    ...
    fn translate<E, C>(&self, statement: usize, source: GqlQueryError<GraphMutationError<E>, C>)
        ...
        let (dimension, local) = match source {
            ...
            GqlQueryError::Source(GraphMutationError::EffectLimit { observed, .. }) => (D::Effects, observed),
        ...
        match local.checked_add(u128::from(used)) {
            Some(observed) if observed > u128::from(limit) => {
                GraphMutationProgramError::Budget { statement, dimension, limit, observed }
            }
