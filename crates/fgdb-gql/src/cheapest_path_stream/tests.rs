...
fn policy() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 1000, 10_000_000, 10_000_000)
}
fn query(
...
fn fixture(mask: u8) -> (Vec<Edge>, BTreeMap<EId, CanonicalScalar>) {
    ...
    (
        selected
            .iter()
            .map(|&(id, from, to, _)| (EId(id), VId(from), R, VId(to)))
            .collect(),
        selected
            .iter()
            .map(|&(id, _, _, weight)| (EId(id), CanonicalScalar::Int(weight)))
            .collect(),
    )
}
fn open<C>(
    q: &PreparedGraphCheapestPath,
    k: u64,
    p: GqlQueryPolicy,
    checkpoint: impl FnMut() -> Result<(), C>,
) -> StreamResult<GraphCheapestPathStream, (), C> {
    let (edges, weights) = fixture(63);
    q.stream_governed_with_edge_properties(
        k,
        edges.len() as u64 + 3,
        [VId(0), VId(1), VId(2)],
        edges,
        |id, _| Ok(weights.get(&id)),
        p,
        checkpoint,
    )
}
fn rows(stream: &mut GraphCheapestPathStream) -> Vec<GraphCostPath> {
    ...
    while let Some(row) = stream.next_with_checkpoint(|| Ok::<_, ()>(())).unwrap() {
        rows.push(row);
    }
    rows
}
    ...
    for depth in 0..=q.bounds().maximum() {
        if depth >= q.bounds().minimum() {
            for (end, cost, steps) in &layer {
                ...
                let allowed = match q.mode() {
                    ...
                    GraphCheapestPathMode::Trail => {
                        steps.iter().map(|s| s.0).collect::<BTreeSet<_>>().len() == steps.len()
                    }
                    mode => {
                        let mut ids: Vec<_> = std::iter::once(q.source())
                            .chain(steps.iter().map(|s| s.1))
                            .collect();
                        if mode == GraphCheapestPathMode::Simple
                            && ids.len() > 1
                            && ids.last() == Some(&q.source())
                        {
                            ids.pop();
                        }
                ...
                if allowed {
                    answers.push((*cost, steps.clone()));
                }
            }
        }
        if depth == q.bounds().maximum() {
            break;
        }
        ...
        for (end, cost, steps) in layer {
            for &(eid, from, relation, to) in edges {
                ...
                if q.direction() != GlaDirection::Reverse && from == end {
                    destinations.push(to);
                }
                if q.direction() != GlaDirection::Forward
                    && to == end
                    && (q.direction() != GlaDirection::Undirected || from != to)
                {
            ...
            for direction in [
                ...
                for lo in [0, 2] {
                    for target in [VId(0), VId(2)] {
                        ...
                        for k in [0, 1, 5] {
                            let eager = q
                                .execute_k_governed_with_edge_properties(
                                    k,
                                    3 + edges.len() as u64,
                                    [VId(0), VId(1), VId(2)],
                                    edges.iter().copied(),
                                    |id, _| Ok::<_, ()>(weights.get(&id)),
                                    policy(),
                                    || Ok::<_, ()>(()),
                                )
                                .unwrap();
                            ...
                            for page_size in [1, 2, 7] {
                                let mut stream = q
                                    .stream_governed_with_edge_properties(
                                        k,
                                        3 + edges.len() as u64,
                                        [VId(0), VId(1), VId(2)],
                                        edges.iter().copied(),
                                ...
                                assert!(
                                    stream
                                        .next_with_checkpoint(|| -> Result<(), ()> {
                                            panic!("terminal checkpoint");
                                        })
                                        .unwrap()
                                        .is_none()
                                );
...
fn exact_limits_include_prior_admission_and_one_less_refuses_without_reset_or_partial_row() {
    for mode in MODES {
        ...
        let (edges, weights) = fixture(63);
        let prior = GlaExecutionStats {
            work_units: 101,
            scratch_entries: 17,
        };
        let total = GqlQueryPolicy::new(
            r.snapshot_records,
            r.result_rows,
            e.work_units + 101,
            e.scratch_entries + 17,
        );
        let mut combined = q
            .stream_governed_with_admission(
                5,
                r.snapshot_records,
                [VId(0), VId(1), VId(2)],
                edges.iter().copied(),
        ...
        assert_eq!(
            combined.evaluator_stats(),
            GlaExecutionStats {
                work_units: e.work_units + 101,
                scratch_entries: e.scratch_entries + 17
            }
        );
    }
}
...
fn cancellation_at_every_open_and_pull_checkpoint_delivers_only_the_completed_prefix() {
    for mode in MODES {
        ...
        let mut good = open(&q, 4, policy(), || {
            checkpoints += 1;
            Ok::<_, usize>(())
        })
        .unwrap();
        let mut expected = Vec::new();
        while let Some(row) = good
        ...
        for stop in 1..=checkpoints {
            ...
            assert!(
                stream
                    .next_with_checkpoint(|| -> Result<(), usize> {
                        panic!("checkpoint after failure");
                    })
                    .unwrap()
                    .is_none()
            );
...
fn close_and_k_boundary_never_expand_the_unrequested_suffix() {
    ...
    let start = |k| {
        q.stream_governed_with_edge_properties(
            ...
            [VId(0)],
            edges,
            |id, _| Ok::<_, ()>(costs.get(&id)),
            GqlQueryPolicy::new(3, 1, 10_000, 10_000),
            || Ok::<_, ()>(()),
        )
        .unwrap()
    };
    ...
    assert_eq!(
        many.next_with_checkpoint(|| Ok::<_, ()>(())).unwrap(),
        Some(first)
    );
...
fn opening_zero_count_or_missing_anchors_still_validates_all_selected_costs() {
    ...
    for count in [0, 1] {
        let missing = q.stream_governed_with_edge_properties(
            count,
            3,
            [VId(0), VId(1)],
            edges,
        ...
        let refused = q.stream_governed_with_edge_properties(
            count,
            3,
            [VId(0), VId(1)],
            edges,
            |_, _| Err::<Option<&CanonicalScalar>, _>("source refused"),
            policy(),
            || Ok::<_, ()>(()),
        );
        assert!(matches!(
...
fn impossible_prior_usage_refuses_before_source_access_and_counter_overflow_is_wide() {
    ...
    for prior in [
        ...
        let result = q.stream_governed_with_admission(
            0,
            0,
            std::iter::from_fn(|| -> Option<VId> {
                panic!("over-budget source driven");
            }),
            [],
            |_, _| Ok::<_, ()>(None),
            prior,
            GqlQueryPolicy::new(0, 0, 1, 1),
            || Ok::<_, ()>(()),
        );
        assert!(
            matches!(result, Err(GqlQueryError::Evaluator(error)) if error.observed == 2 && error.limit == 1)
        );
    }
    let result = q.stream_governed_with_admission(
        0,
    ...
    assert!(
        matches!(result, Err(GqlQueryError::Evaluator(error)) if error.observed == u128::from(u64::MAX) + 1)
    );
}

#[test]
fn stream_owns_costs_and_full_width_identities_without_debug_disclosure() {
    let source = VId(1_u128 << 100);
    let target = VId(u128::MAX);
    let q = PreparedGraphCheapestPath::new(
        source,
        target,
        R,
        GlaDirection::Forward,
    ...
    for terminal in ["limit", "close", "error", "drop", "zero", "eof"] {
        ...
        match terminal {
            ...
            "error" => assert!(matches!(
                cursor.next(),
                Some(Err(GqlQueryError::Interrupted(9)))
            )),
            "eof" => {
        ...
        assert_eq!(
            ...
            1,
            "driver retained after {terminal}"
        );
    }
}
