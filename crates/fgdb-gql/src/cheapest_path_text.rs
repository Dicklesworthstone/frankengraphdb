...
use crate::{
    ...
    GraphCheapestPathMode, GraphPatternTextError, GraphPatternTextErrorKind,
    GraphSymbol, GraphSymbolKind, GraphSymbolResolver, GraphWalkBounds,
    MAX_GRAPH_WALK_HOPS, PreparedGraphCheapestPath, PreparedGraphText,
};
...
impl core::fmt::Display for GraphCheapestPathTextError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        write!(f, "weighted path text error at byte {}: {:?}", self.offset, self.kind)
    }
}
...
impl From<GraphPatternTextError> for GraphCheapestPathTextError {
    fn from(error: GraphPatternTextError) -> Self {
        fail(error.offset, GraphCheapestPathTextErrorKind::Pattern(error.kind))
    }
}
...
impl Number {
    fn literal(&self) -> Option<u64> {
        match self.operand { Operand::Literal(value) => Some(value), Operand::Parameter(_) => None }
    }
    fn bind(&self, values: &[u64]) -> u64 {
        match self.operand { Operand::Literal(value) => value, Operand::Parameter(at) => values[at] }
    }
}
...
impl PreparedGraphCheapestPathText {
    ...
    pub fn prepare(
        ...
        let mut parser = Parser {
            ...
            at: 0, parameters: Vec::new(), parameter_offsets: Vec::new(),
        };
        parser.word("MATCH")?;
        ...
        let mode = if parser.take_word("TRAIL") { GraphCheapestPathMode::Trail }
            else if parser.take_word("ACYCLIC") { GraphCheapestPathMode::Acyclic }
            else if parser.take_word("SIMPLE") { GraphCheapestPathMode::Simple }
            else { parser.take_word("WALK"); GraphCheapestPathMode::Walk };
        parser.punct(b'(', "(")?;
        ...
        if reverse && forward { return Err(expected(parser.offset(), "one traversal direction")); }
        let direction = if reverse { GlaDirection::Reverse }
            else if forward { GlaDirection::Forward } else { GlaDirection::Undirected };
        parser.punct(b'(', "(")?;
        let target = parser.name()?;
        parser.punct(b')', ")")?;
        for name in [source, edge, target] {
            if name.0 == path.0 { return Err(expected(name.1, "distinct path and element variables")); }
        }
        ...
        if cost_edge.0 != edge.0 { return Err(pattern(cost_edge.1, GraphPatternTextErrorKind::UnknownVariable)); }
        parser.punct(b'.', ".")?;
        let weight = parser.catalog_name()?;
        parser.word("RETURN")?;
        let returned = parser.name()?;
        if returned.0 != path.0 { return Err(pattern(returned.1, GraphPatternTextErrorKind::UnknownVariable)); }
        let column = if parser.take_word("AS") { parser.name()? } else { returned };
        ...
        for number in [&minimum, &maximum] {
            if number.literal().is_some_and(|n| n > u64::from(MAX_GRAPH_WALK_HOPS)) {
                return Err(fail(number.at, GraphCheapestPathTextErrorKind::InvalidHopBounds));
            }
        }
        if let (Some(lo), Some(hi)) = (minimum.literal(), maximum.literal()) {
            bounds(lo, hi, maximum.at)?;
        }
        let GraphSymbol::Relation(relation_id) = symbol(&mut resolve, GraphSymbolKind::Relation, relation)?
            else { unreachable!("checked relation domain") };
        let GraphSymbol::Property(weight_id) = symbol(&mut resolve, GraphSymbolKind::Property, weight)?
            else { unreachable!("checked property domain") };
        Ok(Self {
            relation: relation_id, weight: weight_id, direction, mode, count, minimum, maximum,
            parameters: parser.parameters, parameter_offsets: parser.parameter_offsets,
            source: source.0.to_owned(), target: target.0.to_owned(), target_at: target.1,
            column: column.0.to_owned(), return_at: returned.1,
        })
    }
    ...
    #[must_use]
    pub fn source_variable(&self) -> &str { &self.source }
    #[must_use]
    pub fn target_variable(&self) -> &str { &self.target }
    #[must_use]
    pub fn column_name(&self) -> &str { &self.column }
    ...
    pub fn bind(
        &self, source: VId, target: VId, arguments: &GqlParameters,
        ...
        for (spec, &at) in self.parameters.iter().zip(&self.parameter_offsets) {
            match arguments.get(&spec.name) {
                ...
                Some(value) => return Err(pattern(at, GraphPatternTextErrorKind::ParameterTypeMismatch {
                    expected: GqlParameterType::UInt64, found: value.parameter_type(),
                })),
                None => return Err(pattern(at, GraphPatternTextErrorKind::MissingParameter)),
            }
        }
        if arguments.len() != self.parameters.len() {
            return Err(pattern(self.return_at, GraphPatternTextErrorKind::UnexpectedArguments));
        }
        if self.source == self.target && source != target {
            return Err(fail(self.target_at, GraphCheapestPathTextErrorKind::AnchorMismatch));
        }
        let bounds = bounds(self.minimum.bind(&values), self.maximum.bind(&values), self.maximum.at)?;
        let query = PreparedGraphCheapestPath::new(source, target, self.relation, self.direction, self.weight, bounds)
            .map_err(|kind| pattern(self.return_at, GraphPatternTextErrorKind::Build(kind)))?
            .with_mode(self.mode);
        Ok(BoundGraphCheapestPathQuery {
            query, count: self.count.as_ref().map(|n| n.bind(&values)), column: self.column.clone(),
        })
    }
}
...
impl BoundGraphCheapestPathQuery {
    #[must_use]
    pub fn query(&self) -> &PreparedGraphCheapestPath { &self.query }
    /// None selects ANY CHEAPEST; Some(k) selects a K-ranked prefix, including 0.
    #[must_use]
    pub const fn ranked_count(&self) -> Option<u64> { self.count }
    #[must_use]
    pub fn column_name(&self) -> &str { &self.column }
    ...
    pub fn canonical_bytes(&self) -> Vec<u8> {
        ...
        match self.count {
            None => bytes.push(0),
            Some(count) => { bytes.push(1); bytes.extend_from_slice(&count.to_be_bytes()); }
        }
...
fn symbol(
    resolve: &mut impl GraphSymbolResolver, kind: GraphSymbolKind, name: (&str, usize),
) -> Result<GraphSymbol, GraphCheapestPathTextError> {
    let value = resolve.resolve_symbol(kind, name.0)
        .ok_or_else(|| pattern(name.1, GraphPatternTextErrorKind::UnknownSymbol(kind)))?;
    if value.kind() != kind {
        return Err(pattern(name.1, GraphPatternTextErrorKind::WrongSymbolKind { expected: kind, found: value.kind() }));
    }
    Ok(value)
}
...
impl<'a> Parser<'a> {
    ...
    fn offset(&self) -> usize { self.current().at }
    ...
    fn word(&mut self, word: &'static str) -> Result<(), GraphCheapestPathTextError> {
        if self.take_word(word) { Ok(()) } else { Err(expected(self.offset(), word)) }
    }
    ...
    fn punct(&mut self, ch: u8, what: &'static str) -> Result<(), GraphCheapestPathTextError> {
        if self.take(ch) { Ok(()) } else { Err(expected(self.offset(), what)) }
    }
    ...
    fn catalog_name(&mut self) -> Result<(&'a str, usize), GraphCheapestPathTextError> {
        let at = self.offset();
        let TextKind::Word(text) = &self.current().kind else { return Err(expected(at, "catalog identifier")); };
        let text = *text;
        self.at += 1;
        Ok((text, at))
    }
    fn name(&mut self) -> Result<(&'a str, usize), GraphCheapestPathTextError> {
        let at = self.offset();
        let TextKind::Word(text) = &self.current().kind else { return Err(expected(at, "identifier")); };
        let text = *text;
        if ["MATCH", "ANY", "CHEAPEST", "WALK", "TRAIL", "ACYCLIC", "SIMPLE", "COST", "RETURN", "AS",
            "WHERE", "ALL", "DISTINCT", "AND", "OR", "SKIP", "LIMIT", "OPTIONAL", "ORDER", "BY"]
            .iter().any(|word| text.eq_ignore_ascii_case(word)) || text.starts_with("__fgdb_anonymous_") {
    ...
    fn number(&mut self) -> Result<Number, GraphCheapestPathTextError> {
        ...
        let operand = match &self.current().kind {
            TextKind::Digits(digits) => Operand::Literal(digits.parse::<u64>()
            ...
            TextKind::Parameter(name) => {
                ...
                let index = if let Some(index) = self.parameters.iter().position(|p| p.name == name) {
                    ...
                    self.parameters.push(GqlParameterSpec { name: name.to_owned(), parameter_type: GqlParameterType::UInt64,
