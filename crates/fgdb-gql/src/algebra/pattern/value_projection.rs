...
impl PreparedGraphPattern<GraphValueRow> {
    ...
    pub fn value_columns(&self) -> &[ValueProjection] {
        self.logical.operators().iter().find_map(|operator| match operator {
...
mod tests {
    use super::*;
    use crate::{GqlQueryPolicy, GqlQueryError};
    use fgdb_delta_types::PropertyKeyId;
    use fgdb_types::{CanonicalScalar, CanonicalF64};

    fn builder() -> GraphPatternBuilder {
        let mut b = GraphPatternBuilder::new();
        b.vertex("a").unwrap(); b.vertex("b").unwrap();
    ...
    fn property_schema_is_checked_and_canonical_identity_preserves_expressions() {
        let b = builder(); let key = PropertyKeyId(7);
        let columns = [GraphColumn::vertex("owner", "a"), GraphColumn::property("score", "b", key)];
        let prepared = b.prepare_values(&columns, 0, Some(3)).unwrap();
        assert_eq!(prepared.columns(), &["owner".to_owned(), "score".to_owned()]);
        assert!(prepared.plan().projects_properties());
        assert!(prepared.plan().needs_vertex_values());
        assert_eq!(prepared.value_columns().len(), 2);
        assert!(matches!(prepared.value_columns()[1], ValueProjection::Property { key: actual, .. } if actual == key));
        assert_eq!(b.prepare_values(&[], 0, None).unwrap_err(), PatternBuildError::EmptyProjection);
        assert_eq!(b.prepare_values(&[columns[0], columns[0]], 0, None).unwrap_err(), PatternBuildError::DuplicateProjection);
        assert_eq!(b.prepare_values(&[GraphColumn::vertex("bad alias", "a")], 0, None).unwrap_err(), PatternBuildError::InvalidColumnName);
        assert_eq!(b.prepare_values(&[GraphColumn::vertex("valid", "missing")], 0, None).unwrap_err(), PatternBuildError::UnknownVariable);
        assert!(matches!(b.prepare_values(&[columns[0]; MAX_PATTERN_VERTICES + 1], 0, None),
            Err(PatternBuildError::LimitExceeded { dimension: PatternLimitDimension::Columns, .. })));
        let renamed = b.prepare_values(&[GraphColumn::vertex("x", "a"), GraphColumn::property("y", "b", key)], 0, Some(3)).unwrap();
        assert_eq!(prepared.canonical_bytes(), renamed.canonical_bytes());
        let changed = b.prepare_values(&[columns[0], GraphColumn::property("score", "b", PropertyKeyId(8))], 0, Some(3)).unwrap();
        assert_ne!(prepared.canonical_bytes(), changed.canonical_bytes());
        assert_ne!(prepared.canonical_bytes(), b.prepare_values(&[columns[1], columns[0]], 0, Some(3)).unwrap().canonical_bytes());
    ...
    fn property_projection_uses_value_distinctness_order_and_shared_limits() {
        ...
        let values = [CanonicalScalar::Int(9), CanonicalScalar::Int(-3), CanonicalScalar::Int(9),
            CanonicalScalar::Null, CanonicalScalar::Bool(true), CanonicalScalar::Float(CanonicalF64::new(9.0))];
        let edges: Vec<_> = (0..values.len()).map(|at| (VId(100), RelationId(1), VId(at as u128))).collect();
        let run = |policy| pattern.plan().execute_governed_with_properties(
            edges.len() as u64, [], edges.iter().copied(), |_, _| Ok::<_, &str>(true),
            |vid, property| Ok((property == key).then(|| &values[vid.0 as usize])), policy, || Ok::<_, usize>(()));
        let wide = run(GqlQueryPolicy::new(6, 3, u64::MAX, u64::MAX)).unwrap();
        assert_eq!(wide.value.iter().map(|r| r.get(0).unwrap().as_scalar().unwrap()).collect::<Vec<_>>(),
            vec![&values[4], &values[1], &values[0]]);
        let exact = GqlQueryPolicy::new(6, 3, wide.evaluator.work_units, wide.evaluator.scratch_entries);
        assert_eq!(run(exact).unwrap(), wide);
        assert!(matches!(run(GqlQueryPolicy::new(6, 2, u64::MAX, u64::MAX)), Err(GqlQueryError::Rows(_))));
        assert!(matches!(run(GqlQueryPolicy::new(6, 3, exact.evaluator.max_work_units - 1, u64::MAX)), Err(GqlQueryError::Evaluator(_))));
        assert!(matches!(run(GqlQueryPolicy::new(6, 3, u64::MAX, exact.evaluator.max_scratch_entries - 1)), Err(GqlQueryError::Evaluator(_))));
        let result = pattern.plan().execute_governed_with_properties(6, [], edges.iter().copied(), |_, _| Ok::<_, &str>(true),
            |vid, _| if vid == VId(5) { Err("late projected property") } else { Ok(Some(&values[vid.0 as usize])) },
            GqlQueryPolicy::new(6, 0, u64::MAX, u64::MAX), || Ok::<_, usize>(()));
        assert!(matches!(result, Err(GqlQueryError::Source("late projected property"))));
    }
    ...
    fn property_only_node_scan_keeps_nulls_and_cancellation_at_every_event() {
        ...
        let pattern = b.prepare_values(&[GraphColumn::property("p", "n", PropertyKeyId(1))], 0, None).unwrap();
        assert!(!pattern.plan().scans_edges()); assert_eq!(pattern.required_vertex_label(), None);
        let scalar = CanonicalScalar::ucs_basic_text("value").unwrap();
        let mut calls = 0;
        let policy = GqlQueryPolicy::new(3, 2, u64::MAX, u64::MAX);
        let expected = pattern.plan().execute_governed_with_properties(3, [VId(1), VId(2), VId(3)], [],
            |_, _| Ok::<_, ()>(true), |vid, _| Ok((vid == VId(2)).then_some(&scalar)), policy,
            || { calls += 1; Ok::<_, usize>(()) }).unwrap();
        assert_eq!(expected.value.len(), 2); assert!(expected.value[0].get(0).unwrap().is_null());
        for stop in 1..=calls {
            let mut at = 0;
            let result = pattern.plan().execute_governed_with_properties(3, [VId(1), VId(2), VId(3)], [],
                |_, _| Ok::<_, ()>(true), |vid, _| Ok((vid == VId(2)).then_some(&scalar)), policy,
