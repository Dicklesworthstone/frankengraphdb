...
pub enum PatternLimitDimension { Vertices, Edges, Predicates, Identities, Columns }

#[derive(Clone, Debug, PartialEq, Eq)]
pub enum PatternBuildError {
    EmptyPattern, InvalidVariableName, DuplicateVariable, UnknownVariable, Disconnected,
    EmptyProjection, DuplicateProjection,
    LimitExceeded { dimension: PatternLimitDimension, limit: usize, observed: usize },
}
impl core::fmt::Display for PatternBuildError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::LimitExceeded { dimension, limit, observed } => write!(f, "graph-pattern {dimension:?} limit exceeded: {observed} > {limit}"),
        }
    }
}
...
#[derive(Clone)]
struct Variable { name: String, predicates: Vec<VertexPredicate> }
#[derive(Clone, Copy)]
struct Edge { source: usize, destination: usize, relation: RelationId, direction: GlaDirection }
#[derive(Clone, Copy)]
struct Identity { left: usize, right: usize, equal: bool }

/// Bounded definition metadata. Mutators validate before changing state.
#[derive(Clone, Default)]
pub struct GraphPatternBuilder {
    variables: Vec<Variable>, edges: Vec<Edge>, identities: Vec<Identity>, predicate_count: usize,
}
impl core::fmt::Debug for GraphPatternBuilder {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("GraphPatternBuilder").field("variables", &self.variables.len())
            .field("edges", &self.edges.len()).field("predicates", &self.predicate_count)
            .field("identities", &self.identities.len()).field("definition", &"[REDACTED]").finish()
    }
}
...
pub struct PreparedGraphPattern<Row = VId> {
    logical: GlaPlan<Row>, variable_count: usize, edge_count: usize, columns: Vec<String>,
}
impl<Row> core::fmt::Debug for PreparedGraphPattern<Row> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("PreparedGraphPattern").field("variables", &self.variable_count)
            .field("edges", &self.edge_count).field("columns", &self.columns.len())
            .field("definition", &"[REDACTED]").finish()
    }
}
impl<Row> PreparedGraphPattern<Row> {
    #[must_use]
    pub fn plan(&self) -> &GlaPlan<Row> { &self.logical }
    #[must_use]
    pub fn canonical_bytes(&self) -> Vec<u8> { self.logical.canonical_bytes() }
    ...
    pub fn required_vertex_label(&self) -> Option<LabelId> {
        if self.logical.scans_edges() { return None; }
        self.logical.operators().iter().find_map(|op| match op {
            GlaOperator::Select { slot, predicates } if slot.ordinal() == 0 => predicates.iter().find_map(|p| match p {
                VertexPredicate::HasLabel(label) => Some(*label), VertexPredicate::IntegerProperty { .. } => None,
            }),
            _ => None,
        })
    }
...
fn check_next(count: usize, limit: usize, dimension: PatternLimitDimension) -> Result<(), PatternBuildError> {
    if count >= limit { return Err(PatternBuildError::LimitExceeded { dimension, limit, observed: count + 1 }); }
    Ok(())
}
fn reverse(direction: GlaDirection) -> GlaDirection {
    match direction { GlaDirection::Forward => GlaDirection::Reverse, GlaDirection::Reverse => GlaDirection::Forward,
...
impl GraphPatternBuilder {
    ...
    fn variable(&self, name: &str) -> Result<usize, PatternBuildError> {
        self.variables.iter().position(|var| var.name == name).ok_or(PatternBuildError::UnknownVariable)
    }
    pub fn vertex(&mut self, name: &str) -> Result<&mut Self, PatternBuildError> {
        ...
        if self.variables.iter().any(|var| var.name == name) { return Err(PatternBuildError::DuplicateVariable); }
        check_next(self.variables.len(), MAX_PATTERN_VERTICES, PatternLimitDimension::Vertices)?;
        self.variables.push(Variable { name: name.to_owned(), predicates: Vec::new() }); Ok(self)
    }
    pub fn filter(&mut self, variable: &str, predicate: VertexPredicate) -> Result<&mut Self, PatternBuildError> {
        ...
        self.variables[at].predicates.push(predicate); self.predicate_count += 1; Ok(self)
    }
    pub fn edge(&mut self, source: &str, relation: RelationId, direction: GlaDirection, destination: &str) -> Result<&mut Self, PatternBuildError> {
        let source = self.variable(source)?; let destination = self.variable(destination)?;
        check_next(self.edges.len(), MAX_PATTERN_EDGES, PatternLimitDimension::Edges)?;
        self.edges.push(Edge { source, destination, relation, direction }); Ok(self)
    }
    pub fn identity(&mut self, left: &str, right: &str, equal: bool) -> Result<&mut Self, PatternBuildError> {
        let left = self.variable(left)?; let right = self.variable(right)?;
        check_next(self.identities.len(), MAX_PATTERN_IDENTITIES, PatternLimitDimension::Identities)?;
        self.identities.push(Identity { left, right, equal }); Ok(self)
    }
    ...
    fn identities(&self, slots: &[Option<BindingSlot>], emitted: &mut [bool], operators: &mut Vec<GlaOperator>) {
        for (at, identity) in self.identities.iter().enumerate() {
            if !emitted[at] && let (Some(left), Some(right)) = (slots[identity.left], slots[identity.right]) {
                operators.push(GlaOperator::VertexIdentity { left, right, equal: identity.equal }); emitted[at] = true;
            }
        }
    }

    /// Retains the original sorted/distinct single-column result contract.
    pub fn prepare(&self, projection: &str, offset: u64, count: Option<u64>) -> Result<PreparedGraphPattern, PatternBuildError> {
        ...
        operators.extend([GlaOperator::Project { slot: slots[projection_at] }, GlaOperator::Distinct,
            GlaOperator::OrderByVertexId, GlaOperator::Limit { offset, count }]);
        Ok(PreparedGraphPattern { logical: GlaPlan::from_operators(operators),
            variable_count: self.variables.len(), edge_count: self.edges.len(), columns: vec![projection.to_owned()] })
    }
    ...
    pub fn prepare_bindings(&self, projections: &[&str], offset: u64, count: Option<u64>)
        -> Result<PreparedGraphPattern<GraphBindingRow>, PatternBuildError> {
        if self.variables.is_empty() { return Err(PatternBuildError::EmptyPattern); }
        if projections.is_empty() { return Err(PatternBuildError::EmptyProjection); }
        if projections.len() > MAX_PATTERN_VERTICES {
            return Err(PatternBuildError::LimitExceeded { dimension: PatternLimitDimension::Columns,
                limit: MAX_PATTERN_VERTICES, observed: projections.len() });
        }
        let mut projected = Vec::new();
        for name in projections {
            let at = self.variable(name)?;
            if projected.contains(&at) { return Err(PatternBuildError::DuplicateProjection); }
            projected.push(at);
        }
        let (mut operators, slots) = self.compile()?;
        operators.extend([
            GlaOperator::ProjectBindings { slots: projected.into_iter().map(|at| slots[at]).collect() },
            GlaOperator::Distinct, GlaOperator::OrderByBindings, GlaOperator::Limit { offset, count },
        ]);
        Ok(PreparedGraphPattern { logical: GlaPlan::from_operators(operators), variable_count: self.variables.len(),
            edge_count: self.edges.len(), columns: projections.iter().map(|name| (*name).to_owned()).collect() })
    }
    ...
    fn compile(&self) -> Result<(Vec<GlaOperator>, Vec<BindingSlot>), PatternBuildError> {
        ...
        let mut emitted = vec![false; self.identities.len()]; let mut operators = Vec::new();
        if self.edges.is_empty() {
            if self.variables.len() != 1 { return Err(PatternBuildError::Disconnected); }
            operators.push(GlaOperator::ScanVertices); slots[0] = Some(BindingSlot(0));
            self.identities(&slots, &mut emitted, &mut operators); self.select(0, BindingSlot(0), &mut operators);
        } else {
            ...
            if first.source == first.destination {
                operators.push(GlaOperator::VertexIdentity { left: BindingSlot(0), right: BindingSlot(1), equal: true });
            } else { slots[first.destination] = Some(BindingSlot(1)); }
            self.identities(&slots, &mut emitted, &mut operators); self.select(first.source, BindingSlot(0), &mut operators);
            if first.source != first.destination { self.select(first.destination, BindingSlot(1), &mut operators); }
            let mut consumed = vec![false; self.edges.len()]; consumed[0] = true; let mut next_slot = 2_u32;
            for _ in 1..self.edges.len() {
                ...
                let appended = BindingSlot(next_slot); next_slot += 1;
                ...
                } else { slots[target] = Some(appended); }
                self.identities(&slots, &mut emitted, &mut operators);
                if previous.is_none() { self.select(target, appended, &mut operators); }
                consumed[at] = true;
            }
            if slots.iter().any(Option::is_none) { return Err(PatternBuildError::Disconnected); }
        }
        debug_assert!(emitted.iter().all(|emitted| *emitted));
        Ok((operators, slots.into_iter().map(|slot| slot.expect("all variables are connected")).collect()))
    }
}
...
mod tests {
    ...
    fn builder(names: &[&str]) -> GraphPatternBuilder {
        let mut builder = GraphPatternBuilder::new(); for name in names { builder.vertex(name).unwrap(); } builder
    }
    ...
    fn long_paths_branching_cycles_and_reversed_binding_use_existing_operators() {
        ...
        let edges = [(VId(1), RelationId(1), VId(2)), (VId(2), RelationId(1), VId(3)),
            (VId(4), RelationId(2), VId(3)), (VId(4), RelationId(2), VId(1)), (VId(1), RelationId(1), VId(3))];
    ...
    fn assignments(b: &GraphPatternBuilder, edges: &[(VId, RelationId, VId)]) -> Vec<Vec<VId>> {
        ...
        for bits in 0..(1_usize << b.variables.len()) {
            ...
            let matches = b.edges.iter().all(|atom| edges.iter().any(|&(s, r, d)| {
                ...
                let left = assignment[atom.source]; let right = assignment[atom.destination];
            ...
            if identities && matches { result.push(assignment); }
        }
        result
    }
    fn oracle(b: &GraphPatternBuilder, edges: &[(VId, RelationId, VId)], projected: usize) -> Vec<VId> {
        assignments(b, edges).into_iter().map(|row| row[projected]).collect::<BTreeSet<_>>().into_iter().collect()
    }
        ...
        for mask in 0..256_usize {
            ...
            for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
                for shape in 0..3 {
                    ...
                    for columns in [vec!["a", "c"], vec!["d", "b", "a"], vec!["a", "b", "c", "d"]] {
                        let positions: Vec<_> = columns.iter().map(|name| b.variable(name).unwrap()).collect();
                        let expected: Vec<_> = assignments(&b, &edges).into_iter()
                            .map(|row| positions.iter().map(|at| row[*at]).collect::<Vec<_>>())
                            .collect::<BTreeSet<_>>().into_iter().skip(1).take(3).collect();
    ...
    fn validation_is_bounded_and_rejected_edits_leave_the_builder_unchanged() {
        let mut b = builder(&["a"]); let original = b.prepare("a", 0, None).unwrap();
    ...
    fn maximum_length_path_and_node_scan_keep_exact_ordered_pagination() {
        ...
        let b = builder(&["a"]); let query = b.prepare("a", 1, Some(1)).unwrap();
    ...
    fn renamed_patterns_preserve_transcripts_but_identity_and_filters_do_not() {
        ...
        let mut b = builder(&["sensitive_name"]); let original = b.prepare("sensitive_name", 0, None).unwrap();
    ...
    fn general_patterns_share_governed_limits_and_every_interruption_checkpoint() {
        ...
        let edges = [(VId(1), RelationId(1), VId(2)), (VId(2), RelationId(1), VId(3))];
        let wide = GqlQueryPolicy::new(2, 1, u64::MAX, u64::MAX); let mut calls = 0;
        ...
        for stop in 1..=calls {
            let mut at = 0;
            let result = query.plan().execute_governed(2, [], edges, |_, _| Ok::<_, ()>(true), wide, || { at += 1; if at == stop { Err(stop) } else { Ok(()) } });
    ...
    fn tuple_schema_and_column_order_are_immutable_and_validated() {
        ...
        assert_ne!(tuple.canonical_bytes(), b.prepare_bindings(&["a", "b"], 0, None).unwrap().canonical_bytes());
        assert_ne!(b.prepare("a", 0, None).unwrap().canonical_bytes(), b.prepare_bindings(&["a"], 0, None).unwrap().canonical_bytes());
        assert_eq!(b.prepare_bindings(&[], 0, None).unwrap_err(), PatternBuildError::EmptyProjection);
        assert_eq!(b.prepare_bindings(&["a", "a"], 0, None).unwrap_err(), PatternBuildError::DuplicateProjection);
        assert_eq!(b.prepare_bindings(&["missing"], 0, None).unwrap_err(), PatternBuildError::UnknownVariable);
        assert!(matches!(b.prepare_bindings(&["a"; MAX_PATTERN_VERTICES + 1], 0, None),
            Err(PatternBuildError::LimitExceeded { dimension: PatternLimitDimension::Columns, .. })));
        let frozen = tuple.canonical_bytes(); b.identity("a", "b", false).unwrap();
    ...
    fn all_sixty_five_columns_are_retained_without_a_lossy_scalar_conversion() {
        ...
        for i in 0..MAX_PATTERN_EDGES { b.edge(refs[i], RelationId(1), GlaDirection::Forward, refs[i + 1]).unwrap(); }
        let query = b.prepare_bindings(&refs, 0, None).unwrap();
        let rows = query.plan().execute([], (0..MAX_PATTERN_EDGES).map(|i|
            (VId(i as u128), RelationId(1), VId(i as u128 + 1))), |_, _| Ok::<_, ()>(true)).unwrap();
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].values(), &(0..=MAX_PATTERN_EDGES).map(|i| VId(i as u128)).collect::<Vec<_>>());
    }
}
