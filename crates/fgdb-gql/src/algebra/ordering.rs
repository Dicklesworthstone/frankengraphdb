    ...
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::TooManyColumns { limit, observed } => {
                write!(f, "ORDER BY has {observed} columns, limit {limit}")
            }
            Self::UnknownColumn { column } => {
                write!(f, "ORDER BY references unknown output column {column}")
            }
            Self::DuplicateColumn { column } => {
                write!(f, "ORDER BY repeats output column {column}")
            }
            Self::InvalidPlanShape => {
                f.write_str("ORDER BY requires a compiler-owned value projection")
            }
        }
    }
}
...
impl GlaPlan<GraphValueRow> {
    pub(super) fn set_value_order(
        ...
        let previous = order_at
            .checked_sub(1)
            .ok_or(GraphOrderError::InvalidPlanShape)?;
        let projection_at =
            if matches!(self.operators.get(previous), Some(GlaOperator::Distinct)) {
        ...
        for (at, column) in order.iter().enumerate() {
            if column.column >= columns.len() {
                return Err(GraphOrderError::UnknownColumn {
                    column: column.column,
                });
            }
            if order[..at]
                .iter()
                .any(|previous| previous.column == column.column)
            {
                return Err(GraphOrderError::DuplicateColumn {
                    column: column.column,
                });
            }
        }
        ...
        self.operators[order_at] = GlaOperator::OrderByValueColumns {
            columns: std::sync::Arc::from(order),
        };
        Ok(())
    }
}
