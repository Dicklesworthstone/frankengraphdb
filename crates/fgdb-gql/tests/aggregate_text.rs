...
fn mixed_return_order_group_order_and_shared_arguments_match_typed_preparation() {
    ...
    b.edge("b", RelationId(2), GlaDirection::Forward, "c")
        .unwrap();
    b.edge("c", RelationId(1), GlaDirection::Forward, "a")
        .unwrap();
    ...
    let input = b
        .prepare_values(
            &[
                ...
                GraphColumn::property("total", "c", PropertyKeyId(1)),
                GraphColumn::vertex("vias", "b"),
            ],
            0,
            None,
        )
    ...
    let expected = PreparedGraphAggregate::prepare(
        ...
        &[
            ...
            GraphAggregate::count_distinct("vias", 2),
        ],
        1,
        Some(1),
    )
    ...
    assert!(matches!(
        actual.input_pattern().plan().operators().last(),
        ...
        for (direction, arrow) in [(0, "-[:R]->"), (1, "<-[:R]-"), (2, "-[:R]-")] {
            ...
            let result = aggregate
                .execute_governed(
                    ...
                    edges.iter().copied(),
                    |_, _| Ok::<_, ()>(true),
                    |vid, _| Ok((vid == VId(2)).then_some(&high)),
                    policy(),
                    || Ok::<_, ()>(()),
                )
                .unwrap();
            ...
            for &(src, _, dst) in &edges {
                ...
                for (a, b) in pairs {
                    expected
                        .entry(a)
                        .or_default()
                        .push((b == VId(2)).then_some(i64::MAX));
                }
            }
            ...
            for (row, (owner, inputs)) in result.value.iter().zip(expected) {
                ...
                assert_eq!(
                    ...
                    Some(nonnull.iter().collect::<BTreeSet<_>>().len() as u64)
                );
                assert_eq!(
                    row.get(3).unwrap().as_integer(),
                    (!nonnull.is_empty()).then(|| nonnull.iter().map(|x| i128::from(*x)).sum())
                );
            }
            ...
            assert_eq!(
                page.value,
                result.value.into_iter().skip(1).take(1).collect::<Vec<_>>()
            );
        }
    }
}
...
fn malformed_ungrouped_and_unsupported_aggregates_never_resolve_names() {
    ...
    assert!(
        PreparedGraphText::prepare("MATCH (a) RETURN COUNT(*)", symbols).is_err(),
        "ordinary profile must not silently widen"
    );
...
fn shared_argument_errors_and_definition_caps_keep_their_domains() {
    let t = PreparedGraphAggregateText::prepare(
        "MATCH (a:L) WHERE a.n >= $n RETURN COUNT(*) SKIP $page LIMIT $page",
        symbols,
    )
    .unwrap();
    assert_eq!(
        t.bind_parameters(&GqlParameters::new()).unwrap_err().kind,
    ...
    let wrong = GqlParameters::new()
        ...
        .unwrap();
    assert!(matches!(
        t.bind_parameters(&wrong).unwrap_err().kind,
        GraphPatternTextErrorKind::ParameterTypeMismatch { .. }
    ));
    let valid = GqlParameters::new()
        .with_int64("n", i64::MIN)
        .unwrap()
        .with_uint64("page", u64::MAX)
        .unwrap();
    t.bind_parameters(&valid).unwrap();
    assert_eq!(
        t.bind_parameters(&valid.with_int64("extra", 1).unwrap())
            .unwrap_err()
            .kind,
        GraphPatternTextErrorKind::UnexpectedArguments
    );
    let columns = (0..66)
        .map(|at| format!("COUNT(*) AS c{at}"))
        .collect::<Vec<_>>()
        .join(",");
    assert!(
        PreparedGraphAggregateText::prepare(&format!("MATCH (a) RETURN {columns}"), symbols)
            .is_err()
    );
    let text = format!(
        "MATCH (a) RETURN {}",
