...
use fgdb_gql::{
    ...
    GraphExactAverage, GraphIntegerExpression, GraphIntegerOp as Op, GraphNullPlacement as Nulls,
...
fn oracle(
    ...
    let mut groups: Vec<_> = groups
        ...
        .map(|(key, bag)| {
            ...
            let ratio =
                (!present.is_empty()).then(|| (present.iter().sum::<i128>(), present.len() as u64));
            (key, n, ratio)
        })
        .collect();
    groups.sort_by(|a, b| {
    ...
    groups
        ...
        .map(|(key, n, ratio)| {
            ...
            let avg = ratio.map_or_else(
                || sum(None),
                |(s, n)| GraphAggregateValue::Average(GraphExactAverage::new(s, n).unwrap()),
            );
            ...
            for hops in 1..=2 {
                ...
                for desc in [false, true] {
                    for nulls in [Nulls::First, Nulls::Last] {
                        ...
                        {
                            let q = definition(&pattern, hops, offset, count, desc, nulls);
                            ...
                            let expected = oracle(
                                ...
                                hops,
                                offset as usize,
                                count.map_or(usize::MAX, |n| n as usize),
                                desc,
                                nulls,
                            );
...
fn hidden_order_terms_full_keys_and_projection_do_not_change_rank_or_bag_multiplicity() {
    ...
    for q in [
        ...
        q.with_output_projection(vec![GraphSetProjection::new(
            "constant",
            GraphSetValue::Integer(
                GraphIntegerExpression::prepare(&[Op::Literal(Some(1))]).unwrap(),
            ),
        )])
        .unwrap(),
    ] {
    ...
    for (id, end, weight) in [
        ...
        s.edges.insert(
            EId(id),
            (VId(0), R, VId(end), vec![(P, CanonicalScalar::Int(weight))]),
        );
    }
    let rows = run(&q, s, wide()).collect::<Result<Vec<_>, _>>().unwrap();
    assert_eq!(
        rows.iter().map(|r| r.keys()[0].clone()).collect::<Vec<_>>(),
        vec![
...
fn losing_and_zero_page_groups_still_evaluate_having_and_output_failures() {
    for count in [0, 1] {
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN 1/(COUNT(*)-1) AS value GROUP BY b ORDER BY COUNT(*) DESC LIMIT {count}"
        ));
        let mut s = source(0);
        ...
        assert!(cursor.next().is_none());
        let q = prepare(&format!(
            "MATCH (a)-[r:R]->(b) RETURN COUNT(*) AS n GROUP BY b HAVING MIN(r.p)>0 OR TRUE ORDER BY n DESC LIMIT {count}"
        ));
        let mut s = source(0);
        s.edges.insert(
            EId(1),
        ...
        s.edges.insert(
            EId(2),
            (VId(0), R, VId(1), vec![(P, CanonicalScalar::Bool(true))]),
        );
        let mut cursor = run(&q, s, wide());
        assert!(matches!(
            cursor.next(),
...
fn every_ranked_checkpoint_and_exact_limits_preserve_only_completed_deliveries() {
    ...
    for stop in 1..=calls {
        ...
        loop {
            match cursor.next().expect("injected boundary must be reached") {
...
fn ranked_tiny_page_does_not_charge_output_quota_for_offset_or_discarded_groups() {
    let q = prepare(
        "MATCH (a)-[r:R]->(b) RETURN b,SUM(r.p) AS total GROUP BY b ORDER BY total DESC SKIP 3 LIMIT 1",
    );
    let mut s = source(0);
    for id in 0..1024 {
    ...
    assert_eq!(row.keys(), &[GraphValue::Vertex(VId(1020))]);
    assert_eq!(row.values()[0].as_integer(), Some(1020));
    assert_eq!(drops.load(Ordering::SeqCst), 1);
    ...
    for text in [
        ...
        let q = prepare(text);
        assert!(
            run(&q, source(0), GqlQueryPolicy::new(0, 0, u64::MAX, u64::MAX))
                .next()
                .is_none()
        );
    }
