...
use fgdb_gql::algebra::{
    ...
    ScalarPredicate, VertexPredicate,
};
...
fn scalar_predicate_work_is_reserved_before_source_reads_and_memoized() {
    ...
    builder
        .filter(
            ...
            VertexPredicate::ScalarProperty {
                key,
                predicate: ScalarPredicate::new(payload.clone(), IntegerComparison::Equal).unwrap(),
            },
        )
        .unwrap();
    ...
    let result = query
        ...
        .execute_with_control(
            ...
            [],
            |_, predicates| {
                first_read.set(calls.get());
                reads.set(reads.get() + 1);
                Ok::<_, usize>(
                    predicates
                        .iter()
                        .all(|predicate| predicate.matches(&[], &[(key, payload.clone())])),
                )
            },
            |_| {
        ...
        .unwrap();
    assert_eq!(result, vec![VId(1)]);
    assert_eq!(reads.get(), 1);
    assert!(
        first_read.get() >= 6,
        "literal payload was not included in the control seam"
    );
    for stop in 1..=first_read.get() {
        calls.set(0);
        reads.set(0);
        let result = query.plan().execute_with_control(
            [VId(1)],
            [],
            |_, _| {
                reads.set(reads.get() + 1);
...
fn scalar_predicates_share_exact_limits_and_never_convert_source_errors_to_null() {
    ...
    builder
        .filter(
            ...
            VertexPredicate::ScalarProperty {
                key,
                predicate: ScalarPredicate::new(value.clone(), IntegerComparison::Equal).unwrap(),
            },
        )
        .unwrap();
    let pattern = builder
    ...
    for policy in [
        GqlQueryPolicy::new(2, 1, full.evaluator.work_units - 1, u64::MAX),
        GqlQueryPolicy::new(2, 1, u64::MAX, full.evaluator.scratch_entries - 1),
    ] {
        assert!(matches!(run(policy), Err(GqlQueryError::Evaluator(_))));
    }
    assert!(matches!(
        run(GqlQueryPolicy::new(2, 0, u64::MAX, u64::MAX)),
        Err(GqlQueryError::Rows(_))
    ));
    let mut missing = GraphPatternBuilder::new();
    missing.vertex("n").unwrap();
    missing
        .filter("n", VertexPredicate::PropertyNull { key, is_null: true })
        .unwrap();
    let plan = missing.prepare("n", 0, None).unwrap();
    let result = plan
        .plan()
        .execute([VId(1)], [], |_, _| Err::<bool, _>("unreadable vertex"));
...
fn optional_scalar_filters_preserve_null_extension_and_outer_label_scope() {
    ...
    outer
        .filter("a", VertexPredicate::HasLabel(LabelId(1)))
        .unwrap();
    let mut child = GraphPatternBuilder::new();
    child.vertex("a").unwrap();
    ...
    child
        .filter(
            ...
            VertexPredicate::ScalarProperty {
                key,
                predicate: ScalarPredicate::new(accepted.clone(), IntegerComparison::Equal)
                    .unwrap(),
            },
        )
        .unwrap();
    let query = outer
        .prepare_values_with_clauses(
            ...
            &[
                ...
                GraphColumn::property("status", "b", key),
            ],
            0,
            None,
        )
    ...
    let result = query
        ...
        .execute_governed_with_properties(
            ...
            |vid, predicates| {
                ...
                let props = if vid == VId(10) {
                    vec![(key, accepted.clone())]
                } else if vid == VId(20) {
                    vec![(key, rejected.clone())]
                } else {
                    vec![]
                };
                Ok::<_, ()>(
            ...
            |vid, _| {
                assert_eq!(
                    vid,
                    VId(10),
                    "null or rejected optional bindings reached the property source"
                );
                Ok(Some(&accepted))
            },
            wide(),
...
fn literal_type_value_operator_and_null_polarity_are_logical_identity() {
    ...
    let compile = |name: &str, predicate| {
        ...
        builder.filter(name, predicate).unwrap();
        builder.prepare(name, 0, None).unwrap().canonical_bytes()
    };
    let make = |value, comparison| VertexPredicate::ScalarProperty {
        key,
        predicate: ScalarPredicate::new(value, comparison).unwrap(),
    };
    let first = compile(
        "n",
        make(CanonicalScalar::Bool(true), IntegerComparison::Equal),
    );
    assert_eq!(
        first,
        compile(
    ...
    assert_eq!(
        VertexPredicate::HasLabel(LabelId(1)).comparison_work_units(),
        0
    );
    assert_eq!(
