...
use fgdb_gql::{
    GqlParameters, GqlQueryError, GqlQueryExecution, GqlQueryPolicy, GraphIntegerBinary,
    GraphIntegerExpression, GraphIntegerOp, GraphMutationValue, GraphSymbol, GraphSymbolKind,
    PreparedGraphText,
};
...
fn plan(selection: PreparedGraphPattern<GraphValueRow>) -> PreparedGraphInsert {
    ...
    .unwrap();
    PreparedGraphInsert::prepare(
        selection,
        R,
        vec![GraphInsertVertex {
            labels: vec![LabelId(9)],
            properties: vec![
                (Q, GraphMutationValue::Expression(increment)),
                (P, GraphMutationValue::Column(2)),
            ],
        }],
        vec![
            GraphInsertEdge {
...
fn identity(request: GraphInsertRequest) -> Result<ElementId, &'static str> {
    Ok(match request {
        GraphInsertRequest::Vertex { row, vertex } => {
            ElementId::Vertex(VId((1_u128 << 100) + row as u128 * 256 + vertex as u128))
        }
        GraphInsertRequest::Edge { row, edge } => {
            ElementId::Edge(EId((1_u128 << 110) + row as u128 * 256 + edge as u128))
        }
    })
}
fn source(
...
fn values() -> BTreeMap<VId, CanonicalScalar> {
    BTreeMap::from([
        (VId(1), CanonicalScalar::Int(10)),
        (VId(2), CanonicalScalar::Int(20)),
    ])
}
fn run(plan: &PreparedGraphInsert, policy: GraphInsertPolicy) -> ResultOf {
    plan.execute_governed(
        policy,
        |plan, allowance| source(plan, allowance, &edges(), &values()),
        identity,
        || Ok(()),
    )
}
...
fn repeated_matches_create_distinct_row_local_structures_with_frozen_properties() {
    ...
    for (row, (source, _, destination)) in edges().into_iter().enumerate() {
        let ElementId::Vertex(vertex) =
            identity(GraphInsertRequest::Vertex { row, vertex: 0 }).unwrap()
        else {
            unreachable!()
        };
        ...
        for (edge, from, to, properties) in [
            ...
            let ElementId::Edge(id) = identity(GraphInsertRequest::Edge { row, edge }).unwrap()
            else {
                unreachable!()
            };
...
fn declarations_validate_endpoints_and_all_expression_columns_before_execution() {
    ...
    let vertex = GraphInsertVertex {
        labels: vec![],
        properties: vec![],
    };
    for endpoint in [
        GraphInsertEndpoint::Column(1),
        GraphInsertEndpoint::Column(9),
        GraphInsertEndpoint::CreatedVertex(1),
    ] {
        assert!(
            PreparedGraphInsert::prepare(
                input.clone(),
    ...
    assert!(matches!(
        PreparedGraphInsert::prepare(
            ...
            vec![GraphInsertVertex {
                labels: vec![],
                properties: vec![(P, GraphMutationValue::Expression(conditional))]
            }],
            vec![]
        ),
    ...
    assert!(matches!(
        PreparedGraphInsert::prepare(
            ...
            R,
            vec![GraphInsertVertex {
                labels: vec![LabelId(1), LabelId(1)],
                properties: vec![]
            }],
            vec![]
        ),
    ...
    assert!(matches!(
        PreparedGraphInsert::prepare(input.clone(), R, vec![], vec![]),
    ...
    assert!(matches!(
        PreparedGraphInsert::prepare(
            input,
            R,
            vec![vertex; MAX_GRAPH_INSERT_DECLARATIONS + 1],
            vec![]
        ),
...
fn null_endpoints_and_late_bad_values_refuse_before_any_identity_request() {
    ...
    let optional = plan(select(
        "MATCH (n) OPTIONAL MATCH (n)-[:R]->(m) RETURN n,m,n.p AS p",
    ));
    let result = optional.execute_governed(
        policy(),
        |plan, allowance| source(plan, allowance, &edges(), &values()),
        |request| {
    ...
    for bad in [CanonicalScalar::Int(i64::MAX), CanonicalScalar::Bool(true)] {
        ...
        inputs.insert(VId(2), bad);
        let result = standard().execute_governed(
            policy(),
            |plan, allowance| source(plan, allowance, &edges(), &inputs),
            |request| {
                calls.set(calls.get() + 1);
                identity(request)
            },
...
fn allocator_failures_wrong_domains_and_repeated_ids_never_return_partial_batches() {
    ...
    let repeated = standard().execute_governed(
        policy(),
        |plan, allowance| source(plan, allowance, &edges(), &values()),
        |request| {
    ...
    assert!(matches!(
        repeated,
        Err(GqlQueryError::Source(GraphInsertError::DuplicateIdentity {
            request: GraphInsertRequest::Vertex { row: 1, .. }
        }))
    ));
    assert_eq!(count.get(), 4);
    let wrong: ResultOf = standard().execute_governed(
        policy(),
        |plan, allowance| source(plan, allowance, &edges(), &values()),
        |_| Ok(ElementId::Edge(EId(100))),
        || Ok(()),
    );
    assert!(matches!(
        wrong,
        Err(GqlQueryError::Source(GraphInsertError::IdentityKind { .. }))
    ));
    let failed = standard().execute_governed(
        policy(),
        |plan, allowance| source(plan, allowance, &edges(), &values()),
        |request| {
            if request == (GraphInsertRequest::Edge { row: 3, edge: 1 }) {
                Err("identity service failed")
            } else {
                identity(request)
            }
        },
    ...
    assert!(matches!(
        failed,
        Err(GqlQueryError::Source(GraphInsertError::IdentitySource(
...
fn exact_cumulative_limits_and_every_control_checkpoint_are_enforced() {
    ...
    for cap in [
        ...
        GraphInsertPolicy::new(
            GqlQueryPolicy::new(7, 4, measured.evaluator.work_units - 1, u64::MAX),
            4,
            8,
        ),
        GraphInsertPolicy::new(
        ...
        GraphInsertPolicy::new(policy().query, 3, 8),
        GraphInsertPolicy::new(policy().query, 4, 7),
    ] {
    ...
    for stop in 1..=events.get() {
        let seen = Cell::new(0);
        let result = insertion.execute_governed(
            policy(),
            |plan, allowance| source(plan, allowance, &edges(), &values()),
            identity,
            || {
...
fn empty_matches_never_allocate_and_untrusted_source_statistics_cannot_wrap() {
    ...
    let empty = standard()
        .execute_governed(
            GraphInsertPolicy::new(policy().query, 0, 0),
        ...
        let result = standard().execute_governed(
            ...
            |plan, allowance| {
                ...
                match corrupted {
                    ...
                    _ => {
                        result.evaluator.work_units = 0;
                        result.evaluator.scratch_entries = u64::MAX;
                    }
                }
                Ok(result)
            },
