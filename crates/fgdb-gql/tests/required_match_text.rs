...
use fgdb_gql::algebra::{
    ...
    GraphValueRow, GraphWalkSearch, IntegerComparison, VertexPredicate,
};
use fgdb_gql::{
    GqlParameters, GqlQueryPolicy, GraphPatternTextErrorKind, GraphSymbol,
    GraphSymbolKind, GraphWriteStatement, PreparedGraphAggregateText,
...
fn evaluate(text: &str, vertices: &[VId], edges: &[Edge]) -> Vec<GraphValueRow> {
    let pattern = PreparedGraphText::prepare(text, symbols).unwrap()
        .bind_parameters(&GqlParameters::new()).unwrap();
    let values = vertices.iter().map(|vid| (*vid, CanonicalScalar::Int(vid.0 as i64)))
        .collect::<BTreeMap<_, _>>();
    pattern.plan().execute_governed_with_properties(
        (vertices.len() + edges.len()) as u64, vertices.iter().copied(), edges.iter().copied(),
        |vid, predicates| Ok::<_, ()>(predicates.iter().all(|test|
            test.matches(&[L], &[(P, values[&vid].clone())]))),
...
fn repeated_match_binds_to_ordered_typed_clauses_and_resolves_each_symbol_once() {
    ...
    let arguments = GqlParameters::new().with_int64("lo", 1).unwrap()
        .with_int64("cut", 2).unwrap().with_int64("hi", 9).unwrap()
        .with_uint64("off", 1).unwrap().with_uint64("n", 3).unwrap();
...
fn required_match_after_optional_multiplies_real_rows_and_eliminates_absence() {
    ...
    for (tail, expected) in [("c", 4), ("ALL c SKIP 1 LIMIT 2", 2), ("DISTINCT c", 1), ("c LIMIT 0", 0)] {
        let rows = evaluate(&format!("{head} RETURN {tail}"), &vertices, &edges);
        assert_eq!(rows.len(), expected);
        assert!(rows.iter().all(|row| row.values()[0].as_vertex() == Some(VId(3))));
    }
    let absent = evaluate("MATCH (a) OPTIONAL MATCH (a)-[:R]->(b) MATCH (b) WHERE b.p=999 RETURN a", &vertices, &edges);
    assert!(absent.is_empty(), "rejected OPTIONAL witnesses must not invent replacement null rows");
    let independent = evaluate("MATCH (a) OPTIONAL MATCH (a)-[:R]->(b) MATCH (x) RETURN b,x", &vertices, &edges);
    assert_eq!(independent.len(), 20);
    assert_eq!(independent.iter().filter(|row| row.values()[0].is_null()).count(), 12);
    assert_eq!(evaluate("MATCH (a) MATCH (a) WHERE a.p=1 OPTIONAL MATCH (a)-[:T]->(x) RETURN a,x", &vertices, &edges).len(), 1);
}
...
fn each_required_match_keeps_its_own_shortest_or_walk_selector() {
    ...
    let pattern = PreparedGraphText::prepare(text, symbols).unwrap()
        .bind_parameters(&GqlParameters::new()).unwrap();
    let modes = pattern.plan().operators().iter().filter_map(|op| match op {
        GlaOperator::VarLengthExpand { search, .. } => Some(*search), _ => None,
    }).collect::<Vec<_>>();
    assert_eq!(modes, vec![GraphWalkSearch::AllShortest, GraphWalkSearch::AnyShortest, GraphWalkSearch::All]);
    let edges = [(VId(1), R, VId(1)), (VId(1), R, VId(1)),
        (VId(1), S, VId(1)), (VId(1), S, VId(1)), (VId(1), S, VId(1))];
    assert_eq!(evaluate(text, &[VId(1), VId(2)], &edges).len(), 2);
    let null = evaluate("MATCH (a) OPTIONAL MATCH (a)-[:T]->(b) MATCH ANY SHORTEST WALK (b)-[:R*0]->(c) RETURN a,c",
        &[VId(1), VId(2)], &edges);
...
fn aggregation_consumes_the_completed_join_bag_not_optional_prefix_rows() {
    ...
    let rows = query.execute_governed(8, [VId(1), VId(2), VId(3), VId(4)],
        [(VId(1), R, VId(2)), (VId(1), R, VId(2)), (VId(2), S, VId(3)), (VId(2), S, VId(3))],
...
fn malformed_late_clauses_and_scope_overflow_refuse_before_catalog_calls() {
    ...
    for text in [
        "MATCH (a) MATCH RETURN a", "MATCH (a) MATCH () RETURN a",
...
fn original_utf8_parameter_offsets_and_existential_privacy_survive_later_matches() {
    ...
    let wrong = GqlParameters::new().with_uint64("cut", 1).unwrap().with_uint64("take", 2).unwrap();
    ...
    assert_eq!(PreparedGraphText::prepare(hidden, symbols).unwrap_err().kind, GraphPatternTextErrorKind::UnknownVariable);
...
fn joined_write_scripts_and_parameter_batches_share_the_same_selection_compiler() {
    ...
    let args = GqlParameters::new().with_int64("start", 1).unwrap().with_int64("value", 8).unwrap();
    let bound = script.bind_parameters(&args).unwrap();
    let [GraphWriteStatement::Insert(_), GraphWriteStatement::Mutation(update), GraphWriteStatement::Delete(delete)] = bound.statements()
        else { panic!("wrong joined write dispatch") };
    let ops = update.selection().plan().operators();
    let boundary = ops.iter().position(|op| matches!(op, GlaOperator::OptionalEnd { .. })).unwrap();
    assert!(matches!(ops.get(boundary + 1), Some(GlaOperator::BindVertex { .. })));
    assert!(delete.selection().plan().operators().iter().any(|op| matches!(op, GlaOperator::BindVertex { .. })));
