...
use fgdb_gql::algebra::{
    ...
    IntegerComparison, PatternBuildError, PreparedGraphPattern, VertexPredicate,
};
...
fn path(direction: GlaDirection, closed: bool) -> GraphPatternBuilder {
    ...
    b.edge("x", S, direction, if closed { "a" } else { "y" })
        .unwrap();
    b
}
...
fn exhaustive_path_probes_match_concrete_assignment_oracle_and_do_not_multiply_outer_bags() {
    ...
    let universe = [
        (VId(0), R, VId(1)),
        (VId(1), R, VId(1)),
        (VId(u128::MAX), R, VId(0)),
        (VId(1), S, VId(0)),
        (VId(0), S, VId(1)),
        (VId(1), S, VId(u128::MAX)),
    ];
    ...
    for direction in [
        ...
        for closed in [false, true] {
            ...
            for anti in [false, true] {
                ...
                assert!(pattern.plan().reads_edges());
                for encoding in 0..3_usize.pow(universe.len() as u32) {
                    ...
                    expected.sort();
                    assert_eq!(
                        run(&pattern, &vertices, &edges),
                        expected,
                        "case={encoding}, {direction:?}, closed={closed}, anti={anti}"
                    );
                }
            }
        }
...
fn all_outer_correlations_are_enforced_and_inner_variables_are_clause_local() {
    ...
    let mut left = node();
    left.vertex("x").unwrap();
    left.edge("a", S, GlaDirection::Forward, "x").unwrap();
    let mut right = node();
    right.vertex("x").unwrap();
    right.edge("a", T, GlaDirection::Forward, "x").unwrap();
    let scopes = node()
        .prepare_values_with_existence(
            &[
                GraphExistence::exists(&left),
                GraphExistence::exists(&right),
            ],
...
fn existence_stops_at_the_first_complete_witness_but_errors_are_not_absence() {
    let mut probe = node();
    probe.vertex("x").unwrap();
    probe.edge("a", R, GlaDirection::Forward, "x").unwrap();
    probe
        .filter(
            "x",
    ...
    for anti in [false, true] {
        ...
        let output = query
            ...
            .unwrap();
        assert_eq!(reads, 1);
        assert_eq!(output.value.is_empty(), anti);
        let refused = query.plan().execute_governed_with_properties(
            99,
            [VId(1)],
            edges,
            |_, _| Err::<bool, _>("unreadable probe vertex"),
            |_, _| Ok(None),
            wide(),
            || Ok::<_, ()>(()),
        );
        assert!(matches!(
            refused,
            Err(GqlQueryError::Source("unreadable probe vertex"))
        ));
    }
}
...
fn correlated_label_tests_do_not_become_mandatory_outer_scan_labels() {
    let mut probe = node();
    probe
        .filter("a", VertexPredicate::HasLabel(LabelId(7)))
        .unwrap();
    let query = node()
        .prepare_values_with_existence(
            &[GraphExistence::not_exists(&probe)],
        ...
        .execute_governed_with_properties(
            ...
            |vid, predicates| {
                Ok::<_, ()>(
                    predicates
                        .iter()
                        .all(|p| p.matches(if vid == VId(0) { &[LabelId(7)] } else { &[] }, &[])),
                )
            },
            |_, _| Ok(None),
            wide(),
...
fn probes_share_the_policy_and_every_interruption_propagates_without_rows() {
    ...
    assert_eq!(ids(&full.value), vec![VId(2), VId(3)]);
    ...
    assert!(matches!(
        run(GqlQueryPolicy::new(5, 1, u64::MAX, u64::MAX)),
...
fn definitions_are_scoped_bounded_and_change_logical_identity() {
    ...
    assert_eq!(
        base.prepare_values_with_existence(&[], &columns, 0, None)
            .unwrap(),
        base.prepare_values(&columns, 0, None).unwrap()
    );
    let yes = base
        .prepare_values_with_existence(&[GraphExistence::exists(&probe)], &columns, 0, None)
        .unwrap();
    let no = base
        .prepare_values_with_existence(&[GraphExistence::not_exists(&probe)], &columns, 0, None)
        .unwrap();
    assert_ne!(yes.canonical_bytes(), no.canonical_bytes());
    assert_eq!(
        base.prepare_values_with_existence(
            &[GraphExistence::exists(&probe)],
            &[GraphColumn::vertex("x", "x")],
            0,
            None
        )
        .unwrap_err(),
    ...
    let independent = base
        .prepare_values_with_existence(&[GraphExistence::exists(&uncorrelated)], &columns, 0, None)
        .unwrap()
        .with_duplicates();
    assert_eq!(
        run(&independent, &[VId(9), VId(10)], &[]),
        vec![VId(9), VId(10)]
    );
    ...
    assert!(matches!(
        base.prepare_values_with_existence(
            &[GraphExistence::exists(&probe); 33],
            &columns,
            0,
            None
        ),
