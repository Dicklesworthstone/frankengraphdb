...
fn hidden_keys_bind_once_and_public_slots_do_not_use_evaluation_indices() {
    ...
    assert_eq!(
        bound.evaluation_key_columns(),
        &["__fgdb_group_0", "__fgdb_group_1", "target"]
    );
    ...
    let input = builder
        .prepare_values(
            &[
                ...
                GraphColumn::vertex("__fgdb_group_1", "a"),
                GraphColumn::vertex("target", "b"),
            ],
            0,
            None,
        )
    ...
    assert_eq!(
        prepare(visible),
        prepare(&visible.replace("RETURN ", "RETURN DISTINCT "))
    );
}
...
fn all_keeps_equal_projected_groups_but_distinct_runs_before_pagination() {
    ...
    let edges: Vec<_> = counts
        .iter()
        .enumerate()
        .flat_map(|(at, count)| std::iter::repeat_n((VId(at as u128), R, VId(10)), *count as usize))
        .collect();
    ...
    assert!(
        prepare("MATCH (a) RETURN COUNT(*) AS n GROUP BY a")
            .execute_governed(
                0,
                [],
                [],
    ...
    assert_eq!(
        prepare("MATCH (a) RETURN COUNT(*) AS n")
            .execute_governed(
                0,
                [],
                [],
...
fn hidden_scalar_keys_use_declared_parameters_and_generated_names_cannot_capture_aliases() {
    ...
    .unwrap();
    let scalars = [
        CanonicalScalar::ucs_basic_text("x").unwrap(),
        CanonicalScalar::ucs_basic_text("x").unwrap(),
        CanonicalScalar::ucs_basic_text("y").unwrap(),
        CanonicalScalar::Null,
    ];
    for (wanted, count) in [("x", 2), ("y", 1)] {
        let bound = template
            .bind_parameters(&GqlParameters::new().with_text("wanted", wanted).unwrap())
            .unwrap();
        let rows = bound
            .execute_governed(
                4,
    ...
    assert_eq!(calls, 1);
    let query = prepare(
        "MATCH (a) RETURN COUNT(*) AS __fgdb_group_0,COUNT(a) AS __fgdb_hidden_0 \
    ...
    assert_eq!(
        ...
        &["__fgdb_group_1", "__fgdb_hidden_1"]
    );
    assert_eq!(
        query.aggregate_columns(),
        &["__fgdb_group_0", "__fgdb_hidden_0"]
    );
    ...
    assert_eq!(
        alias.ordering()[0].column,
        GraphAggregateColumn::Aggregate(0)
    );
    let keyword = prepare(
        "MATCH (true) RETURN COUNT(*) AS n GROUP BY true HAVING true IS NOT NULL ORDER BY true",
    );
    assert_eq!(
        keyword.having()[0].column,
...
fn distinct_uses_exact_shared_budgets_and_every_interruption_checkpoint() {
    let query =
        prepare("MATCH (a) RETURN DISTINCT COUNT(*) AS n GROUP BY a.p ORDER BY a.p LIMIT 2");
    ...
    assert_eq!(
        run(GqlQueryPolicy::new(5, 2, work, scratch)).unwrap(),
        complete
    );
    for policy in [
        GqlQueryPolicy::new(4, 2, u64::MAX, u64::MAX),
        GqlQueryPolicy::new(5, 1, u64::MAX, u64::MAX),
        GqlQueryPolicy::new(5, 2, work - 1, u64::MAX),
        GqlQueryPolicy::new(5, 2, u64::MAX, scratch - 1),
    ] {
    ...
    let query =
        prepare("MATCH (a) RETURN DISTINCT COUNT(*) AS n GROUP BY a.p HAVING a.p>0 LIMIT 0");
    ...
    let result = query.execute_governed(
        ...
        [],
        |_, _| Ok::<_, &str>(true),
        |_, _| Err::<Option<&CanonicalScalar>, _>("hidden grouping read"),
        wide(),
        || Ok::<_, ()>(()),
    );
    assert!(matches!(
