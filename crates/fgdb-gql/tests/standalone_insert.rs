...
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryExecution, GqlQueryPolicy,
    ...
    GraphMutationValue, PreparedGraphText};
...
fn declarations() -> (Vec<GraphInsertVertex>, Vec<GraphInsertEdge>) {
    let value = GraphMutationValue::Literal(GqlScalarParameter::new(CanonicalScalar::Int(7)).unwrap());
    (vec![
        GraphInsertVertex { labels: vec![LabelId(2)], properties: vec![(P, value)] },
        GraphInsertVertex { labels: vec![], properties: vec![] },
    ], vec![
        GraphInsertEdge { source: GraphInsertEndpoint::CreatedVertex(0),
            destination: GraphInsertEndpoint::CreatedVertex(1), properties: vec![] },
        GraphInsertEdge { source: GraphInsertEndpoint::CreatedVertex(1),
            destination: GraphInsertEndpoint::CreatedVertex(1), properties: vec![] },
    ])
}
...
fn unit_creates_exactly_one_structure_without_observing_any_graph_source() {
    ...
    assert_eq!((batch.stats().selection.snapshot_records, batch.stats().selection.result_rows), (0, 1));
    assert_eq!((batch.stats().created_vertices, batch.stats().created_edges), (2, 2));
    assert_eq!(batch.intents(), &[
        GraphInsertIntent::Vertex { vertex: VId(100), labels: vec![LabelId(2)], properties: vec![(P, CanonicalScalar::Int(7))] },
        GraphInsertIntent::Vertex { vertex: VId(101), labels: vec![], properties: vec![] },
        GraphInsertIntent::Edge { edge: EId(200), source: VId(100), destination: VId(101), properties: vec![] },
        GraphInsertIntent::Edge { edge: EId(201), source: VId(101), destination: VId(101), properties: vec![] },
    ]);
...
fn empty_match_and_unit_have_different_cardinality_and_definition_identity() {
    let selection = PreparedGraphText::prepare("MATCH (n) RETURN n", |_, _| None).unwrap()
    ...
    assert!(matched.canonical_bytes().starts_with(b"fgdb:query-graph-insert:v1\0"));
    assert!(unit().canonical_bytes().starts_with(b"fgdb:standalone-graph-insert:v1\0"));
    ...
    for count in 0..=3_u128 {
        ...
        let result: ResultOf = matched.execute_governed(
            ...
            |pattern, allowance| {
                ...
                pattern.plan().execute_governed_with_properties(count as u64,
                    (1..=count).map(VId), [], |_, _| Ok(true), |_, _| Ok(None), allowance, || Ok(()))
            }, identity, || Ok(()));
        let batch = result.unwrap();
        assert_eq!(calls.get(), 1);
        assert_eq!(batch.stats().created_vertices, count as u64 * 2);
        assert_eq!(batch.stats().created_edges, count as u64 * 2);
        if count == 1 { assert_eq!(batch.intents(), run(&unit(), policy()).unwrap().intents()); }
    }
}
...
fn unit_has_no_column_domain_even_in_lazy_branches() {
    ...
    for value in [GraphMutationValue::Column(0), GraphMutationValue::Expression(hidden)] {
        assert!(matches!(PreparedGraphInsert::prepare_standalone(R,
            vec![GraphInsertVertex { labels: vec![], properties: vec![(P, value)] }], vec![]),
            Err(GraphInsertBuildError::ValueColumn { declaration: 0, column: 0 })));
    }
    let (vertices, mut edges) = declarations();
    edges[0].source = GraphInsertEndpoint::Column(0);
    assert!(matches!(PreparedGraphInsert::prepare_standalone(R, vertices, edges),
        Err(GraphInsertBuildError::EndpointColumn { edge: 0, column: 0 })));
    assert!(matches!(PreparedGraphInsert::prepare_standalone(R, vec![], vec![]), Err(GraphInsertBuildError::Empty)));
}
...
fn standalone_limits_arithmetic_and_cancellation_share_the_original_error_boundaries() {
    ...
    let exact = GraphInsertPolicy::new(GqlQueryPolicy::new(0, 1,
        measured.evaluator.work_units, measured.evaluator.scratch_entries), 2, 2);
    ...
    for cap in [
        ...
        let calls = Cell::new(0);
        let result = plan.execute_governed(cap, no_source,
            |request| { calls.set(calls.get() + 1); identity(request) }, || Ok(()));
        assert!(result.is_err());
        assert_eq!(calls.get(), 0);
    }
    for cap in [
        GraphInsertPolicy::new(GqlQueryPolicy::new(0, 1, measured.evaluator.work_units - 1, u64::MAX), 2, 2),
        GraphInsertPolicy::new(GqlQueryPolicy::new(0, 1, u64::MAX, measured.evaluator.scratch_entries - 1), 2, 2),
    ] { assert!(run(&plan, cap).is_err()); }
    let (mut vertices, edges) = declarations();
    vertices[1].properties.push((P, GraphMutationValue::Expression(GraphIntegerExpression::prepare(&[
    ...
    let calls = Cell::new(0);
    let result = invalid.execute_governed(policy(), no_source,
        |request| { calls.set(calls.get() + 1); identity(request) }, || Ok(()));
    assert!(matches!(result, Err(GqlQueryError::Source(GraphInsertError::Arithmetic { row: 0, declaration: 1, .. }))));
    assert_eq!(calls.get(), 0);
    let mut events = 0;
    plan.execute_governed(policy(), no_source, identity, || { events += 1; Ok(()) }).unwrap();
