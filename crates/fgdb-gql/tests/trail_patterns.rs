...
use fgdb_gql::algebra::{
    GlaDirection, GlaOperator, GraphColumn, GraphPathFunction, GraphPatternBuilder, GraphValueRow,
...
fn endpoint_only_trails_require_real_ids_and_refuse_legacy_projection_sources() {
    ...
    assert_eq!(
        consumed.get(),
        0,
        "never fabricate EIds from triple ordinals"
    );
    ...
    for mask in 0..32 {
        ...
        for direction in [
            ...
            for &(eid, left, _, right) in &edges {
                let (left, right) = if direction == GlaDirection::Reverse {
                    (right, left)
                } else {
                    (left, right)
                };
                oriented.push((eid, left, right));
                if direction == GlaDirection::Undirected && left != right {
                    oriented.push((eid, right, left));
                }
            }
            for maximum in 0..=3 {
                for minimum in 0..=maximum {
                    let mut layer: Vec<_> = VERTICES
                        .into_iter()
                        .map(|v| (v, Vec::<(EId, VId)>::new()))
                        .collect();
                    ...
                    let mut actual: Vec<_> = captured
                        .value
                        .iter()
                        .map(|row| {
                            let path = row.get(2).unwrap().as_path().unwrap();
                            assert_eq!(
                                row.get(3).unwrap().as_scalar(),
                    ...
                    let mut expected: Vec<_> = expected
                        .iter()
                        .map(|(start, path)| (*start, path.last().map_or(*start, |step| step.1)))
                        .collect();
                    expected.sort();
...
fn captured_paths_cost_more_than_endpoint_pulls_without_changing_multiplicity() {
    ...
    assert!(
        run(&query(GlaDirection::Undirected, 2, 2, false), &one, wide())
            .value
            .is_empty()
    );
    ...
    assert_eq!(
        run(
            &query(GlaDirection::Undirected, 2, 2, false),
            &parallel,
            wide()
        )
        .value
    ...
    assert!(
        run(
            &query(GlaDirection::Forward, 1024, 1024, true),
            &looped,
            GqlQueryPolicy::new(4, 0, 1000, 1000)
        )
        .value
        .is_empty()
    );
...
fn exact_aggregates_keep_path_identity_and_endpoint_only_trail_occurrences() {
    ...
    let hops: i128 = source
        .value
        .iter()
        .map(|row| row.get(2).unwrap().as_path().unwrap().len() as i128)
        .sum();
    let aggregate = PreparedGraphAggregate::prepare(
        ...
        &[
            ...
            GraphAggregate::count_distinct("paths", 2),
    ...
    let result = aggregate
        .execute_governed_with_identified_properties(
            ...
            |_, _| Ok::<_, ()>(true),
            |_, _| Ok(None),
            GqlQueryPolicy::new(8, 1, 2_000_000, 2_000_000),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    ...
    let endpoint = PreparedGraphAggregate::prepare(
        query(GlaDirection::Forward, 1, 3, false),
        &[],
        &[GraphAggregate::count_rows("rows")],
        0,
        None,
...
fn late_property_failure_and_rejected_builder_edits_do_not_become_empty_success() {
    ...
    assert!(matches!(
        result,
        Err(GqlQueryError::Source("late property failure"))
    ));
}
