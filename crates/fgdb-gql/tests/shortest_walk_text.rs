...
use fgdb_gql::algebra::{GlaDirection, GlaOperator, GraphColumn, GraphPatternBuilder,
    GraphValueRow, GraphWalkSearch, IntegerComparison, PreparedGraphPattern, VertexPredicate};
use fgdb_gql::{GqlParameters, GqlQueryPolicy, GraphPatternTextErrorKind, GraphSymbol,
    GraphSymbolKind, GraphWalkBounds, GraphWriteStatement, PreparedGraphAggregateText,
...
fn native_directions_bind_to_identical_typed_shortest_atoms_without_catalog_reentry() {
    for (pattern, direction) in [("(a)-[:R*0..3]->(b)", GlaDirection::Forward),
        ("(a)<-[:R*0..3]-(b)", GlaDirection::Reverse),
        ("(a)-[:R*0..3]-(b)", GlaDirection::Undirected)] {
        let text = format!("MATCH ALL SHORTEST WALK {pattern} WHERE a.p=$key RETURN ALL a,b SKIP $off LIMIT $count");
        let mut calls = BTreeSet::new();
        let template = PreparedGraphText::prepare(&text, |kind, name| {
            assert!(calls.insert((kind, name.to_owned())));
            symbols(kind, name)
        }).unwrap();
        let arguments = GqlParameters::new().with_int64("key", 7).unwrap()
        ...
        builder.shortest_walk("a", R, direction, "b", GraphWalkBounds::new(0, 3).unwrap()).unwrap();
...
fn shortest_selector_and_return_quantifier_are_independent() {
    ...
    for (head, tail, expected) in [("ALL SHORTEST WALK", "a,b", 2),
        ("ALL SHORTEST WALK", "DISTINCT a,b", 1),
        ("ALL SHORTEST WALK", "ALL a,b SKIP 1 LIMIT 1", 1),
        ("ALL SHORTEST WALK", "ALL a,b LIMIT 0", 0), ("WALK", "ALL a,b", 14)] {
...
fn zero_hop_isolates_and_positive_lower_bounds_keep_native_semantics() {
    ...
    let rows = zero.plan().execute_governed_with_properties(2, [VId(7), VId(9)], [],
        |_, _| Ok::<_, ()>(true), |_, _| Ok(None), wide(), || Ok::<_, ()>(())).unwrap();
    let pairs = rows.value.iter().map(|row| (row.values()[0].as_vertex().unwrap(),
        row.values()[1].as_vertex().unwrap())).collect::<Vec<_>>();
    assert_eq!(pairs, vec![(VId(7), VId(7)), (VId(9), VId(9))]);
    let positive = prepare("MATCH ALL SHORTEST WALK (a)-[:R*2..3]->(a) RETURN a");
    let rows = positive.plan().execute_governed_with_properties(2, [VId(1)], [(VId(1), R, VId(1))],
        |_, _| Ok::<_, ()>(true), |_, _| Ok(None), wide(), || Ok::<_, ()>(())).unwrap();
    assert_eq!(rows.value.len(), 1, "a lower bound must not post-filter away the admissible closed walk");
}

#[test]
fn each_match_scope_retains_its_own_explicit_search_selector() {
    let mixed = prepare("MATCH WALK (s)-[:R*1..3]->(a) OPTIONAL MATCH ALL SHORTEST WALK (a)-[:S*0..2]->(b) RETURN s,a,b");
    assert_eq!(modes(&mixed), vec![GraphWalkSearch::All, GraphWalkSearch::AllShortest]);
    let reverse = prepare("MATCH ALL SHORTEST WALK (s)-[:R*1..3]->(a) OPTIONAL MATCH WALK (a)-[:S*0..2]->(b) RETURN s,a,b");
    assert_eq!(modes(&reverse), vec![GraphWalkSearch::AllShortest, GraphWalkSearch::All]);
    let vertices = [VId(1), VId(2), VId(3)];
    let edges = [(VId(1), R, VId(2)), (VId(1), R, VId(2))];
    for (quantifier, expected) in [("EXISTS", vec![VId(1)]), ("NOT EXISTS", vec![VId(2), VId(3)])] {
        let query = prepare(&format!("MATCH (a) WHERE {quantifier} {{ MATCH ALL SHORTEST WALK (a)-[:R*1..3]->(b) }} RETURN a"));
        let rows = query.plan().execute_governed_with_properties(5, vertices, edges,
            |_, _| Ok::<_, ()>(true), |_, _| Ok(None), wide(), || Ok::<_, ()>(())).unwrap();
        assert_eq!(rows.value.iter().map(|row| row.values()[0].as_vertex().unwrap()).collect::<Vec<_>>(), expected);
    }
}
...
fn shortest_arguments_and_utf8_diagnostics_use_the_original_statement() {
    let text = "\u{2003}MATCH ALL SHORTEST WALK (a)-[:R*1..3]->(b) WHERE a.p=$key RETURN b LIMIT $count";
...
fn native_shortest_selection_flows_through_write_scripts_without_another_matcher() {
    ...
    let arguments = GqlParameters::new().with_int64("key", 1).unwrap().with_int64("value", 9).unwrap();
    let program = script.bind_parameters(&arguments).unwrap();
    let [GraphWriteStatement::Insert(_), GraphWriteStatement::Mutation(update), GraphWriteStatement::Delete(delete)] = program.statements()
        else { panic!("wrong native write dispatch") };
    assert_eq!(modes(update.selection()), vec![GraphWalkSearch::AllShortest]);
    assert_eq!(modes(delete.selection()), vec![GraphWalkSearch::AllShortest]);
    let batch = script.bind_parameter_sets(&[arguments.clone(), arguments]).unwrap();
...
fn aggregation_counts_shortest_occurrences_instead_of_all_walks_or_distinct_endpoints() {
    ...
    let rows = query.execute_governed(4, [VId(1), VId(2)], [(VId(1), R, VId(1)); 2],
        |_, _| Ok::<_, ()>(true), |_, _| Ok::<Option<&CanonicalScalar>, ()>(None), wide(), || Ok::<_, ()>(())).unwrap().value;
