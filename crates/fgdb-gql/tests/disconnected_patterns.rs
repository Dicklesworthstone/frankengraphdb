...
use fgdb_gql::algebra::{
    ...
    MAX_PATTERN_BINDINGS, MAX_PATTERN_VERTICES, PatternBuildError, PatternLimitDimension,
    PreparedGraphPattern, VertexPredicate,
};
use fgdb_gql::{
    GlaExecutionEvent, GqlParameters, GqlQueryError, GqlQueryPolicy, GraphSymbol, GraphSymbolKind,
    PreparedGraphText,
};
...
fn fixture() -> Vertices {
    ...
    for (id, label, cik) in [
        ...
        vertices.insert(
            ...
            (
                vec![LabelId(label)],
                cik.map(|value| vec![(CIK, CanonicalScalar::Int(value))])
                    .unwrap_or_default(),
            ),
        );
    }
...
fn matches(vertices: &Vertices, vid: VId, predicates: &[VertexPredicate]) -> bool {
    vertices.get(&vid).is_some_and(|(labels, props)| {
        predicates
            .iter()
            .all(|predicate| predicate.matches(labels, props))
    })
}
...
fn isolated_property_joins_preserve_correlation_nulls_bags_and_pagination() {
    ...
    assert_eq!(
        ids(&run(&pattern, &vertices, &[])),
        vec![some(&[1, 10]), some(&[1, 11]), some(&[2, 12])]
    );
    assert_eq!(
        ids(&run(
            &query(&format!("{head} RETURN company")),
            &vertices,
            &[]
        )),
    ...
    assert_eq!(
        ids(&run(
            &query(&format!("{head} RETURN DISTINCT company")),
            &vertices,
            &[]
        )),
        vec![some(&[1]), some(&[2])]
    ...
    assert_eq!(
        ids(&run(
            &query(&format!("{head} RETURN company SKIP 1 LIMIT 1")),
            &vertices,
            &[]
        )),
        vec![some(&[1])]
    ...
    let boolean = query(
        "MATCH (company:Company),(filing:Filing) \
...
fn disconnected_edge_components_and_isolates_match_exhaustive_assignment_bags() {
    ...
    let universe: Vec<_> = (1..=2)
        .flat_map(|relation| {
            (1..=2).flat_map(move |source| {
                (1..=2).map(move |target| (VId(source), RelationId(relation), VId(target)))
            })
        })
        .collect();
    for mask in 0..256_usize {
        ...
        for ((text, atoms, width, selected, distinct, paged), pattern) in
            cases.iter().zip(&patterns)
        {
            assert!(
                !pattern.plan().scans_edges(),
                "independent components require both domains"
            );
            let mut expected = Vec::new();
            for mut encoding in 0..3_usize.pow(*width as u32) {
                let assignment: Vec<_> = (0..*width)
                    .map(|_| {
                        let value = VId((encoding % 3 + 1) as u128);
                        encoding /= 3;
                        value
                    })
                    .collect();
                if assignment[0] == assignment[width - 1] {
                    continue;
                }
            ...
            if *paged {
                expected = expected.into_iter().skip(1).take(5).collect();
            }
            assert_eq!(
                ids(&run(pattern, &vertices, &edges)),
                expected,
                "mask={mask}, {text}"
            );
        }
...
fn optional_frames_after_disconnected_outer_and_children_keep_null_correlations() {
    ...
    assert_eq!(
        ids(&run(&positive, &vertices, &[])),
        vec![some(&[1, 10]), some(&[1, 11])]
    );
...
fn closing_endpoints_and_isolated_child_anchors_do_not_alias_later_slots() {
    let vertices = fixture();
    let pattern = query(
        "MATCH (company:Company)-[:R]->(company),(filing:Filing) \
...
fn independent_scans_respect_exact_limits_and_every_interruption_checkpoint() {
    ...
    assert_eq!(
        run(GqlQueryPolicy::new(count, rows, work, scratch)).unwrap(),
        measured
    );
    for cap in [
        GqlQueryPolicy::new(count - 1, rows, work, scratch),
...
fn independent_existence_stops_at_first_witness_and_never_converts_failure_to_absence() {
    let pattern = query(
        "MATCH (company:Company) WHERE EXISTS { \
    ...
    let anti = query(
        "MATCH (company:Company) WHERE NOT EXISTS { \
        MATCH (company),(filing:Filing) } RETURN company",
    );
    let failed = anti.plan().execute_with_properties_control(
        [VId(1)],
        [],
        |_, predicates| {
            if predicates == [VertexPredicate::HasLabel(LabelId(1))] {
...
fn full_independent_width_and_definition_wide_scope_bounds_are_enforced() {
    ...
    root.vertex("anchor").unwrap();
    let mut full = GraphPatternBuilder::new();
    for index in 0..MAX_PATTERN_VERTICES {
        full.vertex(&format!("v{index}")).unwrap();
    }
    let names: Vec<_> = (0..MAX_PATTERN_VERTICES)
    ...
    assert_eq!(
        full.plan()
            .operators()
            .iter()
            .filter(|op| matches!(op, GlaOperator::ScanVertices))
            .count(),
        MAX_PATTERN_VERTICES
    );
    let mut child = GraphPatternBuilder::new();
    child.vertex("anchor").unwrap();
    for index in 1..64 {
        child.vertex(&format!("inner{index}")).unwrap();
    }
    let clauses = [
    ...
    assert_eq!(
        ids(&run(
            &exact,
            &BTreeMap::from([(VId(1), (vec![], vec![]))]),
            &[]
        )),
        vec![some(&[1])]
    );
    let mut one_more = GraphPatternBuilder::new();
    one_more.vertex("anchor").unwrap();
    let overflowing = [
        GraphMatchClause::exists(&child),
        GraphMatchClause::exists(&child),
        GraphMatchClause::exists(&child),
        GraphMatchClause::exists(&one_more),
    ];
    assert!(
        matches!(root.prepare_values_with_clauses(&overflowing, &columns, 0, None),
    ...
    uncorrelated.vertex("other").unwrap();
    let independent = root
        .prepare_values_with_clauses(
            &[GraphMatchClause::exists(&uncorrelated)],
            &columns,
            0,
            None,
        )
    ...
    let mut connected = GraphPatternBuilder::new();
    connected.vertex("a").unwrap();
    connected.vertex("b").unwrap();
    connected
