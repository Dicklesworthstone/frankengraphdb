...
use fgdb_gql::{
    GqlParameters, GqlQueryError, GqlQueryExecution, GqlQueryPolicy, GraphAggregateError,
    GraphAggregateRow, GraphSymbol, GraphSymbolKind, PreparedGraphAggregate,
...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(10_000, 1000, 1_000_000, 100_000)
}
fn run(
    ...
    GqlQueryError<GraphAggregateError<&'static str>, usize>,
> {
    query.execute_governed(
        edges.len() as u64,
        [],
        edges.iter().copied(),
        |_, _| Err("unexpected predicate"),
        |_, _| Err("unexpected property"),
        wide(),
        || Ok::<_, usize>(()),
    )
}
    ...
    rows.iter()
        .map(|row| {
            (
                ...
                row.get(4).unwrap().as_value().unwrap().as_vertex().unwrap(),
            )
        })
        .collect()
}
...
fn oracle(
    ...
    for bits in 0..1_usize << variables {
        let row: Vec<_> = (0..variables)
            ...
            .collect();
        if identity.is_some_and(|(a, b, equal)| (row[a] == row[b]) != equal) {
            continue;
        }
        let weight: u64 = atoms
            .iter()
            .map(|&(a, r, b)| {
                edges
                    .iter()
                    .filter(|edge| matches(**edge, row[a], r, row[b], direction))
                    .count() as u64
            })
            .product();
        if weight == 0 {
        ...
        let group = groups
            .entry(columns.iter().map(|at| row[*at]).collect())
            .or_default();
        group.0 += weight;
...
fn statement(
    ...
    format!(
        "MATCH {}{condition} RETURN {keys},COUNT(*) AS n,COUNT(n0) AS m,\
        COUNT(DISTINCT n0) AS d,MIN(n0) AS lo,MAX(n0) AS hi GROUP BY {keys}",
        parts.join(",")
    )
}
...
fn nonterminal_branches_cycles_and_observed_ancestors_match_assignment_oracle() {
    ...
    let cases: [Case<'_>; 6] = [
        ...
        (
            &[(0, 1, 1), (1, 2, 2), (2, 2, 3), (1, 1, 4), (4, 2, 0)],
            &[4],
            5,
            None,
        ),
        (
            &[(0, 1, 1), (1, 2, 2), (1, 1, 3), (3, 2, 4), (3, 1, 5)],
            &[3, 5],
            6,
            None,
        ),
    ...
    let universe: Vec<_> = [VId(0), VId(u128::MAX)]
        .into_iter()
        .flat_map(|a| {
            [RelationId(1), RelationId(2)]
                .into_iter()
                .flat_map(move |r| [VId(0), VId(u128::MAX)].into_iter().map(move |b| (a, r, b)))
        })
        .collect();
    for direction in 0..3 {
        let queries: Vec<_> = cases
            .iter()
            .map(|(atoms, columns, _, identity)| {
                prepare(&statement(atoms, columns, direction, *identity))
            })
            .collect();
...
fn a_billion_early_assignments_collapse_before_a_returned_cycle_vertex() {
    let mut edges = vec![
        ...
        (VId(1), RelationId(3), VId(20)),
        (VId(20), RelationId(4), VId(0)),
    ];
    edges.extend((2..10).map(|id| (VId(1), RelationId(2), VId(id))));
    let tail = " RETURN c,COUNT(*) AS n,COUNT(DISTINCT a) AS d GROUP BY c";
    let early = prepare(&format!(
        "MATCH (a)-[:R]->(b){},(b)-[:T]->(c)-[:U]->(a){tail}",
        branches(10)
    ));
    let late = prepare(&format!(
        "MATCH (a)-[:R]->(b)-[:T]->(c)-[:U]->(a){}{tail}",
        branches(10)
    ));
...
fn maximum_width_overflow_support_and_zero_completion_survive_remapping() {
    let edges: Vec<_> = std::iter::once((VId(0), RelationId(1), VId(1)))
        ...
        .chain(std::iter::once((VId(1), RelationId(4), VId(20))))
        .collect();
    // 64 edge atoms and 65 binding positions; the last one is retained.
    let head = format!("MATCH (a)-[:R]->(b){},(b)-[:U]->(c)", branches(62));
    let support = prepare(&format!(
        "{head} RETURN COUNT(DISTINCT c) AS n,MIN(c) AS lo,MAX(c) AS hi"
    ));
    ...
    let zero = prepare(&format!(
        "MATCH (a)-[:R]->(b){},(b)-[:T]->(missing),(b)-[:U]->(c) \
        RETURN COUNT(c) AS n",
        branches(61)
    ));
    assert_eq!(
        run(&zero, &edges).unwrap().value[0]
            .get(0)
            .unwrap()
            .as_count(),
        Some(0)
...
fn branch_partition_source_and_output_share_exact_limits_and_checkpoints() {
    ...
    let measured = query
        .execute_governed(
            5,
            [],
            edges.into_iter().inspect(|_| consumed += 1),
            |_, _| Ok::<_, ()>(true),
            |_, _| Ok(None),
            wide(),
            || {
...
fn observed_branches_keep_per_occurrence_property_errors_even_with_limit_zero() {
    ...
    let edges = [
        ...
        (VId(0), RelationId(1), VId(1)),
        (VId(1), RelationId(2), VId(2)),
        (VId(1), RelationId(3), VId(3)),
    ];
    let scalar = CanonicalScalar::Int(7);
    for tail in [
        " RETURN c,COUNT(h.n) AS n GROUP BY c LIMIT 0",
    ...
    let query = prepare(&format!(
        "{head} WHERE h.n=7 RETURN c,COUNT(*) AS n GROUP BY c"
    ));
    let result = query.execute_governed(
        4,
        [],
        edges,
        |_, _| Err::<bool, _>("predicate source"),
        |_, _| Ok(None),
        wide(),
        || Ok::<_, ()>(()),
    );
