...
use fgdb_gql::algebra::{
    GlaDirection, GraphColumn, GraphMatchClause, GraphOrderError, GraphPatternBuilder, GraphValue,
    GraphValueOrder, GraphValueRow, PreparedGraphPattern,
};
...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 1000, u64::MAX, u64::MAX)
}
    ...
    let p = b
        .prepare_values(
            &[
                ...
                GraphColumn::property("tie", "n", PropertyKeyId(2)),
            ],
            offset,
            count,
        )
...
fn compare(a: &[GraphValue], b: &[GraphValue], order: &[GraphValueOrder]) -> Ordering {
    for key in order {
        ...
        if left.is_null() != right.is_null() {
            return if left.is_null() == key.nulls_first {
                Ordering::Less
            } else {
                Ordering::Greater
            };
        }
        let cmp = if key.descending {
            right.cmp(left)
        } else {
            left.cmp(right)
        };
        ...
        for descending in [false, true] {
            for nulls_first in [false, true] {
                let order = [
                    ...
                    GraphValueOrder::descending(2),
                ];
                for distinct in [false, true] {
                    for offset in 0..=3 {
                        for count in [Some(0), Some(1), Some(3), None] {
                            let query = pattern(distinct, offset, count, &order);
                            let frozen = query.canonical_bytes();
                            let mut expected: Vec<Vec<GraphValue>> = input
                                .iter()
                                .map(|id| {
                                    let at =
                                    ...
                                    vec![
                                        ...
                                        GraphValue::Scalar((*scores[at]).clone()),
                                        GraphValue::Scalar(ties[at].clone()),
                                    ]
                                })
                                .collect();
...
fn ordered_definition_checks_columns_and_preserves_schema_and_multiplicity() {
    let b = builder();
    let base = b
        .prepare_values(&[GraphColumn::vertex("first", "n")], 1, Some(2))
        .unwrap();
    let original = base.canonical_bytes();
    assert_eq!(
        base.clone().with_order_by(&[]).unwrap_err(),
        GraphOrderError::EmptyOrder
    );
    assert_eq!(
        base.clone()
            .with_order_by(&[GraphValueOrder::ascending(1)])
            .unwrap_err(),
        GraphOrderError::UnknownColumn { column: 1 }
    );
    assert_eq!(
        base.clone()
            .with_order_by(&[
    ...
    assert_eq!(
        base.clone()
            ...
            .with_order_by(&order)
            .unwrap(),
        ranked.clone().with_duplicates()
    );
    let renamed = b
        .prepare_values(&[GraphColumn::vertex("second", "n")], 1, Some(2))
        .unwrap()
        .with_order_by(&order)
        .unwrap();
...
fn ordering_preserves_optional_null_bindings_and_parallel_occurrences() {
    ...
    inner.vertex("a").unwrap();
    inner.vertex("b").unwrap();
    inner
    ...
    let query = outer
        .prepare_values_with_clauses(
            ...
            &[
                ...
                GraphColumn::property("score", "b", PropertyKeyId(1)),
            ],
            0,
            None,
        )
    ...
    let result = query
        ...
        .execute_governed_with_properties(
            5,
            [VId(0), VId(1), VId(2)],
            [(VId(1), RelationId(1), VId(8)); 2],
            |_, _| Ok::<_, ()>(true),
            |id, _| {
                assert_eq!(
                    id,
                    VId(8),
                    "a null binding must never reach property storage"
                );
                Ok(Some(&score))
            },
            wide(),
...
fn ordered_reads_keep_source_failures_even_when_zero_or_full_pages_discard_rows() {
    ...
    for count in [Some(0), Some(1)] {
        ...
        let result = query.plan().execute_governed_with_properties(
            ...
            |id, key| {
                reads += 1;
                if id == VId(1) && key == PropertyKeyId(2) {
                    Err("late discarded field")
                } else {
                    Ok(Some(&scalar))
                }
            },
        ...
        assert!(matches!(
            result,
            Err(GqlQueryError::Source("late discarded field"))
        ));
        assert_eq!(reads, 4);
    }
}
