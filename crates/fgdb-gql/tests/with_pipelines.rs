...
use fgdb_gql::{GqlParameters, GqlQueryError, GqlQueryExecution, GqlQueryPolicy, GqlScalarParameter,
    ...
    GraphSetTextErrorKind, GraphSetValue, GraphSymbol, GraphSymbolKind,
...
fn run(query: &PreparedGraphSet, values: &[CanonicalScalar], policy: GqlQueryPolicy,
    ...
    query.execute_governed(policy, |pattern, remaining| {
        pattern.plan().execute_governed_with_properties(values.len() as u64,
            ...
            |vid, predicates| Ok::<_, &'static str>(predicates.iter().all(|test|
...
fn ints(rows: &[GraphValueRow], column: usize) -> Vec<Option<i64>> {
    rows.iter().map(|row| match row.get(column).unwrap().as_scalar() {
        Some(CanonicalScalar::Int(n)) => Some(*n), Some(CanonicalScalar::Null) => None,
...
fn integer(column: usize, n: i64, op: GraphIntegerBinary) -> GraphSetValue {
    GraphSetValue::Integer(GraphIntegerExpression::prepare(&[
        GraphIntegerOp::Column(column), GraphIntegerOp::Literal(Some(n)), GraphIntegerOp::Binary(op),
    ]).unwrap())
}
...
fn native_pipeline_matches_typed_projection_filter_and_page_composition() {
    ...
    let args = GqlParameters::new().with_uint64("top", 3).unwrap().with_int64("min", 2).unwrap();
    let actual = template.bind_parameters(&args).unwrap();
    let mut builder = GraphPatternBuilder::new();
    builder.vertex("n").unwrap();
    let input: PreparedGraphSet = builder.prepare_values(&[
        GraphColumn::vertex("n", "n"), GraphColumn::property("p", "n", P)], 0, None)
        .unwrap().with_duplicates().into();
    let expected = input.project(vec![GraphSetProjection::new("owner", GraphSetValue::Column(0)),
        GraphSetProjection::new("score", integer(1, 1, GraphIntegerBinary::Add))], GraphSetQuantifier::All).unwrap()
        .with_order_by(&[GraphValueOrder::descending(1)]).unwrap().with_page(0, Some(3))
        .filter(&[Op::Compare { left: Arg::Column(1), comparison: IntegerComparison::Greater,
            right: Arg::Literal(GqlScalarParameter::new(CanonicalScalar::Int(2)).unwrap()) }]).unwrap()
        .project(vec![GraphSetProjection::new("owner", GraphSetValue::Column(0)),
            GraphSetProjection::new("bucket", integer(1, 2, GraphIntegerBinary::Remainder))], GraphSetQuantifier::All).unwrap()
        .project(vec![GraphSetProjection::new("owner", GraphSetValue::Column(0)),
            GraphSetProjection::new("bucket", GraphSetValue::Column(1))], GraphSetQuantifier::All).unwrap()
    ...
    assert_eq!(rows.iter().map(|row| row.get(0).unwrap().as_vertex().unwrap()).collect::<Vec<_>>(),
...
fn each_stage_preserves_bags_and_its_page_before_the_next_filter() {
    ...
    for (text, expected) in [
        ...
        ("MATCH (n) WITH DISTINCT n.p AS p RETURN p", vec![1, 2, 3, 4]),
        ("MATCH (n) WITH n.p AS p ORDER BY p DESC LIMIT 2 WHERE p < 4 RETURN p", vec![3]),
        ("MATCH (n) WITH n.p AS p WHERE p < 4 WITH p ORDER BY p DESC LIMIT 2 RETURN p", vec![2, 3]),
        ("MATCH (n) WITH n.p AS p WITH 7 AS x RETURN x", vec![7, 7, 7, 7, 7]),
        ("MATCH (n) WITH n.p AS p WITH DISTINCT 7 AS x RETURN x", vec![7]),
    ] {
        ...
        assert_eq!(ints(&result.value, 0), expected.into_iter().map(Some).collect::<Vec<_>>(), "{text}");
    }
}

#[test]
fn alias_arithmetic_case_and_coalesce_reuse_checked_lazy_execution() {
    let values = [CanonicalScalar::Int(0), CanonicalScalar::Int(2), CanonicalScalar::Null];
    let query = prepare("MATCH (n) WITH n,n.p AS value WITH n,CASE WHEN value IS NULL THEN 7 WHEN value=0 THEN 99 ELSE 10/value END AS score RETURN n,COALESCE(score,1/0) AS result ORDER BY n");
    assert_eq!(ints(&run(&query, &values, wide(), &mut || Ok(())).unwrap().value, 1), vec![Some(99), Some(5), Some(7)]);
    let safe = prepare("MATCH (n) WITH n.p AS value WHERE value <> 0 RETURN 10/value AS result");
    assert_eq!(ints(&run(&safe, &values, wide(), &mut || Ok(())).unwrap().value, 0), vec![Some(5)]);
    let unsafe_page = prepare("MATCH (n) WITH n.p AS value RETURN 10/value AS result LIMIT 0");
    assert!(matches!(run(&unsafe_page, &values, wide(), &mut || Ok(())),
...
fn nullable_aliases_and_boolean_filters_do_not_turn_unknown_into_true() {
    let values = [CanonicalScalar::Null, CanonicalScalar::Int(1), CanonicalScalar::Int(2), CanonicalScalar::Bool(true)];
    let query = prepare("MATCH (n) WITH n,n.p AS value WHERE NOT (value=1 OR value IS NULL) RETURN n");
    let result = run(&query, &values, wide(), &mut || Ok(())).unwrap();
    assert_eq!(result.value.len(), 1);
    assert_eq!(result.value[0].get(0).unwrap().as_vertex(), Some(VId(2)));
    let absent = prepare("MATCH (n) OPTIONAL MATCH (n)-[:R]->(m) WITH n,m WHERE m IS NULL RETURN n,m");
...
fn discarded_names_sibling_aliases_and_unsupported_forms_refuse_before_catalog() {
    ...
    assert!(matches!(PreparedGraphSetText::prepare(&invalid, symbols).unwrap_err().kind, GraphSetTextErrorKind::SetBuild(_)));
}
...
fn parameter_contracts_and_original_utf8_offsets_span_all_stages() {
    ...
    let args = GqlParameters::new().with_int64("add", 1).unwrap().with_uint64("top", 5).unwrap()
        .with_int64("min", 0).unwrap();
    let missing = template.bind_parameters(&args).unwrap_err();
    assert_eq!(missing.offset, text.find("$scale").unwrap());
    let bound = template.bind_parameters(&args.clone().with_int64("scale", 2).unwrap()).unwrap();
    assert_ne!(bound.canonical_bytes(), template.bind_parameters(&args.with_int64("scale", 3).unwrap()).unwrap().canonical_bytes());
    let calls = Cell::new(0);
    let conflict = "MATCH (n) WITH n.p+$x AS score LIMIT $x RETURN score";
    assert!(PreparedGraphSetText::prepare(conflict, |kind, name| { calls.set(calls.get() + 1); symbols(kind, name) }).is_err());
...
fn pipeline_leaves_compose_with_union_and_preserve_literal_token_boundaries() {
    ...
    let rows = run(&prepare(text), &[1, 2, 3].map(CanonicalScalar::Int), wide(), &mut || Ok(())).unwrap();
    ...
    assert_eq!(run(&literal, &[CanonicalScalar::Null, CanonicalScalar::Null], wide(), &mut || Ok(())).unwrap().value.len(), 2);
}
...
fn full_pipeline_shares_exact_resource_limits_and_all_interruption_points() {
    ...
    for dimension in 0..4 {
        let mut limit = caps; limit[dimension] -= 1;
        assert!(run(&query, &values, GqlQueryPolicy::new(limit[0], limit[1], limit[2], limit[3]), &mut || Ok(())).is_err());
    }
    for stop in 1..=calls.get() {
        let mut at = 0;
        assert!(matches!(run(&query, &values, wide(), &mut || { at += 1; if at==stop {Err(stop)} else {Ok(())} }),
