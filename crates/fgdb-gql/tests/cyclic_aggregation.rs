...
fn cycles_parallel_constraints_self_loops_and_output_aliases_match_assignments() {
    ...
    let universe: Vec<_> = [RelationId(2), RelationId(3)]
        .into_iter()
        .flat_map(|r| {
            domain
                .into_iter()
                .flat_map(move |a| domain.into_iter().map(move |b| (a, r, b)))
        })
        .collect();
                ...
                for mask in 0..256 {
                    ...
                    for a in domain {
                        for b in domain {
                            for x in domain {
                                for y in domain {
                                    let mut n = count(a, 1, b, 0)
                                        * count(b, 2, x, direction)
                                        * count(x, 3, y, 0)
                                        * count(y, 2, b, direction);
                                    if repeats {
                                        n *= count(b, 2, x, 0);
                                    }
                                    if n > 0 {
                                        *expected
                                            .entry((a, if project_x { x } else { b }))
                                            .or_insert(0) += n;
                                    }
                                }
                            }
                    ...
                    assert_eq!(
                        actual, expected,
                        "mask={mask} direction={direction} repeats={repeats} projected={project_x}"
                    );
...
fn lobes(n: usize, output: &str) -> String {
    ...
    for at in 0..n {
        text.push_str(&format!(",(b)-[:S]->(x{at})-[:T]->(y{at})-[:U]->(b)"));
    }
...
fn complete_lobe() -> Vec<Edge> {
    ...
    for a in 2..10 {
        edges.push((VId(1), RelationId(2), VId(a)));
        for b in 10..18 {
            edges.push((VId(a), RelationId(3), VId(b)));
        }
    }
    for b in 10..18 {
        edges.push((VId(b), RelationId(4), VId(1)));
    }
    edges
}
...
fn eight_hidden_cycles_count_hundreds_of_trillions_without_enumerating_them() {
    let query = prepare(&lobes(
        8,
        " RETURN a,COUNT(*) AS n,COUNT(DISTINCT b) AS d GROUP BY a",
    ));
...
fn cyclic_overflow_is_not_zero_and_support_does_not_require_representable_counts() {
    ...
    for support in [false, true] {
        ...
        if support {
            assert_eq!(result.unwrap().value[0].get(0).unwrap().as_count(), Some(1));
        } else {
            assert!(matches!(
                result,
...
fn cyclic_compilation_tables_and_output_share_exact_limits_and_interruption() {
    let query = prepare(
        "MATCH (a)-[:R]->(b)-[:S]->(x)-[:T]->(y)-[:U]->(b) RETURN a,COUNT(*) AS n GROUP BY a",
    );
    let edges = [
        (VId(0), RelationId(1), VId(1)),
        (VId(1), RelationId(2), VId(2)),
        (VId(2), RelationId(3), VId(3)),
        (VId(3), RelationId(4), VId(1)),
    ];
    let mut total = 0;
    let measured = query
        .execute_governed(
