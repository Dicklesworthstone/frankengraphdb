...
use fgdb_gql::algebra::{GlaDirection, GraphColumn, GraphMatchClause, GraphPatternBuilder,
    GraphValueRow, IntegerComparison, PatternBuildError, VertexPredicate};
use fgdb_gql::{GraphPatternTextErrorKind, GraphSymbol, GraphSymbolKind, GqlParameterType,
    GqlParameters, GqlQueryError, GqlQueryPolicy, PreparedGraphAggregateText, PreparedGraphText};
...
fn ids(rows: &[GraphValueRow]) -> Vec<Vec<Option<VId>>> {
    rows.iter().map(|row| row.values().iter().map(|cell| {
        if cell.is_null() { None } else { Some(cell.as_vertex().expect("identity fixture")) }
    }).collect()).collect()
}
fn orient(edges: &[Edge], relation: RelationId, source: VId, direction: usize) -> Vec<VId> {
    ...
    for &(left, actual, right) in edges {
        ...
        if direction != 1 && left == source { result.push(right); }
        if direction != 0 && right == source && (direction == 1 || left != right) { result.push(left); }
    }
    result
}
...
fn scoped_lowering_matches_typed_compilation_and_resolves_names_once() {
    ...
    assert_eq!(prepared.parameter_schema()[0].parameter_type, GqlParameterType::Int64);
    let args = GqlParameters::new().with_int64("floor", -1).unwrap()
        .with_int64("ceiling", 9).unwrap().with_uint64("off", 1).unwrap()
        .with_uint64("take", 4).unwrap();
    ...
    root.filter("a", VertexPredicate::IntegerProperty { key: P, comparison: IntegerComparison::GreaterOrEqual, value: -1 }).unwrap();
    let mut anti = GraphPatternBuilder::new();
    for name in ["a", "hidden"] { anti.vertex(name).unwrap(); }
    anti.edge("a", S, GlaDirection::Forward, "hidden").unwrap();
    anti.filter("hidden", VertexPredicate::IntegerProperty { key: P, comparison: IntegerComparison::Less, value: -1 }).unwrap();
    let mut first = GraphPatternBuilder::new();
    for name in ["a", "b"] { first.vertex(name).unwrap(); }
    first.edge("a", R, GlaDirection::Forward, "b").unwrap();
    first.filter("b", VertexPredicate::IntegerProperty { key: P, comparison: IntegerComparison::GreaterOrEqual, value: -1 }).unwrap();
    let mut second = GraphPatternBuilder::new();
    for name in ["b", "c"] { second.vertex(name).unwrap(); }
    second.edge("b", S, GlaDirection::Forward, "c").unwrap();
    second.filter("c", VertexPredicate::IntegerProperty { key: P, comparison: IntegerComparison::LessOrEqual, value: 9 }).unwrap();
    let expected = root.prepare_values_with_clauses(&[
        GraphMatchClause::not_exists(&anti), GraphMatchClause::optional(&first), GraphMatchClause::optional(&second),
    ], &[GraphColumn::vertex("a", "a"), GraphColumn::vertex("b", "b"),
        GraphColumn::vertex("c", "c"), GraphColumn::property("value", "c", P)], 1, Some(4)).unwrap().with_duplicates();
    assert_eq!(actual, expected);
    let frozen = actual.canonical_bytes();
    let changed = GqlParameters::new().with_int64("floor", 0).unwrap()
        .with_int64("ceiling", 8).unwrap().with_uint64("off", 0).unwrap().with_uint64("take", 2).unwrap();
    assert_ne!(prepared.bind_parameters(&changed).unwrap().canonical_bytes(), frozen);
...
fn whole_and_chained_optional_paths_match_independent_oriented_bags() {
    ...
    let universe = [(vertices[0], R, vertices[1]), (vertices[1], R, vertices[0]),
        (vertices[1], R, vertices[1]), (vertices[1], S, vertices[2]),
        (vertices[2], S, vertices[1]), (vertices[0], S, vertices[0])];
    for direction in 0..3 {
        let first = path("a", "R", "b", direction);
        let second = path("b", "S", "c", direction);
        for chained in [false, true] {
            let head = if chained { format!("MATCH (a) OPTIONAL MATCH {first} OPTIONAL MATCH {second}") }
                else { format!("MATCH (a) OPTIONAL MATCH {first},{second}") };
            ...
            for mut encoded in 0..3_usize.pow(universe.len() as u32) {
                ...
                for a in vertices {
                    ...
                    for b in orient(&edges, R, a, direction) {
                        ...
                        if chained && targets.is_empty() { complete.push(vec![Some(a), Some(b), None]); }
                        for c in targets { complete.push(vec![Some(a), Some(b), Some(c)]); }
                    }
                    if complete.is_empty() { complete.push(vec![Some(a), None, None]); }
                ...
                let run = |pattern: &fgdb_gql::algebra::PreparedGraphPattern<GraphValueRow>| {
                    pattern.plan().execute_governed_with_properties((vertices.len() + edges.len()) as u64,
                        vertices, edges.iter().copied(), |_, _| Ok::<_, ()>(true), |_, _| Ok(None), wide(), || Ok::<_, ()>(())).unwrap()
                };
...
fn optional_predicates_are_inside_the_match_and_missing_bindings_never_read_vertex_zero() {
    let q = query("MATCH (a:L) OPTIONAL MATCH (a)-[:R]->(b) WHERE b.n >= 7 RETURN a,b,b.n AS value");
    let value = CanonicalScalar::Int(7);
    let edges = [(VId(0), R, VId(1)), (VId(0), R, VId(2))];
    let result = q.plan().execute_governed_with_properties(4, [VId(0), VId(9)], edges,
        |vid, predicates| Ok::<_, &str>(predicates.iter().all(|predicate| {
            predicate.matches(&[L], &[(P, CanonicalScalar::Int(if vid == VId(2) { 7 } else { 1 }))])
        })),
        |vid, _| if vid == VId(2) { Ok(Some(&value)) } else { Err("null binding called the property resolver") },
        wide(), || Ok::<_, ()>(())).unwrap();
...
fn existential_scopes_are_local_and_do_not_multiply_outer_occurrences() {
    ...
    let edges = [(VId(0), R, VId(1)), (VId(0), R, VId(1)), (VId(1), S, VId(0))];
    let rows = q.plan().execute_governed_with_properties(5, [VId(0), VId(1)], edges,
    ...
    assert_eq!(query("MATCH (exists)-[:R]->(not) WHERE exists <> not RETURN exists,not").columns(), &["exists", "not"]);
}
...
fn malformed_or_out_of_scope_definitions_refuse_before_catalog_access() {
    for text in [
        ...
        let mut calls = 0;
        let result = PreparedGraphText::prepare(text, |kind, name| { calls += 1; symbols(kind, name) });
...
fn definition_limits_do_not_reset_at_optional_or_existential_boundaries() {
    ...
    for at in 0..64 { text.push_str(&format!(" OPTIONAL MATCH (n{at})-[:R]->(n{})", at + 1)); }
    let max = query(&format!("{text} RETURN * LIMIT 0"));
    assert_eq!(max.columns().len(), 65);
    let error = PreparedGraphText::prepare(&format!("{text} OPTIONAL MATCH (n64)-[:R]->(overflow) RETURN *"), symbols).unwrap_err();
    assert!(matches!(error.kind, GraphPatternTextErrorKind::Build(PatternBuildError::LimitExceeded { .. })));
    let clauses = " AND EXISTS { MATCH (a) }".repeat(63);
    assert!(PreparedGraphText::prepare(&format!("MATCH (a) WHERE EXISTS {{ MATCH (a) }}{clauses} RETURN a"), symbols).is_ok());
    ...
    assert!(PreparedGraphText::prepare(&format!("MATCH (a){predicates} RETURN a"), symbols).is_ok());
    assert!(PreparedGraphText::prepare(&format!("MATCH (a:L){predicates} RETURN a"), symbols).is_err());
}
...
fn optional_text_and_aggregate_text_share_zero_preserving_semantics() {
    ...
    let args = GqlParameters::new().with_int64("minimum", 1).unwrap().with_uint64("take", 10).unwrap();
    let aggregate = template.bind_parameters(&args).unwrap();
    let scalar = CanonicalScalar::Int(7);
    let rows = aggregate.execute_governed(7, [VId(0), VId(1), VId(9)],
        [(VId(0), R, VId(2)), (VId(0), R, VId(2)), (VId(2), S, VId(3)), (VId(2), S, VId(3))],
        |_, _| Ok::<_, ()>(true), |_, _| Ok(Some(&scalar)), wide(), || Ok::<_, ()>(())).unwrap();
...
fn scoped_text_retains_all_limits_and_errors_never_become_null_extension() {
    ...
    let edges = [(VId(0), R, VId(1)), (VId(0), R, VId(2))];
    let run = |policy| q.plan().execute_governed_with_properties(4, vertices, edges,
        |_, _| Ok::<_, &str>(true), |_, _| Ok(Some(&scalar)), policy, || Ok::<_, usize>(()));
    let full = run(wide()).unwrap();
    assert_eq!(full.value.len(), 3);
    let exact = GqlQueryPolicy::new(4, 3, full.evaluator.work_units, full.evaluator.scratch_entries);
    assert_eq!(run(exact).unwrap(), full);
    for policy in [GqlQueryPolicy::new(3, 3, u64::MAX, u64::MAX), GqlQueryPolicy::new(4, 2, u64::MAX, u64::MAX),
        GqlQueryPolicy::new(4, 3, full.evaluator.work_units - 1, u64::MAX),
        GqlQueryPolicy::new(4, 3, u64::MAX, full.evaluator.scratch_entries - 1)] { assert!(run(policy).is_err()); }
    let mut total = 0;
    q.plan().execute_governed_with_properties(4, vertices, edges, |_, _| Ok::<_, &str>(true),
        |_, _| Ok(Some(&scalar)), wide(), || { total += 1; Ok::<_, usize>(()) }).unwrap();
    for stop in 1..=total {
        let mut seen = 0;
        let result = q.plan().execute_governed_with_properties(4, vertices, edges,
            |_, _| Ok::<_, &str>(true), |_, _| Ok(Some(&scalar)), wide(), || {
    ...
    let result = q.plan().execute_governed_with_properties(4, vertices, edges,
        |vid, _| if vid == VId(2) { Err("late optional predicate") } else { Ok(true) },
        |_, _| Ok(Some(&scalar)), wide(), || Ok::<_, ()>(()));
    assert!(matches!(result, Err(GqlQueryError::Source("late optional predicate"))));
}
