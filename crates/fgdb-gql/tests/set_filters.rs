...
use fgdb_gql::{
    GqlQueryError, GqlQueryExecution, GqlQueryPolicy, GqlScalarParameter, GraphSetExecutionError,
    GraphSetFilterError, GraphSetOperand as Arg, GraphSetPredicateOp as Op, GraphSetProjection,
    GraphSetQuantifier, GraphSetValue, MAX_GRAPH_SET_DEPTH, PreparedGraphSet,
};
...
fn literal(value: CanonicalScalar) -> Arg {
    Arg::Literal(GqlScalarParameter::new(value).unwrap())
}
fn test(comparison: IntegerComparison, value: i64) -> Op {
    Op::Compare {
        left: Arg::Column(1),
        comparison,
        right: literal(CanonicalScalar::Int(value)),
    }
}
...
fn scalar_filters_use_three_valued_comparisons_not_set_total_order() {
    ...
    for comparison in [
        ...
        for negate in [false, true] {
            ...
            let expected = [1_i64, 2]
                .into_iter()
                .enumerate()
                .filter(|(_, n)| comparison.accepts(*n, 1) != negate)
                .map(|(at, _)| VId(at as u128 + 1))
                .collect::<Vec<_>>();
            assert_eq!(
                ids(
                    &run(&leaf().filter(&ops).unwrap(), &values, wide(), &mut || Ok(
                        ()
                    ))
                    .unwrap()
                    .value
    ...
    for (is_null, expected) in [(true, vec![VId(0)]), (false, vec![VId(1), VId(2), VId(3)])] {
        let query = leaf()
            .filter(&[Op::IsNull {
                operand: Arg::Column(1),
                is_null,
            }])
            .unwrap();
        assert_eq!(
            ids(&run(&query, &values, wide(), &mut || Ok(())).unwrap().value),
            expected
        );
    }
}
...
fn filtering_preserves_projected_bags_and_input_pages() {
    ...
    assert_eq!(
        ids(&run(&after_page, &values, wide(), &mut || Ok(()))
            .unwrap()
            .value),
        vec![VId(3)]
    );
    let before_page = leaf()
        .filter(&filter)
        .unwrap()
        .with_order_by(&[GraphValueOrder::descending(1)])
        .unwrap()
        .with_page(0, Some(2));
    assert_eq!(
        ids(&run(&before_page, &values, wide(), &mut || Ok(()))
            .unwrap()
            .value),
        vec![VId(3), VId(1)]
    );
    let score = leaf()
        .project(
            vec![GraphSetProjection::new("score", GraphSetValue::Column(1))],
            GraphSetQuantifier::All,
        )
        .unwrap();
    let equal = [Op::Compare {
        left: Arg::Column(0),
    ...
    let distinct = bag
        .project(
            vec![GraphSetProjection::new("score", GraphSetValue::Column(0))],
            GraphSetQuantifier::Distinct,
        )
        .unwrap();
    assert_eq!(
        run(&distinct, &values, wide(), &mut || Ok(()))
            .unwrap()
            .value
            .len(),
        1
...
fn complete_schema_stack_and_depth_validation_precedes_execution() {
    ...
    for code in [
        ...
        assert!(matches!(
            leaf().filter(&code),
            Err(GraphSetFilterError::InvalidStack { .. })
        ));
    }
    assert!(matches!(
        leaf().filter(&[Op::IsNull {
            operand: Arg::Column(2),
            is_null: true
        }]),
        Err(GraphSetFilterError::UnknownInput { column: 2, .. })
    ));
    for (right, comparison) in [
        (Arg::Column(1), IntegerComparison::Equal),
        (Arg::Column(0), IntegerComparison::Less),
    ] {
        assert!(matches!(
            leaf().filter(&[Op::Compare {
                left: Arg::Column(0),
                comparison,
                right
            }]),
    ...
    for _ in 1..MAX_GRAPH_SET_DEPTH {
        deepest = deepest.filter(&[Op::Truth(Some(true))]).unwrap();
    }
    assert!(matches!(
        deepest.filter(&[Op::Truth(Some(true))]),
        Err(GraphSetFilterError::SetBuild(_))
    ));
    let mut too_many = vec![Op::Truth(Some(true))];
    for _ in 0..256 {
        too_many.extend([Op::Truth(Some(true)), Op::And]);
    }
    assert!(matches!(
        leaf().filter(&too_many),
        Err(GraphSetFilterError::TooManyPredicates { .. })
    ));
    assert!(matches!(
        leaf().filter(&vec![Op::Not; 1025]),
        Err(GraphSetFilterError::TooManyInstructions { .. })
    ));
}
...
fn each_resource_dimension_and_every_interruption_boundary_is_shared() {
    let query = leaf()
        .filter(&[test(IntegerComparison::Greater, 1)])
        .unwrap();
    ...
    for stop in 1..=calls.get() {
        let mut at = 0;
        let result = run(&query, &values, wide(), &mut || {
            at += 1;
...
fn canonical_identity_binds_predicates_and_placement_without_exposing_payloads() {
    ...
    let a = original
        .clone()
        .filter(&[test(IntegerComparison::Equal, 712_345)])
        .unwrap();
    let b = original
        .clone()
        .filter(&[test(IntegerComparison::NotEqual, 712_345)])
        .unwrap();
    let c = original
        .clone()
        .filter(&[test(IntegerComparison::Equal, 712_346)])
        .unwrap();
