...
use fgdb_gql::{
    ...
    GraphIntegerUnary, GraphSetProjection, GraphSetProjectionError, GraphSetValue,
    GraphSymbol, GraphSymbolKind, PreparedGraphAggregate, PreparedGraphText,
};
...
type ResultOf<C = ()> = Result<GqlQueryExecution<GraphAggregateRow>, GqlQueryError<GraphAggregateError<&'static str>, C>>;
...
fn policy() -> GqlQueryPolicy { GqlQueryPolicy::new(10_000, 10_000, 5_000_000, 5_000_000) }
...
fn product_projection() -> Vec<GraphSetProjection> {
    vec![
        GraphSetProjection::new("bucket", expression(&[
            GraphIntegerOp::Column(0), GraphIntegerOp::Unary(GraphIntegerUnary::Abs),
        ])),
        GraphSetProjection::new("product", expression(&[
            GraphIntegerOp::Column(0), GraphIntegerOp::Column(1),
...
fn summaries() -> [GraphAggregate<'static>; 9] {
    [GraphAggregate::count_rows("rows"), GraphAggregate::count("present", 1),
        GraphAggregate::count_distinct("unique", 1), GraphAggregate::sum_int("total", 1),
        GraphAggregate::sum_int_distinct("distinct_total", 1), GraphAggregate::average_int("mean", 1),
        GraphAggregate::average_int_distinct("distinct_mean", 1),
        GraphAggregate::min("minimum", 1), GraphAggregate::max("maximum", 1)]
}
fn run(plan: &PreparedGraphAggregate, vertices: &[VId], edges: &[Edge], props: &Props,
    budget: GqlQueryPolicy) -> ResultOf {
    plan.execute_governed((vertices.len() + edges.len()) as u64,
        vertices.iter().copied(), edges.iter().copied(), |_, _| Ok::<_, &'static str>(true),
...
fn nullable_products_and_computed_groups_match_the_independent_stored_column_oracle() {
    let computed = PreparedGraphAggregate::prepare_projected(input(), product_projection(), &[0],
        &summaries(), 0, None).unwrap();
    let plain = PreparedGraphAggregate::prepare(
        pattern("MATCH (n) RETURN n.bucket AS bucket,n.product AS product"), &[0], &summaries(), 0, None,
    ).unwrap();
    ...
    for code in 0..625_usize {
        ...
        for vid in [VId(1), VId(2)] {
            let p = domain[digits % domain.len()]; digits /= domain.len();
            let q = domain[digits % domain.len()]; digits /= domain.len();
            properties.insert((vid, P), p.map_or(CanonicalScalar::Null, CanonicalScalar::Int));
            properties.insert((vid, Q), q.map_or(CanonicalScalar::Null, CanonicalScalar::Int));
            // The oracle is ordinary wide arithmetic, not the expression VM.
            let product = p.zip(q).map(|(p, q)| i64::try_from(i128::from(p) * i128::from(q)).unwrap());
            let bucket = p.map(i64::abs);
            properties.insert((vid, PRODUCT), product.map_or(CanonicalScalar::Null, CanonicalScalar::Int));
            properties.insert((vid, BUCKET), bucket.map_or(CanonicalScalar::Null, CanonicalScalar::Int));
        }
        ...
        assert_eq!(actual.rows, expected.rows, "admitted source and final rows, case {code}");
    }
}

#[test]
fn argument_distinct_operates_on_computed_values_and_averages_remain_exact() {
    let plan = PreparedGraphAggregate::prepare_projected(input(), product_projection(), &[],
        &summaries(), 0, None).unwrap();
    let mut properties = Props::new();
    ...
    let result = run(&plan, &[VId(1), VId(2), VId(3), VId(4)], &[], &properties,
        GqlQueryPolicy::new(4, 1, 1_000_000, 1_000_000)).unwrap();
    ...
    assert_eq!(values[7].as_value().unwrap().as_scalar(), Some(&CanonicalScalar::Int(4)));
    assert_eq!(values[8].as_value().unwrap().as_scalar(), Some(&CanonicalScalar::Int(5)));
}
...
fn constant_inputs_preserve_walk_occurrences_isolates_and_empty_group_laws() {
    ...
    let projection = vec![GraphSetProjection::new("value",
        GraphSetValue::Literal(GqlScalarParameter::new(CanonicalScalar::Int(7)).unwrap()))];
    let functions = [GraphAggregate::count_rows("rows"), GraphAggregate::sum_int("total", 0),
        GraphAggregate::count_distinct("unique", 0)];
    let plan = PreparedGraphAggregate::prepare_projected(source.clone(), projection.clone(), &[],
        &functions, 0, None).unwrap();
    ...
    let keyed = PreparedGraphAggregate::prepare_projected(source, projection, &[0], &functions, 0, None).unwrap();
    assert!(run(&keyed, &[], &[], &Props::new(), policy()).unwrap().value.is_empty());
}

#[test]
fn projected_groups_reuse_having_hidden_keys_distinct_ordering_and_pagination() {
    let projection = vec![GraphSetProjection::new("group", GraphSetValue::Column(0)),
        GraphSetProjection::new("value", expression(&[GraphIntegerOp::Column(1),
            GraphIntegerOp::Literal(Some(0)), GraphIntegerOp::Coalesce]))];
    let functions = [GraphAggregate::count_rows("rows"), GraphAggregate::sum_int("total", 1)];
    let base = PreparedGraphAggregate::prepare_projected(input(), projection, &[0], &functions, 0, None).unwrap();
    let properties = BTreeMap::from([
        ((VId(1), P), CanonicalScalar::Int(1)), ((VId(1), Q), CanonicalScalar::Int(5)),
        ((VId(2), P), CanonicalScalar::Int(2)), ((VId(2), Q), CanonicalScalar::Int(5)),
    ]);
    let plan = base.with_result_clauses(&[
    ...
    assert!(result.value.iter().all(|row| row.keys().is_empty() && row.values().len() == 1));
    let distinct = plan.with_distinct_output(true);
    assert_eq!(run(&distinct, &[VId(1), VId(2)], &[], &properties, policy()).unwrap().value.len(), 1);
}
...
fn input_schema_refusals_precede_execution_and_transcripts_bind_computation() {
    ...
    for projection in [
        vec![],
        vec![GraphSetProjection::new("bad name", GraphSetValue::Column(0))],
        vec![GraphSetProjection::new("v", GraphSetValue::Column(usize::MAX))],
        vec![GraphSetProjection::new("v", expression(&[GraphIntegerOp::Column(2)]))],
        vec![GraphSetProjection::new("v", GraphSetValue::Column(0)), GraphSetProjection::new("v", GraphSetValue::Column(1))],
    ] {
        assert!(matches!(PreparedGraphAggregate::prepare_projected(input(), projection, &[], &functions, 0, None),
            Err(GraphAggregateBuildError::InputProjection(_))));
    }
    let projected = vec![GraphSetProjection::new("v", GraphSetValue::Column(0))];
    assert!(matches!(PreparedGraphAggregate::prepare_projected(input(), projected.clone(), &[1], &functions, 0, None),
        Err(GraphAggregateBuildError::UnknownColumn { column: 1 })));
    assert!(matches!(PreparedGraphAggregate::prepare_projected(
        pattern("MATCH (n) RETURN n LIMIT 1"), projected.clone(), &[], &functions, 0, None),
    ...
    let computed = PreparedGraphAggregate::prepare_projected(input(), projected, &[], &functions, 0, None).unwrap();
    ...
    let renamed = PreparedGraphAggregate::prepare_projected(input(),
        vec![GraphSetProjection::new("other_alias", GraphSetValue::Column(0))], &[],
        &[GraphAggregate::count_rows("renamed")], 0, None).unwrap();
    assert_eq!(computed.canonical_bytes(), renamed.canonical_bytes());
    let changed = PreparedGraphAggregate::prepare_projected(input(),
        vec![GraphSetProjection::new("v", expression(&[GraphIntegerOp::Column(0),
            GraphIntegerOp::Literal(Some(1)), GraphIntegerOp::Binary(GraphIntegerBinary::Add)]))],
        &[], &functions, 0, None).unwrap();
    assert_ne!(computed.canonical_bytes(), changed.canonical_bytes());
    assert!(matches!(PreparedGraphAggregate::prepare_projected(input(),
        vec![GraphSetProjection::new("v", expression(&[GraphIntegerOp::Column(2)]))], &[], &functions, 0, None),
        Err(GraphAggregateBuildError::InputProjection(GraphSetProjectionError::IntegerInput { .. }))));
}

#[test]
fn hidden_or_zero_limit_arithmetic_and_late_source_errors_cannot_be_discarded() {
    for limit in [None, Some(0)] {
        let plan = PreparedGraphAggregate::prepare_projected(input(), vec![GraphSetProjection::new("value",
            expression(&[GraphIntegerOp::Literal(Some(10)), GraphIntegerOp::Column(0),
                GraphIntegerOp::Binary(GraphIntegerBinary::Divide)]))], &[],
            &[GraphAggregate::count_rows("rows")], 0, limit).unwrap();
        let properties = BTreeMap::from([
            ((VId(1), P), CanonicalScalar::Int(1)), ((VId(2), P), CanonicalScalar::Int(0)),
        ]);
        assert!(matches!(run(&plan, &[VId(1), VId(2)], &[], &properties, policy()),
            Err(GqlQueryError::Source(GraphAggregateError::InputExpression { error, .. }))
                if error.kind == GraphIntegerErrorKind::DivisionByZero));
        let result: ResultOf = plan.execute_governed(2, [VId(1), VId(2)], [],
            |_, _| Ok(true), |vid, key| if vid == VId(2) { Err("late property failure") }
                else { Ok(properties.get(&(vid, key))) }, policy(), || Ok(()));
        assert!(matches!(result, Err(GqlQueryError::Source(GraphAggregateError::Source("late property failure")))));
    }
    let wrong = BTreeMap::from([((VId(1), P), CanonicalScalar::Bool(true))]);
    let plan = PreparedGraphAggregate::prepare_projected(input(), product_projection(), &[], &summaries(), 0, None).unwrap();
...
fn every_projected_aggregate_checkpoint_and_exact_budget_boundary_is_enforced() {
    let plan = PreparedGraphAggregate::prepare_projected(input(), product_projection(), &[], &summaries(), 0, None).unwrap();
    let properties = BTreeMap::from([
        ((VId(1), P), CanonicalScalar::Int(2)), ((VId(1), Q), CanonicalScalar::Int(3)),
        ((VId(2), P), CanonicalScalar::Int(4)), ((VId(2), Q), CanonicalScalar::Int(5)),
    ]);
    ...
    assert_eq!(run(&plan, &vertices, &[], &properties, exact).unwrap(), measured);
    assert_eq!(measured.rows.result_rows, 1, "private MATCH rows do not consume public output allowance");
    for budget in [
    ...
    let execute = |stop| {
        ...
        let result: ResultOf<usize> = plan.execute_governed(2, vertices, [], |_, _| Ok(true),
            |vid, key| Ok(properties.get(&(vid, key))), policy(), || {
                let next = calls.get() + 1; calls.set(next);
...
fn projected_literals_retain_canonical_kinds_and_do_not_leak_through_debug() {
    ...
    let projection = vec![GraphSetProjection::new("literal", GraphSetValue::Literal(
        GqlScalarParameter::new(secret.clone()).unwrap()))];
    let plan = PreparedGraphAggregate::prepare_projected(input(), projection, &[],
        &[GraphAggregate::min("minimum", 0), GraphAggregate::max("maximum", 0)], 0, None).unwrap();
    let result = run(&plan, &[VId(1)], &[], &Props::new(), policy()).unwrap();
    assert_eq!(result.value[0].get(0).unwrap().as_value(), Some(&GraphValue::Scalar(secret)));
