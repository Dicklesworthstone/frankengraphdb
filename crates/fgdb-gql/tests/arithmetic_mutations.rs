...
use fgdb_gql::{
    GqlParameterType, GqlParameters, GqlQueryError, GqlQueryPolicy, GraphIntegerBuildError,
    GraphIntegerErrorKind, GraphMutationBatch, GraphMutationError, GraphMutationIntent,
    GraphMutationPolicy, GraphMutationTextErrorKind, GraphPatternTextErrorKind, GraphSymbol,
    GraphSymbolKind, PreparedGraphMutation, PreparedGraphMutationText,
};
...
fn reciprocal_arithmetic_and_duplicate_counters_use_the_frozen_pre_statement_values() {
    ...
    assert_eq!(
        result.intents(),
        &[property(1, P, Some(21)), property(2, P, Some(11))]
    );
    assert_eq!(
        (result.stats().selection.result_rows, result.stats().effects),
        (3, 2)
    );
    ...
    assert_eq!(
        result.intents(),
        &[property(1, P, Some(11)), property(2, P, Some(21))]
    );
    let changed = Props::from([
        ((VId(1), P), CanonicalScalar::Int(10)),
        ((VId(2), P), CanonicalScalar::Int(20)),
...
fn arithmetic_errors_are_typed_and_no_successful_prefix_is_returned() {
    for (expression, first, second, kind) in [
        ...
        assert!(
            matches!(failed, GqlQueryError::Source(GraphMutationError::Arithmetic {
    ...
    for expression in [
        ...
        assert!(
            matches!(run(&prepare(&format!("MATCH (n) SET n.q={expression}")),
...
fn nullable_targets_and_lazy_arithmetic_do_not_hide_property_source_failures() {
    ...
    assert!(
        run(&absent, &[VId(1)], &[], &Props::new(), policy())
            .unwrap()
            .intents()
            .is_empty()
    );
    ...
    assert_eq!(
        run(&fallback, &[VId(1)], &[], &Props::new(), policy())
            .unwrap()
            .intents(),
        &[property(1, Q, Some(1))]
    );
        ...
        |selection, budget| {
            selection.plan().execute_governed_with_properties(
                ...
                |_, _| {
                    ...
                    Err::<Option<&CanonicalScalar>, _>("property unavailable")
                },
                budget,
                || Ok::<_, ()>(()),
            )
...
fn computed_and_literal_assignments_share_identity_but_null_is_not_removal() {
    ...
    for text in [
        "MATCH (n) SET n.q=NULL+1 REMOVE n.q",
        "MATCH (n) SET n.q=1+2,n.q=4",
        "MATCH (n) SET n.q=1+0,n.q=TRUE",
    ] {
        assert!(matches!(
            run(&prepare(text), &[VId(1)], &[], &Props::new(), policy()),
            Err(GqlQueryError::Source(
                GraphMutationError::ConflictingAssignment { .. }
            ))
        ));
...
fn typed_rhs_parameters_reuse_the_statement_schema_catalog_and_original_offsets() {
    ...
    let template = PreparedGraphMutationText::prepare_with_parameter_types(
        text,
        R,
        &[("base", GqlParameterType::Scalar(integer))],
        |kind, name| {
    ...
    assert!(
        template
            .bind_parameters(&args.with_int64("unused", 1).unwrap())
            .is_err()
    );
    let textual = CanonicalScalarKind::of(&CanonicalScalar::ucs_basic_text("x").unwrap());
    let calls = Cell::new(0);
    let failed = PreparedGraphMutationText::prepare_with_parameter_types(
        text,
        R,
        &[("base", GqlParameterType::Scalar(textual))],
        |kind, name| {
            calls.set(calls.get() + 1);
            symbols(kind, name)
        },
...
fn malformed_and_excessive_expression_definitions_fail_before_catalog_access() {
    for expression in [
        ...
        assert!(
            PreparedGraphMutationText::prepare(
                &format!("MATCH (n) SET n.q={expression}"),
                R,
                |kind, name| {
                    calls.set(calls.get() + 1);
                    symbols(kind, name)
    ...
    assert_eq!(
        run(&prepare(&valid), &[VId(1)], &[], &Props::new(), policy())
            .unwrap()
            .intents(),
        &[property(1, Q, Some(512))]
    );
    let calls = Cell::new(0);
    let failed = PreparedGraphMutationText::prepare(
        &format!("MATCH (n) SET n.q={}", vec!["1"; 513].join("+")),
        R,
        |kind, name| {
            calls.set(calls.get() + 1);
            symbols(kind, name)
    ...
    prepare(&nested(64));
    assert!(matches!(
        PreparedGraphMutationText::prepare(&nested(65), R, symbols)
            .unwrap_err()
            .kind,
...
fn exact_limits_and_every_interruption_cover_selection_bytecode_and_proposals() {
    let plan =
        prepare("MATCH (a)-[:R]->(b) SET a.q=COALESCE(a.p,0)+ABS(-2)*3,b.q=COALESCE(b.p,0)+6");
    ...
    let measured = run(&plan, &[VId(1), VId(2)], &edges, &props, policy())
        .unwrap()
        .stats();
    ...
    assert_eq!(
        run(&plan, &[VId(1), VId(2)], &edges, &props, exact)
            .unwrap()
            .stats(),
        measured
    );
    ...
    assert_eq!(
        complete.unwrap().intents(),
        &[property(1, Q, Some(6)), property(2, Q, Some(26))]
    );
    for stop in 1..=total {
