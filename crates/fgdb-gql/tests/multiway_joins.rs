...
use fgdb_gql::algebra::{
    GlaDirection, GraphColumn, GraphMatchClause, GraphPatternBuilder, MAX_PATTERN_EDGES,
    VertexPredicate,
};
...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(100_000, 100_000, u64::MAX, u64::MAX)
}
...
fn multiple_closing_constraints_match_independent_complete_assignment_bags() {
    ...
    let universe = [
        ...
        (ids[1], RelationId(2), ids[1]),
        (ids[0], RelationId(3), ids[0]),
        (ids[0], RelationId(3), ids[1]),
        (ids[0], RelationId(4), ids[0]),
        (ids[1], RelationId(4), ids[0]),
        (ids[1], RelationId(5), ids[0]),
        (ids[0], RelationId(5), ids[1]),
    ];
    ...
    for mask in 0..1024_usize {
        ...
        for (directions, shapes) in [
            ...
            for &(source, relation, direction, target) in &atoms {
                b.edge(names[source], relation, direction, names[target])
                    .unwrap();
            }
...
fn late_absence_is_pruned_before_parallel_closing_combinations_are_expanded() {
    ...
    for (source, relation, destination) in [
        ("a", 1, "b"),
        ("b", 2, "c"),
        ("a", 3, "c"),
        ("a", 3, "c"),
    ...
    for i in 0..n {
        ...
        for _ in 0..16 {
            edges.push((VId(0), RelationId(3), candidate));
        }
    ...
    let result = query
        ...
        .execute_with_limits(
            ...
            edges,
            |_, _| Ok::<_, ()>(true),
            GlaExecutionLimits::new(1024 * n, u64::MAX),
        )
        .unwrap();
...
fn maximum_definition_chain_keeps_one_real_witness_and_one_output_shape() {
    ...
    let edges = [
        (VId(0), RelationId(1), VId(u128::MAX)),
        (VId(u128::MAX), RelationId(2), VId(5)),
        (VId(0), RelationId(3), VId(5)),
    ];
    let query = b
        .prepare_bindings(&["c", "a", "b"], 0, None)
        .unwrap()
        .with_duplicates();
    let result = query
        .plan()
...
fn multiway_scopes_preserve_optional_bags_and_existential_multiplicity() {
    ...
    for (s, r, d) in [("a", 2, "c"), ("a", 3, "c"), ("c", 4, "a")] {
        child
            .edge(s, RelationId(r), GlaDirection::Forward, d)
            .unwrap();
    }
    ...
    edges.extend([
        (VId(1), RelationId(2), VId(9)),
        (VId(1), RelationId(3), VId(9)),
    ]);
...
fn multiple_closings_share_exact_resource_limits_and_every_interruption_checkpoint() {
    ...
    let edges = [
        ...
        (VId(1), RelationId(2), VId(2)),
        (VId(0), RelationId(3), VId(2)),
        (VId(0), RelationId(3), VId(2)),
        (VId(2), RelationId(4), VId(0)),
