...
fn oracle(edges: &[Edge], atoms: &[Atom], variables: usize) -> Vec<Summary> {
    ...
    for bits in 0..(1 << variables) {
        let assignment: Vec<_> = (0..variables)
            .map(|at| {
                if bits & (1 << at) == 0 {
                    VId(0)
                } else {
        ...
        let count = atoms
            ...
            .product::<usize>();
        for _ in 0..count {
            occurrences.push((assignment[0], assignment[1]));
        }
    }
    let mut groups: BTreeMap<VId, Vec<VId>> = BTreeMap::new();
    for (a, b) in occurrences {
        groups.entry(a).or_default().push(b);
    }
    groups
        .into_iter()
        .map(|(a, values)| {
...
fn weighted_counts_and_support_match_independent_multigraph_assignments() {
    ...
    let cases: [Case<'_>; 5] = [
        ...
        (
            ...
            &[(0, 1, 0, 1), (1, 1, 1, 2), (2, 2, 0, 0)],
            3,
        ),
        (
            "MATCH (a)-[:R]-(b)-[:S]-(c)-[:R]-(a)",
            &[(0, 1, 2, 1), (1, 2, 2, 2), (2, 1, 2, 0)],
            3,
        ),
    ...
    let universe: Vec<_> = [RelationId(1), RelationId(2)]
        ...
        .flat_map(|r| {
            [VId(0), VId(u128::MAX)]
                .into_iter()
                .flat_map(move |a| [VId(0), VId(u128::MAX)].into_iter().map(move |b| (a, r, b)))
        })
        .collect();
...
fn a_billion_occurrences_are_counted_without_a_billion_binding_visits() {
    let query = prepare(&repeated(
        5,
        " RETURN COUNT(*) AS n,COUNT(a) AS m,COUNT(DISTINCT a) AS d",
    ));
    let edges = vec![(VId(7), RelationId(1), VId(7)); 64];
    let frozen = query.canonical_bytes();
    let measured = run(&query, &edges).unwrap();
    assert_eq!(
        measured.value[0].get(0).unwrap().as_count(),
        Some(64_u64.pow(5))
    );
    assert_eq!(
        measured.value[0].get(1).unwrap().as_count(),
        Some(64_u64.pow(5))
    );
    assert_eq!(measured.value[0].get(2).unwrap().as_count(), Some(1));
    assert_eq!(
        measured.rows.snapshot_records, 64,
        "admission still counts every source edge"
    );
    assert!(
        measured.evaluator.work_units < 4096,
        "weighted traversal regressed to occurrences"
    );
    ...
    let result = query.execute_governed(
        ...
        |_, _| Ok::<_, ()>(true),
        |_, _| Ok(None),
        GqlQueryPolicy::new(63, 1, u64::MAX, u64::MAX),
        || Ok::<_, ()>(()),
    );
...
fn above_u64_weights_refuse_counts_but_not_support_or_doomed_prefixes() {
    ...
    let support = prepare(&repeated(
        64,
        " RETURN COUNT(DISTINCT a) AS n,MIN(a) AS lo,MAX(a) AS hi",
    ));
    ...
    assert_eq!(
        run(&doomed, &edges).unwrap().value[0]
            .get(0)
            .unwrap()
            .as_count(),
        Some(0)
...
fn group_accumulation_overflow_is_checked_after_exact_per_binding_weights() {
    ...
    assert!(
        rows.iter()
            .all(|row| row.get(0).unwrap().as_count() == Some(1_u64 << 63))
    );
...
fn property_sources_and_unused_node_scan_edges_keep_the_ordinary_contract() {
    ...
    let result = query.execute_governed(
        ...
        |_, _| {
            calls += 1;
            if calls == 2 {
                Err("second occurrence source failure")
            } else {
                Ok(Some(&scalar))
            }
        },
    ...
    assert_eq!(
        calls, 2,
        "property-bearing aggregates must not factor away fallible reads"
    );
    ...
    assert_eq!(
        consumed, 0,
        "vertex-only aggregation must not consume a topology iterator"
    );
}
...
fn weighted_preprocessing_and_output_share_exact_limits_and_all_checkpoints() {
    let query = prepare(
        "MATCH (a)-[:R]->(b)-[:S]->(a) RETURN a,COUNT(*) AS n GROUP BY a HAVING n >= 1 ORDER BY n DESC,a ASC LIMIT 1",
    );
    let edges = [
        (VId(0), RelationId(1), VId(1)),
        (VId(0), RelationId(1), VId(1)),
        (VId(1), RelationId(2), VId(0)),
        (VId(1), RelationId(2), VId(0)),
    ];
    let mut calls = 0;
    let mut consumed = 0;
