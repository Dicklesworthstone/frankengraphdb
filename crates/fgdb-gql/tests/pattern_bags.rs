...
fn parallel_edge_products_are_bags_and_mode_is_part_of_logical_identity() {
    ...
    let run =
        |pattern: &fgdb_gql::algebra::PreparedGraphPattern<fgdb_gql::algebra::GraphBindingRow>| {
            pattern
                .plan()
                .execute([], parallel_edges(), |_, _| Ok::<_, ()>(true))
                .unwrap()
                .into_iter()
        ...
        for direction in [
            ...
            for shape in 0..3 {
                ...
                for projection in [&[0, 2][..], &[2, 0][..], &[0][..], &[0, 1, 2][..]] {
                    ...
                    assert_eq!(
                        actual, expected,
                        "shape={shape}, direction={direction:?}, columns={columns:?}"
                    );
                    expected.dedup();
                    let actual = distinct
                        .plan()
...
fn occurrence_pagination_and_all_four_policy_dimensions_are_exact() {
    ...
    for (policy, dimension, observed) in [
        (
            GqlQueryPolicy::new(4, 6, u64::MAX, u64::MAX),
    ...
    for (policy, dimension) in [
        (
            GqlQueryPolicy::new(5, 6, complete.evaluator.work_units - 1, u64::MAX),
            GlaLimitDimension::WorkUnits,
        ),
        (
            GqlQueryPolicy::new(5, 6, u64::MAX, complete.evaluator.scratch_entries - 1),
    ...
    for (offset, count, expected) in [
        ...
        let paged = b
            .prepare_bindings(&["a", "c"], offset, count)
            .unwrap()
            .with_duplicates();
        let result = paged
            .plan()
            .execute_governed(
                ...
                parallel_edges(),
                |_, _| Ok::<_, ()>(true),
                GqlQueryPolicy::new(5, expected, u64::MAX, u64::MAX),
                || Ok::<_, ()>(()),
            )
            .unwrap();
...
fn every_interruption_checkpoint_and_late_source_error_return_no_bag() {
    ...
    for stop in 1..=total {
        let mut calls = 0;
        let result = all.plan().execute_governed(
            5,
            [],
            parallel_edges(),
            |_, _| Ok::<_, ()>(true),
    ...
    b.filter("b", VertexPredicate::HasLabel(LabelId(1)))
        .unwrap();
    let pattern = b
