...
use fgdb_gql::{
    ...
    GraphNullPlacement, GraphPatternTextErrorKind, GraphSymbol, GraphSymbolKind,
...
const HEAD: &str =
    "MATCH (a)-[:R]->(b) RETURN COUNT(*) AS n, a AS owner, SUM(b.p) AS total GROUP BY a";

#[test]
fn text_clauses_equal_typed_definitions_and_rebinding_never_resolves_again() {
    let text = format!(
        "{HEAD} HAVING COUNT(*) >= $minimum AND total IS NOT NULL \
    ...
    assert_eq!(
        template
            .parameter_schema()
            .iter()
            .map(|spec| (spec.name.as_str(), spec.parameter_type))
            .collect::<Vec<_>>(),
        vec![
            ("minimum", GqlParameterType::Int64),
            ("off", GqlParameterType::UInt64),
        ...
        .with_result_clauses(
            ...
            &[
                GraphAggregateOrder {
                    column: GraphAggregateColumn::Aggregate(1),
                    descending: true,
                    nulls: GraphNullPlacement::First,
                },
    ...
    assert!(matches!(
        template
            .bind_parameters(&args(1, 0, 2).with_int64("extra", 9).unwrap())
            .unwrap_err()
            .kind,
...
fn malformed_hidden_or_duplicate_clauses_never_call_the_catalog() {
    for tail in [
        ...
        "ORDER BY total, SUM(b.p)",
        "ORDER BY 1",
        "ORDER BY b.p",
        "ORDER BY total HAVING n > 0",
        "LIMIT 1 ORDER BY total",
    ...
    let text = format!(
        "{HEAD} HAVING {}",
        vec!["n > 0"; MAX_AGGREGATE_FILTERS + 1].join(" AND ")
    );
    let mut calls = 0;
    assert!(
        PreparedGraphAggregateText::prepare(&text, |_, _| {
            calls += 1;
            None
        })
        .is_err()
...
fn filtering_and_ranking_use_complete_aggregates_before_pagination() {
    let text =
        format!("{HEAD} HAVING n >= $minimum ORDER BY total DESC NULLS LAST SKIP $off LIMIT $take");
    ...
    let run = |minimum, offset, count| {
        template
            .bind_parameters(&args(minimum, offset, count))
            .unwrap()
            .execute_governed(
                7,
                [],
    ...
    assert_eq!(
        all.value[0].get(1).unwrap().as_integer(),
...
fn empty_global_null_tests_and_alias_precedence_are_explicit() {
    for (having, expected) in [
        ...
        let text = format!(
            "MATCH (x) RETURN COUNT(*) AS n, SUM(x.p) AS total HAVING {having} ORDER BY total DESC NULLS FIRST"
        );
    ...
    let aggregate = PreparedGraphAggregateText::prepare(text, symbols)
        ...
        .unwrap();
    assert_eq!(
        aggregate.having()[0].column,
        GraphAggregateColumn::Aggregate(0)
    );
    assert_eq!(
        aggregate.ordering()[0].column,
        GraphAggregateColumn::Aggregate(0)
    );
}
...
fn ranked_execution_shares_every_budget_and_interruption_checkpoint() {
    ...
    let aggregate = PreparedGraphAggregateText::prepare(&text, symbols)
        ...
        .unwrap();
    let edges: Vec<_> = (1..=8)
        .map(|id| (VId(id), RelationId(1), VId(id)))
        .collect();
    ...
    let run = |policy| {
        aggregate.execute_governed(
            ...
            edges.iter().copied(),
            |_, _| Ok::<_, ()>(true),
            |vid, _| Ok(scalars.get(vid.0 as usize - 1)),
            policy,
            || Ok::<_, ()>(()),
        )
    };
    ...
    let invalid = PreparedGraphAggregateText::prepare(
        "MATCH (x) RETURN MIN(x.p) AS least HAVING least > 0 ORDER BY least LIMIT 0",
        symbols,
    )
    .unwrap()
    .bind_parameters(&GqlParameters::new())
