...
fn one_exact_map_binds_shared_and_statement_local_parameters_without_catalog_reentry() {
    ...
    let prepare = |text| {
        PreparedGraphMutationText::prepare(text, RelationId(1), |kind, name| {
    ...
    let template =
        PreparedGraphMutationProgramTemplate::prepare(vec![prepare(FIRST), prepare(SECOND)])
            .unwrap();
    ...
    let other = GqlParameters::new()
        ...
        .unwrap();
    assert_ne!(
        template.bind_parameters(&other).unwrap().canonical_bytes(),
        frozen
    );
    ...
    assert!(matches!(
        template.bind_parameters(&arguments.with_int64("extra", 1).unwrap()),
...
fn conflicting_declarations_and_coordinate_or_definition_limits_fail_before_binding() {
    ...
    let boolean = PreparedGraphMutationText::prepare_with_parameter_types(
        ...
        &[(
            "value",
            GqlParameterType::Scalar(CanonicalScalarKind::of(&CanonicalScalar::Bool(true))),
        )],
        symbols,
    )
    .unwrap();
    ...
    assert!(matches!(
        conflict,
        Err(Error::ConflictingParameterTypes {
            parameter: 0,
            first_statement: 0,
            statement: 1,
        })
    ...
    assert!(matches!(
        PreparedGraphMutationProgramTemplate::prepare(vec![]),
    ...
    assert_eq!(
        PreparedGraphMutationProgramTemplate::prepare(maximum.clone())
            .unwrap()
            .statements()
            .len(),
        MAX_GRAPH_MUTATION_STATEMENTS
    );
    let mut excessive = maximum;
    excessive.push(input("MATCH (n) SET n.p=1"));
    assert!(matches!(
        PreparedGraphMutationProgramTemplate::prepare(excessive),
        Err(Error::Definition(
            GraphMutationProgramBuildError::TooManyStatements { .. }
        ))
    ));
    let foreign =
        PreparedGraphMutationText::prepare("MATCH (n) SET n.p=1", RelationId(2), symbols).unwrap();
    assert!(matches!(
        PreparedGraphMutationProgramTemplate::prepare(vec![input("MATCH (n) SET n.p=1"), foreign]),
        Err(Error::Definition(
...
fn missing_late_arguments_keep_statement_and_utf8_offsets_and_scalars_are_not_interpolated() {
    ...
    let template =
        PreparedGraphMutationProgramTemplate::prepare(vec![first, input(later)]).unwrap();
    ...
    assert!(
        matches!(template.bind_parameters(&wrong), Err(Error::Bind { statement: 1, source })
