...
use fgdb_gql::algebra::{
    ...
    IntegerComparison, MAX_PATTERN_PREDICATES, PatternBuildError, VertexPredicate,
};
...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(1000, 10000, u64::MAX, u64::MAX)
}
...
fn plain(rows: &[GraphValueRow]) -> Vec<Vec<Option<VId>>> {
    rows.iter()
        .map(|row| row.values().iter().map(|cell| cell.as_vertex()).collect())
        .collect()
}
fn position(vid: VId) -> usize {
    IDS.iter().position(|id| *id == vid).unwrap()
}
fn expected(
    ...
    let (Some(CanonicalScalar::Int(left)), Some(CanonicalScalar::Int(right))) = (left, right)
    else {
        return false;
    };
    match op {
            ...
            for comparison in OPS {
                    ...
                    for (a, via) in oriented(s, d, direction) {
                        for &(s2, r2, d2) in &edges {
                            ...
                            for (b, c) in oriented(s2, d2, direction) {
                                if via == b
                                    && expected(
                                        left[position(a)].as_ref(),
                                        right[position(c)].as_ref(),
                                        comparison,
                                    )
                                {
                ...
                let actual = query
                    ...
                    .execute_governed_with_properties(
                        ...
                        |vid, key| {
                            Ok(if key == P {
                                left[position(vid)].as_ref()
                            } else {
                                right[position(vid)].as_ref()
                            })
                        },
                        wide(),
                        || Ok::<_, ()>(()),
                ...
                assert_eq!(
                    plain(&actual.value),
                    wanted,
                    "mask={mask}, direction={direction:?}, comparison={comparison:?}"
                );
                assert!(query.plan().needs_vertex_values());
                assert!(
                    !query.plan().projects_properties(),
                    "unprojected predicates still require property admission"
                );
            }
        }
    }
...
fn optional_and_existence_apply_pair_filters_before_scope_success() {
    ...
    child
        .compare_properties("a", P, IntegerComparison::Less, "c", Q)
        .unwrap();
    let left = [
    ...
    for (clause, wanted) in [
        (GraphMatchClause::exists(&child), vec![IDS[0]]),
        (GraphMatchClause::not_exists(&child), vec![IDS[1], IDS[2]]),
    ] {
        let query = root
            .prepare_values_with_clauses(&[clause], &[GraphColumn::vertex("a", "a")], 0, None)
            .unwrap()
            .with_duplicates();
        let result = query
            .plan()
    ...
    let aggregate = PreparedGraphAggregate::prepare(
        ...
        &[
            ...
            GraphAggregate::count("matched", 1),
        ],
        0,
        None,
    )
...
fn comparisons_are_not_cached_by_only_the_left_vertex_and_can_use_one_vertex_twice() {
    ...
    b.compare_properties("a", P, IntegerComparison::Less, "b", P)
        .unwrap();
    let query = b
        .prepare_values(
    ...
    let rows = query
        ...
        .execute_governed_with_properties(
            2,
            [],
            [(IDS[0], R, IDS[1]), (IDS[0], R, IDS[2])],
            |_, _| Ok::<_, ()>(true),
            |vid, _| Ok(Some(&values[position(vid)])),
            wide(),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    ...
    single
        .compare_properties("a", P, IntegerComparison::Less, "a", Q)
        .unwrap();
    let query = single
    ...
    let rows = query
        ...
        .execute_governed_with_properties(
            ...
            [],
            |_, _| Ok::<_, ()>(true),
            |_, key| Ok(Some(if key == P { &values[1] } else { &values[2] })),
            wide(),
            || Ok::<_, ()>(()),
        )
        .unwrap();
...
fn pair_selection_preserves_exact_limits_and_every_interruption_checkpoint() {
    let mut b = builder(&["a", "b"]);
    b.edge("a", R, GlaDirection::Forward, "b").unwrap();
    b.compare_properties("a", P, IntegerComparison::Less, "b", Q)
        .unwrap();
    let query = b
        .prepare_values(&[GraphColumn::vertex("a", "a")], 0, None)
        .unwrap()
        .with_duplicates();
    let values = [
        CanonicalScalar::ucs_basic_text(&"a".repeat(2048)).unwrap(),
        CanonicalScalar::ucs_basic_text(&"b".repeat(4096)).unwrap(),
    ];
    let run = |policy| {
        query.plan().execute_governed_with_properties(
            2,
            [],
            [(IDS[0], R, IDS[1]); 2],
            |_, _| Ok::<_, ()>(true),
    ...
    assert!(matches!(
        run(GqlQueryPolicy::new(2, 1, u64::MAX, u64::MAX)),
...
fn definitions_are_bounded_immutable_and_cannot_escape_property_source_requirements() {
    ...
    let original = b
        ...
        .unwrap();
    assert_eq!(
        b.compare_properties("missing", P, IntegerComparison::Equal, "b", Q)
            .unwrap_err(),
        PatternBuildError::UnknownVariable
    );
    assert_eq!(
        b.prepare_values(&[GraphColumn::vertex("a", "a")], 0, None)
            .unwrap(),
        original
    );
    b.compare_properties("a", P, IntegerComparison::Equal, "b", Q)
        .unwrap();
    assert_eq!(
        b.prepare("a", 0, None).unwrap_err(),
        PatternBuildError::RequiresValueProjection
    );
    assert_eq!(
        b.prepare_bindings(&["a", "b"], 0, None).unwrap_err(),
        PatternBuildError::RequiresValueProjection
    );
    let compared = b
        .prepare_values(&[GraphColumn::vertex("a", "a")], 0, None)
    ...
    for _ in 1..MAX_PATTERN_PREDICATES {
        b.compare_properties("a", P, IntegerComparison::Equal, "b", Q)
            .unwrap();
    }
    let full = b
    ...
    assert_eq!(
        compared
            ...
            .operators()
            .iter()
            .filter(|op| matches!(op, fgdb_gql::algebra::GlaOperator::CompareProperties { .. }))
            .count(),
        1
    );
}
