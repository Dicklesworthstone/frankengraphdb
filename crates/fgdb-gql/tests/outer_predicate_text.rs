...
fn values() -> BTreeMap<VId, CanonicalScalar> {
    BTreeMap::from([(VId(1), CanonicalScalar::Int(7)), (VId(2), CanonicalScalar::Int(7)),
        (VId(3), CanonicalScalar::Int(9)), (VId(4), CanonicalScalar::Null)])
}
...
fn run(text: &str, edges: &[(VId, RelationId, VId)]) -> Vec<GraphValueRow> {
    ...
    pattern.plan().execute_governed_with_properties(
        ...
        |vid, predicates| Ok::<_, ()>(predicates.iter().all(|predicate|
            predicate.matches(&[], &[(P, properties[&vid].clone())]))),
        |vid, key| Ok(if key == P { properties.get(&vid) } else { None }),
        wide(), || Ok::<_, ()>(()),
    ).unwrap().value
}
fn captures(pattern: &PreparedGraphPattern<GraphValueRow>) -> usize {
    pattern.plan().operators().iter().filter(|operator|
        matches!(operator, GlaOperator::BindOuterVertex { .. })).count()
}
...
fn native_outer_property_join_matches_typed_capture_and_resolves_symbols_once() {
    ...
    let arguments = GqlParameters::new().with_uint64("off", 1).unwrap().with_uint64("take", 3).unwrap();
    ...
    child.compare_properties("b", P, IntegerComparison::Equal, "a", P).unwrap();
    let expected = root.prepare_values_with_clauses(&[GraphMatchClause::required(&child)],
        &[GraphColumn::vertex("a", "a"), GraphColumn::vertex("b", "b")], 1, Some(3)).unwrap().with_duplicates();
    ...
    assert_eq!(run("MATCH (a) MATCH (b) WHERE b.p=a.p RETURN a,b", &[]).len(), 5);
}
...
fn captured_nullable_values_do_not_become_positive_matches_or_remove_outer_duplicates() {
    ...
    assert_eq!(rows.iter().filter(|row| row.values()[1].is_null()).count(), 12);
    ...
    assert_eq!(run(&format!("{head} MATCH (c) WHERE b.p IS NULL RETURN a,c"), &edges).len(), 12);
    assert_eq!(run(&format!("{head} MATCH (c) WHERE b.p IS NOT NULL RETURN a,c"), &edges).len(), 8);
...
fn every_native_predicate_form_captures_only_referenced_outer_operands() {
    for (predicate, expected) in [
        ("a.p=7", 8), ("a.p='not an integer'", 0), ("a.p IS NULL", 4),
        ("a IS NULL", 0), ("b.p=a.p", 5), ("b=a", 4),
        ("a.p IN [b.p,NULL]", 5), ("a.p BETWEEN b.p AND b.p", 5),
        ("NOT (a IS NULL) AND (b.p=a.p OR b=a)", 6),
    ] {
        let text = format!("MATCH (unused),(a) MATCH (b) WHERE {predicate} RETURN a,b");
        assert_eq!(captures(&prepare(&text)), 1, "unused outer variable was captured: {predicate}");
...
fn existential_outer_only_correlations_keep_local_names_private_and_do_not_multiply() {
    let rows = run("MATCH (a) WHERE EXISTS { MATCH (b) WHERE b=a } RETURN a", &[]);
    assert_eq!(rows.len(), 4);
    let rows = run("MATCH (a) WHERE NOT EXISTS { MATCH (b) WHERE b.p=a.p } RETURN a", &[]);
...
fn capture_admission_counts_used_names_not_the_temporary_visible_symbol_table() {
    let nodes = (0..65).map(|at| format!("(v{at})")).collect::<Vec<_>>().join(",");
    let unused = format!("MATCH (a) WHERE EXISTS {{ MATCH {nodes} }} RETURN a LIMIT 0");
    assert_eq!(captures(&prepare(&unused)), 0);
    let overflow = format!("MATCH (a) WHERE EXISTS {{ MATCH {nodes} WHERE a.p=1 }} RETURN a LIMIT 0");
    let mut calls = 0;
    let error = PreparedGraphText::prepare(&overflow, |kind, name| { calls += 1; symbols(kind, name) }).unwrap_err();
    assert!(matches!(error.kind, GraphPatternTextErrorKind::Build(PatternBuildError::LimitExceeded {
        dimension: PatternLimitDimension::Vertices, limit: 65, observed: 66,
    })));
    ...
    let shortest = prepare("MATCH (a) MATCH ANY SHORTEST WALK (b)-[:R*0..3]->(c) WHERE c.p=a.p RETURN a,c");
...
fn original_parameter_offsets_rebinding_and_eager_source_failures_survive_captures() {
    ...
    let args = GqlParameters::new().with_int64("key", 7).unwrap().with_uint64("take", 2).unwrap();
    let first = template.bind_parameters(&args).unwrap();
    let frozen = first.canonical_bytes();
    let changed = GqlParameters::new().with_int64("key", 9).unwrap().with_uint64("take", 1).unwrap();
    ...
    for head in ["MATCH (b) WHERE TRUE OR a.p IS NULL",
        ...
        let result = pattern.plan().execute_governed_with_properties(
            4, VERTICES, [], |_, _| Ok::<_, &str>(true),
            |_, _| Err("unreadable outer operand"), wide(), || Ok::<_, ()>(()),
        );
        assert!(matches!(result, Err(GqlQueryError::Source("unreadable outer operand"))));
    }
}
...
fn aggregate_input_and_write_script_batches_reuse_the_same_outer_binding_path() {
    ...
    let properties = values();
    let rows = aggregate.execute_governed(4, VERTICES, [], |_, _| Ok::<_, ()>(true),
        |vid, _| Ok(properties.get(&vid)), wide(), || Ok::<_, ()>(()),
    ).unwrap().value;
    assert_eq!(rows.iter().map(|row| (row.keys()[0].as_vertex().unwrap(),
        row.values()[0].as_count().unwrap(), row.values()[1].as_count().unwrap())).collect::<Vec<_>>(),
        vec![(VId(1), 2, 2), (VId(2), 2, 2), (VId(3), 1, 1), (VId(4), 1, 0)]);
    ...
    let [GraphWriteStatement::Mutation(update), GraphWriteStatement::Delete(delete)] = program.statements()
        else { panic!("wrong shared write dispatch") };
