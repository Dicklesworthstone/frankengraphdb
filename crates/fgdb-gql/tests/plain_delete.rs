...
fn optional_null_targets_are_ignored_without_inventing_an_identity() {
    let delete = deletion(
        "MATCH (a) OPTIONAL MATCH (a)-[:R]->(b) RETURN ALL b",
        vec![0],
    );
...
fn target_schema_and_definition_limits_are_checked_before_execution() {
    ...
    assert!(matches!(
        ...
        Err(GraphDeleteBuildError::TargetColumn {
            target: 1,
            column: 0
        })
    ));
...
fn target_quota_is_exact_and_refusal_returns_no_partial_proposal() {
    ...
    assert!(matches!(
        ...
        Err(GqlQueryError::Source(GraphDeleteError::TargetLimit {
            limit: 2,
            observed: 3
        }))
    ));
    assert_eq!(
        execute(&delete, &vertices, &[], policy(3))
            .unwrap()
            .targets(),
        &[VId(1), VId(2), VId(3)]
    );
...
fn exact_work_scratch_and_every_checkpoint_are_enforced() {
    ...
    let measured = execute(&delete, &vertices, &edges, policy(10))
        .unwrap()
        .stats();
    ...
    assert_eq!(
        execute(&delete, &vertices, &edges, exact).unwrap().stats(),
        measured
    );
