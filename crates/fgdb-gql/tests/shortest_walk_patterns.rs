...
use fgdb_gql::algebra::{
    ...
    GraphWalkSearch, PatternBuildError, VertexPredicate,
};
use fgdb_gql::{GlaExecutionError, GlaExecutionLimits, GqlQueryError, GqlQueryPolicy,
    GraphWalkBounds, MAX_GRAPH_WALK_HOPS};
use fgdb_types::VId;
...
fn oracle(source: VId, direction: GlaDirection, minimum: u32, maximum: u32,
    edges: &[Edge]) -> BTreeMap<VId, usize> {
    let mut layer = BTreeMap::from([(source, 1_usize)]);
    let mut answer = BTreeMap::new();
    for depth in 0..=maximum {
        if depth >= minimum {
            for (&vertex, &count) in &layer { answer.entry(vertex).or_insert(count); }
        }
        ...
        for (vertex, count) in layer {
            for &(left, relation, right) in edges {
                ...
                match direction {
                    GlaDirection::Forward if left == vertex => *next.entry(right).or_default() += count,
                    GlaDirection::Reverse if right == vertex => *next.entry(left).or_default() += count,
...
fn multigraph_shortest_bags_distinct_and_pages_match_unpruned_layers() {
    ...
    let universe = [(VId(0), R, VId(0)), (VId(0), R, VId(1)),
        (VId(0), R, VId(1)), (VId(1), R, VId(0)), (VId(1), R, VId(2)),
        (VId(2), R, VId(2)), (VId(2), S, VId(9))];
    for mask in 0..128_usize {
        ...
        for direction in [GlaDirection::Forward, GlaDirection::Reverse, GlaDirection::Undirected] {
            for (minimum, maximum) in [(0, 0), (0, 2), (1, 1), (1, 3), (2, 3), (3, 4)] {
                ...
                b.shortest_walk("a", R, direction, "b", bounds(minimum, maximum)).unwrap();
                ...
                for distinct in [false, true] {
                    ...
                    for (offset, count) in [(0, None), (1, Some(3)), (0, Some(0))] {
                        ...
                        assert_eq!(actual.iter().map(|row| row.values().to_vec()).collect::<Vec<_>>(),
                            page, "mask={mask}, {direction:?}, {minimum}..{maximum}, distinct={distinct}");
                        let reversed = query.plan().execute(vertices.into_iter().rev(),
                            edges.iter().rev().copied(), |_, _| Ok::<_, ()>(true)).unwrap();
                        assert_eq!(actual, reversed, "input iteration order must not select a tie");
                    }
                }
            }
        }
...
fn ordinary_walk_bytes_are_frozen_and_shortest_has_a_distinct_tag() {
    let make = |shortest, names: [&str; 2]| {
        ...
        if shortest {
            b.shortest_walk(names[0], R, GlaDirection::Forward, names[1], bounds(1, 3)).unwrap();
        } else {
            b.walk(names[0], R, GlaDirection::Forward, names[1], bounds(1, 3)).unwrap();
        }
        b.prepare_bindings(&names, 0, None).unwrap().with_duplicates()
    };
    let transcript = |tag| {
    ...
    assert_eq!(shortest.canonical_bytes(), make(true, ["x", "y"]).canonical_bytes());
    assert!(matches!(shortest.plan().operators()[1], GlaOperator::VarLengthExpand {
...
fn shortest_atom_can_expand_from_its_already_bound_destination() {
    ...
    let edges = [(VId(0), S, VId(3)), (VId(0), S, VId(3)),
        (VId(0), R, VId(1)), (VId(0), R, VId(1)), (VId(1), R, VId(3)),
        (VId(0), R, VId(2)), (VId(2), R, VId(3)), (VId(3), R, VId(3))];
    let mut b = builder(&["p", "b", "a"]);
    b.edge("p", S, GlaDirection::Forward, "b").unwrap();
    b.shortest_walk("a", R, GlaDirection::Forward, "b", bounds(1, 4)).unwrap();
    let query = b.prepare_bindings(&["p", "a", "b"], 0, None).unwrap().with_duplicates();
    assert!(query.plan().operators().iter().any(|op| matches!(op, GlaOperator::VarLengthExpand {
        direction: GlaDirection::Reverse, search: GraphWalkSearch::AllShortest, ..
    })));
    let mut expected = Vec::new();
    for &(p, relation, target) in &edges {
        ...
        for source in vertices {
            let count = oracle(source, GlaDirection::Forward, 1, 4, &edges)
                .get(&target).copied().unwrap_or(0);
            for _ in 0..count { expected.push(vec![p, source, target]); }
        }
    }
    expected.sort();
    let rows = query.plan().execute(vertices, edges, |_, _| Ok::<_, ()>(true)).unwrap();
...
fn optional_and_existence_scopes_keep_shortest_mode_and_real_null_absence() {
    ...
    let edges = [(VId(0), R, VId(1)), (VId(0), R, VId(1)),
        (VId(1), R, VId(2)), (VId(2), R, VId(2))];
    let outer = builder(&["b"]);
    let mut child = builder(&["a", "b"]);
    child.shortest_walk("a", R, GlaDirection::Forward, "b", bounds(1, 3)).unwrap();
    let query = outer.prepare_values_with_clauses(&[GraphMatchClause::optional(&child)],
        &[GraphColumn::vertex("b", "b"), GraphColumn::vertex("a", "a")], 0, None)
        .unwrap().with_duplicates();
    let rows = query.plan().execute_with_properties_control(vertices, edges,
        |_, _| Ok::<_, ()>(true), |_, _| Ok(None), |_| Ok(())).unwrap();
    let mut actual = rows.iter().map(|row| row.values().iter().map(|v| v.as_vertex())
    ...
    for target in vertices {
        ...
        if matches.is_empty() {
            ...
            for (source, count) in matches {
                for _ in 0..count { expected.push(vec![Some(target), Some(source)]); }
            }
        }
    }
    actual.sort(); expected.sort();
    assert_eq!(actual, expected);
    for (clause, expected) in [(GraphMatchClause::exists(&child), present),
        (GraphMatchClause::not_exists(&child), absent)] {
        let query = outer.prepare_values_with_clauses(&[clause],
            &[GraphColumn::vertex("b", "b")], 0, None).unwrap().with_duplicates();
        let rows = query.plan().execute_with_properties_control(vertices, edges,
            |_, _| Ok::<_, ()>(true), |_, _| Ok(None), |_| Ok(())).unwrap();
        assert_eq!(rows.iter().map(|row| row.values()[0].as_vertex().unwrap())
...
fn endpoint_filters_do_not_remove_transit_vertices_or_swallow_source_errors() {
    let mut b = builder(&["a", "b"]);
    b.shortest_walk("a", R, GlaDirection::Forward, "b", bounds(1, 3)).unwrap();
    b.filter("a", VertexPredicate::HasLabel(LabelId(1))).unwrap();
    b.filter("b", VertexPredicate::HasLabel(LabelId(2))).unwrap();
    ...
    let rows = query.plan().execute(vertices, edges, |vid, predicates| {
        let labels = if vid == VId(1) { vec![LabelId(1)] }
            else if vid == VId(3) { vec![LabelId(2)] } else { vec![] };
        Ok::<_, ()>(predicates.iter().all(|predicate| predicate.matches(&labels, &[])))
    }).unwrap();
    assert_eq!(rows.iter().map(|row| row.values().to_vec()).collect::<Vec<_>>(),
        vec![vec![VId(1), VId(3)]]);
    let failure = query.plan().execute(vertices, edges, |vid, _| {
        if vid == VId(3) { Err("unreadable endpoint") } else { Ok(true) }
    });
...
fn exact_resource_limits_and_every_checkpoint_are_enforced_by_the_real_executor() {
    ...
    let measured = query.plan().execute_governed(7, vertices, edges, |_, _| Ok::<_, ()>(true),
        wide(), || { checkpoints += 1; Ok::<_, usize>(()) }).unwrap();
    let caps = [7, measured.rows.result_rows, measured.evaluator.work_units,
        measured.evaluator.scratch_entries];
    let exact = GqlQueryPolicy::new(caps[0], caps[1], caps[2], caps[3]);
    let rerun = query.plan().execute_governed(7, vertices, edges, |_, _| Ok::<_, ()>(true),
        exact, || Ok::<_, usize>(())).unwrap();
    assert_eq!(rerun, measured);
    for dimension in 0..4 {
        let mut limited = caps; limited[dimension] -= 1;
        let result = query.plan().execute_governed(7, vertices, edges,
            |_, _| Ok::<_, ()>(true), GqlQueryPolicy::new(limited[0], limited[1], limited[2], limited[3]),
            || Ok::<_, usize>(()));
        assert!(result.is_err(), "dimension {dimension} must refuse, not return a partial bag");
    }
...
fn shortest_execution_prunes_cycles_instead_of_enumerating_then_filtering() {
    ...
    all.walk("a", R, GlaDirection::Forward, "b", bounds(1, MAX_GRAPH_WALK_HOPS)).unwrap();
    let mut shortest = builder(&["a", "b"]);
    shortest.shortest_walk("a", R, GlaDirection::Forward, "b",
        bounds(1, MAX_GRAPH_WALK_HOPS)).unwrap();
    ...
    let result = shortest.plan().execute_with_limits([VId(1)], edges,
        |_, _| Ok::<_, ()>(true), limits).unwrap();
    assert_eq!(result.value, vec![VId(1), VId(1)]);
    assert!(matches!(all.plan().execute_with_limits([VId(1)], edges,
        |_, _| Ok::<_, ()>(true), limits), Err(GlaExecutionError::Limit(_))));
}

#[test]
fn rejected_shortest_atom_does_not_change_an_existing_builder() {
    let mut b = builder(&["a", "b"]);
    b.walk("a", R, GlaDirection::Forward, "b", bounds(0, 0)).unwrap();
    let before = b.prepare_bindings(&["a", "b"], 0, None).unwrap();
    assert_eq!(b.shortest_walk("a", R, GlaDirection::Forward, "missing", bounds(1, 3))
        .unwrap_err(), PatternBuildError::UnknownVariable);
    assert_eq!(b.prepare_bindings(&["a", "b"], 0, None).unwrap(), before);
    for _ in 1..fgdb_gql::algebra::MAX_PATTERN_EDGES {
        b.shortest_walk("a", R, GlaDirection::Forward, "b", bounds(1, 1)).unwrap();
    }
    let before = b.prepare("a", 0, None).unwrap().canonical_bytes();
    assert!(b.shortest_walk("a", R, GlaDirection::Forward, "b", bounds(1, 1)).is_err());
