...
use fgdb_gql::{
    ...
    GraphVertexMergeStats, GraphVertexUpsertBranch, GraphVertexUpsertStats,
    GraphWriteProgramError, GraphWriteProgramPolicy, GraphWriteStepError,
    GraphWriteStepStats, PreparedGraphVertexMergeText, PreparedGraphVertexUpsertText,
...
fn upsert() -> fgdb_gql::PreparedGraphVertexUpsert {
    PreparedGraphVertexUpsertText::prepare(
        ...
        RelationId(1), symbols,
    ).unwrap().bind_parameters(&GqlParameters::new()).unwrap()
}
fn stats(created: bool) -> GraphVertexMergeStats {
    GraphVertexMergeStats {
        match_selection: GqlExecutionStats { snapshot_records: 2, result_rows: u64::from(!created) },
...
fn exact_merge_program_limits_and_remaining_creation_allowance_are_shared() {
    ...
    let result: Result<_, Error> = program.execute_governed(exact, |statement, _, remaining| {
        ...
        assert_eq!(remaining.vertex_merge_policy().max_created_vertices, remaining.max_created_vertices);
    ...
    assert_eq!(actual.selection, GqlExecutionStats { snapshot_records: 4, result_rows: 1 });
    assert_eq!(actual.evaluator, GlaExecutionStats { work_units: 9, scratch_entries: 4 });
    assert_eq!(actual.proposed_effects(), 1);
    for dimension in 0..5 {
        let mut caps = [4, 1, 9, 4, 1];
        caps[dimension] -= 1;
        let limited = GraphWriteProgramPolicy::new(
            GqlQueryPolicy::new(caps[0], caps[1], caps[2], caps[3]), 0, caps[4], 0,
        );
...
fn create_upsert_actions_are_metered_even_with_zero_selected_rows() {
    ...
    let result: Result<_, Error> = program.execute_governed(policy(), |_, _, _| {
        Ok(GraphWriteStepStats::VertexUpsert(GraphVertexUpsertStats {
            merge: stats(true), branch: GraphVertexUpsertBranch::Create, action_effects: 1,
        }))
    }, || Ok(()));
    let actual = result.unwrap();
    assert_eq!(actual.selection.result_rows, 0);
    assert_eq!((actual.created_vertices, actual.mutation_effects, actual.target_vertex_visits), (1, 1, 1));
...
fn impossible_merge_and_upsert_statistics_refuse_before_later_steps() {
    ...
    for corruption in 0..2 {
        let result: Result<_, Error> = program.execute_governed(policy(), |_, _, _| {
            Ok(GraphWriteStepStats::VertexUpsert(GraphVertexUpsertStats {
                merge: stats(true),
                branch: if corruption == 0 { GraphVertexUpsertBranch::Match } else { GraphVertexUpsertBranch::Create },
...
fn nested_merge_creation_refusal_reports_whole_program_budget() {
    ...
    let result: Result<_, Error> = program.execute_governed(limited, |statement, _, remaining| {
        if statement == 0 {
