...
use fgdb_gql::{
    ...
    GraphDeleteError, GraphDeleteStats, GraphMutationProgramDimension, GraphMutationProgramError,
    GraphWriteProgramError, GraphWriteProgramPolicy, GraphWriteStatement, GraphWriteStepError,
    GraphWriteStepStats, PreparedGraphDeleteText, PreparedGraphMutationText,
...
fn deletion() -> GraphWriteStatement {
    PreparedGraphDeleteText::prepare("MATCH (n) DELETE n", R, |_, _| None)
        .unwrap()
...
fn run(policy: GraphWriteProgramPolicy) -> Result<fgdb_gql::GraphWriteProgramStats, Error> {
    PreparedGraphWriteProgram::prepare(vec![deletion(), deletion()])
        .unwrap()
        .execute_governed(
            policy,
            |_, _, _| Ok(GraphWriteStepStats::Delete(stats())),
            || Ok(()),
        )
}
    ...
    for incidence in [false, true] {
        ...
        let result: Result<_, Error> = program.execute_governed(
            ...
            |at, _, remaining| {
                visited.push(at);
                if at == 0 {
                    return Ok(GraphWriteStepStats::Delete(stats()));
                }
                assert_eq!(remaining.deletion_policy().max_targets, 0);
                Err(GraphWriteStepError::Delete(GqlQueryError::Source(
                    if incidence {
                        GraphDeleteError::IncidentRelationships
                    } else {
                        GraphDeleteError::TargetLimit {
                            limit: 0,
                            observed: 1,
                        }
                    },
...
fn impossible_targets_and_cumulative_source_overflow_are_refused() {
    ...
    let bad: Result<_, Error> = program.execute_governed(
        policy(),
        |_, _, _| {
            Ok(GraphWriteStepStats::Delete(GraphDeleteStats {
                target_vertices: 3,
                ..stats()
            }))
        },
    ...
    let overflow: Result<_, Error> = program.execute_governed(
        allowance,
        |at, _, _| {
            let mut item = stats();
            if at == 1 {
                item.selection.snapshot_records = u64::MAX;
            }
    ...
    assert!(
        matches!(overflow, Err(GraphWriteProgramError::Program(GraphMutationProgramError::Budget {
...
fn empty_delete_continues_and_plain_delete_has_a_distinct_canonical_tag() {
    ...
    let result: Result<_, Error> = program.execute_governed(
        ...
        |at, _, _| {
            let mut item = stats();
            if at == 0 {
                item.target_vertices = 0;
                item.selection.result_rows = 0;
            }
            Ok(GraphWriteStepStats::Delete(item))
        },
        || Ok(()),
