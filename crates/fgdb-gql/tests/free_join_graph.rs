...
use fgdb_gql::free_join::{
    ColumnarTrie, FreeJoin, FreeJoinPlan, GraphTrie, GraphTrieError,
    JoinVariable as V, TrieCursor, TrieRelation,
};
use fgdb_types::VId;
...
fn bag<T: TrieRelation<VId>>(plan: &FreeJoinPlan, inputs: &[T]) -> BTreeMap<Vec<VId>, u128> {
    ...
    FreeJoin::new(plan, inputs).unwrap().for_each_binding(&mut allow, |binding, _| {
        assert!(result.insert(binding.values().copied().collect(), binding.multiplicity().unwrap()).is_none());
        Ok(())
    }).unwrap();
    result
}
...
fn borrowed_keys_duplicate_runs_and_parent_navigation_are_exact() {
    ...
    assert!(std::ptr::eq(root.key().unwrap(), adjacency.get_key_value(&VId(1)).unwrap().0));
    ...
    assert_eq!(child.open(&mut allow).unwrap().unwrap().multiplicity(), Some(2));
    child.seek(&VId(u128::MAX), &mut allow).unwrap();
    assert_eq!(child.open(&mut allow).unwrap().unwrap().multiplicity(), Some(1));
    assert_eq!(root.key(), Some(&VId(1)));
    root.advance(&mut allow).unwrap();
    assert_eq!(root.key(), Some(&VId(u128::MAX)));
    assert_eq!(child.key(), Some(&VId(u128::MAX)), "parent advance cannot invalidate its borrowed child");
}
...
fn vertex_domains_and_self_loops_feed_the_same_factorized_join() {
    ...
    let plan = FreeJoinPlan::generic(vec![vec![V(0)], vec![V(0)], vec![V(0), V(1)]], vec![V(0), V(1)]).unwrap();
    let inputs = [
    ...
    assert_eq!(bag(&plan, &inputs), BTreeMap::from([
        (vec![VId(1), VId(1)], 4), (vec![VId(1), VId(2)], 6),
    ]));
    let factor = FreeJoin::new(&plan, &inputs).unwrap().factorize(&mut allow).unwrap();
        ...
        for rows in &mut raw {
            for a in 0..3_u128 {
                for b in 0..3_u128 {
                    ...
                    for _ in 0..(seed >> 32) % 3 { rows.push(vec![VId(a), VId(b)]); }
                }
            }
        }
        for order in orders {
            let plan = FreeJoinPlan::generic(schemas.clone(), order.into_iter().map(V).collect()).unwrap();
            ...
            for (at, rows) in raw.iter().enumerate() {
                ...
                for row in rows {
                    let (a, b) = if reverse { (row[1], row[0]) } else { (row[0], row[1]) };
                    map.entry(a).or_default().push(b);
                }
                for neighbors in map.values_mut() { neighbors.sort(); }
                maps.push(map);
            }
            let borrowed: Vec<_> = maps.iter().enumerate().map(|(at, map)| {
                let order = plan.required_order(at).unwrap();
                GraphTrie::adjacency([order[0], order[1]], map, 0, &mut allow).unwrap()
            }).collect();
            let owned: Vec<_> = raw.iter().enumerate().map(|(at, rows)| {
                ColumnarTrie::new(&schemas[at], plan.required_order(at).unwrap(), rows.clone(), 0, &mut allow).unwrap()
            }).collect();
...
fn malformed_order_and_repeated_variables_are_not_silently_repaired() {
    ...
    assert!(matches!(GraphTrie::adjacency([V(0), V(1)], &bad, 0, &mut allow), Err(GraphTrieError::UnsortedInput)));
    assert!(matches!(GraphTrie::diagonal(V(0), &bad, 0, &mut allow), Err(GraphTrieError::UnsortedInput)));
    assert!(matches!(GraphTrie::vertices(V(0), &[VId(2), VId(1)], 0, &mut allow), Err(GraphTrieError::UnsortedInput)));
    assert!(matches!(GraphTrie::adjacency([V(0), V(0)], &bad, 0, &mut allow), Err(GraphTrieError::RepeatedVariable)));
}
...
fn every_build_and_seek_checkpoint_preserves_the_callers_refusal() {
    ...
    let mut total = 0;
    let trie = GraphTrie::adjacency([V(0), V(1)], &map, 3, &mut |_| { total += 1; Ok::<_, usize>(()) }).unwrap();
    for stop in 0..total {
        let mut observed = 0;
    ...
    let mut total = 0;
    trie.cursor().open(&mut allow).unwrap().unwrap().seek(&VId(100), &mut |_| { total += 1; Ok::<_, usize>(()) }).unwrap();
    assert!(total < 10);
    for stop in 0..total {
        let mut cursor = trie.cursor().open(&mut allow).unwrap().unwrap();
        let mut observed = 0;
        assert_eq!(cursor.seek(&VId(100), &mut |_| {
            let at = observed; observed += 1;
            if at == stop { Err(at) } else { Ok(()) }
        }), Err(stop));
        assert_eq!(observed, stop + 1);
        assert_eq!(cursor.key(), Some(&VId(0)), "refused seek cannot commit a new position");
    }
}
