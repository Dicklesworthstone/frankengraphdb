...
use fgdb_gql::algebra::{
    GlaDirection, GlaOperator, GraphBindingRow, GraphColumn, GraphMatchClause, GraphPatternBuilder,
    GraphValueRow, IntegerComparison, PatternBuildError, PreparedGraphPattern, VertexPredicate,
};
use fgdb_gql::{
    GqlQueryError, GqlQueryPolicy, GraphAggregate, GraphWalkBounds, MAX_GRAPH_WALK_HOPS,
    ...
    for depth in 0..=maximum {
        if depth >= minimum {
            for (&vertex, &count) in &layer {
                *answer.entry(vertex).or_default() += count;
            }
        }
    ...
    for mask in 0..128_usize {
        ...
        for direction in [
            ...
            for (minimum, maximum) in [(0, 0), (0, 2), (1, 1), (1, 3), (2, 3)] {
                ...
                b.walk("a", R, direction, "b", bounds(minimum, maximum))
                    .unwrap();
                ...
                for distinct in [false, true] {
                    ...
                    for (offset, count) in [(0, None), (1, Some(3)), (0, Some(0))] {
                        ...
                        assert_eq!(
                            reversed, actual,
                            "source iteration order is not result order"
                        );
                    }
                }
            }
...
fn mixed_walk_atoms_close_cycles_and_reverse_binding_without_changing_multiplicity() {
    ...
    let fixed = |left, right| {
        edges
            .iter()
            .filter(|&&(a, r, b)| a == left && r == S && b == right)
            .count()
    };
    let mut expected = Vec::new();
    for a in vertices {
        for b in vertices {
            for c in vertices {
                for d in vertices {
                    ...
                    let count = fixed(a, b)
                        * incoming.get(&b).copied().unwrap_or(0)
                        * outgoing.get(&d).copied().unwrap_or(0)
                        * fixed(a, d);
                    for _ in 0..count {
    ...
    for order in [[0, 1, 2, 3], [1, 0, 2, 3], [3, 2, 1, 0]] {
        ...
        for atom in order {
            match atom {
                0 => {
                    b.edge("a", S, GlaDirection::Forward, "b").unwrap();
                }
                1 => {
                    b.walk("c", R, GlaDirection::Forward, "b", bounds(1, 3))
                        .unwrap();
                }
                2 => {
                    b.walk("c", R, GlaDirection::Forward, "d", bounds(0, 2))
                        .unwrap();
                }
                _ => {
                    b.edge("a", S, GlaDirection::Forward, "d").unwrap();
                }
            }
        }
        let query = b
            .prepare_bindings(&["a", "b", "c", "d"], 0, None)
            .unwrap()
            .with_duplicates();
        assert!(matches!(
...
fn optional_walk_endpoints_do_not_rebind_null_and_probes_resolve_at_complete_witnesses() {
    ...
    child
        .walk("anchor", R, GlaDirection::Forward, "b", bounds(1, 2))
        .unwrap();
    let mut zero = builder(&["b", "c"]);
    zero.walk("b", R, GlaDirection::Forward, "c", bounds(0, 0))
        .unwrap();
    let columns = [
        GraphColumn::vertex("anchor", "anchor"),
    ...
    for (last, expected) in [
        ...
        (
            GraphMatchClause::not_exists(&zero),
            vec![vec![Some(VId(4)), None]],
        ),
    ] {
        let query = root
            .prepare_values_with_clauses(
                &[GraphMatchClause::optional(&child), last],
                &columns,
                0,
                None,
            )
    ...
    backwards
        .walk("b", R, GlaDirection::Forward, "anchor", bounds(0, 2))
        .unwrap();
    let query = root
        .prepare_values_with_clauses(&[GraphMatchClause::optional(&backwards)], &columns, 0, None)
        .unwrap()
        .with_duplicates();
    assert_eq!(
        nullable(&run(&query)),
...
fn endpoint_filters_do_not_remove_transit_vertices_and_late_errors_are_not_hidden_by_limit() {
    ...
    let mut b = builder(&["a", "b"]);
    b.walk("a", R, GlaDirection::Forward, "b", bounds(1, 3))
        .unwrap();
    ...
    let actual = query
        ...
        .execute(vertices, edges, |vid, predicates| {
            ...
            Ok::<_, ()>(
                predicates
                    .iter()
                    .all(|p| p.matches(&labels, &[(key, CanonicalScalar::Int(vid.0 as i64))])),
            )
        })
        .unwrap();
    assert_eq!(
        tuples(&actual),
        vec![vec![VId(1), VId(3)], vec![VId(1), VId(4)]]
    );
    let mut b = builder(&["a", "b"]);
    b.walk("a", R, GlaDirection::Forward, "b", bounds(1, 2))
        .unwrap();
    let value = CanonicalScalar::Int(1);
    for count in [Some(0), Some(1), None] {
        let query = b
            .prepare_values(&[GraphColumn::property("value", "b", key)], 0, count)
            .unwrap();
        let mut released = 0;
        let actual = query.plan().execute_with_properties_control(
            ...
            |event| {
                if event == fgdb_gql::GlaExecutionEvent::ResultRow {
                    released += 1;
                }
                Ok(())
            },
...
fn path_work_scratch_and_result_limits_are_exact_and_every_checkpoint_can_interrupt() {
    ...
    let mut b = builder(&["a", "b"]);
    b.walk("a", R, GlaDirection::Forward, "b", bounds(0, 3))
        .unwrap();
    let query = b
        .prepare_bindings(&["a", "b"], 1, Some(2))
        .unwrap()
        .with_duplicates();
...
fn streaming_aggregates_count_walk_occurrences_not_just_reachable_endpoints() {
    ...
    let aggregate = PreparedGraphAggregate::prepare(
        ...
        &[0],
        &[
            GraphAggregate::count_rows("walks"),
            GraphAggregate::count_distinct("reachable", 1),
        ],
        0,
        None,
    )
...
fn walk_definitions_are_immutable_bounded_and_transcript_sensitive() {
    let mut b = builder(&["a", "b"]);
    b.walk("a", R, GlaDirection::Forward, "b", bounds(0, 2))
        .unwrap();
    let original = b.prepare_bindings(&["a", "b"], 0, None).unwrap();
    let frozen = original.canonical_bytes();
    assert_eq!(
        b.walk("a", R, GlaDirection::Forward, "unknown", bounds(0, 1))
            .unwrap_err(),
    ...
    for (minimum, maximum) in [(0, 0), (0, 2), (1, 2), (1, 3)] {
        for direction in [
            ...
            other
                .walk("x", R, direction, "y", bounds(minimum, maximum))
                .unwrap();
    ...
    let rows = maximum
        ...
        .unwrap()
        .plan()
        .execute([VId(7)], [(VId(7), R, VId(7))], |_, _| Ok::<_, ()>(true))
        .unwrap();
