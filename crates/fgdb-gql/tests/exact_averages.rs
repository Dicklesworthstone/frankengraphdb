...
fn all_and_distinct_averages_match_owned_occurrence_oracles_and_rank_exactly() {
    let query = prepare(
        "MATCH (a)-[:R]->(b) RETURN a,SUM(b.p) AS s,SUM(DISTINCT b.p) AS d,AVG(b.p) AS m,AVG(DISTINCT b.p) AS u,COUNT(b.p) AS n GROUP BY a ORDER BY m DESC NULLS LAST,a",
    );
    let universe = [
        (VId(1), R, VId(10)),
        (VId(1), R, VId(10)),
    ...
    for mask in 0..32 {
        ...
        for code in 0..27 {
            ...
            let values: Vec<_> = (0..3)
                .map(|_| {
                    let value = [None, Some(-3), Some(7)][encoded % 3];
                    encoded /= 3;
                    value
                })
                .collect();
            ...
            for (owner, _, destination) in &edges {
                ...
                if let Some(value) = values[(destination.0 - 10) as usize] {
                    group.push(value);
                }
            }
            let mut expected: Vec<_> = groups
                ...
                .map(|(key, values)| {
                    ...
                    (
                        key,
                        (!values.is_empty())
                            .then(|| values.iter().map(|v| i128::from(*v)).sum::<i128>()),
                        (!distinct.is_empty())
                            .then(|| distinct.iter().map(|v| i128::from(*v)).sum::<i128>()),
                        oracle_mean(&values),
                        oracle_mean(&distinct),
                        values.len() as u64,
                    )
                })
                .collect();
            ...
            let actual: Vec<_> = result
                ...
                .map(|row| {
                    (
                        ...
                        row.get(0).unwrap().as_integer(),
                        row.get(1).unwrap().as_integer(),
                        ratio(row.get(2).unwrap().as_average()),
                        ratio(row.get(3).unwrap().as_average()),
...
fn typed_and_text_forms_share_columns_tags_having_and_ordering() {
    ...
    let expected = PreparedGraphAggregate::prepare(
        ...
        &[
            ...
            GraphAggregate::sum_int_distinct("total", 1),
        ],
        0,
        Some(3),
    )
    ...
    assert_eq!(
        all.canonical_bytes(),
        prepare("MATCH (a) RETURN AVG_INT(ALL a.p) AS mean").canonical_bytes()
    );
}
...
fn sub_float_differences_survive_ordering_flat_and_compound_having() {
    ...
    assert_eq!(
        result
            ...
            .collect::<Vec<_>>(),
        vec![VId(2), VId(1), VId(3)]
    );
    assert_eq!(
        ratio(result[1].get(0).unwrap().as_average()),
        Some((2 * i128::from(n) - 1, 2))
    );
    assert_eq!(ratio(result[2].get(0).unwrap().as_average()), Some((-1, 2)));
    let template = PreparedGraphAggregateText::prepare(
        &format!("{head} HAVING mean > $lo AND mean < $hi"),
        symbols,
    )
    .unwrap();
    ...
    let compound = prepare(&format!(
        "{head} HAVING NOT (mean >= total) OR mean < 0 ORDER BY mean DESC"
    ));
...
fn empty_null_and_noninteger_domains_are_explicit_even_with_zero_output() {
    let query =
        prepare("MATCH (n) RETURN AVG(n.p) AS a,AVG(DISTINCT n.p) AS d,SUM(DISTINCT n.p) AS s");
    let empty = query
        .execute_governed(
            0,
            [],
    ...
    let missing = query
        .execute_governed(
            ...
            [],
            |_, _| Ok::<_, ()>(true),
            |vid, _| Ok((vid == VId(1)).then_some(&null)),
            wide(),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    ...
    assert!(matches!(
        vertex_average.execute_governed(
            1,
            [],
            [(VId(1), R, VId(2))],
            |_, _| Ok::<_, ()>(true),
        ...
        Err(GqlQueryError::Source(
            GraphAggregateError::NonIntegerAverage { .. }
        ))
    ));
    for text in [
        "MATCH (a) RETURN AVG(*)",
        "MATCH (a) RETURN AVG(DISTINCT *)",
        "MATCH (a) RETURN AVG(ALL DISTINCT a.p)",
        "MATCH (a) RETURN AVG(SUM(a.p))",
...
fn exact_numeric_updates_share_all_limits_and_every_interruption_checkpoint() {
    let query = prepare(
        "MATCH (a)-[:R]->(b) RETURN a,AVG(b.p) AS m,SUM(DISTINCT b.p) AS s,AVG(DISTINCT b.p) AS d GROUP BY a HAVING m < s OR d = m ORDER BY d DESC LIMIT 1",
    );
    let scalars = [
