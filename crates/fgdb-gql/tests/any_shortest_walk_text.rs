...
fn native_any_directions_and_rebinding_match_the_typed_builder_exactly() {
    for (atom, direction) in [
        ...
        assert_eq!(
            prepared.bind_parameters(&args).unwrap().canonical_bytes(),
...
fn native_any_preserves_zero_hops_lower_bounds_and_separate_return_quantifiers() {
    ...
    for (selector, interval, tail, expected) in [
        ...
        let query = prepare(&format!(
            "MATCH {selector} (a)-[:R*{interval}]->(b) RETURN {tail}"
        ));
        let rows = query
            .plan()
...
fn scoped_any_aggregation_counts_pairs_and_existence_never_multiplies_outer_rows() {
    ...
    for (quantifier, expected) in [("EXISTS", VId(1)), ("NOT EXISTS", VId(2))] {
        let query = prepare(&format!(
            "MATCH (a) WHERE {quantifier} {{ MATCH ANY SHORTEST WALK (a)-[:R*1..3]->(b) }} RETURN a"
        ));
        let rows = query
            .plan()
            .execute_governed_with_properties(
    ...
    let mixed = prepare(
        "MATCH ALL SHORTEST WALK (s)-[:R*1..3]->(a) OPTIONAL MATCH ANY SHORTEST WALK (a)-[:S*0..2]->(b) RETURN s,a,b",
    );
    assert_eq!(
        modes(&mixed),
        vec![GraphWalkSearch::AllShortest, GraphWalkSearch::AnyShortest]
    );
}
...
fn any_scripts_and_parameter_batches_lower_all_write_selections_without_reparsing() {
    ...
    for group in batch.program().statements().as_chunks::<3>().0 {
        let [
            ...
            panic!("wrong write dispatch")
        };
        for selection in [
            insert.selection().unwrap(),
            update.selection(),
            delete.selection(),
        ] {
...
fn any_binding_errors_preserve_utf8_offsets_and_reject_unrecognized_arguments() {
    let text =
        "\u{2003}MATCH ANY SHORTEST WALK (a)-[:R*1..3]->(b) WHERE a.p=$key RETURN b LIMIT $count";
    ...
    let extra = GqlParameters::new()
        ...
        .unwrap();
    assert_eq!(
        prepared.bind_parameters(&extra).unwrap_err().kind,
