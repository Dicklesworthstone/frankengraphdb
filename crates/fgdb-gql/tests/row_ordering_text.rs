...
fn expected(
    ...
    let mut result: Vec<_> = input
        .iter()
        .map(|vid| (*vid, values[vid.0 as usize]))
        .collect();
    result.sort_by(|a, b| {
                ...
                for offset in 0..=3 {
                    for count in 0..=3 {
                        let args = GqlParameters::new()
                            .with_uint64("off", offset as u64)
                            .unwrap()
                            .with_uint64("take", count as u64)
                            .unwrap();
                        ...
                        for mut code in 0..81 {
                            ...
                            assert_eq!(
                                plain(&actual),
                                expected(
                                    &input, &values, distinct, descending, first, offset, count
                                )
                            );
                        }
                    }
...
fn text_and_typed_ordering_share_transcripts_and_immutable_rebinding() {
    ...
    assert_eq!(
        calls.get(),
        1,
        "ORDER BY never re-resolves the catalog during rebinding"
    );
    assert!(
        template
            .bind_parameters(
    ...
    let renamed = text(
        "MATCH (x) RETURN ALL x AS a,x.score AS b ORDER BY x.score DESC NULLS FIRST,x ASC SKIP 1 LIMIT 2",
    );
    assert_eq!(renamed.canonical_bytes(), frozen);
    assert_eq!(
        text("MATCH (x) RETURN ALL x AS owner,x.score AS rank SKIP 1 LIMIT 2"),
        typed(1, Some(2))
    );
}
...
fn aliases_star_and_bad_order_references_are_resolved_before_catalog_access() {
    ...
    for tail in [
        ...
        "ORDER rank",
        "ORDER BY",
        "ORDER BY rank NULLS MIDDLE",
        "ORDER BY rank ASC DESC",
        "ORDER BY $column",
        "ORDER BY rank LIMIT 1 SKIP 0",
        "ORDER BY rank, x.score",
    ] {
        let calls = Cell::new(0);
...
fn optional_nulls_and_parallel_rows_are_ranked_after_scope_completion() {
    let query = text(
        "MATCH (a) OPTIONAL MATCH (a)-[:R]->(b) RETURN a,b.score AS rank ORDER BY rank DESC NULLS FIRST",
    );
    let score = CanonicalScalar::Int(5);
    let edges = [
        (VId(0), RelationId(1), VId(10)),
        (VId(0), RelationId(1), VId(10)),
    ...
    assert_eq!(
        reads, 3,
        "null-extended bindings never reach the property source"
    );
}
...
fn ranked_pages_do_not_hide_late_source_errors_or_copy_rejected_payloads() {
    ...
    let query =
        text("MATCH (x) RETURN x.score AS rank,x.payload AS data ORDER BY rank DESC LIMIT 1");
    ...
    assert!(
        result.evaluator.scratch_entries < 100,
        "rejected 4KiB payloads must not be cloned"
    );
    for count in [0, 1] {
        let query = text(&format!(
            "MATCH (x) RETURN x.score AS rank ORDER BY rank DESC LIMIT {count}"
        ));
        let mut reads = 0;
        let result = query.plan().execute_with_properties_control(
            (0..3).map(VId),
            [],
            |_, _| Ok(true),
            |vid, _| {
...
fn ordered_text_has_exact_limits_and_propagates_each_interruption() {
    ...
    let query =
        text("MATCH (x) RETURN x AS owner,x.score AS rank ORDER BY rank DESC SKIP 1 LIMIT 2");
...
fn scalar_parameters_and_aggregate_ordering_keep_their_existing_binding_contracts() {
    ...
    let arguments = GqlParameters::new()
        .with_text("wanted", "ORDER BY missing; 'literal'")
        .unwrap()
        .with_uint64("take", 3)
        .unwrap();
    ...
    let aggregate = PreparedGraphAggregateText::prepare(
        "MATCH (x) RETURN x,COUNT(*) AS n GROUP BY x ORDER BY n DESC,x ASC LIMIT 1",
        symbols,
    )
    .unwrap()
    .bind_parameters(&GqlParameters::new())
...
fn all_65_columns_and_utf8_prefixes_preserve_bounded_preparation() {
    let returned = (0..65)
        .map(|at| format!("x AS c{at}"))
        .collect::<Vec<_>>()
        .join(",");
    let order = (0..65)
        .rev()
        .map(|at| format!("c{at} DESC"))
        .collect::<Vec<_>>()
        .join(",");
    let query = text(&format!(
        "MATCH (x) RETURN DISTINCT {returned} ORDER BY {order}"
    ));
    ...
    assert!(
        rows[0]
            .values()
            .iter()
            .all(|value| value.as_vertex() == Some(VId(u128::MAX)))
    );
