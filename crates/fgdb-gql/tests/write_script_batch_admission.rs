...
use fgdb_gql::{
    ...
    GraphWriteProgramPolicy, GraphWriteScriptBatchError, GraphWriteStatement,
    GraphWriteStepStats, MAX_GRAPH_MUTATION_STATEMENTS, PreparedGraphWriteProgram,
...
fn arguments(key: i64) -> GqlParameters {
    GqlParameters::new().with_int64("key", key).unwrap()
        .with_int64("value", key + 10).unwrap()
}
...
fn mutation_batch(records: usize) -> fgdb_gql::BoundGraphWriteScriptBatch {
    ...
    script.bind_parameter_sets_with_limit(&args, records).unwrap()
}
fn step_stats() -> GraphWriteStepStats {
    GraphWriteStepStats::Mutation(GraphMutationStats {
        selection: GqlExecutionStats { snapshot_records: 2, result_rows: 1 },
...
fn explicit_large_batch_preserves_record_order_spans_and_frozen_bindings() {
    ...
    for (record, values) in args.iter().enumerate() {
        ...
        assert_eq!(&batch.program().statements()[range.clone()], single.statements());
        for index in range {
            let location = batch.location(index).unwrap();
            assert_eq!((location.argument_set, location.statement), (record, index % 2));
    ...
    assert_eq!(calls.get(), resolved, "batch binding must not call the catalog");
...
fn existing_program_and_batch_defaults_stay_at_sixty_four() {
    ...
    assert!(matches!(script.bind_parameter_sets(&args),
    ...
    assert!(matches!(PreparedGraphWriteProgram::prepare(batch.program().statements().to_vec()),
        Err(GraphMutationProgramBuildError::TooManyStatements { limit: 64, observed: 65 })));
    let default = script.bind_parameter_sets(&args[..64]).unwrap();
    let explicit = script.bind_parameter_sets_with_limit(&args[..64], 65).unwrap();
    assert_eq!(default.program(), explicit.program());
    assert_eq!(default.program().canonical_bytes(), explicit.program().canonical_bytes());
...
fn expanded_count_admission_precedes_binding_and_cannot_bypass_the_hard_ceiling() {
    let script = insertion();
    assert!(matches!(script.bind_parameter_sets_with_limit(&[], 0),
        Err(GraphWriteScriptBatchError::Empty)));
    assert!(matches!(script.bind_parameter_sets_with_limit(&[GqlParameters::new()], 0),
        Err(GraphWriteScriptBatchError::TooManyStatements { limit: 0, observed: 1 })));
    let invalid = vec![GqlParameters::new(); 65];
    assert!(matches!(script.bind_parameter_sets_with_limit(&invalid, 64),
        Err(GraphWriteScriptBatchError::TooManyStatements { limit: 64, observed: 65 })));
    assert!(matches!(script.bind_parameter_sets_with_limit(&invalid, 65),
        Err(GraphWriteScriptBatchError::Arguments { argument_set: 0, .. })));
    ...
    assert!(matches!(wide_script.bind_parameter_sets_with_limit(&invalid, usize::MAX),
        Err(GraphWriteScriptBatchError::TooManyStatements { limit: 65_536, observed: 65_600 })));
...
fn late_record_binding_failure_keeps_original_utf8_and_statement_coordinates() {
    ...
    let error = script.bind_parameter_sets_with_limit(&args, 162).unwrap_err();
    let GraphWriteScriptBatchError::Arguments { argument_set, source } = error else {
    ...
    assert_eq!(script.bind_parameter_sets_with_limit(&args, 162).unwrap()
        .program().statements().len(), 162);
}
...
fn enlarged_program_uses_one_cumulative_meter_past_record_sixty_four() {
    ...
    let stats = batch.program().execute_governed::<(), (), ()>(
        ...
        |index, input, remaining| {
            ...
            assert_eq!(remaining.mutations.query, GqlQueryPolicy::new(2 * left, left, 4 * left, 2 * left));
            Ok(step_stats())
        },
        || Ok(()),
    ).unwrap();
    assert_eq!(stats.completed_statements, records);
    assert_eq!(stats.selection, GqlExecutionStats { snapshot_records: 258, result_rows: 129 });
    assert_eq!(stats.evaluator, GlaExecutionStats { work_units: 517, scratch_entries: 258 });
...
fn enlarged_program_refuses_each_cumulative_dimension_without_resetting_at_record_boundaries() {
    ...
    for (policy, dimension, at, expected_limit, expected_observed) in [
        (GraphWriteProgramPolicy::new(GqlQueryPolicy::new(257, 129, 517, 258), 129, 0, 0),
            GraphMutationProgramDimension::SnapshotRecords, 128, 257, 258),
        (GraphWriteProgramPolicy::new(GqlQueryPolicy::new(258, 128, 517, 258), 129, 0, 0),
            GraphMutationProgramDimension::SelectedRows, 128, 128, 129),
        (GraphWriteProgramPolicy::new(GqlQueryPolicy::new(258, 129, 516, 258), 129, 0, 0),
            GraphMutationProgramDimension::WorkUnits, 129, 516, 517),
        (GraphWriteProgramPolicy::new(GqlQueryPolicy::new(258, 129, 517, 257), 129, 0, 0),
            GraphMutationProgramDimension::ScratchEntries, 128, 257, 258),
        (GraphWriteProgramPolicy::new(GqlQueryPolicy::new(258, 129, 517, 258), 128, 0, 0),
            GraphMutationProgramDimension::Effects, 128, 128, 129),
    ] {
        let error = batch.program().execute_governed::<(), (), ()>(policy,
            |_, _, _| Ok(step_stats()), || Ok(())).unwrap_err();
        assert!(matches!(error, GraphWriteProgramError::Program(GraphMutationProgramError::Budget {
...
fn every_large_batch_statement_and_final_boundary_is_interruptible() {
    ...
    for stop in 1..=130 {
        ...
        let mut staged = 0;
        let error = batch.program().execute_governed::<(), (), usize>(exact(129),
            |index, _, _| {
            ...
            || {
                checkpoints += 1;
                if checkpoints == stop { Err(stop) } else { Ok(()) }
            },
        ).unwrap_err();
        assert!(matches!(error, GraphWriteProgramError::Program(GraphMutationProgramError::Interrupted {
