...
use fgdb_gql::algebra::{
    ...
    GraphColumn, GraphPatternBuilder, GraphValueRow, IntegerComparison as Cmp, PreparedGraphPattern,
};
use fgdb_gql::stream::{
...
fn prepare(
    ...
    builder.prepare_values(&[GraphColumn::vertex("node", "n")], offset, count).unwrap()
}
            ...
            for offset in [0, 1, 10, u64::MAX] {
                for count in [None, Some(0), Some(1), Some(10)] {
                    for all in [false, true] {
                        ...
                        assert_eq!(cursor.by_ref().map(|row| row.map(identity))
...
fn boolean_interruptions_are_terminal_at_every_checkpoint() {
    ...
    let expected = VertexScanCursor::new(Source::new(&rows), scan.clone(), wide(), || {
        ...
        Ok::<_, usize>(())
    }).map(|row| row.map(identity)).collect::<Result<Vec<_>, _>>().unwrap();
...
fn boolean_work_allowance_is_cumulative_and_source_errors_are_not_nulls() {
    ...
    let expected = cursor.by_ref().map(|row| row.map(identity))
    ...
    for allowed in [work, work - 1] {
        ...
        let result = cursor.by_ref().map(|row| row.map(identity)).collect::<Result<Vec<_>, _>>();
        if allowed == work {
...
fn graph_expansion_with_a_boolean_filter_still_refuses_stream_compilation() {
    ...
    builder.filter_boolean(&condition).unwrap();
    let query = builder.prepare_values(&[GraphColumn::vertex("node", "n")], 0, None).unwrap();
...
fn comparison_query(
    ...
    let mut builder = GraphPatternBuilder::new();
    builder.vertex("n").unwrap();
    builder.compare_properties("n", P, comparison, "n", right_key).unwrap();
    builder.prepare_values(&[GraphColumn::vertex("node", "n")], offset, count).unwrap()
}
...
fn local_property_pairs_match_an_independent_oracle_for_all_comparisons() {
    let rows = rows();
    for comparison in [Cmp::Equal, Cmp::NotEqual, Cmp::Less, Cmp::LessOrEqual,
        Cmp::Greater, Cmp::GreaterOrEqual] {
            ...
            for offset in [0, 1, u64::MAX] {
                for count in [None, Some(0), Some(2)] {
                    for all in [false, true] {
                        ...
                        let actual = VertexScanCursor::new(Source::new(&rows), scan, wide(),
                            || Ok::<_, ()>(())).map(|row| row.map(identity))
                            .collect::<Result<Vec<_>, _>>().unwrap();
                        assert_eq!(actual, expected);
                    }
                }
...
fn property_pair_payload_work_is_charged_before_delivery_and_limit_drops_the_source() {
    let row = Record {
        ...
        visible: true,
        properties: vec![
            (P, CanonicalScalar::ucs_basic_text(&"a".repeat(4096)).unwrap()),
            (Q, CanonicalScalar::ucs_basic_text(&"b".repeat(8192)).unwrap()),
        ],
    };
    let rows = [row];
    ...
    for work in [measured.work_units, measured.work_units - 1] {
        ...
        let dropped = Rc::clone(&input.dropped);
        let mut cursor = VertexScanCursor::new(input, scan.clone(),
            GqlQueryPolicy::new(1, 1, work, measured.scratch_entries), || Ok::<_, ()>(()));
...
fn guarded_scalar() -> GraphIntegerExpression {
    ...
    GraphIntegerExpression::prepare_scalar(&[
        I::Column(0), I::Literal(Some(0)), I::Compare(Cmp::Greater),
        I::Truth(Some(true)),
        I::Literal(Some(1)), I::Literal(Some(0)), I::Binary(GraphIntegerBinary::Divide),
        I::Literal(Some(0)), I::Compare(Cmp::Equal),
        I::Case,
    ]).unwrap()
}
...
fn scalar_filters_preserve_lazy_case_and_exact_cumulative_scratch_limits() {
    ...
    let columns = [Arg::Property { variable: "n", key: P }];
    let condition = GraphBooleanExpression::prepare(&[Op::Expression {
        expression: &scalar, columns: &columns,
    }]).unwrap();
    let query = prepare(&condition, 0, None);
    let expected: Vec<_> = rows.iter().filter(|row| row.visible
...
fn a_true_or_operand_does_not_bypass_scalar_work_or_cancellation() {
    ...
    assert_eq!(baseline, vec![rows.iter().find(|row| row.visible).unwrap().vid]);
    for stop in 1..=calls.get() {
        let input = Source::new(&rows);
        let dropped = Rc::clone(&input.dropped);
        let at = Cell::new(0);
        ...
        assert!(matches!(cursor.next(), Some(Err(GqlQueryError::Interrupted(observed)))
            if observed == stop));
        assert_eq!(cursor.row_stats().result_rows, 0);
        assert_eq!(cursor.state(), VertexScanState::Failed);
        assert!(dropped.get());
    ...
    let mut cursor = VertexScanCursor::new(Source::new(&rows), scan,
        GqlQueryPolicy::new(u64::MAX, u64::MAX, u64::MAX, 0), || Ok::<_, ()>(()));
...
fn property_sensitive_filters_keep_the_legacy_projection_refusal() {
    ...
    assert!(matches!(builder.prepare("n", 0, None),
