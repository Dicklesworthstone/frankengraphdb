...
use fgdb_gql::{
    ...
    GraphMutationStats, GraphSymbol, GraphSymbolKind, GraphWriteIdentityRequest,
    GraphWriteProgramError, GraphWriteProgramPolicy, GraphWriteProgramStats, GraphWriteStatement,
    GraphWriteStepError, GraphWriteStepStats, MAX_GRAPH_MUTATION_STATEMENTS,
    PreparedGraphInsertText, PreparedGraphMutationProgram, PreparedGraphMutationText,
...
fn insertion() -> GraphWriteStatement {
    PreparedGraphInsertText::prepare("CREATE (x {p:7})-[:R]->(x)", R, symbols)
        .unwrap()
...
fn mutation() -> GraphWriteStatement {
    PreparedGraphMutationText::prepare("MATCH (n) SET n.p=7", R, symbols)
        .unwrap()
        .bind_parameters(&GqlParameters::new())
        .unwrap()
        .into()
...
fn source(
    ...
    pattern.plan().execute_governed_with_properties(
        ...
        [],
        |_, _| Ok(true),
        |_, _| Ok::<Option<&CanonicalScalar>, _>(None),
        allowance,
        || Ok(()),
    )
}
fn step(
    statement: usize,
    input: &GraphWriteStatement,
    remaining: GraphWriteProgramPolicy,
    requests: &RefCell<Vec<GraphWriteIdentityRequest>>,
) -> Result<GraphWriteStepStats, StepError> {
    match input {
        GraphWriteStatement::Mutation(input) => input
            .execute_governed(remaining.mutations, source, || Ok(()))
            .map(|batch| GraphWriteStepStats::Mutation(batch.stats()))
            .map_err(GraphWriteStepError::Mutation),
        GraphWriteStatement::Insert(input) => input
            .execute_governed(
                ...
                |request| {
                    requests
                        .borrow_mut()
                        .push(GraphWriteIdentityRequest { statement, request });
                    Ok(match request {
                        GraphInsertRequest::Vertex { row, vertex } => ElementId::Vertex(VId(1000
                            + statement as u128 * 100
                            + row as u128 * 10
                            + vertex as u128)),
                        GraphInsertRequest::Edge { row, edge } => ElementId::Edge(EId(1000
                            + statement as u128 * 100
                            + row as u128 * 10
                            + edge as u128)),
                    })
            ...
            .map(|batch| GraphWriteStepStats::Insert(batch.stats()))
            .map_err(GraphWriteStepError::Insert),
        _ => panic!("this fixture contains only mutation and insertion statements"),
    }
}
fn run(
    input: &PreparedGraphWriteProgram,
    allowance: GraphWriteProgramPolicy,
) -> Result<GraphWriteProgramStats, Error> {
    let requests = RefCell::new(Vec::new());
    input.execute_governed(
        allowance,
        |at, input, remaining| step(at, input, remaining, &requests),
        || Ok(()),
    )
}
...
fn source_order_scoped_identity_requests_and_cumulative_counts_are_explicit() {
    ...
    let stats: Result<_, Error> = program.execute_governed(
        policy(),
        |at, input, remaining| {
        ...
        *requests.borrow(),
        vec![
            GraphWriteIdentityRequest {
                statement: 0,
                request: GraphInsertRequest::Vertex { row: 0, vertex: 0 }
            },
            GraphWriteIdentityRequest {
                statement: 0,
                request: GraphInsertRequest::Edge { row: 0, edge: 0 }
            },
            GraphWriteIdentityRequest {
                statement: 2,
                request: GraphInsertRequest::Vertex { row: 0, vertex: 0 }
    ...
    let reordered =
        PreparedGraphWriteProgram::prepare(vec![mutation(), insertion(), insertion()]).unwrap();
    assert_ne!(reordered.canonical_bytes(), frozen);
}

#[test]
fn common_accounting_is_identical_to_the_existing_mutation_program() {
    let GraphWriteStatement::Mutation(input) = mutation() else {
        unreachable!()
    };
    ...
    let expected = old
        .execute_governed(
            ...
            |input, allowance| {
                input
                    .execute_governed(allowance, source, || Ok(()))
                    .map(|batch| batch.stats())
            },
            || Ok(()),
        )
        .unwrap();
...
fn exact_seven_dimension_limits_and_final_acceptance_refusal_are_cumulative() {
    ...
    let exact = GraphWriteProgramPolicy::new(
        GqlQueryPolicy::new(
            stats.selection.snapshot_records,
            stats.selection.result_rows,
    ...
    for dimension in 0..7 {
        ...
        caps[dimension] -= 1;
        let cap = GraphWriteProgramPolicy::new(
    ...
    let cap = GraphWriteProgramPolicy::new(
        GqlQueryPolicy::new(100, 100, stats.evaluator.work_units - 1, 1_000_000),
        100,
        100,
        100,
    );
    assert!(matches!(
        ...
        Err(GraphWriteProgramError::Program(
            GraphMutationProgramError::Budget {
                statement: 3,
                dimension: GraphMutationProgramDimension::WorkUnits,
                ..
            }
        ))
    ));
    ...
    let result: Result<_, Error> = program.execute_governed(
        limited,
        |at, input, allowance| step(at, input, allowance, &requests),
        || Ok(()),
    );
    assert!(matches!(
        result,
    ...
    assert_eq!(
        requests.borrow().len(),
        2,
        "second CREATE refuses before allocation"
    );
}
...
fn interruption_including_final_boundary_never_runs_a_later_statement() {
    ...
    for stop in 1..=4 {
        ...
        assert!(
            matches!(result, Err(GraphWriteProgramError::Program(GraphMutationProgramError::Interrupted {
    ...
    for corruption in 0..4 {
        ...
        let result: Result<_, Error> = program.execute_governed(
            ...
            |at, input, allowance| {
                ...
                let GraphWriteStepStats::Insert(mut stats) = step(at, input, allowance, &requests)?
                else {
                    unreachable!()
                };
                match corruption {
                    ...
                    2 => {
                        stats.created_vertices = 0;
                        stats.created_edges = 0;
                        stats.selection.result_rows = 0;
                    }
                    _ => {
                        return Ok(GraphWriteStepStats::Mutation(GraphMutationStats {
...
fn definitions_are_bounded_and_mixed_coordinates_refuse_before_execution() {
    ...
    assert!(matches!(
        PreparedGraphWriteProgram::prepare(vec![insertion(); MAX_GRAPH_MUTATION_STATEMENTS + 1]),
        Err(GraphMutationProgramBuildError::TooManyStatements { .. })
    ));
    let foreign = PreparedGraphInsertText::prepare("CREATE (x)", RelationId(2), symbols)
        .unwrap()
        .bind_parameters(&GqlParameters::new())
        .unwrap();
    assert!(matches!(
