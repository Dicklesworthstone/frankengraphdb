...
fn native_selection_and_branches_share_one_frozen_parameter_contract() {
    ...
    assert_eq!(bound.on_create()[0].value.value(), &CanonicalScalar::Int(200));
    assert_eq!(bound.on_match()[0].value.value(), &CanonicalScalar::Int(100));
...
fn malformed_branch_direction_and_target_never_reach_catalog_resolution() {
    for text in [
        ...
        let calls = Cell::new(0);
        assert!(PreparedGraphEdgeUpsertText::prepare(text, R, |kind, name| {
            calls.set(calls.get() + 1);
            symbols(kind, name)
        }).is_err(), "{text}");
...
fn duplicate_fields_fail_in_the_selected_typed_branch_definition() {
    let template = PreparedGraphEdgeUpsertText::prepare(
        "MATCH (a),(b) MERGE (a)-[e:R]->(b) ON CREATE SET e.w=1,e.w=2", R, symbols,
    ).unwrap();
...
fn missing_and_wrong_branch_arguments_keep_original_byte_offsets() {
    ...
    let wrong = template.bind_parameters(&GqlParameters::new().with_uint64("value", 1).unwrap()).unwrap_err();
    assert_eq!(wrong.offset, text.find('$').unwrap());
    assert!(matches!(wrong.kind,
        GraphEdgeUpsertTextErrorKind::Query(GraphPatternTextErrorKind::ParameterTypeMismatch { .. })));
}
...
fn quoted_parameter_payload_is_a_value_not_additional_write_syntax() {
    ...
    let template = PreparedGraphEdgeUpsertText::prepare_with_parameter_types(
        "MATCH (a),(b) MERGE (a)-[e:R]->(b) ON CREATE SET e.w=$payload", R,
        &[("payload", GqlParameterType::Scalar(kind))], symbols,
    ).unwrap();
    let bound = template.bind_parameters(&GqlParameters::new().with_text("payload", payload).unwrap()).unwrap();
    assert_eq!(bound.on_create().len(), 1);
    assert_eq!(bound.on_create()[0].value.value(), &CanonicalScalar::ucs_basic_text(payload).unwrap());
