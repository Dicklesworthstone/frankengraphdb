...
fn scalar_tuple_and_value_pages_match_independent_sorted_multigraph_projections() {
    ...
    let universe = [
        (VId(0), RelationId(1), high),
        (VId(0), RelationId(1), high),
        (high, RelationId(1), VId(0)),
        (VId(2), RelationId(1), VId(0)),
        (high, RelationId(1), high),
        (VId(2), RelationId(1), high),
    ];
    ...
    for mask in 0..64 {
        ...
        for distinct in [false, true] {
            ...
            let mut tuple_expected: Vec<_> =
                edges.iter().map(|edge| vec![edge.2, edge.0]).collect();
            scalar_expected.sort_unstable();
            tuple_expected.sort();
            if distinct {
                scalar_expected.dedup();
                tuple_expected.dedup();
            }
            for offset in 0..3_u64 {
                for count in 0..3_u64 {
                    let scalar = builder.prepare("b", offset, Some(count)).unwrap();
                    let scalar = if distinct {
                        scalar
                    } else {
                        scalar.with_duplicates()
                    };
                    let tuple = builder
                        .prepare_bindings(&["b", "a"], offset, Some(count))
                        .unwrap();
                    let tuple = if distinct {
                        tuple
                    } else {
                        tuple.with_duplicates()
                    };
                    let value = query(&format!(
                        "MATCH (a)-[:R]->(b) RETURN {} b,a SKIP {offset} LIMIT {count}",
                    ...
                    let rows = tuple
                        ...
                        .execute([], edges.iter().copied(), |_, _| Ok::<_, ()>(true))
                        .unwrap();
                    let expected: Vec<_> = tuple_expected
                        .iter()
                        .skip(offset as usize)
                        .take(count as usize)
                        .cloned()
...
fn small_bag_page_avoids_thousands_of_payload_copies_but_reads_every_candidate() {
    ...
    let result = pattern
        ...
        .execute_governed_with_properties(
            ...
            GqlQueryPolicy::new(4096, 3, 1_000_000, 5 * (1 + 1 + 64)),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    ...
    assert!(
        result
            .value
            .iter()
            .all(|row| row.get(0).unwrap().as_scalar() == Some(&payload))
    );
    assert_eq!(pattern.canonical_bytes(), frozen);
    let all = query("MATCH (n) RETURN n.p AS payload");
    assert!(matches!(
        all.plan().execute_governed_with_properties(
            4096,
            (0..4096).map(VId),
            [],
            |_, _| Ok::<_, ()>(true),
            |_, _| Ok(Some(&payload)),
            GqlQueryPolicy::new(4096, 4096, 1_000_000, 330),
            || Ok::<_, ()>(())
        ),
...
fn zero_and_full_pages_never_hide_late_predicate_or_projected_property_failures() {
    for count in [0, 1] {
        ...
        assert!(matches!(
            result,
            Err(GqlQueryError::Source("late projection"))
        ));
        assert_eq!(reads, 2);
        ...
        assert!(matches!(
            result,
            Err(GqlQueryError::Source("late predicate"))
        ));
    }
}
