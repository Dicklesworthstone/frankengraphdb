...
fn forest_counts_and_projected_support_match_complete_assignment_enumeration() {
    ...
    let cases: [Case<'_>; 6] = [
        ...
        (
            ...
            &[(0, 1, 0, 1), (1, 2, 0, 2), (1, 2, 0, 3), (0, 1, 0, 4)],
            5,
            "b",
            1,
        ),
        ...
        (
            ...
            &[(0, 1, 0, 1), (1, 2, 0, 0), (1, 1, 0, 2), (2, 2, 0, 3)],
            4,
            "b",
            1,
        ),
    ...
    for mask in 0..256_usize {
        ...
        for ((_, atoms, variables, _, selected), query) in cases.iter().zip(&prepared) {
            assert_eq!(
                ...
                oracle(&edges, atoms, *variables, *selected),
...
fn a_billion_distinct_hidden_assignments_do_not_require_a_billion_visits() {
    ...
    let query = prepare(&format!(
        "{} RETURN COUNT(*) AS n,COUNT(b) AS m,COUNT(DISTINCT b) AS d",
        star(10, "")
    ));
    ...
    let result = query
        .execute_governed(
            ...
            GqlQueryPolicy::new(9, 1, 4096, 256),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    assert_eq!(
    ...
    assert_eq!(
        result.value[0].get(1).unwrap().as_count(),
        Some(8_u64.pow(10))
    );
...
fn overflow_support_and_impossible_branches_obey_the_positive_weight_law() {
    ...
    let support = prepare(&format!(
        "{} RETURN COUNT(DISTINCT b) AS d,MIN(b) AS lo,MAX(b) AS hi",
        star(63, "")
    ));
    ...
    let missing = prepare(&format!(
        "{} RETURN COUNT(*) AS n",
        star(62, ",(b)-[:T]->(absent)")
    ));
    assert_eq!(
        run(&missing, &edges).unwrap().value[0]
            .get(0)
            .unwrap()
            .as_count(),
        Some(0)
...
fn forest_preprocessing_and_prefix_execution_share_every_limit_and_checkpoint() {
    let query = prepare(&format!(
        "{} RETURN a,COUNT(*) AS n GROUP BY a HAVING n>=1 ORDER BY n DESC,a LIMIT 1",
        star(3, "")
    ));
    let edges = star_edges(3);
    let mut calls = 0;
    ...
    let measured = query
        .execute_governed(
            edges.len() as u64,
            [],
            edges.iter().copied().inspect(|_| consumed += 1),
            |_, _| Ok::<_, ()>(true),
            |_, _| Ok(None),
            wide(),
            || {
...
fn predicates_and_property_arguments_never_disappear_into_completion_weights() {
    ...
    let result = query.execute_governed(
        ...
        [],
        edges,
        |_, _| Err::<bool, _>("hidden vertex predicate"),
        |_, _| Ok(None),
        wide(),
        || Ok::<_, ()>(()),
    );
