...
use fgdb_gql::algebra::{
    GraphColumn, GraphPatternBuilder, GraphValueOrder, IntegerComparison,
};
use fgdb_gql::{
    ...
    GraphHavingOp, GraphHavingOperand, GraphIntegerBinary, GraphIntegerExpression,
    GraphIntegerOp, GraphPatternTextErrorKind, GraphPipelineAggregateTextErrorKind,
    GraphSetExecutionError, GraphSetProjection, GraphSetQuantifier, GraphSetTextErrorKind,
    GraphSetValue, GraphSymbol, GraphSymbolKind, PreparedGraphAggregate,
    PreparedGraphPipelineAggregateText, PreparedGraphSet,
};
...
fn wide() -> GqlQueryPolicy { GqlQueryPolicy::new(100_000, 100_000, 5_000_000, 5_000_000) }
fn query(text: &str) -> PreparedGraphAggregate {
    PreparedGraphPipelineAggregateText::prepare(text, symbols).unwrap()
        .bind_parameters(&GqlParameters::new()).unwrap()
}
fn run(query: &PreparedGraphAggregate, input: &[CanonicalScalar], policy: GqlQueryPolicy)
    -> GqlQueryExecution<GraphAggregateRow> {
    query.execute_governed(input.len() as u64, (0..input.len()).map(|at| VId(at as u128)), [],
        |_, _| Ok::<_, ()>(true), |vid, key| Ok((key == P).then(|| &input[vid.0 as usize])),
        policy, || Ok::<_, ()>(())).unwrap()
}
...
fn native_pipeline_matches_typed_ir_and_retains_written_result_order() {
    ...
    let args = GqlParameters::new().with_uint64("off", 1).unwrap()
        .with_uint64("take", 3).unwrap().with_int64("bump", 1).unwrap()
        .with_int64("min", 3).unwrap().with_uint64("groups", 2).unwrap();
    ...
    let input = input.project(vec![
        ...
        .with_order_by(&[GraphValueOrder::ascending(0)]).unwrap().with_page(1, Some(3));
    ...
    let typed = PreparedGraphAggregate::prepare_relation(input, &[0], &[
        GraphAggregate::sum_int("total", 1), GraphAggregate::count_rows("copies"),
    ], 0, Some(2)).unwrap().with_result_clauses(&[], &[
    ...
    assert_eq!(result.value.iter().map(|row| (
        row.keys()[0].as_vertex().unwrap(), row.values()[0].as_integer().unwrap(),
        row.values()[1].as_count().unwrap(),
    )).collect::<Vec<_>>(), vec![(VId(3), 10, 1), (VId(1), 3, 1)]);
}
...
fn all_numeric_functions_and_boolean_having_preserve_exact_fractions() {
    ...
    let bound = template.bind_parameters(&GqlParameters::new().with_int64("threshold", 6).unwrap()).unwrap();
    let input = [CanonicalScalar::Int(5), CanonicalScalar::Int(5), CanonicalScalar::Null, CanonicalScalar::Int(9)];
    ...
    assert_eq!(values[7].as_value().unwrap().as_scalar(), Some(&CanonicalScalar::Int(5)));
    assert_eq!(values[8].as_value().unwrap().as_scalar(), Some(&CanonicalScalar::Int(9)));
    ...
    assert_eq!(result.value[0].values()[0].as_integer(), Some(i128::from(i64::MAX) * 2));
    assert_eq!(result.value[0].values()[1].as_average(), GraphExactAverage::new(i128::from(i64::MAX), 1));
    let absent = query("MATCH (n) WITH n.p AS x RETURN MAX(x) AS maximum HAVING NOT (maximum=0)");
    assert!(run(&absent, &[CanonicalScalar::Null], wide()).value.is_empty());
    let empty = query("MATCH (n) WITH n.p AS x RETURN COUNT(*) AS n,SUM(x) AS total,AVG(x) AS mean");
...
fn input_distinct_and_output_distinct_have_separate_grouping_boundaries() {
    let input = [CanonicalScalar::Int(2), CanonicalScalar::Int(2), CanonicalScalar::Int(5), CanonicalScalar::Null];
    for (quantifier, expected) in [("", 2), ("DISTINCT ", 1)] {
        let bound = query(&format!("MATCH (n) WITH {quantifier}n.p AS x WHERE x IS NOT NULL RETURN x AS key,COUNT(*) AS occurrences GROUP BY x ORDER BY key"));
        let result = run(&bound, &input, wide());
    ...
    for (quantifier, expected) in [("ALL ", 4), ("DISTINCT ", 1)] {
        let bound = query(&format!("MATCH (n) WITH n AS owner RETURN {quantifier}COUNT(*) AS occurrences GROUP BY owner HAVING occurrences=1 ORDER BY occurrences DESC"));
        let result = run(&bound, &input, wide());
...
fn nullable_optional_and_shortest_bags_survive_the_row_aggregate_boundary() {
    for (mode, atom, expected) in [("", ":R", 2),
        ("ALL SHORTEST WALK ", ":R*1..3", 2),
        ("ANY SHORTEST WALK ", ":R*1..3", 1)] {
        let bound = query(&format!("MATCH (a) OPTIONAL MATCH {mode}(a)-[{atom}]->(b) WITH a AS owner,b AS peer RETURN owner,COUNT(*) AS rows,COUNT(peer) AS matched GROUP BY owner ORDER BY owner"));
        let result = bound.execute_governed(4, [VId(1), VId(2)], [(VId(1), R, VId(2)); 2],
            |_, _| Ok::<_, ()>(true), |_, _| Ok(None), wide(), || Ok::<_, ()>(())).unwrap();
        let counts = result.value.iter().map(|row| (row.keys()[0].as_vertex().unwrap(),
...
fn malformed_grouped_tails_and_definition_limits_refuse_before_catalog_access() {
    let too_deep = format!("MATCH (n) WITH n.p AS x{} RETURN SUM(x) AS s", " WITH x".repeat(62));
    let too_many_having = format!("MATCH (n) WITH n.p AS x RETURN SUM(x) AS s HAVING {}", vec!["TRUE"; 65].join(" AND "));
    for text in [
...
fn one_argument_table_keeps_original_utf8_offsets_and_never_reopens_catalog() {
    ...
    assert!(matches!(missing.kind, GraphPipelineAggregateTextErrorKind::Input(
    ...
    assert_eq!(template.bind_parameters(&partial).unwrap_err().offset, text.find("$count").unwrap());
    ...
    assert_eq!(calls, 1);
    let wrong = GqlParameters::new().with_uint64("cut", 2).unwrap().with_uint64("count", 1).unwrap();
    ...
    assert!(PreparedGraphPipelineAggregateText::prepare_with_parameter_types(text,
        &[("cut", GqlParameterType::UInt64)], symbols).is_err());
...
fn later_arithmetic_observes_surviving_rows_but_failures_are_never_partial_success() {
    let bound = query("MATCH (n) WITH n.p AS x WHERE x <> 0 WITH CASE WHEN x > 0 THEN 10/x ELSE 0 END AS y RETURN SUM(y) AS total");
    let input = [-1, 0, 2, 5].map(CanonicalScalar::Int);
    assert_eq!(run(&bound, &input, wide()).value[0].values()[0].as_integer(), Some(7));
    let fails = query("MATCH (n) WITH n.p AS x WITH 10/x AS y RETURN COUNT(*) AS n LIMIT 0");
    let result = fails.execute_governed(4, (0..4).map(VId), [], |_, _| Ok::<_, ()>(true),
        |vid, _| Ok(Some(&input[vid.0 as usize])), wide(), || Ok::<_, ()>(()));
    assert!(matches!(result, Err(GqlQueryError::Source(GraphAggregateError::InputRelation(
        GraphSetExecutionError::Projection { .. })))));
    let source_error = fails.execute_governed(1, [VId(0)], [], |_, _| Ok(true),
        |_, _| Err::<Option<&CanonicalScalar>, _>("unreadable input"), wide(), || Ok::<_, ()>(()));
    assert!(matches!(source_error, Err(GqlQueryError::Source(GraphAggregateError::InputRelation(
    ...
    assert!(matches!(non_integer.execute_governed(1, [VId(0)], [], |_, _| Ok::<_, ()>(true),
        |_, _| Ok(Some(&bad_value)), wide(), || Ok::<_, ()>(())),
...
fn native_complete_pipeline_uses_one_query_allowance_and_every_checkpoint() {
    let bound = query("MATCH (n) WITH n.p AS x WHERE x > 0 RETURN x AS key,COUNT(*) AS n GROUP BY x HAVING n > 0 ORDER BY key DESC");
    let input = [1, 1, 2].map(CanonicalScalar::Int);
    let mut calls = 0;
    let measured = bound.execute_governed(3, [VId(0), VId(1), VId(2)], [],
        |_, _| Ok::<_, ()>(true), |vid, _| Ok(Some(&input[vid.0 as usize])), wide(), || {
            calls += 1; Ok::<_, usize>(())
        }).unwrap();
    ...
    assert_eq!(run(&bound, &input, GqlQueryPolicy::new(caps[0], caps[1], caps[2], caps[3])), measured);
    for dimension in 0..4 {
        let mut cap = caps; cap[dimension] -= 1;
        assert!(bound.execute_governed(3, [VId(0), VId(1), VId(2)], [],
            |_, _| Ok::<_, ()>(true), |vid, _| Ok(Some(&input[vid.0 as usize])),
            GqlQueryPolicy::new(cap[0], cap[1], cap[2], cap[3]), || Ok::<_, ()>(())).is_err());
    }
    for stop in 1..=calls {
        let mut at = 0;
        let result = bound.execute_governed(3, [VId(0), VId(1), VId(2)], [],
            |_, _| Ok::<_, ()>(true), |vid, _| Ok(Some(&input[vid.0 as usize])), wide(), || {
