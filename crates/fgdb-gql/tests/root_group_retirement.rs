...
fn summary(root: VId, inputs: &[Option<i64>]) -> Expected {
    ...
    let unique_sum =
        (!unique.is_empty()).then(|| unique.iter().map(|n| i128::from(*n)).sum::<i128>());
    Expected {
        root,
        count: inputs.len() as u64,
        distinct: unique.len() as u64,
        sum,
        mean: sum.map(|sum| (sum, values.len() as u64)),
        unique_sum,
        ...
        for (column, ratio) in [(3, expected.mean), (5, expected.unique_mean)] {
            match ratio {
                ...
                Some((sum, count)) => {
                    ...
                    assert_eq!(
                        actual.numerator() * i128::from(count),
                        sum * i128::from(actual.denominator())
                    );
                }
            }
        }
...
fn oriented_multigraph_pages_match_independent_complete_group_enumeration() {
    ...
    for (direction, arrow) in [(0, "-[:R]->"), (1, "<-[:R]-"), (2, "-[:R]-")] {
        let text = format!(
            "MATCH (a:L){arrow}(b) RETURN a,COUNT(*) AS n,COUNT(DISTINCT b.p) AS d,\
        ...
        for mask in 0..64 {
            ...
            for mut encoded in 0..27 {
                let raw: Vec<_> = (0..3)
                    .map(|_| {
                        let value = [None, Some(-7), Some(9)][encoded % 3];
                        encoded /= 3;
                        value
                    })
                    .collect();
                let scalars: Vec<_> = raw
                    .iter()
                ...
                for &(src, _, dst) in &edges {
                    let oriented = if direction == 1 {
                        vec![(dst, src)]
                    } else if direction == 2 && src != dst {
                        vec![(src, dst), (dst, src)]
                    } else {
                        vec![(src, dst)]
                    };
                    for (a, b) in oriented {
                        groups.entry(a).or_default().push(raw[b.0 as usize - 1]);
                    }
                }
                let mut expected: Vec<_> = groups
                    ...
                    .map(|(root, values)| summary(*root, values))
                    .collect();
                ...
                for (offset, count) in [(0, 0), (0, 3), (1, 1)] {
                    ...
                    let result = query
                        .execute_governed(
                            ...
                            |_, _| Ok::<_, ()>(true),
                            |vid, _| Ok(scalars[vid.0 as usize - 1].as_ref()),
                            wide(),
                            || Ok::<_, ()>(()),
                        )
                        .unwrap();
...
fn grouped_payloads_are_copied_only_for_the_final_page_and_all_source_rows_are_read() {
    let query = prepare(
        "MATCH (a)-[:R]->(b) RETURN a,MIN(b.p) AS payload GROUP BY a ORDER BY a DESC SKIP 1 LIMIT 2",
    );
    ...
    let run = |value: &CanonicalScalar| {
        ...
        let mut property_reads = 0;
        let result = query
            .execute_governed(
                16,
                [],
                edges.iter().rev().copied().inspect(|_| {
                    edge_reads += 1;
                }),
                |_, _| Ok::<_, ()>(true),
                |_, _| {
            ...
            .unwrap();
        assert_eq!(edge_reads, 16);
        assert_eq!(property_reads, 16);
        result
    };
    let a = run(&small);
    let b = run(&large);
    assert_eq!(
        b.evaluator.scratch_entries - a.evaluator.scratch_entries,
        2 * 8192 / 64
    );
    assert_eq!(
        b.value
    ...
    assert!(
        b.value
            .iter()
            .all(|row| row.get(0).unwrap().as_value().unwrap().as_scalar() == Some(&large))
    );
}
...
fn all_four_limits_and_every_public_execution_checkpoint_remain_enforced() {
    let query = prepare(
        "MATCH (a)-[:R]->(b) RETURN a,AVG(DISTINCT b.p) AS mean,COUNT(DISTINCT b.p) AS kinds \
    ...
    for policy in [
        GqlQueryPolicy::new(11, 2, u64::MAX, u64::MAX),
        GqlQueryPolicy::new(12, 1, u64::MAX, u64::MAX),
        GqlQueryPolicy::new(12, 2, complete.evaluator.work_units - 1, u64::MAX),
        GqlQueryPolicy::new(12, 2, u64::MAX, complete.evaluator.scratch_entries - 1),
    ] {
...
fn optional_and_existential_scopes_complete_before_the_root_is_retired() {
    ...
    let property = |vid: VId| match vid.0 {
        20 => Some(&values[0]),
        21 => Some(&values[1]),
        22 => Some(&values[2]),
        _ => None,
                ...
                |vid, predicates| {
                    Ok::<_, ()>(predicates.iter().all(|predicate| {
                        predicate.matches_borrowed(
                            (vid.0 < 10).then_some(LabelId(1)),
                            property(vid).map(|value| (P, value)),
                        )
                    }))
                },
                |vid, _| Ok(property(vid)),
                wide(),
                || Ok::<_, ()>(()),
            )
            .unwrap()
    ...
    let optional = prepare(
        "MATCH (a:L)-[:R]->(b) OPTIONAL MATCH (b)-[:S]->(c) \
    ...
    assert_eq!(
        rows[0].get(2).unwrap().as_average().unwrap().denominator(),
        1
    );
    ...
    for (negation, expected) in [
        ("", vec![(VId(1), 2)]),
        ("NOT ", vec![(VId(1), 1), (VId(2), 1)]),
    ] {
        let query = prepare(&format!(
            "MATCH (a:L)-[:R]->(b) WHERE {negation}EXISTS {{ MATCH (b)-[:S]->(c) WHERE c.p>3 }} \
...
fn unproven_input_order_and_nontrivial_output_distinct_keep_general_grouping() {
    ...
    let node =
        prepare("MATCH (a) RETURN a,COUNT(*) AS n,AVG(a.p) AS mean GROUP BY a ORDER BY a LIMIT 5");
    let result = node
        .execute_governed(
            3,
            [VId(2), VId(1), VId(2)],
            [],
            |_, _| Ok::<_, ()>(true),
            |vid, _| Ok(Some(if vid == VId(1) { &low } else { &high })),
            wide(),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    ...
    let distinct = prepare(
        "MATCH (a)-[:R]->(b) RETURN DISTINCT AVG(b.p) AS mean GROUP BY a ORDER BY a LIMIT 2",
    );
    let edges = [
        (VId(1), R, VId(10)),
        (VId(2), R, VId(10)),
    ...
    let result = distinct
        .execute_governed(
            ...
            edges,
            |_, _| Ok::<_, ()>(true),
            |vid, _| Ok(Some(if vid == VId(10) { &low } else { &high })),
            wide(),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    ...
    assert_eq!(
        result
            .value
            .iter()
            .map(|row| row.get(0).unwrap().as_average().unwrap().numerator())
            .collect::<Vec<_>>(),
        vec![3, 7]
    );
}
