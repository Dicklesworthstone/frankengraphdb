...
use fgdb_gql::algebra::{
    GlaDirection, GraphColumn, GraphMatchClause, GraphPatternBuilder, VertexPredicate,
};
...
fn wide() -> GqlQueryPolicy {
    GqlQueryPolicy::new(10_000, 10_000, u64::MAX, u64::MAX)
}
fn oriented(left: VId, right: VId, source: VId, destination: VId, direction: GlaDirection) -> bool {
    match direction {
        ...
        GlaDirection::Undirected => {
            (source == left && destination == right) || (source == right && destination == left)
        }
    }
}
...
fn cyclic_and_fan_in_bags_match_complete_assignment_multiplicities() {
    ...
    let universe = [
        (ids[0], RelationId(1), ids[1]),
        (ids[0], RelationId(1), ids[1]),
        (ids[1], RelationId(1), ids[0]),
        (ids[0], RelationId(1), ids[0]),
        (ids[1], RelationId(2), ids[0]),
        (ids[0], RelationId(2), ids[1]),
        (ids[0], RelationId(3), ids[1]),
        (ids[1], RelationId(3), ids[0]),
    ];
    for mask in 0..256_usize {
        let edges: Vec<_> = universe
            ...
            .map(|(_, edge)| *edge)
            .collect();
        for first_direction in [
            GlaDirection::Forward,
            GlaDirection::Reverse,
            GlaDirection::Undirected,
        ] {
            for last_direction in [
                GlaDirection::Forward,
                GlaDirection::Reverse,
                GlaDirection::Undirected,
            ] {
                for fan_in in [false, true] {
                    ...
                    b.edge("b", RelationId(2), GlaDirection::Forward, "c")
                        .unwrap();
                    b.edge(
                        if fan_in { "a" } else { "c" },
                        RelationId(3),
                    ...
                    for a in ids {
                        for mid in ids {
                            for c in ids {
                                let count = |relation, left, right, direction| {
                                    edges
                                        .iter()
                                        .filter(|&&(s, r, d)| {
                                            r == relation && oriented(left, right, s, d, direction)
                                        })
                                        .count()
                                };
...
fn root_self_loop_seeks_keep_every_parallel_occurrence() {
    let mut b = builder(&["a"]);
    b.edge("a", RelationId(1), GlaDirection::Undirected, "a")
        .unwrap();
    let edges = [
        (VId(0), RelationId(1), VId(9)),
...
fn disjoint_fan_in_finishes_inside_a_linearithmic_work_allowance() {
    ...
    b.edge("b", RelationId(2), GlaDirection::Forward, "c")
        .unwrap();
    b.edge("a", RelationId(3), GlaDirection::Forward, "c")
        .unwrap();
    ...
    let pattern = b
        .prepare_bindings(&["a", "c"], 0, None)
        .unwrap()
        .with_duplicates();
    let execution = pattern
        .plan()
        .execute_with_limits(
            [],
            edges,
            |_, _| Ok::<_, ()>(true),
            GlaExecutionLimits::new(128 * n, u64::MAX),
        )
        .unwrap();
...
fn intersection_never_skips_an_intervening_fallible_property_selection() {
    ...
    b.edge("b", RelationId(2), GlaDirection::Forward, "c")
        .unwrap();
    b.edge("c", RelationId(3), GlaDirection::Forward, "a")
        .unwrap();
    b.filter("c", VertexPredicate::HasLabel(LabelId(1)))
        .unwrap();
...
#[test]
fn property_pair_selection_is_fallible_before_a_following_existence_clause() {
    ...
    .unwrap();
    // The root pattern must bind completely before its comparison runs. Put
    // the missing closing edge in a later clause to exercise the error barrier.
    let operators = query.plan().operators();
    let comparison = operators
        .iter()
        .position(|op| matches!(op, fgdb_gql::algebra::GlaOperator::CompareProperties { .. }))
        .unwrap();
    let probe = operators
        .iter()
        .position(|op| matches!(op, fgdb_gql::algebra::GlaOperator::Probe { .. }))
        .unwrap();
    ...
    assert!(matches!(
        result,
        Err(GqlQueryError::Source("property-pair source failed"))
    ));
    assert_eq!(
        reads, 1,
        "a missing closing edge cannot suppress the earlier read"
    );
}
...
fn optional_semi_and_anti_clauses_keep_scope_and_bag_semantics() {
    ...
    child
        .edge("a", RelationId(2), GlaDirection::Forward, "c")
        .unwrap();
    child
        .edge("c", RelationId(3), GlaDirection::Forward, "a")
        .unwrap();
    let edges = [
        (VId(1), RelationId(2), VId(3)),
        (VId(1), RelationId(2), VId(3)),
        (VId(3), RelationId(3), VId(1)),
        (VId(3), RelationId(3), VId(1)),
        (VId(3), RelationId(3), VId(1)),
    ];
    let run = |clause, columns: &[GraphColumn<'_>]| {
        let query = root
            .prepare_values_with_clauses(&[clause], columns, 0, None)
            .unwrap()
            .with_duplicates();
        query
            .plan()
    ...
    assert_eq!(
        optional
            .iter()
            .filter(|row| row.get(1).unwrap().is_null())
            .count(),
        1
    );
