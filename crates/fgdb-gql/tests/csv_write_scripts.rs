...
fn permuted_headers_preserve_complete_native_program_bytes() {
    ...
    assert_eq!(
        actual.program().canonical_bytes(),
...
fn multiple_statements_are_record_major_and_keep_original_script_coordinates() {
    ...
    assert_eq!(
        calls.get(),
        resolved,
        "CSV rebinding must not resolve symbols"
    );
...
fn default_sixty_four_expanded_statements_and_explicit_larger_admission() {
    ...
    assert_eq!(
        script
            .bind_csv(&csv(64), CsvParameterLimits::default())
            .unwrap()
            .program()
            .statements()
            .len(),
    ...
    let error = csv_error(
        script
            .bind_csv(&csv(65), CsvParameterLimits::default())
            .unwrap_err(),
    );
    ...
    let expected = script
        .bind_parameter_sets_with_limit(&expected_arguments, 65)
        .unwrap();
...
fn default_cap_counts_the_product_not_records_alone() {
    ...
    assert_eq!(
        script
            .bind_csv(&csv(32), CsvParameterLimits::default())
            .unwrap()
            .program()
            .statements()
            .len(),
    ...
    let error = csv_error(
        script
            .bind_csv(&csv(33), CsvParameterLimits::default())
            .unwrap_err(),
    );
    assert_eq!(
        error.kind,
    ...
    assert!(
        script
            .bind_csv_with_statement_limit(&csv(33), CsvParameterLimits::default(), 65)
            .is_err()
    );
    assert_eq!(
        script
            .bind_csv_with_statement_limit(&csv(33), CsvParameterLimits::default(), 66)
            .unwrap()
            .program()
            .statements()
            .len(),
...
fn zero_or_insufficient_statement_allowance_refuses_before_decoding_values() {
    ...
    let two = PreparedGraphWriteScript::prepare(
        "CREATE (n {p:$key});MATCH (n) SET n.q=$value",
        R,
        symbols,
    )
    .unwrap();
    assert_eq!(
        csv_error(
            two.bind_csv_with_statement_limit(source, CsvParameterLimits::default(), 1)
                .unwrap_err()
        )
        .kind,
        error.kind
...
fn malformed_final_record_drops_prefix_and_does_not_change_reusable_script() {
    let script = insertion();
    let prior = script
        .bind_csv(&csv(2), CsvParameterLimits::default())
        .unwrap();
...
fn text_payloads_nulls_empty_strings_and_multiline_fields_bind_as_values() {
    ...
    assert!(
        actual
            ...
            .statements()
            .iter()
            .all(|statement| { matches!(statement, GraphWriteStatement::Insert(_)) })
    );
    assert_eq!(script.script(), text);
}
...
fn csv_error_source_retains_original_input_coordinates_without_values() {
    ...
    let cause = cause
        .downcast_ref::<fgdb_gql::csv_parameters::CsvParameterError>()
        .unwrap();
