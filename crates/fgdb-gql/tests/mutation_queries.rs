...
use fgdb_gql::{GqlParameterType, GqlParameters, GqlQueryError, GqlQueryPolicy,
    GraphMutationBatch, GraphMutationBuildError, GraphMutationError, GraphMutationIntent,
    GraphMutationPolicy, GraphMutationTextErrorKind, GraphPatternTextErrorKind,
    GraphSymbol, GraphSymbolKind, MAX_GRAPH_MUTATION_ACTIONS, PreparedGraphMutation,
...
fn policy() -> GraphMutationPolicy {
    GraphMutationPolicy::new(GqlQueryPolicy::new(1_000, 1_000, 1_000_000, 1_000_000), 1_000)
}
...
fn reciprocal_updates_freeze_rhs_and_collapse_parallel_matches() {
    ...
    assert_eq!(result.intents(), &[
        GraphMutationIntent::Property { vertex: VId(1), key: P, value: Some(CanonicalScalar::Int(20)) },
        GraphMutationIntent::Property { vertex: VId(2), key: P, value: Some(CanonicalScalar::Int(10)) },
    ]);
    assert_eq!(result.stats().selection.result_rows, 3);
    assert_eq!((result.stats().target_vertices, result.stats().effects), (2, 2));
...
fn inconsistent_assignments_refuse_across_rows_or_aliases_instead_of_choosing_order() {
    ...
    for edges in [
        ...
        assert!(matches!(run(&plan, &[VId(1), VId(2), VId(3)], &edges, &props(), policy()),
            Err(GqlQueryError::Source(GraphMutationError::ConflictingAssignment { .. }))));
    }
    let aliases = prepare("MATCH (a),(b) WHERE a=b SET a.p=1,b.p=2");
    assert!(matches!(run(&aliases, &[VId(1)], &[], &props(), policy()),
        Err(GqlQueryError::Source(GraphMutationError::ConflictingAssignment {
            first_row: 0, row: 0, first_action: 0, action: 1,
        }))));
}
...
fn optional_null_targets_are_skipped_and_missing_rhs_is_a_canonical_null() {
    ...
    let result = run(&plan, &[VId(1), VId(2), VId(3)], &[(VId(1), R, VId(2))], &props(), policy()).unwrap();
    assert_eq!(result.stats().selection.result_rows, 3);
    assert_eq!(result.intents(), &[GraphMutationIntent::Property {
        vertex: VId(2), key: Q, value: Some(CanonicalScalar::Int(10)),
    }]);
    let missing = prepare("MATCH (a) SET a.p=a.q REMOVE a.old");
    let result = run(&missing, &[VId(1)], &[], &props(), policy()).unwrap();
    assert_eq!(result.intents(), &[
        GraphMutationIntent::Property { vertex: VId(1), key: P, value: Some(CanonicalScalar::Null) },
        GraphMutationIntent::Property { vertex: VId(1), key: OLD, value: None },
    ]);
    let conflicting_null = prepare("MATCH (a) SET a.q=NULL REMOVE a.q");
    assert!(matches!(run(&conflicting_null, &[VId(1)], &[], &props(), policy()),
        Err(GqlQueryError::Source(GraphMutationError::ConflictingAssignment { .. }))));
}
...
fn bounded_walk_targets_keep_occurrences_but_each_delete_is_proposed_once() {
    ...
    assert_eq!(isolated.intents(), &[GraphMutationIntent::DetachDelete { vertex: VId(9) }]);
}
...
fn typed_arguments_and_catalog_resolution_are_shared_by_read_and_write_clauses() {
    ...
    let template = PreparedGraphMutationText::prepare_with_parameter_types(text, R,
        &[("t", GqlParameterType::Scalar(kind))], |kind, name| {
    ...
    let args = GqlParameters::new().with_int64("n", 10).unwrap().with_text("t", payload).unwrap();
    ...
    assert!(result.intents().contains(&GraphMutationIntent::Property {
        vertex: VId(1), key: TEXT, value: Some(CanonicalScalar::ucs_basic_text(payload).unwrap()),
    }));
    assert!(result.intents().contains(&GraphMutationIntent::Label { vertex: VId(1), label: UPDATED, present: true }));
    ...
    assert!(matches!(missing.kind, GraphMutationTextErrorKind::Query(GraphPatternTextErrorKind::MissingParameter)));
    let wrong = GqlParameters::new().with_int64("n", 1).unwrap().with_bool("t", true).unwrap();
...
fn malformed_unsupported_and_oversized_mutations_refuse_before_catalog_callbacks() {
    for text in [
        ...
        assert!(PreparedGraphMutationText::prepare(text, R, |kind, name| {
            calls.set(calls.get() + 1); symbols(kind, name)
        }).is_err(), "{text}");
        assert_eq!(calls.get(), 0, "{text}");
    }
    let text = format!("MATCH (a) SET {}", vec!["a.p=1"; MAX_GRAPH_MUTATION_ACTIONS + 1].join(","));
    let calls = Cell::new(0);
    let failed = PreparedGraphMutationText::prepare(&text, R, |kind, name| {
        calls.set(calls.get() + 1); symbols(kind, name)
    }).unwrap_err();
    assert!(matches!(failed.kind, GraphMutationTextErrorKind::Build(GraphMutationBuildError::TooManyActions { .. })));
...
fn exact_proposal_budgets_include_source_work_and_scalar_payload_copies() {
    ...
    let exact = GraphMutationPolicy::new(GqlQueryPolicy::new(2, 2,
        stats.evaluator.work_units, stats.evaluator.scratch_entries), 4);
...
fn every_selection_and_proposal_checkpoint_can_interrupt_without_returning_a_batch() {
    ...
    let execute = |stop: usize| {
        ...
        let result: MutationResult<usize> = plan.execute_governed(policy(), |selection, budget| {
            selection.plan().execute_governed_with_properties(2, [], edges,
                |_, _| Ok::<_, ()>(true), |vid, key| Ok(properties.get(&(vid, key))), budget, checkpoint)
        }, checkpoint);
        (result, calls.get())
    };
    ...
    let late_failure: MutationResult<&str> = plan.execute_governed(policy(), |_, _| {
        Err(GqlQueryError::Interrupted("late source failure"))
    }, || Ok(()));
    assert!(matches!(late_failure, Err(GqlQueryError::Interrupted("late source failure"))));
}
