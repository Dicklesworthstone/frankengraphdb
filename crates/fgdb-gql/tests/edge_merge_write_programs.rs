...
use fgdb_gql::{
    ...
    GraphEdgeMergeError, GraphEdgeMergeStats, GraphMutationProgramDimension,
    GraphMutationProgramError, GraphSymbol, GraphSymbolKind, GraphWriteProgramError,
    GraphWriteProgramPolicy, GraphWriteProgramTemplateError, GraphWriteStepError,
    GraphWriteStepStats, PreparedGraphEdgeMerge, PreparedGraphEdgeMergeText,
    PreparedGraphVertexMergeText, PreparedGraphWriteProgram, PreparedGraphWriteProgramTemplate,
};
use std::cell::Cell;
...
fn merge() -> PreparedGraphEdgeMerge {
    PreparedGraphEdgeMergeText::prepare(EDGE, R, symbols)
        .unwrap()
...
fn stats(created: bool) -> GraphEdgeMergeStats {
    GraphEdgeMergeStats {
        ...
        evaluator: GlaExecutionStats {
            work_units: if created { 5 } else { 6 },
            scratch_entries: 1,
        },
...
fn existence_scans_count_toward_one_cumulative_source_budget() {
    ...
    let result: Result<_, Error> = program.execute_governed(
        ...
        |statement, _, remaining| {
            ...
            assert_eq!(
                remaining.edge_merge_policy().max_created_edges,
                remaining.max_created_edges
            );
            assert_eq!(
                remaining.mutations.query.rows.max_snapshot_records(),
                Some(if statement == 0 { 11 } else { 6 })
            );
    ...
    let result: Result<_, Error> = program.execute_governed(
        limited,
        |statement, _, _| Ok(GraphWriteStepStats::EdgeMerge(stats(statement == 0))),
        || Ok(()),
    );
    assert!(matches!(
        result,
        Err(GraphWriteProgramError::Program(
            GraphMutationProgramError::Budget {
                statement: 1,
                dimension: GraphMutationProgramDimension::SnapshotRecords,
                limit: 10,
                observed: 11,
            }
        ))
...
fn combined_match_and_existence_source_count_cannot_wrap() {
    ...
    assert!(
        matches!(result, Err(GraphWriteProgramError::Program(GraphMutationProgramError::Budget {
...
fn no_input_is_valid_but_impossible_branch_statistics_are_rejected() {
    ...
    let result: Result<_, Error> = program.execute_governed(
        policy(),
        |_, _, _| Ok(GraphWriteStepStats::EdgeMerge(empty)),
        || Ok(()),
    );
        ...
        let result: Result<_, Error> = program.execute_governed(
            ...
            |_, _, _| {
                ...
                match corruption {
                    ...
                    _ => {
                        value.match_selection.result_rows = 1;
                        value.created_edges = 2;
                    }
                }
                Ok(GraphWriteStepStats::EdgeMerge(value))
            },
        ...
        |statement, _, remaining| {
            if statement == 0 {
                ...
                Err(GraphWriteStepError::EdgeMerge(GqlQueryError::Source(
                    GraphEdgeMergeError::CreationLimit {
                        limit: 0,
                        observed: 1,
                    },
                )))
...
fn relationship_templates_share_arguments_and_preserve_later_bind_errors() {
    ...
    let vertex =
        PreparedGraphVertexMergeText::prepare("MERGE (n:Person {p:$left})", R, &mut resolve)
            .unwrap();
    ...
    let expected = PreparedGraphWriteProgram::prepare(vec![
        vertex
            .bind_parameters(&GqlParameters::new().with_int64("left", 7).unwrap())
            .unwrap()
            .into(),
    ...
    let error = template
        ...
        .unwrap_err();
    assert!(
        matches!(error, GraphWriteProgramTemplateError::EdgeMergeBind { statement: 1, source }
