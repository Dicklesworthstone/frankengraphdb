...
fn internal_calls_share_typed_state_but_never_extend_the_returned_schema() {
    ...
    assert_eq!(
        bound.input_pattern().columns(),
        &["owner", "__fgdb_hidden_0"]
    );
    ...
    assert_ne!(
        template
            .bind_parameters(&arguments(3, 1, 0, 2))
            .unwrap()
            .canonical_bytes(),
        bytes
    );
    assert_eq!(
        template.bind_parameters(&arguments(2, 0, 1, 3)).unwrap(),
        bound
    );
    ...
    assert!(
        template
            .bind_parameters(&arguments(2, 0, 1, 3).with_int64("extra", 1).unwrap())
            .is_err()
    );
}
...
fn key_only_optional_groups_match_independent_hidden_filter_rank_and_page_oracles() {
    ...
    for (descending, null_first) in [(true, false), (false, true)] {
        let text = format!(
            "MATCH (a:Owner) OPTIONAL MATCH (a)-[:R]->(b) \
        ...
        assert_eq!(
            template.output_slots(),
            &[GraphAggregateTextSlot::GroupKey(0); 2]
        );
        for mask in 0..32 {
            ...
            for code in 0..27 {
                ...
                let values: Vec<_> = (0..3)
                    .map(|_| {
                        let value = [None, Some(-3_i64), Some(7)][encoded % 3];
                        encoded /= 3;
                        value
                    })
                    .collect();
                ...
                for owner in [VId(1), VId(2), VId(3)] {
                    let bag: Vec<_> = edges
                        .iter()
                        .filter(|edge| edge.0 == owner)
                        .filter_map(|edge| values[(edge.2.0 - 10) as usize])
                        .collect();
                    if bag.len() <= 1 && !bag.is_empty() {
                        continue;
                    }
                    let unique: BTreeSet<_> = bag.into_iter().collect();
                    let mean = (!unique.is_empty()).then(|| {
                        (
                            unique.iter().map(|n| i128::from(*n)).sum::<i128>(),
                            unique.len() as i128,
                        )
                    });
                ...
                for (offset, count) in [(0, 0), (0, 3), (1, 1), (4, 1)] {
                    let args = GqlParameters::new()
                        .with_uint64("off", offset)
                        .unwrap()
                        .with_uint64("take", count)
                        .unwrap();
                    ...
                    let result = query
                        .execute_governed(
                            ...
                            edges.iter().copied(),
                            |vid, _| Ok::<_, ()>(vid.0 < 10),
                            |vid, _| Ok(scalars[(vid.0 - 10) as usize].as_ref()),
                            wide(),
                            || Ok::<_, ()>(()),
                        )
                        .unwrap();
                    assert!(
                        result
                            .value
                            .iter()
                            .all(|row| row.values().is_empty() && row.keys().len() == 1)
                    );
                    ...
                    assert_eq!(
                        actual, page,
                        "mask={mask} code={code} off={offset} take={count}"
                    );
                }
            }
        }
...
fn private_aliases_cannot_capture_public_names_and_calls_preserve_distinctness() {
    let query = prepare(
        "MATCH (a) RETURN a AS __fgdb_hidden_0,COUNT(*) AS __fgdb_hidden_1 \
    ...
    assert_eq!(
        query.input_pattern().columns(),
        &["__fgdb_hidden_0", "__fgdb_hidden_2"]
    );
    assert_eq!(
        query.ordering()[1].column,
        GraphAggregateColumn::Aggregate(0)
    );
    let query = prepare(
        "MATCH (a)-[:R]->(b) RETURN a GROUP BY a \
    ...
    let unchanged =
        prepare("MATCH (a) RETURN COUNT(*) AS x,COUNT(*) AS y HAVING COUNT(*) >= 1 ORDER BY y");
    assert_eq!(ordinary.canonical_bytes(), unchanged.canonical_bytes());
    assert_eq!(unchanged.evaluation_aggregate_columns().len(), 2);
    assert_eq!(
        unchanged
            .clone()
            .with_aggregate_output_prefix(2)
            .unwrap()
            .canonical_bytes(),
        unchanged.canonical_bytes()
    );
    let alias = prepare(
        "MATCH (a) RETURN a AS owner,COUNT(*) AS a GROUP BY a HAVING a > 0 ORDER BY MIN(a.p),a",
    );
    assert_eq!(alias.having()[0].column, GraphAggregateColumn::Aggregate(0));
    assert_eq!(
        alias.ordering()[1].column,
        GraphAggregateColumn::Aggregate(0)
    );
}
...
fn computed_width_and_malformed_private_references_refuse_before_catalog_access() {
    ...
    for hidden in [64, 65] {
        let order = (0..hidden)
            .map(|at| format!("SUM(a.p{})", at + 1))
            .collect::<Vec<_>>()
            .join(",");
        let mut calls = 0;
        let result = PreparedGraphAggregateText::prepare(
            &format!("MATCH (a) RETURN a GROUP BY a ORDER BY {order}"),
            |kind, name| {
                calls += 1;
                if kind == GraphSymbolKind::Property {
                    Some(GraphSymbol::Property(PropertyKeyId(
...
fn hidden_inputs_and_invalid_numeric_domains_cannot_be_masked_by_zero_output() {
    ...
    for suffix in [
        "HAVING FALSE ORDER BY SUM(a.p) LIMIT 0",
        "HAVING TRUE OR AVG(a.p) IS NULL LIMIT 0",
    ] {
    ...
    for (tail, rows) in [
        ("HAVING AVG(a.p) IS NULL", 1),
        ("HAVING NOT (AVG(a.p) = 0)", 0),
    ] {
        let query = prepare(&format!(
            "MATCH (a) RETURN COUNT(*) AS n {tail} ORDER BY AVG(a.p)"
        ));
        let empty = query
            .execute_governed(
                0,
        ...
        if rows == 1 {
            assert_eq!(empty.value[0].values().len(), 1);
...
fn hidden_extrema_are_borrowed_without_copying_their_return_payloads() {
    ...
    let hidden = prepare(
        "MATCH (a)-[:R]->(b) RETURN a,COUNT(*) AS n GROUP BY a ORDER BY MIN(b.p),a LIMIT 1",
    );
    let visible = prepare(
        "MATCH (a)-[:R]->(b) RETURN a,COUNT(*) AS n,MIN(b.p) AS m GROUP BY a ORDER BY m,a LIMIT 1",
    );
...
fn hidden_aggregation_uses_all_resource_limits_and_every_checkpoint() {
    ...
    let run = |policy| {
        query.execute_governed(
            ...
            edges,
            |_, _| Ok::<_, ()>(true),
            |vid, _| Ok(scalars.get((vid.0 - 10) as usize)),
            policy,
            || Ok::<_, ()>(()),
        )
    };
    ...
    assert_eq!(
        run(GqlQueryPolicy::new(5, 1, work, scratch)).unwrap(),
        complete
    );
    for policy in [
        GqlQueryPolicy::new(4, 1, u64::MAX, u64::MAX),
        GqlQueryPolicy::new(5, 0, u64::MAX, u64::MAX),
        GqlQueryPolicy::new(5, 1, work - 1, u64::MAX),
        GqlQueryPolicy::new(5, 1, u64::MAX, scratch - 1),
    ] {
...
fn hidden_topology_counts_still_refuse_overflow_before_having_or_limit() {
    ...
    let query = prepare(&format!(
        "MATCH {atoms} RETURN a GROUP BY a HAVING TRUE OR COUNT(*) >= 0 LIMIT 0"
    ));
    let result = query.execute_governed(
        2,
        [],
