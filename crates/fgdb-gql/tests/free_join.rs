...
use fgdb_gql::free_join::{
    ColumnarTrie, FreeJoin, FreeJoinPlan, JoinPlanError, JoinVariable as V, MultiplicityOverflow,
...
fn oracle(order: &[V], schemas: &[Vec<V>], inputs: &[Vec<Vec<i64>>]) -> Vec<Vec<i64>> {
        ...
        for outer in &bindings {
            for tuple in input {
                if schema
                    .iter()
                    .zip(tuple)
                    .all(|(v, key)| outer.get(v).is_none_or(|old| old == key))
                {
                    let mut row = outer.clone();
                    for (&v, &key) in schema.iter().zip(tuple) {
                        row.insert(v, key);
                    }
                    next.push(row);
                }
    ...
    let mut rows: Vec<Vec<i64>> = bindings
        .iter()
        .map(|row| order.iter().map(|v| row[v]).collect())
        .collect();
    rows.sort();
    rows
}
...
fn binary_grouped_generic_and_occurrence_oracle_agree() {
    let cases = [
        ...
        (
            vec![vec![V(0)], vec![V(1)]],
            vec![vec![vec![1], vec![1]], vec![vec![2], vec![3]]],
        ),
        (
            vec![vec![V(0), V(1)], vec![V(1), V(0)]],
            vec![
        ...
        (
            vec![vec![], vec![V(0)]],
            vec![vec![vec![], vec![]], vec![vec![8], vec![8]]],
        ),
        (
            vec![vec![], vec![]],
...
fn exhaustive_two_vertex_triangles_under_every_variable_order() {
    ...
    let relation = |mask: u16| -> Vec<Vec<i64>> {
        (0..4_u32)
            ...
            .map(|bit| vec![i64::from(bit / 2), i64::from(bit % 2)])
            .collect()
    };
    for bits in 0..4096_u16 {
        ...
        for order in orders {
            ...
            assert_eq!(
                execute(&plan, &schemas, &inputs).0,
                oracle(plan.variables(), &schemas, &inputs),
                "bits={bits}, order={order:?}"
            );
        }
    }
}
...
fn multiway_bags_multiply_only_at_complete_bindings() {
    ...
    let mut emissions = 0;
    FreeJoin::new(&plan, &data)
        .unwrap()
        .for_each_binding(&mut |_| Ok::<_, ()>(()), |binding, control| {
            emissions += 1;
            assert_eq!(binding.multiplicity(), Ok(60));
            assert_eq!(binding.get(V(1)), Some(&2));
            let mut occurrences = 0;
            binding.for_each_occurrence(control, |_, _| {
                occurrences += 1;
                Ok(())
            })?;
    ...
    assert_eq!(
        emissions, 1,
        "duplicates must not enumerate incomplete prefixes"
    );
}
...
fn skewed_triangle_does_not_enumerate_quadratic_wedges() {
    ...
    assert!(
        large < small * 3,
        "a doubled adversary must not quadruple join work"
    );
}

#[test]
fn cursor_seeks_are_monotone_and_parents_keep_their_generation() {
    let trie = ColumnarTrie::new(
        &[V(0), V(1)],
        &[V(0), V(1)],
    ...
    assert_eq!(
        root.open(&mut |_| Ok::<_, ()>(())).unwrap().unwrap().key(),
        Some(&3)
    );
...
fn invalid_schemas_orders_and_rows_fail_before_joining() {
    assert_eq!(
        FreeJoinPlan::generic(vec![vec![V(0)]], vec![]),
        Err(JoinPlanError::MissingVariable(V(0)))
    );
    assert_eq!(
        FreeJoinPlan::new(vec![vec![V(0)], vec![V(1)]], vec![vec![V(0), V(1)]]),
        Err(JoinPlanError::UncoveredGroup(0))
    );
    assert_eq!(
        FreeJoinPlan::generic(vec![vec![V(0), V(0)]], vec![V(0)]),
        Err(JoinPlanError::DuplicateVariable(V(0)))
    );
    let touched = Cell::new(false);
    let rows = std::iter::once(vec![1_i64]).inspect(|_| touched.set(true));
    assert!(matches!(
        ColumnarTrie::new(&[V(0)], &[V(1)], rows, 0, &mut |_| Ok::<_, ()>(())),
        Err(TrieBuildError::Schema(_))
    ));
    assert!(!touched.get());
    assert!(matches!(
        ColumnarTrie::new(&[V(0)], &[V(0)], vec![vec![1_i64, 2]], 0, &mut |_| Ok::<
            _,
            (),
        >(
            ()
        )),
        Err(TrieBuildError::RowArity {
            row: 0,
            expected: 1,
            actual: 2
        })
    ...
    .unwrap();
    assert!(matches!(
        FreeJoin::new(&plan, &[wrong]),
        Err(JoinPlanError::AttributeOrder { relation: 0 })
    ));
}
...
fn every_execution_control_boundary_can_refuse_without_resumption() {
    ...
    for refuse in 0..boundaries {
        let mut observed = 0;
        let result = join.for_each_binding(
            &mut |_| {
                let at = observed;
                observed += 1;
...
fn huge_factored_multiplicity_is_checked_and_cancellable_not_wrapped() {
    ...
    FreeJoin::new(&plan, &data)
        .unwrap()
        .for_each_binding(&mut |_| Ok::<_, ()>(()), |binding, _| {
