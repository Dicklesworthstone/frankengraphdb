...
use fgdb_gql::algebra::{GlaDirection, GraphColumn, GraphPatternBuilder, GraphValueRow,
    IntegerComparison, PreparedGraphPattern, VertexPredicate};
use fgdb_gql::{GqlParameters, GqlQueryPolicy, GraphPatternTextErrorKind, GraphSymbol,
    GraphSymbolKind, GraphWalkBounds, MAX_GRAPH_WALK_HOPS,
...
fn predicate(vid: VId, predicates: &[VertexPredicate]) -> Result<bool, ()> {
    let labels = if vid == VId(1) || vid == VId(4) { vec![LabelId(3)] } else { vec![] };
...
fn quantified_text_has_the_same_plan_as_typed_walks_and_rebinds_without_resolving() {
    for (quantifier, minimum, maximum) in [("*0..3", 0, 3), ("*2", 2, 2), ("*..3", 1, 3)] {
        for (left, right, direction) in [("-", "->", GlaDirection::Forward),
            ("<-", "-", GlaDirection::Reverse), ("-", "-", GlaDirection::Undirected)] {
            let text = format!("MATCH WALK (a:L){left}[:R{quantifier}]{right}(b) \
            ...
            let arguments = GqlParameters::new().with_int64("floor", 5).unwrap()
            ...
            b.filter("a", VertexPredicate::HasLabel(LabelId(3))).unwrap();
            b.walk("a", RelationId(1), direction, "b", GraphWalkBounds::new(minimum, maximum).unwrap()).unwrap();
            b.filter("b", VertexPredicate::IntegerProperty {
                key: PropertyKeyId(4), comparison: IntegerComparison::GreaterOrEqual, value: 5,
            }).unwrap();
            let expected = b.prepare_values(&[GraphColumn::vertex("a", "a"),
                GraphColumn::property("score", "b", PropertyKeyId(4))], 1, Some(3)).unwrap().with_duplicates();
            assert_eq!(actual, expected, "{text}");
            let frozen = actual.canonical_bytes();
            let changed = GqlParameters::new().with_int64("floor", 6).unwrap()
                .with_uint64("off", 0).unwrap().with_uint64("take", 2).unwrap();
            assert_ne!(template.bind_parameters(&changed).unwrap().canonical_bytes(), frozen);
            assert_eq!(actual.canonical_bytes(), frozen);
            assert_eq!(calls.get(), 3);
        }
    }
    assert_eq!(prepare("MATCH (walk) RETURN walk"), prepare("MATCH WALK (walk) RETURN walk"));
}
...
fn unsafe_ambiguous_or_unbounded_quantifiers_refuse_before_any_catalog_call() {
    ...
    let unknown = PreparedGraphText::prepare("MATCH WALK (a)-[:Unknown*0]->(b) RETURN a", symbols).unwrap_err();
    assert_eq!(unknown.kind, GraphPatternTextErrorKind::UnknownSymbol(GraphSymbolKind::Relation));
    assert!(!format!("{unknown:?}").contains("Unknown*0"));
    let wrong = PreparedGraphText::prepare("MATCH WALK (a)-[:R*0]->(b) RETURN a", |_, _| {
...
fn text_walks_preserve_zero_hop_isolates_and_repeated_edge_occurrences() {
    ...
    let actual = zero.plan().execute_governed_with_properties(2, [VId(0), VId(u128::MAX)], [],
        |_, _| Ok::<_, ()>(true), |_, _| Ok(None), policy(), || Ok::<_, ()>(())).unwrap();
    let tuples: Vec<_> = actual.value.iter().map(|row| (
        row.values()[0].as_vertex().unwrap(), row.values()[1].as_vertex().unwrap(),
    )).collect();
    assert_eq!(tuples, vec![(VId(0), VId(0)), (VId(u128::MAX), VId(u128::MAX))]);
    ...
    for (tail, count) in [("a,b", 14), ("DISTINCT a,b", 1), ("a,b SKIP 2 LIMIT 3", 3)] {
        ...
        let rows = query.plan().execute_governed_with_properties(3, [VId(1)], edges,
            |_, _| Ok::<_, ()>(true), |_, _| Ok(None), policy(), || Ok::<_, ()>(())).unwrap();
        assert_eq!(rows.value.len(), count, "{tail}");
    }
    let maximal = prepare(&format!("MATCH WALK (a)-[:R*{MAX_GRAPH_WALK_HOPS}]->(a) RETURN a"));
    let rows = maximal.plan().execute_governed_with_properties(2, [VId(1)], [edges[0]],
        |_, _| Ok::<_, ()>(true), |_, _| Ok(None), policy(), || Ok::<_, ()>(())).unwrap();
...
fn scoped_walk_text_feeds_optional_aggregation_and_semijoin_without_multiplying_witnesses() {
    ...
    let template = PreparedGraphAggregateText::prepare(
        "MATCH (a:L) OPTIONAL MATCH WALK (a)-[:R*1..2]->(b) \
         ...
         HAVING matched IN [0,2,4] ORDER BY a", symbols,
    ).unwrap();
    let query = template.bind_parameters(&GqlParameters::new()).unwrap();
    let rows = query.execute_governed(7, vertices, edges, predicate, |_, _| Ok(None),
        policy(), || Ok::<_, ()>(())).unwrap().value;
    let values: Vec<_> = rows.iter().map(|row| (row.keys()[0].as_vertex().unwrap(),
        row.values()[0].as_count().unwrap(), row.values()[1].as_count().unwrap())).collect();
    assert_eq!(values, vec![(VId(1), 4, 4), (VId(4), 1, 0)]);
    for (quantifier, expected) in [("EXISTS", VId(1)), ("NOT EXISTS", VId(4))] {
        let query = prepare(&format!("MATCH (a:L) WHERE {quantifier} {{ \
            MATCH WALK (a)-[:R*2]->(b) }} RETURN a"));
        let rows = query.plan().execute_governed_with_properties(7, vertices, edges,
            predicate, |_, _| Ok(None), policy(), || Ok::<_, ()>(())).unwrap().value;
        assert_eq!(rows.len(), 1, "a witness resolves existence, not another output occurrence");
...
fn parameterized_endpoint_values_preserve_occurrences_across_multiple_walk_lengths() {
    let template = PreparedGraphText::prepare(
        "MATCH WALK (a)-[:R*0..2]->(b) WHERE b.n IN [$lo,$hi] RETURN b.n AS score", symbols,
    ).unwrap();
    let args = GqlParameters::new().with_int64("lo", 2).unwrap().with_int64("hi", 3).unwrap();
    let query = template.bind_parameters(&args).unwrap();
    let values = [CanonicalScalar::Int(1), CanonicalScalar::Int(2), CanonicalScalar::Int(3)];
    let result = query.plan().execute_governed_with_properties(5, [VId(1), VId(2), VId(3)],
        [(VId(1), RelationId(1), VId(2)), (VId(2), RelationId(1), VId(3))],
        |_, _| Ok::<_, ()>(true), |vid, _| Ok(Some(&values[vid.0 as usize - 1])),
        policy(), || Ok::<_, ()>(())).unwrap();
    let actual: Vec<_> = result.value.iter().map(|row| row.values()[0].as_scalar().unwrap().clone()).collect();
    assert_eq!(actual, vec![CanonicalScalar::Int(2), CanonicalScalar::Int(2),
        CanonicalScalar::Int(3), CanonicalScalar::Int(3), CanonicalScalar::Int(3)]);
}
