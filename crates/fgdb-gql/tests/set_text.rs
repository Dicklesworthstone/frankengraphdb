...
use fgdb_gql::{
    ...
    GraphSetTextErrorKind, GraphSymbol, GraphSymbolKind, PreparedGraphSet, PreparedGraphSetText,
    PreparedGraphText, MAX_GRAPH_SET_OPERANDS, MAX_GRAPH_SET_DEPTH, MAX_GRAPH_TEXT_BYTES,
};
...
fn leaf(text: &str) -> PreparedGraphSet {
    PreparedGraphText::prepare(text, symbols).unwrap().bind_parameters(&GqlParameters::new()).unwrap().into()
}
...
fn values() -> [Option<CanonicalScalar>; 6] {
    [Some(CanonicalScalar::Int(1)), Some(CanonicalScalar::Int(1)), Some(CanonicalScalar::Null),
        None, Some(CanonicalScalar::Int(2)), Some(CanonicalScalar::Int(1))]
}
fn run(query: &PreparedGraphSet) -> Vec<GraphValueRow> {
    let values = values();
    query.execute_governed(policy(), |pattern, remaining| {
        pattern.plan().execute_governed_with_properties(6, (1..=6).map(VId), [],
            |vid, predicates| {
                let labels = if vid.0 <= 2 { vec![LabelId(1)] }
                    else if vid.0 <= 4 { vec![LabelId(1), LabelId(2)] } else { vec![LabelId(2), LabelId(3)] };
                Ok::<_, ()>(predicates.iter().all(|p| p.matches(&labels, &[])))
            }, |vid, _| Ok(values[vid.0 as usize - 1].as_ref()), remaining, || Ok::<_, ()>(()))
    }, || Ok::<_, ()>(())).unwrap().value
}
fn integers(rows: &[GraphValueRow]) -> Vec<Option<i64>> {
    let mut values: Vec<_> = rows.iter().map(|row| match row.get(0).unwrap().as_scalar().unwrap() {
...
fn precedence_associativity_and_parentheses_match_the_exact_typed_definition() {
    ...
    let cases = [
        (format!("{A} UNION {B} INTERSECT ALL {C}"),
            a.clone().combine(Op::Union, Quant::Distinct, b.clone().combine(Op::Intersect, Quant::All, c.clone()).unwrap()).unwrap()),
        (format!("{A} EXCEPT ALL {B} UNION ALL {C}"),
            a.clone().combine(Op::Except, Quant::All, b.clone()).unwrap().combine(Op::Union, Quant::All, c.clone()).unwrap()),
        (format!("({A} UNION ALL {B}) INTERSECT {C}"),
            a.clone().combine(Op::Union, Quant::All, b.clone()).unwrap().nested().unwrap().combine(Op::Intersect, Quant::Distinct, c.clone()).unwrap()),
        (format!("{A} EXCEPT ({B} EXCEPT {C})"),
            a.clone().combine(Op::Except, Quant::Distinct, b.clone().combine(Op::Except, Quant::Distinct, c).unwrap().nested().unwrap()).unwrap()),
        (format!("{A} UNION DISTINCT {B}"), a.combine(Op::Union, Quant::Distinct, b).unwrap()),
    ];
    for (text, expected) in cases {
        assert_eq!(query(&text).canonical_bytes(), expected.canonical_bytes(), "{text}");
    }
}
...
fn final_pages_cover_the_whole_set_and_nested_ordering_does_not_replace_inner_selection() {
    ...
    let expected = leaf(A).combine(Op::Union, Quant::All, leaf(B)).unwrap()
    ...
    let expected = leaf("MATCH (n) RETURN n AS x").with_order_by(&[GraphValueOrder::descending(0)]).unwrap()
        .with_page(1, Some(3)).nested().unwrap().with_order_by(&[GraphValueOrder::ascending(0)]).unwrap().with_page(0, Some(1));
    assert_eq!(query(text), expected);
    assert_eq!(run(&query(text))[0].get(0).unwrap().as_vertex(), Some(VId(3)));
...
fn all_six_set_forms_obey_independent_multiset_arithmetic_including_null() {
    ...
    for (name, operation) in [("UNION", Op::Union), ("INTERSECT", Op::Intersect), ("EXCEPT", Op::Except)] {
        for (mode, all) in [("ALL", true), ("DISTINCT", false), ("", false)] {
            ...
            for value in [None, Some(1), Some(2)] {
                ...
                let (l, r) = if all { (l, r) } else { (usize::from(l > 0), usize::from(r > 0)) };
                let count = match operation {
                    Op::Union if !all => usize::from(l + r > 0),
                    Op::Union => l + r, Op::Intersect => l.min(r), Op::Except => l.saturating_sub(r),
                };
    ...
    let ids: Vec<_> = run(&tuples).iter().map(|r| r.get(0).unwrap().as_vertex().unwrap()).collect();
    assert_eq!(ids, vec![VId(3), VId(4)], "null scalar cells do not collapse different vertex cells");
}
...
fn parameters_and_symbols_share_one_contract_without_leaking_arm_variables() {
    ...
    assert_eq!(template.parameter_schema().iter().map(|p| (p.name.as_str(), p.occurrences)).collect::<Vec<_>>(),
        vec![("x", 3), ("lo", 1), ("page", 1)]);
    let args = GqlParameters::new().with_int64("x", 1).unwrap().with_int64("lo", i64::MIN).unwrap()
        .with_uint64("page", 10).unwrap();
    ...
    assert!(calls.values().all(|n| *n == 1));
    assert!(matches!(template.bind_parameters(&args.clone().with_int64("extra", 2).unwrap()).unwrap_err().kind,
    ...
    assert!(matches!(error.kind, GraphSetTextErrorKind::Pattern(GraphPatternTextErrorKind::ConflictingParameterTypes)));
    assert_eq!(calls.get(), 0);
}

#[test]
fn scalar_declarations_and_keyword_looking_literals_are_never_interpolated() {
    let text = "MATCH (a:A) WHERE a.p=$needle RETURN a.p AS value UNION MATCH (b:B) RETURN b.p AS other";
    let template = PreparedGraphSetText::prepare_with_parameter_types(text,
        &[("needle", GqlParameterType::Scalar(CanonicalScalarKind::Text))], symbols).unwrap();
    ...
    let expected = PreparedGraphText::prepare_with_parameter_types(
        ...
        &[("needle", GqlParameterType::Scalar(CanonicalScalarKind::Text))], symbols,
    ).unwrap().bind_parameters(&args).unwrap();
    let expected = PreparedGraphSet::from(expected).combine(Op::Union, Quant::Distinct, leaf(B)).unwrap();
    ...
    template.bind_parameters(&GqlParameters::new().with_null("needle").unwrap()).unwrap();
    assert!(matches!(template.bind_parameters(&GqlParameters::new().with_int64("needle", 1).unwrap()).unwrap_err().kind,
    ...
    assert_eq!(PreparedGraphSetText::prepare(quoted, symbols).unwrap().columns(), &["union"]);
}

#[test]
fn malformed_later_arms_schema_mismatches_and_bad_global_orders_do_not_touch_catalog() {
    for tail in [
        "UNION", "UNION ALL ALL MATCH (b) RETURN b.p", "UNION MATCH (b) RETURN missing",
        "UNION MATCH (b) RETURN b", "UNION MATCH (b) RETURN b.p,b AS extra",
        "UNION (MATCH (b) RETURN b.p", "UNION MATCH (b) WHERE b.p IN [1,] RETURN b.p",
        "UNION MATCH (b) RETURN b.p ORDER BY missing", "UNION MATCH (b) RETURN b.p ORDER BY value,value",
        "UNION MATCH (b) RETURN b.p LIMIT -1", "UNION MATCH (b) RETURN b.p LIMIT 18446744073709551616",
        "LIMIT 1 UNION MATCH (b) RETURN b.p", ";", "UNION MATCH (b) RETURN COUNT(*) AS value",
    ] {
        let calls = Cell::new(0);
        assert!(PreparedGraphSetText::prepare(&format!("{A} {tail}"), |kind, name| {
            calls.set(calls.get() + 1); symbols(kind, name)
        }).is_err(), "{tail}");
        assert_eq!(calls.get(), 0, "{tail}");
    }
    let calls = Cell::new(0);
    assert!(PreparedGraphSetText::prepare_with_parameter_types(A, &[("unused", GqlParameterType::Int64)],
...
fn errors_report_original_utf8_byte_offsets_and_redact_definitions() {
    ...
    let bad = "\u{2003}MATCH (a:A) RETURN a UNION MATCH (private_name:SecretLabel) RETURN private_name";
...
fn statement_wide_limits_and_utf8_prefixes_cannot_be_reset_per_arm() {
    assert!(matches!(PreparedGraphSetText::prepare(&" ".repeat(MAX_GRAPH_TEXT_BYTES + 1), symbols).unwrap_err().kind,
        GraphSetTextErrorKind::Pattern(GraphPatternTextErrorKind::DefinitionTooLarge)));
    let leaf = "MATCH (n) RETURN n";
    let longest = std::iter::repeat_n(leaf, MAX_GRAPH_SET_OPERANDS).collect::<Vec<_>>().join(" UNION ALL ");
    assert_eq!(query(&longest).operand_count(), MAX_GRAPH_SET_OPERANDS);
    assert!(matches!(PreparedGraphSetText::prepare(&format!("{longest} UNION {leaf}"), symbols).unwrap_err().kind,
        GraphSetTextErrorKind::SetBuild(GraphSetBuildError::TooManyOperands { .. })));
    let nested = format!("{}{leaf}{}", "(".repeat(MAX_GRAPH_SET_DEPTH - 1), ")".repeat(MAX_GRAPH_SET_DEPTH - 1));
    query(&nested);
    ...
    assert!(matches!(error.kind, GraphSetTextErrorKind::Pattern(GraphPatternTextErrorKind::TooManyTokens)));
...
fn compiled_compounds_share_exact_budgets_and_every_cancellation_checkpoint() {
    let query = query("(MATCH (n) RETURN n AS x ORDER BY x DESC LIMIT 3) \
    ...
    let execute = |policy, stop| {
        ...
        query.execute_governed(policy, |pattern, remaining| {
            pattern.plan().execute_governed_with_properties(5, (1..=5).map(VId), [],
                |_, _| Ok::<_, ()>(true), |_, _| Ok(None), remaining, checkpoint)
        }, checkpoint)
    };
    ...
    let exact = GqlQueryPolicy::new(10, 2, measured.evaluator.work_units, measured.evaluator.scratch_entries);
    assert_eq!(execute(exact, 0).unwrap(), measured);
    for stop in 1..=checkpoints {
        assert!(matches!(execute(policy(), stop), Err(GqlQueryError::Interrupted(at)) if at == stop));
        assert_eq!(seen.get(), stop);
    }
    for refused in [
        GqlQueryPolicy::new(9, 2, u64::MAX, u64::MAX), GqlQueryPolicy::new(10, 1, u64::MAX, u64::MAX),
