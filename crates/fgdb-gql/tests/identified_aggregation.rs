...
use fgdb_gql::{
    ...
    GraphSetProjection, GraphSetQuantifier, GraphSetValue, GraphWalkBounds, PreparedGraphAggregate,
    PreparedGraphSet,
};
...
static VALUES: [CanonicalScalar; 3] = [
...
fn matches(vid: VId, predicates: &[VertexPredicate]) -> Result<bool, &'static str> {
    Ok(predicates
        .iter()
        .all(|p| p.matches(&[], &[(P, VALUES[vid.0 as usize - 1].clone())])))
}
...
) -> fgdb_gql::GqlQueryExecution<GraphAggregateRow> {
    query
...
fn summary(input: PreparedGraphPattern<GraphValueRow>) -> PreparedGraphAggregate {
    PreparedGraphAggregate::prepare(
        ...
        &[],
        &[
            GraphAggregate::count_rows("occurrences"),
            GraphAggregate::count("nonnull", 2),
            GraphAggregate::count_distinct("routes", 2),
            GraphAggregate::sum_int("hops", 3),
            GraphAggregate::sum_int_distinct("lengths", 3),
            GraphAggregate::average_int("mean", 3),
            GraphAggregate::average_int_distinct("unique_mean", 3),
...
fn parallel_edge_identity_survives_all_nine_aggregate_functions() {
    ...
    assert_eq!(
        values[7].as_value().unwrap().as_path().unwrap().steps(),
        &[(EId(11), VId(2))]
    );
    assert_eq!(
        values[8].as_value().unwrap().as_path().unwrap().steps(),
        &[(EId(14), VId(3))]
    );
    assert_eq!(measured.rows.snapshot_records, 7);
    assert_eq!(measured.rows.result_rows, 1);
    let arrays = PreparedGraphAggregate::prepare(
        input(true, 1),
        &[],
        &[
            GraphAggregate::count_distinct("vertices", 4),
            GraphAggregate::count_distinct("edges", 5),
        ],
        0,
        None,
    )
...
fn captured_group_keys_hidden_outputs_and_ranking_keep_exact_route_identity() {
    let query = PreparedGraphAggregate::prepare(
        ...
        &[
            ...
            GraphAggregate::sum_int("length", 3),
        ],
        1,
        Some(2),
    )
...
fn projected_and_completed_relational_inputs_share_identified_summaries() {
    ...
    let projected = PreparedGraphAggregate::prepare_projected(
        input(true, 1),
        projection.clone(),
        &[],
        &[
            GraphAggregate::count_distinct("routes", 0),
            GraphAggregate::sum_int("hops", 1),
        ],
        0,
        None,
    )
    .unwrap();
    let relation = PreparedGraphSet::from(input(true, 1))
        .project(projection, GraphSetQuantifier::All)
        .unwrap();
    let full = PreparedGraphAggregate::prepare_relation(
        relation.clone(),
        &[],
        &[
            GraphAggregate::count_distinct("routes", 0),
    ...
    let page = PreparedGraphAggregate::prepare_relation(
        relation.with_page(1, Some(2)),
        &[],
        &[
            GraphAggregate::count_distinct("routes", 0),
            GraphAggregate::sum_int("hops", 1),
...
fn ordinary_inputs_keep_their_existing_counters_and_transcripts() {
    ...
    let missing_ids = summary(input(true, 1)).execute_governed(
        7,
        VERTICES,
        EDGES.map(|(_, s, r, d)| (s, r, d)),
        matches,
...
fn one_meter_covers_identified_input_projection_groups_and_every_checkpoint() {
    for relational in [false, true] {
        ...
        for stop in 1..=events.get() {
            let mut seen = 0;
            let failed = query.execute_governed_with_identified_properties(
                7,
                VERTICES,
                EDGES,
                matches,
    ...
    for relational in [false, true] {
        ...
        assert!(matches!(
            ...
            Err(GqlQueryError::Source(GraphAggregateError::Source(
                ...
                GraphSetExecutionError::Source("unreadable")
            )))
        ));
    }
}
...
fn empty_identified_source_keeps_keyless_zero_and_keyed_absence() {
    for keys in [vec![], vec![2]] {
        let query = PreparedGraphAggregate::prepare(
            ...
            &[
                GraphAggregate::count_rows("n"),
                GraphAggregate::min("route_min", 2),
            ],
            0,
            None,
        )
