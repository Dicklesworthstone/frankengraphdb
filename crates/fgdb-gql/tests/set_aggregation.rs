...
use fgdb_gql::{
    GqlQueryError, GqlQueryExecution, GqlQueryPolicy, GqlScalarParameter, GraphAggregate,
    GraphAggregateBuildError, GraphAggregateColumn, GraphAggregateError, GraphAggregateOrder,
    GraphAggregateRow, GraphExactAverage, GraphHavingExpression, GraphHavingOp, GraphHavingOperand,
    GraphSetExecutionError, GraphSetOperation, GraphSetProjection, GraphSetQuantifier,
    GraphSetValue, MAX_GRAPH_SET_DEPTH, PreparedGraphAggregate, PreparedGraphSet,
...
fn leaf(label: LabelId) -> PreparedGraphPattern<GraphValueRow> {
    ...
    builder
        ...
        .unwrap();
    builder
        .prepare_values(&[GraphColumn::property("x", "n", P)], 0, None)
        .unwrap()
        .with_duplicates()
}
fn compound(operation: GraphSetOperation, quantifier: GraphSetQuantifier) -> PreparedGraphSet {
    PreparedGraphSet::from(leaf(A))
        .combine(operation, quantifier, leaf(B).into())
        .unwrap()
}
...
fn source<C>(
    ...
    pattern.plan().execute_governed_with_properties(
        data.len() as u64,
        (0..data.len()).map(|i| VId(i as u128)),
        [],
        |vid, tests| {
            let (label, value) = &data[vid.0 as usize];
            Ok(tests
                .iter()
                .all(|test| test.matches(&[*label], &[(P, value.clone())])))
        },
        |vid, _| Ok(Some(&data[vid.0 as usize].1)),
        policy,
        checkpoint,
    )
}
...
fn assert_summary(row: &GraphAggregateRow, expected: &[CanonicalScalar]) {
    ...
    if integers.is_empty() {
        ...
        assert_eq!(
            cells[4].as_value().unwrap().as_scalar(),
            Some(&CanonicalScalar::Int(*integers.iter().min().unwrap()))
        );
        assert_eq!(
            cells[5].as_value().unwrap().as_scalar(),
            Some(&CanonicalScalar::Int(*integers.iter().max().unwrap()))
        );
        assert_eq!(cells[6].as_integer(), Some(unique_sum));
        assert_eq!(
            cells[7].as_average(),
            GraphExactAverage::new(sum, integers.len() as u64)
        );
        assert_eq!(
            cells[8].as_average(),
            GraphExactAverage::new(unique_sum, unique.len() as u64)
        );
    }
}
            ...
            for quantifier in [GraphSetQuantifier::All, GraphSetQuantifier::Distinct] {
                ...
                for (value, &[left, right]) in &counts {
                    let multiplicity = match (operation, quantifier) {
                        ...
                        (GraphSetOperation::Except, GraphSetQuantifier::All) => {
...
fn final_relation_schema_and_exact_numeric_domains_do_not_come_from_the_first_leaf() {
    ...
    let input = compound(GraphSetOperation::Union, GraphSetQuantifier::All)
        .project(
            vec![
                ...
                GraphSetProjection::new("bucket", GraphSetValue::Literal(value)),
            ],
    ...
    .unwrap();
    let data = [
        (A, CanonicalScalar::Int(i64::MAX)),
        (B, CanonicalScalar::Int(i64::MAX)),
...
fn grouped_results_reuse_having_hidden_keys_order_distinct_and_output_prefix() {
    ...
    let having = GraphHavingExpression::prepare(&[GraphHavingOp::Compare {
        ...
        right: GraphHavingOperand::Integer(2),
    }])
    .unwrap();
    let hidden = query
        .clone()
    ...
    let data = [
        (A, CanonicalScalar::Int(1)),
        (A, CanonicalScalar::Int(2)),
        (B, CanonicalScalar::Int(1)),
        (B, CanonicalScalar::Int(2)),
    ];
...
fn all_sources_and_grouping_share_exact_caps_and_every_cancellation_checkpoint() {
    ...
    assert_eq!(
        caps[1], 1,
        "intermediate rows are not public aggregate rows"
    );
    assert_eq!(
        run(
            &query,
    ...
    for dimension in 0..4 {
        ...
        match error {
            ...
            GqlQueryError::Evaluator(error) if dimension >= 2 => {
                assert_eq!(error.limit, cap[dimension])
            }
...
fn empty_left_and_zero_group_limit_do_not_mask_a_later_source_failure() {
    for operation in [
        ...
        let query = PreparedGraphSetAggregate::prepare(
            compound(operation, GraphSetQuantifier::All),
            &[],
            &[GraphAggregate::count_rows("n")],
            0,
            Some(0),
        ...
        let result = query.execute_governed(
            ...
            |pattern, allowance| {
                calls += 1;
                if calls == 2 {
                    return Err(GqlQueryError::Source("second graph failed"));
                }
        ...
        let error = match result {
            Err(GqlQueryError::Source(error)) => error,
            other => panic!("{other:?}"),
        };
        assert!(matches!(
            error.map_source(|message| message.len()),
...
fn late_source_statistics_and_schema_refusals_keep_the_operand_index() {
    ...
    for schema_error in [false, true] {
        ...
        let result = query.execute_governed(
            ...
            |pattern, remaining| {
                ...
                if calls == 2 && schema_error {
                    ...
                    builder.vertex("n").unwrap();
                    let wrong = builder
                        .prepare_values(&[GraphColumn::vertex("x", "n")], 0, None)
                        .unwrap();
        ...
        assert_eq!(calls, 2);
        assert!(
            matches!(result, Err(GqlQueryError::Source(GraphAggregateError::InputRelation(error)))
...
fn single_source_contract_transcripts_and_definition_bounds_remain_intact() {
    ...
    let old =
        PreparedGraphAggregate::prepare_relation(input.clone(), &[], &aggregates, 0, None).unwrap();
    ...
    let ordinary = old
        .execute_governed(
            ...
            [],
            |_, _| Ok::<_, &str>(true),
            |_, _| Ok(Some(&data[0].1)),
            wide(),
            || Ok::<_, ()>(()),
        )
        .unwrap();
    assert_eq!(
        run(&new, &data, wide(), || Ok::<_, ()>(())).unwrap(),
        ordinary
    );
    let binary = compound(GraphSetOperation::Union, GraphSetQuantifier::All);
    assert!(matches!(
        PreparedGraphAggregate::prepare_relation(binary.clone(), &[], &aggregates, 0, None),
    ...
    assert!(matches!(
        PreparedGraphSetAggregate::prepare(input.clone(), &[1], &aggregates, 0, None),
    ...
    assert!(matches!(
        PreparedGraphSetAggregate::prepare(deepest, &[], &aggregates, 0, None),
...
fn input_ranking_and_group_ranking_are_separate_and_keyed_empty_sets_stay_empty() {
    ...
    let data = [
        (A, CanonicalScalar::Int(1)),
        (A, CanonicalScalar::Int(5)),
        (B, CanonicalScalar::Int(9)),
    ];
    let rows = run(&query, &data, wide(), || Ok::<_, ()>(()))
        .unwrap()
        .value;
    ...
    let empty = PreparedGraphSetAggregate::prepare(
        compound(GraphSetOperation::Except, GraphSetQuantifier::Distinct),
        &[0],
        &[GraphAggregate::count_rows("n")],
        0,
        None,
