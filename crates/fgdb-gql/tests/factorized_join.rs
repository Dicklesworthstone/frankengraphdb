...
use fgdb_gql::free_join::{
    ColumnarTrie, FactorNodeKind, FactorizedBatch, FactorizedReadError,
    FreeJoin, FreeJoinPlan, JoinVariable as V, MultiplicityOverflow,
};

fn inputs(plan: &FreeJoinPlan, schemas: &[Vec<V>], data: &[Vec<Vec<i64>>]) -> Vec<ColumnarTrie<i64>> {
    schemas.iter().zip(data).enumerate().map(|(at, (schema, rows))| {
        ColumnarTrie::new(schema, plan.required_order(at).unwrap(), rows.clone(), at as u64 + 40, &mut |_| Ok::<_, ()>(())).unwrap()
    }).collect()
}
...
fn independent_star_branches_are_products_not_flat_cartesian_intermediates() {
    ...
    let batch = FreeJoin::new(&plan, &tries).unwrap().factorize(&mut |_| Ok::<_, ()>(())).unwrap();
    ...
    assert!(batch.node_kinds().any(|kind| kind == FactorNodeKind::Product));
    assert_eq!(batch.generations(), &[40, 41, 42]);
    let mut cursor = batch.cursor().unwrap();
    cursor.seek((n * n * n - 2) as u128);
    let mut work = 0;
    let last = cursor.next_batch(10, &mut |event| {
        if event == GlaExecutionEvent::Work { work += 1; }
        Ok::<_, ()>(())
    }).unwrap();
    assert_eq!(last.row_count(), 2);
    assert_eq!(last.columns(), &[vec![1, 1], vec![199, 199], vec![199, 199], vec![198, 199]]);
    assert!(work < 200, "rank seek must not traverse eight million occurrences");
...
fn factorization_matches_generic_join_bags_for_cycles_and_interleaved_components() {
    let cases = [
        (vec![vec![V(0), V(1)], vec![V(1), V(2)], vec![V(2), V(0)]], vec![
        ...
        (vec![vec![V(0), V(2)], vec![V(1), V(3)]], vec![
            vec![vec![1, 3], vec![1, 3], vec![2, 4]],
            vec![vec![5, 7], vec![6, 8]],
        ], vec![V(0), V(1), V(2), V(3)]),
        (vec![vec![], vec![V(0)], vec![V(1)]], vec![
            vec![vec![], vec![]], vec![vec![1], vec![1], vec![2]], vec![vec![3], vec![4]],
        ], vec![V(0), V(1)]),
    ];
    for (schemas, rows, order) in cases {
        ...
        join.for_each_binding(&mut |_| Ok::<_, ()>(()), |binding, _| {
            for _ in 0..binding.multiplicity().unwrap() { expected.push(binding.values().copied().collect::<Vec<_>>()); }
            Ok(())
        }).unwrap();
        expected.sort();
        ...
        assert_eq!(flattened(&batch, 1), flattened(&batch, 7), "batch boundaries cannot change occurrence order");
    }
}
...
fn every_factor_build_and_batch_boundary_refuses_without_partial_success() {
    ...
    let tries = inputs(&plan, &schemas, &[vec![vec![1, 2], vec![1, 3]], vec![vec![1, 4], vec![1, 5]]]);
    let join = FreeJoin::new(&plan, &tries).unwrap();
    let mut total = 0;
    let batch = join.factorize(&mut |_| { total += 1; Ok::<_, usize>(()) }).unwrap();
    ...
    let mut total = 0;
    batch.cursor().unwrap().next_batch(4, &mut |_| { total += 1; Ok::<_, usize>(()) }).unwrap();
    for refuse in 0..total {
        let mut cursor = batch.cursor().unwrap();
        let mut observed = 0;
        ...
        assert_eq!(observed, refuse + 1);
        assert_eq!(cursor.position(), 0, "failed batch cannot advance committed cursor position");
        assert!(cursor.is_failed());
        assert!(matches!(cursor.next_batch(1, &mut |_| Ok::<_, usize>(())), Err(FactorizedReadError::Failed)));
    }
}
...
fn nullary_units_empty_products_and_overflow_have_distinct_semantics() {
    ...
    let unit = FreeJoin::new(&plan, &data).unwrap().factorize(&mut |_| Ok::<_, ()>(())).unwrap();
    ...
    let tries = inputs(&plan, &schemas, &rows);
    let overflow = FreeJoin::new(&plan, &tries).unwrap().factorize(&mut |_| Ok::<_, ()>(())).unwrap();
    assert_eq!(overflow.cardinality(), Err(MultiplicityOverflow));
    assert!(matches!(overflow.cursor(), Err(MultiplicityOverflow)));
    rows[49].clear();
    let tries = inputs(&plan, &schemas, &rows);
    let empty = FreeJoin::new(&plan, &tries).unwrap().factorize(&mut |_| Ok::<_, ()>(())).unwrap();
    assert_eq!(empty.cardinality(), Ok(0), "zero annihilates an overflowing product");
