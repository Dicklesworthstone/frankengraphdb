...
use fgdb_gql::{GqlParameterType, GqlParameters, GraphSymbol, GraphSymbolKind,
    GraphWriteProgramTemplateError, GraphWriteScriptBatchError, GraphWriteScriptErrorKind,
    GraphWriteStatement, GraphWriteTemplateStatement, PreparedGraphDeleteText,
    PreparedGraphInsertText, PreparedGraphWriteProgram, PreparedGraphWriteProgramTemplate,
...
fn native_delete_shares_catalog_and_parameters_and_matches_explicit_composition() {
    ...
    assert!(matches!(&script.statements()[1], GraphWriteTemplateStatement::Delete(_)));
    ...
    let deletion = PreparedGraphDeleteText::prepare("MATCH (n) WHERE n.p=$key DELETE n", R, symbols).unwrap();
    let explicit = PreparedGraphWriteProgramTemplate::prepare(vec![creation.into(), deletion.into()]).unwrap();
    assert_eq!(bound, explicit.bind_parameters(&args).unwrap());
    assert!(matches!(&bound.statements()[1], GraphWriteStatement::Delete(_)));
    assert_eq!(calls.get(), 1, "binding cannot re-enter the catalog");
    let detach = PreparedGraphWriteScript::prepare("MATCH (n) WHERE n.p=$key DETACH DELETE n", R, symbols)
...
fn delete_binding_errors_retain_original_utf8_offset_and_statement_index() {
    ...
    for args in [GqlParameters::new(), GqlParameters::new().with_int64("value", 9).unwrap()] {
        let error = script.bind_parameters(&args).unwrap_err();
        assert_eq!(error.statement, Some(1));
        assert_eq!(error.offset, text.find('$').unwrap());
        assert!(matches!(error.kind, GraphWriteScriptErrorKind::Program(
            GraphWriteProgramTemplateError::DeleteBind { statement: 1, .. })));
    }
    assert!(script.bind_parameters(&GqlParameters::new().with_text("value", "λ").unwrap()).is_ok());
}

#[test]
fn malformed_delete_is_not_reinterpreted_as_detach_or_a_read() {
    for text in ["CREATE (n);MATCH (n) DELETE missing", "MATCH (n) DELETE n,n",
        "MATCH (n) DELETE n.p", "MATCH (n) DELETE n RETURN n"] {
        assert!(PreparedGraphWriteScript::prepare(text, R, symbols).is_err(), "{text}");
    }
...
fn delete_parameter_batches_keep_record_mapping_and_reject_a_late_bad_record() {
    ...
    let batch = script.bind_parameter_sets(&[values.clone(), values.clone()]).unwrap();
    assert!(matches!(&batch.program().statements()[2], GraphWriteStatement::Delete(_)));
    ...
    let error = script.bind_parameter_sets(&[values, GqlParameters::new()]).unwrap_err();
    assert!(matches!(error, GraphWriteScriptBatchError::Arguments { argument_set: 1, source }
