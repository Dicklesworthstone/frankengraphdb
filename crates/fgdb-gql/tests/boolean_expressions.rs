...
use fgdb_gql::algebra::{
    ...
    GraphColumn, GraphMatchClause, GraphPatternBuilder, GraphValueRow, IntegerComparison as Cmp,
...
fn columns() -> [GraphColumn<'static>; 2] {
    ...
    for left in &choices {
        for right in &choices {
            for mask in 0..16 {
                ...
                for &(a, _, b) in &edges {
                    let null = values[b.0 as usize]
                        .as_ref()
                        .is_none_or(|v| matches!(v, CanonicalScalar::Null));
                    let less = match (&values[a.0 as usize], &values[b.0 as usize]) {
                        (Some(CanonicalScalar::Int(a)), Some(CanonicalScalar::Int(b))) => a < b,
                        _ => false,
                    };
                    if null || less {
                        expected.push((a, b));
                ...
                expected.sort();
                assert_eq!(run(&query, &values, &edges), expected);
            }
        }
    }
}
...
fn scope_remapping_preserves_optional_and_existence_witnesses_and_aggregation() {
    ...
    let query = outer
        .prepare_values_with_clauses(&[GraphMatchClause::optional(&child)], &columns(), 0, None)
        .unwrap()
        .with_duplicates();
    let rows = query
        .plan()
    ...
    assert_eq!(rows[0].get(1).unwrap().as_vertex(), Some(VId(1)));
    ...
    for anti in [false, true] {
        ...
        let query = outer
            .prepare_values_with_clauses(&[clause], &columns()[..1], 0, None)
            .unwrap();
        let rows = query
            .plan()
            .execute_governed_with_properties(
        ...
        assert_eq!(
            rows[0].get(0).unwrap().as_vertex(),
            Some(VId(if anti { 3 } else { 0 }))
        );
    }
}
...
fn invalid_attachment_leaves_builder_unchanged_and_requires_a_value_source() {
    let mut b = builder(&["a"]);
    let original = b.prepare_values(&columns()[..1], 0, None).unwrap();
    assert_eq!(
        b.filter_boolean(&condition("a", "missing")).unwrap_err(),
        PatternBuildError::UnknownVariable
    );
    assert_eq!(
        b.prepare_values(&columns()[..1], 0, None).unwrap(),
        original
    );
    b.filter_boolean(&GraphBooleanExpression::prepare(&[Op::Truth(Some(true))]).unwrap())
        .unwrap();
    assert_eq!(
    ...
    assert!(matches!(
        b.filter_boolean(&GraphBooleanExpression::prepare(&[Op::Truth(None)]).unwrap()),
...
fn expression_execution_shares_limits_and_never_erases_source_or_interrupt_errors() {
    ...
    b.filter_boolean(&condition("a", "b")).unwrap();
    let query = b
        .prepare_values(&columns(), 0, Some(1))
        .unwrap()
        .with_duplicates();
