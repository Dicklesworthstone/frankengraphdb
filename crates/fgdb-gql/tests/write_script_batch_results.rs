...
fn batch() -> fgdb_gql::BoundGraphWriteScriptBatch {
    ...
    .unwrap();
    script
        .bind_parameter_sets_with_limit(&vec![GqlParameters::new(); 80], 160)
        .unwrap()
}
...
fn each_statement_error_family_keeps_exact_record_coordinates_and_original_source() {
    ...
    let errors: Vec<ProgramError> = vec![
        W::Insert {
            statement: 137,
            source: GqlQueryError::Interrupted("insert"),
        },
        W::VertexMerge {
            statement: 137,
            source: GqlQueryError::Interrupted("vertex merge"),
        },
        W::VertexUpsert {
            statement: 137,
            source: GqlQueryError::Interrupted("vertex upsert"),
        },
        W::EdgeMerge {
            statement: 137,
            source: GqlQueryError::Interrupted("edge merge"),
        },
        W::EdgeUpsert {
            statement: 137,
            source: GqlQueryError::Interrupted("edge upsert"),
        },
        W::Delete {
            statement: 137,
            source: GqlQueryError::Interrupted("delete"),
        },
        W::CreationBudget {
            statement: 137,
        ...
        W::Program(M::Statement {
            statement: 137,
            source: GqlQueryError::Interrupted("mutation"),
        }),
        W::Program(M::Budget {
            statement: 137,
...
fn infrastructure_and_final_acceptance_are_not_assigned_a_fabricated_record() {
    ...
    for original in errors {
        let expected = original.to_string();
        let GraphWriteScriptExecutionError::BatchProgram { location, source } =
            batch.execution_error(original)
        else {
            panic!("wrong error family")
        };
...
fn record_receipts_require_a_complete_shape_and_preserve_order_without_copying() {
    ...
    assert!(core::ptr::eq(
        last.as_ptr(),
        receipt.steps()[158..].as_ptr()
    ));
    assert_eq!(
        last[0].created_vertices(),
        Some(&[VId(u128::MAX - 158)][..])
    );
    assert_eq!(
        last[1].mutation_targets(),
        Some(&[VId(u128::MAX - 159)][..])
    );
...
fn source_chain_preserves_the_original_infrastructure_error() {
    ...
    let original: GraphWriteProgramError<std::io::Error, std::io::Error, std::io::Error> =
        GraphMutationProgramError::Preflight(std::io::Error::other("storage outcome unavailable"))
            .into();
    let mapped = batch().execution_error(original);
