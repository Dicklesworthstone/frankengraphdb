...
fn one_byte_feeds_and_coalesced_frames_do_not_read_ahead() {
    ...
    for (index, byte) in bytes.iter().enumerate() {
        ...
        if index + 1 == bytes.len() {
            assert_eq!(progress.frame, Some(original.clone()));
        } else {
    ...
    let second = decoder
        .decode(&coalesced[first.consumed..], |_| Ok(()))
        .unwrap();
...
fn oversized_length_is_rejected_from_four_bytes_without_header_callback() {
    let mut decoder = Decoder::new(limits());
    assert_eq!(
        decoder.decode(&4097u32.to_be_bytes(), |_| panic!("must not authorize")),
...
fn stale_binding_is_rejected_before_payload_reservation() {
    ...
    let mut decoder = Decoder::new(limits());
    assert_eq!(
        decoder.decode(&bytes, |header| connection.validate_client_header(header)),
...
fn handshake_separates_mechanics_authentication_and_selection() {
    ...
    assert_eq!(
        connection.selected(ready()),
        Err(ProtocolError::InvalidState)
    );
    assert_eq!(
        connection.authenticated(session()),
        Err(ProtocolError::InvalidState)
    );
    connection.negotiated().unwrap();
    assert_eq!(
        connection.validate_client_header(hello.header()),
        Err(ProtocolError::InvalidState)
    );
    connection.authenticated(session()).unwrap();
    let select = frame(
    ...
    let mut other = ready();
    other.session.transcript[0] ^= 1;
    assert_eq!(
        connection.selected(other),
        Err(ProtocolError::InvalidBinding)
    );
...
fn authority_refresh_changes_only_generation_and_fences_prior_headers() {
    ...
    assert_eq!(
        connection.validate_client_header(old.header()),
        Err(ProtocolError::InvalidBinding)
    );
...
fn drain_rejects_new_children_and_waits_for_typed_handoff_and_sends() {
    ...
    connection.begin_drain().unwrap();
    assert_eq!(
        connection.admit_child(StreamId([2; 16]), ChildKind::Query),
        Err(ProtocolError::InvalidState)
    );
    assert_eq!(
        connection.complete_drain(),
        Err(ProtocolError::DrainIncomplete)
    );
    assert_eq!(
        connection.child_terminal(stream, generation + 1, ChildTerminus::ResultDurablyDetached),
        Err(ProtocolError::InvalidStream)
    );
    assert_eq!(
        connection.child_terminal(
            stream,
            generation,
            ChildTerminus::TransactionOwnershipDetached
        ),
        Err(ProtocolError::InvalidState)
    );
    connection
        .child_terminal(stream, generation, ChildTerminus::ResultDurablyDetached)
        .unwrap();
    assert_eq!(
        connection.complete_drain(),
        Err(ProtocolError::DrainIncomplete)
...
fn stale_child_completion_cannot_remove_a_reused_stream() {
    ...
    connection
        .child_terminal(id, first, ChildTerminus::SemanticTerminalDurable)
        .unwrap();
    let second = connection.admit_child(id, ChildKind::Query).unwrap();
    assert!(second > first);
    assert_eq!(
        connection.child_terminal(id, first, ChildTerminus::SemanticTerminalDurable),
        Err(ProtocolError::InvalidStream)
    );
...
fn admission_bounds_and_server_frame_direction_are_checked() {
    ...
    for byte in 1..=2 {
        connection
            .admit_child(StreamId([byte; 16]), ChildKind::Query)
            .unwrap();
    }
    assert_eq!(
        connection.admit_child(StreamId([3; 16]), ChildKind::Query),
        Err(ProtocolError::StreamLimit)
    );
    for _ in 0..4 {
        connection.queue_send().unwrap();
    }
    assert!(matches!(
        connection.queue_send(),
        Err(ProtocolError::SendLimit)
    ));
    let spoof = frame(
        FrameKind::ResultEnd,
        connection.binding(),
    ...
    assert_eq!(
        connection.validate_client_header(spoof.header()),
        Err(ProtocolError::InvalidState)
    );
}
...
fn duplicate_credit_updates_do_not_inflate_either_dimension() {
    ...
    assert_eq!(
        window.grant(CreditUpdate { bytes: 21, ..first }),
        Err(ProtocolError::InvalidCreditUpdate)
    );
    assert_eq!(
        window.grant(CreditUpdate {
            sequence: 3,
            ..first
        }),
        Err(ProtocolError::InvalidCreditUpdate)
...
fn empty_frames_complete_and_zero_row_control_still_charges_bytes() {
    ...
    assert_eq!(
        decoder
            .decode(&original.encode(limits()).unwrap(), |_| Ok(()))
            .unwrap()
            .frame,
        Some(original)
    );
    let mut window = FlowWindow::new(
        SendCost { bytes: 10, rows: 0 },
        SendCost { bytes: 10, rows: 0 },
    )
    .unwrap();
...
fn duplicate_or_foreign_send_completion_cannot_discharge_another_obligation() {
    ...
    assert_eq!(
        first.send_terminal(&foreign, SendTerminus::Sent),
        Err(ProtocolError::InvalidState)
    );
    assert_eq!(first.sends_in_flight(), 2);
    first.send_terminal(&one, SendTerminus::Sent).unwrap();
    assert_eq!(
        first.send_terminal(&one, SendTerminus::Sent),
        Err(ProtocolError::InvalidState)
    );
