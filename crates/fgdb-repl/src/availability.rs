...
pub enum StorageSets {
    Stable(Vec<MemberId>),
    Joint { old: Vec<MemberId>, new: Vec<MemberId> },
}

#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum StorageSide { Stable, Old, New }
...
impl Default for AvailabilityLimits {
    fn default() -> Self {
        Self { max_requirements: 1024, max_source_blocks: 4096, max_spans: 65_536,
            max_policy_items: 16_384, max_failure_cases: 4096, max_work: 4_000_000 }
    }
}
...
impl SystematicAssessment<'_> {
    pub fn input(&self) -> &AvailabilityInput { self.input }
    pub fn checked_failure_cases(&self) -> u64 { self.checked_failure_cases }
    pub fn work(&self) -> u64 { self.work }
}

struct Work<'a, F> { used: u64, limit: u64, checkpoint: &'a mut F }
impl<E, F: FnMut() -> Result<(), E>> Work<'_, F> {
    fn step(&mut self) -> Result<(), AvailabilityError<E>> {
        if self.used >= self.limit { return Err(AvailabilityError::WorkBudget); }
        if self.used % 128 == 0 {
...
fn reserve<T, E>(vec: &mut Vec<T>, count: usize) -> Result<(), AvailabilityError<E>> {
    vec.try_reserve_exact(count).map_err(|_| AvailabilityError::AllocationFailed)
}
...
fn cut_count(n: usize, k: usize) -> u128 {
    ...
    for i in 0..k { count = count * (n - i) as u128 / (i + 1) as u128; }
    count
}

/// Increasing fixed-cardinality masks, including the unique empty cut.
struct Cuts { n: usize, indices: Vec<usize>, done: bool }
impl Cuts {
    fn new(n: usize, k: usize) -> Self { Self { n, indices: (0..k).collect(), done: false } }
}
...
struct Block { requirement: usize, ordinal: u32, symbols: u32 }
...
pub fn assess_systematic<'a, E, F: FnMut() -> Result<(), E>>(
    ...
    if limits.max_requirements == 0 || limits.max_requirements > 65_536
        || limits.max_source_blocks == 0 || limits.max_source_blocks > 65_536
        || limits.max_spans == 0 || limits.max_spans > 1_048_576
        || limits.max_policy_items == 0 || limits.max_policy_items > 65_536
        || limits.max_failure_cases == 0 || limits.max_failure_cases > 65_536
        || limits.max_work == 0
    { return Err(AvailabilityError::InvalidLimits); }
    let policy = &input.policy;
    let groups = storage_groups(&policy.storage_sets);
    if input.requirements.is_empty() || input.requirements.len() > limits.max_requirements
        || input.receipts.len() > 1024 || policy.locations.is_empty()
        || policy.locations.len() > 1024
    { return Err(AvailabilityError::InputBudget); }
    let mut work = Work { used: 0, limit: limits.max_work, checkpoint };
    ...
    for (_, members) in &groups {
        work.step()?;
        policy_items = policy_items.checked_add(members.len()).ok_or(AvailabilityError::InputBudget)?;
    }
    for location in &policy.locations {
        work.step()?;
        policy_items = policy_items.checked_add(location.failure_domains.len())
            .ok_or(AvailabilityError::InputBudget)?;
    }
    for requirement in &input.requirements {
        work.step()?;
        block_count = block_count.checked_add(requirement.source_symbols.len())
            .ok_or(AvailabilityError::InputBudget)?;
    }
    for receipt in &input.receipts {
        work.step()?;
        span_count = span_count.checked_add(receipt.coverage.len()).ok_or(AvailabilityError::InputBudget)?;
    }
    if policy_items > limits.max_policy_items || block_count > limits.max_source_blocks
        || span_count > limits.max_spans
    { return Err(AvailabilityError::InputBudget); }
    for (_, members) in &groups {
        if members.is_empty() || members.len() > 1024 || !strictly_sorted(members)
    ...
    for location in &policy.locations {
        ...
        if prior_placement.is_some_and(|prior| prior >= location.placement_id)
            || !groups.iter().any(|(_, members)| members.binary_search(&location.member).is_ok())
            || location.failure_domains.is_empty() || !strictly_sorted(&location.failure_domains)
        { return Err(AvailabilityError::InvalidPolicy); }
        prior_placement = Some(location.placement_id);
        for domain in &location.failure_domains {
            work.step()?;
            domains.insert(*domain);
            if domains.len() > 64 { return Err(AvailabilityError::InvalidPolicy); }
        }
    }
    let domains: Vec<_> = domains.into_iter().collect();
    let f = policy.tolerated_domain_failures;
    if f >= domains.len() { return Err(AvailabilityError::InvalidPolicy); }
    let cases = cut_count(domains.len(), f) * groups.len() as u128;
    if cases > u128::from(limits.max_failure_cases) { return Err(AvailabilityError::FailureCaseBudget); }
    ...
    for location in &policy.locations {
        ...
        for domain in &location.failure_domains {
            work.step()?;
            let bit = domains.binary_search(domain).map_err(|_| AvailabilityError::InvalidPolicy)?;
            mask |= 1_u64 << bit;
        }
    ...
    for (r, requirement) in input.requirements.iter().enumerate() {
        work.step()?;
        if requirement.source_symbols.is_empty() || requirement.source_symbols.len() > 256
        ...
        for (ordinal, symbols) in requirement.source_symbols.iter().copied().enumerate() {
            work.step()?;
            if symbols == 0 || symbols > MAX_SYSTEMATIC_SYMBOLS { return Err(AvailabilityError::InvalidInventory); }
            blocks.push(Block { requirement: r, ordinal: ordinal as u32, symbols });
        }
    }
    let mut heap = BinaryHeap::<Reverse<Span>>::new();
    heap.try_reserve_exact(span_count).map_err(|_| AvailabilityError::AllocationFailed)?;
    let mut receipt_ids = BTreeSet::new();
    let mut receipt_members = BTreeSet::new();
    for receipt in &input.receipts {
        work.step()?;
        if receipt.basis != policy.basis { return Err(AvailabilityError::WrongBasis); }
        if !receipt_ids.insert(receipt.receipt_id) || !receipt_members.insert(receipt.storage_member) {
            return Err(AvailabilityError::DuplicateReceipt);
        }
        if !groups.iter().any(|(_, members)| members.binary_search(&receipt.storage_member).is_ok()) {
            return Err(AvailabilityError::UnknownMember);
        }
        for span in &receipt.coverage {
            work.step()?;
            let r = input.requirements.binary_search_by_key(&(span.object_id.0, span.encoding_id), key)
                .map_err(|_| AvailabilityError::UnknownEncoding)?;
            let requirement = &input.requirements[r];
            let symbols = requirement.source_symbols.get(span.source_block as usize)
            ...
            let location = policy.locations.binary_search_by_key(&span.placement_id, |location| location.placement_id)
            ...
            heap.push(Reverse((starts[r] + span.source_block as usize, span.first_esi, span.end_esi, location)));
        }
    }
    ...
    while let Some(Reverse(span)) = heap.pop() {
        work.step()?;
        if spans.last() != Some(&span) { spans.push(span); }
    }
    ...
    for block in 0..blocks.len() {
        work.step()?;
        let start = end;
        while end < spans.len() && spans[end].0 == block { work.step()?; end += 1; }
        ranges.push(start..end);
    }
    for (side, members) in groups {
        let eligible: Vec<_> = policy.locations.iter().map(|location| members.binary_search(&location.member).is_ok()).collect();
        for failed in Cuts::new(domains.len(), f) {
            ...
            for (block, range) in blocks.iter().zip(&ranges) {
                ...
                for &(_, first, last, location) in &spans[range.clone()] {
                    work.step()?;
                    if !eligible[location] || location_masks[location] & failed != 0 { continue; }
                    if first > covered { break; }
                    covered = covered.max(last);
                    if covered == block.symbols { break; }
                }
                if covered != block.symbols {
                    ...
                    return Err(AvailabilityError::Unrecoverable(CoverageGap {
                        side, failed_domains: domains.iter().enumerate()
                            .filter_map(|(bit, domain)| (failed & (1_u64 << bit) != 0).then_some(*domain)).collect(),
                        object_id: requirement.object_id, encoding_id: requirement.encoding_id,
                        source_block: block.ordinal, first_missing_esi: covered,
                    }));
                }
            }
        }
    ...
    Ok(SystematicAssessment { input, checked_failure_cases: cases as u64, work: work.used })
}
