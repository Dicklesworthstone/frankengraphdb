...
use fgdb_order::{
    Committed, Configuration, Domain, Error as RaftError, Event, Limits, MemberId,
    Message, Output, PersistentState, Raft, Role, SnapshotTransfer,
};
...
impl ReadIndexReady {
    pub fn id(&self) -> &ReadIndexId { &self.id }
    pub fn domain(&self) -> Domain { self.domain }
    pub fn configuration(&self) -> [u8; 32] { self.configuration }
    pub fn leader(&self) -> MemberId { self.leader }
...
enum FlightKind { Append, Snapshot }
...
impl<C: Clone + Eq> Replica<C> {
    ...
    fn from_new_machine(raft: Raft<C>, maximum_reads: usize) -> Result<Self, RaftError> {
        ...
        Ok(Self {
            raft, sent: BTreeMap::new(), highest_request: 0,
            incarnation: Arc::new(()), next_read: 0, maximum_reads,
            reads: BTreeMap::new(),
        })
    }

    pub fn id(&self) -> MemberId { self.raft.id() }
    pub fn role(&self) -> Result<Role, RaftError> { self.raft.role() }
    ...
    pub fn pending_reads(&self) -> usize { self.reads.len() }
    ...
    pub async fn step<P: RaftPublisher<C>>(
        ...
        let output = sequence(&mut self.raft, publisher, event).await
            .map_err(ReplicaError::Sequence)?;
        // No await separates durability from observation of the released output.
        if compact { self.sent.clear(); }
        if let Some((member, _, _, _)) = reply { self.sent.remove(&member); }
    ...
    pub async fn read_index<P: RaftPublisher<C>>(
        ...
        let serial = self.next_read.checked_add(1).ok_or(ReplicaError::ReadCounterExhausted)?;
        let id = ReadIndexId { incarnation: Arc::clone(&self.incarnation), serial };
        let read = PendingRead {
            id: id.clone(), term: state.term(), index,
        ...
        let mut admission = ReadAdmission { replica: self, serial, armed: true };
    ...
    fn observe(
        ...
        if output.role != Role::Leader {
            ...
            let ready: Vec<_> = self.reads.iter().filter_map(|(serial, read)| {
            ...
            for serial in ready {
                if let Some(read) = self.reads.remove(&serial) {
                    resolutions.push(ReadResolution::Ready(ReadIndexReady {
                        id: read.id, domain: configuration.domain(),
                        configuration: configuration.identity(), leader: self.raft.id(),
                        term: read.term, index: read.index,
                    }));
                }
            }
        }
        for envelope in &output.messages {
            let sent = match &envelope.message {
                Message::Append { term, request, .. } =>
                    Some(Sent { term: *term, request: *request, kind: FlightKind::Append }),
                Message::InstallSnapshot { term, request, .. } =>
                    Some(Sent { term: *term, request: *request, kind: FlightKind::Snapshot }),
                _ => None,
            };
...
impl<C> Drop for ReadAdmission<'_, C> {
    fn drop(&mut self) {
        if self.armed { self.replica.reads.remove(&self.serial); }
    }
}
