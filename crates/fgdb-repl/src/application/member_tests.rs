...
enum Fault { Healthy, Error, Pending, WrongCut, WrongBasis, WrongRoot }
...
impl Drop for View { fn drop(&mut self) { self.active.set(self.active.get() - 1); } }
...
impl RaftPublisher<Command> for Backend {
    type Error = &'static str;
    fn publish(&mut self, state: &PersistentState<Command>)
        -> impl Future<Output = Result<(), Self::Error>>
    {
        ...
        drop(io);
        async move {
            if fault == Fault::Pending { pending::<()>().await; }
            if fault == Fault::Error { Err("Raft root publication unknown") } else { Ok(()) }
        }
    }
}
...
impl ReadApplication<Command> for Backend {
        ...
        -> impl Future<Output = Result<PinnedApplication<View>, <Self as Application<Command>>::Error>>
    {
        ...
        let selected: Vec<_> = store.values.iter().filter(|(index, _)| *index <= at.index).copied().collect();
        let state_root = if at == basis.applied { basis.state_root }
            else { selected.last().map_or(oid(10), |(index, _)| oid(1000 + index)) };
        let mut pinned = PinnedApplication {
            basis: *basis, at, state_root,
            view: View { values: selected.into_iter().map(|(_, value)| value).collect(), active },
        };
        match fault {
        ...
        async move {
            if fault == Fault::Pending { pending::<()>().await; }
            if fault == Fault::Error { Err("pin unavailable") } else { Ok(pinned) }
        }
    }
}
...
impl SeedPublisher<Command> for Backend {
    type Error = &'static str;
    fn publish_object(&mut self, object: ObjectPublication<'_>)
        -> impl Future<Output = Result<(), Self::Error>>
    {
        self.io.borrow_mut().objects.insert(object.object().object_id().0);
        ready(Ok(()))
    }
    fn publish_snapshot(&mut self, snapshot: SnapshotPublication<'_, Command>)
        -> impl Future<Output = Result<RootPublicationEvidence, Self::Error>>
    {
        let anchor = snapshot.seed().plan().anchor();
        let mut io = self.io.borrow_mut();
        for object in snapshot.seed().plan().objects() { assert!(io.objects.contains(&object.object_id.0)); }
        ...
        store.progress = ApplicationProgress {
            ...
            applied: AppliedPosition { index: anchor.raft_index, term: anchor.raft_term },
            visible_index: anchor.raft_index, state_root: anchor.state_root,
            publication_root: anchor.publication_root, publication_generation: anchor.publication_generation,
        };
        store.values.clear();
        ready(Ok(RootPublicationEvidence { written_index: 1,
            slot_generation: anchor.publication_generation, root_manifest_oid: anchor.publication_root.0 }))
    }
}
...
fn backend(progress: ApplicationProgress) -> (Backend, Model) {
    ...
    let io = Rc::new(RefCell::new(IoState {
        raft: None, raft_writes: 0, raft_fault: Fault::Healthy, pin_fault: Fault::Healthy,
        load_fault: Fault::Healthy, reload_after_seed: Fault::Healthy,
        objects: BTreeSet::new(), pin_calls: 0, active_pins: Rc::new(Cell::new(0)),
    }));
    (Backend { app, io: Rc::clone(&io) }, (store, io))
}

fn cluster(count: u128, max_reads: usize) -> (Vec<Member>, Vec<Model>) {
    let configuration = Configuration::stable(Domain([1; 32]), [2; 32],
        (1..=count).map(MemberId), []).unwrap();
    let mut members = Vec::new();
    let mut models = Vec::new();
    for id in 1..=count {
        let state = PersistentState::from_authenticated_parts(configuration.clone(), 0, None, 0, Vec::new());
...
fn pump(members: &mut [Member], messages: Vec<Envelope<Command>>, reachable: &[u128]) {
    ...
    while let Some(envelope) = queue.pop_front() {
        ...
        if reachable.contains(&envelope.from.0) && reachable.contains(&envelope.to.0) {
            let output = immediate(members[envelope.to.0 as usize - 1].step(Event::Receive(envelope))).unwrap();
...
fn confirm_read(members: &mut [Member]) -> ReadIndexId {
    ...
    assert!(members[0].waiting.iter().find(|read| read.id == id).unwrap().quorum.is_some());
    id
}

fn apply_all(member: &mut Member) {
    for _ in 0..64 {
        if immediate(member.apply_next()).unwrap().is_none() { return; }
    }
...
fn three_member_commit_apply_audit_and_pin_are_separate_gates() {
    ...
    for (member, model) in nodes.iter().zip(&models) {
        ...
        assert_eq!(model.0.borrow().calls, 0, "append replies cannot wait for application");
    }
    let read = confirm_read(&mut nodes);
    assert!(matches!(immediate(nodes[0].try_read(&read)).unwrap(),
        ReadState::PendingApplication { required: 2, applied: 0 }));
    apply_all(&mut nodes[0]);
    assert!(matches!(immediate(nodes[0].try_read(&read)).unwrap(),
        ReadState::PendingAudit { required: 2, visible: 1 }));
    assert_eq!(models[0].1.borrow().pin_calls, 0);
    propose(&mut nodes, Command::ReleaseThrough(2));
    apply_all(&mut nodes[0]);
    let ReadState::Ready(snapshot) = immediate(nodes[0].try_read(&read)).unwrap() else { panic!("not ready") };
    assert_eq!(snapshot.required_index(), 2);
    assert_eq!(snapshot.at().index, 3);
    assert_eq!(snapshot.view().values, [10]);
    assert!(matches!(immediate(nodes[0].try_read(&read)),
        Err(ApplicationError::State(ApplicationStateError::UnknownRead))));
    assert_eq!(models[0].1.borrow().active_pins.get(), 1);
    drop(nodes);
    assert_eq!(snapshot.view().values, [10], "owned pin survives member destruction");
    drop(snapshot);
...
fn a_ready_read_pins_visible_history_not_the_hidden_applied_tail() {
    ...
    let ReadState::Ready(snapshot) = immediate(nodes[0].try_read(&early)).unwrap() else { panic!("not ready") };
    assert_eq!(snapshot.at().index, 3);
    assert_eq!(snapshot.view().values, [10], "hidden suffix must not leak");
    let late = confirm_read(&mut nodes);
    assert!(matches!(immediate(nodes[0].try_read(&late)).unwrap(), ReadState::PendingAudit { required: 4, .. }));
    propose(&mut nodes, Command::ReleaseThrough(4));
    apply_all(&mut nodes[0]);
    let ReadState::Ready(later) = immediate(nodes[0].try_read(&late)).unwrap() else { panic!("not ready") };
...
fn losing_leadership_cancels_even_already_confirmed_unpinned_reads() {
    ...
    let term = state.term();
    let message = Envelope { domain: state.configuration().domain(), configuration: state.configuration().identity(),
        from: MemberId(2), to: MemberId(1), message: Message::RequestVote { term: term + 1,
            last_index: 1, last_term: term } };
    ...
    assert!(matches!(immediate(nodes[0].try_read(&id)),
        Err(ApplicationError::State(ApplicationStateError::UnknownRead))));
}
...
fn confirmed_audit_waiters_still_consume_the_admission_budget() {
    ...
    assert!(matches!(immediate(nodes[0].read_index()),
...
fn partitioned_leader_cannot_pin_a_read_without_a_new_quorum() {
    ...
    assert!(matches!(immediate(nodes[0].try_read(&id)).unwrap(), ReadState::PendingQuorum));
    assert_eq!(models[0].1.borrow().pin_calls, 0);
}
...
fn pin_cancellation_releases_the_guard_but_retains_the_original_read() {
    ...
    let ReadState::Ready(view) = immediate(nodes[0].try_read(&id)).unwrap() else { panic!("not ready") };
    assert_eq!(models[0].1.borrow().raft_writes, writes, "retry does not invent another quorum round");
    assert_eq!(nodes[0].pending_reads(), 0);
    drop(view);
    assert_eq!(models[0].1.borrow().active_pins.get(), 0);
}
...
fn bad_snapshot_pins_fence_the_whole_member_and_release_resources() {
    for fault in [Fault::WrongCut, Fault::WrongBasis, Fault::WrongRoot] {
        ...
        assert!(matches!(immediate(nodes[0].try_read(&id)),
            Err(ApplicationError::State(ApplicationStateError::InvalidReadSnapshot))));
        assert_eq!(models[0].1.borrow().active_pins.get(), 0);
        assert_eq!(nodes[0].progress(), Err(ApplicationStateError::RecoveryRequired));
        assert!(matches!(immediate(nodes[0].step(Event::Heartbeat)),
...
fn unavailable_pin_is_retryable_without_readmission_or_leaking_a_guard() {
    ...
    assert!(matches!(immediate(nodes[0].try_read(&id)), Err(ApplicationError::Backend(_))));
    assert_eq!(models[0].1.borrow().active_pins.get(), 0);
    assert_eq!(nodes[0].pending_reads(), 1);
    let writes = models[0].1.borrow().raft_writes;
    models[0].1.borrow_mut().pin_fault = Fault::Healthy;
...
fn foreign_read_ids_cannot_cancel_or_consume_a_local_request() {
    ...
    assert!(matches!(immediate(first[0].try_read(&foreign)),
        Err(ApplicationError::State(ApplicationStateError::UnknownRead))));
    assert_eq!(first[0].pending_reads(), 1);
    assert!(matches!(immediate(first[0].try_read(&local)).unwrap(), ReadState::Ready(_)));
}
...
fn application_cancellation_fences_consensus_and_reads_until_root_recovery() {
    ...
    let backend = Backend { app: MemoryApplication(Rc::clone(&models[0].0)), io: Rc::clone(&models[0].1) };
    ...
    assert!(matches!(immediate(recovered.try_read(&read)),
        Err(ApplicationError::State(ApplicationStateError::UnknownRead))));
}
...
fn compaction_cannot_discard_unapplied_or_audit_hidden_entries() {
    let (mut nodes, models) = cluster(1, 16);
    elect(&mut nodes);
    let cut = |index, state_root| SnapshotCut::from_authenticated_parts(
        &super::super::tests::config(), oid(80 + index).0, state_root, oid(90 + index).0, index, 1,
    ).unwrap();
    assert!(matches!(immediate(nodes[0].step(Event::Compact(cut(1, oid(10).0)))),
        Err(MemberError::State(ApplicationStateError::CompactionNotVisible))));
    apply_all(&mut nodes[0]);
    immediate(nodes[0].step(Event::Compact(cut(1, oid(10).0)))).unwrap();
    propose(&mut nodes, Command::Put(10));
    apply_all(&mut nodes[0]);
    assert!(matches!(immediate(nodes[0].step(Event::Compact(cut(2, oid(1002).0)))),
        Err(MemberError::State(ApplicationStateError::CompactionNotVisible))));
    propose(&mut nodes, Command::ReleaseThrough(2));
    apply_all(&mut nodes[0]);
    let calls = models[0].1.borrow().raft_writes;
    assert!(matches!(immediate(nodes[0].step(Event::Compact(cut(3, oid(999).0)))),
        Err(MemberError::State(ApplicationStateError::SnapshotStateMismatch))));
    assert_eq!(models[0].1.borrow().raft_writes, calls);
    immediate(nodes[0].step(Event::Compact(cut(3, oid(1002).0)))).unwrap();
    assert_eq!(nodes[0].durable_state().unwrap().snapshot().unwrap().index(), 3);
    assert!(immediate(nodes[0].apply_next()).unwrap().is_none());
}

struct CryptoSource { fixtures: Vec<crypto::Fixture>, calls: usize }
impl SeedObjectSource for CryptoSource {
    type Error = &'static str;
    fn recover(&mut self, spec: SeedObjectSpec) -> impl Future<Output = Result<VerifiedObject, Self::Error>> {
        self.calls += 1;
        ready(self.fixtures.iter().find(|fixture| fixture.encoding.object_id() == spec.object_id)
            .map(crypto::Fixture::verified).ok_or("missing fixture"))
    }
}
...
fn offer_seed(nodes: &mut [Member]) -> (SnapshotTransfer, SeedPlan, CryptoSource) {
    ...
    let objects: Vec<_> = fixtures.iter().map(|fixture| SeedObjectSpec {
        object_id: fixture.encoding.object_id(), object_kind: crypto::KIND,
        compressed_len: fixture.plaintext.len() as u64,
    }).collect();
    let config = nodes[1].durable_state().unwrap().configuration().clone();
    let cut = SnapshotCut::from_authenticated_parts(&config, objects[0].object_id.0,
        objects[1].object_id.0, objects[2].object_id.0, 8, 3).unwrap();
    let anchor = SeedAnchor { namespace: crypto::namespace(), consensus_domain: config.domain().0,
        configuration: config.identity(), snapshot_manifest: objects[0].object_id,
        state_root: objects[1].object_id, retention_floor: objects[2].object_id,
        publication_root: objects[3].object_id, publication_generation: 20,
        raft_index: 8, raft_term: 3, logical_command_seq: 5, commit_seq: 4 };
    ...
    let plan = SeedPlan::from_authenticated_inventory(anchor, objects, SeedLimits::default()).unwrap();
    ...
    (output.consensus.snapshot_transfers.into_iter().next().unwrap(), plan, CryptoSource { fixtures, calls: 0 })
}
...
fn bonded_seed_reloads_the_same_atomic_application_cut_before_acknowledging() {
    ...
    let output = immediate(nodes[1].install_snapshot(crypto::namespace(), transfer, plan, &mut source)).unwrap();
    assert_eq!(source.calls, 4);
    assert_eq!(nodes[1].progress().unwrap().applied, AppliedPosition { index: 8, term: 3 });
    assert_eq!(nodes[1].progress().unwrap().visible_index, 8);
    assert_eq!(nodes[1].progress().unwrap().state_root, root);
    assert!(output.consensus.messages.iter().any(|message| matches!(message.message, Message::SnapshotInstalled { .. })));
    assert_eq!(models[1].0.borrow().loads, 2, "reload is mandatory after root installation");
...
fn failed_or_cancelled_post_seed_reload_never_reactivates_the_old_application() {
    for fault in [Fault::Error, Fault::Pending, Fault::WrongCut, Fault::WrongRoot, Fault::WrongBasis] {
        let (mut nodes, models) = cluster(3, 16);
        let (transfer, plan, mut source) = offer_seed(&mut nodes);
        models[1].1.borrow_mut().reload_after_seed = fault;
        if fault == Fault::Pending {
            let mut future = pin!(nodes[1].install_snapshot(crypto::namespace(), transfer, plan, &mut source));
            let waker = Waker::from(Arc::new(NoopWake));
            assert!(future.as_mut().poll(&mut Context::from_waker(&waker)).is_pending());
        } else {
            assert!(immediate(nodes[1].install_snapshot(crypto::namespace(), transfer, plan, &mut source)).is_err());
        }
        assert_eq!(models[1].1.borrow().raft.as_ref().unwrap().snapshot().unwrap().index(), 8);
        assert_eq!(nodes[1].progress(), Err(ApplicationStateError::RecoveryRequired));
        assert!(matches!(immediate(nodes[1].step(Event::Heartbeat)),
            Err(MemberError::State(ApplicationStateError::RecoveryRequired))));
    }
}
