...
use fgdb_repl::application::member::{
    AppliedReplica, MemberSeedError, PinnedApplication, ReadApplication, ReadState,
};
...
fn audited_plan(objects: &[Fixture]) -> SeedPlan {
    plan(anchor(objects), objects)
        .with_authenticated_audit_cut(audit(objects))
        .unwrap()
}
...
fn genesis() -> ApplicationProgress {
    ApplicationProgress {
        ...
        visible_index: 0,
        state_root: ObjectId([90; 32]),
        publication_root: ObjectId([91; 32]),
...
impl Application<u64> for Backend {
    ...
    fn load(&mut self) -> impl Future<Output = Result<ApplicationProgress, Self::Error>> {
        ...
        let fault = if disk.installations == 0 {
            0
        } else {
            disk.fault
        };
        match fault {
            ...
            4 => {
                ...
                self.restored = Some(
                    RestoredSnapshot::from_authenticated_parts(old.snapshot().clone(), audit)
                        .unwrap(),
                );
            }
            5 => {
            ...
            6 => {
                let old = self.restored.as_ref().unwrap();
                let a = old.snapshot();
                let other = SnapshotCut::from_authenticated_parts(
                    &config(),
                    [203; 32],
                    a.state_root(),
                    a.retention_floor(),
                ...
                .unwrap();
                self.restored = Some(
                    RestoredSnapshot::from_authenticated_parts(other, old.audit_cut()).unwrap(),
                );
            }
            7 => {
                self.restored = None;
    ...
    fn restored_snapshot(&self) -> Option<RestoredSnapshot> {
        assert_ne!(
            self.disk.borrow().fault,
            12,
            "restoration projection panicked"
        );
        self.restored.clone()
    }
    async fn apply(
...
impl SeedPublisher<u64> for Backend {
    ...
    async fn publish_snapshot(
        ...
        assert_eq!(
            publication.consensus().snapshot(),
            Some(disk.next_image.snapshot())
        );
...
impl ReadApplication<u64> for Backend {
    ...
    async fn pin_visible(
        ...
        disk.pins += 1;
        Ok(PinnedApplication {
            basis: *basis,
            at,
            state_root: ObjectId([at.index as u8; 32]),
            view: self.visible.clone(),
        })
    }
}
...
fn download(member: &Member, transfer: SnapshotTransfer, objects: &[Fixture]) -> SnapshotDownload {
    member
        .begin_snapshot_download(support::namespace(), transfer, audited_plan(objects))
        .unwrap()
}
...
fn ack(member: &mut Member, messages: Vec<Envelope<u64>>) {
    ...
    for _ in 0..16 {
        let Some((term, request)) = messages.iter().find_map(|message| {
            if message.to != MemberId(1) {
                return None;
            }
            if let Message::Append { term, request, .. } = &message.message {
                Some((*term, *request))
            } else {
                None
            }
        ...
        messages = immediate(member.step(Event::Receive(envelope(
            MemberId(2),
            Message::Appended {
                term,
                request,
...
fn hidden_snapshot_installs_then_ordered_controls_release_the_restored_queue() {
    ...
    assert!(matches!(
        immediate(member.try_read(&read)).unwrap(),
        ReadState::PendingAudit { visible: 50, .. }
    ));
...
fn bound_install_and_live_install_restore_the_same_hidden_cut() {
    ...
    let output = immediate(member.install_snapshot(
        support::namespace(),
        transfer,
        audited_plan(&objects),
        &mut source,
    ))
    .unwrap();
...
fn visibility_cannot_regress_at_admission_or_after_a_live_download() {
    ...
    for before_admission in [false, true] {
        ...
        if before_admission {
            assert!(matches!(
                member.begin_snapshot_download(
                    support::namespace(),
                    transfer,
                    audited_plan(&objects)
                ),
                Err(MemberSeedError::Application(ApplicationError::State(
                    ApplicationStateError::VisibilityRegression
                )))
            ));
        } else {
            assert!(matches!(
                immediate(member.install_download(downloaded.unwrap())),
                Err(MemberSeedError::Application(ApplicationError::State(
                    ApplicationStateError::VisibilityRegression
                )))
            ));
...
fn audit_cut_requires_both_roots_and_exact_position_relationships() {
    ...
    for missing in [4, 5] {
        ...
        .unwrap();
        assert!(matches!(
            p.with_authenticated_audit_cut(valid),
    ...
    assert!(
        plan(anchor(&objects), &objects)
            .with_authenticated_audit_cut(origin)
            .is_ok()
    );
    let complete = SeedAuditCut {
        visible_index: 93,
        visible_term: 11,
        visible_state_root: anchor(&objects).state_root,
        ..valid
    };
    assert!(
        plan(anchor(&objects), &objects)
            .with_authenticated_audit_cut(complete)
            .is_ok()
    );
}
    ...
    for mutation in 0..3 {
        {
            ...
            match mutation {
                ...
                1 => {
                    ...
                    let wrong = SnapshotCut::from_authenticated_parts(
                        &config(),
                        [213; 32],
                        cut.state_root(),
                        cut.retention_floor(),
                    ...
                    .unwrap();
                    d.restored = Some(
                        RestoredSnapshot::from_authenticated_parts(wrong, valid.audit_cut())
                            .unwrap(),
                    );
                }
