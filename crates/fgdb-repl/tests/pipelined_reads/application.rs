...
impl ReadApplication<u64> for Backend {
    ...
    async fn pin_visible(
        ...
        Ok(PinnedApplication {
            basis: *basis,
            at,
            state_root: state_root(at.index),
            view,
        })
    }
}
...
impl Host {
    fn new() -> Self {
        ...
        .unwrap();
        let mut member = immediate(AppliedReplica::recover(
            replica,
            Backend(Rc::clone(&store)),
            1,
            8,
        ))
        .unwrap();
        member.configure_append_pipeline(4).unwrap();
        let peers = nodes(&configuration, 4)
            .into_iter()
            .filter(|(id, _)| *id != MemberId(1))
            .collect();
        let mut host = Self {
            member,
            peers,
        ...
        host.pump(output.consensus.messages);
        assert_eq!(
            immediate(host.member.apply_next())
                .unwrap()
                .unwrap()
                .visible_index,
            1
    ...
    fn propose(&mut self, values: &[u64]) {
        ...
        for value in values {
            messages.extend(
                immediate(self.member.step(Event::Propose(*value)))
                    .unwrap()
                    .consensus
                    .messages,
            );
...
fn pipelined_commit_to_apply_to_audit_release_to_pin_uses_one_backend_and_original_read() {
    ...
    let read = host.read();
    assert!(matches!(
        immediate(host.member.try_read(&read)).unwrap(),
    ...
    immediate(host.member.apply_next()).unwrap().unwrap();
    let ReadState::Ready(visible) = immediate(host.member.try_read(&read)).unwrap() else {
        panic!()
    };
    ...
    assert!(matches!(
        immediate(host.member.try_read(&read)),
    ...
    immediate(host.member.apply_next()).unwrap().unwrap();
    assert!(matches!(
        immediate(host.member.try_read(&pending_audit)).unwrap(),
    ...
    immediate(host.member.apply_next()).unwrap().unwrap();
    let ReadState::Ready(released) = immediate(host.member.try_read(&pending_audit)).unwrap()
    else {
        panic!()
    };
...
fn earlier_floor_pins_history_not_hidden_tail_and_cancelled_pin_retains_the_read() {
    ...
    host.store.borrow_mut().suspend_pin = true;
    {
        ...
        assert_eq!(
            Arc::strong_count(&host.store.borrow().history[&3]),
            references + 1
        );
    }
    assert_eq!(
...
fn owning_application_pipeline_configuration_preserves_both_publication_fences() {
    let mut host = Host::new();
    assert_eq!(
        host.member.configure_append_pipeline(2),
        Err(ApplicationStateError::Raft(
            RaftError::PipelineConfigurationBusy
        ))
    );
