...
use fgdb_order::{Configuration, Domain, Envelope, Error as RaftError, Event, Limits, MemberId,
    Message, PersistentState, Raft, SnapshotCut};
...
use fgdb_repl::driver::{RaftPublisher, SeedCatalog, SeedDriveError, SeedObjectSource, SeedPublisher,
...
fn run_ready<F: Future>(future: F) -> F::Output {
    ...
    for _ in 0..4096 {
        if let Poll::Ready(output) = future.as_mut().poll(&mut cx) { return output; }
    }
...
impl<'a> Transport<'a> {
    fn new(fixtures: &'a [Fixture]) -> Self {
        Self { fixtures, silent: None, corrupt: None, pause_after: Cell::new(None),
            delivered: Cell::new(0), issued: RefCell::new(Vec::new()) }
    }

    fn record(&self, request: PullRequest) -> Vec<u8> {
        let fixture = self.fixtures.iter().find(|f| f.encoding.object_id() == request.object_id).unwrap();
    ...
    fn request(&self, request: PullRequest) -> impl Future<Output = Result<ReplyOutcome, Self::Error>> {
        ...
        async move {
            if self.silent == Some(request.donor)
                || self.pause_after.get().is_some_and(|limit| self.delivered.get() >= limit)
            {
                pending::<()>().await;
            }
            let mut bytes = self.record(request);
            if self.corrupt == Some(request.donor) { *bytes.last_mut().unwrap() ^= 1; }
...
impl PullTransport for Transport<'_> {
    type Error = &'static str;
    fn exchange(&mut self, requests: &[PullRequest]) -> impl Future<Output = Result<Vec<PullReply>, Self::Error>> {
        self.issued.borrow_mut().extend_from_slice(requests);
        let replies = requests.iter().rev().map(|request| PullReply {
            request: *request, outcome: ReplyOutcome::Record(self.record(*request)),
        }).collect();
        ready(Ok(replies))
    }
}
...
fn reordered_batched_replies_with_equal_esis_across_blocks_recover() {
    ...
    let f = &fixtures[0];
    let mut pull = BondedPull::new(&f.encoding, f.target(), &DEK, &DONORS, PullLimits::default()).unwrap();
    let mut transport = Transport::new(&fixtures);
    let object = run_ready(bonded::recover(&mut pull, &mut transport, &mut Vec::new(), 9)).unwrap();
    assert_eq!(object.plaintext(), f.plaintext);
    assert_eq!(pull.pending_count(), 0);
    let issued = transport.issued.borrow();
    let zeros: BTreeSet<_> = issued.iter().filter(|r| r.esi == 0).map(|r| r.source_block).collect();
...
fn streaming_recovers_all_blocks_with_one_silent_and_one_corrupt_donor() {
    ...
    let object = run_ready(streaming::recover(&mut pull, &transport, &mut Vec::new(), 9)).unwrap();
    assert_eq!(object.plaintext(), f.plaintext);
    assert_eq!(pull.pending_count(), 0);
    for block in 0..3 { assert!(pull.block_symbol_count(block).unwrap() >= f.sources[block as usize]); }
    let issued = transport.issued.borrow();
    assert!(issued.iter().any(|r| r.donor == DonorId(1)));
    assert!(issued.iter().any(|r| r.donor == DonorId(2)));
    assert!(issued.iter().any(|r| r.donor == DonorId(3) && r.esi as usize >= f.sources[r.source_block as usize]));
}
...
fn dropping_a_multiblock_stream_retires_only_requests_not_verified_equations() {
    ...
    {
        let mut future = pin!(streaming::recover(&mut pull, &transport, &mut verification, 9));
        let waker = Waker::from(Arc::new(NoopWake));
        assert!(future.as_mut().poll(&mut Context::from_waker(&waker)).is_pending());
    }
    assert_eq!(pull.pending_count(), 0);
    assert_eq!(pull.symbol_count(), 6);
    let before: Vec<_> = (0..3).map(|b| pull.block_symbol_count(b).unwrap()).collect();
    transport.pause_after.set(None);
    let object = run_ready(streaming::recover(&mut pull, &transport, &mut verification, 9)).unwrap();
    assert_eq!(object.plaintext(), f.plaintext);
...
struct Catalog { fixtures: Vec<Fixture> }
impl SeedCatalog for Catalog {
    type Error = &'static str;
    fn recovery(&self, object: SeedObjectSpec) -> Result<SeedRecovery<'_>, Self::Error> {
        let f = self.fixtures.iter().find(|f| f.encoding.object_id() == object.object_id).ok_or("unknown object")?;
        Ok(SeedRecovery { encoding: &f.encoding, target: f.target(), dek: &DEK,
            donors: &DONORS, limits: PullLimits::default() })
    }
}

fn spec(f: &Fixture) -> SeedObjectSpec {
    SeedObjectSpec { object_id: f.encoding.object_id(), object_kind: support::KIND,
        compressed_len: f.plaintext.len() as u64 }
}

#[test]
fn streaming_seed_source_resumes_its_multiblock_object_after_cancellation() {
    let catalog = Catalog { fixtures: vec![Fixture::new(3, 3, 128)] };
    let f = &catalog.fixtures[0];
    let transport = Transport::new(&catalog.fixtures);
    transport.pause_after.set(Some(6));
    let mut verification = Vec::new();
    let mut source = StreamingSeedSource::new(support::namespace(), &catalog, &transport, &mut verification, 9);
    {
    ...
    let issued = transport.issued.borrow();
    assert_eq!(issued.len(), issued.iter().map(|r| (r.source_block, r.esi)).collect::<BTreeSet<_>>().len());
}
...
impl SeedPublisher<u64> for Root {
    type Error = &'static str;
    fn publish_object(&mut self, publication: ObjectPublication<'_>) -> impl Future<Output = Result<(), Self::Error>> {
        assert!(publication.object().encoding().descriptor().source_block_count > 1);
        self.objects.insert(publication.object().object_id().0);
        ready(Ok(()))
    }
    fn publish_snapshot(&mut self, publication: SnapshotPublication<'_, u64>)
        -> impl Future<Output = Result<RootPublicationEvidence, Self::Error>>
    {
        for object in publication.seed().plan().objects() { assert!(self.objects.contains(&object.object_id.0)); }
        let anchor = publication.seed().plan().anchor();
        assert_eq!(publication.consensus().snapshot().unwrap().index(), anchor.raft_index);
        self.state = Some(publication.consensus().clone());
        self.installs += 1;
        ready(if self.fail { Err("model root outcome unknown") } else { Ok(RootPublicationEvidence {
...
fn install(fail: bool) {
    let catalog = Catalog { fixtures: (0..4).map(|i| Fixture::with_shape(
        40 + i, 4093, 256, u16::from(i) + 2, 3, 4, 128,
    )).collect() };
    let config = Configuration::stable(Domain([11; 32]), [12; 32], [MemberId(1), MemberId(2), MemberId(3)], []).unwrap();
    let roots: Vec<_> = catalog.fixtures.iter().map(|f| f.encoding.object_id()).collect();
    let cut = SnapshotCut::from_authenticated_parts(&config, roots[0].0, roots[1].0, roots[2].0, 12, 3).unwrap();
    let anchor = SeedAnchor {
        namespace: support::namespace(), consensus_domain: config.domain().0, configuration: config.identity(),
        snapshot_manifest: roots[0], state_root: roots[1], retention_floor: roots[2],
        publication_root: roots[3], publication_generation: 2, raft_index: 12, raft_term: 3,
        logical_command_seq: 8, commit_seq: 8,
    };
    // Inventory is a synthetic verifier projection for this integration test.
    let plan = SeedPlan::from_authenticated_inventory(anchor, catalog.fixtures.iter().map(spec), SeedLimits::default()).unwrap();
    let mut raft = Raft::new(MemberId(2), config.clone(), Limits::default()).unwrap();
    let mut root = Root { fail, ..Root::default() };
    let offered = run_ready(sequence(&mut raft, &mut root, Event::Receive(Envelope {
    ...
    if fail {
        assert!(matches!(result, Err(SeedDriveError::Publication("model root outcome unknown"))));
        assert_eq!(raft.role(), Err(RaftError::RecoveryRequired));
    } else {
        let output = result.unwrap();
        assert_eq!(output.installed_snapshot, Some(cut));
        assert!(output.messages.iter().any(|envelope| matches!(envelope.message,
            Message::SnapshotInstalled { term: 3, request: 1 })));
...
#[test]
fn multiblock_seed_closure_reaches_the_atomic_raft_installation_gate() { install(false); }

#[test]
fn failed_multiblock_seed_root_publication_releases_no_install_ack() { install(true); }
