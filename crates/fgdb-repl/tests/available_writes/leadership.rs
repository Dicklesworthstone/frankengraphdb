...
fn tracked(node: &mut Node, command: u64) -> fgdb_repl::application::member::write::WriteSubmission<u64> {
    let input = node.state.borrow().expected.clone();
    immediate(node.member.submit_available(command, &input, AvailabilityLimits::default(), &mut || Ok::<_, ()>(()))).unwrap()
}
...
fn handoff_rejects_new_authority_without_losing_preexisting_apply_and_audit_waiters() {
    ...
    pump(&mut nodes, submitted.output.member.consensus.messages, &[1, 2, 3]);
    ...
    assert!(matches!(nodes[0].member.try_write(&submitted.id).unwrap(), WriteState::PendingApplication { .. }));
    assert!(matches!(immediate(nodes[0].member.try_read(&read_id)).unwrap(), ReadState::PendingApplication { .. }));
    ...
    let refused = immediate(nodes[0].member.submit_available(88, &input, AvailabilityLimits::default(), &mut || {
        checkpoints += 1;
        Ok::<_, ()>(())
    }));
    assert!(matches!(refused, Err(SubmitError::Proposal(MemberProposalError::Proposal(
        ProposalError::Raft(RaftError::LeadershipTransferInProgress)
    )))));
    assert_eq!(checkpoints, 0, "refuse before evaluating even the payload assessment");
    assert_eq!(nodes[0].state.borrow().acquires, acquires);
    assert_eq!(nodes[0].state.borrow().active_permits, 0);
    assert_eq!(nodes[0].member.pending_writes(), 1, "failed admission must not leak a slot");
    assert_eq!(nodes[0].member.durable_state().unwrap(), &before);

    apply_all(&mut nodes[0]);
    assert!(matches!(nodes[0].member.try_write(&submitted.id).unwrap(), WriteState::PendingAudit { .. }));
    assert!(matches!(immediate(nodes[0].member.try_read(&read_id)).unwrap(), ReadState::PendingAudit { .. }));
    ...
    apply_all(&mut nodes[0]);
    assert!(matches!(nodes[0].member.try_write(&submitted.id).unwrap(), WriteState::Visible(_)));
    ...
    assert!(matches!(nodes[0].member.try_write(&submitted.id), Err(WriteError::UnknownInvocation)));
}
...
fn actual_transfer_stepdown_cancels_reads_and_keeps_write_unknown_after_later_apply() {
    ...
    assert!(matches!(immediate(nodes[0].member.try_read(&read_id)).unwrap(), ReadState::PendingQuorum));
    let request = output.consensus.messages.into_iter().find(|m| matches!(m.message, Message::TimeoutNow { .. })).unwrap();
    let campaign = immediate(nodes[1].member.step(Event::Receive(request))).unwrap();
    assert_eq!(campaign.consensus.role, Role::Candidate);
    let request = campaign.consensus.messages.into_iter().find(|m| m.to == MemberId(1)).unwrap();
    ...
    assert_eq!(nodes[1].state.borrow().effects.iter().map(|(_, value)| *value).collect::<Vec<_>>(), [17, 18]);
}

#[test]
fn silent_target_deadline_reopens_owned_admission_without_discarding_prior_commands() {
    let mut nodes = nodes(&configuration(3));
    for node in &mut nodes { node.member.configure_append_pipeline(4).unwrap(); }
    elect(&mut nodes, &[1, 3]);
    let submitted = tracked(&mut nodes[0], 33);
    pump(&mut nodes, submitted.output.member.consensus.messages, &[1, 3]);
    let output = immediate(nodes[0].member.step(Event::TransferLeadership(MemberId(2)))).unwrap();
    assert!(!output.consensus.messages.iter().any(|m| matches!(m.message, Message::TimeoutNow { .. })));
    assert_eq!(nodes[0].member.leadership_transfer().unwrap().unwrap().phase(), LeadershipTransferPhase::CatchingUp);
    let id = nodes[0].member.leadership_transfer().unwrap().unwrap().id();
    for _ in 0..3 {
        let output = immediate(nodes[0].member.step(Event::Heartbeat)).unwrap();
...
impl RaftPublisher<u64> for ElectionPublisher {
    ...
    fn publish(&mut self, state: &PersistentState<u64>) -> impl Future<Output = Result<(), Self::Error>> {
        *self.disk.borrow_mut() = Some(state.clone());
        let mode = self.mode;
        if mode == Mode::Panic { panic!("election publication panic"); }
        async move {
            if mode == Mode::SuspendAfter { pending::<()>().await; }
            if mode == Mode::FailAfter { Err("uncertain vote publication") } else { Ok(()) }
        }
    }
}
...
fn target_election_error_cancellation_and_panic_require_root_recovery() {
    for mode in [Mode::FailAfter, Mode::SuspendAfter, Mode::Panic] {
        let mut nodes = nodes(&configuration(3));
        elect(&mut nodes, &[1, 2, 3]);
        let offered = immediate(nodes[0].member.step(Event::TransferLeadership(MemberId(2)))).unwrap();
        let request = offered.consensus.messages.into_iter().find(|m| matches!(m.message, Message::TimeoutNow { .. })).unwrap();
        let state = nodes[1].member.durable_state().unwrap().clone();
        let mut replica = Replica::recover(MemberId(2), state.clone(), Limits::default(), 16).unwrap();
        let disk = Rc::new(RefCell::new(Some(state.clone())));
        let mut publisher = ElectionPublisher { disk: Rc::clone(&disk), mode: Mode::Ready };
        // Recovery cleared the leader. Ordinary authenticated traffic must
        // establish it again before TimeoutNow may bypass pre-vote.
        let heartbeat = Envelope { domain: configuration(3).domain(), configuration: configuration(3).identity(),
            from: MemberId(1), to: MemberId(2), message: Message::Append { term: 1, request: 99,
                prev_index: 1, prev_term: 1, entries: Vec::new(), leader_commit: 1 } };
...
fn new_leader_still_needs_current_term_commit_and_a_fresh_read_quorum() {
    ...
    let timeout = transfer.consensus.messages.into_iter().find(|m| matches!(m.message, Message::TimeoutNow { .. })).unwrap();
    let campaign = immediate(nodes[1].member.step(Event::Receive(timeout))).unwrap();
    let request = campaign.consensus.messages.into_iter().find(|m| m.to == MemberId(1)).unwrap();
    let vote = immediate(nodes[0].member.step(Event::Receive(request))).unwrap().consensus.messages.remove(0);
    ...
    assert!(matches!(immediate(nodes[1].member.read_index()), Err(
        fgdb_repl::application::member::MemberError::Replica(
            fgdb_repl::replica::ReplicaError::CurrentTermNotCommitted
        )
    )));
    let unrelated = Envelope { domain: configuration(3).domain(), configuration: configuration(3).identity(),
        from: MemberId(1), to: MemberId(2), message: Message::QuorumReply { term: 2, round: 10 } };
    ...
    assert!(matches!(immediate(nodes[1].member.try_read(&read)).unwrap(), ReadState::PendingQuorum));
    pump(&mut nodes, output.consensus.messages, &[1, 2, 3]);
...
fn equal_wire_rounds_cannot_reuse_another_incarnations_handoff_cancellation() {
    ...
    let round = |messages: &[Envelope<u64>]| messages.iter().find_map(|envelope| {
        if let Message::TimeoutNow { round, .. } = &envelope.message { Some(*round) } else { None }
    }).unwrap();
    ...
    assert_eq!(right[0].member.leadership_transfer().unwrap().unwrap().id(), b);
}
...
fn grouped_direct_proposals_cannot_bypass_owned_handoff_or_release_hidden_writes() {
    ...
    assert!(matches!(immediate(nodes[0].member.step(Event::ProposeBatch(vec![0, 88]))),
        Err(fgdb_repl::application::member::MemberError::Replica(
            fgdb_repl::replica::ReplicaError::Sequence(
                fgdb_repl::driver::SequenceError::Raft(RaftError::LeadershipTransferInProgress)
            )
        ))));
    assert_eq!(nodes[0].member.durable_state().unwrap(), &before);
