...
// pub(crate): it is the associated Permit type of an impl for the crate-level
// Backend, so it must be visible wherever Backend is (E0446 otherwise).
pub(crate) struct BatchPermit<'a> {
...
impl RaftPublisher<u64> for BatchPermit<'_> {
    ...
    fn publish(
        ...
        assert_eq!(
            &state.entries()[..previous.entries().len()],
            previous.entries()
        );
        ...
        if mode == Mode::Panic {
            panic!("batch publisher panic");
        }
        if fresh && mode != Mode::Refuse {
            self.backend.store(state);
        }
        async move {
            if mode == Mode::SuspendAfter {
                pending::<()>().await;
            }
            if !fresh || matches!(mode, Mode::Refuse | Mode::FailAfter) {
                Err("publication uncertain")
            } else {
                Ok(())
            }
        }
...
fn submit_batch(node: &mut Node, commands: Vec<u64>) -> MemberBatchProposalOutput<u64> {
    ...
    immediate(node.member.propose_batch_available(
        commands,
        &data,
        BatchProposalLimits::default(),
        &mut || Ok::<_, ()>(()),
    ))
    .unwrap()
}
...
fn whole_authorized_group_publishes_once_and_applies_in_bounded_prefixes() {
    let mut group = nodes(&configuration(1));
    elect(&mut group, &[1]);
    let mut scalar = nodes(&configuration(1));
    elect(&mut scalar, &[1]);
    ...
    assert_eq!(
        group[0].state.borrow().trace,
        ["batch-acquire", "batch-raft", "batch-permit-drop"]
    );
    for command in [11, 22, 33] {
        submit(&mut scalar[0], command);
    }
    assert_eq!(
        group[0].member.durable_state().unwrap(),
        scalar[0].member.durable_state().unwrap()
    );
    for expected in 2..=4 {
        assert_eq!(
            immediate(group[0].member.apply_next())
                .unwrap()
                .unwrap()
                .applied
                .index,
    ...
    assert_eq!(
        group[0].state.borrow().effects,
        scalar[0].state.borrow().effects
    );
}
...
fn any_invalid_member_of_the_group_refuses_before_authority_or_append() {
    for mutation in 0..6 {
        let mut cluster = nodes(&configuration(1));
        elect(&mut cluster, &[1]);
        ...
        match mutation {
            ...
            _ => {
                commands = vec![11; 129];
                data = inputs(node, 129);
            }
        }
        let result =
            immediate(
                node.member
                    .propose_batch_available(commands, &data, limits, &mut || Ok::<_, ()>(())),
            );
        assert!(result.is_err());
        if mutation == 3 {
            assert!(matches!(
                result,
                Err(BatchProposalError::WrongBasis { offset: 1 })
            ));
        }
        if mutation == 4 {
            assert!(matches!(
                result,
                Err(BatchProposalError::Availability { offset: 1, .. })
            ));
        }
...
fn aggregate_work_and_failure_case_budgets_do_not_reset_per_command() {
    ...
    let one = assess_systematic(&expected, AvailabilityLimits::default(), &mut || {
        Ok::<_, ()>(())
    })
    .unwrap();
    ...
    for (work, cases, succeeds) in [
        ...
        let limits = BatchProposalLimits {
            max_work: work,
            max_failure_cases: cases,
            ..BatchProposalLimits::default()
        };
        let result = immediate(node.member.propose_batch_available(
            vec![11, 22],
            &data,
            limits,
            &mut || Ok::<_, ()>(()),
        ...
        if !succeeds {
            assert!(matches!(
                result,
                Err(BatchProposalError::WorkBudget | BatchProposalError::FailureCaseBudget)
                    | Err(BatchProposalError::Availability {
                        error: AvailabilityError::WorkBudget | AvailabilityError::FailureCaseBudget,
                        ..
                    })
            ));
            assert_eq!(node.member.durable_state().unwrap(), &before);
    ...
    for after in [false, true] {
        ...
        if after {
            ...
            let result = immediate(node.member.propose_batch_available(
                ...
                BatchProposalLimits::default(),
                &mut || {
                    if store.borrow().active_permits != 0 {
                        Err(())
                    } else {
                        Ok(())
                    }
            ...
            cancel(node.member.propose_batch_available(
                vec![11, 22],
                &data,
                BatchProposalLimits::default(),
                &mut || Ok::<_, ()>(()),
...
fn uncertain_batch_publication_fences_member_and_never_exposes_partial_local_append() {
    for mode in [
        ...
        if mode == Mode::SuspendAfter {
            ...
            assert!(
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    let _ = immediate(node.member.propose_batch_available(
                        vec![11, 22],
                        &data,
                        BatchProposalLimits::default(),
                        &mut || Ok::<_, ()>(()),
        ...
        assert_eq!(node.state.borrow().active_permits, 0);
        assert_eq!(
            node.state.borrow().root.as_ref().unwrap().entries().len(),
            if matches!(mode, Mode::FailAfter | Mode::SuspendAfter) {
                3
            } else {
                1
            }
...
fn current_authority_is_rechecked_at_publication_not_only_at_acquisition() {
    ...
    let result = immediate(node.member.propose_batch_available(
        ...
        &mut || {
            if store.borrow().active_permits != 0 {
                store.borrow_mut().acquire = Mode::Refuse;
            }
            Ok::<_, ()>(())
        },
    ));
...
fn followers_and_handoffs_refuse_before_any_payload_calculation() {
    ...
    assert!(matches!(
        immediate(cluster[0].member.propose_batch_available(
            vec![11],
            &data,
            BatchProposalLimits::default(),
            &mut || -> Result<(), ()> { panic!("preflight must precede calculation") }
        )),
    ...
    immediate(
        cluster[0]
            .member
            .step(Event::TransferLeadership(MemberId(2))),
    )
    .unwrap();
    assert!(matches!(
        immediate(cluster[0].member.propose_batch_available(
            ...
            &data,
            BatchProposalLimits::default(),
            &mut || -> Result<(), ()> { panic!("handoff freezes admission") }
        )),
        Err(BatchProposalError::Raft(
...
fn track_group(node: &mut Node, commands: Vec<u64>) -> WriteBatchSubmission<u64> {
    node.state.borrow_mut().batch_commands = commands.clone();
    let data = inputs(node, commands.len());
    immediate(node.member.submit_batch_available(
        commands,
        &data,
        BatchProposalLimits::default(),
        &mut || Ok::<_, ()>(()),
...
fn each_grouped_invocation_keeps_its_own_apply_and_audit_wait() {
    ...
    for (offset, id) in submitted.ids.iter().enumerate() {
        assert!(
            matches!(node.member.try_write(id).unwrap(), WriteState::PendingApplication { required, .. }
            if required == 2 + offset as u64)
        );
    }
    immediate(node.member.apply_next()).unwrap().unwrap();
    assert!(matches!(
        node.member.try_write(&submitted.ids[0]).unwrap(),
    ...
    assert!(matches!(
        node.member.try_write(&submitted.ids[1]).unwrap(),
    ...
    for id in &submitted.ids {
        assert!(matches!(
            node.member.try_write(id).unwrap(),
            WriteState::PendingAudit { .. }
        ));
    }
    ...
    for (offset, id) in submitted.ids.iter().enumerate() {
        match node.member.try_write(id).unwrap() {
            WriteState::Visible(visible) => {
                ...
                assert_eq!(
                    visible.position(),
                    submitted.output.positions.position(offset).unwrap()
                );
            }
...
fn scalar_and_group_waiters_share_capacity_and_cancellation_never_aborts_entries() {
    ...
    let scalar = immediate(node.member.submit_available(
        9,
        &data[0],
    ...
    assert!(matches!(
        immediate(node.member.submit_batch_available(
            ...
            &data,
            BatchProposalLimits::default(),
            &mut || -> Result<(), ()> { panic!("capacity must precede assessment") }
        )),
    ...
    assert!(matches!(
        node.member.try_write(&submitted.ids[0]),
        Err(WriteError::UnknownInvocation)
    ));
    assert!(matches!(
        node.member.try_write(&scalar.id).unwrap(),
...
fn failed_or_cancelled_group_admission_releases_all_and_only_its_reservations() {
    for mode in [Mode::Refuse, Mode::Suspend, Mode::Panic] {
        ...
        let node = &mut cluster[0];
        let data = inputs(node, 2);
        let existing = immediate(node.member.submit_available(
            9,
            &data[0],
            AvailabilityLimits::default(),
            &mut || Ok::<_, ()>(()),
        ...
        let before = node.member.durable_state().unwrap().clone();
        if mode == Mode::Suspend {
            cancel(node.member.submit_batch_available(
                vec![11, 22],
                &data,
                BatchProposalLimits::default(),
                &mut || Ok::<_, ()>(()),
            ...
            assert!(
                std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    let _ = immediate(node.member.submit_batch_available(
                        vec![11, 22],
                        &data,
                        BatchProposalLimits::default(),
                        &mut || Ok::<_, ()>(()),
        ...
        assert!(matches!(
            node.member.try_write(&existing.id).unwrap(),
            WriteState::PendingApplication { .. }
        ));
        assert_eq!(node.state.borrow().active_permits, 0);
...
fn an_uncertain_group_append_preserves_the_recovery_fence_for_earlier_waiters() {
    ...
    assert_eq!(
        node.member.pending_writes(),
        1,
        "unreturned batch waiters must not leak"
    );
    ...
    assert!(
        node.state.borrow().effects.is_empty(),
...
fn quorum_can_commit_a_prefix_inside_a_locally_published_tracked_group() {
    ...
    let old = PersistentState::from_authenticated_parts(
        config.clone(),
        1,
        None,
        0,
    ...
    let leader = immediate(cluster[0].member.step(Event::Receive(
        vote.consensus.messages.into_iter().next().unwrap(),
    )))
    .unwrap();
    assert_eq!(leader.consensus.role, Role::Leader);
    let initial_probe = leader
        .consensus
        .messages
        .into_iter()
        .find(|m| m.to == MemberId(2))
    ...
    assert!(
        submitted.output.member.consensus.messages.is_empty(),
        "probe lane is still occupied"
    );
    ...
    let first = immediate(cluster[0].member.step(Event::Receive(
        rejection.consensus.messages.into_iter().next().unwrap(),
    )))
    .unwrap();
    let append12 = first
        .consensus
    ...
    let next = immediate(cluster[0].member.step(Event::Receive(
        reply12.consensus.messages.into_iter().next().unwrap(),
    )))
    .unwrap();
    assert_eq!(
        cluster[0].member.durable_state().unwrap().commit_index(),
        0,
        "old-term quorum cannot commit alone"
    );
    let append34 = next
        .consensus
        .messages
    ...
    let after = immediate(cluster[0].member.step(Event::Receive(
        reply34.consensus.messages.into_iter().next().unwrap(),
    )))
    .unwrap();
    assert_eq!(cluster[0].member.durable_state().unwrap().commit_index(), 4);
    assert!(matches!(
        cluster[0].member.try_write(&submitted.ids[0]).unwrap(),
        WriteState::PendingApplication { required: 4, .. }
    ));
    assert!(matches!(
        cluster[0].member.try_write(&submitted.ids[1]).unwrap(),
