...
fn tracked_write_waits_for_each_distinct_gate_and_consumes_visibility_once() {
    ...
    assert!(matches!(nodes[0].member.try_write(&id).unwrap(), WriteState::PendingCommit { required: 2, committed: 1 }));
    pump(&mut nodes, submission.output.member.consensus.messages, &[1, 2]);
    assert!(matches!(nodes[0].member.try_write(&id).unwrap(), WriteState::PendingApplication { required: 2, applied: 1 }));
    apply_all(&mut nodes[0]);
    assert!(matches!(nodes[0].member.try_write(&id).unwrap(), WriteState::PendingAudit { required: 2, visible: 1 }));
...
fn write_admission_counts_apply_and_audit_waits_before_authority_work() {
    ...
    for apply in [false, true] {
        if apply { apply_all(node); }
        let acquisitions = node.state.borrow().acquires;
        assert!(matches!(immediate(node.member.submit_available(88, &input, AvailabilityLimits::default(), &mut || Ok::<_, ()>(()))) ,
...
fn duplicate_commands_are_distinct_invocations_and_limits_never_evict_them() {
    ...
    assert_eq!(node.member.set_write_limit(1), Err(WriteError::Backpressure));
    assert_eq!(node.member.set_write_limit(0), Err(WriteError::InvalidLimit));
    assert_eq!(node.member.set_write_limit(1025), Err(WriteError::InvalidLimit));
    ...
    assert!(matches!(node.member.try_write(&second.id).unwrap(), WriteState::PendingApplication { required: 3, applied: 2 }));
    apply_all(node);
...
fn cancelling_a_waiter_does_not_abort_or_remove_its_later_committed_effect() {
    ...
    assert_eq!(nodes[0].member.pending_writes(), 0);
    assert!(matches!(nodes[0].member.try_write(&submission.id), Err(WriteError::UnknownInvocation)));
}
...
fn cancelled_or_refused_admission_never_leaks_a_bounded_slot() {
    for mode in [Mode::Refuse, Mode::Suspend, Mode::Panic] {
        ...
        if mode == Mode::Suspend {
            cancel(node.member.submit_available(77, &input, AvailabilityLimits::default(), &mut || Ok::<_, ()>(()))) ;
        } else if mode == Mode::Panic {
            assert!(std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| { submit_tracked(node, 77); })).is_err());
        } else {
            assert!(immediate(node.member.submit_available(77, &input, AvailabilityLimits::default(), &mut || Ok::<_, ()>(()))).is_err());
        }
...
fn uncertainty_after_a_second_append_makes_earlier_waits_unknown_not_rejected() {
    ...
    let input = node.state.borrow().expected.clone();
    cancel(node.member.submit_available(88, &input, AvailabilityLimits::default(), &mut || Ok::<_, ()>(()))) ;
    assert_eq!(node.member.pending_writes(), 1); // Only the unreturned waiter was removed.
    assert_eq!(node.state.borrow().root.as_ref().unwrap().entries()[2].command, Some(88));
    ...
    assert_eq!(node.member.pending_writes(), 0);
    assert!(matches!(node.member.try_write(&first.id), Err(WriteError::UnknownInvocation)));
}
...
fn application_errors_cancellation_and_panic_cannot_release_visible_writes() {
    for mode in [Mode::Refuse, Mode::FailAfter, Mode::SuspendAfter, Mode::Panic] {
        ...
        if mode == Mode::SuspendAfter { cancel(node.member.apply_next()); }
        else if mode == Mode::Panic {
            assert!(std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| { let _ = immediate(node.member.apply_next()); })).is_err());
        } else { assert!(immediate(node.member.apply_next()).is_err()); }
        match node.member.try_write(&submission.id).unwrap() {
            WriteState::Unknown(result) => assert_eq!(result.reason(), UnknownReason::RecoveryRequired),
...
fn leadership_loss_is_sticky_even_if_the_same_command_commits_after_reelection() {
    ...
    immediate(nodes[0].member.step(Event::Receive(Envelope {
        domain: config.domain(), configuration: config.identity(), from: MemberId(2), to: MemberId(1),
        message: Message::RequestVote { term: 2, last_index: 1, last_term: 1 },
    }))).unwrap();
    elect(&mut nodes, &[1, 2, 3]);
...
fn durable_commit_evidence_survives_compaction_without_retaining_command_copies() {
    ...
    let cut = SnapshotCut::from_authenticated_parts(&configuration(1), [201; 32], progress.state_root.0,
        [202; 32], progress.applied.index, progress.applied.term).unwrap();
...
fn recovery_and_other_members_cannot_complete_an_old_numeric_write_position() {
    ...
    assert!(matches!(different[0].member.try_write(&original.id), Err(WriteError::UnknownInvocation)));
    ...
    assert!(matches!(recovered.try_write(&original.id), Err(WriteError::UnknownInvocation)));
    assert!(!recovered.cancel_write(&original.id));
}

#[test]
fn joint_membership_needs_both_majorities_before_write_application_can_complete() {
    let config = Configuration::joint(Domain([1; 32]), [2; 32], [MemberId(1), MemberId(2), MemberId(3)],
        [MemberId(3), MemberId(4), MemberId(5)], []).unwrap();
    let mut nodes = nodes(&config);
    elect(&mut nodes, &[1, 2, 3, 4, 5]);
    let submission = submit_tracked(&mut nodes[0], 77);
    pump(&mut nodes, submission.output.member.consensus.messages, &[1, 2, 3]);
    assert!(matches!(nodes[0].member.try_write(&submission.id).unwrap(), WriteState::PendingCommit { .. }));
