...
use fgdb_order::{Configuration, Domain, Envelope, Error as RaftError, Event, Limits, MemberId, Message, PersistentState, Role};
use fgdb_repl::application::{Application, ApplicationBatch, ApplicationError, ApplicationProgress, ApplicationStateError, AppliedPosition};
use fgdb_repl::application::member::{AppliedReplica, AppliedReplicaOutput, PinnedApplication, ReadApplication, ReadState};
...
fn incoming(from: MemberId, message: Message<u64>) -> Event<u64> {
    Event::Receive(Envelope { domain: Domain([1; 32]), configuration: [2; 32], from, to: MemberId(1), message })
}

#[derive(Clone, Copy, PartialEq, Eq)]
enum Mode { Normal, FailBefore, FailAfter, SuspendAfter, PanicAfter }
...
impl RaftPublisher<u64> for Backend {
    ...
    fn publish(&mut self, state: &PersistentState<u64>) -> impl Future<Output = Result<(), Self::Error>> {
        ...
        image.publishes += 1;
        let mode = image.mode;
        if mode != Mode::FailBefore { image.state = Some(state.clone()); }
        if mode == Mode::PanicAfter { panic!("publisher constructor panicked after changing the model root"); }
        drop(image);
        async move {
            if mode == Mode::SuspendAfter { pending::<()>().await; }
            if matches!(mode, Mode::FailBefore | Mode::FailAfter) { Err("publication outcome unknown") } else { Ok(()) }
        }
    }
}
    ...
    fn apply(&mut self, batch: ApplicationBatch<'_, u64>) -> impl Future<Output = Result<ApplicationProgress, Self::Error>> {
        ...
        for (position, entry) in batch.indexed_entries() {
            ...
            match entry.command {
                ...
                None if progress.visible_index + 1 == position.index => progress.visible_index = position.index,
                None => {}
            }
        }
...
impl ReadApplication<u64> for Backend {
    type View = Vec<u64>;
    fn pin_visible(&mut self, basis: &ApplicationProgress, at: AppliedPosition) -> impl Future<Output = Result<PinnedApplication<Self::View>, Self::Error>> {
        let mut image = self.0.borrow_mut();
        assert_eq!(basis, &image.progress);
        assert_eq!(at.index, basis.visible_index);
        let view = image.values.iter().filter(|(index, _)| *index <= at.index).map(|(_, value)| *value).collect();
        image.pins += 1;
        ready(Ok(PinnedApplication { basis: *basis, at, state_root: oid(1000 + at.index), view }))
    }
}
...
fn acknowledge(member: &mut Member, message: &Envelope<u64>) -> AppliedReplicaOutput<u64> {
    let Message::Append { term, request, .. } = &message.message else { panic!("not an append") };
    immediate(member.step(incoming(message.to, Message::Appended {
        term: *term, request: *request, success: true, conflict_next: 0,
    }))).unwrap()
}
fn drain(member: &mut Member, messages: Vec<Envelope<u64>>) {
    ...
    while let Some(message) = messages.pop_front() {
        if message.to != MemberId(2) || !matches!(&message.message, Message::Append { .. }) { continue; }
        calls += 1;
...
fn setup(count: u128, limits: Limits, apply_batch: usize) -> (Member, Rc<RefCell<Image>>) {
    let image = Rc::new(RefCell::new(Image {
        ...
        values: Vec::new(), applied: Vec::new(), publishes: 0, applies: 0, pins: 0, mode: Mode::Normal,
    }));
    let replica = Replica::new(MemberId(1), config(count), limits, 16).unwrap();
    let mut member = immediate(Member::recover(replica, Backend(Rc::clone(&image)), apply_batch, 16)).unwrap();
    member.configure_append_pipeline(4).unwrap();
    let mut output = immediate(member.step(Event::ElectionTimeout)).unwrap();
    if count > 1 {
        output = immediate(member.step(incoming(MemberId(2), Message::Vote { term: 1, granted: true }))).unwrap();
    }
...
fn a_batch_uses_one_root_publication_instead_of_one_per_command() {
    for count in 1..=16 {
        ...
        let output = immediate(batched.step(Event::ProposeBatch((10..10 + count).collect()))).unwrap();
        for command in 10..10 + count { immediate(scalar.step(Event::Propose(command))).unwrap(); }
        assert_eq!(batch_image.borrow().publishes - batch_before, 1);
        assert_eq!(scalar_image.borrow().publishes - scalar_before, count as usize);
...
fn batched_commit_still_waits_for_bounded_apply_and_audit_visibility() {
    ...
    assert!(matches!(immediate(member.try_read(&read)).unwrap(), ReadState::PendingApplication { required: 4, applied: 1 }));
    for index in 2..=4 {
    ...
    assert!(matches!(immediate(member.try_read(&read)).unwrap(), ReadState::PendingAudit { required: 4, visible: 1 }));
...
fn a_multi_member_local_batch_is_not_a_quorum_commit() {
    ...
    let request = output.consensus.messages.iter().find(|message| message.to == MemberId(2)).unwrap();
...
fn invalid_batches_never_call_the_publisher_or_poison_the_member() {
    let (mut member, image) = setup(1, Limits { max_log_entries: 5, max_append_entries: 4 }, 1);
    let before = image.borrow().publishes;
...
fn failed_cancelled_and_panicking_publication_recovers_all_or_no_new_entries() {
    for mode in [Mode::FailBefore, Mode::FailAfter, Mode::SuspendAfter, Mode::PanicAfter] {
        let (mut member, image) = setup(1, Limits::default(), 1);
        ...
        assert!(matches!(member.progress(), Err(ApplicationStateError::Raft(RaftError::RecoveryRequired))));
        ...
        let mut member = immediate(Member::recover(replica, Backend(Rc::clone(&image)), 1, 16)).unwrap();
        while immediate(member.apply_next()).unwrap().is_some() {}
        if expected == 1 { assert!(image.borrow().values.is_empty()); }
        else { assert_eq!(image.borrow().values, [(2, 10), (3, 20)]); }
...
fn a_pre_read_batch_reply_cannot_become_a_fresh_read_confirmation() {
    ...
    let old = batch.consensus.messages.iter().find(|message| message.to == MemberId(2)).unwrap();
    let (read, retries) = immediate(member.read_index()).unwrap();
    assert!(retries.consensus.messages.iter().any(|message| message == old));
    assert!(matches!(immediate(member.try_read(&read)).unwrap(), ReadState::PendingQuorum));
    ...
    let ReadState::Ready(view) = immediate(member.try_read(&read)).unwrap() else { panic!("fresh confirmation missing") };
    ...
    drain(&mut member, output.consensus.messages);
    assert!(matches!(immediate(member.try_read(&after_commit)).unwrap(), ReadState::PendingApplication { required: 3, applied: 1 }));
}
...
fn leader_loss_cancels_a_read_waiting_on_a_batch_without_guessing_its_outcome() {
    ...
    let old = batch.consensus.messages.iter().find(|message| message.to == MemberId(2)).unwrap();
    let (read, _) = immediate(member.read_index()).unwrap();
    let output = immediate(member.step(incoming(MemberId(2), Message::RequestVote { term: 2, last_index: 0, last_term: 0 }))).unwrap();
    ...
    assert!(matches!(immediate(member.try_read(&read)), Err(ApplicationError::State(ApplicationStateError::UnknownRead))));
