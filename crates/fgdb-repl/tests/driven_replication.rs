...
use fgdb_order::{
    ...
    Message, Output, PersistentState, Raft, Role, SnapshotCut, SnapshotTransfer,
};
...
fn minority_loss_and_learner_ack_do_not_release_a_commit() {
    let config = Configuration::stable(
        Domain([1; 32]), [2; 32], [MemberId(1), MemberId(2), MemberId(3)], [MemberId(4)],
    ).unwrap();
    ...
    assert!(!nodes[0].raft.committed_after(0).unwrap().iter()
    ...
    for index in [0, 1] {
        ...
        assert!(committed.iter().any(|entry| entry.entry.command == Some(42)));
        let durable = nodes[index].root.state.as_ref().unwrap();
        assert!(committed.iter().all(|entry| entry.index <= durable.commit_index()));
    }
    let recovered = Raft::recover(
        MemberId(2), nodes[1].root.state.clone().unwrap(), Limits::default(),
    ).unwrap();
    assert!(recovered.committed_after(0).unwrap().iter()
        .any(|entry| entry.entry.command == Some(42)));
}
...
impl SeedCatalog for Catalog {
    ...
    fn recovery(&self, object: SeedObjectSpec) -> Result<SeedRecovery<'_>, Self::Error> {
        let fixture = self.fixtures.iter().find(|fixture| fixture.encoding.object_id() == object.object_id)
...
impl<'a> FixtureTransport<'a> {
    fn healthy(fixtures: &'a [Fixture]) -> Self {
        Self { fixtures, failed: None, corrupt: None, suspend_on: None, calls: 0 }
    }
}
...
impl PullTransport for FixtureTransport<'_> {
    ...
    fn exchange(&mut self, requests: &[PullRequest]) -> impl Future<Output = Result<Vec<PullReply>, Self::Error>> {
        ...
        let replies: Vec<_> = requests.iter().rev().map(|request| {
            let fixture = self.fixtures.iter().find(|fixture| fixture.encoding.object_id() == request.object_id).unwrap();
...
fn real_bonded_recovery_survives_disconnected_and_corrupt_donors() {
    ...
    let object = immediate(bonded::recover(&mut pull, &mut transport, &mut Vec::new(), 8)).unwrap();
...
fn cancelling_a_window_releases_credit_but_preserves_verified_symbols() {
    ...
    {
        let mut future = pin!(bonded::recover(&mut pull, &mut transport, &mut verification, 8));
        let waker = Waker::from(Arc::new(NoopWake));
        let mut cx = Context::from_waker(&waker);
    ...
    let object = immediate(bonded::recover(&mut pull, &mut transport, &mut verification, 8)).unwrap();
    assert_eq!(object.plaintext(), fixture.plaintext);
}

#[test]
fn catalog_inventory_mismatch_is_rejected_before_network_io() {
    let catalog = Catalog { fixtures: vec![Fixture::new(93)] };
    let fixture = &catalog.fixtures[0];
    let mut transport = FixtureTransport::healthy(&catalog.fixtures);
    let mut verification = Vec::new();
    {
        let mut source = BondedSeedSource::new(support::namespace(), &catalog, &mut transport, &mut verification, 8);
        let wrong = SeedObjectSpec {
            object_id: fixture.encoding.object_id(), object_kind: support::KIND + 1,
...
fn seed_offer(catalog: &Catalog) -> (Raft<u64>, SnapshotTransfer, SeedPlan) {
    ...
    let anchor = SeedAnchor {
        namespace: support::namespace(), consensus_domain: config.domain().0,
    ...
    let plan = SeedPlan::from_authenticated_inventory(anchor, catalog.fixtures.iter().map(|fixture| SeedObjectSpec {
        object_id: fixture.encoding.object_id(), object_kind: support::KIND,
        compressed_len: fixture.plaintext.len() as u64,
    ...
    let output = immediate(sequence(&mut raft, &mut store, Event::Receive(Envelope {
...
impl SeedPublisher<u64> for SeedRoot {
    ...
    fn publish_snapshot(&mut self, snapshot: SnapshotPublication<'_, u64>) -> impl Future<Output = Result<RootPublicationEvidence, Self::Error>> {
        ...
        async move {
            if suspend { pending::<()>().await; }
            if fail { Err("root sync outcome unknown") } else { Ok(evidence) }
        }
    }
}

fn seed_catalog() -> Catalog {
    Catalog { fixtures: (101..105).map(Fixture::new).collect() }
}
...
fn seed_driver_installs_only_after_every_object_is_durable() {
    ...
    assert!(output.messages.iter().any(|message| matches!(message.message, Message::SnapshotInstalled { .. })));
}
...
fn cancelled_atomic_seed_publication_fences_the_replica() {
    ...
    let mut verification = Vec::new();
    let mut source = BondedSeedSource::new(support::namespace(), &catalog, &mut transport, &mut verification, 8);
    let mut root = SeedRoot { suspend: true, ..SeedRoot::default() };
    {
...
fn ordinary_sequencer_cannot_install_only_the_raft_half_of_a_snapshot() {
    ...
    assert!(matches!(immediate(sequence(&mut raft, &mut root, Event::SnapshotReady(transfer.id()))),
...
fn failed_atomic_seed_publication_emits_no_success_and_requires_recovery() {
    ...
    let mut verification = Vec::new();
    let mut source = BondedSeedSource::new(support::namespace(), &catalog, &mut transport, &mut verification, 8);
    let mut root = SeedRoot { fail: true, ..SeedRoot::default() };
