...
use fgdb_order::{Configuration, Domain, Envelope, Error, Event, Limits, MemberId, Message, PersistentState, Role};
...
impl RaftPublisher<u64> for Publisher {
    ...
    fn publish(&mut self, state: &PersistentState<u64>) -> impl Future<Output = Result<(), Self::Error>> {
        ...
        async move {
            ...
            if fail { Err("ambiguous root publication") } else { Ok(()) }
        }
    }
}

struct Node { replica: Replica<u64>, publisher: Publisher }
impl Node {
...
fn pump(nodes: &mut [Node], messages: Vec<Envelope<u64>>, reachable: &[u128]) -> Vec<ReadResolution> {
    ...
    while let Some(message) = queue.pop_front() {
        ...
        if reachable.contains(&message.from.0) && reachable.contains(&message.to.0) {
            let node = nodes.iter_mut().find(|node| node.replica.id() == message.to).unwrap();
...
fn check_quorum_replies_never_finish_read_index_but_real_append_confirmation_does() {
    ...
    assert!(check.consensus.messages.iter().all(|m| matches!(m.message, Message::QuorumProbe { .. })));
...
fn quorum_loss_cancels_pending_reads_and_delayed_replies_cannot_revive_them() {
    ...
    let probe = check.consensus.messages.into_iter().find(|m| m.to == MemberId(2)).unwrap();
    let delayed_probe = nodes[1].step(Event::Receive(probe)).consensus.messages.remove(0);
    let (id, output) = nodes[0].read();
    let request = output.consensus.messages.into_iter()
        .find(|m| m.to == MemberId(2) && matches!(m.message, Message::Append { .. })).unwrap();
    let delayed_append = nodes[1].step(Event::Receive(request)).consensus.messages.remove(0);
    ...
    assert!(nodes[0].step(Event::Receive(delayed_probe)).reads.is_empty());
    assert!(nodes[0].step(Event::Receive(delayed_append)).reads.is_empty());
    assert!(matches!(immediate(nodes[0].replica.read_index(&mut Publisher::default())),
...
fn preview_winner_cannot_send_actual_votes_after_failed_or_cancelled_publication() {
    for suspend in [false, true] {
        ...
        let request = preview.consensus.messages.into_iter().find(|m| m.to == MemberId(2)).unwrap();
        let reply = nodes[1].step(Event::Receive(request)).consensus.messages.remove(0);
        let node = &mut nodes[0];
        node.publisher.fail = !suspend;
        node.publisher.suspend = suspend;
        if suspend {
            {
                let mut future = pin!(node.replica.step(&mut node.publisher, Event::Receive(reply)));
                let waker = Waker::from(Arc::new(NoopWake));
                assert!(future.as_mut().poll(&mut Context::from_waker(&waker)).is_pending());
            }
        } else {
            assert!(matches!(immediate(node.replica.step(&mut node.publisher, Event::Receive(reply))),
...
fn higher_term_quorum_probe_cannot_reply_from_uncertain_storage() {
    ...
    let message = Envelope {
        ...
        from: MemberId(1), to: MemberId(2), message: Message::QuorumProbe { term: 10, round: 1 },
    };
    assert!(matches!(immediate(node.replica.step(&mut node.publisher, Event::Receive(message))),
        Err(ReplicaError::Sequence(SequenceError::Publication(_)))));
