...
use fgdb_repl::availability::proposal::{
    PayloadProposalAuthority, ProposalError, ProposalPosition,
};
use fgdb_repl::availability::{
    AvailabilityInput, AvailabilityLimits, AvailabilityPolicy, EncodingRequirement, FailureDomain,
    PayloadBasis, ReceiptCoverage, SourceCoverage, StorageLocation, StorageSets,
...
impl RaftPublisher<u64> for Permit<'_> {
    ...
    fn publish(
        ...
        assert_eq!(
            state.snapshot().map_or(0, |cut| cut.index()) + state.entries().len() as u64,
            self.position.index
        );
        ...
        async move {
            ...
            if matches!(mode, Mode::Refuse | Mode::FailAfter) {
...
impl Application<u64> for Backend {
    ...
    fn apply(
        ...
        async move {
            ...
            if mode == Mode::Refuse {
                return Err("application publication refused");
            }
            // Scope the borrow so it provably ends before any later await.
            let progress = {
                let mut s = self.0.borrow_mut();
                ...
                for (index, command) in batch.commands() {
                    ...
                    if *command == 0 {
                        s.hide = false;
                    } // Explicit audit-release MODEL.
                }
                s.generation += 1;
                s.progress.applied = batch.last();
                if !s.hide {
                    s.progress.visible_index = batch.last().index;
                }
            ...
            if mode == Mode::Panic {
                panic!("application publication panic");
            }
            if mode == Mode::SuspendAfter {
                pending::<()>().await;
            }
            if mode == Mode::FailAfter {
                return Err("application publication uncertain");
            }
            Ok(progress)
        }
    }
...
fn pump(nodes: &mut [Node], messages: Vec<Envelope<u64>>, reachable: &[u128]) {
    ...
    while let Some(message) = messages.pop_front() {
        ...
        let node = nodes
            .iter_mut()
            .find(|node| node.member.id() == message.to)
            .unwrap();
        let output = immediate(node.member.step(Event::Receive(message))).unwrap();
        messages.extend(output.consensus.messages);
    }
}
fn apply_all(node: &mut Node) {
    while immediate(node.member.apply_next()).unwrap().is_some() {}
}
...
fn submit(node: &mut Node, command: u64) -> MemberProposalOutput<u64> {
    ...
    immediate(node.member.propose_available(
        command,
        &input,
...
fn owned_authority_publishes_before_apply_and_never_reports_client_success() {
    ...
    assert_eq!(
        node.state.borrow().trace,
        ["acquire", "authorized-raft", "permit-drop"]
    );
    assert_eq!(node.state.borrow().active_permits, 0);
    assert_eq!(
        node.state.borrow().root.as_ref().unwrap().entries()[1].command,
        Some(77)
    );
    apply_all(node);
...
fn basis_and_coverage_fail_before_owned_authority_and_refusal_leaves_no_append() {
    ...
    for mutation in 0..3 {
        ...
        match mutation {
            0 => input.policy.basis.configuration[0] ^= 1,
            1 => input.receipts.clear(),
            _ => node.state.borrow_mut().acquire = Mode::Refuse,
        }
        assert!(
            immediate(node.member.propose_available(
                9,
...
fn authority_cancellation_and_post_acquire_checkpoint_release_only_ephemeral_custody() {
    ...
    cancel(
        node.member
            .propose_available(9, &input, AvailabilityLimits::default(), &mut || {
                Ok::<_, ()>(())
            }),
    );
    assert_eq!(node.member.durable_state().unwrap(), &before);
    ...
    let result = immediate(node.member.propose_available(
        9,
        &input,
        AvailabilityLimits::default(),
        &mut || {
            if state.borrow().active_permits == 1 {
    ...
    assert!(matches!(
        result,
        Err(MemberProposalError::Proposal(ProposalError::Interrupted(_)))
    ));
    assert_eq!(node.member.durable_state().unwrap(), &before);
    assert_eq!(node.state.borrow().active_permits, 0);
}
...
fn every_uncertain_authorized_publication_fences_the_whole_member() {
    for mode in [
        ...
        if mode == Mode::SuspendAfter {
            cancel(node.member.propose_available(
                9,
                &input,
                AvailabilityLimits::default(),
                &mut || Ok::<_, ()>(()),
...
fn availability_is_not_a_voting_quorum_and_apply_is_not_audit_visibility() {
    ...
    pump(&mut nodes, out.consensus.messages, &[1, 2]);
    assert!(matches!(
        immediate(nodes[0].member.try_read(&id)).unwrap(),
...
fn no_member_can_acquire_proposal_authority_after_application_fencing_or_as_follower() {
    ...
    assert!(
        immediate(nodes[1].member.propose_available(
            1,
            &input,
            AvailabilityLimits::default(),
            &mut || Ok::<_, ()>(())
    ...
    assert!(
        immediate(nodes[0].member.propose_available(
            2,
            &input,
            AvailabilityLimits::default(),
            &mut || Ok::<_, ()>(())
