...
fn put_predicate(out: &mut Vec<u8>, predicate: &PropertyPredicate) -> Result<(), PolicyError> {
    if predicate
        ...
        .map_err(|_| PolicyError::InvalidScalar)?
        > MAX_POLICY_SCALAR_BYTES
    {
        return Err(PolicyError::Limit);
    }
    if matches!(predicate.value, CanonicalScalar::Null) {
        return Err(PolicyError::InvalidScalar);
    }
...
impl ReadCaveat {
    ...
    pub fn to_bytes(&self) -> Result<Vec<u8>, PolicyError> {
        ...
        match self {
            ...
            Self::VertexProperty(predicate) => {
                out.push(8);
                put_predicate(&mut out, predicate)?;
            }
            Self::EdgeProperty(predicate) => {
                out.push(9);
                put_predicate(&mut out, predicate)?;
            }
            Self::TimeWindow {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, PolicyError> {
        ...
        let result = match input.byte()? {
            ...
            10 => Self::TimeWindow {
                not_before: input.u128()?,
                expires_at: input.u128()?,
            },
            11 => Self::Snapshots {
                first: CommitSeq(input.u64()?),
                last: CommitSeq(input.u64()?),
            },
            12 => Self::Limits(ReadLimits {
                rows: input.u64()?,
                work: input.u64()?,
                scratch: input.u64()?,
            }),
...
impl<'a> Reader<'a> {
    fn take(&mut self, len: usize) -> Result<&'a [u8], PolicyError> {
        let (head, tail) = self
            ...
            .ok_or(PolicyError::NonCanonical)?;
        self.0 = tail;
        Ok(head)
    }
    ...
    fn u16(&mut self) -> Result<u16, PolicyError> {
        Ok(u16::from_le_bytes(
            self.take(2)?
                .try_into()
                .map_err(|_| PolicyError::NonCanonical)?,
        ))
    }
    ...
    fn set<T: Ord>(
        &mut self,
        mut read: impl FnMut(&mut Self) -> Result<T, PolicyError>,
    ) -> Result<Vec<T>, PolicyError> {
        let len = usize::from(self.u16()?);
        if len > MAX_SCOPE_ITEMS || len > self.0.len() / 8 {
            return Err(PolicyError::Limit);
        }
    ...
    fn predicate(&mut self) -> Result<PropertyPredicate, PolicyError> {
        ...
        let comparison = match self.byte()? {
            ...
            2 => Comparison::Less,
            3 => Comparison::LessOrEqual,
            4 => Comparison::Greater,
            5 => Comparison::GreaterOrEqual,
            _ => return Err(PolicyError::Unsupported),
        };
        let len = usize::from(self.u16()?);
        if len > MAX_POLICY_SCALAR_BYTES {
            return Err(PolicyError::Limit);
        }
        let value =
            CanonicalScalar::decode(self.take(len)?).map_err(|_| PolicyError::InvalidScalar)?;
