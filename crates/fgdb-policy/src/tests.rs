...
fn all_caveat_arms_round_trip_and_reject_every_truncated_prefix() {
    let caveats = [
        ...
        ReadCaveat::Snapshots {
            first: CommitSeq(0),
            last: CommitSeq(u64::MAX),
        },
        ReadCaveat::Limits(ReadLimits {
            rows: 0,
            work: 100,
            scratch: 200,
        }),
    ...
    for caveat in caveats {
        ...
        for end in 0..bytes.len() {
            assert!(
                ReadCaveat::from_bytes(&bytes[..end]).is_err(),
                "accepted prefix {end}"
            );
        }
...
fn canonical_vector_and_malformed_sets() {
    assert_eq!(
        ReadCaveat::Properties(vec![]).to_bytes().unwrap(),
        vec![1, 5, 0, 0]
    );
    assert_eq!(
        ReadCaveat::Labels(vec![LabelId(1), LabelId(1)]).to_bytes(),
        Err(PolicyError::NonCanonical)
    );
    assert_eq!(
        ReadCaveat::Labels(vec![LabelId(2), LabelId(1)]).to_bytes(),
        Err(PolicyError::NonCanonical)
    );
    assert_eq!(
        ReadCaveat::from_bytes(&[1, 3, 255, 255]),
        Err(PolicyError::Limit)
    );
    assert_eq!(
        ReadCaveat::from_bytes(&[1, 255]),
        Err(PolicyError::Unsupported)
    );
}
...
fn exhaustive_small_scope_attenuation_never_widens() {
    for parent_bits in 0u8..16 {
        for child_bits in 0u8..16 {
            ...
            let child = parent
                .attenuate(&[ReadCaveat::Vertices(ids(child_bits))])
                .unwrap();
            for id in 0..5 {
                let expected =
                    parent.vertex_scope().contains(&VId(id)) && child_bits & (1 << id) != 0;
...
fn predicates_conjoin_and_cannot_be_overwritten_by_delegation() {
    let parent = ReadPolicy::compile(&[ReadCaveat::VertexProperty(predicate(
        1,
        Comparison::Greater,
        10,
    ))])
    .unwrap();
    let child = parent
        .attenuate(&[ReadCaveat::VertexProperty(predicate(
            1,
            Comparison::Less,
            20,
        ))])
    ...
    for value in -1..30 {
        ...
        assert_eq!(
            child.allows_vertex(VId(1), &[], &properties),
            value > 10 && value < 20
        );
    }
    assert!(!child.allows_vertex(VId(1), &[], &[]));
    assert!(!child.allows_vertex(
        VId(1),
        &[],
        &[(PropertyKeyId(1), CanonicalScalar::Bool(true))]
    ));
...
fn authorization_precedes_property_disclosure() {
    ...
    assert_eq!(
        policy.project_properties(&source),
        vec![(PropertyKeyId(2), CanonicalScalar::Int(8))]
    );
}
...
fn windows_and_ceilings_intersect_conservatively() {
    ...
    let empty = child
        .attenuate(&[ReadCaveat::TimeWindow {
            not_before: 30,
            expires_at: 40,
        }])
        .unwrap();
...
fn resource_bounds_and_debug_redaction() {
    ...
    assert_eq!(
        max.attenuate(&[ReadCaveat::HasLabel(LabelId(2))]),
        Err(PolicyError::Limit)
    );
