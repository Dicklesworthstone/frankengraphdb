...
fn refusal(output: Output, code: &str) {
    ...
    assert!(
        out.contains(&format!("\"type\":\"error\",\"code\":\"{code}\"")),
        "{out}"
    );
...
fn typed_writes_and_csv_imports_survive_separate_process_reopens() {
    ...
    let written = success(fixture.run(
        "write",
        &["--query-file", "write.gql", "--params-file", "arguments"],
    ));
    ...
    fixture.file(
        "read.gql",
        "MATCH (n:Person) WHERE n.id=9 RETURN COUNT(n) AS total",
    );
...
fn a_late_execution_failure_rolls_back_every_earlier_csv_record() {
    ...
    refusal(
        fixture.run(
            "import-csv",
            &["--query-file", "script.gql", "--csv-file", "bad.csv"],
        ),
        "write_failed",
    );
    fixture.count(0);
    ...
    let out = success(fixture.run(
        "import-csv",
        &["--query-file", "script.gql", "--csv-file", "good.csv"],
    ));
...
fn read_command_never_falls_back_to_write_and_no_effect_writes_close_read_only() {
    ...
    fixture.file("create.gql", "CREATE (n:Person)");
    refusal(
        fixture.run("query", &["--query-file", "create.gql"]),
        "query_refused",
    );
    fixture.count(0);
...
fn csv_stdin_is_bounded_and_uses_the_same_atomic_path() {
    ...
    let mut child = fixture
        .command("import-csv")
        .args(["--query-file", "script.gql", "--csv-file", "-"])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
...
fn failed_stdout_does_not_relabel_a_durable_write_as_rolled_back() {
    ...
    assert_eq!(
        ...
        "fgdb: write_completed_output_failed\n"
    );
    fixture.count(1);
}
