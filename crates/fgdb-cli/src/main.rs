    ...
    match response {
        Ok(text) => {
            ...
            match result {
                ...
                Err(_) => report(if writes { Error::WriteOutput } else { Error::Output }, Format::Human),
            }
        }
...
fn run(options: Options) -> Result<String, Error> {
    ...
    let prepared_write = if let Some(write) = &options.write {
        let csv = write.csv_file.as_deref()
            .map(|path| read_text_limited(&root, path, true, write.max_input_bytes)).transpose()?;
        let types = write.types_file.as_deref()
            .map(|path| read_text(&root, path, false)).transpose()?;
        let prepared = PreparedCliWrite::prepare(&options,
            statement.as_deref().ok_or(Error::Usage)?, &symbols, &parameters,
            csv.as_deref(), types.as_deref())?;
    ...
    runtime.block_on(async {
        ...
        match options.command {
            ...
            Command::Write | Command::ImportCsv => {
                ...
                let mut db = Database::open(&commit, &options.db, keys).await.map_err(|_| Error::Database)?;
                let (_, completion) = db.execute_graph_write_program_autocommit_engine_governed(
                    &txn, &query, &commit, prepared.program(), prepared.policy(),
                ).await.map_err(|source| classify_execution(&source))?;
...
fn read_text_limited(cx: &Cx, path: &Path, allow_stdin: bool, limit: usize) -> Result<String, Error> {
...
fn read_utf8(cx: &Cx, mut input: impl Read, limit: usize) -> Result<String, Error> {
    ...
    loop {
        ...
        if count == 0 { break; }
        if count > limit - bytes.len() { return Err(Error::Input); }
...
mod tests {
    ...
    struct ShortReads<'a> { data: &'a [u8], interrupted: bool, read: usize }
    ...
    fn bounded_reader_handles_short_reads_and_checks_exact_utf8_eof() {
        ...
        let text = "x\n雪🙂\n";
        let mut input = ShortReads { data: text.as_bytes(), interrupted: false, read: 0 };
        ...
        assert_eq!(input.read, 4, "read only one lookahead byte beyond admission");
