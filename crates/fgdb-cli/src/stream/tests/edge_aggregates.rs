...
const EDGE_SUMMARY: &str =
    "MATCH (a)-[r:R]-(b) RETURN COUNT(*) AS total,SUM(r.p) AS sum,COUNT(r.p) AS present";
    ...
    let ((), report) = run_async_under_lab(0x636c_e101, |root| async move {
        ...
        for (text, independent) in cases {
            ...
            assert!(bytes.starts_with(&format!(
                "{{\"v\":1,\"event\":\"columns\",\"stream\":true,\"seq\":{},",
                at.0
            )));
            ...
            assert!(
                String::from_utf8(human)
                    .unwrap()
                    .ends_with(&format!("1 row(s) (stream complete at seq {})\n", at.0))
            );
        }
        let mut options = edge_options(
            "MATCH (a)-[r:R]->(b) FOR SYSTEM_TIME AS OF SEQ $seq WHERE r.p>$floor RETURN SUM(r.p) AS sum,COUNT(*) AS count",
        );
        options.params = GqlParameters::new()
            .with_uint64("seq", at.0)
            .unwrap()
            .with_int64("floor", 0)
            .unwrap();
        ...
        for text in [EDGE_SUMMARY, DISTINCT_SUMMARY] {
            ...
            let result = deliver(
                ...
                || {
                    polls += 1;
                    if polls == 2 {
                        Err(Failure::query("stop before demand"))
                    } else {
                        Ok(())
                    }
                },
...
fn edge_shape_refusals_precede_headers_and_late_data_refusals_emit_no_success() {
    let ((), report) = run_async_under_lab(0x636c_e103, |root| async move {
        ...
        for text in [
            ...
            let mut output = Vec::new();
            assert!(
                run(&db, &cx, &edge_options(text), true, &mut output).is_err(),
                "{text}"
            );
        ...
        edit.set_edge_property(
            EId(3),
            PropertyKeyId(1),
            Some(CanonicalScalar::ucs_basic_text("private edge operand").unwrap()),
        );
        db.write(&commit, edit).await.unwrap();
        let mut output = Vec::new();
...
fn cli_having_output_expressions_and_pages_use_native_layouts_and_completion_counts() {
    let ((), report) = run_async_under_lab(0x636c_e111, |root| async move {
        ...
        assert!(
            String::from_utf8(human)
                .unwrap()
                .ends_with("1 row(s) (stream complete at seq 1)\n")
        );
        for text in [
            ...
            let mut bytes = Vec::new();
            okay(run(&db, &cx, &edge_options(text), true, &mut bytes));
            let output = String::from_utf8(bytes).unwrap();
            assert!(row_lines(&output).is_empty());
...
fn cli_offpage_errors_never_emit_a_partial_group_or_success_record() {
    let ((), report) = run_async_under_lab(0x636c_e112, |root| async move {
        ...
        let mut bytes = Vec::new();
        assert!(run(&db, &cx, &edge_options(text), true, &mut bytes).is_err());
...
fn cli_ranked_pages_preserve_exact_return_order_and_complete_empty_windows() {
    let ((), report) = run_async_under_lab(0x636c_e121, |root| async move {
        ...
        for (text, count) in [
            ...
            (
                "MATCH (a)-[r:R]->(b) RETURN SUM(r.p)+1 AS n ORDER BY SUM(r.p)",
                1,
            ),
            (
                "MATCH (a)-[r:R]-(b) RETURN COUNT(*)+1 AS n GROUP BY b ORDER BY COUNT(*) DESC LIMIT 1",
                1,
            ),
            (
            ...
            (
                "MATCH (a)-[r:R]->(b) RETURN 1/(COUNT(*)-3) AS bad HAVING COUNT(*)<0 ORDER BY COUNT(*) LIMIT 1",
                0,
            ),
        ] {
            let opts = edge_options(text);
...
fn cli_ordered_late_failures_and_backpressure_never_emit_a_success_marker() {
    let ((), report) = run_async_under_lab(0x636c_e122, |root| async move {
        ...
        for limit in [0, 1] {
            let text = format!(
                "MATCH (a)-[r:R]-(b) RETURN b,1/(COUNT(*)-3) AS bad GROUP BY b ORDER BY COUNT(*) DESC LIMIT {limit}"
            );
            let mut bytes = Vec::new();
            let error = run(&db, &cx, &edge_options(&text), true, &mut bytes)
                .err()
