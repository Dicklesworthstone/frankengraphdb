...
impl<K, L> ReadOptions<K, L> {
    ...
    pub fn text(property: K) -> Self {
        Self {
            as_of: None,
            vertex_label: None,
            projection: Projection { text: Some(property), vector: Vec::new() },
    ...
    pub fn config_for(&self, query: Search<'_>) -> Result<IndexConfig, BeaconError> {
        ...
        if !vector { config.vector = None; }
        if !text { config.text = None; }
        config.validate()?;
        if text && (config.text.is_none() || self.projection.text.is_none()) {
            return Err(BeaconError::InvalidConfig("text search needs a text property and lane"));
        }
        if vector {
            let vector = config.vector.as_ref().ok_or(BeaconError::Disabled("vector"))?;
...
pub enum Search<'a> {
    Text { query: &'a str, k: usize, mode: TextMatch },
    Vector { query: &'a [f32], k: usize, mode: VectorSearch },
...
impl Search<'_> {
    ...
    pub fn validate(self, config: &IndexConfig, work: &mut dyn WorkControl) -> Result<(), BeaconError> {
        work.charge(1)?;
        let vector = |query: &[f32], mode, work: &mut dyn WorkControl| {
            config.vector.as_ref().ok_or(BeaconError::Disabled("vector"))?
    ...
    pub fn execute(self, snapshot: &IndexSnapshot, work: &mut dyn WorkControl) -> Result<Rows, BeaconError> {
        ...
        match self {
            Self::Text { query, k, mode } => snapshot.text_search(query, k, mode, |_| true, work).map(Rows::Text),
            Self::Vector { query, k, mode } => snapshot.knn(query, k, mode, |_| true, work).map(Rows::Vector),
            Self::Hybrid(query) => snapshot.hybrid_search_exact_fusion(query, |_| true, work).map(Rows::Hybrid),
        }
    }
}
...
impl Rows {
    ...
    pub fn len(&self) -> usize {
        match self { Self::Text(rows) => rows.len(), Self::Vector(rows) => rows.len(), Self::Hybrid(rows) => rows.len() }
    }
    #[must_use]
...
impl<R: core::fmt::Display, C: core::fmt::Display> core::fmt::Display for ReadError<R, C> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self { Self::Read(e) => e.fmt(f), Self::Interrupted(e) => e.fmt(f), Self::Index(e) => e.fmt(f) }
    }
}
impl<R: core::error::Error + 'static, C: core::error::Error + 'static> core::error::Error for ReadError<R, C> {
    fn source(&self) -> Option<&(dyn core::error::Error + 'static)> {
        match self { Self::Read(e) => Some(e), Self::Interrupted(e) => Some(e), Self::Index(e) => Some(e) }
    }
}
    ...
    pub fn project<'a>(
            ...
            match self.text.and_then(&mut property) {
                ...
                Some(CanonicalScalar::Text(text)) => {
                    ...
                    if text.len() > config.max_text_bytes {
                        return Err(BeaconError::ResourceLimit { resource: "staged text bytes", limit: config.max_text_bytes });
                    }
                    work.charge(text.len())?;
                    let mut owned = String::new();
                    owned.try_reserve_exact(text.len()).map_err(|_| BeaconError::ResourceLimit {
                        resource: "text allocation", limit: config.max_text_bytes,
                    })?;
                    owned.push_str(text);
                    Some(owned)
                }
                Some(_) => return Err(BeaconError::InvalidQuery("indexed text property must be text")),
            }
        } else { None };
        let vector = if let Some(vector) = &config.vector {
            ...
            if self.vector.len() > config.max_vector_values {
                return Err(BeaconError::ResourceLimit { resource: "staged vector values", limit: config.max_vector_values });
            }
            ...
            for &key in &self.vector {
                work.charge(1)?;
                if matches!(property(key), None | Some(CanonicalScalar::Null)) { complete = false; }
            }
            if complete {
                let mut values = Vec::new();
                values.try_reserve_exact(self.vector.len()).map_err(|_| BeaconError::ResourceLimit {
                    resource: "vector allocation", limit: config.max_vector_values,
                })?;
                for &key in &self.vector {
                    work.charge(1)?;
                    let value = match property(key) {
                        Some(CanonicalScalar::Int(value)) => {
                            ...
                            if narrowed as i128 != i128::from(*value) {
                                return Err(BeaconError::InvalidQuery("vector integer is not exactly representable as f32"));
                            }
                            narrowed
                        }
                        Some(CanonicalScalar::Float(value)) => {
                            ...
                            if !narrowed.is_finite() || f64::from(narrowed) != value {
                                return Err(BeaconError::InvalidQuery("vector float is not finite exact f32"));
                            }
                            narrowed
                        }
                        _ => return Err(BeaconError::InvalidQuery("vector coordinates must be integers or floats")),
                    };
                    values.push(value);
                }
                Some(values)
