...
fn cell(row: &[u8; 5], low: usize, high: usize, at: usize, dead: u8) -> u8 {
    if at < low || at > high { dead } else { row[at - low] }
}
...
fn matches(
    ...
    for (index, (byte, ch)) in candidate.char_indices().enumerate() {
        ...
        for column in next_low..=next_high {
            ...
            let insert = if column > next_low { next[column - next_low - 1] + 1 } else { dead };
    ...
    work.charge(1)?;
    Ok(if cell(&row, low, high, pattern.len(), dead) < dead { Match::Yes } else { Match::No })
}
...
fn after_prefix(prefix: &str, work: &mut dyn WorkControl) -> Result<Option<String>, BeaconError> {
    for (offset, ch) in prefix.char_indices().rev() {
        ...
        let length = offset.checked_add(next.len_utf8()).ok_or(BeaconError::WorkBudgetExceeded)?;
        work.charge(length)?;
        let mut bound = String::new();
        bound.try_reserve_exact(length).map_err(|_| BeaconError::ResourceLimit {
            resource: "fuzzy prefix bound", limit: length,
        })?;
...
impl<'a> Expansion<'a> {
    fn build(
        ...
        let mut result = Self { terms: BTreeMap::new(), groups: query.frequencies.len() };
        for (group, term) in query.frequencies.keys().enumerate() {
            work.charge(term.len())?;
            let mut pattern = Vec::new();
            pattern.try_reserve_exact(term.len()).map_err(|_| BeaconError::ResourceLimit {
                resource: "fuzzy pattern scalars", limit: term.len(),
            })?;
            ...
            loop {
                ...
                let bound_bytes = skip.as_ref().map_or_else(|| after.map_or(1, str::len), String::len);
                ...
                let entry = if let Some(bound) = skip.as_deref() {
                    corpus.frequencies.range::<str, _>((Included(bound), Unbounded)).next()
                } else if let Some(bound) = after {
                    corpus.frequencies.range::<str, _>((Excluded(bound), Unbounded)).next()
                } else {
                    corpus.frequencies.first_key_value()
                };
                let Some((candidate, &frequency)) = entry else { break };
                if frequency == 0 || frequency > corpus.documents {
                    return Err(BeaconError::Invariant("invalid fuzzy live document frequency"));
                }
                ...
                match matches(&pattern, candidate, distance, work)? {
                    ...
                    Match::Yes => {
                            ...
                            Entry::Vacant(entry) => {
                                if count == maximum {
                                    return Err(BeaconError::ResourceLimit {
                                        resource: "fuzzy expanded terms", limit: maximum,
                                    });
                                }
                        ...
                        work.charge(1)?;
                        groups.try_reserve(1).map_err(|_| BeaconError::ResourceLimit {
                            resource: "fuzzy term groups", limit: result.groups,
                        })?;
                        groups.push(group);
                    }
                }
...
impl TextSegment {
    ...
    pub(super) fn search_fuzzy_into(
        ...
        streams.try_reserve_exact(expansion.terms.len()).map_err(|_| BeaconError::ResourceLimit {
            resource: "fuzzy posting streams", limit: maximum,
        })?;
        let mut covered = Vec::new();
        work.charge(expansion.groups)?;
        covered.try_reserve_exact(expansion.groups).map_err(|_| BeaconError::ResourceLimit {
            resource: "fuzzy group coverage", limit: expansion.groups,
        })?;
        ...
        loop {
            ...
            let Some(id) = streams.iter_mut()
                .filter_map(|(_, _, stream)| stream.peek().map(|(id, _)| **id)).min()
            else { break };
            let visible = eligible(id);
            work.charge(covered.len())?;
            covered.fill(false);
            let length = *self.lengths.get(&id).ok_or(BeaconError::Invariant("fuzzy posting length"))?;
            ...
            for (groups, idf, stream) in &mut streams {
                ...
                if stream.peek().is_some_and(|(next, _)| **next == id) {
                    let (_, posting) = stream.next().ok_or(BeaconError::Invariant("fuzzy posting cursor"))?;
                    if visible {
...
mod tests {
    use super::*;
    use crate::{BeaconIndex, Bm25, IndexConfig, IndexDocument, IndexMutation, TextMatch, WorkBudget};

    fn mode(distance: EditDistance, require_all: bool, max_expansions: usize) -> TextMatch {
        TextMatch::Fuzzy { distance, require_all, max_expansions }
    }
    ...
    fn words() -> Vec<String> {
        ...
        for _ in 0..4 {
            layer = layer.into_iter().flat_map(|word| {
                ['a', 'é', '界'].into_iter().map(move |ch| format!("{word}{ch}"))
            }).collect();
    ...
    fn scalar_band_and_dictionary_pruning_match_an_independent_unicode_matrix() {
        ...
        let corpus = CorpusStats {
            ...
            frequencies: words.iter().filter(|word| !word.is_empty()).map(|word| (word.clone(), 1)).collect(),
            term_bytes: words.iter().map(String::len).sum(),
        };
        let config = Bm25Config { max_query_terms: 1000, ..Bm25Config::default() };
        for pattern in &words {
            for distance in [EditDistance::Zero, EditDistance::One, EditDistance::Two] {
                ...
                for candidate in &words {
                    ...
                    assert_eq!(matches(&chars, candidate, distance, &mut work).unwrap() == Match::Yes,
                        expected, "{pattern:?} {candidate:?} {distance:?}");
                }
                if !pattern.is_empty() {
                    let query = config.query_terms(pattern, &mut work).unwrap();
                    let expanded = Expansion::build(&query, &corpus, distance, words.len(), &mut work).unwrap();
                    let mut expected: Vec<_> = words.iter().filter(|word| !word.is_empty()
                        && reference(pattern, word) <= distance as usize).map(String::as_str).collect();
                    expected.sort();
                    assert_eq!(expanded.terms.keys().copied().collect::<Vec<_>>(), expected);
                }
            }
        }
        assert_eq!(after_prefix("x\u{d7ff}", &mut work).unwrap().as_deref(), Some("x\u{e000}"));
        assert_eq!(after_prefix("x\u{10ffff}", &mut work).unwrap().as_deref(), Some("y"));
    ...
    fn all_covers_input_groups_and_overlapping_expansions_score_each_term_once() {
        ...
        let actual = corpus.search("cat coat", 10, any, |_| true, &mut work).unwrap();
        let expected = corpus.search("cat cot coat", 10, TextMatch::Any, |_| true, &mut work).unwrap();
        assert_eq!(actual, expected, "overlapping alternatives must not duplicate scores");
        assert_eq!(corpus.search("cat cat coat", 10, any, |_| true, &mut work).unwrap(), actual);
        let actual = corpus.search("cat dog", 10, all, |_| true, &mut work).unwrap();
        let expected = corpus.search("cat cot coat dog", 10, TextMatch::Any,
            |id| id == VId(2) || id == VId(3), &mut work).unwrap();
        assert_eq!(actual, expected);
        assert!(actual.iter().all(|hit| hit.id != VId(1)), "two alternatives are not two groups");
        assert_eq!(corpus.search("cat dog", 1, all, |_| true, &mut work).unwrap(), actual[..1]);
        ...
        assert_eq!(corpus.search("cat coat", 10, mode(EditDistance::One, true, 3), |_| true, &mut work).unwrap(),
            corpus.search("cat cot coat", 10, TextMatch::Any, |_| true, &mut work).unwrap());
        assert!(corpus.search("cat absent", 10, all, |_| true, &mut work).unwrap().is_empty());
        assert!(matches!(corpus.search("cat coat", 1, mode(EditDistance::One, false, 2), |_| true, &mut work),
            Err(BeaconError::ResourceLimit { resource: "fuzzy expanded terms", limit: 2 })));
    }
    ...
    fn zero_distance_preserves_exact_scores_and_unicode_edits_are_not_bytes() {
        ...
        for (all, exact) in [(false, TextMatch::Any), (true, TextMatch::All)] {
            for query in ["cat dog", "coat cat", "é 界", "cat cat", "missing", "--"] {
                assert_eq!(corpus.search(query, 10, mode(EditDistance::Zero, all, 100), |_| true, &mut work).unwrap(),
                    corpus.search(query, 10, exact, |_| true, &mut work).unwrap());
            }
        }
        let hits = corpus.search("a", 10, mode(EditDistance::One, false, 2), |_| true, &mut work).unwrap();
        assert_eq!(hits.iter().map(|hit| hit.id).collect::<Vec<_>>(), [VId(5)]);
        assert_eq!(hits, corpus.search("é 界", 10, TextMatch::Any, |_| true, &mut work).unwrap());
        // Substitution/insertion/deletion yes; no Damerau single-edit swap.
        assert!(corpus.search("cta", 10, mode(EditDistance::One, false, 10), |_| true, &mut work).unwrap().is_empty());
        assert!(!corpus.search("cta", 10, mode(EditDistance::Two, false, 10), |_| true, &mut work).unwrap().is_empty());
        assert!(corpus.search("anything", 0, mode(EditDistance::Two, false, 0), |_| true, &mut work).unwrap().is_empty());
    }

    fn document(id: u128, text: &str) -> IndexDocument {
        IndexDocument { id: VId(id), vector: None, text: Some(text.to_owned()) }
    }

    #[test]
    fn live_vocabulary_caps_ignore_retired_terms_and_segment_layout() {
        let mut work = WorkBudget::new(10_000_000);
        let mut index = BeaconIndex::build(IndexConfig::default(),
            [document(1, "cat"), document(2, "bat")], &mut work).unwrap();
        let old = index.snapshot();
        let bounded = mode(EditDistance::One, false, 1);
        assert!(matches!(old.text_search("cat", 1, bounded, |_| true, &mut work),
            Err(BeaconError::ResourceLimit { resource: "fuzzy expanded terms", limit: 1 })));
        index.apply_batch([IndexMutation::Delete(VId(2))], &mut work).unwrap();
        assert_eq!(index.snapshot().text_search("cat", 10, bounded, |_| true, &mut work).unwrap()
            .iter().map(|hit| hit.id).collect::<Vec<_>>(), [VId(1)]);
        index.apply_batch([IndexMutation::Upsert(document(3, "hat"))], &mut work).unwrap();
        assert!(matches!(index.snapshot().text_search("cat", 1, bounded, |_| true, &mut work),
            Err(BeaconError::ResourceLimit { resource: "fuzzy expanded terms", limit: 1 })));
        let old_rows = old.text_search("cat", 10, mode(EditDistance::One, false, 2), |_| true, &mut work).unwrap();
        let replacement = [document(1, "cat dog"), document(3, "coat dog")];
        index.apply_batch(replacement.clone().map(IndexMutation::Upsert), &mut work).unwrap();
        let rebuilt = BeaconIndex::build(IndexConfig::default(), replacement, &mut work).unwrap();
        for all in [false, true] {
            let query = mode(EditDistance::One, all, 3);
            assert_eq!(index.snapshot().text_search("cat dog", 10, query, |_| true, &mut work).unwrap(),
                rebuilt.snapshot().text_search("cat dog", 10, query, |_| true, &mut work).unwrap());
        }
        assert_eq!(old.text_search("cat", 10, mode(EditDistance::One, false, 2), |_| true, &mut work).unwrap(), old_rows);
    }
    ...
    fn every_expansion_and_posting_control_cut_refuses_without_a_partial_answer() {
        ...
        let expected = corpus.search("cat dog", 3, mode, |_| true, &mut trace).unwrap();
        assert!(!expected.is_empty());
        for stop in 1..=trace.calls {
            let mut cut = Cut { stop: Some(stop), ..Cut::default() };
            assert!(matches!(corpus.search("cat dog", 3, mode, |_| true, &mut cut), Err(BeaconError::Cancelled)), "cut {stop}");
            assert_eq!(cut.calls, stop);
            assert_eq!(corpus.search("cat dog", 3, mode, |_| true, &mut cut).unwrap(), expected);
        }
    }
}
