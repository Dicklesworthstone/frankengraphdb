...
fn build(documents: &[&str]) -> Bm25 {
    Bm25::build(
        ...
        documents
            .iter()
            .enumerate()
            .map(|(id, text)| (VId(id as u128), (*text).into())),
        &mut budget(),
    )
    .unwrap()
}
...
fn ids(index: &Bm25, query: &str, mode: TextMatch) -> Vec<VId> {
    let mut ids: Vec<_> = index
        .search(query, 1000, mode, |_| true, &mut budget())
        .unwrap()
        .into_iter()
        .map(|hit| hit.id)
        .collect();
...
fn phrases_keep_order_repetitions_boundaries_and_existing_tokenization() {
    ...
    assert_eq!(
        ids(&index, "red blue", TextMatch::Phrase),
    ...
    assert_eq!(
        ids(&index, "red", TextMatch::Phrase),
        ids(&index, "red", TextMatch::All)
    );
    assert_eq!(
        ids(&index, "red red blue", TextMatch::All),
...
fn phrase_admission_precedes_top_k_and_does_not_change_bm25_scores() {
    ...
    let plain = index
        .search("red blue", 3, TextMatch::All, |_| true, &mut budget())
        .unwrap();
    assert_eq!(plain[0].id, VId(0));
    let phrase = index
        .search("red blue", 1, TextMatch::Phrase, |_| true, &mut budget())
        .unwrap();
    assert_eq!(phrase.len(), 1);
    assert_eq!(phrase[0].id, VId(2));
    assert_eq!(
        phrase[0].score,
        plain.iter().find(|hit| hit.id == VId(2)).unwrap().score
    );
    assert!(
        index
            .search(
    ...
    let repeated = index
        .search("red red", 1, TextMatch::Phrase, |_| true, &mut budget())
        .unwrap();
    assert!(
        repeated.is_empty(),
        "a query repetition is not a deduplicated term filter"
    );
}
...
fn exhaustive_small_corpus_matches_independent_token_windows_and_legacy_scores() {
    ...
    let index = Bm25::build(
        ...
        documents
            .iter()
            .enumerate()
            .map(|(id, words)| (VId(id as u128), words.join(" "))),
        &mut budget(),
    )
    .unwrap();
    for query in words(4) {
        ...
        let all = index
            .search(&text, 1000, TextMatch::All, |_| true, &mut budget())
            .unwrap();
        let expected_hits: Vec<_> = all
            .into_iter()
            .filter(|hit| expected.contains(&hit.id))
            .collect();
        for k in [0, 1, 3, 1000] {
            let actual = index
                .search(&text, k, TextMatch::Phrase, |_| true, &mut budget())
                .unwrap();
            assert_eq!(
                actual,
                expected_hits.iter().take(k).copied().collect::<Vec<_>>(),
                "{text:?}, k={k}"
            );
        }
    }
...
fn updates_deletes_compaction_and_failed_apply_preserve_positional_generations() {
    for max_segments in [1, 8] {
        ...
        let mut index = BeaconIndex::build(
            config,
            [document(1, "red blue"), document(2, "blue red")],
            &mut budget(),
        )
        .unwrap();
        let old = index.snapshot();
        let before = old
            .text_search("red blue", 10, TextMatch::Phrase, |_| true, &mut budget())
            .unwrap();
        assert_eq!(
        ...
        index
            ...
            .unwrap();
        let rebuilt = BeaconIndex::build(
            index.config().clone(),
            [
                document(1, "blue red"),
                document(2, "red blue"),
                document(3, "red red blue"),
            ],
            &mut budget(),
        )
        .unwrap();
        for phrase in ["red blue", "blue red", "red red blue"] {
            assert_eq!(
                index
                    .snapshot()
                    .text_search(phrase, 10, TextMatch::Phrase, |_| true, &mut budget())
                    .unwrap(),
                rebuilt
                    .snapshot()
                    .text_search(phrase, 10, TextMatch::Phrase, |_| true, &mut budget())
        ...
        assert!(
            index
                .apply_batch(
                    [IndexMutation::Upsert(document(2, "red red"))],
        ...
        index
            .apply_batch(
                [IndexMutation::Delete(VId(2)), IndexMutation::Delete(VId(3))],
                &mut budget(),
            )
            .unwrap();
        assert!(
        ...
        assert_eq!(
            old.text_search("red blue", 10, TextMatch::Phrase, |_| true, &mut budget())
                .unwrap(),
            before
        );
    }
...
fn positional_payloads_are_shared_and_repeated_terms_remain_byte_bounded() {
    ...
    assert!(Arc::ptr_eq(
        &text.frequencies["a"],
        &segment.postings["a"][&VId(1)]
    ));
    assert!(matches!(
        config.analyze_document("a a a a", &mut budget()),
    ...
    assert!(matches!(
        config.query_terms("a a a a", &mut budget()),
        Err(BeaconError::ResourceLimit {
            resource: "text bytes",
            limit: 5
        })
...
fn every_phrase_checkpoint_refuses_without_partial_results_or_mutating_the_index() {
    ...
    for query in ["a a b", "b a a", "a a a", ""] {
        ...
        let expected = index
            .search(query, 10, TextMatch::Phrase, |_| true, &mut trace)
            .unwrap();
        ...
        for stop in 1..=trace.calls {
            ...
            assert!(
                matches!(
                    index.search(query, 10, TextMatch::Phrase, |_| true, &mut cut),
                    Err(BeaconError::Cancelled)
                ),
                "{query:?}, cut={stop}"
            );
            ...
            assert_eq!(
                index
                    .search(query, 10, TextMatch::Phrase, |_| true, &mut budget())
                    .unwrap(),
                expected
            );
        }
