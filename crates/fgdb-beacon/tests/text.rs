...
fn hand_calculated_bm25_scores() {
    let index = corpus();
    let hits = index
        .search("a", 10, TextMatch::Any, |_| true, &mut budget())
        .unwrap();
    assert_eq!(
...
fn and_or_unknown_and_repeated_terms() {
    let index = corpus();
    let both = index
        .search("a c", 10, TextMatch::All, |_| true, &mut budget())
        .unwrap();
    assert_eq!(both.iter().map(|hit| hit.id).collect::<Vec<_>>(), [VId(2)]);
    assert!(
        index
            .search("a missing", 10, TextMatch::All, |_| true, &mut budget())
            .unwrap()
            .is_empty()
    );
    let a = index
    ...
    assert_eq!(
        a,
        index
            .search("A a A", 10, TextMatch::Any, |_| true, &mut budget())
            .unwrap()
    );
    assert_eq!(
        a,
        index
            .search("a missing", 10, TextMatch::Any, |_| true, &mut budget())
            .unwrap()
    );
    assert!(
        index
            .search("...", 10, TextMatch::Any, |_| true, &mut budget())
            .unwrap()
            .is_empty()
    );
}
...
fn analyzer_is_case_insensitive_and_splits_punctuation() {
    let index = Bm25::build(
        Bm25Config::default(),
        [(VId(1), "CAFÉ, Straße/東京42".to_owned())],
        &mut budget(),
    )
    .unwrap();
    assert_eq!(
...
fn empty_text_counts_in_the_corpus_but_cannot_match() {
    let index = Bm25::build(
        Bm25Config::default(),
        [(VId(1), String::new()), (VId(2), "word".to_owned())],
        &mut budget(),
    )
    .unwrap();
    ...
    let hits = index
        .search("word", 10, TextMatch::Any, |_| true, &mut budget())
        .unwrap();
    assert_eq!(hits.len(), 1);
    assert_eq!(hits[0].id, VId(2));
    let all_empty = Bm25::build(
        Bm25Config::default(),
        [(VId(1), String::new())],
        &mut budget(),
    )
...
fn equal_scores_are_ordered_by_id_and_filters_do_not_change_idf() {
    let index = Bm25::build(
        Bm25Config::default(),
        [(VId(8), "x".to_owned()), (VId(2), "x".to_owned())],
        &mut budget(),
    )
    .unwrap();
    let all = index
        .search("x", 2, TextMatch::Any, |_| true, &mut budget())
        .unwrap();
    assert_eq!(all[0].id, VId(2));
    let filtered = index
        .search("x", 1, TextMatch::Any, |id| id == VId(8), &mut budget())
        .unwrap();
...
fn length_normalization_can_be_disabled() {
    ...
    let index = Bm25::build(
        config,
        [
            (VId(1), "x padding padding".to_owned()),
            (VId(2), "x".to_owned()),
        ],
        &mut budget(),
    )
...
fn invalid_bm25_parameters_and_resource_limits_fail() {
    ...
    assert!(matches!(
        Bm25::build(config, [(VId(1), "four".to_owned())], &mut budget()),
    ...
    assert!(matches!(
        index.search("a b", 0, TextMatch::Any, |_| true, &mut budget()),
        Err(BeaconError::ResourceLimit { .. })
    ));
}
...
fn duplicate_documents_and_exhausted_searches_are_refused() {
    assert!(matches!(
        Bm25::build(
            Bm25Config::default(),
            [(VId(1), "a".to_owned()), (VId(1), "b".to_owned())],
            &mut budget()
        ),
    ...
    assert_eq!(
        corpus()
            .search("a", 10, TextMatch::Any, |_| true, &mut WorkBudget::new(1))
            .unwrap_err(),
