...
fn config() -> IndexConfig {
    IndexConfig {
        vector: Some(HnswConfig::new(2, DistanceMetric::SquaredEuclidean)),
        ..IndexConfig::default()
    }
}
fn projection() -> Projection<u8> {
    Projection {
        text: Some(0),
        vector: vec![1, 2],
    }
...
fn scalar_coordinates_keep_order_and_do_not_invent_missing_modalities() {
    ...
    assert_eq!(row.text.as_deref(), Some("search this"));
    let row = p
        .project(VId(8), &config(), |_| None, &mut budget())
        .unwrap();
...
fn exact_f32_admission_rejects_rounding_nonfinite_and_wrong_scalar_types() {
    ...
    for value in [
        ...
        CanonicalScalar::Int(16_777_217),
        ...
        CanonicalScalar::Float(CanonicalF64::new(f64::MAX)),
        text("1"),
    ] {
        assert!(matches!(
            p.project(VId(1), &c, |_| Some(&value), &mut budget()),
            Err(BeaconError::InvalidQuery(_))
        ));
    }
    for value in [
        CanonicalScalar::Int(i64::MIN),
        CanonicalScalar::Int(16_777_216),
...
fn unused_lanes_are_not_validated_or_resolved() {
    ...
    let row = options
        ...
        .project(
            ...
            &config,
            |key| {
                assert_eq!(key, 0, "unused vector was resolved");
                Some(&value)
            },
            &mut budget(),
        )
    ...
    hybrid
        .validate(&options.config_for(hybrid).unwrap(), &mut budget())
        .unwrap();
}
...
fn projection_and_result_limits_refuse_instead_of_truncating() {
    ...
    assert!(matches!(
        options.config_for(Search::Text {
            query: "x",
            k: 2,
            mode: TextMatch::Any
        }),
    ...
    assert!(matches!(
        options
            .projection
            .project(VId(1), &options.index, |_| Some(&value), &mut budget()),
        Err(BeaconError::ResourceLimit {
    ...
    assert!(matches!(
        projection().project(VId(1), &c, |_| None, &mut budget()),
        Err(BeaconError::ResourceLimit {
...
fn exact_projection_budget_succeeds_but_every_shorter_allowance_refuses() {
    ...
    p.project(VId(1), &config(), |k| props.get(usize::from(k)), &mut count)
        .unwrap();
