...
fn approximate_line_agrees_with_exact() {
    ...
    for x in [0.0, 11.1, 42.4, 98.9] {
        let exact = graph
            .search(&[x, 0.0], 10, VectorSearch::Exact, |_| true, &mut budget())
            .unwrap();
        let approximate = graph
            .search(
                &[x, 0.0],
                10,
                VectorSearch::Approximate { ef_search: 100 },
                |_| true,
                &mut budget(),
            )
            .unwrap();
...
fn no_eligible_vertex_is_an_empty_answer() {
    ...
    for mode in [
        VectorSearch::Exact,
        VectorSearch::Approximate { ef_search: 10 },
    ] {
        assert!(
            graph
                .search(&[0.0, 0.0], 10, mode, |_| false, &mut budget())
                .unwrap()
                .is_empty()
        );
    }
...
fn equal_distance_ties_use_vertex_id() {
    ...
    for mode in [
        VectorSearch::Exact,
        VectorSearch::Approximate { ef_search: 3 },
    ] {
        let answer = graph
            .search(&[0.0, 0.0], 2, mode, |_| true, &mut budget())
            .unwrap();
        assert_eq!(
            answer.iter().map(|hit| hit.id).collect::<Vec<_>>(),
            [VId(3), VId(7)]
        );
    }
}
...
fn cosine_and_dot_product_have_explicit_distance_semantics() {
    ...
    .unwrap();
    let hits = cosine
        .search(&[1.0, 0.0], 3, VectorSearch::Exact, |_| true, &mut budget())
        .unwrap();
    assert_eq!(
    ...
    let dot = Hnsw::build(
        HnswConfig::new(2, DistanceMetric::NegativeDotProduct),
        rows,
        &mut budget(),
    )
    .unwrap();
    let hits = dot
        .search(&[1.0, 0.0], 1, VectorSearch::Exact, |_| true, &mut budget())
        .unwrap();
...
fn invalid_vectors_are_rejected_even_on_zero_limit_queries() {
    ...
    assert_eq!(
        graph
            .search(&[0.0], 0, VectorSearch::Exact, |_| true, &mut budget())
            .unwrap_err(),
        BeaconError::Dimension {
            expected: 2,
            actual: 1
        }
    );
...
fn empty_index_preserves_query_validation() {
    ...
    assert!(matches!(
        graph.search(&[0.0, 0.0], 0, VectorSearch::Exact, |_| true, &mut budget()),
        Err(BeaconError::ZeroVector)
    ));
    assert!(matches!(
        graph.search(
            &[1.0, 0.0],
            0,
            VectorSearch::Approximate { ef_search: 0 },
            |_| true,
            &mut budget()
        ),
...
fn duplicate_ids_and_invalid_configuration_fail() {
    assert!(matches!(
        Hnsw::build(
            ...
            [(VId(1), vec![1.0]), (VId(1), vec![2.0])],
            &mut budget()
        ),
        Err(BeaconError::DuplicateVertex(VId(1)))
    ));
    for dimensions in [0, 65_537] {
        assert!(
            HnswConfig::new(dimensions, DistanceMetric::Cosine)
                .validate()
                .is_err()
        );
    }
...
fn f32_extremes_do_not_overflow_distance_accumulation() {
    for metric in [
        ...
        let graph = Hnsw::build(
            HnswConfig::new(2, metric),
            [(VId(1), vec![f32::MAX, f32::MAX])],
            &mut budget(),
        )
        .unwrap();
        let answer = graph
...
fn work_exhaustion_and_cancellation_never_return_partial_answers() {
    ...
    assert_eq!(
        graph
            .search(&[42.0, 0.0], 20, VectorSearch::Exact, |_| true, &mut Cancel)
            .unwrap_err(),
...
fn full_width_vertex_identities_are_distinct_and_totally_ordered() {
    ...
    let graph = Hnsw::build(
        HnswConfig::new(1, DistanceMetric::SquaredEuclidean),
        [(high, vec![1.0]), (low, vec![1.0])],
        &mut budget(),
    )
    .unwrap();
    for mode in [
        VectorSearch::Exact,
        VectorSearch::Approximate { ef_search: 2 },
    ] {
        let hits = graph
            .search(&[0.0], 2, mode, |_| true, &mut budget())
            .unwrap();
        assert_eq!(
            hits.iter().map(|hit| hit.id).collect::<Vec<_>>(),
            [low, high]
