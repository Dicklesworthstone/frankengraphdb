...
use fgdb_beacon::{
    BeaconError, BeaconIndex, DistanceMetric, ExactHybridQuery, ExactRrfProfile, ExactRrfScore,
    GraphHybridQuery, HnswConfig, IndexConfig, IndexDocument, TextMatch, VectorSearch, WorkBudget,
    WorkControl,
};
use fgdb_types::VId;
...
fn rank(n: u32) -> Option<NonZeroU32> {
    NonZeroU32::new(n)
}
fn graph(
    vertices: &[VId],
    ...
    for &id in vertices {
        graph.insert_vertex(id, &mut budget()).unwrap();
    }
    for &(a, b) in edges {
        graph.insert_edge(a, b, direction, &mut budget()).unwrap();
    }
    graph
}
...
fn exhaustive_three_vertex_topologies_match_independent_all_pairs_distances() {
    ...
    for mask in 0_u16..512 {
        ...
        for seed_mask in 0_u8..8 {
            let mut seeds: Vec<_> = (0..3)
                .filter(|i| seed_mask & (1 << i) != 0)
                .map(|i| vertices[i])
                .collect();
            seeds.reverse();
            ...
            for max_hops in 0..=3 {
                for include_seeds in [false, true] {
                    ...
                    for i in 0..3 {
                        let hops = (0..3)
                            .filter(|s| seed_mask & (1 << s) != 0)
                            .map(|s| shortest[s][i])
                            .min()
                            .unwrap_or(4);
                        if hops <= max_hops && (include_seeds || hops != 0) {
                            expected.push(GraphHit {
                                id: vertices[i],
                                hops,
                            });
                        }
                    ...
                    for k in [1_u32, 3] {
                        let actual = graph
                            .expand(&seeds, max_hops, include_seeds, k, &mut budget())
                            .unwrap();
                        assert_eq!(
                            actual,
                            expected[..expected.len().min(k as usize)],
                            "mask={mask}, seeds={seed_mask}"
                        );
                    }
                }
            }
...
fn orientation_parallel_edges_cycles_and_seed_inclusion_have_explicit_laws() {
    ...
    for (direction, expected) in [
        ...
        assert_eq!(
            graph
                .expand(&[VId(2)], u32::MAX, false, 4, &mut budget())
                .unwrap(),
            expected
        );
        assert!(
            graph
                .expand(&[VId(2)], 0, false, 4, &mut budget())
                .unwrap()
                .is_empty()
        );
        assert_eq!(
            graph.expand(&[VId(2)], 0, true, 4, &mut budget()).unwrap(),
            [GraphHit {
                id: VId(2),
                hops: 0
            }]
        ...
        assert!(
            graph
                .expand(&[VId(99)], 10, true, 4, &mut budget())
                .unwrap()
                .is_empty()
        );
    }
...
fn construction_refusals_are_terminal_and_small_top_k_cannot_hide_incomplete_expansion() {
    ...
    assert_eq!(
        g.expand(&[VId(1)], 0, true, 1, &mut budget()).unwrap_err(),
        error
    );
    ...
    let g = graph(
        &[VId(1), VId(2), VId(3)],
        &[(VId(1), VId(2)), (VId(2), VId(3))],
        D::Outgoing,
        limits,
    );
    assert!(matches!(
        g.expand(&[VId(1)], 2, true, 1, &mut budget()),
    ...
    assert_eq!(
        g.expand(&[VId(1)], 1, false, 1, &mut budget()).unwrap(),
        [GraphHit {
            id: VId(2),
            hops: 1
        }]
    ...
    assert!(
        g.expand(&[VId(1), VId(1)], 0, true, 1, &mut budget())
            .is_err()
    );
}
...
fn every_shorter_expansion_allowance_fails_without_a_successful_prefix() {
    ...
    for units in 0..count.0 {
        assert_eq!(
            g.expand(&[VId(1)], 5, true, 3, &mut WorkBudget::new(units))
                .unwrap_err(),
            BeaconError::WorkBudgetExceeded
        );
    }
    assert_eq!(
        g.expand(&[VId(1)], 5, true, 3, &mut WorkBudget::new(count.0))
            .unwrap(),
        expected
    );
    ...
    assert!(
        g.insert_edge(VId(1), VId(2), D::Undirected, &mut WorkBudget::new(5))
            .is_err()
    );
...
fn three_lane_rationals_are_reduced_and_overflow_paths_match_exact_vectors() {
    ...
    let b = ExactRrfScore::from_graph_ranks(
        p,
        rank(u32::MAX - 6),
        rank(u32::MAX - 12),
        rank(u32::MAX - 18),
    ...
    assert_eq!(
        ...
        (
            3_626_722_107_352_839_133_790_085,
            158_456_324_290_658_913_295_267_790_416
        )
    );
    assert_eq!(
        (b.numerator(), b.denominator()),
        (
            67_161_520_496_109_217_986_785,
            2_934_376_375_069_730_097_716_624_184
        )
    );
    ...
    let permuted = ExactRrfScore::from_graph_ranks(
        p,
        rank(u32::MAX - 14),
        rank(u32::MAX - 16),
        rank(u32::MAX - 4),
...
fn third_lane_can_promote_a_base_loser_and_add_graph_only_candidates() {
    ...
    let q = query();
    let base = index
        .hybrid_search_exact_fusion(q.retrieval, |_| true, &mut budget())
        .unwrap();
...
fn disabled_graph_matches_two_lane_results_and_malformed_graph_ranks_refuse() {
    ...
    let actual = index
        .hybrid_search_graph(q, &malformed, &mut budget())
        .unwrap();
    let expected = index
        .hybrid_search_exact_fusion(q.retrieval, |_| true, &mut budget())
        .unwrap();
    for (a, b) in actual.iter().zip(&expected) {
        assert_eq!(
            (a.id, a.score, a.decimal_score, a.vector_rank, a.text_rank),
            (b.id, b.score, b.decimal_score, b.vector_rank, b.text_rank)
        );
    ...
    assert!(
        index
            .hybrid_search_graph(q, &duplicates, &mut budget())
            .is_err()
    );
...
fn fusion_budget_is_shared_across_sources_and_final_output() {
    ...
    for units in 0..count.0 {
        assert_eq!(
            index
                .hybrid_search_graph(q, &graph, &mut WorkBudget::new(units))
                .unwrap_err(),
            BeaconError::WorkBudgetExceeded
        );
    }
    assert_eq!(
        index
            .hybrid_search_graph(q, &graph, &mut WorkBudget::new(count.0))
            .unwrap(),
        expected
    );
}
