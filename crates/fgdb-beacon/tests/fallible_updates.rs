...
use fgdb_beacon::{
    BeaconError, BeaconIndex, Bm25Config, DistanceMetric, HnswConfig, IndexConfig, IndexDocument,
    IndexMutation, IndexSnapshot, TextMatch, VectorSearch, WorkBudget, WorkControl,
};
use fgdb_types::VId;
...
fn fallible_and_infallible_batches_preserve_last_write_wins_and_snapshot_parity() {
    for max_segments in [1, 8] {
        let original = initial(IndexConfig {
            max_segments,
            ..config()
        });
...
fn staging_limits_stop_the_source_before_another_poll() {
    let mut index = initial(IndexConfig {
        max_text_bytes: 32,
        ..config()
    });
    let old = index.snapshot();
    ...
    let source = std::iter::from_fn(|| {
        ...
        polls.set(1);
        Some(Ok::<_, SourceError>(IndexMutation::Upsert(document(
            1,
            0.0,
            &"x".repeat(33),
        ))))
...
fn compaction_can_refuse_at_final_publication_without_replacing_the_generation() {
    ...
    index
        .apply_batch(
            [IndexMutation::Upsert(document(3, 2.0, "new"))],
            &mut budget(),
        )
        .unwrap();
    let old = index.snapshot();
...
fn cancellation_observed_at_empty_source_eof_is_not_reported_as_success() {
    ...
    assert_eq!(
        index.try_apply_batch(source, &mut CancelAfterEof(&cancelled)),
        Err(SourceError::Index(BeaconError::Cancelled))
    );
    assert_same(&index.snapshot(), &old);
}
